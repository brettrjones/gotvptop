...
template <typename T> class GoTvPointGen
{
public:
  typedef GoTvPointGen<T> this_type;

  GoTvPointGen() noexcept = default;

  constexpr GoTvPointGen(T a, T b)
  : x{a}, y{b}
  {}

  template<class U> explicit constexpr GoTvPointGen(const GoTvPointGen<U>& rhs)
...
template<typename T>
constexpr bool operator==(const GoTvPointGen<T> &point1, const GoTvPointGen<T> &point2) noexcept
{
...
using GoTvPoint = GoTvPointGen<float>;
using GoTvPointInt = GoTvPointGen<int>;
 ...
 * is not changed.
 */
template <typename T> class GoTvSizeGen
{
  T m_w{}, m_h{};
...
public:
  typedef GoTvSizeGen<T> this_type;

  GoTvSizeGen() noexcept = default;

  GoTvSizeGen(T width, T height)
  {
  ...
  GoTvPointGen<T> ToPoint() const
  {
    return {m_w, m_h};
  }

  template<class U> explicit GoTvSizeGen<T>(const GoTvSizeGen<U>& rhs)
  {
...
template<typename T>
inline bool operator==(const GoTvSizeGen<T>& size1, const GoTvSizeGen<T>& size2) noexcept
{
...
using GoTvSize = GoTvSizeGen<float>;
using GoTvSizeInt = GoTvSizeGen<int>;


template <typename T> class GoTvRectGen
{
public:
  typedef GoTvRectGen<T> this_type;
  typedef GoTvPointGen<T> point_type;
  typedef GoTvSizeGen<T> size_type;

  GoTvRectGen() noexcept = default;

  constexpr GoTvRectGen(T left, T top, T right, T bottom)
  : x1{left}, y1{top}, x2{right}, y2{bottom}
  {}

  constexpr GoTvRectGen(const point_type &p1, const point_type &p2)
  : x1{p1.x}, y1{p1.y}, x2{p2.x}, y2{p2.y}
  {}

  constexpr GoTvRectGen(const point_type &origin, const size_type &size)
  : x1{origin.x}, y1{origin.y}, x2{x1 + size.Width()}, y2{y1 + size.Height()}
  {}

  template<class U> explicit constexpr GoTvRectGen(const GoTvRectGen<U>& rhs)
...
template<typename T>
constexpr bool operator==(const GoTvRectGen<T> &rect1, const GoTvRectGen<T> &rect2) noexcept
{
...
using GoTvRect = GoTvRectGen<float>;
using GoTvRectInt = GoTvRectGen<int>;
