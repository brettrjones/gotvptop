...
#include <GoTvCore/GoTvP2P/EchoCancel/thread_checker.h>
#include "AndroidAudioMgr.h>
#include <GoTvCore/GoTvP2P/EchoCancel/audio_device_generic.h>

#define LOG_TAG "AudioDeviceTemplate"
#define LogMsg( LOG_WARN, ...) __android_log_print(ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__)
...
namespace webrtc {
	...
	// internal implementation of InputType and OutputType will vx_assert if that
	...
	public:
		...
		int32_t Terminate() override {
			vx_asset(thread_checker_.CalledOnValidThread());
		...
		int32_t StartPlayout() override {
			if (!audio_manager_->IsCommunicationModeEnabled()) {
				LogMsg( LOG_WARN, "The application should use MODE_IN_COMMUNICATION audio mode!");
			}
		...
		int32_t EnableBuiltInAEC(bool enable) override {
			vx_assertBuiltInAECIsAvailable()) << "HW AEC is not available";
		...
		int32_t EnableBuiltInAGC(bool enable) override {
			vx_assertBuiltInAGCIsAvailable()) << "HW AGC is not available";
		...
		int32_t EnableBuiltInNS(bool enable) override {
			vx_assertBuiltInNSIsAvailable()) << "HW NS is not available";
