        ...
        if ((m_flags & DIR_FLAG_NO_FILE_INFO)==0 && g_advancedSettings.m_sambastatfiles)
        {
          ...
          if( smbc_stat(strFullName.c_str(), &info) == 0 )
          {
            ...
            if (smbc_getxattr(strFullName.c_str(), "system.dos_attr.mode", value, sizeof(value)) > 0)
            {
              ...
              CLog::Log(LOGERROR, "Getting extended attributes for the share: '%s'\nunix_err:'%x' error: '%s'", GoTvUrl::GetRedacted(strFullName).c_str(), errno, strerror(errno));
            ...
            CLog::Log(LOGERROR, "%s - Failed to stat file %s", __FUNCTION__, GoTvUrl::GetRedacted(strFullName).c_str());
...
int CSMBDirectory::OpenDir(const GoTvUrl& url, std::string& strAuth)
{
  ...
  CLog::LogF(LOGDEBUG, LOGSAMBA, "Using authentication url %s", GoTvUrl::GetRedacted(s).c_str());
  ...
  if (fd < 0)
  {
    ...
    CLog::Log(LOGERROR, "SMBDirectory->GetDirectory: Unable to open directory : '%s'\nunix_err:'%x' error : '%s'", GoTvUrl::GetRedacted(strAuth).c_str(), errno, strerror(errno));
  }
