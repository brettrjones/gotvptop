...
CSFTPSessionPtr CSFTPSessionManager::CreateSession(const GoTvUrl &url)
{
...
bool CSFTPFile::Open(const GoTvUrl& url)
{
...
bool CSFTPFile::Exists(const GoTvUrl& url)
{
...
int CSFTPFile::Stat(const GoTvUrl& url, struct __stat64* buffer)
{
  CSFTPSessionPtr session = CSFTPSessionManager::CreateSession(url);
  if (session)
