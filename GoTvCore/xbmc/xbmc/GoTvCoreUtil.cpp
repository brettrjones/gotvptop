...
namespace
{
    ...
    bool IsDirectoryValidRoot( std::string path )
    {
        ...
        auto h = CreateFile2( path.c_str(), GENERIC_READ, 0, OPEN_EXISTING, NULL );
#else
        auto h = CreateFileA( path.c_str(), GENERIC_READ, 0, nullptr,
                              OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr );
#endif
        if( h != INVALID_HANDLE_VALUE )
        {
...
char CUtil::GetPathSeperator( void )
{
#ifdef TARGET_OS_WINDOWS
...
std::string  CUtil::GetHomePath( std::string strTarget )
{
    ...
    {
#ifdef TARGET_OS_WINDOWS
        std::string strHomePath = strPath;
        // strip off exe name
        ...
        // app resources are bin directory ../Resources
        if( !strPath.empty() )
        {
            std::string resourcePath1 = strPath;
            size_t last_sep = resourcePath1.find_last_of( GetPathSeperator() );
            if( last_sep != std::string::npos )
                strPath = resourcePath1.substr( 0, last_sep );
            else
                strPath = resourcePath1;

            // now add the AppResources subdirectory
            strPath += "\\Resources";
        }
#endif // TARGET_OS_WINDOWS
...
*/
    ...
    if( IsDirectoryValidRoot( strPath ) )
    {
        return strPath;
    }
...
std::string CUtil::GetExecutablePath( void )
{
    ...
    std::string strPath = CUtil::ResolveExecutablePath();
    auto last_sep = strPath.find_last_of( PATH_SEPARATOR_CHAR );
    if( last_sep != std::string::npos )
        strPath = strPath.substr( 0, last_sep );
#ifdef TARGET_OS_WINDOWS
    strPath = WindowsRelativeToAbsolutePath( strPath );
#endif // TARGET_OS_WINDOWS
...
{
    GoTvUrl pathToUrl( strFileNameAndPath );
    return GetTitleFromPath( pathToUrl, bIsFolder );
}

//========================================================================
std::string CUtil::GetTitleFromPath( const GoTvUrl& url, bool bIsFolder /* = false */ )
{
  ...
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_FILELISTS_SHOWEXTENSIONS) && !bIsFolder)
    ...
    strFilename = GoTvUrl::Decode( strFilename );
    return strFilename;
}
...
//std::string CUtil::GetHomePath()
//{
//    std::string strTarget = CCompileInfo::GetHomeEnvName();
//    std::string strPath;
//    strTarget = GetHomePath( strPath );
//    return strPath;
//}
...
//========================================================================
void CUtil::StatI64ToStat64( structStatOsDef *result, struct _stati64 *stat )
{
    result->st_dev = stat->st_dev;
    result->st_ino = stat->st_ino;
    result->st_mode = stat->st_mode;
...
//========================================================================
void CUtil::StatToStat64( structStatOsDef *result, const struct stat *stat )
{
...
//========================================================================
#ifdef TARGET_OS_WINDOWS
void CUtil::Stat64ToStat64i32( struct _stat64i32 *result, structStatOsDef *stat )
{
    result->st_dev = stat->st_dev;
    result->st_ino = stat->st_ino;
    result->st_mode = stat->st_mode;
  ...
  for (const auto &pItem : items)
    {
        ...
        if( StringUtils::StartsWithNoCase( strCandidate, videoName ) )
        {
            ...
            else
            {
                ...
                CLog::Log( LOGINFO, "%s: found associated file %s\n", __FUNCTION__, GoTvUrl::GetRedacted( pItem->GetPath() ).c_str() );
            }
        }
        else
        {
...
int CUtil::ScanArchiveForAssociatedItems( const std::string& strArchivePath,
                                          ...
                                          std::vector<std::string>& associatedFiles )
{
  CLog::LogF(LOGDEBUG, "Scanning archive %s", GoTvUrl::GetRedacted(strArchivePath).c_str());
    int nItemsAdded = 0;
    ...
    {
        GoTvUrl pathToUrl( strArchivePath );
        GoTvUrl zipURL = URIUtils::CreateArchivePath( "zip", pathToUrl, "" );
    ...
    {
        GoTvUrl pathToUrl( strArchivePath );
        GoTvUrl rarURL = URIUtils::CreateArchivePath( "rar", pathToUrl, "" );
        ...
        if( URIUtils::HasExtension( vecSubtitles[ i ], ".smi" ) )
        {
          ...
          for (const auto &lang : TagConv.m_Langclass)
            ...
            std::string strDest = StringUtils::Format("special://temp/subtitle.%s.%zu.smi", lang.Name.c_str(), i);
                        ...
                        {
                            CLog::Log( LOGINFO, " cached subtitle %s->%s\n", GoTvUrl::GetRedacted( vecSubtitles[ i ] ).c_str(), strDest.c_str() );
...
ExternalStreamInfo CUtil::GetExternalStreamDetailsFromFilename( const std::string& videoPath, const std::string& associatedFile )
{
    ...
    else if( URIUtils::GetExtension( associatedFile ) == ".sub" && URIUtils::IsInArchive( associatedFile ) )
    {
        ...
        CLog::Log( LOGDEBUG, "%s - skipping archived vobsub filename parsing: %s", __FUNCTION__, GoTvUrl::GetRedacted( associatedFile ).c_str() );
        toParse.clear();
    }
    ...
    CLog::Log( LOGDEBUG, "%s - Language = '%s' / Name = '%s' / Flag = '%u' from %s",
               __FUNCTION__, info.language.c_str(), info.name.c_str(), info.flag, GoTvUrl::GetRedacted( associatedFile ).c_str() );
    return info;
}
...
bool CUtil::FindVobSubPair( const std::vector<std::string>& vecSubtitles, const std::string& strIdxPath, std::string& strSubPath )
{
    ...
    for (const auto &subtitlePath : vecSubtitles)
      ...
      if (URIUtils::IsInArchive(subtitlePath))
        strSubDirectory = GoTvUrl::Decode(strSubDirectory);
...
std::string CUtil::GetVobSubSubFromIdx( const std::string& vobSubIdx )
{
  ...
  for (const std::string& archType : archTypes)
    ...
    vobSub = URIUtils::CreateArchivePath(archType,
                                         GoTvUrl(URIUtils::ReplaceExtension(vobSubIdx, std::string(".") + archType)),
                                              vobSubFilename ).Get();
        if( CFile::Exists( vobSub ) )
            return vobSub;
    }
