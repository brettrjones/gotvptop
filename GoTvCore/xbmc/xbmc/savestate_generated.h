...
struct Savestate FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  SaveType type() const {
    return static_cast<SaveType>(GetField<uint8_t>(VT_TYPE, 0));
  }
  uint8_t slot() const {
  ...
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           verifier.Verify(game_file_name()) &&
           ...
           verifier.Verify(emulator_addon_id()) &&
           VerifyOffset(verifier, VT_EMULATOR_VERSION) &&
           verifier.Verify(emulator_version()) &&
           VerifyOffset(verifier, VT_MEMORY_DATA) &&
           verifier.Verify(memory_data()) &&
           verifier.EndTable();
  }
};
...
struct SavestateBuilder {
  ...
  SavestateBuilder &operator=(const SavestateBuilder &);
...
inline flatbuffers::Offset<Savestate> CreateSavestate(
    ...
    uint8_t version = 0,
    SaveType type = SaveType_Unknown,
    uint8_t slot = 0,
...
inline flatbuffers::Offset<Savestate> CreateSavestateDirect(
  ...
  return KODI::RETRO::CreateSavestate(
      ...
      game_file_name ? _fbb.CreateString(game_file_name) : 0,
      timestamp_frames,
      timestamp_wall_clock_ns,
      emulator_addon_id ? _fbb.CreateString(emulator_addon_id) : 0,
      emulator_version ? _fbb.CreateString(emulator_version) : 0,
      memory_data ? _fbb.CreateVector<uint8_t>(*memory_data) : 0);
}
