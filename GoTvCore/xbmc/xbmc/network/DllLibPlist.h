...
#include <libplist/plist/plist.h>
...
class DllLibPlist : public StaticDll, public DllLibPlistInterface
{
public:
    DllLibPlist();
    DllLibPlist( const DllLibPlist& ) = default;

    virtual void        plist_from_bin( const char *plist_bin, uint32_t length, plist_t * plist );
    virtual plist_t     plist_new_dict( void );
    virtual uint32_t    plist_dict_get_size( plist_t node );
    virtual void        plist_get_string_val( plist_t node, char **val );
    virtual void        plist_get_real_val( plist_t node, double *val );
    virtual plist_t     plist_dict_get_item( plist_t node, const char* key );
    virtual void        plist_free( plist_t plist );
    virtual void        plist_to_xml( plist_t plist, char **plist_xml, uint32_t * length );
    virtual void        plist_dict_new_iter( plist_t node, plist_dict_iter *iter );
    virtual void        plist_dict_next_item( plist_t node, plist_dict_iter iter, char **key, plist_t *val );
