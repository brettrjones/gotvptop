...
class DllLibShairplay : public StaticDll, public DllLibShairplayInterface
{
public:
    DllLibShairplay();
    DllLibShairplay( const DllLibShairplay& ) = default;

    virtual raop_t *    raop_init( int max_clients, raop_callbacks_t *callbacks, const char *pemkey );
    virtual raop_t *    raop_init_from_keyfile( int max_clients, raop_callbacks_t *callbacks, const char *keyfile );
    virtual void        raop_set_log_level( raop_t *raop, int level );
    virtual void        raop_set_log_callback( raop_t *raop, raop_log_callback_t callback, void *cls );
    virtual int         raop_is_running( raop_t *raop );
    virtual int         raop_start( raop_t *raop, unsigned short *port, const char *hwaddr, int hwaddrlen, const char *password );
    virtual void        raop_stop( raop_t *raop );
    virtual void        raop_destroy( raop_t *raop );
