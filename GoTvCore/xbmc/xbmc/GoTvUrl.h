...
// NOTE: GoTvUrl is a replacement class for CURL to avoid name clash with curl library
// NOTE: SetPort has been renamed to SetUrlPort to avoid name clash with microsoft definition of SetPort
class GoTvUrl
{
public:
	explicit GoTvUrl( const std::string& strURL )
	{
		Parse( strURL );
	}

	GoTvUrl():m_iPort( 0 ) {}
	virtual ~GoTvUrl( void );
	...
	// SetPort is defined in windows winspool.. renamed to avoid name conflict
