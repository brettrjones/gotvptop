...
void GoTvUrl::Parse( const std::string& strURL1 )
{
	Reset();
		...
		while( 1 )
		{
			...
			if( XFILE::CFile::Stat( archiveName, &s ) == 0 )
			{
...
#endif
				{
					...
					if( is_apk )
					{
						GoTvUrl c( "apk://" + archiveName + "/" + strURL.substr( iPos + 1 ) );
						*this = c;
					}
					else
					{
						GoTvUrl c( "zip://" + archiveName + "/" + strURL.substr( iPos + 1 ) );
						*this = c;
					}
					return;
				}
...
void GoTvUrl::SetFileName( const std::string& strFileName )
{
...
void GoTvUrl::SetProtocol( const std::string& strProtocol )
{
	m_strProtocol = strProtocol;
	StringUtils::ToLower( m_strProtocol );
}

void GoTvUrl::SetOptions( const std::string& strOptions )
{
	m_strOptions.clear();
	m_options.Clear();
...
void GoTvUrl::SetProtocolOptions( const std::string& strOptions )
{
	m_strProtocolOptions.clear();
	m_protocolOptions.Clear();
	if( strOptions.length() > 0 )
...
const std::string GoTvUrl::GetTranslatedProtocol() const
{
	if( IsProtocol( "shout" )
		|| IsProtocol( "dav" )
		|| IsProtocol( "rss" ) )
...
const std::string GoTvUrl::GetFileNameWithoutPath() const
{
...
char GoTvUrl::GetDirectorySeparator() const
{
#ifndef TARGET_POSIX
...
std::string GoTvUrl::GetWithoutOptions() const
{
	if( m_strProtocol.empty() )
		return m_strFileName;
...
std::string GoTvUrl::GetWithoutUserDetails( bool redact ) const
{
	std::string strURL;
  ...
  if (redact && !m_strUserName.empty())
  {
	...
	if( !m_strHostName.empty() )
	{
		...
		if( URIUtils::HasParentInHostname( *this ) )
			strHostName = GoTvUrl( m_strHostName ).GetWithoutUserDetails();
		else
...
std::string GoTvUrl::GetWithoutFilename() const
{
	if( m_strProtocol.empty() )
		return "";
...
std::string GoTvUrl::GetRedacted() const
{
	return GetWithoutUserDetails( true );
}

std::string GoTvUrl::GetRedacted( const std::string& path )
{
	return GoTvUrl( path ).GetRedacted();
}

bool GoTvUrl::IsLocal() const
{
  return (m_strProtocol.empty() || IsLocalHost() || IsProtocol("win-lib"));
}

bool GoTvUrl::IsLocalHost() const
{
  return CServiceBroker::GetNetwork().IsLocalHost(m_strHostName);
}

bool GoTvUrl::IsFileOnly( const std::string &url )
{
	return url.find_first_of( "/\\" ) == std::string::npos;
}

bool GoTvUrl::IsFullPath( const std::string &url )
{
...
std::string GoTvUrl::Decode( const std::string& strURLData )
...
std::string GoTvUrl::Encode( const std::string& strURLData )
{
	std::string strResult;
...
bool GoTvUrl::IsProtocolEqual( const std::string &protocol, const char *type )
{
	/*
	 ...
	 We possibly shouldn't do this (as GoTvUrl(foo).Get() != foo, though there are other reasons for this as well)
...
void GoTvUrl::GetOptions( std::map<std::string, std::string> &options ) const
{
...
bool GoTvUrl::HasOption( const std::string &key ) const
{
	return m_options.HasOption( key );
}

bool GoTvUrl::GetOption( const std::string &key, std::string &value ) const
{
	CVariant valueObj;
...
std::string GoTvUrl::GetOption( const std::string &key ) const
{
	std::string value;
...
void GoTvUrl::SetOption( const std::string &key, const std::string &value )
{
	m_options.AddOption( key, value );
	SetOptions( m_options.GetOptionsString( true ) );
}

void GoTvUrl::RemoveOption( const std::string &key )
{
	m_options.RemoveOption( key );
	SetOptions( m_options.GetOptionsString( true ) );
}

void GoTvUrl::GetProtocolOptions( std::map<std::string, std::string> &options ) const
{
...
bool GoTvUrl::HasProtocolOption( const std::string &key ) const
{
	return m_protocolOptions.HasOption( key );
}

bool GoTvUrl::GetProtocolOption( const std::string &key, std::string &value ) const
{
	CVariant valueObj;
...
std::string GoTvUrl::GetProtocolOption( const std::string &key ) const
{
	std::string value;
...
void GoTvUrl::SetProtocolOption( const std::string &key, const std::string &value )
{
	m_protocolOptions.AddOption( key, value );
	m_strProtocolOptions = m_protocolOptions.GetOptionsString( false );
}

void GoTvUrl::RemoveProtocolOption( const std::string &key )
{
