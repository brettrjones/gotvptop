...
void CWin32StorageProvider::GetLocalDrives(VECSOURCES &localDrives)
{
  ...
  if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_PROFILE, nullptr, 0, profilePath)) ||
      GetEnvironmentVariableW(L"USERPROFILE", profilePath, MAX_PATH) > 0)
...
bool CWin32StorageProvider::Eject(const std::string& mountpath)
{
  ...
  HANDLE hVolume = CreateFileW(strVolFormat.c_str(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, NULL, NULL);
  ...
  for(int i=0;i<3;i++)
  {
    res = CM_Request_Device_EjectW(DevInst, &VetoType, VetoName, MAX_PATH, 0);
...
std::vector<std::string > CWin32StorageProvider::GetDiskUsage()
{
  ...
  if( dwStrLength != 0 )
  {
    ...
    do
    {
      ...
      if( DRIVE_FIXED == GetDriveTypeW( strDrive.c_str()  ) &&
        GetDiskFreeSpaceExW( ( strDrive.c_str() ), nullptr, &ULTotal, &ULTotalFree ) )
      {
