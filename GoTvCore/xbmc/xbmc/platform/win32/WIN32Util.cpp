...
int CWIN32Util::GetDriveStatus(const std::string &strPath, bool bStatusEx)
{
  ...
  hDevice = CreateFileW( strPathW.c_str(),                  // drive
  ...
  hDevice = CreateFileW( strPathW.c_str(),
...
HRESULT CWIN32Util::ToggleTray(const char cDriveLetter)
{
  ...
  HANDLE hDrive= CreateFileW( strVolFormat.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
                             NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  auto strRootFormat = ToW(StringUtils::Format("%c:\\", cDL));
  if( ( hDrive != INVALID_HANDLE_VALUE || GetLastError() == NO_ERROR) &&
    ( GetDriveTypeW( strRootFormat.c_str() ) == DRIVE_CDROM ) )
  {
    DWORD dwDummy;
...
LONG CWIN32Util::UtilRegGetValue( const HKEY hKey, const char *const pcKey, DWORD *const pdwType, char **const ppcBuffer, DWORD *const pdwSizeBuff, const DWORD dwSizeAdd )
{
  ...
  DWORD dwSize;
  LONG lRet= RegQueryValueExW(hKey, pcKeyW.c_str(), nullptr, pdwType, nullptr, &dwSize );
  if (lRet == ERROR_SUCCESS)
  {
    if (ppcBuffer)
    {
      ...
      lRet= RegQueryValueExW(hKey, pcKeyW.c_str(), nullptr, nullptr, reinterpret_cast<LPBYTE>(pcValue), &dwSize);
...
bool CWIN32Util::UtilRegOpenKeyEx( const HKEY hKeyParent, const char *const pcKey, const REGSAM rsAccessRights, HKEY *hKey, const bool bReadX64 )
{
  ...
  bool bRet = ( ERROR_SUCCESS == RegOpenKeyExW(hKeyParent, ToW(pcKey).c_str(), 0, rsAccessRightsTmp, hKey));
  return bRet;
}
