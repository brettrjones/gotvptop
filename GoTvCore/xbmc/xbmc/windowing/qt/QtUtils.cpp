...
static float currentRefreshRate()
{
  ...
  if (window)
  {
    ...
    {
      CLog::Log(LOGINFO, "CQtUtils: Preferred refresh rate: %f", preferredRate);
    ...
    if (view) {
      ...
      if (display)
      {
        ...
        {
          CLog::Log(LOGINFO, "CQtUtils: Current display refresh rate: %f", reportedRate);
          return reportedRate;
        }
      }
    }
  ...
  if (display)
  {
    ...
    if (m)
    {
      if (m.getPhysicalWidth() > m.getPhysicalHeight())   // Assume unusable if portrait is returned
      {
        ...
        CLog::Log(LOGDEBUG, "CQtUtils: current mode: %d: %dx%d@%f", m.getModeId(), m.getPhysicalWidth(), m.getPhysicalHeight(), m.getRefreshRate());
        ...
        for (auto m : modes)
        {
          CLog::Log(LOGDEBUG, "CQtUtils: available mode: %d: %dx%d@%f", m.getModeId(), m.getPhysicalWidth(), m.getPhysicalHeight(), m.getRefreshRate());
...
CQtUtils::CQtUtils()
{
  ...
  if (!m_width || !m_height)
  {
    ...
    if (!displaySize.empty())
    {
      ...
      CLog::Log(LOGDEBUG, "CQtUtils: display-size: %s(%dx%d)", displaySize.c_str(), m_width, m_height);
    }
  }

  CLog::Log(LOGDEBUG, "CQtUtils: maximum/current resolution: %dx%d", m_width, m_height);
  ...
  CLog::Log(LOGDEBUG, "CQtUtils: selected resolution: %dx%d", m_width, m_height);
  */
}
...
bool CQtUtils::GetNativeResolution( RESOLUTION_INFO *res ) const
{
    /*
    ...
    if( !m_width || !m_height )
    {
        ...
        CLog::Log( LOGNOTICE, "CQtUtils: window resolution: %dx%d", m_width, m_height );
    }
    ...
    res->strMode = StringUtils::Format( "%dx%d @ %.6f%s - Full Screen", res->iScreenWidth, res->iScreenHeight, res->fRefreshRate,
                                        res->dwFlags & D3DPRESENTFLAG_INTERLACED ? "i" : "" );
    CLog::Log( LOGNOTICE, "CQtUtils: Current resolution: %dx%d %s\n", res->iWidth, res->iHeight, res->strMode.c_str() );
    */
    return true;
}

bool CQtUtils::SetNativeResolution( const RESOLUTION_INFO &res )
{
    CLog::Log( LOGDEBUG, "CQtUtils: SetNativeResolution: %s: %dx%d %dx%d@%f", res.strId.c_str(), res.iWidth, res.iHeight, res.iScreenWidth, res.iScreenHeight, res.fRefreshRate );
...
bool CQtUtils::ProbeResolutions( std::vector<RESOLUTION_INFO> &resolutions )
{
#if 0
    RESOLUTION_INFO cur_res;
    bool ret = GetNativeResolution( &cur_res );

    CLog::Log( LOGDEBUG, "CQtUtils: ProbeResolutions: %dx%d", m_width, m_height );
