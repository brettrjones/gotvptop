...
bool CWinSystemWin32::CreateNewWindow(const std::string& name, bool fullScreen, RESOLUTION_INFO& res)
{
  ...
  {
      CLog::LogF( LOGERROR, " RegisterClassExA failed with %d", GetLastError() );
      return false;
  }
...
bool CWinSystemWin32::CreateBlankWindows()
{
  ...
  {
#ifdef UNICODE
    HWND hBlankWindow = CreateWindowExW(WS_EX_TOPMOST, L"BlankWindowClass", L"", WS_POPUP | WS_DISABLED,
    CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr, nullptr, nullptr);
#else
      HWND hBlankWindow = CreateWindowExA( WS_EX_TOPMOST, "BlankWindowClass", "", WS_POPUP | WS_DISABLED,
                                           CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr, nullptr, nullptr );
#endif // UNICODE
...
#else
         outputs->push_back( display.MonitorNameA );
#endif
    }
}
...
#else
  info.strOutput = details->DeviceNameA;
  CLog::LogF( LOGDEBUG, "restoring desktop resolution for '{}'.", details->MonitorNameA );
#endif // UNICODE
...
MONITOR_DETAILS* CWinSystemWin32::GetDisplayDetails(const std::string& name)
{
  ...
  if (!name.empty() && name != "Default")
  {
    ...
    auto it = std::find_if(m_displays.begin(), m_displays.end(), [&nameW](MONITOR_DETAILS& m)
    {
      ...
      auto it = std::find_if( m_displays.begin(), m_displays.end(), [&nameA]( MONITOR_DETAILS& m )
      {
          if( nameA[ 0 ] == '\\' ) // name is device name
              return m.DeviceNameA == nameA;
          return m.MonitorNameA == nameA;
      } );
#endif // UNICODE
...
RECT CWinSystemWin32::ScreenRect(HMONITOR handle)
{
  ...
  ZeroMemory( &sDevMode, sizeof( sDevMode ) );
  sDevMode.dmSize = sizeof( sDevMode );
  if( !EnumDisplaySettingsA( details->DeviceNameA.c_str(), ENUM_CURRENT_SETTINGS, &sDevMode ) )
      CLog::LogF( LOGERROR, " EnumDisplaySettings failed with %d", GetLastError() );
#endif // UNICODE
  RECT rc;
...
#else
  DISPLAY_DEVICEA ddAdapter = { 0 };
  ddAdapter.cb = sizeof( ddAdapter );

  for( DWORD adapter = 0; EnumDisplayDevicesA( nullptr, adapter, &ddAdapter, 0 ); ++adapter )
  {
      // Exclude displays that are not part of the windows desktop. Using them is too different: no windows,
      // direct access with GDI CreateDC() or DirectDraw for example. So it may be possible to play video, but GUI?
      if( ( ddAdapter.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER )
      ...
      DISPLAY_DEVICEA ddMon = { 0 };
      ddMon.cb = sizeof( ddMon );
      bool foundScreen = false;

      DWORD screen = 0;
      // Just look for the first active output, we're actually only interested in the information at the adapter level.
      for( ; EnumDisplayDevicesA( ddAdapter.DeviceName, screen, &ddMon, 0 ); ++screen )
      {
          if( ddMon.StateFlags & ( DISPLAY_DEVICE_ACTIVE | DISPLAY_DEVICE_ATTACHED ) )
          {
              foundScreen = true;
      ...
      {
          //lstrcpyW( ddMon.DeviceString, L"Dummy Monitor" ); // safe: large static array
          strcpy( ddMon.DeviceString, "Dummy Monitor" ); // safe: large static array
          foundScreen = true;
      }
      ...
      if( foundScreen )
      {
          ...
          ZeroMemory( &dm, sizeof( dm ) );
          dm.dmSize = sizeof( dm );
          if( EnumDisplaySettingsExA( ddAdapter.DeviceName, ENUM_CURRENT_SETTINGS, &dm, 0 ) == FALSE )
              EnumDisplaySettingsExA( ddAdapter.DeviceName, ENUM_REGISTRY_SETTINGS, &dm, 0 );
          ...
          {
              // `Monitor #N`
              md.MonitorNameA = std::string( ddMon.DeviceString ) + " #" + std::to_string( num++ );
          } while( std::any_of( outputs.begin(), outputs.end(), [&]( MONITOR_DETAILS& m ) { return m.MonitorNameA == md.MonitorNameA; } ) );

          md.CardNameA = ddAdapter.DeviceString;
          md.DeviceNameA = ddAdapter.DeviceName;
          md.ScreenWidth = dm.dmPelsWidth;
          md.ScreenHeight = dm.dmPelsHeight;
          md.hMonitor = hm;
          md.RefreshRate = dm.dmDisplayFrequency;
          ...
          MONITORINFO mi = { 0 };
          mi.cbSize = sizeof( mi );
          if( GetMonitorInfoA( hm, &mi ) && ( mi.dwFlags & MONITORINFOF_PRIMARY ) )
              md.IsPrimary = true;
      ...
      || forceChange)
  {
...
#else
    std::string outputA = res.strOutput;

    DEVMODEA sDevMode = { 0 };
    sDevMode.dmSize = sizeof( sDevMode );

    // If we can't read the current resolution or any detail of the resolution is different than res
    if( !EnumDisplaySettingsA( outputA.c_str(), ENUM_CURRENT_SETTINGS, &sDevMode ) ||
        sDevMode.dmPelsWidth != res.iWidth || sDevMode.dmPelsHeight != res.iHeight ||
        sDevMode.dmDisplayFrequency != static_cast< int >( res.fRefreshRate ) ||
        ( ( sDevMode.dmDisplayFlags & DM_INTERLACED ) && !( res.dwFlags & D3DPRESENTFLAG_INTERLACED ) ) ||
        ( !( sDevMode.dmDisplayFlags & DM_INTERLACED ) && ( res.dwFlags & D3DPRESENTFLAG_INTERLACED ) )
        || forceChange )
    {
            ...
            && ( res.fRefreshRate == 24.0 || res.fRefreshRate == 48.0 || res.fRefreshRate == 60.0 ) )
        {
            ...
            // Get current resolution stored in registry
            DEVMODEA sDevModeRegistry = { 0 };
            sDevModeRegistry.dmSize = sizeof( sDevModeRegistry );
            if( EnumDisplaySettingsA( outputA.c_str(), ENUM_REGISTRY_SETTINGS, &sDevModeRegistry ) )
            {
                // Set requested mode in registry without actually changing resolution
                rc = ChangeDisplaySettingsExA( outputA.c_str(), &sDevMode, nullptr, CDS_UPDATEREGISTRY | CDS_NORESET, nullptr );
                if( rc == DISP_CHANGE_SUCCESSFUL )
                {
                    // Change resolution based on registry setting
                    rc = ChangeDisplaySettingsExA( outputA.c_str(), nullptr, nullptr, CDS_FULLSCREEN, nullptr );
                    if( rc == DISP_CHANGE_SUCCESSFUL )
                        bResChanged = true;
                    else
                        CLog::LogF( LOGERROR, "ChangeDisplaySettingsEx (W8+ change resolution) failed with %d, using fallback", rc );
                    ...
                    sDevModeRegistry.dmDriverExtra = 0;
                    sDevModeRegistry.dmFields = DM_PELSWIDTH | DM_PELSHEIGHT | DM_DISPLAYFREQUENCY | DM_DISPLAYFLAGS;
                    rc = ChangeDisplaySettingsExA( outputA.c_str(), &sDevModeRegistry, nullptr, CDS_UPDATEREGISTRY | CDS_NORESET, nullptr );
                    if( rc != DISP_CHANGE_SUCCESSFUL )
                        CLog::LogF( LOGERROR, "ChangeDisplaySettingsEx (W8+ restore registry) failed with %d", rc );
                }
                else
        ...
        if( !bResChanged )
        {
            // CDS_FULLSCREEN is for temporary fullscreen mode and prevents icons and windows from moving
            // to fit within the new dimensions of the desktop
            rc = ChangeDisplaySettingsExA( outputA.c_str(), &sDevMode, nullptr, CDS_FULLSCREEN, nullptr );
            if( rc == DISP_CHANGE_SUCCESSFUL )
                bResChanged = true;
            else
                CLog::LogF( LOGERROR, "ChangeDisplaySettingsEx failed with %d", rc );
...
#else
  std::string strOuput = details->DeviceNameA;
#endif // UNICODE
...
#else
  std::string monitorName = details->MonitorNameA;
#endif // UNICODE
    for(int mode = 0;; mode++)
    {
#ifdef UNICODE
    ...
    if(EnumDisplaySettingsW(details->DeviceNameW.c_str(), mode, &devmode) == 0)
        ...
        DEVMODEA devmode = { 0 };
        devmode.dmSize = sizeof( devmode );
        if( EnumDisplaySettingsA( details->DeviceNameA.c_str(), mode, &devmode ) == 0 )
            break;
#endif // UNICODE
