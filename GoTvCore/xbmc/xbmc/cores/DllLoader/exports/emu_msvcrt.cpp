...
extern "C" void __stdcall init_emu_environ()
{
  ...
  using KODI::PLATFORM::WINDOWS::FromW;
    ...
    if( lpvEnv != NULL )
    {
        lpszVariable = (char * )lpvEnv;
        while( *lpszVariable )
        {
            std::string utf8Str = lpszVariable;
            //CLog::Log( LOGDEBUG, "environment %s", utf8Str.c_str());
...
extern "C"
{
    ...
    int dll_read( int fd, void* buffer, unsigned int uiSize )
    {
        ...
        else if( !IS_STD_DESCRIPTOR( fd ) )
        {
            // it might be something else than a file, or the file is not emulated
            // let the operating system handle it
            return read_os( fd, buffer, uiSize );
        }
    ...
    int dll_write( int fd, const void* buffer, unsigned int uiSize )
    {
        ...
        else if( !IS_STD_DESCRIPTOR( fd ) )
        {
            // it might be something else than a file, or the file is not emulated
            // let the operating system handle it
            return write_os( fd, buffer, uiSize );
        }
        CLog::Log( LOGERROR, "%s emulated function failed", __FUNCTION__ );
        errno = EBADF;
        return -1;
...
#ifndef TARGET_POSIX
            return static_cast< long long >( tell_os( fd ) );
