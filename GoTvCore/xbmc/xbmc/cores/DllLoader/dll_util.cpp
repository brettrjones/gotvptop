...
uintptr_t get_win_function_address(const char* strDllName, const char* strFunctionName)
{
  ...
  HMODULE handle = GetModuleHandleW(strDllNameW.c_str());
  if(handle == nullptr)
  {
    handle = LoadLibraryW(strDllNameW.c_str());
  }
#else
    std::string strDllNameA = strDllName;
    HMODULE handle = GetModuleHandleA( strDllNameA.c_str() );
    if( handle == nullptr )
    {
        handle = LoadLibraryA( strDllNameA.c_str() );
    }
#endif // UNICODE
