...
/*
    ...
    if( g_StreamActive && firstDump && bytes > 400 )
    {
        tryCnt++;
        bool hasData = false;
        int srcPlanes = frame.planes > 1 ? 2 : 1;
        int srcSamples = bytes / ( sizeof( float ) * srcPlanes );

        for( int i = 0; i < srcPlanes; i++ )
        {
            float* srcBuf = ( float* )frame.data[ i ];
        ...
        if( hasData )
        {
            ...
            LogMsg( LOG_DEBUG, "BRJ CDVDAudioCodecFFmpeg::AddData start try %d offs %d\n", tryCnt, tryOffs );
            int srcPlanes = frame.planes > 1 ? 2 : 1;
            int srcSamples = bytes / ( sizeof( float ) * srcPlanes );
            for( int i = 0; i < srcPlanes; i++ )
            {
                float* buf = ( float* )frame.data[ i ];
                DumpFloat( LOG_DEBUG, ( float * )buf, srcSamples * sizeof( float ) );
            }

            LogMsg( LOG_DEBUG, "BRJ CDVDAudioCodecFFmpeg::AddData done\n" );
        }
    }
*/
