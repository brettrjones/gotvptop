...
bool CDVDInputStreamBluray::Open()
{
  ...
  else
  {
    ...
    if(m_dll->bd_select_playlist(m_bd, m_titleInfo->playlist) == 0 )
    {
...
void CDVDInputStreamBluray::ProcessEvent() {
  ...
  case BD_EVENT_ANGLE:
    ...
    {
        FreeTitleInfo();
      m_titleInfo = m_dll->bd_get_playlist_info(m_bd, m_playlist, m_angle);
    }
    break;
...
{
  if(m_titleInfo)
    return m_titleInfo->chapter_count;
  else
    return 0;
}
...
{
  if(m_titleInfo)
    return static_cast<int>(m_dll->bd_get_current_chapter(m_bd) + 1);
  else
    return 0;
}
...
{
  return static_cast<int64_t>(m_dll->bd_get_title_size(m_bd));
}
...
bool CDVDInputStreamBluray::MouseMove(const CPoint &point)
{
  ...
  if (m_dll->bd_mouse_select(m_bd, -1, static_cast<uint16_t>(point.x), static_cast<uint16_t>(point.y)) < 0)
  {
...
void CDVDInputStreamBluray::SetupPlayerSettings()
{
  ...
  m_dll->bd_set_player_setting(m_bd, BLURAY_PLAYER_SETTING_REGION_CODE, static_cast<uint32_t>(region));
