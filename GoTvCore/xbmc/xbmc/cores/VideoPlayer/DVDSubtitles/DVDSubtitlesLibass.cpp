...
bool CDVDSubtitlesLibass::DecodeDemuxPkt(const char* data, int size, double start, double duration)
{
  ...
  m_dll.ass_process_chunk(m_track, const_cast<char*>(data), size, DVD_TIME_TO_MSEC(start), DVD_TIME_TO_MSEC(duration));
  return true;
}
                                            ...
                                            double pts, int useMargin, double position, int *changes)
{
  ...
  m_dll.ass_set_aspect_ratio(m_renderer, dar, sar);
