...
char * describeAESinkState( int state )
{
    switch( state )
    {
    case S_TOP:
    ...
    case S_TOP_UNCONFIGURED:
        return "S_TOP_UNCONFIGURED";
    case S_TOP_CONFIGURED:
        return "S_TOP_CONFIGURED";
    case S_TOP_CONFIGURED_SUSPEND:
        return "S_TOP_CONFIGURED_SUSPEND";
    case S_TOP_CONFIGURED_IDLE:
        return "S_TOP_CONFIGURED_IDLE";
    case S_TOP_CONFIGURED_PLAY:
        return "S_TOP_CONFIGURED_PLAY";
    case S_TOP_CONFIGURED_SILENCE:
        return "S_TOP_CONFIGURED_SILENCE";
...
char * describeSinkControlSignal( int state )
{
    switch( state )
    {
...
char * describeSinkDataSignal( int state )
{
    switch( state )
    {
...
void CActiveAESink::StateMachine( int signal, Protocol *port, Message *msg )
{
    for( int state = m_state; ; state = SINK_parentStates[ state ] )
    {
        ...
        if( g_StreamActive && ( signal != CSinkDataProtocol::SAMPLE ) && ( signal != CActiveAEDataProtocol::STREAMSAMPLE ) )
        {
            ...
            {
                CLog::Log( LOGWARNING, "BRJ CActiveAESink::StateMachine state %s signal %s port 0x%x", describeAESinkState( state ), describeSinkControlSignal( signal ), ( unsigned int )port );
            }
            else if( port == &m_dataPort )
            {
                CLog::Log( LOGWARNING, "BRJ CActiveAESink::StateMachine state %s signal %s port 0x%x", describeAESinkState( state ), describeSinkDataSignal( signal ), ( unsigned int )port );
            }

            if( ( state == S_TOP_CONFIGURED_PLAY )
                && ( port == nullptr ) )
            {
        ...
        case S_TOP_CONFIGURED:
            ...
            else if( port == &m_dataPort )
            {
                ...
                case CSinkDataProtocol::SAMPLE:
                    ...
                    //CLog::Log(LOGERROR, "BRJ OutputSamples SAMPLE delay %d", delay);
                    msg->Reply( CSinkDataProtocol::RETURNSAMPLE, &samples, sizeof( CSampleBuffer* ) );
                    if( m_extError )
                    {
                        m_sink->Deinitialize();
        ...
        default: // we are in no state, should not happen
            CLog::Log( LOGERROR, "CActiveAESink::%s - no valid state: %s", __FUNCTION__, describeAESinkState( m_state ) );
            return;
        }
    } // for
}
...
unsigned int CActiveAESink::OutputSamples( CSampleBuffer* samples )
{
    ...
    while( frames > 0 )
    {
        ...
        m_sink->GetDelay( status );
       // CLog::Log( LOGERROR, "BRJ OutputSamples Audio Delay after write %3.3f ", status.delay );
        if( m_requestedFormat.m_dataFormat != AE_FMT_RAW )
    ...
    //CLog::Log(LOGERROR, "BRJ OutputSamples Audio Delay return %3.3f ", status.delay  * 1000 );
    return status.delay * 1000;
}
