    ...
    while( copied < frames )
    {
      ...
      if (extData && extData->hasDownmix)
        m_currentBuffer->centerMixLevel = extData->centerMixLevel;
            ...
            //static int tryCnt = 0;
            //static int tryOffs = 0;
            //int srcPlanes = planes;
            //int srcSamples = m_currentBuffer->pkt->nb_samples;

            //if(  ( m_format.m_dataFormat == AE_FMT_FLOATP ) && firstDump && srcSamples > 400 )
            //{
            //    tryCnt++;
            //    bool hasData = false;
            ...
            //    {
            //        float* srcBuf = ( float* )m_currentBuffer->pkt->data[ i ];
            //        for( int j = 0; j < srcSamples; j++ )
            //        {
            //            if( ( srcBuf[ j ] > 0.01 ) || ( srcBuf[ j ] < -0.01 ) )
            //            {
            //                hasData = true;
            //                tryOffs = j + ( i * srcSamples );
            //                break;
            //            }
            //        }
            //    }
            ...
            //    {
            //        firstDump = false;
            //        int dumpCnt = std::min( 1000, srcSamples );

            //        LogMsg( LOG_DEBUG, "BRJ CActiveAEStream::AddData start try %d offs %d\n", tryCnt, tryOffs );
            //        for( int i = 0; i < srcPlanes; i++ )
            //        {
            //            float* buf = ( float* )m_currentBuffer->pkt->data[ i ];
            //            DumpFloat( LOG_DEBUG, buf, srcSamples * sizeof( float ), i, "CActiveAEStream::AddData" );
            //        }

            //        LogMsg( LOG_DEBUG, "BRJ CActiveAEStream::AddData done\n" );
            //    }
            //}
