...
int CActiveAEResampleFFMPEG::Resample( uint8_t **dst_buffer, int dst_samples, uint8_t **src_buffer, int src_samples, double ratio )
{
   ...
   // int planes3 = av_sample_fmt_is_planar( m_src_fmt ) ? m_src_channels : 1;
   // int samples3 = src_samples / planes3;
   // static int frameCnt = 0;
   // LogMsg( LOG_DEBUG, "BRJ Resample delta %d distance %d ratio %3.3f resample %d samples %d\n", delta, distance, ratio, m_doesResample, samples3 );
   //if( samples3 > 16 )
   // {
   //     frameCnt++;
   //     if( m_src_fmt == AV_SAMPLE_FMT_U8 )
   //     {
   //         DumpInt8( LOG_DEBUG, ( int8_t * )src_buffer[ 0 ], 16, frameCnt, "pre-resample u8" );
   //     }
   //     else if( m_src_fmt == AV_SAMPLE_FMT_S16 || m_src_fmt == AV_SAMPLE_FMT_S16P )
   //     {
   //         DumpInt16( LOG_DEBUG, ( int16_t * )src_buffer[ 0 ], 16 * 2, frameCnt, "pre-resample s16" );
   //     }
   //     else if( m_src_fmt == AV_SAMPLE_FMT_FLT || m_src_fmt == AV_SAMPLE_FMT_FLTP )
   //     {
   //         DumpFloat( LOG_DEBUG, ( float * )src_buffer[ 0 ], 16 * 4, frameCnt, "pre-resample float" );
   //     }
   // }
   // else
   // {
   ...
   // }
    ...
    //int planes2 = av_sample_fmt_is_planar( m_dst_fmt ) ? m_dst_channels : 1;
    //int samples2 = ret * m_dst_channels / planes2;
    //if( samples2 > 16 )
    //{
    //    frameCnt++;
    //    if( m_dst_fmt == AV_SAMPLE_FMT_U8 )
    //    {
    //        DumpInt8( LOG_DEBUG, ( int8_t * )dst_buffer[ 0 ], 16, frameCnt, "resample u8" );
    //    }
    //    else if( m_dst_fmt == AV_SAMPLE_FMT_S16 || m_dst_fmt == AV_SAMPLE_FMT_S16P )
    //    {
    //        DumpInt16( LOG_DEBUG, ( int16_t * )dst_buffer[ 0 ], 16 * 2, frameCnt, "resample s16" );
    //    }
    //    else if( m_dst_fmt == AV_SAMPLE_FMT_FLT || m_dst_fmt == AV_SAMPLE_FMT_FLTP )
    //    {
    //        DumpFloat( LOG_DEBUG, ( float * )dst_buffer[ 0 ], 16 * 4, frameCnt, "resample float" );
    //    }
    //}
    //else
    //{
