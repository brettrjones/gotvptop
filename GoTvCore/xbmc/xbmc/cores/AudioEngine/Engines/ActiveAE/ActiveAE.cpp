...
{
    g_BeginDebug = false;
    for( int i = 0; i < floatCnt/4; i++ )
    {
        if( ( data[i] > 0.001f ) || ( data[i] < -0.001f ) )
        {
            g_BeginDebug = true;
            break;
        }
...
char * describeActiveAEState( int state )
{
    ...
    case AE_TOP_WAIT_PRECOND:
        return "AE_TOP_WAIT_PRECOND";
    case AE_TOP_ERROR:
        return "AE_TOP_ERROR";
    case AE_TOP_UNCONFIGURED:
        return "AE_TOP_UNCONFIGURED";
    case AE_TOP_RECONFIGURING:
        return "AE_TOP_RECONFIGURING";
    case AE_TOP_CONFIGURED:
        return "AE_TOP_CONFIGURED";
    case AE_TOP_CONFIGURED_SUSPEND:
        return "AE_TOP_CONFIGURED_SUSPEND";
    case AE_TOP_CONFIGURED_IDLE:
        return "AE_TOP_CONFIGURED_IDLE";
    case AE_TOP_CONFIGURED_PLAY:
        return "AE_TOP_CONFIGURED_PLAY";
    default:
        return "UNKNOWN AE STATE";
    }
}
...
char * describeActiveAESinkDataSignal( int signal )
{
    ...
    case CSinkDataProtocol::DRAIN:
        return "DRAIN";
    default:
        return "UNKNOWN SINK DATA SIGNAL";
    }
};
...
void CActiveAE::StateMachine( int signal, Protocol *port, Message *msg )
{
    for( int state = m_state; ; state = AE_parentStates[ state ] )
    {
        ...
        if( g_StreamActive && ( signal != CSinkDataProtocol::SAMPLE ) && ( signal != CActiveAEDataProtocol::STREAMSAMPLE ) )
        {
            ...
            {
                sprintf( buf, "BRJ CActiveAE::StateMachine state %s signal %s port 0x%x", describeActiveAEState( state ), describeActiveAEControlOutSignal( signal ), ( unsigned int )port );
            }
            else if( port == &m_dataPort )
            {
                sprintf( buf, "BRJ CActiveAE::StateMachine state %s signal %s port 0x%x", describeActiveAEState( state ), describeActiveAEDataSignal( signal ), ( unsigned int )port );
            }
            else if( port == &m_sink.m_dataPort )
            {
                sprintf( buf, "BRJ CActiveAE::StateMachine state %s signal %s port 0x%x", describeActiveAEState( state ), describeActiveAESinkDataSignal( signal ), ( unsigned int )port );
            }
            else
            {
                buf[ 0 ] = 0;
        ...
        case AE_TOP_CONFIGURED_IDLE:
            ...
            else if( port == NULL ) // timeout
            {
                ...
                case CActiveAEControlProtocol::TIMEOUT:
                    ...
                    {
                        m_extTimeout = 5000;
                        //LogMsg( LOG_DEBUG, "BRJ AE_TOP_CONFIGURED_IDLE waiting 5 seconds for ActiveAE to set port" );
                    }
                    return;
                default:
                    break;
...
void CActiveAE::Process()
{
    ...
    while( !m_bStop )
    {
        ...
        if( m_bStateMachineSelfTrigger )
        {
            ...
            LogMsg( LOG_DEBUG, "CActiveAE::Process self trigger for port 0x%x", port );
...
void CActiveAE::LoadSettings()
{
  const std::shared_ptr<CSettings> settings = CServiceBroker::GetSettingsComponent()->GetSettings();
    ...
    m_settings.stereoupmix = false;
    m_settings.normalizelevels = false;
    m_settings.guisoundmode = 1;
    m_settings.passthrough = false;
    m_settings.ac3passthrough = false;
    m_settings.ac3transcode = false;
    m_settings.eac3passthrough = false;
    m_settings.truehdpassthrough = false;
    m_settings.dtspassthrough = false;
    m_settings.dtshdpassthrough = false;
