...
class CUtil
{
  CUtil() = delete;
public:
    static char                 GetPathSeperator( void ); // get os specific path separator windows = '\\', everybody else '/'
	...
	static std::string			GetTitleFromPath( const GoTvUrl& url, bool bIsFolder = false );
