...
{
  int fd = open_os(path.c_str(), O_RDWR, 0644);
  int ret = 0;
  if (fd >= 0)
  {
    if (write_os(fd, valstr.c_str(), valstr.size()) < 0)
      ret = -1;
    close_os(fd);
  }
  if (ret)
...
int SysfsUtils::GetString(const std::string& path, std::string& valstr)
{
  ...
  int fd = open_os(path.c_str(), O_RDONLY);
  if (fd >= 0)
  {
    valstr.clear();
    while ((len = read_os(fd, buf, 256)) > 0)
...
int SysfsUtils::SetInt(const std::string& path, const int val)
{
  ...
  if (fd >= 0)
  {
    char bcmd[16];
    sprintf(bcmd, "%d", val);
    if (write_os(fd, bcmd, strlen(bcmd)) < 0)
      ret = -1;
    close_os(fd);
  }
  if (ret)
...
int SysfsUtils::GetInt(const std::string& path, int& val)
{
  ...
  {
    char bcmd[16];
    if (read_os(fd, bcmd, sizeof(bcmd)) < 0)
      ret = -1;
    else
...
{
  int fd = open_os(path.c_str(), O_RDWR);
  if (fd >= 0)
  {
    close_os(fd);
    return true;
