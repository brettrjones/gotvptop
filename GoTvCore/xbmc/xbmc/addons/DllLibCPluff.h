...
#else
#include "StaticDll.h"
class DllLibCPluff : public StaticDll, public DllLibCPluffInterface
{
public:
    DllLibCPluff( );
    DllLibCPluff( const DllLibCPluff& ) = default;
    DllLibCPluff& operator=( const DllLibCPluff& rhs );

    DllLibCPluff( DllLibCPluff&& );
    DllLibCPluff& operator=( DllLibCPluff&& );

    virtual ~DllLibCPluff() override;

    const char* get_version();
    void set_fatal_error_handler( cp_fatal_error_func_t p1 );
    cp_status_t init();
    void destroy();
    cp_context_t* create_context( cp_status_t *p1 );
    void destroy_context( cp_context_t *p1 );

    cp_status_t register_pcollection( cp_context_t *p1, const char *p2 );
    void unregister_pcollection( cp_context_t *p1, const char *p2 );
    void unregister_pcollections( cp_context_t *p1 );

    cp_status_t register_logger( cp_context_t *p1, cp_logger_func_t p2, void *p3, cp_log_severity_t p4 );
    void unregister_logger( cp_context_t *p1, cp_logger_func_t p2 );
    cp_status_t scan_plugins( cp_context_t *p1, int p2 );
    cp_plugin_info_t* get_plugin_info( cp_context_t *p1, const char *p2, cp_status_t *p3 );
    cp_plugin_info_t** get_plugins_info( cp_context_t *p1, cp_status_t *p2, int *p3 );
    cp_extension_t** get_extensions_info( cp_context_t *p1, const char *p2, cp_status_t *p3, int *p4 );
    void release_info( cp_context_t *p1, void *p2 );

    cp_cfg_element_t* lookup_cfg_element( cp_cfg_element_t *p1, const char *p2 );
    char* lookup_cfg_value( cp_cfg_element_t *p1, const char *p2 );

    cp_status_t define_symbol( cp_context_t *p1, const char *p2, void *p3 );
    void* resolve_symbol( cp_context_t *p1, const char *p2, const char *p3, cp_status_t *p4 );
    void release_symbol( cp_context_t *p1, const void *p2 );
    cp_plugin_info_t* load_plugin_descriptor( cp_context_t *p1, const char *p2, cp_status_t *p3 );
    cp_plugin_info_t* load_plugin_descriptor_from_memory( cp_context_t *p1, const char *p2, unsigned int p3, cp_status_t *p4 );
    cp_status_t uninstall_plugin( cp_context_t *p1, const char *p2 );
};
