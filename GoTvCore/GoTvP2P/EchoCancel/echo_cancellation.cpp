...
static void ProcessExtended(Aec* self,
  ...
  if (!self->farend_started) {
    for (i = 0; i < num_bands; ++i) {
      // Only needed if they don't already point to the same place.
      if (nearSamples[i] != out[i]) {
        memcpy(out[i], nearSamples[i], sizeof(nearSamples[i][0]) * num_samples);
      }
    }
    return;
  }
...

  {
    ...
    WebRtcAec_ProcessFrames(self->aec, nearSamples, num_bands, num_samples,
...
static void EstBufDelayExtended(Aec* self) {
  ...
  if (self->filtDelay == -1) {
    self->filtDelay = (short)WEBRTC_SPL_MAX(0, 0.5 * current_delay);
  } else {
    self->filtDelay = (short)WEBRTC_SPL_MAX(
