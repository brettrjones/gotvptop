...
namespace rtc {
	...
	bool DirectoryIterator::Iterate(const Pathname &dir) {
		...
		handle_ = ::FindFirstFile(d.c_str(), &data_);
		if (handle_ == INVALID_HANDLE_VALUE)
			return false;
#else
		if (dir_ != NULL)
...
#endif
		return ( TimeDiff(time(NULL), file_modify_time) >= seconds ) ? true : false;
	}
	...
	{
		LogMsg(LOG_DEBUG, "Create UniquieFile Path %s\n", path.pathname().c_str());
