...
namespace rtc {
	...
	bool Win32Filesystem::CreateFolder(const Pathname &pathname) {
		...
		DWORD res = ::GetFileAttributesW(path16.c_str());
		...
		return (::CreateDirectoryW(path16.c_str(), NULL) != 0);
	}
	...
	bool Win32Filesystem::CreatePrivateFile(const Pathname &filename) {
		...
		TOKEN_QUERY,
		&process_token)) {
		LogMsg( LOG_ERROR, "Win32Filesystem::CreatePrivateFile OpenProcessToken() failed" );
		return false;
		}
		...
		if (!success) {
		LogMsg( LOG_ERROR, "GetTokenInformation() failed" );
		return false;
		}

		if (!IsValidSid(token_user->User.Sid)) {
		LogMsg( LOG_ERROR, "Current process has invalid user SID" );
		return false;
		}
		...
		LogMsg( LOG_ERROR,  "InitializeAcl() failed" );
		return false;
		}
		...
		token_user->User.Sid)) {
		LogMsg( LOG_ERROR,  "AddAccessAllowedAce() failed" );
		return false;
		}
		...
		LogMsg( LOG_ERROR,  "InitializeSecurityDescriptor() failed" );
		return false;
		}
		...
		acl,
		FALSE)) {
		LogMsg( LOG_ERROR,  "SetSecurityDescriptorDacl() failed" );
		return false;
		}
		...
		LogMsg( LOG_ERROR,  "CreateFile() failed" );
		return false;
		}
		if (!::CloseHandle(handle)) {
		LogMsg( LOG_ERROR,  "CloseFile() failed" );
		// Continue.
		}
		return true;
		*/
	...
	bool Win32Filesystem::DeleteFile(const Pathname &filename) 
	{
		...
		if (!IsFile(filename)) {
			vx_assert(IsFile(filename));
			return false;
		}
		return ::DeleteFileW(ToUtf16(filename.pathname()).c_str()) != 0;
	}
	...
	bool Win32Filesystem::DeleteEmptyFolder(const Pathname &folder) {
		...
		return ::RemoveDirectoryW(ToUtf16(no_slash).c_str()) != 0;
	}

	bool Win32Filesystem::GetTemporaryFolder(Pathname &pathname, bool create,
		const std::string *append) {
			wchar_t buffer[VX_MAX_PATH + 1];
			if (!::GetTempPathW(arraysize(buffer), buffer))
				return false;
			if (!IsCurrentProcessLowIntegrity() &&
				!::GetLongPathNameW(buffer, buffer, arraysize(buffer)))
				return false;
	...
	std::string Win32Filesystem::TempFilename(const Pathname &dir,
		const std::string &prefix) {
			wchar_t filename[VX_MAX_PATH];
			if (::GetTempFileNameW(ToUtf16(dir.pathname()).c_str(),
	...
	bool Win32Filesystem::MoveFile(const Pathname &old_path,
		const Pathname &new_path) {
			if (!IsFile(old_path)) {
				vx_assert(IsFile(old_path));
				return false;
			}
			//LOG(LS_INFO) << "Moving " << old_path.pathname()
			//             << " to " << new_path.pathname();
			return ::MoveFileW(ToUtf16(old_path.pathname()).c_str(),
	...
	bool Win32Filesystem::MoveFolder(const Pathname &old_path,
		const Pathname &new_path) {
			if (!IsFolder(old_path)) {
				vx_assert(IsFolder(old_path));
				return false;
			}
			//LOG(LS_INFO) << "Moving " << old_path.pathname()
			//             << " to " << new_path.pathname();
			if (::MoveFileW(ToUtf16(old_path.pathname()).c_str(),
				ToUtf16(new_path.pathname()).c_str()) == 0) {
					if (::GetLastError() != ERROR_NOT_SAME_DEVICE) {
						LogMsg( LOG_ERROR, "Win32Filesystem::MoveFolder Failed to move file" );
						return false;
					}
	...
	bool Win32Filesystem::IsFolder(const Pathname &path) {
		...
		if (0 == ::GetFileAttributesExW(ToUtf16(path.pathname()).c_str(),
	...
	bool Win32Filesystem::CopyFile(const Pathname &old_path,
		const Pathname &new_path) {
			return ::CopyFileW(ToUtf16(old_path.pathname()).c_str(),
	...
	bool Win32Filesystem::GetFileSize(const Pathname &pathname, size_t *size) {
		WIN32_FILE_ATTRIBUTE_DATA data = {0};
		if (::GetFileAttributesExW(ToUtf16(pathname.pathname()).c_str(),
	...
	bool Win32Filesystem::GetFileTime(const Pathname& path, FileTimeType which,
		time_t* time) {
			WIN32_FILE_ATTRIBUTE_DATA data = {0};
			if (::GetFileAttributesExW(ToUtf16(path.pathname()).c_str(),
				GetFileExInfoStandard, &data) == 0)
				return false;
			switch (which) {
  case FTT_CREATED:
  ...
  default:
	...
	bool Win32Filesystem::GetAppPathname(Pathname* path) {
		wchar_t buffer[MAX_PATH + 1];
		if (0 == ::GetModuleFileNameW(NULL, buffer, arraysize(buffer)))
			return false;
	...
	bool Win32Filesystem::GetAppDataFolder(Pathname* path, bool per_user) {
		vx_assert(!organization_name_.empty());
		vx_assert(!application_name_.empty());
		wchar_t buffer[MAX_PATH + 1];
		...
		if (!IsCurrentProcessLowIntegrity() &&
			!::GetLongPathNameW(buffer, buffer, arraysize(buffer)))
			return false;
		size_t len = strcatn(buffer, arraysize(buffer), L"\\");
	...
	bool Win32Filesystem::GetDiskFreeSpace(const Pathname& path,
		int64_t* free_bytes) {
			...
			UINT drive_type = ::GetDriveTypeW(target_drive);
			...
			vx_assert(sizeof(ULARGE_INTEGER) == sizeof(uint64_t));  // NOLINT
			if (::GetDiskFreeSpaceExW(target_drive,
				(PULARGE_INTEGER)free_bytes,
				(PULARGE_INTEGER)&total_number_of_bytes,
				(PULARGE_INTEGER)&total_number_of_free_bytes)) {
					return true;
			} else {
				LogMsg( LOG_ERROR, "GetDiskFreeSpaceEx returns error." );
				return false;
			}
	}
