...
namespace webrtc {
	...
	{
		LogMsg( LOG_INFO,  "AudioDeviceBuffer::SetId(id=%d)", id);
		_id = id;
	}
		...
		int clockDrift) {
			if (high_delay_counter_ < kLogHighDelayIntervalFrames) {
				...
				if (playDelayMs + recDelayMs > kHighDelayThresholdMs) {
					...
					LogMsg( LOG_WARN, "High audio device delay reported (render=%d ms, capture=%d ms)\n", playDelayMs, recDelayMs);
				}
			}
	...
	int32_t AudioDeviceBuffer::DeliverRecordedData()
	{
		...
		{
			LogMsg( LOG_WARN, "failed to deliver recorded data (AudioTransport does not exist)\n");
			return 0;
		}
	...
	int32_t AudioDeviceBuffer::RequestPlayoutData(size_t nSamples)
	{
		...
		size_t playChannels = 0;
		{
			...
			{
				LogMsg( LOG_WARN, "invalid number of samples to be played out (%d)\n", nSamples);
				return -1;
			}
		}
		...
		if (_ptrCbAudioTransport == NULL)
		{
			LogMsg( LOG_WARN, "failed to feed data to playout (AudioTransport does not exist)\n");
			return 0;
		}
		...
		if (_ptrCbAudioTransport)
		{
			...
			if (res != 0)
			{
				LogMsg( LOG_ERROR, "NeedMorePlayData() failed\n");
			}
		}
