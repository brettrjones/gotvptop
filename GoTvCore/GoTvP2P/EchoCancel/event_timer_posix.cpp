...
namespace webrtc {
...
#if !(defined(WEBRTC_MAC) || defined(WEBRTC_IOS)) && !defined(TARGET_OS_ANDROID) && defined(HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC)
	...
	bool EventTimerPosix::Set() {
		vx_assert(0 == pthread_mutex_lock(&mutex_));
		event_set_ = true;
	...
	EventTypeWrapper EventTimerPosix::Wait(unsigned long timeout_ms) {
...
#if defined(TARGET_OS_ANDROID) && defined(HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC)
					ret_val = pthread_cond_timedwait_monotonic_np(&cond_, &mutex_, &end_at);
#else
					ret_val = pthread_cond_timedwait(&cond_, &mutex_, &end_at);
#endif  // TARGET_OS_ANDROID && HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC
				}
			} else {
				while (ret_val == 0 && !event_set_)
		...
		vx_assert(ret_val == 0 || ret_val == ETIMEDOUT);
	...
	bool EventTimerPosix::StartTimer(bool periodic, unsigned long time_ms) {
		...
		timer_event_			= new EventTimerPosix();
		timer_thread_			= CreateThread();
		periodic_				= periodic;
		time_ms_				= time_ms;
		timer_thread_->Start();
