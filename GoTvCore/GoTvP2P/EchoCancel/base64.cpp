...
bool Base64::GetNextBase64Char(char ch, char* next_ch) {
  ...
  const char* p = strchr( (char *)Base64Table, ch);
  if (!p)
    return false;
  ++p;
...
{
  vx_assert(NULL != result);
  vx_assert(flags <= (DO_PARSE_MASK | DO_PAD_MASK | DO_TERM_MASK));
  ...
  vx_assert(0 != parse_flags);
  vx_assert(0 != pad_flags);
  vx_assert(0 != term_flags);
