...
FineAudioBuffer::FineAudioBuffer(AudioDeviceBuffer* device_buffer,
    ...
    : device_buffer_(device_buffer),
      ...
      record_write_pos_(0) 
{
  playout_cache_buffer_ = new int8_t[bytes_per_10_ms_];
  record_cache_buffer_ = new int8_t[required_record_buffer_size_bytes_];
  memset( record_cache_buffer_, 0, required_record_buffer_size_bytes_);
}
...
void FineAudioBuffer::ResetPlayout() {
  playout_cached_buffer_start_ = 0;
  playout_cached_bytes_ = 0;
  memset(playout_cache_buffer_, 0, bytes_per_10_ms_);
}
...
void FineAudioBuffer::GetPlayoutData(int8_t* buffer) {
  if (desired_frame_size_bytes_ <= playout_cached_bytes_) {
    memcpy(buffer, &playout_cache_buffer_[playout_cached_buffer_start_],
  ...
  playout_cached_buffer_start_ = 0;
  memcpy(playout_cache_buffer_, cache_ptr, playout_cached_bytes_);
}
...
void FineAudioBuffer::DeliverRecordedData(const int8_t* buffer,
  ...
  if (record_write_pos_ + size_in_bytes > required_record_buffer_size_bytes_) {
    if (record_cached_bytes_ > 0) {
      memmove(record_cache_buffer_,
              record_cache_buffer_ + record_read_pos_,
  ...
  memcpy(record_cache_buffer_ + record_write_pos_, buffer, size_in_bytes);
  ...
  while (record_cached_bytes_ >= bytes_per_10_ms_) {
    device_buffer_->SetRecordedBuffer(
        record_cache_buffer_ + record_read_pos_, samples_per_10_ms_);
