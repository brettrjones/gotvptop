/*****************************************************************************
 * gotvptop_aout.h : audio output interface
 *****************************************************************************
 * Copyright (C) 2002-2011 GOTV authors and VideoLAN
 *
...
#define GOTV_CODEC_SPDIFL GOTV_FOURCC('s','p','d','i')
#define GOTV_CODEC_SPDIFB GOTV_FOURCC('s','p','d','b')

#define AOUT_FMT_SPDIF( p_format ) \
    ( ((p_format)->i_format == GOTV_CODEC_SPDIFL)       \
       || ((p_format)->i_format == GOTV_CODEC_SPDIFB)   \
       || ((p_format)->i_format == GOTV_CODEC_A52)      \
       || ((p_format)->i_format == GOTV_CODEC_DTS) )

#define AOUT_FMT_HDMI( p_format )                   \
    ( (p_format)->i_format == GOTV_CODEC_EAC3        \
    ||(p_format)->i_format == GOTV_CODEC_TRUEHD      \
    ||(p_format)->i_format == GOTV_CODEC_MLP         \
    )
...
struct audio_output
{
    ...
    /**< Starts a new stream (mandatory, cannot be NULL).
      ...
      * \return GOTV_SUCCESS on success, non-zero on failure
...
/**
 * It describes the audio channel order GOTV expect.
 */
static const uint32_t pi_gotvptop_chan_order_wg4[] =
{
