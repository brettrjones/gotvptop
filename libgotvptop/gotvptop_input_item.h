/*****************************************************************************
 * gotvptop_input_item.h: Core input item
 *****************************************************************************
 * Copyright (C) 1999-2009 GOTV authors and VideoLAN
 * $Id: 62a81d31deb28d559f0bd6eac28074c8c83a29bf $
 *
...
#ifndef GOTV_INPUT_ITEM_H
#define GOTV_INPUT_ITEM_H 1

/**
 * \file
 * This file defines functions, structures and enums for input items in gotvptop
 */
...
struct input_item_t
{
    ...
    gotvptop_meta_t *p_meta;

    int         i_epg;               /**< Number of EPG entries */
    gotvptop_epg_t   **pp_epg;            /**< EPG entries */
    int64_t     i_epg_time;          /** EPG timedate as epoch time */
    const gotvptop_epg_t *p_epg_table;    /** running/selected program cur/next EPG table */

    int         i_slaves;            /**< Number of slaves */
    input_item_slave_t **pp_slaves;  /**< Slave entries that will be loaded by
                                          the input_thread */

    gotvptop_event_manager_t event_manager;

    gotvptop_mutex_t lock;                 /**< Lock for the item */
...
{
    /* Allow GOTV to trust the given option.
     * By default options are untrusted */
    GOTV_INPUT_OPTION_TRUSTED = 0x2,

    /* Add the option, unless the same option
     * is already present. */
    GOTV_INPUT_OPTION_UNIQUE  = 0x100,
};
...
{ \
    input_item_SetMeta (p_input, gotvptop_meta_ ## name, val); \
} \
static inline \
char *input_item_Get ## name (input_item_t *p_input) \
{ \
    return input_item_GetMeta (p_input, gotvptop_meta_ ## name); \
}
...
/* status of the gotvptop_InputItemPreparseEnded event */
 ...
 * \see gotvptop_readdir_helper_init()
 * \see gotvptop_readdir_helper_additem()
 * \see gotvptop_readdir_helper_finish()
 */
struct gotvptop_readdir_helper
{
...
/**
 * Init a gotvptop_readdir_helper struct
 *
 ...
 *
 * \param p_rdh previously inited gotvptop_readdir_helper struct
