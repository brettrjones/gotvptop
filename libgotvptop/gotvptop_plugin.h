/*****************************************************************************
 * gotvptop_plugin.h : Macros used from within a module.
 *****************************************************************************
 * Copyright (C) 2001-2006 GOTV authors and VideoLAN
...
#ifndef LIBGOTV_MODULES_MACROS_H
# define LIBGOTV_MODULES_MACROS_H 1

/**
 * \file
 * This file implements plugin (module) macros used to define a gotvptop module.
 */

enum gotvptop_module_properties
{
    GOTV_MODULE_CREATE,
    GOTV_CONFIG_CREATE,
    ...
    GOTV_MODULE_CPU_REQUIREMENT=0x100,
    GOTV_MODULE_SHORTCUT,
    GOTV_MODULE_CAPABILITY,
    GOTV_MODULE_SCORE,
    GOTV_MODULE_CB_OPEN,
    GOTV_MODULE_CB_CLOSE,
    GOTV_MODULE_NO_UNLOAD,
    GOTV_MODULE_NAME,
    GOTV_MODULE_SHORTNAME,
    GOTV_MODULE_DESCRIPTION,
    GOTV_MODULE_HELP,
    GOTV_MODULE_TEXTDOMAIN,
    /* Insert new GOTV_MODULE_* here */

    /* DO NOT EVER REMOVE, INSERT OR REPLACE ANY ITEM! It would break the ABI!
     * Append new items at the end ONLY. */
    GOTV_CONFIG_NAME=0x1000,
    ...
    GOTV_CONFIG_ADVANCED,
    /* enable advanced flag (args=none) */

    GOTV_CONFIG_VOLATILE,
    /* don't write variable to storage (args=none) */

    GOTV_CONFIG_PERSISTENT_OBSOLETE,
    /* unused (ignored) */
    ...
    GOTV_CONFIG_LIST_OBSOLETE,
    /* unused (ignored) */

    GOTV_CONFIG_ADD_ACTION_OBSOLETE,
    /* unused (ignored) */
     ...
     * (args=const char *, size_t (*)(gotvptop_object_t *, <type> **, char ***)) */

    /* Insert new GOTV_CONFIG_* here */
};
...
#ifdef __PLUGIN__
# define __GOTV_SYMBOL( symbol  ) CONCATENATE( symbol, MODULE_SYMBOL )
# define GOTV_MODULE_NAME_HIDDEN_SYMBOL \
    const char gotvptop_module_name[] = MODULE_STRING;
#else
# define __GOTV_SYMBOL( symbol )  CONCATENATE( symbol, MODULE_NAME )
# define GOTV_MODULE_NAME_HIDDEN_SYMBOL
#endif
...
EXTERN_SYMBOL typedef int (*gotvptop_set_cb) (void *, void *, int, ...);

#define gotvptop_plugin_set(...) gotvptop_set (opaque,   NULL, __VA_ARGS__)
#define gotvptop_module_set(...) gotvptop_set (opaque, module, __VA_ARGS__)
#define gotvptop_config_set(...) gotvptop_set (opaque, config, __VA_ARGS__)
 ...
 */
#define gotvptop_module_begin() \
EXTERN_SYMBOL DLL_SYMBOL \
int CDECL_SYMBOL __GOTV_SYMBOL(gotvptop_entry) (gotvptop_set_cb, void *); \
EXTERN_SYMBOL DLL_SYMBOL \
int CDECL_SYMBOL __GOTV_SYMBOL(gotvptop_entry) (gotvptop_set_cb gotvptop_set, void *opaque) \
{ \
    module_t *module; \
    module_config_t *config = NULL; \
    if (gotvptop_plugin_set (GOTV_MODULE_CREATE, &module)) \
        goto error; \
    if (gotvptop_module_set (GOTV_MODULE_NAME, (MODULE_STRING))) \
        goto error;

#define gotvptop_module_end() \
    (void) config; \
    return 0; \
error: \
    return -1; \
} \
GOTV_MODULE_NAME_HIDDEN_SYMBOL \
GOTV_METADATA_EXPORTS
...
{ \
    ...
    if (gotvptop_module_set (GOTV_MODULE_SHORTCUT, \
                        sizeof(shortcuts)/sizeof(shortcuts[0]), shortcuts)) \
        goto error; \
}

#define set_shortname( shortname ) \
    if (gotvptop_module_set (GOTV_MODULE_SHORTNAME, (const char *)(shortname))) \
        goto error;

#define set_description( desc ) \
    if (gotvptop_module_set (GOTV_MODULE_DESCRIPTION, (const char *)(desc))) \
        goto error;

#define set_help( help ) \
    if (gotvptop_module_set (GOTV_MODULE_HELP, (const char *)(help))) \
        goto error;

#define set_capability( cap, score ) \
    if (gotvptop_module_set (GOTV_MODULE_CAPABILITY, (const char *)(cap)) \
     || gotvptop_module_set (GOTV_MODULE_SCORE, (int)(score))) \
        goto error;

#define set_callbacks( activate, deactivate ) \
    if (gotvptop_module_set(GOTV_MODULE_CB_OPEN, #activate, (void *)(activate)) \
     || gotvptop_module_set(GOTV_MODULE_CB_CLOSE, #deactivate, \
                       (void *)(deactivate))) \
        goto error;

#define cannot_unload_broken_library( ) \
    if (gotvptop_module_set (GOTV_MODULE_NO_UNLOAD)) \
        goto error;

#define set_text_domain( dom ) \
    if (gotvptop_plugin_set (GOTV_MODULE_TEXTDOMAIN, (dom))) \
        goto error;
...
#define add_module( name, psz_caps, value, text, longtext, advc ) \
    add_string_inner( CONFIG_ITEM_MODULE, name, text, longtext, advc, \
                      value ) \
    gotvptop_config_set (GOTV_CONFIG_CAPABILITY, (const char *)(psz_caps));
...
#define add_float( name, v, text, longtext, advc ) \
    ...
    gotvptop_config_set (GOTV_CONFIG_VALUE, (double)(v));
...
#define add_bool( name, v, text, longtext, advc ) \
    ...
    if (v) gotvptop_config_set (GOTV_CONFIG_VALUE, (int64_t)true);

/* For removed option */
#define add_obsolete_inner( name, type ) \
    add_type_inner( type ) \
    gotvptop_config_set (GOTV_CONFIG_NAME, (const char *)(name)); \
    gotvptop_config_set (GOTV_CONFIG_REMOVED);
...
#define change_short( ch ) \
    gotvptop_config_set (GOTV_CONFIG_SHORTCUT, (int)(ch));

#define change_string_list( list, list_text ) \
    gotvptop_config_set (GOTV_CONFIG_LIST, \
                    (size_t)(sizeof (list) / sizeof (char *)), \
                    (const char *const *)(list), \
                    (const char *const *)(list_text));

#define change_string_cb( cb ) \
    gotvptop_config_set (GOTV_CONFIG_LIST_CB, #cb, (void *)(cb));
...
#define change_integer_cb( cb ) \
    gotvptop_config_set (GOTV_CONFIG_LIST_CB, #cb, (cb));

#define change_integer_range( minv, maxv ) \
    gotvptop_config_set (GOTV_CONFIG_RANGE, (int64_t)(minv), (int64_t)(maxv));

#define change_float_range( minv, maxv ) \
    gotvptop_config_set (GOTV_CONFIG_RANGE, (double)(minv), (double)(maxv));

/* For options that are saved but hidden from the preferences panel */
#define change_private() \
    gotvptop_config_set (GOTV_CONFIG_PRIVATE);

/* For options that cannot be saved in the configuration */
#define change_volatile() \
    change_private() \
    gotvptop_config_set (GOTV_CONFIG_VOLATILE);

#define change_safe() \
    gotvptop_config_set (GOTV_CONFIG_SAFE);

/* Meta data plugin exports */
#define GOTV_META_EXPORT( name, value ) \
    EXTERN_SYMBOL DLL_SYMBOL const char * CDECL_SYMBOL \
    __GOTV_SYMBOL(gotvptop_entry_ ## name) (void); \
    EXTERN_SYMBOL DLL_SYMBOL const char * CDECL_SYMBOL \
    __GOTV_SYMBOL(gotvptop_entry_ ## name) (void) \
    { \
         return value; \
...
#define GOTV_COPYRIGHT_VIDEOLAN \
...
#define GOTV_LICENSE_LGPL_2_1_PLUS \
...
#define GOTV_LICENSE_GPL_2_PLUS \
    "\x4c\x69\x63\x65\x6e\x73\x65\x64\x20\x75\x6e\x64\x65\x72\x20\x74" \
    "\x68\x65\x20\x74\x65\x72\x6d\x73\x20\x6f\x66\x20\x74\x68\x65\x20" \
...
#if defined (__LIBGOTV__)
# define GOTV_MODULE_COPYRIGHT GOTV_COPYRIGHT_VIDEOLAN
# ifndef GOTV_MODULE_LICENSE
#  define GOTV_MODULE_LICENSE GOTV_LICENSE_LGPL_2_1_PLUS
# endif
#endif

#ifdef GOTV_MODULE_COPYRIGHT
# define GOTV_COPYRIGHT_EXPORT GOTV_META_EXPORT(copyright, GOTV_MODULE_COPYRIGHT)
#else
# define GOTV_COPYRIGHT_EXPORT
#endif
#ifdef GOTV_MODULE_LICENSE
# define GOTV_LICENSE_EXPORT GOTV_META_EXPORT(license, GOTV_MODULE_LICENSE)
#else
# define GOTV_LICENSE_EXPORT
#endif

#define GOTV_METADATA_EXPORTS \
    GOTV_COPYRIGHT_EXPORT \
    GOTV_LICENSE_EXPORT
