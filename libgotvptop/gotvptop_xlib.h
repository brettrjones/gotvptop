/*****************************************************************************
 * gotvptop_xlib.h: initialization of Xlib
 *****************************************************************************
...
static inline bool gotvptop_xlib_init (gotvptop_object_t *obj)
{
    ...
    gotvptop_global_lock (GOTV_XLIB_MUTEX);
    ...
    if (_Xglobal_lock == NULL && unlikely(_XErrorFunction != NULL))
        ...
        fprintf (stderr, "%s:%u:%s: Xlib not initialized for threads.\n"
                 "This process is probably using LibGOTV incorrectly.\n"
                 "Pass \"--no-xlib\" to libgotvptop_new() to fix this.\n",
                 __FILE__, __LINE__, __func__);
    else if (XInitThreads ())
        ok = true;

    gotvptop_global_unlock (GOTV_XLIB_MUTEX);
