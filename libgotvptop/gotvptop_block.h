/*****************************************************************************
 * gotvptop_block.h: Data blocks management functions
 *****************************************************************************
 * Copyright (C) 2003 GOTV authors and VideoLAN
 ...
 *
 * @ref block_t is a generic structure to represent a binary blob within GOTV.
 ...
 * - i_dts/i_pts could be GOTV_TS_INVALID, it means no pts/dts
 ...
 * This is provided by LibGOTV so that manually heap-allocated blocks can safely
 * be deallocated even after the origin plugin has been unloaded from memory.
 *
 * When block_Release() is called, GOTV will free() the specified pointer.
 *
 ...
 * This is provided by LibGOTV so that mmap blocks can safely be deallocated
 ...
 * This is provided by LibGOTV so that segments can safely be deallocated
 * even after the allocating plugin has been unloaded from memory.
 *
...
#define block_cleanup_push( block ) gotvptop_cleanup_push (block_Cleanup, block)
 ...
 * gotvptop_fifo_Unlock() releases the lock.
 *
 ...
 * gotvptop_fifo_Lock(). Otherwise, the behaviour is undefined.
 *
 ...
 * previously empty FIFO or by calling gotvptop_fifo_Signal() directly.
 ...
 * to a certain date, then locks the FIFO again. See gotvptop_fifo_Wait().
 */
int gotvptop_fifo_TimedWaitCond(gotvptop_fifo_t *, gotvptop_cond_t *, mtime_t);
 ...
 *
 * This is equivalent to calling gotvptop_fifo_DequeueUnlocked() repeatedly until
...
{
    return gotvptop_fifo_GetCount(fifo) == 0;
}

static inline void gotvptop_fifo_Cleanup(void *fifo)
{
    gotvptop_fifo_Unlock((gotvptop_fifo_t *)fifo);
}
#define gotvptop_fifo_CleanupPush(fifo) gotvptop_cleanup_push(gotvptop_fifo_Cleanup, fifo)
