/*****************************************************************************
 * gotvptop_arrays.h : Arrays and data structures handling
 *****************************************************************************
...
#ifndef GOTV_ARRAYS_H_
#define GOTV_ARRAYS_H_

/**
 * \file
 * This file defines functions, structures and macros for handling arrays in gotvptop
 */
 ...
 ************************************************************************/
typedef struct gotvptop_array_t
{
    size_t i_count;
    void ** pp_elems;
} gotvptop_array_t;

static inline void gotvptop_array_init( gotvptop_array_t * p_array )
{
    p_array->i_count = 0;
    p_array->pp_elems = NULL;
}

static inline void gotvptop_array_clear( gotvptop_array_t * p_array )
{
    free( p_array->pp_elems );
    gotvptop_array_init( p_array );
}

/* Read */
static inline size_t gotvptop_array_count( gotvptop_array_t * p_array )
{
    return p_array->i_count;
}

#ifndef __cplusplus
# define gotvptop_array_item_at_index(ar, idx) \
    _Generic((ar), \
        const gotvptop_array_t *: ((ar)->pp_elems[idx]), \
        gotvptop_array_t *: ((ar)->pp_elems[idx]))
#else
static inline void *gotvptop_array_item_at_index( gotvptop_array_t *ar, size_t idx )
{
    return ar->pp_elems[idx];
}

static inline const void *gotvptop_array_item_at_index( const gotvptop_array_t *ar,
                                                   size_t idx )
{
    return ar->pp_elems[idx];
}
...
static inline ssize_t gotvptop_array_index_of_item( const gotvptop_array_t *ar,
                                               const void *elem )
{
...
/* Write */
static inline int gotvptop_array_insert( gotvptop_array_t *ar, void *elem, int idx )
{
...
static inline void gotvptop_array_insert_or_abort( gotvptop_array_t *ar, void *elem, int idx )
{
    if( gotvptop_array_insert( ar, elem, idx ) )
        abort();
}

static inline int gotvptop_array_append( gotvptop_array_t *ar, void *elem )
{
    void **pp = (void **)realloc( ar->pp_elems,
                                  sizeof( void * ) * (ar->i_count + 1) );
    if( unlikely(pp == NULL) )
...
static inline void gotvptop_array_append_or_abort( gotvptop_array_t *ar, void *elem )
{
    if( gotvptop_array_append( ar, elem ) != 0 )
        abort();
}

static inline void gotvptop_array_remove( gotvptop_array_t *ar, size_t idx )
{
    void **pp = ar->pp_elems;
...
typedef struct gotvptop_dictionary_entry_t
{
    char *   psz_key;
    void *   p_value;
    struct gotvptop_dictionary_entry_t * p_next;
} gotvptop_dictionary_entry_t;

typedef struct gotvptop_dictionary_t
{
    int i_size;
    gotvptop_dictionary_entry_t ** p_entries;
} gotvptop_dictionary_t;

static void * const kGOTVDictionaryNotFound = NULL;

static inline void gotvptop_dictionary_init( gotvptop_dictionary_t * p_dict, int i_size )
{
    p_dict->p_entries = NULL;

    if( i_size > 0 )
    {
        p_dict->p_entries = (gotvptop_dictionary_entry_t **)calloc( i_size, sizeof(*p_dict->p_entries) );
        if( !p_dict->p_entries )
            i_size = 0;
    }
...
static inline void gotvptop_dictionary_clear( gotvptop_dictionary_t * p_dict,
                                         ...
                                         void * p_obj )
{
    if( p_dict->p_entries )
    {
        ...
        {
            gotvptop_dictionary_entry_t * p_current, * p_next;
...
static inline int
gotvptop_dictionary_has_key( const gotvptop_dictionary_t * p_dict, const char * psz_key )
{
    if( !p_dict->p_entries )
        return 0;

    int i_pos = (int)DictHash( psz_key, p_dict->i_size );
    const gotvptop_dictionary_entry_t * p_entry = p_dict->p_entries[i_pos];
...
static inline void *
gotvptop_dictionary_value_for_key( const gotvptop_dictionary_t * p_dict, const char * psz_key )
{
    if( !p_dict->p_entries )
        return kGOTVDictionaryNotFound;

    int i_pos =(int) DictHash( psz_key, p_dict->i_size );
    gotvptop_dictionary_entry_t * p_entry = p_dict->p_entries[i_pos];
...
static inline int
gotvptop_dictionary_keys_count( const gotvptop_dictionary_t * p_dict )
{
    gotvptop_dictionary_entry_t * p_entry;
    int i, count = 0;
...
static inline bool
gotvptop_dictionary_is_empty( const gotvptop_dictionary_t * p_dict )
{
    if( p_dict->p_entries )
        for( int i = 0; i < p_dict->i_size; i++ )
...
static inline char **
gotvptop_dictionary_all_keys( const gotvptop_dictionary_t * p_dict )
{
    gotvptop_dictionary_entry_t * p_entry;
    char ** ppsz_ret;
    int i, count = gotvptop_dictionary_keys_count( p_dict );
...
static inline void
gotvptop_dictionary_insert_impl_( gotvptop_dictionary_t * p_dict, const char * psz_key,
                             void * p_value, bool rebuild )
{
    if( !p_dict->p_entries )
        gotvptop_dictionary_init( p_dict, 1 );

    int i_pos = (int)DictHash( psz_key, p_dict->i_size );
    gotvptop_dictionary_entry_t * p_entry;

    p_entry = (gotvptop_dictionary_entry_t *)malloc(sizeof(*p_entry));
    ...
    if( rebuild )
    {
        ...
        if( count > 3 ) /* XXX: this need tuning */
        {
            ...
            struct gotvptop_dictionary_t new_dict;
            int i_new_size = ( (p_dict->i_size+2) * 3) / 2; /* XXX: this need tuning */
            int i;
            gotvptop_dictionary_init( &new_dict, i_new_size );
            for( i = 0; i < p_dict->i_size; i++ )
            {
                p_entry = p_dict->p_entries[i];
                while( p_entry )
                {
                    gotvptop_dictionary_insert_impl_( &new_dict, p_entry->psz_key,
                                             p_entry->p_value,
            ...
            gotvptop_dictionary_clear( p_dict, NULL, NULL );
...
static inline void
gotvptop_dictionary_insert( gotvptop_dictionary_t * p_dict, const char * psz_key, void * p_value )
{
    gotvptop_dictionary_insert_impl_( p_dict, psz_key, p_value, true );
}

static inline void
gotvptop_dictionary_remove_value_for_key( const gotvptop_dictionary_t * p_dict, const char * psz_key,
                                     void ( * pf_free )( void * p_data, void * p_obj ),
                                     void * p_obj )
{
    if( !p_dict->p_entries )
    ...
    int i_pos = (int)DictHash( psz_key, p_dict->i_size );
    gotvptop_dictionary_entry_t * p_entry = p_dict->p_entries[i_pos];
    gotvptop_dictionary_entry_t * p_prev;
...
// C++ helpers
template <typename T>
void gotvptop_delete_all( T &container )
{
