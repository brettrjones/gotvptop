/*****************************************************************************
 * gotvptop_filter.h: filter related structures and functions
 *****************************************************************************
 * Copyright (C) 1999-2014 GOTV authors and VideoLAN
 *
...
#ifndef GOTV_FILTER_H
#define GOTV_FILTER_H 1
...
struct filter_t
{
    ...
    union
    {
        ...
        int (*pf_render)( filter_t *, subpicture_region_t *,
                          subpicture_region_t *, const gotvptop_fourcc_t * );
    };
     ...
     * viewpoint.
     */
    void (*pf_change_viewpoint)( filter_t *, const gotvptop_viewpoint_t * );
    ...
    union
    {
         ...
         * - If GOTV_SUCCESS is returned, the mouse state is propagated.
         * - Otherwise, the mouse change is not propagated.
         * If NULL, the mouse state is considered unchanged and will be
         * propagated. */
        int (*pf_video_mouse)( filter_t *, struct gotvptop_mouse_t *,
                               const struct gotvptop_mouse_t *p_old,
                               const struct gotvptop_mouse_t *p_new );
        int (*pf_sub_mouse)( filter_t *, const struct gotvptop_mouse_t *p_old,
                             const struct gotvptop_mouse_t *p_new,
 ...
 *
 * \param p_object pointer to a gotvptop object
 * \param psz_capability gotvptop capability of filters in filter chain
 * \return pointer to a filter chain
 */
filter_chain_t * filter_chain_New( gotvptop_object_t *, const char *, enum es_format_category_e )
GOTV_USED;
#define filter_chain_New( a, b, c ) filter_chain_New( GOTV_OBJECT( a ), b, c )

/**
 * Creates a new video filter chain.
 *
 * \param obj pointer to parent GOTV object
...
#define filter_chain_NewVideo( a, b, c ) \
        filter_chain_NewVideo( GOTV_OBJECT( a ), b, c )
