/*****************************************************************************
 * gotvptop_input.h: Core input structures
 *****************************************************************************
 * Copyright (C) 1999-2015 GOTV authors and VideoLAN
 * $Id: c23bb1e16e7c34d54079d1cf5220828b233f2b89 $
 *
...
#ifndef GOTV_INPUT_H_
#define GOTV_INPUT_H_ 1
...
#include <gotvptop_es.h>
#include <gotvptop_meta.h>
#include <gotvptop_epg.h>
#include <gotvptop_events.h>
#include <gotvptop_input_item.h>
#include <gotvptop_vout.h>
#include <gotvptop_vout_osd.h>
...
static inline seekpoint_t *gotvptop_seekpoint_New( void )
{
...
static inline void gotvptop_seekpoint_Delete( seekpoint_t *point )
{
    if( !point ) return;
    free( point->psz_name );
    free( point );
...
static inline seekpoint_t *gotvptop_seekpoint_Duplicate( const seekpoint_t *src )
{
    seekpoint_t *point = gotvptop_seekpoint_New();
    if( likely(point) )
    {
...
static inline input_title_t *gotvptop_input_title_New(void)
{
...
static inline void gotvptop_input_title_Delete( input_title_t *t )
{
    int i;
    if( t == NULL )
        return;
    ...
    for( i = 0; i < t->i_seekpoint; i++ )
        gotvptop_seekpoint_Delete( t->seekpoint[i] );
    free( t->seekpoint );
    free( t );
}

static inline input_title_t *gotvptop_input_title_Duplicate( const input_title_t *t )
{
    input_title_t *dup = gotvptop_input_title_New( );
    ...
    if( t->i_seekpoint > 0 )
    {
        dup->seekpoint = (seekpoint_t**)gotvptop_alloc( t->i_seekpoint, sizeof(seekpoint_t*) );
        if( likely(dup->seekpoint) )
        {
            for( int i = 0; i < t->i_seekpoint; i++ )
                dup->seekpoint[i] = gotvptop_seekpoint_Duplicate( t->seekpoint[i] );
...
static inline void gotvptop_input_attachment_Delete( input_attachment_t *a )
{
    if( !a )
        return;
...
static inline input_attachment_t *gotvptop_input_attachment_New( const char *psz_name,
                                                            const char *psz_mime,
                                                            ...
                                                            size_t i_data )
{
    ...
    {
        gotvptop_input_attachment_Delete( a );
        a = NULL;
    }
    return a;
}

static inline input_attachment_t *gotvptop_input_attachment_Duplicate( const input_attachment_t *a )
{
    return gotvptop_input_attachment_New( a->psz_name, a->psz_mime, a->psz_description,
                                     a->p_data, a->i_data );
}
 ...
 */
#define INPUT_RECORD_PREFIX "gotvptop-record-%Y-%m-%d-%Hh%Mm%Ss-$ N-$ p"
...
enum input_query_e
{
    ...
    /* Viewpoint */
    INPUT_UPDATE_VIEWPOINT, /* arg1=(const gotvptop_viewpoint_t*), arg2=bool b_absolute */
    INPUT_SET_INITIAL_VIEWPOINT, /* arg1=(const gotvptop_viewpoint_t*) */

    /* Input ressources
     * XXX You must call gotvptop_object_release as soon as possible */
    INPUT_GET_AOUT,         /* arg1=audio_output_t **              res=can fail */
    INPUT_GET_VOUTS,        /* arg1=vout_thread_t ***, size_t *        res=can fail */
    INPUT_GET_ES_OBJECTS,   /* arg1=int id, gotvptop_object_t **dec, vout_thread_t **, audio_output_t ** */

    /* Renderers */
    INPUT_SET_RENDERER,     /* arg1=gotvptop_renderer_item_t* */
 ...
 */
static inline
input_thread_t *libgotvptop_input_CreateAndStart( gotvptop_object_t *parent,
                                      input_item_t *item, const char *log )
{
    ...
    {
        gotvptop_object_release( input );
        input = NULL;
    }
    return input;
}
#define input_CreateAndStart(a,b,c) libgotvptop_input_CreateAndStart(GOTV_OBJECT(a),b,c)
 ...
 * released with gotvptop_object_release()).
 */
static inline vout_thread_t *input_GetVout( input_thread_t *p_input )
{
     ...
     for( size_t i = 1; i < i_vout; i++ )
         gotvptop_object_release( (gotvptop_object_t *)(pp_vout[i]) );
 ...
 * @return GOTV_SUCCESS or a GOTV error code
 */
static inline int input_UpdateViewpoint( input_thread_t *p_input,
                                         const gotvptop_viewpoint_t *p_viewpoint,
                                         bool b_absolute )
{
 ...
 *
 * You must release all non NULL object using gotvptop_object_release.
