/*****************************************************************************
 * gotvptop_timestamp_helper.h : timestamp handling helpers
 *****************************************************************************
 * Copyright (C) 2014 GOTV authors and VideoLAN
...
#ifndef GOTV_TIMESTAMP_H
#define GOTV_TIMESTAMP_H 1
...
static inline timestamp_fifo_t *timestamp_FifoNew(uint32_t capacity)
{
    ...
    if (!fifo)
        return NULL;
    fifo->buffer = gotvptop_alloc(capacity, sizeof(*fifo->buffer));
    if (!fifo->buffer) {
        free(fifo);
        return NULL;
    }
