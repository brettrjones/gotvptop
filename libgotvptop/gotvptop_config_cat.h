/*****************************************************************************
 * gotvptop_config_cat.h : Definition of configuration categories
 *****************************************************************************
...
#define INTF_CONTROL_HELP N_( "Settings for GOTV's control interfaces" )
...
#define TEXT_HELP N_( \
    ...
    "want GOTV to use for text rendering (to display subtitles for example).")
*/
/* Input */
...
#define STREAM_FILTER_HELP N_( \
    ...
    "the input side of GOTV. Use with care..." )
...
#define SOUT_VOD_HELP N_( "GOTV's implementation of Video On Demand" )
...
static inline const char *config_CategoryNameGet( int i_value )
{
    ...
    while( categories_array[i].psz_name != NULL )
    {
        ...
        {
            return gotvptop_gettext(categories_array[i].psz_name);
        }
        i++;
    }
    return NULL;
...
static inline const char *config_CategoryHelpGet( int i_value )
{
    ...
    while( categories_array[i].psz_help != NULL )
    {
        if( categories_array[i].i_id == i_value )
        {
            return gotvptop_gettext(categories_array[i].psz_help);
        }
        i++;
    }
    return NULL;
