/*****************************************************************************
 * gotvptop_common.h: common definitions
 * Collection of useful common types and macros definitions
 *****************************************************************************
 * Copyright (C) 1998-2011 GOTV authors and VideoLAN
 *
...
#ifndef GOTV_COMMON_H
# define GOTV_COMMON_H 1

/*****************************************************************************
 * Required gotvptop headers
 *****************************************************************************/
...
#ifdef __GNUC__
# define GOTV_GCC_VERSION(maj,min) \
    ((__GNUC__ > (maj)) || (__GNUC__ == (maj) && __GNUC_MINOR__ >= (min)))
#else
# define GOTV_GCC_VERSION(maj,min) (0)
#endif
...
#ifdef __GNUC__
# define GOTV_DEPRECATED __attribute__((deprecated))
# if GOTV_GCC_VERSION(6,0)
#  define GOTV_DEPRECATED_ENUM __attribute__((deprecated))
# else
#  define GOTV_DEPRECATED_ENUM
# endif

# if defined( _WIN32 )
#  define GOTV_FORMAT(x,y) __attribute__ ((format(gnu_printf,x,y)))
# else
#  define GOTV_FORMAT(x,y) __attribute__ ((format(printf,x,y)))
# endif
# define GOTV_FORMAT_ARG(x) __attribute__ ((format_arg(x)))
# define GOTV_MALLOC __attribute__ ((malloc))
# define GOTV_USED __attribute__ ((warn_unused_result))

#else
# define GOTV_DEPRECATED
# define GOTV_DEPRECATED_ENUM
# define GOTV_FORMAT(x,y)
# define GOTV_FORMAT_ARG(x)
# define GOTV_MALLOC
# define GOTV_USED
#endif
...
#define gotvptop_assert_unreachable() (assert(!"unreachable"), unreachable())
...
/**
 * The gotvptop_fourcc_t type.
 *
 * See http://www.webartz.com/fourcc/ for a very detailed list.
 */
typedef uint32_t gotvptop_fourcc_t;

#ifdef WORDS_BIGENDIAN
#   define GOTV_FOURCC( a, b, c, d ) \
        ( ((uint32_t)d) | ( ((uint32_t)c) << 8 ) \
           | ( ((uint32_t)b) << 16 ) | ( ((uint32_t)a) << 24 ) )
#   define GOTV_TWOCC( a, b ) \
...
/**
 * Translate a gotvptop_fourcc into its string representation. This function
 * assumes there is enough room in psz_fourcc to store 4 characters in.
 *
 * \param fcc a gotvptop_fourcc_t
 * \param psz_fourcc string to store string representation of gotvptop_fourcc in
 */
static inline void gotvptop_fourcc_to_char( gotvptop_fourcc_t fcc, char *psz_fourcc )
{
...
/* Internal types */
typedef struct gotvptop_list_t gotvptop_list_t;
typedef struct gotvptop_object_t gotvptop_object_t;
typedef struct libgotvptop_int_t libgotvptop_int_t;
...
typedef struct gotvptop_renderer_discovery_t gotvptop_renderer_discovery_t;
typedef struct gotvptop_renderer_item_t gotvptop_renderer_item_t;
...
typedef struct gotvptop_viewpoint_t gotvptop_viewpoint_t;
...
/* Network */
typedef struct gotvptop_url_t gotvptop_url_t;
...
/* misc */
typedef struct gotvptop_meta_t    gotvptop_meta_t;
 ...
 * GOTV list structure
 */
struct gotvptop_list_t
{
    int          i_type;
    int          i_count;
    gotvptop_value_t *p_values;
};

/*****************************************************************************
 * Error values (shouldn't be exposed)
 *****************************************************************************/
#define GOTV_SUCCESS        (-0) /**< No error */
#define GOTV_EGENERIC       (-1) /**< Unspecified error */
#define GOTV_ENOMEM         (-2) /**< Not enough memory */
#define GOTV_ETIMEOUT       (-3) /**< Timeout */
#define GOTV_ENOMOD         (-4) /**< Module not found */
#define GOTV_ENOOBJ         (-5) /**< Object not found */
#define GOTV_ENOVAR         (-6) /**< Variable not found */
#define GOTV_EBADVAR        (-7) /**< Bad variable value */
#define GOTV_ENOITEM        (-8) /**< Item not found */

/*****************************************************************************
 * Variable callbacks: called when the value is modified
 *****************************************************************************/
typedef int ( * gotvptop_callback_t ) ( gotvptop_object_t *,      /* variable's object */
                                   char const *,            /* variable name */
                                   gotvptop_value_t,                 /* old value */
                                   gotvptop_value_t,                 /* new value */
                                   void * );                /* callback data */

/*****************************************************************************
 * List callbacks: called when elements are added/removed from the list
 *****************************************************************************/
typedef int ( * gotvptop_list_callback_t ) ( gotvptop_object_t *,      /* variable's object */
                                        char const *,            /* variable name */
                                        int,                  /* GOTV_VAR_* action */
                                        gotvptop_value_t *,      /* new/deleted value  */
                                        void *);                 /* callback data */
...
/* clip v in [min, max] */
#define GOTV_CLIP(v, min, max)    __MIN(__MAX((v), (min)), (max))
...
GOTV_USED
static inline uint8_t clip_uint8_gotvptop( int32_t a )
{
...
{
#if GOTV_GCC_VERSION(5,0) || defined(__clang__)
...
static inline void *gotvptop_alloc(size_t count, size_t size)
{
...
GOTV_FORMAT_ARG(2)
static inline const char *gotvptop_pgettext_aux( const char *ctx, const char *id )
{
    const char *tr = gotvptop_gettext( ctx );
    return (tr == ctx) ? id : tr;
}
 ...
 *****************************************************************************/
#include "gotvptop_messages.h"
#include "gotvptop_objects.h"
#include "gotvptop_variables.h"
#include "gotvptop_configuration.h"
