/*****************************************************************************
 * gotvptop_stream.h: Stream (between access and demux) descriptor and methods
 *****************************************************************************
 * Copyright (C) 1999-2004 GOTV authors and VideoLAN
 * $Id: ebfe692d1762cf5ff5d9ebc065f377665fe95916 $
 *
...
#ifndef GOTV_STREAM_H
#define GOTV_STREAM_H 1

#include <gotvptop_block.h>
...
/**
 * Possible commands to send to gotvptop_stream_Control() and gotvptop_stream_vaControl()
 */
enum stream_query_e
{
    /* capabilities */
    ...
    STREAM_GET_META,        /**< arg1= gotvptop_meta_t *       res=can fail */
    ...
    /* XXX only data read through gotvptop_stream_Read/Block will be recorded */
 ...
 *
 * Unlike gotvptop_stream_Read(), this function does not wait for the full requested
 ...
 * necessary. Then it stores a pointer to the buffer. Unlike gotvptop_stream_Read()
 * or gotvptop_stream_Block(), this function does not modify the stream read offset.
 *
 * \note
 ...
 * error.
 *
 * This function should be used instead of gotvptop_stream_Read() or
 * gotvptop_stream_Peek() when the caller can handle reads of any size.
 *
 ...
 * The value is initially false, and is reset to false by gotvptop_stream_Seek().
 *
...
static inline int gotvptop_stream_Control(stream_t *s, int query, ...)
{
    ...
    va_start(ap, query);
    ret = gotvptop_stream_vaControl(s, query, ap);
    va_end(ap);
    return ret;
}
 ...
 */
GOTV_USED static inline int gotvptop_stream_GetSize( stream_t *s, uint64_t *size )
{
    return gotvptop_stream_Control( s, STREAM_GET_SIZE, size );
}
...
static inline int64_t stream_Size( stream_t *s )
{
    ...
    if( gotvptop_stream_GetSize( s, &i_pos ) )
        return 0;
    if( i_pos >> 62 )
        return (int64_t)1 << 62;
    return i_pos;
...
{
    char *res;
    if( gotvptop_stream_Control( s, STREAM_GET_CONTENT_TYPE, &res ) )
        return NULL;
    return res;
}
 ...
 * by another thread in the process. This is the LibGOTV equivalent of an
 * anonymous pipe/FIFO.
 *
 * On the reader side, the normal stream functions are used,
 * e.g. gotvptop_stream_Read() and gotvptop_stream_Delete().
 *
 * The created stream object is automatically destroyed when both the reader
 * and the writer sides have been closed, with gotvptop_stream_Delete() and
 * gotvptop_stream_fifo_Close() respectively.
 *
 ...
 *
 * This is a convenience helper for gotvptop_stream_fifo_Queue().
 * \param s FIFO stream created by gotvptop_stream_fifo_New()
...
/**
 * Sets gotvptop_stream_FilterDefaultReadDir as the pf_readdir callback for this
 * stream filter.
 */
#define stream_FilterSetDefaultReadDir(stream) \
do { \
    (stream)->pf_readdir = gotvptop_stream_FilterDefaultReadDir; \
} while (0)
