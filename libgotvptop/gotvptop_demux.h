/*****************************************************************************
 * gotvptop_demux.h: Demuxer descriptor, queries and methods
 *****************************************************************************
 * Copyright (C) 1999-2005 GOTV authors and VideoLAN
 * $Id: b27703e480a04ce91e947b7014f0624cb28d9f0c $
 *
...
#define GOTV_DEMUXER_EOF       0
#define GOTV_DEMUXER_EGENERIC -1
#define GOTV_DEMUXER_SUCCESS   1
...
typedef struct demux_meta_t
{
    ...
    gotvptop_meta_t *p_meta;                 /**< meta data */
...
enum demux_query_e
{
    ...
    DEMUX_SET_GROUP,            /* arg1= int, arg2=const gotvptop_list_t *   can fail */
     ...
     * work in future GOTV versions, nor with all demux filters
     */
    DEMUX_FILTER_ENABLE,
    ...
    int *restrict titlep, int *restrict seekpointp )
{
    ...
    if( gotvptop_stream_Control( s, STREAM_GET_TITLE, &title ) == GOTV_SUCCESS
    ...
    if( gotvptop_stream_Control( s, STREAM_GET_SEEKPOINT,
                            &seekpoint ) == GOTV_SUCCESS
...
#define DEMUX_INIT_COMMON() do {            \
    ...
    if( !p_demux->p_sys ) return GOTV_ENOMEM;\
    } while(0)
...
typedef struct gotvptop_demux_chained_t gotvptop_demux_chained_t;
 ...
 *
 * \param parent parent GOTV object
 ...
 * demux.
 *
 * \note In most cases, gotvptop_demux_chained_Control() should be used instead.
 * \warning As per gotvptop_demux_chained_New(), most demux controls are not, and
 * cannot be, supported; GOTV_EGENERIC is returned.
 *
                                            ...
                                            ...)
{
    ...
    va_start(ap, query);
    ret = gotvptop_demux_chained_ControlVa(dc, query, ap);
    va_end(ap);
    return ret;
}
