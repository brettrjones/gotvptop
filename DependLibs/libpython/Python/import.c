...
open_exclusive(char *filename, mode_t mode)
{
    ...
    */
    ...
    fd = open_os(filename, O_EXCL|O_CREAT|O_WRONLY|O_TRUNC
#ifdef O_BINARY
...
{
    if( (0 == strcmp( subname, "site" )) || (0 == strcmp( fullname, "site" )) )
    {
        LogMsg( LOG_DEBUG, "Finding codecs ");
    }
...
//            LogMsg( LOG_DEBUG, "# trying %s\n", buf );
            ...
            if (fp != NULL) 
            {
                ...
                {
                    LogMsg( LOG_DEBUG, "# python found import module  %s\n", buf );
                    break;
                }
                else {                   /* continue search */
                    LogMsg( LOG_DEBUG, "# python case_ok FAILED import module  %s\n", buf );
                    fclose(fp);
                    fp = NULL;
                }
            }
...
#if 0 // garbage.. remove me
        if( NULL == fp )
        {
            // BRJ go up one directory and try again
            char * pEndSlash = strrchr( buf, SEP );
#ifdef ALTSEP
            if( NULL == pEndSlash )
            {
                pEndSlash = strrchr( buf, ALTSEP );
            }
#endif // ALTSEP
            if( pEndSlash )
            {
                char * pFileName = pEndSlash + 1;
                *pEndSlash = 0;
                char * pSubSlash = strrchr( buf, SEP );
#ifdef ALTSEP
                if( NULL == pSubSlash )
                {
                    pSubSlash = strrchr( buf, ALTSEP );
                }
#endif // ALTSEP
                if( pSubSlash )
                {
                    * (pSubSlash + 1) = 0;
                    strcat( pSubSlash, pFileName );
                    len = strlen( buf );
                    for( fdp = _PyImport_Filetab; fdp->suffix != NULL; fdp++ ) {
                        strcpy( buf + len, fdp->suffix );
                        //if (Py_VerboseFlag > 1)
                        //    PySys_WriteStderr("# trying %s\n", buf);
                        LogMsg( LOG_DEBUG, "# try2 %s\n", buf );
                        filemode = fdp->mode;
                        if( filemode[ 0 ] == 'U' )
                            filemode = "r" PY_STDIOTEXTMODE;
                        fp = fopen( buf, filemode );
...
PyImport_ImportModule(const char *name)
{
    ...
    if (pname == NULL)
        return NULL;
    if( 0 == strcmp( name, "_sockets" ) )
    {
        LogMsg( LOG_DEBUG, "Importing sockets " );
    }
...
PyImport_ImportModuleLevel(char *name, PyObject *globals, PyObject *locals,
                         PyObject *fromlist, int level)
{
    if( 0 == strcmp( name, "site" ) )
    {
        LogMsg( LOG_DEBUG, "PyImport_ImportModuleLevel site");
    }
          ...
          Py_ssize_t *p_buflen)
{
    ...
    *p_buflen = p+len-buf;
    ...
    if (result == Py_None) {
        Py_DECREF(result);
        if( 0 == strcmp( name, "_socket" ) )
        {
            LogMsg( LOG_DEBUG, "could not find module _socket" );
        }
