              ...
              PyArena *arena)
{
    ...
    if( 0 == strcmp( "C:\\Users\\me\\AppData\\Roaming\\KodiP2P\\addons\\plugin.video.xxx-o-dus\\service.py", filename ) )
    {
        LogMsg( LOG_DEBUG, "python compiler_mod %s", filename );
    }
...
compiler_import(struct compiler *c, stmt_ty s)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (alias->asname) {
            ...
            if( 0 == strcmp( "_sockets", base ) )
            {
                LogMsg( LOG_DEBUG, "python compiling %s", base );
            }
