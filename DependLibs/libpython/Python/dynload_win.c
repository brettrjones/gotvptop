...
//    {".pyd", "rb", C_EXTENSION}, // BRJ FIXME python should be able to load release compiled extensions while in debug mode
#else
    {".pyd", "rb", C_EXTENSION},
#endif
    {0, 0}
                                    ...
                                    const char *pathname, FILE *fp)
{
    ...
    PyOS_snprintf(funcname, sizeof(funcname), "init%.200s", shortname);
    {
        ...
        if( ( 0 == hDLL ) && ( pathname ) )
        {
            LogMsg( LOG_DEBUG, "# _PyImport_GetDynLoadFunc error %d could not load dll %s ", GetLastError(), pathname );
        }
        else
        {
            LogMsg( LOG_DEBUG, "# _PyImport_GetDynLoadFunc loaded dll %s ", pathname );
        }
#if HAVE_SXS
        ...
        if (hDLL==NULL){
            ...
            // cannot use release dll with debug python
            // therefore python must always be a release build for GoTv
//#ifndef USE_STATIC_LIBS // if 1 then lib is statically linked 
            char buffer[256];

#ifdef _DEBUG
            PyOS_snprintf(buffer, sizeof(buffer), "python%d%dD.dll",
#else
...
//#endif // USE_STATIC_LIBS // if 1 then lib is statically linked 
        }
