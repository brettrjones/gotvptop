...
fileio_init(PyObject *oself, PyObject *args, PyObject *kwds)
{
...
#endif
            self->fd = open_os(name, flags, 0666);
        Py_END_ALLOW_THREADS
        fd_is_own = 1;
        if (self->fd < 0) {
#ifdef MS_WINDOWS
    ...
    if (self->appending) {
        /* For consistent behavior, we explicitly seek to the
...
fileio_readinto(fileio *self, PyObject *args)
{
    ...
    if (_PyVerify_fd(self->fd)) {
        ...
        if (len > INT_MAX)
            len = INT_MAX;
        n = read_os(self->fd, pbuf.buf, (int)len);
#else
...
fileio_read(fileio *self, PyObject *args)
{
    ...
    if (_PyVerify_fd(self->fd)) {
        ...
        errno = 0;
#if defined(MS_WIN64) || defined(MS_WINDOWS)
        n = read_os(self->fd, ptr, (int)size);
#else
        n = read_os(self->fd, ptr, size);
#endif
        Py_END_ALLOW_THREADS
    } else
        n = -1;
...
fileio_write(fileio *self, PyObject *args)
{
    ...
    if (_PyVerify_fd(self->fd)) {
        ...
        if (len > INT_MAX)
            len = INT_MAX;
        n = write_os(self->fd, pbuf.buf, (int)len);
#else
        n = write_os(self->fd, pbuf.buf, len);
#endif
        Py_END_ALLOW_THREADS
    } else
        n = -1;
