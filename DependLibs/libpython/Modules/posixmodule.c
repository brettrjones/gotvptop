...
posix_read(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
    n = read_os(fd, PyString_AsString(buffer), size);
    Py_END_ALLOW_THREADS
    if (n < 0) {
        Py_DECREF(buffer);
        return posix_error();
...
posix_write(PyObject *self, PyObject *args)
{
    ...
    if (len > INT_MAX)
        len = INT_MAX;
    size = write_os(fd, pbuf.buf, (int)len);
#else
    size = write_os(fd, pbuf.buf, len);
#endif
    Py_END_ALLOW_THREADS
    PyBuffer_Release(&pbuf);
    if (size < 0)
