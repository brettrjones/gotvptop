...
def main():
    ...
    perls = find_all_on_path("perl.exe", ["C:\\Perl\\bin", "\\perl\\bin" ])
    ...
    old_cd = os.getcwd()
    try:
        print("try: Creating the makefiles 1")
        os.chdir(ssl_dir)
        # rebuild makefile when we do the role over from 32 to 64 build
        if arch == "amd64" and os.path.isfile(m32) and not os.path.isfile(makefile):
            os.unlink(m32)

        print("try: Creating the makefiles 2")
        ...
        print("try: Creating the makefiles runing make")
        # Now run make.
        if arch == "amd64":
        ...
        makeCommand = "nmake -f %s -all" % makefile
        print("Executing ssl makefiles:", makeCommand)
        print("From openssl dir",  os.getcwd())
        sys.stdout.flush()
