...
static enum TIFFReadDirEntryErr TIFFReadDirEntryArray(TIFF* tif, TIFFDirEntry* direntry, uint32_t* count, uint32_t desttypesize, void** value)
{
	...
	if ((direntry->tdir_count==0)||(typesize==0))
	{
		*value=0;
         ...
         */
	if ((uint64_t)(2147483647/typesize)<direntry->tdir_count)
		return(TIFFReadDirEntryErrSizesan);
	if ((uint64_t)(2147483647/desttypesize)<direntry->tdir_count)
		return(TIFFReadDirEntryErrSizesan);
...
{
	if ((value < 0) || (value > (int64_t) TIFF_UINT32_MAX))
		return(TIFFReadDirEntryErrRange);
	else
		return(TIFFReadDirEntryErrOk);
}
 ...
 */
#define TIFF_INT64_MAX ((int64_t)(((uint64_t) ~0) >> 1))
                   ...
                   uint64_t *nextdiroff)
{
	...
	if (!isMapped(tif)) {
		...
		if ((uint64_t)off!=tif->tif_diroff)
		{
...
TIFFFetchStripThing(TIFF* tif, TIFFDirEntry* dir, uint32_t nstrips, uint64_t** lpp)
{
	...
	if (dir->tdir_count!=(uint64_t)nstrips)
	{
		...
		if (dir->tdir_count<(uint64_t)nstrips)
		{
			...
			_TIFFmemcpy(resizeddata,data,nstrips*sizeof(uint64_t));
		_TIFFfree(data);
		data=resizeddata;
	}
	*lpp=data;
...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	newcounts = (uint64_t*) _TIFFCheckMalloc(tif, nstrips32, sizeof (uint64_t),
				"for chopped \"StripByteCounts\" array");
	newoffsets = (uint64_t*) _TIFFCheckMalloc(tif, nstrips32, sizeof (uint64_t),
