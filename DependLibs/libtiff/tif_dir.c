...
void _TIFFsetLong8Array(uint64_t** lpp, uint64_t* lp, uint32_t n)
...
_TIFFVGetField(TIFF* tif, uint32_t tag, va_list ap)
{
	...
	switch (standard_tag) {
		...
		case TIFFTAG_TILEOFFSETS:
			...
			*va_arg(ap, uint64_t**) = td->td_stripoffset;
			break;
		case TIFFTAG_STRIPBYTECOUNTS:
		case TIFFTAG_TILEBYTECOUNTS:
			_TIFFFillStriles( tif );
			*va_arg(ap, uint64_t**) = td->td_stripbytecount;
			break;
		case TIFFTAG_MATTEING:
			*va_arg(ap, uint16_t*) =
		...
		case TIFFTAG_SUBIFD:
			...
			*va_arg(ap, uint64_t**) = td->td_subifd;
			break;
		case TIFFTAG_YCBCRPOSITIONING:
