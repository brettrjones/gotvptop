	...
	if( td->td_stripbytecount[strip] > 0 )
        {
            ...
            if( tif->tif_rawdatasize <= (tmsize_t)td->td_stripbytecount[strip] )
            {
                if( !(TIFFWriteBufferSetup(tif, NULL,
                    (tmsize_t)TIFFroundup_64((uint64_t)(td->td_stripbytecount[strip] + 1), 1024))) )
                    return ((tmsize_t)(-1));
            }
	...
	if( td->td_stripbytecount[tile] > 0 )
        {
            ...
            {
                if( !(TIFFWriteBufferSetup(tif, NULL,
                    (tmsize_t)TIFFroundup_64((uint64_t)(td->td_stripbytecount[tile] + 1), 1024))) )
                    return ((tmsize_t)(-1));
            }
...
TIFFSetupStrips(TIFF* tif)
{
	...
	td->td_stripoffset = (uint64_t *)
	    _TIFFmalloc(td->td_nstrips * sizeof (uint64_t));
	td->td_stripbytecount = (uint64_t *)
	    _TIFFmalloc(td->td_nstrips * sizeof (uint64_t));
	 ...
	 */
	_TIFFmemset(td->td_stripoffset, 0, td->td_nstrips*sizeof (uint64_t));
	_TIFFmemset(td->td_stripbytecount, 0, td->td_nstrips*sizeof (uint64_t));
...
TIFFGrowStrips(TIFF* tif, uint32_t delta, const char* module)
{
	...
	new_stripoffset = (uint64_t*)_TIFFrealloc(td->td_stripoffset,
		(td->td_nstrips + delta) * sizeof (uint64_t));
	new_stripbytecount = (uint64_t*)_TIFFrealloc(td->td_stripbytecount,
		(td->td_nstrips + delta) * sizeof (uint64_t));
...
TIFFAppendToStrip(TIFF* tif, uint32_t strip, uint8_t* data, tmsize_t cc)
{
	...
	if (td->td_stripoffset[strip] == 0 || tif->tif_curoff == 0) {
            ...
            if( td->td_stripbytecount[strip] != 0 
                ...
                && td->td_stripbytecount[strip] >= (uint64_t) cc )
            {
                /* 
	...
	td->td_stripbytecount[strip] += cc;
        ...
        if( (int64_t) td->td_stripbytecount[strip] != old_byte_count )
            tif->tif_flags |= TIFF_DIRTYSTRIP;
