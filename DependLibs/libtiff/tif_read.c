...
TIFFFillStripPartial( TIFF *tif, int strip, tmsize_t read_ahead, int restart )
{
	...
	register TIFFDirectory *td = &tif->tif_dir;
        ...
        */
        ...
        if( (uint64_t) to_read > td->td_stripbytecount[strip] 
            - tif->tif_rawdataoff - tif->tif_rawdataloaded )
        {
        ...
        */
        else if( !whole_strip )
        {
                if( ((tif->tif_rawdata + tif->tif_rawdataloaded) - tif->tif_rawcp) < read_ahead 
                    && (uint64_t) tif->tif_rawdataoff+tif->tif_rawdataloaded < td->td_stripbytecount[strip] )
                {
    ...
    const char* module)
{
	...
	if (!isMapped(tif)) {
		...
		mb=ma+size;
		if (((uint64_t)ma!=td->td_stripoffset[strip])||(ma>tif->tif_size))
			n=0;
...
TIFFFillStrip(TIFF* tif, uint32_t strip)
{
	...
	{
		uint64_t bytecount = td->td_stripbytecount[strip];
		if ((int64_t)bytecount <= 0) {
#if defined(__WIN32__) && (defined(_MSC_VER) || defined(__MINGW32__))
			TIFFErrorExt(tif->tif_clientdata, module,
		...
		if (isMapped(tif) &&
			...
			if (bytecount > (uint64_t)tif->tif_size ||
			    td->td_stripoffset[strip] > (uint64_t)tif->tif_size - bytecount) {
				/*
...
TIFFReadRawTile1(TIFF* tif, uint32_t tile, void* buf, tmsize_t size, const char* module)
{
	...
	"Read error at row %lu, col %lu; got %llu bytes, expected %llu",
		...
		mb=ma+size;
		if (((uint64_t)ma!=td->td_stripoffset[tile])||(ma>tif->tif_size))
			n=0;
		else if ((mb<ma)||(mb<size)||(mb>tif->tif_size))
			n=tif->tif_size-ma;
		else
...
TIFFReadRawTile(TIFF* tif, uint32_t tile, void* buf, tmsize_t size)
{
	...
	if (size != (tmsize_t)(-1) && (uint64_t)size < bytecount64)
		bytecount64 = (uint64_t)size;
	bytecountm = (tmsize_t)bytecount64;
	if ((uint64_t)bytecountm!=bytecount64)
	{
		TIFFErrorExt(tif->tif_clientdata,module,"Integer overflow");
		return ((tmsize_t)(-1));
	}
...
TIFFFillTile(TIFF* tif, uint32_t tile)
{
	...
	if ((tif->tif_flags&TIFF_NOREADRAW)==0)
	{
		uint64_t bytecount = td->td_stripbytecount[tile];
		if ((int64_t)bytecount <= 0) {
#if defined(__WIN32__) && (defined(_MSC_VER) || defined(__MINGW32__))
			TIFFErrorExt(tif->tif_clientdata, module,
		...
		if (isMapped(tif) &&
			 ...
			 */
			if (bytecount > (uint64_t)tif->tif_size ||
			    td->td_stripoffset[tile] > (uint64_t)tif->tif_size - bytecount) {
