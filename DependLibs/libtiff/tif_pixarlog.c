...
#define  CLAMP(v) ( (v<(float)0.)   ? 0				\
	...
	if (stride == 3) {
		...
		wp += 3;
		ip += 3;
		r1 = (int32_t) CLAMP(ip[0]); wp[0] = (r1-r2) & mask; r2 = r1;
		g1 = (int32_t) CLAMP(ip[1]); wp[1] = (g1-g2) & mask; g2 = g1;
		b1 = (int32_t) CLAMP(ip[2]); wp[2] = (b1-b2) & mask; b2 = b1;
	    }
	} else if (stride == 4) {
	    r2 = wp[0] = (uint16_t) CLAMP(ip[0]);
	    g2 = wp[1] = (uint16_t) CLAMP(ip[1]);
		...
		g1 = (int32_t) CLAMP(ip[1]); wp[1] = (g1-g2) & mask; g2 = g1;
		b1 = (int32_t) CLAMP(ip[2]); wp[2] = (b1-b2) & mask; b2 = b1;
		a1 = (int32_t) CLAMP(ip[3]); wp[3] = (a1-a2) & mask; a2 = a1;
	    }
	} else {
