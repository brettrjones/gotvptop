...
bool CSceneCollisionManager::getCollisionPoint(const core::line3d<f32>& ray,
											   ...
											   core::triangle3df& outTriangle,
											   ISceneNode*& outNode,
												s32 * ps32RetTriangleIndex ) // BRJ 2010-06-02 added to return index of triangle so can get uv coords etc

{
	...
	//log_msg( LOG_DEBUG, "getCollisionPoint getTriangles\n");
	...
	for (s32 i=0; i<cnt; ++i)
	{
		...
		//log_msg( LOG_DEBUG, "getCollisionPoint getIntersectionWithLine idx %d\n", i );
		if (triangle.getIntersectionWithLine(ray.start, linevect, intersection))
		{
			...
			if (tmp < raylength && tmp2 < raylength && tmp < nearest)
			{
				...
				//log_msg( LOG_DEBUG, "getCollisionPoint getSceneNodeForTriangle idx %d\n", i );
				outNode = selector->getSceneNodeForTriangle(i);
				found = true;
				//log_msg( LOG_DEBUG, "getCollisionPoint found = true idx %d\n", i );
				if( ps32RetTriangleIndex )
				{
					* ps32RetTriangleIndex = i;
				}
			}
		}
	}

	//log_msg( LOG_DEBUG, "getCollisionPoint done found ? %d\n", found );
	_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
	return found;
}
	...
	const core::vector3df & pos3d, ICameraSceneNode* camera, bool useViewPort)
{
	...
	if (!camera)
		return core::position2d<s32>(-1000,-1000);
	//log_msg( 0, "getScreenCoordinatesFrom3DPosition:: getting viewport\n" );
...
GOTV_INLINE bool CSceneCollisionManager::getLowestRoot(f32 a, f32 b, f32 c, f32 maxR, f32* root)
{
