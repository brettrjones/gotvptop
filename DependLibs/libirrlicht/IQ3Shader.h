...
namespace quake3
{
	...
	*/
	...
	GOTV_INLINE s16 isEqual ( const core::stringc &string, u32 &pos, const c8 *list[], u16 listSize )
	{
	...
	GOTV_INLINE f32 getAsFloat ( const core::stringc &string, u32 &pos )
	{
		const char * in = string.c_str () + pos;
	...
	GOTV_INLINE core::vector3df getAsVector3df ( const core::stringc &string, u32 &pos )
	{
		core::vector3df v;
	...
	/*
		extract substrings
	*/
	GOTV_INLINE void getAsStringList ( tStringList &list, s32 max, const core::stringc &string, u32 &startPos )
	{
		list.clear ();
	...
	GOTV_INLINE bool getCullingFunction ( const core::stringc &cull )
	{
		if ( cull.size() == 0 )
			return true;
	...
	// return a z-test
	GOTV_INLINE u8 getDepthFunction ( const core::stringc &string )
	{
	...
	*/
	GOTV_INLINE static void getBlendFunc ( const core::stringc &string, SBlendFunc &blendfunc )
	{
		if ( string.size() == 0 )
			return;
	...
	GOTV_INLINE core::vector3df getMD3Normal ( u32 i, u32 j )
	{
	...
	//
	GOTV_INLINE void getModifierFunc ( SModifierFunction& fill, const core::stringc &string, u32 &pos )
	{
		if ( string.size() == 0 )
			return;
	...
	*/
	GOTV_INLINE void dumpVarGroup ( core::stringc &dest, const SVarGroup * group, s32 stack )
	{
		core::stringc buf;
		s32 i;
	...
	*/
	GOTV_INLINE core::stringc & dumpShader ( core::stringc &dest, const IShader * shader, bool entity = false )
	{
		if ( 0 == shader )
			return dest;
	...
	*/
	GOTV_INLINE void getTextures(tTexArray &textures,
