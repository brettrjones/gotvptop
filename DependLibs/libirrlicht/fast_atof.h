...
*/
GOTV_INLINE u32 strtoul10(const char* in, const char** out=0)
{
	if (!in)
	{
		if (out)
...
/** \param[in] in: The string of digits to convert. Only a leading - or +
    ...
    returned.
*/
GOTV_INLINE s32 strtol10(const char* in, const char** out=0)
{
	if (!in)
	{
		if (out)
...
/** \param[in] in The digit to convert. Only digits 0 to 9 and chars A-F,a-f
    ...
    not hex
*/
GOTV_INLINE u32 ctoul16(char in)
{
	if (in >= '0' && in <= '9')
		return in - '0';
	else if (in >= 'a' && in <= 'f')
...
/** \param[in] in: The string of digits to convert. No leading chars are
    ...
    too many digits to encode in an u32 then INT_MAX will be returned.
*/
GOTV_INLINE u32 strtoul16(const char* in, const char** out=0)
{
	if (!in)
	{
		if (out)
...
/** \param[in] in The string of digits to convert. No leading chars are
    ...
    too many digits to encode in an u32 then INT_MAX will be returned.
*/
GOTV_INLINE u32 strtoul8(const char* in, const char** out=0)
{
	if (!in)
	{
		if (out)
...
/** \param[in] in The string of digits to convert. If string starts with 0x the
    ...
    too many digits to encode in an u32 then INT_MAX will be returned.
*/
GOTV_INLINE u32 strtoul_prefix(const char* in, const char** out=0)
{
	if (!in)
	{
		if (out)
...
/** Only digits 0 to 9 are parsed.  Parsing stops at any other character,
    ...
    sequence.
*/
GOTV_INLINE f32 strtof10(const char* in, const char** out = 0)
{
	if (!in)
	{
		if (out)
...
*/
GOTV_INLINE const char* fast_atof_move(const char* in, f32& result)
{
...
*/
GOTV_INLINE float fast_atof(const char* floatAsString, const char** out=0)
{
	float ret;
	if (out)
