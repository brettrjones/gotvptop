...
namespace
{
	void OglesInitLog( int step )
	{
		log_msg( LOG_DEBUG, "COGLES1Driver step %d\n", 1);
	}
}
...
#endif // NO_EGL_AVAIL
#endif // defined(_IRR_COMPILE_WITH_ANDROID_DEVICE_)
...
GOTV_INLINE void COGLES1Driver::createGLMatrix(GLfloat gl_matrix[16], const core::matrix4& m)
{
...
GOTV_INLINE void COGLES1Driver::createGLTextureMatrix(GLfloat *o, const core::matrix4& m)
{
	o[0] = m[0];
	o[1] = m[1];
	o[2] = 0.f;
	...
	bool resetAllRenderStates)
{
	...
	for (u32 i=0; i<MaxTextureUnits; ++i)
	{
		...
		if (material.getTexture(i) && material.getTexture(i)->hasMipMaps())
			...
			glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
				(material.TextureLayer[i].BilinearFilter || material.TextureLayer[i].TrilinearFilter) ? GL_LINEAR : GL_NEAREST);
#ifndef _IRR_COMPILE_WITH_ANDROID_DEVICE_ //BRJ cause GL_INVALID_ENUM error in android
