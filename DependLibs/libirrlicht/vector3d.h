...
namespace core
{
	...
	*/
	...
	// Implementor note: GOTV_INLINE keyword needed due to template specialization for s32. Otherwise put specialization into a .cpp
	template <>
	GOTV_INLINE vector3d<s32> vector3d<s32>::operator /(s32 val) const {return core::vector3d<s32>(X/val,Y/val,Z/val);}
	template <>
	GOTV_INLINE vector3d<s32>& vector3d<s32>::operator /=(s32 val) {X/=val;Y/=val;Z/=val; return *this;}

	template <>
	GOTV_INLINE vector3d<s32> vector3d<s32>::getSphericalCoordinateAngles() const
	{
		vector3d<s32> angle;
		const f64 length = X*X + Y*Y + Z*Z;
