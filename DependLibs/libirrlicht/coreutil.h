...
GOTV_INLINE s32 isFileExtension (	const io::path& filename,
								const io::path& ext0,
								const io::path& ext1,
								const io::path& ext2)
{
...
//! search if a filename has a proper extension
GOTV_INLINE bool hasFileExtension (	const io::path& filename,
								const io::path& ext0,
								const io::path& ext1 = "",
								const io::path& ext2 = "")
{
...
GOTV_INLINE io::path& cutFilenameExtension ( io::path &dest, const io::path &source )
{
...
GOTV_INLINE io::path& getFileNameExtension ( io::path &dest, const io::path &source )
{
	s32 endPos = source.findLast ( '.' );
	if ( endPos < 0 )
		dest = "";
...
GOTV_INLINE io::path& deletePathFromFilename(io::path& filename)
{
	// delete path from filename
...
//! trim paths
GOTV_INLINE io::path& deletePathFromPath(io::path& filename, s32 pathCount)
{
	// delete path from filename
	s32 i = filename.size();
...
GOTV_INLINE s32 isInSameDirectory ( const io::path& path, const io::path& file )
{
	s32 subA = 0;
	s32 subB = 0;
	s32 pos;
...
static GOTV_INLINE void splitFilename(const io::path &name, io::path* path=0,
...
#undef isspace
#undef isupper
GOTV_INLINE s32 isdigit(s32 c) { return c >= '0' && c <= '9'; }
GOTV_INLINE s32 isspace(s32 c) { return c == ' ' || c == '\f' || c == '\n' || c == '\r' || c == '\t' || c == '\v'; }
GOTV_INLINE s32 isupper(s32 c) { return c >= 'A' && c <= 'Z'; }
