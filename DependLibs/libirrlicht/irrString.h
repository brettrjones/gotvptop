...
static GOTV_INLINE void locale_set ( eLocaleID id )
{
	locale_current = id;
}

//! Returns a character converted to lower case
static GOTV_INLINE u32 locale_lower ( u32 x )
{
...
static GOTV_INLINE u32 locale_upper ( u32 x )
{
	switch ( locale_current )
	{
		case IRR_LOCALE_GERMAN:
...
public:
	...
	*/
	...
	//------------------------------------------------------------------------------
	//! BRJ added for programming convenience 07-10-2010
	//! format string.. NOTE: be careful to not format strings greater than 1024 bytes
	void Format( const char* msg, ...)
	{
		...
		va_end(argList);
		// count string length
		while( 0 != as8Buf[ u32StrLen ] )
		{
			u32StrLen++;
		}
		// allocate memory for string
		reallocate( u32StrLen + 1 );
		// copy string
		for( u32Idx = 0; u32Idx < u32StrLen; u32Idx++ )
		{
			array[u32Idx] = as8Buf[u32Idx];
		}
		array[u32StrLen]=0;
		used = u32StrLen + 1;
	}
