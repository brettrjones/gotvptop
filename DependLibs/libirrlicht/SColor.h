...
namespace video
{
	...
	GOTV_INLINE u16 RGBA16(u32 r, u32 g, u32 b, u32 a=0xFF)
	{
		return (u16)((a & 0x80) << 8 |
			(r & 0xF8) << 7 |
			(g & 0xF8) << 2 |
	...
	//! Creates a 16 bit A1R5G5B5 color
	GOTV_INLINE u16 RGB16(u32 r, u32 g, u32 b)
	{
		return RGBA16(r,g,b);
	}
	...
	GOTV_INLINE u16 RGB16from16(u16 r, u16 g, u16 b)
	{
		return (0x8000 |
				(r & 0x1F) << 10 |
				(g & 0x1F) << 5  |
	...
	GOTV_INLINE u16 X8R8G8B8toA1R5G5B5(u32 color)
	{
		return (u16)(0x8000 |
			( color & 0x00F80000) >> 9 |
			( color & 0x0000F800) >> 6 |
	...
	GOTV_INLINE u16 A8R8G8B8toA1R5G5B5(u32 color)
	{
	...
	GOTV_INLINE u16 A8R8G8B8toR5G6B5(u32 color)
	{
	...
	GOTV_INLINE u32 A1R5G5B5toA8R8G8B8(u16 color)
	{
	...
	GOTV_INLINE u32 R5G6B5toA8R8G8B8(u16 color)
	{
		return 0xFF000000 |
			((color & 0xF800) << 8)|
			((color & 0x07E0) << 5)|
	...
	GOTV_INLINE u16 R5G6B5toA1R5G5B5(u16 color)
	{
	...
	GOTV_INLINE u16 A1R5G5B5toR5G6B5(u16 color)
	{
	...
	GOTV_INLINE u32 getAlpha(u16 color)
	{
		return ((color >> 15)&0x1);
	}
	...
	GOTV_INLINE u32 getRed(u16 color)
	{
	...
	/** Shift left by 3 to get 8 bit value. */
	GOTV_INLINE u32 getGreen(u16 color)
	{
		return ((color >> 5)&0x1F);
	}
	...
	/** Shift left by 3 to get 8 bit value. */
	GOTV_INLINE u32 getBlue(u16 color)
	{
		return (color & 0x1F);
	}
	...
	GOTV_INLINE s32 getAverage(s16 color)
	{
	...
	*/
	...
	public:
		...
		\param c2: second color to interpolate with
		\param d: value between 0.0f and 1.0f. */
		GOTV_INLINE SColorf getInterpolated_quadratic(const SColorf& c1, const SColorf& c2,
				f32 d) const
		{
			d = core::clamp(d, 0.f, 1.f);
			// this*(1-d)*(1-d) + 2 * c1 * (1-d) + c2 * d * d;
	...
	*/
	...
	private:
		GOTV_INLINE f32 toRGB1(f32 rm1, f32 rm2, f32 rh) const;

	};

	GOTV_INLINE void SColorHSL::fromRGB(const SColorf &color)
	{
	...
	GOTV_INLINE void SColorHSL::toRGB(SColorf &color) const
	{
	...
	GOTV_INLINE f32 SColorHSL::toRGB1(f32 rm1, f32 rm2, f32 rh) const
	{
		if (rh<0)
			rh += 1;
		if (rh>1)
