...
class aabbox3d
{
	public:
		...
		GOTV_INLINE bool operator==(const aabbox3d<T>& other) const { return (MinEdge == other.MinEdge && other.MaxEdge == MaxEdge);}
		//! Inequality operator
		/** \param other box to compare with.
		...
		//! added to scale bbox after mesh resize
		void setScale( const vector3d<T>& v3fScale )
		{
			MinEdge.X *= v3fScale.X;
			MinEdge.Y *= v3fScale.Y;
			MinEdge.Z *= v3fScale.Z;

			MaxEdge.X *= v3fScale.X;
			MaxEdge.Y *= v3fScale.Y;
			MaxEdge.Z *= v3fScale.Z;
		}
		//! added to scale bbox after mesh resize
		void setScaleFromCenter( const vector3d<T>& v3fScale )
		{
			const core::vector3d<T> middle = getCenter();
			MinEdge.X = ( middle.X - MinEdge.X ) * v3fScale.X;
			MinEdge.Y = ( middle.Y - MinEdge.Y ) * v3fScale.Y;
			MinEdge.Z = ( middle.Z - MinEdge.Z ) * v3fScale.Z;

			MaxEdge.X = ( MaxEdge.X - middle.X ) * v3fScale.X;
			MaxEdge.Y = ( MaxEdge.Y - middle.Y ) * v3fScale.Y;
			MaxEdge.Z = ( MaxEdge.Z - middle.Z ) * v3fScale.Z;
		}

		void getMinEdge(vector3d<T>& p) const
		{
			p.X = MinEdge.X;
			p.Y = MinEdge.Y;
			p.Z = MinEdge.Z;
		}
		void getMaxEdge(vector3d<T>& p) const
		{
			p.X = MaxEdge.X;
			p.Y = MaxEdge.Y;
			p.Z = MaxEdge.Z;
		}
		#define IrrAbs(x) ((x)<0 ? -(x) : (x))
		//! get width of box
		T getWidth( void ) const
		{
			return IrrAbs( MaxEdge.X - MinEdge.X );
		}
		//! get height of box
		T getHeight( void ) const
		{
			return IrrAbs( MaxEdge.Y - MinEdge.Y );
		}
		//! get length of box
		T getLength( void ) const
		{
			return IrrAbs( MaxEdge.Z - MinEdge.Z );
		}
