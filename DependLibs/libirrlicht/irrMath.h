...
#undef PI64
	...
	*/
	GOTV_INLINE f32 radToDeg(f32 radians)
	{
	...
	\param radians	The radians value to convert to degrees.
	*/
	GOTV_INLINE f64 radToDeg(f64 radians)
	{
	...
	*/
	GOTV_INLINE f32 degToRad(f32 degrees)
	{
	...
	\param degrees	The degrees value to convert to radians.
	*/
	GOTV_INLINE f64 degToRad(f64 degrees)
	{
	...
	template<class T>
	GOTV_INLINE const T& min_(const T& a, const T& b)
	{
		return a < b ? a : b;
	}
	...
	template<class T>
	GOTV_INLINE const T& min_(const T& a, const T& b, const T& c)
	{
	...
	template<class T>
	GOTV_INLINE const T& max_(const T& a, const T& b)
	{
		return a < b ? b : a;
	}
	...
	template<class T>
	GOTV_INLINE const T& max_(const T& a, const T& b, const T& c)
	{
	...
	template<class T>
	GOTV_INLINE T lerp(const T& a, const T& b, const f32 t)
	{
	...
	template <class T>
	GOTV_INLINE const T clamp (const T& value, const T& low, const T& high)
	{
	...
	GOTV_INLINE void swap(T1& a, T2& b)
	{
		T1 c(a);
		a = b;
		b = c;
	...
	GOTV_INLINE bool equals(const f64 a, const f64 b, const f64 tolerance = ROUNDING_ERROR_f64)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}

	//! returns if a equals b, taking possible rounding errors into account
	GOTV_INLINE bool equals(const f32 a, const f32 b, const f32 tolerance = ROUNDING_ERROR_f32)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}
#if 0
	//! returns if a equals b, not using any rounding tolerance
	GOTV_INLINE bool equals(const s32 a, const s32 b)
	{
		return (a == b);
	}

	//! returns if a equals b, not using any rounding tolerance
	GOTV_INLINE bool equals(const u32 a, const u32 b)
	{
		return (a == b);
	}
#endif
	//! returns if a equals b, taking an explicit rounding tolerance into account
	GOTV_INLINE bool equals(const s32 a, const s32 b, const s32 tolerance = ROUNDING_ERROR_S32)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}

	//! returns if a equals b, taking an explicit rounding tolerance into account
	GOTV_INLINE bool equals(const u32 a, const u32 b, const s32 tolerance = ROUNDING_ERROR_S32)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}
	...
	GOTV_INLINE bool iszero(const f64 a, const f64 tolerance = ROUNDING_ERROR_f64)
	{
		return fabs(a) <= tolerance;
	}

	//! returns if a equals zero, taking rounding errors into account
	GOTV_INLINE bool iszero(const f32 a, const f32 tolerance = ROUNDING_ERROR_f32)
	{
		return fabsf(a) <= tolerance;
	}

	//! returns if a equals not zero, taking rounding errors into account
	GOTV_INLINE bool isnotzero(const f32 a, const f32 tolerance = ROUNDING_ERROR_f32)
	{
		return fabsf(a) > tolerance;
	}

	//! returns if a equals zero, taking rounding errors into account
	GOTV_INLINE bool iszero(const s32 a, const s32 tolerance = 0)
	{
		return ( a & 0x7ffffff ) <= tolerance;
	}

	//! returns if a equals zero, taking rounding errors into account
	GOTV_INLINE bool iszero(const u32 a, const u32 tolerance = 0)
	{
		return a <= tolerance;
	}

	GOTV_INLINE s32 s32_min(s32 a, s32 b)
	{
		const s32 mask = (a - b) >> 31;
		return (a & mask) | (b & ~mask);
	}

	GOTV_INLINE s32 s32_max(s32 a, s32 b)
	{
		const s32 mask = (a - b) >> 31;
		return (b & mask) | (a & ~mask);
	}

	GOTV_INLINE s32 s32_clamp (s32 value, s32 low, s32 high)
	{
	...
	*/
	...
	#define IR(x)                           ((u32&)(x))
#else
	GOTV_INLINE u32 IR(f32 x) {inttofloat tmp; tmp.f=x; return tmp.u;}
#endif
	...
	#define FR(x)                           ((f32&)(x))
#else
	GOTV_INLINE f32 FR(u32 x) {inttofloat tmp; tmp.u=x; return tmp.f;}
	GOTV_INLINE f32 FR(s32 x) {inttofloat tmp; tmp.s=x; return tmp.f;}
#endif
...
#  warn IRRLICHT_FAST_MATH not supported.
	...
	GOTV_INLINE f32 f32_max3(const f32 a, const f32 b, const f32 c)
	{
		return a > b ? (a > c ? a : c) : (b > c ? b : c);
	}

	GOTV_INLINE f32 f32_min3(const f32 a, const f32 b, const f32 c)
	{
		return a < b ? (a < c ? a : c) : (b < c ? b : c);
	}

	GOTV_INLINE f32 fract ( f32 x )
	{
		return x - floorf ( x );
	}
