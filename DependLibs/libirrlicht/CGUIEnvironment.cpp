...
{
	io::path filename = GetAssetPathAndName( "font_lucida.bmp", eGameAssetTypeGui ).c_str();
...
IGUIFont* CGUIEnvironment::getFont(const io::path& filename)
{
	...
	if (xml)
	{
		...
		if (t==EGFT_BITMAP)
		{
			...
			// load the font
			//NOTE: add asset path.. font texture must be in same directory as the xml file
			core::stringc strAssetPath;
			strAssetPath = f.NamedPath.getPath();
			strAssetPath.replace( '\\', '/' );
			s32 lastSlash = strAssetPath.findLast('/'); 
			log_msg( 0, "font xml path %s\n", strAssetPath.c_str() );
			if( lastSlash > 0 )
			{
				strAssetPath = strAssetPath.subString(0, lastSlash + 1 );
			}
			log_msg( 0, "font path only %s\n", strAssetPath.c_str() );

			if (!font->load(xml, strAssetPath ) )
			{
				font->drop();
				font  = 0;
				ifont = 0;
