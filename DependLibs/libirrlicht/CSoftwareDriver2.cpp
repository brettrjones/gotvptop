...
GOTV_INLINE float dot (float x, float y) { return x * y; }
GOTV_INLINE float dot ( const vec2 &x, const vec2 &y) { return x.x * y.x + x.y * y.y; }
GOTV_INLINE float dot ( const vec3 &x, const vec3 &y) { return x.x * y.x + x.y * y.y + x.z * y.z; }
GOTV_INLINE float dot ( const vec4 &x, const vec4 &y) { return x.x * y.x + x.y * y.y + x.z * y.z + x.w * y.w; }

GOTV_INLINE float reflect (float I, float N)				{ return I - 2.0 * dot (N, I) * N; }
GOTV_INLINE vec2 reflect (const vec2 &I, const vec2 &N)	{ return I - N * 2.0 * dot (N, I); }
GOTV_INLINE vec3 reflect (const vec3 &I, const vec3 &N)	{ return I - N * 2.0 * dot (N, I); }
GOTV_INLINE vec4 reflect (const vec4 &I, const vec4 &N)	{ return I - N * 2.0 * dot (N, I); }


GOTV_INLINE float refract (float I, float N, float eta){
...
GOTV_INLINE vec2 refract (const vec2 &I, const vec2 &N, float eta){
    const float k = 1.0 - eta * eta * (1.0 - dot (N, I) * dot (N, I));
    if (k < 0.0)
        return vec2 (0.0);
...
GOTV_INLINE vec3 refract (const vec3 &I, const vec3 &N, float eta) {
    const float k = 1.0 - eta * eta * (1.0 - dot (N, I) * dot (N, I));
    if (k < 0.0)
        return vec3 (0.0);
    return I * eta - N * (eta * dot (N, I) + sqrt (k));
...
GOTV_INLINE vec4 refract (const vec4 &I, const vec4 &N, float eta) {
    const float k = 1.0 - eta * eta * (1.0 - dot (N, I) * dot (N, I));
    if (k < 0.0)
        return vec4 (0.0);
    return I * eta - N * (eta * dot (N, I) + sqrt (k));
...
GOTV_INLINE float length ( const vec3 &v ) { return sqrtf ( v.x * v.x + v.y * v.y + v.z * v.z ); }
...
/*!
 ...
 Part II:
	...
	replace w/w by 1/w
*/
GOTV_INLINE void CBurningVideoDriver::ndc_2_dc_and_project ( s4DVertex *dest,s4DVertex *source, u32 vIn ) const
{
	u32 g;
...
GOTV_INLINE void CBurningVideoDriver::ndc_2_dc_and_project2 ( const s4DVertex **v, const u32 size ) const
{
	u32 g;
...
*/
GOTV_INLINE f32 CBurningVideoDriver::screenarea ( const s4DVertex *v ) const
{
...
/*!
*/
GOTV_INLINE f32 CBurningVideoDriver::texelarea ( const s4DVertex *v, int tex ) const
{
	f32 z;
...
*/
GOTV_INLINE f32 CBurningVideoDriver::screenarea2 ( const s4DVertex **v ) const
{
...
/*!
*/
GOTV_INLINE f32 CBurningVideoDriver::texelarea2 ( const s4DVertex **v, s32 tex ) const
{
	f32 z;
...
/*!
*/
GOTV_INLINE void CBurningVideoDriver::select_polygon_mipmap ( s4DVertex *v, u32 vIn, u32 tex, const core::dimension2du& texSize ) const
{
	f32 f[2];
...
GOTV_INLINE void CBurningVideoDriver::select_polygon_mipmap2 ( s4DVertex **v, u32 tex, const core::dimension2du& texSize ) const
{
	f32 f[2];
