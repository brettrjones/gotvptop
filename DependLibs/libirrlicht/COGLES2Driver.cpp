...
//BRJ #include "android_native_app_glue.h"
#define NO_EGL_AVAIL // BRJ EGL is handled by android in java
#endif
#endif
...
//! constructor and init code
...
#endif
	{
		...
		//BRJ EglWindow =	((struct android_app *)(params.PrivateData))->window;
		EglWindow = NULL;
...
#endif
#else 
	CNullDriver::ScreenSize = params.WindowSize;
	CNullDriver::ViewPort = core::rect<s32>(core::position2d<s32>(0,0), core::dimension2di(params.WindowSize));
	genericDriverInit(params.WindowSize, params.Stencilbuffer);
#endif // NO_EGL_AVAIL
	}
...
//		DriverAttributes->setAttribute("MaxMultipleRenderTargets", MaxMultipleRenderTargets);
	...
	void COGLES2Driver::createMaterialRenderers()
	{
		...
		core::stringc FPVSPath = GetShaderFilesPath().c_str();
		FPVSPath += "COGLES2FixedPipeline.vsh";

		core::stringc FPFSPath = GetShaderFilesPath().c_str();
		...
		core::stringc NMVSPath = GetShaderFilesPath().c_str();
		NMVSPath += "COGLES2NormalMap.vsh";

		core::stringc NMFSPath = GetShaderFilesPath().c_str();
		...
		core::stringc PMVSPath = GetShaderFilesPath().c_str();
		PMVSPath += "COGLES2ParallaxMap.vsh";

		core::stringc PMFSPath = GetShaderFilesPath().c_str();
		...
		core::stringc R2DVSPath = GetShaderFilesPath().c_str();
		R2DVSPath += "COGLES2Renderer2D.vsh";

		core::stringc R2DFSPath = GetShaderFilesPath().c_str();
	...
	static GOTV_INLINE u8* buffer_offset(const long offset)
	{
		return ((u8*)0 + offset);
	}
...
//		glDrawElements(GL_POLYGON, primitiveCount, indexSize, indexList);
	...
	GOTV_INLINE void COGLES2Driver::createGLMatrix(float gl_matrix[16], const core::matrix4& m)
	{
	...
	GOTV_INLINE void COGLES2Driver::createGLTextureMatrix(float *o, const core::matrix4& m)
	{
		o[0] = m[0];
		o[1] = m[1];
		o[2] = 0.f;
	...
	{
#if defined(EGL_VERSION_1_0) && defined(_DEBUG) && !defined(NO_EGL_AVAIL)
		EGLint g = eglGetError();
		switch (g)
		{
			case EGL_SUCCESS:
