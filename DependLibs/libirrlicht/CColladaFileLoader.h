...
struct SColladaMaterial
{
	...
	GOTV_INLINE bool operator< (const SColladaMaterial & other) const
	{
		return Id < other.Id;
	}
};
...
struct SColladaEffect
{
	...
	GOTV_INLINE bool operator< (const SColladaEffect & other) const
	{
		return Id < other.Id;
	}
};
...
private:
	...
	GOTV_INLINE f32 readFloat(const c8** p);

	//! parses an int from a char pointer and moves the pointer to
	//! the end of the parsed float
	GOTV_INLINE s32 readInt(const c8** p);
