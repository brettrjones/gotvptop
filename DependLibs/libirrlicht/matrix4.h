...
namespace core
{
	...
	class CMatrix4
	{
		public:
			...
			GOTV_INLINE CMatrix4<T>& operator=(const CMatrix4<T> &other);

			//! Sets all elements of this matrix to the value.
			GOTV_INLINE CMatrix4<T>& operator=(const T& scalar);
			...
			/** Calculate b*a */
			GOTV_INLINE CMatrix4<T>& setbyproduct(const CMatrix4<T>& other_a,const CMatrix4<T>& other_b );
			...
			GOTV_INLINE CMatrix4<T>& makeIdentity();

			//! Returns true if the matrix is the identity matrix
			GOTV_INLINE bool isIdentity() const;

			//! Returns true if the matrix is orthogonal
			GOTV_INLINE bool isOrthogonal() const;
			...
			GOTV_INLINE CMatrix4<T>& setRotationRadians( const vector3d<T>& rotation );
			...
			GOTV_INLINE CMatrix4<T>& setInverseRotationRadians( const vector3d<T>& rotation );
			...
			//! Gets transposed matrix
			GOTV_INLINE void getTransposed( CMatrix4<T>& dest ) const;
	...
	// Default constructor
	template <class T>
	GOTV_INLINE CMatrix4<T>::CMatrix4( eConstructor constructor )
#if defined ( USE_MATRIX_TEST )
	...
	// Copy constructor
	template <class T>
	GOTV_INLINE CMatrix4<T>::CMatrix4( const CMatrix4<T>& other, eConstructor constructor)
#if defined ( USE_MATRIX_TEST )
		: definitelyIdentityMatrix(BIT_UNTESTED)
#endif
#if defined ( USE_MATRIX_TEST_DEBUG )
	...
	//! Add another matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T> CMatrix4<T>::operator+(const CMatrix4<T>& other) const
	{
	...
	//! Add another matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::operator+=(const CMatrix4<T>& other)
	{
		M[0]+=other[0];
		M[1]+=other[1];
		M[2]+=other[2];
	...
	//! Multiply by scalar.
	template <class T>
	GOTV_INLINE CMatrix4<T> CMatrix4<T>::operator*(const T& scalar) const
	{
		CMatrix4<T> temp ( EM4CONST_NOTHING );
	...
	//! Multiply by scalar.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::operator*=(const T& scalar)
	{
		M[0]*=scalar;
		M[1]*=scalar;
		M[2]*=scalar;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setbyproduct_nocheck(const CMatrix4<T>& other_a,const CMatrix4<T>& other_b )
	{
	...
	// goal is to reduce stack use and copy
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setbyproduct(const CMatrix4<T>& other_a, const CMatrix4<T>& other_b )
	{
#if defined ( USE_MATRIX_TEST )
	...
	//! multiply by another matrix
	template <class T>
	GOTV_INLINE CMatrix4<T> CMatrix4<T>::operator*(const CMatrix4<T>& m2) const
	{
#if defined ( USE_MATRIX_TEST )
		// Testing purpose..
		if ( this->isIdentity() )
	...
	template <class T>
	GOTV_INLINE vector3d<T> CMatrix4<T>::getTranslation() const
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTranslation( const vector3d<T>& translation )
	{
		M[12] = translation.X;
		M[13] = translation.Y;
		M[14] = translation.Z;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseTranslation( const vector3d<T>& translation )
	{
		M[12] = -translation.X;
		M[13] = -translation.Y;
		M[14] = -translation.Z;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setScale( const vector3d<T>& scale )
	{
		M[0] = scale.X;
		M[5] = scale.Y;
		M[10] = scale.Z;
	...
	*/
	template <class T>
	GOTV_INLINE vector3d<T> CMatrix4<T>::getScale() const
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setRotationDegrees( const vector3d<T>& rotation )
	{
		return setRotationRadians( rotation * core::DEGTORAD );
	}

	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseRotationDegrees( const vector3d<T>& rotation )
	{
		return setInverseRotationRadians( rotation * core::DEGTORAD );
	}

	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setRotationRadians( const vector3d<T>& rotation )
	{
	...
	template <class T>
	GOTV_INLINE core::vector3d<T> CMatrix4<T>::getRotationDegrees() const
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setInverseRotationRadians( const vector3d<T>& rotation )
	{
	...
	*/
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::makeIdentity()
	{
		memset(M, 0, 16*sizeof(T));
	...
	*/
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::isIdentity() const
	{
#if defined ( USE_MATRIX_TEST )
...
*/
	...
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::isOrthogonal() const
	{
	...
	*/
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::isIdentity_integer_base() const
	{
#if defined ( USE_MATRIX_TEST )
		if (definitelyIdentityMatrix)
			return true;
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::rotateVect( vector3df& vect ) const
	{
		vector3df tmp = vect;
	...
	//! An alternate transform vector method, writing into a second vector
	template <class T>
	GOTV_INLINE void CMatrix4<T>::rotateVect(core::vector3df& out, const core::vector3df& in) const
	{
	...
	//! An alternate transform vector method, writing into an array of 3 floats
	template <class T>
	GOTV_INLINE void CMatrix4<T>::rotateVect(T *out, const core::vector3df& in) const
	{
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::inverseRotateVect( vector3df& vect ) const
	{
		vector3df tmp = vect;
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformVect( vector3df& vect) const
	{
		f32 vector[3];
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformVect( vector3df& out, const vector3df& in) const
	{
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformVect(T *out, const core::vector3df &in) const
	{
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformVec3(T *out, const T * in) const
	{
	...
	//! Transforms a plane by this matrix
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformPlane( core::plane3d<f32> &plane) const
	{
		vector3df member;
	...
	//! Transforms a plane by this matrix
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformPlane( const core::plane3d<f32> &in, core::plane3d<f32> &out) const
	{
		out = in;
		transformPlane( out );
	}
	...
	//! Transforms a axis aligned bounding box
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformBox(core::aabbox3d<f32>& box) const
	{
#if defined ( USE_MATRIX_TEST )
		if (isIdentity())
			return;
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::transformBoxEx(core::aabbox3d<f32>& box) const
	{
#if defined ( USE_MATRIX_TEST )
		if (isIdentity())
			return;
	...
	//! Multiplies this matrix by a 1x4 matrix
	template <class T>
	GOTV_INLINE void CMatrix4<T>::multiplyWith1x4Matrix(T* matrix) const
	{
		/*
		0  1  2  3
		4  5  6  7
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::inverseTranslateVect( vector3df& vect ) const
	{
		vect.X = vect.X-M[12];
		vect.Y = vect.Y-M[13];
		vect.Z = vect.Z-M[14];
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::translateVect( vector3df& vect ) const
	{
		vect.X = vect.X+M[12];
		vect.Y = vect.Y+M[13];
		vect.Z = vect.Z+M[14];
	...
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::getInverse(CMatrix4<T>& out) const
	{
	...
	//! \param out: where result matrix is written to.
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::getInversePrimitive ( CMatrix4<T>& out ) const
	{
		out.M[0 ] = M[0];
		out.M[1 ] = M[4];
		out.M[2 ] = M[8];
	...
	*/
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::makeInverse()
	{
#if defined ( USE_MATRIX_TEST )
		if (definitelyIdentityMatrix)
			return true;
	...
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::operator==(const CMatrix4<T> &other) const
	{
#if defined ( USE_MATRIX_TEST )
	...
	// Builds a right-handed perspective projection matrix based on a field of view
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovRH(
...
//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
	...
	// Builds a left-handed perspective projection matrix based on a field of view
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovLH(
			f32 fieldOfViewRadians, f32 aspectRatio, f32 zNear, f32 zFar)
	{
		const f64 h = reciprocal(tan(fieldOfViewRadians*0.5));
		_IRR_DEBUG_BREAK_IF(aspectRatio==0.f); //divide by zero
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovInfinityLH(
	...
	// Builds a left-handed orthogonal projection matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoLH(
	...
	// Builds a right-handed orthogonal projection matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoRH(
			f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
	{
		_IRR_DEBUG_BREAK_IF(widthOfViewVolume==0.f); //divide by zero
		_IRR_DEBUG_BREAK_IF(heightOfViewVolume==0.f); //divide by zero
	...
	// Builds a right-handed perspective projection matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveRH(
			f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
	{
		_IRR_DEBUG_BREAK_IF(widthOfViewVolume==0.f); //divide by zero
		_IRR_DEBUG_BREAK_IF(heightOfViewVolume==0.f); //divide by zero
	...
	// Builds a left-handed perspective projection matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveLH(
			f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
	{
		_IRR_DEBUG_BREAK_IF(widthOfViewVolume==0.f); //divide by zero
		_IRR_DEBUG_BREAK_IF(heightOfViewVolume==0.f); //divide by zero
	...
	// Builds a matrix that flattens geometry into a plane.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildShadowMatrix(const core::vector3df& light, core::plane3df plane, f32 point)
	{
	...
	// Builds a left-handed look-at matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildCameraLookAtMatrixLH(
				const vector3df& position,
				const vector3df& target,
				const vector3df& upVector)
	{
	...
	// Builds a right-handed look-at matrix.
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildCameraLookAtMatrixRH(
				const vector3df& position,
				const vector3df& target,
				const vector3df& upVector)
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T> CMatrix4<T>::interpolate(const core::CMatrix4<T>& b, f32 time) const
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T> CMatrix4<T>::getTransposed() const
	{
	...
	// returns transposed matrix
	template <class T>
	GOTV_INLINE void CMatrix4<T>::getTransposed( CMatrix4<T>& o ) const
	{
		o[ 0] = M[ 0];
		o[ 1] = M[ 4];
		o[ 2] = M[ 8];
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildNDCToDCMatrix( const core::rect<s32>& viewport, f32 zScale)
	{
	 ...
	 */
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildRotateFromTo(const core::vector3df& from, const core::vector3df& to)
	{
		// unit vectors
	 ...
	 */
	template <class T>
	GOTV_INLINE void CMatrix4<T>::buildAxisAlignedBillboard(
	...
	template <class T>
	GOTV_INLINE void CMatrix4<T>::setRotationCenter(const core::vector3df& center, const core::vector3df& translation)
	{
	...
	*/
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::buildTextureTransform( f32 rotateRad,
			const core::vector2df &rotatecenter,
			const core::vector2df &translate,
			const core::vector2df &scale)
	{
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureRotationCenter( f32 rotateRad )
	{
		const f32 c = cosf(rotateRad);
		const f32 s = sinf(rotateRad);
		M[0] = (T)c;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureTranslate ( f32 x, f32 y )
	{
		M[8] = (T)x;
		M[9] = (T)y;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureTranslateTransposed ( f32 x, f32 y )
	{
		M[2] = (T)x;
		M[6] = (T)y;
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureScale ( f32 sx, f32 sy )
	{
		M[0] = (T)sx;
		M[5] = (T)sy;
#if defined ( USE_MATRIX_TEST )
	...
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setTextureScaleCenter( f32 sx, f32 sy )
	{
		M[0] = (T)sx;
		M[5] = (T)sy;
		M[8] = (T)(0.5f - 0.5f * sx);
	...
	// sets all matrix data members at once
	template <class T>
	GOTV_INLINE CMatrix4<T>& CMatrix4<T>::setM(const T* data)
	{
	...
	// sets if the matrix is definitely identity matrix
	template <class T>
	GOTV_INLINE void CMatrix4<T>::setDefinitelyIdentityMatrix( bool isDefinitelyIdentityMatrix)
	{
#if defined ( USE_MATRIX_TEST )
	...
	// gets if the matrix is definitely identity matrix
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::getDefinitelyIdentityMatrix() const
	{
#if defined ( USE_MATRIX_TEST )
	...
	//! Compare two matrices using the equal method
	template <class T>
	GOTV_INLINE bool CMatrix4<T>::equals(const core::CMatrix4<T>& other, const T tolerance) const
	{
#if defined ( USE_MATRIX_TEST )
		if (definitelyIdentityMatrix && other.definitelyIdentityMatrix)
			return true;
	...
	// Multiply by scalar.
	template <class T>
	GOTV_INLINE CMatrix4<T> operator*(const T scalar, const CMatrix4<T>& mat)
	{
		return mat*scalar;
	}
