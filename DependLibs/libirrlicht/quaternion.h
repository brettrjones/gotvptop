...
class quaternion
{
	public:
		...
		//! Assignment operator
		GOTV_INLINE quaternion& operator=(const quaternion& other);

		//! Matrix assignment operator
		GOTV_INLINE quaternion& operator=(const matrix4& other);
		...
		GOTV_INLINE f32 dotProduct(const quaternion& other) const;

		//! Sets new quaternion
		GOTV_INLINE quaternion& set(f32 x, f32 y, f32 z, f32 w);

		//! Sets new quaternion based on euler angles (radians)
		GOTV_INLINE quaternion& set(f32 x, f32 y, f32 z);

		//! Sets new quaternion based on euler angles (radians)
		GOTV_INLINE quaternion& set(const core::vector3df& vec);

		//! Sets new quaternion from other quaternion
		GOTV_INLINE quaternion& set(const core::quaternion& quat);

		//! returns if this quaternion equals the other one, taking floating point rounding errors into account
		GOTV_INLINE bool equals(const quaternion& other,
				const f32 tolerance = ROUNDING_ERROR_f32 ) const;

		//! Normalizes the quaternion
		GOTV_INLINE quaternion& normalize();
		...
		*/
		...
		//! Creates a matrix from this quaternion
		GOTV_INLINE void getMatrix_transposed( matrix4 &dest ) const;
...
GOTV_INLINE quaternion::quaternion(f32 x, f32 y, f32 z)
{
	set(x,y,z);
}
...
// Constructor which converts euler angles to a quaternion
GOTV_INLINE quaternion::quaternion(const vector3df& vec)
{
...
// Constructor which converts a matrix to a quaternion
GOTV_INLINE quaternion::quaternion(const matrix4& mat)
{
	(*this) = mat;
}
...
// equal operator
GOTV_INLINE bool quaternion::operator==(const quaternion& other) const
{
	return ((X == other.X) &&
		(Y == other.Y) &&
		(Z == other.Z) &&
...
// assignment operator
GOTV_INLINE quaternion& quaternion::operator=(const quaternion& other)
{
	X = other.X;
	Y = other.Y;
	Z = other.Z;
...
// matrix assignment operator
GOTV_INLINE quaternion& quaternion::operator=(const matrix4& m)
{
...
// multiplication operator
GOTV_INLINE quaternion quaternion::operator*(const quaternion& other) const
{
	quaternion tmp;
...
// multiplication operator
GOTV_INLINE quaternion quaternion::operator*(f32 s) const
{
	return quaternion(s*X, s*Y, s*Z, s*W);
}

// multiplication operator
GOTV_INLINE quaternion& quaternion::operator*=(f32 s)
{
	X*=s;
	Y*=s;
	Z*=s;
...
// add operator
GOTV_INLINE quaternion quaternion::operator+(const quaternion& b) const
{
...
// Creates a matrix from this quaternion
GOTV_INLINE matrix4 quaternion::getMatrix() const
{
	core::matrix4 m;
...
/*!
	Creates a matrix from this quaternion
*/
GOTV_INLINE void quaternion::getMatrix( matrix4 &dest, const core::vector3df &center ) const
{
	f32 * m = dest.pointer();
...
/*!
	...
	lookat *= m2;
*/
GOTV_INLINE void quaternion::getMatrixCenter(matrix4 &dest,
...
// Creates a matrix from this quaternion
GOTV_INLINE void quaternion::getMatrix_transposed( matrix4 &dest ) const
{
...
// Inverts this quaternion
GOTV_INLINE quaternion& quaternion::makeInverse()
{
	X = -X; Y = -Y; Z = -Z;
	return *this;
}
...
// sets new quaternion
GOTV_INLINE quaternion& quaternion::set(f32 x, f32 y, f32 z, f32 w)
{
	X = x;
	Y = y;
	Z = z;
...
GOTV_INLINE quaternion& quaternion::set(f32 x, f32 y, f32 z)
{
	f64 angle;
...
// sets new quaternion based on euler angles
GOTV_INLINE quaternion& quaternion::set(const core::vector3df& vec)
{
	return set(vec.X, vec.Y, vec.Z);
}

// sets new quaternion based on other quaternion
GOTV_INLINE quaternion& quaternion::set(const core::quaternion& quat)
{
	return (*this=quat);
}
...
//! returns if this quaternion equals the other one, taking floating point rounding errors into account
GOTV_INLINE bool quaternion::equals(const quaternion& other, const f32 tolerance) const
{
...
// normalizes the quaternion
GOTV_INLINE quaternion& quaternion::normalize()
{
...
// set this quaternion to the result of the linear interpolation between two quaternions
GOTV_INLINE quaternion& quaternion::lerp(quaternion q1, quaternion q2, f32 time)
{
...
GOTV_INLINE quaternion& quaternion::slerp(quaternion q1, quaternion q2, f32 time)
{
...
// calculates the dot product
GOTV_INLINE f32 quaternion::dotProduct(const quaternion& q2) const
{
...
//! axis must be unit length
//! angle in radians
GOTV_INLINE quaternion& quaternion::fromAngleAxis(f32 angle, const vector3df& axis)
{
...
GOTV_INLINE void quaternion::toAngleAxis(f32 &angle, core::vector3df &axis) const
{
...
GOTV_INLINE void quaternion::toEuler(vector3df& euler) const
{
	const f64 sqw = W*W;
	const f64 sqx = X*X;
	const f64 sqy = Y*Y;
...
GOTV_INLINE vector3df quaternion::operator* (const vector3df& v) const
{
...
// set quaternion to identity
GOTV_INLINE core::quaternion& quaternion::makeIdentity()
{
	W = 1.f;
	X = 0.f;
	Y = 0.f;
...
GOTV_INLINE core::quaternion& quaternion::rotationFromTo(const vector3df& from, const vector3df& to)
{
