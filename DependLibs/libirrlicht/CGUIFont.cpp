...
//! NOTE: 06-14-2010 added asset path for android
bool CGUIFont::load(io::IXMLReader* xml, core::stringc strAssetPath )
{
	if (!SpriteBank)
		return false;
	...
	while (xml->read())
	{
		if (io::EXN_ELEMENT == xml->getNodeType())
		{
			if (core::stringw(L"Texture") == xml->getNodeName())
			{
				...
				// load texture
				// BRJ added asset path
				//SpriteBank->setTexture(i, Driver->getTexture(fn));
				SpriteBank->setTexture(i, Driver->getTexture(strAssetPath + fn));
