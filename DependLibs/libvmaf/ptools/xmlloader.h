...
#include "libvmaf/ptools/opencontainers_1_8_4/include/opencontainers.h"
#include "libvmaf/ptools/opencontainers_1_8_4/include/ocreader.h"
#include "arraydisposition.h"

#include "libvmaf/ptools/opencontainers_1_8_4/include/ocval.h"
...
class XMLLoaderA {
 ...
 protected:
  ...
  std::string prependChar_;                     // When unfolding, prepend char
  bool suppressWarning_;                   // The warnings can be obnoxious
  std::string emptyString_;                     // thread safe empty

  // ///// Helper methods
    
  void syntaxError_ (const std::string& s)
  { reader_->syntaxError(s); }

  void XMLNotKeyValueWarning_ (const std::string& name, const Val& output)
  {
    if (suppressWarning_) { 
      return;
    }
    ...
    std::cerr << "... Continuing with best effort ..." << std::endl;
  }

  void XMLAttributesWithPrimitiveContentWarning_ (const std::string& tag,
						  const Val& value,
						  const Val& output)
  {
    if (suppressWarning_) { 
...
{    
  ...
  void handlePODArraysHelper_ (T, Val& output, Arr& name_attrs_content)
  {
    ...
    if (content.tag == 'a') {
      string_content = std::string(content);
    } else if (content.tag=='n' && content.length()>0 && content[0].tag=='a') {
      string_content = std::string(content[0]);
...
#define LOADPODARRAY(T) handlePODArraysHelper_(T(0),output,name_attrs_content); 
  void handlePODArrays_ (Arr& name_attrs_content, // not const so can swap
			 Val& output)             
  {
    const std::string name = name_attrs_content[0];
		      ...
		      Val& result)
  {
	...
	result.append(content);
      ...
      std::cerr << "Unknown tag '" << stag << "' .. ignoring " << std::endl;
...
if (EXPECT!=get) { string t; t=get; string get_string = (get==EOF) ? "EOF":t; \
  ...
  std::string expectUntil_ (char end_char) 
  {
    Array<char> ret(8);
    while (1) {
      int ii = getChar_();
	...
	syntaxError_("Unexpected EOF before "+std::string(cc)+" encountered");
      }
      ...
      if (c==end_char) {
	std::string name = string(ret.data(), ret.length());
	if (name.length()>1 && name[1] == '#') { // Numeric char. references
	  if (name.length()>2 && (tolower(name[2])) == 'x') { // hex number
	    ...
	    return std::string(1, char(hexme));
	  } else { // decimal number
	...
	if (escapeSeqToSpecialChar_.findValue(name, special_char)) {
	  return std::string(1,special_char);
	} else {
  ...
  std::string getElementName_ ()
  {
	...
	syntaxError_("element names can't start with '"+std::string(1, c)+"'");
      } else {
  ...
  // by either " or ' quotes.
  void getKeyValuePair_ (std::string& key, std::string& value)
  {
    // Simple name
    key = getElementName_();
    consumeWS_();
  ...
  bool expectTag_ (Arr& a)
  {
    ...
    std::string element_name = getElementName_();
  ...
  void expectBaseContent_ (std::string& content)
  {
    Array<char> ret;
    while (1) {
      int c = peekChar_();
	...
	std::string entity = expectUntil_(';'); // Handles escapes for us
	...
	content += std::string(ret.data(), ret.length());
	return;
      }
    }
  }
  ...
  void expectElement_ (Arr& element, bool already_consumed_begin_tag=false)
  {
	...
	std::string new_tag_name = new_tag[0]; // name always at front of list
...
class XMLStreamReader_ : public StreamReader {
 ...
 public:
  ...
  XMLStreamReader_ (std::istream& is) : StreamReader(is) { }
...
class StreamXMLLoader : public XMLLoaderA {
 ...
 public:
  ...
  StreamXMLLoader (std::istream& is, 
		   int options,
		   ArrayDisposition_e arr_disp = AS_LIST,
		   char prepend_char=XML_PREPEND_CHAR, 
		   bool suppress_warnings_when_not_key_value_xml=false) : 
...
// although AS_NUMERIC is less compatible, you are not going to lose
// any information.
inline void ReadValFromXMLStream (std::istream& ifs, Val& v,
				  int options = XML_STRICT_HDR | XML_LOAD_DROP_TOP_LEVEL | XML_LOAD_EVAL_CONTENT, // best option for invertibility 
				  ArrayDisposition_e arr_disp = AS_NUMERIC,
				  char prepend_char=XML_PREPEND_CHAR)
{
...
inline void ReadValFromXMLFile (const std::string& filename, Val& v,
				int options = XML_STRICT_HDR | XML_LOAD_DROP_TOP_LEVEL | XML_LOAD_EVAL_CONTENT, // best option for invertibility 
				ArrayDisposition_e arr_disp = AS_NUMERIC,
				char prepend_char=XML_PREPEND_CHAR)
{
...
inline Val ConvertFromXML (const std::string& given_xml_string) 
{
  Val return_value; // RVO
  std::istringstream stream_thing(given_xml_string, std::istringstream::in);
