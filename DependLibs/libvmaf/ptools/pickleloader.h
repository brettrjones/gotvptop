...
#include "cpickle.h"
#include "libvmaf/ptools/opencontainers_1_8_4/include/ocavltreet.h"
#include "m2pythontools.h"
#include "libvmaf/ptools/opencontainers_1_8_4/include/ocnumerictools.h"
...
class Stack {
 public:
  ...
  // Print hook
  std::ostream& print (std::ostream& os) { return os << impl_ << endl; }
...
inline void BuildScottFactoryFunction (const Val& type_object, 
				       ...
				       Val& output_result)
{
  std::cout << "SCOTT:type_object:" << type_object << std::endl;
  std::cout << "SCOTT:input:" << input << std::endl;

  std::string name = type_object(0);
  Tup&   args = type_object(1);
  std::cout << "name:" << name << " args:" << args << std::endl;
				       ...
				       Val& result)
{
  ...
  if (elements != computed_elements) {
    if (type_char=='l') { // almost certainally 32-bit vs. 64-bit issue
      ...
      std::cerr << "Miscomputed number of elements? .. continuing ..." << std::endl;
    }
  }
...
inline size_t FlattenShape (const Val& tuple)
{
  ...
  if (described_shape.tag == 'u') { // flatten!
    ...
    if (len!=1) {
      std::cerr << "Warning: saw " << len << "dimensional array:flattening" << std::endl;
    }
    for (size_t ii=0; ii<len; ii++) {
