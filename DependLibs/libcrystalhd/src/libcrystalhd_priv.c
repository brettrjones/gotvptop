...
#if 0 // BRJ cannot build until can emmulate shared memory in windows

#include <libcrystalhd/include/config_libcrystalhd.h>
#include <libcrystalhd/include/bc_dts_defs.h>
#include <stdlib.h>
#include <sys/shm.h>
#include <sys/types.h>
#include <sys/ipc.h>
    ...
    *shmem_id =shmid;
    ...
    if((shmid= shmget(shmkey, 1024, 0644|IPC_CREAT|IPC_EXCL))== -1 ) {
        if(errno==EEXIST) {
            ...
            //we got the shmid, see if any process is already attached to it
...
static void DtsCopyAppPIB(DTS_LIB_CONTEXT *Ctx, BC_DEC_OUT_BUFF *decOut, BC_DTS_PROC_OUT *pOut)
{
    ...
    if(srcPib->ppb.flags & VDEC_FLAG_PICTURE_META_DATA_PRESENT){
        sNum = (uint16_t) ( ((srcPib->ppb.picture_meta_payload & 0xFF) << 8) |
...
#endif // #if 0 // BRJ cannot build until can emmulate shared memory in windows
