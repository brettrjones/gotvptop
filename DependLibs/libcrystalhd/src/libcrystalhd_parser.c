...
//         return BC_STS_SUCCESS;
    ...
    if((iSHSize > 0) && (pSrc))
    {
		...
		// For MPEG-4 part 2 In general the demuxers give all of Video
		// Object Layer start codes in the sequence header
		// Haali for example copies the entire MPEG-4 header up to the
		// first Video Object Plane start
		// We may need to handle error checking for the MPEG4 part 2
		// streams, but for now we just copy the entire sequence header
		// and assume that demuxers do not give us any part of a VOP.
		// Otherwise the same issue will exist for MP4p2
