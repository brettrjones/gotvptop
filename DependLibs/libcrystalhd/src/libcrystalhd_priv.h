...
enum _bc_ldil_log_level{
        LDIL_ERR            = LOG_ERROR,   /* Don't disable this option */

        /* Following are allowed only in debug mode */
        LDIL_INFO             = LOG_INFO,   /* Generic informational */
        LDIL_DBG              = LOG_DEBUG,   /* First level Debug info */
};

#define LDIL_PRINTS_ON 1
#if LDIL_PRINTS_ON
# ifndef _MSC_VER
#define DebugLog_Trace(_tl, fmt, args...)    LogMsg( LOG_DEBUG, fmt, ##args);
# else
#define DebugLog_Trace   LogMsg
# endif // _MSC_VER
#else
#define DebugLog_Trace(_tl, fmt, ...)
#endif // LDIL_PRINTS_ON

#define bc_sleep_ms(x)    VxSleep(x)
