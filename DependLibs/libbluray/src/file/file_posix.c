...
#include "file.h"
#include "libbluray/src/util/macro.h"
...
{
    if (file) {
        if (close_os((int)(intptr_t)file->internal)) {
...
static int64_t _file_read(BD_FILE_H *file, uint8_t *buf, int64_t size)
{
    ...
    for (got = 0; got < (ssize_t)size; got += result) {
        result = read_os((int)(intptr_t)file->internal, buf + got, size - got);
        if (result < 0) {
            if (errno != EINTR) {
...
static int64_t _file_write(BD_FILE_H *file, const uint8_t *buf, int64_t size)
{
    ...
    for (written = 0; written < (ssize_t)size; written += result) {
        result = write_os((int)(intptr_t)file->internal, buf + written, size - written);
        if (result < 0) {
            if (errno != EINTR) {
...
static BD_FILE_H *_file_open(const char* filename, const char *cmode)
{
    ...
    if ((fd = open_os(filename, flags, mode)) < 0) {
