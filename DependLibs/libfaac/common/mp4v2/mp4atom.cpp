...
MP4Atom* MP4Atom::ReadAtom(MP4File* pFile, MP4Atom* pParentAtom)
{
	...
	VERBOSE_READ(pFile->GetVerbosity(), 
		printf("ReadAtom: type = \"%s\" data-size = "uint64_t" (0x"X64") hdr %u\n", 
		       type, dataSize, dataSize, hdrSize));

	if (pos + hdrSize + dataSize > pParentAtom->GetEnd()) {
		VERBOSE_ERROR(pFile->GetVerbosity(), 
			printf("ReadAtom: invalid atom size, extends outside parent atom - skipping to end of \"%s\" \"%s\" "uint64_t" vs "uint64_t"\n", 
			       pParentAtom->GetType(), type,
			       pos + hdrSize + dataSize, 
			       pParentAtom->GetEnd()));
		VERBOSE_READ(pFile->GetVerbosity(),
			     printf("parent %s ("uint64_t") pos "uint64_t" hdr %d data "uint64_t" sum "uint64_t"\n",
				    pParentAtom->GetType(),
				    pParentAtom->GetEnd(),
				    pos, 
				    hdrSize, 
...
void MP4Atom::Read()
{
	...
	if (ATOMID(m_type) != 0 && m_size > 1000000) {
		VERBOSE_READ(GetVerbosity(), 
			printf("Warning: %s atom size "uint64_t" is suspect\n",
				m_type, m_size));
	}
...
void MP4Atom::FinishWrite(bool use64)
{
  ...
  VERBOSE_WRITE(GetVerbosity(), 
		printf("end: type %s "uint64_t" "uint64_t" size "uint64_t"\n", m_type, 
		       m_start, m_end,
		       m_size));
	//use64 = m_pFile->Use64Bits();
	if (use64) {
