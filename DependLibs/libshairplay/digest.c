#include <libshairplay/config_libshairplay.h>
                    ...
                    char *response)
{
	...
	MD5_Init_SharePlay(&md5ctx);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)username, strlen(username));
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)":", 1);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)realm, strlen(realm));
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)":", 1);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)password, strlen(password));
	MD5_Final_SharePlay(md5buf, &md5ctx);
	digest_md5_to_hex(md5buf, md5hex);

	/* Calculate second inner MD5 hash */
	MD5_Init_SharePlay(&md5ctx);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)method, strlen(method));
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)":", 1);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)uri, strlen(uri));
	MD5_Final_SharePlay(md5buf, &md5ctx);

	/* Calculate outer MD5 hash */
	MD5_Init_SharePlay(&md5ctx);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)md5hex, sizeof(md5hex));
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)":", 1);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)nonce, strlen(nonce));
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)":", 1);
	digest_md5_to_hex(md5buf, md5hex);
	MD5_Update_SharePlay(&md5ctx, (const unsigned char *)md5hex, sizeof(md5hex));
	MD5_Final_SharePlay(md5buf, &md5ctx);
...
digest_generate_nonce(char *result, int resultlen)
{
	...
	MD5_Init_SharePlay(&md5ctx);
	MD5_Update_SharePlay(&md5ctx, (unsigned char *)&time, sizeof(time));
	MD5_Final_SharePlay(md5buf, &md5ctx);
	digest_md5_to_hex(md5buf, md5hex);
