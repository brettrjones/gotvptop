...
#include <libxml2/xmlregexp.h>
#include <libxml2/xmlautomata.h>
#include <libxml2/xmlunicode.h>
...
xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
    ...
    if (atom == NULL) {
	ERROR_XML2("add range: atom is NULL");
	return;
    }
    if (atom->type != XML_REGEXP_RANGES) {
	ERROR_XML2("add range: atom is not ranges");
	return;
    }
    if (atom->maxRanges == 0) {
	atom->maxRanges = 4;
...
xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    if (atom == NULL) {
	ERROR_XML2("atom push: atom is NULL");
	return(-1);
    }
    if (ctxt->maxAtoms == 0) {
	ctxt->maxAtoms = 4;
...
xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
    ...
    if (state == NULL) {
	ERROR_XML2("add state: state is NULL");
	return;
    }
    if (target == NULL) {
	ERROR_XML2("add state: target is NULL");
	return;
    }
    /*
...
xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from,
    ...
    if (atom == NULL) {
	ERROR_XML2("genrate transition: atom == NULL");
	return(-1);
    }
...
xmlFAParseCharProp(xmlRegParserCtxtPtr ctxt) {
	...
	ERROR_XML2("Unknown char property");
	return;
    }
    if (ctxt->atom == NULL) {
...
xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
    ...
    if (CUR != '\\') {
	ERROR_XML2("Escaped sequence: expecting \\");
	return;
    }
    NEXT;
    cur = CUR;
	...
	ERROR_XML2("Wrong escape sequence, misuse of character '\\'");
    }
}
...
xmlFAParseCharRange(xmlRegParserCtxtPtr ctxt) {
	...
	switch (cur) {
		...
		start = cur; break;
	    default:
		ERROR_XML2("Invalid escape value");
		return;
	}
	end = start;
        len = 1;
	...
	ERROR_XML2("Expecting a char range");
	return;
    }
    /*
	...
	ERROR_XML2("Expecting the end of a char range");
	return;
    }
    NEXTL(len);
    /* TODO check that the values are acceptable character ranges for XML */
    if (end < start) {
	ERROR_XML2("End of range is before start of range");
    } else {
        xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
		           XML_REGEXP_CHARVAL, start, end, NULL);
    }
...
xmlFAParseCharGroup(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == '^') {
		...
		ERROR_XML2("charClassExpr: ']' expected");
		break;
	    }
	    ctxt->neg = neg;
	    break;
...
xmlFAParseCharClass(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ']') {
	    ...
	    ERROR_XML2("xmlFAParseCharClass: ']' expected");
	}
    } else {
	xmlFAParseCharClassEsc(ctxt);
    }
...
xmlFAParseQuantifier(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ',') {
	    ...
	    if (CUR == '}')
		    ...
		    ERROR_XML2("Improper quantifier");
		}
	    }
	}
	if (CUR == '}') {
	    ...
	    ERROR_XML2("Unterminated quantifier");
	}
	if (max == 0)
	    max = min;
	if (ctxt->atom != NULL) {
...
xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ')') {
	    ...
	    ERROR_XML2("xmlFAParseAtom: expecting ')'");
	}
...
xmlFAParsePiece(xmlRegParserCtxtPtr ctxt) {
	...
	return(0);
    if (ctxt->atom == NULL) {
	ERROR_XML2("internal: no atom generated");
    }
...
xmlRegexpCompile(const xmlChar *regexp) {
    ...
    if (CUR != 0) {
	ERROR_XML2("xmlFAParseRegExp: extra characters");
    }
    if (ctxt->error != 0) {
