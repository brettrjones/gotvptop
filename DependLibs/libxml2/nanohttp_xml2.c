...
#include <libxml2/globals.h>
#include <libxml2/xmlerror.h>
#include <libxml2/xmlmemory.h>
#include <libxml2/parser.h> /* for xmlStr(n)casecmp() */
...
retry:
	...
	xmlGenericError(xmlGenericErrorContext,
		"ERROR_XML2: overflowed buffer by %d bytes\n", -blen);
#endif
...
xmlNanoHTTPFetch(const char *URL, const char *filename, char **contentType) {
    ...
    if (!strcmp(filename, "-")) 
        ...
        fd = open_os(filename, O_CREAT | O_WRONLY, 00644);
	if (fd < 0) {
...
xmlNanoHTTPSave(void *ctxt, const char *filename) {
    ...
    if (!strcmp(filename, "-")) 
        ...
        fd = open_os(filename, O_CREAT | O_WRONLY, 0666);
	if (fd < 0) {
	    xmlNanoHTTPClose(ctxt);
	    return(-1);
	}
