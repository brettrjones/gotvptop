...
#include <libcdio/config_libcdio.h>

//#include <platform/win32/my_ntddscsi.h> // for SCSI_PASS_THROUGH
...
//# include <ntddk_win32/ntddcdrm.h>
# include <winnt.h>
# include <ntddcdrm.h>
# include <ntddstor.h>
# include <ntddscsi.h>
...
{
  const _img_private_t *p_env = ( const _img_private_t *)p_user_data;
...
{
  const _img_private_t *p_env = (_img_private_t *)p_user_data;
                        ...
                        unsigned int u_buf, /*in/out*/ void *p_buf )
{
  ...
  p_sptwb = (SCSI_PASS_THROUGH_WITH_BUFFERS *)malloc(u_swb_len);

  ((_img_private_t *)p_env)->gen.scsi_mmc_sense_valid = 0;
  ...
  if (p_sptwb->Spt.ScsiStatus && p_sptwb->Spt.SenseInfoLength > 0) {
    ...
    memcpy((void *) p_env->gen.scsi_mmc_sense, &(p_sptwb->ucSenseBuf),
	   i_sense_size);
    ((_img_private_t *)p_env)->gen.scsi_mmc_sense_valid = p_sptwb->Spt.SenseInfoLength;
  }
  free(p_sptwb);
...
dvd_discmode_win32ioctl (_img_private_t *p_env)
{
  ...
  rc = (driver_return_code_t)mmc_get_dvd_struct_physical_private (p_env, (mmc_run_cmd_fn_t)&run_mmc_cmd_win32ioctl,
                                            &dvd);
