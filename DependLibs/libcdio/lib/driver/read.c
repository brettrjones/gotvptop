...
/*!
  mycd_lseek - reposition read/write file offset
...
cdio_lseek (const CdIo_t *p_cdio, off_t offset, int whence)
{
  ...
  if (p_cdio->op.mycd_lseek)
    return (p_cdio->op.mycd_lseek) (p_cdio->env, offset, whence);
...
cdio_read (const CdIo_t *p_cdio, void *p_buf, size_t i_size)
{
  ...
  if (p_cdio->op.mycd_read)
    return (p_cdio->op.mycd_read) (p_cdio->env, p_buf, i_size);
  return DRIVER_OP_UNSUPPORTED;
}
                        ...
                        bool b_form2)
{
  ...
  if (p_cdio->op.read_mode1_sector) {
    cdio_debug("Reading mode 1 sector lsn %u", i_lsn);
    return p_cdio->op.read_mode1_sector(p_cdio->env, p_buf, i_lsn, b_form2);
  } else if (p_cdio->op.mycd_lseek && p_cdio->op.mycd_read) {
