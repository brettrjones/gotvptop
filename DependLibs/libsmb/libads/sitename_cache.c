...
bool sitename_store(const char *realm, const char *sitename)
{
	...
	if (!realm || (strlen(realm) == 0)) {
		DEBUG_OUT (0,("sitename_store: no realm\n"));
		return False;
	}
	...
	if (!sitename || (sitename && !*sitename)) {
		DEBUG_OUT (5,("sitename_store: deleting empty sitename!\n"));
	...
	DEBUG_OUT (10,("sitename_store: realm = [%s], sitename = [%s], expire = [%u]\n",
...
char *sitename_fetch(const char *realm)
{
	...
	SAFE_FREE(key);
	if ( !ret ) {
		DEBUG_OUT (5,("sitename_fetch: No stored sitename for %s\n",
			query_realm));
	} else {
		DEBUG_OUT (5,("sitename_fetch: Returning sitename for %s: \"%s\"\n",
...
bool stored_sitename_changed(const char *realm, const char *sitename)
{
	...
	if (!realm || (strlen(realm) == 0)) {
		DEBUG_OUT (0,("stored_sitename_changed: no realm\n"));
		return False;
	}
