				       ...
				       const char *servername)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1, ("ads_find_printer_on_server: cannot find host %s in ads\n",
			  servername));
		return status;
	}
	...
	for (i=0; map[i].valname; i++) {
		if (StrCaseCmp(map[i].valname, regval_name(value)) == 0) {
			if (!map[i].fn(ctx, mods, value)) {
				DEBUG_OUT (5, ("Add of value %s to modlist failed\n", regval_name(value)));
			} else {
				DEBUG_OUT (7, ("Mapped value %s\n", regval_name(value)));
			}
					  ...
					  const char *printer)
{
	...
	if ((asprintf(&printername, "%s\\%s", cli->srv_name_slash, printer) == -1)) {
		DEBUG_OUT (3, ("Insufficient memory\n"));
		return WERR_NOMEM;
	}
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (3, ("Unable to open printer %s, error is %s.\n",
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (3, ("Unable to do enumdataex on %s, error is %s.\n",
			  printername, win_errstr(result)));
	} else {
