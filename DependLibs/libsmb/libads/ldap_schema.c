					  ...
					  struct posix_schema **s ) 
{
	...
	DEBUG_OUT (10,("ads_check_posix_schema_mapping for schema mode: %d\n", map_type));

	switch (map_type) {
	
		case WB_POSIX_MAP_TEMPLATE:
		case WB_POSIX_MAP_UNIXINFO:
			DEBUG_OUT (10,("ads_check_posix_schema_mapping: nothing to do\n"));
		...
		default:
			DEBUG_OUT (0,("ads_check_posix_schema_mapping: "
				 "unknown enum %d\n", map_type));
			return ADS_ERROR(LDAP_PARAM_ERROR);
	}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (3,("ads_check_posix_mapping: Unable to retrieve schema DN!\n"));
		goto done;
	}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (3,("ads_check_posix_schema_mapping: failed %s\n", 
			ads_errstr(status)));
		goto done;
	}
