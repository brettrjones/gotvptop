			   ...
			   int rc, int minor_status)
{
	...
	if (etype == ENUM_ADS_ERROR_NT) {
		DEBUG_OUT (0,("don't use ads_build_error with ENUM_ADS_ERROR_NT!\n"));
		ret.err.rc = -1;
			   ...
			   NTSTATUS nt_status)
{
	...
	if (etype != ENUM_ADS_ERROR_NT) {
		DEBUG_OUT (0,("don't use ads_build_nt_error without ENUM_ADS_ERROR_NT!\n"));
		ret.err.rc = -1;
		ret.error_type = ENUM_ADS_ERROR_SYSTEM;
		ret.minor_status = 0;
		return ret;	
...
NTSTATUS gss_err_to_ntstatus(uint32 maj, uint32 min)
{
        ...
        DEBUG_OUT (10,("gss_err_to_ntstatus: Error %s\n",
                ads_errstr(adss) ));
