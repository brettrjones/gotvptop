...
static ADS_STATUS ads_sasl_spnego_ntlmssp_bind(ADS_STRUCT *ads)
{
	...
	do {
		...
		if ((turn == 1) && 
			...
			if (!spnego_parse_challenge(talloc_tos(), blob, &blob_in, 
				...
				TALLOC_FREE(ntlmssp_state);
				data_blob_free(&blob);
				DEBUG_OUT (3,("Failed to parse challenges\n"));
	...
	if (ads->ldap.wrap_type > ADS_SASLWRAP_TYPE_PLAIN) {
		...
		if (!ADS_ERR_OK(status)) {
			DEBUG_OUT (0, ("ads_setup_sasl_wrapping() failed: %s\n",
				ads_errstr(status)));
			TALLOC_FREE(ntlmssp_state);
			return status;
		}
...
static ADS_STATUS ads_sasl_spnego_bind(ADS_STRUCT *ads)
{
	...
	for (i=0;OIDs[i];i++) {
		DEBUG_OUT (3,("ads_sasl_spnego_bind: got OID=%s\n", OIDs[i]));
#ifdef HAVE_KRB5
	...
	DEBUG_OUT (3,("ads_sasl_spnego_bind: got server principal name = %s\n", given_principal));
	    ...
	    got_kerberos_mechanism) 
	{
		...
		DEBUG_OUT (10,("ads_sasl_spnego_krb5_bind failed with: %s, "
			  "calling kinit\n", ads_errstr(status)));
		...
		if (ADS_ERR_OK(status)) {
			status = ads_sasl_spnego_krb5_bind(ads, &p);
			if (!ADS_ERR_OK(status)) {
				DEBUG_OUT (0,("kinit succeeded but "
...
static ADS_STATUS ads_sasl_gssapi_do_bind(ADS_STRUCT *ads, const gss_name_t serv_name)
{
	...
	if (!(wrap_type & ads->ldap.wrap_type)) {
		 ...
		 * type we want :-(
		 */
		DEBUG_OUT (0,("The ldap sasl wrap type doesn't match wanted[%d] server[%d]\n",
...
static ADS_STATUS ads_sasl_gssapi_bind(ADS_STRUCT *ads)
{
	...
	DEBUG_OUT (10,("ads_sasl_gssapi_do_bind failed with: %s, "
		  "calling kinit\n", ads_errstr(status)));
	...
	for (i=0;sasl_mechanisms[i].name;i++) {
		...
		for (j=0;values && values[j];j++) {
			if (strcmp(values[j], sasl_mechanisms[i].name) == 0) {
				DEBUG_OUT (4,("Found SASL mechanism %s\n", values[j]));
retry:
				...
				if (status.error_type == ENUM_ADS_ERROR_LDAP &&
				    ...
				    ads->ldap.wrap_type == ADS_SASLWRAP_TYPE_PLAIN)
				{
					DEBUG_OUT (3,("SASL bin got LDAP_STRONG_AUTH_REQUIRED "
