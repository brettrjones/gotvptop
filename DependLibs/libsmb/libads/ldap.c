 ...
 LDAP *ldap_open_with_timeout(const char *server, int port, unsigned int to)
{
	...
	DEBUG_OUT (10, ("Opening connection to LDAP server '%s:%d', timeout "
		   "%u seconds\n", server, port, to));
	...
	if (ldp == NULL) {
		DEBUG_OUT (2,("Could not open connection to LDAP server %s:%d: %s\n",
			 server, port, strerror(errno)));
	} else {
		DEBUG_OUT (10, ("Connected to LDAP server '%s:%d'\n", server, port));
	}
...
bool ads_sitename_match(ADS_STRUCT *ads)
{
	if (ads->config.server_site_name == NULL &&
		...
		DEBUG_OUT (10,("ads_sitename_match: both null\n"));
		return True;
	}
	if (ads->config.server_site_name &&
	    ...
	    strequal(ads->config.server_site_name,
		     ads->config.client_site_name)) {
		DEBUG_OUT (10,("ads_sitename_match: name %s match\n", ads->config.server_site_name));
		return True;
	}
	DEBUG_OUT (10,("ads_sitename_match: no match between server: %s and client: %s\n",
...
bool ads_closest_dc(ADS_STRUCT *ads)
{
	if (ads->config.flags & NBT_SERVER_CLOSEST) {
		DEBUG_OUT (10,("ads_closest_dc: NBT_SERVER_CLOSEST flag set\n"));
		return True;
	}

	/* not sure if this can ever happen */
	if (ads_sitename_match(ads)) {
		DEBUG_OUT (10,("ads_closest_dc: NBT_SERVER_CLOSEST flag not set but sites match\n"));
		return True;
	}

	if (ads->config.client_site_name == NULL) {
		DEBUG_OUT (10,("ads_closest_dc: client belongs to no site\n"));
		return True;
	}

	DEBUG_OUT (10,("ads_closest_dc: %s is not the closest DC\n", 
...
static bool ads_try_connect(ADS_STRUCT *ads, const char *server, bool gc)
{
	...
	if (!is_ipaddress(server)) {
		...
		if (!resolve_name(server, &ss, 0x20, true)) {
			DEBUG_OUT (5,("ads_try_connect: unable to resolve name %s\n",
				server ));
			TALLOC_FREE(frame);
			return false;
		}
	...
	DEBUG_OUT (5,("ads_try_connect: sending CLDAP request to %s (realm: %s)\n", 
		srv, ads->server.realm));

	ZERO_STRUCT( cldap_reply );

	if ( !ads_cldap_netlogon_5(frame, srv, ads->server.realm, &cldap_reply ) ) {
		DEBUG_OUT (3,("ads_try_connect: CLDAP request %s failed.\n", srv));
		ret = false;
		goto out;
	}
	...
	if ( !(cldap_reply.server_type & NBT_SERVER_LDAP) ) {
		DEBUG_OUT (1,("ads_try_connect: %s's CLDAP reply says it is not an LDAP server!\n",
			srv));
		ret = false;
		goto out;
	}
	...
	if (!interpret_string_addr(&ads->ldap.ss, srv, 0)) {
		DEBUG_OUT (1,("ads_try_connect: unable to convert %s "
			"to an address\n",
			srv));
		ret = false;
		goto out;
...
static NTSTATUS ads_find_dc(ADS_STRUCT *ads)
{
	...
	if ( !c_realm || !*c_realm ) {
		DEBUG_OUT (0,("ads_find_dc: no realm or workgroup!  Don't know what to do\n"));
	...
	if (!(ads->auth.flags & ADS_AUTH_NO_BIND)) {
		...
		DEBUG_OUT (6,("ads_find_dc: (ldap) looking for %s '%s'\n",
 ...
 again:
	...
	DEBUG_OUT (6,("ads_find_dc: (cldap) looking for %s '%s'\n",
		(got_realm ? "realm" : "domain"), realm));
	...
	if (sitename) {
		DEBUG_OUT (1,("ads_find_dc: failed to find a valid DC on our site (%s), "
...
static NTSTATUS ads_lookup_site(void)
{
	...
	if (!ADS_ERR_OK(ads_status)) {
		DEBUG_OUT (4, ("ads_lookup_site: ads_connect to our realm failed! (%s)\n",
...
ADS_STATUS ads_connect(ADS_STRUCT *ads)
{
	...
	if (DEBUGLEVEL >= 11) {
		...
		DEBUG_OUT (11,("ads_connect: entering\n"));
		DEBUGADD(11,("%s\n", s));
		TALLOC_FREE(s);
	}
...
got_connection:
	...
	DEBUG_OUT (3,("Successfully contacted LDAP server %s\n", addr));
	...
	if (!ads->auth.user_name) {
		...
		if (asprintf(&ads->auth.user_name, "%s$", global_myname() ) == -1) {
			DEBUG_OUT (0,("ads_connect: asprintf fail.\n"));
	...
	DEBUG_OUT (3,("Connected to LDAP server %s\n", ads->config.ldap_server_name));
 ...
 out:
	if (DEBUGLEVEL >= 11) {
		char *s = NDR_PRINT_STRUCT_STRING(talloc_tos(), ads_struct, ads);
		DEBUG_OUT (11,("ads_connect: leaving with: %s\n",
			ads_errstr(status)));
		DEBUGADD(11,("%s\n", s));
		TALLOC_FREE(s);
	}
...
static char **ads_pull_strvals(TALLOC_CTX *ctx, const char **in_vals)
{
	...
	for (i=0; in_vals[i]; i++) {
		if (!pull_utf8_talloc(ctx, &values[i], in_vals[i],
				      &converted_size)) {
			DEBUG_OUT (0,("ads_pull_strvals: pull_utf8_talloc failed: "
				 "%s", strerror(errno)));
		}
	}
	return values;
					   ...
					   int *count, struct berval **cookie)
{
	...
	*/
	...
	if (rc) {
		DEBUG_OUT (3,("ads_do_paged_search_args: ldap_search_with_timeout(%s) -> %s\n", expr,
			 ldap_err2string(rc)));
		goto done;
	}
...
#else
	DEBUG_OUT (0, ("no ldap_add_result_entry() support in LDAP libs!\n"));
			  ...
			  const char **attrs, LDAPMessage **res)
{
	...
	*res = NULL;
	if (!(ctx = talloc_init("ads_do_search"))) {
		DEBUG_OUT (1,("ads_do_search: talloc_init() failed!"));
		return ADS_ERROR(LDAP_NO_MEMORY);
	}
	...
	if (!push_utf8_talloc(ctx, &utf8_expr, expr, &converted_size) ||
	    !push_utf8_talloc(ctx, &utf8_path, bind_path, &converted_size))
	{
		DEBUG_OUT (1,("ads_do_search: push_utf8_talloc() failed!"));
		rc = LDAP_NO_MEMORY;
		goto done;
	}
	...
	if (!attrs || !(*attrs))
		...
		if (!(search_attrs = str_list_copy(talloc_tos(), attrs)))
		{
			DEBUG_OUT (1,("ads_do_search: str_list_copy() failed!"));
			rc = LDAP_NO_MEMORY;
			goto done;
		}
	}
	...
	if (rc == LDAP_SIZELIMIT_EXCEEDED) {
		DEBUG_OUT (3,("Warning! sizelimit exceeded in ldap. Truncating.\n"));
		rc = 0;
	}
 ...
 char *ads_get_dn(ADS_STRUCT *ads, TALLOC_CTX *mem_ctx, LDAPMessage *msg)
{
	...
	if (!utf8_dn) {
		DEBUG_OUT (5, ("ads_get_dn: ldap_get_dn failed\n"));
		return NULL;
	}

	if (!pull_utf8_talloc(mem_ctx, &unix_dn, utf8_dn, &converted_size)) {
		DEBUG_OUT (0,("ads_get_dn: string conversion failure utf8 [%s]\n",
			utf8_dn ));
		return NULL;
	}
...
ADS_STATUS ads_gen_add(ADS_STRUCT *ads, const char *new_dn, ADS_MODLIST mods)
{
	...
	if (!push_utf8_talloc(talloc_tos(), &utf8_dn, new_dn, &converted_size)) {
		DEBUG_OUT (1, ("ads_gen_add: push_utf8_talloc failed!"));
		return ADS_ERROR_NT(NT_STATUS_NO_MEMORY);
	}
...
ADS_STATUS ads_del_dn(ADS_STRUCT *ads, char *del_dn)
{
	...
	if (!push_utf8_talloc(talloc_tos(), &utf8_dn, del_dn, &converted_size)) {
		DEBUG_OUT (1, ("ads_del_dn: push_utf8_talloc failed!"));
		return ADS_ERROR_NT(NT_STATUS_NO_MEMORY);
	}
...
char *ads_default_ou_string(ADS_STRUCT *ads, const char *wknguid)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1,("Failed while searching for: %s\n", base));
		goto out;
	}
...
uint32 ads_get_kvno(ADS_STRUCT *ads, const char *account_name)
{
	...
	DEBUG_OUT (5,("ads_get_kvno: Searching for account %s\n", account_name));
	...
	if (!ADS_ERR_OK(ret) || (ads_count_replies(ads, res) != 1)) {
		DEBUG_OUT (1,("ads_get_kvno: Account for %s not found.\n", account_name));
		ads_msgfree(ads, res);
		return kvno;
	}
	...
	if (!dn_string) {
		DEBUG_OUT (0,("ads_get_kvno: out of memory.\n"));
		ads_msgfree(ads, res);
		return kvno;
	}
	DEBUG_OUT (5,("ads_get_kvno: Using: %s\n", dn_string));
	...
	if (!ads_pull_uint32(ads, res, "msDS-KeyVersionNumber", &kvno)) {
		DEBUG_OUT (3,("ads_get_kvno: Error Determining KVNO!\n"));
		DEBUG_OUT (3,("ads_get_kvno: Windows 2000 does not support KVNO's, so this may be normal.\n"));
		ads_msgfree(ads, res);
		return kvno;
	}

	/* Success */
	DEBUG_OUT (5,("ads_get_kvno: Looked Up KVNO of: %d\n", kvno));
	ads_msgfree(ads, res);
	return kvno;
}
...
ADS_STATUS ads_clear_service_principal_names(ADS_STRUCT *ads, const char *machine_name)
{
	...
	if (!ADS_ERR_OK(ret) || ads_count_replies(ads, res) != 1) {
		DEBUG_OUT (5,("ads_clear_service_principal_names: WARNING: Host Account for %s not found... skipping operation.\n", machine_name));
		DEBUG_OUT (5,("ads_clear_service_principal_names: WARNING: Service Principals for %s have NOT been cleared.\n", machine_name));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}

	DEBUG_OUT (5,("ads_clear_service_principal_names: Host account for %s found\n", machine_name));
	...
	if (!ADS_ERR_OK(ret)) {
		DEBUG_OUT (1,("ads_clear_service_principal_names: Error creating strlist.\n"));
		ads_msgfree(ads, res);
		talloc_destroy(ctx);
		return ret;
	}
	...
	TALLOC_FREE(dn_string);
	if (!ADS_ERR_OK(ret)) {
		DEBUG_OUT (1,("ads_clear_service_principal_names: Error: Updating Service Principals for machine %s in LDAP\n",
			machine_name));
		ads_msgfree(ads, res);
		talloc_destroy(ctx);
		return ret;
                                          ...
                                          const char *my_fqdn, const char *spn)
{
	...
	ret = ads_find_machine_acct(ads, &res, machine_name);
	if (!ADS_ERR_OK(ret) || ads_count_replies(ads, res) != 1) {
		DEBUG_OUT (1,("ads_add_service_principal_name: WARNING: Host Account for %s not found... skipping operation.\n",
			machine_name));
		DEBUG_OUT (1,("ads_add_service_principal_name: WARNING: Service Principal '%s/%s@%s' has NOT been added.\n",
			spn, machine_name, ads->config.realm));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}

	DEBUG_OUT (1,("ads_add_service_principal_name: Host account for %s found\n", machine_name));
	...
	DEBUG_OUT (5,("ads_add_service_principal_name: INFO: Adding %s to host %s\n", 
		psp1, machine_name));
	...
	if (!ADS_ERR_OK(ret)) {
		DEBUG_OUT (1,("ads_add_service_principal_name: Error: Updating Service Principals in LDAP\n"));
		goto out;
	}
...
static void dump_sd(ADS_STRUCT *ads, const char *filed, struct berval **values)
{
	...
	status = unmarshall_sec_desc(talloc_tos(), (uint8 *)values[0]->bv_val,
				     values[0]->bv_len, &psd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("unmarshall_sec_desc failed: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(frame);
		return;
	}
	...
	for (msg = ads_first_entry(ads, res); msg; 
		...
		for (utf8_field=ldap_first_attribute(ads->ldap.ld,
			...
			if (!pull_utf8_talloc(ctx, &field, utf8_field,
					      &converted_size))
			{
				DEBUG_OUT (0,("ads_process_results: "
			       ...
			       bool *more_strings)
{
		...
		*more_strings = True;
		...
		if (sscanf(&range_attr[strlen(expected_range_attrib)], "%lu-*", 
			...
			*more_strings = False;
			...
			DEBUG_OUT (1, ("ads_pull_strings_range:  Cannot parse Range attriubte (%s)\n", 
				  range_attr));
	...
	if ((*num_strings) != range_start) {
		DEBUG_OUT (1, ("ads_pull_strings_range: Range attribute (%s) doesn't start at %u, but at %lu"
	...
	if (*more_strings && ((*num_strings + num_new_strings) != (range_end + 1))) {
		DEBUG_OUT (1, ("ads_pull_strings_range: Range attribute (%s) tells us we have %lu "
	...
	if (*more_strings) {
		*next_attribute = talloc_asprintf(mem_ctx,
		...
		if (!*next_attribute) {
			DEBUG_OUT (1, ("talloc_asprintf for next attribute failed!\n"));
			ldap_memfree(range_attr);
			*more_strings = False;
			return NULL;
		}
...
ADS_STATUS ads_current_time(ADS_STRUCT *ads)
{
	...
	if (ads->config.current_time != 0) {
		...
		DEBUG_OUT (4,("time offset is %d seconds\n", ads->auth.time_offset));
	}
...
ADS_STATUS ads_domain_func_level(ADS_STRUCT *ads, uint32 *val)
{
	...
	*val = DS_DOMAIN_FUNCTION_2000;
	...
	if ( !ads_pull_uint32(ads_s, res, "domainFunctionality", val) ) {
		DEBUG_OUT (5,("ads_domain_func_level: Failed to pull the domainFunctionality attribute.\n"));
	}
	DEBUG_OUT (3,("ads_domain_func_level: %d\n", *val));
					...
					struct dom_sid *sid)
{
	...
	if (strncmp(p, ";<SID=", strlen(";<SID=")) != 0) {
		DEBUG_OUT (5,("No SID present in extended dn\n"));
	...
	*q = '\0';
	...
	DEBUG_OUT (100,("ads_get_sid_from_extended_dn: sid string is %s\n", p));
	...
	case ADS_EXTENDED_DN_HEX_STRING: {
		...
		if (!sid_parse(buf, buf_len, sid)) {
			DEBUG_OUT (10,("failed to parse sid\n"));
			return ADS_ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
		break;
		}
	default:
		DEBUG_OUT (10,("unknown extended dn format\n"));
		return ADS_ERROR_NT(NT_STATUS_INVALID_PARAMETER);
	}
...
char* ads_get_dnshostname( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (0,("ads_get_dnshostname: Failed to find account for %s\n",
			global_myname()));
		goto out;
	}

	if ( (count = ads_count_replies(ads, res)) != 1 ) {
		DEBUG_OUT (1,("ads_get_dnshostname: %d entries returned!\n", count));
		goto out;
	}

	if ( (name = ads_pull_string(ads, ctx, res, "dNSHostName")) == NULL ) {
		DEBUG_OUT (0,("ads_get_dnshostname: No dNSHostName attribute!\n"));
	}
...
char* ads_get_upn( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (0,("ads_get_upn: Failed to find account for %s\n",
			global_myname()));
		goto out;
	}

	if ( (count = ads_count_replies(ads, res)) != 1 ) {
		DEBUG_OUT (1,("ads_get_upn: %d entries returned!\n", count));
		goto out;
	}

	if ( (name = ads_pull_string(ads, ctx, res, "userPrincipalName")) == NULL ) {
		DEBUG_OUT (2,("ads_get_upn: No userPrincipalName attribute!\n"));
	}
...
char* ads_get_samaccountname( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	if ( (name = ads_pull_string(ads, ctx, res, "sAMAccountName")) == NULL ) {
		DEBUG_OUT (0,("ads_get_dnshostname: No sAMAccountName attribute!\n"));
	}
			...
			uint32 account_type, const char *org_unit)
{
	...
	if (ADS_ERR_OK(status) && ads_count_replies(ads, res) == 1) {
		DEBUG_OUT (0, ("Host account for %s already exists - deleting old account\n", machine));
		status = ads_leave_realm(ads, machine);
		if (!ADS_ERR_OK(status)) {
			DEBUG_OUT (0, ("Failed to delete host '%s' from the '%s' realm.\n",
	...
	*/
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (0, ("ads_join_realm: ads_add_machine_acct failed (%s): %s\n", machine, ads_errstr(status)));
		SAFE_FREE(machine);
		return status;
	}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (0, ("ads_join_realm: Host account test failed for machine %s\n", machine));
		SAFE_FREE(machine);
		return status;
	}
...
ADS_STATUS ads_leave_realm(ADS_STRUCT *ads, const char *hostname)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (0, ("Host account for %s does not exist.\n", host));
		SAFE_FREE(host);
		return status;
	}
	...
	if (rc) {
		DEBUG_OUT (3,("ldap_delete_ext_s failed with error code %d\n", rc));
	}else {
		DEBUG_OUT (3,("ldap_delete_ext_s succeeded with error code %d\n", rc));
	}
	...
	if (rc != LDAP_SUCCESS) {
		...
		for (msg_sub = ads_first_entry(ads, res); msg_sub;
			...
			if (!ADS_ERR_OK(status)) {
				DEBUG_OUT (3,("failed to delete dn %s: %s\n", dn, ads_errstr(status)));
				SAFE_FREE(host);
				TALLOC_FREE(dn);
				TALLOC_FREE(hostnameDN);
				return status;
	...
	status = ads_find_machine_acct(ads, &res, host);
	if (ADS_ERR_OK(status) && ads_count_replies(ads, res) == 1) {
		DEBUG_OUT (3, ("Failed to remove host account.\n"));
		SAFE_FREE(host);
		return status;
	}
			      ...
			      size_t *num_sids)
{
	...
	DEBUG_OUT (10,("ads_get_tokensids: returned %d sids\n", (int)tmp_num_sids + 2));
