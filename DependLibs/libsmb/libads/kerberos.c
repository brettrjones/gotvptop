								  ...
								  NTSTATUS *nt_status)
{
	...
	if (ret) {
		DEBUG_OUT (1,("krb5_get_init_creds_opt_get_error gave: %s\n", 
			error_message(ret)));
		return False;
	}
...
#endif /* HAVE_E_DATA_POINTER_IN_KRB5_ERROR */
		DEBUG_OUT (1,("no edata in krb5_error\n")); 
				...
				NTSTATUS *ntstatus)
{
	...
	DEBUG_OUT (10,("kerberos_kinit_password: as %s using [%s] as ccache and config [%s]\n",
			principal,
...
int ads_kdestroy(const char *cc_name)
{
	...
	initialize_krb5_error_table();
	if ((code = krb5_init_context (&ctx))) {
		DEBUG_OUT (3, ("ads_kdestroy: kdb5_init_context failed: %s\n", 
			error_message(code)));
		return code;
	}
	...
	if (!cc_name) {
		...
		if ((code = krb5_cc_resolve(ctx, cc_name, &cc))) {
			DEBUG_OUT (3, ("ads_kdestroy: krb5_cc_resolve failed: %s\n",
				  error_message(code)));
			krb5_free_context(ctx);
			return code;
		}
	...
	if ((code = krb5_cc_destroy (ctx, cc))) {
		DEBUG_OUT (3, ("ads_kdestroy: krb5_cc_destroy failed: %s\n", 
			error_message(code)));
	}
...
bool kerberos_secrets_store_des_salt( const char* salt )
{
	...
	if ( (key = des_salt_key()) == NULL ) {
		DEBUG_OUT (0,("kerberos_secrets_store_des_salt: failed to generate key!\n"));
		return False;
	}

	if ( !salt ) {
		DEBUG_OUT (8,("kerberos_secrets_store_des_salt: deleting salt\n"));
		secrets_delete( key );
		return True;
	}

	DEBUG_OUT (3,("kerberos_secrets_store_des_salt: Storing salt \"%s\"\n", salt));
...
char* kerberos_secrets_fetch_des_salt( void )
{
	...
	if ( (key = des_salt_key()) == NULL ) {
		DEBUG_OUT (0,("kerberos_secrets_fetch_des_salt: failed to generate key!\n"));
		return False;
	}
...
char *kerberos_get_default_realm_from_ccache( void )
{
	...
	DEBUG_OUT (5,("kerberos_get_default_realm_from_ccache: "
		"Trying to read krb5 cache: %s\n",
		krb5_cc_default_name(ctx)));
	if (krb5_cc_default(ctx, &cc)) {
		DEBUG_OUT (0,("kerberos_get_default_realm_from_ccache: "
...
char *kerberos_get_realm_from_hostname(const char *hostname)
{
	...
	if (kerr != 0) {
		DEBUG_OUT (3,("kerberos_get_realm_from_hostname %s: "
			"failed %s\n",
					      ...
					      const char *principal)
{
	...
	if (((code = krb5_init_context(&context)) != 0) || (context == NULL)) {
		DEBUG_OUT (5, ("kerberos_secrets_store_salting_pricipal: kdb5_init_context failed: %s\n",
			  error_message(code)));
		return False;
	}
	if (strchr_m(service, '@')) {
			...
			const char *kdc_name)
{
	...
	if (pss->ss_family == AF_INET) {
		...
		DEBUG_OUT (10,("print_kdc_line: IPv6 case for kdc_name: %s, port: %d\n",
			kdc_name, port));
		...
		if (port != 0 && port != DEFAULT_KRB5_PORT) {
			...
			int ret = sys_getnameinfo((const struct sockaddr *)pss,
					...
					NI_NAMEREQD);
			if (ret) {
				DEBUG_OUT (0,("print_kdc_line: can't resolve name "
		...
		const char *kdc_name)
{
	...
	DEBUG_OUT (10,("get_kdc_ip_string: Returning %s\n",
		kdc_str ));
						...
						const char *kdc_name)
{
	...
	if ((mkdir(dname, 0755)==-1) && (errno != EEXIST)) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: "
	...
	DEBUG_OUT (10,("create_local_private_krb5_conf_for_domain: fname = %s, realm = %s, domain = %s\n",
		fname, realm, domain ));
	...
	fd = mkstemp(tmpname);
	if (fd == -1) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: smb_mkstemp failed,"
			" for file %s. Errno %s\n",
	...
	if (fchmod(fd, 0644)==-1) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: fchmod failed for %s."
			" Errno %s\n",
			tmpname, strerror(errno) ));
		unlink(tmpname);
		close(fd);
	...
	if (flen != ret) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: write failed,"
	...
	if (close(fd)==-1) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: close failed."
			" Errno %s\n", strerror(errno) ));
		unlink(tmpname);
		goto done;
	}
	...
	if (rename(tmpname, fname) == -1) {
		DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: rename "
	...
	DEBUG_OUT (5,("create_local_private_krb5_conf_for_domain: wrote "
...
#define SYSTEM_KRB5_CONF_PATH "/etc/krb5.conf"
	...
	if (strequal(realm, lp_realm())) {
		...
		if (symlink(fname, SYSTEM_KRB5_CONF_PATH) == -1) {
			...
			if (errno != EEXIST) {
				DEBUG_OUT (0,("create_local_private_krb5_conf_for_domain: symlink "
					"of %s to %s failed. Errno %s\n",
