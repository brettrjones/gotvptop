					   ...
					   krb5_data *packet)
{
	...
	if (ret) {
		DEBUG_OUT (1,("krb5_mk_priv failed (%s)\n", error_message(ret)));
		return ret;
	}
	...
	*/
	...
	if (use_tcp) {
		...
		if (RIVAL(p, 0) != msg_length) {
			DEBUG_OUT (1,("Bad TCP packet length (%d/%d) from kpasswd server\n",
	...
	if (RSVAL(p, 0) != msg_length) {
		DEBUG_OUT (1,("Bad packet length (%d/%d) from kpasswd server\n",
	...
	if (vnum != KRB5_KPASSWD_VERS_SETPW && 
		...
		DEBUG_OUT (1,("Bad vnum (%d) from kpasswd server\n", vnum));
	...
	if (p + ap_rep.length >= (char *)packet->data + packet->length) {
		DEBUG_OUT (1,("ptr beyond end of packet from kpasswd server\n"));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	
	if (ap_rep.length == 0) {
		DEBUG_OUT (1,("got unencrypted setpw result?!\n"));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	if (ret) {
		DEBUG_OUT (1,("failed to rd setpw reply (%s)\n", error_message(ret)));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	ret = krb5_rd_priv(context, auth_context, &cipherresult, &clearresult,
			   &replay);
	if (ret) {
		DEBUG_OUT (1,("failed to decrypt setpw reply (%s)\n", error_message(ret)));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	if (res_code == KRB5_KPASSWD_SUCCESS) {
		...
		DEBUG_OUT (1, ("Error changing password: %s (%d)\n", errstr, res_code));
					  ...
					  const char *newpw)
{
	...
	ret = krb5_mk_req_extended(context, &auth_context, AP_OPTS_USE_SUBKEY,
				   NULL, credsp, &ap_req);
	if (ret) {
		DEBUG_OUT (1,("krb5_mk_req_extended failed (%s)\n", error_message(ret)));
	...
	do {
		...
		if (!use_tcp) {
			...
			if (sock == -1) {
				...
				DEBUG_OUT (1,("failed to open kpasswd socket to %s "
		...
		if (getpeername(sock, (struct sockaddr *)&remote_addr, &addr_len) != 0) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("getpeername() failed (%s)\n", error_message(errno)));
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("build_setpw_request failed (%s)\n", error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
		...
		if (ret != chpw_req.length) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("send of chpw failed (%s)\n", strerror(errno)));
			return ADS_ERROR_SYSTEM(errno);
		}
		...
		if (ret < 0) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("recv of chpw reply failed (%s)\n", strerror(errno)));
			return ADS_ERROR_SYSTEM(errno);
		}
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("krb5_auth_con_setaddrs on reply failed (%s)\n", 
				 error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
		...
		if (ret) {
			...
			if (ret == KRB5KRB_ERR_RESPONSE_TOO_BIG && !use_tcp) {
				DEBUG_OUT (5, ("Trying setpw with TCP!!!\n"));
				use_tcp = True;
				continue;
			}
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG_OUT (1,("parse_setpw_reply failed (%s)\n", 
				 error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
				 ...
				 const char *newpw, int time_offset)
{
	...
	if (ret) {
		DEBUG_OUT (1,("Failed to init krb5 context (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		...
		DEBUG_OUT (1,("Failed to get default creds (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		krb5_cc_close(context, ccache);
                krb5_free_context(context);
		DEBUG_OUT (1,("Failed to get principal from ccache (%s)\n",
			 error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		krb5_cc_close(context, ccache);
	        ...
	        krb5_free_context(context);
		DEBUG_OUT (1,("krb5_build_prinipal_ext (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		krb5_cc_close(context, ccache);
	        ...
	        krb5_free_principal(context, creds.server);
	        krb5_free_context(context);
		DEBUG_OUT (1,("krb5_get_credentials failed (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
					...
					int time_offset)
{
    ...
    if ((ret = smb_krb5_parse_name(context, principal,
                                    &princ))) {
	krb5_free_context(context);
	DEBUG_OUT (1,("Failed to parse %s (%s)\n", principal, error_message(ret)));
	return ADS_ERROR_KRB5(ret);
    }
	...
	krb5_free_context(context);
	DEBUG_OUT (1,("ads_krb5_chg_password: asprintf fail\n"));
	...
	DEBUG_OUT (1,("Password incorrect while getting initial ticket"));
      else
	DEBUG_OUT (1,("krb5_get_init_creds_password failed (%s)\n", error_message(ret)));
				 ...
				 int time_offset)
{
	...
	DEBUG_OUT (1,("Failed kinit for principal %s (%s)\n", auth_principal, error_message(ret)));
	return ADS_ERROR_KRB5(ret);
    }
