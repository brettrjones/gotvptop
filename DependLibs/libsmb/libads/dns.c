                       ...
                       uint8 **ptr, struct dns_rr_srv *srv )
{
	...
	if ( !ads_dns_parse_rr( ctx, start, end, ptr, &rr ) ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_srv: Failed to parse RR record\n"));
		return False;
	}

	if ( rr.type != T_SRV ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_srv: Bad answer type (%d)\n",
					rr.type));
		return False;
	}
	...
	if ( namelen < 0 ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_srv: Failed to uncompress name!\n"));
		return False;
	}

	srv->hostname = talloc_strdup( ctx, dcname );

	DEBUG_OUT (10,("ads_dns_parse_rr_srv: Parsed %s [%u, %u, %u]\n", 
		  srv->hostname, 
		  srv->priority,
		  srv->weight,
		  srv->port));
                       ...
                       uint8 **ptr, struct dns_rr_ns *nsrec )
{
	...
	if ( !ads_dns_parse_rr( ctx, start, end, ptr, &rr ) ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_ns: Failed to parse RR record\n"));
		return False;
	}

	if ( rr.type != T_NS ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_ns: Bad answer type (%d)\n",
					rr.type));
		return False;
	}
	...
	if ( namelen < 0 ) {
		DEBUG_OUT (1,("ads_dns_parse_rr_ns: Failed to uncompress name!\n"));
		return False;
	}
                              ...
                              uint8 **buf, int *resp_length )
{
	...
	{
		DEBUG_OUT (10,("last_dns_check: Returning cached status (%s)\n",
	...
	do {
		...
		if (buf_len) {
			if ((buffer = TALLOC_ARRAY(ctx, uint8, buf_len))
					== NULL ) {
				DEBUG_OUT (0,("ads_dns_lookup_srv: "
					"talloc() failed!\n"));
		...
		if ((resp_len = res_query(name, C_IN, q_type, buffer, buf_len))
				< 0 ) {
			DEBUG_OUT (3,("ads_dns_lookup_srv: "
		...
		if (buf_len == resp_len) {
			if (resp_len == MAX_DNS_PACKET_SIZE) {
				DEBUG_OUT (1,("dns_send_req: DNS reply too large when resolving %s\n",
					name));
				TALLOC_FREE( buffer );
				...
				int *numdcs)
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (3,("ads_dns_lookup_srv: Failed to send DNS query (%s)\n",
			nt_errstr(status)));
		return status;
	}
	p = buffer;
	...
	DEBUG_OUT (4,("ads_dns_lookup_srv: "
	...
	for ( rrnum=0; rrnum<query_count; rrnum++ ) {
		...
		if (!ads_dns_parse_query(ctx, buffer,
					buffer+resp_len, &p, &q)) {
			DEBUG_OUT (1,("ads_dns_lookup_srv: "
	...
	for ( rrnum=0; rrnum<additional_count; rrnum++ ) {
		...
		if (!ads_dns_parse_rr(ctx, buffer, buffer+resp_len,
					&p, &rr)) {
			DEBUG_OUT (1,("ads_dns_lookup_srv: Failed "
				...
				int *numns)
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (3,("ads_dns_lookup_ns: Failed to send DNS query (%s)\n",
			nt_errstr(status)));
		return status;
	}
	p = buffer;
	...
	for ( rrnum=0; rrnum<additional_count; rrnum++ ) {
		...
		if (!ads_dns_parse_rr(ctx, buffer, buffer+resp_len,
					&p, &rr)) {
			DEBUG_OUT (1,("ads_dns_lookup_ns: Failed "
