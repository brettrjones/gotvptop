...
#error UNKNOWN_KRB5_KEYUSAGE
	...
	if (ret) {
		DEBUG_OUT (2,("check_pac_checksum: PAC Verification failed: %s (%d)\n",
			 ...
			 struct PAC_DATA **pac_data_out)
{
	...
	*pac_data_out = NULL;
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		...
		DEBUG_OUT (0,("can't parse the PAC: %s\n",
			nt_errstr(status)));
		return status;
	}
	...
	if (pac_data->num_buffers < 4) {
		...
		DEBUG_OUT (0,("less than 4 PAC buffers\n"));
	...
	if (pac_data->num_buffers != pac_data_raw->num_buffers) {
		/* we need logon_ingo, service_key and kdc_key */
		DEBUG_OUT (0, ("misparse! PAC_DATA has %d buffers while "
	...
	for (i=0; i < pac_data->num_buffers; i++) {
		...
		if (data_buf->type != raw_buf->type) {
			DEBUG_OUT (0, ("misparse! PAC_DATA buffer %d has type "
	...
	if (!logon_info) {
		DEBUG_OUT (0,("PAC no logon_info\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (!logon_name) {
		DEBUG_OUT (0,("PAC no logon_name\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (!srv_sig_ptr || !srv_sig_blob) {
		DEBUG_OUT (0,("PAC no srv_key\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (!kdc_sig_ptr || !kdc_sig_blob) {
		DEBUG_OUT (0,("PAC no kdc_key\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		status = ndr_map_error2ntstatus(ndr_err);
		DEBUG_OUT (0,("can't parse the KDC signature: %s\n",
			nt_errstr(status)));
		return status;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		status = ndr_map_error2ntstatus(ndr_err);
		DEBUG_OUT (0,("can't parse the SRV signature: %s\n",
			nt_errstr(status)));
		return status;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		status = ndr_map_error2ntstatus(ndr_err);
		DEBUG_OUT (0,("can't repack the KDC signature: %s\n",
			nt_errstr(status)));
		return status;
	}
	ndr_err = ndr_push_struct_blob(
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		status = ndr_map_error2ntstatus(ndr_err);
		DEBUG_OUT (0,("can't repack the SRV signature: %s\n",
			nt_errstr(status)));
		return status;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		status = ndr_map_error2ntstatus(ndr_err);
		DEBUG_OUT (0,("can't repack the RAW PAC: %s\n",
			nt_errstr(status)));
		return status;
	}
	...
	ret = check_pac_checksum(mem_ctx,
				 ...
				 service_keyblock);
	if (ret) {
		DEBUG_OUT (1, ("PAC Decode: Failed to verify the service "
			  "signature: %s\n", error_message(ret)));
	...
	if (tgs_authtime_nttime != logon_name->logon_time) {
		DEBUG_OUT (2, ("PAC Decode: "
			  "Logon time mismatch between ticket and PAC!\n"));
		DEBUG_OUT (2, ("PAC Decode: PAC: %s\n",
			  nt_time_string(mem_ctx, logon_name->logon_time)));
		DEBUG_OUT (2, ("PAC Decode: Ticket: %s\n",
	...
	if (ret) {
		DEBUG_OUT (2, ("Could not parse name from PAC: [%s]:%s\n",
	...
	if (!bool_ret) {
		DEBUG_OUT (2, ("Name in PAC [%s] does not match principal name "
			  "in ticket\n", logon_name->account_name));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG_OUT (3,("Found account name from PAC: %s [%s]\n",
		 logon_info->info3.base.account_name.string,
		 logon_info->info3.base.full_name.string));

	DEBUG_OUT (10,("Successfully validated Kerberos PAC\n"));
			     ...
			     struct PAC_LOGON_INFO **logon_info)
{
	...
	ret = kerberos_kinit_password_ext(auth_princ,
					  ...
					  &status);
	if (ret) {
		DEBUG_OUT (1,("kinit failed for '%s' with: %s (%d)\n",
			auth_princ, error_message(ret), ret));
		/* status already set */
		goto out;
	}

	DEBUG_OUT (10,("got TGT for %s in %s\n", auth_princ, cc));
	if (expire_time) {
	...
	ret = cli_krb5_get_ticket(mem_ctx,
				  ...
				  impersonate_princ_s);
	if (ret) {
		DEBUG_OUT (1,("failed to get ticket for %s: %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1,("ads_verify_ticket failed: %s\n",
			nt_errstr(status)));
		goto out;
	}
