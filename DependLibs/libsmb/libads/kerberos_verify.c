					  ...
					  krb5_error_code *perr)
{
	...
	*perr = 0;
	...
	ret = smb_krb5_open_keytab(context, lp_dedicated_keytab_file(), true,
	    &keytab);
	if (ret) {
		DEBUG_OUT (1, ("smb_krb5_open_keytab failed (%s)\n",
			error_message(ret)));
		goto out;
	}
	...
	ret = krb5_rd_req(context, &auth_context, &packet, NULL, keytab,
	    NULL, &dec_ticket);
	if (ret) {
		DEBUG_OUT (0, ("krb5_rd_req failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG_OUT (0, ("krb5_kt_get_entry failed (%s)\n",
			  error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG_OUT (0, ("failed to copy key: %s\n",
			  error_message(ret)));
		goto out;
	}
					...
					krb5_error_code *perr)
{
	...
	*perr = 0;
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": krb5_kt_start_seq_get failed (%s)\n",
			  error_message(ret)));
		goto out;
	}
	...
	while (!auth_ok &&
		...
		ret = smb_krb5_unparse_name(talloc_tos(), context,
					    ...
					    &entry_princ_s);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": smb_krb5_unparse_name "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}
		...
		for (i = 0; i < ARRAY_SIZE(valid_princ_formats); i++) {
			...
			*pp_tkt = NULL;
			...
			if (ret) {
				DEBUG_OUT (10, (__location__ ": krb5_rd_req_return"
				...
				* entries - Guenther */
				...
				DEBUG_OUT (3, (__location__ ": krb5_rd_req_return"
...
out:
	...
	if (!auth_ok) {
		if (!number_matched_principals) {
			DEBUG_OUT (3, (__location__ ": no keytab principals "
				  "matched expected file service name.\n"));
		} else {
			DEBUG_OUT (3, (__location__ ": krb5_rd_req failed for "
						...
						krb5_error_code *perr)
{
	...
	*perr = 0;
	...
	if (!secrets_init()) {
		DEBUG_OUT (1,("ads_secrets_verify_ticket: secrets_init failed\n"));
	...
	if (password_s) {
		DEBUG_OUT (10, (__location__ ": found previous password\n"));
	...
	for (j=0; j<2 && passwords[j].length; j++) {
		...
		for (i=0;enctypes[i];i++) {
			...
			if (!(ret = krb5_rd_req(context, &auth_context,
						&packet, NULL, NULL,
						NULL, pp_tkt))) {
				DEBUG_OUT (10, (__location__ ": enc type [%u] "
					   "decrypted message !\n",
			...
			DEBUG_OUT ((ret != KRB5_BAD_ENCTYPE) ? 3 : 10,
			   ...
			   bool use_replay_cache)
{
	...
	*session_key = data_blob_null;
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": krb5_init_context failed (%s)\n",
			  error_message(ret)));
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": krb5_set_default_realm "
			  "failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": krb5_auth_con_init failed (%s)\n",
			  error_message(ret)));
		goto out;
	}
	...
	if (use_replay_cache) {
		...
		if (mutex == NULL) {
			DEBUG_OUT (1, (__location__ ": unable to protect replay "
				  "cache with mutex.\n"));
			ret = KRB5_CC_IO;
			goto out;
		}
		...
		ret = krb5_get_server_rcache(
				...
				&rcache);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": krb5_get_server_rcache "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}
		...
		if (ret) {
			DEBUG_OUT (1, (__location__ ": krb5_auth_con_setrcache "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}
	}
	...
	if (!auth_ok) {
		DEBUG_OUT (3, (__location__ ": krb5_rd_req with auth "
			  "failed (%s)\n", error_message(ret)));
		...
		DEBUG_OUT (10, (__location__ ": returning error %s\n",
			   nt_errstr(sret) ));
		goto out;
	}
	...
	if (ret) {
		DEBUG_OUT (3, (__location__ ": Failed to generate mutual "
	...
	*ap_rep = data_blob(packet.data, packet.length);
	...
	if (!got_auth_data) {
		DEBUG_OUT (3, (__location__ ": did not retrieve auth data. "
	...
	if (got_auth_data) {
		...
		if (!NT_STATUS_IS_OK(pac_ret)) {
			DEBUG_OUT (3, (__location__ ": failed to decode "
				  "PAC_DATA: %s\n", nt_errstr(pac_ret)));
		} else {
			uint32_t i;
			...
			if (!*logon_info) {
				DEBUG_OUT (1, ("correctly decoded PAC but found "
					  "no logon_info! "
	...
	if (ret) {
		DEBUG_OUT (3, (__location__ ": smb_krb5_unparse_name "
			  "failed (%s)\n", error_message(ret)));
		sret = NT_STATUS_LOGON_FAILURE;
		goto out;
	}
