					       ...
					       LDAPMessage **res)
{
	...
	*res = NULL;
	...
	if (ADS_ERR_OK(status)) {
               DEBUG_OUT (5,("Search for %s in <%s> gave %d replies\n",
	...
	while (--count) {
		...
		if (NT_STATUS_EQUAL(ads_ntstatus(status), NT_STATUS_IO_TIMEOUT) && ads->config.ldap_page_size >= 250) {
			...
			DEBUG_OUT (1, ("Reducing LDAP page size from %d to %d due to IO_TIMEOUT\n",
		...
		*res = NULL;
		...
		DEBUG_OUT (3,("Reopening ads connection to realm '%s' after error %s\n", 
		...
		if (!ADS_ERR_OK(status)) {
			DEBUG_OUT (1,("ads_search_retry: failed to reconnect (%s)\n",
				 ads_errstr(status)));
			ads_destroy(&ads);
			SAFE_FREE(bp);
			return status;
		...
		*res = NULL;
		...
		if (ADS_ERR_OK(status)) {
			DEBUG_OUT (5,("Search for filter: %s, base: %s gave %d replies\n",
				 expr, bp, ads_count_replies(ads, *res)));
			SAFE_FREE(bp);
			return status;
		}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1,("ads reopen failed after error %s\n", 
			 ads_errstr(status)));
	}
	return status;
}
			     ...
			     size_t *num_strings)
{
	...
	*strings = NULL;
	...
	DEBUG_OUT (10,("returning with %d strings\n", (int)*num_strings));
				      ...
				      bool *more_values)
{
	...
	DEBUG_OUT (10, ("Searching for attrs[0] = %s, attrs[1] = %s\n", attrs[0], attrs[1]));
	...
	*more_values = False;
	...
	if (*num_strings == 0) {
		if (!ads_pull_uint32(ads, res, "usnChanged", first_usn)) {
			DEBUG_OUT (1, ("could not pull first usnChanged!\n"));
			ads_msgfree(ads, res);
			return ADS_ERROR(LDAP_NO_MEMORY);
		}
	}
	...
	if (!ads_pull_uint32(ads, res, "usnChanged", &current_usn)) {
		DEBUG_OUT (1, ("could not pull current usnChanged!\n"));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_MEMORY);
	}

	if (*first_usn != current_usn) {
		DEBUG_OUT (5, ("USN on this record changed"
			  " - restarting search\n"));
		if (*num_retries < 5) {
			(*num_retries)++;
			*num_strings = 0;
	...
	*strings = ads_pull_strings_range(ads, mem_ctx, res,
	...
	if (*strings == NULL && *more_values) {
		DEBUG_OUT (0,("no strings found but more values???\n"));
		return ADS_ERROR(LDAP_NO_MEMORY);
	}
