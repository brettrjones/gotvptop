			...
			struct netlogon_samlogon_response **_reply)
{
	...
	if (!interpret_string_addr_prefer_ipv4(&ss, server, 0)) {
		DEBUG_OUT (2,("Failed to resolve[%s] into an address for cldap\n",
			server));
		return false;
	}
	...
	if (ret != 0) {
		...
		DEBUG_OUT (2,("Failed to create cldap tsocket_address for %s - %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("Failed to create cldap socket to %s: %s\n",
			 dest_str, nt_errstr(status)));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("cldap_netlogon() failed: %s\n", nt_errstr(status)));
		goto failed;
	}
			  ...
			  struct NETLOGON_SAM_LOGON_RESPONSE_EX *reply5)
{
	...
	if (reply->ntver != NETLOGON_NT_VERSION_5EX) {
		DEBUG_OUT (0,("ads_cldap_netlogon_5: nt_version mismatch: 0x%08x\n",
			reply->ntver));
		return false;
	}
