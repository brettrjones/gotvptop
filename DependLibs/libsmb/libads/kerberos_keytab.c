						   ...
						   bool keep_old_entries)
{
	...
	DEBUG_OUT (3, (__location__ ": Will try to delete old keytab entries\n"));
	while (!krb5_kt_next_entry(context, keytab, &kt_entry, &cursor)) {
		...
		if (!flush && (princ_s != NULL)) {
			...
			if (!name_ok) {
				DEBUG_OUT (10, (__location__ ": ignoring keytab "
		...
		if (!flush && (kt_entry.vno == kvno - 1)) {
			DEBUG_OUT (5, (__location__ ": Saving previous (kvno %d) "
		...
		if (keep_old_entries) {
			DEBUG_OUT (5, (__location__ ": Saving old (kvno %d) "
				  "entry for principal: %s.\n",
				  kvno, princ_s));
			continue;
		}

		DEBUG_OUT (5, (__location__ ": Found old entry for principal: %s "
		...
		ZERO_STRUCT(cursor);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": krb5_kt_end_seq_get() "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}
		ret = krb5_kt_remove_entry(context, keytab, &kt_entry);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": krb5_kt_remove_entry() "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}

		DEBUG_OUT (5, (__location__ ": removed old entry for principal: "
			  "%s (kvno %d).\n", princ_s, kt_entry.vno));

		ret = krb5_kt_start_seq_get(context, keytab, &cursor);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": krb5_kt_start_seq() failed "
				  "(%s)\n", error_message(ret)));
			goto out;
		}
				 ...
				 bool keep_old_entries)
{
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": smb_krb5_parse_name(%s) "
	...
	for (i = 0; enctypes[i]; i++) {
		...
		DEBUG_OUT (3, (__location__ ": adding keytab entry for (%s) with "
		...
		ZERO_STRUCT(kt_entry);
		if (ret) {
			DEBUG_OUT (1, (__location__ ": adding entry to keytab "
				  "failed (%s)\n", error_message(ret)));
			goto out;
		}
	}
...
int ads_keytab_add_entry(ADS_STRUCT *ads, const char *srvPrinc)
{
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": could not krb5_init_context: %s\n",
			  error_message(ret)));
		return -1;
	}
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": smb_krb5_open_keytab failed (%s)\n",
			  error_message(ret)));
		goto out;
	}
	...
	if (!secrets_init()) {
		DEBUG_OUT (1, (__location__ ": secrets_init failed\n"));
		ret = -1;
		goto out;
	}
	password_s = secrets_fetch_machine_password(lp_workgroup(), NULL, NULL);
	if (!password_s) {
		DEBUG_OUT (1, (__location__ ": failed to fetch machine password\n"));
		ret = -1;
		goto out;
	}
	...
	if (!tmpctx) {
		DEBUG_OUT (0, (__location__ ": talloc_init() failed!\n"));
		ret = -1;
		goto out;
	}
	...
	if (!my_fqdn) {
		DEBUG_OUT (0, (__location__ ": unable to determine machine "
	...
	if (strchr_m(srvPrinc, '@')) {
		...
		if (!strequal(srvPrinc, "cifs") &&
			...
			DEBUG_OUT (3, (__location__ ": Attempting to add/update "
				  "'%s'\n", princ_s));

			aderr = ads_add_service_principal_name(ads,
					global_myname(), my_fqdn, srvPrinc);
			if (!ADS_ERR_OK(aderr)) {
				DEBUG_OUT (1, (__location__ ": failed to "
	...
	if (kvno == -1) {
		...
		DEBUG_OUT (1, (__location__ ": ads_get_machine_kvno failed to "
	...
	ret = smb_krb5_kt_add_entry(context, keytab, kvno,
				    ...
				    false, false);
	if (ret) {
		DEBUG_OUT (1, (__location__ ": Failed to add entry to keytab\n"));
		goto out;
	}
...
int ads_keytab_flush(ADS_STRUCT *ads)
{
	...
	if (kvno == -1) {
		...
		DEBUG_OUT (1, (__location__ ": Error determining the kvno.\n"));
		goto out;
	}
	...
	if (!ADS_ERR_OK(aderr)) {
		DEBUG_OUT (1, (__location__ ": Error while clearing service "
...
int ads_keytab_create_default(ADS_STRUCT *ads)
{
	...
	if (ret != 0) {
		DEBUG_OUT (1, (__location__ ": ads_keytab_add_entry failed while "
	...
	if (ret != 0) {
		DEBUG_OUT (1, (__location__ ": ads_keytab_add_entry() failed "
	...
	if (upn) {
		...
		if (ret != 0) {
			DEBUG_OUT (1, (__location__ ": ads_keytab_add_entry() "
	...
	DEBUG_OUT (3, (__location__ ": Searching for keytab entries to preserve "
		  "and update.\n"));
	 ...
	 */
	DEBUG_OUT (3, (__location__ ": Found %d entries in the keytab.\n", found));
	if (!found) {
		goto done;
	}
	...
	if (!oldEntries) {
		DEBUG_OUT (1, (__location__ ": Failed to allocate space to store "
