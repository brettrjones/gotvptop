...
{
	DEBUG_OUT (0,("Got SIGTERM: going down...\n"));
...
static void nmbd_sig_hup_handler(struct tevent_context *ev,
				 ...
				 void *private_data)
{
	DEBUG_OUT (0,("Got SIGHUP dumping debug info.\n"));
...
static bool nmbd_setup_sig_hup_handler(void)
{
	...
	se = tevent_add_signal(nmbd_event_context(),
			       ...
			       NULL);
	if (!se) {
		DEBUG_OUT (0,("failed to setup SIGHUP handler"));
		return false;
	}
...
static void reload_interfaces(time_t t)
{
  ...
  try_again:
	...
	for (n=iface_count() - 1; n >= 0; n--) {
		...
		if (!iface) {
			DEBUG_OUT (2,("reload_interfaces: failed to get interface %d\n", n));
			continue;
		}

		/* Ensure we're only dealing with IPv4 here. */
		if (iface->ip.ss_family != AF_INET) {
			DEBUG_OUT (2,("reload_interfaces: "
		...
		if (is_loopback_addr((struct sockaddr *)(void *)&iface->ip)) {
			DEBUG_OUT (2,("reload_interfaces: Ignoring loopback "
				"interface %s\n",
		...
		if (!subrec) {
			...
			DEBUG_OUT (2,("Found new interface %s\n",
				 print_sockaddr(str,
					 sizeof(str), &iface->ip) ));
	...
	for (subrec=subnetlist; subrec; subrec=subrec->next) {
		...
		if (n == -1) {
			/* oops, an interface has disapeared. This is
			 ...
			 freeing it */
			DEBUG_OUT (2,("Deleting dead interface %s\n",
...
static bool reload_nmbd_services(bool test)
{
	...
	if ( !test ) {
		DEBUG_OUT ( 3, ( "services not loaded\n" ) );
				 ...
				 DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("Received send_packet from %u\n", (unsigned int)procid_to_pid(&src)));

	if (data->length != sizeof(struct packet_struct)) {
		DEBUG_OUT (2, ("Discarding invalid packet length from %u\n",
			  (unsigned int)procid_to_pid(&src)));
		return;
	}
	...
	if ((p->packet_type != NMB_PACKET) &&
		...
		DEBUG_OUT (2, ("Discarding invalid packet type from %u: %d\n",
	...
	if (pss == NULL) {
		DEBUG_OUT (2, ("Could not find ip for packet from %u\n",
			  (unsigned int)procid_to_pid(&src)));
		return;
	}
...
static bool open_sockets(bool isdaemon, int port)
{
	...
	if (!interpret_string_addr(&ss, sock_addr,
		...
		DEBUG_OUT (0,("open_sockets: unable to get socket address "
			"from string %s", sock_addr));
		return false;
	}
	if (ss.ss_family != AF_INET) {
		DEBUG_OUT (0,("open_sockets: unable to use IPv6 socket"
			"%s in nmbd\n",
			sock_addr));
		return false;
	}
	...
	DEBUG_OUT ( 3, ( "open_sockets: Broadcast sockets opened.\n" ) );
	return( True );
}
 ...
 int main(int argc, const char *argv[])
{
	...
	DEBUG_OUT (0,("nmbd version %s started.\n", samba_version_string()));
	...
	if (strequal(lp_workgroup(),"*")) {
		DEBUG_OUT (0,("ERROR: a workgroup name of * is no longer supported\n"));
		exit(1);
	}
	...
	if( False == create_subnets() ) {
		DEBUG_OUT (0,("ERROR: Failed when creating subnet lists. Exiting.\n"));
		kill_async_dns_child();
		exit(1);
	}
	...
	DEBUG_OUT (3,("Loaded hosts file %s\n", get_dyn_LMHOSTSFILE()));

	/* If we are acting as a WINS server, initialise data structures. */
	if( !initialise_wins() ) {
		DEBUG_OUT ( 0, ( "nmbd: Failed when initialising WINS server.\n" ) );
		kill_async_dns_child();
		exit(1);
	}
	...
	if( False == register_my_workgroup_and_names() ) {
		DEBUG_OUT (0,("ERROR: Failed when creating my my workgroup. Exiting.\n"));
		kill_async_dns_child();
		exit(1);
	}

	if (!initialize_nmbd_proxy_logon()) {
		DEBUG_OUT (0,("ERROR: Failed setup nmbd_proxy_logon.\n"));
		kill_async_dns_child();
		exit(1);
	}
