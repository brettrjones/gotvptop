...
void remove_all_servers(struct work_record *work)
{
	...
	for (servrec = work->serverlist; servrec; servrec = nexts) {
		DEBUG_OUT (7,("remove_all_servers: Removing server %s\n",servrec->serv.name));
		nexts = servrec->next;
                                                 ...
                                                 int ttl, const char *comment)
{
	...
	if (name[0] == '*') {
		DEBUG_OUT (7,("create_server_on_workgroup: not adding name starting with '*' (%s)\n",
			name));
		return (NULL);
	}
  
	if(find_server_in_workgroup(work, name) != NULL) {
		DEBUG_OUT (0,("create_server_on_workgroup: Server %s already exists on \
workgroup %s. This is a bug.\n", name, work->work_group));
		return NULL;
	}
  
	if((servrec = SMB_MALLOC_P(struct server_record)) == NULL) {
		DEBUG_OUT (0,("create_server_entry_on_workgroup: malloc fail !\n"));
		return NULL;
	}
	...
	DEBUG_OUT (3,("create_server_on_workgroup: Created server entry %s of type %x (%s) on \
...
void expire_servers(struct work_record *work, time_t t)
{
	...
	for (servrec = work->serverlist; servrec; servrec = nexts) {
		...
		if ((servrec->death_time != PERMANENT_TTL) && ((t == -1) || (servrec->death_time < t))) {
			DEBUG_OUT (3,("expire_old_servers: Removing timed out server %s\n",servrec->serv.name));
...
void write_browse_list(time_t t, bool force_write)
{
	...
	if (!fp) {
		DEBUG_OUT (0,("write_browse_list: Can't open file %s. Error was %s\n",
	...
	if((work = find_workgroup_on_subnet(FIRST_SUBNET, lp_workgroup())) == NULL) { 
		DEBUG_OUT (0,("write_browse_list: Fatal error - cannot find my workgroup %s\n",
			lp_workgroup()));
		x_fclose(fp);
		return;
	}
	...
	DEBUG_OUT (3,("write_browse_list: Wrote browse list into file %s\n",fname));
}
