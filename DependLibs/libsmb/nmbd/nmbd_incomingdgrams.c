  ...
  for (subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_EXCLUDING_UNICAST(subrec))
  {
    ...
    for (work = subrec->workgrouplist; work; work = work->next)
    {
      ...
      for (servrec = work->serverlist; servrec; servrec = servrec->next)
      {
        ...
        DEBUG_OUT (3,("num servers: %d num backups: %d\n", 
        ...
        DEBUG_OUT (2,("sending become backup to %s %s for %s\n",
...
void process_host_announce(struct subnet_record *subrec, struct packet_struct *p, const char *buf)
{
	...
	DEBUG_OUT (3,("process_host_announce: from %s<%02x> IP %s to \
%s for server %s.\n", source_name, source_name[15], inet_ntoa(p->ip),
			nmb_namestr(&dgram->dest_name),announce_name));

	DEBUG_OUT (5,("process_host_announce: ttl=%d server type=%08x comment=%s\n",
	...
	if(dgram->dest_name.name_type != 0x1d) {
		DEBUG_OUT (2,("process_host_announce: incorrect name type for destination from IP %s \
...
void process_workgroup_announce(struct subnet_record *subrec, struct packet_struct *p, const char *buf)
{
	...
	DEBUG_OUT (3,("process_workgroup_announce: from %s<%02x> IP %s to \
%s for workgroup %s.\n", source_name, source_name[15], inet_ntoa(p->ip),
			nmb_namestr(&dgram->dest_name),workgroup_announce_name));

	DEBUG_OUT (5,("process_workgroup_announce: ttl=%d server type=%08x master browser=%s\n",
	...
	if (!strequal(dest_name, MSBROWSE) || (dgram->dest_name.name_type != 0x1)) {
		DEBUG_OUT (0,("process_workgroup_announce: from IP %s should be to __MSBROWSE__<0x01> not %s\n",
...
void process_local_master_announce(struct subnet_record *subrec, struct packet_struct *p, const char *buf)
{
	...
	DEBUG_OUT (3,("process_local_master_announce: from %s<%02x> IP %s to \
%s for server %s.\n", source_name, source_name[15], inet_ntoa(p->ip),
		nmb_namestr(&dgram->dest_name),server_name));

	DEBUG_OUT (5,("process_local_master_announce: ttl=%d server type=%08x comment=%s\n",
		ttl, servertype, comment));

	/* A local master announcement must be sent to the name WORKGROUP<1e>. */
	if(dgram->dest_name.name_type != 0x1e) {
		DEBUG_OUT (0,("process_local_master_announce: incorrect name type for destination from IP %s \
(was %02x) should be 0x1e. Ignoring packet.\n",
	...
	*/
	if(AM_LOCAL_MASTER_BROWSER(work)) {
		DEBUG_OUT (0,("process_local_master_announce: Server %s at IP %s is announcing itself as \
                                     ...
                                     struct packet_struct *p,const char *buf)
{
	...
	DEBUG_OUT (3,("process_master_browser_announce: Local master announce from %s IP %s.\n",
		local_master_name, inet_ntoa(p->ip)));
  
	if (!lp_domain_master()) {
		DEBUG_OUT (0,("process_master_browser_announce: Not configured as domain \
master - ignoring master announce.\n"));
		goto done;
	}
  
	if((work = find_workgroup_on_subnet(subrec, lp_workgroup())) == NULL) {
		DEBUG_OUT (0,("process_master_browser_announce: Cannot find workgroup %s on subnet %s\n",
			lp_workgroup(), subrec->subnet_name));
		goto done;
	}

	if(!AM_DOMAIN_MASTER_BROWSER(work)) {
		DEBUG_OUT (0,("process_master_browser_announce: Local master announce made to us from \
...
void process_lm_host_announce(struct subnet_record *subrec, struct packet_struct *p, const char *buf, int len)
{
	...
	DEBUG_OUT (3,("process_lm_host_announce: LM Announcement from %s IP %s to \
%s for server %s.\n", nmb_namestr(&dgram->source_name), inet_ntoa(p->ip),
		nmb_namestr(&dgram->dest_name),announce_name));

	DEBUG_OUT (5,("process_lm_host_announce: os=(%d,%d) ttl=%d server type=%08x comment=%s\n",
		osmajor, osminor, ttl, servertype,comment));

	if ((osmajor < 36) || (osmajor > 38) || (osminor !=0)) {
		DEBUG_OUT (5,("process_lm_host_announce: LM Announcement packet does not \
originate from OS/2 Warp client. Ignoring packet.\n"));
	...
	if(dgram->dest_name.name_type != 0x00) {
		DEBUG_OUT (2,("process_lm_host_announce: incorrect name type for destination from IP %s \
				      ...
				      int port)
{
  ...
  struct server_record *servrec;
	...
	DEBUG_OUT (3,("send_backup_list_response: sending backup list for workgroup %s to %s IP %s\n",
  ...
  { 
    ...
    DEBUG_OUT (5,("send_backup_list_response: Adding server %s number %d\n",
              p, count));
	...
	DEBUG_OUT (4,("send_backup_list_response: sending response to %s<00> IP %s with %d servers.\n",
                                     ...
                                     struct packet_struct *p,const char *buf)
{
	...
	DEBUG_OUT (3,("process_get_backup_list_request: request from %s IP %s to %s.\n",
		nmb_namestr(&dgram->source_name), inet_ntoa(p->ip),
	...
	if(strequal(workgroup_name, lp_workgroup()) == False) {
		DEBUG_OUT (7,("process_get_backup_list_request: Ignoring announce request for workgroup %s.\n",
			workgroup_name));
		goto done;
	}

	if((work = find_workgroup_on_subnet(search_subrec, workgroup_name)) == NULL) {
		DEBUG_OUT (0,("process_get_backup_list_request: Cannot find workgroup %s on \
subnet %s.\n", workgroup_name, search_subrec->subnet_name));
	...
	if(name_type == 0x1b) {
		...
		if(!AM_DOMAIN_MASTER_BROWSER(work)) {
			DEBUG_OUT (0,("process_get_backup_list_request: domain list requested for workgroup %s \
...
and I am not a local master browser.\n", workgroup_name));
		...
		DEBUG_OUT (0,("process_get_backup_list_request: Invalid name type %x - should be 0x1b or 0x1d.\n",
			name_type));
		goto done;
	}
                                  ...
                                  struct packet_struct *p,const char *buf)
{
	...
	DEBUG_OUT (1,("process_reset_browser: received diagnostic browser reset \
request from %s IP %s state=0x%X\n",
	...
	if (state & 0x4)
		DEBUG_OUT (1,("process_reset_browser: ignoring request to stop being a browser.\n"));
...
void process_announce_request(struct subnet_record *subrec, struct packet_struct *p, const char *buf)
{
	...
	pull_ascii_nstring(workgroup_name, sizeof(workgroup_name), dgram->dest_name.name);
	DEBUG_OUT (3,("process_announce_request: Announce request from %s IP %s to %s.\n",
		nmb_namestr(&dgram->source_name), inet_ntoa(p->ip),
		nmb_namestr(&dgram->dest_name)));
  
	/* We only send announcement requests on our workgroup. */
	if(strequal(workgroup_name, lp_workgroup()) == False) {
		DEBUG_OUT (7,("process_announce_request: Ignoring announce request for workgroup %s.\n",
			workgroup_name));
		goto done;
	}

	if((work = find_workgroup_on_subnet(subrec, workgroup_name)) == NULL) {
		DEBUG_OUT (0,("process_announce_request: Unable to find workgroup %s on subnet !\n",
			workgroup_name));
		goto done;
	}
...
void process_lm_announce_request(struct subnet_record *subrec, struct packet_struct *p, const char *buf, int len)
{
	...
	pull_ascii_nstring(workgroup_name, sizeof(workgroup_name), dgram->dest_name.name);
	DEBUG_OUT (3,("process_lm_announce_request: Announce request from %s IP %s to %s.\n",
		nmb_namestr(&dgram->source_name), inet_ntoa(p->ip),
		nmb_namestr(&dgram->dest_name)));

	/* We only send announcement requests on our workgroup. */
	if(strequal(workgroup_name, lp_workgroup()) == False) {
		DEBUG_OUT (7,("process_lm_announce_request: Ignoring announce request for workgroup %s.\n",
			workgroup_name));
		goto done;
	}
