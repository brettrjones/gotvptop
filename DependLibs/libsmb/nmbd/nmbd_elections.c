                                ...
                                uint32 criterion, int timeup,const char *server_name)
{
	...
	DEBUG_OUT (2,("send_election_dgram: Sending election packet for workgroup %s on subnet %s\n",
                                 ...
                                 struct in_addr answer_ip, struct res_rec *rrec)
{
	...
	DEBUG_OUT (3,("check_for_master_browser_success: Local master browser for workgroup %s exists at \
                                           ...
                                           int fail_code)
{
	...
	if(work == NULL) {
		DEBUG_OUT (0,("check_for_master_browser_fail: Unable to find workgroup %s on subnet %s.=\n",
			workgroup_name, subrec->subnet_name ));
		return;
	}
	...
	if (strequal(work->work_group, lp_workgroup())) {
		...
		if (lp_local_master()) {
			...
			*/
			DEBUG_OUT (2,("check_for_master_browser_fail: Forcing election on workgroup %s subnet %s\n",
		...
		for (work = subrec->workgrouplist; work; work = work->next) {
			if (work->RunningElection) {
				...
				if(find_name_on_subnet( subrec, &nmbname, FIND_SELF_NAME)==NULL) {
					DEBUG_OUT (8,("run_elections: Cannot send election packet yet as name %s not \
yet registered on subnet %s\n", nmb_namestr(&nmbname), subrec->subnet_name ));
				...
				if (work->ElectionCount++ >= 4) {
					...
					DEBUG_OUT (2,("run_elections: >>> Won election for workgroup %s on subnet %s <<<\n",
						work->work_group, subrec->subnet_name ));
...
{  
	...
	if(!lp_local_master()) {
		DEBUG_OUT (3,("win_election: Losing election as local master == False\n"));
		return False;
	}
 
	DEBUG_OUT (4,("win_election: election comparison: %x:%x %x:%x %d:%d %s:%s\n",
...
void process_election(struct subnet_record *subrec, struct packet_struct *p, const char *buf)
{
	...
	DEBUG_OUT (3,("process_election: Election request from %s at IP %s on subnet %s for workgroup %s.\n",
		server_name,inet_ntoa(p->ip), subrec->subnet_name, workgroup_name ));

	DEBUG_OUT (5,("process_election: vers=%d criterion=%08x timeup=%d\n", version,criterion,timeup));

	if(( work = find_workgroup_on_subnet(subrec, workgroup_name)) == NULL) {
		DEBUG_OUT (0,("process_election: Cannot find workgroup %s on subnet %s.\n",
			workgroup_name, subrec->subnet_name ));
		goto done;
	}

	if (!strequal(work->work_group, lp_workgroup())) {
		DEBUG_OUT (3,("process_election: ignoring election request for workgroup %s on subnet %s as this \
is not my workgroup.\n", work->work_group, subrec->subnet_name ));
		goto done;
	}
	...
	if (win_election(work, version,criterion,timeup,server_name)) {
		...
		if (work->RunningElection || AM_LOCAL_MASTER_BROWSER(work)) {
			work->RunningElection = False;
			DEBUG_OUT (3,("process_election: >>> Lost election for workgroup %s on subnet %s <<<\n",
				work->work_group, subrec->subnet_name ));
	...
	for (subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_EXCLUDING_UNICAST(subrec)) {
		...
		for (work = subrec->workgrouplist; work; work = work->next) {
			...
			if (work->needelection && !work->RunningElection && lp_local_master()) {
				...
				make_nmb_name(&nmbname, work->work_group, 0x1e);
				if(find_name_on_subnet( subrec, &nmbname, FIND_SELF_NAME)==NULL) {
					DEBUG_OUT (8,("check_elections: Cannot send election packet yet as name %s not \
yet registered on subnet %s\n", nmb_namestr(&nmbname), subrec->subnet_name ));
					continue;
				}

				DEBUG_OUT (3,("check_elections: >>> Starting election for workgroup %s on subnet %s <<<\n",
					work->work_group, subrec->subnet_name ));
