...
struct name_record *find_name_on_wins_subnet(const struct nmb_name *nmbname, bool self_only)
{
	...
	if( self_only && (namerec->data.source != SELF_NAME) && (namerec->data.source != PERMANENT_NAME) ) {
		DEBUG_OUT ( 9, ( "find_name_on_wins_subnet: self name %s NOT FOUND\n", nmb_namestr(nmbname) ) );
...
static void update_wins_flag(struct name_record *namerec, int flags)
{
	...
	DEBUG_OUT (8,("update_wins_flag: nbflags: 0x%x, ttl: %d, flags: 0x%x, winsflags: 0x%x\n", 
...
static void get_global_id_and_update(uint64_t *current_id, bool update)
{
	...
	DEBUG_OUT (5,("get_global_id_and_update: updating version ID: %d\n", (int)general_id));
...
static void wins_hook(const char *operation, struct name_record *namerec, int ttl)
{
	...
	for (p=namerec->name.name; *p; p++) {
		if (!(isalnum((int)*p) || strchr_m("._-",*p))) {
			DEBUG_OUT (3,("not calling wins hook for invalid name %s\n", nmb_namestr(&namerec->name)));
			return;
		}
	}
	...
	DEBUG_OUT (3,("calling wins hook for %s\n", nmb_namestr(&namerec->name)));
	smbrun(command, NULL);
	TALLOC_FREE(command);
}
...
bool packet_is_for_wins_server(struct packet_struct *packet)
{
	...
	if((wins_server_subnet == NULL) || (nmb->header.nm_flags.bcast == True)) {
		DEBUG_OUT (10, ("packet_is_for_wins_server: failing WINS test #1.\n"));
		return False;
	}
	...
	switch(nmb->header.opcode) { 
		...
		case NMB_WACK_OPCODE:
			DEBUG_OUT (10, ("packet_is_for_wins_server: failing WINS test #2 (WACK).\n"));
			return False;
		/*
		...
		case NMB_NAME_REFRESH_OPCODE_9: /* WinNT uses 8 by default. */
			if(nmb->header.response) {
				DEBUG_OUT (10, ("packet_is_for_wins_server: failing WINS test #3 (response = 1).\n"));
				return False;
			}
			break;
		...
		case NMB_NAME_RELEASE_OPCODE:
			if(nmb->header.response) {
				DEBUG_OUT (10, ("packet_is_for_wins_server: failing WINS test #4 (response = 1).\n"));
				return False;
			}
			break;
		...
		case NMB_NAME_QUERY_OPCODE:
			if(!nmb->header.response && !nmb->header.nm_flags.recursion_desired) {
				DEBUG_OUT (10, ("packet_is_for_wins_server: failing WINS test #5 (response = 1).\n"));
				return False;
			}
			break;
	}
...
bool initialise_wins(void)
{
	...
	wins_tdb = tdb_open_log(state_path("wins.tdb"), 0, TDB_DEFAULT|TDB_CLEAR_IF_FIRST|TDB_INCOMPATIBLE_HASH,
			O_CREAT|O_RDWR, 0600);
	if (!wins_tdb) {
		DEBUG_OUT (0,("initialise_wins: failed to open wins.tdb. Error was %s\n",
			strerror(errno) ));
		return False;
	}
	...
	if((fp = x_fopen(state_path(WINS_LIST),O_RDONLY,0)) == NULL) {
		DEBUG_OUT (2,("initialise_wins: Can't open wins database file %s. Error was %s\n",
	...
	while (!x_feof(fp)) {
		...
		if (strncmp(line,"VERSION ", 8) == 0) {
			if (sscanf(line,"VERSION %d %u", &version, &hash) != 2 ||
				...
				DEBUG_OUT (0,("Discarding invalid wins.dat file [%s]\n",line));
				x_fclose(fp);
				return True;
			}
			continue;
		...
		if (!next_token_talloc(frame,&ptr,&name_str,NULL)) {
			DEBUG_OUT (0,("initialise_wins: Failed to parse name when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}

		if (!next_token_talloc(frame,&ptr,&ttl_str,NULL)) {
			DEBUG_OUT (0,("initialise_wins: Failed to parse time to live when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}
		...
		if(num_ips == 0) {
			DEBUG_OUT (0,("initialise_wins: Missing IP address when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}

		if(!got_token) {
			DEBUG_OUT (0,("initialise_wins: Missing nb_flags when parsing line %s\n", line ));
			TALLOC_FREE(frame);
			continue;
		}
		...
		if((ip_list = SMB_MALLOC_ARRAY( struct in_addr, num_ips)) == NULL) {
			DEBUG_OUT (0,("initialise_wins: Malloc fail !\n"));
			x_fclose(fp);
			TALLOC_FREE(frame);
			return False;
		}
		...
		if(nb_flags_str[strlen(nb_flags_str)-1] == 'S') {
			DEBUG_OUT (5,("initialise_wins: Ignoring SELF name %s\n", line));
			SAFE_FREE(ip_list);
			TALLOC_FREE(frame);
			continue;
		}
		...
		if ((ttl - 60) > time_now || ttl == PERMANENT_TTL) {
			...
			DEBUG_OUT ( 4, ("initialise_wins: add name: %s#%02x ttl = %d first IP %s flags = %2x\n",
				name, type, ttl, inet_ntoa(ip_list[0]), nb_flags));

			(void)add_name_to_subnet( wins_server_subnet, name, type, nb_flags, 
					ttl, REGISTER_NAME, num_ips, ip_list );
		} else {
			DEBUG_OUT (4, ("initialise_wins: not adding name (ttl problem) "
                                             ...
                                             struct res_rec *answers)
{
	...
	DEBUG_OUT (3,("wins_register_query_success: Original client at IP %s still wants the \
                                          ...
                                          int rcode)
{
	...
	if(namerec == NULL) {
		...
		DEBUG_OUT (2,("wins_register_query_fail: The state of the WINS database changed between "
                                            ...
                                            struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG_OUT (0,("wins_process_name_registration_request: broadcast name registration request \
received for name %s from IP %s on subnet %s. Error - should not be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}

	DEBUG_OUT (3,("wins_process_name_registration_request: %s name registration for name %s \
IP %s\n", registering_group_name ? "Group" : "Unique", nmb_namestr(question), inet_ntoa(from_ip) ));
	 ...
	 */
	if ( (namerec != NULL) && !WINS_STATE_ACTIVE(namerec)) {
		DEBUG_OUT (5,("wins_process_name_registration_request: Name (%s) in WINS was \
...
a dns lookup - removing it.\n", nmb_namestr(question) ));
	...
	if((namerec != NULL) && (namerec->data.source != REGISTER_NAME)) {
		DEBUG_OUT ( 3, ( "wins_process_name_registration_request: Attempt \
to register name %s. Name already exists in WINS with source type %d.\n",
	...
	if(!registering_group_name && (question->name_type == 0x1d)) {
		DEBUG_OUT (3,("wins_process_name_registration_request: Ignoring request \
to register name %s from IP %s.\n", nmb_namestr(question), inet_ntoa(p->ip) ));
	...
	if((namerec != NULL) && NAME_GROUP(namerec)) {
		if(registering_group_name) {
			...
			DEBUG_OUT (3,("wins_process_name_registration_request: Adding IP %s to group name %s.\n",
			...
			if(!find_ip_in_name_record(namerec, from_ip)) {
				...
				while(namerec->data.num_ips>=25) {
					...
					DEBUG_OUT (3,("wins_process_name_registration_request: "
			...
			DEBUG_OUT (3,("wins_process_name_registration_request: Attempt to register name %s. Name \
                                             ...
                                             struct res_rec *answers)
{
	...
	if( (namerec == NULL) || (namerec->data.source != REGISTER_NAME) || !WINS_STATE_ACTIVE(namerec) ) {
		DEBUG_OUT (3,("wins_multihomed_register_query_success: name %s is not in the correct state to add \
                                          ...
                                          int rcode)
{
	...
	DEBUG_OUT (3,("wins_multihomed_register_query_fail: Registering machine at IP %s failed to answer \
                                                        ...
                                                        struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG_OUT (0,("wins_process_multihomed_name_registration_request: broadcast name registration request \
received for name %s from IP %s on subnet %s. Error - should not be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}
	...
	if(group) {
		DEBUG_OUT (0,("wins_process_multihomed_name_registration_request: group name registration request \
received for name %s from IP %s on subnet %s. Errror - group names should not be multihomed.\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}

	DEBUG_OUT (3,("wins_process_multihomed_name_registration_request: name registration for name %s \
IP %s\n", nmb_namestr(question), inet_ntoa(from_ip) ));
	...
	if(question->name_type == 0x1d) {
		DEBUG_OUT (3,("wins_process_multihomed_name_registration_request: Ignoring request \
to register name %s from IP %s.", nmb_namestr(question), inet_ntoa(p->ip) ));
	...
	if ((namerec != NULL) && !WINS_STATE_ACTIVE(namerec)) {
		DEBUG_OUT (5,("wins_process_multihomed_name_registration_request: Name (%s) in WINS was not active - removing it.\n", nmb_namestr(question)));
		remove_name_from_namelist(subrec, namerec);
		namerec = NULL;
	}
	...
	if( (namerec != NULL) && ( (namerec->data.source == DNS_NAME) || (namerec->data.source == DNSFAIL_NAME) ) ) {
		DEBUG_OUT (5,("wins_process_multihomed_name_registration_request: Name (%s) in WINS was a dns lookup \
- removing it.\n", nmb_namestr(question) ));
		remove_name_from_namelist( subrec, namerec);
		namerec = NULL;
	}
	...
	if( (namerec != NULL) && (namerec->data.source != REGISTER_NAME) ) {
		DEBUG_OUT ( 3, ( "wins_process_multihomed_name_registration_request: Attempt \
to register name %s. Name already exists in WINS with source type %d.\n",
			nmb_namestr(question), namerec->data.source ));
		send_wins_name_registration_response(RFS_ERR, 0, p);
		return;
	...
	if((namerec != NULL) && NAME_GROUP(namerec) && WINS_STATE_ACTIVE(namerec)) {
		DEBUG_OUT (3,("wins_process_multihomed_name_registration_request: Attempt to register name %s. Name \
already exists in WINS as a GROUP name.\n", nmb_namestr(question) ));
		send_wins_name_registration_response(RFS_ERR, 0, p);
		return;
	} 
...
{  
	...
	if((prdata = (char *)SMB_MALLOC( num_ips * 6 )) == NULL) {
		DEBUG_OUT (0,("process_wins_dmb_query_request: Malloc fail !.\n"));
		return;
	}
	...
	if(rcode == 0) {
		...
		if( ip_count == 1 ) {
			...
			if((prdata = (char *)SMB_MALLOC( ip_count * 6 )) == NULL) {
				DEBUG_OUT (0,("send_wins_name_query_response: malloc fail !\n"));
				return;
			}
		}
                                     ...
                                     struct packet_struct *p)
{
	...
	DEBUG_OUT (3,("wins_process_name_query: name query for name %s from IP %s\n", 
		nmb_namestr(question), inet_ntoa(p->ip) ));
	...
	if(namerec != NULL) {
		...
		if (!WINS_STATE_ACTIVE(namerec)) {
			DEBUG_OUT (3,("wins_process_name_query: name query for name %s - name expired. Returning fail.\n",
				nmb_namestr(question) ));
		...
		if( namerec->data.source == DNSFAIL_NAME ) {
			DEBUG_OUT (3,("wins_process_name_query: name query for name %s returning DNS fail.\n",
				nmb_namestr(question) ));
			send_wins_name_query_response(NAM_ERR, p, namerec);
			return;
		}
		...
		DEBUG_OUT (3,("wins_process_name_query: name query for name %s returning first IP %s.\n",
	...
	if(lp_dns_proxy() && ((question->name_type == 0x20) || question->name_type == 0)) {
		DEBUG_OUT (3,("wins_process_name_query: name query for name %s not found - doing dns lookup.\n",
				nmb_namestr(question) ));
                                       ...
                                       struct packet_struct *p)
{
	...
	if(bcast) {
		...
		DEBUG_OUT (0,("wins_process_name_release_request: broadcast name registration request \
received for name %s from IP %s on subnet %s. Error - should not be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		return;
	}
  
	DEBUG_OUT (3,("wins_process_name_release_request: %s name release for name %s \
IP %s\n", releasing_group_name ? "Group" : "Unique", nmb_namestr(question), inet_ntoa(from_ip) ));
	...
	if(!releasing_group_name && (question->name_type == 0x1d)) {
		DEBUG_OUT (3,("wins_process_name_release_request: Ignoring request \
to release name %s from IP %s.", nmb_namestr(question), inet_ntoa(p->ip) ));
	...
	if(!find_ip_in_name_record(namerec, from_ip)) {
		DEBUG_OUT (3,("wins_process_name_release_request: Refusing request to \
...
release name %s as this record is not active anymore.\n", nmb_namestr(question) ));
	...
	if(releasing_group_name && (question->name_type == 0x1c) && (namerec->data.num_ips > 1)) {
		...
		DEBUG_OUT (3,("wins_process_name_release_request: Remove IP %s from NAME: %s\n",
				inet_ntoa(from_ip),nmb_namestr(question)));
		wins_hook("delete", namerec, 0);
		send_wins_name_release_response(0, p);
		return;
...
static int wins_processing_traverse_fn(TDB_CONTEXT *tdb, TDB_DATA kbuf, TDB_DATA dbuf, void *state)
{
	...
	if( (namerec->data.death_time != PERMANENT_TTL) && (namerec->data.death_time < t) ) {
		if( namerec->data.source == SELF_NAME ) {
			DEBUG_OUT ( 3, ( "wins_processing_traverse_fn: Subnet %s not expiring SELF name %s\n", 
			...
			DEBUG_OUT (3,("wins_processing_traverse_fn: deleting timed out DNS name %s\n",
					nmb_namestr(&namerec->name)));
		...
		if (ip_equal_v4(namerec->data.wins_ip, our_fake_ip)) {
			switch (namerec->data.wins_flags & WINS_STATE_MASK) {
				case WINS_ACTIVE:
					...
					DEBUG_OUT (3,("wins_processing_traverse_fn: expiring %s\n",
						nmb_namestr(&namerec->name)));
					store_record = True;
					goto done;
				case WINS_RELEASED:
					...
					get_global_id_and_update(&namerec->data.id, True);
					DEBUG_OUT (3,("wins_processing_traverse_fn: tombstoning %s\n",
						nmb_namestr(&namerec->name)));
					store_record = True;
					goto done;
				case WINS_TOMBSTONED:
					DEBUG_OUT (3,("wins_processing_traverse_fn: deleting %s\n",
						nmb_namestr(&namerec->name)));
					remove_name_from_wins_namelist(namerec );
					goto done;
			}
			...
			switch (namerec->data.wins_flags & WINS_STATE_MASK) {
				...
				case WINS_TOMBSTONED:
					...
					goto done;
				case WINS_RELEASED:
					DEBUG_OUT (0,("wins_processing_traverse_fn: %s is in released state and\
...
void wins_write_database(time_t t, bool background)
{
	...
	if (background) {
		...
		if (tdb_reopen(wins_tdb)) {
			DEBUG_OUT (0,("wins_write_database: tdb_reopen failed. Error was %s\n",
				strerror(errno)));
			_exit(0);
			return;
		}
	...
	if((fp = x_fopen(fnamenew,O_WRONLY|O_CREAT,0644)) == NULL) {
		DEBUG_OUT (0,("wins_write_database: Can't open %s. Error was %s\n", fnamenew, strerror(errno)));
		goto err_exit;
	}

	DEBUG_OUT (4,("wins_write_database: Dump of WINS name list.\n"));
                                       ...
                                       DATA_BLOB *data)
{
	...
	if (namerec == NULL) {
		DEBUG_OUT (3,("nmbd_wins_new_entry: adding new replicated record: %s<%02x> for wins server: %s\n", 
	...
	if (namerec != NULL) {
		...
		if (overwrite == False)
			DEBUG_OUT (3, ("nmbd_wins_new_entry: conflict in adding record: %s<%02x> from wins server: %s\n", 
				  record->name, record->type, inet_ntoa(record->wins_ip)));
		else {
			DEBUG_OUT (3, ("nmbd_wins_new_entry: replacing record: %s<%02x> from wins server: %s\n", 
				  record->name, record->type, inet_ntoa(record->wins_ip)));
