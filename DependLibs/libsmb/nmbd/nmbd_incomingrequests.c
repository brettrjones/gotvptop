                                  ...
                                  struct packet_struct *p)
{
	...
	if(!bcast) {
		...
		*/
		DEBUG_OUT (0,("process_name_release_request: unicast name release request \
received for name %s from IP %s on subnet %s. Error - should be sent to WINS server\n",
	...
	DEBUG_OUT (3,("process_name_release_request: Name release on name %s, \
subnet %s from owner IP %s\n",
	...
	if( !group && !ismyip_v4(owner_ip) && strequal(qname, lp_workgroup()) && 
		...
		DEBUG_OUT (6,("process_name_release_request: FTP OnNet bug workaround. Ignoring \
group release name %s from IP %s on subnet %s with no group bit set.\n",
	...
	if( namerec && ( (namerec->data.source == SELF_NAME)
		...
		rcode = ACT_ERR;
		DEBUG_OUT (0, ("process_name_release_request: Attempt to release name %s from IP %s \
...
{    
	...
	if(!bcast) { 
		/* We should only get broadcast name refresh packets here.
		   ...
		   and send an error reply back.
		*/
		DEBUG_OUT (0,("process_name_refresh_request: unicast name registration request \
received for name %s from IP %s on subnet %s.\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
		DEBUG_OUT (0,("Error - should be sent to WINS server\n"));
	...
	DEBUG_OUT (3,("process_name_refresh_request: Name refresh for name %s \
IP %s on subnet %s\n", nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
}
                                       ...
                                       struct packet_struct *p)
{
	...
	if(!bcast) {
		/* We should only get broadcast name registration packets here.
		   ...
		   and send an error reply back.
		*/
		DEBUG_OUT (0,("process_name_registration_request: unicast name registration request \
received for name %s from IP %s on subnet %s. Error - should be sent to WINS server\n",
			nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));      
	...
	DEBUG_OUT (3,("process_name_registration_request: Name registration for name %s \
IP %s on subnet %s\n", nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
	...
	if (!group) {
		...
		if( (namerec != NULL)
			...
			DEBUG_OUT (3,("process_name_registration_request: Updated name record %s \
...
void process_node_status_request(struct subnet_record *subrec, struct packet_struct *p)
{
	...
	DEBUG_OUT (3,("process_node_status_request: status request for name %s from IP %s on \
subnet %s.\n", nmb_namestr(&nmb->question.question_name), inet_ntoa(p->ip), subrec->subnet_name));
	...
	if(find_name_on_subnet(subrec, &nmb->question.question_name, FIND_SELF_NAME) == 0) {
		DEBUG_OUT (1,("process_node_status_request: status request for name %s from IP %s on \
...
void process_name_query_request(struct subnet_record *subrec, struct packet_struct *p)
{
	...
	DEBUG_OUT (3,("process_name_query_request: Name query from %s on subnet %s for name %s\n", 
	...
	if (namerec && 
		...
		DEBUG_OUT (5,("process_name_query_request: expired name %s\n", nmb_namestr(&namerec->name)));
		namerec = NULL;
	}
		...
		if (!bcast || 
			...
			if (namerec->data.source == WINS_PROXY_NAME) {
				for( i = 0; i < namerec->data.num_ips; i++) {
					if (same_net_v4(namerec->data.ip[i], subrec->myip, subrec->mask_ip)) {
						DEBUG_OUT (5,("process_name_query_request: name %s is a WINS proxy name and is also on the same subnet (%s) as the requestor. Not replying.\n", 
			...
			if (namerec->data.num_ips == 1) {
				...
				if ((prdata = (char *)SMB_MALLOC( namerec->data.num_ips * 6 )) == NULL) {
					DEBUG_OUT (0,("process_name_query_request: malloc fail !\n"));
					return;
				}
			}
