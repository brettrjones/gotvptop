		       ...
		       struct in_addr ip, bool local, bool servers)
{
	...
	DEBUG_OUT (2,("Initiating browse sync for %s to %s(%s)\n",
			 ...
			 char *sname, uint32 stype, char *comment)
{
	...
	if (!work) {
		DEBUG_OUT (3,("workgroup %s doesn't exist on unicast subnet?\n",
			 s->workgroup));
		return;
	}
...
static void complete_sync(struct sync_record *s)
{
	...
	DEBUG_OUT (2,("sync with %s(%s) for workgroup %s completed (%d records)\n",
