                                   ...
                                   bool force_new_election )
{
	...
	if((work = find_workgroup_on_subnet( subrec, workgroup_name)) == NULL) {
		DEBUG_OUT (0,("reset_workgroup_state: Error - cannot find workgroup %s on \
subnet %s.\n", workgroup_name, subrec->subnet_name ));
		return;
	}

	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("reset_workgroup_state: Error - cannot find server %s \
...
{ 
	...
	DEBUG_OUT (3,("unbecome_local_master_success: released name %s.\n",
                       ...
                       struct nmb_name *fail_name)
{
	...
	DEBUG_OUT (0,("unbecome_local_master_fail: failed to release name %s. \
                             ...
                             bool force_new_election)
{
	...
	if((namerec = find_name_on_subnet( subrec, &nmbname, FIND_SELF_NAME))!=NULL) {
		...
		if((userdata = (struct userdata_struct *)SMB_MALLOC(size)) == NULL) {
			DEBUG_OUT (0,("release_1d_name: malloc fail.\n"));
			return;
		}
...
static void release_msbrowse_name_success(struct subnet_record *subrec,
                      ...
                      struct in_addr released_ip)
{
	DEBUG_OUT (4,("release_msbrowse_name_success: Released name %s on subnet %s\n.",
                       ...
                       struct nmb_name *fail_name)
{
	...
	DEBUG_OUT (4,("release_msbrowse_name_fail: Failed to release name %s on subnet %s\n.",
                                   ...
                                   bool force_new_election)
{
  ...
  /* Sanity check. */
	...
	DEBUG_OUT (2,("unbecome_local_master_browser: unbecoming local master for workgroup %s \
on subnet %s\n",work->work_group, subrec->subnet_name));
  
	if(find_server_in_workgroup( work, global_myname()) == NULL) {
		DEBUG_OUT (0,("unbecome_local_master_browser: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
			global_myname(), work->work_group, subrec->subnet_name));
			work->mst_state = lp_local_master() ? MST_POTENTIAL : MST_NONE;
		return;
                                        ...
                                        int ttl, struct in_addr registered_ip)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_local_master_stage2: Error - cannot find \
workgroup %s on subnet %s\n", regname, subrec->subnet_name));
		return;
	}

	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("become_local_master_stage2: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
	...
	DEBUG_OUT (3,("become_local_master_stage2: registered as master browser for workgroup %s \
on subnet %s\n", work->work_group, subrec->subnet_name));
                                      ...
                                      struct nmb_name *fail_name)
{
	...
	DEBUG_OUT (0,("become_local_master_fail2: failed to register name %s on subnet %s. \
Failed to become a local master browser.\n", nmb_namestr(fail_name), subrec->subnet_name));
	...
	if(!work) {
		DEBUG_OUT (0,("become_local_master_fail2: Error - cannot find \
                                        ...
                                        int ttl, struct in_addr registered_ip)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_local_master_stage1: Error - cannot find \
			%s on subnet %s\n", work_name, subrec->subnet_name));
		return;
	}

	DEBUG_OUT (3,("become_local_master_stage1: go to stage 2: register the %s<1d> name.\n",
		work->work_group));
                                      ...
                                      struct nmb_name *fail_name)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_local_master_fail1: Error - cannot find \
workgroup %s on subnet %s\n", work_name, subrec->subnet_name));
		return;
	}

	if(find_server_in_workgroup(work, global_myname()) == NULL) {
		DEBUG_OUT (0,("become_local_master_fail1: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
			global_myname(), work->work_group, subrec->subnet_name));
		return;
	}
	...
	DEBUG_OUT (0,("become_local_master_fail1: Failed to become a local master browser for \
...
void become_local_master_browser(struct subnet_record *subrec, struct work_record *work)
{
	...
	/* Sanity check. */
	if (!lp_local_master()) { 
		DEBUG_OUT (0,("become_local_master_browser: Samba not configured as a local master browser.\n"));
		return;
	}

	if(!AM_POTENTIAL_MASTER_BROWSER(work)) {
		DEBUG_OUT (2,("become_local_master_browser: Awaiting potential browser state. Current state is %d\n",
			work->mst_state ));
		return;
	}

	if(find_server_in_workgroup( work, global_myname()) == NULL) {
		DEBUG_OUT (0,("become_local_master_browser: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
			global_myname(), work->work_group, subrec->subnet_name));
		return;
	}

	DEBUG_OUT (2,("become_local_master_browser: Starting to become a master browser for workgroup \
%s on subnet %s\n", work->work_group, subrec->subnet_name));
  
	DEBUG_OUT (3,("become_local_master_browser: first stage - attempt to register ^1^2__MSBROWSE__^2^1\n"));
	...
	if((userdata = (struct userdata_struct *)SMB_MALLOC(size)) == NULL) {
		DEBUG_OUT (0,("become_local_master_browser: malloc fail.\n"));
		return;
	}
...
{
	DEBUG_OUT (5,("set_workgroup_local_master_browser_name: setting local master name to '%s' \
for workgroup %s.\n", newname, work->work_group ));
  ...
  {
    DEBUG_OUT (5, ("set_workgroup_local_master_browser_name: Refusing to set \
