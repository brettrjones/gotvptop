                       ...
                       struct response_record *rrec, struct packet_struct *p)
{
	...
	if(!nmb_name_equal(question_name, answer_name)) {
		DEBUG_OUT (0,("node_status_response: Answer name %s differs from question \
name %s.\n", nmb_namestr(answer_name), nmb_namestr(question_name)));
		return;
	}

	DEBUG_OUT (5,("node_status_response: response from name %s on subnet %s.\n",
                       ...
                       struct response_record *rrec)
{
	...
	DEBUG_OUT (5,("node_status_timeout_response: failed to get node status from name %s on subnet %s\n",
                 ...
                 node_status_fail_function fail_fn, struct userdata_struct *userdata)
{
	if(queue_node_status( subrec, node_status_response, node_status_timeout_response,
		...
		DEBUG_OUT (0,("node_status: Failed to send packet trying to get node status for \
