...
bool nmbd_init_packet_server(void)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("ERROR: nb_packet_server_create failed: %s\n",
			  nt_errstr(status)));
		return false;
	}
	return true;
...
static bool send_netbios_packet(struct packet_struct *p)
{
	...
	if(loopback_this_packet) {
		...
		DEBUG_OUT (5,("send_netbios_packet: sending packet to ourselves.\n"));
		if((lo_packet = copy_packet(p)) == NULL)
			return False;
		queue_packet(lo_packet);
	} else if (!send_packet(p)) {
		DEBUG_OUT (0,("send_netbios_packet: send_packet() to IP %s port %d failed\n",
                                                            ...
                                                            struct in_addr to_ip)
{
	...
	if((packet = SMB_MALLOC_P(struct packet_struct)) == NULL) {
		DEBUG_OUT (0,("create_and_init_netbios_packet: malloc fail (1) for packet struct.\n"));
		return NULL;
	}
                                                     ...
                                                     const struct in_addr *register_ip)
{
	...
	if((nmb->additional = SMB_MALLOC_P(struct res_rec)) == NULL) {
		DEBUG_OUT (0,("create_and_init_additional_record: malloc fail for additional record.\n"));
		return False;
	}
...
static bool initiate_name_query_packet( struct packet_struct *packet)
{
	...
	DEBUG_OUT (4,("initiate_name_query_packet: sending query for name %s (bcast=%s) to IP %s\n",
...
{   
	...
	DEBUG_OUT (4,("initiate_name_query_packet_from_wins_server: sending query for name %s (bcast=%s) to IP %s\n",
		nmb_namestr(&nmb->question.question_name),
		BOOLSTR(nmb->header.nm_flags.bcast), inet_ntoa(packet->ip)));
                                    ...
                                    uint16 nb_flags, const struct in_addr *register_ip)
{
	...
	DEBUG_OUT (4,("initiate_name_register_packet: sending registration for name %s (bcast=%s) to IP %s\n",
						     ...
						     uint16 nb_flags, struct in_addr *register_ip)
{
	...
	DEBUG_OUT (4,("initiate_multihomed_name_register_packet: sending registration \
                                   ...
                                   uint16 nb_flags, struct in_addr *refresh_ip)
{
	...
	DEBUG_OUT (4,("initiate_name_refresh_packet: sending refresh for name %s (bcast=%s) to IP %s\n",
		nmb_namestr(&nmb->additional->rr_name),
		BOOLSTR(nmb->header.nm_flags.bcast), inet_ntoa(packet->ip)));
                                   ...
                                   uint16 nb_flags, struct in_addr *release_ip)
{
	...
	DEBUG_OUT (4,("initiate_name_release_packet: sending release for name %s (bcast=%s) to IP %s\n",
		nmb_namestr(&nmb->additional->rr_name),
		BOOLSTR(nmb->header.nm_flags.bcast), inet_ntoa(packet->ip)));
...
static bool initiate_node_status_packet( struct packet_struct *packet )
{
	...
	DEBUG_OUT (4,("initiate_node_status_packet: sending node status request for name %s to IP %s\n",
		nmb_namestr(&nmb->question.question_name),
		inet_ntoa(packet->ip)));
...
static bool assert_check_subnet(struct subnet_record *subrec)
{
	if( subrec == remote_broadcast_subnet) {
		DEBUG_OUT (0,("assert_check_subnet: Attempt to send packet on remote broadcast subnet. \
This is a bug.\n"));
		return True;
	}
	return False;
			...
			const char *tag)
{
	...
	DEBUG_OUT (6,("Refreshing name %s IP %s with WINS server %s using tag '%s'\n",
	...
	if (!userdata) {
		p->locked = False;
		free_packet(p);
		DEBUG_OUT (0,("Failed to allocate userdata structure!\n"));
		return;
	}
							...
							struct in_addr wins_ip)
{
	...
	if(subrec != unicast_subnet) {
		DEBUG_OUT (0,("queue_register_multihomed_name: should only be done on \
                          ...
                          struct nmb_name *nmbname)
{
	...
	if(lp_bind_interfaces_only()) {
		...
		DEBUG_OUT (10,("queue_query_name: bind_interfaces_only is set, looking for suitable source IP\n"));
		for(i = 0; i < iface_count(); i++) {
			const struct in_addr *ifip = iface_n_ip_v4(i);

			if (ifip == NULL) {
				DEBUG_OUT (0,("queue_query_name: interface %d has NULL IP address !\n", i));
				continue;
			}

			if (is_loopback_ip_v4(*ifip)) {
				DEBUG_OUT (5,("queue_query_name: ignoring loopback interface (%d)\n", i));
				continue;
			}

			DEBUG_OUT (10,("queue_query_name: using source IP %s\n",inet_ntoa(*ifip)));
                          ...
                          int ttl, char *data,int len)
{
	...
	switch (rcv_code) {
		...
		case WINS_QUERY:
			...
			break;
		default:
			DEBUG_OUT (0,("reply_netbios_packet: Unknown packet type: %s %s to ip %s\n",
				packet_type, nmb_namestr(&orig_nmb->question.question_name),
				inet_ntoa(packet.ip)));
			return;
	}

	DEBUG_OUT (4,("reply_netbios_packet: sending a reply of packet type: %s %s to ip %s \
for id %hu\n", packet_type, nmb_namestr(&orig_nmb->question.question_name),
	...
	if (data && len) {
		if (len < 0 || len > sizeof(nmb->answers->rdata)) {
			DEBUG_OUT (5,("reply_netbios_packet: "
	...
	if(loopback_this_packet) {
		...
		DEBUG_OUT (5,("reply_netbios_packet: sending packet to ourselves.\n"));
		if((lo_packet = copy_packet(&packet)) == NULL)
			return;
		queue_packet(lo_packet);
	} else if (!send_packet(&packet)) {
		DEBUG_OUT (0,("reply_netbios_packet: send_packet to IP %s port %d failed\n",
...
static void process_browse_packet(struct packet_struct *p, const char *buf,int len)
{
	...
	if (!strequal(scope, global_scope())) {
		DEBUG_OUT (7,("process_browse_packet: Discarding datagram from IP %s. Scope (%s) \
mismatch with our scope (%s).\n", inet_ntoa(p->ip), scope, global_scope()));
	...
	if (is_myname(src_name)) {
		DEBUG_OUT (7,("process_browse_packet: Discarding datagram from IP %s. Source name \
%s is one of our names !\n", inet_ntoa(p->ip), nmb_namestr(&dgram->source_name)));
	...
	switch (command) {
		...
		case ANN_GetBackupListResp:
			...
			DEBUG_OUT (0,("process_browse_packet: Discarding GetBackupListResponse \
packet from %s IP %s\n", nmb_namestr(&dgram->source_name), inet_ntoa(p->ip)));
		...
		case ANN_BecomeBackup:
			 ...
			 */
			debug_browse_data(buf, len);
			DEBUG_OUT (10,("process_browse_packet: On subnet %s ignoring browse packet \
command ANN_BecomeBackup from %s IP %s to %s\n", subrec->subnet_name, nmb_namestr(&dgram->source_name),
					inet_ntoa(p->ip), nmb_namestr(&dgram->dest_name)));
			break;
		default:
			debug_browse_data(buf, len);
			DEBUG_OUT (0,("process_browse_packet: On subnet %s ignoring browse packet \
...
static void process_lanman_packet(struct packet_struct *p, const char *buf,int len)
{
	...
	pull_ascii(scope, dgram->dest_name.scope, 64, 64, STR_TERMINATE);
	if (!strequal(scope, global_scope())) {
		DEBUG_OUT (7,("process_lanman_packet: Discarding datagram from IP %s. Scope (%s) \
mismatch with our scope (%s).\n", inet_ntoa(p->ip), scope, global_scope()));
		return;
	}
	...
	pull_ascii_nstring(src_name, sizeof(src_name), dgram->source_name.name);
	if (is_myname(src_name)) {
		DEBUG_OUT (0,("process_lanman_packet: Discarding datagram from IP %s. Source name \
%s is one of our names !\n", inet_ntoa(p->ip), nmb_namestr(&dgram->source_name)));
		return;
	}
	...
	switch (command) {
		...
		case ANN_AnnouncementRequest:
			...
			break;
		default:
			DEBUG_OUT (0,("process_lanman_packet: On subnet %s ignoring browse packet \
command code %d from %s IP %s to %s\n", subrec->subnet_name, command, nmb_namestr(&dgram->source_name),
				inet_ntoa(p->ip), nmb_namestr(&dgram->dest_name)));
			break;
	}
...
static void process_dgram(struct packet_struct *p)
{
	...
	if (!listening(p,&dgram->dest_name)) {
			...
			DEBUG_OUT (5,("process_dgram: ignoring dgram packet sent to name %s from %s\n",
	...
	if (dgram->header.msg_type != 0x10 && dgram->header.msg_type != 0x11 && dgram->header.msg_type != 0x12) {
		...
		DEBUG_OUT (5,("process_dgram: ignoring dgram packet sent to name %s from IP %s as it is \
an error packet of type %x\n", nmb_namestr(&dgram->dest_name), inet_ntoa(p->ip), dgram->header.msg_type));
	...
	if (dgram->datasize < (smb_vwv12 - 2)) {
		...
		DEBUG_OUT (0,("process_dgram: ignoring too short dgram packet (%u) sent to name %s from IP %s\n",
	...
	if (len <= 0 || len > dgram->datasize) {
		DEBUG_OUT (0,("process_dgram: ignoring malformed1 (datasize = %d, len = %d) datagram \
packet sent to name %s from IP %s\n",
	...
	if (buf2 < dgram->data || (buf2 >= dgram->data + dgram->datasize)) {
		DEBUG_OUT (0,("process_dgram: ignoring malformed2 (datasize = %d, len=%d, off=%d) datagram \
packet sent to name %s from IP %s\n",
			dgram->datasize,
			len,
	...
	if ((buf2 + len < dgram->data) || (buf2 + len > dgram->data + dgram->datasize)) {
		DEBUG_OUT (0,("process_dgram: ignoring malformed3 (datasize = %d, len=%d, off=%d) datagram \
packet sent to name %s from IP %s\n",
			dgram->datasize,
			len,
			(int)PTR_DIFF(buf2, dgram->data),
	...
	DEBUG_OUT (4,("process_dgram: datagram from %s to %s IP %s for %s of type %d len=%d\n",
	...
	switch (nmb->header.opcode) {
		...
		case NMB_NAME_REFRESH_OPCODE_9: /* WinNT uses 8 by default. */
			if (nmb->header.ancount == 0) {
				DEBUG_OUT (0,("validate_nmb_response_packet: Bad REG/REFRESH Packet. "));
				ignore = True;
			}
			break;
		...
		case NMB_NAME_QUERY_OPCODE:
			if ((nmb->header.ancount != 0) && (nmb->header.ancount != 1)) {
				DEBUG_OUT (0,("validate_nmb_response_packet: Bad QUERY Packet. "));
				ignore = True;
			}
			break;
		...
		case NMB_NAME_RELEASE_OPCODE:
			if (nmb->header.ancount == 0) {
				DEBUG_OUT (0,("validate_nmb_response_packet: Bad RELEASE Packet. "));
				ignore = True;
			}
			break;
		...
		case NMB_WACK_OPCODE:
			...
			if (nmb->header.ancount != 1) {
				DEBUG_OUT (0,("validate_nmb_response_packet: Bad WACK Packet. "));
				ignore = True;
			}
			break;
		default:
			DEBUG_OUT (0,("validate_nmb_response_packet: Ignoring packet with unknown opcode %d.\n",
					nmb->header.opcode));
			return True;
	}

	if(ignore)
		DEBUG_OUT (0,("Ignoring response packet with opcode %d.\n", nmb->header.opcode));
	...
	switch (nmb->header.opcode) {
		...
		case NMB_NAME_MULTIHOMED_REG_OPCODE:
			if (nmb->header.qdcount==0 || nmb->header.arcount==0) {
				DEBUG_OUT (0,("validate_nmb_packet: Bad REG/REFRESH Packet. "));
				ignore = True;
			}
			break;
		...
		case NMB_NAME_QUERY_OPCODE:
			if ((nmb->header.qdcount == 0) || ((nmb->question.question_type != QUESTION_TYPE_NB_QUERY) &&
					(nmb->question.question_type != QUESTION_TYPE_NB_STATUS))) {
				DEBUG_OUT (0,("validate_nmb_packet: Bad QUERY Packet. "));
				ignore = True;
			}
			break;
		...
		case NMB_NAME_RELEASE_OPCODE:
			if (nmb->header.qdcount==0 || nmb->header.arcount==0) {
				DEBUG_OUT (0,("validate_nmb_packet: Bad RELEASE Packet. "));
				ignore = True;
			}
			break;
		default:
			DEBUG_OUT (0,("validate_nmb_packet: Ignoring packet with unknown opcode %d.\n",
				nmb->header.opcode));
			return True;
	}

	if(ignore)
		DEBUG_OUT (0,("validate_nmb_packet: Ignoring request packet with opcode %d.\n", nmb->header.opcode));
                                                         ...
                                                         struct response_record **pprrec)
{
	...
	if(nmb->header.response) {
		...
		if(rrec == NULL) {
			DEBUG_OUT (3,("find_subnet_for_nmb_packet: response record not found for response id %hu\n",
		...
		if(subrec == NULL) {
			DEBUG_OUT (0,("find_subnet_for_nmb_packet: subnet record not found for response id %hu\n",
				nmb->header.name_trn_id));
			return NULL;
		}
	...
	switch (nmb->header.opcode) {
		...
		case NMB_NAME_MULTIHOMED_REG_OPCODE:
			if(subrec == wins_server_subnet) {
				...
				DEBUG_OUT (0,("process_nmb_request: Multihomed registration request must be \
		...
		case NMB_NAME_QUERY_OPCODE:
			switch (nmb->question.question_type) {
				...
				case QUESTION_TYPE_NB_STATUS:
					if(subrec == wins_server_subnet) {
						DEBUG_OUT (0,("process_nmb_request: NB_STATUS request directed at WINS server is \
not allowed.\n"));
						break;
					} else {
...
static void process_nmb_response(struct packet_struct *p)
{
	...
	if(rrec == NULL) {
		DEBUG_OUT (0,("process_nmb_response: response packet received but no response record \
			...
			if (rrec->repeat_time <= t) {
				if (rrec->repeat_count > 0) {
					...
					if(!send_packet(rrec->packet)) {
						DEBUG_OUT (0,("retransmit_or_expire_response_records: Failed to resend packet id %hu \
to IP %s on subnet %s\n", rrec->response_id, inet_ntoa(rrec->packet->ip), subrec->subnet_name));
					}
					rrec->repeat_time = t + rrec->repeat_interval;
					rrec->repeat_count--;
				} else {
					DEBUG_OUT (4,("retransmit_or_expire_response_records: timeout for packet id %hu to IP %s \
on subnet %s\n", rrec->response_id, inet_ntoa(rrec->packet->ip), subrec->subnet_name));
				  ...
				  int *pnum_sockets)
{
	...
	if (fds == NULL) {
		DEBUG_OUT (1, ("create_listen_pollfds: malloc fail for fds. "
			  "size %d\n", count));
		return true;
	}
	...
	if (fds == NULL) {
		DEBUG_OUT (1, ("create_listen_pollfds: malloc fail for attrs. "
			  "size %d\n", count));
		SAFE_FREE(fds);
		return true;
	}
...
bool listen_for_packets(bool run_election)
{
	...
	if ((fds == NULL) || rescan_listen_set) {
		if (create_listen_pollfds(&fds, &attrs, &listen_number)) {
			DEBUG_OUT (0,("listen_for_packets: Fatal error. unable to create listen set. Exiting.\n"));
			return True;
		}
	...
	for(i = 0; i < listen_number; i++) {
		...
		if (lp_bind_interfaces_only() &&
			...
			DEBUG_OUT (7,("discarding %s packet sent to broadcast socket from %s:%d\n",
		    ...
		    packet->port == client_port)
		{
			if (client_port == DGRAM_PORT) {
				DEBUG_OUT (7,("discarding own dgram packet from %s:%d\n",
					inet_ntoa(packet->ip),packet->port));
				free_packet(packet);
				continue;
			}
			...
			if (packet->packet.nmb.header.nm_flags.bcast) {
				DEBUG_OUT (7,("discarding own nmb bcast packet from %s:%d\n",
					inet_ntoa(packet->ip),packet->port));
				free_packet(packet);
				continue;
			}
		...
		if (is_processed_packet(processed_packet_list, packet)) {
			DEBUG_OUT (7,("discarding duplicate packet from %s:%d\n",
				inet_ntoa(packet->ip),packet->port));
			free_packet(packet);
			continue;
		}
		   ...
		   int dest_port)
{
	...
	if (smb_size + 17*2 + strlen(mailslot) + 1 + len > MAX_DGRAM_SIZE) {
		DEBUG_OUT (0, ("send_mailslot: Cannot write beyond end of packet\n"));
		return false;
	}
	...
	DEBUG_OUT (4,("send_mailslot: Sending to mailslot %s from %s IP %s ", mailslot,
			nmb_namestr(&dgram->source_name), inet_ntoa(src_ip)));
	DEBUG_OUT (4,("to %s IP %s\n", nmb_namestr(&dgram->dest_name), inet_ntoa(dest_ip)));

	debug_browse_data(buf, len);

	if(loopback_this_packet) {
		struct packet_struct *lo_packet = NULL;
		DEBUG_OUT (5,("send_mailslot: sending packet to ourselves.\n"));
		if((lo_packet = copy_packet(&p)) == NULL)
			return False;
		queue_packet(lo_packet);
		return True;
