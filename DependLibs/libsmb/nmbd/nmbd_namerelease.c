				  ...
				  struct response_record *rrec, struct packet_struct *p)
{
	...
	if (!nmb_name_equal(question_name, answer_name)) {
		DEBUG_OUT (0,("release_name_response: Answer name %s differs from question name %s.\n", 
	...
	if (nmb->header.rcode != 0) {
		...
		DEBUG_OUT (0,("release_name_response: WINS server at IP %s rejected our \
name release of name %s with error code %d.\n", 
		...
		DEBUG_OUT (5,("release_name_response: WACK from WINS server %s in releasing \
name %s on subnet %s.\n", 
	...
	DEBUG_OUT (5,("release_name_response: %s in releasing name %s on subnet %s.\n",
					  ...
					  struct response_record *rrec)
{
	...
	DEBUG_OUT (5,("release_name_timeout_response: success in releasing name %s on subnet %s.\n",
	...
	for (t=0;wins_tags && wins_tags[t]; t++) {
		for (i = 0; i < namerec->data.num_ips; i++) {
			...
			if (queue_release_name(unicast_subnet,
					       ...
					       namerec->data.ip[i],
					       wins_ip) == NULL) {
				DEBUG_OUT (0,("release_name: Failed to send packet trying to release name %s IP %s\n",
		  ...
		  struct userdata_struct *userdata)
{
	...
	if ((namerec->data.source != SELF_NAME) || !NAME_IS_ACTIVE(namerec)) {
		DEBUG_OUT (0,("release_name: Cannot release name %s from subnet %s. Source was %d \n",
