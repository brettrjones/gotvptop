				...
				struct response_record *rrec)
{
	...
	DEBUG_OUT (4,("add_response_record: adding response record id:%hu to subnet %s. num_records:%d\n",
					      ...
					      struct userdata_struct *userdata)
{
	...
	if (!(rrec = SMB_MALLOC_P(struct response_record))) {
		DEBUG_OUT (0,("make_response_queue_record: malloc fail for response_record.\n"));
		return NULL;
	}
	...
	if(userdata) {
		...
		if(userdata->copy_fn) {
			if((rrec->userdata = (*userdata->copy_fn)(userdata)) == NULL) {
				DEBUG_OUT (0,("make_response_queue_record: copy fail for userdata.\n"));
				ZERO_STRUCTP(rrec);
				SAFE_FREE(rrec);
				return NULL;
			}
			...
			if((rrec->userdata = (struct userdata_struct *)
				...
				DEBUG_OUT (0,("make_response_queue_record: malloc fail for userdata.\n"));
				ZERO_STRUCTP(rrec);
				SAFE_FREE(rrec);
				return NULL;
			}
...
{  
	...
	for (rrec = subrec->responselist; rrec; rrec = rrec->next) {
		if (rrec->response_id == id) {
			DEBUG_OUT (4, ("find_response_record: found response record id = %hu on subnet %s\n",
...
{  
	...
	if(remote_broadcast_subnet->responselist != NULL) {
		DEBUG_OUT (0,("find_response_record: response record found on subnet %s. This should \
never happen !\n", remote_broadcast_subnet->subnet_name));
	...
	DEBUG_OUT (3,("find_response_record: response packet id %hu received with no \
matching record.\n", id));
