                                      ...
                                      struct nmb_name *fail_name)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_domain_master_fail: Error - cannot find \
workgroup %s on subnet %s\n", failname, subrec->subnet_name));
	...
	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("become_domain_master_fail: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
	...
	DEBUG_OUT (0,("become_domain_master_fail: Failed to become a domain master browser for \
                                        ...
                                        int ttl, struct in_addr registered_ip)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_domain_master_stage2: Error - cannot find \
workgroup %s on subnet %s\n", regname, subrec->subnet_name));
		return;
	}

	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("become_domain_master_stage2: Error - cannot find server %s \
in workgroup %s on subnet %s\n", 
	...
	if( subrec == unicast_subnet ) {
		...
		if (!nip) {
			DEBUG_OUT (0,("become_domain_master_stage2: "
...
{ 
	...
	DEBUG_OUT (2,("become_domain_master_stage1: Becoming domain master browser for \
workgroup %s on subnet %s\n", wg_name, subrec->subnet_name));

	/* First, find the workgroup on the subnet. */
	if((work = find_workgroup_on_subnet( subrec, wg_name )) == NULL) {
		DEBUG_OUT (0,("become_domain_master_stage1: Error - unable to find workgroup %s on subnet %s.\n",
			wg_name, subrec->subnet_name));
		return;
	}

	DEBUG_OUT (3,("become_domain_master_stage1: go to first stage: register <1b> name\n"));
                                    ...
                                    struct nmb_name *question_name, int fail_code)
{
	...
	if((subrec == unicast_subnet) && (fail_code != NAM_ERR)) {
		DEBUG_OUT (0,("become_domain_master_query_fail: Error %d returned when \
		...
		if (work && (work->dom_state == DOMAIN_NONE)) {
			...
			if (find_name_on_subnet(subrec, &nmbname, FIND_SELF_NAME) == NULL) {
				...
				*/
				...
				DEBUG_OUT (0,("become_domain_master_browser_bcast: querying subnet %s \
	...
	if (work && (work->dom_state == DOMAIN_NONE)) {
		...
		if (find_name_on_subnet(unicast_subnet, &nmbname, FIND_SELF_NAME) == NULL) {
			...
			*/
			...
			DEBUG_OUT (0,("become_domain_master_browser_wins: querying WINS server from IP %s \
