				       ...
				       void *private_data)
{
	...
	DEBUG_OUT (10, ("delayed_init_logon_handler (%lx): re-queuing packet.\n",
		   (unsigned long)te));
...
bool initialize_nmbd_proxy_logon(void)
{
	...
	if (!server_str || strcmp("0.0.0.0", server_str) == 0) {
		DEBUG_OUT (0,("Failed to resolve[%s] for nmbd_proxy_logon\n",
			 cldap_server));
		return false;
	}
	...
	if (ret != 0) {
		...
		DEBUG_OUT (0,("Failed to create cldap tsocket_address for %s - %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		TALLOC_FREE(ctx);
		DEBUG_OUT (0,("failed to create cldap socket for %s: %s\n",
			server_str, nt_errstr(status)));
		return false;
	}
			     ...
			     uint32_t len)
{
	...
	if (!state) {
		DEBUG_OUT (0,("failed to allocate nmbd_proxy_logon_state\n"));
		return;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		...
		DEBUG_OUT (0,("failed parse nbt_netlogon_packet: %s\n",
			nt_errstr(status)));
		TALLOC_FREE(state);
		return;
	}
	...
	case LOGON_SAM_LOGON_REQUEST:
		...
		if (state->req.req.logon.sid_size > 0) {
			...
			if (!domain_sid) {
				DEBUG_OUT (0,("failed to get a string for sid\n"));
				TALLOC_FREE(state);
				return;
			}
		}
	...
	subreq = cldap_netlogon_send(state,
				     ...
				     &state->io);
	if (!subreq) {
		DEBUG_OUT (0,("failed to send cldap netlogon call\n"));
		TALLOC_FREE(state);
		return;
	}
...
static void nmbd_proxy_logon_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("failed to recv cldap netlogon call: %s\n",
			nt_errstr(status)));
		TALLOC_FREE(state);
		return;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("failed to push netlogon_samlogon_response: %s\n",
			nt_errstr(status)));
		TALLOC_FREE(state);
		return;
	}
                          ...
                          const char *mailslot)
{
	...
	if (!pss) {
		DEBUG_OUT (5,("process_logon_packet:can't find outgoing interface "
	...
	if (!lp_domain_logons()) {
		DEBUG_OUT (5,("process_logon_packet: Logon packet received from IP %s and domain \
logons are not enabled.\n", inet_ntoa(p->ip) ));
	...
	ndr_err = ndr_pull_struct_blob(&blob_in, talloc_tos(), &request,
		(ndr_pull_flags_fn_t)ndr_pull_nbt_netlogon_packet);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (1,("process_logon_packet: Failed to pull logon packet\n"));
		return;
	}
	...
	DEBUG_OUT (4,("process_logon_packet: Logon from %s: code = 0x%x\n",
	...
	case LOGON_REQUEST: {
		...
		DEBUG_OUT (5,("process_logon_packet: Domain login request from %s at IP %s user=%s token=%x\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("process_logon_packet: failed to push packet\n"));
			return;
		}
	...
	case LOGON_PRIMARY_QUERY: {
		...
		DEBUG_OUT (5,("process_logon_packet: GETDC request from %s at IP %s, "
	...
	case LOGON_SAM_LOGON_REQUEST: {
		...
		if (source_addr == NULL) {
			DEBUG_OUT (3, ("out of memory copying client"
				  " address string\n"));
			return;
		}

		DEBUG_OUT (5,("process_logon_packet: LOGON_SAM_LOGON_REQUEST request from %s(%s) for %s, returning logon svr %s domain %s code %x token=%x\n",
		...
		if ((request.req.logon.nt_version < (NETLOGON_NT_VERSION_1 | NETLOGON_NT_VERSION_5 | NETLOGON_NT_VERSION_5EX_WITH_IP)) ||
			...
			if (!domain) {
				DEBUG_OUT (2,("get_mydnsdomname failed.\n"));
				return;
			}

			hostname = get_mydnsfullname();
			if (!hostname) {
				DEBUG_OUT (2,("get_mydnsfullname failed.\n"));
				return;
			}

			if (!secrets_fetch_domain_guid(domain, &domain_guid)) {
				DEBUG_OUT (2,("Could not fetch DomainGUID for %s\n", domain));
				return;
			}
		    ...
		    delay_logon(source_name, source_addr))
		{
			...
			DEBUG_OUT (3, ("process_logon_packet: "
	...
	case NETLOGON_ANNOUNCE_UAS:
		DEBUG_OUT (5, ("Got NETLOGON_ANNOUNCE_UAS\n"));
		break;

	default:
		DEBUG_OUT (3,("process_logon_packet: Unknown domain request %d\n",
			request.command));
		return;
	}
}
