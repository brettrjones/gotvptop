...
{
	DEBUG_OUT (0,("my_name_register_failed: Failed to register my name %s on subnet %s.\n",
...
void register_my_workgroup_one_subnet(struct subnet_record *subrec)
{
	...
	if((work = create_workgroup_on_subnet(subrec, lp_workgroup(), 
					      PERMANENT_TTL)) == NULL) {
		DEBUG_OUT (0,("register_my_workgroup_and_names: Failed to create my workgroup %s on subnet %s. \
