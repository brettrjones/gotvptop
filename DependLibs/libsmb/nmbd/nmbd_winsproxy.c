                        ...
                        struct nmb_name *nmbname, struct in_addr ip, struct res_rec *rrec)
{
	...
	if(num_ips == 0) {
		DEBUG_OUT (0,("wins_proxy_name_query_request_success: Invalid number of IP records (0) \
returned for name %s.\n", nmb_namestr(nmbname) ));
		return;
	}
	...
	if(num_ips == 1) {
		...
		if((iplist = SMB_MALLOC_ARRAY( struct in_addr, num_ips )) == NULL) {
			DEBUG_OUT (0,("wins_proxy_name_query_request_success: malloc fail !\n"));
			return;
		}
	...
	if (!namerec) {
		DEBUG_OUT (0,("wins_proxy_name_query_request_success: failed to add "
			"name %s to subnet %s !\n",
			name,
	...
	if(namerec && original_packet->packet.nmb.header.nm_flags.bcast) {
		for( i = 0; i < namerec->data.num_ips; i++) {
			if( same_net_v4( namerec->data.ip[i], orig_broadcast_subnet->myip,
				...
				DEBUG_OUT ( 5, ( "wins_proxy_name_query_request_success: name %s is a WINS \
...
{
	DEBUG_OUT (4,("wins_proxy_name_query_request_fail: WINS server returned error code %d for lookup \
