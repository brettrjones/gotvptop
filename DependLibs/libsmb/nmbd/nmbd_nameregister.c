                       ...
                       struct response_record *rrec, struct packet_struct *p)
{
	...
	if(!question_name || !answer_name) {
		DEBUG_OUT (0,("register_name_response: malformed response (%s is NULL).\n",
			 question_name ? "question_name" : "answer_name" ));
		return;
	}

	if(!nmb_name_equal(question_name, answer_name)) {
		DEBUG_OUT (0,("register_name_response: Answer name %s differs from question name %s.\n", 
	...
	if(bcast) {
		...
		if((nmb->header.rcode == ACT_ERR) && strequal(lp_workgroup(), ans_name) &&
		   (answer_name->name_type == 0x1b)) {
			...
			DEBUG_OUT (5,("register_name_response: Ignoring broadcast response to registration of name %s due to old Samba server bug.\n", 
		...
		DEBUG_OUT (1,("register_name_response: Failed to register name %s IP %s on subnet %s via broadcast. Error code was %d. Reject came from IP %s\n", 
			 nmb_namestr(answer_name), 
			 reg_name,
		...
		if (!ip_equal_v4(rrec->packet->ip, p->ip)) {
			DEBUG_OUT (5,("register_name_response: Ignoring WINS server response "
		...
		if (nmb->header.opcode == NMB_WACK_OPCODE) {
			...
			DEBUG_OUT (5,("register_name_response: WACK from WINS server %s in registering name %s IP %s\n", 
			...
			DEBUG_OUT (0,("register_name_response: %sserver at IP %s rejected our name registration of %s IP %s with error code %d.\n", 
	...
	DEBUG_OUT (5,("register_name_response: %s in registering %sname %s IP %s with %s.\n",
				      ...
				      struct response_record *rrec)
{
	...
	DEBUG_OUT (2,("wins_registration_timeout: WINS server %s timed out registering IP %s\n", 
	...
	DEBUG_OUT (6,("Retrying register of name %s IP %s with WINS server %s\n",
					   ...
					   struct response_record *rrec)
{
	...
	DEBUG_OUT (5,("register_name_timeout_response: %s in registering name %s on subnet %s.\n",
				    ...
				    const char *tag)
{
	...
	DEBUG_OUT (6,("Registering name %s IP %s with WINS server %s using tag '%s'\n",
	...
	if (queue_register_multihomed_name(unicast_subnet,
					   ...
					   ip,
					   wins_ip) == NULL) {
		DEBUG_OUT (0,("multihomed_register_one: Failed to send packet trying to register name %s IP %s\n", 
				     ...
				     register_name_fail_function fail_fn)
{
	...
	*/
	...
	if((ip_list = SMB_MALLOC_ARRAY(struct in_addr, num_ips))==NULL) {
		DEBUG_OUT (0,("multihomed_register_name: malloc fail !\n"));
		return;
	}
                   ...
                   struct userdata_struct *userdata)
{
	...
	push_ascii_nstring(nname, name);
		...
		DEBUG_OUT (0,("register_name: NetBIOS name %s is too long. Truncating to %s\n",
			name, tname));
	...
	*/
	...
	if (queue_register_name(subrec,
				...
				&nmbname,
				nb_flags) == NULL) {
		DEBUG_OUT (0,("register_name: Failed to send packet trying to register name %s\n",
			 nmb_namestr(&nmbname)));
	}
}
