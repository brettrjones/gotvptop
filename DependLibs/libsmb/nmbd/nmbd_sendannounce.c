...
void send_browser_reset(int reset_type, const char *to_name, int to_type, struct in_addr to_ip)
{
	...
	DEBUG_OUT (3,("send_browser_reset: sending reset request type %d to %s<%02x> IP %s.\n",
...
void broadcast_announce_request(struct subnet_record *subrec, struct work_record *work)
{
	...
	DEBUG_OUT (3,("broadcast_announce_request: sending announce request for workgroup %s \
                                           ...
                                           struct server_record *servrec)
{
	...
	DEBUG_OUT (3,("send_local_master_announcement: type %x for name %s on subnet %s for workgroup %s\n",
...
{
	DEBUG_OUT (3,("send_workgroup_announcement: on subnet %s for workgroup %s\n",
                                   ...
                                   struct server_record *servrec)
{
	...
	DEBUG_OUT (3,("send_host_announcement: type %x for host %s on subnet %s for workgroup %s\n",
                                   ...
                                   struct server_record *servrec, int lm_interval)
{
	...
	DEBUG_OUT (3,("send_lm_host_announcement: type %x for host %s on subnet %s for workgroup %s, ttl: %d\n",
...
void announce_myself_to_domain_master_browser(time_t t)
{
	...
	if(!we_are_a_wins_client()) {
		DEBUG_OUT (10,("announce_myself_to_domain_master_browser: no unicast subnet, ignoring.\n"));
		return;
	}
	...
	if ((t-announce_timer_last) < (CHECK_TIME_MST_ANNOUNCE * 60)) {
		DEBUG_OUT (10,("announce_myself_to_domain_master_browser: t (%d) - last(%d) < %d\n",
	...
	for (subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_EXCLUDING_UNICAST(subrec)) {
		for (work = subrec->workgrouplist; work; work = work->next) {
			if (AM_LOCAL_MASTER_BROWSER(work)) {
				DEBUG_OUT (4,( "announce_myself_to_domain_master_browser: I am a local master browser for \
...
void announce_remote(time_t t)
{
	...
	for (ptr=s; next_token_talloc(frame,&ptr,&s2,NULL); ) {
		...
		for(i=0; my_netbios_names(i); i++) {
			...
			DEBUG_OUT (5,("announce_remote: Doing remote announce for server %s to IP %s.\n",
				name, inet_ntoa(addr) ));
...
void browse_sync_remote(time_t t)
{
	...
	if((work = find_workgroup_on_subnet(FIRST_SUBNET, lp_workgroup())) == NULL) {   
		DEBUG_OUT (0,("browse_sync_remote: Cannot find workgroup %s on subnet %s\n",
			lp_workgroup(), FIRST_SUBNET->subnet_name ));
		return;
	}

	if(!AM_LOCAL_MASTER_BROWSER(work)) {
		DEBUG_OUT (5,("browse_sync_remote: We can only do this if we are a local master browser \
for workgroup %s on subnet %s.\n", lp_workgroup(), FIRST_SUBNET->subnet_name ));
	...
	for (ptr=s; next_token_talloc(frame,&ptr,&s2,NULL); ) {
		...
		DEBUG_OUT (5,("announce_remote: Doing remote browse sync announce for server %s to IP %s.\n",
