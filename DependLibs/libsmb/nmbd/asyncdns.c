...
static struct name_record *add_dns_result(struct nmb_name *question, struct in_addr addr)
{
	...
	if (!addr.s_addr) {
		...
		DEBUG_OUT (3,("add_dns_result: Negative DNS answer for %s\n", qname));
	...
	DEBUG_OUT (3,("add_dns_result: DNS gave answer for %s of %s\n", qname, inet_ntoa(addr)));
...
void start_async_dns(void)
{
	...
	if (pipe(fd1) || pipe(fd2)) {
		DEBUG_OUT (0,("can't create asyncdns pipes\n"));
		return;
	}
	...
	if (child_pid) {
		...
		close(fd1[1]);
		close(fd2[0]);
		DEBUG_OUT (0,("started asyncdns process %d\n", (int)child_pid));
		return;
	}
...
void run_dns_queue(void)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("read from child failed: %s\n", nt_errstr(status)));
		fd_in = -1;
		return;
	}
	...
	if (dns_current) {
		if (query_current(&r)) {
			DEBUG_OUT (3,("DNS calling send_wins_name_query_response\n"));
			in_dns = 1;
			if(namerec == NULL)
	...
	if (dns_queue) {
		...
		if (!write_child(dns_current)) {
			DEBUG_OUT (3,("failed to send DNS query to child!\n"));
			return;
		}
	}
}
...
bool queue_dns_query(struct packet_struct *p,struct nmb_name *question)
{
	...
	DEBUG_OUT (3,("added DNS query for %s\n", nmb_namestr(question)));
	return True;
}
...
bool queue_dns_query(struct packet_struct *p,struct nmb_name *question)
{
	...
	DEBUG_OUT (3,("DNS search for %s - ", nmb_namestr(question)));
