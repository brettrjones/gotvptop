					 ...
					 struct in_addr mask_ip)
{
	...
	if (type == NORMAL_SUBNET) {
		...
		nmb_sock = open_socket_in(SOCK_DGRAM, global_nmb_port,
					  0, &ss, true);
		if (nmb_sock == -1) {
			DEBUG_OUT (0,   ("nmbd_subnetdb:make_subnet()\n"));
	...
	if (!subrec) {
		DEBUG_OUT (0,("make_subnet: malloc fail !\n"));
		goto failed;
	}
	...
	if((subrec->subnet_name = SMB_STRDUP(name)) == NULL) {
		DEBUG_OUT (0,("make_subnet: malloc fail for subnet name !\n"));
		goto failed;
	}

	DEBUG_OUT (2, ("making subnet name:%s ", name ));
	DEBUG_OUT (2, ("Broadcast address:%s ", inet_ntoa(bcast_ip)));
	DEBUG_OUT (2, ("Subnet mask:%s\n", inet_ntoa(mask_ip)));
...
bool create_subnets(void)
{
  ...
  try_interfaces_again:
	...
	if (iface_count_v4_nl() == 0) {
		DEBUG_OUT (0,("create_subnets: No local IPv4 non-loopback interfaces !\n"));
		DEBUG_OUT (0,("create_subnets: Waiting for an interface to appear ...\n"));
	}
	...
	for (i = 0 ; i < num_interfaces; i++) {
		...
		if (!iface) {
			DEBUG_OUT (2,("create_subnets: can't get interface %d.\n", i ));
			continue;
		}

		/* Ensure we're only dealing with IPv4 here. */
		if (iface->ip.ss_family != AF_INET) {
			DEBUG_OUT (2,("create_subnets: "
		...
		if (is_loopback_addr((struct sockaddr *)&iface->ip)) {
			DEBUG_OUT (2,("create_subnets: Ignoring loopback interface.\n" ));
			continue;
		}
	...
	if (subnetlist == NULL) {
		...
		DEBUG_OUT (0,("create_subnets: Unable to create any subnet from "
