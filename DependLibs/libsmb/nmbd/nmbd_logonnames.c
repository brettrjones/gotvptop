                                      ...
                                      struct nmb_name *fail_name)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_logon_server_fail: Error - cannot find \
workgroup %s on subnet %s\n", failname, subrec->subnet_name));
		return;
	}

	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("become_logon_server_fail: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
	...
	DEBUG_OUT (0,("become_logon_server_fail: Failed to become a domain master for \
                                        ...
                                        int ttl, struct in_addr registered_ip)
{
	...
	if(!work) {
		DEBUG_OUT (0,("become_logon_server_success: Error - cannot find \
workgroup %s on subnet %s\n", reg_name, subrec->subnet_name));
		return;
	}

	if((servrec = find_server_in_workgroup( work, global_myname())) == NULL) {
		DEBUG_OUT (0,("become_logon_server_success: Error - cannot find server %s \
in workgroup %s on subnet %s\n",
	...
	*/
	...
	DEBUG_OUT (0,("become_logon_server_success: Samba is now a logon server \
...
static void become_logon_server(struct subnet_record *subrec,
                                struct work_record *work)
{
	DEBUG_OUT (2,("become_logon_server: Atempting to become logon server for workgroup %s \
on subnet %s\n", work->work_group,subrec->subnet_name));

	DEBUG_OUT (3,("become_logon_server: go to first stage: register %s<1c> name\n",
		work->work_group));
