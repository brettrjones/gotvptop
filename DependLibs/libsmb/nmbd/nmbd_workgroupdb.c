...
static void name_to_unstring(unstring unname, const char *name)
{
	...
	if (errno == E2BIG) {
		...
		DEBUG_OUT (0,("name_to_nstring: workgroup name %s is too long. Truncating to %s\n",
			name, tname));
	} else {
...
static struct work_record *create_workgroup(const char *name, int ttl)
{
	...
	if((work = SMB_MALLOC_P(struct work_record)) == NULL) {
		DEBUG_OUT (0,("create_workgroup: malloc fail !\n"));
		return NULL;
	}
                                     ...
                                     struct work_record *work)
{
	...
	DEBUG_OUT (3,("remove_workgroup: Removing workgroup %s\n", work->work_group));
                                             ...
                                             const char *name)
{
	...
	DEBUG_OUT (4, ("find_workgroup_on_subnet: workgroup search for %s on subnet %s: ",
                                               ...
                                               const char *name, int ttl)
{
	...
	DEBUG_OUT (4,("create_workgroup_on_subnet: creating group %s on subnet %s\n",
		name, subrec->subnet_name));
...
{  
	DEBUG_OUT (0,("fail_register: Failed to register name %s on subnet %s.\n",
...
void initiate_myworkgroup_startup(struct subnet_record *subrec, struct work_record *work)
{
	...
	if ((subrec != unicast_subnet) && (subrec != remote_broadcast_subnet) &&
		...
		DEBUG_OUT (3, ("initiate_myworkgroup_startup: preferred master startup for \
workgroup %s on subnet %s\n", work->work_group, subrec->subnet_name));
	...
	for( i = 0; my_netbios_names(i); i++) {
		...
		DEBUG_OUT (3,("initiate_myworkgroup_startup: Added server name entry %s \
on subnet %s\n", name, subrec->subnet_name));
	}
} 
	...
	for (subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_INCLUDING_UNICAST(subrec)) {
		...
		for (work = subrec->workgrouplist; work; work = nextwork) {
			...
			if ((work->serverlist == NULL) && (work->death_time != PERMANENT_TTL) && 
				...
				DEBUG_OUT (3,("expire_workgroups_and_servers: Removing timed out workgroup %s\n",
						work->work_group));
