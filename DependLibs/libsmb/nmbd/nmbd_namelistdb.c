				...
				bool self_only)
{
	...
	if( name_ret ) {
		...
		if( self_only && (name_ret->data.source != SELF_NAME) && (name_ret->data.source != PERMANENT_NAME) ) {
			DEBUG_OUT ( 9, ( "find_name_on_subnet: on subnet %s - self name %s NOT FOUND\n",
						subrec->subnet_name, nmb_namestr(nmbname) ) );
			return NULL;
		}

		DEBUG_OUT ( 9, ("find_name_on_subnet: on subnet %s - found name %s source=%d\n",
			subrec->subnet_name, nmb_namestr(nmbname), name_ret->data.source) );

		return name_ret;
	}

	DEBUG_OUT ( 9, ( "find_name_on_subnet: on subnet %s - name %s NOT FOUND\n",
		subrec->subnet_name, nmb_namestr(nmbname) ) );
			...
			struct in_addr *iplist)
{
	...
	if( NULL == namerec ) {
		DEBUG_OUT ( 0, ( "add_name_to_subnet: malloc fail.\n" ) );
		return False;
	}
	...
	if( NULL == namerec->data.ip ) {
		DEBUG_OUT ( 0, ( "add_name_to_subnet: malloc fail when creating ip_flgs.\n" ) );
		ZERO_STRUCTP(namerec);
		SAFE_FREE(namerec);
		return False;
	}
	...
	DEBUG_OUT ( 3, ( "add_name_to_subnet: Added netbios name %s with first IP %s \
                             ...
                             struct nmb_name        *nmbname )
{
	...
	DEBUG_OUT ( 0, ( "standard_fail_register: Failed to register/refresh name %s \
...
void add_ip_to_name_record( struct name_record *namerec, struct in_addr new_ip )
{
	...
	if( NULL == new_list ) {
		DEBUG_OUT (0,("add_ip_to_name_record: Malloc fail !\n"));
		return;
	}
                               ...
                               struct in_addr          released_ip )
{
	...
	if( namerec == NULL ) {
		DEBUG_OUT ( 0, ( "standard_success_release: Name release for name %s IP %s \
	...
	for( namerec = subrec->namelist; namerec; namerec = next_namerec ) {
		...
		if( (namerec->data.death_time != PERMANENT_TTL) && (namerec->data.death_time < t) ) {
			if( namerec->data.source == SELF_NAME ) {
				DEBUG_OUT ( 3, ( "expire_names_on_subnet: Subnet %s not expiring SELF \
name %s\n", subrec->subnet_name, nmb_namestr(&namerec->name) ) );
			...
			DEBUG_OUT (3,("expire_names_on_subnet: Subnet %s - removing expired name %s\n",
				subrec->subnet_name, nmb_namestr(&namerec->name)));
...
void add_samba_names_to_subnet( struct subnet_record *subrec )
{
	...
	if( (subrec == unicast_subnet) || (subrec == wins_server_subnet) || (subrec == remote_broadcast_subnet) ) {
		...
		if( NULL == iplist ) {
			DEBUG_OUT (0,("add_samba_names_to_subnet: Malloc fail !\n"));
			return;
		}
...
void dump_all_namelists(void)
{
	...
	if (!fp) { 
		DEBUG_OUT (0,("dump_all_namelists: Can't open file %s. Error was %s\n",
