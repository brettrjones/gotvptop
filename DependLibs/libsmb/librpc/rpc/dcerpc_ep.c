			    ...
			    struct dcerpc_binding_handle **pbh)
{
	...
	if (StrCaseCmp(rpcsrv_type, "embedded") == 0) {
		...
		status = rpcint_binding_handle(tmp_ctx,
					       ...
					       &h);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("dcerpc_ep_register: Could not connect to "
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("Failed to bind ncalrpc socket.\n"));
			goto done;
		}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_ep_register: Could not insert tower (%s)\n",
			  nt_errstr(status)));
		goto done;
	}
	if (result != EPMAPPER_STATUS_OK) {
		DEBUG_OUT (0, ("dcerpc_ep_register: Could not insert tower (0x%.8x)\n",
			  result));
		status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
