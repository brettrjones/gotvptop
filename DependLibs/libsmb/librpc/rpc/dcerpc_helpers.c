					...
					DATA_BLOB *rpc_out)
{
	...
	if (!data_blob_append(NULL, rpc_out,
		...
		DEBUG_OUT (0, ("Failed to add %u bytes auth blob.\n",
					...
					DATA_BLOB *rpc_out)
{
	...
	DEBUG_OUT (10,("add_schannel_auth_footer: SCHANNEL seq_num=%d\n",
			sas->seq_num));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1,("add_schannel_auth_footer: failed to process packet: %s\n",
			nt_errstr(status)));
		return status;
	}
					...
					DATA_BLOB *rpc_out)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to process packet: %s\n",
			  nt_errstr(status)));
		return status;
	}
			   ...
			   size_t *pad_len)
{
	...
	switch (auth->auth_level) {
	case DCERPC_AUTH_LEVEL_PRIVACY:
		DEBUG_OUT (10, ("Requested Privacy.\n"));
		break;

	case DCERPC_AUTH_LEVEL_INTEGRITY:
		DEBUG_OUT (10, ("Requested Integrity.\n"));
		break;
	...
	case DCERPC_AUTH_LEVEL_NONE:
		if (pkt->auth_length != 0) {
			DEBUG_OUT (3, ("Got non-zero auth len on non "
	...
	default:
		DEBUG_OUT (3, ("Unimplemented Auth Level %d",
			  auth->auth_level));
		return NT_STATUS_INVALID_PARAMETER;
	}
