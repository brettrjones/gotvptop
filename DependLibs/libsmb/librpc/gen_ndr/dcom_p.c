...
static NTSTATUS dcom_proxy_IClassFactory_init(void)
{
	...
	if (base_vtable == NULL) {
		DEBUG_OUT (0, ("No proxy registered for base interface 'IUnknown'\n"));
...
static NTSTATUS dcom_proxy_ISCMLocalActivator_init(void)
{
	...
	base_vtable = dcom_proxy_vtable_by_iid(&base_iid);
	if (base_vtable == NULL) {
		DEBUG_OUT (0, ("No proxy registered for base interface 'IClassActivator'\n"));
		return NT_STATUS_FOOBAR;
	}
...
static NTSTATUS dcom_proxy_IRemUnknown2_init(void)
{
	...
	base_vtable = dcom_proxy_vtable_by_iid(&base_iid);
	if (base_vtable == NULL) {
		DEBUG_OUT (0, ("No proxy registered for base interface 'IRemUnknown'\n"));
		return NT_STATUS_FOOBAR;
	}
