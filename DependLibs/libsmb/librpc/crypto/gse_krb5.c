...
static krb5_error_code flush_keytab(krb5_context krbctx, krb5_keytab keytab)
{
	...
	if (ret != KRB5_KT_END && ret != ENOENT) {
		DEBUG_OUT (1, (__location__ ": flushing keytab we got [%s]!\n",
			  error_message(ret)));
	}
						 ...
						 krb5_data *password)
{
	...
	for (i = 0; enctypes[i]; i++) {
		...
		if (create_kerberos_key_from_string(krbctx, princ,
						    password, key,
						    enctypes[i], false)) {
			DEBUG_OUT (10, ("Failed to create key for enctype %d "
				   "(error: %s)\n",
		...
		*(KRB5_KT_KEY(&kt_entry)) = *key;
		...
		if (ret) {
			DEBUG_OUT (1, (__location__ ": Failed to add entry to "
						   ...
						   krb5_keytab *keytab)
{
	...
	if (!pwd) {
		DEBUG_OUT (2, (__location__ ": failed to fetch machine password\n"));
	...
	if (*keytab == NULL) {
		...
		if (ret) {
			DEBUG_OUT (1, (__location__ ": Failed to get memory "
				  "keytab!\n"));
			return ret;
		}
	}
	...
	if (ret != KRB5_KT_END && ret != ENOENT ) {
		...
		if (ret != 0 && ret != KRB5_KT_END && ret != ENOENT ) {
			...
			DEBUG_OUT (1, (__location__ ": Failed to parse memory "
				  "keytab!\n"));
			goto out;
		}
		...
		if (ret == 0) {
			...
			if (ret) {
				DEBUG_OUT (1, (__location__ ": Failed to flush "
					  "memory keytab!\n"));
				goto out;
			}
		}
	...
	if (ret) {
		DEBUG_OUT (1, (__location__ ": Failed to get host principal!\n"));
		goto out;
	}
	...
	ret = fill_keytab_from_password(krbctx, *keytab,
					princ, kvno, &password);
	if (ret) {
		DEBUG_OUT (1, (__location__ ": Failed to fill memory keytab!\n"));
		goto out;
	}

	pwd_old = secrets_fetch_machine_password(lp_workgroup(), NULL, NULL);
	if (!pwd_old) {
		DEBUG_OUT (10, (__location__ ": no prev machine password\n"));
	} else {
					   ...
					   krb5_keytab *keytab)
{
	...
	*keytab = NULL;
	...
	case KERBEROS_VERIFY_SECRETS_AND_KEYTAB:
		ret = get_mem_keytab_from_secrets(krbctx, keytab);
		if (ret) {
			DEBUG_OUT (3, (__location__ ": Warning! Unable to set mem "
				  "keytab from secrets!\n"));
		}
