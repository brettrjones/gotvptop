				 ...
				 struct gse_context **_gse_ctx)
{
	...
	if (k5ret) {
		DEBUG_OUT (0, ("Failed to initialize kerberos context! (%s)\n",
			  error_message(k5ret)));
	...
	k5ret = krb5_cc_resolve(gse_ctx->k5ctx, ccache_name,
				&gse_ctx->ccache);
	if (k5ret) {
		DEBUG_OUT (1, ("Failed to resolve credential cache! (%s)\n",
			  error_message(k5ret)));
		status = NT_STATUS_INTERNAL_ERROR;
		goto err_out;
	}
			  ...
			  struct gse_context **_gse_ctx)
{
	...
	gss_maj = gss_import_name(&gss_min, &name_buffer,
				  ...
				  &gse_ctx->server_name);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_import_name failed for %s, with [%s]\n",
	...
	gss_maj = gss_acquire_cred(&gss_min,
				   ...
				   NULL, NULL);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_acquire_creds failed for %s, with [%s]\n",
			  (char *)name_buffer.value,
			  gse_errstr(gse_ctx, gss_maj, gss_min)));
		status = NT_STATUS_INTERNAL_ERROR;
		goto err_out;
				   ...
				   DATA_BLOB *token_out)
{
	...
	case GSS_S_CONTINUE_NEEDED:
		...
		break;
	default:
		DEBUG_OUT (0, ("gss_init_sec_context failed with [%s]\n",
			 ...
			 struct gse_context **_gse_ctx)
{
	...
	gss_maj = gss_acquire_cred(&gss_min,
				   ...
				   NULL, NULL);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_acquire_creds failed with [%s]\n",
			  gse_errstr(gse_ctx, gss_maj, gss_min)));
		status = NT_STATUS_INTERNAL_ERROR;
		goto done;
	}
				...
				struct gse_context *gse_ctx)
{
	...
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_inquire_sec_context_by_oid failed [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
	...
	if ((set == GSS_C_NO_BUFFER_SET) ||
		...
		DEBUG_OUT (0, ("gss_inquire_sec_context_by_oid returned unknown "
			     ...
			     TALLOC_CTX *mem_ctx, char **cli_name)
{
	...
	gss_maj = gss_display_name(&gss_min,
				   ...
				   &name_buffer, NULL);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_display_name failed [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
 ...
 */
	...
	if (gss_maj != 0) {
		DEBUG_OUT (0, ("obtaining PAC via GSSAPI gss_get_name_attribute "
			  "failed: %s\n",
	...
	DEBUG_OUT (0, ("obtaining PAC via GSSAPI failed: authenticated: %s, "
		  "complete: %s, more: %s\n",
				...
				int seal, size_t payload_size)
{
	...
	gss_maj = gss_wrap_iov_length(&gss_min, gse_ctx->gss_ctx,
					...
					&sealed, iov, 2);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_wrap_iov_length failed with [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
		return 0;
	}
		  ...
		  DATA_BLOB *data, DATA_BLOB *signature)
{
	...
	gss_maj = gss_wrap_iov(&gss_min, gse_ctx->gss_ctx,
				...
				&sealed, iov, 2);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_wrap_iov failed with [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
	...
	if (!sealed) {
		DEBUG_OUT (0, ("gss_wrap_iov says data was not sealed!\n"));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
	...
	DEBUG_OUT (10, ("Sealed %d bytes, and got %d bytes header/signature.\n",
		    ...
		    DATA_BLOB *data, DATA_BLOB *signature)
{
	...
	gss_maj = gss_unwrap_iov(&gss_min, gse_ctx->gss_ctx,
				 &sealed, NULL, iov, 2);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_unwrap_iov failed with [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
	...
	if (!sealed) {
		DEBUG_OUT (0, ("gss_unwrap_iov says data is not sealed!\n"));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
	...
	DEBUG_OUT (10, ("Unsealed %d bytes, with %d bytes header/signature.\n",
		   (int)iov[1].buffer.length, (int)iov[0].buffer.length));
		  ...
		  DATA_BLOB *data, DATA_BLOB *signature)
{
	...
	gss_maj = gss_get_mic(&gss_min, gse_ctx->gss_ctx,
			      ...
			      &in_data, &out_data);
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_get_mic failed with [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
		      ...
		      DATA_BLOB *data, DATA_BLOB *signature)
{
	...
	if (gss_maj) {
		DEBUG_OUT (0, ("gss_verify_mic failed with [%s]\n",
			  gse_errstr(talloc_tos(), gss_maj, gss_min)));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
