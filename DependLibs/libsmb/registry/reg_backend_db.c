					 ...
					 const char *add_path)
{
	...
	DEBUG_OUT (6, ("init_registry_key: Adding [%s]\n", add_path));
	...
	while (next_token_talloc(frame, &p, &keyname, "\\")) {
		...
		DEBUG_OUT (10,("init_registry_key: Storing key [%s] with "
			  "subkey [%s]\n", base,
				...
				struct builtin_regkey_value *value)
{
	...
	default:
		DEBUG_OUT (0, ("regdb_ctr_add_value: invalid value type in "
			  "registry values [%d]\n", value->type));
	}
}
				       ...
				       void *private_data)
{
	...
	if (keyname) {
		...
		DEBUG_OUT (2, ("regdb_normalize_keynames_fn: Convert %s to %s\n",
			  (const char *) rec->key.dptr,
			  keyname));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("regdb_normalize_keynames_fn: "
...
static WERROR regdb_store_regdb_version(uint32_t version)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("regdb_store_regdb_version: error storing %s = %d: %s\n",
			  version_keyname, version, nt_errstr(status)));
		return ntstatus_to_werror(status);
	} else {
		DEBUG_OUT (10, ("regdb_store_regdb_version: stored %s = %d\n",
...
WERROR regdb_init(void)
{
	...
	if (regdb) {
		DEBUG_OUT (10, ("regdb_init: incrementing refcount (%d->%d)\n",
	...
	if (!regdb) {
		...
		if (!regdb) {
			...
			DEBUG_OUT (1,("regdb_init: Failed to open registry %s (%s)\n",
				state_path("registry.tdb"), strerror(errno) ));
			return werr;
		}

		DEBUG_OUT (10,("regdb_init: Successfully created registry tdb\n"));
	}

	regdb_refcount = 1;
	DEBUG_OUT (10, ("regdb_init: registry db openend. refcount reset (%d)\n",
		   regdb_refcount));
	...
	if (vers_id == -1) {
		DEBUG_OUT (10, ("regdb_init: registry version uninitialized "
	...
	if (vers_id > expected_version || vers_id == 0) {
		DEBUG_OUT (1, ("regdb_init: unknown registry version %d "
	...
	if (vers_id == REGVER_V1) {
		DEBUG_OUT (10, ("regdb_init: got registry db version %d, upgrading "
			   "to version %d\n", REGVER_V1, REGVER_V2));
...
WERROR regdb_open( void )
{
	...
	if ( regdb ) {
		DEBUG_OUT (10, ("regdb_open: incrementing refcount (%d->%d)\n",
			   regdb_refcount, regdb_refcount+1));
		regdb_refcount++;
		return WERR_OK;
	}
	...
	if ( !regdb ) {
		...
		DEBUG_OUT (0,("regdb_open: Failed to open %s! (%s)\n",
			state_path("registry.tdb"), strerror(errno) ));
	}
	...
	regdb_refcount = 1;
	DEBUG_OUT (10, ("regdb_open: registry db opened. refcount reset (%d)\n",
		   regdb_refcount));
...
int regdb_close( void )
{
	...
	DEBUG_OUT (10, ("regdb_close: decrementing refcount (%d->%d)\n",
...
static WERROR regdb_delete_key_lists(struct db_context *db, const char *keyname)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, (__location__ " Deleting %s\\%s failed: %s\n",
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, (__location__ " Deleting %s failed: %s\n",
	...
	for (i=0; i<num_subkeys; i++) {
		...
		if (len+thistime > buflen) {
			...
			buffer = SMB_REALLOC_ARRAY(buffer, uint8_t,
						   (len+thistime)*2);
			if(buffer == NULL) {
				DEBUG_OUT (0, ("regdb_store_keys: Failed to realloc "
					  "memory of size [%u]\n",
					...
					void *private_data)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("regdb_store_keys: Failed to store new subkey list "
			 "for parent [%s]: %s\n", store_ctx->key,
			 win_errstr(werr)));
		goto done;
	}
	...
	if (num_subkeys == 0) {
		...
		if (!W_ERROR_IS_OK(werr)) {
			DEBUG_OUT (0,("regdb_store_keys: Failed to store "
				      ...
				      struct regsubkey_ctr *ctr)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("regdb_store_keys: talloc() failure!\n"));
		goto done;
	}
					   ...
					   void *private_data)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, (__location__ " failed to store new subkey list for "
			 "parent key %s: %s\n", create_ctx->key,
			 win_errstr(werr)));
	}
					   ...
					   void *private_data)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, (__location__ " failed to store new subkey_list for "
			 "parent key %s: %s\n", delete_ctx->key,
			 win_errstr(werr)));
	}
...
static bool regdb_key_is_base_key(const char *key)
{
	...
	path = normalize_reg_path(mem_ctx, key);
	if (path == NULL) {
		DEBUG_OUT (0, ("out of memory! (talloc failed)\n"));
		goto done;
	}
				...
				const char *name)
{
	...
	if (state.scanned) {
		...
		if (res != 0) {
			DEBUG_OUT (0, ("error starting transaction\n"));
			goto fail;
		}

		DEBUG_OUT (2, (__location__ " WARNING: recreating the sorted "
		...
		if (res != 0) {
			DEBUG_OUT (0, ("error committing transaction\n"));
			result = false;
		}
	}
					...
					struct regsubkey_ctr *ctr)
{
	...
	DEBUG_OUT (11,("regdb_fetch_keys: Enter key => [%s]\n", key ? key : "NULL"));

	if (!regdb_key_exists(db, key)) {
		DEBUG_OUT (10, ("key [%s] not found\n", key));
		werr = WERR_NOT_FOUND;
		goto done;
	}
	...
	if (count > 1) {
		DEBUG_OUT (5, ("regdb_fetch_keys_internal: it took %d attempts to "
	...
	if (value.dsize == 0 || value.dptr == NULL) {
		DEBUG_OUT (10, ("regdb_fetch_keys: no subkeys found for key [%s]\n",
			   key));
		goto done;
	}
	...
	for (i=0; i<num_items; i++) {
		...
		if (!W_ERROR_IS_OK(werr)) {
			DEBUG_OUT (5, ("regdb_fetch_keys: regsubkey_ctr_addkey "
				  "failed: %s\n", win_errstr(werr)));
			num_items = 0;
			goto done;
		}
	...
	DEBUG_OUT (11,("regdb_fetch_keys: Exit [%d] items\n", num_items));
...
static int regdb_unpack_values(struct regval_ctr *values, uint8 *buf, int buflen)
{
	...
	for ( i=0; i<num_values; i++ ) {
		...
		DEBUG_OUT (10, ("regdb_unpack_values: value[%d]: name[%s] len[%d]\n",
			   i, valuename, size));
	}
				       ...
				       struct regval_ctr *values)
{
	...
	DEBUG_OUT (10,("regdb_fetch_values: Looking for values of key [%s]\n", key));

	if (!regdb_key_exists(db, key)) {
		DEBUG_OUT (10, ("regb_fetch_values: key [%s] does not exist\n",
			   key));
		ret = -1;
		goto done;
	}
	...
	if (count > 1) {
		DEBUG_OUT (5, ("regdb_fetch_values_internal: it took %d attempts "
					...
					struct regval_ctr *values)
{
	...
	DEBUG_OUT (10,("regdb_store_values: Looking for values of key [%s]\n", key));
	...
	if (len <= 0) {
		DEBUG_OUT (0,("regdb_store_values: unable to pack values. len <= 0\n"));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("regdb_store_values_internal: error storing: %s\n", nt_errstr(status)));
		goto done;
	}
				...
				struct security_descriptor **psecdesc)
{
	...
	DEBUG_OUT (10, ("regdb_get_secdesc: Getting secdesc of key [%s]\n", key));
