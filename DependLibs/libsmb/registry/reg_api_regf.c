			    ...
			    REGF_NK_REC *key)
{
	...
	if (!registry_key.ops) {
		DEBUG_OUT (0, ("reg_load_tree: Failed to assign registry_ops "
			  "to [%s]\n", topkeypath));
		return WERR_BADFILE;
	}
	...
	if (!registry_key.name) {
		DEBUG_OUT (0, ("reg_load_tree: Talloc failed for reg_key.name!\n"));
		return WERR_NOMEM;
	}
	...
	{
		DEBUG_OUT (0,("reg_load_tree: Failed to load %s!\n", topkeypath));
				   ...
				   const char *fname)
{
	...
	if (regfile == NULL) {
		DEBUG_OUT (0, ("restore_registry_key: failed to open \"%s\" (%s)\n",
			     ...
			     REGF_NK_REC *parent)
{
	...
	DEBUG_OUT (6, ("reg_write_tree: wrote key [%s]\n", keypath));
				  ...
				  const char *fname)
{
	...
	regfile = regfio_open(fname, (O_RDWR|O_CREAT|O_EXCL),
			      (S_IRUSR|S_IWUSR));
	if (regfile == NULL) {
		DEBUG_OUT (0,("backup_registry_key: failed to open \"%s\" (%s)\n",
			 fname, strerror(errno) ));
		return ntstatus_to_werror(map_nt_error_from_unix(errno));
	}
