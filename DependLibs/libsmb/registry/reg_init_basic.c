...
WERROR registry_init_common(void)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("Failed to initialize the registry: %s\n",
			  win_errstr(werr)));
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("Failed to initialize the reghook cache: %s\n",
			  win_errstr(werr)));
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("Failed to initialize data in registry!\n"));
	}
...
WERROR registry_init_basic(void)
{
	...
	DEBUG_OUT (10, ("registry_init_basic called\n"));
