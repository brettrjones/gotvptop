				   ...
				   struct registry_key **pregkey)
{
	...
	DEBUG_OUT (7,("regkey_open_onelevel: name = [%s]\n", name));
	...
	if (key->ops == NULL) {
		DEBUG_OUT (0,("reg_open_onelevel: Failed to assign "
			 "registry_ops to [%s]\n", key->name ));
		result = WERR_BADFILE;
		goto done;
	}
		     ...
		     enum winreg_CreateAction *paction)
{
	...
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_createkey: failed to start transaction: %s\n",
			  win_errstr(err)));
		goto done;
	}
...
trans_done:
	if (W_ERROR_IS_OK(err)) {
		...
		if (!W_ERROR_IS_OK(err)) {
			DEBUG_OUT (0, ("reg_createkey: Error committing transaction: %s\n", win_errstr(err)));
		}
	} else {
		WERROR err1 = regdb_transaction_cancel();
		if (!W_ERROR_IS_OK(err1)) {
			DEBUG_OUT (0, ("reg_createkey: Error cancelling transaction: %s\n", win_errstr(err1)));
		}
	}
...
WERROR reg_deletekey(struct registry_key *parent, const char *path)
{
	...
	err = regdb_transaction_start();
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_deletekey: Error starting transaction: %s\n",
			  win_errstr(err)));
		goto done;
	}
...
trans_done:
	if (W_ERROR_IS_OK(err)) {
		err = regdb_transaction_commit();
		if (!W_ERROR_IS_OK(err)) {
			DEBUG_OUT (0, ("reg_deletekey: Error committing transaction: %s\n", win_errstr(err)));
		}
	} else {
		WERROR err1 = regdb_transaction_cancel();
		if (!W_ERROR_IS_OK(err1)) {
			DEBUG_OUT (0, ("reg_deletekey: Error cancelling transaction: %s\n", win_errstr(err1)));
		}
	}
		    ...
		    const struct registry_value *val)
{
	...
	err = regdb_transaction_start();
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_setvalue: Failed to start transaction: %s\n",
			  win_errstr(err)));
		return err;
	}
	...
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_setvalue: Error filling value cache: %s\n", win_errstr(err)));
		goto done;
	}
	...
	if (!store_reg_values(key->key, key->values)) {
		TALLOC_FREE(key->values);
		DEBUG_OUT (0, ("reg_setvalue: store_reg_values failed\n"));
	...
	if (W_ERROR_IS_OK(err)) {
		err = regdb_transaction_commit();
		if (!W_ERROR_IS_OK(err)) {
			DEBUG_OUT (0, ("reg_setvalue: Error committing transaction: %s\n", win_errstr(err)));
		}
	} else {
		WERROR err1 = regdb_transaction_cancel();
		if (!W_ERROR_IS_OK(err1)) {
			DEBUG_OUT (0, ("reg_setvalue: Error cancelling transaction: %s\n", win_errstr(err1)));
		}
	}
...
WERROR reg_deletevalue(struct registry_key *key, const char *name)
{
	...
	err = regdb_transaction_start();
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_deletevalue: Failed to start transaction: %s\n",
			  win_errstr(err)));
		return err;
	}
	...
	err = fill_value_cache(key);
	if (!W_ERROR_IS_OK(err)) {
		DEBUG_OUT (0, ("reg_deletevalue; Error filling value cache: %s\n",
			  win_errstr(err)));
		goto done;
	}
	...
	if (!store_reg_values(key->key, key->values)) {
		TALLOC_FREE(key->values);
		err = WERR_REG_IO_FAILURE;
		DEBUG_OUT (0, ("reg_deletevalue: store_reg_values failed\n"));
		goto done;
	}
	...
	if (W_ERROR_IS_OK(err)) {
		err = regdb_transaction_commit();
		if (!W_ERROR_IS_OK(err)) {
			DEBUG_OUT (0, ("reg_deletevalue: Error committing transaction: %s\n", win_errstr(err)));
		}
	} else {
		WERROR err1 = regdb_transaction_cancel();
		if (!W_ERROR_IS_OK(err1)) {
			DEBUG_OUT (0, ("reg_deletevalue: Error cancelling transaction: %s\n", win_errstr(err1)));
		}
	}
					    ...
					    bool del_key)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("reg_deletekey_recursive_trans: "
	...
	if (!W_ERROR_IS_OK(werr)) {
		...
		DEBUG_OUT (1, (__location__ " failed to delete key '%s' from key "
