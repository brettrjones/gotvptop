...
cbuf_print_hive(cbuf* ost, const char* hive, int len, const struct fmt_key* fmt)
{
	if (fmt->hive_fmt != FMT_HIVE_PRESERVE) {
		...
		if (hinfo == NULL) {
			DEBUG_OUT (0, ("Unknown hive %*s", len, hive));
		} else {
			switch(fmt->hive_fmt) {
			case FMT_HIVE_SHORT:
			...
			case FMT_HIVE_LONG:
				...
				break;
			default:
				DEBUG_OUT (0, ("Unsupported hive format %d",
					  (int)fmt->hive_fmt));
				return -1;
			}
		}
	...
	case REG_SZ:
		    ...
		    && is_zero_terminated_ucs2(data, len))
		{
			...
			if (pull_ucs2_talloc(mem_ctx, &str, (const smb_ucs2_t*)data, &dlen)) {
				...
				DEBUG_OUT (0, ("reg_format_value %s: "
				  ...
				  const char* sep)
{
	...
	if (str_enc && !set_iconv(&f->fromUTF16, str_enc, "UTF-16LE")) {
		DEBUG_OUT (0, ("reg_format_new: failed to set encoding: %s\n",
			  str_enc));
		goto fail;
	}
	...
	while (srprs_option(&options, ctx, &key, &val)) {
		if ((strcmp(key, "enc") == 0) || (strcmp(key, "strenc") == 0)) {
			if (!set_iconv(&fmt->fromUTF16, val, "UTF-16LE")) {
				DEBUG_OUT (0, ("Failed to set encoding: %s\n", val));
				ret = -1;
			}
...
static int reg_format_file_writeline(void* ptr, const char* line)
{
	...
	if (size == -1 ) {
		DEBUG_OUT (0, ("reg_format_file_writeline: iconvert_talloc failed >%s<\n",  line));
		return -1;
	}
				   ...
				   const char* options)
{
	...
	if (!set_iconv(&fmt->fromUTF16, opt.str_enc, "UTF-16LE")) { /* HACK */
		DEBUG_OUT (0, ("reg_format_file: failed to set string encoding %s",
			      opt.str_enc));
		goto fail;
	}

	if (!set_iconv(&fmt_ctx->fromUnix, opt.enc, "unix")) {
		DEBUG_OUT (0, ("reg_format_file: failed to set file encoding %s",
			  opt.enc));
		goto fail;
	}
	...
	if (fmt_ctx->file == NULL) {
		DEBUG_OUT (0, ("reg_format_file: fopen failed: %s\n", strerror(errno)));
		goto fail;
	}

	if (setvbuf(fmt_ctx->file, NULL, _IOFBF, 64000) < 0) {
		DEBUG_OUT (0, ("reg_format_file: setvbuf failed: %s\n", strerror(errno)));
	}
