...
uint32 reg_perfcount_get_base_index(void)
{
	...
	if ( !names ) {
		DEBUG_OUT (1, ("reg_perfcount_get_base_index: unable to open [%s].\n", fname));
		return 0;
	}    
	...
	if(dbuf.dptr == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_get_base_index: failed to find key \'1\' in [%s].\n", fname));
		tdb_close(names);
		return 0;
	}
	else
					       ...
					       uint32 buffer_size)
{
	...
	if(dbuf.dptr == NULL)
	{
		...
		DEBUG_OUT (3, ("_reg_perfcount_multi_sz_from_tdb: failed to find key [%s] in [%s].\n",
			  temp, tdb_name(tdb)));
		return buffer_size;
	}
...
uint32 reg_perfcount_get_counter_help(uint32 base_index, char **retbuf)
{
	...
	if(names == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_get_counter_help: unable to open [%s].\n", fname));
		return 0;
	}    
...
uint32 reg_perfcount_get_counter_names(uint32 base_index, char **retbuf)
{
	...
	if(names == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_get_counter_names: unable to open [%s].\n", fname));
		return 0;
	}    
...
static bool _reg_perfcount_get_counter_data(TDB_DATA key, TDB_DATA *data)
{
	...
	if(counters == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_get_counter_data: unable to open [%s].\n", fname));
		return False;
	}    
					    ...
					    TDB_CONTEXT *names)
{
	...
	if(data.dptr == NULL)
	{
		DEBUG_OUT (3, ("_reg_perfcount_get_counter_info: No type data for counter [%d].\n", CounterIndex));
		return False;
	}
	memset(buf, 0, PERFCOUNT_MAX_LEN);
	memcpy(buf, data.dptr, data.dsize);
	obj->counters[obj->NumCounters].CounterType = atoi(buf);
	DEBUG_OUT (10, ("_reg_perfcount_get_counter_info: Got type [%d] for counter [%d].\n",
	...
	if(data.dptr == NULL)
	{
		DEBUG_OUT (3, ("_reg_perfcount_get_counter_info: No counter data for counter [%d].\n", CounterIndex));
		return False;
	}
	...
	{
		DEBUG_OUT (3,("Improperly aligned counter [%d]\n", obj->NumCounters));
	}
	...
	while(start < end) {
		...
		*stop = '\0';
		...
		if(obj == NULL) {
			...
			DEBUG_OUT (3, ("_reg_perfcount_add_counter: Could not find parent object [%d] for counter [%d].\n",
				  parent, num));
			return False;
		}
					     ...
					     TDB_CONTEXT *names)
{
	...
	if (!_reg_perfcount_get_counter_data(key, &data)) {
		DEBUG_OUT (3, ("_reg_perfcount_get_counter_data failed\n"));
		return false;
	}
	if(data.dptr == NULL)
	{
		DEBUG_OUT (3, ("_reg_perfcount_get_instance_info: No instance data for instance [%s].\n",
			  buf));
		return False;
	}
	...
	if(data.dptr == NULL)
	{
		...
		DEBUG_OUT (3, ("_reg_perfcount_get_instance_info: No instance name for instance [%s].\n",
			  buf));
		inst->NameLength = 0;
	}
	else
					  ...
					  TDB_CONTEXT *names)
{
	...
	for(i = 1; i <= base_index; i++)
	{
		...
		if(data.dptr != NULL)
		{
			...
			{
				DEBUG_OUT (3, ("Bogus relationship [%s] for counter [%d].\n", data.dptr, j));
				success = False;
			}
			if(success == False)
			{
				DEBUG_OUT (3, ("_reg_perfcount_assemble_global: Failed to add new relationship for counter [%d].\n", j));
				retval = -1;
			}
			SAFE_FREE(data.dptr);
		}
		else
			DEBUG_OUT (3, ("NULL relationship for counter [%d] using key [%s].\n", j, keybuf));
	}	
	return retval;
}
				  ...
				  const char *key_part2)
{
	...
	if(data.dptr == NULL)
	{
		DEBUG_OUT (3,("_reg_perfcount_get_64: No data found for key [%s].\n", key.dptr));
		return False;
	}
						...
						TDB_CONTEXT *names)
{
	...
	if(counters == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_init_data_block_perf: unable to open [%s].\n", fname));
		return False;
	}    
						...
						bool bigendian_data)
{
	...
	if(names == NULL)
	{
		DEBUG_OUT (1, ("reg_perfcount_get_perf_data_block: unable to open [%s].\n", fname));
		return 0;
	}

	if (!_reg_perfcount_init_data_block(block, mem_ctx, names, bigendian_data)) {
		DEBUG_OUT (0, ("_reg_perfcount_init_data_block failed\n"));
		tdb_close(names);
		return 0;
	}
