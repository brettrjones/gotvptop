		...
		case REG_MULTI_SZ:
			if (p->str2UTF16 != NULL) {
				...
				if (dlen != -1) {
					...
					DEBUG_OUT (0, ("iconvert_talloc failed\n"));
				}
				talloc_free(dst);
			}
		default:
...
static bool act_val_sz(struct reg_parse* p, cbuf* value, bool cont)
{
	...
	if (cont) {
					  ...
					  &dst, &dlen, true))
		{
			...
			DEBUG_OUT (0, ("convert_string_talloc failed: >%s<\n"
				  "use it as is\t", src));
		}
		talloc_free(dst);
				...
				const char* str_enc, unsigned flags)
{
	...
	if (str_enc && !set_iconv(&s->str2UTF16, "UTF-16LE", str_enc)) {
		DEBUG_OUT (0, ("reg_parse_new: failed to set encoding: %s",
			  str_enc));
		goto fail;
	}
...
int reg_parse_set_options(struct reg_parse* parser, const char* options)
{
	...
	while (srprs_option(&options, ctx, &key, &val)) {
		if ((strcmp(key, "enc") == 0) || (strcmp(key, "strenc") == 0)) {
			...
			if ((end==NULL) || (*end != '\0')) {
				DEBUG_OUT (0, ("Invalid flags format: %s\n",
					  val ? val : "<NULL>"));
				ret = -1;
			}
		}
		...
		/* 	} else { */
		/* 		DEBUG_OUT (0, ("Invalid hive format: %s\n", val)); */
		/* 		ret = -1; */
		/* 	} */
		/* } */
	}
...
int reg_parse_line(struct reg_parse* parser, const char* line)
{
	...
	/* value line ?*/
		...
		if (parser->state != STATE_KEY_OPEN) {
			DEBUG_OUT (0, ("value \"%s\" without a key at line: %i",
		...
		if (srprs_char(&pos, '-')) {
			...
			DEBUG_OUT (0, ("value \"%s\" parse error"
				  "at line: %i pos: %li : %s",
				  cbuf_gets(parser->valname, 0), parser->linenum,
		...
		DEBUG_OUT (0, ("unrecognized line %i : %s\n", parser->linenum, line));
		return -3;
	}
	...
	if (!srprs_skipws(&pos) || !srprs_eol(&pos, NULL)) {
		DEBUG_OUT (0, ("trailing garbage at line: %i pos: %li : %s\n",
		...
		*ptr = pos;
		if (*file_enc == NULL) {
			*file_enc = charset;
			...
			DEBUG_OUT (0, ("file encoding forced to %s\n",
				  *file_enc));
		}
	}
...
reg_parse_fd_opt(void* mem_ctx, const char* options)
{
	...
	while (srprs_option(&options, ctx, &key, &val)) {
		if (strcmp(key, "enc") == 0) {
			...
			if ((end==NULL) || (*end != '\0')) {
				DEBUG_OUT (0, ("Invalid format \"%s\": %s\n",
					  key, val ? val : "<NULL>"));
			}
		   ...
		   char** optr, size_t *olen)
{
	...
	switch(err) {
	case EINVAL:
		/* DEBUG_OUT (0, ("Incomplete multibyte sequence\n")); */
	case E2BIG:
		return;
	case EILSEQ:
		break;
	...
	**optr = '\0';
	...
	DEBUG_OUT (0, ("Illegal multibyte sequence at line %lu: %s",
...
int reg_parse_fd(int fd, const struct reg_parse_callback* cb, const char* opts)
{
	...
	if (cb == NULL) {
		DEBUG_OUT (0,("reg_parse_fd: NULL callback\n"));
		goto done;
	}

	nread = read(fd, buf_raw, sizeof(buf_raw));
	if (nread < 0) {
		DEBUG_OUT (0, ("reg_parse_fd: read failed: %s\n", strerror(errno)));
		ret = nread;
		goto done;
	}
	...
	{
		DEBUG_OUT (0, ("reg_parse_fd: failed to guess encoding\n"));
		goto done;
	}

	DEBUG_OUT (10, ("reg_parse_fd: encoding file: %s str: %s\n",
		  opt.file_enc, opt.str_enc));


	if (!set_iconv(&cd, "unix", opt.file_enc)) {
		DEBUG_OUT (0, ("reg_parse_fd: failed to set file encoding %s\n",
			  opt.file_enc));
		goto done;
	}
		   ...
		   const char* opt)
{
	...
	if (fd < 0) {
		DEBUG_OUT (0, ("reg_parse_file: open failed: %s\n", strerror(errno)));
		return -1;
	}
