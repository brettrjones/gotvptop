		       ...
		       char** pdst)
{
	...
	if (dst == NULL) {
		...
		if (dst == NULL) {
			DEBUG_OUT (0,("iconver_talloc no mem\n"));
			return -1;
		}
	} else {
...
convert:
	...
	if(ret == -1) {
		...
		DEBUG_OUT (0,("Conversion error: %s(%.80s) %li\n", reason, iptr,
			 (long int)(iptr-src)));
		talloc_free(dst);
		return -1;
	}
...
bool srprs_bom(const char** ptr, const char** name, charset_t* ctype)
{
	...
	if (BOM[i].name != NULL) {
		DEBUG_OUT (0, ("Found Byte Order Mark for : %s\n", BOM[i].name));
...
int write_bom(FILE* file, const char* charset, charset_t ctype)
{
	...
	if ( charset == NULL ) {
		...
		DEBUG_OUT (0, ("No Byte Order Mark for charset_t: %u\n", (unsigned)ctype));
	} else {
		for (i=0; BOM[i].name; i++) {
		...
		DEBUG_OUT (0, ("No Byte Order Mark for charset_t: %s\n", charset));
	}
	return 0;
}
