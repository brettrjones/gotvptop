					 ...
					 uint32_t idx)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, ("error hashing new key '%s' in container: %s\n",
					   ...
					   const char *keyname)
{
	...
	werr = ntstatus_to_werror(dbwrap_delete_bystring_upper(ctr->subkeys_hash,
				  keyname));
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, ("error unhashing key '%s' in container: %s\n",
			  keyname, win_errstr(werr)));
	}
...
struct regval_blob* dup_registry_value(struct regval_blob *val)
{
	...
	if ( !(copy = SMB_MALLOC_P( struct regval_blob)) ) {
		DEBUG_OUT (0,("dup_registry_value: malloc() failed!\n"));
		return NULL;
	}
	...
	if ( val->data_p && val->size )
	{
		if ( !(copy->data_p = (uint8_t *)memdup( val->data_p,
						       val->size )) ) {
			DEBUG_OUT (0,("dup_registry_value: memdup() failed for [%d] "
				 "bytes!\n", val->size));
			SAFE_FREE( copy );
			return NULL;
		}
