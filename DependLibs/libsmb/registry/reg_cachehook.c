...
WERROR reghook_cache_init(void)
{
	...
	DEBUG_OUT (10, ("reghook_cache_init: new tree with default "
		   "ops %p for key [%s]\n", (void *)&regdb_ops,
		   KEY_TREE_ROOT));
	return WERR_OK;
}
...
WERROR reghook_cache_add(const char *keyname, struct registry_ops *ops)
{
	...
	DEBUG_OUT (10, ("reghook_cache_add: Adding ops %p for key [%s]\n",
		   (void *)ops, key));
...
struct registry_ops *reghook_cache_find(const char *keyname)
{
	...
	DEBUG_OUT (10,("reghook_cache_find: Searching for keyname [%s]\n", key));

	ops = (struct registry_ops *)pathtree_find(cache_tree, key);

	DEBUG_OUT (10, ("reghook_cache_find: found ops %p for key [%s]\n",
		   ops ? (void *)ops : 0, key));
...
{
	DEBUG_OUT (debuglevel,("reghook_dump_cache: Starting cache dump now...\n"));
