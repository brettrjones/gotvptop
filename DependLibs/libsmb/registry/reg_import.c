			   ...
			   const char* key[], size_t n, bool del)
{
	...
	if (del) {
		...
		if (!W_ERROR_IS_OK(werr)) {
			DEBUG_OUT (0, ("deletekey %s failed: %s\n",
		...
		werr = p->call.createkey(p->call.data, NULL, key[0],
					 &p->open_key, &existing);
		if (W_ERROR_IS_OK(werr)) {
			DEBUG_OUT (TL, ("createkey %s %s\n",
				  existing ? "opened" : "created", key[0]));
		} else {
			DEBUG_OUT (0, ("createkey %s failed: %s\n",
				  key[0], win_errstr(werr)));
		}
	}
				       ...
				       const uint8_t* data, uint32_t len)
{
	...
	if (v == NULL) {
		DEBUG_OUT (0, ("regval_compose %s failed\n", name));
		werr = WERR_NOMEM;
		goto done;
	}
			       ...
			       const char* name)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("deleteval %s failed: %s\n",
			  name, win_errstr(werr)));
	}
