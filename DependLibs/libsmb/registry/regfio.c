...
static int write_block( REGF_FILE *file, prs_struct *ps, uint32 offset )
{
	...
	if (sys_fstat(file->fd, &sbuf, false)) {
		DEBUG_OUT (0,("write_block: stat() failed! (%s)\n", strerror(errno)));
		return -1;
	}

	if ( lseek( file->fd, offset, SEEK_SET ) == -1 ) {
		DEBUG_OUT (0,("write_block: lseek() failed! (%s)\n", strerror(errno) ));
		return -1;
	}
	...
	while ( bytes_written < buffer_size ) {
		if ( (returned = write( file->fd, buffer+bytes_written, buffer_size-bytes_written )) == -1 ) {
			DEBUG_OUT (0,("write_block: write() failed! (%s)\n", strerror(errno) ));
			return False;
		}
...
static int read_block( REGF_FILE *file, prs_struct *ps, uint32 file_offset, uint32 block_size )
{
	...
	if (sys_fstat(file->fd, &sbuf, false)) {
		DEBUG_OUT (0,("read_block: stat() failed! (%s)\n", strerror(errno)));
		return -1;
	}
	...
	if ( block_size == 0 ) {
		...
		if ( lseek( file->fd, file_offset, SEEK_SET ) == -1 ) {
			DEBUG_OUT (0,("read_block: lseek() failed! (%s)\n", strerror(errno) ));
			return -1;
		}

		returned = read( file->fd, hdr, 0x20 );
		if ( (returned == -1) || (returned < 0x20) ) {
			DEBUG_OUT (0,("read_block: failed to read in HBIN header. Is the file corrupt?\n"));
			return -1;
		}
		...
		if ( strncmp( hdr, "hbin", HBIN_HDR_SIZE ) != 0 ) {
			DEBUG_OUT (0,("read_block: invalid block header!\n"));
			return -1;
		}
	...
	DEBUG_OUT (10,("read_block: block_size == 0x%x\n", block_size ));
	...
	if (!prs_init( ps, block_size, file->mem_ctx, UNMARSHALL )) {
		DEBUG_OUT (0,("read_block: prs_init() failed! (%s)\n", strerror(errno) ));
		return -1;
	}
	...
	while ( bytes_read < block_size ) {
		if ( (returned = read( file->fd, buffer+bytes_read, block_size-bytes_read )) == -1 ) {
			DEBUG_OUT (0,("read_block: read() failed (%s)\n", strerror(errno) ));
			return False;
		}
		if ( (returned == 0) && (bytes_read < block_size) ) {
			DEBUG_OUT (0,("read_block: not a vald registry file ?\n" ));
			return False;
		}	
...
static bool hbin_block_close( REGF_FILE *file, REGF_HBIN *hbin )
{
	...
	if ( p == hbin ) {
		...
		DEBUG_OUT (0,("hbin_block_close: block not in open list!\n"));
...
static bool prs_nk_rec( const char *desc, prs_struct *ps, int depth, REGF_NK_REC *nk )
{
	...
	if ( data_size > nk->rec_size )
		DEBUG_OUT (10,("Encountered reused record (0x%x < 0x%x)\n", data_size, nk->rec_size));
...
static bool read_regf_block( REGF_FILE *file )
{
	...
	if ( file->checksum !=  checksum ) {
		DEBUG_OUT (0,("read_regf_block: invalid checksum\n" ));
		return False;
	}
...
static REGF_HBIN* read_hbin_block( REGF_FILE *file, off64_t offset )
{
	...
	DEBUG_OUT (10,("read_hbin_block: free space offset == 0x%x\n", hbin->free_off));
...
static REGF_HBIN* lookup_hbin_block( REGF_FILE *file, uint32 offset )
{
	...
	for ( hbin=file->block_list; hbin; hbin=hbin->next ) {
		DEBUG_OUT (10,("lookup_hbin_block: address = 0x%x [0x%lx]\n", hbin->file_off, (unsigned long)hbin ));
...
static bool hbin_prs_vk_rec( const char *desc, REGF_HBIN *hbin, int depth, REGF_VK_REC *vk, REGF_FILE *file )
{
	...
	if ( data_size !=  vk->rec_size )
		DEBUG_OUT (10,("prs_vk_rec: data_size check failed (0x%x < 0x%x)\n", data_size, vk->rec_size));
...
static bool hbin_prs_vk_records( const char *desc, REGF_HBIN *hbin, int depth, REGF_NK_REC *nk, REGF_FILE *file )
{
	...
	for ( i=0; i<nk->num_values; i++ ) {
		...
		if ( !hbin_contains_offset( hbin, nk->values[i].rec_off ) ) {
			...
			if ( !sub_hbin ) {
				DEBUG_OUT (0,("hbin_prs_vk_records: Failed to find HBIN block containing offset [0x%x]\n", 
...
static bool hbin_prs_key( REGF_FILE *file, REGF_HBIN *hbin, REGF_NK_REC *nk )
{
	...
	if ( nk->num_values && (nk->values_off!=REGF_OFFSET_NONE) ) {
		...
		if ( !hbin_contains_offset( hbin, nk->values_off ) ) {
			...
			if ( !sub_hbin ) {
				DEBUG_OUT (0,("hbin_prs_key: Failed to find HBIN block containing value_list_offset [0x%x]\n", 
					nk->values_off));
				return False;
			}
		}
	...
	if ( nk->num_subkeys && (nk->subkeys_off!=REGF_OFFSET_NONE) ) {
		...
		if ( !hbin_contains_offset( hbin, nk->subkeys_off ) ) {
			...
			if ( !sub_hbin ) {
				DEBUG_OUT (0,("hbin_prs_key: Failed to find HBIN block containing subkey_offset [0x%x]\n", 
					nk->subkeys_off));
				return False;
			}
		}
	...
	if ( (nk->sk_off!=REGF_OFFSET_NONE) && !( nk->sec_desc = find_sk_record_by_offset( file, nk->sk_off )) ) {
		...
		if ( !hbin_contains_offset( hbin, nk->sk_off ) ) {
			...
			if ( !sub_hbin ) {
				DEBUG_OUT (0,("hbin_prs_key: Failed to find HBIN block containing sk_offset [0x%x]\n", 
					nk->subkeys_off));
				return False;
			}
		}
...
{	
	...
	if ( write_block( file, &ps, 0 ) == -1 ) {
		DEBUG_OUT (0,("init_regf_block: Failed to initialize registry header block!\n"));
		result = False;
		goto out;
	}
 ...
 REGF_FILE* regfio_open( const char *filename, int flags, int mode )
{
	...
	if ( !(rb = SMB_MALLOC_P(REGF_FILE)) ) {
		DEBUG_OUT (0,("ERROR allocating memory\n"));
		return NULL;
	}
	ZERO_STRUCTP( rb );
	rb->fd = -1;
	...
	if ( (rb->fd = open(filename, flags, mode)) == -1 ) {
		DEBUG_OUT (0,("regfio_open: failure to open %s (%s)\n", filename, strerror(errno)));
		regfio_close( rb );
		return NULL;
	}
	...
	if ( flags & (O_CREAT|O_TRUNC) ) {
		if ( !init_regf_block( rb ) ) {
			DEBUG_OUT (0,("regfio_open: Failed to read initial REGF block\n"));
			regfio_close( rb );
			return NULL;
		}
 ...
 int regfio_close( REGF_FILE *file )
{
	...
	if ((file->fd != -1) && (file->open_flags & (O_WRONLY|O_RDWR))) {
		...
		if ( read_block( file, &ps, 0, REGF_BLOCKSIZE ) != -1 ) {
			...
			if ( write_block( file, &ps, 0 ) == -1 )
				DEBUG_OUT (0,("regfio_close: failed to update the regf header block!\n"));
		}
...
REGF_NK_REC* regfio_rootkey( REGF_FILE *file )
{
	...
	if ( !(nk = TALLOC_ZERO_P( file->mem_ctx, REGF_NK_REC )) ) {
		DEBUG_OUT (0,("regfio_rootkey: talloc() failed!\n"));
		return NULL;
	}
	...
	if ( !found ) {
		DEBUG_OUT (0,("regfio_rootkey: corrupt registry file ?  No root key record located\n"));
		return NULL;
	}
 ...
 REGF_NK_REC* regfio_fetch_subkey( REGF_FILE *file, REGF_NK_REC *nk )
{
	...
	if ( !(hbin = lookup_hbin_block( file, nk->subkeys.hashes[nk->subkey_index].nk_off )) ) {
		DEBUG_OUT (0,("hbin_prs_key: Failed to find HBIN block containing offset [0x%x]\n", 
...
static REGF_HBIN* regf_hbin_allocate( REGF_FILE *file, uint32 block_size )
{
	...
	if (sys_fstat(file->fd, &sbuf, false)) {
		DEBUG_OUT (0,("regf_hbin_allocate: stat() failed! (%s)\n", strerror(errno)));
		return NULL;
	}
...
static REGF_HBIN* find_free_space( REGF_FILE *file, uint32 size )
{
	...
	if ( !hbin ) {
		...
		if ( !(hbin = regf_hbin_allocate( file, alloc_size )) ) {
			DEBUG_OUT (0,("find_free_space: regf_hbin_allocate() failed!\n"));
			return NULL;
		}
		DLIST_ADD( file->block_list, hbin );
	}
