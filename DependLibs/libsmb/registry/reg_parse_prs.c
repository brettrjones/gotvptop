...
bool prs_init(prs_struct *ps, uint32 size, TALLOC_CTX *ctx, bool io)
{
	...
	if (size != 0) {
		...
		if(ps->data_p == NULL) {
			DEBUG_OUT (0,("prs_init: talloc fail for %u bytes.\n", (unsigned int)size));
			return False;
		}
...
bool prs_grow(prs_struct *ps, uint32 extra_space)
{
	...
	if(UNMARSHALLING(ps) || !ps->is_dynamic) {
		DEBUG_OUT (0,("prs_grow: Buffer overflow - unable to expand buffer by %u bytes.\n",
	...
	if(ps->buffer_size == 0) {
		...
		if(ps->data_p == NULL) {
			DEBUG_OUT (0,("prs_grow: talloc failure for size %u.\n", (unsigned int)new_size));
			return False;
		}
	} else {
		/*
		...
		ps->data_p = talloc_realloc(ps->mem_ctx,
						...
						new_size);
		if (ps->data_p == NULL) {
			DEBUG_OUT (0,("prs_grow: Realloc failure for size %u.\n",
				(unsigned int)new_size));
			return False;
		}
...
char *prs_mem_get(prs_struct *ps, uint32 extra_size)
{
	if(UNMARSHALLING(ps)) {
		...
		if (ps->data_offset + extra_size > ps->buffer_size) {
			DEBUG_OUT (0,("prs_mem_get: reading data of size %u would overrun "
				"buffer by %u bytes.\n",
