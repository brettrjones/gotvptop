...
{
	DEBUG_OUT (lvl,("debug_dsdcinfo_flags: 0x%08x\n\t", flags));
				   ...
				   struct netr_DsRGetDCNameInfo **info)
{
	...
	if (!NT_STATUS_IS_OK(status)
		...
		DEBUG_OUT (10,("dsgetdcname_cached: cache fetch failed with: %s\n",
			nt_errstr(status)));
				    ...
				    int *returned_count)
{
	...
	*returned_count = 0;
	...
	status = internal_resolve_name(domain_name, name_type, NULL,
				       ...
				       "lmhosts wins bcast");
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("discover_dc_netbios: failed to find DC\n"));
		return status;
	}
				   ...
				   struct netr_DsRGetDCNameInfo **info)
{
	...
	DEBUG_OUT (10,("process_dc_netbios\n"));
				       ...
				       struct netr_DsRGetDCNameInfo **info)
{
	...
	DEBUG_OUT (10,("dsgetdcname_rediscover\n"));
		     ...
		     struct netr_DsRGetDCNameInfo **info)
{
	...
	DEBUG_OUT (10,("dsgetdcname: domain_name: %s, "
	...
	*info = NULL;
	...
	if (!check_allowed_required_flags(flags, site_name)) {
		DEBUG_OUT (0,("invalid flags specified\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
