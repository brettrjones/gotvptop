			...
			fstring srv_name)
{
	...
	for( i =0 ; i < 3; i++) {
		...
		DEBUG_OUT (4,("ads_dc_name: domain=%s\n", domain));
	...
	if (i == 3) {
		DEBUG_OUT (1,("ads_dc_name: sitename (now \"%s\") keeps changing ???\n",
			sitename ? sitename : ""));
		SAFE_FREE(sitename);
		return False;
	}
	...
	*dc_ss = ads->ldap.ss;
	...
	DEBUG_OUT (4,("ads_dc_name: using server='%s' IP=%s\n",
		 srv_name, addr));
			...
			struct sockaddr_storage *ss_out)
{
	...
	if (!NT_STATUS_IS_OK(get_sorted_dc_list(domain, NULL, &ip_list, &count,
						False))) {
		DEBUG_OUT (3, ("Could not look up dc's for domain %s\n", domain));
		return False;
	}
	...
	print_sockaddr(addr, sizeof(addr), &dc_ss);
	DEBUG_OUT (3, ("rpc_dc_name: Returning DC %s (%s) for domain %s\n", srv_name,
		  addr, domain));
