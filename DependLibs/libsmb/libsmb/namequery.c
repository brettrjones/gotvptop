...
bool saf_store( const char *domain, const char *servername )
{
	...
	DEBUG_OUT (10,("saf_store: domain = [%s], server = [%s], expire = [%u]\n",
...
bool saf_join_store( const char *domain, const char *servername )
{
	...
	if ( !domain || !servername ) {
		DEBUG_OUT (2,("saf_join_store: Refusing to store empty domain or servername!\n"));
		return False;
	}

	if ( (strlen(domain) == 0) || (strlen(servername) == 0) ) {
		DEBUG_OUT (0,("saf_join_store: refusing to store 0 length domain or servername!\n"));
		return False;
	}
	...
	DEBUG_OUT (10,("saf_join_store: domain = [%s], server = [%s], expire = [%u]\n",
		domain, servername, (unsigned int)expire ));
...
bool saf_delete( const char *domain )
{
	...
	if ( !domain ) {
		DEBUG_OUT (2,("saf_delete: Refusing to delete empty domain\n"));
		return False;
	}
	...
	if (ret) {
		DEBUG_OUT (10,("saf_delete[join]: domain = [%s]\n", domain ));
	}
	...
	if (ret) {
		DEBUG_OUT (10,("saf_delete: domain = [%s]\n", domain ));
	}
...
char *saf_fetch( const char *domain )
{
	...
	if ( !domain || strlen(domain) == 0) {
		DEBUG_OUT (2,("saf_fetch: Empty domain name!\n"));
		return NULL;
	}
	...
	if ( ret ) {
		DEBUG_OUT (5,("saf_fetch[join]: Returning \"%s\" for \"%s\" domain\n",
			server, domain ));
		return server;
	}
	...
	if ( !ret ) {
		DEBUG_OUT (5,("saf_fetch: failed to find server for \"%s\" domain\n",
					domain ));
	} else {
		DEBUG_OUT (5,("saf_fetch: Returning \"%s\" for \"%s\" domain\n",
			server, domain ));
	}
				...
				struct node_status_extra *extra)
{
	...
	*num_names = CVAL(p,0);
	...
	for (i=0;i< *num_names;i++) {
		...
		ret[i].flags = p[16];
		p += 18;
		DEBUG_OUT (10, ("%s#%02x: flags = 0x%02x\n", ret[i].name,
...
static void sock_packet_read_got_socket(struct tevent_req *subreq)
{
	...
	if (state->packet == NULL) {
		DEBUG_OUT (10, ("parse_packet failed\n"));
		goto retry;
	}
	if ((state->trn_id != -1) &&
	    (state->trn_id != packet_trn_id(state->packet))) {
		DEBUG_OUT (10, ("Expected transaction id %d, got %d\n",
			   state->trn_id, packet_trn_id(state->packet)));
		goto retry;
	}
	...
	void *private_data)
{
	...
	if (state->sock == -1) {
		...
		DEBUG_OUT (10, ("open_socket_in failed: %s\n", strerror(errno)));
		return tevent_req_post(req, ev);
	}
					  ...
					  const struct sockaddr_storage *addr)
{
	...
	if (state->buflen == 0) {
		tevent_req_nterror(req, NT_STATUS_INTERNAL_ERROR);
		DEBUG_OUT (10, ("build_packet failed\n"));
		return tevent_req_post(req, ev);
	}
	...
	if (tevent_req_nomem(subreq, req)) {
		DEBUG_OUT (10, ("nb_trans_send failed\n"));
		return tevent_req_post(req, ev);
	}
			...
			fstring name)
{
	...
	if (lp_disable_netbios()) {
		DEBUG_OUT (5,("name_status_find(%s#%02x): netbios is disabled\n",
					q_name, q_type));
		return False;
	}
	...
	DEBUG_OUT (10, ("name_status_find: looking up %s#%02x at %s\n", q_name,
		   q_type, addr));
	...
	DEBUG_OUT (10, ("name_status_find: name %sfound", result ? "" : "not "));
...
static int remove_duplicate_addrs2(struct ip_service *iplist, int count )
{
	...
	DEBUG_OUT (10,("remove_duplicate_addrs2: "
				   ...
				   const struct sockaddr_storage *addr)
{
	...
	if (lp_disable_netbios()) {
		DEBUG_OUT (5,("name_query(%s#%02x): netbios is disabled\n",
					name, name_type));
...
static bool name_query_validator(struct packet_struct *p, void *private_data)
{
	...
	DEBUG_OUT (2,("Got a positive name query response "
		...
		int *pcount)
{
	...
	if ((*return_iplist = SMB_MALLOC_ARRAY(struct ip_service, real_count)) ==
			NULL) {
		DEBUG_OUT (0,("convert_ip2service: malloc failed "
			"for %d enetries!\n", real_count ));
		return False;
	}
			...
			int *return_count)
{
	...
	if (lp_disable_netbios()) {
		DEBUG_OUT (5,("name_resolve_bcast(%s#%02x): netbios is disabled\n",
					name, name_type));
	...
	*return_count = 0;
	...
	DEBUG_OUT (3,("name_resolve_bcast: Attempting broadcast lookup "
		"for name %s<0x%x>\n", name, name_type));
		...
		int *return_count)
{
	...
	if (lp_disable_netbios()) {
		DEBUG_OUT (5,("resolve_wins(%s#%02x): netbios is disabled\n",
					name, name_type));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	*return_count = 0;
	...
	DEBUG_OUT (3,("resolve_wins: Attempting wins lookup for name %s<0x%x>\n",
				name, name_type));

	if (wins_srv_count() < 1) {
		DEBUG_OUT (3,("resolve_wins: WINS server resolution selected "
	...
	if (src_ss.ss_family != AF_INET) {
		...
		DEBUG_OUT (3,("resolve_wins: cannot receive WINS replies "
			"on IPv6 address %s\n",
			addr));
	...
	for (t=0; wins_tags && wins_tags[t]; t++) {
		...
		for (i=0; i<srv_count; i++) {
			...
			DEBUG_OUT (3,("resolve_wins: using WINS server %s "
				"and tag '%s'\n",
			      ...
			      int *return_count)
{
	...
	if ( name_type != 0x20 && name_type != 0x0) {
		DEBUG_OUT (5, ("resolve_hosts: not appropriate "
			"for name type <0x%x>\n",
			name_type));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	*return_count = 0;
	...
	DEBUG_OUT (3,("resolve_hosts: Attempting host lookup for name %s<0x%x>\n",
				name, name_type));
	...
	ret = getaddrinfo(name,
			...
			&ailist);
	if (ret) {
		DEBUG_OUT (3,("resolve_hosts: getaddrinfo failed for name %s [%s]\n",
			name,
			gai_strerror(ret) ));
	}
	...
	for (res = ailist; res; res = res->ai_next) {
		...
		*return_iplist = SMB_REALLOC_ARRAY(*return_iplist,
						...
						*return_count);
		if (!*return_iplist) {
			DEBUG_OUT (3,("resolve_hosts: malloc fail !\n"));
			freeaddrinfo(ailist);
			return NT_STATUS_NO_MEMORY;
		}
			    ...
			    int *return_count)
{
	...
	if ( (ctx = talloc_init("resolve_ads")) == NULL ) {
		DEBUG_OUT (0,("resolve_ads: talloc_init() failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	switch (name_type) {
		case 0x1b:
			DEBUG_OUT (5,("resolve_ads: Attempting to resolve "
				 "PDC for %s using DNS\n", name));
	...
	if ((*return_iplist = SMB_MALLOC_ARRAY(struct ip_service, numaddrs)) ==
			NULL ) {
		DEBUG_OUT (0,("resolve_ads: malloc failed for %d entries\n",
					numaddrs ));
		talloc_destroy(ctx);
		return NT_STATUS_NO_MEMORY;
	}
				...
				const char *resolve_order)
{
	...
	*return_count = 0;
	...
	DEBUG_OUT (10, ("internal_resolve_name: looking up %s#%x (sitename %s)\n",
			name, name_type, sitename ? sitename : "(null)"));

	if (is_ipaddress(name)) {
		if ((*return_iplist = SMB_MALLOC_P(struct ip_service)) ==
				NULL) {
			DEBUG_OUT (0,("internal_resolve_name: malloc fail !\n"));
			return NT_STATUS_NO_MEMORY;
		}
		...
		if (!interpret_string_addr(&(*return_iplist)->ss,
					name, AI_NUMERICHOST)) {
			DEBUG_OUT (1,("internal_resolve_name: interpret_string_addr "
				"failed on %s\n",
				name));
	...
	if (strcmp( resolve_order, "NULL") == 0) {
		DEBUG_OUT (8,("internal_resolve_name: all lookups disabled\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	while (next_token_talloc(frame, &ptr, &tok, LIST_SEP)) {
		if((strequal(tok, "host") || strequal(tok, "hosts"))) {
			...
			DEBUG_OUT (0,("resolve_name: unknown name switch type %s\n",
				tok));
		}
	}
	...
	*return_count = remove_duplicate_addrs2(*return_iplist, *return_count );
	...
	if ( DEBUGLEVEL >= 100 ) {
		for (i = 0; i < *return_count && DEBUGLEVEL == 100; i++) {
			...
			print_sockaddr(addr, sizeof(addr),
					&(*return_iplist)[i].ss);
			DEBUG_OUT (100, ("Storing name %s of type %d (%s:%d)\n",
					name,
					name_type,
					addr,
	...
	if ( DEBUGLEVEL >= 10 ) {
		DEBUG_OUT (10, ("internal_resolve_name: returning %d addresses: ",
					*return_count));
...
bool find_master_ip(const char *group, struct sockaddr_storage *master_ss)
{
	...
	if (lp_disable_netbios()) {
		DEBUG_OUT (5,("find_master_ip(%s): netbios is disabled\n", group));
		return false;
	}
...
bool get_pdc_ip(const char *domain, struct sockaddr_storage *pss)
{
	...
	if ( count > 1 ) {
		DEBUG_OUT (6,("get_pdc_ip: PDC has %d IP addresses!\n", count));
			...
			bool *ordered)
{
	...
	*ordered = False;
	...
	if (!*pserver ) {
		DEBUG_OUT (10,("get_dc_list: no preferred domain controllers.\n"));
		status = internal_resolve_name(domain, 0x1C, sitename, ip_list,
					     count, resolve_order);
		goto out;
	}

	DEBUG_OUT (3,("get_dc_list: preferred server list: \"%s\"\n", pserver ));
	...
	while (next_token_talloc(ctx, &p, &name, LIST_SEP)) {
		if (!done_auto_lookup && strequal(name, "*")) {
			...
			DEBUG_OUT (8,("Adding %d DC's from auto lookup\n",
						auto_count));
		} else  {
			num_addresses++;
		}
	...
	if ((num_addresses == 0)) {
		if (done_auto_lookup) {
			DEBUG_OUT (4,("get_dc_list: no servers found\n"));
	...
	if ((return_iplist = SMB_MALLOC_ARRAY(struct ip_service,
					num_addresses)) == NULL) {
		DEBUG_OUT (3,("get_dc_list: malloc fail !\n"));
		status = NT_STATUS_NO_MEMORY;
		goto out;
	}
	...
	while ((local_count<num_addresses) &&
		...
		if (resolve_name( name, &name_ss, 0x20, true )) {
			...
			if( !NT_STATUS_IS_OK(check_negative_conn_cache(domain,
							addr)) ) {
				DEBUG_OUT (5,("get_dc_list: negative entry %s "
	...
	if ( DEBUGLEVEL >= 4 ) {
		DEBUG_OUT (4,("get_dc_list: returning %d ip addresses "
				"in an %sordered list\n",
				local_count,
				*ordered ? "":"un"));
		DEBUG_OUT (4,("get_dc_list: "));
			...
			bool ads_only )
{
	...
	*count = 0;
	...
	DEBUG_OUT (8,("get_sorted_dc_list: attempting lookup "
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_LOGON_SERVERS)
	    && sitename) {
		DEBUG_OUT (3,("get_sorted_dc_list: no server for name %s available"
