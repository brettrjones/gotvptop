    ...
    if (home) {
        ...
        if (asprintf(&conf, "%s/.smb/smb.conf", home) > 0) {
            if (lp_load(conf, True, False, False, True)) {
                ...
                DEBUG_OUT (5, ("Could not load config file: %s\n",
                          conf));
            }
            SAFE_FREE(conf);
        }
                  ...
                  int shutdown_ctx)
{
        ...
        if (shutdown_ctx) {
                SMBCFILE * f;
                DEBUG_OUT (1,("Performing aggressive shutdown.\n"));
                ...
                if (smbc_getFunctionPurgeCachedServers(context)(context)) {
                        SMBCSRV * s;
                        SMBCSRV * next;
                        DEBUG_OUT (1, ("Could not purge all servers, "
                                  "Nice way shutdown failed.\n"));
                        s = context->internal->servers;
                        while (s) {
                                DEBUG_OUT (1, ("Forced shutdown: %p (fd=%d)\n",
                                          s, s->cli->fd));
                                cli_shutdown(s->cli);
                ...
                if (context->internal->servers) {
                        DEBUG_OUT (1, ("Active servers in context, "
                                  "free_context failed.\n"));
                        errno = EBUSY;
                        return 1;
                }
                if (context->internal->files) {
                        DEBUG_OUT (1, ("Active files in context, "
                                  "free_context failed.\n"));
                        errno = EBUSY;
                        return 1;
                }
        ...
        DEBUG_OUT (3, ("Context %p successfully freed\n", context));
...
smbc_init_context(SMBCCTX *context)
{
        ...
        DEBUG_OUT (1, ("Using netbios name %s.\n", smbc_getNetbiosName(context)));
					...
					const char *password)
{
	...
	if (! auth_info) {
		DEBUG_OUT (0, ("smbc_set_credentials_with_fallback: allocation fail\n"));
		return;
	}
