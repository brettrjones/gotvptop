...
static ssize_t client_receive_smb(struct cli_state *cli, size_t maxlen)
{
	...
	for(;;) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10,("client_receive_smb failed\n"));
			show_msg(cli->inbuf);
	...
	if (cli_encryption_on(cli)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("SMB decryption failed on incoming packet! Error %s\n",
				nt_errstr(status)));
 ...
 again:
	...
	if (len < 0) {
		...
		if (cli->fd != -1) {
			...
			DEBUG_OUT (0, ("Receiving SMB: Server %s stopped responding\n",
				addr));
			close(cli->fd);
			cli->fd = -1;
		}
	...
	if (!cli_check_sign_mac(cli, cli->inbuf, seqnum+1)) {
		...
		DEBUG_OUT (0, ("SMB Signature verification failed on incoming packet!\n"));
		cli->smb_rw_error = SMB_READ_BAD_SIG;
		close(cli->fd);
		cli->fd = -1;
		return false;
...
static ssize_t write_socket(int fd, const char *buf, size_t len)
{
        ...
        DEBUG_OUT (6,("write_socket(%d,%d)\n",fd,(int)len));
        ret = write_data(fd,buf,len);

        DEBUG_OUT (6,("write_socket(%d,%d) wrote %d\n",fd,(int)len,(int)ret));
        if(ret <= 0)
                DEBUG_OUT (0,("write_socket: Error writing %d bytes to socket %d: ERRNO = %s\n",
...
bool cli_send_smb(struct cli_state *cli)
{
	...
	if (!cli_state_set_seqnum(cli, cli->mid, seqnum)) {
		DEBUG_OUT (0,("Failed to store mid[%u]/seqnum[%u]\n",
	...
	if (enc_on) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			DEBUG_OUT (0,("Error in encrypting client message. Error %s\n",
				nt_errstr(status) ));
			return false;
		}
	}
	...
	while (nwritten < len) {
		...
		if (ret <= 0) {
			...
			cli->fd = -1;
			cli->smb_rw_error = SMB_WRITE_ERROR;
			DEBUG_OUT (0,("Error writing %d bytes to client. %d (%s)\n",
...
NTSTATUS cli_init_creds(struct cli_state *cli, const char *username, const char *domain, const char *password)
{
	...
	DEBUG_OUT (10,("cli_init_creds: user %s domain %s\n", cli->user_name, cli->domain));
...
struct cli_state *cli_initialise_ex(int signing_state)
{
	...
	if (is_setuid_root()) {
		DEBUG_OUT (0,("libsmb based programs must *NOT* be setuid root.\n"));
		return NULL;
	}
