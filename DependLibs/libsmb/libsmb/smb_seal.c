...
static NTSTATUS common_gss_decrypt_buffer(struct smb_tran_enc_state_gss *gss_state, char *buf)
{
	...
	if (ret != GSS_S_COMPLETE) {
		...
		DEBUG_OUT (0,("common_gss_encrypt_buffer: gss_unwrap failed. Error %s\n",
			ads_errstr(adss) ));
		return map_nt_error_from_gss(ret, minor);
	}

	if (out_buf.length > in_buf.length) {
		DEBUG_OUT (0,("common_gss_encrypt_buffer: gss_unwrap size (%u) too large (%u) !\n",
					...
					char **ppbuf_out)
{
	...
	*ppbuf_out = NULL;
	...
	if (ret != GSS_S_COMPLETE) {
		ADS_STATUS adss = ADS_ERROR_GSS(ret, minor);
		DEBUG_OUT (0,("common_gss_encrypt_buffer: gss_wrap failed. Error %s\n",
			ads_errstr(adss) ));
		return map_nt_error_from_gss(ret, minor);
	}
