				  ...
				  const char *principal)
{
	...
	if (data->has_error) {
		DEBUG_OUT (1,("Failed to build negTokenInit at offset %d\n", (int)data->ofs));
	}
...
DATA_BLOB spnego_gen_krb5_wrap(TALLOC_CTX *ctx, const DATA_BLOB ticket, const uint8 tok_id[2])
{
	...
	if (data->has_error) {
		DEBUG_OUT (1,("Failed to build krb5 wrapper at offset %d\n", (int)data->ofs));
	}
				...
				DATA_BLOB *auth, DATA_BLOB *signature)
{
	...
	if (len == -1) {
		DEBUG_OUT (3,("spnego_parse_auth: spnego_read_data failed\n"));
		return false;
	}

	if (token.type != SPNEGO_NEG_TOKEN_TARG) {
		DEBUG_OUT (3,("spnego_parse_auth: wrong token type: %d\n",
			token.type));
				...
				DATA_BLOB *auth)
{
	...
	*auth = data_blob_null;
	...
	if (asn1_tag_remaining(data)) {
		...
		DEBUG_OUT (5,("spnego_parse_auth_response received mechListMIC, "
		    "ignoring.\n"));
	}
	...
	if (data->has_error) {
		DEBUG_OUT (3,("spnego_parse_auth_response failed at %d\n", (int)data->ofs));
		asn1_free(data);
		data_blob_free(auth);
		return False;
	}
			   ...
			   char **oid_list, DATA_BLOB *raw_data)
{
	...
	if (data->has_error) {
		DEBUG_OUT (3, (__location__ " failed at %d\n", (int)data->ofs));
		asn1_free(data);
		return false;
	}
	...
	*raw_data = data_blob_talloc(mem_ctx, data->data, data->length);
	if (!raw_data->data) {
		DEBUG_OUT (3, (__location__": data_blob_talloc() failed!\n"));
		asn1_free(data);
		return false;
	}
