				     ...
				     struct iovec *iov, int iov_count)
{
	...
	if (cli_encryption_on(state->cli)) {
		...
		TALLOC_FREE(buf);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Error in encrypting client message: %s\n",
				  nt_errstr(status)));
			return status;
		}
...
static void cli_smb_received(struct tevent_req *subreq)
{
	...
	if (cli_encryption_on(cli) && (CVAL(inbuf, 0) == 0)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("get_enc_ctx_num returned %s\n",
				   nt_errstr(status)));
			goto fail;
		}

		if (enc_ctx_num != cli->trans_enc_state->enc_ctx_num) {
			DEBUG_OUT (10, ("wrong enc_ctx %d, expected %d\n",
				   enc_ctx_num,
		...
		status = common_decrypt_buffer(cli->trans_enc_state,
					       (char *)inbuf);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("common_decrypt_buffer returned %s\n",
				   nt_errstr(status)));
			goto fail;
		}
	}
	...
	if (!oplock_break /* oplock breaks are not signed */
		...
		DEBUG_OUT (10, ("cli_check_sign_mac failed\n"));
		TALLOC_FREE(inbuf);
