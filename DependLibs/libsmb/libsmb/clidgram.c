		       ...
		       struct packet_struct *p)
{
	...
	if (smb_size + 17*2 + strlen(mailslot) + 1 + len > MAX_DGRAM_SIZE) {
		DEBUG_OUT (0, ("cli_send_mailslot: Cannot write beyond end of packet\n"));
		return False;
	}
	...
	DEBUG_OUT (4,("send_mailslot: Sending to mailslot %s from %s ",
	...
	struct netlogon_samlogon_response **samlogon_response)
{
	...
	if (blob.length < 4) {
		DEBUG_OUT (1, ("invalid length: %d\n", (int)blob.length));
		return false;
	}

	if (RIVAL(blob.data,0) != DGRAM_SMB) {
		DEBUG_OUT (1, ("invalid packet\n"));
		return false;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (1, ("failed to parse packet\n"));
		return false;
	}

	if (p.smb.smb_command != SMB_TRANSACTION) {
		DEBUG_OUT (1, ("invalid smb_command: %d\n", p.smb.smb_command));
		return false;
	}
	...
	*nt_version = r->ntver;
	...
	if (!strequal(returned_domain, domain_name)) {
		DEBUG_OUT (3, ("GetDC: Expected domain %s, got %s\n",
	...
	*dc_name = talloc_strdup(mem_ctx, returned_dc);
	...
	DEBUG_OUT (10, ("GetDC gave name %s for domain %s\n",
				  ...
				  uint32_t nt_version)
{
	...
	if (!prep_getdc_request(dc_addr, domain_name, sid, nt_version,
				...
				&state->p)) {
		DEBUG_OUT (3, ("prep_getdc_request failed\n"));
...
static void nbt_getdc_got_reader(struct tevent_req *subreq)
{
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG_OUT (10, ("nb_packet_reader_recv returned %s\n",
			   nt_errstr(status)));
		return;
	}
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG_OUT (10, ("messaging_send_buf returned %s\n",
			   nt_errstr(status)));
		return;
	}
