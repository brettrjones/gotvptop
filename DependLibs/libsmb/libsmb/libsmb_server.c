                          ...
                          SMBCSRV * srv)
{
	...
	for (file = context->internal->files; file; file = file->next) {
		if (file->srv == srv) {
			/* Still used */
			DEBUG_OUT (3, ("smbc_remove_usused_server: "
                                  "%p still used by %p.\n",
				  srv, file));
			return 1;
		}
	...
	DEBUG_OUT (3, ("smbc_remove_usused_server: %p removed.\n", srv));
	    ...
	    bool *in_cache)
{
	...
	*in_cache = false;
			...
			if (!NT_STATUS_IS_OK(status)) {
				...
				srv->cli = NULL;
                        ...
                        if (is_ipc) {
                                ...
                                DEBUG_OUT (4, ("Could not retrieve "
	...
	DEBUG_OUT (4,("SMBC_server: server_n=[%s] server=[%s]\n", server_n, server));

	DEBUG_OUT (4,(" -> server_n=[%s] server=[%s]\n", server_n, server));
...
again:
		...
		if (strcmp(called.name, "*SMBSERVER")) {
			...
			if (is_ipaddress(server) && !tried_reverse) {
				...
				if (!interpret_string_addr(&rem_ss, server,
                                                           NI_NUMERICHOST)) {
					DEBUG_OUT (4, ("Could not convert IP address "
	...
	DEBUG_OUT (4,(" session request ok\n"));
	...
	DEBUG_OUT (4,(" tconx ok\n"));

        /* Determine if this share supports case sensitivity */
	if (is_ipc) {
                DEBUG_OUT (4, ("IPC$ so ignore case sensitivity\n"));
        } else if (!NT_STATUS_IS_OK(cli_get_fs_attr_info(c, &fs_attrs))) {
                DEBUG_OUT (4, ("Could not retrieve case sensitivity flag: %s.\n",
                          cli_errstr(c)));
	...
	if (context->internal->smb_encryption_level) {
		...
		if (!NT_STATUS_IS_OK(cli_force_encryption(c,
			...
			DEBUG_OUT (4,(" SMB encrypt failed\n"));
		...
		char **pp_password)
{
	...
	if (smbc_getFunctionAddCachedServer(context)(context, srv,
		...
		DEBUG_OUT (3, (" Failed to add server to cache\n"));
		errno = saved_errno;
		if (errno == 0) {
			errno = ENOMEM;
		}
	...
	DEBUG_OUT (2, ("Server connect ok: //%s/%s: %p\n",
		server, share, srv));
	...
	share = srv->cli->share;
		...
		if (!*pp_password || (*pp_password)[0] == '\0') {
			...
			if (!*pp_workgroup || !*pp_username || !*pp_password) {
                ...
                if (! NT_STATUS_IS_OK(nt_status)) {
                        DEBUG_OUT (1,("cli_full_connection failed! (%s)\n",
                                 nt_errstr(nt_status)));
                        errno = ENOTSUP;
                        return NULL;
                }
		...
		if (context->internal->smb_encryption_level) {
			...
			if (!NT_STATUS_IS_OK(cli_force_encryption(ipc_cli,
				...
				DEBUG_OUT (4,(" SMB encrypt failed on IPC$\n"));
			...
			DEBUG_OUT (4,(" SMB encrypt ok on IPC$\n"));
		}
			...
			ipc_srv->cli, &ndr_table_lsarpc.syntax_id, &pipe_hnd);
                if (!NT_STATUS_IS_OK(nt_status)) {
                        DEBUG_OUT (1, ("cli_nt_session_open fail!\n"));
                        errno = ENOTSUP;
