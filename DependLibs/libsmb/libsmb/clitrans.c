			     ...
			     int *piov_count)
{
	...
	DEBUG_OUT (10, ("num_setup=%u, max_setup=%u, "
	...
	uint8_t *data, uint32_t num_data, uint32_t max_data)
{
	...
	if ((cmd == SMBtrans) || (cmd == SMBtrans2)) {
		if ((num_param > 0xffff) || (max_param > 0xffff)
			...
			DEBUG_OUT (3, ("Attempt to send invalid trans2 request "
...
static void cli_trans_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Pulling params failed: %s\n", nt_errstr(status)));
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Pulling data failed: %s\n", nt_errstr(status)));
		goto fail;
	}
