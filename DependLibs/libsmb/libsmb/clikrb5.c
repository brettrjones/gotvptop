						  ...
						  krb5_enctype enctype)
{
	...
	if (ret) {
		DEBUG_OUT (1,("krb5_principal2salt failed (%s)\n", error_message(ret)));
		return ret;
	}
						  ...
						  krb5_enctype enctype)
{
	...
	if (ret) {
		DEBUG_OUT (1,("krb5_get_pw_salt failed (%s)\n", error_message(ret)));
		return ret;
	}
			   ...
			   DATA_BLOB *edata_out)
{
	...
	if (edata_type != KRB5_PADATA_PW_SALT) {
		DEBUG_OUT (0,("edata is not of required type %d but of type %d\n", 
...
bool unwrap_pac(TALLOC_CTX *mem_ctx, DATA_BLOB *auth_data, DATA_BLOB *unwrapped_pac_data)
{
	...
	if (data_type != KRB5_AUTHDATA_WIN2K_PAC ) {
		DEBUG_OUT (10,("authorization data is not a Windows PAC (type: %d)\n", data_type));
		asn1_free(data);
		return False;
	}
	    ...
	    tkt->enc_part2->authorization_data[0]->length)
	{
		for (i = 0; tkt->enc_part2->authorization_data[i] != NULL; i++) {
			...
			if (tkt->enc_part2->authorization_data[i]->ad_type != 
				...
				DEBUG_OUT (10,("get_auth_data_from_tkt: ad_type is %d\n", 
				      ...
				      krb5_creds  *credsp)
{
	...
	DEBUG_OUT (3, ("ads_cleanup_expired_creds: Ticket in ccache[%s:%s] expiration %s\n",
	...
	*/
	if (strequal(cc_type, "FILE")) {
		DEBUG_OUT (5, ("ads_cleanup_expired_creds: We do not remove creds from a %s ccache\n", cc_type));
		return False;
	}

	retval = krb5_cc_remove_cred(context, ccache, 0, credsp);
	if (retval) {
		DEBUG_OUT (1, ("ads_cleanup_expired_creds: krb5_cc_remove_cred failed, err %s\n",
			...
			krb5_auth_context *auth_context)
{
	...
	if (retval) {
		DEBUG_OUT (1,("krb5_auth_con_init failed (%s)\n",
			error_message(retval)));
		return retval;
	}
	...
	if (retval) {
		DEBUG_OUT (1,("krb5_auth_con_setaddrs failed (%s)\n",
			error_message(retval)));
	}
				       ...
				       const char *impersonate_princ_s)
{
	...
	if (retval) {
		DEBUG_OUT (1,("ads_krb5_mk_req: Failed to parse principal %s\n", principal));
		return retval;
	}
	...
	if ((retval = krb5_copy_principal(context, server, &creds.server))) {
		DEBUG_OUT (1,("ads_krb5_mk_req: krb5_copy_principal failed (%s)\n", 
			 error_message(retval)));
		goto cleanup_princ;
	}
	...
	if ((retval = krb5_cc_get_principal(context, ccache, &creds.client))) {
		...
		DEBUG_OUT (3,("ads_krb5_mk_req: krb5_cc_get_principal failed (%s)\n",
			 error_message(retval)));
		goto cleanup_creds;
	}
	...
	while (!creds_ready && (i < maxtries)) {
		...
		if ((retval = smb_krb5_get_credentials(context, ccache,
						       ...
						       impersonate_princ,
						       &credsp))) {
			DEBUG_OUT (1,("ads_krb5_mk_req: smb_krb5_get_credentials failed for %s (%s)\n",
		...
		if ((unsigned)credsp->times.starttime > time(NULL)) {
			...
			DEBUG_OUT (4,("ads_krb5_mk_req: Advancing clock by %d seconds to cope with clock skew\n", time_offset));
	...
	DEBUG_OUT (10,("ads_krb5_mk_req: Ticket (%s) in ccache (%s:%s) is valid until: (%s - %u)\n",
	...
	if (retval) {
		DEBUG_OUT (1,("setup_auth_context failed (%s)\n",
			error_message(retval)));
		goto cleanup_creds;
	}
	...
	if( credsp->ticket_flags & TKT_FLG_OK_AS_DELEGATE ) {
		...
		DEBUG_OUT ( 3, ("ads_krb5_mk_req: server marked as OK to delegate to, building forwardable TGT\n")  );

		retval = krb5_auth_con_setuseruserkey(context,
					*auth_context,
					&credsp->keyblock );
		if (retval) {
			DEBUG_OUT (1,("krb5_auth_con_setuseruserkey failed (%s)\n",
				error_message(retval)));
			goto cleanup_creds;
		}
		...
		if (retval) {
			DEBUG_OUT (1,("krb5_auth_con_setflags failed (%s)\n",
				error_message(retval)));
			goto cleanup_creds;
		}
		...
		if (retval) {
			DEBUG_OUT ( 3, ("krb5_fwd_tgt_creds failed (%s)\n",
				   error_message( retval ) ) );
	...
	if (retval) {
		DEBUG_OUT (1,("krb5_auth_con_set_req_cksumtype failed (%s)\n",
			error_message(retval)));
		goto cleanup_data;
	}
#endif
	...
	if (retval) {
		DEBUG_OUT (1,("ads_krb5_mk_req: krb5_mk_req_extended failed (%s)\n", 
			 error_message(retval)));
	}
...

{
	...
	if (retval) {
		DEBUG_OUT (1, ("krb5_init_context failed (%s)\n",
			 error_message(retval)));
		goto failed;
	}
	...
	if ((retval = krb5_cc_resolve(context, ccname ?
		...
		DEBUG_OUT (1, ("krb5_cc_default failed (%s)\n",
			 error_message(retval)));
		goto failed;
	}

	if ((retval = krb5_set_default_tgs_ktypes(context, enc_types))) {
		DEBUG_OUT (1, ("krb5_set_default_tgs_ktypes failed (%s)\n",
			 error_message(retval)));
		goto failed;
	}
			      ...
			      DATA_BLOB *session_key, bool remote)
{
	...
	if (err || skey == NULL) {
		DEBUG_OUT (10, ("KRB5 error getting session key %d\n", err));
		goto done;
	}

	DEBUG_OUT (10, ("Got KRB5 session key of length %d\n",
					 ...
					 size_t length)
{
...
#if defined(HAVE_KRB5_C_VERIFY_CHECKSUM)
	{
		...
		ret = krb5_c_verify_checksum(context, 
					     ...
					     &checksum_valid);
		if (ret) {
			DEBUG_OUT (3,("smb_krb5_verify_checksum: krb5_c_verify_checksum() failed: %s\n", 
				error_message(ret)));
			return ret;
		}
...

	{
		...
		ret = krb5_crypto_init(context,
				       ...
				       &crypto);
		if (ret) {
			DEBUG_OUT (0,("smb_krb5_verify_checksum: krb5_crypto_init() failed: %s\n", 
				error_message(ret)));
			return ret;
		}
		    ...
		    krb5_keyblock **out_key)
{
	...
	if (ret) {
		DEBUG_OUT (1,("get_key_from_keytab: smb_krb5_open_keytab failed (%s)\n", error_message(ret)));
		return ret;
	}

	if ( DEBUGLEVEL >= 10 ) {
		if (smb_krb5_unparse_name(talloc_tos(), context, server, &name) == 0) {
			DEBUG_OUT (10,("get_key_from_keytab: will look for kvno %d, enctype %d and name: %s\n", 
				kvno, enctype, name));
			TALLOC_FREE(name);
		}
	}
	...
	if (ret) {
		DEBUG_OUT (0,("get_key_from_keytab: failed to retrieve key: %s\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG_OUT (0,("get_key_from_keytab: failed to copy key: %s\n", error_message(ret)));
		goto out;
	}
							...
							krb5_keyblock **keyblock)
{
	...
	ret = get_key_from_keytab(context, 
				  ...
				  &local_keyblock);
	if (ret) {
		DEBUG_OUT (0,("krb5_rd_req_return_keyblock_from_keytab: failed to call get_key_from_keytab\n"));
		goto out;
	}
				       ...
				       time_t *expire_time)
{
	...
	DEBUG_OUT (10,("smb_krb5_renew_ticket: using %s as ccache\n", ccache_string));
...
#ifdef HAVE_KRB5_GET_RENEWED_CREDS	/* MIT */
	{
		...
		if (ret) {
			DEBUG_OUT (10,("smb_krb5_renew_ticket: krb5_get_kdc_cred failed: %s\n", error_message(ret)));
			goto done;
		}
	}
					...
					krb5_data *packet)
{
	...
	DEBUG_OUT (10,("handle_krberror_packet: got error packet\n"));
...
#ifdef HAVE_E_DATA_POINTER_IN_KRB5_ERROR /* Heimdal */
	{
		...
		if ((ret = krb5_rd_error(context, packet, &krberror))) {
			DEBUG_OUT (10,("handle_krberror_packet: krb5_rd_error failed with: %s\n", 
				error_message(ret)));
			return ret;
		}
...
#endif
	if (got_error_code) {
		DEBUG_OUT (5,("handle_krberror_packet: got KERBERR from kpasswd: %s (%d)\n", 
				      ...
				      krb5_keytab *keytab)
{
	...
	DEBUG_OUT (10,("smb_krb5_open_keytab: krb5_kt_default_name returned %s\n", keytab_string));
 ...
 resolve:
 	DEBUG_OUT (10,("smb_krb5_open_keytab: resolving: %s\n", tmp));
				   ...
				   krb5_creds **creds_p)
{
	...
	*creds_p = NULL;
	...
	DEBUG_OUT (1,("smb_krb5_get_creds: got ticket for %s\n",
		server_s));
				   ...
				   krb5_principal principal)
{
 ...
 int cli_krb5_get_ticket(TALLOC_CTX *mem_ctx,
			...
			const char *impersonate_princ_s)
{
	 DEBUG_OUT (0,("NO KERBEROS SUPPORT\n"));
	 return 1;
}
