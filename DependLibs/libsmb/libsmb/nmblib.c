...
static bool parse_nmb(char *inbuf,int length,struct nmb_packet *nmb)
{
	...
	DEBUG_OUT (10,("parse_nmb: packet id = %d\n", nmb->header.name_trn_id));
...
static struct packet_struct *copy_nmb_packet(struct packet_struct *packet)
{
	...
	if(( pkt_copy = SMB_MALLOC_P(struct packet_struct)) == NULL) {
		DEBUG_OUT (0,("copy_nmb_packet: malloc fail.\n"));
		return NULL;
	}
 ...
 free_and_exit:
	...
	DEBUG_OUT (0,("copy_nmb_packet: malloc fail in resource records.\n"));
	return NULL;
}
...
static struct packet_struct *copy_dgram_packet(struct packet_struct *packet)
{
	...
	if(( pkt_copy = SMB_MALLOC_P(struct packet_struct)) == NULL) {
		DEBUG_OUT (0,("copy_dgram_packet: malloc fail.\n"));
		return NULL;
	}
...
struct packet_struct *read_packet(int fd,enum packet_type packet_type)
{
	...
	DEBUG_OUT (5,("Received a packet of len %d from (%s) port %d\n",
...
static bool send_udp(int fd,char *buf,int len,struct in_addr ip,int port)
{
	...
	DEBUG_OUT ( 5, ( "Sending a packet of len %d to (%s) on port %d\n",
	...
	if (!ret)
		DEBUG_OUT (0,("Packet send failed to %s(%d) ERRNO=%s\n",
...
static unsigned char *name_ptr(unsigned char *buf, size_t buf_len, unsigned int ofs)
{
	...
	if ((c & 0xC0) == 0xC0) {
		...
		DEBUG_OUT (5,("name ptr to pos %d from %d is %s\n",l,ofs,buf+l));
		return(buf + l);
	} else {
		return(buf+ofs);
	}
