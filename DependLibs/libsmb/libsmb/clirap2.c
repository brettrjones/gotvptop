...
int cli_NetGroupDelete(struct cli_state *cli, const char *group_name)
{
	      ...
	      &rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (1, ("Access Denied\n"));
		} else if (res == 2220) {
			DEBUG_OUT (1, ("Group does not exist\n"));
		} else {
			DEBUG_OUT (4,("NetGroupDelete res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetGroupDelete failed\n"));
	}
...
int cli_NetGroupAdd(struct cli_state *cli, struct rap_group_info_1 *grinfo)
{
	      ...
	      &rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (1, ("Group already exists\n"));
		} else {
			DEBUG_OUT (4,("NetGroupAdd res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetGroupAdd failed\n"));
	}
...
int cli_RNetGroupEnum(struct cli_state *cli, void (*fn)(const char *, const char *, void *), void *state)
{
	...
	if (cli_api(cli,
		...
		if(cli->rap_error == 234) {
			DEBUG_OUT (1,("Not all group names were returned (such as those longer than 21 characters)\n"));
		} else if (cli->rap_error != 0) {
			DEBUG_OUT (1,("NetGroupEnum gave error %d\n", cli->rap_error));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetGroupEnum no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetGroupEnum res=%d\n", res));
	}
...
int cli_NetGroupDelUser(struct cli_state * cli, const char *group_name, const char *user_name)
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		case 65:
			...
			break;
		case 50:
			DEBUG_OUT (1, ("Not supported by server\n"));
			break;
		case 2220:
			DEBUG_OUT (1, ("Group does not exist\n"));
			break;
		case 2221:
			DEBUG_OUT (1, ("User does not exist\n"));
			break;
		case 2237:
			DEBUG_OUT (1, ("User is not in group\n"));
			break;
		default:
			DEBUG_OUT (4,("NetGroupDelUser res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetGroupDelUser failed\n"));
	}
...
int cli_NetGroupAddUser(struct cli_state * cli, const char *group_name, const char *user_name)
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		case 2221:
			...
			break;
		default:
			DEBUG_OUT (4,("NetGroupAddUser res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetGroupAddUser failed\n"));
	}
...
int cli_NetGroupGetUsers(struct cli_state * cli, const char *group_name, void (*fn)(const char *, void *), void *state )
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetGroupGetUsers gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetGroupGetUsers no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetGroupGetUsers res=%d\n", res));
	}
...
int cli_NetUserGetGroups(struct cli_state * cli, const char *user_name, void (*fn)(const char *, void *), void *state )
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetUserGetGroups gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetUserGetGroups no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetUserGetGroups res=%d\n", res));
	}
...
int cli_NetUserDelete(struct cli_state *cli, const char * user_name )
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (4,("NetUserDelete res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetUserDelete failed\n"));
	}
...
int cli_NetUserAdd(struct cli_state *cli, struct rap_user_info_1 * userinfo )
{
		      ...
		      &rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (1, ("User already exists\n"));
		} else {
			DEBUG_OUT (4,("NetUserAdd res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetUserAdd failed\n"));
	}
...
int cli_RNetUserEnum(struct cli_state *cli, void (*fn)(const char *, const char *, const char *, const char *, void *), void *state)
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (cli->rap_error != 0) {
			DEBUG_OUT (1,("NetUserEnum gave error %d\n", cli->rap_error));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetUserEnum no data returned\n"));
		goto out;
	}
...
int cli_NetFileClose(struct cli_state *cli, uint32 file_id )
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (1, ("NetFileClose2 - attempt to close non-existant file open instance\n"));
		} else {
			DEBUG_OUT (4,("NetFileClose2 res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetFileClose2 failed\n"));
	}
...
int cli_NetFileGetInfo(struct cli_state *cli, uint32 file_id, void (*fn)(const char *, const char *, uint16, uint16, uint32))
{
			...
			&rdata, &rdrcnt))               /* return data, length */
	{
		...
		if (res == 0 || res == ERRmoredata) {
			...
			DEBUG_OUT (4,("NetFileGetInfo2 res=%d\n", res));
		}
	} else {
		res = -1;
		DEBUG_OUT (4,("NetFileGetInfo2 failed\n"));
	}
			       ...
			       uint32))
{
			...
			&rdata, &rdrcnt))               /* return data, length */
	{
		...
		if (res == 0 || res == ERRmoredata) {
			...
			DEBUG_OUT (4,("NetFileEnum2 res=%d\n", res));
		}
	} else {
		DEBUG_OUT (4,("NetFileEnum2 failed\n"));
	}
...
int cli_NetShareAdd(struct cli_state *cli, struct rap_share_info_2 * sinfo )
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (4,("NetShareAdd res=%d\n", res));
		}
	} else {
		DEBUG_OUT (4,("NetShareAdd failed\n"));
	}
...
int cli_NetShareDelete(struct cli_state *cli, const char * share_name )
{
			...
			&rdata, &rdrcnt))   /* return data, length */
	{
		...
		if (res == 0) {
			...
			DEBUG_OUT (4,("NetShareDelete res=%d\n", res));
		}
	} else {
		DEBUG_OUT (4,("NetShareDelete failed\n"));
	}
	...
	*pdc_name = NULL;
	...
	if (cli_api(cli,
		...
		if (cli->rap_error == 0) {
			...
			DEBUG_OUT (4,("cli_get_pdc_name: machine %s failed the NetServerEnum call. "
			 ...
			 char **servername)
{
	...
	if (rdrcnt < 16) {
		DEBUG_OUT (10, ("invalid data count %d, expected >= 16\n", rdrcnt));
		goto failed;
	}

	if (pull_ascii(tmp, rdata, sizeof(tmp)-1, 16, STR_TERMINATE) == -1) {
		DEBUG_OUT (10, ("pull_ascii failed\n"));
		goto failed;
	}

	if (!(*servername = talloc_strdup(mem_ctx, tmp))) {
		DEBUG_OUT (1, ("talloc_strdup failed\n"));
		goto failed;
	}
...
bool cli_ns_check_server_type(struct cli_state *cli, char *workgroup, uint32 stype)
{
	...
	if (cli_api(cli,
		...
		if (res == 0 || res == ERRmoredata) {
			...
			DEBUG_OUT (4,("cli_ns_check_server_type: machine %s failed the NetServerEnum call. "
				"Error was : %s.\n", cli->desthost, cli_errstr(cli) ));
		}
	}
...
bool cli_NetWkstaUserLogoff(struct cli_state *cli, const char *user, const char *workstation)
{
	...
	if (cli_api(cli,
		...
		if (cli->rap_error != 0) {
			DEBUG_OUT (4,("NetwkstaUserLogoff gave error %d\n", cli->rap_error));
		}
	}
		...
		void (*jfn)(uint16,const char*,const char*,const char*,const char*,uint16,uint16,const char*,unsigned int,unsigned int,const char*))
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetPrintQEnum gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetPrintQEnum no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetPrintQEnum res=%d\n", res));
	}
	...
	void (*jfn)(uint16,const char*,const char*,const char*,const char*,uint16,uint16,const char*,unsigned int,unsigned int,const char*))
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetPrintQGetInfo gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetPrintQGetInfo no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetPrintQGetInfo res=%d\n", res));
	}
...
int cli_RNetServiceEnum(struct cli_state *cli, void (*fn)(const char *, const char *, void *), void *state)
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if(cli->rap_error == 234) {
			DEBUG_OUT (1,("Not all service names were returned (such as those longer than 15 characters)\n"));
		} else if (cli->rap_error != 0) {
			DEBUG_OUT (1,("NetServiceEnum gave error %d\n", cli->rap_error));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetServiceEnum no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetServiceEnum res=%d\n", res));
	}
...
int cli_NetSessionEnum(struct cli_state *cli, void (*fn)(char *, char *, uint16, uint16, uint16, unsigned int, unsigned int, unsigned int, char *))
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetSessionEnum gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetSesssionEnum no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetSessionEnum res=%d\n", res));
	}
		...
		void (*fn)(const char *, const char *, uint16, uint16, uint16, unsigned int, unsigned int, unsigned int, const char *))
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (cli->rap_error != 0) {
			DEBUG_OUT (1,("NetSessionGetInfo gave error %d\n", cli->rap_error));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetSessionGetInfo no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetSessionGetInfo res=%d\n", res));
	}
				...
				const char *netname))
{
	...
	if (cli_api(cli,
		...
		cli->rap_error = res;
		if (res != 0) {
			DEBUG_OUT (1,("NetConnectionEnum gave error %d\n", res));
		}
	}

	if (!rdata) {
		DEBUG_OUT (4,("NetConnectionEnum no data returned\n"));
		goto out;
	}
	...
	if (res == 0 || res == ERRmoredata) {
		...
		DEBUG_OUT (4,("NetConnectionEnum res=%d\n", res));
	}
