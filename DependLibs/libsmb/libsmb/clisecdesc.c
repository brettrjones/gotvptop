			    ...
			    TALLOC_CTX *mem_ctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("NT_TRANSACT_QUERY_SECURITY_DESC failed: %s\n",
			  nt_errstr(status)));
		goto cleanup;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("unmarshall_sec_desc failed: %s\n",
			   nt_errstr(status)));
		goto cleanup;
	}
			 ...
			 struct security_descriptor *sd)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("marshall_sec_desc failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
	...
	TALLOC_FREE(data);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to send NT_TRANSACT_SET_SECURITY_DESC: %s\n",
			  nt_errstr(status)));
	}
	return status;
}
