			    ...
			    SMB_NTQUOTA_STRUCT *pqt)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("NT_TRANSACT_GET_USER_QUOTA failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	if (!parse_user_quota_record(rdata, rdata_count, &offset, pqt)) {
		...
		DEBUG_OUT (0,("Got INVALID NT_TRANSACT_GET_USER_QUOTA reply.\n"));
	}
			    ...
			    SMB_NTQUOTA_STRUCT *pqt)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("NT_TRANSACT_SET_USER_QUOTA failed: %s\n",
			  nt_errstr(status)));
	}
			     ...
			     SMB_NTQUOTA_LIST **pqt_list)
{
	...
	for (curdata=rdata,curdata_count=rdata_count;
		...
		if (!parse_user_quota_record((uint8_t *)curdata, curdata_count,
					     &offset, &qt)) {
			DEBUG_OUT (1,("Failed to parse the quota record\n"));
			goto cleanup;
		}
			       ...
			       SMB_NTQUOTA_STRUCT *pqt)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("SMB_FS_QUOTA_INFORMATION failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
