	...
	const char *workgroup)
{
	...
	if (passlen > 0
		...
		if (!SMBencrypt(pass, cli->secblob.data,
			...
			DEBUG_OUT (1, ("Password is > 14 chars in length, and is "
						  ...
						  DATA_BLOB blob)
{
	...
	if (cli->max_xmit < BASE_SESSSETUP_BLOB_PACKET_SIZE + 1) {
		DEBUG_OUT (1, ("cli_session_setup_blob: cli->max_xmit too small "
	...
	const char *principal, const char *workgroup)
{
	...
	DEBUG_OUT (2,("Doing kerberos session setup\n"));
	...
	if (rc) {
		DEBUG_OUT (1, ("cli_session_setup_kerberos: "
...
static void cli_session_setup_ntlmssp_done(struct tevent_req *subreq)
{
	...
	if (!parse_ret) {
		DEBUG_OUT (3,("Failed to parse auth response\n"));
		if (NT_STATUS_IS_OK(status)
			      ...
			      const char * dest_realm)
{
	...
	DEBUG_OUT (3,("Doing spnego session setup (blob length=%lu)\n", (unsigned long)cli->secblob.length));

	/* the server might not even do spnego */
	if (cli->secblob.length <= 16) {
		DEBUG_OUT (3,("server didn't supply a full spnego negprot\n"));
		goto ntlmssp;
	}
	...
	if (cli->got_kerberos_mechanism && cli->use_kerberos) {
		...
		if (principal == NULL &&
			...
			char *realm = NULL;
			char *host = NULL;
			DEBUG_OUT (3,("cli_session_setup_spnego: using target "
			...
			if (realm == NULL || *realm == '\0') {
				...
				strupper_m(realm);
				DEBUG_OUT (3,("cli_session_setup_spnego: cannot "
			...
			DEBUG_OUT (3,("cli_session_setup_spnego: guessed "
				"server principal=%s\n",
				principal ? principal : "<null>"));
			   ...
			   const char *workgroup)
{
	...
	if (cli->protocol < PROTOCOL_NT1) {
		if (!lp_client_lanman_auth() && passlen != 24 && (*pass)) {
			DEBUG_OUT (1, ("Server requested LM password but 'client lanman auth = no'"
		...
		if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0 &&
			...
			DEBUG_OUT (1, ("Server requested LM password but 'client plaintext auth = no'"
				  " or 'client ntlmv2 auth = yes'\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	if (cli->capabilities & CAP_EXTENDED_SECURITY) {
		...
		if (!ADS_ERR_OK(status)) {
			DEBUG_OUT (3, ("SPNEGO login failed: %s\n", ads_errstr(status)));
		...
		status = cli_session_setup_nt1(cli, user, pass, passlen,
					       ntpass, ntpasslen, workgroup);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3,("cli_session_setup: NT1 session setup "
				 "failed: %s\n", nt_errstr(status)));
			return status;
		}
	}
					...
					struct tevent_req **psmbreq)
{
	...
	*psmbreq = NULL;
	...
	if (cli->sec_mode & NEGOTIATE_SECURITY_USER_LEVEL) {
		...
		DEBUG_OUT (1, ("Server not using user level security and no "
			  "password supplied.\n"));
		goto access_denied;
	}
	...
	if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) &&
	    *pass && passlen != 24) {
		if (!lp_client_lanman_auth()) {
			DEBUG_OUT (1, ("Server requested LANMAN password "
		...
		if((cli->sec_mode & (NEGOTIATE_SECURITY_USER_LEVEL
		   ...
		   == 0) {
			...
			if (!lp_client_plaintext_auth() && (*pass)) {
				DEBUG_OUT (1, ("Server requested plaintext "
					  "password but "
					  "'client lanman auth = no' or 'client ntlmv2 auth = yes'\n"));
				goto access_denied;
			}
...
static void cli_negprot_done(struct tevent_req *subreq)
{
	...
	if ((cli->protocol < PROTOCOL_NT1) &&
		...
		DEBUG_OUT (0,("cli_negprot: SMB signing is mandatory and the selected protocol level doesn't support it.\n"));
		tevent_req_nterror(req, NT_STATUS_ACCESS_DENIED);
		return;
	}
	...
	if (cli->protocol >= PROTOCOL_NT1) {    
		...
		if (cli->sec_mode & NEGOTIATE_SECURITY_SIGNATURES_REQUIRED) {
			...
			if (!client_is_signing_allowed(cli)) {
				DEBUG_OUT (0,("cli_negprot: SMB signing is mandatory and we have disabled it.\n"));
			...
			if (!(cli->sec_mode & NEGOTIATE_SECURITY_SIGNATURES_ENABLED)) {
				DEBUG_OUT (1,("cli_negprot: SMB signing is mandatory and the server doesn't support it.\n"));
				tevent_req_nterror(req,
						   NT_STATUS_ACCESS_DENIED);
				return;
			}
			 ...
			 struct nmb_name *calling, struct nmb_name *called)
{
	...
	cli_send_smb(cli);
	DEBUG_OUT (5,("Sent session request\n"));
	...
	if (CVAL(cli->inbuf,0) == 0x84) {
		...
		*/
		...
		/* Try again */
		{
			...
			bool ret;
			if (depth > 4) {
				DEBUG_OUT (0,("Retarget recursion - failing\n"));
				return False;
			}
			depth++;
...

{
	...
	for (i = 0; i < num_addrs; i++) {
		...
		if (cli->fd == -1) {
			...
			DEBUG_OUT (2,("Error connecting to %s (%s)\n",
...
again:
	...
	DEBUG_OUT (3,("Connecting to host=%s\n", dest_host));

	nt_status = cli_connect(cli, dest_host, &ss);
	if (!NT_STATUS_IS_OK(nt_status)) {
		char addr[INET6_ADDRSTRLEN];
		print_sockaddr(addr, sizeof(addr), &ss);
		DEBUG_OUT (1,("cli_start_connection: failed to connect to %s (%s). Error %s\n",
	...
	if (!cli_session_request(cli, &calling, &called)) {
		char *p;
		DEBUG_OUT (1,("session request to %s failed (%s)\n",
	...
	*output_cli = NULL;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		...
		if (!(flags & CLI_FULL_CONNECTION_ANONYMOUS_FALLBACK)) {
			DEBUG_OUT (1,("failed session setup with %s\n",
				 nt_errstr(nt_status)));
			cli_shutdown(cli);
			return nt_status;
		}
		...
		if (!NT_STATUS_IS_OK(nt_status)) {
			DEBUG_OUT (1,("anonymous failed session setup with %s\n",
				 nt_errstr(nt_status)));
			cli_shutdown(cli);
			return nt_status;
		}
                                     ...
                                     struct sockaddr_storage *pdest_ss)
{
	...
	if (!cli_session_request(*ppcli, &calling, &called)) {
		...
		if (nmb_name_equal(&called, &smbservername)) {
			...
			DEBUG_OUT (0,("attempt_netbios_session_request: %s rejected the session for name *SMBSERVER \
with error %s.\n", desthost, cli_errstr(*ppcli) ));
		...
		*ppcli = cli_initialise();
		...
		if (!NT_STATUS_IS_OK(status) ||
			...
			DEBUG_OUT (0,("attempt_netbios_session_request: %s rejected the session for \
		      ...
		      uint16 *max_xmit, uint16 *tid)
{
	...
	if (!lp_client_plaintext_auth() && (*pass)) {
		DEBUG_OUT (1, ("Server requested plaintext password but 'client "
				...
				char **pp_workgroup_out)
{
	...
	*pp_workgroup_out = NULL;
	...
	print_sockaddr(addr, sizeof(addr), mb_ip);
        DEBUG_OUT (99, ("Looking up name of master browser %s\n",
                   addr));
        ...
        if (!name_status_find("*", 0, 0x1d, mb_ip, name) &&
            !name_status_find(MSBROWSE, 1, 0x1d, mb_ip, name)) {
                ...
                DEBUG_OUT (99, ("Could not retrieve name status for %s\n",
                           addr));
                return NULL;
        }

        if (!find_master_ip(name, &server_ss)) {
                DEBUG_OUT (99, ("Could not find master ip for %s\n", name));
                return NULL;
        }

	*pp_workgroup_out = talloc_strdup(ctx, name);

	DEBUG_OUT (4, ("found master browser %s, %s\n", name, addr));
					...
					char **pp_workgroup_out)
{
	...
	*pp_workgroup_out = NULL;
        ...
        DEBUG_OUT (99, ("Do broadcast lookup for workgroups on local network\n"));
	...
	status = name_resolve_bcast(MSBROWSE, 1, talloc_tos(),
				    &ip_list, &count);
        if (!NT_STATUS_IS_OK(status)) {
                DEBUG_OUT (99, ("No master browsers responded: %s\n",
			   nt_errstr(status)));
                return False;
        }
	...
	for (i = 0; i < count; i++) {
		...
		DEBUG_OUT (99, ("Found master browser %s\n", addr));
