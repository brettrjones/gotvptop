...
again:
	tdb = tdb_open_log(path, 0, TDB_DEFAULT|TDB_INCOMPATIBLE_HASH,
			   O_RDWR | O_CREAT, 0600);
	if (tdb == NULL) {
		DEBUG_OUT (0,("tdb_open_log('%s') - failed\n", path));
		goto clear;
	}
	...
	if (ret != 0) {
		tdb_close(tdb);
		DEBUG_OUT (0,("tdb_check('%s') - failed\n", path));
		goto clear;
	}
...
clear:
	...
	DEBUG_OUT (0,("retry after CLEAR_IF_FIRST for '%s'\n", path));
	...
	DEBUG_OUT (0,("tdb_open_log(%s) with CLEAR_IF_FIRST - failed\n", path));
...
void netsamlogon_clear_cached_user(const struct dom_sid *user_sid)
{
	...
	if (!netsamlogon_cache_init()) {
		DEBUG_OUT (0,("netsamlogon_clear_cached_user: cannot open "
			"%s for write!\n",
			NETSAMLOGON_TDB));
		return;
	}
	...
	DEBUG_OUT (10,("netsamlogon_clear_cached_user: SID [%s]\n", keystr));
...
bool netsamlogon_cache_store(const char *username, struct netr_SamInfo3 *info3)
{
	...
	if (!netsamlogon_cache_init()) {
		DEBUG_OUT (0,("netsamlogon_cache_store: cannot open %s for write!\n",
			NETSAMLOGON_TDB));
		return false;
	}
	...
	DEBUG_OUT (10,("netsamlogon_cache_store: SID [%s]\n", keystr));

	/* Prepare data */

	if (!(mem_ctx = TALLOC_P( NULL, int))) {
		DEBUG_OUT (0,("netsamlogon_cache_store: talloc() failed!\n"));
		return false;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0,("netsamlogon_cache_store: failed to push entry to cache\n"));
		TALLOC_FREE(mem_ctx);
		return false;
	}
...
struct netr_SamInfo3 *netsamlogon_cache_get(TALLOC_CTX *mem_ctx, const struct dom_sid *user_sid)
{
	...
	if (!netsamlogon_cache_init()) {
		DEBUG_OUT (0,("netsamlogon_cache_get: cannot open %s for write!\n",
			NETSAMLOGON_TDB));
		return false;
	}
	...
	DEBUG_OUT (10,("netsamlogon_cache_get: SID [%s]\n", keystr));
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0,("netsamlogon_cache_get: failed to pull entry from cache\n"));
	   ...
	   --jerry */
	{
		...
		if ( (time_diff < 0 ) || (time_diff > lp_winbind_cache_time()) ) {
			DEBUG_OUT (10,("netsamlogon_cache_get: cache entry expired \n"));
