               ...
               const char *str)
{
	...
	while (next_token_talloc(ctx, &p, &tok, "\t,\r\n")) {
		...
		if (StrnCaseCmp(tok,"OWNER:", 6) == 0) {
			if (owner_sid) {
				DEBUG_OUT (5,("OWNER specified more than once!\n"));
				goto done;
			}
			...
			if (!owner_sid ||
			    !convert_string_to_sid(ipc_cli, pol,
                                                   numeric,
                                                   owner_sid, tok+6)) {
				DEBUG_OUT (5, ("Failed to parse owner sid\n"));
				goto done;
			}
			continue;
		}
		...
		if (StrnCaseCmp(tok,"GROUP:", 6) == 0) {
			if (group_sid) {
				DEBUG_OUT (5,("GROUP specified more than once!\n"));
				goto done;
			}
			...
			if (!group_sid ||
			    !convert_string_to_sid(ipc_cli, pol,
                                                   numeric,
                                                   group_sid, tok+6)) {
				DEBUG_OUT (5, ("Failed to parse group sid\n"));
				goto done;
			}
			continue;
		}
		...
		if (StrnCaseCmp(tok,"ACL:", 4) == 0) {
			...
			if (!parse_ace(ipc_cli, pol, &ace, numeric, tok+4)) {
				DEBUG_OUT (5, ("Failed to parse ACL %s\n", tok));
				goto done;
			}
			if(!add_ace(&dacl, &ace, ctx)) {
				DEBUG_OUT (5, ("Failed to add ACL %s\n", tok));
				goto done;
			}
			continue;
		}
		...
		DEBUG_OUT (5, ("Failed to parse security descriptor\n"));
		goto done;
	}
               ...
               SMBCSRV *srv)
{
	...
	SMB_INO_T inode = 0;
        ...
        if (!SMBC_getatr(context, srv, CONST_DISCARD(char *, filename),
                ...
                DEBUG_OUT (5, ("dos_attr_query Failed to query old attributes\n"));
                return NULL;
        }
         ...
         int bufsize)
{
	...
	struct cli_state *cli = srv->cli;
                ...
                for (;
                     ...
                     pExclude = (p == NULL ? NULL : p + 1)) {
                        ...
                        if (StrCaseCmp(pExclude,
                                ...
                                DEBUG_OUT (5, ("cacl_get received unknown exclusion: %s\n",
                                          pExclude));
                                errno = ENOATTR;
                                return -1;
                        }
		...
		if (!cli_resolve_path(ctx, "", context->internal->auth_info,
			...
			DEBUG_OUT (5, ("cacl_get Could not resolve %s\n",
				filename));
                        errno = ENOENT;
                        return -1;
		}
			...
			DEBUG_OUT (5, ("cacl_get failed to open %s: %s\n",
	...
	int flags)
{
	...
	if (!cli_resolve_path(ctx, "", context->internal->auth_info,
			cli, filename,
			&targetcli, &targetpath)) {
		DEBUG_OUT (5,("cacl_set: Could not resolve %s\n", filename));
		errno = ENOENT;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(cli_ntcreate(targetcli, targetpath, 0, CREATE_ACCESS_READ, 0,
				FILE_SHARE_READ|FILE_SHARE_WRITE, FILE_OPEN, 0x0, 0x0, &fnum))) {
                DEBUG_OUT (5, ("cacl_set failed to open %s: %s\n",
                          targetpath, cli_errstr(targetcli)));
                errno = 0;
		return -1;
	}
	...
	if (!old) {
                DEBUG_OUT (5, ("cacl_set Failed to query old descriptor\n"));
                errno = 0;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("ERROR: secdesc set failed: %s\n",
			  nt_errstr(status)));
		ret = -1;
	}
                  ...
                  int flags)
{
	...
	DEBUG_OUT (4, ("smbc_setxattr(%s, %s, %.*s)\n",
                     ...
                     const char *name)
{
	...
	if (!context || !context->internal->initialized) {
        ...
        DEBUG_OUT (4, ("smbc_removexattr(%s, %s)\n", fname, name));
