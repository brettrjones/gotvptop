                          ...
                          time_t timeout)
{
	...
	DEBUG_OUT (5, ("trustdom_store: storing SID %s of domain %s\n",
...
bool trustdom_cache_fetch(const char* name, struct dom_sid* sid)
{
	...
	if (!gencache_get(key, &value, &timeout)) {
		DEBUG_OUT (5, ("no entry for trusted domain %s found.\n", name));
		SAFE_FREE(key);
		return False;
	} else {
		SAFE_FREE(key);
		DEBUG_OUT (5, ("trusted domain %s found (%s)\n", name, value));
	}
...
uint32 trustdom_cache_fetch_timestamp( void )
{
	...
	if (!gencache_get(TDOMTSKEY, &value, &timeout)) {
		DEBUG_OUT (5, ("no timestamp for trusted domain cache located.\n"));
		SAFE_FREE(value);
		return 0;
	} 
...
bool trustdom_cache_store_timestamp( uint32 t, time_t timeout )
{
	...
	if (!gencache_set(TDOMTSKEY, value, timeout)) {
		DEBUG_OUT (5, ("failed to set timestamp for trustdom_cache\n"));
		return False;
	} 
...
void trustdom_cache_flush(void)
{
	...
	DEBUG_OUT (5, ("Trusted domains cache flushed\n"));
}
...
void update_trustdom_cache( void )
{
	...
	if ( (time_diff > 0) && (time_diff < TRUSTDOM_UPDATE_INTERVAL) ) {
		DEBUG_OUT (10,("update_trustdom_cache: not time to update trustdom_cache yet\n"));
		return;
	}
	...
	if ( !(mem_ctx = talloc_init("update_trustdom_cache")) ) {
		DEBUG_OUT (0,("update_trustdom_cache: talloc_init() failed!\n"));
		goto done;
	}
