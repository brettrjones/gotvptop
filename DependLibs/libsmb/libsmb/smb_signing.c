			     ...
			     bool good, uint32_t seq)
{
	...
	if (!si->mandatory && !si->active) {
		...
		DEBUG_OUT (5, ("smb_signing_good: signing negotiated but not required and peer\n"
	...
	DEBUG_OUT (0, ("smb_signing_good: BAD SIG: seq %u\n", (unsigned int)seq));
	return false;
}
			    ...
			    uint8_t calc_md5_mac[16])
{
	...
	DEBUG_OUT (10,("smb_signing_md5: sequence number %u\n", seq_number ));
			  ...
			  uint8_t *outbuf, uint32_t seqnum)
{
	...
	if (smb_len(outbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG_OUT (1,("smb_signing_sign_pdu: Logic error. "
	...
	DEBUG_OUT (10, ("smb_signing_sign_pdu: sent SMB signature of\n"));
			   ...
			   const uint8_t *inbuf, uint32_t seqnum)
{
	...
	if (smb_len(inbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG_OUT (1,("smb_signing_check_pdu: Can't check signature "
	...
	if (!good) {
		...
		DEBUG_OUT (5, ("smb_signing_check_pdu: BAD SIG: wanted SMB signature of\n"));
		dump_data(5, calc_md5_mac, 8);

		DEBUG_OUT (5, ("smb_signing_check_pdu: BAD SIG: got SMB signature of\n"));
		...
		for (i = -sign_range; i < sign_range; i++) {
			...
			if (memcmp(reply_sent_mac, calc_md5_mac, 8) == 0) {
				DEBUG_OUT (0,("smb_signing_check_pdu: "
		...
		DEBUG_OUT (10, ("smb_signing_check_pdu: seq %u: "
			  ...
			  const DATA_BLOB response)
{
	...
	DEBUG_OUT (10, ("smb_signing_activate: user_session_key\n"));
	...
	if (response.length) {
		...
		DEBUG_OUT (10, ("smb_signing_activate: response_data\n"));
		dump_data(10, response.data, response.length);
	} else {
		DEBUG_OUT (10, ("smb_signing_activate: NULL response_data\n"));
	}
