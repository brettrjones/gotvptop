			...
			const DATA_BLOB input, DATA_BLOB *out)
{
	...
	if (ntlmssp_state->expected_state == NTLMSSP_DONE) {
		...
		DEBUG_OUT (1, ("Called NTLMSSP after state machine was 'done'\n"));
	...
	*out = data_blob_null;
	...
	if (!input.length) {
		...
		if (!msrpc_parse(ntlmssp_state, &input, "Cd",
				 "NTLMSSP",
				 &ntlmssp_command)) {
			DEBUG_OUT (1, ("Failed to parse NTLMSSP packet, could not extract NTLMSSP command\n"));
	...
	if (ntlmssp_command != ntlmssp_state->expected_state) {
		DEBUG_OUT (1, ("got NTLMSSP command %u, expected %u\n", ntlmssp_command, ntlmssp_state->expected_state));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG_OUT (1, ("failed to find NTLMSSP callback for NTLMSSP mode %u, command %u\n",
				  ...
				  DATA_BLOB reply, DATA_BLOB *next_request)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("ntlmssp_client_initial: failed to generate "
...
noccache:
	...
	if (!msrpc_parse(ntlmssp_state, &reply, "CdBd",
			 ...
			 &server_domain_blob,
			 &chal_flags)) {
		DEBUG_OUT (1, ("Failed to parse the NTLMSSP Challenge: (#1)\n"));
	...
	DEBUG_OUT (3, ("Got challenge flags:\n"));
	...
	DEBUG_OUT (3, ("NTLMSSP: Set final flags:\n"));
	...
	if (!msrpc_parse(ntlmssp_state, &reply, chal_parse_string,
			 ...
			 &unkn1, &unkn2,
			 &struct_blob)) {
		DEBUG_OUT (1, ("Failed to parse the NTLMSSP Challenge: (#2)\n"));
		dump_data(2, reply.data, reply.length);
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (!ntlmssp_state->nt_hash || !ntlmssp_state->lm_hash) {
		...
		if (!struct_blob.length) {
			...
			DEBUG_OUT (1, ("Server did not provide 'target information', required for NTLMv2\n"));
			return NT_STATUS_INVALID_PARAMETER;
		}
		...
		DEBUG_OUT (5, ("NTLMSSP challenge set by NTLM2\n"));
	...
	if (!NT_STATUS_IS_OK(nt_status = ntlmssp_sign_init(ntlmssp_state))) {
		DEBUG_OUT (1, ("Could not setup NTLMSSP signing/sealing system (error was: %s)\n", nt_errstr(nt_status)));
	}
