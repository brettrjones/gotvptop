				      ...
				      void *private_data)
{
	...
	if (client->out_queue == NULL) {
		DEBUG_OUT (10, ("tevent_queue_create failed\n"));
		TALLOC_FREE(client);
		return;
	}
	...
	if (req == NULL) {
		DEBUG_OUT (10, ("read_packet_send failed\n"));
		TALLOC_FREE(client);
		return;
	}
	...
	if (server->num_clients > server->max_clients) {
		DEBUG_OUT (10, ("Too many clients, dropping oldest\n"));
				     ...
				     void *private_data)
{
	...
	if (q.mailslot_namelen > 1024) {
		DEBUG_OUT (10, ("Got invalid mailslot namelen %d\n",
...
static void nb_packet_got_query(struct tevent_req *req)
{
	...
	if (nread < (ssize_t)sizeof(struct nb_packet_query)) {
		DEBUG_OUT (10, ("read_packet_recv returned %d (%s)\n",
			   (int)nread,
	...
	if (nread != sizeof(struct nb_packet_query) + q.mailslot_namelen) {
		DEBUG_OUT (10, ("nb_packet_got_query: Invalid mailslot namelength\n"));
		TALLOC_FREE(client);
		return;
	}
	...
	if (nwritten != sizeof(c)) {
		DEBUG_OUT (10, ("Could not write success indicator to client: %s\n",
			   strerror(errno)));
		TALLOC_FREE(client);
		return;
	}
	...
	if (client->read_req == NULL) {
		DEBUG_OUT (10, ("Could not activate reader for client exit "
			   "detection\n"));
		TALLOC_FREE(client);
		return;
	}
...
static void nb_packet_client_read_done(struct tevent_req *req)
{
	...
	TALLOC_FREE(req);
	if (nread == 1) {
		DEBUG_OUT (10, ("Protocol error, received data on write-only "
			...
			struct packet_struct *p)
{
	...
	case DGRAM_PACKET:
		...
		break;
	default:
		DEBUG_OUT (10, ("Got invalid packet type %d\n",
			   (int)p->packet_type));
		return;
	}
	for (c = server->clients; c != NULL; c = c->next) {
		...
		if (c->type != p->packet_type) {
			DEBUG_OUT (10, ("client expects packet %d, got %d\n",
		...
		if (p->packet_type == NMB_PACKET) {
			...
			if ((c->trn_id != -1) &&
			    (c->trn_id != trn_id)) {
				DEBUG_OUT (10, ("client expects trn %d, got %d\n",
					   c->trn_id, trn_id));
				continue;
			}
		} else {
				  ...
				  struct packet_struct *p)
{
	...
	if (req == NULL) {
		DEBUG_OUT (10, ("writev_send failed\n"));
		return;
	}
...
static void nb_packet_reader_connected(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (res == -1) {
		DEBUG_OUT (10, ("async_connect failed: %s\n", strerror(err)));
...
static void nb_packet_reader_got_ack(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (nread == -1) {
		DEBUG_OUT (10, ("read_packet_recv returned %s\n",
			   strerror(err)));
		tevent_req_nterror(req, map_nt_error_from_unix(err));
		return;
	}
	if (nread != sizeof(state->c)) {
		DEBUG_OUT (10, ("read = %d, expected %d\n", (int)nread,
			   (int)sizeof(state->c)));
		tevent_req_nterror(req, NT_STATUS_UNEXPECTED_IO_ERROR);
		return;
	}
