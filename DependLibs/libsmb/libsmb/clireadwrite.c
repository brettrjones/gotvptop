					...
					struct tevent_req **psmbreq)
{
	...
	if (size > cli_read_max_bufsize(cli)) {
		DEBUG_OUT (0, ("cli_read_andx_send got size=%d, can only handle "
			  "size=%d\n", (int)size,
	...
	if (cli->capabilities & CAP_LARGE_FILES) {
		...
		if ((((uint64_t)offset) & 0xffffffff00000000LL) != 0) {
			DEBUG_OUT (10, ("cli_read_andx_send got large offset where "
...
static void cli_read_andx_done(struct tevent_req *subreq)
{
	...
	if (state->received > state->size) {
		DEBUG_OUT (5,("server returned more than we wanted!\n"));
	...
	if ((state->received < 0xffff) && (state->received > num_bytes)) {
		DEBUG_OUT (5, ("server announced more bytes than sent\n"));
	...
	if (trans_oob(smb_len_large(inbuf), SVAL(vwv+6, 0), state->received)
		...
		DEBUG_OUT (5, ("server returned invalid read&x data offset\n"));
		tevent_req_nterror(req, NT_STATUS_INVALID_NETWORK_RESPONSE);
		return;
	}
	tevent_req_done(req);
...
static void cli_pull_read_done(struct tevent_req *subreq)
{
	...
	while (state->reqs[state->top_req].req != NULL) {
		...
		DEBUG_OUT (11, ("cli_pull_read_done: top_req = %d\n",
			   state->top_req));

		top_subreq = &state->reqs[state->top_req];

		if (tevent_req_is_in_progress(top_subreq->req)) {
			DEBUG_OUT (11, ("cli_pull_read_done: top request not yet "
				   "done\n"));
			return;
		}

		DEBUG_OUT (10, ("cli_pull_read_done: Pushing %d bytes, %d already "
		...
		if (state->requested < state->size) {
			...
			DEBUG_OUT (10, ("cli_pull_read_done: Requesting %d bytes "
				   "at %d, position %d\n",
