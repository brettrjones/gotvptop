	...
	if (cli_api(cli,
		    ...
		    &rdata, &rdrcnt))                /* return data, length */
		{
			...
			if (res == 0 || res == ERRmoredata) {
				...
				DEBUG_OUT (4,("NetShareEnum res=%d\n", res));
			}
		} else {
			DEBUG_OUT (4,("NetShareEnum failed\n"));
		}
                             ...
                             const char *old_password)
{
	...
	if (strlen(user) >= sizeof(fstring)-1) {
		DEBUG_OUT (0,("cli_oem_change_password: user name %s is too long.\n", user));
		return False;
	}
	...
	DEBUG_OUT (100,("make_oem_passwd_hash\n"));
	...
	if (!cli_api(cli,
		     ...
		     &rparam, &rprcnt,
		     &rdata, &rdrcnt)) {
		DEBUG_OUT (0,("cli_oem_change_password: Failed to send password change for user %s\n",
			user ));
		return False;
	}
