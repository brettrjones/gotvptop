...
static char *negative_conn_cache_keystr(const char *domain, const char *server)
{
	...
	keystr = talloc_asprintf(talloc_tos(), "NEG_CONN_CACHE/%s,%s",
				 domain, server);
	if (keystr == NULL) {
		DEBUG_OUT (0, ("negative_conn_cache_keystr: malloc error\n"));
	}
...
static char *negative_conn_cache_valuestr(NTSTATUS status)
{
	...
	if (valuestr == NULL) {
		DEBUG_OUT (0, ("negative_conn_cache_valuestr: malloc error\n"));
	}
...
static NTSTATUS negative_conn_cache_valuedecode(const char *value)
{
	...
	if (sscanf(value, "%x", &v) != 1) {
		DEBUG_OUT (0, ("negative_conn_cache_valuedecode: unable to parse "
			  "value field '%s'\n", value));
	}
	return NT_STATUS(v);
}
...
NTSTATUS check_negative_conn_cache( const char *domain, const char *server)
{
	...
	DEBUG_OUT (9,("check_negative_conn_cache returning result %d for domain %s "
    ...
    NTSTATUS result)
{
	...
	key = negative_conn_cache_keystr(domain, server);
	if (key == NULL) {
		DEBUG_OUT (0, ("add_failed_connection_entry: key creation error\n"));
		goto done;
	}

	value = negative_conn_cache_valuestr(result);
	if (value == NULL) {
		DEBUG_OUT (0, ("add_failed_connection_entry: value creation error\n"));
		goto done;
	}

	if (gencache_set(key, value,
			 time(NULL) + FAILED_CONNECTION_CACHE_TIMEOUT))
		DEBUG_OUT (9,("add_failed_connection_entry: added domain %s (%s) "
			  "to failed conn cache\n", domain, server ));
	else
		DEBUG_OUT (1,("add_failed_connection_entry: failed to add "
...
void flush_negative_conn_cache_for_domain(const char *domain)
{
	...
	if (key_pattern == NULL) {
		DEBUG_OUT (0, ("flush_negative_conn_cache_for_domain: "
			  "key creation error\n"));
		goto done;
	}

	gencache_iterate(delete_matches, NULL, key_pattern);
	DEBUG_OUT (8, ("flush_negative_conn_cache_for_domain: flushed domain %s\n",
		  domain));
