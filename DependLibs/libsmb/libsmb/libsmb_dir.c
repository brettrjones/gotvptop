                   ...
                   void *state)
{
	...
	struct dcerpc_binding_handle *b;
        ...
        if (!NT_STATUS_IS_OK(nt_status)) {
                DEBUG_OUT (1, ("net_share_enum_rpc pipe open fail!\n"));
                return -1;
        }
...
SMBC_check_options(char *server,
                   ...
                   char *options)
{
        DEBUG_OUT (4, ("SMBC_check_options(): server='%s' share='%s' "
                  "path='%s' options='%s'\n",
                 ...
                 const char *fname)
{
	...
	if (SMBC_parse_path(frame,
                            ...
                            &password,
                            &options)) {
	        DEBUG_OUT (4, ("no valid path\n"));
		TALLOC_FREE(frame);
		errno = EINVAL + 8194;
		return NULL;
	}

	DEBUG_OUT (4, ("parsed path: fname='%s' server='%s' share='%s' "
                  "path='%s' options='%s'\n",
                  fname, server, share, path, options));

        /* Ensure the options are valid */
        if (SMBC_check_options(server, share, path, options)) {
                DEBUG_OUT (4, ("unacceptable options (%s)\n", options));
		TALLOC_FREE(frame);
                errno = EINVAL + 8195;
                return NULL;
        }
	...
	if (server[0] == (char)0) {
                ...
                if (!NT_STATUS_IS_OK(status))
		{
			...
			print_sockaddr(addr, sizeof(addr), &ip_list[i]);
                        DEBUG_OUT (99, ("Found master browser %d of %d: %s\n",
			...
			if (!workgroup || !server) {
				...
				return NULL;
                        ...
                        DEBUG_OUT (4, ("using workgroup %s %s\n",
                                  workgroup, server));
			...
			if (!srv &&
				...
				if (!name_status_find(wgroup, 0, 0,
					...
					print_sockaddr(addr, sizeof(addr), &rem_ss);
                                        DEBUG_OUT (0,("Could not get name of "
				...
				srv = SMBC_server(frame, context, True,
                                                  ...
                                                  &user, &password);
				if (!srv) {
				        DEBUG_OUT (0, ("got no contact to IPC$\n"));
					if (dir) {
						SAFE_FREE(dir->fname);
						SAFE_FREE(dir);
					}
                 ...
                 SMBCFILE *dir)
{
	...
	if (!context || !context->internal->initialized) {
		...
		errno = EINVAL;
                DEBUG_OUT (0, ("Invalid context in SMBC_readdir_ctx()\n"));
		TALLOC_FREE(frame);
		return NULL;
	...
	if (!dir || !SMBC_dlist_contains(context->internal->files, dir)) {
		...
		errno = EBADF;
                DEBUG_OUT (0, ("Invalid dir in SMBC_readdir_ctx()\n"));
		TALLOC_FREE(frame);
		return NULL;
	...
	if (dir->file != False) { /* FIXME, should be dir, perhaps */
		...
		errno = ENOTDIR;
                DEBUG_OUT (0, ("Found file vs directory in SMBC_readdir_ctx()\n"));
		TALLOC_FREE(frame);
		return NULL;
