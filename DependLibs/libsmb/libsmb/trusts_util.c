				      ...
				      enum netr_SchannelType sec_channel_type)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("%s : trust_pw_change_and_store_it: Changed password.\n", 
					   ...
					   const char *domain) 
{
	...
	if (!get_trust_pw_hash(domain, old_trust_passwd_hash, &account_name,
			       &sec_channel_type)) {
		DEBUG_OUT (0, ("could not fetch domain secrets for domain %s!\n", domain));
				     ...
				     struct dom_sid **sids )
{
	...
	*sids = NULL;
	...
	if ( !get_dc_name(domain, NULL, dc_name, &dc_ss) ) {
		DEBUG_OUT (3,("enumerate_domain_trusts: can't locate a DC for domain %s\n",
			domain));
		return False;
	}
	...
	*sids = TALLOC_ZERO_ARRAY(mem_ctx, struct dom_sid, *num_domains);
	...
	/* cleanup */
	if (cli) {
		DEBUG_OUT (10,("enumerate_domain_trusts: shutting down connection...\n"));
		cli_shutdown( cli );
	}
...
NTSTATUS change_trust_account_password( const char *domain, const char *remote_machine)
{
	...
	DEBUG_OUT (5,("change_trust_account_password: Attempting to change trust account password in domain %s....\n",
		domain));
	...
	if (remote_machine == NULL || !strcmp(remote_machine, "*")) {
		...
		if ( !get_pdc_ip(domain, &pdc_ss) ) {
			DEBUG_OUT (0,("Can't get IP for PDC for domain %s\n", domain));
			goto failed;
		}
	...
	if (!NT_STATUS_IS_OK(cli_full_connection(&cli, global_myname(), dc_name,
					   ...
					   "", "",
					   "", 0, Undefined))) {
		DEBUG_OUT (0,("modify_trust_password: Connection to %s failed!\n", dc_name));
		nt_status = NT_STATUS_UNSUCCESSFUL;
		goto failed;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("modify_trust_password: unable to open the domain client session to machine %s. Error was : %s.\n",
...
failed:
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("%s : change_trust_account_password: Failed to change password for domain %s.\n",
			current_timestring(talloc_tos(), False), domain));
	}
	else
		DEBUG_OUT (5,("change_trust_account_password: sucess!\n"));
