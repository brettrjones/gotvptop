			...
			struct ip_service *ip_list)
{
	...
	if ( DEBUGLEVEL >= 5 ) {
		...
		DEBUG_OUT (5, ("namecache_store: storing %d address%s for %s#%02x: ",
			...
			int *num_names)
{
	...
	*num_names = 0;
	...
	if (!gencache_get(key, &value, &timeout)) {
		DEBUG_OUT (5, ("no entry for %s#%02X found.\n", name, name_type));
		SAFE_FREE(key);
		return False;
	}

	DEBUG_OUT (5, ("name %s#%02X found.\n", name, name_type));
		...
		const char *srvname)
{
	...
	if (ret) {
		DEBUG_OUT (5, ("namecache_status_store: entry %s -> %s\n",
					key, srvname ));
	} else {
		DEBUG_OUT (5, ("namecache_status_store: entry %s store failed.\n",
					key ));
	}
				...
				char *srvname_out)
{
	...
	if (!gencache_get(key, &value, &timeout)) {
		DEBUG_OUT (5, ("namecache_status_fetch: no entry for %s found.\n",
					key));
		SAFE_FREE(key);
		return False;
	} else {
		DEBUG_OUT (5, ("namecache_status_fetch: key %s -> %s\n",
					key, value ));
	}
