					       ...
					       const char *name)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("svcctl_set_secdesc: Could not open %s - %s\n",
	...
	status = dcerpc_winreg_query_sd(mem_ctx,
					...
					&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("svcctl_get_secdesc: error getting value 'Security': "
			  "%s\n", nt_errstr(status)));
		return NULL;
	}
...
fallback_to_default_sd:
	DEBUG_OUT (6, ("svcctl_get_secdesc: constructing default secdesc for "
		  "service [%s]\n", name));
			...
			struct security_descriptor *sd)
{
	...
	status = dcerpc_winreg_int_hklm_openkey(tmp_ctx,
						...
						&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("svcctl_set_secdesc: Could not open %s - %s\n",
			  key, nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(result)) {
...

	{
		...
		status = dcerpc_winreg_CreateKey(h,
						 ...
						 &result);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("svcctl_set_secdesc: Could not create key %s: %s\n",
				    ...
				    const char *value_name)
{
	...
	status = dcerpc_winreg_int_hklm_openkey(tmp_ctx,
						...
						&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("svcctl_get_string_value: Could not open %s - %s\n",
