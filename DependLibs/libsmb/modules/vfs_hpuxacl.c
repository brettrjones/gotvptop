				      ...
				      SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("hpuxacl_sys_acl_get_file called for file '%s'.\n", 
		   path_p));
	...
	if (type != SMB_ACL_TYPE_ACCESS && type != SMB_ACL_TYPE_DEFAULT) {
		DEBUG_OUT (10, ("invalid SMB_ACL_TYPE given (%d)\n", type));
		errno = EINVAL;
		goto done;
	}
	...
	if (result == NULL) {
		DEBUG_OUT (10, ("conversion hpux_acl -> smb_acl failed (%s).\n",
			   strerror(errno)));
	}
	
 done:
	DEBUG_OUT (10, ("hpuxacl_sys_acl_get_file %s.\n",
				 ...
				 files_struct *fsp)
{
	 ...
	 * parameter, but get it again to be safe --- necessary? */
         ...
         */
	DEBUG_OUT (10, ("redirecting call of hpuxacl_sys_acl_get_fd to "
			     ...
			     SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("hpuxacl_sys_acl_set_file called for file '%s'\n",
		   name));
	...
	if ((type != SMB_ACL_TYPE_ACCESS) && (type != SMB_ACL_TYPE_DEFAULT)) {
		errno = EINVAL;
		DEBUG_OUT (10, ("invalid smb acl type given (%d).\n", type));
		goto done;
	}
	...
	if(!smb_acl_to_hpux_acl(theacl, &hpux_acl, &count, type)) {
		DEBUG_OUT (10, ("conversion smb_acl -> hpux_acl failed (%s).\n",
			   strerror(errno)));
                goto done;
	}
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("Error in stat call: %s\n", strerror(errno)));
		goto done;
	}
	if (S_ISDIR(smb_fname->st.st_ex_mode)) {
		...
		if (!hpux_acl_get_file(smb_fname->base_name, &other_acl,
				       &other_count)) {
			DEBUG_OUT (10, ("error getting acl from directory\n"));
			goto done;
		}
		DEBUG_OUT (10, ("adding %s part of fs acl to given acl\n",
		...
		{
			DEBUG_OUT (10, ("error adding other acl.\n"));
			SAFE_FREE(other_acl);
			goto done;
		}
		SAFE_FREE(other_acl);
	...
	if (!hpux_acl_sort(hpux_acl, count)) {
		DEBUG_OUT (10, ("resulting acl is not valid!\n"));
		goto done;
	}
	DEBUG_OUT (10, ("resulting acl is valid.\n"));

	ret = acl(CONST_DISCARD(char *, smb_fname->base_name), ACL_SET, count,
		  hpux_acl);
	if (ret != 0) {
		DEBUG_OUT (0, ("ERROR calling acl: %s\n", strerror(errno)));
	}
	
 done:
	DEBUG_OUT (10, ("hpuxacl_sys_acl_set_file %s.\n",
			      ...
			      SMB_ACL_T theacl)
{
	 ...
	 * parameter, but get it again to be safe --- necessary? */
         ...
         * can use the relative path.
         */
	DEBUG_OUT (10, ("redirecting call of hpuxacl_sys_acl_set_fd to "
				    ...
				    const char *path)
{
	...
	DEBUG_OUT (10, ("entering hpuxacl_sys_acl_delete_def_file.\n"));
	
	smb_acl = hpuxacl_sys_acl_get_file(handle, path, 
					   SMB_ACL_TYPE_ACCESS);
	if (smb_acl == NULL) {
		DEBUG_OUT (10, ("getting file acl failed!\n"));
		goto done;
	}
	if (!smb_acl_to_hpux_acl(smb_acl, &hpux_acl, &count, 
				 SMB_ACL_TYPE_ACCESS))
	{
		DEBUG_OUT (10, ("conversion smb_acl -> hpux_acl failed.\n"));
		goto done;
	}
	if (!hpux_acl_sort(hpux_acl, count)) {
		DEBUG_OUT (10, ("resulting acl is not valid!\n"));
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("settinge file acl failed!\n"));
	}
	
 done:
	DEBUG_OUT (10, ("hpuxacl_sys_acl_delete_def_file %s.\n",
		   ((ret != 0) ? "failed" : "succeeded" )));
	SAFE_FREE(smb_acl);
	return ret;
}
				   ...
				   SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("entering smb_acl_to_hpux_acl\n"));
	...
	*count = 0;
	...
	for (i = 0; i < smb_acl->count; i++) {
		...
		if (hpux_entry.a_type == 0) {
			DEBUG_OUT (10, ("smb_tag to hpux_tag failed\n"));
			goto fail;
		}
		switch(hpux_entry.a_type) {
		case USER:
			DEBUG_OUT (10, ("got tag type USER with uid %d\n", 
				   smb_entry->uid));
			hpux_entry.a_id = (uid_t)smb_entry->uid;
			break;
		case GROUP:
			DEBUG_OUT (10, ("got tag type GROUP with gid %d\n", 
				   smb_entry->gid));
		...
		if (type == SMB_ACL_TYPE_DEFAULT) {
			DEBUG_OUT (10, ("adding default bit to hpux ace\n"));
			hpux_entry.a_type |= ACL_DEFAULT;
		}
		
		hpux_entry.a_perm = 
			smb_perm_to_hpux_perm(smb_entry->a_perm);
		DEBUG_OUT (10, ("assembled the following hpux ace:\n"));
		...
		if (!hpux_add_to_acl(hpux_acl, count, &hpux_entry, 
					1, type))
		{
			DEBUG_OUT (10, ("error adding acl entry\n"));
			goto fail;
		}
		DEBUG_OUT (10, ("count after adding: %d (i: %d)\n", *count, i));
		DEBUG_OUT (10, ("test, if entry has been copied into acl:\n"));
		DEBUGADD(10, (" - type: 0x%04x\n",
 ...
 fail:
	...
	DEBUG_OUT (10, ("smb_acl_to_hpux_acl %s\n",
					...
					SMB_ACL_TYPE_T type)
{
	...
	if ((result = sys_acl_init(0)) == NULL) {
		DEBUG_OUT (10, ("error allocating memory for SMB_ACL\n"));
		goto fail;
	}
	for (i = 0; i < count; i++) {
		...
		result = SMB_REALLOC(result, 
				     ...
				     (sizeof(struct smb_acl_entry) *
				      (result->count + 1)));
		if (result == NULL) {
			DEBUG_OUT (10, ("error reallocating memory for SMB_ACL\n"));
			goto fail;
		}
		...
		{
			DEBUG_OUT (10, ("invalid tag type given: 0x%04x\n",
				   hpux_acl[i].a_type));
			goto fail;
		}
		...
		if (sys_acl_set_permset(smb_entry, &smb_perm) != 0) {
			DEBUG_OUT (10, ("invalid permset given: %d\n", 
				   hpux_acl[i].a_perm));
			goto fail;
		}
		result->count += 1;
 ...
 fail:
	...
	DEBUG_OUT (10, ("hpux_acl_to_smb_acl %s\n",
		   ((result == NULL) ? "failed" : "succeeded")));
	return result;
}
...
static HPUX_ACL_TAG_T smb_tag_to_hpux_tag(SMB_ACL_TAG_T smb_tag)
{
	...
	DEBUG_OUT (10, ("smb_tag_to_hpux_tag\n"));
...
static SMB_ACL_TAG_T hpux_tag_to_smb_tag(HPUX_ACL_TAG_T hpux_tag)
{
	...
	DEBUG_OUT (10, ("hpux_tag_to_smb_tag:\n"));
				 ...
				 int *count)
{
	...
	DEBUG_OUT (10, ("hpux_acl_get_file called for file '%s'\n", name));
	...
	*count = acl(CONST_DISCARD(char *, name), ACL_CNT, 0, &dummy_ace);
	if (*count < 0) {
		DEBUG_OUT (10, ("acl ACL_CNT failed: %s\n", strerror(errno)));
		goto done;
	}
	*hpux_acl = hpux_acl_init(*count);
	if (*hpux_acl == NULL) {
		DEBUG_OUT (10, ("error allocating memory for hpux acl...\n"));
		goto done;
	}
	*count = acl(CONST_DISCARD(char *, name), ACL_GET, *count, *hpux_acl);
	if (*count < 0) {
		DEBUG_OUT (10, ("acl ACL_GET failed: %s\n", strerror(errno)));
		goto done;
	}
	result = True;

 done:
        DEBUG_OUT (10, ("hpux_acl_get_file %s.\n",
			       ...
			       SMB_ACL_TYPE_T type)
{
	...
	if ((type != SMB_ACL_TYPE_ACCESS) && (type != SMB_ACL_TYPE_DEFAULT)) 
	{
		DEBUG_OUT (10, ("invalid acl type given: %d\n", type));
		errno = EINVAL;
		return False;
	}
	for (i = 0; i < add_count; i++) {
		...
		ADD_TO_ARRAY(NULL, HPUX_ACE_T, add_acl[i], 
			     hpux_acl, count);
		if (hpux_acl == NULL) {
			DEBUG_OUT (10, ("error enlarging acl.\n"));
			errno = ENOMEM;
			return False;
		}
	}
...
static int hpux_internal_aclsort(int acl_count, int calclass, HPUX_ACL_T aclp)
{
	...
	DEBUG_OUT (10,("Entering hpux_internal_aclsort. (calclass = %d)\n", calclass));

	if (hpux_aclsort_call_present()) {
		DEBUG_OUT (10, ("calling hpux aclsort\n"));
		return aclsort(acl_count, calclass, aclp);
	}

	DEBUG_OUT (10, ("using internal aclsort\n"));

	if(!acl_count) {
		DEBUG_OUT (10,("Zero acl count passed. Returning Success\n"));
		return 0;
	}

	if(aclp == NULL) {
		DEBUG_OUT (0,("Null ACL pointer in hpux_acl_sort. Returning Failure. \n"));
		return -1;
	}
	...
	{
		DEBUG_OUT (0,("hpux_internal_aclsort: More than one entry or no entries for \
USER OBJ or GROUP_OBJ or OTHER_OBJ or CLASS_OBJ\n"));
	...
	{
		DEBUG_OUT (0,("hpux_internal_aclsort: More than one entry for DEF_CLASS_OBJ \
			...
			if( aclp[i].a_type > aclp[j].a_type ) {
				...
				if(aclp[i].a_id > aclp[j].a_id) {
					...
					if(hpux_prohibited_duplicate_type(aclp[i].a_type)) {
						DEBUG_OUT (0, ("hpux_internal_aclsort: Duplicate entry: Type(hex): %x Id: %d\n",
...
static bool hpux_acl_call_present(void)
{
	...
	if(ret_val != 0) {
		DEBUG_OUT (5, ("hpux_acl_call_present: shl_findsym() returned %d, errno = %d, error %s\n",
			ret_val, errno, strerror(errno)));
		DEBUG_OUT (5,("hpux_acl_call_present: acl() system call is not present. Check if you have JFS 3.3 and above?\n"));
		errno = ENOSYS;
		return False;
	}

	DEBUG_OUT (10,("hpux_acl_call_present: acl() system call is present. We have JFS 3.3 or above \n"));
...
static bool hpux_aclsort_call_present(void) 
{
	...
	if (ret_val != 0) {
		DEBUG_OUT (5, ("hpux_aclsort_call_present: shl_findsym "
			"returned %d, errno = %d, error %s", 
			ret_val, errno, strerror(errno)));
		DEBUG_OUT (5, ("hpux_aclsort_call_present: "
			"aclsort() function not available.\n"));
		return False;
	}
	DEBUG_OUT (10,("hpux_aclsort_call_present: aclsort() function present.\n"));
	already_checked = True;
	return True;
}
