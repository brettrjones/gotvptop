...
static void perfcount_test_dump_id(struct perfcount_test_identity *id, int lvl)
{
	...
	DEBUG_OUT (lvl,("uid - %d\n", id->uid));
	DEBUG_OUT (lvl,("user - %s\n", id->user));
	DEBUG_OUT (lvl,("domain - %s\n", id->domain));
}
					...
					int lvl)
{
	DEBUG_OUT (lvl, ("OP: %s\n", smb_fn_name(ptc->op)));
	if (ptc->sub_op > 0) {
		DEBUG_OUT (lvl, ("SUBOP: %s\n",
			smb_subop_name(ptc->op, ptc->sub_op)));
	}

	if (ptc->ioctl > 0) {
		DEBUG_OUT (lvl, ("IOCTL: %d\n", ptc->ioctl));
	}

	DEBUG_OUT (lvl, ("Count: %d\n\n", ptc->count));
}
...
static void perfcount_test_dump_counters(void)
{
	...
	DEBUG_OUT (0,("#####  Dumping Performance Counters #####\n"));
...
static void perfcount_test_start(struct smb_perfcount_data *pcd)
{
	 ...
	 */
	if (pcd->context) {
		DEBUG_OUT (0,("perfcount_test_start - starting "
