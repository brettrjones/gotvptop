				...
				vfs_handle_struct *handle, const char *name)
{
	...
	if (strptime(name, fmt, &timestamp) == NULL) {
		DEBUG_OUT (10, ("shadow_copy2_snapshot_to_gmt: no match %s: %s\n",
			   fmt, name));
		return NULL;
	}

	DEBUG_OUT (10, ("shadow_copy2_snapshot_to_gmt: match %s: %s\n", fmt, name));
...
static const char *shadow_copy2_normalise_path(TALLOC_CTX *mem_ctx, const char *path, const char *gmt_start)
{
	...
	DEBUG_OUT (10, ("path=%s, gmt_start=%s, prefix_len=%d\n", path, gmt_start,
		   (int)prefix_len));
	...
	DEBUG_OUT (10, ("shadow_copy2_normalise_path: %s -> %s\n", path, pcopy));
...
static char *convert_shadow2_name(vfs_handle_struct *handle, const char *fname, const char *gmt_path)
{
	...
	if (snapdir == NULL) {
		DEBUG_OUT (2,("no snapdir found for share at %s\n", handle->conn->connectpath));
		talloc_free(tmp_ctx);
		return NULL;
	}
	...
	if (basedir == NULL) {
		DEBUG_OUT (2,("no basedir found for share at %s\n", handle->conn->connectpath));
		talloc_free(tmp_ctx);
		return NULL;
	}
	...
	if (strncmp(basedir, handle->conn->connectpath, baselen) != 0 ||
		...
		DEBUG_OUT (0,("convert_shadow2_name: basedir %s is not a parent of %s\n",
	...
	ret = talloc_asprintf(handle->data, "%s/%s/%s/%s",
			      ...
			      baseoffset, 
			      relpath);
	DEBUG_OUT (6,("convert_shadow2_name: '%s' -> '%s'\n", fname, ret));
	talloc_free(tmp_ctx);
	return ret;
}
			    ...
			    const char *fname)
{
	...
	if (shadow_copy2_match_name(fname, &gmt)
		...
		DEBUG_OUT (10, ("calling NEXT_REALPATH with %s\n", copy));
					    ...
					    const char *fname)
{
	...
	DEBUG_OUT (10, ("shadow_copy2_connectpath called with %s\n", fname));
	...
	if (strncmp(basedir, handle->conn->connectpath, baselen) != 0 ||
		...
		DEBUG_OUT (0,("shadow_copy2_connectpath: basedir %s is not a "
			 "parent of %s\n", basedir,
			 handle->conn->connectpath));
		TALLOC_FREE(tmp_ctx);
		return NULL;
	...
	ret = talloc_asprintf(talloc_tos(), "%s/%.*s/%s",
			      ...
			      GMT_NAME_LEN, fname,
			      baseoffset);
	DEBUG_OUT (6,("shadow_copy2_connectpath: '%s' -> '%s'\n", fname, ret));
	TALLOC_FREE(tmp_ctx);
	return ret;
}
					      ...
					      bool labels)
{
	...
	snapdir = shadow_copy2_find_snapdir(tmp_ctx, handle);
	if (snapdir == NULL) {
		DEBUG_OUT (0,("shadow:snapdir not found for %s in get_shadow_copy_data\n",
			 handle->conn->connectpath));
		errno = EINVAL;
		talloc_free(tmp_ctx);
		return -1;
	...
	if (!p) {
		DEBUG_OUT (2,("shadow_copy2: SMB_VFS_NEXT_OPENDIR() failed for '%s'"
	...
	while ((d = SMB_VFS_NEXT_READDIR(handle, p, NULL))) {
		...
		snapshot = shadow_copy2_snapshot_to_gmt(tmp_ctx, handle,
							d->d_name);
		DEBUG_OUT (6,("shadow_copy2_get_shadow_copy2_data: %s -> %s\n",
			 d->d_name, snapshot));
		if (!snapshot) {
			continue;
		}
		...
		if (tlabels == NULL) {
			DEBUG_OUT (0,("shadow_copy2: out of memory\n"));
...
NTSTATUS vfs_shadow_copy2_init(void)
{
	...
	if (vfs_shadow_copy2_debug_level == -1) {
		...
		DEBUG_OUT (0, ("%s: Couldn't register custom debugging class!\n",
			"vfs_shadow_copy2_init"));
	} else {
		DEBUG_OUT (10, ("%s: Debug class number of '%s': %d\n", 
