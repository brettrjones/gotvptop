			 ...
			 const char *user)
{
	...
	if (ret) {
		...
		DEBUG_OUT (3, ("Load config failed: %s\n", strerror(errno)));
		return ret;
	}
			 ...
			 vfs_statvfs_struct *statbuf)
{
	...
	DEBUG_OUT (5, ("Calling SMB_STAT_VFS \n"));
				   ...
				   char **found_name)
{
	...
	if (*path != '\0') {
		if (!(full_name = talloc_asprintf(mem_ctx, "%s/%s", path, name))) {
			errno = ENOMEM;
			DEBUG_OUT (2, ("talloc_asprintf failed\n"));
			result = -1;
			goto done;
		}
	}
	...
	if ((result = estat(full_name ? full_name : name, &sb, &se)) != 0) {
		DEBUG_OUT (2, ("error calling estat: %s\n", strerror(errno)));
		goto done;
	}
			...
			struct smb_file_time *ft)
{
	...
	if (!null_timespec(ft->atime)) {
		flags |= VT_ATIME;
		times[0] = ft->atime;
		DEBUG_OUT (6,("**** onefs_ntimes: actime: %s.%d\n",
	...
	if (!null_timespec(ft->mtime)) {
		flags |= VT_MTIME;
		times[1] = ft->mtime;
		DEBUG_OUT (6,("**** onefs_ntimes: modtime: %s.%d\n",
	...
	if (!null_timespec(ft->create_time)) {
		...
		DEBUG_OUT (6,("**** onefs_ntimes: createtime: %s.%d\n",
