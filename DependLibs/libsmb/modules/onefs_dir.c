...
rdp_fill_cache(struct rdp_dir_state *dsp)
{
	...
	if (dirfd < 0) {
		DEBUG_OUT (1, ("Could not retrieve fd for DIR\n"));
		return -1;
	}
	...
	DEBUG_OUT (9, ("Calling readdirplus() with DIR %p, dirfd: %d, "
	...
	nread = readdirplus(dirfd,
			    ...
			    rdp_cookies);
	if (nread < 0) {
		DEBUG_OUT (1, ("Error calling readdirplus(): %s\n",
			 strerror(errno)));
		return -1;
	}

	DEBUG_OUT (9, ("readdirplus() returned %u entries from DIR %p\n",
...
onefs_rdp_add_dir_state(connection_struct *conn, SMB_STRUCT_DIR *dirp)
{
	...
	if (!dsp) {
		DEBUG_OUT (0, ("Error allocating struct rdp_dir_state.\n"));
		return ENOMEM;
	}
	...
	ret = rdp_init(dsp);
	if (ret) {
		DEBUG_OUT (0, ("Error initializing readdirplus() buffers: %s\n",
			 strerror(ret)));
		return ret;
	}
	      ...
	      uint32 attr)
{
	...
	if (!ret_dirp) {
		DEBUG_OUT (3, ("Unable to open directory: %s\n", fname));
		return NULL;
	}
	...
	if (ret) {
		DEBUG_OUT (0, ("Error adding dir_state to the list\n"));
		return NULL;
	}

	DEBUG_OUT (9, ("Opened handle on directory: \"%s\", DIR %p\n",
		 fname, ret_dirp));
	      ...
	      SMB_STRUCT_STAT *sbuf)
{
	...
	if (ret) {
		DEBUG_OUT (1, ("Could not retrieve dir_state struct for "
	...
	if (same_as_last) {
		...
		filled_cache = true;
		DEBUG_OUT (8, ("Switched global rdp cache to new DIR entry.\n"));
	}
	...
	DEBUG_OUT (9, ("Read from DIR %p, direntry: \"%s\", resume cookie: %#llx, "
...
onefs_seekdir(vfs_handle_struct *handle, SMB_STRUCT_DIR *dirp, long offset)
{
	...
	/* Validate inputs */
	if (offset < 0) {
		DEBUG_OUT (1, ("Invalid offset %ld passed.\n", offset));
		return;
	}
	...
	DEBUG_OUT (9, ("Seek DIR %p, offset: %ld, resume_cookie: %#llx\n",
...
onefs_telldir(vfs_handle_struct *handle,  SMB_STRUCT_DIR *dirp)
{
	...
	if (offset < 0) {
		DEBUG_OUT (1, ("Unable to convert resume_cookie: %#llx to a "
...
onefs_rewinddir(vfs_handle_struct *handle,  SMB_STRUCT_DIR *dirp)
{
	...
	ret = rdp_init(dsp);
	if (ret) {
		DEBUG_OUT (0, ("Error re-initializing rdp cursors: %s\n",
		    strerror(ret)));
		return;
	}

	DEBUG_OUT (9, ("Rewind DIR: %p, to resume_cookie: %#llx\n", dsp->dirp,
		 dsp->resume_cookie));
...
onefs_closedir(vfs_handle_struct *handle,  SMB_STRUCT_DIR *dirp)
{
	...
	DEBUG_OUT (9, ("Closed handle on DIR %p\n", dsp->dirp));
