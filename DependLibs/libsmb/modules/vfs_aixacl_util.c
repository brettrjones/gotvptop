...
SMB_ACL_T aixacl_to_smbacl(struct acl *file_acl)
{
	...
	DEBUG_OUT (10,("acl_entry is %p\n",(void *)acl_entry));
	DEBUG_OUT (10,("acl_last(file_acl) id %p\n",(void *)acl_last(file_acl)));
	...
	if(file_acl->acl_mode & S_IXACL){
		...
		while(acl_entry < acl_last(file_acl)) {
			...
			DEBUG_OUT (10,("idp->id_data is %d\n",idp->id_data[0]));
			
			result = SMB_REALLOC(result, sizeof(struct smb_acl_t) +
				     (sizeof(struct smb_acl_entry) *
				      (result->count+1)));
			if (result == NULL) {
				DEBUG_OUT (0, ("SMB_REALLOC failed\n"));
				errno = ENOMEM;
				return NULL;
			}
			

			DEBUG_OUT (10,("idp->id_type is %d\n",idp->id_type));
			...
			DEBUG_OUT (10,("case ACEID_USER ace->uid is %d\n",ace->uid));
			...
			DEBUG_OUT (10,("case ACEID_GROUP ace->gid is %d\n",ace->gid));
			...
			case ACC_SPECIFY:
				...
				ace->a_perm <<= 6;
				DEBUG_OUT (10,("ace->a_perm is %d\n",ace->a_perm));
				break;
			case ACC_DENY:
				/* Since there is no way to return a DENY acl entry *
				 * change to PERMIT and then shift.                 */
				DEBUG_OUT (10,("acl_entry->ace_access is %d\n",acl_entry->ace_access));
				ace->a_perm = ~acl_entry->ace_access & 7;
				DEBUG_OUT (10,("ace->a_perm is %d\n",ace->a_perm));
				ace->a_perm <<= 6;
				break;
			default:
				DEBUG_OUT (0, ("unknown ace->type\n"));
			 	SAFE_FREE(result);
				return(0);
			}
			...
			DEBUG_OUT (10,("acl_entry = %p\n",(void *)acl_entry));
			DEBUG_OUT (10,("The ace_type is %d\n",acl_entry->ace_type));
	...
	for( i = 1; i < 4; i++) {
		DEBUG_OUT (10,("i is %d\n",i));
			...
			if (result == NULL) {
				DEBUG_OUT (0, ("SMB_REALLOC failed\n"));
				errno = ENOMEM;
				DEBUG_OUT (0,("Error in AIX sys_acl_get_file is %d\n",errno));
				return NULL;
			}
		...
		result->count++;
		DEBUG_OUT (10,("ace->a_perm = %d\n",ace->a_perm));
		DEBUG_OUT (10,("ace->a_type = %d\n",ace->a_type));
	}
...
struct acl *aixacl_smb_to_aixacl(SMB_ACL_TYPE_T acltype, SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10,("Entering aixacl_smb_to_aixacl\n"));
	...
	if(file_acl == NULL) {
		errno = ENOMEM;
		DEBUG_OUT (0,("Error in aixacl_smb_to_aixacl is %d\n",errno));
		return NULL;
	}
	...
	for(i=0; i<theacl->count; i++ ) {
		...
		switch(id_type) {
			...
			default: /* abnormal case */
				DEBUG_OUT (10,("The id_type is unknown !\n"));
				continue;
		}
