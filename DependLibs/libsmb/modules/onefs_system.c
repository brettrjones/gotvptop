...
{
	if (sml == NULL) {
		DEBUG_OUT (debuglevel, ("sml == NULL\n"));
		return;
	}

	DEBUG_OUT (debuglevel,
			  ...
			  int *granted_oplock)
{
	...
	if (sd != NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("SD initialization failure: %s\n",
				  nt_errstr(status)));
			errno = EINVAL;
			goto out;
		}
	...
	if (lp_parm_bool(SNUM(conn), PARM_ONEFS_TYPE,
		...
		DEBUG_OUT (3, ("Stripping execute bit from %s: (0x%x)\n", mapped_name,
			  open_access_mask));
	...
	DEBUG_OUT (10,("onefs_sys_create_file: base_fd = %d, fname = %s "
	...
	DEBUG_OUT (10,("onefs_sys_create_file(%s): ret_fd = %d, "
    ...
    const DATA_BLOB *header, SMB_OFF_T offset, size_t count, bool atomic)
{
	...
	while (total + hdtrl.iov_len) {
		...
		if (atomic && nwritten != total + hdtrl.iov_len) {
			DEBUG_OUT (0,("Atomic sendfile() sent partial data: "
				 "%llu of %d\n", nwritten,
				 total + hdtrl.iov_len));
			return -1;
		}
			   ...
			   size_t count)
{
	...
	if (!atomic) {
		DEBUG_OUT (10, ("non-atomic sendfile read %ul bytes\n", ret));
	...
	if (ret == -1 && errno == E2BIG) {
		...
		if (!lp_parm_bool(SNUM(conn), PARM_ONEFS_TYPE,
			...
			DEBUG_OUT (3, ("Not attempting non-atomic large sendfile: "
				  "%lu bytes\n", count));
			END_PROFILE(syscall_sendfile);
			return 0;
		}
		...
		if (count < 0x10000) {
			DEBUG_OUT (0, ("Count < 2^16 and E2BIG was returned! %lu\n",
				  count));
		}

		DEBUG_OUT (10, ("attempting non-atomic large sendfile: %lu bytes\n",
			   count));
		...
		if (ret == -1) {
			DEBUG_OUT (1, ("error on non-atomic large sendfile "
				  "(%lu bytes): %s\n", count,
				  strerror(errno)));
			END_PROFILE(syscall_sendfile);
			return ret;
		...
		if (ret != count + header->length) {
			DEBUG_OUT (1, ("short read on non-atomic large sendfile "
				  "(%lu of %lu bytes): %s\n", ret, count,
				  strerror(errno)));
		...
		DEBUG_OUT (10, ("non-atomic large sendfile successful\n"));
	}

	/* There was error in the atomic sendfile. */
	if (ret == -1) {
		DEBUG_OUT (1, ("error on %s sendfile (%lu bytes): %s\n",
			   ...
			   size_t count)
{
	...
	DEBUG_OUT (10,("onefs_recvfile: from = %d, to = %d, offset=%llu, count = "
	...
	do {
		...
		DEBUG_OUT (10, ("calling recvfile loop, offset + total_wbytes = "
		...
		DEBUG_OUT (10, ("recvfile ret = %d, errno = %d, rbytes = %llu, "
			   "wbytes = %llu\n", ret, ret >= 0 ? 0 : errno,
			   rbytes, wbytes));
	...
	while (total_rbytes < count) {
		...
		DEBUG_OUT (3, ("shallow recvfile (%s), reading %llu\n",
		...
		if (ret <= 0) {
			if (ret == 0) {
				DEBUG_OUT (0, ("shallow recvfile read: EOF\n"));
			} else {
				DEBUG_OUT (0, ("shallow recvfile read failed: %s\n",
					  strerror(errno)));
			}
	...
	while (total_wbytes < count) {
		...
		DEBUG_OUT (3, ("partial recvfile, writing %llu\n", count - total_wbytes));

		ret = sys_pwrite(tofd, spill_buffer, count - total_wbytes,
				 offset + total_wbytes);

		if (ret == -1) {
			DEBUG_OUT (0, ("partial recvfile write failed: %s\n",
				  strerror(errno)));
			goto out;
		}
...
out:
	...
	if (!socket_drained && count - total_rbytes) {
		...
		if (drain_socket(fromfd, count - total_rbytes) !=
		    count - total_rbytes) {
			/* Socket is dead! */
			DEBUG_OUT (0, ("drain socket failed: %d\n", errno));
		}
		errno = saved_errno;
	}
