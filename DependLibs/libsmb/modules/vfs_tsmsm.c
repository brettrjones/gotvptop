...
static int tsmsm_connect(struct vfs_handle_struct *handle,
	...
	if (!tsmd) {
		...
		DEBUG_OUT (0,("tsmsm_connect: out of memory!\n"));
		return -1;
	}

	if (!dmapi_have_session()) {
		SMB_VFS_NEXT_DISCONNECT(handle);
		DEBUG_OUT (0,("tsmsm_connect: no DMAPI session for Samba is available!\n"));
		TALLOC_FREE(tsmd);
		return -1;
	}
	...
	if (fres == NULL) {
		...
		if (tsmd->online_ratio > 1.0 ||
			...
			DEBUG_OUT (1, ("tsmsm_connect: invalid online ration %f - using %f.\n",
			     ...
			     SMB_STRUCT_STAT *stbuf)
{
	...
	if (512 * stbuf->st_ex_blocks >=
		...
		DEBUG_OUT (10,("%s not offline: st_blocks=%llu st_size=%llu "
			  "online_ratio=%.2f\n", path,
	...
	if (dmsession_id == NULL) {
		DEBUG_OUT (2, ("tsmsm_is_offline: no DMAPI session available? "
	...
	if (dm_path_to_handle((char*)path, &dmhandle, &dmhandle_len) != 0) {
		DEBUG_OUT (2,("dm_path_to_handle failed - assuming offline (%s) - %s\n", 
			 path, strerror(errno)));
		offline = true;
		goto done;
	}
	...
	if (buf == NULL) {
		DEBUG_OUT (0,("out of memory in tsmsm_is_offline -- assuming online (%s)\n", path));
		errno = ENOMEM;
		offline = false;
		goto done;
	}
	...
	do {
		...
		if (ret == -1 && errno == EINVAL) {
			DEBUG_OUT (0, ("Stale DMAPI session, re-creating it.\n"));
			lerrno = EINVAL;
	...
	DEBUG_OUT (10,("dm_get_dmattr %s ret=%d (%s)\n", path, ret, strerror(errno)));
...
static bool tsmsm_aio_force(struct vfs_handle_struct *handle, struct files_struct *fsp)
{
	...
	*/
	if(SMB_VFS_FSTAT(fsp, &sbuf) == 0) {
		DEBUG_OUT (10,("tsmsm_aio_force st_blocks=%llu st_size=%llu "
			      ...
			      SMB_OFF_T offset, size_t n)
{
	...
	if (file_offline) {
		DEBUG_OUT (10,("tsmsm_sendfile on offline file - rejecting\n"));
		errno = ENOSYS;
		return -1;
	}
                             ...
                             const struct smb_filename *fname)
{
	...
	if (tsmd->hsmscript == NULL) {
		/* no script enabled */
		DEBUG_OUT (1, ("tsmsm_set_offline: No 'tsmsm:hsm script' configured\n"));
		return 0;
	}
	...
	if(!command) {
		DEBUG_OUT (1, ("tsmsm_set_offline: can't allocate memory to run hsm script"));
		return -1;
	}
	DEBUG_OUT (10, ("tsmsm_set_offline: Running [%s]\n", command));
	if((result = smbrun(command, NULL)) != 0) {
		DEBUG_OUT (1,("tsmsm_set_offline: Running [%s] returned %d\n", command, result));
	}
	TALLOC_FREE(command);
	return result;
}
