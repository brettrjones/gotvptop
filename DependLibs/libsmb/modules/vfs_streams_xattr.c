...
static SMB_INO_T stream_inode(const SMB_STRUCT_STAT *sbuf, const char *sname)
{
	...
	DEBUG_OUT (10, ("stream_inode called for %lu/%lu [%s]\n",
	...
	DEBUG_OUT (10, ("stream_inode returns %lu\n", (unsigned long)result));
				       ...
				       char **xattr_name)
{
	...
	*xattr_name = talloc_asprintf(ctx, "%s%s",
	...
	DEBUG_OUT (10, ("xattr_name: %s, stream_name: %s\n", *xattr_name,
		   stream_name));
			       ...
			       SMB_STRUCT_STAT *sbuf)
{
	...
	DEBUG_OUT (10, ("streams_xattr_fstat called for %d\n", fsp->fh->fd));
	...
	*sbuf = smb_fname_base->st;
	...
	DEBUG_OUT (10, ("sbuf->st_ex_size = %d\n", (int)sbuf->st_ex_size));
			      ...
			      files_struct *fsp, int flags, mode_t mode)
{
	...
	DEBUG_OUT (10, ("streams_xattr_open called for %s\n",
	...
	DEBUG_OUT (10, ("get_ea_value returned %s\n", nt_errstr(status)));
	...
	if (!NT_STATUS_IS_OK(status)
		 ...
		 * file for us.
		 */
		DEBUG_OUT (10, ("streams_xattr_open: base file %s not around, "
			   "returning ENOENT\n", smb_fname->base_name));
		errno = ENOENT;
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		 ...
		 */
			...
			DEBUG_OUT (10, ("creating attribute %s on file %s\n",
				   ...
				   void *private_data)
{
	...
	for (i=0; i<num_names; i++) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("Could not get ea %s for file %s: %s\n",
				    ...
				    size_t n, SMB_OFF_T offset)
{
	...
	DEBUG_OUT (10, ("streams_xattr_pwrite called for %d bytes\n", (int)n));
				   ...
				   size_t n, SMB_OFF_T offset)
{
	...
	DEBUG_OUT (10, ("streams_xattr_pread: offset=%d, size=%d\n",
		   (int)offset, (int)n));
	...
	DEBUG_OUT (10, ("streams_xattr_pread: get_ea_value returned %d bytes\n",
		   (int)length));
					...
					SMB_OFF_T offset)
{
	...
	DEBUG_OUT (10, ("streams_xattr_ftruncate called for file %s offset %.0f\n",
					...
					SMB_OFF_T len)
{
	...
	DEBUG_OUT (10, ("streams_xattr_fallocate called for file %s offset %.0f"
		"len = %.0f\n",
