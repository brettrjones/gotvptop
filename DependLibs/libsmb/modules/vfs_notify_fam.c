				    ...
				    struct event_context *event_ctx)
{
	...
	if (res < 0) {
		DEBUG_OUT (10, ("FAM file change notifications not available\n"));
		/*
		       ...
		       struct fam_watch_context *notify_list)
{
	...
	DEBUG_OUT (5, ("Re-opening FAM connection\n"));

	FAMClose(fam_conn);

	if (!NT_STATUS_IS_OK(fam_open_connection(fam_conn, event_ctx))) {
		DEBUG_OUT (5, ("Re-opening fam connection failed\n"));
		return;
	}
			...
			void *private_data)
{
	...
	if (FAMPending(fam_conn) == 0) {
		DEBUG_OUT (10, ("fam_handler called but nothing pending\n"));
		return;
	}

	if (FAMNextEvent(fam_conn, &fam_event) != 1) {
		DEBUG_OUT (5, ("FAMNextEvent returned an error\n"));
		TALLOC_FREE(fd_event);
		fam_reopen(fam_conn, event_ctx, fam_notify_list);
		return;
	}

	DEBUG_OUT (10, ("Got FAMCode %d for %s\n", fam_event.code,
		   fam_event.filename));
	...
	case FAMDeleted:
		...
		break;
	default:
		DEBUG_OUT (10, ("Ignoring code FAMCode %d for file %s\n",
	...
	if (ctx == NULL) {
		DEBUG_OUT (5, ("Discarding event for file %s\n",
			  fam_event.filename));
		return;
	}
			  ...
			  void *handle_p)
{
	...
	if ((e->filter & fam_mask) == 0) {
		DEBUG_OUT (10, ("filter = %u, ignoring in FAM\n", e->filter));
		return NT_STATUS_OK;
	}
