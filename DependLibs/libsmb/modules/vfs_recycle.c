...
#define ALLOC_CHECK(ptr, label) do { if ((ptr) == NULL) { DEBUG_OUT (0, ("recycle.bin: out of memory!\n")); errno = ENOMEM; goto label; } } while(0)
...
static int recycle_connect(vfs_handle_struct *handle, const char *service, const char *user)
{
	...
	DEBUG_OUT (10,("recycle_connect() connect to service[%s] as user[%s].\n",
		service,user));
...
static void recycle_disconnect(vfs_handle_struct *handle)
{
	DEBUG_OUT (10,("recycle_disconnect() connect to service[%s].\n",
...
static const char *recycle_repository(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: repository = %s\n", tmp_str));
...
static bool recycle_keep_dir_tree(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle_bin: keeptree = %s\n", ret?"True":"False"));
...
static bool recycle_versions(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: versions = %s\n", ret?"True":"False"));
...
static bool recycle_touch_mtime(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: touch_mtime = %s\n", ret?"True":"False"));
...
static const char **recycle_exclude(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: exclude = %s ...\n", tmp_lp?*tmp_lp:""));
...
static const char **recycle_exclude_dir(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: exclude_dir = %s ...\n", tmp_lp?*tmp_lp:""));
...
static const char **recycle_noversions(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: noversions = %s\n", tmp_lp?*tmp_lp:""));
...
static SMB_OFF_T recycle_maxsize(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: maxsize = %lu\n", (long unsigned int)maxsize));
...
static SMB_OFF_T recycle_minsize(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: minsize = %lu\n", (long unsigned int)minsize));
...
static mode_t recycle_directory_mode(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: directory_mode = %o\n", dirmode));
...
static mode_t recycle_subdir_mode(vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10, ("recycle: subdir_mode = %o\n", dirmode));
	return (mode_t)dirmode;
}
				       ...
				       const struct smb_filename *smb_fname)
{
	...
	if (SMB_VFS_STAT(handle->conn, smb_fname_tmp) != 0) {
		DEBUG_OUT (0,("recycle: stat for %s returned %s\n",
	...
	*new_dir = '\0';
	...
	for(token = strtok_r(tok_str, "/", &saveptr); token;
		...
		if (recycle_directory_exist(handle, new_dir))
			DEBUG_OUT (10, ("recycle: dir %s already exists\n", new_dir));
		else {
			DEBUG_OUT (5, ("recycle: creating new dir %s\n", new_dir));
			if (SMB_VFS_NEXT_MKDIR(handle, new_dir, mode) != 0) {
				DEBUG_OUT (1,("recycle: mkdir failed for %s with error: %s\n", new_dir, strerror(errno)));
				ret = False;
				goto done;
			}
		}
			     ...
			     bool touch_mtime)
{
	...
	unbecome_root();
	if (ret == -1 ) {
		DEBUG_OUT (0, ("recycle: touching %s failed, reason = %s\n",
    ...
    const struct smb_filename *smb_fname)
{
	...
	if(!repository || *(repository) == '\0') {
		DEBUG_OUT (3, ("recycle: repository path not set, purging %s...\n",
	...
	if (strncmp(smb_fname->base_name, repository,
		    strlen(repository)) == 0) {
		DEBUG_OUT (3, ("recycle: File is within recycling bin, unlinking ...\n"));
		rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
		goto done;
	}
	 ...
	 *
	if(fsize == 0) {
		DEBUG_OUT (3, ("recycle: File %s is empty, purging...\n", file_name));
	...
	if(maxsize > 0 && file_size > maxsize) {
		DEBUG_OUT (3, ("recycle: File %s exceeds maximum recycle size, "
			  "purging... \n", smb_fname_str_dbg(smb_fname)));
		rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
		goto done;
	}
	...
	if(minsize > 0 && file_size < minsize) {
		DEBUG_OUT (3, ("recycle: File %s lowers minimum recycle size, "
			  "purging... \n", smb_fname_str_dbg(smb_fname)));
		rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
		goto done;
	}
	...
	DEBUG_OUT (5, ("space_avail = %Lu, file_size = %Lu\n", space_avail, file_size));
	if(space_avail < file_size) {
		DEBUG_OUT (3, ("recycle: Not enough diskspace, purging file %s\n", file_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
	 */
	...
	DEBUG_OUT (10, ("recycle: fname = %s\n", smb_fname_str_dbg(smb_fname)));
	/* original path */
	DEBUG_OUT (10, ("recycle: fpath = %s\n", path_name));
	/* filename without path */
	DEBUG_OUT (10, ("recycle: base = %s\n", base));

	if (matchparam(recycle_exclude(handle), base)) {
		DEBUG_OUT (3, ("recycle: file %s is excluded \n", base));
		rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
		goto done;
	}

	if (matchdirparam(recycle_exclude_dir(handle), path_name)) {
		DEBUG_OUT (3, ("recycle: directory %s is excluded \n", path_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
		goto done;
	}
	...
	if (exist) {
		DEBUG_OUT (10, ("recycle: Directory already exists\n"));
	} else {
		DEBUG_OUT (10, ("recycle: Creating directory %s\n", temp_name));
		if (recycle_create_dir(handle, temp_name) == False) {
			DEBUG_OUT (3, ("recycle: Could not create directory, "
				  "purging %s...\n",
				  smb_fname_str_dbg(smb_fname)));
			rc = SMB_VFS_NEXT_UNLINK(handle, smb_fname);
			goto done;
	...
	DEBUG_OUT (10, ("recycle: recycled file name: %s\n",
	...
	if (recycle_file_exist(handle, smb_fname_final)) {
		if (recycle_versions(handle) == False || matchparam(recycle_noversions(handle), base) == True) {
			DEBUG_OUT (3, ("recycle: Removing old file %s from recycle "
				  "bin\n", smb_fname_str_dbg(smb_fname_final)));
			if (SMB_VFS_NEXT_UNLINK(handle,
						smb_fname_final) != 0) {
				DEBUG_OUT (1, ("recycle: Error deleting old file: %s\n", strerror(errno)));
			}
		}
	}
	...
	DEBUG_OUT (10, ("recycle: Moving %s to %s\n", smb_fname_str_dbg(smb_fname),
		smb_fname_str_dbg(smb_fname_final)));
	rc = SMB_VFS_NEXT_RENAME(handle, smb_fname, smb_fname_final);
	if (rc != 0) {
		DEBUG_OUT (3, ("recycle: Move error %d (%s), purging file %s "
...
NTSTATUS vfs_recycle_init(void)
{
	...
	if (vfs_recycle_debug_level == -1) {
		...
		DEBUG_OUT (0, ("vfs_recycle: Couldn't register custom debugging class!\n"));
	} else {
		DEBUG_OUT (10, ("vfs_recycle: Debug class number of 'recycle': %d\n", vfs_recycle_debug_level));
	}
