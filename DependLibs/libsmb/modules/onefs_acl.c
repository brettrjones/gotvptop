    ...
    bool is_group)
{
	...
	if (aclu_initialize_identity(id, type, uid, gid, is_group)) {
		DEBUG_OUT (3, ("Call to aclu_initialize_identity failed! id=%x, "
...
onefs_identity_to_sid(struct ifs_identity *id, struct dom_sid *sid)
{
	...
	switch (id->type) {
		...
		break;
	    default:
		DEBUG_OUT (0, ("Unknown identity type: %d\n", id->type));
		return false;
	}
    ...
    bool is_group, int snum)
{
	...
	if (!onefs_sid_to_identity(sid, ident, is_group)) {
		if (!lp_parm_bool(snum, PARM_ONEFS_TYPE,
			...
			DEBUG_OUT (3, ("Unresolvable SID (%s) found.\n",
				sid_string_dbg(sid)));
			return false;
		}
		...
		DEBUG_OUT (3, ("Mapping unresolvable owner SID (%s) to Builtin "
	...
	if (onefs_sid_to_identity(&samba_ace->trustee, &ident, false)) {
		*mapped = true;
		...
		if (sid_in_ignore_list(&samba_ace->trustee, snum)) {
			DEBUG_OUT (3, ("Silently failing to set ACE for SID (%s) "
			...
			*mapped = false;
			...
			DEBUG_OUT (3, ("Mapping unresolvable deny ACE SID (%s) "
				"to Everyone.\n",
				sid_string_dbg(&samba_ace->trustee)));
			if (aclu_initialize_identity(&ident,
				IFS_ID_TYPE_EVERYONE, 0, 0, False) != 0) {
				DEBUG_OUT (2, ("aclu_initialize_identity() "
...
onefs_acl_to_samba_acl(struct ifs_security_acl *acl, struct security_acl **samba_acl)
{
	...
	if (num_aces > 0) {
		...
		{
			DEBUG_OUT (0, ("Unable to malloc space for %d aces.\n",
			    num_aces));
			return false;
		}
	...
	if ((tmp_samba_acl = make_sec_acl(talloc_tos(), acl->revision, num_aces,
	    samba_aces)) == NULL) {
	       DEBUG_OUT (0, ("Unable to malloc space for acl.\n"));
	       goto err_free;
	}
...
onefs_canon_acl(files_struct *fsp, struct ifs_security_descriptor *sd)
{
	...
	case ACL_FORMAT_WINDOWS_SD:
		...
		if ((sbuf.st_ex_flags & SF_HASNTFSACL) != 0) {
			DEBUG_OUT (10, ("Did not canonicalize ACLs because a "
	...
	DEBUG_OUT (10, ("Performed canonicalization of ACLs for file %s\n",
		   fsp_str_dbg(fsp)));
...
static bool add_sfs_aces(files_struct *fsp, struct ifs_security_descriptor *sd)
{
	...
	error = SMB_VFS_FSTAT(fsp, &sbuf);
	if (error) {
		DEBUG_OUT (0, ("Failed to stat %s in simple files sharing "
	...
	if (S_ISDIR(sbuf.st_ex_mode) &&
		...
		if (num_aces_to_add != 0) {
			...
			if (!sd->dacl->aces) {
				DEBUG_OUT (0, ("Unable to malloc space for "
				    "new_aces: %d.\n",
				     sd->dacl->num_aces + num_aces_to_add));
				return false;
			}
		  ...
		  uint32 security_info, struct security_descriptor **ppdesc)
{
	...
	*ppdesc = NULL;
	...
	if (fsp->fh->fd == -1) {
		if ((fsp->fh->fd = onefs_sys_create_file(handle->conn,
							 ...
							 0,
							 NULL)) == -1) {
			DEBUG_OUT (0, ("Error opening file %s. errno=%d (%s)\n",
		...
		if (sd_size > 0) {
			...
			if (!sd) {
				DEBUG_OUT (0, ("Unable to malloc %u bytes of space "
			...
			DEBUG_OUT (0, ("Failed getting size of security descriptor! "
			    "errno=%d\n", errno));
			status = map_nt_error_from_unix(errno);
			goto out;
                }
	...
	DEBUG_OUT (5, ("Finished canonicalizing ACL\n"));
	...
	if (!pdesc) {
		DEBUG_OUT (0, ("Problem with make_sec_desc. Memory?\n"));
		status = map_nt_error_from_unix(errno);
		goto out;
	}
	...
	*ppdesc = pdesc;
	...
	DEBUG_OUT (5, ("Finished retrieving/canonicalizing SD!\n"));
	/* FALLTHROUGH */
out:
		  ...
		  uint32_t sec_info_sent, const struct security_descriptor *psd)
{
	...
	DEBUG_OUT (5,("Setting SD on file %s.\n", fsp_str_dbg(fsp)));

	status = onefs_samba_sd_to_sd(sec_info_sent, psd, &sd,
				      SNUM(handle->conn), &sec_info_effective);

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("SD initialization failure: %s\n", nt_errstr(status)));
		goto out;
	}

	fd = fsp->fh->fd;
	if (fd == -1) {
		DEBUG_OUT (10,("Reopening file %s.\n", fsp_str_dbg(fsp)));
	...
	if (ifs_set_security_descriptor(fd, sec_info_effective, &sd)) {
		DEBUG_OUT (0, ("Error setting security descriptor = %s\n",
			  strerror(errno)));
		status = map_nt_error_from_unix(errno);
		goto out;
	}

	DEBUG_OUT (5, ("Security descriptor set correctly!\n"));
	status = NT_STATUS_OK;
