...
static bool init_afs_acl(struct afs_acl *acl)
{
	...
	if (acl->ctx == NULL) {
		DEBUG_OUT (10, ("Could not init afs_acl"));
		return False;
	}
	return True;
}
				   ...
				   const char *name, uint32 rights)
{
	...
	if (strcmp(name, "system:administrators") == 0) {
		...
		if (!lookup_name(talloc_tos(), name, LOOKUP_NAME_ALL,
			...
			DEBUG_OUT (10, ("Could not find AFS user %s\n", name));
	...
	if (result == NULL) {
		DEBUG_OUT (0, ("Could not talloc AFS ace\n"));
		return NULL;
	}

	result->name = talloc_strdup(mem_ctx, name);
	if (result->name == NULL) {
		DEBUG_OUT (0, ("Could not talloc AFS ace name\n"));
		return NULL;
	}
			...
			const char *name, uint32 rights)
{
	...
	DEBUG_OUT (10, ("add_afs_ace: Added %s entry for %s with rights %d\n",
...
static bool parse_afs_acl(struct afs_acl *acl, const char *acl_str)
{
	...
	DEBUG_OUT (10, ("Found %d positive entries\n", nplus));
	...
	DEBUG_OUT (10, ("Found %d negative entries\n", nminus));
...
static uint32 nt_to_afs_dir_rights(const char *filename, const struct security_ace *ace)
{
	...
	DEBUG_OUT (1, ("AFSACL FALLBACK: 0x%X 0x%X 0x%X %s %X\n",
				   ...
				   struct security_descriptor **ppdesc)
{
	...
	while (afs_ace != NULL) {
		...
		if (afs_ace->type == SID_NAME_UNKNOWN) {
			DEBUG_OUT (10, ("Ignoring unknown name %s\n",
				   afs_ace->name));
			  ...
			  struct afs_acl *afs_acl)
{
	...
	for (i = 0; i < dacl->num_aces; i++) {
		...
		if (!mappable_sid(&ace->trustee)) {
			DEBUG_OUT (10, ("Ignoring unmappable SID %s\n",
		...
		if (dom_sid_compare(&ace->trustee,
			...
			if (!lookup_sid(talloc_tos(), &ace->trustee,
				...
				DEBUG_OUT (1, ("AFSACL: Could not lookup SID %s on file %s\n",
					  sid_string_dbg(&ace->trustee),
					  filename));
				continue;
			}
...
static bool afs_get_afs_acl(char *filename, struct afs_acl *acl)
{
	...
	if (ret) {
		DEBUG_OUT (1, ("got error from PIOCTL: %d\n", ret));
		return False;
	}
	...
	if (!parse_afs_acl(acl, space)) {
		DEBUG_OUT (1, ("Could not parse AFS acl\n"));
		free_afs_acl(acl);
		return False;
	}
...
static void merge_unknown_aces(struct afs_acl *src, struct afs_acl *dst)
{
	...
	for (ace = src->acelist; ace != NULL; ace = ace->next)
	{
		...
		if (ace->type != SID_NAME_UNKNOWN) {
			DEBUG_OUT (10, ("Not merging known ACE for %s\n",
				   ace->name));
			continue;
		}

		DEBUG_OUT (10, ("Merging unknown ACE for %s\n", ace->name));

		copy = clone_afs_ace(dst->ctx, ace);

		if (copy == NULL) {
			DEBUG_OUT (0, ("Could not clone ACE for %s\n", ace->name));
			continue;
		}
			   ...
			   const struct security_descriptor *psd)
{
	...
	if (!afs_get_afs_acl(name, &old_afs_acl)) {
		DEBUG_OUT (3, ("Could not get old ACL of %s\n", fsp_str_dbg(fsp)));
		goto done;
	}
	...
	DEBUG_OUT (10, ("trying to set acl '%s' on file %s\n", iob.in, name));

	ret = afs_syscall(AFSCALL_PIOCTL, name, VIOCSETAL, (char *)&iob, 0);

	if (ret != 0) {
		DEBUG_OUT (10, ("VIOCSETAL returned %d\n", ret));
	}
				   ...
				   struct security_descriptor **ppdesc)
{
	...
	DEBUG_OUT (5, ("afsacl_fget_nt_acl: %s\n", fsp_str_dbg(fsp)));
				  ...
				  struct security_descriptor **ppdesc)
{
	...
	DEBUG_OUT (5, ("afsacl_get_nt_acl: %s\n", name));
