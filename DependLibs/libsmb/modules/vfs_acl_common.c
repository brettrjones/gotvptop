				...
				uint8_t hash[XATTR_SD_HASH_SIZE])
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (5, ("parse_acl_blob: ndr_pull_xattr_NTACL failed: %s\n",
			ndr_errstr(ndr_err)));
			...
			uint8_t hash[XATTR_SD_HASH_SIZE])
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (5, ("create_acl_blob: ndr_push_xattr_NTACL failed: %s\n",
			ndr_errstr(ndr_err)));
		return ndr_map_error2ntstatus(ndr_err);
	}
				...
				struct security_descriptor *psd)
{
	...
	DEBUG_OUT (10, ("add_directory_inheritable_components: directory %s, "
		"mode = 0%o\n",
		name,
		(unsigned int)mode ));
				...
				struct security_descriptor **ppdesc)
{
	...
	DEBUG_OUT (10, ("get_nt_acl_internal: name=%s\n", name));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("get_nt_acl_internal: get_next_acl for file %s "
			"returned %s\n",
			name,
			nt_errstr(status)));
		return status;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("get_nt_acl_internal: get_acl_blob returned %s\n",
			nt_errstr(status)));
		psd = pdesc_next;
		goto out;
	}
	...
	status = parse_acl_blob(&blob, &psd,
				&hash_type, &hash[0]);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("parse_acl_blob returned %s\n",
				nt_errstr(status)));
		psd = pdesc_next;
		goto out;
	}
	...
	switch (hash_type) {
		...
		case XATTR_SD_HASH_TYPE_SHA256:
			break;
		default:
			DEBUG_OUT (10, ("get_nt_acl_internal: ACL blob revision "
	...
	if (memcmp(&hash[0], &hash_tmp[0], XATTR_SD_HASH_SIZE) == 0) {
		...
		DEBUG_OUT (10, ("get_nt_acl_internal: blob hash "
			"matches for file %s\n",
			name ));
		goto out;
	}
  ...
  out:
	...
	*ppdesc = psd;
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("get_nt_acl_internal: returning acl for %s is:\n",
			name ));
					...
					bool is_directory)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("inherit_new_acl: parent acl for %s is:\n",
			fsp_str_dbg(fsp) ));
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("inherit_new_acl: child acl for %s is:\n",
			fsp_str_dbg(fsp) ));
		NDR_PRINT_DEBUG(security_descriptor, psd);
	}
				...
				struct security_descriptor **pp_parent_desc)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("get_parent_acl_common: get_nt_acl_internal "
			"on directory %s for "
			"path %s returned %s\n",
			parent_name,
			path,
				...
				struct security_descriptor **pp_parent_desc)
{
	...
	status = smb1_file_se_access_check(handle->conn,
					...
					&access_granted);
	if(!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("check_parent_acl_common: access check "
			...
			mode_t mode)
{
	...
	if (fsp->base_fsp) {
		...
		DEBUG_OUT (10,("open_acl_common: stream open on %s\n",
			fsp_str_dbg(fsp) ));
	...
	status = get_nt_acl_internal(handle,
		...
		if (NT_STATUS_EQUAL(status, NT_STATUS_ACCESS_DENIED) &&
			...
			DEBUG_OUT (10,("open_acl_xattr: %s open "
	...
	DEBUG_OUT (10,("open_acl_xattr: get_nt_acl_attr_internal for "
		"%s returned %s\n",
		fsp_str_dbg(fsp),
		nt_errstr(status) ));
        ...
        uint32_t security_info_sent, const struct security_descriptor *orig_psd)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("fset_nt_acl_xattr: incoming sd for file %s\n",
			  fsp_str_dbg(fsp)));
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("fset_nt_acl_xattr: storing xattr sd for file %s\n",
			  fsp_str_dbg(fsp)));
		NDR_PRINT_DEBUG(security_descriptor,
	...
	status = create_acl_blob(psd, &blob, XATTR_SD_HASH_TYPE_SHA256, hash);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("fset_nt_acl_xattr: create_acl_blob failed\n"));
		return status;
	}
			...
			const char *fname, const char *mask, uint32 attr)
{
	...
	status = get_nt_acl_internal(handle,
				...
				&sd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("opendir_acl_common: "
	...
	status = smb1_file_se_access_check(handle->conn,
				...
				&access_granted);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("opendir_acl_common: %s open "
					...
					bool is_directory)
{
	...
	DEBUG_OUT (10,("acl_common_remove_object: removing %s %s/%s\n",
	...
	if (!fsp) {
		DEBUG_OUT (10,("acl_common_remove_object: %s %s/%s "
			"not an open file\n",
			is_directory ? "directory" : "file",
			parent_dir, final_component ));
		saved_errno = EACCES;
				...
				const char *path)
{
	...
	DEBUG_OUT (10,("rmdir_acl_common: unlink of %s failed %s\n",
		path,
		strerror(errno) ));
	return -1;
}
				...
				int *pinfo)
{
	...
	if (!NT_STATUS_IS_OK(status1)) {
		DEBUG_OUT (1,("create_file_acl_common: error setting "
			"sd for %s (%s)\n",
			fsp_str_dbg(fsp),
			nt_errstr(status1) ));
	}
			...
			const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("unlink_acl_common: unlink of %s failed %s\n",
		smb_fname->base_name,
		strerror(errno) ));
	return -1;
}
