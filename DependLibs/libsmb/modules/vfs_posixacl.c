			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("Calling acl_set_file: %s, %d\n", name, type));
				...
				struct smb_acl_entry *ace)
{
	...
	if (acl_get_tag_type(posix_ace, &tag) != 0) {
		DEBUG_OUT (0, ("smb_acl_get_tag_type failed\n"));
		return False;
	}
	...
	case SMB_ACL_USER: {
		...
		if (puid == NULL) {
			DEBUG_OUT (0, ("smb_acl_get_qualifier failed\n"));
			return False;
		}
		ace->uid = *puid;
		acl_free(puid);
	...
	if (acl_get_permset(posix_ace, &permset) != 0) {
		DEBUG_OUT (0, ("smb_acl_get_mode failed\n"));
		return False;
	}
	ace->a_perm = 0;
...
static acl_t smb_acl_to_posix(const struct smb_acl_t *acl)
{
	...
	for (i=0; i<acl->count; i++) {
		...
		if (acl_create_entry(&result, &e) != 0) {
			DEBUG_OUT (1, ("acl_create_entry failed: %s\n",
				  strerror(errno)));
			goto fail;
		}
		...
		case SMB_ACL_MASK:
			...
			break;
		default:
			DEBUG_OUT (1, ("Unknown tag value %d\n", entry->a_type));
			goto fail;
		}

		if (acl_set_tag_type(e, tag) != 0) {
			DEBUG_OUT (10, ("acl_set_tag_type(%d) failed: %s\n",
				   tag, strerror(errno)));
			goto fail;
		}
		...
		case SMB_ACL_USER:
			if (acl_set_qualifier(e, &entry->uid) != 0) {
				DEBUG_OUT (1, ("acl_set_qualifiier failed: %s\n",
					  strerror(errno)));
				goto fail;
			}
			break;
	...
	if (acl_valid(result) != 0) {
		DEBUG_OUT (0, ("smb_acl_to_posix: ACL is invalid for set (%s)\n",
			  strerror(errno)));
		goto fail;
	}
