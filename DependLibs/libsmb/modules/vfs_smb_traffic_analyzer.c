					...
					const char *name, uint16_t port)
{
	...
	ret = getaddrinfo(name,
        ...
        if (ret) {
		DEBUG_OUT (3,("smb_traffic_analyzer_connect_inet_socket: "
	...
	DEBUG_OUT (3,("smb_traffic_analyzer: Internet socket mode. Hostname: %s,"
		"Port: %i\n", name, port));
	...
	if (ailist) {
        ...
        if (sockfd == -1) {
		DEBUG_OUT (1, ("smb_traffic_analyzer: unable to create "
			"socket, error is %s",
			strerror(errno)));
		return -1;
	}
						...
						const char *name)
{
	...
	DEBUG_OUT (7, ("smb_traffic_analyzer_connect_unix_socket: "
			"Unix domain socket mode. Using %s\n",
			name ));

	if ((sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
		DEBUG_OUT (1, ("smb_traffic_analyzer_connect_unix_socket: "
	...
	const char *akey, const char *str, size_t *len)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_send_data_socket: created %s"
		" as filling block.\n", filler));
	...
	const char *state_flags, size_t data_len)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_send_data_socket: created Header:\n"));
			...
			int dlength, int _socket)
{
		...
		if (write_data( _socket, header, len) != len) {
			DEBUG_OUT (1, ("smb_traffic_analyzer_send_data_socket: "
						"error sending the header"
						" over the socket!\n"));
                }
		DEBUG_OUT (10,("smb_traffic_analyzer_write_data: sending data:\n"));
		dump_data( 10, (uint8_t *)data, dlength);

                if (write_data( _socket, data, dlength) != dlength) {
                        DEBUG_OUT (1, ("smb_traffic_analyzer_write_data: "
					...
					enum vfs_id vfs_operation )
{
	...
	if (rf_sock == NULL || rf_sock->sock == -1) {
		DEBUG_OUT (1, ("smb_traffic_analyzer_send_data: socket is "
			"closed\n"));
		return;
	}
	...
	if (protocol_version != NULL && strcmp(protocol_version,"V1") == 0) {
		...
		case vfs_id_pread: ;
			...
			break;
		default:
			DEBUG_OUT (1, ("smb_traffic_analyzer: error! "
	...
	if (!str) {
		DEBUG_OUT (1, ("smb_traffic_analyzer_send_data: "
	...
	if ( akey != NULL ) {
		state_flags[2] = 'E';
		DEBUG_OUT (10, ("smb_traffic_analyzer_send_data_socket: a key was"
		...
		DEBUG_OUT (10, ("smb_traffic_analyzer_send_data_socket:"
			...
			const char *path)
{
	...
	s_data.path = path;
	DEBUG_OUT (10, ("smb_traffic_analyzer_chdir: CHDIR: %s\n", path));
		...
		const struct smb_filename *smb_fname_dst)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_rename: RENAME: %s / %s\n",
			...
			const char *path)
{
	...
	s_data.path = path;
	DEBUG_OUT (10, ("smb_traffic_analyzer_rmdir: RMDIR: %s\n", path));
			...
			const char *path, mode_t mode)
{
	...
	s_data.path = path;
	s_data.mode = mode;
	DEBUG_OUT (10, ("smb_traffic_analyzer_mkdir: MKDIR: %s\n", path));
				...
				size_t n)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_sendfile: sendfile(r): %s\n",
		fsp_str_dbg(fromfsp)));
	smb_traffic_analyzer_send_data(handle,
		&s_data,
		vfs_id_read);
				...
				size_t n)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_recvfile: recvfile(w): %s\n",
		fsp_str_dbg(tofsp)));
	smb_traffic_analyzer_send_data(handle,
		&s_data,
		vfs_id_write);
				...
				files_struct *fsp, void *data, size_t n)
{
	...
	DEBUG_OUT (10, ("smb_traffic_analyzer_read: READ: %s\n", fsp_str_dbg(fsp)));
		...
		files_struct *fsp, void *data, size_t n, SMB_OFF_T offset)
{
	...
	s_data.filename = fsp->fsp_name->base_name;
	DEBUG_OUT (10, ("smb_traffic_analyzer_pread: PREAD: %s\n",
		   fsp_str_dbg(fsp)));
			...
			files_struct *fsp, const void *data, size_t n)
{
	...
	s_data.filename = fsp->fsp_name->base_name;
	DEBUG_OUT (10, ("smb_traffic_analyzer_write: WRITE: %s\n",
		   fsp_str_dbg(fsp)));
	     ...
	     files_struct *fsp, const void *data, size_t n, SMB_OFF_T offset)
{
	...
	s_data.filename = fsp->fsp_name->base_name;
	DEBUG_OUT (10, ("smb_traffic_analyzer_pwrite: PWRITE: %s\n", \
		fsp_str_dbg(fsp)));
	...
	int flags, mode_t mode)
{
	...
	s_data.result = SMB_VFS_NEXT_OPEN( handle, smb_fname, fsp,
			flags, mode);
	DEBUG_OUT (10,("smb_traffic_analyzer_open: OPEN: %s\n",
		fsp_str_dbg(fsp)));
	s_data.filename = fsp->fsp_name->base_name;
	s_data.mode = mode;
	smb_traffic_analyzer_send_data(handle,
	...
	files_struct *fsp)
{
	...
	DEBUG_OUT (10,("smb_traffic_analyzer_close: CLOSE: %s\n",
		fsp_str_dbg(fsp)));
	s_data.filename = fsp->fsp_name->base_name;
	smb_traffic_analyzer_send_data(handle,
			&s_data,
...
NTSTATUS vfs_smb_traffic_analyzer_init(void)
{
	...
	if (vfs_smb_traffic_analyzer_debug_level == -1) {
		...
		DEBUG_OUT (1, ("smb_traffic_analyzer_init: Couldn't register custom"
			 "debugging class!\n"));
	} else {
		DEBUG_OUT (3, ("smb_traffic_analyzer_init: Debug class number of"
