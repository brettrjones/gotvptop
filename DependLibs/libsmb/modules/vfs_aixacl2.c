	...
	int32_t len;
		...
		if(type->u64 == ACL_NFS4) {
			...
			if(type->u64 == ACL_AIXC) {
				...
				DEBUG_OUT (0,("aixacl2_getlen:unknown type:%d\n",type->u64));
                		return False;
			}	
		}		
		DEBUG_OUT (10,("aixacl2_getlen:%d\n",len));
	return len;
}
...
static AIXJFS2_ACL_T *aixjfs2_getacl_alloc(const char *fname, acl_type_t *type)
{
	...
	if ((ret != 0) && (errno == ENOSPC)) {
		...
		DEBUG_OUT (10,("aixjfs2_getacl_alloc - acl_len:%d\n",len));
	...
	if (ret != 0) {
		DEBUG_OUT (8, ("aclx_get failed with %s\n", strerror(errno)));
		return NULL;
	}
	...
	SMB4ACL_T **ppacl, bool *pretryPosix)
{
	...
	DEBUG_OUT (10,("jfs2 get_nt_acl invoked for %s\n", name));
	...
	pacl = aixjfs2_getacl_alloc(name, &type);
        if (pacl == NULL) {
		DEBUG_OUT (9, ("aixjfs2_getacl_alloc failed for %s with %s\n",
				name, strerror(errno)));
		if (errno==ENOSYS)
			*pretryPosix = True;
		return False;
	...
	DEBUG_OUT (10, ("len: %d, version: %d, nace: %d, type: 0x%x\n",
	...
	*ppacl = smb_create_smb4acl();
	...
	DEBUG_OUT (10,("jfs2 get_nt_acl finished successfully\n"));
...
static SMB_ACL_T aixjfs2_get_posix_acl(const char *path, acl_type_t type)
{
	...
	int ret;
        ...
        if (acl == NULL) {
                DEBUG_OUT (10, ("aixjfs2_getacl failed for %s with %s\n",
                           path, strerror(errno)));
                if (errno == 0) {
                        errno = EINVAL;
                }
                                    ...
                                    SMB_ACL_TYPE_T type)
{
        ...
        case SMB_ACL_TYPE_DEFAULT:
                DEBUG_OUT (0, ("Got AIX JFS2 unsupported type: %d\n", type));
                return NULL;
        default:
...
)
{
	...
	if (aclx_gettypes(filepath, &acl_type_list, &acl_type_list_len)) {
		DEBUG_OUT (2, ("aclx_gettypes failed with error %s for %s\n",
	...
	for(i=0; i<acl_type_list.num_entries; i++) {
		if (acl_type_list.entries[i].u64==aclType) {
			...
			DEBUG_OUT (10, ("found %s ACL support for %s\n",
...
static bool aixjfs2_process_smbacl(files_struct *fsp, SMB4ACL_T *smbacl)
{
	...
	DEBUG_OUT (10, ("jfs2_process_smbacl invoked on %s\n", fsp_str_dbg(fsp)));
	...
	if (rc) {
		DEBUG_OUT (8, ("aclx_put failed with %s\n", strerror(errno)));
		return False;
	}

	DEBUG_OUT (10, ("jfs2_process_smbacl succeeded.\n"));
	return True;
}
			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("aixjfs2_sys_acl_set_file invoked for %s", name));

	rc = aixjfs2_query_acl_support((char *)name, ACL_AIXC, &acl_type_info);
	if (rc) {
		DEBUG_OUT (8, ("jfs2_set_nt_acl: AIXC support not found\n"));
		return -1;
	}
	...
	if (rc) {
		DEBUG_OUT (2, ("aclx_put failed with %s for %s\n",
			strerror(errno), name));
		return -1;
	}
			    ...
			    SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("aixjfs2_sys_acl_set_fd invoked for %s", fsp_str_dbg(fsp)));
	...
	if (rc) {
		DEBUG_OUT (2, ("aclx_fput failed with %s for %s\n",
