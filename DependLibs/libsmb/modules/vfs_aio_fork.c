...
static struct mmap_area *mmap_area_init(TALLOC_CTX *mem_ctx, size_t size)
{
	...
	if (fd == -1) {
		DEBUG_OUT (3, ("open(\"/dev/zero\") failed: %s\n",
			  strerror(errno)));
		goto fail;
	}
...
#error Can not pass file descriptors
	...
	if ((cmptr = CMSG_FIRSTHDR(&msg)) != NULL
		...
		if (cmptr->cmsg_level != SOL_SOCKET) {
			DEBUG_OUT (10, ("control level != SOL_SOCKET"));
			errno = EINVAL;
			return -1;
		}
		if (cmptr->cmsg_type != SCM_RIGHTS) {
			DEBUG_OUT (10, ("control type != SCM_RIGHTS"));
			errno = EINVAL;
			return -1;
		}
			      ...
			      void *private_data)
{
	...
	for (child = list->children; child != NULL; child = next) {
		...
		if (child->aiocb != NULL) {
			DEBUG_OUT (10, ("child %d currently active\n",
				   (int)child->pid));
			continue;
		}

		if (child->dont_delete) {
			DEBUG_OUT (10, ("Child %d was active since last cleanup\n",
				   (int)child->pid));
			child->dont_delete = false;
			continue;
		}

		DEBUG_OUT (10, ("Child %d idle for more than 30 seconds, "
			   "deleting\n", (int)child->pid));
...
static void aio_child_loop(int sockfd, struct mmap_area *map)
{
	while (true) {
		...
		if (ret != sizeof(cmd_struct)) {
			DEBUG_OUT (10, ("read_fd returned %d: %s\n", (int)ret,
				   strerror(errno)));
			exit(1);
		}

		DEBUG_OUT (10, ("aio_child_loop: %s %d bytes at %d from fd %d\n",
...
#ifdef ENABLE_BUILD_FARM_HACKS
		{
			...
			msecs = randval + 20;
			DEBUG_OUT (10, ("delaying for %u msecs\n", msecs));
			smb_msleep(msecs);
		}
#endif
...
/* This breaks "make test" when run with aio_fork module. */
		...
		DEBUG_OUT (10, ("aio_child_loop: syscall returned %d\n",
			   (int)ret_struct.size));
		...
		if (ret != sizeof(ret_struct)) {
			DEBUG_OUT (10, ("could not write ret_struct: %s\n",
				   strerror(errno)));
			exit(2);
		}
	}
				  ...
				  void *p)
{
	...
	DEBUG_OUT (10, ("handle_aio_completion called with flags=%d\n", flags));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("aio child %d died: %s\n", (int)child->pid,
			  nt_errstr(status)));
		child->retval.size = -1;
	...
	if (child->aiocb == NULL) {
		DEBUG_OUT (1, ("Inactive child died\n"));
		TALLOC_FREE(child);
		return;
	}
...
static int aio_child_destructor(struct aio_child *child)
{
	...
	DEBUG_OUT (10, ("aio_child_destructor: removing child %d on fd %d\n",
				 ...
				 struct aio_child **presult)
{
	...
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, fdpair) == -1) {
		...
		DEBUG_OUT (10, ("socketpair() failed: %s\n", strerror(errno)));
		goto fail;
	}

	DEBUG_OUT (10, ("fdpair = %d/%d\n", fdpair[0], fdpair[1]));

	result->map = mmap_area_init(result, map_size);
	if (result->map == NULL) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0, ("Could not create mmap area\n"));
		goto fail;
	}
	...
	DEBUG_OUT (10, ("Child %d created with sockfd %d\n",
			result->pid, fdpair[0]));
			       ...
			       struct aio_child **pchild)
{
	...
	if (child == NULL) {
		DEBUG_OUT (10, ("no idle child found, creating new one\n"));

		status = create_aio_child(handle->conn->sconn, children,
					  128*1024, &child);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("create_aio_child failed: %s\n",
				   nt_errstr(status)));
			return status;
		}
	}
			 ...
			 struct files_struct *fsp, SMB_STRUCT_AIOCB *aiocb)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Could not get an idle child\n"));
		return -1;
	}
	...
	DEBUG_OUT (10, ("sending fd %d to child %d\n", fsp->fh->fd,
		   (int)child->pid));

	ret = write_fd(child->sockfd, &cmd, sizeof(cmd), fsp->fh->fd);
	if (ret == -1) {
		DEBUG_OUT (10, ("write_fd failed: %s\n", strerror(errno)));
		return -1;
	}
