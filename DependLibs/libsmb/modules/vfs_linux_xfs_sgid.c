...
static int linux_xfs_sgid_mkdir(vfs_handle_struct *handle,  const char *path, mode_t mode)
{
	...
	DEBUG_OUT (10, ("Calling linux_xfs_sgid_mkdir(%s)\n", path));

	mkdir_res = SMB_VFS_NEXT_MKDIR(handle, path, mode);
	if (mkdir_res == -1) {
		DEBUG_OUT (10, ("SMB_VFS_NEXT_MKDIR returned error: %s\n",
			   strerror(errno)));
		return mkdir_res;
	}

	if (!parent_dirname(talloc_tos(), path, &fname.base_name, NULL)) {
		DEBUG_OUT (1, ("parent_dirname failed\n"));
	...
	if (res == -1) {
		DEBUG_OUT (10, ("NEXT_STAT(%s) failed: %s\n", fname.base_name,
			   strerror(errno)));
		/* return success, we did the mkdir */
		return mkdir_res;
	}
	...
	if ((fname.st.st_ex_mode & S_ISGID) == 0) {
		/* No SGID to inherit */
		DEBUG_OUT (10, ("No SGID to inherit\n"));
		return mkdir_res;
	}
	...
	res = SMB_VFS_NEXT_STAT(handle, &fname);
	if (res == -1) {
		DEBUG_OUT (2, ("Could not stat just created dir %s: %s\n", path,
			  strerror(errno)));
		/* return success, we did the mkdir */
		return mkdir_res;
	}
	...
	if (res == -1) {
		DEBUG_OUT (2, ("CHMOD(%s, %o) failed: %s\n", path,
