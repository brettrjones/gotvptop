				       ...
				       bool labels)
{
	...
	if (!p) {
		DEBUG_OUT (0, ("shadow_copy_get_shadow_copy_data: osc_opendir() "
	...
	while (true) {
		...
		if (!shadow_copy_match_name(d, &snap_component)) {
			DEBUG_OUT (10,("shadow_copy_get_shadow_copy_data: ignore "
				  "[%s]\n",d));
			continue;
		}

		DEBUG_OUT (7,("shadow_copy_get_shadow_copy_data: not ignore "
			 "[%s]\n",d));
		...
		if (tlabels == NULL) {
			DEBUG_OUT (0,("shadow_copy_get_shadow_copy_data: Out of "
				 "memory\n"));
...
NTSTATUS vfs_shadow_copy_init(void)
{
	...
	if (vfs_onefs_shadow_copy_debug_level == -1) {
		...
		DEBUG_OUT (0, ("Couldn't register custom debugging class!\n"));
	} else {
		DEBUG_OUT (10, ("Debug class number of 'onefs_shadow_copy': %d\n",
