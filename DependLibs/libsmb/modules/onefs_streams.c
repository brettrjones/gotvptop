				     ...
				     struct smb_filename **smb_fname_out)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("Failed to prep stream name for %s: %s\n",
			  *smb_fname_out ?
			     ...
			     int *base_fdp)
{
	...
	DEBUG_OUT (10, ("Getting stream directory fd: %s (%d)\n", base,
		   base_fdp ? *base_fdp : -1));
	...
	if (base_fdp && *base_fdp >= 0) {
		...
		base_fd = onefs_sys_create_file(conn,
						...
						NULL);
		if (base_fd < 0) {
			DEBUG_OUT (5, ("Failed getting base fd: %s\n",
				  strerror(errno)));
			return -1;
		}
	}
	...
	if (dir_fd < 0) {
		DEBUG_OUT (5, ("Failed getting stream directory fd: %s\n",
			  strerror(errno)));
	}
		 ...
		 const struct smb_filename *smb_fname_dst)
{
	...
	if (is_ntfs_default_stream_smb_fname(smb_fname_src) ||
		...
		DEBUG_OUT (3, ("Unable to rename to/from a default stream: %s -> "
	...
	DEBUG_OUT (8, ("onefs_rename called for %s => %s\n",
		       ...
		       const char *stream, SMB_STRUCT_STAT *sbuf, int flags)
{
	...
	if (ret != -1) {
		DEBUG_OUT (10, ("stat of stream '%s' failed: %s\n", stream,
			   strerror(errno)));
	} else {
				   ...
				   SMB_STRUCT_STAT *base_sbuf)
{
	...
	if ((dirp = fdopendir(dir_fd)) == NULL) {
		DEBUG_OUT (0, ("Error on opendir %s. errno=%d (%s)\n",
	...
	while ((dp = SMB_VFS_READDIR(conn, dirp, NULL)) != NULL) {
		...
		if ((stream_fd = onefs_sys_create_file(conn,
							 ...
							 0,
							 NULL)) == -1) {
			DEBUG_OUT (0, ("Error opening stream %s:%s. "
		...
		if (ret) {
			DEBUG_OUT (0, ("Error fstating stream %s:%s. "
				  "errno=%d (%s)\n", fname, dp->d_name, errno,
				  strerror(errno)));
			continue;
		}
