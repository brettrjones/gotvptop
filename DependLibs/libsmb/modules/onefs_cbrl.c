...
static void onefs_cbrl_enumerate_blq(const char *fn)
{
	...
	DEBUG_OUT (10, ("CBRL BLR records (%s):\n", fn));
...
static struct blocking_lock_record *onefs_cbrl_find_blr(uint64_t id)
{
	...
	if (blr == NULL) {
		DEBUG_OUT (5, ("Could not find CBRL BLR for id %llu\n", id));
		return NULL;
	}
...
static void onefs_cbrl_async_success(uint64_t id)
{
	...
	DEBUG_OUT (10, ("CBRL async success!\n"));
...
static void onefs_cbrl_async_failure(uint64_t id)
{
	...
	DEBUG_OUT (10, ("CBRL async failure!\n"));
...
static void onefs_cbrl_events_handler(struct event_context *ev,
					...
					void *private_data)
{
	DEBUG_OUT (10, ("onefs_cbrl_events_handler\n"));

	if (cbrl_event_dispatcher(&cbrl_ops)) {
		DEBUG_OUT (0, ("cbrl_event_dispatcher failed: %s\n",
			strerror(errno)));
	}
}
...
static void onefs_init_cbrl(void)
{
	...
	DEBUG_OUT (10, ("onefs_init_cbrl\n"));

	/* Register the event channel for CBRL. */
	cbrl_event_fd = cbrl_event_register();
	if (cbrl_event_fd == -1) {
		DEBUG_OUT (0, ("cbrl_event_register failed: %s\n",
			strerror(errno)));
		return;
	}

	DEBUG_OUT (10, ("cbrl_event_fd = %d\n", cbrl_event_fd));
				...
				struct blocking_lock_record *blr)
{
	...
	DEBUG_OUT (10, ("Calling ifs_cbrl(LOCK)...\n"));
	...
	if (!error) {
		...
		DEBUG_OUT (0, ("onefs_brl_lock_windows failure: error=%d (%s).\n",
...
success:
	...
	DEBUG_OUT (10, ("returning NT_STATUS_OK.\n"));
	return NT_STATUS_OK;
}
			      ...
			      const struct lock_struct *plock)
{
	...
	DEBUG_OUT (10, ("Calling ifs_cbrl(UNLOCK)...\n"));
			      ...
			      struct blocking_lock_record *blr)
{
	...
	if (bs->state == ONEFS_CBRL_DONE || bs->state == ONEFS_CBRL_ERROR) {
		/* No-op. */
		DEBUG_OUT (10, ("State=%d, returning true\n", bs->state));
	...
	/* A real cancel. */
	DEBUG_OUT (10, ("Calling ifs_cbrl(CANCEL)...\n"));
