					...
					size_t count)
{
	...
	if ( offset % rhd->off_bound == 0) {
		...
		DEBUG_OUT (10,("readahead_sendfile: readahead on fd %u, offset %llu, len %u returned %d\n",
		...
		DEBUG_OUT (10,("readahead_sendfile: posix_fadvise on fd %u, offset %llu, len %u returned %d\n",
			(unsigned int)fromfsp->fh->fd,
			(unsigned long long)offset,
			(unsigned int)rhd->len,
			err ));
#else
		if (!rhd->didmsg) {
			DEBUG_OUT (0,("readahead_sendfile: no readahead on this platform\n"));
			rhd->didmsg = True;
		}
#endif
	}
				...
				SMB_OFF_T offset)
{
	...
	if ( offset % rhd->off_bound == 0) {
		...
		DEBUG_OUT (10,("readahead_pread: readahead on fd %u, offset %llu, len %u returned %d\n",
		...
		DEBUG_OUT (10,("readahead_pread: posix_fadvise on fd %u, offset %llu, len %u returned %d\n",
			(unsigned int)fsp->fh->fd,
			(unsigned long long)offset,
			(unsigned int)rhd->len,
			err ));
#else
		if (!rhd->didmsg) {
			DEBUG_OUT (0,("readahead_pread: no readahead on this platform\n"));
			rhd->didmsg = True;
		}
#endif
        }
				...
				const char *user)
{
	...
	if (!rhd) {
		...
		DEBUG_OUT (0,("readahead_connect: out of memory\n"));
		return -1;
	}
	ZERO_STRUCTP(rhd);
