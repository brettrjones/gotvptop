			  ...
			  const char *service, const char *user)
{
	...
	if (strcmp("fsname", algorithm) == 0) {
		...
		SMB_VFS_NEXT_DISCONNECT(handle);
		DEBUG_OUT (0,("fileid_connect(): unknown algorithm[%s]\n", algorithm));
		return -1;
	}
	...
	DEBUG_OUT (10, ("fileid_connect(): connect to service[%s] with algorithm[%s]\n",
		service, algorithm));
...
{
	DEBUG_OUT (10,("fileid_disconnect() connect to service[%s].\n",
...
NTSTATUS vfs_fileid_init(void)
{
	...
	if (vfs_fileid_debug_level == -1) {
		...
		DEBUG_OUT (0, ("vfs_fileid: Couldn't register custom debugging class!\n"));
	} else {
		DEBUG_OUT (10, ("vfs_fileid: Debug class number of 'fileid': %d\n", vfs_fileid_debug_level));
	}
