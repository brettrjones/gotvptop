		   ...
		   char **path)
{
	...
	if (!(*path)) {
		...
		if (error) {
			 ...
			 * events. */
			DEBUG_OUT (3,("Path lookup failed. LINS are invalid: "
			...
			*path = talloc_asprintf(wc, "%s", path_buf);
			DEBUG_OUT (5, ("Using path from event LIN = %s\n",
			    path_buf));
			SAFE_FREE(path_buf);
		}
	}
		      ...
		      struct ifs_event *e)
{
	...
	DEBUG_OUT (5, ("Retrieved ifs event from kernel: lin=%#llx, ifs_events=%#x, "
	...
	if (e->lin == 0) {
		...
		DEBUG_OUT (3, ("We've missed some kernel ifs events!\n"));
	...
	if (e->lin == wc->watch_lin) {
		...
		DEBUG_OUT (5, ("Skipping notification on root of the watched "
			 "path.\n"));
		return;
	}
	...
	if(!get_ifs_event_path(wc, e, &path)) {
		DEBUG_OUT (3, ("Failed to convert the ifs_event lins to a path. "
			 "Skipping this event\n"));
		return;
	}
	...
	DEBUG_OUT (5, ("Converted smb_filter=%#x, ifs_events=%#x, to "
		     ...
		     void *private_data)
{
	...
	if (nread == 0) {
		DEBUG_OUT (0,("No data found while reading ifs event fd?!\n"));
		return;
	}
	if (nread < 0) {
		DEBUG_OUT (0,("Failed to read ifs event data: %s\n",
			strerror(errno)));
		return;
	}
	...
	DEBUG_OUT (5, ("Got %d notification events in %d bytes.\n", count, nread));
		   ...
		   void *handle_p)
{
	...
	if (smb_filter & ONEFS_NOTIFY_UNSUPPORTED) {
		...
		DEBUG_OUT (3,("One or more of the Windows completion filter bits "
	...
	if (ifs_filter == 0) {
		...
		DEBUG_OUT (3,("No bits in the Windows completion filter could be "
	...
	ifs_event_fd = ifs_create_listener(watch_tree ?
					   ...
					   e->dir_fd);
	if (ifs_event_fd < 0) {
		DEBUG_OUT (0,("Failed to create listener for %s with \"%s\". "
	...
	if (onefs_sys_fstat(e->dir_fd, &sbuf)) {
		DEBUG_OUT (0, ("stat on directory fd failed: %s\n",
			  strerror(errno)));
	...
	DEBUG_OUT (10, ("Watching for changes on \"%s\" smb_filter=0x%x, "
