				     ...
				     struct tdb_xattrs **presult)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_pull_tdb_xattrs failed: %s\n",
			  ndr_errstr(ndr_err)));
		TALLOC_FREE(result);
				     ...
				     TDB_DATA *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_push_tdb_xattrs failed: %s\n",
			  ndr_errstr(ndr_err)));
		return ndr_map_error2ntstatus(ndr_err);
	}
				     ...
				     const struct tdb_xattrs *attribs)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("xattr_tdb_push_attrs failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
				 ...
				 const char *name, void *value, size_t size)
{
	...
	DEBUG_OUT (10, ("xattr_tdb_getattr called for file %s, name %s\n",
		   file_id_string_tos(id), name));

	status = xattr_tdb_load_attrs(talloc_tos(), db_ctx, id, &attribs);

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("xattr_tdb_fetch_attrs failed: %s\n",
			   nt_errstr(status)));
		errno = EINVAL;
		return -1;
	}
			     ...
			     const void *value, size_t size, int flags)
{
	...
	DEBUG_OUT (10, ("xattr_tdb_setattr called for file %s, name %s\n",
		   file_id_string_tos(id), name));

	rec = xattr_tdb_lock_attrs(talloc_tos(), db_ctx, id);

	if (rec == NULL) {
		DEBUG_OUT (0, ("xattr_tdb_lock_attrs failed\n"));
		errno = EINVAL;
		return -1;
	}
	...
	if (i == attribs->num_eas) {
		...
		if (tmp == NULL) {
			DEBUG_OUT (0, ("TALLOC_REALLOC_ARRAY failed\n"));
			TALLOC_FREE(rec);
			errno = ENOMEM;
			return -1;
		}
				  ...
				  size_t size)
{
	...
	DEBUG_OUT (10, ("xattr_tdb_listattr: Found %d xattrs\n",
		   attribs->num_eas));

	for (i=0; i<attribs->num_eas; i++) {
		size_t tmp;

		DEBUG_OUT (10, ("xattr_tdb_listattr: xattrs[i].name: %s\n",
			   attribs->eas[i].name));
			  ...
			  const char *user)
{
	...
	if (!xattr_tdb_init(snum, &db)) {
		DEBUG_OUT (5, ("Could not init xattr tdb\n"));
