...
static SMB_STRUCT_DIR *shadow_copy_opendir(vfs_handle_struct *handle, const char *fname, const char *mask, uint32 attr)
{
	...
	if (!p) {
		DEBUG_OUT (0,("shadow_copy_opendir: SMB_VFS_NEXT_OPENDIR() failed for [%s]\n",fname));
		return NULL;
	}

	dirp = SMB_MALLOC_P(shadow_copy_Dir);
	if (!dirp) {
		DEBUG_OUT (0,("shadow_copy_opendir: Out of memory\n"));
	...
	while (True) {
		...
		if (shadow_copy_match_name(d->d_name)) {
			DEBUG_OUT (8,("shadow_copy_opendir: hide [%s]\n",d->d_name));
			continue;
		}

		DEBUG_OUT (10,("shadow_copy_opendir: not hide [%s]\n",d->d_name));
...
static SMB_STRUCT_DIR *shadow_copy_fdopendir(vfs_handle_struct *handle, files_struct *fsp, const char *mask, uint32 attr)
{
	...
	if (!p) {
		DEBUG_OUT (10,("shadow_copy_opendir: SMB_VFS_NEXT_FDOPENDIR() failed for [%s]\n",
	...
	dirp = SMB_MALLOC_P(shadow_copy_Dir);
	if (!dirp) {
		DEBUG_OUT (0,("shadow_copy_fdopendir: Out of memory\n"));
		SMB_VFS_NEXT_CLOSEDIR(handle,p);
	...
	while (True) {
		...
		if (shadow_copy_match_name(d->d_name)) {
			DEBUG_OUT (8,("shadow_copy_fdopendir: hide [%s]\n",d->d_name));
			continue;
		}

		DEBUG_OUT (10,("shadow_copy_fdopendir: not hide [%s]\n",d->d_name));
					    ...
					    bool labels)
{
	...
	if (!p) {
		DEBUG_OUT (0,("shadow_copy_get_shadow_copy_data: SMB_VFS_NEXT_OPENDIR() failed for [%s]\n",fsp->conn->connectpath));
		return -1;
	}
	...
	while (True) {
		...
		/* */
		if (!shadow_copy_match_name(d->d_name)) {
			DEBUG_OUT (10,("shadow_copy_get_shadow_copy_data: ignore [%s]\n",d->d_name));
			continue;
		}

		DEBUG_OUT (7,("shadow_copy_get_shadow_copy_data: not ignore [%s]\n",d->d_name));
		...
		if (tlabels == NULL) {
			DEBUG_OUT (0,("shadow_copy_get_shadow_copy_data: Out of memory\n"));
			SMB_VFS_NEXT_CLOSEDIR(handle,p);
			return -1;
		}
