			  ...
			  bool check_valid)
{
	...
	DEBUG_OUT (10, ("file_is_valid (%s) called\n", path));

	if (SMB_VFS_GETXATTR(handle->conn, path, SAMBA_XATTR_MARKER,
				  &buf, sizeof(buf)) != sizeof(buf)) {
		DEBUG_OUT (10, ("GETXATTR failed: %s\n", strerror(errno)));
		return false;
	}

	if (buf != '1') {
		DEBUG_OUT (10, ("got wrong buffer content: '%c'\n", buf));
		return false;
	}
			    ...
			    bool check_valid)
{
	...
	DEBUG_OUT (10, ("marking file %s as valid\n", path));

	ret = SMB_VFS_SETXATTR(handle->conn, path, SAMBA_XATTR_MARKER,
				    &buf, sizeof(buf), 0);

	if (ret == -1) {
		DEBUG_OUT (10, ("SETXATTR failed: %s\n", strerror(errno)));
		return false;
	}
				 ...
				 bool create_dir)
{
	...
	*smb_fname_out = NULL;
	...
	DEBUG_OUT (10, ("stream filename = %s\n", stream_fname));
 ...
 fail:
	DEBUG_OUT (5, ("stream_name failed: %s\n", strerror(errno)));
			     ...
			     void *private_data)
{
	...
	DEBUG_OUT (10, ("walk_streams: dirname=%s\n", dirname));
	...
	while ((dirent = vfs_readdirname(handle->conn, dirhandle, NULL,
		...
		DEBUG_OUT (10, ("walk_streams: dirent=%s\n", dirent));
			      ...
			      struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10, ("streams_depot_stat called for [%s]\n",
			       ...
			       struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10, ("streams_depot_lstat called for [%s]\n",
		   smb_fname_str_dbg(smb_fname)));
				...
				const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10, ("streams_depot_unlink called for %s\n",
		   smb_fname_str_dbg(smb_fname)));
...
static int streams_depot_rmdir(vfs_handle_struct *handle, const char *path)
{
	...
	DEBUG_OUT (10, ("streams_depot_rmdir called for %s\n", path));
				...
				const struct smb_filename *smb_fname_dst)
{
	...
	DEBUG_OUT (10, ("streams_depot_rename called for %s => %s\n",
			       ...
			       void *private_data)
{
	...
	if (SMB_VFS_NEXT_STAT(state->handle, smb_fname) == -1) {
		DEBUG_OUT (10, ("Could not stat %s: %s\n", sname,
			   strerror(errno)));
		ret = true;
		goto out;
	}
