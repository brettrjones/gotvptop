				    ...
				    SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10,("Entering AIX sys_acl_get_file\n"));
	DEBUG_OUT (10,("path_p is %s\n",path_p));

	file_acl = (struct acl *)SMB_MALLOC(BUFSIZ);
 
	if(file_acl == NULL) {
		errno=ENOMEM;
		DEBUG_OUT (0,("Error in AIX sys_acl_get_file: %d\n",errno));
		return(NULL);
	}
	...
	if( (rc == -1) && (errno == ENOSPC)) {
		...
		if( rc == -1) {
			DEBUG_OUT (0,("statacl returned %d with errno %d\n",rc,errno));
			SAFE_FREE(file_acl);
			return(NULL);
		}
	}

	DEBUG_OUT (10,("Got facl and returned it\n"));
				...
				files_struct *fsp)
{
	...
	DEBUG_OUT (10,("Entering AIX sys_acl_get_fd\n"));
	DEBUG_OUT (10,("fd is %d\n",fsp->fh->fd));
	file_acl = (struct acl *)SMB_MALLOC(BUFSIZ);

	if(file_acl == NULL) {
		errno=ENOMEM;
		DEBUG_OUT (0,("Error in AIX sys_acl_get_fd is %d\n",errno));
		return(NULL);
	}
	...
	if( (rc == -1) && (errno == ENOSPC)) {
		...
		if( rc == -1) {
			DEBUG_OUT (0,("fstatacl returned %d with errno %d\n",rc,errno));
			SAFE_FREE(file_acl);
			return(NULL);
		}
	}
			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10,("errno is %d\n",errno));
	DEBUG_OUT (10,("return code is %d\n",rc));
	SAFE_FREE(file_acl);
	DEBUG_OUT (10,("Exiting the aixacl_sys_acl_set_file\n"));
			    ...
			    SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10,("return code is %d\n",rc));
	SAFE_FREE(file_acl);
	DEBUG_OUT (10,("Exiting aixacl_sys_acl_set_fd\n"));
