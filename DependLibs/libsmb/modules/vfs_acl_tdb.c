				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (10,("acl_tdb_delete: rec == NULL\n"));
		TALLOC_FREE(rec);
		return NT_STATUS_OK;
	}
			...
			DATA_BLOB *pblob)
{
	...
	DEBUG_OUT (10,("get_acl_blob: returned %u bytes from file %s\n",
				...
				DATA_BLOB *pblob)
{
	...
	DEBUG_OUT (10,("store_acl_blob_fsp: storing blob length %u on file %s\n",
	...
	rec = db->fetch_locked(db, talloc_tos(),
				make_tdb_data(id_buf,
					sizeof(id_buf)));
	if (rec == NULL) {
		DEBUG_OUT (0, ("store_acl_blob_fsp_tdb: fetch_lock failed\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
				...
				const char *user)
{
	 ...
	 * using this module. */
	DEBUG_OUT (2,("connect_acl_tdb: setting 'inherit acls = true' "
