				      ...
				      char **found_name)
{
	...
	if (result == -1) {
		DEBUG_OUT (10, ("smbd_gpfs_get_realfilename_path returned %s\n",
			   strerror(errno)));
		return -1;
	}
	...
	DEBUG_OUT (10, ("smbd_gpfs_get_realfilename_path: %s/%s -> %s\n",
...
static void gpfs_dumpacl(int level, struct gpfs_acl *gacl)
{
	...
	if (gacl==NULL)
	{
		DEBUG_OUT (0, ("gpfs acl is NULL\n"));
		return;
	}

	DEBUG_OUT (level, ("gpfs acl: nace: %d, type:%d, version:%d, level:%d, len:%d\n",
	...
	for(i=0; i<gacl->acl_nace; i++)
	{
		...
		DEBUG_OUT (level, ("\tace[%d]: type:%d, flags:0x%x, mask:0x%x, iflags:0x%x, who:%u\n",
...
static struct gpfs_acl *gpfs_getacl_alloc(const char *fname, gpfs_aclType_t type)
{
	...
	if (ret != 0)
	{
		DEBUG_OUT (8, ("smbd_gpfs_getacl failed with %s\n",strerror(errno)));
		return NULL;
	}
...
static int gpfs_get_nfs4_acl(const char *fname, SMB4ACL_T **ppacl)
{
	...
	DEBUG_OUT (10, ("gpfs_get_nfs4_acl invoked for %s\n", fname));

	/* First get the real acl length */
	gacl = gpfs_getacl_alloc(fname, 0);
	if (gacl == NULL) {
		DEBUG_OUT (9, ("gpfs_getacl failed for %s with %s\n",
			   fname, strerror(errno)));
		return -1;
	}

	if (gacl->acl_type != GPFS_ACL_TYPE_NFS4) {
		DEBUG_OUT (10, ("Got non-nfsv4 acl\n"));
	...
	*ppacl = smb_create_smb4acl();
	...
	DEBUG_OUT (10, ("len: %d, level: %d, version: %d, nace: %d\n",
	...
	for (i=0; i<gacl->acl_nace; i++) {
		...
		DEBUG_OUT (10, ("type: %d, iflags: %x, flags: %x, mask: %x, "
		...
		if (gace->aceIFlags & ACE4_IFLAG_SPECIAL_ID) {
			...
			case ACE4_SPECIAL_EVERYONE:
				...
				break;
			default:
				DEBUG_OUT (8, ("invalid special gpfs id %d "
					  "ignored\n", gace->aceWho));
	...
	struct security_descriptor **ppdesc)
{
	...
	*ppdesc = NULL;
	...
	if (result > 0) {
		DEBUG_OUT (10, ("retrying with posix acl...\n"));
...
static bool gpfsacl_process_smbacl(files_struct *fsp, SMB4ACL_T *smbacl)
{
	...
	for (smbace=smb_first_ace4(smbacl); smbace!=NULL; smbace = smb_next_ace4(smbace)) {
		...
		if (!fsp->is_directory
			...
			DEBUG_OUT (2, ("vfs_gpfs.c: file [%s]: ACE contains "
		...
		if (aceprop->flags&SMB_ACE4_ID_SPECIAL)
		{
			...
			case SMB_ACE4_WHO_GROUP:
				...
				break;
			default:
				DEBUG_OUT (8, ("unsupported special_id %d\n", aceprop->who.special_id));
				continue; /* don't add it !!! */
			}
		} else {
	...
	if (ret != 0) {
		DEBUG_OUT (8, ("gpfs_putacl failed with %s\n", strerror(errno)));
		gpfs_dumpacl(8, gacl);
		return False;
	}

	DEBUG_OUT (10, ("gpfs_putacl succeeded\n"));
	return True;
}
...
static NTSTATUS gpfsacl_set_nt_acl_internal(files_struct *fsp, uint32 security_info_sent, const struct security_descriptor *psd)
{
	...
	if (acl->acl_version&GPFS_ACL_VERSION_NFS4)
	{
		if (lp_parm_bool(fsp->conn->params->service, "gpfs",
			...
			DEBUG_OUT (2, ("Rejecting unsupported ACL with DACL_PROTECTED bit set\n"));
...
static SMB_ACL_T gpfs2smb_acl(const struct gpfs_acl *pacl)
{
	...
	for (i=0; i<pacl->acl_nace; i++) {
		...
		DEBUG_OUT (10, ("Converting type %d id %lu perm %x\n",
		...
		case GPFS_ACL_MASK:
			...
			break;
		default:
			DEBUG_OUT (10, ("Got invalid ace_type: %d\n",
				   g_ace->ace_type));
			errno = EINVAL;
			SAFE_FREE(result);
			return NULL;
...
static SMB_ACL_T gpfsacl_get_posix_acl(const char *path, gpfs_aclType_t type)
{
	...
	if (pacl == NULL) {
		DEBUG_OUT (10, ("gpfs_getacl failed for %s with %s\n",
			   path, strerror(errno)));
		if (errno == 0) {
			errno = EINVAL;
		}
	...
	if (pacl->acl_version != GPFS_ACL_VERSION_POSIX) {
		DEBUG_OUT (10, ("Got acl version %d, expected %d\n",
					  ...
					  SMB_ACL_TYPE_T type)
{
	...
	case SMB_ACL_TYPE_DEFAULT:
		...
		break;
	default:
		DEBUG_OUT (0, ("Got invalid type: %d\n", type));
		smb_panic("exiting");
	}
				     ...
				     SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("smb2gpfs_acl: Got ACL with %d entries\n", pacl->count));
	...
	for (i=0; i<pacl->count; i++) {
		...
		DEBUG_OUT (10, ("Converting type %d perm %x\n",
...
static int gpfsacl_emu_chmod(const char *path, mode_t mode)
{
	...
	DEBUG_OUT (10, ("gpfsacl_emu_chmod invoked for %s mode %o\n", path, mode));
	...
	if (mode & ~(S_IRWXU | S_IRWXG | S_IRWXO)) {
		DEBUG_OUT (2, ("WARNING: cutting extra mode bits %o on %s\n", mode, path));
	}
	...
	for (smbace=smb_first_ace4(pacl); smbace!=NULL; smbace = smb_next_ace4(smbace)) {
		...
		if (ace->flags&SMB_ACE4_ID_SPECIAL &&
			...
			if (ace->aceMask!=newMask) {
				DEBUG_OUT (10, ("ace changed for %s (%o -> %o) id=%d\n",
	...
	for(i = SMB_ACE4_WHO_OWNER; i<=SMB_ACE4_WHO_EVERYONE; i++) {
		...
		DEBUG_OUT (10, ("Added ALLOW ace for %s, mode=%o, id=%d, aceMask=%x\n",
...
static int gpfs_set_xattr(struct vfs_handle_struct *handle,  const char *path,
	...
	struct xattr_DOSATTRIB dosattrib;
        ...
        DEBUG_OUT (10, ("gpfs_set_xattr: %s \n",path));

        /* Only handle DOS Attributes */
        if (strcmp(name,SAMBA_XATTR_DOS_ATTRIB) != 0){
		DEBUG_OUT (5, ("gpfs_set_xattr:name is %s\n",name));
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (1, ("gpfs_set_xattr: bad ndr decode "
	...
	if (dosattrib.version != 3) {
		DEBUG_OUT (1, ("gpfs_set_xattr: expected dosattrib version 3, got "
			  "%d\n", (int)dosattrib.version));
		return false;
	}
	if (!(dosattrib.info.info3.valid_flags & XATTR_DOSINFO_ATTRIB)) {
		DEBUG_OUT (10, ("gpfs_set_xattr: XATTR_DOSINFO_ATTRIB not "
			   "valid, ignoring\n"));
		return true;
	}
	...
	dosmode = dosattrib.info.info3.attrib;
		...
		if (errno == ENOSYS) {
			return SMB_VFS_NEXT_SETXATTR(handle, path, name, value,
                ...
                DEBUG_OUT (1, ("gpfs_set_xattr:Set GPFS attributes failed %d\n",ret));
                return -1;
        }

        DEBUG_OUT (10, ("gpfs_set_xattr:Set attributes: 0x%x\n",attrs.winAttrs));
        return 0;
}
...
static ssize_t gpfs_get_xattr(struct vfs_handle_struct *handle,  const char *path,
        ...
        DEBUG_OUT (10, ("gpfs_get_xattr: %s \n",path));

        /* Only handle DOS Attributes */
        if (strcmp(name,SAMBA_XATTR_DOS_ATTRIB) != 0){
		DEBUG_OUT (5, ("gpfs_get_xattr:name is %s\n",name));
		...
		if (errno == ENOSYS) {
			return SMB_VFS_NEXT_GETXATTR(handle, path, name, value,
                ...
                DEBUG_OUT (1, ("gpfs_get_xattr: Get GPFS attributes failed: "
			  "%d (%s)\n", ret, strerror(errno)));
                return -1;
        }

        DEBUG_OUT (10, ("gpfs_get_xattr:Got attributes: 0x%x\n",attrs.winAttrs));
        ...
        DEBUG_OUT (10, ("gpfs_get_xattr: returning %s\n",attrstr));
        return 4;
}
			...
			struct smb_file_time *ft)
{
        ...
        if(ret == -1){
                DEBUG_OUT (1,("vfs_gpfs_ntimes: SMB_VFS_NEXT_NTIMES failed\n"));
                return -1;
        }

        if(null_timespec(ft->create_time)){
                DEBUG_OUT (10,("vfs_gpfs_ntimes:Create Time is NULL\n"));
                return 0;
        }
        ...
        if(ret == -1 && errno != ENOSYS){
                DEBUG_OUT (1,("vfs_gpfs_ntimes: set GPFS ntimes failed %d\n",ret));
	        return -1;
        }
        return 0;
				...
				SMB_STRUCT_STAT *sbuf)
{
	...
	if ((attrs.winAttrs & GPFS_WINATTR_OFFLINE) != 0) {
		DEBUG_OUT (10, ("%s is offline\n", path));
		TALLOC_FREE(path);
		return true;
	}
	DEBUG_OUT (10, ("%s is online\n", path));
	TALLOC_FREE(path);
