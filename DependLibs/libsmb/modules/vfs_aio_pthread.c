...
static bool init_aio_threadpool(struct vfs_handle_struct *handle)
{
	...
	DEBUG_OUT (10,("init_aio_threadpool: initialized with up to %d threads\n",
			num_threads));
				...
				SMB_STRUCT_AIOCB *aiocb)
{
	...
	if (pd == NULL) {
		DEBUG_OUT (10, ("aio_pthread_read: Could not create private data.\n"));
		return -1;
	}
	...
	DEBUG_OUT (10, ("aio_pthread_read: jobid=%d pread requested "
				...
				SMB_STRUCT_AIOCB *aiocb)
{
	...
	pd = create_private_data(aio_ex, aiocb);
	if (pd == NULL) {
		DEBUG_OUT (10, ("aio_pthread_write: Could not create private data.\n"));
		return -1;
	}
	...
	DEBUG_OUT (10, ("aio_pthread_write: jobid=%d pwrite requested "
		"of %llu bytes at offset %llu\n",
		pd->jobid,
		(unsigned long long)pd->aiocb->aio_nbytes,
		(unsigned long long)pd->aiocb->aio_offset));
				...
				void *p)
{
	...
	DEBUG_OUT (10, ("aio_pthread_handle_completion called with flags=%d\n",
			(int)flags));
	...
	while (true) {
		...
		if (pd == NULL) {
			DEBUG_OUT (1, ("aio_pthread_handle_completion cannot find "
				  "jobid %d\n", jobid));
			return;
		}
		...
		DEBUG_OUT (10,("aio_pthread_handle_completion: jobid %d "
			  "completed\n", jobid ));
		TALLOC_FREE(aio_ex);
	}
}
				...
				SMB_STRUCT_AIOCB *aiocb)
{
	...
	if (pd == NULL) {
		errno = EINVAL;
		DEBUG_OUT (0, ("aio_pthread_return_fn: returning EINVAL\n"));
		return -1;
	}
				...
				void *private_data)
{
	...
	if (pd == NULL) {
		DEBUG_OUT (1, ("aio_pthread_handle_immediate cannot find jobid %d\n",
			  *pjobid));
		TALLOC_FREE(pjobid);
		return;
	}
				...
				void *p)
{
	...
	DEBUG_OUT (10, ("aio_pthread_handle_suspend_completion called with flags=%d\n",
			(int)flags));
	...
	if (pd == NULL) {
		DEBUG_OUT (1, ("aio_pthread_handle_completion cannot find jobid %d\n",
			  *pjobid));
		TALLOC_FREE(pjobid);
		return;
	}
	...
	DEBUG_OUT (10,("aio_pthread_handle_suspend_completion: "
