				      ...
				      SMB4ACL_T **ppacl)
{
	...
	if((naces = acl(name, ACE_GETACLCNT, 0, NULL)) == -1) {
		if(errno == ENOSYS) {
			DEBUG_OUT (9, ("acl(ACE_GETACLCNT, %s): Operation is not "
				  "supported on the filesystem where the file "
				  "reside", name));
		} else {
			DEBUG_OUT (9, ("acl(ACE_GETACLCNT, %s): %s ", name,
					strerror(errno)));
		}
	...
	if(acl(name, ACE_GETACL, naces, acebuf) < 0) {
		DEBUG_OUT (9, ("acl(ACE_GETACL, %s): %s ", name,
				strerror(errno)));
		return map_nt_error_from_unix(errno);
	}
	/* create SMB4ACL data */
...
static bool zfs_process_smbacl(files_struct *fsp, SMB4ACL_T *smbacl)
{
	...
	if(acl(fsp->fsp_name->base_name, ACE_SETACL, naces, acebuf)) {
		if(errno == ENOSYS) {
			DEBUG_OUT (9, ("acl(ACE_SETACL, %s): Operation is not "
				  "supported on the filesystem where the file "
				  "reside", fsp_str_dbg(fsp)));
		} else {
			DEBUG_OUT (9, ("acl(ACE_SETACL, %s): %s ", fsp_str_dbg(fsp),
				  strerror(errno)));
		}
		return 0;
	}
