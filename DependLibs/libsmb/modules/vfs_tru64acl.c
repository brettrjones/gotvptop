				    ...
				    SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("Hi! This is tru64acl_sys_acl_get_file.\n"));
			      ...
			      SMB_ACL_T theacl)
{
        ...
        DEBUG_OUT (10, ("tru64acl_sys_acl_set_file called with name %s, type %d\n", 
			name, type));
	...
	tru64_acl = smb_acl_to_tru64_acl(theacl);
        if (tru64_acl == NULL) {
		DEBUG_OUT (10, ("smb_acl_to_tru64_acl failed!\n"));
                goto fail;
        }
	DEBUG_OUT (10, ("got tru64 acl...\n"));
        res = acl_set_file((char *)name, the_acl_type, tru64_acl);
        acl_free(tru64_acl);
        if (res != 0) {
                DEBUG_OUT (10, ("acl_set_file failed: %s\n", strerror(errno)));
		goto fail;
        }
        return res;
fail:
	DEBUG_OUT (1, ("tru64acl_sys_acl_set_file failed!\n"));
	return -1;
}
...
static struct smb_acl_t *tru64_acl_to_smb_acl(const struct acl *tru64_acl) 
{
	...
	DEBUG_OUT (10, ("Hi! This is tru64_acl_to_smb_acl.\n"));
	
	if ((result = SMB_MALLOC_P(struct smb_acl_t)) == NULL) {
		DEBUG_OUT (0, ("SMB_MALLOC_P failed in tru64_acl_to_smb_acl\n"));
		errno = ENOMEM;
		goto fail;
	}
	ZERO_STRUCTP(result);
	if (acl_first_entry((struct acl *)tru64_acl) != 0) {
		DEBUG_OUT (10, ("acl_first_entry failed: %s\n", strerror(errno)));
		goto fail;
	}
	while ((entry = acl_get_entry((struct acl *)tru64_acl)) != NULL) {
		result = SMB_REALLOC(result, sizeof(struct smb_acl_t) +
					(sizeof(struct smb_acl_entry) * 
					 (result->count + 1)));
		if (result == NULL) {
			DEBUG_OUT (0, ("SMB_REALLOC failed in tru64_acl_to_smb_acl\n"));
			errno = ENOMEM;
			goto fail;
		}
		/* XYZ */
...
fail:
	...
	DEBUG_OUT (1, ("tru64_acl_to_smb_acl failed!\n"));
	return NULL;
}
				...
				struct smb_acl_entry *smb_ace) 
{
	...
	if (acl_get_tag_type(tru64_ace, &tru64_tag) != 0) {
		DEBUG_OUT (0, ("acl_get_tag_type failed: %s\n", strerror(errno)));
		return False;
	}
	...
	if (smb_tag_type == 0) {
		DEBUG_OUT (3, ("invalid tag type given: %d\n", tru64_tag));
		return False;
	}
	if (sys_acl_set_tag_type(smb_ace, smb_tag_type) != 0) {
		DEBUG_OUT (3, ("sys_acl_set_tag_type failed: %s\n", 
				strerror(errno)));
		return False;
	}
...
static acl_t smb_acl_to_tru64_acl(const SMB_ACL_T smb_acl) 
{
	...
	DEBUG_OUT (10, ("Hi! This is smb_acl_to_tru64_acl.\n"));

	result = acl_init(1);

	if (result == NULL) {
		DEBUG_OUT (3, ("acl_init failed!\n"));
		goto fail;
	}
	...
	for (i = 0; i < smb_acl->count; i++) {
		...
		if (tru64_tag == -1) {
			DEBUG_OUT (3, ("smb_tag_to_tru64 failed!\n"));
			goto fail;
		}
		...
		if (tru64_entry == NULL) {
			DEBUG_OUT (3, ("acl_create_entry failed: %s\n", 
					strerror(errno)));
			goto fail;
		}

		if (acl_set_tag_type(tru64_entry, tru64_tag) != 0) {
			DEBUG_OUT (3, ("acl_set_tag_type(%d) failed: %s\n",
					strerror(errno)));
			goto fail;
		}
		...
		case SMB_ACL_USER:
			if (acl_set_qualifier(tru64_entry, 
						(int *)&smb_entry->uid) != 0) 
			{
				DEBUG_OUT (3, ("acl_set_qualifier failed: %s\n",
					strerror(errno)));
				goto fail;
			}
	...
	if (acl_valid(result, &tru64_entry) != 0) {
		DEBUG_OUT (1, ("smb_acl_to_tru64_acl: ACL is invalid (%s)\n",
				strerror(errno)));
		if (tru64_entry != NULL) {
	...
	if (acl_text == NULL) {
		DEBUG_OUT (3, ("acl_to_text failed: %s\n", strerror(errno)));
		goto fail;
	}
	DEBUG_OUT (1, ("acl_text: %s\n", acl_text));
	free(acl_text);
...
fail:
	...
	DEBUG_OUT (1, ("smb_acl_to_tru64_acl failed!\n"));
	return NULL;
}
...
static acl_tag_t smb_tag_to_tru64(SMB_ACL_TAG_T smb_tag)
{
	...
	case SMB_ACL_MASK:
		...
		break;
	default:
		DEBUG_OUT (1, ("Unknown tag type %d\n", smb_tag));
		result = -1;
	}
	return result;
}
...
static SMB_ACL_TAG_T tru64_tag_to_smb(acl_tag_t tru64_tag)
{
	...
	case ACL_MASK:
		...
		break;
	default:
		DEBUG_OUT (0, ("Unknown tag type %d\n", (unsigned int)tru64_tag));
		smb_tag_type = 0;
	}
	return smb_tag_type;
}
...
static acl_perm_t smb_permset_to_tru64(SMB_ACL_PERM_T smb_permset)
{
	...
	if (acl_clear_perm(&tru64_permset) != 0) {
		DEBUG_OUT (5, ("acl_clear_perm failed: %s\n", strerror(errno)));
		return -1;
	}
