...
#error Isilon, please remove this after testing the code below
	...
	for (tmp = ctxt->ops_chain; tmp; tmp = tmp->next) {
		...
		ISP_OP_END(&tmp->iod);
#ifdef ONEFS_PERF_DEBUG
		DEBUG_OUT (0,("********  Finalized CHAIN op %s uid %llu in:%llu"
			", out:%llu\n",
	...
	ISP_OP_END(&ctxt->iod);
#ifdef ONEFS_PERF_DEBUG
	DEBUG_OUT (0,("********  Finalized op %s uid %llu in:%llu, out:%llu\n",
