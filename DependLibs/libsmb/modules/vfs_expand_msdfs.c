			      ...
			      const char *clientaddr)
{
	...
	if (f == NULL) {
		DEBUG_OUT (0,("can't open IP map %s. Error %s\n",
			 mapfile, strerror(errno) ));
		return NULL;
	}

	DEBUG_OUT (10, ("Scanning mapfile [%s]\n", mapfile));
	...
	while (x_fgets(buf, sizeof(buf), f) != NULL) {
		...
		DEBUG_OUT (10, ("Scanning line [%s]\n", buf));

		space = strchr_m(buf, ' ');

		if (space == NULL) {
			DEBUG_OUT (0, ("Ignoring invalid line %s\n", buf));
			continue;
		}
				...
				char *target)
{
	...
	if (filename_start == NULL) {
		DEBUG_OUT (10, ("No filename start in %s\n", target));
		return NULL;
	}
	...
	if (filename_end == NULL) {
		DEBUG_OUT (10, ("No filename end in %s\n", target));
		return NULL;
	}
	...
	DEBUG_OUT (10, ("Expanding from table [%s]\n", mapfilename));

	targethost = read_target_host(
		ctx, conn->sconn->client_id.addr, mapfilename);
	if (targethost == NULL) {
		DEBUG_OUT (1, ("Could not expand target host from file %s\n",
			  mapfilename));
		return NULL;
	}
	...
	DEBUG_OUT (10, ("Expanded targethost to %s\n", targethost));
