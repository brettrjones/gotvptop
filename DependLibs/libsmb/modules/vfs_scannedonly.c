...
static int connect_to_scanner(vfs_handle_struct * handle)
{
	...
	if (so->domain_socket) {
		...
		DEBUG_OUT (SCANNEDONLY_DEBUG, ("socket=%s\n", so->socketname));
		if ((so->socket = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
			DEBUG_OUT (2, ("failed to create socket %s\n",
				  so->socketname));
			return -1;
		}
		...
		if (connect(so->socket, (struct sockaddr *)(void *)&saun,
			    SUN_LEN(&saun)) < 0) {
			DEBUG_OUT (2, ("failed to connect to socket %s\n",
				  so->socketname));
			return -1;
		}
		DEBUG_OUT (SCANNEDONLY_DEBUG,("bound %s to socket %d\n",
					 saun.sun_path, so->socket));

	} else {
		so->socket = open_udp_socket(so->scanhost, so->portnum);
		if (so->socket < 0) {
			DEBUG_OUT (2,("failed to open UDP socket to %s:%d\n",
	...
	{/* increasing the socket buffer is done because we have large bursts
		...
		getsockopt(so->socket, SOL_SOCKET, SO_RCVBUF,
			   (char *)&sndsize, &size);
		DEBUG_OUT (SCANNEDONLY_DEBUG, ("current socket buffer size=%d\n",
					  sndsize));
		sndsize = 262144;
...
static void flush_sendbuffer(vfs_handle_struct * handle)
{
	...
	do {
		...
		if (ret == -1) {
			DEBUG_OUT (3,("scannedonly flush_sendbuffer: "
				     ...
				     int recheck_size, int loop)
{
	...
	if (!VALID_STAT(smb_fname->st)) {
		DEBUG_OUT (SCANNEDONLY_DEBUG,("stat %s\n",smb_fname->base_name));
		retval = SMB_VFS_NEXT_STAT(handle, smb_fname);
		if (retval != 0) {
			/* failed to stat this file?!? --> hide it */
			DEBUG_OUT (SCANNEDONLY_DEBUG,("no valid stat, return"
						 " allow_nonexistant=%d\n",
						 allow_nonexistant));
	...
	if (smb_fname->st.st_ex_size == 0) {
		DEBUG_OUT (SCANNEDONLY_DEBUG,("empty file, return 1\n"));
			    ...
			    struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (SCANNEDONLY_DEBUG, ("scannedonly_stat: %s returned %d\n",
	...
	if (ret != 0 && errno == ENOENT) {
		...
		if (test_base_name) {
			...
			ret = SMB_VFS_NEXT_STAT(handle, smb_fname);
			DEBUG_OUT (5, ("_stat: %s returned %d\n",
				  test_base_name, ret));
			     ...
			     struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (SCANNEDONLY_DEBUG, ("scannedonly_lstat: %s returned %d\n",
				  smb_fname->base_name, ret));
	if (ret != 0 && errno == ENOENT) {
		TALLOC_CTX *ctx=talloc_tos();
		char *test_base_name, *tmp_base_name = smb_fname->base_name;
		...
		if (test_base_name) {
			smb_fname->base_name = test_base_name;
			ret = SMB_VFS_NEXT_LSTAT(handle, smb_fname);
			DEBUG_OUT (5, ("_lstat: %s returned %d\n",
				  test_base_name, ret));
			smb_fname->base_name = tmp_base_name;
		}
	}
			    ...
			    files_struct * fsp, int flags, mode_t mode)
{
	...
	allowed = scannedonly_allow_access(
		...
		0);
	flush_sendbuffer(handle);
	DEBUG_OUT (SCANNEDONLY_DEBUG, ("scannedonly_open: allow=%d for %s\n",
			      ...
			      const struct smb_filename *smb_fname)
{
	...
	if (SMB_VFS_NEXT_UNLINK(handle, smb_fname_cache) != 0) {
		DEBUG_OUT (SCANNEDONLY_DEBUG, ("_unlink: failed to unlink %s\n",
...
static int scannedonly_rmdir(vfs_handle_struct * handle, const char *path)
{
	...
	if (have_files && only_deletable_files) {
		...
		while ((dire = SMB_VFS_NEXT_READDIR(handle, dirp, NULL))
			...
			create_synthetic_smb_fname(ctx, fullpath,NULL,NULL,
						   &smb_fname);
			DEBUG_OUT (SCANNEDONLY_DEBUG, ("unlink %s\n", fullpath));
