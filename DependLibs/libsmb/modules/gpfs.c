			...
			uint32 share_access)
{
	...
	if (allow == GPFS_SHARE_NONE) {
		DEBUG_OUT (10, ("special case am=no_access:%x\n",access_mask));
	}
	else {	
	...
	DEBUG_OUT (10, ("am=%x, allow=%d, sa=%x, deny=%d\n",
		   access_mask, allow, share_access, deny));

	result = gpfs_set_share_fn(fsp->fh->fd, allow, deny);
	if (result != 0) {
		if (errno == ENOSYS) {
			DEBUG_OUT (5, ("VFS module vfs_gpfs loaded, but no gpfs "
				  "support has been compiled into Samba. Allowing access\n"));
			return True;
		} else {
			DEBUG_OUT (10, ("gpfs_set_share failed: %s\n",
				   strerror(errno)));
		}
	}
...
int get_gpfs_winattrs(char *pathname,struct gpfs_winattr *attrs)
{
        ...
        DEBUG_OUT (10, ("gpfs_get_winattrs_path:open call %s\n",pathname));
...
int smbd_fget_gpfs_winattrs(int fd, struct gpfs_winattr *attrs)
{
        ...
        DEBUG_OUT (10, ("gpfs_get_winattrs_path:open call %d\n", fd));
...
int set_gpfs_winattrs(char *pathname,int flags,struct gpfs_winattr *attrs)
{
        ...
        DEBUG_OUT (10, ("gpfs_set_winattrs_path:open call %s\n",pathname));
...
void smbd_gpfs_lib_init()
{
	if (gpfs_lib_init_fn) {
		...
		DEBUG_OUT (10, ("gpfs_lib_init() finished with rc %d "
			   "and errno %d\n", rc, errno));
	} else {
		DEBUG_OUT (10, ("libgpfs lacks gpfs_lib_init\n"));
	}
}
				   ...
				   void *pfn_pointer, const char *fn_name)
{
	...
	DEBUG_OUT (10, ("trying to load name %s from %s\n",
		   fn_name, libname));
	...
	if (*libhandle_pointer == NULL) {
		DEBUG_OUT (10, ("Could not open lib %s\n", libname));
		return false;
	}

	*fn_pointer = dlsym(*libhandle_pointer, fn_name);
	if (*fn_pointer == NULL) {
		DEBUG_OUT (10, ("Did not find symbol %s in lib %s\n",
			   fn_name, libname));
		if (did_open_here) {
...
{
	DEBUG_OUT (0, ("'VFS module smbgpfs loaded, without gpfs support compiled\n"));
...
bool set_gpfs_sharemode(files_struct *fsp, uint32 access_mask,
			uint32 share_access)
{
	DEBUG_OUT (0, ("VFS module - smbgpfs.so loaded, without gpfs support compiled\n"));
