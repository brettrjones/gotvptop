				      ...
				      SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("solarisacl_sys_acl_get_file called for file '%s'.\n", 
		   path_p));
	...
	if (result == NULL) {
		DEBUG_OUT (10, ("conversion solaris_acl -> smb_acl failed (%s).\n",
			   strerror(errno)));
	}
	
 done:
	DEBUG_OUT (10, ("solarisacl_sys_acl_get_file %s.\n",
				    ...
				    files_struct *fsp)
{
	...
	DEBUG_OUT (10, ("entering solarisacl_sys_acl_get_fd.\n"));
	...
	DEBUG_OUT (10, ("solarisacl_sys_acl_get_fd %s.\n", 
		   ((result == NULL) ? "failed" : "succeeded")));
	SAFE_FREE(solaris_acl);
	return result;
}
				...
				SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("solarisacl_sys_acl_set_file called for file '%s'\n",
		   name));
	...
	if(!smb_acl_to_solaris_acl(theacl, &solaris_acl, &count, type)) {
		DEBUG_OUT (10, ("conversion smb_acl -> solaris_acl failed (%s).\n",
			   strerror(errno)));
                goto done;
	}
	...
	DEBUG_OUT (10, ("solarisacl_sys_acl_set_file %s.\n",
			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG_OUT (10, ("entering solarisacl_sys_acl_set_fd\n"));
	...
	if (!solaris_acl_get_fd(fsp->fh->fd, &default_acl, &default_count)) {
		DEBUG_OUT (10, ("error getting (default) acl from fd\n"));
		goto done;
	}
	if (!solaris_add_to_acl(&solaris_acl, &count,
				...
				SMB_ACL_TYPE_DEFAULT))
	{
		DEBUG_OUT (10, ("error adding default acl to solaris acl\n"));
		goto done;
	}
	if (!solaris_acl_sort(solaris_acl, count)) {
		DEBUG_OUT (10, ("resulting acl is not valid!\n"));
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("call of facl failed (%s).\n", strerror(errno)));
	}

 done:
	DEBUG_OUT (10, ("solarisacl_sys_acl_set_fd %s.\n",
				       ...
				       const char *path)
{
	...
	DEBUG_OUT (10, ("entering solarisacl_sys_acl_delete_def_file.\n"));
	...
	if (!smb_acl_to_solaris_acl(smb_acl, &solaris_acl, &count, 
				    SMB_ACL_TYPE_ACCESS))
	{
		DEBUG_OUT (10, ("conversion smb_acl -> solaris_acl failed.\n"));
		goto done;
	}
	if (!solaris_acl_sort(solaris_acl, count)) {
		DEBUG_OUT (10, ("resulting acl is not valid!\n"));
	...
	DEBUG_OUT (10, ("solarisacl_sys_acl_delete_def_file %s.\n",
		   ((ret != 0) ? "failed" : "succeeded" )));
	SAFE_FREE(smb_acl);
	return ret;
}
				   ...
				   SMB_ACL_TYPE_T type)
{
	...
	DEBUG_OUT (10, ("entering smb_acl_to_solaris_acl\n"));
	...
	*count = 0;
	...
	for (i = 0; i < smb_acl->count; i++) {
		...
		if (solaris_entry.a_type == 0) {
			DEBUG_OUT (10, ("smb_tag to solaris_tag failed\n"));
			goto fail;
		}
		switch(solaris_entry.a_type) {
		case USER:
			DEBUG_OUT (10, ("got tag type USER with uid %u\n", 
				   (unsigned int)smb_entry->uid));
			solaris_entry.a_id = (uid_t)smb_entry->uid;
			break;
		case GROUP:
			DEBUG_OUT (10, ("got tag type GROUP with gid %u\n", 
		...
		if (type == SMB_ACL_TYPE_DEFAULT) {
			DEBUG_OUT (10, ("adding default bit to solaris ace\n"));
			solaris_entry.a_type |= ACL_DEFAULT;
		}
		
		solaris_entry.a_perm = 
			smb_perm_to_solaris_perm(smb_entry->a_perm);
		DEBUG_OUT (10, ("assembled the following solaris ace:\n"));
 ...
 fail:
	...
	DEBUG_OUT (10, ("smb_acl_to_solaris_acl %s\n",
					...
					SMB_ACL_TYPE_T type)
{
 ...
 fail:
	...
	DEBUG_OUT (10, ("solaris_acl_to_smb_acl %s\n",
		   ((result == NULL) ? "failed" : "succeeded")));
	return result;
}
...
static SOLARIS_ACL_TAG_T smb_tag_to_solaris_tag(SMB_ACL_TAG_T smb_tag)
{
	...
	DEBUG_OUT (10, ("smb_tag_to_solaris_tag\n"));
...
static SMB_ACL_TAG_T solaris_tag_to_smb_tag(SOLARIS_ACL_TAG_T solaris_tag)
{
	...
	DEBUG_OUT (10, ("solaris_tag_to_smb_tag:\n"));
				 ...
				 int *count)
{
	...
	DEBUG_OUT (10, ("solaris_acl_get_file called for file '%s'\n", name));
	...
	*count = acl(name, GETACLCNT, 0, NULL);
	if (*count < 0) {
		DEBUG_OUT (10, ("acl GETACLCNT failed: %s\n", strerror(errno)));
		goto done;
	}
	*solaris_acl = solaris_acl_init(*count);
	if (*solaris_acl == NULL) {
		DEBUG_OUT (10, ("error allocating memory for solaris acl...\n"));
		goto done;
	}
	*count = acl(name, GETACL, *count, *solaris_acl);
	if (*count < 0) {
		DEBUG_OUT (10, ("acl GETACL failed: %s\n", strerror(errno)));
		goto done;
	}
	result = True;

 done:
        DEBUG_OUT (10, ("solaris_acl_get_file %s.\n",
...
static bool solaris_acl_get_fd(int fd, SOLARIS_ACL_T *solaris_acl, int *count)
{
	...
	DEBUG_OUT (10, ("entering solaris_acl_get_fd\n"));
	...
	*count = facl(fd, GETACLCNT, 0, NULL);
	if (*count < 0) {
		DEBUG_OUT (10, ("facl GETACLCNT failed: %s\n", strerror(errno)));
		goto done;
	}
	*solaris_acl = solaris_acl_init(*count);
	if (*solaris_acl == NULL) {
	...
	*count = facl(fd, GETACL, *count, *solaris_acl);
	if (*count < 0) {
		DEBUG_OUT (10, ("facl GETACL failed: %s\n", strerror(errno)));
		goto done;
	}
	ret = True;

 done:
	DEBUG_OUT (10, ("solaris_acl_get_fd %s\n",
		   ((ret == True) ? "succeeded" : "failed")));
	return ret;
}
