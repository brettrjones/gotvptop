...
static struct bitmap *init_bitmap(TALLOC_CTX *mem_ctx, const char **ops)
{
	...
	if (bm == NULL) {
		DEBUG_OUT (0, ("Could not alloc bitmap -- "
	...
	for (; *ops != NULL; ops += 1) {
		...
		if (i == SMB_VFS_OP_LAST) {
			DEBUG_OUT (0, ("Could not find opname %s, logging all\n",
				  *ops));
			TALLOC_FREE(bm);
			return NULL;
		}
...
NTSTATUS vfs_full_audit_init(void)
{
	...
	if (vfs_full_audit_debug_level == -1) {
		...
		DEBUG_OUT (0, ("vfs_full_audit: Couldn't register custom debugging "
			  "class!\n"));
	} else {
		DEBUG_OUT (10, ("vfs_full_audit: Debug class number of "
