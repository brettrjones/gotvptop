				...
				int *granted_oplock)
{
	...
	if ((accmode == O_RDONLY) && ((flags & O_TRUNC) == O_TRUNC)) {
		DEBUG_OUT (10,("onefs_open_file: truncate requested on read-only "
			  "open for file %s\n", smb_fname_str_dbg(smb_fname)));
	...
	if ((local_flags & O_CREAT) && !file_existed &&
		 ...
		 */
		DEBUG_OUT (1, ("onefs_open_file: creating file with wildcard: %s\n",
			  smb_fname_str_dbg(smb_fname)));
	...
	if ((conn->fs_capabilities & FILE_NAMED_STREAMS) &&
		...
		DEBUG_OUT (10, ("Opening a stream: base=%s(%d), stream=%s\n",
				...
				struct smb_request *req)
{
	...
	if (!request_timed_out(request_time, timeout)) {
		...
		DEBUG_OUT (0, ("Deferred open timeout! request_time=%d.%d, "
				  ...
				  struct onefs_fsp_data *fsp_data)
{
	...
	if (conn->printer) {
		...
		DEBUG_OUT (10, ("onefs_open_file_ntcreate: printer open fname=%s\n",
			   smb_fname_str_dbg(smb_fname)));
	...
	DEBUG_OUT (10,("onefs_open_file_ntcreate: fname=%s, dos_attrs=0x%x "
	...
	if (req) {
		...
		if (get_deferred_open_message_state(req,
			 ...
			 */
			if (state->failed) {
				DEBUG_OUT (0, ("onefs_open_file_ntcreate: "
	...
	if (!lp_posix_pathnames() && strstr(smb_fname->base_name,".+,;=[].")) {
		 ...
		 * release. */
		DEBUG_OUT (5,("onefs_open_file_ntcreate: OS/2 long filenames are "
			  "not supported.\n"));
		if (use_nt_status()) {
	...
	switch( create_disposition ) {
		...
		case FILE_OPEN:
			...
			if (!file_existed) {
				DEBUG_OUT (5,("onefs_open_file_ntcreate: FILE_OPEN "
		...
		case FILE_OVERWRITE:
			 ...
			 * error. */
			if (!file_existed) {
				DEBUG_OUT (5, ("onefs_open_file_ntcreate: "
	...
	if (!posix_open && file_existed &&
		...
		if (!open_match_attributes(conn, existing_dos_attributes,
			...
			DEBUG_OUT (5, ("onefs_open_file_ntcreate: attributes "
	...
	DEBUG_OUT (10, ("onefs_open_file_ntcreate: fname=%s, after mapping "
	...
	if ((access_mask & (FILE_WRITE_DATA | FILE_APPEND_DATA)) ||
		...
		if (create_options & NTCREATEX_OPTIONS_PRIVATE_DENY_DOS) {
			...
			DEBUG_OUT (10,("onefs_open_file_ntcreate: deny_dos: "
	...
	if (flags != O_RDONLY && file_existed &&
		...
		DEBUG_OUT (5, ("onefs_open_file_ntcreate: write access requested "
	...
	DEBUG_OUT (4,("calling onefs_open_file with flags=0x%X flags2=0x%X "
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		if (errno == EINPROGRESS) {
			...
			if (lck == NULL) {
				...
				DEBUG_OUT (3, ("Someone created file %s with an "
			...
			DEBUG_OUT (5,("Async createfile because a client has an "
				 "oplock on %s\n",
				 smb_fname_str_dbg(smb_fname)));
		...
		if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
			...
			if (SMB_VFS_STAT(conn, fname, psbuf) == -1) {
				DEBUG_OUT (0,("Error doing stat on file %s "
	...
	if (!file_existed) {
		...
		if (lck == NULL) {
			DEBUG_OUT (0, ("onefs_open_file_ntcreate: Could not get "
	...
	if (fsp->oplock_type == LEVEL_II_OPLOCK &&
		...
		DEBUG_OUT (5, ("Downgrading level2 oplock on open "
	...
	if (!posix_open && !file_existed && !def_acl &&
...

		{
			...
			if (ret == -1 && errno == ENOSYS) {
				...
				DEBUG_OUT (5, ("onefs_open_file_ntcreate: reset "
		...
		if ((ret == -1) &&
			...
			DEBUG_OUT (5, ("onefs_open_file_ntcreate: failed to reset "
				  "attributes of file %s to 0%o\n",
				  smb_fname_str_dbg(smb_fname),
				  (unsigned int)new_unx_mode));
	}
				     ...
				     int *pinfo)
{
	...
	DEBUG_OUT (5, ("onefs_open_directory: opening directory %s, "
		  "access_mask = 0x%x, "
	...
	if (!(file_attributes & FILE_FLAG_POSIX_SEMANTICS) &&
		...
		DEBUG_OUT (2, ("onefs_open_directory: %s is a stream name!\n",
	...
	switch (create_disposition) {
		...
		case FILE_OVERWRITE_IF:
		default:
			DEBUG_OUT (5, ("onefs_open_directory: invalid "
 ...
 retry_open:
	...
	if (fsp->fh->fd == -1) {
		DEBUG_OUT (3, ("Error opening %s. Errno=%d (%s).\n",
		...
		if ((errno == ENOENT) && (create_disposition == FILE_OPEN)) {
			DEBUG_OUT (5, ("onefs_open_directory: FILE_OPEN requested "
				  "for directory %s and it doesn't "
				  "exist.\n", smb_fname_str_dbg(smb_dname)));
			return NT_STATUS_OBJECT_NAME_NOT_FOUND;
		} else if ((errno == EEXIST) &&
		    (create_disposition == FILE_CREATE)) {
			DEBUG_OUT (5, ("onefs_open_directory: FILE_CREATE "
	...
	if (info == FILE_WAS_CREATED) {
		...
		if (SMB_VFS_LSTAT(conn, smb_dname) == -1) {
			DEBUG_OUT (2, ("Could not stat directory '%s' just "
				  "created: %s\n",
				  smb_fname_str_dbg(smb_dname),
				  strerror(errno)));
			return map_nt_error_from_unix(errno);
		...
		if (!S_ISDIR(smb_dname->st.st_ex_mode)) {
			DEBUG_OUT (0, ("Directory just '%s' created is not a "
				  "directory\n",
				  smb_fname_str_dbg(smb_dname)));
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	if (lck == NULL) {
		DEBUG_OUT (0, ("onefs_open_directory: Could not get share mode "
			  "lock for %s\n", smb_fname_str_dbg(smb_dname)));
		fd_close(fsp);
		file_free(req, fsp);
		return NT_STATUS_SHARING_VIOLATION;
					   ...
					   struct onefs_fsp_data *fsp_data)
{
	...
	DEBUG_OUT (10,("onefs_create_file_unixpath: access_mask = 0x%x "
	...
	if ((conn->fs_capabilities & FILE_NAMED_STREAMS)
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("onefs_create_file_unixpath for base %s "
				   "failed: %s\n", smb_fname->base_name,
				   nt_errstr(status)));
			goto fail;
		}
		...
		if ((oplock_request & ~SAMBA_PRIVATE_OPLOCK_MASK) !=
		     NO_OPLOCK) {
			DEBUG_OUT (5, ("Oplock(%d) being requested on a stream! "
	...
	DEBUG_OUT (10, ("onefs_create_file_unixpath: info=%d\n", info));
 ...
 fail:
	DEBUG_OUT (10, ("onefs_create_file_unixpath: %s\n", nt_errstr(status)));
			   ...
			   int *pinfo)
{
	...
	DEBUG_OUT (10,("onefs_create_file: access_mask = 0x%x "
		  "file_attributes = 0x%x, share_access = 0x%x, "
		  "create_disposition = 0x%x create_options = 0x%x "
		  "oplock_request = 0x%x private_flags = 0x%x"
	...
	DEBUG_OUT (10, ("onefs_create_file: info=%d\n", info));
 ...
 fail:
	DEBUG_OUT (10, ("onefs_create_file: %s\n", nt_errstr(status)));
