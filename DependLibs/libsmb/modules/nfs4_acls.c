...
static SMB_ACL4_INT_T *get_validated_aclint(SMB4ACL_T *theacl)
{
	...
	if (theacl==NULL)
	{
		DEBUG_OUT (2, ("acl is NULL\n"));
		errno = EINVAL;
		return NULL;
	}
	if (aclint->magic!=SMB_ACL4_INT_MAGIC)
	{
		DEBUG_OUT (2, ("aclint bad magic 0x%x\n", aclint->magic));
		errno = EINVAL;
		return NULL;
	}
	return aclint;
...
static SMB_ACE4_INT_T *get_validated_aceint(SMB4ACE_T *ace)
{
	...
	if (ace==NULL)
	{
		DEBUG_OUT (2, ("ace is NULL\n"));
		errno = EINVAL;
		return NULL;
	}
	if (aceint->magic!=SMB_ACE4_INT_MAGIC)
	{
		DEBUG_OUT (2, ("aceint bad magic 0x%x\n", aceint->magic));
		errno = EINVAL;
		return NULL;
	}
	return aceint;
...
SMB4ACL_T *smb_create_smb4acl(void)
{
	...
	if (theacl==NULL)
	{
		DEBUG_OUT (0, ("TALLOC_SIZE failed\n"));
		errno = ENOMEM;
		return NULL;
	}
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	{
		DEBUG_OUT (8, ("vfs_stat_smb_fname failed with error %s\n",
			strerror(errno)));
		return -1;
	}
...
static int smbacl4_fGetFileOwner(files_struct *fsp, SMB_STRUCT_STAT *psbuf)
{
	...
	{
		DEBUG_OUT (8, ("SMB_VFS_FSTAT failed with error %s\n",
			strerror(errno)));
		return -1;
	}
...
)
{
	...
	DEBUG_OUT (10, ("smbacl_nfs42win entered\n"));
	...
	for (aceint=aclint->first; aceint!=NULL; aceint=(SMB_ACE4_INT_T *)aceint->next) {
		...
		DEBUG_OUT (10, ("magic: 0x%x, type: %d, iflags: %x, flags: %x, mask: %x, "
		...
		if (ace->flags & SMB_ACE4_ID_SPECIAL) {
			...
			case SMB_ACE4_WHO_EVERYONE:
				...
				break;
			default:
				DEBUG_OUT (8, ("invalid special who id %d "
					"ignored\n", ace->who.special_id));
			}
		} else {
		...
		if (!is_directory && (win_ace_flags & (SEC_ACE_FLAG_OBJECT_INHERIT|SEC_ACE_FLAG_CONTAINER_INHERIT))) {
			 ...
			 */
			DEBUG_OUT (10, ("removing inherit flags from nfs4 ace\n"));
			win_ace_flags &= ~(SEC_ACE_FLAG_OBJECT_INHERIT|SEC_ACE_FLAG_CONTAINER_INHERIT);
		}
		DEBUG_OUT (10, ("Windows mapped ace flags: 0x%x => 0x%x\n",
	...
	struct security_descriptor **ppdesc, SMB4ACL_T *theacl)
{
	...
	if (smbacl4_nfs42win(mem_ctx, theacl, &sid_owner, &sid_group,
		...
		DEBUG_OUT (8,("smbacl4_nfs42win failed\n"));
		return map_nt_error_from_unix(errno);
	}

	psa = make_sec_acl(mem_ctx, NT4_ACL_REVISION, good_aces, nt_ace_list);
	if (psa == NULL) {
		DEBUG_OUT (2,("make_sec_acl failed\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (10,("after make sec_acl\n"));
	*ppdesc = make_sec_desc(mem_ctx, SD_REVISION, SEC_DESC_SELF_RELATIVE,
	                        ...
	                        NULL, psa, &sd_size);
	if (*ppdesc==NULL) {
		DEBUG_OUT (2,("make_sec_desc failed\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (10, ("smb_get_nt_acl_nfs4_common successfully exited with sd_size %d\n",
			       ...
			       struct security_descriptor **ppdesc, SMB4ACL_T *theacl)
{
	...
	DEBUG_OUT (10, ("smb_fget_nt_acl_nfs4 invoked for %s\n", fsp_str_dbg(fsp)));
			      ...
			      struct security_descriptor **ppdesc, SMB4ACL_T *theacl)
{
	...
	DEBUG_OUT (10, ("smb_get_nt_acl_nfs4 invoked for %s\n", name));
...
)
{
	...
	DEBUG_OUT (10, ("mode:%s, do_chown:%s, acedup: %s\n",
...
static void smbacl4_dump_nfs4acl(int level, SMB4ACL_T *theacl)
{
	...
	DEBUG_OUT (level, ("NFS4ACL: size=%d\n", aclint->naces));

	for(aceint = aclint->first; aceint!=NULL; aceint=(SMB_ACE4_INT_T *)aceint->next) {
		SMB_ACE4PROP_T *ace = &aceint->prop;

		DEBUG_OUT (level, ("\tACE: type=%d, flags=0x%x, fflags=0x%x, mask=0x%x, id=%d\n",
			ace->aceType,
	...
	SMB_ACE4PROP_T *aceNew)
{
	...
	for(aceint = aclint->first; aceint!=NULL; aceint=(SMB_ACE4_INT_T *)aceint->next) {
		SMB_ACE4PROP_T *ace = &aceint->prop;
                ...
                DEBUG_OUT (10,("ace type:0x%x flags:0x%x aceFlags:0x%x "
			 ...
			 struct dom_sid *dst)
{
	...
	if (mapping_db == NULL) {
		...
		if (dbname == NULL) {
			DEBUG_OUT (10, ("%s:sidmap not defined\n",
		...
		if (mapping_db == NULL) {
			DEBUG_OUT (1, ("could not open sidmap: %s\n",
				  strerror(errno)));
			return False;
		}
	}
	...
	if (mapping_db->fetch(mapping_db, NULL,
			      ...
			      &data) == -1) {
		DEBUG_OUT (10, ("could not find mapping for SID %s\n",
			   sid_string_dbg(src)));
		return False;
	}
	...
	if ((data.dptr == NULL) || (data.dsize <= 0)
		...
		DEBUG_OUT (5, ("invalid mapping for SID %s\n",
			  sid_string_dbg(src)));
	...
	if (!string_to_sid(dst, (char *)data.dptr)) {
		DEBUG_OUT (1, ("invalid mapping %s for SID %s\n",
...
)
{
	...
	if (ace_v4->aceFlags!=ace_nt->flags)
		DEBUG_OUT (9, ("ace_v4->aceFlags(0x%x)!=ace_nt->flags(0x%x)\n",
			ace_v4->aceFlags, ace_nt->flags));

	if (ace_v4->aceMask!=ace_nt->access_mask)
		DEBUG_OUT (9, ("ace_v4->aceMask(0x%x)!=ace_nt->access_mask(0x%x)\n",
			ace_v4->aceMask, ace_nt->access_mask));
	...
	if (dom_sid_equal(&ace_nt->trustee, &global_sid_World)) {
		...
		if (!lookup_sid(mem_ctx, &sid, &dom, &name, &type)) {
			...
			if (!nfs4_map_sid(params, &sid, &mapped)) {
				DEBUG_OUT (1, ("nfs4_acls.c: file [%s]: SID %s "
					  "unknown\n", filename, sid_string_dbg(&sid)));
				errno = EINVAL;
				return False;
			}
			
			DEBUG_OUT (2, ("nfs4_acls.c: file [%s]: mapped SID %s "
		...
		if (type == SID_NAME_USER) {
			if (!sid_to_uid(&sid, &uid)) {
				DEBUG_OUT (1, ("nfs4_acls.c: file [%s]: could not "
					  "convert %s to uid\n", filename,
					  sid_string_dbg(&sid)));
				return False;
			}
...
)
{
	...
	if (ace4found)
	{
		...
		case e_reject: /* do an error */
			DEBUG_OUT (8, ("ACL rejected by duplicate nt ace#%d\n", i));
...
)
{
	...
	DEBUG_OUT (10, ("smbacl4_win2nfs4 invoked\n"));
	...
	for(i=0; i<dacl->num_aces; i++) {
		...
		if (!smbacl4_fill_ace4(mem_ctx, filename, pparams,
				       ownerUID, ownerGID,
				       dacl->aces + i, &ace_v4)) {
			DEBUG_OUT (3, ("Could not fill ace for file %s, SID %s\n",
				  filename,
	...
	set_nfs4acl_native_fn_t set_nfs4_native)
{
	...
	DEBUG_OUT (10, ("smb_set_nt_acl_nfs4 invoked for %s\n", fsp_str_dbg(fsp)));

	if ((security_info_sent & (SECINFO_DACL |
		SECINFO_GROUP | SECINFO_OWNER)) == 0)
	{
		DEBUG_OUT (9, ("security_info_sent (0x%x) ignored\n",
			security_info_sent));
	...
	if (params.do_chown) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (8, ("unpack_nt_owners failed"));
			return status;
		}
		if (((newUID != (uid_t)-1) && (sbuf.st_ex_uid != newUID)) ||
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (3,("chown %s, %u, %u failed. Error = "
					 "%s.\n", fsp_str_dbg(fsp),
					 (unsigned int)newUID,
					 (unsigned int)newGID,
					 nt_errstr(status)));
			...
			DEBUG_OUT (10,("chown %s, %u, %u succeeded.\n",
	...
	if (!(security_info_sent & SECINFO_DACL) || psd->dacl ==NULL) {
		DEBUG_OUT (10, ("no dacl found; security_info_sent = 0x%x\n", security_info_sent));
		return NT_STATUS_OK;
	}
	...
	if (result!=True) {
		errno = saved_errno;
		DEBUG_OUT (10, ("set_nfs4_native failed with %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	DEBUG_OUT (10, ("smb_set_nt_acl_nfs4 succeeded\n"));
	return NT_STATUS_OK;
}
