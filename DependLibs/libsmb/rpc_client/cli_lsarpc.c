					       ...
					       NTSTATUS *presult)
{
	...
	DEBUG_OUT (10, ("LSA_LOOKUPSIDS returned status: '%s', result: '%s', "
		   "mapped count = %d'\n",
	...
	for (i = 0; i < num_sids; i++) {
		...
		if (dom_idx != 0xffffffff) {
			...
			if (name) {
				...
				if ((names)[i] == NULL) {
					DEBUG_OUT (0, ("cli_lsa_lookup_sids_noalloc(): out of memory\n"));
					...
					NTSTATUS *presult)
{
	...
	if (num_sids) {
		if (!(domains = TALLOC_ARRAY(mem_ctx, char *, num_sids))) {
			DEBUG_OUT (0, ("rpccli_lsa_lookup_sids(): out of memory\n"));
	...
	while (sids_left > 0) {
		...
		DEBUG_OUT (10, ("rpccli_lsa_lookup_sids: processing items "
			   "%d -- %d of %d.\n",
			   sids_processed,
					 ...
					 NTSTATUS *presult)
{
	...
	if (num_names) {
		if (!((*sids = TALLOC_ARRAY(mem_ctx, struct dom_sid, num_names)))) {
			DEBUG_OUT (0, ("cli_lsa_lookup_sids(): out of memory\n"));
