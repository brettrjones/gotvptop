				...
				WERROR *pwerr)
{
	...
	if (data) {
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			DEBUG_OUT (2, ("dcerpc_winreg_query_sd: Failed to marshall "
				  "security descriptor\n"));
			*pwerr = WERR_NOMEM;
			return NT_STATUS_OK;
		}
			      ...
			      WERROR *pwerr)
{
	...
	if (data == NULL) {
		...
		if (!push_reg_sz(mem_ctx, &blob, data)) {
			DEBUG_OUT (2, ("dcerpc_winreg_set_sz: Could not marshall "
				  "string %s for %s\n",
				  data, wvalue.name));
			*pwerr = WERR_NOMEM;
			return NT_STATUS_OK;
				     ...
				     WERROR *pwerr)
{
	...
	if (data == NULL) {
		...
		if (!push_reg_sz(mem_ctx, &blob, data)) {
			DEBUG_OUT (2, ("dcerpc_winreg_set_expand_sz: Could not marshall "
				  "string %s for %s\n",
				  data, wvalue.name));
			*pwerr = WERR_NOMEM;
			return NT_STATUS_OK;
				    ...
				    WERROR *pwerr)
{
	...
	if (!push_reg_multi_sz(mem_ctx, &blob, data)) {
		DEBUG_OUT (2, ("dcerpc_winreg_set_multi_sz: Could not marshall "
			      ...
			      WERROR *pwerr)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (2, ("dcerpc_winreg_set_sd: Failed to marshall security "
			  "descriptor\n"));
		*pwerr = WERR_NOMEM;
		return NT_STATUS_OK;
	}
				 ...
				 WERROR *pwerr)
{
	...
	for (i = 0; i < num_subkeys; i++) {
		...
		status = dcerpc_winreg_EnumKey(h,
					       ...
					       &result);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (5, ("dcerpc_winreg_enum_keys: Could not enumerate keys: %s\n",
				  nt_errstr(status)));
			goto error;
		}
