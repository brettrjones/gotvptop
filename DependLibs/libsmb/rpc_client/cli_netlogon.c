				     ...
				     uint32_t *neg_flags_inout)
{
 ...
 again:
	...
	if (!netlogon_creds_client_check(cli->dc, &srv_chal_recv)) {
		 ...
		 */
		DEBUG_OUT (0,("rpccli_netlogon_setup_creds: server %s "
			"replied with bad credential\n",
			cli->desthost ));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG_OUT (5,("rpccli_netlogon_setup_creds: server %s credential "
		"chain established.\n",
		cli->desthost ));
				   ...
				   int logon_type)
{
	...
	default:
		DEBUG_OUT (0, ("switch value %d not supported\n",
			logon_type));
	...
	if (!netlogon_creds_client_check(cli->dc, &ret_creds.cred)) {
		DEBUG_OUT (0,("rpccli_netlogon_sam_logon: credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
					   ...
					   struct netr_SamInfo3 **info3)
{
	...
	*info3 = NULL;
	...
	/* Always check returned credentials. */
	if (!netlogon_creds_client_check(cli->dc, &ret_creds.cred)) {
		DEBUG_OUT (0,("rpccli_netlogon_sam_network_logon: credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
					    ...
					    enum netr_SchannelType sec_channel_type)
{
	...
	if (!cli->dc) {
		...
		result = rpccli_netlogon_setup_creds(cli,
						     ...
						     &neg_flags);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (3,("rpccli_netlogon_set_trust_password: unable to setup creds (%s)!\n",
				 nt_errstr(result)));
			return result;
		}
	}
	...
	if (cli->dc->negotiate_flags & NETLOGON_NEG_PASSWORD_SET2) {
		...
		status = dcerpc_netr_ServerPasswordSet2(b, mem_ctx,
							...
							&result);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("dcerpc_netr_ServerPasswordSet2 failed: %s\n",
				nt_errstr(status)));
			return status;
		}
	} else {
		...
		status = dcerpc_netr_ServerPasswordSet(b, mem_ctx,
						       ...
						       &result);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("dcerpc_netr_ServerPasswordSet failed: %s\n",
				nt_errstr(status)));
			return status;
		}
	}
	...
	if (!netlogon_creds_client_check(cli->dc, &srv_cred.cred)) {
		DEBUG_OUT (0,("credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("dcerpc_netr_ServerPasswordSet{2} failed: %s\n",
			nt_errstr(result)));
		return result;
	}
