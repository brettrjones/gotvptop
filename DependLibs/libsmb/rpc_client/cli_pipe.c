					...
					uint8_t *data, size_t size)
{
	...
	DEBUG_OUT (5, ("rpc_read_send: data_to_read: %u\n", (unsigned int)size));
					 ...
					 const uint8_t *data, size_t size)
{
	...
	DEBUG_OUT (5, ("rpc_write_send: data_to_write: %u\n", (unsigned int)size));
						...
						DATA_BLOB *reply_pdu)
{
	...
	*rdata = *pdu;
	...
	case DCERPC_PKT_RESPONSE:
		...
		DEBUG_OUT (10, ("Got pdu len %lu, data_len %lu, ss_len %u\n",
		...
		if ((reply_pdu->length == 0) &&
			...
			if (!data_blob_realloc(mem_ctx, reply_pdu,
							r->alloc_hint)) {
				DEBUG_OUT (0, ("reply alloc hint %d too "
					  "large to allocate\n",
					  (int)r->alloc_hint));
	...
	case DCERPC_PKT_BIND_NAK:
		DEBUG_OUT (1, (__location__ ": Bind NACK received from %s!\n",
	...
	case DCERPC_PKT_FAULT:
		...
		DEBUG_OUT (1, (__location__ ": RPC fault code %s received "
			  "from %s!\n",
	...
	default:
		DEBUG_OUT (0, (__location__ "Unknown packet type %u received "
			  "from %s!\n",
	...
	if (pkt->ptype != expected_pkt_type) {
		DEBUG_OUT (3, (__location__ ": Connection to %s got an unexpected "
	...
	if ((pkt->ptype == DCERPC_PKT_BIND_ACK) &&
		...
		DEBUG_OUT (5, (__location__ ": bug in server (AS/U?), setting "
...
static void rpc_api_pipe_trans_done(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("cli_api_pipe failed: %s\n", nt_errstr(status)));
		tevent_req_nterror(req, status);
		return;
	}

	if (rdata == NULL) {
		DEBUG_OUT (3,("rpc_api_pipe: %s failed to return data.\n",
...
static void rpc_api_pipe_got_pdu(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("get_complete_frag failed: %s\n",
			  nt_errstr(status)));
		tevent_req_nterror(req, status);
		return;
	}
	...
	if (state->incoming_frag.length != state->pkt->frag_length) {
		DEBUG_OUT (5, ("Incorrect pdu length %u, expected %u\n",
	...
	DEBUG_OUT (10,("rpc_api_pipe: got frag len of %u at offset %u: %s\n",
	...
	if ((state->pkt->pfc_flags & DCERPC_PFC_FLAG_FIRST)
		 ...
		 * first packet.
		 */
		DEBUG_OUT (10,("rpc_api_pipe: On %s PDU data format is "
			  "big-endian.\n",
			  rpccli_pipe_txt(talloc_tos(), state->cli)));
	...
	if (state->endianess != state->pkt->drep[0]) {
		DEBUG_OUT (0,("rpc_api_pipe: Error : Endianness changed from %s to "
			 "%s\n",
	...
	if (state->pkt->pfc_flags & DCERPC_PFC_FLAG_LAST) {
		...
		DEBUG_OUT (10,("rpc_api_pipe: %s returned %u bytes.\n",
			  rpccli_pipe_txt(talloc_tos(), state->cli),
					    ...
					    DATA_BLOB *auth_token)
{
	...
	DEBUG_OUT (5, ("Created GSS Authentication Token:\n"));
						 ...
						 DATA_BLOB *auth_token)
{
	...
	DEBUG_OUT (5, ("create_ntlmssp_auth_rpc_bind_req: Processing NTLMSSP Negotiate\n"));
	...
	DEBUG_OUT (5, ("create_ntlmssp_auth_rpc_bind_req: NTLMSSP Negotiate:\n"));
	dump_data(5, auth_token->data, auth_token->length);
						...
						DATA_BLOB *blob)
{
	...
	status = dcerpc_push_ncacn_packet(mem_ctx,
					  ...
					  blob);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to marshall bind/alter ncacn_packet.\n"));
		return status;
	}
				...
				const struct ndr_syntax_id *transfer)
{
	...
	if (r->secondary_address_size == 0) {
		DEBUG_OUT (4,("Ignoring length check -- ASU bug (server didn't fill in the pipe name correctly)"));
	}
	...
	if ((ctx.syntax.if_version != transfer->if_version) ||
		...
		DEBUG_OUT (2,("bind_rpc_pipe: transfer syntax differs\n"));
		return False;
	}

	if (r->num_results != 0x1 || ctx.result != 0) {
		DEBUG_OUT (2,("bind_rpc_pipe: bind denied results: %d reason: %x\n",
		          r->num_results, ctx.reason));
	}

	DEBUG_OUT (5,("check_bind_response: accepted!\n"));
	return True;
}
				...
				DATA_BLOB *rpc_out)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR_RB.\n"));
		return status;
	}
				      ...
				      struct pipe_auth_data *auth)
{
	...
	DEBUG_OUT (5,("Bind RPC Pipe: %s auth_type %u, auth_level %u\n",
		rpccli_pipe_txt(talloc_tos(), cli),
...
static void rpc_pipe_bind_step_one_done(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("rpc_pipe_bind: %s bind request returned %s\n",
			  rpccli_pipe_txt(talloc_tos(), state->cli),
			  nt_errstr(status)));
		tevent_req_nterror(req, status);
		return;
	...
	if (!check_bind_response(&pkt->u.bind_ack, &state->cli->transfer_syntax)) {
		DEBUG_OUT (2, ("rpc_pipe_bind: check_bind_response failed.\n"));
	...
	case DCERPC_AUTH_TYPE_KRB5:
		...
		status = dcerpc_pull_dcerpc_auth(talloc_tos(),
						 ...
						 &auth, false);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Failed to pull dcerpc auth: %s.\n",
				  nt_errstr(status)));
			tevent_req_nterror(req, status);
			return;
		}
...
err_out:
	DEBUG_OUT (0,("cli_finish_bind_auth: unknown auth type %u\n",
			       ...
			       struct rpc_pipe_client **presult)
{
	...
	if (sys_connect(fd, (struct sockaddr *)(void *)&addr) == -1) {
		DEBUG_OUT (0, ("connect(%s) failed: %s\n", socket_path,
			  strerror(errno)));
		close(fd);
					    ...
					    struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("rpccli_anon_bind_data returned %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (lvl, ("cli_rpc_pipe_open_noauth: rpc_pipe_bind for pipe "
	...
	DEBUG_OUT (10,("cli_rpc_pipe_open_noauth: opened pipe %s to machine "
				   ...
				   struct rpc_pipe_client **presult)
{
	...
	status = rpccli_ntlmssp_bind_data(result,
					  ...
					  &auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("rpccli_ntlmssp_bind_data returned %s\n",
			  nt_errstr(status)));
		goto err;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("cli_rpc_pipe_open_ntlmssp_internal: cli_rpc_pipe_bind failed with error %s\n",
			nt_errstr(status) ));
		goto err;
	}

	DEBUG_OUT (10,("cli_rpc_pipe_open_ntlmssp_internal: opened pipe %s to "
					     ...
					     struct rpc_pipe_client **presult)
{
	...
	status = rpccli_schannel_bind_data(result, domain, auth_level,
					   *pdc, &auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("rpccli_schannel_bind_data returned %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("cli_rpc_pipe_open_schannel_with_key: "
	...
	DEBUG_OUT (10,("cli_rpc_pipe_open_schannel_with_key: opened pipe %s to machine %s "
				...
				struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("gse_init_client returned %s\n",
			  nt_errstr(status)));
		goto err_out;
	}
	...
	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("cli_rpc_pipe_bind failed with error %s\n",
			  nt_errstr(status)));
		goto err_out;
	}
					...
					struct rpc_pipe_client **presult)
{
	...
	status = spnego_gssapi_init_client(auth,
					   ...
					   &spnego_ctx);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("spnego_init_client returned %s\n",
			  nt_errstr(status)));
		goto err_out;
	}
