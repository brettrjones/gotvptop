						...
						uint32 *pneg_flags)
{
	...
	if (!get_trust_pw_hash(domain, machine_pwd, &machine_account,
			       &sec_chan_type))
	{
		DEBUG_OUT (0, ("get_schannel_session_key: could not fetch "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("get_schannel_session_key_common: "
	...
	if (((*pneg_flags) & NETLOGON_NEG_SCHANNEL) == 0) {
		DEBUG_OUT (3, ("get_schannel_session_key: Server %s did not offer schannel\n",
			cli->desthost));
						 ...
						 struct rpc_pipe_client **presult)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("cli_rpc_pipe_open_ntlmssp_auth_schannel: failed to get schannel session "
				    ...
				    struct rpc_pipe_client **presult)
{
	...
	status = get_schannel_session_key(cli, domain, &neg_flags,
					  &netlogon_pipe);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("cli_rpc_pipe_open_schannel: failed to get schannel session "
			"key from server %s for domain %s.\n",
			cli->desthost, domain ));
		return status;
	}
