				    ...
				    WERROR *pwerr)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_winreg_int_openkey: Could not connect to winreg pipe: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	if (create_key) {
		...
		switch (action) {
			case REG_ACTION_NONE:
				DEBUG_OUT (8, ("dcerpc_winreg_int_openkey: createkey"
					  " did nothing -- huh?\n"));
				break;
				   ...
				   WERROR *pwerr)
{
	...
	if (strequal(hivename, "HKLM") ||
		...
		DEBUG_OUT (10,("dcerpc_winreg_int_openkey: unrecognised hive key %s\n",
			  key));
