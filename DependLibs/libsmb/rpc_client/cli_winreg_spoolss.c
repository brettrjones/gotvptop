			      ...
			      struct policy_handle *key_handle)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_openkey: Could not open HKLM hive: %s\n",
			  nt_errstr(status)));
	...
	if (create_key) {
		...
		switch (action) {
			case REG_ACTION_NONE:
				DEBUG_OUT (8, ("winreg_printer_openkey:createkey did nothing -- huh?\n"));
				break;
			case REG_CREATED_NEW_KEY:
				DEBUG_OUT (8, ("winreg_printer_openkey: createkey created %s\n", keyname));
				break;
			case REG_OPENED_EXISTING_KEY:
				DEBUG_OUT (8, ("winreg_printer_openkey: createkey opened existing %s\n", keyname));
				break;
		}
	} else {
					...
					struct spoolss_PrinterEnumValues **penum_values)
{
	...
	status = dcerpc_winreg_QueryInfoKey(winreg_handle,
					    ...
					    &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_enumvalues: Could not query info: %s\n",
			  nt_errstr(status)));
	...
	for (i = 0; i < num_values; i++) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("winreg_printer_enumvalues: Could not enumerate values: %s\n",
				  nt_errstr(status)));
			result = ntstatus_to_werror(status);
			goto error;
		}
					    ...
					    const char *key)
{
	...
	DEBUG_OUT (2, ("winreg_printer_delete_subkeys: delete key %s\n", key));
	/* open the key */
	status = dcerpc_winreg_OpenKey(winreg_handle,
				       ...
				       &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_delete_subkeys: Could not open key %s: %s\n",
	...
	for (i = 0; i < num_subkeys; i++) {
		...
		DEBUG_OUT (2, ("winreg_printer_delete_subkeys: delete subkey %s\n", subkey));
					...
					NTTIME data)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_write_date: Could not set value %s: %s\n",
			     ...
			     const char *sharename)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_create_printer: Skipping, %s already exists\n", path));
		goto done;
	} else if (W_ERROR_EQUAL(result, WERR_BADFILE)) {
		DEBUG_OUT (2, ("winreg_create_printer: Creating default values in %s\n", path));
	} else if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_create_printer: Could not open key %s: %s\n",
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_create_printer_keys: Could not create key %s: %s\n",
			path, win_errstr(result)));
		goto done;
	}
			     ...
			     struct security_descriptor *secdesc)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_update_printer: Could not open key %s: %s\n",
			path, win_errstr(result)));
		goto done;
	}
	...
	if (info2_mask & SPOOLSS_PRINTER_INFO_DEVMODE) {
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			DEBUG_OUT (0, ("winreg_update_printer: Failed to marshall device mode\n"));
			result = WERR_NOMEM;
			goto done;
		}
			  ...
			  struct spoolss_PrinterInfo2 **pinfo2)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_get_printer: Could not open key %s: %s\n",
			  path, win_errstr(result)));
		goto done;
	}
	...
	result = winreg_printer_enumvalues(tmp_ctx,
					   ...
					   &enum_values);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_get_printer: Could not enumerate values in %s: %s\n",
			  path, win_errstr(result)));
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_get_printer: winreg_enumval_to_TYPE() failed "
					"for %s: %s\n",
					v->value_name,
					win_errstr(result)));
		goto done;
	...
	if (W_ERROR_IS_OK(result)) {
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			DEBUG_OUT (0, ("winreg_get_printer: Failed to unmarshall device mode\n"));
			result = WERR_NOMEM;
			goto done;
		}
	}
				 ...
				 uint32_t data_size)
{
	...
	DEBUG_OUT (8, ("winreg_set_printer_dataex: Open printer key %s, value %s, access_mask: 0x%05x for [%s]\n",
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_set_printer_dataex: Could not open key %s: %s\n",
	...
	status = dcerpc_winreg_SetValue(winreg_handle,
					...
					&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_set_printer_dataex: Could not set value %s: %s\n",
				 ...
				 uint32_t *data_size)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_get_printer_dataex: Could not open key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_QueryValue(winreg_handle,
					  ...
					  &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_get_printer_dataex: Could not query value %s: %s\n",
			  value, nt_errstr(status)));
		result = ntstatus_to_werror(status);
		goto done;
	}
				  ...
				  struct spoolss_PrinterEnumValues **penum_values)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_enum_printer_dataex: Could not open key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
	...
	result = winreg_printer_enumvalues(tmp_ctx,
					   ...
					   &enum_values);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_enum_printer_dataex: Could not enumerate values in %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
				    ...
				    const char *value)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_delete_printer_dataex: Could not open key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_DeleteValue(winreg_handle,
					   ...
					   &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_delete_printer_dataex: Could not delete value %s: %s\n",
			  value, nt_errstr(status)));
		result = ntstatus_to_werror(status);
	}
			       ...
			       const char ***psubkeys)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_enum_printer_key: Could not open key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_enum_printer_key: Could not enumerate subkeys in %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
				 ...
				 const char *key)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		...
		DEBUG_OUT (0, ("winreg_delete_printer_key: Could not open key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
	...
	result = winreg_printer_delete_subkeys(tmp_ctx,
					       ...
					       keyname);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_delete_printer_key: Could not delete key %s: %s\n",
			  key, win_errstr(result)));
		goto done;
	}
				      ...
				      const char *printer)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_update_changeid: Could not open key %s: %s\n",
			  path, win_errstr(result)));
		goto done;
	}
				   ...
				   uint32_t *pchangeid)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_printer_get_changeid: Could not open key %s: %s\n",
			  path, win_errstr(result)));
		goto done;
	}

	DEBUG_OUT (10, ("winreg_printer_get_changeid: get changeid from %s\n", path));
			       ...
			       struct spoolss_AddFormInfo1 *form)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_addform1: Could not open key %s: %s\n",
	...
	result = winreg_printer_enumforms1(tmp_ctx, winreg_handle,
					   &num_info, &info);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_addform: Could not enum keys %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_SetValue(winreg_handle,
					...
					&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_addform1: Could not set value %s: %s\n",
				 ...
				 union spoolss_FormInfo **pinfo)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		...
		DEBUG_OUT (0, ("winreg_printer_enumforms1: Could not open key %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		goto done;
	}
	...
	result = winreg_printer_enumvalues(tmp_ctx,
					   ...
					   &enum_values);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_enumforms1: Could not enumerate values in %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		goto done;
	}
				  ...
				  const char *form_name)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_deleteform1: Could not open key %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		if (W_ERROR_EQUAL(result, WERR_BADFILE)) {
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (0, ("winreg_printer_delteform1: Could not delete value %s: %s\n",
			  wvalue.name, nt_errstr(status)));
		result = ntstatus_to_werror(status);
		goto done;
	}
			       ...
			       struct spoolss_AddFormInfo1 *form)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_printer_setform1: Could not open key %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_SetValue(winreg_handle,
					...
					&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_setform1: Could not set value %s: %s\n",
			  wvalue.name, nt_errstr(status)));
		result = ntstatus_to_werror(status);
	}
			       ...
			       struct spoolss_FormInfo1 *r)
{
	...
	result = winreg_printer_openkey(tmp_ctx,
					...
					&key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("winreg_printer_getform1: Could not open key %s: %s\n",
			  TOP_LEVEL_CONTROL_FORMS_KEY, win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_QueryValue(winreg_handle,
					  ...
					  &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("winreg_printer_getform1: Could not query value %s: %s\n",
			  wvalue.name, nt_errstr(status)));
		result = ntstatus_to_werror(status);
		goto done;
	}
			 ...
			 struct spoolss_DriverInfo8 **_info8)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_enumval_to_TYPE() failed "
			  "for %s: %s\n", v->value_name,
			  win_errstr(result)));
		goto done;
	}
			      ...
			      const char ***drivers_p)
{
	...
	*drivers_p = NULL;
	...
	result = winreg_printer_opendriver(tmp_ctx,
					   ...
					   &key_hnd);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (5, ("winreg_get_driver_list: "
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("winreg_get_driver_list: "
