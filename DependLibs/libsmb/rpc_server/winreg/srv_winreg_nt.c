					       ...
					       struct policy_handle *hnd)
{
	...
	if(!find_policy_by_hnd(p,hnd,(void **)(void *)&regkey)) {
		DEBUG_OUT (2,("find_regkey_index_by_hnd: Registry Key not found: "));
		return NULL;
	}
			       ...
			       struct policy_handle *hnd)
{
	...
	if ( !regkey ) {
		DEBUG_OUT (2,("close_registry_key: Invalid handle (%s:%u:%u)\n",
			 OUR_HANDLE(hnd)));
		return False;
	}
			  ...
			  struct winreg_QueryValue *r)
{
	...
	DEBUG_OUT (7,("_winreg_QueryValue: policy key name = [%s]\n", regkey->key->name));
	DEBUG_OUT (7,("_winreg_QueryValue: policy key type = [%08x]\n", regkey->key->type));
	...
	if(regkey->key->type == REG_KEY_HKPD)
	{
		if (strequal(r->in.value_name->name, "Global"))	{
			...
			DEBUG_OUT (3,("Unsupported key name [%s] for HKPD.\n",
				 r->in.value_name->name));
			return WERR_BADFILE;
		}
		...
		*r->out.type = REG_BINARY;
		...
		if (!W_ERROR_IS_OK(status)) {
			...
			DEBUG_OUT (10,("_winreg_QueryValue: reg_queryvalue failed with: %s\n",
				win_errstr(status)));
		       ...
		       struct winreg_EnumKey *r)
{
	...
	DEBUG_OUT (8,("_winreg_EnumKey: enumerating key [%s]\n", key->key->name));
			 ...
			 struct winreg_EnumValue *r)
{
	...
	DEBUG_OUT (8,("_winreg_EnumValue: enumerating values for key [%s]\n",
		 key->key->name));
					...
					struct winreg_InitiateSystemShutdownEx *r)
{
	...
	DEBUG_OUT (3,("_reg_shutdown_ex: Running the command `%s' gave %d\n",
		shutdown_script, ret));
				   ...
				   struct winreg_AbortSystemShutdown *r)
{
	...
	DEBUG_OUT (3,("_winreg_AbortSystemShutdown: Running the command `%s' gave %d\n",
			  ...
			  struct winreg_RestoreKey *r)
{
	...
	DEBUG_OUT (8,("_winreg_RestoreKey: verifying restore of key [%s] from "
	...
	DEBUG_OUT (2,("_winreg_RestoreKey: Restoring [%s] from %s in share %s\n",
		       ...
		       struct winreg_SaveKey *r)
{
	...
	DEBUG_OUT (8,("_winreg_SaveKey: verifying backup of key [%s] to \"%s\"\n",
		 regkey->key->name, fname));

	if ((snum = validate_reg_filename(p->mem_ctx, &fname)) == -1 )
		return WERR_OBJECT_PATH_INVALID;

	DEBUG_OUT (2,("_winreg_SaveKey: Saving [%s] to %s in share %s\n",
		 regkey->key->name, fname, lp_servicename(snum) ));
			 ...
			 struct winreg_CreateKey *r)
{
	...
	DEBUG_OUT (10, ("_winreg_CreateKey called with parent key '%s' and "
			...
			struct winreg_SetValue *r)
{
	...
	DEBUG_OUT (8,("_winreg_SetValue: Setting value for [%s:%s]\n",
