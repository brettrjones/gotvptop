	...
	for (d = endpoint_list; d != NULL; d = d->next) {
		...
		for (iface = d->iface_list; iface != NULL; iface = iface->next) {
			...
			if (NT_STATUS_IS_ERR(status)) {
				DEBUG_OUT (1, ("Unable to build tower for %s\n",
					  iface->iface->name));
				continue;
			}
			total++;
...
bool srv_epmapper_delete_endpoints(struct pipes_struct *p)
{
	...
	while (el) {
		...
		DEBUG_OUT (10, ("Delete_endpoints for: %s\n",
			   ...
			   struct epm_Insert *r)
{
	...
	DEBUG_OUT (3, ("_epm_Insert: Trying to add %u new entries.\n",
		  r->in.num_ents));
	...
	for (i = 0; i < r->in.num_ents; i++) {
		...
		DEBUG_OUT (3, ("_epm_Insert: Adding transport %s for %s\n",
		...
		if (find_interface(ep, iface) != NULL) {
			DEBUG_OUT (0, ("dcesrv_interface_register: interface '%s' "
			   ...
			   struct epm_Delete *r)
{
	...
	DEBUG_OUT (3, ("_epm_Delete: Trying to delete %u entries.\n",
		  r->in.num_ents));
	...
	for (i = 0; i < r->in.num_ents; i++) {
		...
		DEBUG_OUT (3, ("_epm_Delete: Deleting transport '%s' for '%s'\n",
			  derpc_transport_string_by_transport(b->transport),
			  r->in.entries[i].annotation));
		...
		if (iflist == NULL) {
			DEBUG_OUT (0, ("_epm_Delete: No interfaces left, delete endpoint\n"));
			DLIST_REMOVE(endpoint_table, ep);
			talloc_free(ep);
			   ...
			   struct epm_Lookup *r)
{
	...
	*r->out.num_ents = 0;
	...
	DEBUG_OUT (3, ("_epm_Lookup: Trying to lookup max. %u entries.\n",
		  r->in.max_ents));
	...
	if (r->in.entry_handle == NULL ||
		...
		DEBUG_OUT (5, ("_epm_Lookup: No entry_handle found, creating it.\n"));
		...
		DEBUG_OUT (5, ("_epm_Lookup: Trying to find entry_handle.\n"));
	...
	DEBUG_OUT (3, ("_epm_Lookup: Find %u entries\n", count));
	...
	for (i = 0; i < count; i++) {
		...
		if (match) {
			...
			DEBUG_OUT (10, ("_epm_Lookup: Adding tower for '%s'\n",
				   eps->e[i].name));
			...
			struct epm_Map *r)
{
	...
	*r->out.num_towers = 0;
	...
	DEBUG_OUT (3, ("_epm_Map: Trying to map max. %u towers.\n",
		  r->in.max_towers));
	...
	if (transport == NCA_UNKNOWN) {
		DEBUG_OUT (2, ("epm_Map: Client requested unknown transport with"
			  "levels: "));
		for (i = 2; i < r->in.map_tower->tower.num_floors; i++) {
			DEBUG_OUT (2, ("%d, ", r->in.map_tower->tower.floors[i].lhs.protocol));
		}
		DEBUG_OUT (2, ("\n"));
		rc = EPMAPPER_STATUS_NO_MORE_ENTRIES;
		goto done;
	...
	if (r->in.entry_handle == NULL ||
		...
		DEBUG_OUT (5, ("_epm_Map: No entry_handle found, creating it.\n"));
		...
		DEBUG_OUT (5, ("_epm_Map: Trying to find entry_handle.\n"));
	...
	for (i = 0; i < count; i++) {
		DEBUG_OUT (5, ("_epm_Map: Map tower for '%s'\n",
			   eps->e[i].name));
