			    ...
			    DATA_BLOB *data, size_t start, size_t end)
{
	...
	if (fd != -1) {
		...
		i = close(fd);
		if ((sz != end - start) || (i != 0) ) {
			DEBUG_OUT (0, ("Error writing/closing %s: %ld!=%ld %d\n",
				  fname, (unsigned long)sz,
				  (unsigned long)end - start, i));
		} else {
			DEBUG_OUT (0,("created %s\n", fname));
		}
	}
	TALLOC_FREE(fname);
}
				   ...
				   size_t *pdu_size)
{
	...
	if (!data_left) {
		DEBUG_OUT (0, ("No data left to send !\n"));
	...
	status = dcerpc_push_ncacn_packet(mem_ctx, DCERPC_PKT_RESPONSE,
					  ...
					  &u, frag);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to marshall RPC Packet.\n"));
		return status;
	}
...
bool create_next_pdu(struct pipes_struct *p)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to create packet with error %s, "
			  "(auth level %u / type %u)\n",
			  nt_errstr(status),
			   ...
			   uint32 context_id)
{
	...
	if (context_fns == NULL) {
		DEBUG_OUT (0,("check_bind_req: malloc() failed!\n"));
		return False;
	}
...
bool is_known_pipename(const char *cli_filename, struct ndr_syntax_id *syntax)
{
	...
	if (lp_disable_spoolss() && strequal(pipename, "spoolss")) {
		DEBUG_OUT (10, ("refusing spoolss access\n"));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("is_known_pipename: %s unknown\n", cli_filename));
		return false;
	}
	DEBUG_OUT (10, ("is_known_pipename: %s loaded dynamically\n", pipename));
	...
	DEBUG_OUT (10, ("is_known_pipename: pipe %s did not register itself!\n",
		   pipename));
				  ...
				  DATA_BLOB *response)
{
	...
	status = spnego_server_auth_start(p,
					  ...
					  &spnego_ctx);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed SPNEGO negotiate (%s)\n",
			  nt_errstr(status)));
		return false;
	}
	...
	DEBUG_OUT (10, ("SPNEGO auth started\n"));
				    ...
				    DATA_BLOB *response)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0,("pipe_schannel_auth_bind: Could not unmarshal SCHANNEL auth neg\n"));
		return false;
	}
	...
	if (!(neg.Flags & NL_FLAG_OEM_NETBIOS_COMPUTER_NAME)) {
		DEBUG_OUT (0,("pipe_schannel_auth_bind: Did not receive netbios computer name\n"));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("pipe_schannel_auth_bind: Attempt to bind using schannel without successful serverauth2\n"));
		return False;
	}
	...
	if (!ret) {
		DEBUG_OUT (0, ("session_info_set_session_key failed\n"));
		return false;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0,("Failed to marshall NL_AUTH_MESSAGE.\n"));
		return false;
	}
	...
	DEBUG_OUT (10,("pipe_schannel_auth_bind: schannel auth: domain [%s] myname [%s]\n",
				   ...
				   DATA_BLOB *response)
{
	...
	if (strncmp((char *)auth_info->credentials.data, "NTLMSSP", 7) != 0) {
		DEBUG_OUT (0, ("Failed to read NTLMSSP in blob\n"));
                return false;
        }
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_OK)) {
		DEBUG_OUT (0, (__location__ ": auth_ntlmssp_start failed: %s\n",
			  nt_errstr(status)));
		return false;
	}
	...
	DEBUG_OUT (10, (__location__ ": NTLMSSP auth started\n"));
				...
				struct auth_serversupplied_info **session_info)
{
	...
	DEBUG_OUT (5, (__location__ ": pipe %s checking user details\n",
	...
	status = ntlmssp_server_check_flags(ntlmssp_ctx,
						...
						DCERPC_AUTH_LEVEL_PRIVACY));
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, (__location__ ": Client failed to negotatie proper "
			  "security for pipe %s\n",
			  get_pipe_name_from_syntax(talloc_tos(), syntax)));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, (__location__ ": failed to obtain the server info "
			  "for authenticated user: %s\n", nt_errstr(status)));
		return false;
	}

	if ((*session_info)->security_token == NULL) {
		DEBUG_OUT (1, ("Auth module failed to provide nt_user_token\n"));
		return false;
	}
	...
	if (!ret) {
		DEBUG_OUT (0, ("Failed to set session key!\n"));
		return false;
	}
				  ...
				  DATA_BLOB *response)
{
	...
	status = gssapi_server_auth_start(p,
					  ...
					  &gse_ctx);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to init dcerpc gssapi server (%s)\n",
			  nt_errstr(status)));
		goto err;
	}
	...
	DEBUG_OUT (10, ("KRB5 auth started\n"));
					 ...
					 struct auth_serversupplied_info **session_info)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Requested Security Layers not honored!\n"));
		return status;
	}
	...
	if ((*session_info)->security_token == NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("Failed to create local user token (%s)\n",
				  nt_errstr(status)));
...
static NTSTATUS pipe_auth_verify_final(struct pipes_struct *p)
{
	...
	case DCERPC_AUTH_TYPE_KRB5:
		...
		status = pipe_gssapi_verify_final(p, gse_ctx,
						  ...
						  &p->session_info);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("gssapi bind failed with: %s",
				  nt_errstr(status)));
			return status;
		}
		break;
	case DCERPC_AUTH_TYPE_SPNEGO:
		...
		status = spnego_get_negotiated_mech(spnego_ctx,
						    &auth_type, &mech_ctx);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Bad SPNEGO state (%s)\n",
				  nt_errstr(status)));
			return status;
		}
		switch(auth_type) {
		...
		case SPNEGO_NTLMSSP:
			...
			break;
		default:
			DEBUG_OUT (0, (__location__ ": incorrect spnego type "
				  "(%d).\n", auth_type));
			return NT_STATUS_ACCESS_DENIED;
		}
		break;
	default:
		DEBUG_OUT (0, (__location__ ": incorrect auth type (%u).\n",
			  (unsigned int)p->auth.auth_type));
		return NT_STATUS_ACCESS_DENIED;
	}
				...
				struct ncacn_packet *pkt)
{
	...
	if (p->pipe_bound) {
		DEBUG_OUT (2,("api_pipe_bind_req: rejecting bind request on bound "
			 "pipe %s.\n",
			 get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));
	...
	if (pkt->u.bind.num_contexts == 0) {
		DEBUG_OUT (0, ("api_pipe_bind_req: no rpc contexts around\n"));
		goto err_exit;
	}
	...
	if (rpc_srv_pipe_exists_by_id(&id)) {
		DEBUG_OUT (3, ("api_pipe_bind_req: \\PIPE\\%s -> \\PIPE\\%s\n",
		...
		if (NT_STATUS_IS_ERR(status)) {
                       DEBUG_OUT (3,("api_pipe_bind_req: Unknown pipe name %s in bind request.\n",
		...
		if (rpc_srv_get_pipe_interface_by_cli_name(
			...
			DEBUG_OUT (0, ("module %s doesn't provide functions for "
				  "pipe %s!\n",
				  get_pipe_name_from_syntax(talloc_tos(),
							    &p->syntax),
				  get_pipe_name_from_syntax(talloc_tos(),
	...
	DEBUG_OUT (5,("api_pipe_bind_req: make response. %d\n", __LINE__));
	...
	if (pkt->auth_length) {
		...
		if (pkt->frag_length < RPC_HEADER_LEN +
					DCERPC_AUTH_TRAILER_LENGTH +
					pkt->auth_length) {
			DEBUG_OUT (0,("api_pipe_bind_req: auth_len (%u) "
		...
		status = dcerpc_pull_dcerpc_auth(pkt,
						 ...
						 &auth_info, p->endian);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Unable to unmarshall dcerpc_auth.\n"));
			goto err_exit;
		}
		...
		case DCERPC_AUTH_LEVEL_CONNECT:
			...
			break;
		default:
			DEBUG_OUT (0, ("Unexpected auth level (%u).\n",
		...
		default:
			DEBUG_OUT (0, ("Unknown auth type %x requested.\n", auth_type));
			goto err_exit;
		}
	}
	...
	status = dcerpc_push_ncacn_packet(p->mem_ctx,
					  ...
					  &p->out_data.frag);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to marshall bind_ack packet. (%s)\n",
			  nt_errstr(status)));
	}
	...
	if (auth_resp.length) {
		...
		status = dcerpc_push_dcerpc_auth(pkt,
						 ...
						 &auth_blob);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Marshalling of dcerpc_auth failed.\n"));
			goto err_exit;
		}
	}
	...
	if (auth_blob.length) {
		...
		if (!data_blob_append(p->mem_ctx, &p->out_data.frag,
			...
			DEBUG_OUT (0, ("Append of auth info failed.\n"));
			goto err_exit;
		}
	}
...
bool api_pipe_bind_auth3(struct pipes_struct *p, struct ncacn_packet *pkt)
{
	...
	DEBUG_OUT (5, ("api_pipe_bind_auth3: decode request. %d\n", __LINE__));

	if (pkt->auth_length == 0) {
		DEBUG_OUT (0, ("No auth field sent for bind request!\n"));
		goto err;
	}
	...
	if (pkt->frag_length < RPC_HEADER_LEN
				+ DCERPC_AUTH_TRAILER_LENGTH
				+ pkt->auth_length) {
			DEBUG_OUT (0,("api_pipe_ntlmssp_auth_process: auth_len "
				"%u is too large.\n",
                        (unsigned int)pkt->auth_length));
		goto err;
	}
	...
	status = dcerpc_pull_dcerpc_auth(pkt,
					 ...
					 &auth_info, p->endian);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to unmarshall dcerpc_auth.\n"));
		goto err;
	}
	...
	if (auth_info.auth_type != p->auth.auth_type) {
		DEBUG_OUT (0, ("Auth type mismatch! Client sent %d, "
	...
	if (NT_STATUS_EQUAL(status,
	    ...
	    response.length) {
		DEBUG_OUT (0, (__location__ ": This was supposed to be the final "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Auth Verify failed (%s)\n", nt_errstr(status)));
		goto err;
	}
					...
					struct ncacn_packet *pkt)
{
	...
	DEBUG_OUT (5,("api_pipe_alter_context: make response. %d\n", __LINE__));
	...
	if (pkt->auth_length) {
		...
		if (pkt->frag_length < RPC_HEADER_LEN +
					DCERPC_AUTH_TRAILER_LENGTH +
					pkt->auth_length) {
			DEBUG_OUT (0,("api_pipe_alter_context: auth_len (%u) "
				"too long for fragment %u.\n",
				(unsigned int)pkt->auth_length,
				(unsigned int)pkt->frag_length ));
			goto err_exit;
		...
		if (p->pipe_bound) {
			DEBUG_OUT (0, (__location__ ": Pipe already bound, "
		...
		default:
			DEBUG_OUT (3, (__location__ ": Usupported auth type (%d) "
		...
		if (NT_STATUS_IS_OK(status)) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				...
				goto err_exit;
					...
					NT_STATUS_MORE_PROCESSING_REQUIRED)) {
			DEBUG_OUT (10, ("More auth legs required.\n"));
		} else {
			DEBUG_OUT (0, ("Auth step returned an error (%s)\n",
				  nt_errstr(status)));
			goto err_exit;
		}
	}
	...
	if (auth_resp.length) {
		if (pad_len) {
			...
			if (!data_blob_append(p->mem_ctx,
						&p->out_data.frag,
						pad, pad_len)) {
				DEBUG_OUT (0, ("api_pipe_bind_req: failed to add "
					  "%u bytes of pad data.\n",
					  (unsigned int)pad_len));
				goto err_exit;
			}
...
static PIPE_RPC_FNS* find_pipe_fns_by_context( PIPE_RPC_FNS *list, uint32 context_id )
{
	...
	if ( !list ) {
		DEBUG_OUT (0,("find_pipe_fns_by_context: ERROR!  No context list for pipe!\n"));
		return NULL;
	}
				...
				struct ncacn_packet *pkt)
{
	...
	if ( pipe_fns ) {
		...
		DEBUG_OUT (0, ("No rpc function table associated with context "
			  "[%d] on pipe [%s]\n",
			  pkt->u.request.context_id,
			  get_pipe_name_from_syntax(talloc_tos(),
						    &p->syntax)));
		       ...
		       const struct api_struct *api_rpc_cmds, int n_cmds)
{
	...
	DEBUG_OUT (4,("api_rpcTNP: %s op 0x%x - ",
		 get_pipe_name_from_syntax(talloc_tos(), &p->syntax),
		 pkt->u.request.opnum));
	...
	for (fn_num = 0; fn_num < n_cmds; fn_num++) {
		if (api_rpc_cmds[fn_num].opnum == pkt->u.request.opnum &&
			...
			DEBUG_OUT (3, ("api_rpcTNP: rpc command: %s\n",
	...
	offset1 = p->out_data.rdata.length;
        ...
        DEBUG_OUT (6, ("api_rpc_cmds[%d].fn == %p\n", 
                fn_num, api_rpc_cmds[fn_num].fn));
	/* do the actual command */
	if(!api_rpc_cmds[fn_num].fn(p)) {
		DEBUG_OUT (0,("api_rpcTNP: %s: %s failed.\n",
			 get_pipe_name_from_syntax(talloc_tos(), &p->syntax),
			 api_rpc_cmds[fn_num].name));
		data_blob_free(&p->out_data.rdata);
		return False;
	...
	if (p->bad_handle_fault_state) {
		DEBUG_OUT (4,("api_rpcTNP: bad handle fault return.\n"));
	...
	if (p->rng_fault_state) {
		DEBUG_OUT (4, ("api_rpcTNP: rng fault return\n"));
	...
	DEBUG_OUT (5,("api_rpcTNP: called %s successfully\n",
		 get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));

	/* Check for buffer underflow in rpc parsing */
	if ((DEBUGLEVEL >= 10) &&
	    (pkt->frag_length < p->in_data.data.length)) {
		DEBUG_OUT (10, ("api_rpcTNP: rpc input buffer underflow (parse error?)\n"));
...
void set_incoming_fault(struct pipes_struct *p)
{
	...
	p->fault_state = True;
	DEBUG_OUT (10, ("set_incoming_fault: Setting fault state on pipe %s\n",
		   get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));
}
				    ...
				    DATA_BLOB *raw_pkt)
{
	...
	DEBUG_OUT (10, ("Checking request auth.\n"));
...
static bool process_request_pdu(struct pipes_struct *p, struct ncacn_packet *pkt)
{
	...
	if (!p->pipe_bound) {
		DEBUG_OUT (0,("process_request_pdu: rpc request with no bind.\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to check packet auth. (%s)\n",
			  nt_errstr(status)));
		set_incoming_fault(p);
		return false;
	}
	...
	if (p->in_data.data.length + data.length > MAX_RPC_DATA_SIZE) {
		DEBUG_OUT (0, ("process_request_pdu: "
	...
	if (data.length) {
		if (!data_blob_append(p->mem_ctx, &p->in_data.data,
			...
			DEBUG_OUT (0, ("Unable to append data size %u "
...
void process_complete_pdu(struct pipes_struct *p)
{
	...
	if(p->fault_state) {
		DEBUG_OUT (10,("process_complete_pdu: pipe %s in fault state.\n",
			  get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));
		goto done;
	}
	...
	DEBUG_OUT (10, ("PDU is in %s Endian format!\n", p->endian?"Big":"Little"));

	status = dcerpc_pull_ncacn_packet(pkt, &p->in_data.pdu,
					  pkt, p->endian);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to unmarshal rpc packet: %s!\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	DEBUG_OUT (10, ("Processing packet type %d\n", (int)pkt->ptype));
	...
	case DCERPC_PKT_PING: /* CL request - ignore... */
		DEBUG_OUT (0, ("process_complete_pdu: Error. "
	...
	case DCERPC_PKT_CO_CANCEL:
		 ...
		 * processing. */
		DEBUG_OUT (3,("process_complete_pdu: DCERPC_PKT_CO_CANCEL."
			 " Abandoning rpc call.\n"));
	...
	case DCERPC_PKT_ORPHANED:
		 ...
		 * For now just free all client data and continue
		 * processing. */
		DEBUG_OUT (3, ("process_complete_pdu: DCERPC_PKT_ORPHANED."
			  " Abandoning rpc call.\n"));
		reply = True;
		break;

	default:
		DEBUG_OUT (0, ("process_complete_pdu: "
	...
	if (!reply) {
		DEBUG_OUT (3,("process_complete_pdu: DCE/RPC fault sent on "
			 "pipe %s\n", get_pipe_name_from_syntax(talloc_tos(),
								&p->syntax)));
		set_incoming_fault(p);
		setup_fault_pdu(p, NT_STATUS(DCERPC_FAULT_OP_RNG_ERROR));
