...
static ssize_t fill_rpc_header(struct pipes_struct *p, char *data, size_t data_to_copy)
{
	...
	DEBUG_OUT (10, ("fill_rpc_header: data_to_copy = %u, "
...
static bool get_pdu_size(struct pipes_struct *p)
{
	...
	if (p->in_data.pdu.length != RPC_HEADER_LEN) {
		DEBUG_OUT (0, ("Unexpected RPC Header size! "
			  "got %d, expected %d)\n",
	...
	if ((frag_len < RPC_HEADER_LEN) ||
		...
		DEBUG_OUT (0, ("Unexpected RPC Fragment size! (%d)\n",
			  frag_len));
		set_incoming_fault(p);
		return false;
	}
	...
	p->in_data.pdu.data = talloc_realloc(p, p->in_data.pdu.data,
						uint8_t, frag_len);
	if (p->in_data.pdu.data == NULL) {
		DEBUG_OUT (0, ("talloc_realloc failed\n"));
		set_incoming_fault(p);
		return false;
	}
...
ssize_t process_incoming_data(struct pipes_struct *p, char *data, size_t n)
{
	...
	DEBUG_OUT (10, ("process_incoming_data: Start: pdu.length = %u, "
	...
	if(data_to_copy == 0) {
		 ...
		 * fault state.
		 */
		DEBUG_OUT (0, ("process_incoming_data: "
	...
	DEBUG_OUT (10, ("process_incoming_data: not a complete PDU yet. "
...
static ssize_t write_to_internal_pipe(struct pipes_struct *p, char *data, size_t n)
{
	...
	while(data_left) {
		...
		DEBUG_OUT (10, ("write_to_pipe: data_left = %u\n",
			  (unsigned int)data_left));

		data_used = process_incoming_data(p, data, data_left);

		DEBUG_OUT (10, ("write_to_pipe: data_used = %d\n",
			   (int)data_used));
				       ...
				       size_t n, bool *is_data_outstanding)
{
	...
	if (!p) {
		DEBUG_OUT (0,("read_from_pipe: pipe not open\n"));
		return -1;
	}

	DEBUG_OUT (6,(" name: %s len: %u\n",
	...
	if(n > RPC_MAX_PDU_FRAG_LEN) {
                DEBUG_OUT (5,("read_from_pipe: too large read (%u) requested on "
	...
	if (pdu_remaining > 0) {
		...
		DEBUG_OUT (10,("read_from_pipe: %s: current_pdu_len = %u, "
	...
	DEBUG_OUT (10,("read_from_pipe: %s: fault_state = %d : data_sent_length "
	...
	if(!create_next_pdu(p)) {
		DEBUG_OUT (0,("read_from_pipe: %s: create_next_pdu failed.\n",
			 get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));
		return -1;
	}
				 ...
				 const uint8_t *data, size_t len)
{
	...
	DEBUG_OUT (6, ("np_write_send: len: %d\n", (int)len));
		      ...
		      bool *is_data_outstanding)
{
	...
	DEBUG_OUT (10, ("Received %d bytes. There is %smore data outstanding\n",
