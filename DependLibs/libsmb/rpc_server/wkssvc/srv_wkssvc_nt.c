			       ...
			       struct wkssvc_NetWkstaGetInfo *r)
{
	...
	case 101:
		...
		if (!nt_token_check_sid(&global_sid_Authenticated_Users,
			...
			DEBUG_OUT (1,("User not allowed for NetWkstaGetInfo level "
				 "101\n"));
	...
	TALLOC_CTX *mem_ctx)
{
	...
	if (users == NULL && errno != 0) {
		DEBUG_OUT (1,("get_logged_on_userlist error %d: %s\n",
				 ...
				 struct wkssvc_NetWkstaEnumUsers *r)
{
	...
	if (!nt_token_check_sid(&global_sid_Builtin_Administrators,
				p->session_info->security_token)) {
		DEBUG_OUT (1,("User not allowed for NetWkstaEnumUsers\n"));
			       ...
			       struct wkssvc_NetrJoinDomain2 *r)
{
	...
	if (!security_token_has_privilege(token, SEC_PRIV_MACHINE_ACCOUNT) &&
		...
		DEBUG_OUT (5,("_wkssvc_NetrJoinDomain2: account doesn't have "
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (5,("_wkssvc_NetrJoinDomain2: libnet_Join failed with: %s\n",
				 ...
				 struct wkssvc_NetrUnjoinDomain2 *r)
{
	...
	if (!security_token_has_privilege(token, SEC_PRIV_MACHINE_ACCOUNT) &&
	    !nt_token_check_domain_rid(token, DOMAIN_RID_ADMINS) &&
	    !nt_token_check_sid(&global_sid_Builtin_Administrators, token)) {
		DEBUG_OUT (5,("_wkssvc_NetrUnjoinDomain2: account doesn't have "
			"sufficient privileges\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (5,("_wkssvc_NetrUnjoinDomain2: libnet_Unjoin failed with: %s\n",
