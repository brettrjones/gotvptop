					    ...
					    struct auth_session_info_transport **session_info)
{
	...
	status = make_user_info_dc_netlogon_validation(mem_ctx,
						       ...
						       &u);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("conversion of info3 into user_info_dc failed!\n"));
		return status;
	}
				    ...
				    int *perrno)
{
	...
	if (!ok) {
		DEBUG_OUT (1, ("Failed to init handles\n"));
		TALLOC_FREE(p);
		*perrno = EINVAL;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to convert auth_user_info_dc into netr_SamInfo3\n"));
		TALLOC_FREE(p);
		*perrno = EINVAL;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to init server info\n"));
		TALLOC_FREE(p);
		*perrno = EINVAL;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to init local auth token\n"));
		TALLOC_FREE(p);
		*perrno = EINVAL;
		return -1;
	}
			     ...
			     struct tevent_context *ev_ctx)
{
	...
	if (!directory_create_or_exist(lp_ncalrpc_dir(), geteuid(), 0755)) {
		DEBUG_OUT (0, ("Failed to create pipe directory %s - %s\n",
	...
	if (state->fd == -1) {
		DEBUG_OUT (0, ("Failed to create pipe socket! [%s/%s]\n",
			  np_dir, pipe_name));
		goto out;
	}

	DEBUG_OUT (10, ("Openened pipe socket fd %d for %s\n",
		   state->fd, pipe_name));
	...
	if (!fde) {
		DEBUG_OUT (0, ("Failed to add event handler!\n"));
		goto out;
	}
				...
				void *private_data)
{
	...
	if (sd == -1) {
		if (errno != EINTR) {
			DEBUG_OUT (6, ("Failed to get a valid socket [%s]\n",
				  strerror(errno)));
		}
		return;
	}
...
static void named_pipe_accept_function(const char *pipe_name, int fd)
{
	...
	if (ret != 0) {
		DEBUG_OUT (2, ("Failed to make socket non-blocking\n"));
		TALLOC_FREE(npc);
		close(fd);
		return;
	}
	...
	if (ret != 0) {
		DEBUG_OUT (2, ("Failed to create tstream socket\n"));
		TALLOC_FREE(npc);
		close(fd);
		return;
	}
	...
	subreq = tstream_npa_accept_existing_send(npc, npc->ev, plain,
						  ...
						  npc->allocation_size);
	if (!subreq) {
		DEBUG_OUT (2, ("Failed to start async accept procedure\n"));
		TALLOC_FREE(npc);
		close(fd);
		return;
	}
...
static void named_pipe_accept_done(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (ret != 0) {
		DEBUG_OUT (2, ("Failed to accept named pipe connection! (%s)\n",
			  strerror(error)));
		TALLOC_FREE(npc);
		return;
	}
	...
	ret = make_server_pipes_struct(npc,
					...
					&npc->p, &error);
	if (ret != 0) {
		DEBUG_OUT (2, ("Failed to create pipes_struct! (%s)\n",
			  strerror(error)));
		goto fail;
	}
	...
	if (!npc->write_queue) {
		DEBUG_OUT (2, ("Failed to set up write queue!\n"));
		goto fail;
	}
	...
	if (!subreq) {
		DEBUG_OUT (2, ("Failed to start receving packets\n"));
		goto fail;
	}
...
fail:
	DEBUG_OUT (2, ("Fatal error. Terminating client(%s) connection!\n",
		  npc->client_name));
...
static void named_pipe_packet_process(struct tevent_req *subreq)
{
	...
	while (data_left) {
		...
		if (data_used < 0) {
			DEBUG_OUT (3, ("Failed to process dceprc request!\n"));
	...
	if (to_send > 0) {
		...
		DEBUG_OUT (10, ("Current_pdu_len = %u, "
			   "current_pdu_sent = %u "
			   "Returning %u bytes\n",
	...
	while (out->data_sent_length < out->rdata.length) {
		...
		if (!ok) {
			DEBUG_OUT (3, ("Failed to create next PDU!\n"));
			status = NT_STATUS_UNEXPECTED_IO_ERROR;
			goto fail;
		}
		...
		DEBUG_OUT (10, ("PDU number: %d, PDU Length: %u\n",
	...
	DEBUG_OUT (10, ("Sending a total of %u bytes\n",
	...
	subreq = tstream_writev_queue_send(npc, npc->ev,
					   ...
					   npc->iov, npc->count);
	if (!subreq) {
		DEBUG_OUT (2, ("Failed to send packet\n"));
		status = NT_STATUS_NO_MEMORY;
		goto fail;
	}
					 ...
					 uint16_t port)
{
	...
	state = talloc(ev_ctx, struct dcerpc_ncacn_listen_state);
	if (state == NULL) {
		DEBUG_OUT (0, ("setup_dcerpc_ncacn_tcpip_socket: Out of memory\n"));
		return 0;
	}
	...
	if (state->fd == -1) {
		DEBUG_OUT (0, ("setup_dcerpc_ncacn_tcpip_socket: Failed to create "
			  "socket on port %u!\n", state->ep.port));
		goto out;
	}
	...
	if (rc == -1) {
		DEBUG_OUT (0,("setup_tcpip_socket: listen - %s\n", strerror(errno)));
		goto out;
	}

	DEBUG_OUT (10, ("setup_tcpip_socket: openened socket fd %d for port %u\n",
	...
	fde = tevent_add_fd(state->ev_ctx,
			    ...
			    state);
	if (fde == NULL) {
		DEBUG_OUT (0, ("setup_tcpip_socket: Failed to add event handler!\n"));
		goto out;
	}
					...
					void *private_data)
{
	...
	if (s == -1) {
		if (errno != EINTR) {
			DEBUG_OUT (0,("tcpip_listener accept: %s\n",
				 strerror(errno)));
		}
		return;
	}
	...
	DEBUG_OUT (6, ("tcpip_listener: Accepted socket %d\n", s));
				 ...
				 dcerpc_ncacn_disconnect_fn fn)
{
	...
	fde = tevent_add_fd(state->ev_ctx,
			    ...
			    state);
	if (fde == NULL) {
		DEBUG_OUT (0, ("Failed to add event handler for ncalrpc!\n"));
		goto out;
	}
					...
					void *private_data)
{
	...
	if (sd == -1) {
		if (errno != EINTR) {
			DEBUG_OUT (0, ("ncalrpc accept() failed: %s\n", strerror(errno)));
		}
		return;
	}
	...
	DEBUG_OUT (10, ("Accepted ncalrpc socket %d\n", sd));
...
static void dcerpc_ncacn_accept(struct tevent_context *ev_ctx,
	...
	DEBUG_OUT (10, ("dcerpc_ncacn_accept\n"));
	...
	switch (transport) {
		...
		case NCALRPC:
			...
			if (rc < 0) {
				DEBUG_OUT (2, ("Failed to get ncalrpc connecting uid!"));
			} else {
		...
		case NCACN_NP:
			...
			break;
		default:
			DEBUG_OUT (0, ("unknown dcerpc transport: %u!\n",
				  transport));
			talloc_free(ncacn_conn);
			close(s);
			return;
	...
	if (rc < 0) {
		DEBUG_OUT (2, ("Failed to set dcerpc socket to non-blocking\n"));
		talloc_free(ncacn_conn);
		close(s);
		return;
	}
	...
	if (rc < 0) {
		DEBUG_OUT (2, ("Failed to create tstream socket for dcerpc\n"));
		talloc_free(ncacn_conn);
		close(s);
		return;
	}
	...
	rc = make_server_pipes_struct(ncacn_conn,
				      ...
				      &sys_errno);
	if (rc < 0) {
		DEBUG_OUT (2, ("Failed to create pipe struct - %s",
			  strerror(sys_errno)));
		talloc_free(ncacn_conn);
		return;
	}
	...
	subreq = dcerpc_read_ncacn_packet_send(ncacn_conn,
					       ...
					       ncacn_conn->tstream);
	if (subreq == NULL) {
		DEBUG_OUT (2, ("Failed to send ncacn packet\n"));
		talloc_free(ncacn_conn);
		return;
	}
	...
	DEBUG_OUT (10, ("dcerpc_ncacn_accept done\n"));
...
static void dcerpc_ncacn_packet_process(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		if (ncacn_conn->disconnect_fn != NULL) {
			...
			if (!ok) {
				DEBUG_OUT (3, ("Failed to call disconnect function\n"));
			}
		}
		goto fail;
	}
	...
	while (data_left) {
		...
		if (data_used < 0) {
			DEBUG_OUT (3, ("Failed to process dcerpc request!\n"));
			status = NT_STATUS_UNEXPECTED_IO_ERROR;
			goto fail;
		}
...
fail:
	DEBUG_OUT (3, ("Terminating client(%s) connection! - '%s'\n",
