...
{
        DEBUG_OUT (3, ("sending sam synchronisation message\n"));
			     ...
			     struct netr_LogonControl2Ex *r)
{
	...
	default:
		...
		DEBUG_OUT (0,("%s: unimplemented function level [%d]\n",
					   ...
					   struct netr_NetrEnumerateTrustedDomains *r)
{
	...
	DEBUG_OUT (6,("_netr_NetrEnumerateTrustedDomains: %d\n", __LINE__));
			  ...
			  struct messaging_context *msg_ctx)
{
	...
	if (!allow_access(lp_domain_hostsdeny(), lp_domain_hostsallow(),
			  p->client_id.name,
			  p->client_id.addr)) {
		DEBUG_OUT (0,("get_md4pw: Workstation %s denied access to domain\n", mach_acct));
		return False;
	}
#endif /* 0 */
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG_OUT (0,("get_md4pw: Workstation %s: account is disabled\n", mach_acct));
	...
	{
		DEBUG_OUT (0,("get_md4pw: Workstation %s: account is not a trust account\n", mach_acct));
	...
	switch (sec_chan_type) {
		case SEC_CHAN_BDC:
			if (!(acct_ctrl & ACB_SVRTRUST)) {
				DEBUG_OUT (0,("get_md4pw: Workstation %s: BDC secure channel requested "
		...
		case SEC_CHAN_WKSTA:
			if (!(acct_ctrl & ACB_WSTRUST)) {
				DEBUG_OUT (0,("get_md4pw: Workstation %s: WORKSTATION secure channel requested "
		...
		case SEC_CHAN_DOMAIN:
			if (!(acct_ctrl & ACB_DOMTRUST)) {
				DEBUG_OUT (0,("get_md4pw: Workstation %s: DOMAIN secure channel requested "
	...
	if (info->info18.nt_pwd_active == 0) {
		DEBUG_OUT (0,("get_md4pw: Workstation %s: account does not have a password\n", mach_acct));
				  ...
				  struct netr_ServerReqChallenge *r)
{
	...
	if (pipe_state) {
		DEBUG_OUT (10,("_netr_ServerReqChallenge: new challenge requested. Clearing old state.\n"));
				   ...
				   struct netr_ServerAuthenticate3 *r)
{
	...
	if (!pipe_state) {
		DEBUG_OUT (0,("%s: no challenge sent to client %s\n", fn,
			r->in.computer_name));
	...
	if ( (lp_server_schannel() == true) &&
		...
		DEBUG_OUT (0,("%s: schannel required but client failed "
	...
	status = get_md4pw(&mach_pwd,
			   ...
			   &sid, p->msg_ctx);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("%s: failed to get machine password for "
			"account %s: %s\n",
	...
	if (!creds) {
		DEBUG_OUT (0,("%s: netlogon_creds_server_check failed. Rejecting auth "
					...
					bool integrity, bool privacy)
{
	...
	/* test didn't pass */
	DEBUG_OUT (0, ("schannel_check_required: [%s] is not using schannel\n",
		  computer_name));
				 ...
				 struct netr_ServerPasswordSet *r)
{
	...
	DEBUG_OUT (5,("_netr_ServerPasswordSet: %d\n", __LINE__));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("_netr_ServerPasswordSet: netlogon_creds_server_step failed. Rejecting auth "
			"request from client %s machine account %s\n",
	...
	DEBUG_OUT (3,("_netr_ServerPasswordSet: Server Password Set by remote machine:[%s] on account [%s]\n",
			r->in.computer_name, creds->computer_name));

	netlogon_creds_des_decrypt(creds, r->in.new_password);

	DEBUG_OUT (100,("_netr_ServerPasswordSet: new given value was :\n"));
	for(i = 0; i < sizeof(r->in.new_password->hash); i++)
		DEBUG_OUT (100,("%02X ", r->in.new_password->hash[i]));
	DEBUG_OUT (100,("\n"));
				  ...
				  struct netr_ServerPasswordSet2 *r)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (2,("_netr_ServerPasswordSet2: netlogon_creds_server_step "
	...
	case NetlogonServiceTransitiveInformation:
		...
		case NetlogonValidationSamInfo4: /* 6 */
			if ((pdb_capabilities() & PDB_CAP_ADS) == 0) {
				DEBUG_OUT (10,("Not adding validation info level 6 "
					 ...
					 struct netlogon_creds_CredentialState *creds)
{
	...
	*r->out.authoritative = true; /* authoritative response */
	...
	case 6:
		...
		break;
	default:
		DEBUG_OUT (0,("%s: bad validation_level value %d.\n",
	...
	case NetlogonServiceTransitiveInformation:
		...
		DEBUG_OUT (3,("SAM Logon (Interactive). Domain:[%s].  ", lp_workgroup()));
		break;
	case NetlogonNetworkInformation:
	case NetlogonNetworkTransitiveInformation:
		...
		DEBUG_OUT (3,("SAM Logon (Network). Domain:[%s].  ", lp_workgroup()));
		break;
	default:
		DEBUG_OUT (2,("SAM Logon: unsupported switch value\n"));
		return NT_STATUS_INVALID_INFO_CLASS;
	} /* end switch */

	DEBUG_OUT (3,("User:[%s@%s] Requested Domain:[%s]\n", nt_username, nt_workstation, nt_domain));
	fstrcpy(current_user_info.smb_name, nt_username);
	sub_set_smb_name(nt_username);

	DEBUG_OUT (5,("Attempting validation level %d for unmapped username %s.\n",
	...
	DEBUG_OUT (5,("%s: check_password returned status %s\n",
		  fn, nt_errstr(status)));
	...
	if (server_info->guest) {
		...
		DEBUG_OUT (5,("%s: Attempted domain logon as GUEST "
			 "denied.\n", fn));
		TALLOC_FREE(server_info);
			       ...
			       struct netr_LogonSamLogonEx *r)
{
	...
	*r->out.authoritative = true;
	...
	if (p->auth.auth_type != DCERPC_AUTH_TYPE_SCHANNEL) {
		DEBUG_OUT (0,("_netr_LogonSamLogonEx: client %s not using schannel for netlogon\n",
