					...
					DATA_BLOB *token_out)
{
	...
	case SPNEGO_KRB5:
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Failed to init gssapi server "
				  "(%s)\n", nt_errstr(status)));
			return status;
		}
	...
	case SPNEGO_NTLMSSP:
		status = ntlmssp_server_auth_start(sp_ctx,
						   ...
						   &ntlmssp_ctx);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Failed to init ntlmssp server "
				  "(%s)\n", nt_errstr(status)));
			return status;
		}
	...
	default:
		DEBUG_OUT (3, ("No known mechanisms available\n"));
			    ...
			    DATA_BLOB *spnego_out)
{
	...
	if (len_in == -1) {
		DEBUG_OUT (1, (__location__ ": invalid SPNEGO blob.\n"));
	...
	*spnego_out = spnego_gen_auth_response_and_mic(mem_ctx, status,
							...
							&signature);
	if (!spnego_out->data) {
		DEBUG_OUT (1, ("SPNEGO wrapping failed!\n"));
		status = NT_STATUS_UNSUCCESSFUL;
	}
				  ...
				  struct spnego_context **spnego_ctx)
{
	...
	if (!ret || sp_ctx->oid_list[0] == NULL) {
		DEBUG_OUT (3, ("Invalid SPNEGO message\n"));
		status = NT_STATUS_INVALID_PARAMETER;
		goto done;
	}
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10, ("Client Provided OIDs:\n"));
		for (i = 0; sp_ctx->oid_list[i]; i++) {
			DEBUG_OUT (10, ("  %d: %s\n", i, sp_ctx->oid_list[i]));
		}
		DEBUG_OUT (10, ("Chosen OID: %s\n", sp_ctx->mech_oid));
	}
	...
	DEBUG_OUT (10, ("SPNEGO(%d) auth started\n", sp_ctx->mech));
