			     ...
			     struct samr_UserInfo20 *from)
{
	...
	if (from->parameters.array) {
		...
		DEBUG_OUT (10,("INFO_20 PARAMETERS: %s -> %s\n",
			old_string, new_string));
			     ...
			     struct samr_UserInfo21 *from)
{
	...
	if (from->fields_present & SAMR_FIELD_LAST_LOGON) {
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_LAST_LOGON: %lu -> %lu\n", l,
	...
	if (from->fields_present & SAMR_FIELD_LAST_LOGOFF) {
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_LAST_LOGOFF: %lu -> %lu\n", l,
			(long unsigned int)stored_time,
			(long unsigned int)unix_time));
		if (stored_time != unix_time) {
	...
	if (from->fields_present & SAMR_FIELD_ACCT_EXPIRY) {
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_ACCT_EXPIRY: %lu -> %lu\n", l,
			(long unsigned int)stored_time,
			(long unsigned int)unix_time));
		if (stored_time != unix_time) {
	...
	if (from->fields_present & SAMR_FIELD_LAST_PWD_CHANGE) {
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_LAST_PWD_CHANGE: %lu -> %lu\n", l,
			(long unsigned int)stored_time,
			(long unsigned int)unix_time));
		if (stored_time != unix_time) {
	...
	if ((from->fields_present & SAMR_FIELD_ACCOUNT_NAME) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_ACCOUNT_NAME: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_FULL_NAME) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_FULL_NAME: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_HOME_DIRECTORY) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_HOME_DIRECTORY: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_HOME_DRIVE) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_HOME_DRIVE: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_LOGON_SCRIPT) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_LOGON_SCRIPT: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_PROFILE_PATH) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_PROFILE_PATH: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_DESCRIPTION) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_DESCRIPTION: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_WORKSTATIONS) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_WORKSTATIONS: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_COMMENT) &&
		...
		DEBUG_OUT (10,("%s SAMR_FIELD_COMMENT: %s -> %s\n", l,
			old_string, new_string));
		if (STRING_CHANGED) {
	...
	if ((from->fields_present & SAMR_FIELD_PARAMETERS) &&
		...
		newstr = (mung.length == 0) ?
			NULL : base64_encode_data_blob(talloc_tos(), mung);
		DEBUG_OUT (10,("%s SAMR_FIELD_PARAMETERS: %s -> %s\n", l,
			old_string, newstr));
	...
	if (from->fields_present & SAMR_FIELD_RID) {
		if (from->rid == 0) {
			DEBUG_OUT (10,("%s: Asked to set User RID to 0 !? Skipping change!\n", l));
		} else if (from->rid != pdb_get_user_rid(to)) {
			DEBUG_OUT (10,("%s SAMR_FIELD_RID: %u -> %u NOT UPDATED!\n", l,
	...
	if (from->fields_present & SAMR_FIELD_PRIMARY_GID) {
		if (from->primary_gid == 0) {
			DEBUG_OUT (10,("%s: Asked to set Group RID to 0 !? Skipping change!\n", l));
		} else if (from->primary_gid != pdb_get_group_rid(to)) {
			DEBUG_OUT (10,("%s SAMR_FIELD_PRIMARY_GID: %u -> %u\n", l,
	...
	if (from->fields_present & SAMR_FIELD_ACCT_FLAGS) {
		DEBUG_OUT (10,("%s SAMR_FIELD_ACCT_FLAGS: %08X -> %08X\n", l,
	...
	if (from->fields_present & SAMR_FIELD_LOGON_HOURS) {
		...
		char newstr[44];
		DEBUG_OUT (15,("%s SAMR_FIELD_LOGON_HOURS (units_per_week): %08X -> %08X\n", l,
		...
		DEBUG_OUT (15,("%s SAMR_FIELD_LOGON_HOURS (units_per_week/8): %08X -> %08X\n", l,
		...
		DEBUG_OUT (15,("%s SAMR_FIELD_LOGON_HOURS (bits): %s -> %s\n", l,
	...
	if (from->fields_present & SAMR_FIELD_BAD_PWD_COUNT) {
		DEBUG_OUT (10,("%s SAMR_FIELD_BAD_PWD_COUNT: %08X -> %08X\n", l,
	...
	if (from->fields_present & SAMR_FIELD_NUM_LOGONS) {
		DEBUG_OUT (10,("%s SAMR_FIELD_NUM_LOGONS: %08X -> %08X\n", l,
	...
	if (from->fields_present & SAMR_FIELD_EXPIRED_FLAG) {
		DEBUG_OUT (10,("%s SAMR_FIELD_EXPIRED_FLAG: %02X\n", l,
		...
		if (from->password_expired != 0) {
			...
			if (pdb_get_pass_can_change(to)) {
				...
				DEBUG_OUT (10,("%s Disallowing set of 'User Must "
	...
	if (from->fields_present & SAMR_FIELD_COUNTRY_CODE) {
		DEBUG_OUT (10,("%s SAMR_FIELD_COUNTRY_CODE: %08X -> %08X\n", l,
	...
	if (from->fields_present & SAMR_FIELD_CODE_PAGE) {
		DEBUG_OUT (10,("%s SAMR_FIELD_CODE_PAGE: %08X -> %08X\n", l,
