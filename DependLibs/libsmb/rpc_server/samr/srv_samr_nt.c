...
{
	DEBUG_OUT (10, ("free_samr_cache: deleting cache for SID %s\n",
						 ...
						 void *private_data)
{
	...
	DEBUG_OUT (10, ("disp_info_cache_idle_timeout_handler: caching timed "
		   "out\n"));
...
static void set_disp_info_cache_timeout(DISP_INFO *disp_info, time_t secs_fromnow)
{
	...
	DEBUG_OUT (10,("set_disp_info_cache_timeout: caching enumeration for "
		  "SID %s for %u seconds\n", sid_string_dbg(&disp_info->sid),
...
static void force_flush_samr_cache(const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10,("force_flush_samr_cache: clearing idle event\n"));
	TALLOC_FREE(disp_info->cache_timeout_event);
	free_samr_cache(disp_info);
}
			  ...
			  struct samr_OpenDomain *r)
{
	...
	DEBUG_OUT (5,("_samr_OpenDomain: %d\n", __LINE__));
			     ...
			     struct samr_GetUserPwInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_GetUserPwInfo: %d\n", __LINE__));
			     ...
			     struct samr_QuerySecurity *r)
{
	...
	cinfo = policy_handle_find(p, r->in.handle,
				   ...
				   struct samr_connect_info, &status);
	if (NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("_samr_QuerySecurity: querying security on SAM\n"));
	...
	dinfo = policy_handle_find(p, r->in.handle,
				   ...
				   struct samr_domain_info, &status);
	if (NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("_samr_QuerySecurity: querying security on Domain "
	...
	uinfo = policy_handle_find(p, r->in.handle,
				   ...
				   struct samr_user_info, &status);
	if (NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("_samr_QuerySecurity: querying security on user "
			  "Object with SID: %s\n",
			  sid_string_dbg(&uinfo->sid)));
	...
	if (NT_STATUS_IS_OK(status)) {
		 ...
		 */
		DEBUG_OUT (10,("_samr_QuerySecurity: querying security on group "
			  "Object with SID: %s\n",
	...
	if (NT_STATUS_IS_OK(status)) {
		 ...
		 * and users.  Currently all three get a default user SD
		 */
		DEBUG_OUT (10,("_samr_QuerySecurity: querying security on alias "
			  "Object with SID: %s\n",
					 ...
					 struct samr_displayentry *entries)
{
	...
	*sam_pp = NULL;
	...
	if (sam == NULL) {
		DEBUG_OUT (0, ("make_user_sam_entry_list: TALLOC_ZERO failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}
			       ...
			       struct samr_EnumDomainUsers *r)
{
	...
	DEBUG_OUT (5,("_samr_EnumDomainUsers: %d\n", __LINE__));
	...
	*r->out.sam = samr_array;
	...
	if (sid_check_is_builtin(&dinfo->sid)) {
		...
		*r->out.resume_handle = *r->in.resume_handle;
		DEBUG_OUT (5,("_samr_EnumDomainUsers: No users in BUILTIN\n"));
		return status;
	}
	...
	if (num_account == 0) {
		DEBUG_OUT (5, ("_samr_EnumDomainUsers: enumeration handle over "
			  "total entries\n"));
		*r->out.resume_handle = *r->in.resume_handle;
		return NT_STATUS_OK;
	}
				...
				struct samr_EnumDomainGroups *r)
{
	...
	DEBUG_OUT (5,("_samr_EnumDomainGroups: %d\n", __LINE__));
	...
	*r->out.sam = samr_array;
	...
	if (sid_check_is_builtin(&dinfo->sid)) {
		/* No groups in builtin. */
		*r->out.resume_handle = *r->in.resume_handle;
		DEBUG_OUT (5,("_samr_EnumDomainGroups: No groups in BUILTIN\n"));
		return status;
	}
				 ...
				 struct samr_EnumDomainAliases *r)
{
	...
	DEBUG_OUT (5,("_samr_EnumDomainAliases: sid %s\n",
		 sid_string_dbg(&dinfo->sid)));
	...
	DEBUG_OUT (5,("_samr_EnumDomainAliases: %d\n", __LINE__));
				     ...
				     struct samr_displayentry *entries)
{
	...
	DEBUG_OUT (10, ("init_samr_dispinfo_1: num_entries: %d\n", num_entries));
				     ...
				     struct samr_displayentry *entries)
{
	...
	DEBUG_OUT (10, ("init_samr_dispinfo_2: num_entries: %d\n", num_entries));
				     ...
				     struct samr_displayentry *entries)
{
	...
	DEBUG_OUT (5, ("init_samr_dispinfo_3: num_entries: %d\n", num_entries));
				     ...
				     struct samr_displayentry *entries)
{
	...
	DEBUG_OUT (5, ("init_samr_dispinfo_4: num_entries: %d\n", num_entries));
				     ...
				     struct samr_displayentry *entries)
{
	...
	DEBUG_OUT (5, ("init_samr_dispinfo_5: num_entries: %d\n", num_entries));
				...
				struct samr_QueryDisplayInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_QueryDisplayInfo: %d\n", __LINE__));
	...
	if (sid_check_is_builtin(&dinfo->sid)) {
		DEBUG_OUT (5,("_samr_QueryDisplayInfo: no users in BUILTIN\n"));
		return NT_STATUS_OK;
	}
	...
	if ((r->in.level < 1) || (r->in.level > 5)) {
		DEBUG_OUT (0,("_samr_QueryDisplayInfo: Unknown info level (%u)\n",
	...
	if (r->in.max_entries > max_sam_entries) {
		DEBUG_OUT (5, ("_samr_QueryDisplayInfo: client requested %d "
	...
	if (temp_size > r->in.buf_size) {
		...
		DEBUG_OUT (5, ("_samr_QueryDisplayInfo: buffer size limits to "
			  "only %d entries\n", max_entries));
	}
	...
	case 4:
		if (dinfo->disp_info->users == NULL) {
			...
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: starting user enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		} else {
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: using cached user enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		}
	...
	case 2:
		if (dinfo->disp_info->machines == NULL) {
			...
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: starting machine enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		} else {
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: using cached machine enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		}
	...
	case 5:
		if (dinfo->disp_info->groups == NULL) {
			...
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: starting group enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		} else {
			DEBUG_OUT (10,("_samr_QueryDisplayInfo: using cached group enumeration at index %u\n",
				(unsigned  int)r->in.start_idx));
		}
			      ...
			      struct samr_QueryAliasInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_QueryAliasInfo: %d\n", __LINE__));
			   ...
			   struct samr_LookupNames *r)
{
	...
	DEBUG_OUT (5,("_samr_LookupNames: %d\n", __LINE__));
	...
	if (num_rids > MAX_SAM_ENTRIES) {
		...
		DEBUG_OUT (5,("_samr_LookupNames: truncating entries to %d\n", num_rids));
	}
	...
	DEBUG_OUT (5,("_samr_LookupNames: looking name on SID %s\n",
		 sid_string_dbg(&dinfo->sid)));
				  ...
				  struct samr_ChangePasswordUser *r)
{
	...
	DEBUG_OUT (5,("_samr_ChangePasswordUser: sid:%s\n",
		  sid_string_dbg(&uinfo->sid)));
				   ...
				   struct samr_ChangePasswordUser2 *r)
{
	...
	DEBUG_OUT (5,("_samr_ChangePasswordUser2: %d\n", __LINE__));
	...
	DEBUG_OUT (5,("_samr_ChangePasswordUser2: user: %s wks: %s\n", user_name, wks));
				      ...
				      struct samr_OemChangePasswordUser2 *r)
{
	...
	DEBUG_OUT (5,("_samr_OemChangePasswordUser2: %d\n", __LINE__));
	...
	DEBUG_OUT (5,("_samr_OemChangePasswordUser2: user: %s wks: %s\n", user_name, wks));
				   ...
				   struct samr_ChangePasswordUser3 *r)
{
	...
	DEBUG_OUT (5,("_samr_ChangePasswordUser3: %d\n", __LINE__));
	...
	DEBUG_OUT (5,("_samr_ChangePasswordUser3: user: %s wks: %s\n", user_name, wks));
			  ...
			  struct samr_LookupRids *r)
{
	...
	DEBUG_OUT (5,("_samr_LookupRids: %d\n", __LINE__));
	...
	if (num_rids > 1000) {
		DEBUG_OUT (0, ("Got asked for %d rids (more than 1000) -- according "
	...
	if (ret) {
		...
		if (acb_info & (ACB_SVRTRUST|ACB_DOMTRUST)) {
			if (lp_enable_privileges() && nt_token_check_domain_rid(p->session_info->security_token,
				...
				DEBUG_OUT (4,("_samr_OpenUser: Allowing "
				...
				struct dom_sid *domain_sid)
{
	...
	if (!sid_peek_check_rid(domain_sid, sid_group, &primary_gid)) {
		DEBUG_OUT (0, ("get_user_info_1: User %s has Primary Group SID %s, \n"
				...
				struct dom_sid *domain_sid)
{
	...
	if (!sid_peek_check_rid(domain_sid, sid_user, &rid)) {
		DEBUG_OUT (0, ("get_user_info_3: User %s has SID %s, \nwhich conflicts with "
	...
	if (!sid_peek_check_rid(domain_sid, sid_group, &primary_gid)) {
		DEBUG_OUT (0, ("get_user_info_3: User %s has Primary Group SID %s, \n"
			  "which conflicts with the domain sid %s.  Failing operation.\n",
			  pdb_get_username(pw), sid_string_dbg(sid_group),
			  sid_string_dbg(domain_sid)));
		return NT_STATUS_UNSUCCESSFUL;
				...
				struct dom_sid *domain_sid)
{
	...
	if (!sid_peek_check_rid(domain_sid, sid_user, &rid)) {
		DEBUG_OUT (0, ("get_user_info_5: User %s has SID %s, \nwhich conflicts with "
			  "the domain sid %s.  Failing operation.\n",
			  pdb_get_username(pw), sid_string_dbg(sid_user),
			  sid_string_dbg(domain_sid)));
		return NT_STATUS_UNSUCCESSFUL;
	...
	if (!sid_peek_check_rid(domain_sid, sid_group, &primary_gid)) {
		DEBUG_OUT (0, ("get_user_info_5: User %s has Primary Group SID %s, \n"
			  "which conflicts with the domain sid %s.  Failing operation.\n",
			  pdb_get_username(pw), sid_string_dbg(sid_group),
			  sid_string_dbg(domain_sid)));
		return NT_STATUS_UNSUCCESSFUL;
				 ...
				 struct samu *sampass)
{
	...
	DEBUG_OUT (3,("User:[%s] has [%s] (length: %d)\n", pdb_get_username(sampass),
				 ...
				 uint32_t acc_granted)
{
	...
	if (!sid_peek_check_rid(domain_sid, sid_user, &rid)) {
		DEBUG_OUT (0, ("get_user_info_21: User %s has SID %s, \nwhich conflicts with "
			  "the domain sid %s.  Failing operation.\n",
			  pdb_get_username(pw), sid_string_dbg(sid_user),
			  sid_string_dbg(domain_sid)));
		return NT_STATUS_UNSUCCESSFUL;
	...
	if (!sid_peek_check_rid(domain_sid, sid_group, &primary_gid)) {
		DEBUG_OUT (0, ("get_user_info_21: User %s has Primary Group SID %s, \n"
			  "which conflicts with the domain sid %s.  Failing operation.\n",
			  pdb_get_username(pw), sid_string_dbg(sid_group),
			  sid_string_dbg(domain_sid)));
		return NT_STATUS_UNSUCCESSFUL;
			     ...
			     struct samr_QueryUserInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_QueryUserInfo: sid:%s\n",
		 sid_string_dbg(&uinfo->sid)));
	...
	DEBUG_OUT (5,("_samr_QueryUserInfo: user info level: %d\n", r->in.level));
	...
	*r->out.info = user_info;
	...
	DEBUG_OUT (5,("_samr_QueryUserInfo: %d\n", __LINE__));
				...
				struct samr_GetGroupsForUser *r)
{
	...
	DEBUG_OUT (5,("_samr_GetGroupsForUser: %d\n", __LINE__));
	...
	if (!ret) {
		DEBUG_OUT (10, ("pdb_getsampwsid failed for %s\n",
			   sid_string_dbg(&uinfo->sid)));
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10, ("pdb_enum_group_memberships failed for %s\n",
			   sid_string_dbg(&uinfo->sid)));
		return result;
	}

	if ( !success ) {
		DEBUG_OUT (5, ("Group sid %s for user %s not in our domain\n",
	...
	for (i=0; i<num_groups; i++) {
		...
		if (!sid_peek_check_rid(get_global_sam_sid(),
					&(sids[i]), &dom_gid.rid)) {
			DEBUG_OUT (10, ("Found sid %s not in our domain\n",
			       ...
			       struct samr_QueryDomainInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_QueryDomainInfo: %d\n", __LINE__));
...
static NTSTATUS can_create(TALLOC_CTX *mem_ctx, const char *new_name)
{
	...
	DEBUG_OUT (10, ("Checking whether [%s] can be created\n", new_name));
	...
	if (!result) {
		DEBUG_OUT (10, ("%s does not exist, can create it\n", new_name));
		return NT_STATUS_OK;
	}

	DEBUG_OUT (5, ("trying to create %s, exists as %s\n",
			   ...
			   struct samr_CreateUser2 *r)
{
	...
	if (sid_check_is_builtin(&dinfo->sid)) {
		DEBUG_OUT (5,("_samr_CreateUser2: Refusing user create in BUILTIN\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	DEBUG_OUT (5, ("_samr_CreateUser2: %s can add this account : %s\n",
		       ...
		       struct samr_Connect *r)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to _samr_Connect\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
			...
			struct samr_Connect2 *r)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to %s\n", fn));
		return NT_STATUS_ACCESS_DENIED;
	}
			    ...
			    struct samr_LookupDomain *r)
{
	...
	DEBUG_OUT (2,("Returning domain sid for domain %s -> %s\n", domain_name,
		 sid_string_dbg(sid)));
...
{
	if (id2 == NULL) {
		DEBUG_OUT (5,("set_user_info_2: NULL id2\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id4 == NULL) {
		DEBUG_OUT (5,("set_user_info_2: NULL id4\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id6 == NULL) {
		DEBUG_OUT (5,("set_user_info_6: NULL id6\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
				...
				struct samu *pwd)
{
	...
	if (id7 == NULL) {
		DEBUG_OUT (5, ("set_user_info_7: NULL id7\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	if (!id7->account_name.string) {
	        DEBUG_OUT (5, ("set_user_info_7: failed to get new username\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id8 == NULL) {
		DEBUG_OUT (5,("set_user_info_8: NULL id8\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id10 == NULL) {
		DEBUG_OUT (5,("set_user_info_8: NULL id10\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id11 == NULL) {
		DEBUG_OUT (5,("set_user_info_11: NULL id11\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id12 == NULL) {
		DEBUG_OUT (5,("set_user_info_12: NULL id12\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id13 == NULL) {
		DEBUG_OUT (5,("set_user_info_13: NULL id13\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id14 == NULL) {
		DEBUG_OUT (5,("set_user_info_14: NULL id14\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id16 == NULL) {
		DEBUG_OUT (5,("set_user_info_16: NULL id16\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id17 == NULL) {
		DEBUG_OUT (5,("set_user_info_17: NULL id17\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...
{
	if (id18 == NULL) {
		DEBUG_OUT (2, ("set_user_info_18: id18 is NULL\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
{
	if (id20 == NULL) {
		DEBUG_OUT (5,("set_user_info_20: NULL id20\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
				 ...
				 struct samu *pwd)
{
	...
	if (id21 == NULL) {
		DEBUG_OUT (5, ("set_user_info_21: NULL id21\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	    ...
	    (!strequal(id21->account_name.string, pdb_get_username(pwd))))
	{
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("set_user_info_21: failed to rename account: %s\n",
				nt_errstr(status)));
			return status;
		}
				 ...
				 struct samu *pwd)
{
	...
	if (id23 == NULL) {
		DEBUG_OUT (5, ("set_user_info_23: NULL id23\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if ((id23->info.fields_present & SAMR_FIELD_NT_PASSWORD_PRESENT) ||
		...
		DEBUG_OUT (5, ("Attempting administrator password change (level 23) for user %s\n",
			  pdb_get_username(pwd)));
	...
	if (    ( (acct_ctrl &  ACB_DOMTRUST) == ACB_DOMTRUST ) ||
		...
		DEBUG_OUT (5, ("Changing trust account.  Not updating /etc/passwd\n"));
	} else if (plaintext_buf) {
		/* update the UNIX password */
		if (lp_unix_password_sync() ) {
			struct passwd *passwd;
			if (pdb_get_username(pwd) == NULL) {
				DEBUG_OUT (1, ("chgpasswd: User without name???\n"));
				return NT_STATUS_ACCESS_DENIED;
			}

			passwd = Get_Pwnam_alloc(pwd, pdb_get_username(pwd));
			if (passwd == NULL) {
				DEBUG_OUT (1, ("chgpasswd: Username does not exist in system !?!\n"));
			}
...
static bool set_user_info_pw(uint8 *pass, const char *rhost, struct samu *pwd)
{
	...
	DEBUG_OUT (5, ("Attempting administrator password change for user %s\n",
		  pdb_get_username(pwd)));
	...
	if ( ( (acct_ctrl &  ACB_DOMTRUST) == ACB_DOMTRUST ) ||
		( (acct_ctrl &  ACB_WSTRUST) ==  ACB_WSTRUST) ||
		( (acct_ctrl &  ACB_SVRTRUST) ==  ACB_SVRTRUST) ) {
		DEBUG_OUT (5, ("Changing trust account or non-unix-user password, not updating /etc/passwd\n"));
	} else {
		/* update the UNIX password */
		if (lp_unix_password_sync()) {
			struct passwd *passwd;
	...
	DEBUG_OUT (5,("set_user_info_pw: pdb_update_pwd()\n"));
				 ...
				 struct samu *pwd)
{
	...
	if (id24 == NULL) {
		DEBUG_OUT (5, ("set_user_info_24: NULL id24\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
				 ...
				 struct samu *pwd)
{
	...
	if (id25 == NULL) {
		DEBUG_OUT (5, ("set_user_info_25: NULL id25\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
				 ...
				 struct samu *pwd)
{
	...
	if (id26 == NULL) {
		DEBUG_OUT (5, ("set_user_info_26: NULL id26\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
			   ...
			   struct samr_SetUserInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_SetUserInfo: %d\n", __LINE__));
	...
	DEBUG_OUT (5, ("_samr_SetUserInfo: sid:%s, level:%d\n",
		  sid_string_dbg(&uinfo->sid), r->in.level));

	if (info == NULL) {
		DEBUG_OUT (5, ("_samr_SetUserInfo: NULL info level\n"));
		return NT_STATUS_INVALID_INFO_CLASS;
	}
				  ...
				  struct samr_GetAliasMembership *r)
{
	...
	DEBUG_OUT (5,("_samr_GetAliasMembership: %d\n", __LINE__));
				...
				struct samr_QueryGroupMember *r)
{
	...
	if (!sid_check_is_in_our_domain(&ginfo->sid)) {
		DEBUG_OUT (3, ("sid %s is not in our domain\n",
			  sid_string_dbg(&ginfo->sid)));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	DEBUG_OUT (10, ("lookup on Domain SID\n"));
				 ...
				 struct samr_DeleteAliasMember *r)
{
	...
	DEBUG_OUT (10, ("_samr_del_aliasmem:sid is %s\n",
		   sid_string_dbg(&ainfo->sid)));
			  ...
			  struct samr_DeleteUser *r)
{
	...
	DEBUG_OUT (5, ("_samr_DeleteUser: %d\n", __LINE__));
	...
	if(!ret) {
		DEBUG_OUT (5,("_samr_DeleteUser: User %s doesn't exist.\n",
			sid_string_dbg(&uinfo->sid)));
		TALLOC_FREE(sam_pass);
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (5,("_samr_DeleteUser: Failed to delete entry for "
			 "user %s: %s.\n", pdb_get_username(sam_pass),
			 nt_errstr(status)));
		TALLOC_FREE(sam_pass);
		return status;
				 ...
				 struct samr_DeleteDomainGroup *r)
{
	...
	DEBUG_OUT (5, ("samr_DeleteDomainGroup: %d\n", __LINE__));
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (5,("_samr_DeleteDomainGroup: Failed to delete mapping "
			 "entry for group %s: %s\n",
			 sid_string_dbg(&ginfo->sid),
			 nt_errstr(status)));
		return status;
			      ...
			      struct samr_DeleteDomAlias *r)
{
	...
	DEBUG_OUT (5, ("_samr_DeleteDomAlias: %d\n", __LINE__));
	...
	DEBUG_OUT (10, ("lookup on Local SID\n"));
			      ...
			      struct samr_CreateDomAlias *r)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10, ("pdb_create_alias failed: %s\n",
			   nt_errstr(result)));
		return result;
	}
	...
	if (!sid_to_gid(&info_sid, &gid)) {
		DEBUG_OUT (10, ("Could not find alias just created\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	/* check if the group has been successfully created */
	if ( getgrgid(gid) == NULL ) {
		DEBUG_OUT (1, ("getgrgid(%u) of just created alias failed\n",
			   (unsigned int)gid));
		return NT_STATUS_ACCESS_DENIED;
	}
			    ...
			    struct samr_GetDomPwInfo *r)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to _samr_GetDomPwInfo\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
...

{
	...
	DEBUG_OUT (10, ("_samr_OpenGroup:Opening SID: %s\n",
					     ...
					     struct samr_RemoveMemberFromForeignDomain *r)
{
	...
	DEBUG_OUT (5,("_samr_RemoveMemberFromForeignDomain: removing SID [%s]\n",
	...
	DEBUG_OUT (8, ("_samr_RemoveMemberFromForeignDomain: sid is %s\n",
		  sid_string_dbg(&dinfo->sid)));
	...
	if (!sid_check_is_builtin(&dinfo->sid)) {
		DEBUG_OUT (1,("_samr_RemoveMemberFromForeignDomain: domain_sid = %s, "
			 "global_sam_sid() = %s\n",
			 sid_string_dbg(&dinfo->sid),
			     ...
			     struct samr_SetDomainInfo *r)
{
	...
	DEBUG_OUT (5,("_samr_SetDomainInfo: %d\n", __LINE__));
	...
	DEBUG_OUT (5,("_samr_SetDomainInfo: level: %d\n", r->in.level));
					  ...
					  struct samr_GetDisplayEnumerationIndex *r)
{
	...
	DEBUG_OUT (5,("_samr_GetDisplayEnumerationIndex: %d\n", __LINE__));
	...
	if ((r->in.level < 1) || (r->in.level > 3)) {
		DEBUG_OUT (0,("_samr_GetDisplayEnumerationIndex: "
	...
	case 1:
		if (dinfo->disp_info->users == NULL) {
			...
			DEBUG_OUT (10,("_samr_GetDisplayEnumerationIndex: "
	...
	DEBUG_OUT (10,("_samr_GetDisplayEnumerationIndex: looking for :%s\n",
		r->in.name->string));
