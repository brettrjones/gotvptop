		   ...
		   const char *passwordprogram, bool as_root)
{
	...
	{
		DEBUG_OUT (3, ("More weirdness, could not open %s\n", slavedev));
		return (False);
	}
	...
	{
		DEBUG_OUT (3, ("Error in ioctl call for slave pty\n"));
		/* return(False); */
	}
	...
	{
		DEBUG_OUT (3, ("Could not re-direct stdin\n"));
		return (False);
	}
	if (dup2(slave, STDOUT_FILENO) != STDOUT_FILENO)
	{
		DEBUG_OUT (3, ("Could not re-direct stdout\n"));
		return (False);
	}
	if (dup2(slave, STDERR_FILENO) != STDERR_FILENO)
	{
		DEBUG_OUT (3, ("Could not re-direct stderr\n"));
		return (False);
	}
	if (slave > 2)
		close(slave);
	...
	{
		DEBUG_OUT (3, ("could not set attributes of pty\n"));
		return (False);
	}
	...
	{
		DEBUG_OUT (3, ("Bad status returned from %s\n", passwordprogram));
		return (False);
	}
	return (True);
}
	...
	for (attempts = 0; attempts < 2; attempts++) {
		...
		if (!strequal(issue, ".")) {
			if (lp_passwd_chat_debug())
				DEBUG_OUT (100, ("expect: sending [%s]\n", issue));

			if ((len = sys_write(master, issue, strlen(issue))) != strlen(issue)) {
				DEBUG_OUT (2,("expect: (short) write returned %d\n",
					 (int)len ));
				return False;
			}
		}
		...
		while (True) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (2, ("expect: read error %s\n",
					  nt_errstr(status)));
				break;
			}
			nread += len;
		...
		if (lp_passwd_chat_debug())
			DEBUG_OUT (100, ("expect: expected [%s] received [%s] match %s\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("expect: %s\n", nt_errstr(status)));
			return False;
		}
	}

	DEBUG_OUT (10,("expect: returning %s\n", match ? "True" : "False" ));
	return match;
}
...
static int talktochild(int master, const char *seq)
{
	...
	while (next_token_talloc(frame, &seq, &expected, NULL)) {
		...
		if (!expect(master, issue, expected)) {
			DEBUG_OUT (3, ("Response %d incorrect\n", count));
			TALLOC_FREE(frame);
			return false;
		}
			      ...
			      char *chatsequence, bool as_root)
{
	...
	if (pass == NULL) {
		DEBUG_OUT (0, ("chat_with_program: user doesn't exist in the UNIX password database.\n"));
		return False;
	}

	/* allocate a pseudo-terminal device */
	if ((master = findpty(&slavedev)) < 0) {
		DEBUG_OUT (3, ("chat_with_program: Cannot Allocate pty for password change: %s\n", pass->pw_name));
		return (False);
	}
	...
	if ((pid = sys_fork()) < 0) {
		DEBUG_OUT (3, ("chat_with_program: Cannot fork() child for password change: %s\n", pass->pw_name));
		SAFE_FREE(slavedev);
		close(master);
		CatchChild();
		return (False);
	...
	if (pid > 0) {			/* This is the parent process */
		...
		if ((chstat = talktochild(master, chatsequence)) == False) {
			DEBUG_OUT (3, ("chat_with_program: Child failed to change password: %s\n", pass->pw_name));
		...
		if (wpid < 0) {
			DEBUG_OUT (3, ("chat_with_program: The process is no longer waiting!\n\n"));
			close(master);
			CatchChild();
			return (False);
		}
		...
		if (pid != wpid) {
			DEBUG_OUT (3, ("chat_with_program: We were waiting for the wrong process ID\n"));
			return (False);
		}
		if (WIFEXITED(wstat) && (WEXITSTATUS(wstat) != 0)) {
			DEBUG_OUT (3, ("chat_with_program: The process exited with status %d \
while we were waiting\n", WEXITSTATUS(wstat)));
			return (False);
		}
#if defined(WIFSIGNALLED) && defined(WTERMSIG)
		else if (WIFSIGNALLED(wstat)) {
                        DEBUG_OUT (3, ("chat_with_program: The process was killed by signal %d \
while we were waiting\n", WTERMSIG(wstat)));
			return (False);
		}
#endif
		...
		DEBUG_OUT (3, ("chat_with_program: Dochild for user %s (uid=%d,gid=%d) (as_root = %s)\n", pass->pw_name,
		...
		DEBUG_OUT (0, ("chat_with_program: Error: dochild() returned %d\n", chstat));
		exit(1);
	}

	if (chstat)
		DEBUG_OUT (3, ("chat_with_program: Password change %ssuccessful for user %s\n",
	       ...
	       const char *oldpass, const char *newpass, bool as_root)
{
	...
	DEBUG_OUT (3, ("chgpasswd: Password change (as_root=%s) for user: %s\n", BOOLSTR(as_root), name));

#ifdef DEBUG_PASSWORD
	DEBUG_OUT (100, ("chgpasswd: Passwords: old=%s new=%s\n", oldpass, newpass));
#endif
	...
	if (strcmp(oldpass, newpass) == 0) {
		...
		DEBUG_OUT (2, ("chgpasswd: Password Change: %s, New password is same as old\n", name));	/* log the attempt */
	...
	for (i = 0; i < len; i++) {
		if (iscntrl((int)oldpass[i])) {
			DEBUG_OUT (0, ("chgpasswd: oldpass contains control characters (disallowed).\n"));
			return False;
		}
	}
	...
	for (i = 0; i < len; i++) {
		if (iscntrl((int)newpass[i])) {
			DEBUG_OUT (0, ("chgpasswd: newpass contains control characters (disallowed).\n"));
			return False;
		}
	}
	...
	if (pass == NULL) {
		DEBUG_OUT (0, ("chgpasswd: user %s doesn't exist in the UNIX password database.\n", name));
		return false;
	}

	passwordprogram = talloc_strdup(ctx, lp_passwd_program());
	if (!passwordprogram || !*passwordprogram) {
		DEBUG_OUT (2, ("chgpasswd: Null password program - no password changing\n"));
		return false;
	}
	chatsequence = talloc_strdup(ctx, lp_passwd_chat());
	if (!chatsequence || !*chatsequence) {
		DEBUG_OUT (2, ("chgpasswd: Null chat sequence - no password changing\n"));
		return false;
	}
	...
	if (as_root) {
		...
		if (strstr_m(passwordprogram, "%u") == NULL) {
			DEBUG_OUT (0,("chgpasswd: Running as root the 'passwd program' parameter *MUST* contain \
...
bool chgpasswd(const char *name, const struct passwd *pass,
	       const char *oldpass, const char *newpass, bool as_root)
{
	DEBUG_OUT (0, ("chgpasswd: Unix Password changing not compiled in (user=%s)\n", name));
	return (False);
}
				   ...
				   char **pp_new_passwd)
{
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG_OUT (2,("check_lanman_password: account %s disabled.\n", user));
	...
	if (nt_pw && nt_pass_set) {
		...
		DEBUG_OUT (1, ("NT password change supplied for user %s, but we have no NT password to check it with\n",
			  user));
		return NT_STATUS_WRONG_PASSWORD;
	} else if (lm_pass_set) {
		if (lp_lanman_auth()) {
			DEBUG_OUT (1, ("LM password change supplied for user %s, but we have no LanMan password to check it with\n",
				  user));
		} else {
			DEBUG_OUT (1, ("LM password change supplied for user %s, but we have disabled LanMan authentication\n",
				  user));
		}
		return NT_STATUS_WRONG_PASSWORD;
	} else {
		DEBUG_OUT (1, ("password change requested for user %s, but no password supplied!\n",
			  user));
		return NT_STATUS_WRONG_PASSWORD;
	}
	...
	if (nt_pass_set) {
		...
		if (nt_pw) {
			...
			if (memcmp(verifier, old_nt_hash_encrypted, 16)) {
				DEBUG_OUT (0, ("check_oem_password: old nt "
		...
		if (lanman_pw) {
			...
			if (memcmp(verifier, old_lm_hash_encrypted, 16)) {
				DEBUG_OUT (0,("check_oem_password: old lm password doesn't match.\n"));
				return NT_STATUS_WRONG_PASSWORD;
			}
#ifdef DEBUG_PASSWORD
			DEBUG_OUT (100,
...
static bool check_passwd_history(struct samu *sampass, const char *plaintext)
{
	...
	if (!memcmp(nt_pw, new_nt_p16, NT_HASH_LEN)) {
		DEBUG_OUT (10,("check_passwd_history: proposed new password for user %s is the same as the current password !\n",
			pdb_get_username(sampass) ));
		return True;
	}

	if (password_in_history(new_nt_p16, pwHisLen, pwhistory)) {
		DEBUG_OUT (1,("check_passwd_history: proposed new password for "
				   ...
				   enum samPwdChangeReason *samr_reject_reason)
{
	...
	DEBUG_OUT (5,("check_password_complexity: check password script (%s) "
		 "returned [%d]\n", cmd, check_ret));
	TALLOC_FREE(cmd);

	if (check_ret != 0) {
		DEBUG_OUT (1,("check_password_complexity: "
				    ...
				    enum samPwdChangeReason *samr_reject_reason)
{
	...
	if (!pdb_get_pass_can_change(hnd)) {
		DEBUG_OUT (1, ("user %s does not have permissions to change password\n", username));
		if (samr_reject_reason) {
			*samr_reject_reason = SAM_PWD_CHANGE_NO_ERROR;
		}
	...
	if (pdb_get_acct_ctrl(hnd) & ACB_WSTRUST) {
		if (pdb_get_account_policy(PDB_POLICY_REFUSE_MACHINE_PW_CHANGE, &refuse) && refuse) {
			DEBUG_OUT (1, ("Machine %s cannot change password now, "
	...
	if ((can_change_time != 0) && (time(NULL) < can_change_time)) {
		DEBUG_OUT (1, ("user %s cannot change password now, must "
			  "wait until %s\n", username,
	...
	if (pdb_get_account_policy(PDB_POLICY_MIN_PASSWORD_LEN, &min_len) && (str_charnum(new_passwd) < min_len)) {
		DEBUG_OUT (1, ("user %s cannot change password - password too short\n",
			  username));
...
/* 		return NT_STATUS_PWD_TOO_SHORT; */
	...
	if (!pass) {
		DEBUG_OUT (1, ("change_oem_password: Username %s does not exist in system !?!\n", username));
			 ...
			 enum samPwdChangeReason *reject_reason)
{
	...
	if (ret == false) {
		DEBUG_OUT (0,("pass_oem_change: getsmbpwnam returned NULL\n"));
		TALLOC_FREE(sampass);
