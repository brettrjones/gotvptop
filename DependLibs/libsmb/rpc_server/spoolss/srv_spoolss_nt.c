					  ...
					  struct printer_handle *prn_hnd)
{
	...
	if (!print_notify_deregister_pid(snum)) {
		DEBUG_OUT (0, ("Failed to register our pid for printer %s\n",
	...
	if (prn_hnd->notify.cli_chan == NULL ||
		...
		DEBUG_OUT (0, ("Trying to close unexisting backchannel!\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_spoolss_ReplyClosePrinter failed [%s].\n",
			  nt_errstr(status)));
		result = ntstatus_to_werror(status);
	} else if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("reply_close_printer failed [%s].\n",
			  win_errstr(result)));
	}
							...
							struct policy_handle *hnd)
{
	...
	if(!find_policy_by_hnd(p,hnd,(void **)(void *)&find_printer)) {
		DEBUG_OUT (2,("find_printer_index_by_hnd: Printer handle not found: "));
		return NULL;
	}
...
static bool close_printer_handle(struct pipes_struct *p, struct policy_handle *hnd)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("close_printer_handle: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(hnd)));
		return false;
	}
...
static WERROR delete_printer_handle(struct pipes_struct *p, struct policy_handle *hnd)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("delete_printer_handle: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(hnd)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG_OUT (3, ("delete_printer_handle: denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (3,("Error deleting printer %s\n", Printer->sharename));
		return WERR_BADFID;
	}
			     ...
			     int *number, struct share_params **params)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("get_printer_snum: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(hnd)));
		return false;
	}
...
{
	DEBUG_OUT (3,("Setting printer type=%s\n", handlename));
				   ...
				   const char *handlename)
{
	...
	DEBUG_OUT (4,("Setting printer name=%s (len=%lu)\n", handlename,
	...
	if ((cache_key != NULL) && gencache_get(cache_key, &tmp, NULL)) {
		...
		if (!found) {
			DEBUG_OUT (4, ("Printer %s not found\n", aprinter));
			SAFE_FREE(tmp);
			return WERR_INVALID_PRINTER_NAME;
		}
		fstrcpy(sname, tmp);
	...
	for (snum = 0; !found && snum < n_services; snum++) {
		...
		result = winreg_get_printer_internal(mem_ctx,
					    ...
					    &info2);
		if ( !W_ERROR_IS_OK(result) ) {
			DEBUG_OUT (2,("set_printer_hnd_name: failed to lookup printer [%s] -- result [%s]\n",
			       ...
			       uint32_t access_granted)
{
	...
	DEBUG_OUT (10,("open_printer_hnd: name [%s]\n", name));
	...
	DEBUG_OUT (5, ("%d printer handles active\n",
				...
				uint16_t notify_field)
{
	...
	DEBUG_OUT (10, ("Open handle for \\\\%s\\%s is not monitoring 0x%02x/0x%02x\n",
			       ...
			       TALLOC_CTX *mem_ctx)
{
	...
	if (msg->len != sizeof(time_t)) {
		DEBUG_OUT (5, ("notify_system_time: received wrong sized message (%d)\n",
			  msg->len));
		return;
	}
...
static int notify_msg_ctr_addmsg( SPOOLSS_NOTIFY_MSG_CTR *ctr, SPOOLSS_NOTIFY_MSG *msg )
{
	...
	if ( i == ctr->num_groups ) {
		...
		if ( !(groups = TALLOC_REALLOC_ARRAY( ctr->ctx, ctr->msg_groups, SPOOLSS_NOTIFY_MSG_GROUP, ctr->num_groups)) ) {
			DEBUG_OUT (0,("notify_msg_ctr_addmsg: talloc_realloc() failed!\n"));
			return 0;
		}
	...
	if ( !(msg_list = TALLOC_REALLOC_ARRAY( ctr->ctx, msg_grp->msgs, SPOOLSS_NOTIFY_MSG, msg_grp->num_msgs )) ) {
		DEBUG_OUT (0,("notify_msg_ctr_addmsg: talloc_realloc() failed for new message [%d]!\n", msg_grp->num_msgs));
		return 0;
	}
				  ...
				  int *_count)
{
	...
	for (i = 0; i < num_msgs; i++) {
		...
		DEBUG_OUT (10, ("Sending message type [0x%x] field [0x%2x] "
			   "for printer [%s]\n",
		...
		if (msg->flags & SPOOLSS_NOTIFY_MSG_UNIX_JOBID) {
			...
			if (id == -1) {
				DEBUG_OUT (3, ("no such unix jobid %d\n",
					  msg->id));
				continue;
			}
		}
		...
		default:
			DEBUG_OUT (5, ("Unknown notification type %d\n",
				  msg->type));
			continue;
		}
				...
				SPOOLSS_NOTIFY_MSG_GROUP *msg_group)
{
	...
	DEBUG_OUT (10, ("Client connected! [\\\\%s\\%s]\n",
	...
	status = dcerpc_spoolss_RouterReplyPrinterEx(
				...
				info, &werr);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("dcerpc_spoolss_RouterReplyPrinterEx to client: %s "
			  "failed: %s\n",
			  prn_hnd->notify.cli_chan->cli_pipe->srv_name_slash,
			  nt_errstr(status)));
		werr = ntstatus_to_werror(status);
	} else if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, ("RouterReplyPrinterEx to client: %s "
			  "failed: %s\n",
			  prn_hnd->notify.cli_chan->cli_pipe->srv_name_slash,
			  win_errstr(werr)));
	}
...
static void send_notify2_changes( SPOOLSS_NOTIFY_MSG_CTR *ctr, uint32_t idx )
{
	...
	if ( !msg_group ) {
		DEBUG_OUT (5,("send_notify2_changes() called with no msg group!\n"));
		return;
	}

	if (!msg_group->msgs) {
		DEBUG_OUT (5, ("send_notify2_changes() called with no messages!\n"));
		return;
	}

	DEBUG_OUT (8,("send_notify2_changes: Enter...[%s]\n", msg_group->printername));
	...
	DEBUG_OUT (8,("send_notify2_changes: Exit...\n"));
	return;
}
...
static bool notify2_unpack_msg( SPOOLSS_NOTIFY_MSG *msg, struct timeval *tv, void *buf, size_t len )
{
	...
	DEBUG_OUT (3, ("notify2_unpack_msg: got NOTIFY2 message for printer %s, jobid %u type %d, field 0x%02x, flags 0x%04x\n",
	...
	if (msg->len == 0)
		DEBUG_OUT (3, ("notify2_unpack_msg: value1 = %d, value2 = %d\n", msg->notify.value[0],
			  msg->notify.value[1]));
	else
					 ...
					 DATA_BLOB *data)
{
	...
	if (data->length < 4) {
		DEBUG_OUT (0,("receive_notify2_message_list: bad message format (len < 4)!\n"));
		return;
	}
	...
	DEBUG_OUT (5, ("receive_notify2_message_list: got %lu messages in list\n", (unsigned long)msg_count));

	if (msg_count == 0) {
		DEBUG_OUT (0,("receive_notify2_message_list: bad message format (msg_count == 0) !\n"));
		return;
	}
	...
	for ( i=0; i<msg_count; i++ ) {
		...
		if (msg_ptr + 4 - buf > data->length) {
			DEBUG_OUT (0,("receive_notify2_message_list: bad message format (len > buf_size) !\n"));
			return;
		}
		...
		if (msg_ptr + msg_len - buf > data->length) {
			DEBUG_OUT (0,("receive_notify2_message_list: bad message format (bad len) !\n"));
			return;
		}
	...
	DEBUG_OUT (10,("receive_notify2_message_list: processed %u messages\n",
		(uint32_t)msg_count ));
					    ...
					    struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (10,("srv_spoolss_drv_upgrade_printer: Sending message about driver upgrade [%s]\n",
		drivername));
			    ...
			    DATA_BLOB *data)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("do_drv_upgrade_printer: "
	...
	if (!drivername) {
		DEBUG_OUT (0, ("do_drv_upgrade_printer: Out of memoery ?!\n"));
		goto done;
	}

	DEBUG_OUT (10, ("do_drv_upgrade_printer: "
	...
	for (snum = 0; snum < n_services; snum++) {
		...
		DEBUG_OUT (6,("Updating printer [%s]\n", pinfo2->printername));
		...
		if (!W_ERROR_IS_OK(result)) {
			DEBUG_OUT (3, ("do_drv_upgrade_printer: "
			      ...
			      struct spoolss_OpenPrinterEx *r)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0,("_spoolss_OpenPrinterEx: Cannot open a printer handle "
			"for printer %s\n", r->in.printername));
	...
	if ( !Printer ) {
		DEBUG_OUT (0,("_spoolss_OpenPrinterEx: logic error.  Can't find printer "
	...
	case SPLHND_PORTMON_LOCAL:
		...
		if (r->in.access_mask &
			...
			DEBUG_OUT (3, ("access DENIED for non-printserver bits\n"));
			close_printer_handle(p, r->out.handle);
			ZERO_STRUCTP(r->out.handle);
			return WERR_ACCESS_DENIED;
		}
		...
		if ( r->in.access_mask & SERVER_ACCESS_ADMINISTER )
		{
			...
			if ((p->session_info->utok.uid != sec_initial_uid()) &&
				...
				close_printer_handle(p, r->out.handle);
				ZERO_STRUCTP(r->out.handle);
				DEBUG_OUT (3,("access DENIED as user is not root, "
		...
		DEBUG_OUT (4,("Setting print server access = %s\n", (r->in.access_mask == SERVER_ACCESS_ADMINISTER)
			? "SERVER_ACCESS_ADMINISTER" : "SERVER_ACCESS_ENUMERATE" ));
	...
	case SPLHND_PRINTER:
		...
		if (!allow_access(lp_hostsdeny(snum), lp_hostsallow(snum),
			...
			DEBUG_OUT (3, ("access DENIED (hosts allow/deny) for printer open\n"));
			ZERO_STRUCTP(r->out.handle);
			return WERR_ACCESS_DENIED;
		}
		...
		if (!user_ok_token(uidtoname(p->session_info->utok.uid), NULL,
		    ...
		    !print_access_check(p->session_info,
					...
					snum,
					r->in.access_mask)) {
			DEBUG_OUT (3, ("access DENIED for printer open\n"));
			close_printer_handle(p, r->out.handle);
			ZERO_STRUCTP(r->out.handle);
			return WERR_ACCESS_DENIED;
		}
		...
		if ((r->in.access_mask & SEC_MASK_SPECIFIC)& ~(PRINTER_ACCESS_ADMINISTER|PRINTER_ACCESS_USE)) {
			DEBUG_OUT (3, ("access DENIED for printer open - unknown bits\n"));
			close_printer_handle(p, r->out.handle);
			ZERO_STRUCTP(r->out.handle);
			return WERR_ACCESS_DENIED;
		}
		...
		DEBUG_OUT (4,("Setting printer access = %s\n", (r->in.access_mask == PRINTER_ACCESS_ADMINISTER)
				    ...
				    struct spoolss_DeletePrinterDriver *r)
{
	...
	for (found = false, i = 0; drv_cversion[i] >= 0; i++) {
		status = winreg_get_driver(tmp_ctx, b,
					   ...
					   drv_cversion[i], &info);
		if (!W_ERROR_IS_OK(status)) {
			DEBUG_OUT (5, ("skipping del of driver with version %d\n",
				  drv_cversion[i]));
			continue;
		}
		found = true;
		...
		if (!W_ERROR_IS_OK(status)) {
			DEBUG_OUT (0, ("failed del of driver with version %d\n",
				  drv_cversion[i]));
			goto done;
		}
	}
	if (found == false) {
		DEBUG_OUT (0, ("driver %s not found for deletion\n", r->in.driver));
				      ...
				      struct spoolss_DeletePrinterDriverEx *r)
{
	...
	for (found = false, i = 0; drv_cversion[i] >= 0; i++) {
		...
		if (!W_ERROR_IS_OK(status)) {
			DEBUG_OUT (0, ("failed to delete driver with version %d\n",
				  drv_cversion[i]));
			goto done;
		}
	}
...
{
	DEBUG_OUT (8,("getprinterdata_printer_server:%s\n", value));
			...
			struct sockaddr_storage *client_ss, const char *remote_machine)
{
	...
	if ( is_zero_addr(client_ss) ) {
		DEBUG_OUT (2,("spoolss_connect_to_client: resolving %s\n",
			remote_machine));
		if ( !resolve_name( remote_machine, &rm_addr, 0x20, false) ) {
			DEBUG_OUT (2,("spoolss_connect_to_client: Can't resolve address for %s\n", remote_machine));
			return false;
		}
		...
		rm_addr = *client_ss;
		print_sockaddr(addr, sizeof(addr), &rm_addr);
		DEBUG_OUT (5,("spoolss_connect_to_client: Using address %s (no name resolution necessary)\n",
			addr));
	}

	if (ismyaddr((struct sockaddr *)(void *)&rm_addr)) {
		DEBUG_OUT (0,("spoolss_connect_to_client: Machine %s is one of our addresses. Cannot add to ourselves.\n",
			addr));
		return false;
	}
	...
	if ( !NT_STATUS_IS_OK( ret ) ) {
		DEBUG_OUT (2,("spoolss_connect_to_client: connection to [%s] failed!\n",
			remote_machine ));
		return false;
	}

	if ( the_cli->protocol != PROTOCOL_NT1 ) {
		DEBUG_OUT (0,("spoolss_connect_to_client: machine %s didn't negotiate NT protocol.\n", remote_machine));
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (2,("spoolss_connect_to_client: unable to open the spoolss pipe on machine %s. Error was : %s.\n",
					...
					struct messaging_context *msg_ctx)
{
	...
	status = dcerpc_spoolss_ReplyOpenPrinter(chan->binding_handle,
						 ...
						 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("dcerpc_spoolss_ReplyOpenPrinter returned [%s]\n", nt_errstr(status)));
		result = ntstatus_to_werror(status);
	} else if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (5, ("ReplyOpenPrinter returned [%s]\n", win_errstr(result)));
	}
						     ...
						     struct spoolss_RemoteFindFirstPrinterChangeNotifyEx *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_RemoteFindFirstPrinterChangeNotifyEx: "
	...
	DEBUG_OUT (10,("_spoolss_RemoteFindFirstPrinterChangeNotifyEx: "
		"client_address is %s\n", p->client_id->addr));
								  ...
								  uint16_t field)
{
	...
	DEBUG_OUT (5, ("invalid notify data type %d/%d\n", type, field));
					  ...
					  TALLOC_CTX *mem_ctx)
{
	...
	DEBUG_OUT (4,("construct_notify_printer_info: Notify type: [%s], number of notify info: [%d] on printer: [%s]\n",
	...
	for(field_num=0; field_num < option_type->count; field_num++) {
		...
		DEBUG_OUT (4,("construct_notify_printer_info: notify [%d]: type [%x], field [%x]\n", field_num, type, field));
		...
		if (info->notifies == NULL) {
			DEBUG_OUT (2,("construct_notify_printer_info: failed to enlarge buffer info->data!\n"));
			return false;
		}
		...
		DEBUG_OUT (10, ("construct_notify_printer_info: "
				       ...
				       TALLOC_CTX *mem_ctx)
{
	...
	DEBUG_OUT (4,("construct_notify_jobs_info\n"));
	...
	for(field_num=0; field_num<option_type->count; field_num++) {
		...
		info->notifies = TALLOC_REALLOC_ARRAY(info, info->notifies,
						      ...
						      info->count + 1);
		if (info->notifies == NULL) {
			DEBUG_OUT (2,("construct_notify_jobs_info: failed to enlarg buffer info->data!\n"));
			return false;
		}
				      ...
				      TALLOC_CTX *mem_ctx)
{
	...
	DEBUG_OUT (4,("printserver_notify_info\n"));
	...
	DEBUG_OUT (1,("dumping the NOTIFY_INFO\n"));
				  ...
				  TALLOC_CTX *mem_ctx)
{
	...
	DEBUG_OUT (4,("printer_notify_info\n"));
						 ...
						 struct spoolss_RouterRefreshPrinterChangeNotify *r)
{
	...
	*r->out.info = info;
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_RouterRefreshPrinterChangeNotify: "
			"Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		goto done;
	}

	DEBUG_OUT (4,("Printer type %x\n",Printer->printer_type));
	...
	if (Printer->notify.cli_chan != NULL &&
	    Printer->notify.cli_chan->active_connections > 0) {
		DEBUG_OUT (10,("_spoolss_RouterRefreshPrinterChangeNotify: "
				      ...
				      int snum)
{
	...
	if (info2->devmode != NULL) {
		...
		r->devmode = NULL;
		DEBUG_OUT (8,("Returning NULL Devicemode!\n"));
	}
				      ...
				      int snum)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("construct_printer_info7: "
			  "Could not create system session_info\n"));
		return WERR_NOMEM;
	}
					   ...
					   uint32_t *count_p)
{
	...
	*info_p = NULL;
	...
	for (snum = 0; snum < n_services; snum++) {
		...
		DEBUG_OUT (4,("Found a printer in smb.conf: %s[%x]\n",
			printer, snum));
...
static WERROR enumprinters_level0(TALLOC_CTX *mem_ctx,
				  ...
				  uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_0\n"));
...
static WERROR enum_all_printers_info_1(TALLOC_CTX *mem_ctx,
				       ...
				       uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_1\n"));
...
static WERROR enum_all_printers_info_1_local(TALLOC_CTX *mem_ctx,
					     ...
					     uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_1_local\n"));
					    ...
					    uint32_t *count)
{
	...
	DEBUG_OUT (4,("enum_all_printers_info_1_name\n"));
					       ...
					       uint32_t *count)
{
	...
	DEBUG_OUT (4,("enum_all_printers_info_1_network\n"));
...
static WERROR enum_all_printers_info_2(TALLOC_CTX *mem_ctx,
				       ...
				       uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_2\n"));
...
static WERROR enumprinters_level4(TALLOC_CTX *mem_ctx,
				  ...
				  uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_4\n"));
...
static WERROR enumprinters_level5(TALLOC_CTX *mem_ctx,
				  ...
				  uint32_t *count)
{
	DEBUG_OUT (4,("enum_all_printers_info_5\n"));
			     ...
			     struct spoolss_EnumPrinters *r)
{
	...
	DEBUG_OUT (4,("_spoolss_EnumPrinters\n"));
			   ...
			   struct spoolss_GetPrinter *r)
{
 ...
 out:
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("_spoolss_GetPrinter: failed to construct printer info level %d - %s\n",
						  ...
						  uint32_t version)
{
	...
	DEBUG_OUT (8,("construct_printer_driver_info_level: status: %s\n",
		win_errstr(result)));
	...
	if (!W_ERROR_IS_OK(result)) {
		...
		result = winreg_get_driver(mem_ctx, b,
					   ...
					   pinfo2->drivername,
					   version, &driver);
		DEBUG_OUT (8,("construct_printer_driver_level: status: %s\n",
			win_errstr(result)));
		if (!W_ERROR_IS_OK(result)) {
			talloc_free(pinfo2);
			return WERR_UNKNOWN_PRINTER_DRIVER;
				  ...
				  struct spoolss_GetPrinterDriver2 *r)
{
	...
	DEBUG_OUT (4,("_spoolss_GetPrinterDriver2\n"));

	if (!(printer = find_printer_index_by_hnd(p, r->in.handle))) {
		DEBUG_OUT (0,("_spoolss_GetPrinterDriver2: invalid printer handle!\n"));
		return WERR_INVALID_PRINTER_NAME;
	}
				 ...
				 struct spoolss_StartPagePrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (3,("_spoolss_StartPagePrinter: "
			       ...
			       struct spoolss_EndPagePrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_EndPagePrinter: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
				...
				struct spoolss_StartDocPrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_StartDocPrinter: "
			"Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
			      ...
			      struct spoolss_EndDocPrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_EndDocPrinter: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("_spoolss_EndDocPrinter: "
			     ...
			     struct spoolss_WritePrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_WritePrinter: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(r->in.handle)));
			      ...
			      struct pipes_struct *p)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("control_printer: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
			     ...
			     struct spoolss_AbortPrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_AbortPrinter: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
				 ...
				 struct sec_desc_buf *secdesc_ctr)
{
	...
	if (!Printer || !get_printer_snum(p, handle, &snum, NULL)) {
		DEBUG_OUT (2,("update_printer_sec: Invalid handle (%s:%u:%u)\n",
			 OUR_HANDLE(handle)));
	...
	if (secdesc_ctr == NULL) {
		DEBUG_OUT (10,("update_printer_sec: secdesc_ctr is NULL !\n"));
	...
	if ( Printer->access_granted != PRINTER_ACCESS_ADMINISTER ) {
		DEBUG_OUT (4,("update_printer_sec: updated denied by printer permissions\n"));
	...
	result = winreg_get_printer_secdesc(p->mem_ctx, b,
					    ...
					    &old_secdesc);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2,("update_printer_sec: winreg_get_printer_secdesc_internal() failed\n"));
		result = WERR_BADFID;
		goto done;
	}
	...
	if (DEBUGLEVEL >= 10) {
		...
		DEBUG_OUT (10, ("old_secdesc_ctr for %s has %d aces:\n",
			   printer, the_acl->num_aces));

		for (i = 0; i < the_acl->num_aces; i++) {
			DEBUG_OUT (10, ("%s 0x%08x\n", sid_string_dbg(
		...
		if (the_acl) {
			DEBUG_OUT (10, ("secdesc_ctr for %s has %d aces:\n",
				   printer, the_acl->num_aces));
			...
			DEBUG_OUT (10, ("dacl for secdesc_ctr is NULL\n"));
		}
	}
			     ...
			     int snum)
{
	...
	DEBUG_OUT (5,("check_printer_ok: servername=%s printername=%s sharename=%s "
			       ...
			       struct spoolss_PrinterInfo2 *old_printer)
{
	...
	if (force_update || !strequal(printer->drivername, old_printer->drivername)) {
		...
		if (!force_update) {
			DEBUG_OUT (10,("update_printer: changing driver [%s]!  Sending event!\n",
				printer->drivername));
			     ...
			     struct spoolss_DeviceMode *devmode)
{
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG_OUT (3, ("update_printer: printer property change denied by handle\n"));
		result = WERR_ACCESS_DENIED;
		goto done;
	}
					   ...
					   struct spoolss_SetPrinterInfo7 *info7)
{
	...
	DEBUG_OUT (5,("publish_or_unpublish_printer, action = %d\n",info7->action));
				     ...
				     struct spoolss_DeviceMode *devmode)
{
	...
	DEBUG_OUT (8,("update_printer_devmode\n"));
			   ...
			   struct spoolss_SetPrinter *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_SetPrinter: Invalid handle (%s:%u:%u)\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
				       ...
				       struct spoolss_FindClosePrinterNotify *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_FindClosePrinterNotify: "
			"Invalid handle (%s:%u:%u)\n", OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	*count = num_queues;
	...
	for (i=0; i<*count; i++) {
		...
		result = spoolss_create_default_devmode(info,
							...
							&devmode);
		if (!W_ERROR_IS_OK(result)) {
			DEBUG_OUT (3, ("Can't proceed w/o a devmode!"));
			goto out;
		}
						       ...
						       uint32_t *count_p)
{
	...
	*info_p = NULL;
	...
	for (version=0; version<DRIVER_MAX_VERSION; version++) {
		...
		DEBUG_OUT (4, ("we have:[%d] drivers in environment"
			  " [%s] and version [%d]\n",
		...
		if (num_drivers != 0) {
			info = TALLOC_REALLOC_ARRAY(mem_ctx, info,
						    ...
						    count + num_drivers);
			if (!info) {
				DEBUG_OUT (0,("enumprinterdrivers_level_by_architecture: "
				   ...
				   struct spoolss_EnumPrinterDrivers *r)
{
	...
	DEBUG_OUT (4,("_spoolss_EnumPrinterDrivers\n"));
			  ...
			  struct spoolss_EnumForms *r)
{
	...
	*r->out.info = NULL;
	...
	DEBUG_OUT (4,("_spoolss_EnumForms\n"));
				...
				uint32_t *count)
{
	...
	if (numlines) {
		...
		if (!info) {
			DEBUG_OUT (10,("Returning WERR_NOMEM\n"));
			result = WERR_NOMEM;
			goto out;
		}

		for (i=0; i<numlines; i++) {
			DEBUG_OUT (6,("Filling port number [%d] with port [%s]\n", i, qlines[i]));
			  ...
			  struct spoolss_EnumPorts *r)
{
	...
	DEBUG_OUT (4,("_spoolss_EnumPorts\n"));
					   ...
					   struct policy_handle *handle)
{
	...
	if ((info2->sharename == NULL || info2->sharename[0] == '\0') &&
		...
		DEBUG_OUT (5, ("spoolss_addprinterex_level_2: "
	...
	if ((snum = print_queue_snum(info2->sharename)) != -1) {
		DEBUG_OUT (5, ("spoolss_addprinterex_level_2: Attempted to add a printer named [%s] when one already existed!\n",
			info2->sharename));
	...
	if (*lp_addprinter_cmd() ) {
		...
		DEBUG_OUT (0,("spoolss_addprinterex_level_2: add printer for printer %s called and no "
				   ...
				   struct spoolss_AddPrinterDriverEx *r)
{
	...
	if (r->in.info_ctr->level != 3 && r->in.info_ctr->level != 6) {
		 ...
		 */
		DEBUG_OUT (0,("%s: level %d not yet implemented\n", fn,
			r->in.info_ctr->level));
		return WERR_UNKNOWN_LEVEL;
	}

	DEBUG_OUT (5,("Cleaning driver's information\n"));
	err = clean_up_driver_struct(p->mem_ctx, p->session_info, r->in.info_ctr);
	if (!W_ERROR_IS_OK(err))
		goto done;

	DEBUG_OUT (5,("Moving driver to final destination\n"));
	...
	if (!srv_spoolss_drv_upgrade_printer(driver_name, p->msg_ctx)) {
		DEBUG_OUT (0,("%s: Failed to send message about upgrading driver [%s]!\n",
			fn, driver_name));
	}
					  ...
					  struct spoolss_DriverDirectoryInfo1 *r)
{
	...
	DEBUG_OUT (4,("printer driver directory: [%s]\n", path));
					  ...
					  struct spoolss_GetPrinterDriverDirectory *r)
{
	...
	DEBUG_OUT (5,("_spoolss_GetPrinterDriverDirectory: level %d\n",
		r->in.level));
				...
				struct spoolss_EnumPrinterData *r)
{
	...
	if (!r->in.value_offered && !r->in.data_offered) {
		...
		for (i=0; i<count; i++) {
			...
			DEBUG_OUT (6,("current values: [%d], [%d]\n", biggest_valuesize,
				biggest_datasize));
		}
			     ...
			     struct spoolss_ResetPrinter *r)
{
	...
	DEBUG_OUT (5,("_spoolss_ResetPrinter\n"));
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_ResetPrinter: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
			...
			struct spoolss_AddForm *r)
{
	...
	DEBUG_OUT (5,("_spoolss_AddForm\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_AddForm: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if ((p->session_info->utok.uid != sec_initial_uid()) &&
	    ...
	    !token_contains_name_in_list(uidtoname(p->session_info->utok.uid),
					  ...
					  p->session_info->security_token,
					  lp_printer_admin(snum))) {
		DEBUG_OUT (2,("_spoolss_Addform: denied by insufficient permissions.\n"));
		return WERR_ACCESS_DENIED;
	}
			   ...
			   struct spoolss_DeleteForm *r)
{
	...
	DEBUG_OUT (5,("_spoolss_DeleteForm\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_DeleteForm: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if ((p->session_info->utok.uid != sec_initial_uid()) &&
	    ...
	    !token_contains_name_in_list(uidtoname(p->session_info->utok.uid),
					  ...
					  p->session_info->security_token,
					  lp_printer_admin(snum))) {
		DEBUG_OUT (2,("_spoolss_DeleteForm: denied by insufficient permissions.\n"));
		return WERR_ACCESS_DENIED;
	}
			...
			struct spoolss_SetForm *r)
{
	...
	DEBUG_OUT (5,("_spoolss_SetForm\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_SetForm: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if ((p->session_info->utok.uid != sec_initial_uid()) &&
	     ...
	     !token_contains_name_in_list(uidtoname(p->session_info->utok.uid),
					  ...
					  p->session_info->security_token,
					  lp_printer_admin(snum))) {
		DEBUG_OUT (2,("_spoolss_Setform: denied by insufficient permissions.\n"));
		return WERR_ACCESS_DENIED;
	}
				    ...
				    struct spoolss_EnumPrintProcessors *r)
{
	...
	DEBUG_OUT (5,("_spoolss_EnumPrintProcessors\n"));
				       ...
				       struct spoolss_EnumPrintProcDataTypes *r)
{
	...
	DEBUG_OUT (5,("_spoolss_EnumPrintProcDataTypes\n"));
			     ...
			     struct spoolss_EnumMonitors *r)
{
	...
	DEBUG_OUT (5,("_spoolss_EnumMonitors\n"));
				 ...
				 struct spoolss_GetPrinterDataEx *r)
{
	...
	DEBUG_OUT (4,("_spoolss_GetPrinterDataEx\n"));

	DEBUG_OUT (10, ("_spoolss_GetPrinterDataEx: key => [%s], value => [%s]\n",
	...
	*r->out.type	= REG_NONE;
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_GetPrinterDataEx: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		result = WERR_BADFID;
		goto done;
	}
				 ...
				 struct spoolss_SetPrinterDataEx *r)
{
	...
	DEBUG_OUT (4,("_spoolss_SetPrinterDataEx\n"));

        /* From MSDN documentation of SetPrinterDataEx: pass request to
           SetPrinterData if key is "PrinterDriverData" */

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_SetPrinterDataEx: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}

	if (Printer->printer_type == SPLHND_SERVER) {
		DEBUG_OUT (10,("_spoolss_SetPrinterDataEx: "
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG_OUT (3, ("_spoolss_SetPrinterDataEx: "
				    ...
				    struct spoolss_DeletePrinterDataEx *r)
{
	...
	DEBUG_OUT (5,("_spoolss_DeletePrinterDataEx\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_DeletePrinterDataEx: "
			"Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG_OUT (3, ("_spoolss_DeletePrinterDataEx: "
			       ...
			       struct spoolss_EnumPrinterKey *r)
{
	...
	DEBUG_OUT (4,("_spoolss_EnumPrinterKey\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_EnumPrinterKey: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
				 ...
				 struct spoolss_DeletePrinterKey *r)
{
	...
	DEBUG_OUT (5,("_spoolss_DeletePrinterKey\n"));

	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_DeletePrinterKey: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG_OUT (3, ("_spoolss_DeletePrinterKey: "
			"printer properties change denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
				  ...
				  struct spoolss_EnumPrinterDataEx *r)
{
	...
	DEBUG_OUT (4,("_spoolss_EnumPrinterDataEx\n"));
	...
	*r->out.info = NULL;
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_EnumPrinterDataEx: Invalid handle (%s:%u:%u1<).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
						 ...
						 struct spoolss_PrintProcessorDirectoryInfo1 *r)
{
	...
	DEBUG_OUT (4,("print processor directory: [%s]\n", path));
					   ...
					   struct spoolss_GetPrintProcessorDirectory *r)
{
	...
	DEBUG_OUT (5,("_spoolss_GetPrintProcessorDirectory: level %d\n",
		r->in.level));
				     ...
				     uint32_t *needed )
{
	...
	DEBUG_OUT (10,("process_xcvtcp_command: Received command \"%s\"\n", command));
				       ...
				       uint32_t *needed)
{
	...
	DEBUG_OUT (10,("process_xcvlocal_command: Received command \"%s\"\n", command));
			...
			struct spoolss_XcvData *r)
{
	...
	if (!Printer) {
		DEBUG_OUT (2,("_spoolss_XcvData: Invalid handle (%s:%u:%u).\n",
			OUR_HANDLE(r->in.handle)));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->printer_type & (SPLHND_PORTMON_LOCAL|SPLHND_PORTMON_TCP)) ) {
		DEBUG_OUT (2,("_spoolss_XcvData: Call only valid for Port Monitors\n"));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->access_granted & SERVER_ACCESS_ADMINISTER) ) {
		DEBUG_OUT (2,("_spoolss_XcvData: denied by handle permissions.\n"));
		return WERR_ACCESS_DENIED;
	}
