				  ...
				  struct gse_context **ctx)
{
	...
	status = gse_get_server_auth_token(mem_ctx, gse_ctx,
					   token_in, token_out);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to parse initial client token (%s)\n",
			  nt_errstr(status)));
		goto done;
	}
				     ...
				     struct auth_serversupplied_info **server_info)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (1, ("Failed to parse the PAC for %s\n", princ_name));
	...
	if (!logon_info) {
		DEBUG_OUT (1, ("Invalid PAC data, missing logon info!\n"));
	...
	status = get_user_from_kerberos_info(tmp_ctx, client_id->name,
					     ...
					     &username, &pw);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to map kerberos principal to system user "
			  "(%s)\n", nt_errstr(status)));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to map kerberos pac to server info (%s)\n",
			  nt_errstr(status)));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}

	DEBUG_OUT (5, (__location__ "OK: user: %s domain: %s client: %s\n",
