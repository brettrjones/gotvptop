...
static int pipe_enum_fn( struct db_record *rec, void *p)
{
	...
	if ( !f ) {
		DEBUG_OUT (0,("conn_enum_fn: realloc failed for %d items\n", i+1));
		return 1;
	}
	fenum->ctr3->array = f;
			     ...
			     uint32_t resume )
{
	...
	if (connections_traverse(pipe_enum_fn, &fenum) == -1) {
		DEBUG_OUT (0,("net_enum_pipes: traverse of connections.tdb "
			 "failed\n"));
		return WERR_NOMEM;
	}
				      ...
				      bool all_shares)
{
	...
	DEBUG_OUT (5,("init_srv_share_info_ctr\n"));
        ...
        for (snum = 0; snum < num_services; snum++) {
                if (lp_browseable(snum) && lp_snum_ok(snum) &&
                    ...
                    (all_shares || !is_hidden_share(snum)) ) {
                        DEBUG_OUT (10, ("counting service %s\n",
				lp_servicename(snum) ? lp_servicename(snum) : "(null)"));
                        allowed[snum] = true;
                        num_entries++;
                } else {
                        DEBUG_OUT (10, ("NOT counting service %s\n",
				lp_servicename(snum) ? lp_servicename(snum) : "(null)"));
                }
        }
	...
	default:
		DEBUG_OUT (5,("init_srv_share_info_ctr: unsupported switch value %d\n",
			info_ctr->level));
				   ...
				   uint32_t *total_entries)
{
	...
	*total_entries = list_sessions(p->mem_ctx, &session_list);
	...
	DEBUG_OUT (5,("init_srv_sess_info_0\n"));
				   ...
				   uint32_t *total_entries)
{
	...
	*total_entries = list_sessions(p->mem_ctx, &session_list);
	...
	for (; resume_handle < *total_entries; resume_handle++) {
		...
		if ( !pw ) {
			DEBUG_OUT (10,("init_srv_sess_info_1: failed to find owner: %s\n",
				   ...
				   uint32_t *total_entries)
{
	...
	DEBUG_OUT (5,("init_srv_conn_info_0\n"));
				   ...
				   uint32_t *total_entries)
{
	...
	DEBUG_OUT (5,("init_srv_conn_info_1\n"));
			   ...
			   struct srvsvc_NetFileEnum *r)
{
	...
	if (!nt_token_check_sid(&global_sid_Builtin_Administrators,
		...
		DEBUG_OUT (1, ("Enumerating files only allowed for "
			  "administrators\n"));
			     ...
			     struct srvsvc_NetSrvGetInfo *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetSrvGetInfo: %d\n", __LINE__));

	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to _srvsvc_NetSrvGetInfo\n"));
		return WERR_ACCESS_DENIED;
	}
			     ...
			     struct srvsvc_NetSrvSetInfo *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetSrvSetInfo: %d\n", __LINE__));
			   ...
			   struct srvsvc_NetConnEnum *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetConnEnum: %d\n", __LINE__));

	if (!nt_token_check_sid(&global_sid_Builtin_Administrators,
				p->session_info->security_token)) {
		DEBUG_OUT (1, ("Enumerating connections only allowed for "
			  "administrators\n"));
		return WERR_ACCESS_DENIED;
	}
			   ...
			   struct srvsvc_NetSessEnum *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetSessEnum: %d\n", __LINE__));

	if (!nt_token_check_sid(&global_sid_Builtin_Administrators,
				p->session_info->security_token)) {
		DEBUG_OUT (1, ("Enumerating sessions only allowed for "
			  "administrators\n"));
		return WERR_ACCESS_DENIED;
	}
			  ...
			  struct srvsvc_NetSessDel *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetSessDel: %d\n", __LINE__));
			       ...
			       struct srvsvc_NetShareEnumAll *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareEnumAll: %d\n", __LINE__));

	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to _srvsvc_NetShareEnumAll\n"));
		return WERR_ACCESS_DENIED;
	}
			    ...
			    struct srvsvc_NetShareEnum *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareEnum: %d\n", __LINE__));

	if (!pipe_access_check(p)) {
		DEBUG_OUT (3, ("access denied to _srvsvc_NetShareEnum\n"));
		return WERR_ACCESS_DENIED;
	}
			       ...
			       struct srvsvc_NetShareGetInfo *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareGetInfo: %d\n", __LINE__));
	...
	switch (r->in.level) {
		...
		case 1501:
			...
			break;
		default:
			DEBUG_OUT (5,("_srvsvc_NetShareGetInfo: unsupported switch value %d\n",
				r->in.level));
			status = WERR_UNKNOWN_LEVEL;
			break;
	}
			       ...
			       struct srvsvc_NetShareSetInfo *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareSetInfo: %d\n", __LINE__));
	...
	{
		DEBUG_OUT (5,("_srvsvc_NetShareSetInfo: share %s cannot be "
	...
	if ( p->session_info->utok.uid != sec_initial_uid() && !is_disk_op ) {
		DEBUG_OUT (2,("_srvsvc_NetShareSetInfo: uid %u doesn't have the "
	...
	case 1005:
		...
		if (((info->info1005->dfs_flags &
			...
			DEBUG_OUT (3, ("_srvsvc_NetShareSetInfo: client is trying to change csc policy from the network; must be done with smb.conf\n"));
			return WERR_ACCESS_DENIED;
		}
	case 1006:
	case 1007:
	...
	case 1501:
		...
		break;
	default:
		DEBUG_OUT (5,("_srvsvc_NetShareSetInfo: unsupported switch value %d\n",
			r->in.level));
		return WERR_UNKNOWN_LEVEL;
	}
	...
	if (type != STYPE_DISKTREE) {
		DEBUG_OUT (5,("_srvsvc_NetShareSetInfo: share %s is not a "
			"disk share\n",
			share_name ));
		return WERR_ACCESS_DENIED;
	}
	...
	if (!(path = valid_share_pathname(p->mem_ctx, pathname ))) {
		DEBUG_OUT (5,("_srvsvc_NetShareSetInfo: invalid pathname %s\n",
			pathname ));
	...
	DEBUG_OUT (10,("_srvsvc_NetShareSetInfo: change share command = %s\n",
	...
	if (strcmp(path, lp_pathname(snum)) || strcmp(comment, lp_comment(snum))
		...
		if (!lp_change_share_cmd() || !*lp_change_share_cmd()) {
			DEBUG_OUT (10,("_srvsvc_NetShareSetInfo: No change share command\n"));
			return WERR_ACCESS_DENIED;
		}
		...
		DEBUG_OUT (10,("_srvsvc_NetShareSetInfo: Running [%s]\n", command ));
		...
		DEBUG_OUT (3,("_srvsvc_NetShareSetInfo: Running [%s] returned (%d)\n",
			command, ret ));
		...
		DEBUG_OUT (10,("_srvsvc_NetShareSetInfo: No change to share name (%s)\n",
			share_name ));
	}
	...
	if (psd) {
		...
		if (old_sd && !security_descriptor_equal(old_sd, psd)) {
			if (!set_share_security(share_name, psd))
				DEBUG_OUT (0,("_srvsvc_NetShareSetInfo: Failed to change security info in share %s.\n",
					share_name ));
		}
	}
			   ...
			   struct srvsvc_NetShareAdd *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareAdd: %d\n", __LINE__));
	...
	if (!lp_add_share_cmd() || !*lp_add_share_cmd()) {
		DEBUG_OUT (10,("_srvsvc_NetShareAdd: No add share command\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	case 1501:
		...
		return WERR_ACCESS_DENIED;
	default:
		DEBUG_OUT (5,("_srvsvc_NetShareAdd: unsupported switch value %d\n",
			r->in.level));
		return WERR_UNKNOWN_LEVEL;
	}
	...
	if (!share_name_in || !validate_net_name(share_name_in,
		...
		DEBUG_OUT (5,("_srvsvc_NetShareAdd: Bad sharename \"%s\"\n",
	...
	DEBUG_OUT (10,("_srvsvc_NetShareAdd: Running [%s]\n", command ));
	...
	DEBUG_OUT (3,("_srvsvc_NetShareAdd: Running [%s] returned (%d)\n",
		command, ret ));
	...
	if (psd) {
		...
		if (!set_share_security(share_name, psd)) {
			DEBUG_OUT (0,("_srvsvc_NetShareAdd: Failed to add security info to share %s.\n",
				share_name ));
		}
	}
			   ...
			   struct srvsvc_NetShareDel *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareDel: %d\n", __LINE__));
	...
	if (!lp_delete_share_cmd() || !*lp_delete_share_cmd()) {
		DEBUG_OUT (10,("_srvsvc_NetShareDel: No delete share command\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	DEBUG_OUT (10,("_srvsvc_NetShareDel: Running [%s]\n", command ));
	...
	DEBUG_OUT (3,("_srvsvc_NetShareDel: Running [%s] returned (%d)\n", command, ret ));
				 ...
				 struct srvsvc_NetShareDelSticky *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetShareDelSticky: %d\n", __LINE__));
			    ...
			    struct srvsvc_NetRemoteTOD *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetRemoteTOD: %d\n", __LINE__));
				  ...
				  struct srvsvc_NetGetFileSecurity *r)
{
	...
	if (snum == -1) {
		DEBUG_OUT (10, ("Could not find service %s\n", servicename));
		werr = WERR_NET_NAME_NOT_FOUND;
		goto error_exit;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (10, ("create_conn_struct failed: %s\n",
			   nt_errstr(nt_status)));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("_srvsvc_NetGetFileSecurity: can't open %s\n",
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("_srvsvc_NetGetFileSecurity: Unable to get NT ACL "
			"for file %s\n", smb_fname_str_dbg(smb_fname)));
		werr = ntstatus_to_werror(nt_status);
		goto error_exit;
	}
				  ...
				  struct srvsvc_NetSetFileSecurity *r)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("_srvsvc_NetSetFileSecurity: can't open %s\n",
			 smb_fname_str_dbg(smb_fname)));
		werr = ntstatus_to_werror(nt_status);
		goto error_exit;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status) ) {
		DEBUG_OUT (3,("_srvsvc_NetSetFileSecurity: Unable to set NT ACL "
			 "on file %s\n", r->in.share));
		werr = WERR_ACCESS_DENIED;
		goto error_exit;
	}
...
static const char *next_server_disk_enum(uint32 *resume)
{
	...
	DEBUG_OUT (10, ("next_server_disk_enum: reporting disk %s. resume handle %d.\n", disk, *resume));
			       ...
			       struct srvsvc_NetNameValidate *r)
{
	...
	case 0x9:
		if (!validate_net_name(r->in.name, INVALID_SHARENAME_CHARS,
				       strlen_m(r->in.name)))
		{
			DEBUG_OUT (5,("_srvsvc_NetNameValidate: Bad sharename \"%s\"\n",
				r->in.name));
			return WERR_INVALID_NAME;
		}
		break;
			  ...
			  void *private_data )
{
	...
	DEBUG_OUT (10,("enum_file_close_fn: request to close file %s, %s\n",
		sharepath,
			    ...
			    struct srvsvc_NetFileClose *r)
{
	...
	DEBUG_OUT (5,("_srvsvc_NetFileClose: %d\n", __LINE__));
