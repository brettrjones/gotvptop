				...
				uint32_t *pmapped_count)
{
	...
	*pmapped_count = 0;
	...
	for (i = 0; i < num_entries; i++) {
		...
		DEBUG_OUT (5, ("lookup_lsa_rids: looking up name %s\n", full_name));
		...
		case SID_NAME_WKN_GRP:
			DEBUG_OUT (5, ("init_lsa_rids: %s found\n", full_name));
			/* Leave these unchanged */
			break;
		default:
			/* Don't hand out anything but the list above */
			DEBUG_OUT (5, ("init_lsa_rids: %s not found\n", full_name));
				...
				uint32 *pmapped_count)
{
	...
	*pmapped_count = 0;
	...
	for (i = 0; i < num_entries; i++) {
		...
		DEBUG_OUT (5, ("init_lsa_sids: looking up name %s\n", full_name));
		...
		case SID_NAME_ALIAS:
		case SID_NAME_WKN_GRP:
			DEBUG_OUT (5, ("init_lsa_sids: %s found\n", full_name));
			/* Leave these unchanged */
			break;
		default:
			/* Don't hand out anything but the list above */
			DEBUG_OUT (5, ("init_lsa_sids: %s not found\n", full_name));
			type = SID_NAME_UNKNOWN;
			break;
		}
			      ...
			      struct lsa_QueryInfoPolicy *r)
{
	...
	case LSA_POLICY_INFO_AUDIT_EVENTS:
		...
		if (!(handle->access & LSA_POLICY_VIEW_AUDIT_INFORMATION)) {
			DEBUG_OUT (10,("_lsa_QueryInfoPolicy: insufficient access rights\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	case LSA_POLICY_INFO_DNS_INT: {
		...
		if ((pdb_capabilities() & PDB_CAP_ADS) == 0) {
			DEBUG_OUT (10, ("Not replying to LSA_POLICY_INFO_DNS "
	...
	default:
		DEBUG_OUT (0,("_lsa_QueryInfoPolicy: unknown info level in Lsa Query: %d\n",
			r->in.level));
		status = NT_STATUS_INVALID_INFO_CLASS;
		break;
	}
	...
	*pp_ref = NULL;
	...
	for (i=0; i<LSA_REF_DOMAIN_LIST_MULTIPLIER; i++) {
		...
		if (init_lsa_ref_domain_list(mem_ctx, ref,
					     dom_infos[i].name,
					     &dom_infos[i].sid) != i) {
			DEBUG_OUT (0, ("Domain %s mentioned twice??\n",
				  dom_infos[i].name));
	...
	DEBUG_OUT (10, ("num_sids %d, mapped_count %d, status %s\n",
			 ...
			 struct lsa_LookupSids *r)
{
	...
	if (num_sids >  MAX_LOOKUP_SIDS) {
		DEBUG_OUT (5,("_lsa_LookupSids: limit of %d exceeded, requested %d\n",
			  ...
			  struct lsa_LookupSids2 *r)
{
	...
	if (num_sids >  MAX_LOOKUP_SIDS) {
		DEBUG_OUT (5,("_lsa_LookupSids2: limit of %d exceeded, requested %d\n",
			 MAX_LOOKUP_SIDS, num_sids));
		return NT_STATUS_NONE_MAPPED;
	}
			  ...
			  struct lsa_LookupSids3 *r)
{
	...
	if (p->auth.auth_type != DCERPC_AUTH_TYPE_SCHANNEL) {
		DEBUG_OUT (0,("_lsa_LookupSids3: client %s not using schannel for netlogon\n",
			  ...
			  struct lsa_LookupNames *r)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		...
		DEBUG_OUT (5,("_lsa_LookupNames: truncating name lookup list to %d\n",
			num_entries));
	}
			   ...
			   struct lsa_LookupNames3 *r)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		num_entries = MAX_LOOKUP_SIDS;
		DEBUG_OUT (5,("_lsa_LookupNames3: truncating name lookup list to %d\n", num_entries));
	}
			   ...
			   struct lsa_LookupNames4 *r)
{
	...
	/* No policy handle on this call. Restrict to crypto connections. */
	if (p->auth.auth_type != DCERPC_AUTH_TYPE_SCHANNEL) {
		DEBUG_OUT (0,("_lsa_lookup_names4: client %s not using schannel for netlogon\n",
			get_remote_machine_name() ));
		return NT_STATUS_INVALID_PARAMETER;
	}
				  ...
				  struct lsa_DeleteTrustedDomain *r)
{
	...
	if (td->netbios_name == NULL || *td->netbios_name == '\0') {
		DEBUG_OUT (10, ("Missing netbios name for for trusted domain %s.\n",
			   ...
			   struct lsa_DeleteObject *r)
{
	...
	case LSA_HANDLE_ACCOUNT_TYPE:
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10,("_lsa_DeleteObject: privilege_delete_account gave: %s\n",
				nt_errstr(status)));
			return status;
		}
		break;
			...
			struct lsa_EnumPrivs *r)
{
	...
	DEBUG_OUT (10,("_lsa_EnumPrivs: enum_context:%d total entries:%d\n",
				    ...
				    struct lsa_LookupPrivDisplayName *r)
{
	...
	DEBUG_OUT (10,("_lsa_LookupPrivDisplayName: name = %s\n", r->in.name->string));

	description = get_privilege_dispname(r->in.name->string);
	if (!description) {
		DEBUG_OUT (10,("_lsa_LookupPrivDisplayName: doesn't exist\n"));
		return NT_STATUS_NO_SUCH_PRIVILEGE;
	}

	DEBUG_OUT (10,("_lsa_LookupPrivDisplayName: display name = %s\n", description));
			       ...
			       struct lsa_EnumPrivsAccount *r)
{
	...
	*r->out.privs = priv_set = TALLOC_ZERO_P(p->mem_ctx, struct lsa_PrivilegeSet);
	...
	DEBUG_OUT (10,("_lsa_EnumPrivsAccount: %s has %d privileges\n",
				     ...
				     struct lsa_GetSystemAccessAccount *r)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("_lsa_GetSystemAccessAccount: "
				     ...
				     struct lsa_AddPrivilegesToAccount *r)
{
	...
	if ( !grant_privilege_set( &info->sid, set ) ) {
		DEBUG_OUT (3,("_lsa_AddPrivilegesToAccount: grant_privilege_set(%s) failed!\n",
			 sid_string_dbg(&info->sid) ));
		return NT_STATUS_NO_SUCH_PRIVILEGE;
	}
					  ...
					  struct lsa_RemovePrivilegesFromAccount *r)
{
	...
	if ( !revoke_privilege_set( &info->sid, set) ) {
		DEBUG_OUT (3,("_lsa_RemovePrivilegesFromAccount: revoke_privilege(%s) failed!\n",
			 sid_string_dbg(&info->sid) ));
		return NT_STATUS_NO_SUCH_PRIVILEGE;
	}
			       ...
			       struct lsa_AddAccountRights *r)
{
	...
	for ( i=0; i < r->in.rights->count; i++ ) {
		...
		if ( !grant_privilege_by_name( &sid, privname ) ) {
			DEBUG_OUT (2,("_lsa_AddAccountRights: Failed to add privilege [%s]\n",
				privname ));
			return NT_STATUS_NO_SUCH_PRIVILEGE;
		}
	}
				  ...
				  struct lsa_RemoveAccountRights *r)
{
	...
	for ( i=0; i < r->in.rights->count; i++ ) {
		...
		if ( !revoke_privilege_by_name( &sid, privname ) ) {
			DEBUG_OUT (2,("_lsa_RemoveAccountRights: Failed to revoke privilege [%s]\n",
				privname ));
			return NT_STATUS_NO_SUCH_PRIVILEGE;
		}
	}
				...
				struct lsa_EnumAccountRights *r)
{
	...
	DEBUG_OUT (10,("_lsa_EnumAccountRights: %s has %d privileges\n",
			      ...
			      struct lsa_LookupPrivValue *r)
{
	...
	DEBUG_OUT (10,("_lsa_lookup_priv_value: name = %s\n", name));
