...
{
	if (!p) {
		DEBUG_OUT (0,("Invalid pipe in close_internal_rpc_pipe_hnd\n"));
		return False;
	}
					      ...
					      struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (4,("Create pipe requested %s\n",
		 get_pipe_name_from_syntax(talloc_tos(), syntax)));

	p = TALLOC_ZERO_P(mem_ctx, struct pipes_struct);

	if (!p) {
		DEBUG_OUT (0,("ERROR! no memory for pipes_struct!\n"));
		return NULL;
	}
	...
	p->mem_ctx = talloc_named(p, 0, "pipe %s %p",
				 get_pipe_name_from_syntax(talloc_tos(),
							   syntax), p);
	if (p->mem_ctx == NULL) {
		DEBUG_OUT (0,("open_rpc_pipe_p: talloc_init failed.\n"));
		TALLOC_FREE(p);
		return NULL;
	}

	if (!init_pipe_handles(p, syntax)) {
		DEBUG_OUT (0,("open_rpc_pipe_p: init_pipe_handles failed.\n"));
		TALLOC_FREE(p);
		return NULL;
	}
	...
	if (p->session_info == NULL) {
		DEBUG_OUT (0, ("open_rpc_pipe_p: copy_serverinfo failed\n"));
		close_policy_by_pipe(p);
		TALLOC_FREE(p);
		return NULL;
	}
	...
	DEBUG_OUT (4,("Created internal pipe %s (pipes_open=%d)\n",
				...
				const struct auth_serversupplied_info *session_info)
{
	...
	if (result->read_queue == NULL) {
		DEBUG_OUT (0, ("tevent_queue_create failed\n"));
		goto fail;
	}
	...
	if (ev == NULL) {
		DEBUG_OUT (0, ("s3_tevent_context_init failed\n"));
		goto fail;
	}
	...
	socket_dir = lp_parm_const_string(
		...
		lp_ncalrpc_dir());
	if (socket_dir == NULL) {
		DEBUG_OUT (0, ("externan_rpc_pipe:socket_dir not set\n"));
		goto fail;
	}
	...
	if (subreq == NULL) {
		unbecome_root();
		DEBUG_OUT (0, ("tstream_npa_connect_send to %s for pipe %s and "
			  "user %s\\%s failed\n",
	...
	unbecome_root();
	if (!ok) {
		DEBUG_OUT (0, ("tevent_req_poll to %s for pipe %s and user %s\\%s "
	...
	TALLOC_FREE(subreq);
	if (ret != 0) {
		DEBUG_OUT (0, ("tstream_npa_connect_recv  to %s for pipe %s and "
			  "user %s\\%s failed: %s\n",
			  socket_np_dir, pipe_name, session_info_t->info->domain_name,
			  session_info_t->info->account_name,
			  strerror(sys_errno)));
				...
				struct rpc_pipe_client **_result)
{
	...
	if (result->binding_handle == NULL) {
		status = NT_STATUS_NO_MEMORY;
		DEBUG_OUT (0, ("Failed to create binding handle.\n"));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to initialize anonymous bind.\n"));
		goto done;
	}

	status = rpc_pipe_bind(result, auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to bind external pipe.\n"));
		goto done;
	}
				 ...
				 struct rpc_pipe_client **cli_pipe)
{
	...
	DEBUG_OUT (5, ("Connecting to %s pipe.\n", pipe_name));
	...
	if (StrCaseCmp(server_type, "embedded") == 0) {
		...
		DEBUG_OUT (0, ("Wrong servertype specified in config file: %s",
			  nt_errstr(status)));
		goto done;
        }
