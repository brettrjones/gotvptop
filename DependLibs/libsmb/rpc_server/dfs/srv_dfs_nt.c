...
WERROR _dfs_Add(struct pipes_struct *p, struct dfs_Add *r)
{
	...
	if (p->session_info->utok.uid != sec_initial_uid()) {
		DEBUG_OUT (10,("_dfs_add: uid != 0. Access denied.\n"));
	...
	DEBUG_OUT (5,("init_reply_dfs_add: Request to add %s -> %s\\%s.\n",
	...
	if(jn->referral_list == NULL) {
		DEBUG_OUT (0,("init_reply_dfs_add: talloc failed for referral list!\n"));
...
WERROR _dfs_Remove(struct pipes_struct *p, struct dfs_Remove *r)
{
	...
	if (p->session_info->utok.uid != sec_initial_uid()) {
		DEBUG_OUT (10,("_dfs_remove: uid != 0. Access denied.\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	if (r->in.servername && r->in.sharename) {
		...
		strlower_m(altpath);
		DEBUG_OUT (5,("init_reply_dfs_remove: Request to remove %s -> %s\\%s.\n",
	...
	if(!r->in.servername && !r->in.sharename) {
		...
		DEBUG_OUT (10,("altpath: .%s. refcnt: %d\n", altpath, jn->referral_count));
		for(i=0;i<jn->referral_count;i++) {
			...
			trim_char(refpath, '\\', '\\');
			DEBUG_OUT (10,("_dfs_remove:  refpath: .%s.\n", refpath));
			if(strequal(refpath, altpath)) {
				*(jn->referral_list[i].alternate_path)='\0';
				DEBUG_OUT (10,("_dfs_remove: Removal request matches referral %s\n",
					refpath));
				found = True;
			}
		}
...
static bool init_reply_dfs_info_1(TALLOC_CTX *mem_ctx, struct junction_map* j,struct dfs_Info1* dfs1)
{
	...
	DEBUG_OUT (5,("init_reply_dfs_info_1: initing entrypath: %s\n",dfs1->path));
	return True;
}
...
static bool init_reply_dfs_info_3(TALLOC_CTX *mem_ctx, struct junction_map* j, struct dfs_Info3* dfs3)
{
	...
	for(ii=0;ii<j->referral_count;ii++) {
		...
		p = strrchr_m(path,'\\');
		if(p==NULL) {
			DEBUG_OUT (4,("init_reply_dfs_info_3: invalid path: no \\ found in %s\n",path));
			continue;
		}
		*p = '\0';
		DEBUG_OUT (5,("storage %d: %s.%s\n",ii,path,p+1));
...
WERROR _dfs_Enum(struct pipes_struct *p, struct dfs_Enum *r)
{
	...
	DEBUG_OUT (5,("_dfs_Enum: %u junctions found in Dfs, doing level %d\n",
