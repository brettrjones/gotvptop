				   ...
				   struct auth_ntlmssp_state **ctx)
{
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG_OUT (0, (__location__ ": auth_ntlmssp_update failed: %s\n",
			  nt_errstr(status)));
		goto done;
	}
				    ...
				    bool do_sign, bool do_seal)
{
	if (do_sign && !auth_ntlmssp_negotiated_sign(ctx)) {
		DEBUG_OUT (1, (__location__ "Integrity was requested but client "
			  "failed to negotiate signing.\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	if (do_seal && !auth_ntlmssp_negotiated_seal(ctx)) {
		DEBUG_OUT (1, (__location__ "Privacy was requested but client "
				      ...
				      struct auth_serversupplied_info **session_info)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, (__location__ ": Failed to get authenticated user "
			  "info: %s\n", nt_errstr(status)));
		return status;
	}

	DEBUG_OUT (5, (__location__ "OK: user: %s domain: %s workstation: %s\n",
