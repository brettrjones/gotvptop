...
static void epmd_smb_conf_updated(struct messaging_context *msg,
				  ...
				  DATA_BLOB *data)
{
	DEBUG_OUT (10, ("Got message saying smb.conf was updated. Reloading.\n"));
	change_to_root_user();
	epmd_reopen_logs();
}
				    ...
				    void *private_data)
{
	...
	DEBUG_OUT (1,("Reloading printers after SIGHUP\n"));
	epmd_reopen_logs();
}
		...
		struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (1, ("Forking Endpoint Mapper Daemon\n"));

	pid = sys_fork();

	if (pid == -1) {
		DEBUG_OUT (0, ("Failed to fork Endpoint Mapper [%s], aborting ...\n",
			  strerror(errno)));
		exit(1);
	}
	...
	if (!ok) {
		DEBUG_OUT (0, ("Failed to register serverid in epmd!\n"));
		exit(1);
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to register epmd rpc inteface! (%s)\n",
			  nt_errstr(status)));
		exit(1);
	}
	...
	if (!ok) {
		DEBUG_OUT (0, ("Failed to open epmd ncalrpc pipe!\n"));
		exit(1);
	}

	ok = epmd_open_sockets(ev_ctx, msg_ctx);
	if (!ok) {
		DEBUG_OUT (0, ("Failed to open epmd tcpip sockets!\n"));
		exit(1);
	}

	ok = setup_named_pipe_socket("epmapper", ev_ctx);
	if (!ok) {
		DEBUG_OUT (0, ("Failed to open epmd named pipe!\n"));
		exit(1);
	}

	DEBUG_OUT (1, ("Endpoint Mapper Daemon Started (%d)\n", getpid()));
