...
bool init_service_op_table( void )
{
	...
	if ( !(svcctl_ops = TALLOC_ARRAY( NULL, struct service_control_op, num_services+1)) ) {
		DEBUG_OUT (0,("init_service_op_table: talloc() failed!\n"));
		return False;
	}
                                     ...
                                     uint32 access_desired, uint32 *access_granted )
{
	if ( geteuid() == sec_initial_uid() ) {
		DEBUG_OUT (5,("svcctl_access_check: using root's token\n"));
					      ...
					      struct policy_handle *hnd)
{
	...
	if( !find_policy_by_hnd( p, hnd, (void **)(void *)&service_info) ) {
		DEBUG_OUT (2,("find_service_info_by_hnd: handle not found\n"));
		return NULL;
	}
			    ...
			    struct svcctl_OpenServiceW *r)
{
	...
	DEBUG_OUT (5, ("_svcctl_OpenServiceW: Attempting to open Service [%s], \n", service));
	...
	sec_desc = svcctl_get_secdesc(p->mem_ctx,
				      ...
				      service);
	if (sec_desc == NULL) {
		DEBUG_OUT (0, ("_svcctl_OpenServiceW: Failed to get a valid security "
			  "descriptor"));
		return WERR_NOMEM;
	}
			    ...
			    struct ENUM_SERVICE_STATUSW **status)
{
	...
	if ( !(st = TALLOC_ARRAY( ctx, struct ENUM_SERVICE_STATUSW, num_services )) ) {
		DEBUG_OUT (0,("enumerate_status: talloc() failed!\n"));
		return -1;
	}
