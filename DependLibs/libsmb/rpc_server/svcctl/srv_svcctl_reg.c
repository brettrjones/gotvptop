			   ...
			   struct rcinit_file_information **service_info)
{
	...
	if (f == NULL) {
		DEBUG_OUT (0,("read_init_file: failed to open [%s]\n", filepath));
		return false;
	}
			       ...
			       const char *name)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("svcctl_init_winreg_keys: Could not create key %s: %s\n",
	...
	status = dcerpc_winreg_set_dword(mem_ctx,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("svcctl_init_winreg_keys: Could not create value: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(result)) {
	...
	if (sd == NULL) {
		DEBUG_OUT (0, ("add_new_svc_name: Failed to create default "
			  "sec_desc!\n"));
		goto done;
	}
	...
	status = dcerpc_winreg_CreateKey(h,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("eventlog_init_winreg_keys: Could not create key %s: %s\n",
			wkey.name, nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(result)) {
...
bool svcctl_init_winreg(struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (3, ("Initialise the svcctl registry keys if needed.\n"));
	...
	result = regdb_open();
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (10, ("regdb_open failed: %s\n",
			   win_errstr(result)));
		goto done;
	}
	result = regdb_transaction_start();
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (10, ("regdb_transaction_start failed: %s\n",
			   win_errstr(result)));
		goto done;
	}
	...
	status = dcerpc_winreg_int_hklm_openkey(tmp_ctx,
						...
						&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("svcctl_init_winreg: Could not open %s - %s\n",
	...
	status = dcerpc_winreg_enum_keys(tmp_ctx,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("svcctl_init_winreg: Could enum keys at %s - %s\n",
			  key, nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(result)) {
	...
	if (ok) {
		...
		if (!W_ERROR_IS_OK(result)) {
			DEBUG_OUT (10, ("regdb_transaction_commit failed: %s\n",
				   win_errstr(result)));
		}
	} else {
		result = regdb_transaction_cancel();
		if (!W_ERROR_IS_OK(result)) {
			DEBUG_OUT (10, ("regdb_transaction_cancel failed: %s\n",
				   win_errstr(result)));
		}
	}
	regdb_close();
