...
bool init_pipe_handles(struct pipes_struct *p, const struct ndr_syntax_id *syntax)
{
	...
	if (plist != NULL) {
		...
		DEBUG_OUT (10,("init_pipe_handle_list: created handle list for "
			  "pipe %s\n",
	...
	DEBUG_OUT (10,("init_pipe_handle_list: pipe_handles ref count = %lu for "
				...
				struct policy_handle *hnd, void *data_ptr)
{
	...
	if (p->pipe_handles->count > MAX_OPEN_POLS) {
		DEBUG_OUT (0,("create_policy_hnd: ERROR: too many handles (%d) on this pipe.\n",
	...
	if (!rpc_hnd) {
		DEBUG_OUT (0,("create_policy_hnd: ERROR: out of memory!\n"));
		return NULL;
	}
	...
	*hnd = rpc_hnd->wire_handle;
	...
	DEBUG_OUT (4, ("Opened policy hnd[%d] ", (int)p->pipe_handles->count));
				...
				const struct policy_handle *hnd, void **data_p)
{
	...
	for (h = p->pipe_handles->handles; h != NULL; h = h->next) {
		if (memcmp(&h->wire_handle, hnd, sizeof(*hnd)) == 0) {
			DEBUG_OUT (4,("Found policy hnd[%u] ", count));
...
bool close_policy_hnd(struct pipes_struct *p, struct policy_handle *hnd)
{
	...
	if (rpc_hnd == NULL) {
		DEBUG_OUT (3, ("Error closing policy (policy not found)\n"));
		return false;
	}

	DEBUG_OUT (3,("Closed policy\n"));
...
void close_policy_by_pipe(struct pipes_struct *p)
{
	...
	if (p->pipe_handles->pipe_ref_count == 0) {
		...
		DEBUG_OUT (10,("close_policy_by_pipe: deleted handle list for "
			  "pipe %s\n",
			    ...
			    const char *type, NTSTATUS *pstatus)
{
	...
	if (p->pipe_handles->count > MAX_OPEN_POLS) {
		DEBUG_OUT (0, ("policy_handle_create: ERROR: too many handles (%d) "
			  "on pipe %s.\n", (int)p->pipe_handles->count,
			  get_pipe_name_from_syntax(talloc_tos(), &p->syntax)));
			  ...
			  NTSTATUS *pstatus)
{
	...
	if ((access_required & rpc_hnd->access_granted) != access_required) {
		if (geteuid() == sec_initial_uid()) {
			DEBUG_OUT (4, ("%s: ACCESS should be DENIED (granted: "
				  "%#010x; required: %#010x)\n", location,
				  rpc_hnd->access_granted, access_required));
			DEBUGADD(4,("but overwritten by euid == 0\n"));
			goto okay;
		}
		DEBUG_OUT (2,("%s: ACCESS DENIED (granted: %#010x; required: "
 ...
 okay:
	DEBUG_OUT (10, ("found handle of type %s\n", talloc_get_name(data)));
