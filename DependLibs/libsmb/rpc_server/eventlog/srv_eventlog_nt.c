...
static bool elog_check_access( EVENTLOG_INFO *info, const struct security_token *token )
{
	...
	if ( !sec_desc ) {
		DEBUG_OUT (5,("elog_check_access: Unable to get NT ACL for %s\n",
			tdbname));
		return False;
	}
	...
	if ( geteuid() == sec_initial_uid() ) {
		DEBUG_OUT (5,("elog_check_access: running as root, using system token\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (8,("elog_check_access: se_access_check() return %s\n",
			nt_errstr(status)));
		return False;
	}
...
static bool get_num_records_hook( EVENTLOG_INFO * info )
{
	...
	if ( !info->etdb ) {
		DEBUG_OUT ( 10, ( "No open tdb for %s\n", info->logname ) );
		return False;
	}
...
static NTSTATUS elog_open( struct pipes_struct * p, const char *logname, struct policy_handle *hnd )
{
	...
	if ( !get_oldest_entry_hook( elog ) ) {
		DEBUG_OUT (3,("elog_open: Successfully opened eventlog but can't "
...
{
	if ( !info || !info->etdb ) {
		DEBUG_OUT (0,("elog_size: Invalid info* structure!\n"));
		return 0;
	}
				 ...
				 EVENTLOG_INFO *info)
{
	...
	DEBUG_OUT ( 4, ( "sync_eventlog_params with %s\n", elogname ) );

	if ( !info->etdb ) {
		DEBUG_OUT ( 4, ( "No open tdb! (%s)\n", info->logname ) );
		goto done;
	}
	...
	status = dcerpc_winreg_int_hklm_openkey(ctx,
						...
						&wresult);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (4,("sync_eventlog_params: Failed to open key [%s] (%s)\n",
	...
	status = dcerpc_winreg_query_dword(ctx,
					   ...
					   &wresult);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (4, ("Failed to query value \"Retention\": %s\n",
			  nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(wresult)) {
	...
	if (!W_ERROR_IS_OK(wresult)) {
		DEBUG_OUT (4, ("Failed to query value \"MaxSize\": %s\n",
			  win_errstr(wresult)));
		goto done;
	}
				 ...
				 struct eventlog_OpenEventLogW *r)
{
	...
	DEBUG_OUT ( 10,("_eventlog_OpenEventLogW: Server [%s], Log [%s]\n",
	...
	if ( !(info = find_eventlog_info_by_hnd( p, r->out.handle )) ) {
		DEBUG_OUT (0,("_eventlog_OpenEventLogW: eventlog (%s) opened but unable to find handle!\n",
			r->in.logname->string ));
		elog_close( p, r->out.handle );
		return NT_STATUS_INVALID_HANDLE;
	}

	DEBUG_OUT (10,("_eventlog_OpenEventLogW: Size [%d]\n", elog_size( info )));
				  ...
				  struct eventlog_ClearEventLogW *r)
{
	...
	if (r->in.backupfile && r->in.backupfile->string) {
		...
		DEBUG_OUT (8,( "_eventlog_ClearEventLogW: Using [%s] as the backup "
			"file name for log [%s].",
				 ...
				 struct eventlog_ReadEventLogW *r)
{
	...
	{
		DEBUG_OUT (3,("_eventlog_ReadEventLogW: "
