...
bool eventlog_init_winreg(struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (3, ("Initialise the eventlog registry keys if needed.\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("eventlog_init_winreg: Could not open %s - %s\n",
	...
	status = dcerpc_winreg_enum_keys(tmp_ctx,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("eventlog_init_winreg: Could enum keys at %s - %s\n",
			  key, nt_errstr(status)));
		goto done;
	}
	if (!W_ERROR_IS_OK(result)) {
