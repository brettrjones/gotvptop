			      ...
			      const char *debug )
{
	...
	if ((needed_priv_1 != SEC_PRIV_INVALID && security_token_has_privilege(token, needed_priv_1)) ||
		...
		DEBUG_OUT (4,("access_check_object: user rights access mask [0x%x]\n",
			rights_mask));
	}
	...
	if ( geteuid() == sec_initial_uid() ) {
		...
		DEBUG_OUT (4,("%s: ACCESS should be DENIED  (requested: %#010x)\n", debug, des_access));
	...
	DEBUG_OUT (4,("%s: access %s (requested: 0x%08x, granted: 0x%08x)\n",
