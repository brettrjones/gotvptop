		      ...
		      unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	DEBUG_OUT (10, ("sys_get_nfs_quota: path[%s] bdev[%s] qtype[%d]\n",
		   path, bdev, qtype));
	...
	if (qtype != SMB_USER_QUOTA_TYPE) {
		DEBUG_OUT (3, ("sys_get_nfs_quota: got unsupported quota type '%d', "
	...
	DEBUG_OUT (10, ("sys_get_nfs_quotas: Asking for quota of path '%s' on "
	...
	if (clnt->cl_auth == NULL) {
		DEBUG_OUT (3, ("sys_get_nfs_quotas: authunix_create_default "
			  "failed\n"));
		ret = -1;
		goto out;
	}
	...
	if (clnt_stat != RPC_SUCCESS) {
		DEBUG_OUT (3, ("sys_get_nfs_quotas: clnt_call failed\n"));
		ret = -1;
		goto out;
	}

	DEBUG_OUT (10, ("sys_get_nfs_quotas: getquota_rslt:\n"
		   "status       : '%i'\n"
		   "bsize        : '%i'\n"
		   "active       : '%s'\n"
		   "bhardlimit   : '%u'\n"
	...
	case 0:
		DEBUG_OUT (3, ("sys_get_nfs_quotas: Remote Quotas Failed! "
			  "Error '%i'\n", gq_rslt.status));
		ret = -1;
		goto out;

	case 1:
		DEBUG_OUT (10, ("sys_get_nfs_quotas: Good quota data\n"));
	...
	case 2:
		DEBUG_OUT (5, ("sys_get_nfs_quotas: No quota set\n"));
		SMB_QUOTAS_SET_NO_LIMIT(dp);
		break;

	case 3:
		DEBUG_OUT (3, ("sys_get_nfs_quotas: no permission to get quota\n"));
		errno = EPERM;
		ret = -1;
		goto out;

	default:
		DEBUG_OUT (5, ("sys_get_nfs_quotas: Unknown remote quota status "
			  "code '%i'\n", gq_rslt.status));
		ret = -1;
		goto out;
		break;
...
out:
	...
	DEBUG_OUT (10, ("sys_get_nfs_quotas: finished\n" ));
	return ret;
}
...
int sys_set_nfs_quota(const char *path, const char *bdev,
		      ...
		      unid_t id, SMB_DISK_QUOTA *dp)
{
	DEBUG_OUT (1, ("sys_set_nfs_quota : not supported\n"));
	errno = ENOSYS;
	return -1;
}
