...
TDB_CONTEXT *elog_init_tdb( char *tdbfilename )
{
	...
	DEBUG_OUT (10,("elog_init_tdb: Initializing eventlog tdb (%s)\n",
		tdbfilename));
	...
	if ( !tdb ) {
		DEBUG_OUT ( 0, ( "Can't open tdb for [%s]\n", tdbfilename ) );
		return NULL;
	}
				    ...
				    bool whack_by_date )
{
	...
	for ( i = start_record; i < end_record; i++ ) {
		...
		if (len == -1) {
			DEBUG_OUT ( 10,("make_way_for_eventlogs: tdb_unpack failed.\n"));
			tdb_unlock_bystring( the_tdb, EVT_NEXT_RECORD );
			SAFE_FREE( ret.dptr );
			return False;
		}
...
bool prune_eventlog( TDB_CONTEXT * tdb )
{
	...
	if ( !tdb ) {
		DEBUG_OUT ( 4, ( "No eventlog tdb handle\n" ) );
		return False;
	}
...
ELOG_TDB *elog_open_tdb( const char *logname, bool force_clear, bool read_only )
{
	...
	if (force_clear && read_only) {
		DEBUG_OUT (1,("elog_open_tdb: Invalid flags\n"));
		return NULL;
	}
	...
	DEBUG_OUT (7,("elog_open_tdb: Opening %s...(force_clear == %s)\n",
	...
	if ( !force_clear ) {
		...
		if ( tdb ) {
			...
			if ( vers_id != EVENTLOG_DATABASE_VERSION_V1 ) {
				DEBUG_OUT (1,("elog_open_tdb: Invalid version [%d] on file [%s].\n",
					vers_id, tdbpath));
				tdb_close( tdb );
	...
	if ( tdb ) {
		...
		if ( !(tdb_node = TALLOC_ZERO_P( NULL, ELOG_TDB)) ) {
			DEBUG_OUT (0,("elog_open_tdb: talloc() failure!\n"));
			tdb_close( tdb );
			return NULL;
		}
...
bool parse_logentry( TALLOC_CTX *mem_ctx, char *line, struct eventlog_Record_tdb *entry, bool * eor )
{
	...
	DEBUG_OUT ( 6, ( "parse_logentry: trying to parse [%s].\n", line ) );
...
*/
		...
		DEBUG_OUT ( 10, ( "Unknown entry [%s]. Ignoring.\n", line ) );
						  ...
						  uint32_t record_number)
{
	...
	if (data.dsize == 0) {
		DEBUG_OUT (8,("evlog_pull_record_tdb: "
			"Can't find a record for the key, record %d\n",
			record_number));
		return NULL;
	}
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (10,("evlog_pull_record_tdb: failed to decode record %d\n",
			record_number));
		TALLOC_FREE(r);
		goto done;
	}
	...
	DEBUG_OUT (10,("evlog_pull_record_tdb: retrieved entry for record %d\n",
		record_number));
 done:
	SAFE_FREE(data.dptr);
