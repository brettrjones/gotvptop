...
static NTSTATUS do_smb_load_module(const char *module_name, bool is_probe)
{
	...
	if(!handle) {
		...
		DEBUG_OUT (level, ("Error loading module '%s': %s\n", module_name, error ? error : ""));
	...
	error = dlerror();
	if (error) {
		DEBUG_OUT (0, ("Error trying to resolve symbol 'init_samba_module' "
			  "in %s: %s\n", module_name, error));
		dlclose(handle);
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG_OUT (2, ("Module '%s' loaded\n", module_name));

	status = init();
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Module '%s' initialization failed: %s\n",
...
int smb_load_modules(const char **modules)
{
	...
	DEBUG_OUT (2, ("%d modules successfully loaded\n", success));
...
NTSTATUS smb_probe_module(const char *subsystem, const char *module)
{
	...
	DEBUG_OUT (5, ("Probing module '%s': Trying to load from %s\n",
		module, full_path));
...
NTSTATUS smb_load_module(const char *module_name)
{
	DEBUG_OUT (0,("This samba executable has not been built with plugin support\n"));
...
NTSTATUS smb_probe_module(const char *subsystem, const char *module)
{
	DEBUG_OUT (0,("This samba executable has not been built with plugin support, not probing\n")); 
	return NT_STATUS_NOT_SUPPORTED;
}
