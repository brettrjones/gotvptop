			   ...
			   uint8 **data, size_t *len)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_push_security_descriptor failed: %s\n",
			  ndr_errstr(ndr_err)));
			       ...
			       uint8_t **data, size_t *len)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_push_sec_desc_buf failed: %s\n",
			  ndr_errstr(ndr_err)));
		return ndr_map_error2ntstatus(ndr_err);
	}
			     ...
			     struct security_descriptor **psecdesc)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_pull_security_descriptor failed: %s\n",
			  ndr_errstr(ndr_err)));
		TALLOC_FREE(result);
		return ndr_map_error2ntstatus(ndr_err);
	}
				 ...
				 struct sec_desc_buf **psecdesc_buf)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_pull_sec_desc_buf failed: %s\n",
			  ndr_errstr(ndr_err)));
		TALLOC_FREE(result);
		return ndr_map_error2ntstatus(ndr_err);
	}
	...
	*psize = 0;
	...
	for (i = 0; i < the_acl->num_aces; i++) {
		...
		if (creator && container &&
			...
			DEBUG_OUT (5,("se_create_child_secdesc(): %s:%d/0x%02x/0x%08x"
