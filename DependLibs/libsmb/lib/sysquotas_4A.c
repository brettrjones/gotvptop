...
int sys_get_vfs_quota(const char *path, const char *bdev, enum SMB_QUOTA_TYPE qtype, unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	switch (qtype) {
		case SMB_USER_QUOTA_TYPE:
			DEBUG_OUT (10,("sys_get_vfs_quota: path[%s] bdev[%s] SMB_USER_QUOTA_TYPE uid[%u]\n",
		...
		case SMB_GROUP_QUOTA_TYPE:
			DEBUG_OUT (10,("sys_get_vfs_quota: path[%s] bdev[%s] SMB_GROUP_QUOTA_TYPE gid[%u]\n",
		...
		case SMB_USER_FS_QUOTA_TYPE:
			...
			DEBUG_OUT (10,("sys_get_vfs_quota: path[%s] bdev[%s] SMB_USER_FS_QUOTA_TYPE (uid[%u])\n",
		...
		case SMB_GROUP_FS_QUOTA_TYPE:
			...
			DEBUG_OUT (10,("sys_get_vfs_quota: path[%s] bdev[%s] SMB_GROUP_FS_QUOTA_TYPE (gid[%u])\n",
				path, bdev, (unsigned)id.gid));
...
int sys_set_vfs_quota(const char *path, const char *bdev, enum SMB_QUOTA_TYPE qtype, unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	switch (qtype) {
		case SMB_USER_QUOTA_TYPE:
			DEBUG_OUT (10,("sys_set_vfs_quota: path[%s] bdev[%s] SMB_USER_QUOTA_TYPE uid[%u]\n",
				path, bdev, (unsigned)id.uid));
...
#ifdef HAVE_GROUP_QUOTA
		case SMB_GROUP_QUOTA_TYPE:
			DEBUG_OUT (10,("sys_set_vfs_quota: path[%s] bdev[%s] SMB_GROUP_QUOTA_TYPE gid[%u]\n",
				path, bdev, (unsigned)id.gid));
		...
		case SMB_USER_FS_QUOTA_TYPE:
			 ...
			 */
			id.uid = getuid();
			DEBUG_OUT (10,("sys_set_vfs_quota: path[%s] bdev[%s] SMB_USER_FS_QUOTA_TYPE (uid[%u])\n",
				path, bdev, (unsigned)id.uid));
			...
			DEBUG_OUT (0,("sys_set_vfs_quota: ret(%d) errno(%d)[%s] uid(%d) bdev[%s]\n",
		...
		case SMB_GROUP_FS_QUOTA_TYPE:
			 ...
			 */
			id.gid = getgid();
			DEBUG_OUT (10,("sys_set_vfs_quota: path[%s] bdev[%s] SMB_GROUP_FS_QUOTA_TYPE (gid[%u])\n",
				path, bdev, (unsigned)id.gid));
