...
static char *wins_srv_keystr(struct in_addr wins_ip, struct in_addr src_ip)
{
	...
	if ( !wins_ip_addr || !src_ip_addr ) {
		DEBUG_OUT (0,("wins_srv_keystr: malloc error\n"));
		goto done;
	}

	if (asprintf(&keystr, WINS_SRV_FMT, wins_ip_addr, src_ip_addr) == -1) {
		DEBUG_OUT (0, (": ns_srv_keystr: malloc error for key string\n"));
	}
...
bool wins_srv_is_dead(struct in_addr wins_ip, struct in_addr src_ip)
{
	...
	DEBUG_OUT (4, ("wins_srv_is_dead: %s is %s\n", inet_ntoa(wins_ip),
		  result ? "dead" : "alive"));
...
void wins_srv_alive(struct in_addr wins_ip, struct in_addr src_ip)
{
	...
	DEBUG_OUT (4, ("wins_srv_alive: marking wins server %s alive\n", 
		  inet_ntoa(wins_ip)));
}
...
void wins_srv_died(struct in_addr wins_ip, struct in_addr src_ip)
{
	...
	DEBUG_OUT (4,("Marking wins server %s dead for %u seconds from source %s\n",
...
struct in_addr wins_srv_ip_tag(const char *tag, struct in_addr src_ip)
{
	...
	for (i=0; list[i]; i++) {
		...
		if (!wins_srv_is_dead(t_ip.ip, src_ip)) {
			...
			DEBUG_OUT (6,("Current wins server for tag '%s' with source %s is %s\n", 
				 tag, 
				 src_name,
				 inet_ntoa(t_ip.ip)));
			return t_ip.ip;
