			     ...
			     DATA_BLOB *ticket, struct ClearToken *ct)
{
	...
	*cell = SMB_STRDUP(t);
	...
	if (sscanf(t, "%u", &result_ct.AuthHandle) != 1) {
		DEBUG_OUT (10, ("sscanf AuthHandle failed\n"));
		return False;
	}
	...
	if (sscanf(t, "%u", &result_ct.ViceId) != 1) {
		DEBUG_OUT (10, ("sscanf ViceId failed\n"));
		return False;
	}
	...
	if (sscanf(t, "%u", &result_ct.BeginTimestamp) != 1) {
		DEBUG_OUT (10, ("sscanf BeginTimestamp failed\n"));
		return False;
	}
	...
	if (sscanf(t, "%u", &result_ct.EndTimestamp) != 1) {
		DEBUG_OUT (10, ("sscanf EndTimestamp failed\n"));
		return False;
	}
	...
	if (blob.data == NULL) {
		DEBUG_OUT (10, ("Could not get ticket\n"));
		return False;
	}
			 ...
			 DATA_BLOB ticket)
{
	...
	*p = 0;
	...
	DEBUG_OUT (10, ("afs VIOCSETTOK returned %d\n", ret));
	return (ret == 0);
}
