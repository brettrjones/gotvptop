...
static void addrchange_done(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (received == -1) {
		DEBUG_OUT (10, ("recvfrom returned %s\n", strerror(errno)));
		tevent_req_nterror(req, map_nt_error_from_unix(err));
		return;
	}
	if ((state->fromaddr_len != sizeof(struct sockaddr_nl))
	    || (state->fromaddr.ss_family != AF_NETLINK)) {
		DEBUG_OUT (10, ("Got message from wrong addr\n"));
		goto retry;
	}

	addr = (struct sockaddr_nl *)(void *)&state->addr;
	if (addr->nl_pid != 0) {
		DEBUG_OUT (10, ("Got msg from pid %d, not from the kernel\n",
			   (int)addr->nl_pid));
		goto retry;
	}

	if (received < sizeof(struct nlmsghdr)) {
		DEBUG_OUT (10, ("received %d, expected at least %d\n",
	...
	if (h->nlmsg_len < sizeof(struct nlmsghdr)) {
		DEBUG_OUT (10, ("nlmsg_len=%d, expected at least %d\n",
			   (int)h->nlmsg_len, (int)sizeof(struct nlmsghdr)));
		goto retry;
	}
	if (h->nlmsg_len > received) {
		DEBUG_OUT (10, ("nlmsg_len=%d, expected at most %d\n",
	...
	case RTM_DELADDR:
		...
		break;
	default:
		DEBUG_OUT (10, ("Got unexpected type %d - ignoring\n", h->nlmsg_type));
		goto retry;
	}
