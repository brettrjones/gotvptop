					  ...
					  const char *user, char *user2)
{
	...
	strlower_m(user2);
	DEBUG_OUT (5,("Trying _Get_Pwnam(), username as lowercase is %s\n",user2));
	...
	if(strcmp(user, user2) != 0) {
		DEBUG_OUT (5,("Trying _Get_Pwnam(), username as given is %s\n",
			 user));
	...
	strupper_m(user2);
	if(strcmp(user, user2) != 0) {
		DEBUG_OUT (5,("Trying _Get_Pwnam(), username as uppercase is %s\n",
			 user2));
		ret = getpwnam_alloc_cached(mem_ctx, user2);
		if(ret)
			goto done;
	...
	strlower_m(user2);
	DEBUG_OUT (5,("Checking combinations of %d uppercase letters in %s\n",
		 lp_usernamelevel(), user2));
	ret = uname_string_combinations(user2, mem_ctx, getpwnam_alloc_cached,
					lp_usernamelevel());

done:
	DEBUG_OUT (5,("Get_Pwnam_internals %s find user [%s]!\n",ret ?
		 "did":"didn't", user));
...
struct passwd *Get_Pwnam_alloc(TALLOC_CTX *mem_ctx, const char *user)
{
	...
	if ( *user == '\0' ) {
		DEBUG_OUT (10,("Get_Pwnam: empty username!\n"));
		return NULL;
	}

	fstrcpy(user2, user);

	DEBUG_OUT (5,("Finding user %s\n", user));
