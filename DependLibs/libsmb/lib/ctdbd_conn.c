...
{
	DEBUG_OUT (0,("cluster fatal event: %s - exiting immediately\n", why));
...
static bool ctdbd_working(struct ctdbd_connection *conn, uint32_t vnn)
{
	...
	if ((cstatus != 0) || (outdata.dptr == NULL)) {
		DEBUG_OUT (2, ("Received invalid ctdb data\n"));
		return false;
	}
	...
	if (i == m->num) {
		DEBUG_OUT (2, ("Did not find ourselves (node %d) in nodemap\n",
			  (int)vnn));
		goto fail;
	}
	...
	if ((m->nodes[i].flags & failure_flags) != 0) {
		DEBUG_OUT (2, ("Node has status %x, not active\n",
			  (int)m->nodes[i].flags));
		goto fail;
	}
			      ...
			      struct packet_context **presult)
{
	...
	if (fd == -1) {
		DEBUG_OUT (3, ("Could not create socket: %s\n", strerror(errno)));
	...
	if (sys_connect(fd, (struct sockaddr *)(void *)&addr) == -1) {
		DEBUG_OUT (1, ("connect(%s) failed: %s\n", sockname,
			  strerror(errno)));
		close(fd);
		return map_nt_error_from_unix(errno);
	}
			      ...
			      void *private_data)
{
	...
	DEBUG_OUT (10, ("msglen = %d\n", msglen));

	if (msglen < sizeof(struct ctdb_req_header)) {
		DEBUG_OUT (0, ("Got invalid msglen: %d, expected at least %d for "
			  "the req_header\n", (int)msglen,
						     ...
						     struct ctdb_req_message *msg)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_pull_struct_blob failed: %s\n",
			  ndr_errstr(ndr_err)));
		TALLOC_FREE(result);
		return NULL;
	}
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10, ("ctdb_pull_messaging_rec:\n"));
			      ...
			      TALLOC_CTX *mem_ctx, void *result)
{
 ...
 again:
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("packet_fd_read failed: %s\n", nt_errstr(status)));
 ...
 next_pkt:
	...
	if (!packet_handler(conn->pkt, ctdb_req_complete, ctdb_req_pull,
		 ...
		 * Not enough data
		 */
		DEBUG_OUT (10, ("not enough data from ctdb socket, retrying\n"));
		goto again;
	}

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not read packet: %s\n", nt_errstr(status)));
		cluster_fatal("ctdbd died\n");
	}

	hdr = (struct ctdb_req_header *)state.req.data;

	DEBUG_OUT (10, ("Received ctdb packet\n"));
	...
	if (hdr->operation == CTDB_REQ_MESSAGE) {
		...
		if (conn->msg_ctx == NULL) {
			DEBUG_OUT (1, ("Got a message without having a msg ctx, "
				  "dropping msg %llu\n",
		...
		if ((conn->release_ip_handler != NULL)
			...
			DEBUG_OUT (10, ("received CTDB_SRVID_RELEASE_IP\n"));
		...
		if ((msg->srvid == CTDB_SRVID_RECONFIGURE)
			...
			DEBUG_OUT (1, ("ctdb_read_req: Got %s message\n",
		...
		if (!(msg_state->rec = ctdb_pull_messaging_rec(
			...
			DEBUG_OUT (0, ("ctdbd_pull_messaging_rec failed\n"));
	...
	if (hdr->reqid != reqid) {
		...
		DEBUG_OUT (0,("Discarding mismatched ctdb reqid %u should have "
			 "been %u\n", hdr->reqid, reqid));
		TALLOC_FREE(hdr);
		goto again;
	}
				      ...
				      struct ctdbd_connection **pconn)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("ctdbd_connect failed: %s\n", nt_errstr(status)));
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("get_cluster_vnn failed: %s\n", nt_errstr(status)));
		goto fail;
	}

	if (!ctdbd_working(conn, conn->our_vnn)) {
		DEBUG_OUT (2, ("Node is not working, can not connect\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("Could not register random srvid: %s\n",
			  nt_errstr(status)));
		goto fail;
	}
				    ...
				    void *private_data)
{
	...
	if (msg->hdr.operation != CTDB_REQ_MESSAGE) {
		DEBUG_OUT (0, ("Received async msg of type %u, discarding\n",
			  msg->hdr.operation));
		TALLOC_FREE(buf);
	...
	if ((msg->srvid == CTDB_SRVID_RECONFIGURE)
	    || (msg->srvid == CTDB_SRVID_SAMBA_NOTIFY)){
		DEBUG_OUT (0,("Got cluster reconfigure message\n"));
		/*
	...
	if (msg->srvid != sys_getpid() && msg->srvid != MSG_SRVID_SAMBA) {
		DEBUG_OUT (0,("Got unexpected message with srvid=%llu\n", 
	...
	if (!(msg_rec = ctdb_pull_messaging_rec(NULL, length, msg))) {
		DEBUG_OUT (10, ("ctdb_pull_messaging_rec failed\n"));
		TALLOC_FREE(buf);
		return NT_STATUS_NO_MEMORY;
	}
				 ...
				 void *private_data)
{
	...
	while (packet_handler(conn->pkt, ctdb_req_complete,
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("could not handle incoming message: %s\n",
				   nt_errstr(status)));
		}
	}
}
				...
				struct messaging_context *msg_ctx)
{
	...
	if (!(conn->fde = event_add_fd(msg_ctx->event_ctx, conn,
				       ...
				       conn))) {
		DEBUG_OUT (0, ("event_add_fd failed\n"));
		return NT_STATUS_NO_MEMORY;
	}
			      ...
			      struct messaging_rec *msg)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0, ("ndr_push_struct_blob failed: %s\n",
			  ndr_errstr(ndr_err)));
	...
	DEBUG_OUT (10, ("ctdbd_messaging_send: Sending ctdb packet\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("packet_send failed: %s\n", nt_errstr(status)));
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("write to ctdbd failed: %s\n", nt_errstr(status)));
			      ...
			      int *cstatus)
{
	...
	if (conn == NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("Could not init temp connection: %s\n",
				   nt_errstr(status)));
			goto fail;
		}
	...
	DEBUG_OUT (10, ("ctdbd_control: Sending ctdb packet\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("packet_send failed: %s\n", nt_errstr(status)));
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("ctdb_read_req failed: %s\n", nt_errstr(status)));
		goto fail;
	}

	if (reply->hdr.operation != CTDB_REPLY_CONTROL) {
		DEBUG_OUT (0, ("received invalid reply\n"));
		goto fail;
	}
...
bool ctdbd_process_exists(struct ctdbd_connection *conn, uint32 vnn, pid_t pid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, (__location__ " ctdb_control for process_exists "
			  "failed\n"));
		return False;
	}
		   ...
		   TALLOC_CTX *mem_ctx, uint32_t db_id)
{
	...
	if (!NT_STATUS_IS_OK(status) || cstatus != 0) {
		DEBUG_OUT (0,(__location__ " ctdb_control for getdbpath failed\n"));
		return NULL;
	}
			 ...
			 const char *name, uint32_t *db_id, int tdb_flags)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, (__location__ " ctdb_control for db_attach "
			  "failed: %s\n", nt_errstr(status)));
		return status;
	}

	if (cstatus != 0 || data.dsize != sizeof(uint32_t)) {
		DEBUG_OUT (0,(__location__ " ctdb_control for db_attach failed\n"));
	...
	*db_id = *(uint32_t *)data.dptr;
	...
	status = ctdbd_control(conn, CTDB_CURRENT_NODE,
			       ...
			       NULL, NULL, &cstatus);
	if (!NT_STATUS_IS_OK(status) || cstatus != 0) {
		DEBUG_OUT (0,(__location__ " ctdb_control for enable seqnum "
			 "failed\n"));
		       ...
		       TDB_DATA key)
{
	...
	DEBUG_OUT (10, ("ctdbd_migrate: Sending ctdb packet\n"));
	ctdb_packet_dump(&req.hdr);
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("ctdb_read_req failed: %s\n", nt_errstr(status)));
		goto fail;
	}
				      ...
				      void *private_data)
{
	...
	if (length < sizeof(*m) || m->hdr.length != length) {
		DEBUG_OUT (0, ("Got invalid message of length %d\n", (int)length));
		TALLOC_FREE(buf);
	...
	if (m->datalen < sizeof(uint32_t) || m->datalen != d->length) {
		DEBUG_OUT (0, ("Got invalid traverse data of length %d\n",
			  (int)m->datalen));
		TALLOC_FREE(buf);
		return NT_STATUS_UNEXPECTED_IO_ERROR;
	}
	...
	if (data.dsize < sizeof(struct ctdb_ltdb_header)) {
		DEBUG_OUT (0, ("Got invalid ltdb header length %d\n",
			  (int)data.dsize));
		TALLOC_FREE(buf);
		return NT_STATUS_UNEXPECTED_IO_ERROR;
	}
			...
			void *private_data)
{
	...
	unbecome_root();
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("ctdbd_init_connection failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status) || (cstatus != 0)) {
		...
		DEBUG_OUT (0,("ctdbd_control failed: %s, %d\n", nt_errstr(status),
			 cstatus));
	...
	while (True) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("packet_fd_read_sync failed: %s\n", nt_errstr(status)));
			cluster_fatal("ctdbd died\n");
		}
	}
...
NTSTATUS ctdb_watch_us(struct ctdbd_connection *conn)
{
	...
	status = ctdbd_control_local(
		...
		NULL, NULL, &cstatus);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("ctdbd_control_local failed: %s\n",
			  nt_errstr(status)));
	}
	return status;
}
