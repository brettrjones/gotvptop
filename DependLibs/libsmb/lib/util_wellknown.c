			  ...
			  const char **domain, const char **name)
{
	...
	if (!sid_split_rid(&dom_sid, &rid)) {
		DEBUG_OUT (2, ("Could not split rid from SID\n"));
		return False;
	}
	...
	if (users == NULL) {
		DEBUG_OUT (10, ("SID %s is no special sid\n", sid_string_dbg(sid)));
		return False;
	}
	...
	DEBUG_OUT (10, ("RID of special SID %s not found\n", sid_string_dbg(sid)));
			   ...
			   struct dom_sid *sid, const char **domain)
{
	...
	DEBUG_OUT (10,("map_name_to_wellknown_sid: looking up %s\n", name));
