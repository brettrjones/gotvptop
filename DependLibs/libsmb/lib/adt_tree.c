					      ...
					      char* key )
{
	...
	if ( node->num_children == 1 ) {
		DEBUG_OUT (11,("pathtree_birth_child: First child of node [%s]! [%s]\n", 
	...
	else 
	{
		...
		{
			DEBUG_OUT (11,("pathtree_birth_child: Looking for crib; infant -> [%s], child -> [%s]\n",
			...
			if ( StrCaseCmp( infant->key, node->children[i-1]->key ) > 0 ) {
				DEBUG_OUT (11,("pathtree_birth_child: storing infant in i == [%d]\n", 
					i));
		...
		DEBUG_OUT (11,("pathtree_birth_child: Exiting loop (i == [%d])\n", i ));
					     ...
					     char *key )
{
	...
	if ( !node ) {
		DEBUG_OUT (0,("pathtree_find_child: NULL node passed into function!\n"));
		return NULL;
	}

	if ( !key ) {
		DEBUG_OUT (0,("pathtree_find_child: NULL key string passed into function!\n"));
		return NULL;
	}

	for ( i=0; i<node->num_children; i++ )
	{	
		DEBUG_OUT (11,("pathtree_find_child: child key => [%s]\n",
			node->children[i]->key));
	...
	DEBUG_OUT (11,("pathtree_find_child: %s [%s]\n",
...
WERROR pathtree_add(struct sorted_tree *tree, const char *path, void *data_p)
{
	...
	DEBUG_OUT (8,("pathtree_add: Enter\n"));

	if ( !path || *path != '\\' ) {
		DEBUG_OUT (0,("pathtree_add: Attempt to add a node with a bad path [%s]\n",
			path ? path : "NULL" ));
		return WERR_INVALID_PARAM;
	}

	if ( !tree ) {
		DEBUG_OUT (0,("pathtree_add: Attempt to add a node to an uninitialized tree!\n"));
		return WERR_INVALID_PARAM;
	}
	...
	if ( !path2 ) {
		DEBUG_OUT (0,("pathtree_add: strdup() failed on string [%s]!?!?!\n", path));
		return WERR_NOMEM;
	}
	...
	do {
		...
		if ( !next ) {
			...
			if ( !next ) {
				DEBUG_OUT (0,("pathtree_add: Failed to create new child!\n"));
				ret = WERR_NOMEM;
				goto done;
			}
		}
	...
	DEBUG_OUT (10,("pathtree_add: Successfully added node [%s] to tree\n",
		path ));

	DEBUG_OUT (8,("pathtree_add: Exit\n"));
...
void* pathtree_find(struct sorted_tree *tree, char *key )
{
	...
	DEBUG_OUT (10,("pathtree_find: Enter [%s]\n", key ? key : "NULL" ));

	/* sanity checks first */

	if ( !key ) {
		DEBUG_OUT (0,("pathtree_find: Attempt to search tree using NULL search string!\n"));
		return NULL;
	}

	if ( !tree ) {
		DEBUG_OUT (0,("pathtree_find: Attempt to search an uninitialized tree using string [%s]!\n",
			key ? key : "NULL" ));
		return NULL;
	}
	...
	if ( !keystr ) {
		DEBUG_OUT (0,("pathtree_find: strdup() failed on string [%s]!?!?!\n", key));
		return NULL;
	}
	...
	do
	{
		...
		DEBUG_OUT (11,("pathtree_find: [loop] base => [%s], new_path => [%s]\n", 
			base ? base : "",
			str ? str : ""));
	...
	if ( result )
		DEBUG_OUT (11,("pathtree_find: Found data_p!\n"));

	SAFE_FREE( keystr );

	DEBUG_OUT (10,("pathtree_find: Exit\n"));
