...
pid_t pidfile_pid(const char *program_name)
{
	...
	if (ret == 0) {
		...
		DEBUG_OUT (1, ("Could not parse contents of pidfile %s\n",
			  pidFile));
		goto noproc;
	}
...
void pidfile_create(const char *program_name)
{
	...
	if (pid != 0) {
		DEBUG_OUT (0,("ERROR: %s is already running. File %s exists and process id %d is running.\n", 
	...
	fd = sys_open(pidFile_name, O_NONBLOCK | O_CREAT | O_WRONLY | O_EXCL,
		      0644);
	if (fd == -1) {
		DEBUG_OUT (0,("ERROR: can't open %s: Error was %s\n", pidFile_name,
			 strerror(errno)));
		exit(1);
	}

	if (fcntl_lock(fd,SMB_F_SETLK,0,1,F_WRLCK)==False) {
		DEBUG_OUT (0,("ERROR: %s : fcntl lock of file %s failed. Error was %s\n",  
	...
	if (write(fd, buf, strlen(buf)) != (ssize_t)strlen(buf)) {
		DEBUG_OUT (0,("ERROR: can't write to file %s: %s\n", 
