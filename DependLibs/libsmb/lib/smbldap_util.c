					 	...
					 	const char *domain_name)
{
	...
	DEBUG_OUT (3,("add_new_domain_account_policies: Adding new account policies for domain\n"));
	...
	for (i=1; decode_account_policy_name(i) != NULL; i++) {
		...
		if (!policy_attr) {
			DEBUG_OUT (0,("add_new_domain_account_policies: ops. no policy!\n"));
			continue;
		}

		if (!account_policy_get_default(i, &policy_default)) {
			DEBUG_OUT (0,("add_new_domain_account_policies: failed to get default account policy\n"));
			SAFE_FREE(dn);
			return ntstatus;
		}

		DEBUG_OUT (10,("add_new_domain_account_policies: adding \"%s\" with value: %d\n", policy_attr, policy_default));
		...
		if (rc!=LDAP_SUCCESS) {
			...
			DEBUG_OUT (1,("add_new_domain_account_policies: failed to add account policies to dn= %s with: %s\n\t%s\n",
                                    ...
                                    const char *domain_name)
{
	...
	if (num_result > 1) {
		DEBUG_OUT (0, ("add_new_domain_info: More than domain with that name exists: bailing "
			   "out!\n"));
		ldap_msgfree(result);
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	DEBUG_OUT (3,("add_new_domain_info: Adding new domain\n"));
...

	{
		...
		DEBUG_OUT (10,("add_new_domain_info: setting next available user rid [%s]\n", rid_str));
		smbldap_set_mod(&mods, LDAP_MOD_ADD,
			get_attr_key2string(dominfo_attr_list,
	...
	if (rc!=LDAP_SUCCESS) {
		...
		DEBUG_OUT (1,("add_new_domain_info: failed to add domain dn= %s with: %s\n\t%s\n",
			 dn, ldap_err2string(rc),
			 ld_error?ld_error:"unknown"));
		SAFE_FREE(ld_error);
		SAFE_FREE(dn);
	...
	DEBUG_OUT (2,("add_new_domain_info: added: domain = %s in the LDAP database\n", domain_name));
	ldap_mods_free(mods, True);
	SAFE_FREE(dn);
	return NT_STATUS_OK;
}
                                    ...
                                    bool try_add)
{
	...
	DEBUG_OUT (2, ("smbldap_search_domain_info: Searching for:[%s]\n", filter));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (2,("smbldap_search_domain_info: Problem during LDAPsearch: %s\n", ldap_err2string (rc)));
		DEBUG_OUT (2,("smbldap_search_domain_info: Query was: %s, %s\n", lp_ldap_suffix(), filter));
		goto failed;
	}
	...
	*result = NULL;
	...
	if (count < 1) {
		...
		DEBUG_OUT (3, ("smbldap_search_domain_info: Got no domain info entries for domain\n"));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("smbldap_search_domain_info: Adding domain info for %s failed with %s\n", 
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("smbldap_search_domain_info: Adding domain account policies for %s failed with %s\n", 
				domain_name, nt_errstr(status)));
			goto failed;
		}
	...
	if (count > 1 ) {
		...
		DEBUG_OUT (0, ("smbldap_search_domain_info: Got too many (%d) domain info entries for domain %s\n",
			  count, domain_name));
		goto failed;
	}
