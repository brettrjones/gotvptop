 ...
 const char** get_attr_list( TALLOC_CTX *mem_ctx, ATTRIB_MAP_ENTRY table[] )
{
	...
	if ( !names ) {
		DEBUG_OUT (0,("get_attr_list: out of memory\n"));
		return NULL;
	}
				    ...
				    int max_len)
{
	...
	if ((values = ldap_get_values (ldap_struct, entry, attribute)) == NULL) {
		DEBUG_OUT (10, ("smbldap_get_single_attribute: [%s] = [<does not exist>]\n", attribute));

		return False;
	}

	if (convert_string(CH_UTF8, CH_UNIX,values[0], -1, value, max_len, False) == (size_t)-1) {
		DEBUG_OUT (1, ("smbldap_get_single_attribute: string conversion of [%s] = [%s] failed!\n", 
			  attribute, values[0]));
	...
	DEBUG_OUT (100, ("smbldap_get_single_attribute: [%s] = [%s]\n", attribute, value));
#endif	
	return True;
}
					...
					TALLOC_CTX *mem_ctx)
{
	...
	if (values == NULL) {
		DEBUG_OUT (10, ("attribute %s does not exist\n", attribute));
		return NULL;
	}

	if (ldap_count_values(values) != 1) {
		DEBUG_OUT (10, ("attribute %s has %d values, expected only one\n",
	...
	if (!pull_utf8_talloc(mem_ctx, &result, values[0], &converted_size)) {
		DEBUG_OUT (10, ("pull_utf8_talloc failed\n"));
		ldap_value_free(values);
		return NULL;
	}
...
#ifdef DEBUG_PASSWORDS
	DEBUG_OUT (100, ("smbldap_get_first_attribute: [%s] = [%s]\n",
		     attribute, result));
#endif
	return result;
}
				 ...
				 DATA_BLOB *blob)
{
	...
	if (ldap_count_values_len(values) != 1) {
		DEBUG_OUT (10, ("Expected one value for %s, got %d\n", attrib,
				      ...
				      const DATA_BLOB *newblob)
{
	...
	if (existed) {
		...
		if (equal) {
			/* Believe it or not, but LDAP will deny a delete and
			   ...
			   same... */
			DEBUG_OUT (10,("smbldap_make_mod: attribute |%s| not changed.\n", attribute));
			return;
		}
		...
		if (op & LDAP_MOD_BVALUES) {
			DEBUG_OUT (10,("smbldap_make_mod: deleting attribute |%s| blob\n", attribute));
			smbldap_set_mod_blob(mods, LDAP_MOD_DELETE, attribute, &oldblob);
		} else {
			DEBUG_OUT (10,("smbldap_make_mod: deleting attribute |%s| values |%s|\n", attribute, oldval));
	...
	if (op & LDAP_MOD_BVALUES) {
		if (newblob && newblob->length) {
			DEBUG_OUT (10,("smbldap_make_mod: adding attribute |%s| blob\n", attribute));
			smbldap_set_mod_blob(mods, LDAP_MOD_ADD, attribute, newblob);
		}
	} else {
		if ((newval != NULL) && (strlen(newval) > 0)) {
			DEBUG_OUT (10,("smbldap_make_mod: adding attribute |%s| value |%s|\n", attribute, newval));
...
{ 
	...
	if (version != LDAP_VERSION3) {
		DEBUG_OUT (0, ("Need LDAPv3 for Start TLS\n"));
		return LDAP_OPERATIONS_ERROR;
	}

	if ((rc = ldap_start_tls_s (ldap_struct, NULL, NULL)) != LDAP_SUCCESS)	{
		DEBUG_OUT (0,("Failed to issue the StartTLS instruction: %s\n",
			 ldap_err2string(rc)));
		return rc;
	}

	DEBUG_OUT (3, ("StartTLS issued: using a TLS connection\n"));
	return LDAP_SUCCESS;
#else
	DEBUG_OUT (0,("StartTLS not supported by LDAP client libraries!\n"));
	return LDAP_OPERATIONS_ERROR;
#endif
}
...
static int smb_ldap_setup_conn(LDAP **ldap_struct, const char *uri)
{
	...
	DEBUG_OUT (10, ("smb_ldap_setup_connection: %s\n", uri));

#ifdef HAVE_LDAP_INITIALIZE

	rc = ldap_initialize(ldap_struct, uri);
	if (rc) {
		DEBUG_OUT (0, ("ldap_initialize: %s\n", ldap_err2string(rc)));
		return rc;
	}
	...
	if (lp_ldap_follow_referral() != Auto) {
		...
		if (rc != LDAP_SUCCESS)
			DEBUG_OUT (0, ("Failed to set LDAP_OPT_REFERRALS: %s\n",
				ldap_err2string(rc)));
	}
...

	{
		...
		if (port == 0) {
			if (strequal(protocol, "ldap")) {
				...
				DEBUG_OUT (0, ("unrecognised protocol (%s)!\n", protocol));
			}
		}

		if ((*ldap_struct = ldap_init(host, port)) == NULL)	{
			DEBUG_OUT (0, ("ldap_init failed !\n"));
			return LDAP_OPERATIONS_ERROR;
		}
			...
			{
				DEBUG_OUT (0, ("Failed to setup a TLS session\n"));
			}

			DEBUG_OUT (3,("LDAPS option set...!\n"));
#else
			DEBUG_OUT (0,("smbldap_open_connection: Secure connection not supported by LDAP client libraries!\n"));
			return LDAP_OPERATIONS_ERROR;
...
#ifdef LDAP_X_OPT_CONNECT_TIMEOUT /* Netscape */
	{
		...
		if (rc != LDAP_SUCCESS) {
			DEBUG_OUT (0,("Failed to setup an ldap connection timeout %d: %s\n",
...

{
	...
	if (deref != -1) {
		if (ldap_set_option (*ldap_struct, LDAP_OPT_DEREF, &deref) != LDAP_OPT_SUCCESS) {
			DEBUG_OUT (1,("smbldap_open_connection: Failed to set dereferencing method: %d\n", deref));
		} else {
			DEBUG_OUT (5,("Set dereferencing method: %d\n", deref));
		}
	}

	DEBUG_OUT (2, ("smbldap_open_connection: connection opened\n"));
	return rc;
}
				   ...
				   int *methodp, int freeit, void *arg)
{
	...
	if (freeit) {
		...
		DEBUG_OUT (5,("rebind_proc_with_state: Rebinding as \"%s\"\n", 
					  ...
					  ber_int_t msgid, void *arg)
{
	...
	DEBUG_OUT (5,("rebindproc_connect_with_state: Rebinding to %s as \"%s\"\n", 
	...
	switch (request) {
		...
		case LDAP_REQ_EXTENDED:
			DEBUG_OUT (10,("rebindproc_connect_with_state: "
...
static int smbldap_connect_system(struct smbldap_state *ldap_state)
{
	...
	if (!ldap_state->anonymous && !ldap_state->bind_dn) {
		...
		if (!fetch_ldap_pw(&bind_dn, &bind_secret)) {
			DEBUG_OUT (0, ("ldap_connect_system: Failed to retrieve password from secrets.tdb\n"));
	...
	DEBUG_OUT (10,("ldap_connect_system: Binding to ldap server %s as \"%s\"\n",
...
# endif
	...
	if (rc != LDAP_SUCCESS) {
		...
		ldap_get_option(ldap_state->ldap_struct, LDAP_OPT_ERROR_STRING,
				&ld_error);
		DEBUG_OUT (ldap_state->num_failures ? 2 : 0,
	...
	DEBUG_OUT (3, ("ldap_connect_system: successful connection to the LDAP server\n"));
...
static int smbldap_open(struct smbldap_state *ldap_state)
{
	...
	if (ldap_state->ldap_struct != NULL) {
		DEBUG_OUT (11,("smbldap_open: already connected to the LDAP server\n"));
		return LDAP_SUCCESS;
	}
	...
	DEBUG_OUT (4,("The LDAP server is successfully connected\n"));
...
static NTSTATUS smbldap_close(struct smbldap_state *ldap_state)
{
	...
	DEBUG_OUT (5,("The connection to the LDAP server was closed\n"));
			    ...
			    int *attempts, time_t endtime)
{
	...
	while (1) {
		...
		*attempts += 1;
		...
		if (open_rc != LDAP_SUCCESS) {
			DEBUG_OUT (1, ("Connection to LDAP server failed for the "
				  "%d try!\n", *attempts));
		}
	}
			      ...
			      int sizelimit, LDAPMessage **res)
{
	...
	DEBUG_OUT (5,("smbldap_search_ext: base => [%s], filter => [%s], "
		 "scope => [%d]\n", base, filter, scope));
	...
	if (ldap_state->last_rebind.tv_sec > 0) {
		...
		if (sleep_time > 0) {
			...
			DEBUG_OUT (5,("smbldap_search_ext: waiting %d milliseconds "
				 "for LDAP replication.\n",sleep_time));
			smb_msleep(sleep_time);
			DEBUG_OUT (5,("smbldap_search_ext: go on!\n"));
		}
		ZERO_STRUCT(ldap_state->last_rebind);
	}
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			DEBUG_OUT (10, ("Failed search for base: %s, error: %d (%s) "
				   "(%s)\n", base, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
			 ...
			 LDAPMessage **res, void **cookie)
{
	...
	*res = NULL;
	...
	DEBUG_OUT (3,("smbldap_search_paged: base => [%s], filter => [%s],"
		 "scope => [%d], pagesize => [%d]\n",
		 base, filter, scope, pagesize));

	cookie_be = ber_alloc_t(LBER_USE_DER);
	if (cookie_be == NULL) {
		DEBUG_OUT (0,("smbldap_create_page_control: ber_alloc_t returns "
			 "NULL\n"));
		return LDAP_NO_MEMORY;
	}
	...
	if (rc != 0) {
		DEBUG_OUT (3,("smbldap_search_paged: smbldap_search_ext(%s) "
			 "failed with [%s]\n", filter, ldap_err2string(rc)));
		goto done;
	}

	DEBUG_OUT (3,("smbldap_search_paged: search was successful\n"));

	rc = ldap_parse_result(ldap_state->ldap_struct, *res, NULL, NULL, 
			       NULL, NULL, &rcontrols,  0);
	if (rc != 0) {
		DEBUG_OUT (3,("smbldap_search_paged: ldap_parse_result failed " \
			 "with [%s]\n", ldap_err2string(rc)));
		goto done;
	}
...
int smbldap_modify(struct smbldap_state *ldap_state, const char *dn, LDAPMod *attrs[])
{
	...
	DEBUG_OUT (5,("smbldap_modify: dn => [%s]\n", dn ));
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG_OUT (10, ("Failed to modify dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
...
int smbldap_add(struct smbldap_state *ldap_state, const char *dn, LDAPMod *attrs[])
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG_OUT (10, ("Failed to add dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
...
int smbldap_delete(struct smbldap_state *ldap_state, const char *dn)
{
	...
	DEBUG_OUT (5,("smbldap_delete: dn => [%s]\n", dn ));
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG_OUT (10, ("Failed to delete dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
			       ...
			       char **retoidp, struct berval **retdatap)
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG_OUT (10, ("Extended operation failed with error: "
				   "%d (%s) (%s)\n", ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
			    ...
			    void *private_data)
{
	...
	if (state->ldap_struct == NULL) {
		DEBUG_OUT (10,("ldap connection not connected...\n"));
		return;
	}

	if ((state->last_use+SMBLDAP_IDLE_TIME) > time_mono(NULL)) {
		DEBUG_OUT (10,("ldap connection not idle...\n"));
	...
	DEBUG_OUT (7,("ldap connection idle...closing connection\n"));
	smbldap_close(state);
}
		      ...
		      struct smbldap_state **smbldap_state)
{
	*smbldap_state = TALLOC_ZERO_P(mem_ctx, struct smbldap_state);
	if (!*smbldap_state) {
		DEBUG_OUT (0, ("talloc() failed for ldapsam private_data!\n"));
			 ...
			 LDAPMessage *entry)
{
	...
	if (!utf8_dn) {
		DEBUG_OUT (5, ("smbldap_talloc_dn: ldap_get_dn failed\n"));
		return NULL;
	}
	if (!pull_utf8_talloc(mem_ctx, &unix_dn, utf8_dn, &converted_size)) {
		DEBUG_OUT (0, ("smbldap_talloc_dn: String conversion failure utf8 "
			   "[%s]\n", utf8_dn));
		return NULL;
	}
...
static bool smbldap_check_root_dse(LDAP *ld, const char **attrs, const char *value) 
{
	...
	if (!attrs[0]) {
		DEBUG_OUT (3,("smbldap_check_root_dse: nothing to look for\n"));
		return False;
	}
	...
	if (!strequal(attrs[0], "supportedExtension") && 
		...
		DEBUG_OUT (3,("smbldap_check_root_dse: no idea what to query root-dse for: %s ?\n", attrs[0]));
		return False;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (3,("smbldap_check_root_dse: Could not search rootDSE\n"));
		return False;
	}
	...
	if (num_result != 1) {
		DEBUG_OUT (3,("smbldap_check_root_dse: Expected one rootDSE, got %d\n", num_result));
		goto done;
	}
	...
	if (entry == NULL) {
		DEBUG_OUT (3,("smbldap_check_root_dse: Could not retrieve rootDSE\n"));
		goto done;
	}
	...
	if (values == NULL) {
		DEBUG_OUT (5,("smbldap_check_root_dse: LDAP Server does not support any %s\n", attrs[0]));
		goto done;
	}
	...
	if (num_values == 0) {
		DEBUG_OUT (5,("smbldap_check_root_dse: LDAP Server does not have any %s\n", attrs[0]));
		goto done;
	}
