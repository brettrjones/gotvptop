				 ...
				 DATA_BLOB *data)
{
	...
	DEBUG_OUT (2,("Got MSG_REQ_DMALLOC_MARK: mark set\n"));
#else
	DEBUG_OUT (2,("Got MSG_REQ_DMALLOC_MARK but dmalloc not in this process\n"));
#endif
}
					...
					DATA_BLOB *data)
{
	...
	DEBUG_OUT (2,("Got MSG_REQ_DMALLOC_LOG_CHANGED: done\n"));
#else
	DEBUG_OUT (2,("Got MSG_REQ_DMALLOC_LOG_CHANGED but dmalloc not in this process\n"));
#endif
}
...
void register_dmalloc_msgs(struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (2, ("Registered MSG_REQ_DMALLOC_MARK and LOG_CHANGED\n"));
}	
