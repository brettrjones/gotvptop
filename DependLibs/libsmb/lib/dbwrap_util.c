...
static NTSTATUS dbwrap_delete_action(struct db_context * db, void *private_data)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("delete_rec returned %s\n", nt_errstr(status)));
	}
			 ...
			 void *private_data)
{
	...
	if (res != 0) {
		DEBUG_OUT (5, ("transaction_start failed\n"));
	...
	DEBUG_OUT (2, ("transaction_commit failed\n"));
	return NT_STATUS_INTERNAL_DB_CORRUPTION;
}
