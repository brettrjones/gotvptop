...
static void fault_report(int sig)
{
	...
	DEBUGSEP(0);
	DEBUG_OUT (0,("INTERNAL ERROR: Signal %d in pid %d (%s)",sig,(int)sys_getpid(),samba_version_string()));
...
static char *get_freebsd_corepath(void)
{
	...
	do {
		...
		if (ret == -1) {
			if (errno != ENOMEM) {
				DEBUG_OUT (0, ("sysctlbyname failed getting "
					  "kern.corefile %s\n",
					  strerror(errno)));
				goto err_out;
			}
...
void dump_core_setup(const char *progname)
{
	...
	if (!corepath) {
		DEBUG_OUT (0, ("Unable to setup corepath for %s: %s\n", progname,
			  strerror(errno)));
		goto out;
	}
...
#ifdef RLIMIT_CORE
	{
		...
		getrlimit(RLIMIT_CORE, &rlp);
		DEBUG_OUT (3,("Maximum core file size limits now %d(soft) %d(hard)\n",
 ...
 void dump_core(void)
{
	...
	if (called) {
		DEBUG_OUT (0, ("dump_core() called recursive\n"));
		exit(1);
	}
	called = true;
	...
	if (!lp_enable_core_files()) {
		DEBUG_OUT (0, ("Exiting on internal error (core file administratively disabled)\n"));
		exit(1);
	}
	...
	if (corepath == NULL) {
		DEBUG_OUT (0, ("Can not dump core: corepath not set up\n"));
		exit(1);
	}
	...
	if (*corepath != '\0') {
		 ...
		 */
		if (chdir(corepath) != 0) {
			DEBUG_OUT (0, ("unable to change to %s\n", corepath));
			DEBUGADD(0, ("refusing to dump core\n"));
			exit(1);
		}

		DEBUG_OUT (0,("dumping core in %s\n", corepath));
	}
