...
static int setup_out_fd(void)
{
	...
	if (fd == -1) {
		DEBUG_OUT (0,("setup_out_fd: Failed to create file %s. (%s)\n",
			path, strerror(errno) ));
		TALLOC_FREE(ctx);
		return -1;
	}

	DEBUG_OUT (10,("setup_out_fd: Created tmp file %s\n", path ));
...
static int smbrun_internal(const char *cmd, int *outfd, bool sanitize)
{
	...
	if ((pid=sys_fork()) < 0) {
		DEBUG_OUT (0,("smbrun: fork failed with error %s\n", strerror(errno) ));
		CatchChild(); 
		if (outfd) {
			close(*outfd);
			*outfd = -1;
	...
	if (outfd) {
		close(1);
		if (dup2(*outfd,1) != 1) {
			DEBUG_OUT (2,("Failed to create stdout file descriptor\n"));
			close(*outfd);
			exit(80);
		}
	}
...
int smbrunsecret(const char *cmd, const char *secret)
{
	...
	if ((pid=sys_fork()) < 0) {
		DEBUG_OUT (0, ("smbrunsecret: fork failed with error %s\n", strerror(errno)));
		CatchChild(); 
		return errno;
    	}
	...
	if (pid) {
		...
		if ( wrote != towrite ) {
		    DEBUG_OUT (0,("smbrunsecret: wrote %ld of %lu bytes\n",(long)wrote,(unsigned long)towrite));
		}
		fsync(ifd[1]);
		close(ifd[1]);
	...
	close(0);
	if (dup2(ifd[0], 0) != 0) {
		DEBUG_OUT (2,("Failed to create stdin file descriptor\n"));
		close(ifd[0]);
		exit(80);
	}
