		      ...
		      void *dest, size_t destlen, bool allow_bad_conv)
{
	...
	if (descriptor == (smb_iconv_t)-1 || descriptor == (smb_iconv_t)0) {
		if (!conv_silent)
			DEBUG_OUT (0,("convert_string_internal: Conversion not supported.\n"));
		return (size_t)-1;
	}
	...
	if(retval==(size_t)-1) {
		...
		switch(errno) {
			case EINVAL:
				...
				if (!conv_silent)
					DEBUG_OUT (3,("convert_string_internal: Conversion error: %s(%s)\n",reason,inbuf));
				if (allow_bad_conv)
					goto use_as_is;
				return (size_t)-1;
			case E2BIG:
				...
				if (!conv_silent) {
					if (from == CH_UNIX) {
						DEBUG_OUT (3,("E2BIG: convert_string(%s,%s): srclen=%u destlen=%u - '%s'\n",
							 charset_name(ic, from), charset_name(ic, to),
							(unsigned int)srclen, (unsigned int)destlen, (const char *)src));
					} else {
						DEBUG_OUT (3,("E2BIG: convert_string(%s,%s): srclen=%u destlen=%u\n",
							 charset_name(ic, from), charset_name(ic, to),
			...
			default:
				if (!conv_silent)
					DEBUG_OUT (0,("convert_string_internal: Conversion error: %s(%s)\n",reason,inbuf));
				return (size_t)-1;
		}
		/* smb_panic(reason); */
	}
...

{
	...
	*dest = NULL;
	...
	if (descriptor == (smb_iconv_t)-1 || descriptor == (smb_iconv_t)0) {
		if (!conv_silent)
			DEBUG_OUT (0,("convert_string_talloc: Conversion not supported.\n"));
		errno = EOPNOTSUPP;
		return false;
	}
  ...
  convert:
	...
	if ((destlen*2)+2 < destlen) {
		/* wrapped ! abort. */
		if (!conv_silent)
			DEBUG_OUT (0, ("convert_string_talloc: destlen wrapped !\n"));
		TALLOC_FREE(outbuf);
		errno = EOPNOTSUPP;
		return false;
	} else {
	...
	if (!ob) {
		DEBUG_OUT (0, ("convert_string_talloc: realloc failed!\n"));
		errno = ENOMEM;
		return false;
	}
	outbuf = ob;
 ...
 again:
	...
	if(retval == (size_t)-1) 		{
		...
		switch(errno) {
			case EINVAL:
				reason="Incomplete multibyte sequence";
				if (!conv_silent)
					DEBUG_OUT (3,("convert_string_talloc: Conversion error: %s(%s)\n",reason,inbuf));
				if (allow_bad_conv)
					goto use_as_is;
				break;
			case E2BIG:
		...
		if (!conv_silent)
			DEBUG_OUT (0,("Conversion error: %s(%s)\n",reason,inbuf));
		/* smb_panic(reason); */
		TALLOC_FREE(ob);
		return false;
	}
  ...
  out:
	...
	if (destlen && !ob) {
		DEBUG_OUT (0, ("convert_string_talloc: out of memory!\n"));
		errno = ENOMEM;
		return false;
	}
