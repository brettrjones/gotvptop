			    ...
			    int *ptimeout)
{
	...
	if (talloc_array_length(state->pollfd_idx) < idx_len) {
		...
		if (state->pollfd_idx == NULL) {
			DEBUG_OUT (10, ("talloc_realloc failed\n"));
			return false;
		}
	}
		     ...
		     struct pollfd *pfds, int num_pfds)
{
	...
	if ((ev->timer_events != NULL)
		...
		DEBUG_OUT (10, ("Running timed event \"%s\" %p\n",
	...
	for (fde = ev->fd_events; fde; fde = fde->next) {
		...
		if (pollfd_idx[fde->fd] >= num_pfds) {
			DEBUG_OUT (1, ("internal error: pollfd_idx[fde->fd] (%d) "
				  ">= num_pfds (%d)\n", pollfd_idx[fde->fd],
				  num_pfds));
			return false;
		}
		...
		if (pfd->fd != fde->fd) {
			DEBUG_OUT (1, ("internal error: pfd->fd (%d) "
				  "!= fde->fd (%d)\n", pollfd_idx[fde->fd],
                                  num_pfds));
			return false;
		}
					 ...
					 struct timeval *to_ret)
{
	...
	*to_ret = timeval_until(&now, &ev->timer_events->next_event);
	...
	DEBUG_OUT (10, ("timed_events_timeout: %d/%d\n", (int)to_ret->tv_sec,
		(int)to_ret->tv_usec));
			   ...
			   const char *fmt, va_list ap)
{
	...
	DEBUG_OUT (samba_level, ("s3_event: %s", s));
	free(s);
}
