...
bool serverid_parent_init(TALLOC_CTX *mem_ctx)
{
	...
	db = tdb_wrap_open(mem_ctx, lock_path("serverid.tdb"),
			   ...
			   0644);
	if (db == NULL) {
		DEBUG_OUT (1, ("could not open serverid.tdb: %s\n",
			  strerror(errno)));
		return false;
	}
	return true;
...
bool serverid_register(const struct server_id id, uint32_t msg_flags)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (1, ("Could not fetch_lock serverid.tdb record\n"));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Storing serverid.tdb record failed: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	ret = true;
				 ...
				 uint32_t msg_flags)
{
	...
	if (rec->value.dsize != sizeof(struct serverid_data)) {
		DEBUG_OUT (1, ("serverid record has unexpected size %d "
			  "(wanted %d)\n", (int)rec->value.dsize,
...
bool serverid_deregister(struct server_id id)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Deleting serverid.tdb record failed: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	ret = true;
			       ...
			       struct server_id *id, uint32_t *msg_flags)
{
	...
	if (rec->key.dsize != sizeof(key)) {
		DEBUG_OUT (1, ("Found invalid key length %d in serverid.tdb\n",
			  (int)rec->key.dsize));
		return false;
	}
	if (rec->value.dsize != sizeof(data)) {
		DEBUG_OUT (1, ("Found invalid value length %d in serverid.tdb\n",
			  (int)rec->value.dsize));
		return false;
	}
