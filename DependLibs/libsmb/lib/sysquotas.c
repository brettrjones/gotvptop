	...
	if (get_quota_command && *get_quota_command) {
		...
		switch(qtype) {
			...
			case SMB_GROUP_FS_QUOTA_TYPE:
				...
				break;
			default:
				DEBUG_OUT (0,("invalid quota type.\n"));
				return -1;
		}
		...
		DEBUG_OUT (3, ("get_quota: Running command %s\n", syscmd));
		...
		if (lines) {
			...
			DEBUG_OUT (3, ("Read output from get_quota, \"%s\"\n", line));
			...
			DEBUG_OUT (3, ("Parsed output of get_quota, ...\n"));
		...
		DEBUG_OUT (0, ("get_quota_command failed!\n"));
		return -1;
	}
...
invalid_param:
	...
	TALLOC_FREE(lines);
	DEBUG_OUT (0,("The output of get_quota_command is invalid!\n"));
	return -1;
}
...
static int command_set_quota(const char *path, enum SMB_QUOTA_TYPE qtype, unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	if (set_quota_command && *set_quota_command) {
		...
		if (lines) {
			...
			DEBUG_OUT (3, ("Read output from set_quota, \"%s\"\n", line));

			TALLOC_FREE(lines);

			return 0;
		}
		DEBUG_OUT (0, ("set_quota_command failed!\n"));
		return -1;
	}
...
int sys_get_quota(const char *path, enum SMB_QUOTA_TYPE qtype, unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	if ((ret=sys_path_to_bdev(path,&mntpath,&bdev,&fs))!=0) {
		DEBUG_OUT (0,("sys_path_to_bdev() failed for path [%s]!\n",path));
		return ret;
	}

	errno = 0;
	DEBUG_OUT (10,("sys_get_quota() uid(%u, %u)\n", (unsigned)getuid(), (unsigned)geteuid()));
	...
	for (i=0;(fs && sys_quota_backends[i].name && sys_quota_backends[i].get_quota);i++) {
		if (strcmp(fs,sys_quota_backends[i].name)==0) {
			...
			if (ret!=0) {
				DEBUG_OUT (3,("sys_get_%s_quota() failed for mntpath[%s] bdev[%s] qtype[%d] id[%d]: %s.\n",
					fs,mntpath,bdev,qtype,(qtype==SMB_GROUP_QUOTA_TYPE?id.gid:id.uid),strerror(errno)));
			} else {
				DEBUG_OUT (10,("sys_get_%s_quota() called for mntpath[%s] bdev[%s] qtype[%d] id[%d].\n",
	...
	if ((ret!=0)&& (errno == EDQUOT)) {
		DEBUG_OUT (10,("sys_get_quota() warning over quota!\n"));
		return 0;
	}
...
int sys_set_quota(const char *path, enum SMB_QUOTA_TYPE qtype, unid_t id, SMB_DISK_QUOTA *dp)
{
	...
	errno = 0;
	DEBUG_OUT (10,("sys_set_quota() uid(%u, %u)\n", (unsigned)getuid(), (unsigned)geteuid())); 
	...
	for (i=0;(fs && sys_quota_backends[i].name && sys_quota_backends[i].set_quota);i++) {
		if (strcmp(fs,sys_quota_backends[i].name)==0) {
			...
			if (ret!=0) {
				DEBUG_OUT (3,("sys_set_%s_quota() failed for mntpath[%s] bdev[%s] qtype[%d] id[%d]: %s.\n",
					fs,mntpath,bdev,qtype,(qtype==SMB_GROUP_QUOTA_TYPE?id.gid:id.uid),strerror(errno)));
			} else {
				DEBUG_OUT (10,("sys_set_%s_quota() called for mntpath[%s] bdev[%s] qtype[%d] id[%d].\n",
					fs,mntpath,bdev,qtype,(qtype==SMB_GROUP_QUOTA_TYPE?id.gid:id.uid)));
			}
			ready = True;
			break;
	...
	if ((ret!=0)&& (errno == EDQUOT)) {
		DEBUG_OUT (10,("sys_set_quota() warning over quota!\n"));
		return 0;
	}
