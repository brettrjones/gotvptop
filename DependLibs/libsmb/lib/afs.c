			      ...
			      const struct ClearToken *ct)
{
	...
	DEBUG_OUT (10, ("Got ticket string:\n%s\n", result));
			    ...
			    DATA_BLOB *ticket, struct ClearToken *ct)
{
	...
	if (!secrets_fetch_afs_key(cell, &key)) {
		DEBUG_OUT (1, ("Could not fetch AFS service key\n"));
		return False;
	}
...
bool afs_login(connection_struct *conn)
{
	...
	if (cell == NULL) {
		DEBUG_OUT (1, ("AFS username doesn't contain a @, "
			  "could not find cell\n"));
		return false;
	}
	...
	*cell = '\0';
	...
	DEBUG_OUT (10, ("Trying to log into AFS for user %s@%s\n",
		   afs_username, cell));
