				 ...
				 int *grpcnt)
{
	...
	DEBUG_OUT (10, ("getgrset returned %s\n", grplist));
	...
	*grpcnt = num_gids;
	...
	DEBUG_OUT (10, ("Found %d groups for user %s\n", *grpcnt, user));
				  ...
				  int *grpcnt)
{
	...
	if (initgroups(user, gid) == -1) {
		DEBUG_OUT (0, ("getgrouplist_internals: initgroups() failed!\n"));
		SAFE_FREE(gids_saved);
		return -1;
	}
	...
	if (sys_setgroups(gid, ngrp_saved, gids_saved) != 0) {
		/* yikes! */
		DEBUG_OUT (0,("ERROR: getgrouplist: failed to reset group list!\n"));
...
static int sys_getgrouplist(const char *user, gid_t gid, gid_t *groups, int *grpcnt)
{
	...
	DEBUG_OUT (10,("sys_getgrouplist: user [%s]\n", user));
			 ...
			 gid_t **ret_groups, uint32_t *p_ngroups)
{
	...
	if (sys_getgrouplist(user, primary_gid, temp_groups, &max_grp) == -1) {
		...
		if (sys_getgrouplist(user, primary_gid,
			...
			DEBUG_OUT (0, ("get_user_groups: failed to get the unix "
				  "group list\n"));
