						      ...
						      const DATA_BLOB *d2))
{
	...
	if ((oldval.data != NULL) && (newval.data != NULL)
		...
		/* Believe it or not, but LDAP will deny a delete and
		   ...
		   same... */
		DEBUG_OUT (10,("tldap_make_mod_blob_int: attribute |%s| not "
			  "changed.\n", attrib));
		return true;
	}
	...
	if (oldval.data != NULL) {
		...
		DEBUG_OUT (10, ("tldap_make_mod_blob_int: deleting attribute |%s|\n",
			   attrib));
	...
	if (newval.data != NULL) {
		DEBUG_OUT (10, ("tldap_make_mod_blob_int: adding attribute |%s| value len "
		       ...
		       uint64_t *presult)
{
	...
	if (str == NULL) {
		DEBUG_OUT (10, ("Could not find attribute %s\n", attr));
		return false;
	}
