...
static char * realloc_expand_env_var(char *str, char *p)
{
	...
	if ((q = strchr_m(p,')')) == NULL) {
		DEBUG_OUT (0,("expand_env_var: Unterminated environment variable [%s]\n", p));
		return str;
	}
	...
	if ((envval = getenv(envname)) == NULL) {
		DEBUG_OUT (0,("expand_env_var: Environment variable [%s] not set\n", envname));
		SAFE_FREE(envname);
		return str;
	}
...
static char *longvar_domainsid( void )
{
	...
	if ( !sid_string ) {
		DEBUG_OUT (0,("longvar_domainsid: failed to dup SID string!\n"));
	}
...
static char *get_longvar_val( const char *varname )
{
	...
	DEBUG_OUT (7,("get_longvar_val: expanding variable [%s]\n", varname));
...
static char *realloc_expand_longvar(char *str, char *p)
{
	...
	if ((q = strchr_m(p,')')) == NULL) {
		DEBUG_OUT (0,("realloc_expand_longvar: Unterminated environment variable [%s]\n", p));
		return str;
	}
	...
	if ((value = get_longvar_val(varname)) == NULL) {
		DEBUG_OUT (0,("realloc_expand_longvar: Variable [%s] not set.  Skipping\n", varname));
		return str;
	}
	...
	if (lp_nis_home_map()) {
		...
		if(automount_value && strlen(automount_value) > 0) {
			...
			if (home_path_start != NULL) {
				DEBUG_OUT (5, ("NIS lookup succeeded. "
					"Home path is: %s\n",
					home_path_start ?
						(home_path_start+1):""));
			...
			DEBUG_OUT (5, ("NIS lookup failed. Using Home path from "
			"passwd file. Home path is: %s\n", server_path ));
		}
	}
#endif

	DEBUG_OUT (4,("Home server path: %s\n", server_path));
	return server_path;
}
...
static const char *automount_server(const char *user_name)
{
	...
	if (lp_nis_home_map()) {
		...
		*p = '\0';
		server_name = srv;
		DEBUG_OUT (5, ("NIS lookup succeeded.  Home server %s\n",
					server_name));
	}
#endif

	DEBUG_OUT (4,("Home server: %s\n", server_name));
	return server_name;
}
			     ...
			     const char *str)
{
	...
	if (!str) {
		DEBUG_OUT (0,("alloc_sub_basic: NULL source string!  This should not happen\n"));
		return NULL;
	}

	a_string = SMB_STRDUP(str);
	if (a_string == NULL) {
		DEBUG_OUT (0, ("alloc_sub_basic: Out of memory!\n"));
		return NULL;
	}
			...
			gid_t gid)
{
	...
	if (a_string == NULL) {
		DEBUG_OUT (0, ("talloc_sub_specified: Out of memory!\n"));
		goto done;
	}
			 ...
			 const char *str)
{
	...
	a_string = SMB_STRDUP(str);
	if (a_string == NULL) {
		DEBUG_OUT (0, ("alloc_sub_advanced: Out of memory!\n"));
		return NULL;
	}
