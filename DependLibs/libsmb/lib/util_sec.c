...
static void assert_uid(uid_t ruid, uid_t euid)
{
	if ((euid != (uid_t)-1 && geteuid() != euid) ||
		...
		if (!non_root_mode()) {
			DEBUG_OUT (0,("Failed to set uid privileges to (%d,%d) now set to (%d,%d)\n",
				 (int)ruid, (int)euid,
...
static void assert_gid(gid_t rgid, gid_t egid)
{
	if ((egid != (gid_t)-1 && getegid() != egid) ||
		...
		if (!non_root_mode()) {
			DEBUG_OUT (0,("Failed to set gid privileges to (%d,%d) now set to (%d,%d) uid=(%d,%d)\n",
				 (int)rgid, (int)egid,
...
void set_effective_uid(uid_t uid)
{
	...
	if (setresuid(uid,uid,-1) == -1) {
		if (errno == EAGAIN) {
			DEBUG_OUT (0, ("setresuid failed with EAGAIN. uid(%d) "
