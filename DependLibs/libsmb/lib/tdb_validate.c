			      ...
			      tdb_validate_data_func validate_fn)
{
	...
	DEBUG_OUT (10,("tdb_validate_child: tdb %s freelist has %d entries\n",
	...
	DEBUG_OUT (10,("tdb_validate_child: tdb %s is good with %d entries\n",
		  tdb_name(tdb), num_entries));
	ret = 0; /* Cache is good. */

out:
	DEBUG_OUT (10,   ("tdb_validate_child: summary of validation status:\n"));
...
int tdb_validate(struct tdb_context *tdb, tdb_validate_data_func validate_fn)
{
	...
	if (tdb == NULL) {
		DEBUG_OUT (1, ("Error: tdb_validate called with tdb == NULL\n"));
		return ret;
	}

	DEBUG_OUT (5, ("tdb_validate called for tdb '%s'\n", tdb_name(tdb)));

	/* fork and let the child do the validation.
	 * benefit: no need to twist signal handlers and panic functions.
	 * just let the child panic. we catch the signal. */

	DEBUG_OUT (10, ("tdb_validate: forking to let child do validation.\n"));
	child_pid = sys_fork();
	if (child_pid == 0) {
		/* child code */
		DEBUG_OUT (10, ("tdb_validate (validation child): created\n"));
		DEBUG_OUT (10, ("tdb_validate (validation child): "
			   "calling tdb_validate_child\n"));
		exit(tdb_validate_child(tdb, validate_fn));
	}
	else if (child_pid < 0) {
		DEBUG_OUT (1, ("tdb_validate: fork for validation failed.\n"));
		goto done;
	}

	/* parent */

	DEBUG_OUT (10, ("tdb_validate: fork succeeded, child PID = %u\n",
		(unsigned int)child_pid));

	DEBUG_OUT (10, ("tdb_validate: waiting for child to finish...\n"));
	while  ((wait_pid = sys_waitpid(child_pid, &child_status, 0)) < 0) {
		if (errno == EINTR) {
			DEBUG_OUT (10, ("tdb_validate: got signal during waitpid, "
				   "retrying\n"));
			errno = 0;
			continue;
		}
		DEBUG_OUT (1, ("tdb_validate: waitpid failed with error '%s'.\n",
			  strerror(errno)));
		goto done;
	}
	if (wait_pid != child_pid) {
		DEBUG_OUT (1, ("tdb_validate: waitpid returned pid %d, "
			  "but %u was expected\n", wait_pid, (unsigned int)child_pid));
		goto done;
	}

	DEBUG_OUT (10, ("tdb_validate: validating child returned.\n"));
	if (WIFEXITED(child_status)) {
		DEBUG_OUT (10, ("tdb_validate: child exited, code %d.\n",
			   WEXITSTATUS(child_status)));
		ret = WEXITSTATUS(child_status);
	}
	if (WIFSIGNALED(child_status)) {
		DEBUG_OUT (10, ("tdb_validate: child terminated by signal %d\n",
	...
	if (WIFSTOPPED(child_status)) {
		DEBUG_OUT (10, ("tdb_validate: child was stopped by signal %d\n",
			   WSTOPSIG(child_status)));
		ret = WSTOPSIG(child_status);
	}

done:
	DEBUG_OUT (5, ("tdb_validate returning code '%d' for tdb '%s'\n", ret,
		  tdb_name(tdb)));
...
int tdb_validate_open(const char *tdb_path, tdb_validate_data_func validate_fn)
{
	...
	DEBUG_OUT (5, ("tdb_validate_open called for tdb '%s'\n", tdb_path));

	tdb = tdb_open_log(tdb_path, 0, TDB_DEFAULT, O_RDWR, 0);
	if (!tdb) {
		DEBUG_OUT (1, ("Error opening tdb %s\n", tdb_path));
		return ret;
	}
			    ...
			    TDB_DATA dbuf, void *private_data)
{
	...
	if (tdb_store(data->dst, key, dbuf, TDB_INSERT) != 0) {
		DEBUG_OUT (4, ("Failed to insert into %s: %s\n", tdb_name(data->dst),
			  strerror(errno)));
		data->success = False;
		return 1;
	}
		      ...
		      const char *dst_path, int hash_size)
{
	...
	if (stat(src_path, &st) != 0) {
		DEBUG_OUT (3, ("Could not stat '%s': %s\n", src_path,
			  strerror(errno)));
		goto done;
	}
	...
	if (src_tdb == NULL) {
		DEBUG_OUT (3, ("Failed to open tdb '%s'\n", src_path));
		goto done;
	}

	if (tdb_lockall(src_tdb) != 0) {
		DEBUG_OUT (3, ("Failed to lock tdb '%s'\n", src_path));
		goto done;
	}

	tmp_path = talloc_asprintf(ctx, "%s%s", dst_path, ".tmp");
	if (!tmp_path) {
		DEBUG_OUT (3, ("talloc fail\n"));
		goto done;
	}
	...
	dst_tdb = tdb_open_log(tmp_path,
			       ...
			       st.st_mode & 0777);
	if (dst_tdb == NULL) {
		DEBUG_OUT (3, ("Error creating tdb '%s': %s\n", tmp_path,
			  strerror(errno)));
		saved_errno = errno;
		unlink(tmp_path);
		goto done;
	...
	if (count1 < 0) {
		DEBUG_OUT (3, ("Failed to copy tdb '%s': %s\n", src_path,
			  strerror(errno)));
		tdb_close(dst_tdb);
		goto done;
	}
	...
	if (!dst_tdb) {
		DEBUG_OUT (3, ("Failed to reopen tdb '%s': %s\n", tmp_path,
			  strerror(errno)));
		goto done;
	}
	count2 = tdb_verify_basic(dst_tdb);
	if (count2 != count1) {
		DEBUG_OUT (3, ("Failed to verify result of copying tdb '%s'.\n",
			  src_path));
		tdb_close(dst_tdb);
		goto done;
	}

	DEBUG_OUT (10, ("tdb_backup: successfully copied %d entries\n", count1));
	...
	if (rename(tmp_path, dst_path) != 0) {
		DEBUG_OUT (3, ("Failed to rename '%s' to '%s': %s\n",
				   ...
				   const char *suffix)
{
	...
	if (dst_path == NULL) {
		DEBUG_OUT (3, ("error out of memory\n"));
		return ret;
	}
	...
	if (ret == 0) {
		DEBUG_OUT (5, ("moved '%s' to '%s'\n", path, dst_path));
	} else if (errno == ENOENT) {
		DEBUG_OUT (3, ("file '%s' does not exist - so not moved\n", path));
		ret = 0;
	} else {
		DEBUG_OUT (3, ("error renaming %s to %s: %s\n", path, dst_path,
			  strerror(errno)));
	}
				  ...
				  bool rename_as_last_resort_if_nospc)
{
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("backup of %s failed: %s\n", src_path, strerror(errno)));
	}
	if ((ret != 0) && (errno == ENOSPC) && retry_norotate_if_nospc)
	{
		char *rotate_path = talloc_asprintf(ctx, "%s%s", dst_path,
						    rotate_suffix);
		if (rotate_path == NULL) {
			DEBUG_OUT (10, ("talloc fail\n"));
			return -1;
		}
		DEBUG_OUT (10, ("backup of %s failed due to lack of space\n",
			   src_path));
		DEBUGADD(10, ("trying to free some space by removing rotated "
			      "dst %s\n", rotate_path));
		if (unlink(rotate_path) == -1) {
			DEBUG_OUT (10, ("unlink of %s failed: %s\n", rotate_path,
				   strerror(errno)));
		} else {
			ret = tdb_backup(ctx, src_path, dst_path, hash_size);
		}
			    ...
			    tdb_validate_data_func validate_fn)
{
	...
	ctx = talloc_new(NULL);
	if (ctx == NULL) {
		DEBUG_OUT (0, ("tdb_validate_and_backup: out of memory\n"));
		goto done;
	}
	...
	if (ret == 0) {
		DEBUG_OUT (1, ("tdb '%s' is valid\n", tdb_path));
		ret = tdb_backup_with_rotate(ctx, tdb_path, tdb_path_backup, 0,
					     rotate_suffix, True, False);
		if (ret != 0) {
			DEBUG_OUT (1, ("Error creating backup of tdb '%s'\n",
				  tdb_path));
			/* the actual validation was successful: */
			ret = 0;
		} else {
			DEBUG_OUT (1, ("Created backup '%s' of tdb '%s'\n",
				  tdb_path_backup, tdb_path));
		}
	} else {
		DEBUG_OUT (1, ("tdb '%s' is invalid\n", tdb_path));

		ret =stat(tdb_path_backup, &st);
		if (ret != 0) {
			DEBUG_OUT (5, ("Could not stat '%s': %s\n", tdb_path_backup,
				  strerror(errno)));
			DEBUG_OUT (1, ("No backup found.\n"));
		} else {
			DEBUG_OUT (1, ("backup '%s' found.\n", tdb_path_backup));
			ret = tdb_validate_open(tdb_path_backup, validate_fn);
			if (ret != 0) {
				DEBUG_OUT (1, ("Backup '%s' is invalid.\n",
					  tdb_path_backup));
			}
		}
		...
		if (ret != 0) {
			int renamed = rename_file_with_suffix(ctx, tdb_path,
							      corrupt_suffix);
			if (renamed != 0) {
				DEBUG_OUT (1, ("Error moving tdb to '%s%s'\n",
					  tdb_path, corrupt_suffix));
			} else {
				DEBUG_OUT (1, ("Corrupt tdb stored as '%s%s'\n",
					  tdb_path, corrupt_suffix));
			}
			goto done;
		}

		DEBUG_OUT (1, ("valid backup '%s' found\n", tdb_path_backup));
		ret = tdb_backup_with_rotate(ctx, tdb_path_backup, tdb_path, 0,
					     corrupt_suffix, True, True);
		if (ret != 0) {
			DEBUG_OUT (1, ("Error restoring backup from '%s'\n",
				  tdb_path_backup));
		} else {
			DEBUG_OUT (1, ("Restored tdb backup from '%s'\n",
				  tdb_path_backup));
		}
	}
