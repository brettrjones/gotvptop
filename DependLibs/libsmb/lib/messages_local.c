					 ...
					 void *_info, void *private_data)
{
	...
	DEBUG_OUT (10, ("messaging_tdb_signal_handler: sig[%d] count[%d] msgs[%d]\n",
			    ...
			    struct messaging_backend **presult)
{
	...
	if (!ctx->tdb) {
		...
		DEBUG_OUT (2, ("ERROR: Failed to initialise messages database: "
			  "%s\n", strerror(errno)));
		TALLOC_FREE(result);
		return status;
	}
	...
	if (!ctx->se) {
		NTSTATUS status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0, ("ERROR: Failed to initialise messages signal handler: "
			  "%s\n", strerror(errno)));
		TALLOC_FREE(result);
		return status;
	}
...
bool messaging_tdb_parent_init(TALLOC_CTX *mem_ctx)
{
	...
	db = tdb_wrap_open(mem_ctx, lock_path("messages.tdb"), 0,
			   ...
			   O_RDWR|O_CREAT,0600);
	if (db == NULL) {
		DEBUG_OUT (1, ("could not open messaging.tdb: %s\n",
			  strerror(errno)));
		return false;
	}
	return true;
				    ...
				    struct messaging_array **presult)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10, ("messaging_tdb_fetch:\n"));
				    ...
				    struct messaging_array *array)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10, ("messaging_tdb_store:\n"));
...
static NTSTATUS message_notify(struct server_id procid)
{
	...
	DEBUG_OUT (2,("message to process %d failed - %s\n", (int)pid,
		 strerror(errno)));
				   ...
				   struct messaging_backend *backend)
{
	...
	if ((msg_type & MSG_FLAG_LOWPRIORITY)
		...
		DEBUG_OUT (5, ("Dropping message for PID %s\n",
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_INVALID_HANDLE)) {
		DEBUG_OUT (2, ("pid %s doesn't exist - deleting messages record\n",
			  procid_str_static(&pid)));
...
static void message_dispatch(struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (10, ("message_dispatch: received_messages = %d\n",
		   ctx->received_messages));

	status = retrieve_all_messages(tdb->tdb, msg_ctx->id, NULL, &msg_array);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("message_dispatch: failed to retrieve messages: %s\n",
			   nt_errstr(status)));
		return;
	}
