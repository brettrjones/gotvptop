					     ...
					     struct registry_key **newkey)
{
	...
	if (W_ERROR_IS_OK(werr) && (action != REG_CREATED_NEW_KEY)) {
		DEBUG_OUT (10, ("Key '%s' already exists.\n", subkeyname));
		err = SBC_ERR_FILE_EXISTS;
	}
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (5, ("Error creating key %s: %s\n",
				    ...
				    const char *valstr)
{
						  ...
						  &canon_valstr))
	{
		if (canon_valname == NULL) {
			DEBUG_OUT (5, ("invalid parameter '%s' given\n",
				  valname));
		} else {
			DEBUG_OUT (5, ("invalid value '%s' given for "
				  "parameter '%s'\n", valstr, valname));
		}
	...
	if (smbconf_reg_valname_forbidden(canon_valname)) {
		DEBUG_OUT (5, ("Parameter '%s' not allowed in registry.\n",
			  canon_valname));
		err = SBC_ERR_INVALID_PARAM;
		goto done;
	}
	...
	if ((subkeyname == NULL) || (*(subkeyname +1) == '\0')) {
		DEBUG_OUT (5, ("Invalid registry key '%s' given as "
			  "smbconf section.\n", key->key->name));
		err = SBC_ERR_INVALID_PARAM;
		goto done;
	}
	...
	{
		DEBUG_OUT (5, ("Global parameter '%s' not allowed in "
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (5, ("Error adding value '%s' to "
			  "key '%s': %s\n",
					     ...
					     const char **strings)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (5, ("Error adding value '%s' to key '%s': %s\n",
...
static sbcErr smbconf_reg_delete_values(struct registry_key *key)
{
	...
	if (!W_ERROR_EQUAL(WERR_NO_MORE_ITEMS, werr)) {
		DEBUG_OUT (1, ("smbconf_reg_delete_values: "
...
static sbcErr smbconf_reg_init(struct smbconf_ctx *ctx, const char *path)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (1, ("Error creating admin token\n"));
		err = SBC_ERR_UNKNOWN_FAILURE;
		goto done;
	}
	rpd(ctx)->open = false;
	...
	if (!SBC_ERROR_IS_OK(err)) {
		DEBUG_OUT (1, ("Error opening the registry.\n"));
		goto done;
	}
