...
static bool string_match(const char *tok,const char *s)
{
	...
	if (tok[0] == '.') {			/* domain: match last fields */
		...
		if (!mydomain) {
			DEBUG_OUT (0,("Unable to get default yp domain. "
				"Try without it.\n"));
		}
		...
		DEBUG_OUT (5,("looking for %s of domain %s in netgroup %s gave %s\n",
			 hostname,
		...
		if (netgroup_ok)
			return true;
#else
		DEBUG_OUT (0,("access: netgroup support is not configured\n"));
		return false;
#endif
