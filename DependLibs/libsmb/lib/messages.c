			 ...
			 DATA_BLOB *data)
{
	...
	DEBUG_OUT (1,("INFO: Received PING message from PID %s [%s]\n",
		       ...
		       uint32_t msg_flags, void *state)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_INVALID_HANDLE)) {
		...
		DEBUG_OUT (2, ("pid %s doesn't exist\n", procid_str_static(id)));
					 ...
					 struct event_context *ev)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("messaging_tdb_init failed: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(ctx);
		return NULL;
	}
	...
	if (lp_clustering()) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("messaging_ctdb_init failed: %s\n",
				  nt_errstr(status)));
			TALLOC_FREE(ctx);
			return NULL;
		}
			  ...
			  struct server_id id)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("messaging_tdb_init failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	if (lp_clustering()) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("messaging_ctdb_init failed: %s\n",
				  nt_errstr(status)));
			return status;
		}
	}
				       ...
				       DATA_BLOB *data))
{
	...
	for (cb = msg_ctx->callbacks; cb != NULL; cb = cb->next) {
		...
		if (cb->msg_type == msg_type && private_data == cb->private_data) {
			DEBUG_OUT (5,("Overriding messaging pointer for type %u - private_data=%p\n",
			  ...
			  void *private_data)
{
	...
	for (cb = ctx->callbacks; cb; cb = next) {
		...
		if ((cb->msg_type == msg_type)
		    && (cb->private_data == private_data)) {
			DEBUG_OUT (5,("Deregistering messaging pointer for type %u - private_data=%p\n",
				  (unsigned)msg_type, private_data));
