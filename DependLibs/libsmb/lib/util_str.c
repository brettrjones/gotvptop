		...
		size_t maxlength)
{
	...
	if (len > maxlength) {
		DEBUG_OUT (0,("ERROR: string overflow by "
		...
		size_t maxlength)
{
	...
	if (src_len + dest_len > maxlength) {
		DEBUG_OUT (0,("ERROR: string overflow by %d "
			"in safe_strcat [%.50s]\n",
...
char *StrnCpy_fn(const char *fn, int line,char *dest,const char *src,size_t n)
{
	...
	if (!dest) {
		DEBUG_OUT (0,("ERROR: NULL dest in StrnCpy, "
			"called from [%s][%d]\n", fn, line));
		return(NULL);
	}
			...
			bool allow_trailing_dollar)
{
	...
	if (!in) {
		DEBUG_OUT (0, ("realloc_string_sub: out of memory!\n"));
		return NULL;
	}
	ls = (ssize_t)strlen(s);
	lp = (ssize_t)strlen(pattern);
	...
	while ((p = strstr_m(s,pattern))) {
		if (ld > 0) {
			...
			if (!string) {
				DEBUG_OUT (0, ("realloc_string_sub: "
					"out of memory!\n"));
				SAFE_FREE(in);
				return NULL;
			}
			...
			bool allow_trailing_dollar)
{
	...
	if (string == NULL) {
		DEBUG_OUT (0, ("talloc_string_sub2: "
	...
	in = SMB_STRDUP(insert);
	if (!in) {
		DEBUG_OUT (0, ("talloc_string_sub2: ENOMEM\n"));
		return NULL;
	}
	ls = (ssize_t)strlen(s);
	lp = (ssize_t)strlen(pattern);
	...
	while ((p = strstr_m(s,pattern))) {
		if (ld > 0) {
			...
			string = (char *)TALLOC_REALLOC(mem_ctx, string,
							ls + ld + 1);
			if (!string) {
				DEBUG_OUT (0, ("talloc_string_sub: out of "
					  "memory!\n"));
				SAFE_FREE(in);
				return NULL;
			}
...
char *strstr_m(const char *src, const char *findstr)
{
	...
	if (!push_ucs2_talloc(talloc_tos(), &src_w, src, &converted_size)) {
		DEBUG_OUT (0,("strstr_m: src malloc fail\n"));
		return NULL;
	}

	if (!push_ucs2_talloc(talloc_tos(), &find_w, findstr, &converted_size)) {
		TALLOC_FREE(src_w);
		DEBUG_OUT (0,("strstr_m: find malloc fail\n"));
		return NULL;
	}
	...
	*p = 0;
	if (!pull_ucs2_talloc(talloc_tos(), &s2, src_w, &converted_size)) {
		TALLOC_FREE(src_w);
		TALLOC_FREE(find_w);
		DEBUG_OUT (0,("strstr_m: dest malloc fail\n"));
		return NULL;
	}
			 ...
			 const char *domain_name )
{
	...
	while ( *list ) {
		...
		if ( !tmpstr ) {
			DEBUG_OUT (0,("str_list_sub_basic: "
	...
	while (*list) {
		...
		while ((p = strstr_m(s, pattern))) {
			...
			if (ld) {
				...
				if (!t) {
					DEBUG_OUT (0,("str_list_substitute: "
...
int ipstr_list_parse(const char *ipstr_list, struct ip_service **ip_list)
{
	...
	if ( (*ip_list = SMB_MALLOC_ARRAY(struct ip_service, count)) == NULL ) {
		DEBUG_OUT (0,("ipstr_list_parse: malloc failed for %lu entries\n",
					(unsigned long)count));
		return 0;
	}
...
char *sstring_sub(const char *src, char front, char back)
{
	...
	if (temp3 == NULL) {
		DEBUG_OUT (1,("Malloc failure in sstring_sub\n"));
		return NULL;
	}
	...
	const char *sep)
{
	...
	if (s == NULL) {
		DEBUG_OUT (0,("str_list_make: Unable to allocate memory"));
		TALLOC_FREE(list);
		return NULL;
	}
	if (!sep) sep = LIST_SEP;
