...
NTSTATUS packet_fd_read(struct packet_context *ctx)
{
	...
	if (res == -1) {
		DEBUG_OUT (10, ("ioctl(FIONREAD) failed: %s\n", strerror(errno)));
...
NTSTATUS packet_fd_read_sync(struct packet_context *ctx, int timeout)
{
	...
	if (res == 0) {
		DEBUG_OUT (10, ("poll timed out\n"));
		return NT_STATUS_IO_TIMEOUT;
	}

	if (res == -1) {
		DEBUG_OUT (10, ("poll returned %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}
	if ((revents & (POLLIN|POLLHUP|POLLERR)) == 0) {
		DEBUG_OUT (10, ("socket not readable\n"));
		return NT_STATUS_IO_TIMEOUT;
	}
