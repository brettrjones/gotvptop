...
static int tdb_chainlock_with_timeout_internal( TDB_CONTEXT *tdb, TDB_DATA key, unsigned int timeout, int rw_type)
{
	...
	if (timeout) {
		...
		if (gotalarm && (ret == -1)) {
			DEBUG_OUT (0,("tdb_chainlock_with_timeout_internal: alarm (%u) timed out for key %s in tdb %s\n",
...
static size_t tdb_pack_va(uint8 *buf, int bufsize, const char *fmt, va_list ap)
{
	...
	while (*fmt) {
		...
		case 'B': /* fixed-length string */
			...
			break;
		default:
			DEBUG_OUT (0,("Unknown tdb_pack format %c in %s\n", 
				 c, fmt));
			len = 0;
			break;
		}
	...
	DEBUG_OUT (18,("tdb_pack_va(%s, %d) -> %d\n", 
	...
	while (*fmt) {
		...
		case 'B': /* fixed-length string */
			...
			*b = (char *)SMB_MALLOC(*i);
			...
			break;
		default:
			DEBUG_OUT (0,("Unknown tdb_unpack format %c in %s\n",
				 c, fmt));
...
/****************************************************************************
 Log tdb messages via DEBUG_OUT ().
****************************************************************************/
...
static void tdb_log(TDB_CONTEXT *tdb, enum tdb_debug_level level, const char *format, ...)
{
	...
	DEBUG_OUT ((int)level, ("tdb(%s): %s", tdb_name(tdb) ? tdb_name(tdb) : "unnamed", ptr));
	SAFE_FREE(ptr);
}

/****************************************************************************
 Like tdb_open() but also setup a logging function that redirects to
 the samba DEBUG_OUT () system.
****************************************************************************/
		    ...
		    int flag)
{
	...
	if ((res = tdb_transaction_start(tdb)) != 0) {
		DEBUG_OUT (5, ("tdb_transaction_start failed\n"));
		return res;
	}

	if ((res = tdb_store(tdb, key, dbuf, flag)) != 0) {
		DEBUG_OUT (10, ("tdb_store failed\n"));
	...
	if ((res = tdb_transaction_commit(tdb)) != 0) {
		DEBUG_OUT (5, ("tdb_transaction_commit failed\n"));
	}
...
int tdb_trans_delete(struct tdb_context *tdb, TDB_DATA key)
{
	...
	if ((res = tdb_delete(tdb, key)) != 0) {
		DEBUG_OUT (10, ("tdb_delete failed\n"));
		if (tdb_transaction_cancel(tdb) != 0) {
			smb_panic("Cancelling transaction failed");
		}
		return res;
