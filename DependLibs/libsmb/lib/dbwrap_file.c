					      ...
					      TDB_DATA key)
{
 ...
 again:
	...
	if (file->fd < 0) {
		DEBUG_OUT (3, ("Could not open/create %s: %s\n",
	...
	if (ret == -1) {
		DEBUG_OUT (3, ("Could not get lock on %s: %s\n",
			  file->path, strerror(errno)));
		TALLOC_FREE(result);
		return NULL;
	}
	...
	if (sys_fstat(file->fd, &statbuf) != 0) {
		DEBUG_OUT (3, ("Could not fstat %s: %s\n",
			  file->path, strerror(errno)));
		TALLOC_FREE(result);
		return NULL;
	}
	...
	if (statbuf.st_size != 0) {
		...
		if (nread != result->value.dsize) {
			DEBUG_OUT (3, ("read_data failed: %s\n", strerror(errno)));
			TALLOC_FREE(result);
			return NULL;
		}
	}
...
static NTSTATUS db_file_store_root(int fd, TDB_DATA data)
{
	if (sys_lseek(fd, 0, SEEK_SET) != 0) {
		DEBUG_OUT (0, ("sys_lseek failed: %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	if (write_data(fd, (char *)data.dptr, data.dsize) != data.dsize) {
		DEBUG_OUT (3, ("write_data failed: %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	if (sys_ftruncate(fd, data.dsize) != 0) {
		DEBUG_OUT (3, ("sys_ftruncate failed: %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}
			    ...
			    void *private_data)
{
	...
	for (i=0; i<256; i++) {
		...
		dir = opendir(dirname);
		if (dir == NULL) {
			DEBUG_OUT (3, ("Could not open dir %s: %s\n", dirname,
				  strerror(errno)));
			TALLOC_FREE(mem_ctx);
			return -1;
		}
		...
		while ((dirent = readdir(dir)) != NULL) {
			...
			if (keyblob.data == NULL) {
				DEBUG_OUT (5, ("strhex_to_data_blob failed\n"));
				continue;
			}
				...
				int open_flags, mode_t mode)
{
	...
	if (open_flags & O_CREAT) {
		...
		if ((ret != 0) && (errno != EEXIST)) {
			DEBUG_OUT (5, ("mkdir(%s,%o) failed: %s\n", name, mode,
				  strerror(errno)));
			TALLOC_FREE(result);
			return NULL;
		}
