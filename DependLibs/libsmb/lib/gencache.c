...
static bool gencache_init(void)
{
	...
	DEBUG_OUT (5, ("Opening cache file at %s\n", cache_fname));
...
again:
	...
	if (cache) {
		...
		if (ret != 0) {
			...
			cache = NULL;
			if (!first_try) {
				DEBUG_OUT (0, ("gencache_init: tdb_check(%s) failed\n",
					  cache_fname));
				return false;
			}
			first_try = false;
			DEBUG_OUT (0, ("gencache_init: tdb_check(%s) failed - retry after CLEAR_IF_FIRST\n",
				  cache_fname));
	...
	if (!cache && (errno == EACCES)) {
		...
		cache = tdb_open_log(cache_fname, 0, TDB_DEFAULT|TDB_INCOMPATIBLE_HASH, open_flags,
				     0644);
		if (cache) {
			DEBUG_OUT (5, ("gencache_init: Opening cache file %s read-only.\n", cache_fname));
		}
	}

	if (!cache) {
		DEBUG_OUT (5, ("Attempt to open gencache.tdb has failed.\n"));
		return False;
	}
	...
	if (cache_notrans == NULL) {
		DEBUG_OUT (5, ("Opening %s failed: %s\n", cache_fname,
			  strerror(errno)));
		tdb_close(cache);
		cache = NULL;
		return false;
			    ...
			    time_t timeout)
{
	...
	if (tdb_data_cmp(string_term_tdb_data(keystr),
		...
		DEBUG_OUT (10, ("Can't store %s as a key\n", keystr));
		return false;
	}
	...
	DEBUG_OUT (10, ("Adding cache entry with key = %s and timeout ="
...
bool gencache_del(const char *keystr)
{
	...
	DEBUG_OUT (10, ("Deleting cache entry (key = %s)\n", keystr));
...
static bool gencache_pull_timeout(char *val, time_t *pres, char **pendptr)
{
	...
	if ((endptr == NULL) || (*endptr != '/')) {
		DEBUG_OUT (2, ("Invalid gencache data format: %s\n", val));
		return false;
	}
	if (pres != NULL) {
		*pres = res;
...
bool gencache_stabilize(void)
{
	...
	if (res == -1) {
		...
		DEBUG_OUT (10, ("Could not start transaction on gencache.tdb: "
			   "%s\n", tdb_errorstr(cache)));
		return false;
	}
	...
	if (res == -1) {
		...
		DEBUG_OUT (10, ("Could not start transaction on "
	...
	if (res == -1) {
		DEBUG_OUT (10, ("tdb_transaction_commit on gencache.tdb failed: "
			   "%s\n", tdb_errorstr(cache)));
		if (tdb_transaction_cancel(cache_notrans) == -1) {
			smb_panic("tdb_transaction_cancel failed\n");
		}
			...
			void *priv)
{
	...
	if (!gencache_pull_timeout((char *)val.dptr, &timeout, NULL)) {
		DEBUG_OUT (10, ("Ignoring invalid entry\n"));
		return 0;
	}
	...
	if (res == -1) {
		DEBUG_OUT (10, ("Transfer to gencache.tdb failed: %s\n",
			   tdb_errorstr(cache)));
		state->error = true;
		return -1;
	}
	...
	if (tdb_delete(cache_notrans, key) == -1) {
		DEBUG_OUT (10, ("tdb_delete from gencache_notrans.tdb failed: "
			   "%s\n", tdb_errorstr(cache_notrans)));
		state->error = true;
		return -1;
	}
				     ...
				     TDB_DATA data, void *priv)
{
	...
	DEBUG_OUT (10, ("Calling function with arguments (key=%s, timeout=%s)\n",
		   keystr, ctime(&timeout)));
			    ...
			    void *private_data, const char *pattern)
{
	...
	DEBUG_OUT (5, ("Searching cache keys with pattern %s\n", pattern));
				...
				time_t timeout, void *private_data)
{
	...
	DEBUG_OUT (10, ("Calling function with arguments "
