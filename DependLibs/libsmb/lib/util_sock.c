...
void set_socket_options(int fd, const char *options)
{
	...
	while (next_token_talloc(ctx, &options, &tok," \t,")) {
		...
		if (!socket_options[i].name) {
			DEBUG_OUT (0,("Unknown socket option %s\n",tok));
			continue;
		}
		...
		if (ret != 0) {
			...
			DEBUG_OUT (2,("Failed to set socket option %s (Error %s)\n",
				tok, strerror(errno) ));
		}
	}
			...
			struct sockaddr_storage *psa)
{
	...
	if (ret <= 0) {
		...
		if (errno == EAGAIN) {
			DEBUG_OUT (10,("read_udp_v4_socket: returned EAGAIN\n"));
		} else {
			DEBUG_OUT (2,("read_udp_v4_socket: failed. errno=%s\n",
				strerror(errno)));
		}
		return 0;
	}
	...
	if (psa->ss_family != AF_INET) {
		DEBUG_OUT (2,("read_udp_v4_socket: invalid address family %d "
			"(not IPv4)\n", (int)psa->ss_family));
		return 0;
	}

	DEBUG_OUT (10,("read_udp_v4_socket: ip %s port %d read: %lu\n",
				  ...
				  size_t *size_ret)
{
	...
	if (time_out == 0) {
		...
		while (nread < mincnt) {
			...
			if (readret == 0) {
				DEBUG_OUT (5,("read_fd_with_timeout: "
	...
	for (nread=0; nread < mincnt; ) {
		...
		if ((pollrtn == 0) ||
			...
			DEBUG_OUT (10,("read_fd_with_timeout: timeout read. "
				"select timed out.\n"));
		...
		if (readret == 0) {
			...
			DEBUG_OUT (5,("read_fd_with_timeout: timeout read. "
				"EOF from client.\n"));
			return NT_STATUS_END_OF_FILE;
		}
					  ...
					  size_t *len)
{
	...
	*len = smb_len(inbuf);
	...
	if (msg_type == SMBkeepalive) {
		DEBUG_OUT (5,("Got keepalive packet\n"));
	}

	DEBUG_OUT (10,("got smb length of %lu\n",(unsigned long)(*len)));
			 ...
			 size_t maxlen, size_t *p_len)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("read_fd_with_timeout failed, read "
			  "error = %s.\n", nt_errstr(status)));
		return status;
	}
	...
	if(len > 0) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("read_fd_with_timeout failed, read error = "
				  "%s.\n", nt_errstr(status)));
			return status;
		}
		...
		bool rebind)
{
	...
	DEBUG_OUT ( 10, ( "bind succeeded on port %d\n", port ) );
	return( res );
 }
					...
					int timeout)
{
	...
	DEBUG_OUT (3,("Connecting to %s at port %u\n", addr,	(unsigned int)port));
...
int open_udp_socket(const char *host, int port)
{
	...
	if (!interpret_string_addr(&ss, host, 0)) {
		DEBUG_OUT (10,("open_udp_socket: can't resolve name %s\n",
			host));
		return -1;
	}
				...
				socklen_t *plength)
{
	...
	if (getpeername(fd, (struct sockaddr *)pss, plength) < 0) {
		...
		DEBUG_OUT (level, ("getpeername failed. Error was %s\n",
			       strerror(errno)));
		return addr_buf;
	}
		...
		socklen_t len)
{
	...
	if (!ret || ailist == NULL) {
		DEBUG_OUT (3,("matchname: getaddrinfo failed for "
			"name %s [%s]\n",
			remotehost,
			gai_strerror(ret) ));
		return false;
	...
	if (ailist->ai_canonname == NULL ||
		...
		DEBUG_OUT (0,("matchname: host name/name mismatch: %s != %s\n",
			 remotehost,
			 ailist->ai_canonname ?
	...
	DEBUG_OUT (0,("matchname: host name/address mismatch: %s != %s\n",
		print_sockaddr_len(addr_buf,
			sizeof(addr_buf),
			pss,
			len),
...
const char *get_peer_name(int fd, bool force_lookup)
{
	...
	if (ret) {
		DEBUG_OUT (1,("get_peer_name: getnameinfo failed "
			"for %s with error %s\n",
			p,
			gai_strerror(ret)));
		...
		if (!matchname(name_buf, (struct sockaddr *)&ss, length)) {
			DEBUG_OUT (0,("Matchname failed on %s %s\n",name_buf,p));
		     ...
		     mode_t dir_perms)
{
	...
	if (lstat(socket_dir, &st) == -1) {
		if (errno == ENOENT) {
			...
			if (mkdir(socket_dir, dir_perms) == -1) {
				DEBUG_OUT (0, ("error creating socket directory "
					"%s: %s\n", socket_dir,
					strerror(errno)));
				goto out_umask;
			}
		} else {
			DEBUG_OUT (0, ("lstat failed on socket directory %s: %s\n",
		...
		if (!S_ISDIR(st.st_mode)) {
			DEBUG_OUT (0, ("socket directory %s isn't a directory\n",
				socket_dir));
			goto out_umask;
		}
		if ((st.st_uid != sec_initial_uid()) ||
				((st.st_mode & 0777) != dir_perms)) {
			DEBUG_OUT (0, ("invalid permissions on socket directory "
				"%s\n", socket_dir));
			goto out_umask;
		}
	}
	...
	if (sock == -1) {
		DEBUG_OUT (0, ("create_pipe_sock: socket error %s\n",
			strerror(errno) ));
                goto out_close;
	}
	...
	if (bind(sock, (struct sockaddr *)&sunaddr, sizeof(sunaddr)) == -1) {
		DEBUG_OUT (0, ("bind failed on pipe socket %s: %s\n", path,
			strerror(errno)));
		goto out_close;
	}

	if (listen(sock, 5) == -1) {
		DEBUG_OUT (0, ("listen failed on pipe socket %s: %s\n", path,
			strerror(errno)));
		goto out_close;
	}
...
#else
        DEBUG_OUT (0, ("create_pipe_sock: No Unix sockets on this system\n"));
        return -1;
...
const char *get_mydnsfullname(void)
{
	...
	if (gethostname(my_hostname, sizeof(my_hostname)) == -1) {
		DEBUG_OUT (0,("get_mydnsfullname: gethostname failed\n"));
		return NULL;
	}
	...
	if (!ret || res == NULL) {
		DEBUG_OUT (3,("get_mydnsfullname: getaddrinfo failed for "
			"name %s [%s]\n",
			my_hostname,
			gai_strerror(ret) ));
		return NULL;
	...
	if (res->ai_canonname == NULL) {
		DEBUG_OUT (3,("get_mydnsfullname: failed to get "
