...
bool init_names(void)
{
	...
	if (global_myname() == NULL || *global_myname() == '\0') {
		if (!set_global_myname(myhostname())) {
			DEBUG_OUT ( 0, ( "init_names: malloc fail.\n" ) );
			return False;
		}
	}
...
ssize_t write_data_at_offset(int fd, const char *buffer, size_t N, SMB_OFF_T pos)
{
	...
	while (total < N) {
		...
		if (ret == -1) {
			DEBUG_OUT (0,("write_data_at_offset: write failure. Error = %s\n", strerror(errno) ));
			return -1;
		}
		if (ret == 0) {
			return total;
			   ...
			   bool parent_longlived)
{
	...
	if (tdb_reopen_all(parent_longlived ? 1 : 0) == -1) {
		DEBUG_OUT (0,("tdb_reopen_all failed.\n"));
	...
	if (msg_ctx) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("messaging_reinit() failed: %s\n",
				 nt_errstr(status)));
		}
	}
 done:
...
void *Realloc(void *p, size_t size, bool free_old_on_error)
{
	...
	if (size == 0) {
		...
		DEBUG_OUT (2,("Realloc asked for 0 bytes\n"));
		return NULL;
	}
	...
	if (!ret) {
		...
		DEBUG_OUT (0,("Memory allocation error: failed to expand to %d bytes\n",(int)size));
	}
...
int interpret_protocol(const char *str,int def)
{
	...
	DEBUG_OUT (0,("Unrecognised protocol level %s\n",str));
...
char *automount_lookup(TALLOC_CTX *ctx, const char *user_name)
{
	...
	DEBUG_OUT (5, ("NIS+ querystring: %s\n", buffer));

	if (result = nis_list(buffer, FOLLOW_PATH|EXPAND_NAME|HARD_LOOKUP, NULL, NULL)) {
		if (result->status != NIS_SUCCESS) {
			DEBUG_OUT (3, ("NIS+ query failed: %s\n", nis_sperrno(result->status)));
		} else {
			object = result->objects.objects_val;
			if (object->zo_data.zo_type == ENTRY_OBJ) {
				entry = &object->zo_data.objdata_u.en_data;
				DEBUG_OUT (5, ("NIS+ entry type: %s\n", entry->en_type));
				DEBUG_OUT (3, ("NIS+ result: %s\n", entry->en_cols.en_cols_val[1].ec_value.ec_value_val));
	...
	if (value) {
		...
		DEBUG_OUT (4, ("NIS+ Lookup: %s resulted in %s\n",
					user_name, value));
	}
	return value;
}
...
char *automount_lookup(TALLOC_CTX *ctx, const char *user_name)
{
	...
	if ((nis_error = yp_get_default_domain(&nis_domain)) != 0) {
		DEBUG_OUT (3, ("YP Error: %s\n", yperr_string(nis_error)));
		return NULL;
	}

	DEBUG_OUT (5, ("NIS Domain: %s\n", nis_domain));
	...
	if ((nis_error = yp_match(nis_domain, nis_map, user_name,
		...
		DEBUG_OUT (3, ("YP Key not found:  while looking up \"%s\" in map \"%s\"\n", 
				user_name, nis_map));
		DEBUG_OUT (3, ("using defaults for server and home directory\n"));
	} else {
		DEBUG_OUT (3, ("YP Error: \"%s\" while looking up \"%s\" in map \"%s\"\n", 
				yperr_string(nis_error), user_name, nis_map));
	}

	if (value) {
		DEBUG_OUT (4, ("YP Lookup: %s resulted in %s\n", user_name, value));
	}
	return value;
}
#endif /* WITH_NISPLUS_HOME */
...
void smb_panic(const char *const why)
{
...
#ifdef DEVELOPER
	{
		...
		if (global_clobber_region_function) {
			DEBUG_OUT (0,("smb_panic: clobber_region() last called from [%s(%u)]\n",
	...
	DEBUG_OUT (0,("PANIC (pid %llu): %s\n",
		    (unsigned long long)sys_getpid(), why));
	log_stack_trace();

	cmd = lp_panic_action();
	if (cmd && *cmd) {
		DEBUG_OUT (0, ("smb_panic(): calling panic action [%s]\n", cmd));
		result = system(cmd);

		if (result == -1)
			DEBUG_OUT (0, ("smb_panic(): fork failed in panic action: %s\n",
					  strerror(errno)));
		else
			DEBUG_OUT (0, ("smb_panic(): action returned status %d\n",
					  WEXITSTATUS(result)));
	}
...
libunwind_failed:
	DEBUG_OUT (0, ("unable to produce a stack trace with libunwind\n"));
	...
	DEBUG_OUT (0, ("BACKTRACE: %lu stack frames:\n", 
...
#define NAMESIZE 32 /* Arbitrary */
	...
	DEBUG_OUT (0, ("BACKTRACE: %d stack frames:\n", levels));
	for (i = 0; i < levels; i++) {
...
#else
	DEBUG_OUT (0, ("unable to produce a stack trace on this platform\n"));
#endif
}
	...
	for(; namelist->name != NULL; namelist++) {
		if(namelist->is_wild) {
			if (mask_match(last_component, namelist->name, case_sensitive)) {
				DEBUG_OUT (8,("is_in_path: mask match succeeded\n"));
				return True;
			}
		} else {
			if((case_sensitive && (strcmp(last_component, namelist->name) == 0))||
						(!case_sensitive && (StrCaseCmp(last_component, namelist->name) == 0))) {
				DEBUG_OUT (8,("is_in_path: match succeeded\n"));
				return True;
			}
		}
	}
	DEBUG_OUT (8,("is_in_path: match not found\n"));
	return False;
}
...
void set_namearray(name_compare_entry **ppname_array, const char *namelist)
{
	...
	*/
	...
	if(( (*ppname_array) = SMB_MALLOC_ARRAY(name_compare_entry, num_entries + 1)) == NULL) {
		DEBUG_OUT (0,("set_namearray: malloc fail\n"));
		return;
	}
	...
	while(*nameptr) {
		...
		if(((*ppname_array)[i].name = SMB_STRDUP(nameptr)) == NULL) {
			DEBUG_OUT (0,("set_namearray: malloc fail (1)\n"));
			return;
		}
...
bool fcntl_getlock(int fd, SMB_OFF_T *poffset, SMB_OFF_T *pcount, int *ptype, pid_t *ppid)
{
	...
	DEBUG_OUT (8,("fcntl_getlock fd=%d offset=%.0f count=%.0f type=%d\n",
	...
	if (ret == -1) {
		int sav = errno;
		DEBUG_OUT (3,("fcntl_getlock: lock request failed at offset %.0f count %.0f type %d (%s)\n",
	...
	*ppid = lock.l_pid;
	...
	DEBUG_OUT (3,("fcntl_getlock: fd %d is returned info %d pid %u\n",
...
bool is_myname(const char *s)
{
	...
	DEBUG_OUT (8, ("is_myname(\"%s\") returns %d\n", s, ret));
	return(ret);
}
...
bool is_myworkgroup(const char *s)
{
	...
	DEBUG_OUT (8, ("is_myworkgroup(\"%s\") returns %d\n", s, ret));
	return(ret);
}
...
void set_remote_arch(enum remote_arch_types type)
{
	...
	DEBUG_OUT (10,("set_remote_arch: Client arch is \'%s\'\n",
				remote_arch_str));
}
...
int set_maxfiles(int requested_max)
{
	...
	if(getrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG_OUT (0,("set_maxfiles: getrlimit (1) for RLIMIT_NOFILE failed with error %s\n",
			strerror(errno) ));
		/* just guess... */
	...
	if (rlp.rlim_max != RLIM_INFINITY) {
		...
		if(setrlimit(RLIMIT_NOFILE, &rlp)) {
			DEBUG_OUT (3,("set_maxfiles: setrlimit for RLIMIT_NOFILE for %d max files failed with error %s\n", 
	...
	if(setrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG_OUT (0,("set_maxfiles: setrlimit for RLIMIT_NOFILE for %d files failed with error %s\n", 
	...
	if(getrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG_OUT (0,("set_maxfiles: getrlimit (2) for RLIMIT_NOFILE failed with error %s\n",
			strerror(errno) ));
		/* just guess... */
		return saved_current_limit;
    }
...
void *smb_xmalloc_array(size_t size, unsigned int count)
{
	...
	if ((p = SMB_MALLOC(size*count)) == NULL) {
		DEBUG_OUT (0, ("smb_xmalloc_array failed to allocate %lu * %lu bytes\n",
...
static char *xx_path(const char *name, const char *rootpath)
{
	...
	if (!directory_exist(fname)) {
		if (!mkdir(fname,0755))
			DEBUG_OUT (1, ("Unable to create directory %s for file %s. "
...
bool name_to_fqdn(fstring fqdn, const char *name)
{
	...
	if (!hp || !hp->h_name || !*hp->h_name) {
		DEBUG_OUT (10,("name_to_fqdn: lookup for %s failed.\n", name));
		fstrcpy(fqdn, name);
		return false;
	}
	...
	if (full && (StrCaseCmp(full, "localhost.localdomain") == 0)) {
		DEBUG_OUT (1, ("WARNING: your /etc/hosts file may be broken!\n"));
	...
	DEBUG_OUT (10,("name_to_fqdn: lookup for %s -> %s.\n", name, full));
	fstrcpy(fqdn, full);
	return true;
}
				 ...
				 uint32_t *pprivate_flags)
{
	...
	DEBUG_OUT (10,("map_open_params_to_ntcreate: fname = %s, deny_mode = 0x%x, "
		  "open_func = 0x%x\n",
	...
	switch (GET_OPENX_MODE(deny_mode)) {
		...
		case DOS_OPEN_FCB:
			...
			break;
		default:
			DEBUG_OUT (10,("map_open_params_to_ntcreate: bad open mode = 0x%x\n",
	...
	switch (open_func) {
		...
		default:
			...
			DEBUG_OUT (10,("map_open_params_to_ntcreate: bad "
				  "open_func 0x%x\n", (unsigned int)open_func));
			return False;
	}
	...
	switch (GET_DENY_MODE(deny_mode)) {
		...
		default:
			DEBUG_OUT (10,("map_open_params_to_ntcreate: bad deny_mode 0x%x\n",
				(unsigned int)GET_DENY_MODE(deny_mode) ));
			return False;
	}

	DEBUG_OUT (10,("map_open_params_to_ntcreate: file %s, access_mask = 0x%x, "
