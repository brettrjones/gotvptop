...
void *sys_memalign( size_t align, size_t size )
{
	...
	if (pagesize == (size_t)-1) {
		DEBUG_OUT (0,("memalign functionalaity not available on this platform!\n"));
		return NULL;
	}
	if (size < pagesize) {
		size = pagesize;
				   ...
				   bool enable)
{
	...
	cap = cap_get_proc();
	if (cap == NULL) {
		DEBUG_OUT (0,("set_process_capability: cap_get_proc failed: %s\n",
			strerror(errno)));
		return False;
	}
	...
	if (cap_set_proc(cap) == -1) {
		DEBUG_OUT (0, ("set_process_capability: cap_set_proc failed: %s\n",
			strerror(errno)));
		cap_free(cap);
		return False;
	}
...
long sys_random(void)
{
	...
	return (long)rand();
#else
	DEBUG_OUT (0,("Error - no random function available !\n"));
	exit(1);
#endif
}
...
void sys_srandom(unsigned int seed)
{
	...
	srand(seed);
#else
	DEBUG_OUT (0,("Error - no srandom function available !\n"));
	exit(1);
#endif
}
...
static int sys_broken_getgroups(int setlen, gid_t *gidset)
{
	...
	if((group_list = SMB_MALLOC_ARRAY(GID_T, setlen)) == NULL) {
		DEBUG_OUT (0,("sys_getgroups: Malloc fail.\n"));
		return -1;
	}
...
static int sys_broken_setgroups(int setlen, gid_t *gidset)
{
	...
	if((group_list = SMB_MALLOC_ARRAY(GID_T, setlen)) == NULL) {
		DEBUG_OUT (0,("sys_setgroups: Malloc fail.\n"));
		return -1;    
	}
...
static int sys_bsd_setgroups(gid_t primary_gid, int setlen, const gid_t *gidset)
{
	...
	if (setlen > max) {
		DEBUG_OUT (3, ("forced to truncate group list from %d to %d\n",
			setlen, max));
		setlen = max;
	}
...
ssize_t sys_getxattr (const char *path, const char *name, void *value, size_t size)
{
	...
	DEBUG_OUT (10,("sys_getxattr: extattr_get_file() failed with: %s\n", strerror(errno)));
	return -1;
...
ssize_t sys_lgetxattr (const char *path, const char *name, void *value, size_t size)
{
	...
	DEBUG_OUT (10,("sys_lgetxattr: extattr_get_link() failed with: %s\n", strerror(errno)));
	return -1;
#elif defined(HAVE_ATTR_GET)
...
ssize_t sys_fgetxattr (int filedes, const char *name, void *value, size_t size)
{
	...
	DEBUG_OUT (10,("sys_fgetxattr: extattr_get_fd() failed with: %s\n", strerror(errno)));
	return -1;
...
static ssize_t solaris_list_xattr(int attrdirfd, char *list, size_t size)
{
	...
	if (closedir(dirp) == -1) {
		DEBUG_OUT (0,("closedir dirp failed: %s\n",strerror(errno)));
		return -1;
	}
	return len;
}
...
static int solaris_attropen(const char *path, const char *attrpath, int oflag, mode_t mode)
{
	...
	if (filedes == -1) {
		DEBUG_OUT (10,("attropen FAILED: path: %s, name: %s, errno: %s\n",path,attrpath,strerror(errno)));
		if (errno == EINVAL) {
			errno = ENOTSUP;
		} else {
			errno = ENOATTR;
...
static int solaris_openat(int fildes, const char *path, int oflag, mode_t mode)
{
	...
	if (filedes == -1) {
		DEBUG_OUT (10,("openat FAILED: fd: %d, path: %s, errno: %s\n",filedes,path,strerror(errno)));
		if (errno == EINVAL) {
			errno = ENOTSUP;
		} else {
			errno = ENOATTR;
...
static int solaris_write_xattr(int attrfd, const char *value, size_t size)
{
	if ((ftruncate(attrfd, 0) == 0) && (write(attrfd, value, size) == size)) {
		...
		DEBUG_OUT (10,("solaris_write_xattr FAILED!\n"));
		return -1;
	}
}
#endif /*HAVE_ATTROPEN*/
