				   ...
				   struct messaging_context *msg)
{
	...
	if (result->db == NULL) {
		DEBUG_OUT (1, ("g_lock_init: Could not open g_lock.tdb"));
		TALLOC_FREE(result);
		return NULL;
	}
	return result;
			 ...
			 int *pnum_locks, struct g_lock_rec **plocks)
{
	...
	if ((data.dsize % sizeof(struct g_lock_rec)) != 0) {
		DEBUG_OUT (1, ("invalid lock record length %d\n", (int)data.dsize));
		return false;
	}
...
static void g_lock_cleanup(int *pnum_locks, struct g_lock_rec *locks)
{
	...
	DEBUG_OUT (10, ("g_lock_cleanup: %d locks\n", num_locks));
...
again:
	...
	if (rec == NULL) {
		DEBUG_OUT (10, ("fetch_locked(\"%s\") failed\n", name));
		status = NT_STATUS_LOCK_NOT_GRANTED;
		goto done;
	}

	if (!g_lock_parse(talloc_tos(), rec->value, &num_locks, &locks)) {
		DEBUG_OUT (10, ("g_lock_parse for %s failed\n", name));
	...
	for (i=0; i<num_locks; i++) {
		if (procid_equal(&self, &locks[i].pid)) {
			if (our_index != -1) {
				DEBUG_OUT (1, ("g_lock_trylock: Added ourself "
					  "twice!\n"));
				status = NT_STATUS_INTERNAL_ERROR;
				goto done;
			}
			if ((locks[i].lock_type & G_LOCK_PENDING) == 0) {
				DEBUG_OUT (1, ("g_lock_trylock: Found ourself not "
					  "pending!\n"));
				status = NT_STATUS_INTERNAL_ERROR;
				goto done;
			}
		...
		if (g_lock_conflicts(lock_type, &locks[i])) {
			...
			if (!process_exists(pid)) {
				...
				if (!NT_STATUS_IS_OK(status)) {
					DEBUG_OUT (1, ("Could not unlock dead lock "
						  "holder!\n"));
					goto done;
				}
				goto again;
	...
	if (our_index == -1) {
		...
		locks = g_lock_addrec(talloc_tos(), locks, &num_locks,
				      self, lock_type);
		if (locks == NULL) {
			DEBUG_OUT (10, ("g_lock_addrec failed\n"));
	...
	if (!NT_STATUS_IS_OK(store_status)) {
		DEBUG_OUT (1, ("rec->store failed: %s\n",
		     ...
		     enum g_lock_type lock_type, struct timeval timeout)
{
	...
	DEBUG_OUT (10, ("Trying to acquire lock %d for %s\n", (int)lock_type,
		   name));

	if (lock_type & ~1) {
		DEBUG_OUT (1, ("Got invalid lock type %d for %s\n",
			  (int)lock_type, name));
	...
	if (lp_clustering()) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("could not register retry with ctdb: %s\n",
				   nt_errstr(status)));
			goto done;
		}
	}
	...
	status = messaging_register(ctx->msg, &retry, MSG_DBWRAP_G_LOCK_RETRY,
				    g_lock_got_retry);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("messaging_register failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
	...
	while (true) {
		...
		if (NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("Got lock %s\n", name));
			break;
		}
		if (!NT_STATUS_EQUAL(status, STATUS_PENDING)) {
			DEBUG_OUT (10, ("g_lock_trylock failed: %s\n",
				   nt_errstr(status)));
			break;
		}

		DEBUG_OUT (10, ("g_lock_trylock: Did not get lock, waiting...\n"));
		...
		if (ret == -1) {
			if (errno != EINTR) {
				DEBUG_OUT (1, ("error calling select: %s\n",
					  strerror(errno)));
				status = NT_STATUS_INTERNAL_ERROR;
				break;
			}
			...
			if (timeval_expired(&timeout_end)) {
				DEBUG_OUT (10, ("g_lock_lock timed out\n"));
				status = NT_STATUS_LOCK_NOT_GRANTED;
				break;
			} else {
				DEBUG_OUT (10, ("select returned 0 but timeout not "
					   "not expired, retrying\n"));
			}
		} else if (ret != 1) {
			DEBUG_OUT (1, ("invalid return code of select: %d\n", ret));
			status = NT_STATUS_INTERNAL_ERROR;
			break;
		}
		/*
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		if (!NT_STATUS_IS_OK(unlock_status)) {
			DEBUG_OUT (1, ("Could not remove ourself from the locking "
				  "db: %s\n", nt_errstr(status)));
		}
	}
			     ...
			     DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("Got retry message from pid %s\n",
				    ...
				    struct server_id pid)
{
	...
	if (i == num_locks) {
		DEBUG_OUT (10, ("g_lock_force_unlock: Lock not found\n"));
		status = NT_STATUS_INTERNAL_ERROR;
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("g_lock_force_unlock: Could not store record: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	if ((lock_type & G_LOCK_PENDING) == 0) {
		...
		for (i=0; i<num_locks; i++) {
			...
			status = messaging_send(ctx->msg, locks[i].pid,
						...
						&data_blob_null);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (1, ("sending retry to %s failed: %s\n",
					  procid_str(talloc_tos(),
						     &locks[i].pid),
					  nt_errstr(status)));
			} else {
...
static int g_lock_locks_fn(struct db_record *rec, void *priv)
{
	...
	if ((rec->key.dsize == 0) || (rec->key.dptr[rec->key.dsize-1] != 0)) {
		DEBUG_OUT (1, ("invalid key in g_lock.tdb, ignoring\n"));
		return 0;
	}
