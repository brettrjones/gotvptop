...
static int upgrade_v2_to_v3(struct db_record *rec, void *priv)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("upgrade_v2_to_v3: Failed to delete secdesc for "
			  "%s: %s\n", rec->key.dptr, nt_errstr(status)));
		TALLOC_FREE(c_servicename);
		*p_upgrade_ok = false;
		return -1;
	} else {
		DEBUG_OUT (10, ("upgrade_v2_to_v3: deleted secdesc for "
                          "%s\n", rec->key.dptr ));
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("upgrade_v2_to_v3: Failed to store secdesc for "
		...
		*p_upgrade_ok = false;
		...
		DEBUG_OUT (10, ("upgrade_v2_to_v3: stored secdesc for "
                          "%s\n", newkey ));
	}
...
bool share_info_db_init(void)
{
	...
	if (share_db == NULL) {
		DEBUG_OUT (0,("Failed to open share info database %s (%s)\n",
	...
	if (share_db->transaction_start(share_db) != 0) {
		DEBUG_OUT (0, ("transaction_start failed\n"));
	...
	if ((vers_id == SHARE_DATABASE_VERSION_V1) || (IREV(vers_id) == SHARE_DATABASE_VERSION_V1)) {
		...
		if (dbwrap_store_int32(share_db, vstring,
			...
			DEBUG_OUT (0, ("dbwrap_store_int32 failed\n"));
			goto cancel;
		}
...
struct security_descriptor *get_share_security_default( TALLOC_CTX *ctx, size_t *psize, uint32 def_access)
{
	...
	if (!psd) {
		DEBUG_OUT (0,("get_share_security: Failed to make SEC_DESC.\n"));
		return NULL;
	}
...
bool set_share_security(const char *share_name, struct security_descriptor *psd)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("marshall_sec_desc failed: %s\n",
			  nt_errstr(status)));
		goto out;
	}
	...
	status = dbwrap_trans_store(share_db, string_term_tdb_data(key), blob,
				    TDB_REPLACE);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("set_share_security: Failed to store secdesc for "
			  "%s: %s\n", share_name, nt_errstr(status)));
		goto out;
	}

	DEBUG_OUT (5,("set_share_security: stored secdesc for %s\n", share_name ));
	ret = True;
...
bool delete_share_security(const char *servicename)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("delete_share_security: Failed to delete entry for "
			  "share %s: %s\n", c_servicename, nt_errstr(status)));
		TALLOC_FREE(c_servicename);
		return False;
	}
	...
	*ppsd = NULL;
	...
	for (i = 0; i < num_aces; i++) {
		...
		if (!next_token_talloc(ctx, &pacl, &sidstr, ":")) {
			DEBUG_OUT (0,("parse_usershare_acl: malformed usershare acl looking "
				"for ':' in string '%s'\n", pacl));
			return False;
		}

		if (!string_to_sid(&sid, sidstr)) {
			DEBUG_OUT (0,("parse_usershare_acl: failed to convert %s to sid.\n",
				sidstr ));
			return False;
		}
		...
		switch (*pacl) {
			...
			case 'd': /* Deny all to this SID. */
				...
				break;
			default:
				DEBUG_OUT (0,("parse_usershare_acl: unknown acl type at %s.\n",
					pacl ));
				return False;
		}
		...
		pacl++;
		if (*pacl && *pacl != ',') {
			DEBUG_OUT (0,("parse_usershare_acl: bad acl string at %s.\n",
				pacl ));
			return False;
		}
		pacl++; /* Go past any ',' */
	...
	if (!psd) {
		DEBUG_OUT (0,("parse_usershare_acl: Failed to make SEC_DESC.\n"));
		return False;
	}
