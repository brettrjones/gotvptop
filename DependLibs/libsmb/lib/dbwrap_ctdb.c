...
static int db_ctdb_transaction_destructor(struct db_ctdb_transaction_handle *h)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("g_lock_unlock failed: %s\n", nt_errstr(status)));
		return -1;
	}
	return 0;
}
...
static int db_ctdb_transaction_start(struct db_context *db)
{
	...
	if (!db->persistent) {
		DEBUG_OUT (0,("transactions not supported on non-persistent database 0x%08x\n", 
			 ctx->db_id));
		return -1;
	}
	...
	if (h == NULL) {
		DEBUG_OUT (0,(__location__ " oom for transaction handle\n"));		
		return -1;
	}
	...
	status = g_lock_lock(ctx->lock_ctx, h->lock_name, G_LOCK_WRITE,
			     timeval_set(86400, 0));
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("g_lock_lock failed: %s\n", nt_errstr(status)));
		TALLOC_FREE(h);
		return -1;
	}
	...
	DEBUG_OUT (5,(__location__ " Started transaction on db 0x%08x\n", ctx->db_id));
				     ...
				     TDB_DATA key, TDB_DATA *data)
{
	...
	h->m_all = db_ctdb_marshall_add(h, h->m_all, h->ctx->db_id, 1, key,
					NULL, *data);
	if (h->m_all == NULL) {
		DEBUG_OUT (0,(__location__ " Failed to add to marshalling "
			 "record\n"));
		data->dsize = 0;
...
static int db_ctdb_record_destructor(struct db_record **recp)
{
	...
	if (ret != 0) {
		DEBUG_OUT (0,(__location__ " transaction_commit failed\n"));
	}
	return 0;
}
					  ...
					  TDB_DATA key, TDB_DATA data)
{
	...
	if (h->m_write == NULL) {
		DEBUG_OUT (0,(__location__ " Failed to add to marshalling record\n"));
		talloc_free(tmp_ctx);
		return NT_STATUS_NO_MEMORY;
	}
...
static int db_ctdb_transaction_commit(struct db_context *db)
{
	...
	if (h == NULL) {
		DEBUG_OUT (0,(__location__ " transaction commit with no open transaction on db 0x%08x\n", ctx->db_id));
		return -1;
	}

	if (h->nested_cancel) {
		db->transaction_cancel(db);
		DEBUG_OUT (5,(__location__ " Failed transaction commit after nested cancel\n"));
		return -1;
	}
	...
	DEBUG_OUT (5,(__location__ " Commit transaction on db 0x%08x\n", ctx->db_id));
	...
	if (!NT_STATUS_IS_OK(rets)) {
		DEBUG_OUT (1, (__location__ " failed to fetch the db sequence number "
	...
	if (!NT_STATUS_IS_OK(rets)) {
		DEBUG_OUT (1, (__location__ "failed to store the db sequence number "
			  " in transaction commit on db 0x%08x\n", ctx->db_id));
		ret = -1;
		goto done;
	}
...
again:
	...
	if (!NT_STATUS_IS_OK(rets) || status != 0) {
		...
		if (!NT_STATUS_IS_OK(rets)) {
			DEBUG_OUT (1, (__location__ " failed to refetch db sequence "
		...
		if (new_seqnum == old_seqnum) {
			...
			DEBUG_OUT (0, (__location__ " ERROR: new_seqnum[%lu] != "
...
static int db_ctdb_transaction_cancel(struct db_context *db)
{
	...
	if (h == NULL) {
		DEBUG_OUT (0,(__location__ " transaction cancel with no open transaction on db 0x%08x\n", ctx->db_id));
		return -1;
	}
	...
	DEBUG_OUT (5,(__location__ " Cancel transaction on db 0x%08x\n", ctx->db_id));
...
static NTSTATUS db_ctdb_send_schedule_for_deletion(struct db_record *rec)
{
	...
	if (indata.dptr == NULL) {
		DEBUG_OUT (0, (__location__ " talloc failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if (!NT_STATUS_IS_OK(status) || cstatus != 0) {
		DEBUG_OUT (1, (__location__ " Error sending local control "
...
static int db_ctdb_record_destr(struct db_record* data)
{
	...
	if (tdb_chainunlock(crec->ctdb_ctx->wtdb->tdb, data->key) != 0) {
		DEBUG_OUT (0, ("tdb_chainunlock failed\n"));
		return -1;
	}
	...
	if (threshold != 0) {
		...
		if ((timediff * 1000) > threshold) {
			DEBUG_OUT (0, ("Held tdb lock %f seconds\n", timediff));
		}
	}
...
again:
	...
	if (tdb_chainlock(ctx->wtdb->tdb, key) != 0) {
		DEBUG_OUT (3, ("tdb_chainlock failed\n"));
		TALLOC_FREE(result);
		return NULL;
	}
	...
	if ((ctdb_data.dptr == NULL) ||
		...
		DEBUG_OUT (10, ("ctdb_data.dptr = %p, dmaster = %u (%u)\n",
		...
		status = ctdbd_migrate(messaging_ctdbd_connection(), ctx->db_id,
				       key);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (5, ("ctdb_migrate failed: %s\n",
				  nt_errstr(status)));
			TALLOC_FREE(result);
			return NULL;
		}
	...
	if (migrate_attempts > 10) {
		DEBUG_OUT (0, ("db_ctdb_fetch_locked needed %d attempts\n",
			  migrate_attempts));
	}
			 ...
			 TDB_DATA key, TDB_DATA *data)
{
	...
	status = ctdbd_fetch(messaging_ctdbd_connection(), ctx->db_id, key,
			     mem_ctx, data);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("ctdbd_fetch failed: %s\n", nt_errstr(status)));
		return -1;
	}
				...
				int open_flags, mode_t mode)
{
	...
	if (!lp_clustering()) {
		DEBUG_OUT (10, ("Clustering disabled -- no ctdb\n"));
		return NULL;
	}
	...
	if (conn == NULL) {
		DEBUG_OUT (1, ("Could not connect to ctdb\n"));
		TALLOC_FREE(result);
		return NULL;
	}

	if (!NT_STATUS_IS_OK(ctdbd_db_attach(conn, name, &db_ctdb->db_id, tdb_flags))) {
		DEBUG_OUT (0, ("ctdbd_db_attach failed for %s\n", name));
		TALLOC_FREE(result);
		return NULL;
	}
	...
	if (db_ctdb->wtdb == NULL) {
		DEBUG_OUT (0, ("Could not open tdb %s: %s\n", db_path, strerror(errno)));
		TALLOC_FREE(result);
		return NULL;
	}
	talloc_free(db_path);
	...
	if (result->persistent) {
		...
		if (db_ctdb->lock_ctx == NULL) {
			DEBUG_OUT (0, ("g_lock_ctx_init failed\n"));
			TALLOC_FREE(result);
			return NULL;
		}
	}
	...
	DEBUG_OUT (3,("db_open_ctdb: opened database '%s' with dbid 0x%x\n",
		 name, db_ctdb->db_id));
