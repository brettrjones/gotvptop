...
static void add_interface(const struct iface_struct *ifs)
{
	...
	if (iface_find((struct sockaddr *)&ifs->ip, False)) {
		DEBUG_OUT (3,("add_interface: not adding duplicate interface %s\n",
			print_sockaddr(addr, sizeof(addr), &ifs->ip) ));
		return;
	}

	if (!(ifs->flags & (IFF_BROADCAST|IFF_LOOPBACK))) {
		DEBUG_OUT (3,("not adding non-broadcast interface %s\n",
					ifs->name ));
		return;
	}
	...
	DEBUG_OUT (2,("added interface %s ip=%s ",
		iface->name,
		print_sockaddr(addr, sizeof(addr), &iface->ip) ));
	DEBUG_OUT (2,("bcast=%s ",
		print_sockaddr(addr, sizeof(addr),
			&iface->bcast) ));
	DEBUG_OUT (2,("netmask=%s\n",
		print_sockaddr(addr, sizeof(addr),
			&iface->netmask) ));
}
...
static void interpret_interface(char *token)
{
	...
	if (p == NULL) {
		if (!interpret_string_addr(&ss, token, 0)) {
			DEBUG_OUT (2, ("interpret_interface: Can't find address "
				  "for %s\n", token));
			return;
		}
		...
		DEBUG_OUT (2,("interpret_interface: "
	...
	*p++ = '/';
	...
	if (sockaddr_equal((struct sockaddr *)&ss_bcast, (struct sockaddr *)&ss) ||
		...
		DEBUG_OUT (2,("interpret_interface: Can't determine ip for "
			"broadcast address %s\n",
			token));
		return;
	}
	...
	DEBUG_OUT (2,("interpret_interface: Adding interface %s\n",
		token));
...
void load_interfaces(void)
{
	...
	if (total_probed > 0) {
		...
		if (!probed_ifaces) {
			DEBUG_OUT (0,("ERROR: memdup failed\n"));
			exit(1);
		}
	}
	TALLOC_FREE(ifaces);
	...
	if (!ptr || !*ptr || !**ptr) {
		if (total_probed <= 0) {
			DEBUG_OUT (0,("ERROR: Could not determine network "
	...
	if (!local_interfaces) {
		DEBUG_OUT (0,("WARNING: no network interfaces found\n"));
	}
}
