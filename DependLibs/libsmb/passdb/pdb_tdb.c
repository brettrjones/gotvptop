...
static int tdbsam_convert_one(struct db_record *rec, void *priv)
{
	...
	if (user == NULL) {
		DEBUG_OUT (0,("tdbsam_convert: samu_new() failed!\n"));
		state->success = false;
		return -1;
	}

	DEBUG_OUT (10,("tdbsam_convert: Try unpacking a record with (key:%s) "
	...
	if (!ret) {
		DEBUG_OUT (0,("tdbsam_convert: Bad struct samu entry returned "
	...
	if (data.dsize == -1) {
		DEBUG_OUT (0,("tdbsam_convert: cannot pack the struct samu into "
			 "the new format\n"));
		state->success = false;
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not store the new record: %s\n",
			  nt_errstr(status)));
		state->success = false;
		return -1;
	}
...
static bool tdbsam_convert_backup(const char *dbname, struct db_context **pp_db)
{
	...
	if (tmp_db == NULL) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: Failed to create backup TDB passwd "
			  "[%s]\n", tmp_fname));
		TALLOC_FREE(frame);
		return false;
	}
	...
	if (orig_db->transaction_start(orig_db) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: Could not start transaction (1)\n"));
		unlink(tmp_fname);
		TALLOC_FREE(tmp_db);
		TALLOC_FREE(frame);
		return false;
	...
	if (tmp_db->transaction_start(tmp_db) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: Could not start transaction (2)\n"));
	...
	bs.success = true;
        ...
        if (ret < 0) {
                DEBUG_OUT (0, ("tdbsam_convert_backup: traverse failed\n"));
                goto cancel;
        }

	if (!bs.success) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: Rewriting records failed\n"));
		goto cancel;
	}
	...
	if (rename(tmp_fname, dbname) == -1) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: rename of %s to %s failed %s\n",
			tmp_fname,
			dbname,
			strerror(errno)));
	...
	orig_db = db_open(NULL, dbname, 0,
			  TDB_DEFAULT, O_CREAT|O_RDWR, 0600);
	if (orig_db == NULL) {
		DEBUG_OUT (0, ("tdbsam_convert_backup: Failed to re-open "
			  "converted passdb TDB [%s]\n", dbname));
		return false;
	}

	DEBUG_OUT (1, ("tdbsam_convert_backup: updated %s file.\n",
		dbname ));
...
static bool tdbsam_convert(struct db_context **pp_db, const char *name, int32 from)
{
	...
	if (db_is_local(name) && !tdbsam_convert_backup(name, pp_db)) {
		DEBUG_OUT (0, ("tdbsam_convert: Could not backup %s\n", name));
		return false;
	}
	...
	if (db->transaction_start(db) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert: Could not start transaction\n"));
		return false;
	}

	if (!tdbsam_upgrade_next_rid(db)) {
		DEBUG_OUT (0, ("tdbsam_convert: tdbsam_upgrade_next_rid failed\n"));
		goto cancel;
	}

	ret = db->traverse(db, tdbsam_convert_one, &state);
	if (ret < 0) {
		DEBUG_OUT (0, ("tdbsam_convert: traverse failed\n"));
		goto cancel;
	}

	if (!state.success) {
		DEBUG_OUT (0, ("tdbsam_convert: Converting records failed\n"));
		goto cancel;
	}

	if (dbwrap_store_int32(db, TDBSAM_VERSION_STRING,
			       TDBSAM_VERSION) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert: Could not store tdbsam version\n"));
		goto cancel;
	}

	if (dbwrap_store_int32(db, TDBSAM_MINOR_VERSION_STRING,
			       TDBSAM_MINOR_VERSION) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert: Could not store tdbsam minor version\n"));
		goto cancel;
	}

	if (db->transaction_commit(db) != 0) {
		DEBUG_OUT (0, ("tdbsam_convert: Could not commit transaction\n"));
		return false;
	}
...
static bool tdbsam_open( const char *name )
{
	...
	if (db_sam == NULL) {
		DEBUG_OUT (0, ("tdbsam_open: Failed to open/create TDB passwd "
			  "[%s]\n", name));
		return false;
	}
	...
	if (version > TDBSAM_VERSION) {
		...
		DEBUG_OUT (0, ("tdbsam_open: unknown version => %d\n", version));
		TALLOC_FREE(db_sam);
		return false;
	}
	...
	if ( version < TDBSAM_VERSION ||
		...
		if (!mtx) {
			DEBUG_OUT (0, ("tdbsam_open: failed to grab mutex.\n"));
			TALLOC_FREE(db_sam);
			return false;
		}
		...
		if ( version < TDBSAM_VERSION ||
			...
			DEBUG_OUT (1, ("tdbsam_open: Converting version %d.%d database to "
				  "version %d.%d.\n",
					version,
					minor_version,
					TDBSAM_VERSION,
			...
			if ( !tdbsam_convert(&db_sam, name, version) ) {
				DEBUG_OUT (0, ("tdbsam_open: Error when trying to convert "
					  "tdbsam [%s]\n",name));
				TALLOC_FREE(db_sam);
				TALLOC_FREE(mtx);
				return false;
			...
			DEBUG_OUT (3, ("TDBSAM converted successfully.\n"));
		}
		TALLOC_FREE(mtx);
	}

	DEBUG_OUT (4,("tdbsam_open: successfully opened %s\n", name ));
				    ...
				    struct samu *user, const char *sname)
{
	...
	if ( !user ) {
		DEBUG_OUT (0,("pdb_getsampwnam: struct samu is NULL.\n"));
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG_OUT (0,("tdbsam_getsampwnam: failed to open %s!\n", tdbsam_filename));
	...
	if (!data.dptr) {
		DEBUG_OUT (5,("pdb_getsampwnam (TDB): error fetching database.\n"));
		DEBUGADD(5, (" Key: %s\n", keystr));
	...
	if (!init_samu_from_buffer(user, SAMU_BUFFER_LATEST, data.dptr, data.dsize)) {
		DEBUG_OUT (0,("pdb_getsampwent: Bad struct samu entry returned from TDB!\n"));
		SAFE_FREE(data.dptr);
		return NT_STATUS_NO_MEMORY;
	}
				    ...
				    struct samu *user, uint32 rid)
{
	...
	if ( !user ) {
		DEBUG_OUT (0,("pdb_getsampwrid: struct samu is NULL.\n"));
		return nt_status;
	}
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG_OUT (0,("tdbsam_getsampwrid: failed to open %s!\n", tdbsam_filename));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	data = dbwrap_fetch_bystring(db_sam, talloc_tos(), keystr);
	if (!data.dptr) {
		DEBUG_OUT (5,("pdb_getsampwrid (TDB): error looking up RID %d by key %s.\n", rid, keystr));
...
static bool tdb_delete_samacct_only( struct samu *sam_pass )
{
	...
	/* it's outaa here!  8^) */
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG_OUT (0,("tdb_delete_samacct_only: failed to open %s!\n",
			 tdbsam_filename));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("Error deleting entry from tdb passwd "
			  "database: %s!\n", nt_errstr(status)));
		return false;
	}
					  ...
					  struct samu *sam_pass)
{
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG_OUT (0,("tdbsam_delete_sam_account: failed to open %s!\n",
			 tdbsam_filename));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if (db_sam->transaction_start(db_sam) != 0) {
		DEBUG_OUT (0, ("Could not start transaction\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	nt_status = dbwrap_delete_bystring(db_sam, keystr);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (5, ("Error deleting entry from tdb rid "
			  "database: %s!\n", nt_errstr(nt_status)));
		goto cancel;
	}

	if (db_sam->transaction_commit(db_sam) != 0) {
		DEBUG_OUT (0, ("Could not commit transaction\n"));
...
static bool tdb_update_samacct_only( struct samu* newpwd, int flag )
{
	...
	if ( (data.dsize=init_buffer_from_samu(&buf, newpwd, False)) == -1 ) {
		DEBUG_OUT (0,("tdb_update_sam: ERROR - Unable to copy struct samu info BYTE buffer!\n"));
		goto done;
	}
	data.dptr = buf;
	...
	DEBUG_OUT (5, ("Storing %saccount %s with RID %d\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Unable to modify passwd TDB: %s!",
			  nt_errstr(status)));
		goto done;
	}
...
static bool tdb_update_ridrec_only( struct samu* newpwd, int flag )
{
	...
	status = dbwrap_store_bystring(db_sam, keystr, data, flag);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Unable to modify TDB passwd: %s!\n",
			  nt_errstr(status)));
		return false;
	}
			   ...
			   int flag)
{
	...
	if (!(newrid = pdb_get_user_rid(newpwd))) {
		DEBUG_OUT (0,("tdb_update_sam: struct samu (%s) with no RID!\n",
	...
	if (flag == TDB_MODIFY) {
		...
		if (account == NULL) {
			DEBUG_OUT (0,("tdb_update_sam: samu_new() failed\n"));
			goto cancel;
		}
		if (!NT_STATUS_IS_OK(tdbsam_getsampwnam(my_methods, account, pdb_get_username(newpwd)))) {
			DEBUG_OUT (0,("tdb_update_sam: tdbsam_getsampwnam() for %s failed\n",
				pdb_get_username(newpwd)));
			TALLOC_FREE(account);
			goto cancel;
		}
		if (!(oldrid = pdb_get_user_rid(account))) {
			DEBUG_OUT (0,("tdb_update_sam: pdb_get_user_rid() failed\n"));
			TALLOC_FREE(account);
			goto cancel;
		}
		TALLOC_FREE(account);
	...
	if (flag == TDB_MODIFY && newrid != oldrid) { 
		...
		/* Delete old RID key */
		DEBUG_OUT (10, ("tdb_update_sam: Deleting key for RID %u\n", oldrid));
		slprintf(keystr, sizeof(keystr) - 1, "%s%.8x", RIDPREFIX, oldrid);
		if (!NT_STATUS_IS_OK(dbwrap_delete_bystring(db_sam, keystr))) {
			DEBUG_OUT (0, ("tdb_update_sam: Can't delete %s\n", keystr));
			goto cancel;
		}
		/* Insert new RID key */
		DEBUG_OUT (10, ("tdb_update_sam: Inserting key for RID %u\n", newrid));
		if (!tdb_update_ridrec_only(newpwd, TDB_INSERT)) {
			goto cancel;
		}
	} else {
		DEBUG_OUT (10, ("tdb_update_sam: %s key for RID %u\n",
					  ...
					  const char *newname)
{
	...
	if (db_sam->transaction_commit(db_sam) != 0) {
		 ...
		 * account back?
		 */
		DEBUG_OUT (0, ("transaction_commit failed\n"));
		TALLOC_FREE(new_acct);
		return NT_STATUS_INTERNAL_DB_CORRUPTION;	
	}
...
static bool tdbsam_new_rid(struct pdb_methods *methods, uint32 *prid)
{
	...
	if (!tdbsam_open(tdbsam_filename)) {
		DEBUG_OUT (0,("tdbsam_new_rid: failed to open %s!\n",
			tdbsam_filename));
		return false;
	}
	...
	status = dbwrap_trans_change_uint32_atomic(db_sam, NEXT_RID_STRING,
						   &rid, 1);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("tdbsam_new_rid: Failed to increase %s: %s\n",
				     ...
				     struct samr_displayentry *entry)
{
 ...
 again:
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("tdbsam_getsampwrid failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(user);
		return false;
	}
