...
NTSTATUS smb_register_passdb(int version, const char *name, pdb_init_function init) 
{
	...
	if(version != PASSDB_INTERFACE_VERSION) {
		DEBUG_OUT (0,("Can't register passdb backend!\n"
	...
	DEBUG_OUT (5,("Attempting to register passdb backend %s\n", name));

	/* Check for duplicates */
	if (pdb_find_backend_entry(name)) {
		DEBUG_OUT (0,("There already is a passdb backend registered with the name %s!\n", name));
	...
	DEBUG_OUT (5,("Successfully added passdb backend '%s'\n", name));
	return NT_STATUS_OK;
}
...
NTSTATUS make_pdb_method_name(struct pdb_methods **methods, const char *selected)
{
	...
	DEBUG_OUT (5,("Attempting to find a passdb backend to match %s (%s)\n", selected, module_name));

	entry = pdb_find_backend_entry(module_name);

	/* Try to find a module that contains this module */
	if (!entry) { 
		DEBUG_OUT (2,("No builtin backend found, trying to load plugin\n"));
		if(NT_STATUS_IS_OK(smb_probe_module("pdb", module_name)) && !(entry = pdb_find_backend_entry(module_name))) {
			DEBUG_OUT (0,("Plugin is available, but doesn't register passdb backend %s\n", module_name));
	...
	if(!entry) { 
		DEBUG_OUT (0,("No builtin nor plugin backend for %s found\n", module_name));
		SAFE_FREE(module_name);
		return NT_STATUS_INVALID_PARAMETER;
	}

	DEBUG_OUT (5,("Found pdb backend %s\n", module_name));

	if ( !NT_STATUS_IS_OK( nt_status = entry->init(methods, module_location) ) ) {
		DEBUG_OUT (0,("pdb backend %s did not correctly init (error was %s)\n", 
	...
	DEBUG_OUT (5,("pdb backend %s has a valid init\n", selected));
...
static bool pdb_try_account_unlock(struct samu *sampass)
{
	...
	if ((acb_info & ACB_NORMAL) && (acb_info & ACB_AUTOLOCK)) {
		...
		ok = pdb_get_account_policy(PDB_POLICY_LOCK_ACCOUNT_DURATION,
					    &lockout_duration);
		if (!ok) {
			DEBUG_OUT (0, ("pdb_try_account_unlock: "
		...
		if (lockout_duration == (uint32_t) -1 ||
		    lockout_duration == 0) {
			DEBUG_OUT (9, ("pdb_try_account_unlock: No reset duration, "
		...
		if (bad_password_time == (time_t) 0) {
			DEBUG_OUT (2, ("pdb_try_account_unlock: Account %s "
		...
		if ((bad_password_time +
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (0, ("_samr_OpenUser: Couldn't "
					  "update account %s - %s\n",
					  pdb_get_username(sampass),
					  nt_errstr(status)));
				return false;
...
bool pdb_getsampwnam(struct samu *sam_acct, const char *username) 
{
	...
	if (!ok) {
		DEBUG_OUT (1, ("pdb_getsampwnam: Failed to unlock account %s\n",
			  username));
	}
...
static bool guest_user_info( struct samu *user )
{
	...
	if (pwd == NULL) {
		DEBUG_OUT (0,("guest_user_info: Unable to locate guest account [%s]!\n", 
			guestname));
		return False;
	}
...
bool pdb_getsampwsid(struct samu *sam_acct, const struct dom_sid *sid)
{
	...
	if ( rid == DOMAIN_RID_GUEST ) {
		DEBUG_OUT (6,("pdb_getsampwsid: Building guest account\n"));
	...
	ok = pdb_try_account_unlock(sam_acct);
	if (!ok) {
		DEBUG_OUT (1, ("pdb_getsampwsid: Failed to unlock account %s\n",
			  sam_acct->username));
	}
					...
					uint32_t acb_info, uint32_t *rid)
{
	...
	if ( !(pwd = Get_Pwnam_alloc(tmp_ctx, name)) ) {
		...
		if (!add_script || add_script[0] == '\0') {
			DEBUG_OUT (3, ("Could not find user %s and no add script "
				  "defined\n", name));
		...
		DEBUG_OUT (add_ret ? 0 : 3, ("_samr_create_user: Running the command `%s' gave %d\n",
					add_script, add_ret));
		if (add_ret == 0) {
		...
		if(pwd == NULL) {
			DEBUG_OUT (3, ("Could not find user %s, add script did not work\n", name));
			return NT_STATUS_NO_SUCH_USER;
		}
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("pdb_default_create_user: failed to create a new user structure: %s\n", nt_errstr(status)));
		return status;
	}

	if (!sid_peek_check_rid(get_global_sam_sid(),
				pdb_get_user_sid(sam_pass), rid)) {
		DEBUG_OUT (0, ("Could not get RID of fresh user\n"));
...
static int smb_delete_user(const char *unix_user)
{
	...
	if ( strequal( unix_user, "root" ) ) {
		DEBUG_OUT (0,("smb_delete_user: Refusing to delete local system root account!\n"));
		return -1;
	}
	...
	DEBUG_OUT (ret ? 0 : 3,("smb_delete_user: Running the command `%s' gave %d\n",del_script,ret));
					     ...
					     uint32_t rid)
{
	...
	if (!get_domain_group_from_sid(group_sid, &map)) {
		DEBUG_OUT (10, ("Could not find group for rid %d\n", rid));
...
bool pdb_new_rid(uint32_t *rid)
{
	...
	if ((pdb_capabilities() & PDB_CAP_STORE_RIDS) == 0) {
		DEBUG_OUT (0, ("Trying to allocate a RID when algorithmic RIDs "
			  "are active\n"));
		return False;
	}

	if (algorithmic_rid_base() != BASE_RID) {
		DEBUG_OUT (0, ("'algorithmic rid base' is set but a passdb backend "
	...
	if ( (ctx = talloc_init("pdb_new_rid")) == NULL ) {
		DEBUG_OUT (0,("pdb_new_rid: Talloc initialization failure\n"));
		return False;
	}
	...
	if ( allocated_rid == 0 ) {
		DEBUG_OUT (0,("pdb_new_rid: Failed to find unused RID\n"));
		return False;
	}
				   ...
				   struct dom_sid *sid)
{
	...
	if ( !unix_pw ) {
		DEBUG_OUT (4,("pdb_default_uid_to_sid: host has no idea of uid "
			 "%lu\n", (unsigned long)uid));
		return False;
	}

	if ( !(sampw = samu_new( NULL )) ) {
		DEBUG_OUT (0,("pdb_default_uid_to_sid: samu_new() failed!\n"));
		return False;
	}
	...
	if (!ret) {
		DEBUG_OUT (5, ("pdb_default_uid_to_sid: Did not find user "
				  ...
				  union unid_t *id, enum lsa_SidType *type)
{
	...
	if (sid_check_is_in_builtin(sid) ||
		...
		if (!NT_STATUS_IS_OK(methods->getgrsid(methods, &map, *sid))) {
			DEBUG_OUT (10, ("Could not find map for sid %s\n",
				   sid_string_dbg(sid)));
			goto done;
		}
		if ((map.sid_name_use != SID_NAME_ALIAS) &&
		    (map.sid_name_use != SID_NAME_WKN_GRP)) {
			DEBUG_OUT (10, ("Map for sid %s is a %s, expected an "
				   "alias\n", sid_string_dbg(sid),
	...
	DEBUG_OUT (5, ("Sid %s is neither ours, a Unix SID, nor builtin\n",
		  sid_string_dbg(sid)));
	...
	*pp_member_rids = TALLOC_ZERO_ARRAY(mem_ctx, uint32_t, num_uids);
	...
	for (i=0; i<num_uids; i++) {
		...
		if (!sid_check_is_in_our_domain(&sid)) {
			DEBUG_OUT (5, ("Inconsistent SAM -- group member uid not "
				  "in our domain\n"));
			continue;
		}
				  ...
				  union unid_t *unix_id)
{
	...
	*psid_name_use = SID_NAME_UNKNOWN;
	...
	DEBUG_OUT (5,("lookup_global_sam_rid: looking up RID %u.\n",
		 (unsigned int)rid));
	...
	if ( unix_id ) {
		DEBUG_OUT (5, ("Can't find a unix id for an unmapped group\n"));
		return False;
	}
					 ...
					 enum lsa_SidType *attrs)
{
	...
	for (i = 0; i < num_names; i++) {
		if (lookup_global_sam_name(names[i], 0, &rids[i], &attrs[i])) {
			DEBUG_OUT (5,("lookup_names: %s-> %d:%d\n", names[i],
				 rids[i], attrs[i]));
			have_mapped = True;
		} else {
			have_unmapped = True;
				 ...
				 const struct dom_sid *sid, enum lsa_SidType type)
{
	...
	if (!pdb_enum_group_mapping(sid, type, &state->groups, &state->num_groups,
				    True)) {
		DEBUG_OUT (0, ("Could not enum groups\n"));
		return False;
	}
