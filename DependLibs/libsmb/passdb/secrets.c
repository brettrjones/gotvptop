                                           ...
                                           struct dom_sid *sid, time_t *pass_last_set_time)
{
	...
	if (!(blob.data = (uint8_t *)secrets_fetch(trustdom_keystr(domain),
						   &blob.length))) {
		DEBUG_OUT (5, ("secrets_fetch failed!\n"));
		return False;
	}
...
bool secrets_store_ldap_pw(const char* dn, char* pw)
{
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, dn) < 0) {
		DEBUG_OUT (0, ("secrets_store_ldap_pw: asprintf failed!\n"));
		return False;
	}
...
bool fetch_ldap_pw(char **dn, char** pw)
{
	...
	*dn = smb_xstrdup(lp_ldap_admin_dn());
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, *dn) < 0) {
		SAFE_FREE(*dn);
		DEBUG_OUT (0, ("fetch_ldap_pw: asprintf failed!\n"));
		return false;
	}
	...
	*pw=(char *)secrets_fetch(key, &size);
	...
	if (!size) {
		...
		if (!old_style_key) {
			DEBUG_OUT (0, ("fetch_ldap_pw: strdup failed!\n"));
			return False;
		}
		...
		if ((data == NULL) || (size < sizeof(old_style_pw))) {
			DEBUG_OUT (0,("fetch_ldap_pw: neither ldap secret retrieved!\n"));
		...
		if (!secrets_store_ldap_pw(*dn, old_style_pw)) {
			DEBUG_OUT (0,("fetch_ldap_pw: ldap secret could not be upgraded!\n"));
			SAFE_FREE(old_style_key);
			SAFE_FREE(*dn);
			return False;
		}
		if (!secrets_delete(old_style_key)) {
			DEBUG_OUT (0,("fetch_ldap_pw: old ldap secret could not be deleted!\n"));
		}
...
static int list_trusted_domain(struct db_record *rec, void *private_data)
{
	...
	if (pass.domain_sid.num_auths != 4) {
		DEBUG_OUT (0, ("SID %s is not a domain sid, has %d "
			  "auths instead of 4\n",
...
void secrets_fetch_ipc_userpass(char **username, char **domain, char **password)
{
	...
	*password = (char *)secrets_fetch(SECRETS_AUTH_PASSWORD, NULL);
	...
	if (*username && **username) {
		...
		DEBUG_OUT (3, ("IPC$ connections done by user %s\\%s\n",
			  *domain, *username));

	} else {
		DEBUG_OUT (3, ("IPC$ connections done anonymously\n"));
...
char *secrets_fetch_generic(const char *owner, const char *key)
{
	...
	if (( ! owner) || ( ! key)) {
		DEBUG_OUT (1, ("Invalid Parameters"));
		return NULL;
	}
