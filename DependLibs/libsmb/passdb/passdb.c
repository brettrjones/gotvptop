...
struct samu *samu_new( TALLOC_CTX *ctx )
{
	...
	if ( !(user = TALLOC_ZERO_P( ctx, struct samu )) ) {
		DEBUG_OUT (0,("samuser_new: Talloc failed!\n"));
		return NULL;
	}
...
static NTSTATUS samu_set_unix_internal(struct samu *user, const struct passwd *pwd, bool create)
{
	...
	if (pwd->pw_name[strlen(pwd->pw_name)-1] == '$') {
		...
		if (!pdb_set_acct_ctrl(user, ACB_WSTRUST, PDB_DEFAULT)) {
			DEBUG_OUT (1, ("Failed to set 'workstation account' flags for user %s.\n", 
				pwd->pw_name));
		...
		if (!pdb_set_acct_ctrl(user, ACB_NORMAL, PDB_DEFAULT)) {
			DEBUG_OUT (1, ("Failed to set 'normal account' flags for user %s.\n", 
				pwd->pw_name));
	...
	if ( create && (pdb_capabilities() & PDB_CAP_STORE_RIDS)) {
		...
		if ( !pdb_new_rid( &user_rid ) ) {
			DEBUG_OUT (3, ("Could not allocate a new RID\n"));
		...
		if ( !pdb_set_user_sid(user, &user_sid, PDB_SET) ) {
			DEBUG_OUT (3, ("pdb_set_user_sid failed\n"));
...
int algorithmic_rid_base(void)
{
	...
	if (rid_offset < BASE_RID) {  
		...
		DEBUG_OUT (0, ("'algorithmic rid base' must be equal to or above %ld\n", BASE_RID));
		rid_offset = BASE_RID;
	}
	if (rid_offset & 1) {
		DEBUG_OUT (0, ("algorithmic rid base must be even\n"));
		rid_offset += 1;
	}
	return rid_offset;
}
			    ...
			    enum lsa_SidType *type)
{
	...
	if ((flags & LOOKUP_NAME_GROUP) == 0) {
		...
		if (ret) {
			if (!sid_check_is_in_our_domain(&user_sid)) {
				DEBUG_OUT (0, ("User %s with invalid SID %s in passdb\n",
	...
	if (!sid_check_is_in_our_domain(&map.sid)) {
		DEBUG_OUT (10, ("Found group %s (%s) not in our domain -- "
...
static bool init_samu_from_buffer_v0(struct samu *sampass, uint8_t *buf, uint32_t buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG_OUT (0, ("init_samu_from_buffer_v0: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v1(struct samu *sampass, uint8_t *buf, uint32_t buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG_OUT (0, ("init_samu_from_buffer_v1: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v2(struct samu *sampass, uint8_t *buf, uint32_t buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG_OUT (0, ("init_samu_from_buffer_v2: NULL parameters found!\n"));
		return False;
	}
...
static bool init_samu_from_buffer_v3(struct samu *sampass, uint8_t *buf, uint32_t buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG_OUT (0, ("init_samu_from_buffer_v3: NULL parameters found!\n"));
		return False;
	}
...
/* SAMU_BUFFER_FORMAT_V3       "dddddddBBBBBBBBBBBBddBBBdwdBwwd" */
	...
	if ( (*buf=(uint8_t*)SMB_MALLOC(len)) == NULL) {
		DEBUG_OUT (0,("init_buffer_from_samu_v3: Unable to malloc() memory for buffer!\n"));
		return (-1);
	}
	...
	if (buflen != len) {
		DEBUG_OUT (0, ("init_buffer_from_samu_v3: somthing odd is going on here: bufflen (%lu) != len (%lu) in tdb_pack operations!\n", 
...
bool pdb_update_bad_password_count(struct samu *sampass, bool *updated)
{
	...
	if (!BadPasswordCount) {
		DEBUG_OUT (9, ("No bad password attempts.\n"));
		return True;
	}
	...
	if (!res) {
		DEBUG_OUT (0, ("pdb_update_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* First, check if there is a reset time to compare */
	if ((resettime == (uint32_t) -1) || (resettime == 0)) {
		DEBUG_OUT (9, ("No reset time, can't reset bad pw count\n"));
		return True;
	}

	LastBadPassword = pdb_get_bad_password_time(sampass);
	DEBUG_OUT (7, ("LastBadPassword=%d, resettime=%d, current time=%d.\n", 
...
bool pdb_update_autolock_flag(struct samu *sampass, bool *updated)
{
	...
	if (!(pdb_get_acct_ctrl(sampass) & ACB_AUTOLOCK)) {
		DEBUG_OUT (9, ("pdb_update_autolock_flag: Account %s not autolocked, no check needed\n",
	...
	if (!res) {
		DEBUG_OUT (0, ("pdb_update_autolock_flag: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* First, check if there is a duration to compare */
	if ((duration == (uint32_t) -1)  || (duration == 0)) {
		DEBUG_OUT (9, ("pdb_update_autolock_flag: No reset duration, can't reset autolock\n"));
		return True;
	}

	LastBadPassword = pdb_get_bad_password_time(sampass);
	DEBUG_OUT (7, ("pdb_update_autolock_flag: Account %s, LastBadPassword=%d, duration=%d, current time =%d.\n",
		  pdb_get_username(sampass), (uint32_t)LastBadPassword, duration*60, (uint32_t)time(NULL)));

	if (LastBadPassword == (time_t)0) {
		DEBUG_OUT (1,("pdb_update_autolock_flag: Account %s "
...
bool pdb_increment_bad_password_count(struct samu *sampass)
{
	...
	unbecome_root();
	if ( !ret ) {
		DEBUG_OUT (0, ("pdb_increment_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* If there is no policy, we don't need to continue checking */
	if (!account_policy_lockout) {
		DEBUG_OUT (9, ("No lockout policy, don't track bad passwords\n"));
		return True;
	}
	...
	*/
	...
	if (!pdb_set_acct_ctrl(sampass,
			       pdb_get_acct_ctrl(sampass) | ACB_AUTOLOCK,
			       PDB_CHANGED)) {
		DEBUG_OUT (1, ("pdb_increment_bad_password_count:failed to set 'autolock' flag. \n")); 
		return False;
	}
			...
			enum netr_SchannelType *channel)
{
	...
	if (is_dc_trusted_domain_situation(domain)) {
		...
		if (!pdb_get_trusteddom_pw(domain, ret_pwd, NULL,
					   &last_set_time))
		{
			DEBUG_OUT (0, ("get_trust_pw: could not fetch trust "
	...
	DEBUG_OUT (5, ("get_trust_pw_clear: could not fetch clear text trust "
		       ...
		       enum netr_SchannelType *channel)
{
	...
	DEBUG_OUT (5, ("get_trust_pw_hash: could not fetch trust account "
