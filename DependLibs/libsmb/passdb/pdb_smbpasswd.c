...
static bool do_file_lock(int fd, int waitsecs, int type)
{
	...
	if (gotalarm && ret == -1) {
		DEBUG_OUT (0, ("do_file_lock: failed to %s file.\n",
...
static bool pw_file_lock(int fd, int type, int secs, int *plock_depth)
{
	...
	if(*plock_depth == 0) {
		if (!do_file_lock(fd, secs, type)) {
			DEBUG_OUT (10,("pw_file_lock: locking file failed, error = %s.\n",
				strerror(errno)));
			return False;
		}
	}
...
static bool pw_file_unlock(int fd, int *plock_depth)
{
	...
	if(!ret) {
		DEBUG_OUT (10,("pw_file_unlock: unlocking file failed, error = %s.\n",
			strerror(errno)));
	}
	return ret;
}
...
static FILE *startsmbfilepwent(const char *pfile, enum pwf_access_type type, int *lock_depth)
{
	...
	if (!*pfile) {
		DEBUG_OUT (0, ("startsmbfilepwent: No SMB password file set\n"));
		return (NULL);
	}
	...
	switch(type) {
		...
		case PWF_CREATE:
			 ...
			 */
			{
				...
				if(fd == -1) {
					DEBUG_OUT (0,("startsmbfilepwent_internal: too many race conditions \
creating file %s\n", pfile));
					return NULL;
				}
				close(fd);
		...
		default:
			DEBUG_OUT (10, ("Invalid open mode: %d\n", type));
			return NULL;
	}

	for(race_loop = 0; race_loop < 5; race_loop++) {
		DEBUG_OUT (10, ("startsmbfilepwent_internal: opening file %s\n", pfile));
		...
		if((fp = sys_fopen(pfile, open_mode)) == NULL) {
			...
			if (errno == ENOENT) {
				if ((fp = sys_fopen(pfile, "a+")) != NULL) {
					DEBUG_OUT (0, ("startsmbfilepwent_internal: file %s did not \
...
exist. Couldn't create new one. Error was: %s",
				...
				DEBUG_OUT (0, ("startsmbfilepwent_internal: unable to open file %s. \
Error was: %s\n", pfile, strerror(errno)));
				return NULL;
			}
		}
		...
		if (!pw_file_lock(fileno(fp), lock_type, 5, lock_depth)) {
			DEBUG_OUT (0, ("startsmbfilepwent_internal: unable to lock file %s. \
Error was %s\n", pfile, strerror(errno) ));
			fclose(fp);
			return NULL;
		}
		...
		if(type == PWF_READ) {
			...
			if (sys_stat(pfile, &sbuf1, false) != 0) {
				DEBUG_OUT (0, ("startsmbfilepwent_internal: unable to stat file %s. \
Error was %s\n", pfile, strerror(errno)));
			...
			if (sys_fstat(fileno(fp), &sbuf2, false) != 0) {
				DEBUG_OUT (0, ("startsmbfilepwent_internal: unable to fstat file %s. \
Error was %s\n", pfile, strerror(errno)));
				pw_file_unlock(fileno(fp), lock_depth);
				fclose(fp);
				return NULL;
	...
	if(race_loop == 5) {
		DEBUG_OUT (0, ("startsmbfilepwent_internal: too many race conditions opening file %s\n", pfile));
		return NULL;
	}
...
#endif
		DEBUG_OUT (0, ("startsmbfilepwent_internal: failed to set 0600 permissions on password file %s. \
Error was %s\n.", pfile, strerror(errno) ));
		pw_file_unlock(fileno(fp), lock_depth);
		fclose(fp);
		return NULL;
...
static void endsmbfilepwent(FILE *fp, int *lock_depth)
{
	...
	pw_file_unlock(fileno(fp), lock_depth);
	fclose(fp);
	DEBUG_OUT (7, ("endsmbfilepwent_internal: closed password file.\n"));
}
...
static struct smb_passwd *getsmbfilepwent(struct smbpasswd_privates *smbpasswd_state, FILE *fp)
{
	...
	if(fp == NULL) {
		DEBUG_OUT (0,("getsmbfilepwent: Bad password file pointer.\n"));
		return NULL;
	}
	...
	while (status && !feof(fp)) {
		...
		DEBUG_OUT (100, ("getsmbfilepwent: got line |%s|\n", linebuf));
#endif
		if ((linebuf[0] == 0) && feof(fp)) {
			DEBUG_OUT (4, ("getsmbfilepwent: end of file reached\n"));
			break;
		}
		...
		if (linebuf[0] == '#' || linebuf[0] == '\0') {
			DEBUG_OUT (6, ("getsmbfilepwent: skipping comment or blank line\n"));
			continue;
		}
		p = (unsigned char *) strchr_m(linebuf, ':');
		if (p == NULL) {
			DEBUG_OUT (0, ("getsmbfilepwent: malformed password entry (no :)\n"));
			continue;
		}
		...
		if(*p == '-') {
			DEBUG_OUT (0, ("getsmbfilepwent: user name %s has a negative uid.\n", user_name));
			continue;
		}

		if (!isdigit(*p)) {
			DEBUG_OUT (0, ("getsmbfilepwent: malformed password entry for user %s (uid not number)\n",
				user_name));
			continue;
		}
		...
		if (*p != ':') {
			DEBUG_OUT (0, ("getsmbfilepwent: malformed password entry for user %s (no : after uid)\n",
				user_name));
			continue;
		}
		...
		if (linebuf_len < (PTR_DIFF(p, linebuf) + 33)) {
			DEBUG_OUT (0, ("getsmbfilepwent: malformed password entry for user %s (passwd too short)\n",
				user_name ));
			continue;
		}

		if (p[32] != ':') {
			DEBUG_OUT (0, ("getsmbfilepwent: malformed password entry for user %s (no terminating :)\n",
				user_name));
			continue;
		}
		...
		if (strnequal((char *) p, "NO PASSWORD", 11)) {
			...
			if (*p == '*' || *p == 'X') {
				/* NULL LM password */
				pw_buf->smb_passwd = NULL;
				DEBUG_OUT (10, ("getsmbfilepwent: LM password for user %s invalidated\n", user_name));
			} else if (pdb_gethexpwd((char *)p, smbpwd)) {
				pw_buf->smb_passwd = smbpwd;
			} else {
				pw_buf->smb_passwd = NULL;
				DEBUG_OUT (0, ("getsmbfilepwent: Malformed Lanman password entry for user %s \
(non hex chars)\n", user_name));
			}
		}
		...
		DEBUG_OUT (5,("getsmbfilepwent: returning passwd entry for user %s, uid %ld\n",
			user_name, uidval));
	...
	DEBUG_OUT (5,("getsmbfilepwent: end of file reached.\n"));
	return NULL;
}
...
static char *format_new_smbpasswd_entry(const struct smb_passwd *newpwd)
{
	...
	if((new_entry = (char *)SMB_MALLOC( new_entry_length )) == NULL) {
		DEBUG_OUT (0, ("format_new_smbpasswd_entry: Malloc failed adding entry for user %s.\n",
			newpwd->smb_name ));
		return NULL;
	}
				     ...
				     struct smb_passwd *newpwd)
{
	...
	if (fp == NULL) {
		DEBUG_OUT (0, ("add_smbfilepwd_entry: unable to open file.\n"));
	...
	while ((pwd = getsmbfilepwent(smbpasswd_state, fp)) != NULL) {
		if (strequal(newpwd->smb_name, pwd->smb_name)) {
			DEBUG_OUT (0, ("add_smbfilepwd_entry: entry with name %s already exists\n", pwd->smb_name));
	...
	if((offpos = sys_lseek(fd, 0, SEEK_END)) == -1) {
		...
		DEBUG_OUT (0, ("add_smbfilepwd_entry(sys_lseek): Failed to add entry for user %s to file %s. \
Error was %s\n", newpwd->smb_name, pfile, strerror(errno)));
	...
	if((new_entry = format_new_smbpasswd_entry(newpwd)) == NULL) {
		DEBUG_OUT (0, ("add_smbfilepwd_entry(malloc): Failed to add entry for user %s to file %s. \
Error was %s\n", newpwd->smb_name, pfile, strerror(errno)));
		endsmbfilepwent(fp, &smbpasswd_state->pw_file_lock_depth);
...
#ifdef DEBUG_PASSWORD
	DEBUG_OUT (100, ("add_smbfilepwd_entry(%d): new_entry_len %d made line |%s|", 
			fd, (int)new_entry_length, new_entry));
#endif

	if ((wr_len = write(fd, new_entry, new_entry_length)) != new_entry_length) {
		NTSTATUS result = map_nt_error_from_unix(errno);
		DEBUG_OUT (0, ("add_smbfilepwd_entry(write): %d Failed to add entry for user %s to file %s. \
Error was %s\n", wr_len, newpwd->smb_name, pfile, strerror(errno)));

		/* Remove the entry we just wrote. */
		if(sys_ftruncate(fd, offpos) == -1) {
			DEBUG_OUT (0, ("add_smbfilepwd_entry: ERROR failed to ftruncate file %s. \
...
static bool mod_smbfilepwd_entry(struct smbpasswd_privates *smbpasswd_state, const struct smb_passwd* pwd)
{
	...
	if (!*pfile) {
		DEBUG_OUT (0, ("No SMB password file set\n"));
		return False;
	}
	DEBUG_OUT (10, ("mod_smbfilepwd_entry: opening file %s\n", pfile));

	fp = sys_fopen(pfile, "r+");

	if (fp == NULL) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: unable to open file %s\n", pfile));
		return False;
	}
	/* Set a buffer to do more efficient reads */
	...
	if (!pw_file_lock(lockfd, F_WRLCK, 5, &smbpasswd_state->pw_file_lock_depth)) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: unable to lock file %s\n", pfile));
		fclose(fp);
		return False;
	}
	...
	while (status && !feof(fp)) {
...
#ifdef DEBUG_PASSWORD
		DEBUG_OUT (100, ("mod_smbfilepwd_entry: got line |%s|\n", linebuf));
#endif

		if ((linebuf[0] == 0) && feof(fp)) {
			DEBUG_OUT (4, ("mod_smbfilepwd_entry: end of file reached\n"));
			break;
		}
		...
		if (linebuf[0] == '#' || linebuf[0] == '\0') {
			DEBUG_OUT (6, ("mod_smbfilepwd_entry: skipping comment or blank line\n"));
			continue;
		}
		...
		if (p == NULL) {
			DEBUG_OUT (0, ("mod_smbfilepwd_entry: malformed password entry (no :)\n"));
			continue;
		}
	...
	if (!found_entry) {
		...
		DEBUG_OUT (2, ("Cannot update entry for user %s, as they don't exist in the smbpasswd file!\n",
			pwd->smb_name));
		return False;
	}

	DEBUG_OUT (6, ("mod_smbfilepwd_entry: entry exists for user %s\n", pwd->smb_name));

	/* User name matches - get uid and password */
	p++; /* Go past ':' */

	if (!isdigit(*p)) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: malformed password entry for user %s (uid not number)\n",
			pwd->smb_name));
		pw_file_unlock(lockfd, &smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	...
	if (*p != ':') {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: malformed password entry for user %s (no : after uid)\n",
			pwd->smb_name));
		pw_file_unlock(lockfd, &smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	...
	if (linebuf_len < (PTR_DIFF(p, linebuf) + 33)) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: malformed password entry for user %s (passwd too short)\n",
			pwd->smb_name));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return (False);
	...
	if (p[32] != ':') {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: malformed password entry for user %s (no terminating :)\n",
			pwd->smb_name));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	...
	if (*p == '[') {
		...
		if(i == NEW_PW_FORMAT_SPACE_PADDED_LEN) {
			...
			DEBUG_OUT (0,("mod_smbfilepwd_entry:  Using old smbpasswd format for user %s. \
This is no longer supported.!\n", pwd->smb_name));
			DEBUG_OUT (0,("mod_smbfilepwd_entry:  No changes made, failing.!\n"));
			pw_file_unlock(lockfd, &smbpasswd_state->pw_file_lock_depth);
			fclose(fp);
			return False;
		}
...
#ifdef DEBUG_PASSWORD
	DEBUG_OUT (100,("mod_smbfilepwd_entry: "));
	dump_data(100, (uint8 *)ascii_p16, wr_len);
#endif

	if(wr_len > sizeof(linebuf)) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: line to write (%d) is too long.\n", wr_len+1));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return (False);
	}
	...
	if (sys_lseek(fd, pwd_seekpos - 1, SEEK_SET) != pwd_seekpos - 1) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: seek fail on file %s.\n", pfile));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	}
	...
	if (read(fd, linebuf, wr_len+1) != wr_len+1) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: read fail on file %s.\n", pfile));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	}
	...
	if ((linebuf[0] != ':') || (linebuf[wr_len] != ':'))	{
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: check on passwd file %s failed.\n", pfile));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	}
	...
	if (write(fd, ascii_p16, wr_len) != wr_len) {
		DEBUG_OUT (0, ("mod_smbfilepwd_entry: write failed in passwd file %s\n", pfile));
		pw_file_unlock(lockfd,&smbpasswd_state->pw_file_lock_depth);
		fclose(fp);
		return False;
	}
...
static bool del_smbfilepwd_entry(struct smbpasswd_privates *smbpasswd_state, const char *name)
{
	...
	if((fp = startsmbfilepwent(pfile, PWF_UPDATE, &smbpasswd_state->pw_file_lock_depth)) == NULL) {
		DEBUG_OUT (0, ("del_smbfilepwd_entry: unable to open file %s.\n", pfile));
		return False;
	}
	...
	while ((pwd = getsmbfilepwent(smbpasswd_state, fp)) != NULL) {
		...
		if (strequal(name, pwd->smb_name)) {
			DEBUG_OUT (10, ("del_smbfilepwd_entry: found entry with "
				   "name %s - deleting it.\n", name));
			continue;
		}
		...
		if((new_entry = format_new_smbpasswd_entry(pwd)) == NULL) {
			DEBUG_OUT (0, ("del_smbfilepwd_entry(malloc): Failed to copy entry for user %s to file %s. \
Error was %s\n", pwd->smb_name, pfile2, strerror(errno)));
		...
		if(fwrite(new_entry, 1, new_entry_length, fp_write) != new_entry_length) {
			DEBUG_OUT (0, ("del_smbfilepwd_entry(write): Failed to copy entry for user %s to file %s. \
Error was %s\n", pwd->smb_name, pfile2, strerror(errno)));
			unlink(pfile2);
			endsmbfilepwent(fp, &smbpasswd_state->pw_file_lock_depth);
			endsmbfilepwent(fp_write, &pfile2_lockdepth);
	...
	if(fflush(fp_write) != 0) {
		DEBUG_OUT (0, ("del_smbfilepwd_entry: Failed to flush file %s. Error was %s\n", pfile2, strerror(errno)));
		endsmbfilepwent(fp, &smbpasswd_state->pw_file_lock_depth);
		endsmbfilepwent(fp_write,&pfile2_lockdepth);
		return False;
	}
...
static bool build_smb_pass (struct smb_passwd *smb_pw, const struct samu *sampass)
{
	...
	if (!IS_SAM_DEFAULT(sampass, PDB_USERSID)) {
		...
		if (rid == DOMAIN_RID_GUEST) {
			...
			if (!passwd) {
				DEBUG_OUT (0, ("Could not find guest account via Get_Pwnam_alloc()! (%s)\n", lp_guestaccount()));
				return False;
			}
			...
			DEBUG_OUT (0,("build_sam_pass: Failing attempt to store user with non-uid based user RID. \n"));
			return False;
		}
	}
			      ...
			      struct samu *sam_pass, const struct smb_passwd *pw_buf)
{
	...
	if ( !sam_pass ) {
		DEBUG_OUT (5,("build_sam_account: struct samu is NULL\n"));
		return False;
	}
	...
	if ( !(pwfile = Get_Pwnam_alloc(NULL, pw_buf->smb_name )) ) {
		DEBUG_OUT (0,("build_sam_account: smbpasswd database is corrupt!  username %s with uid "
				  ...
				  struct samu *sam_acct, const char *username)
{
	...
	DEBUG_OUT (10, ("getsampwnam (smbpasswd): search by name: %s\n", username));
	...
	if (fp == NULL) {
		DEBUG_OUT (0, ("Unable to open passdb database.\n"));
		return nt_status;
	}
	...
	DEBUG_OUT (10, ("getsampwnam (smbpasswd): found by name: %s\n", smb_pw->smb_name));

	if (!sam_acct) {
		DEBUG_OUT (10,("getsampwnam (smbpasswd): struct samu is NULL\n"));
		return nt_status;
	}
...
static NTSTATUS smbpasswd_getsampwsid(struct pdb_methods *my_methods, struct samu *sam_acct, const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10, ("smbpasswd_getsampwrid: search by sid: %s\n",
		   sid_string_dbg(sid)));
	...
	if (rid == DOMAIN_RID_GUEST) {
		...
		if (!(guest_account && *guest_account)) {
			DEBUG_OUT (1, ("Guest account not specfied!\n"));
			return nt_status;
		}
	...
	DEBUG_OUT (10, ("getsampwrid (smbpasswd): found by name: %s\n", smb_pw->smb_name));

	if (!sam_acct) {
		DEBUG_OUT (10,("getsampwrid: (smbpasswd) struct samu is NULL\n"));
		return nt_status;
	}
	...
	if (NT_STATUS_IS_OK(nt_status) && !dom_sid_equal(pdb_get_user_sid(sam_acct), sid)) {
		DEBUG_OUT (1, ("looking for user with sid %s instead returned %s "
			  "for account %s!?!\n", sid_string_dbg(sid),
...
static NTSTATUS smbpasswd_update_sam_account(struct pdb_methods *my_methods, struct samu *sampass)
{
	...
	/* convert the struct samu */
	if (!build_smb_pass(&smb_pw, sampass)) {
		DEBUG_OUT (0, ("smbpasswd_update_sam_account: build_smb_pass failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}

	/* update the entry */
	if(!mod_smbfilepwd_entry(smbpasswd_state, &smb_pw)) {
		DEBUG_OUT (0, ("smbpasswd_update_sam_account: mod_smbfilepwd_entry failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
					      ...
					      const char *newname)
{
	...
	if (*rename_script) {
		...
		DEBUG_OUT (rename_ret ? 0 : 3,("Running the command `%s' gave %d\n", rename_script, rename_ret));
					...
					struct samr_displayentry *entry)
{
	...
	if ((entry->account_name == NULL) || (entry->fullname == NULL)
		...
		DEBUG_OUT (0, ("talloc_strdup failed\n"));
		return false;
	}
				   ...
				   uint32_t acct_flags)
{
	...
	if (fp == NULL) {
		DEBUG_OUT (10, ("Unable to open smbpasswd file.\n"));
...
static NTSTATUS pdb_init_smbpasswd( struct pdb_methods **pdb_method, const char *location )
{
	...
	if ( !(privates = TALLOC_ZERO_P( *pdb_method, struct smbpasswd_privates )) ) {
		DEBUG_OUT (0, ("talloc() failed for smbpasswd private_data!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if (!privates->smbpasswd_file) {
		DEBUG_OUT (0, ("talloc_strdp() failed for storing smbpasswd location!\n"));
		return NT_STATUS_NO_MEMORY;
	}
