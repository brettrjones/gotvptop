		 ...
		 struct dom_sid *ret_sid, enum lsa_SidType *ret_type)
{
	...
	DEBUG_OUT (10,("lookup_name: %s => domain=[%s], name=[%s]\n",
		full_name, domain, name));
	DEBUG_OUT (10, ("lookup_name: flags = 0x0%x\n", flags));
	    ...
	    strequal(name, get_global_sam_name()))
	{
		if (!secrets_fetch_domain_sid(name, &sid)) {
			DEBUG_OUT (3, ("Could not fetch my SID\n"));
			TALLOC_FREE(tmp_ctx);
			return false;
		}
		/* Swap domain and name */
	...
	{
		if (!secrets_fetch_domain_sid(name, &sid)) {
			DEBUG_OUT (3, ("Could not fetch the domain SID\n"));
			TALLOC_FREE(tmp_ctx);
			return false;
		}
		/* Swap domain and name */
	...
	if (IS_DC && winbind_lookup_name("", name, &sid, &type)) {
		...
		if (!winbind_lookup_sid(tmp_ctx, &dom_sid, &domain, NULL,
			...
			DEBUG_OUT (2, ("winbind could not find the domain's name "
			...
			const char ***names, enum lsa_SidType **types)
{
	...
	DEBUG_OUT (10, ("lookup_rids called for domain sid '%s'\n",
...
static bool check_dom_sid_to_level(const struct dom_sid *sid, int level)
{
	...
	DEBUG_OUT (10, ("%s SID %s in level %d\n",
		...
		enum lsa_SidType *ret_type)
{
	...
	DEBUG_OUT (10, ("lookup_sid called for SID '%s'\n", sid_string_dbg(sid)));
	...
	if (ret) {
		DEBUG_OUT (10, ("Sid %s -> %s\\%s(%d)\n", sid_string_dbg(sid),
			   domain->name, name->name, name->type));
	} else {
		DEBUG_OUT (10, ("failed to lookup sid %s\n", sid_string_dbg(sid)));
	}
	TALLOC_FREE(tmp_ctx);
	return ret;
}
...
static void legacy_uid_to_sid(struct dom_sid *psid, uid_t uid)
{
	...
	DEBUG_OUT (10,("LEGACY: uid %u -> sid %s\n", (unsigned int)uid,
		  sid_string_dbg(psid)));
...
static void legacy_gid_to_sid(struct dom_sid *psid, gid_t gid)
{
	...
	DEBUG_OUT (10,("LEGACY: gid %u -> sid %s\n", (unsigned int)gid,
		  sid_string_dbg(psid)));
...
static bool legacy_sid_to_uid(const struct dom_sid *psid, uid_t *puid)
{
	...
	if (sid_check_is_in_our_domain(psid)) {
		...
		if (ret) {
			if (type != SID_NAME_USER) {
				DEBUG_OUT (5, ("sid %s is a %s, expected a user\n",
					  sid_string_dbg(psid),
	...
	DEBUG_OUT (10,("LEGACY: mapping failed for sid %s\n",
		  sid_string_dbg(psid)));
	return false;

done:
	DEBUG_OUT (10,("LEGACY: sid %s -> uid %u\n", sid_string_dbg(psid),
		  (unsigned int)*puid ));
...
static bool legacy_sid_to_gid(const struct dom_sid *psid, gid_t *pgid)
{
	...
	if (sid_check_is_in_our_domain(psid)) {
		...
		if (ret) {
			if ((type != SID_NAME_DOM_GRP) &&
			    (type != SID_NAME_ALIAS)) {
				DEBUG_OUT (5, ("LEGACY: sid %s is a %s, expected "
					  "a group\n", sid_string_dbg(psid),
					  sid_type_lookup(type)));
				return false;
			}
	...
	DEBUG_OUT (10,("LEGACY: sid %s -> gid %u\n", sid_string_dbg(psid),
		  (unsigned int)*pgid ));
...
void uid_to_sid(struct dom_sid *psid, uid_t uid)
{
	...
	if (!ret || expired) {
		...
		if (!winbind_uid_to_sid(psid, uid)) {
			 ...
			 */
			DEBUG_OUT (5, ("uid_to_sid: winbind failed to find a sid "
				  "for uid %u\n", (unsigned int)uid));
...
void gid_to_sid(struct dom_sid *psid, gid_t gid)
{
	...
	if (!ret || expired) {
		...
		if (!winbind_gid_to_sid(psid, gid)) {
			 ...
			 * the next time we ask.
			 */
			DEBUG_OUT (5, ("gid_to_sid: winbind failed to find a sid "
				  "for gid %u\n", (unsigned int)gid));
		      ...
		      struct wbcUnixId *ids)
{
	...
	if (!WBC_ERROR_IS_OK(err)) {
		DEBUG_OUT (10, ("wbcSidsToUnixIds returned %s\n",
			   wbcErrorString(err)));
	}
...
bool sid_to_uid(const struct dom_sid *psid, uid_t *puid)
{
	...
	if (!ret || expired) {
		...
		if (!winbind_sid_to_uid(puid, psid)) {
			DEBUG_OUT (5, ("winbind failed to find a uid for sid %s\n",
				  sid_string_dbg(psid)));
...
bool sid_to_gid(const struct dom_sid *psid, gid_t *pgid)
{
	...
	if (!ret || expired) {
		...
		if ( !winbind_sid_to_gid(pgid, psid) ) {
			...
			DEBUG_OUT (10,("winbind failed to find a gid for sid %s\n",
				  sid_string_dbg(psid)));
			/* winbind failed. do legacy */
			return legacy_sid_to_gid(psid, pgid);
		}
				...
				struct dom_sid **_group_sid)
{
	...
	if (!pwd) {
		...
		if (!pwd) {
			DEBUG_OUT (0, ("Failed to find a Unix account for %s",
				  username));
			TALLOC_FREE(tmp_ctx);
	...
	if (need_lookup_sid) {
		...
		DEBUG_OUT (10, ("do lookup_sid(%s) for group of user %s\n",
		...
		DEBUG_OUT (3, ("Primary group %s for user %s is"
	...
	DEBUG_OUT (3, ("Forcing Primary Group to 'Domain Users' for %s\n",
		  username));
...
bool delete_uid_cache(uid_t puid)
{
	...
	if (!memcache_lookup(NULL, UID_SID_CACHE, uid, &sid)) {
		DEBUG_OUT (3, ("UID %d is not memcached!\n", (int)puid));
		return false;
	}
	DEBUG_OUT (3, ("Delete mapping UID %d <-> %s from memcache\n", (int)puid,
...
bool delete_gid_cache(gid_t pgid)
{
	...
	if (!memcache_lookup(NULL, GID_SID_CACHE, gid, &sid)) {
		DEBUG_OUT (3, ("GID %d is not memcached!\n", (int)pgid));
		return false;
	}
	DEBUG_OUT (3, ("Delete mapping GID %d <-> %s from memcache\n", (int)pgid,
		  sid_string_dbg((struct dom_sid*)sid.data)));
...
{
	DEBUG_OUT (3, ("Flush GID <-> SID memcache\n"));
...
{
	DEBUG_OUT (3, ("Flush UID <-> SID memcache\n"));
