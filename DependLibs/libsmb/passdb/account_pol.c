...
bool account_policy_get_default(enum pdb_policy_type type, uint32_t *val)
{
	...
	DEBUG_OUT (0,("no default for account_policy index %d found. This should never happen\n",
		type));
	return False;
}
...
bool init_account_policy(void)
{
	...
	if (db == NULL) { /* the account policies files does not exist or open
		...
		if (db == NULL) {
			DEBUG_OUT (0,("Failed to open account policy database\n"));
			return False;
		}
	}
	...
	if (version != DATABASE_VERSION) {
		if (dbwrap_store_uint32(db, vstring, DATABASE_VERSION) != 0) {
			DEBUG_OUT (0, ("dbwrap_store_uint32 failed\n"));
			goto cancel;
		}
		...
		for (i=0; account_policy_names[i].type; i++) {
			...
			if (!account_policy_set_default_on_empty(account_policy_names[i].type)) {
				DEBUG_OUT (0,("failed to set default value in account policy tdb\n"));
				goto cancel;
			}
		}
	}
	...
	if ( lp_enable_privileges() ) {
		if ( !grant_all_privileges( &global_sid_Builtin_Administrators ) ) {
			DEBUG_OUT (1,("init_account_policy: Failed to grant privileges "
...
bool account_policy_get(enum pdb_policy_type type, uint32_t *value)
{
	...
	if (name == NULL) {
		DEBUG_OUT (1, ("account_policy_get: Field %d is not a valid account policy type!  Cannot get, returning 0.\n", type));
		return False;
	}

	if (!dbwrap_fetch_uint32(db, name, &regval)) {
		DEBUG_OUT (1, ("account_policy_get: tdb_fetch_uint32 failed for type %d (%s), returning 0\n", type, name));
		return False;
	}
	...
	DEBUG_OUT (10,("account_policy_get: name: %s, val: %d\n", name, regval));
	return True;
}
...
bool account_policy_set(enum pdb_policy_type type, uint32_t value)
{
	...
	name = decode_account_policy_name(type);
	if (name == NULL) {
		DEBUG_OUT (1, ("Field %d is not a valid account policy type!  Cannot set.\n", type));
		return False;
	}

	status = dbwrap_trans_store_uint32(db, name, value);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("store_uint32 failed for type %d (%s) on value "
			  "%u: %s\n", type, name, value, nt_errstr(status)));
		return False;
	}

	DEBUG_OUT (10,("account_policy_set: name: %s, value: %d\n", name, value));
...
bool cache_account_policy_set(enum pdb_policy_type type, uint32_t value)
{
	...
	if (policy_name == NULL) {
		DEBUG_OUT (0,("cache_account_policy_set: no policy found\n"));
		return False;
	}
	...
	DEBUG_OUT (10,("cache_account_policy_set: updating account pol cache\n"));
