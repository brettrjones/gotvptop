					   ...
					   struct pdb_ads_samu_private *priv)
{
	...
	if (str == NULL) {
		DEBUG_OUT (10, ("no samAccountName\n"));
		goto fail;
	}
	...
	if (tldap_get_single_valueblob(entry, "unicodePwd", &blob)) {
		if (blob.length != NT_HASH_LEN) {
			DEBUG_OUT (0, ("Got NT hash of length %d, expected %d\n",
	...
	if (tldap_get_single_valueblob(entry, "dBCSPwd", &blob)) {
		if (blob.length != LM_HASH_LEN) {
			DEBUG_OUT (0, ("Got LM hash of length %d, expected %d\n",
				    ...
				    struct pdb_ads_samu_private **presult)
{
	...
	rc = pdb_ads_search_fmt(state, state->domaindn, TLDAP_SCOPE_SUB,
				...
				&users, "%s", filter);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_search failed %s\n",
	...
	if (count != 1) {
		DEBUG_OUT (10, ("Expected 1 user, got %d\n", count));
		TALLOC_FREE(result);
	...
	if (!tldap_entry_dn(result->ldapmsg, &result->dn)) {
		DEBUG_OUT (10, ("Could not extract dn\n"));
		TALLOC_FREE(result);
				       ...
				       const char *filter)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("pdb_ads_getsamupriv failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("pdb_ads_init_sam_from_priv failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(priv);
		return status;
	}
				    ...
				    uint32 *rid)
{
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_add failed %s\n",
	...
	rc = pdb_ads_search_fmt(state, state->domaindn, TLDAP_SCOPE_SUB,
				...
				name);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not find just created user %s: %s\n",
	...
	if (talloc_array_length(user) != 1) {
		DEBUG_OUT (10, ("Got %d users, expected one\n",
	...
	if (!tldap_pull_binsid(user[0], "objectSid", &sid)) {
		DEBUG_OUT (10, ("Could not fetch objectSid from user %s\n",
			   name));
		TALLOC_FREE(dn);
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
				    ...
				    struct samu *sam)
{
	...
	TALLOC_FREE(dn);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_delete for %s failed: %s\n", dn,
			   tldap_errstr(talloc_tos(), ld, rc)));
		return NT_STATUS_LDAP(rc);
	}
	return NT_STATUS_OK;
					   ...
					   struct samu *sam)
{
	...
	TALLOC_FREE(mods);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_modify for %s failed: %s\n", priv->dn,
			   tldap_errstr(talloc_tos(), ld, rc)));
		return NT_STATUS_LDAP(rc);
	}
				    ...
				    struct tldap_message **pmsg)
{
	...
	if (talloc_array_length(group) != 1) {
		DEBUG_OUT (10, ("Expected 1 group, got %d\n",
					 ...
					 uint32 *rid)
{
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not find just created alias %s: %s\n",
			   name, tldap_errstr(talloc_tos(), state->ld, rc)));
		TALLOC_FREE(frame);
		return NT_STATUS_LDAP(rc);
	}
	...
	if (talloc_array_length(alias) != 1) {
		DEBUG_OUT (10, ("Got %d alias, expected one\n",
	...
	if (!tldap_pull_binsid(alias[0], "objectSid", &sid)) {
		DEBUG_OUT (10, ("Could not fetch objectSid from alias %s\n",
			   name));
		TALLOC_FREE(frame);
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
					 ...
					 TALLOC_CTX *mem_ctx, uint32 rid)
{
	...
	TALLOC_FREE(msg);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_delete failed: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		return NT_STATUS_LDAP(rc);
	}
					       ...
					       uint32_t *p_num_groups)
{
	...
	if (priv != NULL) {
		 ...
		 */
		DEBUG_OUT (10, ("Could not get pdb_ads_samu_private\n"));
		count = 0;
	}
				     ...
				     int mod_op)
{
	...
	TALLOC_FREE(frame);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_modify failed: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
				     ...
				     const struct dom_sid *sid)
{
	...
	if (talloc_array_length(alias) != 1) {
		DEBUG_OUT (10, ("Expected 1 alias, got %d\n",
			   (int)talloc_array_length(alias)));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	if (!tldap_entry_dn(alias[0], &dn)) {
		DEBUG_OUT (10, ("Could not get DN for alias %s\n",
			   sid_string_dbg(sid)));
				     ...
				     int mod_op)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("pdb_ads_sid2dn (%s) failed: %s\n",
			       ...
			       struct dom_sid *psid)
{
	...
	if (talloc_array_length(msg) != 1) {
		DEBUG_OUT (10, ("Got %d objects, expected one\n",
					       ...
					       size_t *pnum_alias_rids)
{
	...
	for (i=0; i<num_members; i++) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("pdb_ads_sid2dn failed for %s: %s\n",
	...
	TALLOC_FREE(filter);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("tldap_search failed %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		return NT_STATUS_LDAP(rc);
	}
	...
	for (i=0; i<count; i++) {
		...
		if (!tldap_pull_binsid(msg[i], "objectSid", &sid)) {
			DEBUG_OUT (10, ("Could not pull SID for member %d\n", i));
			continue;
		}
				    ...
				    enum lsa_SidType *lsa_attrs)
{
	...
	for (i=0; i<num_rids; i++) {
		...
		if (!tldap_pull_uint32(msg[0], "samAccountType", &attr)) {
			DEBUG_OUT (10, ("no samAccountType"));
			continue;
		}
				  ...
				  struct pdb_ads_search_state **pstate)
{
	...
	rc = pdb_ads_search_fmt(
		...
		"%s", filter);
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("ldap_search_ext_s failed: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		return false;
	}
	...
	for (i=0; i<num_users; i++) {
		...
		if (tldap_pull_uint32(users[i], "userAccountControl", &ctrl)) {
			...
			DEBUG_OUT (10, ("pdb_ads_search_filter: Found %x, "
				   "filter %x\n", (int)e->acct_flags,
				 ...
				 uint32 acct_flags)
{
	...
	DEBUG_OUT (10, ("pdb_ads_search_users got flags %x\n", acct_flags));
			      ...
			      union unid_t *id, enum lsa_SidType *type)
{
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("pdb_ads_search_fmt failed: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		return false;
	}
	if (talloc_array_length(msg) != 1) {
		DEBUG_OUT (10, ("Got %d objects, expected 1\n",
			   (int)talloc_array_length(msg)));
		goto fail;
	}
	if (!tldap_pull_uint32(msg[0], "samAccountType", &atype)) {
		DEBUG_OUT (10, ("samAccountType not found\n"));
		goto fail;
	}
	if (atype == ATYPE_ACCOUNT) {
		...
		*type = SID_NAME_USER;
		if (!tldap_pull_uint32(msg[0], "uidNumber", &uid)) {
			DEBUG_OUT (10, ("Did not find uidNumber\n"));
			goto fail;
		}
		id->uid = uid;
	} else {
		...
		*type = SID_NAME_DOM_GRP;
		if (!tldap_pull_uint32(msg[0], "gidNumber", &gid)) {
			DEBUG_OUT (10, ("Did not find gidNumber\n"));
			goto fail;
		}
		id->gid = gid;
	}
			   ...
			   const char *fmt, va_list ap)
{
	...
	DEBUG_OUT (samba_level, ("tldap: %s", s));
	free(s);
}
...
static struct tldap_context *pdb_ads_ld(struct pdb_ads_state *state)
{
	...
	status = open_socket_out(
		...
		0, 0, &fd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Could not connect to %s: %s\n",
				...
				const char *location)
{
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not retrieve rootdse: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
	...
	if (state->domaindn == NULL) {
		DEBUG_OUT (10, ("Could not get defaultNamingContext\n"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	DEBUG_OUT (10, ("defaultNamingContext = %s\n", state->domaindn));

	state->configdn = tldap_talloc_single_attribute(
		rootdse, "configurationNamingContext", state);
	if (state->configdn == NULL) {
		DEBUG_OUT (10, ("Could not get configurationNamingContext\n"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	DEBUG_OUT (10, ("configurationNamingContext = %s\n", state->configdn));
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not retrieve domain: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		status = NT_STATUS_LDAP(rc);
		goto done;
	}
	...
	if (num_domains != 1) {
		DEBUG_OUT (10, ("Got %d domains, expected one\n", num_domains));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	if (!tldap_pull_binsid(domain[0], "objectSid", &state->domainsid)) {
		DEBUG_OUT (10, ("Could not retrieve domain SID\n"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	if (!tldap_pull_guid(domain[0], "objectGUID", &state->domainguid)) {
		DEBUG_OUT (10, ("Could not retrieve domain GUID\n"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	DEBUG_OUT (10, ("Domain SID: %s\n", sid_string_dbg(&state->domainsid)));
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not retrieve ncname: %s\n",
			   tldap_errstr(talloc_tos(), state->ld, rc)));
		status = NT_STATUS_LDAP(rc);
		goto done;
	}
	...
	if (state->netbiosname == NULL) {
		DEBUG_OUT (10, ("Could not get netbiosname\n"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	DEBUG_OUT (10, ("netbiosname: %s\n", state->netbiosname));

	if (!strequal(lp_workgroup(), state->netbiosname)) {
		DEBUG_OUT (1, ("ADS is different domain (%s) than ours (%s)\n",
			     ...
			     const char *location)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("pdb_ads_connect failed: %s\n", nt_errstr(status)));
		goto fail;
	}
