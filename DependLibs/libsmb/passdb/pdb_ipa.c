				   ...
				   const char *filter, LDAPMessage **entry)
{
	...
	if (num_result > 1) {
		DEBUG_OUT (1, ("get_trusted_domain_int: more than one "
	...
	if (num_result == 0) {
		DEBUG_OUT (1, ("get_trusted_domain_int: no "
			  "%s object with filter '%s'.\n",
			  LDAP_OBJ_TRUSTED_DOMAIN, filter));
		*entry = NULL;
	} else {
				       ...
				       uint32_t *val)
{
	...
	dummy = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry,
						attr, talloc_tos());
	if (dummy == NULL) {
		DEBUG_OUT (9, ("Attribute %s not present.\n", attr));
		*val = 0;
		return true;
	}
					  ...
					  struct pdb_trusted_domain **td)
{
	...
	DEBUG_OUT (10, ("ipasam_get_trusted_domain called for domain %s\n", domain));
	...
	if (entry == NULL) {
		DEBUG_OUT (5, ("ipasam_get_trusted_domain: no such trusted domain: "
			  "%s\n", domain));
						 ...
						 struct pdb_trusted_domain **td)
{
	...
	DEBUG_OUT (10, ("ipasam_get_trusted_domain_by_sid called for sid %s\n",
		   sid_str));
	...
	if (entry == NULL) {
		DEBUG_OUT (5, ("ipasam_get_trusted_domain_by_sid: no trusted domain "
			  "with sid: %s\n", sid_str));
		return NT_STATUS_NO_SUCH_DOMAIN;
	}
					  ...
					  const struct pdb_trusted_domain *td)
{
	...
	DEBUG_OUT (10, ("ipasam_set_trusted_domain called for domain %s\n", domain));
	...
	if (ret != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("error writing trusted domain data!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	return NT_STATUS_OK;
}
					   ...
					   const char *domain)
{
	...
	if (entry == NULL) {
		DEBUG_OUT (5, ("ipasam_del_trusted_domain: no such trusted domain: "
			  "%s\n", domain));
		return NT_STATUS_NO_SUCH_DOMAIN;
	}
	...
	if (dn == NULL) {
		DEBUG_OUT (0,("ipasam_del_trusted_domain: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
					    ...
					    struct pdb_trusted_domain ***domains)
{
	...
	*num_domains = 0;
	...
	DEBUG_OUT (5, ("ipasam_enum_trusted_domains: got %d domains\n", *num_domains));
	return NT_STATUS_OK;
}
					 ...
					 struct samu *sampass)
{
	...
	if (ber == NULL) {
		DEBUG_OUT (7, ("ber_alloc_t failed.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	ber_free(ber, 1);
	if (ret == -1) {
		DEBUG_OUT (1, ("ber_flatten failed.\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (ret != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("smbldap_extended_operation LDAP_EXOP_MODIFY_PASSWD failed.\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
					 ...
					 uint32_t *has_objectclass)
{
	...
	if (objectclasses == NULL) {
		DEBUG_OUT (0, ("Entry [%s] does not have any objectclasses.\n", dn));
			 ...
			 uint32_t *_has_objectclass)
{
	...
	switch (type) {
		...
		case IPA_GROUP_OBJ:
			...
			break;
		default:
			DEBUG_OUT (0, ("Unsupported IPA object.\n"));
			return NT_STATUS_INVALID_PARAMETER;
	}
	...
	ret = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL,
				    &result);
	if (ret != LDAP_SUCCESS) {
		DEBUG_OUT (0, ("smbldap_search_suffix failed.\n"));
	...
	if (num_result != 1) {
		if (num_result == 0) {
			...
			DEBUG_OUT (0, ("find_user: More than one object with name [%s] ?!\n",
				   name));
	...
	entry = ldap_first_entry(priv2ld(ldap_state), result);
	if (!entry) {
		DEBUG_OUT (0,("find_user: Out of memory!\n"));
						     ...
						     const char *username)
{
	...
	if (ret != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("failed to modify/add user with uid = %s (dn = %s)\n",
			  username, dn));
						   ...
						   uint32_t has_objectclass)
{
	...
	ldap_mods_free(mods, 1);
	if (ret != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("failed to modify/add group %s (dn = %s)\n",
			  name, dn));
		return NT_STATUS_LDAP(ret);
	}
