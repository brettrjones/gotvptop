...
bool login_cache_init(void)
{
	...
	if (cache_fname == NULL) {
		DEBUG_OUT (0, ("Filename allocation failed.\n"));
		return False;
	}
	...
	if (!cache)
		DEBUG_OUT (5, ("Attempt to open %s failed.\n", cache_fname));
...
bool login_cache_shutdown(void)
{
	...
	DEBUG_OUT (5, ("Closing cache file\n"));
...
bool login_cache_read(struct samu *sampass, struct login_cache *entry)
{
	...
	DEBUG_OUT (7, ("Looking up login cache for user %s\n",
		  keystr));
	...
	if (tdb_unpack (databuf.dptr, databuf.dsize, SAM_CACHE_FORMAT,
			...
			&bad_password_time) == -1) {
		DEBUG_OUT (7, ("No cache entry found\n"));
	...
	DEBUG_OUT (5, ("Found login cache entry: timestamp %12u, flags 0x%x, count %d, time %12u\n",
...
bool login_cache_delentry(const struct samu *sampass)
{
	...
	DEBUG_OUT (9, ("About to delete entry for %s\n", keystr));
	ret = tdb_delete_bystring(cache, keystr);
	DEBUG_OUT (9, ("tdb_delete returned %d\n", ret));
