	...
	char *pwd )
{
	...
	if (rc == LDAP_SUCCESS) {
		...
		DEBUG_OUT (100,("nmasldap_get_password returned %s for %s\n", pwd, object_dn));
#endif    
		DEBUG_OUT (5, ("NDS Universal Password retrieved for %s\n", object_dn));
	} else {
		DEBUG_OUT (3, ("NDS Universal Password NOT retrieved for %s\n", object_dn));
	}
	...
	if (rc != LDAP_SUCCESS) {
		...
		if (rc == LDAP_SUCCESS) {
#ifdef DEBUG_PASSWORD
			DEBUG_OUT (100,("nmasldap_get_simple_pwd returned %s for %s\n", pwd, object_dn));
#endif    
			DEBUG_OUT (5, ("NDS Simple Password retrieved for %s\n", object_dn));
		} else {
			/* We couldn't get the password */
			DEBUG_OUT (3, ("NDS Simple Password NOT retrieved for %s\n", object_dn));
	...
	const char *pwd )
{
	...
	if (rc == LDAP_SUCCESS) {
		DEBUG_OUT (5,("NDS Universal Password changed for user %s\n", object_dn));
	} else {
		char *ld_error = NULL;
		ldap_get_option(ld, LDAP_OPT_ERROR_STRING, &ld_error);
		
		/* This will fail if Universal Password is not enabled for the user's context */
		DEBUG_OUT (3,("NDS Universal Password could not be changed for user %s: %s (%s)\n",
					...
					struct samu *sam_acct, bool success)
{
	...
	if ((!methods) || (!sam_acct)) {
		DEBUG_OUT (3,("pdb_nds_update_login_attempts: invalid parameter.\n"));
	...
	if (ldap_state) {
		...
		DEBUG_OUT (5,("pdb_nds_update_login_attempts: %s login for %s\n",
		...
		if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) == 0) {
			DEBUG_OUT (0, ("pdb_nds_update_login_attempts: No user to modify!\n"));
			return NT_STATUS_OBJECT_NAME_NOT_FOUND;
		}
		...
		DEBUG_OUT (3, ("pdb_nds_update_login_attempts: username %s found dn '%s'\n", username, dn));
		...
		if (success == True) {
			...
			clear_text_pw[24] = '\0';
			DEBUG_OUT (5,("pdb_nds_update_login_attempts: using random password %s\n", clear_text_pw));
		}
		...
		if((success != True) || (got_clear_text_pw == True)) {
			...
			ldap_unbind(ld);
			if (rc == LDAP_SUCCESS) {
				DEBUG_OUT (5,("pdb_nds_update_login_attempts: ldap_simple_bind_s Successful for %s\n", username));
			} else {
				NTSTATUS nt_status = NT_STATUS_ACCOUNT_RESTRICTION;
				DEBUG_OUT (5,("pdb_nds_update_login_attempts: ldap_simple_bind_s Failed for %s\n", username));
				switch(rc) {
