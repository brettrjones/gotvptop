				   ...
				   const struct dom_sid *dom_sid)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_MEMBER_IN_ALIAS)) {
		DEBUG_OUT (5, ("add_sid_to_builtin %s is already a member of %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (4, ("add_sid_to_builtin %s could not be added to %s: "
			  "%s\n", sid_string_dbg(dom_sid),
...
NTSTATUS create_builtin_users(const struct dom_sid *dom_sid)
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (5,("create_builtin_users: Failed to create Users\n"));
		return status;
	}
...
NTSTATUS create_builtin_administrators(const struct dom_sid *dom_sid)
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (5,("create_builtin_administrators: Failed to create Administrators\n"));
		return status;
	}
