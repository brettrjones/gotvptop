...
static const char* get_userattr_key2string( int schema_ver, int key )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG_OUT (0,("get_userattr_key2string: unknown schema version specified\n"));
			break;
	}
	return NULL;
}
...
const char** get_userattr_list( TALLOC_CTX *mem_ctx, int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG_OUT (0,("get_userattr_list: unknown schema version specified!\n"));
			break;
	}
					      ...
					      int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG_OUT (0,("get_userattr_delete_list: unknown schema version specified!\n"));
			break;
	}
...
static const char* get_objclass_filter( int schema_ver )
{
	...
	switch( schema_ver ) {
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG_OUT (0,("get_objclass_filter: Invalid schema version specified!\n"));
			objclass_filter[0] = '\0';
			break;
	}
...
static NTSTATUS ldapsam_get_seq_num(struct pdb_methods *my_methods, time_t *seq_num)
{
	...
	if (!seq_num) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: no sequence_number\n"));
		return ntstatus;
	}

	if (!smbldap_has_naming_context(ldap_state->smbldap_state->ldap_struct, lp_ldap_suffix())) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: DIT not configured to hold %s "
	...
	if (num_result != 1) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: Expected one entry, got %d\n", num_result));
		goto done;
	}

	entry = ldap_first_entry(ldap_state->smbldap_state->ldap_struct, msg);
	if (entry == NULL) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: Could not retrieve entry\n"));
		goto done;
	}

	values = ldap_get_values(ldap_state->smbldap_state->ldap_struct, entry, attrs[0]);
	if (values == NULL) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: no values\n"));
		goto done;
	}

	num_values = ldap_count_values(values);
	if (num_values == 0) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: not a single value\n"));
		goto done;
	}

	p = values[0];
	if (!next_token_talloc(mem_ctx, &p, &tok, "#")) {
		DEBUG_OUT (0,("ldapsam_get_seq_num: failed to parse sequence number\n"));
		goto done;
	}
	...
	DEBUG_OUT (10,("ldapsam_get_seq_num: got %s: %s\n", attrs[0], p));

	*seq_num = generalized_to_unix_time(p);

	/* very basic sanity check */
	if (*seq_num <= 0) {
		DEBUG_OUT (3,("ldapsam_get_seq_num: invalid sequence number: %d\n", 
			(int)*seq_num));
		goto done;
	}
	...
	for (name = ldap_first_attribute(priv2ld(priv), entry, &ptr);
		...
		for (attrib = attrs; *attrib != NULL; attrib++) {
			if (strequal(*attrib, name)) {
				DEBUG_OUT (10, ("ldapsam_delete_entry: deleting "
					   "attribute %s\n", name));
...
static time_t ldapsam_get_entry_timestamp( struct ldapsam_privates *ldap_state, LDAPMessage * entry)
{
	...
	if ( !strptime(temp, "%Y%m%d%H%M%SZ", &tm)) {
		DEBUG_OUT (2,("ldapsam_get_entry_timestamp: strptime failed on: %s\n",
			(char*)temp));
		TALLOC_FREE(temp);
		return (time_t) 0;
	}
				...
				LDAPMessage * entry)
{
	...
	if (sampass == NULL || ldap_state == NULL || entry == NULL) {
		DEBUG_OUT (0, ("init_sam_from_ldap: NULL parameters found!\n"));
		goto fn_exit;
	}

	if (priv2ld(ldap_state) == NULL) {
		DEBUG_OUT (0, ("init_sam_from_ldap: ldap_state->smbldap_state->"
			  "ldap_struct is NULL!\n"));
		goto fn_exit;
	}
	...
	if (!(username = smbldap_talloc_first_attribute(priv2ld(ldap_state),
					...
					"uid",
					ctx))) {
		DEBUG_OUT (1, ("init_sam_from_ldap: No uid attribute found for "
			  "this user!\n"));
		goto fn_exit;
	}

	DEBUG_OUT (2, ("init_sam_from_ldap: Entry found for user: %s\n", username));
	...
	if (IS_SAM_DEFAULT(sampass, PDB_USERSID)) {
		DEBUG_OUT (1, ("init_sam_from_ldap: no %s or %s attribute found for this user %s\n", 
			get_userattr_key2string(ldap_state->schema_ver,
				LDAP_ATTR_USER_SID),
			get_userattr_key2string(ldap_state->schema_ver,
				LDAP_ATTR_USER_RID),
	...
	if (ldap_state->is_nds_ldap) {
		...
		if (user_dn != NULL) {
			DEBUG_OUT (3, ("init_sam_from_ldap: smbldap_talloc_dn(ctx, %s) returned '%s'\n", username, user_dn));
			...
			DEBUG_OUT (0, ("init_sam_from_ldap: failed to get user_dn for '%s'\n", username));
		}
	}
	...
	if (pwHistLen > 0){
		...
		if (pwhist == NULL) {
			DEBUG_OUT (0, ("init_sam_from_ldap: talloc failed!\n"));
			goto fn_exit;
		}
		...
		if (smbldap_get_single_attribute(
			...
			if (hex_failed) {
				DEBUG_OUT (2,("init_sam_from_ldap: Failed to get password history for user %s\n",
					username));
				memset(pwhist, '\0', pwHistLen * PW_HISTORY_ENTRY_LEN);
			}
		}
	...
	if (lp_parm_bool(-1, "ldapsam", "trusted", False)) {
		...
		if (sampass->unix_pw == NULL) {
			DEBUG_OUT (0,("init_sam_from_ldap: Failed to find Unix account for %s\n",
	...
	if (!login_cache_read(sampass, &cache_entry)) {
		DEBUG_OUT (9, ("No cache entry, bad count = %u, bad time = %u\n",
	...
	DEBUG_OUT (7, ("ldap time is %u, cache time is %u, bad time = %u\n",
						    ...
						    enum pdb_elements))
{
	...
	if (mods == NULL || sampass == NULL) {
		DEBUG_OUT (0, ("init_ldap_from_sam: NULL parameters found!\n"));
		return False;
	}
	...
	*mods = NULL;
	...
	DEBUG_OUT (2, ("init_ldap_from_sam: Setting entry for user: %s\n", pdb_get_username(sampass)));
		...
		switch ( ldap_state->schema_ver ) {
			case SCHEMAVER_SAMBAACCOUNT:
				if (!sid_peek_check_rid(&ldap_state->domain_sid, user_sid, &rid)) {
					DEBUG_OUT (1, ("init_ldap_from_sam: User's SID (%s) is not for this domain (%s), cannot add to LDAP!\n", 
			...
			default:
				DEBUG_OUT (0,("init_ldap_from_sam: unknown schema version specified\n"));
				break;
		}
	}
		...
		switch ( ldap_state->schema_ver ) {
			case SCHEMAVER_SAMBAACCOUNT:
				if (!sid_peek_check_rid(&ldap_state->domain_sid, group_sid, &rid)) {
					DEBUG_OUT (1, ("init_ldap_from_sam: User's Primary Group SID (%s) is not for this domain (%s), cannot add to LDAP!\n",
	...
	if (need_update(sampass, PDB_BAD_PASSWORD_COUNT))  /* &&
		...
		DEBUG_OUT (3, ("updating bad password fields, policy=%u, count=%u, time=%u\n",
			(unsigned int)pol, (unsigned int)badcount, (unsigned int)badtime));

		if ((badcount >= pol) || (badcount == 0)) {
			DEBUG_OUT (7, ("making mods to update ldap, count=%u, time=%u\n",
		...
		if (badcount == 0) {
			DEBUG_OUT (7, ("bad password count is reset, deleting login cache entry for %s\n", pdb_get_nt_username(sampass)));
			login_cache_delentry(sampass);
		} else {
			struct login_cache cache_entry;
			...
			DEBUG_OUT (7, ("Updating bad password count and time in login cache\n"));
...
static NTSTATUS ldapsam_getsampwnam(struct pdb_methods *my_methods, struct samu *user, const char *sname)
{
	...
	if (count < 1) {
		DEBUG_OUT (4, ("ldapsam_getsampwnam: Unable to locate user [%s] count=%d\n", sname, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	} else if (count > 1) {
		DEBUG_OUT (1, ("ldapsam_getsampwnam: Duplicate entries for this user [%s] Failing. count=%d\n", sname, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if (entry) {
		if (!init_sam_from_ldap(ldap_state, user, entry)) {
			DEBUG_OUT (1,("ldapsam_getsampwnam: init_sam_from_ldap failed for user '%s'!\n", sname));
			ldap_msgfree(result);
			return NT_STATUS_NO_SUCH_USER;
		}
...
static NTSTATUS ldapsam_getsampwsid(struct pdb_methods *my_methods, struct samu * user, const struct dom_sid *sid)
{
	...
	if (count < 1) {
		DEBUG_OUT (4, ("ldapsam_getsampwsid: Unable to locate SID [%s] "
			  "count=%d\n", sid_string_dbg(sid), count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}  else if (count > 1) {
		DEBUG_OUT (1, ("ldapsam_getsampwsid: More than one user with SID "
			  "[%s]. Failing. count=%d\n", sid_string_dbg(sid),
			  count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	...
	if (!init_sam_from_ldap(ldap_state, user, entry)) {
		DEBUG_OUT (1,("ldapsam_getsampwsid: init_sam_from_ldap failed!\n"));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}
				     ...
				     bool (*need_update)(const struct samu *, enum pdb_elements))
{
	...
	if (!(pdb_get_acct_ctrl(newpwd)&(ACB_WSTRUST|ACB_SVRTRUST|ACB_DOMTRUST)) &&
		...
		if (!ldap_state->is_nds_ldap) {
			...
			if (!smbldap_has_extension(ldap_state->smbldap_state->ldap_struct, 
				...
				DEBUG_OUT (2, ("ldap password change requested, but LDAP "
		...
		if ((ber = ber_alloc_t(LBER_USE_DER))==NULL) {
			DEBUG_OUT (0,("ber_alloc_t returns NULL\n"));
			TALLOC_FREE(utf8_password);
			TALLOC_FREE(utf8_dn);
		...
		if ((ber_printf (ber, "{") < 0) ||
		    (ber_printf (ber, "ts", LDAP_TAG_EXOP_MODIFY_PASSWD_ID,
				 utf8_dn) < 0)) {
			DEBUG_OUT (0,("ldapsam_modify_entry: ber_printf returns a "
				 "value <0\n"));
			ber_free(ber,1);
			TALLOC_FREE(utf8_dn);
			TALLOC_FREE(utf8_password);
		...
		if (ret < 0) {
			...
			DEBUG_OUT (0,("ldapsam_modify_entry: ber_flatten returns a value <0\n"));
			ber_free(ber,1);
			TALLOC_FREE(utf8_dn);
			TALLOC_FREE(utf8_password);
			return NT_STATUS_UNSUCCESSFUL;
		...
		if (rc != LDAP_SUCCESS) {
			...
			if (rc == LDAP_OBJECT_CLASS_VIOLATION) {
				DEBUG_OUT (3, ("Could not set userPassword "
			...
			ldap_get_option(ldap_state->smbldap_state->ldap_struct, LDAP_OPT_ERROR_STRING,
					&ld_error);
			DEBUG_OUT (0,("ldapsam_modify_entry: LDAP Password could not be changed for user %s: %s\n\t%s\n",
			...
			DEBUG_OUT (3,("ldapsam_modify_entry: LDAP Password changed for user %s\n",pdb_get_username(newpwd)));
#ifdef DEBUG_PASSWORD
			DEBUG_OUT (100,("ldapsam_modify_entry: LDAP Password changed to %s\n",pdb_get_plaintext_passwd(newpwd)));
#endif    
			if (retdata)
				ber_bvfree(retdata);
			if (retoid)
	...
	if (!mods) {
		DEBUG_OUT (5,("ldapsam_modify_entry: mods is empty: nothing to modify\n"));
		...
		switch(ldap_op) {
			...
			case LDAP_MOD_REPLACE:
				...
				break;
			default:
				DEBUG_OUT (0,("ldapsam_modify_entry: Wrong LDAP operation type: %d!\n",
					 ldap_op));
				return NT_STATUS_INVALID_PARAMETER;
		}
					   ...
					   struct samu * sam_acct)
{
	...
	if (!sam_acct) {
		DEBUG_OUT (0, ("ldapsam_delete_sam_account: sam_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	sname = pdb_get_username(sam_acct);

	DEBUG_OUT (3, ("ldapsam_delete_sam_account: Deleting user %s from "
		  "LDAP.\n", sname));

	mem_ctx = talloc_new(NULL);
	if (mem_ctx == NULL) {
		DEBUG_OUT (0, ("talloc_new failed\n"));
		goto done;
	}
	...
	if ((rc != LDAP_SUCCESS) ||
		...
		DEBUG_OUT (5, ("Could not find user %s\n", sname));
...
static NTSTATUS ldapsam_update_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) == 0) {
		DEBUG_OUT (0, ("ldapsam_update_sam_account: No user to modify!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	DEBUG_OUT (4, ("ldapsam_update_sam_account: user %s to be modified has dn: %s\n", pdb_get_username(newpwd), dn));

	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
				element_is_changed)) {
		DEBUG_OUT (0, ("ldapsam_update_sam_account: init_ldap_from_sam failed!\n"));
		TALLOC_FREE(dn);
		if (mods != NULL)
	...
	if ((lp_ldap_passwd_sync() != LDAP_PASSWD_SYNC_ONLY)
	    && (mods == NULL)) {
		DEBUG_OUT (4,("ldapsam_update_sam_account: mods is empty: nothing to update for user: %s\n",
			 pdb_get_username(newpwd)));
		TALLOC_FREE(dn);
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (2, ("ldapsam_update_sam_account: successfully modified uid = %s in the LDAP database\n",
		  pdb_get_username(newpwd)));
	return NT_STATUS_OK;
}
					   ...
					   const char *newname)
{
	...
	if (!old_acct) {
		DEBUG_OUT (0, ("ldapsam_rename_sam_account: old_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	if (!newname) {
		DEBUG_OUT (0, ("ldapsam_rename_sam_account: newname was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG_OUT (3, ("ldapsam_rename_sam_account: Renaming user %s to %s.\n",
		   oldname, newname));
	...
	DEBUG_OUT (rc ? 0 : 3,("Running the command `%s' gave %d\n",
			  rename_script, rc));
...
static NTSTATUS ldapsam_add_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (!username || !*username) {
		DEBUG_OUT (0, ("ldapsam_add_sam_account: Cannot add user without a username!\n"));
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
		DEBUG_OUT (0,("ldapsam_add_sam_account: User '%s' already in the base, with samba attributes\n", 
			 username));
		goto fn_exit;
	}
	ldap_msgfree(result);
	...
	if (element_is_set_or_changed(newpwd, PDB_USERSID)) {
		...
		if (rc == LDAP_SUCCESS) {
			if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
				DEBUG_OUT (0,("ldapsam_add_sam_account: SID '%s' "
	...
	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_add_sam_account: More than one user with that uid exists: bailing out!\n"));
		goto fn_exit;
	}

	/* Check if we need to update an existing entry */
	if (num_result == 1) {
		DEBUG_OUT (3,("ldapsam_add_sam_account: User exists without samba attributes: adding them\n"));
		...
		if (num_result > 1) {
			DEBUG_OUT (0, ("ldapsam_add_sam_account: More than one user with specified Sid exists: bailing out!\n"));
			goto fn_exit;
		}
	...
	if (num_result == 0) {
		...
		DEBUG_OUT (3,("ldapsam_add_sam_account: Adding new user\n"));
	...
	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
		...
		DEBUG_OUT (0, ("ldapsam_add_sam_account: init_ldap_from_sam failed!\n"));
		if (mods != NULL) {
			ldap_mods_free(mods, true);
		}
		goto fn_exit;
	...
	if (mods == NULL) {
		DEBUG_OUT (0,("ldapsam_add_sam_account: mods is empty: nothing to add for user: %s\n",pdb_get_username(newpwd)));
		goto fn_exit;
	}
	switch ( ldap_state->schema_ver ) {
		case SCHEMAVER_SAMBAACCOUNT:
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG_OUT (0,("ldapsam_add_sam_account: invalid schema version specified\n"));
			break;
	}

	ret = ldapsam_modify_entry(my_methods,newpwd,dn,mods,ldap_op, element_is_set_or_changed);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (0,("ldapsam_add_sam_account: failed to modify/add user with uid = %s (dn = %s)\n",
			 pdb_get_username(newpwd),dn));
		ldap_mods_free(mods, true);
		goto fn_exit;
	}

	DEBUG_OUT (2,("ldapsam_add_sam_account: added: uid == %s in the LDAP database\n", pdb_get_username(newpwd)));
	ldap_mods_free(mods, true);
				 ...
				 GROUP_MAP *map, LDAPMessage *entry)
{
	...
	if (ldap_state == NULL || map == NULL || entry == NULL ||
		...
		DEBUG_OUT (0, ("init_group_from_ldap: NULL parameters found!\n"));
		TALLOC_FREE(ctx);
		return false;
	}
	...
	temp = smbldap_talloc_single_attribute(
			...
			ctx);
	if (!temp) {
		DEBUG_OUT (0, ("init_group_from_ldap: Mandatory attribute %s not found\n", 
			get_attr_key2string( groupmap_attr_list, LDAP_ATTR_GIDNUMBER)));
		TALLOC_FREE(ctx);
		return false;
	}
	DEBUG_OUT (2, ("init_group_from_ldap: Entry found for group: %s\n", temp));
	...
	if (!string_to_sid(&map->sid, temp)) {
		DEBUG_OUT (1, ("SID string [%s] could not be read as a valid SID\n", temp));
		TALLOC_FREE(ctx);
		return false;
	}
	...
	if ((map->sid_name_use < SID_NAME_USER) ||
		...
		DEBUG_OUT (0, ("init_group_from_ldap: Unknown Group type: %d\n", map->sid_name_use));
		TALLOC_FREE(ctx);
		return false;
	}
	...
	if (!temp) {
		temp = smbldap_talloc_single_attribute(
				...
				ctx);
		if (!temp) {
			DEBUG_OUT (0, ("init_group_from_ldap: Attributes cn not found either \
				 ...
				 GROUP_MAP *map)
{
	...
	if (count < 1) {
		DEBUG_OUT (4, ("ldapsam_getgroup: Did not find group, filter was "
			  "%s\n", filter));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	if (count > 1) {
		DEBUG_OUT (1, ("ldapsam_getgroup: Duplicate entries for filter %s: "
			  "count=%d\n", filter, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	if (!init_group_from_ldap(ldap_state, map, entry)) {
		DEBUG_OUT (1, ("ldapsam_getgroup: init_group_from_ldap failed for "
			  "group filter %s\n", filter));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}
					   ...
					   uint32_t *rid)
{
	...
	if (!smbldap_get_single_attribute(ldap_struct, entry, "sambaSID",
					  str, sizeof(str)-1)) {
		DEBUG_OUT (10, ("Could not find sambaSID attribute\n"));
		return False;
	}

	if (!string_to_sid(&sid, str)) {
		DEBUG_OUT (10, ("Could not convert string %s to sid\n", str));
		return False;
	}

	if (dom_sid_compare_domain(&sid, domain_sid) != 0) {
		DEBUG_OUT (10, ("SID %s is not in expected domain %s\n",
			   str, sid_string_dbg(domain_sid)));
		return False;
	}

	if (!sid_peek_rid(&sid, rid)) {
		DEBUG_OUT (10, ("Could not peek into RID\n"));
		return False;
	}
					   ...
					   size_t *p_num_members)
{
	...
	*p_num_members = 0;
	...
	if (count > 1) {
		DEBUG_OUT (1, ("Found more than one groupmap entry for %s\n",
	...
	if (!gidstr) {
		DEBUG_OUT (0, ("ldapsam_enum_group_members: Unable to find the group's gid!\n"));
		ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	...
	if ((values != NULL) && (values[0] != NULL)) {
		...
		count = ldap_count_entries(conn->ldap_struct, result);
		DEBUG_OUT (10,("ldapsam_enum_group_members: found %d accounts\n", count));
		     ...
		     entry = ldap_next_entry(conn->ldap_struct, entry))
		{
			...
			sidstr = smbldap_talloc_single_attribute(conn->ldap_struct,
								 ...
								 mem_ctx);
			if (!sidstr) {
				DEBUG_OUT (0, ("Severe DB error, %s can't miss the sambaSID"
			...
			if (!sid_check_is_in_our_domain(&sid)) {
				DEBUG_OUT (0, ("Inconsistent SAM -- group member uid not "
					  "in our domain\n"));
				ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
				goto done;
			}
	     ...
	     entry = ldap_next_entry(conn->ldap_struct, entry))
	{
		...
		if (!ldapsam_extract_rid_from_entry(conn->ldap_struct,
						    ...
						    get_global_sam_sid(),
						    &rid)) {
			DEBUG_OUT (0, ("Severe DB error, %s can't miss the samba SID"								"attribute\n", LDAP_OBJ_SAMBASAMACCOUNT));
			ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
			goto done;
		}
					       ...
					       uint32_t *p_num_groups)
{
	...
	*pp_sids = NULL;
	...
	if (user->unix_pw) {
		...
		switch (count) {
		case 0:
			DEBUG_OUT (1, ("User account [%s] not found!\n", pdb_get_username(user)));
		...
		case 1:
			...
			gidstr = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "gidNumber", mem_ctx);
			if (!gidstr) {
				DEBUG_OUT (1, ("Unable to find the member's gid!\n"));
				ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
				goto done;
			}
			...
			break;
		default:
			DEBUG_OUT (1, ("found more than one account with the same user name ?!\n"));
			ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
			goto done;
		}
	}
	...
	*pp_sids = NULL;
	...
	if (dom_sid_compare(&global_sid_NULL, &(*pp_sids)[0]) == 0) {
		DEBUG_OUT (3, ("primary group of [%s] not found\n",
			  pdb_get_username(user)));
		ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
						...
						GROUP_MAP *map)
{
	...
	if ((rc == LDAP_SUCCESS) &&
		...
		DEBUG_OUT (3, ("SID %s already present in LDAP, refusing to add "
	...
	case SID_NAME_ALIAS:
		...
		{
			DEBUG_OUT (3, ("Refusing to map sid %s as an alias, not in our domain\n",
				  sid_string_dbg(&map->sid)));
	...
	default:
		DEBUG_OUT (3, ("Got invalid use '%s' for mapping\n",
	...
	if (map->gid == -1) {
		DEBUG_OUT (10, ("Refusing to map gid==-1\n"));
		result = NT_STATUS_INVALID_PARAMETER;
		goto done;
	}

	if (pdb_gid_to_sid(map->gid, &sid)) {
		DEBUG_OUT (3, ("Gid %u is already mapped to SID %s, refusing to "
						   ...
						   GROUP_MAP *map)
{
	...
	if (mods == NULL) {
		DEBUG_OUT (4, ("ldapsam_update_group_mapping_entry: mods is empty: "
			  "nothing to do\n"));
		result = NT_STATUS_OK;
		goto done;
	}
	...
	DEBUG_OUT (2, ("ldapsam_update_group_mapping_entry: successfully modified "
		  "group %lu in LDAP\n", (unsigned long)map->gid));
				    ...
				    bool update)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0, ("ldapsam_setsamgrent: LDAP search failed: %s\n",
			  ldap_err2string(rc)));
		DEBUG_OUT (3, ("ldapsam_setsamgrent: Query was: %s, %s\n",
	...
	DEBUG_OUT (2, ("ldapsam_setsamgrent: %d entries in the base!\n",
					   ...
					   bool unix_only)
{
	...
	*pp_rmap = NULL;
	...
	if (!NT_STATUS_IS_OK(ldapsam_setsamgrent(methods, False))) {
		DEBUG_OUT (0, ("ldapsam_enum_group_mapping: Unable to open "
			  "passdb\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	while (NT_STATUS_IS_OK(ldapsam_getsamgrent(methods, &map))) {
		if (sid_name_use != SID_NAME_UNKNOWN &&
			...
			DEBUG_OUT (11,("ldapsam_enum_group_mapping: group %s is "
		...
		if (!(*pp_rmap)) {
			DEBUG_OUT (0,("ldapsam_enum_group_mapping: Unable to "
				 "enlarge group map!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
					...
					int modop)
{
	...
	if (type == SID_NAME_USE_NONE) {
		DEBUG_OUT (5, ("SID %s is neither in builtin nor in our domain!\n",
	...
	if (count < 1) {
		DEBUG_OUT (4, ("ldapsam_modify_aliasmem: Did not find alias\n"));
		ldap_msgfree(result);
		SAFE_FREE(filter);
		return NT_STATUS_NO_SUCH_ALIAS;
	}
	...
	if (count > 1) {
		DEBUG_OUT (1, ("ldapsam_modify_aliasmem: Duplicate entries for "
			  "filter %s: count=%d\n", filter, count));
		ldap_msgfree(result);
		SAFE_FREE(filter);
		return NT_STATUS_NO_SUCH_ALIAS;
				      ...
				      size_t *p_num_members)
{
	...
	*p_num_members = 0;
	...
	if (count < 1) {
		DEBUG_OUT (4, ("ldapsam_enum_aliasmem: Did not find alias\n"));
		ldap_msgfree(result);
		SAFE_FREE(filter);
		return NT_STATUS_NO_SUCH_ALIAS;
	}
	...
	if (count > 1) {
		DEBUG_OUT (1, ("ldapsam_enum_aliasmem: Duplicate entries for "
			  "filter %s: count=%d\n", filter, count));
		ldap_msgfree(result);
		SAFE_FREE(filter);
		return NT_STATUS_NO_SUCH_ALIAS;
					  ...
					  size_t *p_num_alias_rids)
{
	...
	*p_num_alias_rids = 0;
	...
	if (type == SID_NAME_USE_NONE) {
		DEBUG_OUT (5, ("SID %s is neither builtin nor domain!\n",
						   ...
						   uint32_t value)
{
	...
	DEBUG_OUT (10,("ldapsam_set_account_policy_in_ldap\n"));
	...
	if (policy_attr == NULL) {
		DEBUG_OUT (0,("ldapsam_set_account_policy_in_ldap: invalid "
			 "policy\n"));
		return ntstatus;
	}
	...
	if (!cache_account_policy_set(type, value)) {
		DEBUG_OUT (0,("ldapsam_set_account_policy_in_ldap: failed to "
						     ...
						     uint32_t *value)
{
	...
	DEBUG_OUT (10,("ldapsam_get_account_policy_from_ldap\n"));
	...
	policy_attr = get_account_policy_attr(type);
	if (!policy_attr) {
		DEBUG_OUT (0,("ldapsam_get_account_policy_from_ldap: invalid "
			 "policy index: %d\n", type));
		return ntstatus;
	}
					   ...
					   uint32_t *value)
{
	...
	if (cache_account_policy_get(type, value)) {
		DEBUG_OUT (11,("ldapsam_get_account_policy: got valid value from "
			  "cache\n"));
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (10,("ldapsam_get_account_policy: failed to retrieve from "
		  "ldap\n"));
	...
	DEBUG_OUT (10,("ldapsam_get_account_policy: no tdb for %d, trying "
		  "default\n", type));
#endif
...
/* update_ldap: */
 ...
 update_cache:
	...
	if (!cache_account_policy_set(type, *value)) {
		DEBUG_OUT (0,("ldapsam_get_account_policy: failed to update local "
			 "tdb as a cache\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
				    ...
				    enum lsa_SidType *attrs)
{
	...
	for (entry = ldap_first_entry(ld, msg);
		...
		if (!ldapsam_extract_rid_from_entry(ld, entry, domain_sid,
						    &rid)) {
			DEBUG_OUT (2, ("Could not find sid from ldap entry\n"));
			continue;
		}
		...
		name = smbldap_talloc_single_attribute(ld, entry, "uid",
						       names);
		if (name == NULL) {
			DEBUG_OUT (2, ("Could not retrieve uid attribute\n"));
			continue;
		}
		...
		if (rid_index == num_rids) {
			DEBUG_OUT (2, ("Got a RID not asked for: %d\n", rid));
			continue;
		}
	     ...
	     entry = ldap_next_entry(ld, entry))
	{
		...
		attr = smbldap_talloc_single_attribute(ld, entry, "sambaGroupType",
						       mem_ctx);
		if (attr == NULL) {
			DEBUG_OUT (2, ("Could not extract type from ldap entry %s\n",
				  dn));
			continue;
		}
		...
		if ((is_builtin && (type != SID_NAME_ALIAS)) ||
			...
			DEBUG_OUT (2, ("Rejecting invalid group mapping entry %s\n", dn));
		}

		if (!ldapsam_extract_rid_from_entry(ld, entry, domain_sid,
						    &rid)) {
			DEBUG_OUT (2, ("Could not find sid from ldap entry %s\n", dn));
			continue;
		}
		...
		if (attr == NULL) {
			DEBUG_OUT (10, ("Could not retrieve 'displayName' attribute from %s\n",
				   dn));
			attr = smbldap_talloc_single_attribute(ld, entry, "cn", names);
		}

		if (attr == NULL) {
			DEBUG_OUT (2, ("Could not retrieve naming attribute from %s\n",
				  dn));
			continue;
		}
...
static bool ldapsam_search_firstpage(struct pdb_search *search)
{
	...
	if ((rc != LDAP_SUCCESS) || (state->entries == NULL)) {
        ...
        if ( ld == NULL) {
                DEBUG_OUT (5, ("Don't have an LDAP connection right after a "
			  "search\n"));
                return False;
        }
...
static void ldapsam_search_end(struct pdb_search *search)
{
	...
	if (rc != LDAP_SUCCESS)
		DEBUG_OUT (5, ("Could not end search properly\n"));
				  ...
				  struct samr_displayentry *result)
{
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG_OUT (5, ("\"sambaAcctFlags\" not found\n"));
		return False;
	}
	...
	{
		DEBUG_OUT (0,("ldapuser2displayentry: pull_utf8_talloc failed: %s",
			 strerror(errno)));
	}
	...
	if ((vals == NULL) || (vals[0] == NULL))
		DEBUG_OUT (8, ("\"displayName\" not found\n"));
	...
	if ((vals == NULL) || (vals[0] == NULL))
		DEBUG_OUT (8, ("\"description\" not found\n"));
	else if (!pull_utf8_talloc(mem_ctx,
	...
	if ((result->account_name == NULL) ||
		...
		DEBUG_OUT (0, ("talloc failed\n"));
		return False;
	}

	vals = ldap_get_values(ld, entry, "sambaSid");
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG_OUT (0, ("\"objectSid\" not found\n"));
		return False;
	}

	if (!string_to_sid(&sid, vals[0])) {
		DEBUG_OUT (0, ("Could not convert %s to SID\n", vals[0]));
		ldap_value_free(vals);
		return False;
	}
	ldap_value_free(vals);
	...
	if (!sid_peek_check_rid(get_global_sam_sid(), &sid, &result->rid)) {
		DEBUG_OUT (0, ("sid %s does not belong to our domain\n",
			  sid_string_dbg(&sid)));
		return False;
	}
				   ...
				   struct samr_displayentry *result)
{
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG_OUT (5, ("\"sambaGroupType\" not found\n"));
		if (vals != NULL) {
			ldap_value_free(vals);
		}
		return False;
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		...
		if (!pull_utf8_talloc(mem_ctx,
				      ...
				      vals[0], &converted_size))
		{
			DEBUG_OUT (0,("ldapgroup2displayentry: pull_utf8_talloc "
				  "failed: %s", strerror(errno)));
				   ...
				   vals[0], &converted_size))
	{
		DEBUG_OUT (0,("ldapgroup2displayentry: pull_utf8_talloc failed: %s",
			  strerror(errno)));
	}
	...
	switch (group_type) {
		...
		case SID_NAME_ALIAS:
			...
			{
				DEBUG_OUT (0, ("%s is not in our domain\n",
					  sid_string_dbg(&sid)));
				return False;
			}
			break;
		...
		default:
			DEBUG_OUT (0,("unknown group type: %d\n", group_type));
			return False;
	}
				    ...
				    uint32_t *rid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Could not get domain info: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	if (entry == NULL) {
		DEBUG_OUT (0, ("Could not get domain info entry\n"));
			      ...
			      union unid_t *id, enum lsa_SidType *type)
{
	...
	if (filter == NULL) {
		DEBUG_OUT (5, ("talloc_asprintf failed\n"));
		goto done;
	}
	...
	if (ldap_count_entries(priv2ld(priv), result) != 1) {
		DEBUG_OUT (10, ("Got %d entries, expected one\n",
	...
	if (value != NULL) {
		...
		if (gid_str == NULL) {
			DEBUG_OUT (1, ("%s has sambaGroupType but no gidNumber\n",
	...
	value = smbldap_talloc_single_attribute(priv2ld(priv), entry,
						"uidNumber", mem_ctx);
	if (value == NULL) {
		DEBUG_OUT (1, ("Could not find uidNumber in %s\n",
			       ...
			       struct dom_sid *sid)
{
	...
	filter = talloc_asprintf(tmp_ctx,
				 ...
				 LDAP_OBJ_SAMBASAMACCOUNT);
	if (filter == NULL) {
		DEBUG_OUT (3, ("talloc_asprintf failed\n"));
		goto done;
	}
	...
	if (ldap_count_entries(priv2ld(priv), result) != 1) {
		DEBUG_OUT (3, ("ERROR: Got %d entries for uid %u, expected one\n",
			   ldap_count_entries(priv2ld(priv), result),
			   (unsigned int)uid));
		goto done;
	}
	...
	if (user_sid_string == NULL) {
		DEBUG_OUT (1, ("Could not find sambaSID in object '%s'\n",
			  smbldap_talloc_dn(tmp_ctx, priv2ld(priv), entry)));
		goto done;
	}

	if (!string_to_sid(&user_sid, user_sid_string)) {
		DEBUG_OUT (3, ("Error calling sid_string_talloc for sid '%s'\n",
			  user_sid_string));
		goto done;
	}
			       ...
			       struct dom_sid *sid)
{
	...
	if (ldap_count_entries(priv2ld(priv), result) != 1) {
		DEBUG_OUT (3, ("ERROR: Got %d entries for gid %u, expected one\n",
			   ldap_count_entries(priv2ld(priv), result),
			   (unsigned int)gid));
		goto done;
	}
				    ...
				    uint32_t acb_info, uint32_t *rid)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_create_user: ldap search failed!\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_create_user: More than one user with name [%s] ?!\n", name));
	...
	if (num_result == 1) {
		...
		if (tmp) {
			DEBUG_OUT (1, ("ldapsam_create_user: The user [%s] already exist!\n", name));
		...
		if (!dn) {
			DEBUG_OUT (0,("ldapsam_create_user: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (!NT_STATUS_IS_OK((ret = ldapsam_new_rid_internal(my_methods, rid)))) {
		DEBUG_OUT (1, ("ldapsam_create_user: Could not allocate a new RID\n"));
		return ret;
	}
	...
	if (!user) {
		DEBUG_OUT (1,("ldapsam_create_user: Unable to allocate user struct\n"));
		return NT_STATUS_NO_MEMORY;
	}

	if (!pdb_set_username(user, name, PDB_SET)) {
		DEBUG_OUT (1,("ldapsam_create_user: Unable to fill user structs\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (ldap_state->schema_ver != SCHEMAVER_SAMBASAMACCOUNT) {
		DEBUG_OUT (1,("ldapsam_create_user: Unsupported schema version\n"));
	}
	smbldap_set_mod(&mods, LDAP_MOD_ADD, "objectClass", LDAP_OBJ_SAMBASAMACCOUNT);
	...
	if (add_posix) {
		...
		DEBUG_OUT (3,("ldapsam_create_user: Creating new posix user\n"));

		/* retrieve the Domain Users group gid */
		if (!sid_compose(&group_sid, get_global_sam_sid(), DOMAIN_RID_USERS) ||
		    !sid_to_gid(&group_sid, &gid)) {
			DEBUG_OUT (0, ("ldapsam_create_user: Unable to get the Domain Users gid: bailing out!\n"));
			return NT_STATUS_INVALID_PRIMARY_GROUP;
		}

		/* lets allocate a new userid for this user */
		if (!winbind_allocate_uid(&uid)) {
			DEBUG_OUT (0, ("ldapsam_create_user: Unable to allocate a new user id: bailing out!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_create_user: failed to create a new user [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG_OUT (2,("ldapsam_create_user: added account [%s] in the LDAP database\n", name));
...
static NTSTATUS ldapsam_delete_user(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, struct samu *sam_acct)
{
	...
	DEBUG_OUT (0,("ldapsam_delete_user: Attempt to delete user [%s]\n", pdb_get_username(sam_acct)));
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_delete_user: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG_OUT (0,("ldapsam_delete_user: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_delete_user: More than one user with name [%s] ?!\n", pdb_get_username(sam_acct)));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG_OUT (0,("ldapsam_delete_user: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
					 ...
					 uint32_t *rid)
{
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_create_group: ldap search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_create_group: There exists more than one group with name [%s]: bailing out!\n", name));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (num_result == 1) {
		...
		tmp = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "sambaSID", tmp_ctx);
		if (tmp) {
			DEBUG_OUT (1, ("ldapsam_create_group: The group [%s] already exist!\n", name));
		...
		if (!tmp) {
			DEBUG_OUT (1, ("ldapsam_create_group: Couldn't retrieve the gidNumber for [%s]?!?!\n", name));
			return NT_STATUS_INTERNAL_DB_CORRUPTION;
		}
		...
		dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
		if (!dn) {
			DEBUG_OUT (0,("ldapsam_create_group: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (!NT_STATUS_IS_OK((ret = ldapsam_new_rid_internal(my_methods, rid)))) {
		DEBUG_OUT (1, ("ldapsam_create_group: Could not allocate a new RID\n"));
		return ret;
	}
	...
	if (is_new_entry) {
		...
		DEBUG_OUT (3,("ldapsam_create_user: Creating new posix group\n"));

		/* lets allocate a new groupid for this group */
		if (!winbind_allocate_gid(&gid)) {
			DEBUG_OUT (0, ("ldapsam_create_group: Unable to allocate a new group id: bailing out!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_create_group: failed to create a new group [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG_OUT (2,("ldapsam_create_group: added group [%s] in the LDAP database\n", name));
...
static NTSTATUS ldapsam_delete_dom_group(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, uint32_t rid)
{
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1,("ldapsam_delete_dom_group: group search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG_OUT (1,("ldapsam_delete_dom_group: group not found!\n"));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_delete_dom_group: More than one group with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG_OUT (0,("ldapsam_delete_dom_group: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	gidstr = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "gidNumber", tmp_ctx);
	if (!gidstr) {
		DEBUG_OUT (0, ("ldapsam_delete_dom_group: Unable to find the group's gid!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1,("ldapsam_delete_dom_group: accounts search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result != 0) {
		DEBUG_OUT (3,("ldapsam_delete_dom_group: Can't delete group, it is a primary group for %d users\n", num_result));
					...
					int modop)
{
	...
	switch (modop) {
	case LDAP_MOD_ADD:
		DEBUG_OUT (1,("ldapsam_change_groupmem: add new member(rid=%d) to a domain group(rid=%d)", member_rid, group_rid));
		break;
	case LDAP_MOD_DELETE:
		DEBUG_OUT (1,("ldapsam_change_groupmem: delete member(rid=%d) from a domain group(rid=%d)", member_rid, group_rid));
		break;
	default:
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1,("ldapsam_change_groupmem: member search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG_OUT (1,("ldapsam_change_groupmem: member not found!\n"));
		return NT_STATUS_NO_SUCH_MEMBER;
	}

	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_change_groupmem: More than one account with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (modop == LDAP_MOD_DELETE) {
		...
		gidstr = smbldap_talloc_single_attribute(priv2ld(ldap_state), entry, "gidNumber", tmp_ctx);
		if (!gidstr) {
			DEBUG_OUT (0, ("ldapsam_change_groupmem: Unable to find the member's gid!\n"));
			return NT_STATUS_INTERNAL_DB_CORRUPTION;
		}
		...
		if (!sid_to_gid(&group_sid, &group_gid)) {
			DEBUG_OUT (0, ("ldapsam_change_groupmem: Unable to get group gid from SID!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}

		if (user_gid == group_gid) {
			DEBUG_OUT (3, ("ldapsam_change_groupmem: can't remove user from its own primary group!\n"));
			return NT_STATUS_MEMBERS_PRIMARY_GROUP;
		}
	}
	...
	if (!uidstr) {
		DEBUG_OUT (0, ("ldapsam_change_groupmem: Unable to find the member's name!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1,("ldapsam_change_groupmem: group search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG_OUT (1,("ldapsam_change_groupmem: group not found!\n"));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_change_groupmem: More than one group with the same SID ?!\n"));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG_OUT (0,("ldapsam_change_groupmem: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if (rc != LDAP_SUCCESS) {
		if (rc == LDAP_TYPE_OR_VALUE_EXISTS && modop == LDAP_MOD_ADD) {
			DEBUG_OUT (1,("ldapsam_change_groupmem: member is already in group, add failed!\n"));
			return NT_STATUS_MEMBER_IN_GROUP;
		}
		if (rc == LDAP_NO_SUCH_ATTRIBUTE && modop == LDAP_MOD_DELETE) {
			DEBUG_OUT (1,("ldapsam_change_groupmem: member is not in group, delete failed!\n"));
					  ...
					  struct samu *sampass)
{
	...
	DEBUG_OUT (0,("ldapsam_set_primary_group: Attempt to set primary group for user [%s]\n", pdb_get_username(sampass)));

	if (!sid_to_gid(pdb_get_group_sid(sampass), &gid)) {
		DEBUG_OUT (0,("ldapsam_set_primary_group: failed to retrieve gid from user's group SID!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	gidstr = talloc_asprintf(mem_ctx, "%u", (unsigned int)gid);
	if (!gidstr) {
		DEBUG_OUT (0,("ldapsam_set_primary_group: Out of Memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_set_primary_group: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(mem_ctx, result);
	...
	if (num_result == 0) {
		DEBUG_OUT (0,("ldapsam_set_primary_group: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (num_result > 1) {
		DEBUG_OUT (0, ("ldapsam_set_primary_group: More than one user with name [%s] ?!\n", pdb_get_username(sampass)));
		return NT_STATUS_INTERNAL_DB_CORRUPTION;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("ldapsam_set_primary_group: failed to modify [%s] primary group to [%s]\n",
				  ...
				  const char *domain, LDAPMessage **entry)
{
	...
	if (num_result > 1) {
		DEBUG_OUT (1, ("ldapsam_get_trusteddom_pw: more than one "
	...
	if (num_result == 0) {
		DEBUG_OUT (1, ("ldapsam_get_trusteddom_pw: no "
			  "%s object for domain %s.\n",
			  LDAP_OBJ_TRUSTDOM_PASSWORD, domain));
		*entry = NULL;
	} else {
	        	 	      ...
	        	 	      time_t *pass_last_set_time)
{
	...
	DEBUG_OUT (10, ("ldapsam_get_trusteddom_pw called for domain %s\n", domain));
				      ...
				      const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10, ("ldapsam_set_trusteddom_pw called for domain %s\n", domain));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("error writing trusted domain password!\n"));
		return False;
	}
				      ...
				      const char *domain)
{
	...
	if (entry == NULL) {
		DEBUG_OUT (5, ("ldapsam_del_trusteddom_pw: no such trusted domain: "
			  "%s\n", domain));
		return True;
	}
	...
	trusted_dn = smbldap_talloc_dn(talloc_tos(), priv2ld(ldap_state),
				       entry);
	if (trusted_dn == NULL) {
		DEBUG_OUT (0,("ldapsam_del_trusteddom_pw: Out of memory!\n"));
		return False;
	}
	...
	*num_domains = 0;
	     ...
	     entry = ldap_next_entry(priv2ld(ldap_state), entry))
	{
		...
		if (!string_to_sid(&dom_info->sid, dom_sid_str)) {
			DEBUG_OUT (1, ("Error calling string_to_sid on SID %s\n",
				  dom_sid_str));
			return NT_STATUS_UNSUCCESSFUL;
		}
	...
	DEBUG_OUT (5, ("ldapsam_enum_trusteddoms: got %d domains\n", *num_domains));
	return NT_STATUS_OK;
}
...
static NTSTATUS pdb_init_ldapsam_common(struct pdb_methods **pdb_method, const char *location)
{
	...
	if ( !(ldap_state = TALLOC_ZERO_P(*pdb_method, struct ldapsam_privates)) ) {
		DEBUG_OUT (0, ("pdb_init_ldapsam_common: talloc() failed for ldapsam private_data!\n"));
		return NT_STATUS_NO_MEMORY;
	}
...
NTSTATUS pdb_init_ldapsam(struct pdb_methods **pdb_method, const char *location)
{
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		DEBUG_OUT (2, ("pdb_init_ldapsam: WARNING: Could not get domain "
	...
	entry = ldap_first_entry(ldap_state->smbldap_state->ldap_struct,
				 result);
	if (!entry) {
		DEBUG_OUT (0, ("pdb_init_ldapsam: Could not get domain info "
			  "entry\n"));
		ldap_msgfree(result);
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (domain_sid_string) {
		...
		if (!string_to_sid(&ldap_domain_sid, domain_sid_string)) {
			DEBUG_OUT (1, ("pdb_init_ldapsam: SID [%s] could not be "
				  "read as a valid SID\n", domain_sid_string));
			ldap_msgfree(result);
		...
		if (!found_sid || !dom_sid_equal(&secrets_domain_sid,
					     &ldap_domain_sid)) {
			DEBUG_OUT (1, ("pdb_init_ldapsam: Resetting SID for domain "
			...
			secrets_store_domain_sid(ldap_state->domain_name,
						 &ldap_domain_sid);
			DEBUG_OUT (1, ("New global sam SID: %s\n",
	...
	if (alg_rid_base_string) {
		...
		if (alg_rid_base != algorithmic_rid_base()) {
			DEBUG_OUT (0, ("The value of 'algorithmic RID base' has "
