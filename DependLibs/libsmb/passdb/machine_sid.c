...
static struct dom_sid *pdb_generate_sam_sid(void)
{
	...
	if (secrets_fetch_domain_sid(global_myname(), sam_sid)) {
		...
		if (!secrets_fetch_domain_sid(lp_workgroup(), &domain_sid)) {
			...
			if (!secrets_store_domain_sid(lp_workgroup(), sam_sid)) {
				DEBUG_OUT (0,("pdb_generate_sam_sid: Can't store domain SID as a pdc/bdc.\n"));
				SAFE_FREE(sam_sid);
				return NULL;
			}
			return sam_sid;
		...
		if (!dom_sid_equal(&domain_sid, sam_sid)) {
			...
			DEBUG_OUT (0,("pdb_generate_sam_sid: Mismatched SIDs as a pdc/bdc.\n"));
			if (!secrets_store_domain_sid(global_myname(), &domain_sid)) {
				DEBUG_OUT (0,("pdb_generate_sam_sid: Can't re-store domain SID for local sid as PDC/BDC.\n"));
				SAFE_FREE(sam_sid);
				return NULL;
			}
			return sam_sid;
	...
	if (read_sid_from_file(fname, sam_sid)) {
		...
		if (!secrets_store_domain_sid(global_myname(), sam_sid)) {
			DEBUG_OUT (0,("pdb_generate_sam_sid: Failed to store SID from file.\n"));
			SAFE_FREE(fname);
			SAFE_FREE(sam_sid);
			return NULL;
		}
		...
		if ( !IS_DC ) {
			if (!secrets_store_domain_sid(lp_workgroup(), sam_sid)) {
				DEBUG_OUT (0,("pdb_generate_sam_sid: Failed to store domain SID from file.\n"));
				SAFE_FREE(fname);
				SAFE_FREE(sam_sid);
				return NULL;
			}
	...
	if (!secrets_store_domain_sid(global_myname(), sam_sid)) {
		DEBUG_OUT (0,("pdb_generate_sam_sid: Failed to store generated machine SID.\n"));
		SAFE_FREE(sam_sid);
		return NULL;
	}
	if ( IS_DC ) {
		if (!secrets_store_domain_sid(lp_workgroup(), sam_sid)) {
			DEBUG_OUT (0,("pdb_generate_sam_sid: Failed to store generated domain SID.\n"));
			SAFE_FREE(sam_sid);
			return NULL;
		}
	}
