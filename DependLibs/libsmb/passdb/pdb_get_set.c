...
enum pdb_value_state pdb_get_init_flags(const struct samu *sampass, enum pdb_elements element)
{
	enum pdb_value_state ret = PDB_DEFAULT;
		...
		DEBUG_OUT (11, ("element %d: SET\n", element)); 
        	ret = PDB_SET;
	}

        if (bitmap_query(sampass->change_flags, element)) {
		DEBUG_OUT (11, ("element %d: CHANGED\n", element)); 
        	ret = PDB_CHANGED;
	}

	if (ret == PDB_DEFAULT) {
		DEBUG_OUT (11, ("element %d: DEFAULT\n", element)); 
	}
...
bool pdb_set_init_flags(struct samu *sampass, enum pdb_elements element, enum pdb_value_state value_flag)
{
        if (!sampass->set_flags) {
        	if ((sampass->set_flags = 
        		bitmap_talloc(sampass, 
        				PDB_COUNT))==NULL) {
        		DEBUG_OUT (0,("bitmap_talloc failed\n"));
        		return False;
        	}
        }
        ...
        switch(value_flag) {
        	case PDB_CHANGED:
				...
				DEBUG_OUT (0,("Can't set flag: %d in change_flags.\n",element));
				return False;
			}
        		if (!bitmap_set(sampass->set_flags, element)) {
				DEBUG_OUT (0,("Can't set flag: %d in set_flags.\n",element));
				return False;
			}
			DEBUG_OUT (11, ("element %d -> now CHANGED\n", element)); 
        		break;
        	case PDB_SET:
			...
			DEBUG_OUT (11, ("element %d -> now SET\n", element)); 
        		break;
        	case PDB_DEFAULT:
        	default:
        		if (!bitmap_clear(sampass->change_flags, element)) {
			...
			DEBUG_OUT (11, ("element %d -> now DEFAULT\n", element)); 
        		break;
	}
...
bool pdb_set_user_sid(struct samu *sampass, const struct dom_sid *u_sid, enum pdb_value_state flag)
{
	...
	DEBUG_OUT (10, ("pdb_set_user_sid: setting user sid %s\n", 
...
bool pdb_set_user_sid_from_string(struct samu *sampass, fstring u_sid, enum pdb_value_state flag)
{
	...
	DEBUG_OUT (10, ("pdb_set_user_sid_from_string: setting user sid %s\n",
		   u_sid));

	if (!string_to_sid(&new_sid, u_sid)) { 
		DEBUG_OUT (1, ("pdb_set_user_sid_from_string: %s isn't a valid SID!\n", u_sid));
		return False;
	}

	if (!pdb_set_user_sid(sampass, &new_sid, flag)) {
		DEBUG_OUT (1, ("pdb_set_user_sid_from_string: could not set sid %s on struct samu!\n", u_sid));
		return False;
	}
...
bool pdb_set_group_sid(struct samu *sampass, const struct dom_sid *g_sid, enum pdb_value_state flag)
{
	...
	DEBUG_OUT (10, ("pdb_set_group_sid: setting group sid %s\n", 
...
{
	if (username) { 
		DEBUG_OUT (10, ("pdb_set_username: setting username %s, was %s\n", username,
			(sampass->username)?(sampass->username):"NULL"));

		sampass->username = talloc_strdup(sampass, username);

		if (!sampass->username) {
			DEBUG_OUT (0, ("pdb_set_username: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
{
	if (domain) { 
		DEBUG_OUT (10, ("pdb_set_domain: setting domain %s, was %s\n", domain,
			(sampass->domain)?(sampass->domain):"NULL"));

		sampass->domain = talloc_strdup(sampass, domain);

		if (!sampass->domain) {
			DEBUG_OUT (0, ("pdb_set_domain: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
{
	if (logon_script) { 
		DEBUG_OUT (10, ("pdb_set_logon_script: setting logon script %s, was %s\n", logon_script,
		...
		if (!sampass->logon_script) {
			DEBUG_OUT (0, ("pdb_set_logon_script: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
{
	if (profile_path) { 
		DEBUG_OUT (10, ("pdb_set_profile_path: setting profile path %s, was %s\n", profile_path,
		...
		if (!sampass->profile_path) {
			DEBUG_OUT (0, ("pdb_set_profile_path: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
{
	if (home_dir) { 
		DEBUG_OUT (10, ("pdb_set_homedir: setting home dir %s, was %s\n", home_dir,
			(sampass->home_dir)?(sampass->home_dir):"NULL"));

		sampass->home_dir = talloc_strdup(sampass, home_dir);

		if (!sampass->home_dir) {
			DEBUG_OUT (0, ("pdb_set_home_dir: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_acct_desc(struct samu *sampass, const char *acct_desc, enum pdb_value_state flag)
{
	if (acct_desc) { 
		...
		if (!sampass->acct_desc) {
			DEBUG_OUT (0, ("pdb_set_acct_desc: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
{
	if (workstations) { 
		DEBUG_OUT (10, ("pdb_set_workstations: setting workstations %s, was %s\n", workstations,
		...
		if (!sampass->workstations) {
			DEBUG_OUT (0, ("pdb_set_workstations: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_comment(struct samu *sampass, const char *comment, enum pdb_value_state flag)
{
	if (comment) { 
		...
		if (!sampass->comment) {
			DEBUG_OUT (0, ("pdb_set_comment: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_munged_dial(struct samu *sampass, const char *munged_dial, enum pdb_value_state flag)
{
	if (munged_dial) { 
		...
		if (!sampass->munged_dial) {
			DEBUG_OUT (0, ("pdb_set_munged_dial: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_pw_history(struct samu *sampass, const uint8 *pwd, uint32_t historyLen, enum pdb_value_state flag)
{
	if (historyLen && pwd){
		...
		if (!sampass->nt_pw_his.length) {
			DEBUG_OUT (0, ("pdb_set_pw_history: data_blob_talloc() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_plaintext_pw_only(struct samu *sampass, const char *password, enum pdb_value_state flag)
{
	if (password) { 
		...
		if (!sampass->plaintext_pw) {
			DEBUG_OUT (0, ("pdb_set_unknown_str: talloc_strdup() failed!\n"));
			return False;
		}
	} else {
...
bool pdb_set_plaintext_passwd(struct samu *sampass, const char *plaintext)
{
	...
	if ((current_history_len != 0) && (pwhistory == NULL)) {
		DEBUG_OUT (1, ("pdb_set_plaintext_passwd: pwhistory == NULL!\n"));
		return false;
	}
