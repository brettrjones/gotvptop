					   ...
					   int alloced)
{
	...
	if (i == alloced){
		DEBUG_OUT (1, ("Could not find rid %d in groupmap array\n",
			  group_rid));
					    ...
					    FILE *mod_fd, int alloced)
{
	...
	if (r->num_rids > 0) {
		...
		for (i=0; i < r->num_rids; i++) {
			...
			if (k == alloced){
				DEBUG_OUT (1, ("Could not find rid %d in "
					  "accountmap array\n", rid));
				return NT_STATUS_UNSUCCESSFUL;
			}
				  ...
				  struct samsync_ldif_context **ctx)
{
	...
	if (!(lp_workgroup()) || !*(lp_workgroup())) {
		DEBUG_OUT (0,("workgroup missing from smb.conf--exiting\n"));
		exit(1);
	}

	/* Get the ldap suffix */
	if (!(lp_ldap_suffix()) || !*(lp_ldap_suffix())) {
		DEBUG_OUT (0,("ldap suffix missing from smb.conf--exiting\n"));
		exit(1);
	}
	...
	if (r->groupmap == NULL || r->accountmap == NULL) {
		DEBUG_OUT (1,("GROUPMAP talloc failed\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
...
static void ldif_free_context(struct samsync_ldif_context *r)
{
	...
	if ((r->add_name != NULL) &&
		...
		DEBUG_OUT (1,("unlink(%s) failed, error was (%s)\n",
