						     ...
						     struct libnet_JoinCtx *r)
{
	...
	if (ADS_ERR_OK(status)) {
		DEBUG_OUT (1,("machine account creation created\n"));
		return status;
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1,("machine account creation failed\n"));
		return status;
	}
	...
	status = ads_move_machine_acct(r->in.ads,
				       ...
				       &moved);
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1,("failure to locate/move pre-existing "
			"machine account\n"));
		return status;
	}

	DEBUG_OUT (1,("The machine account %s the specified OU.\n",
						 ...
						 struct libnet_JoinCtx *r)
{
	if (!secrets_store_domain_sid(r->out.netbios_domain_name,
				      r->out.domain_sid))
	{
		DEBUG_OUT (1,("Failed to save domain sid\n"));
		return false;
	}
	...
	{
		DEBUG_OUT (1,("Failed to save machine password\n"));
		return false;
	}
					  ...
					  struct cli_state **cli)
{
	...
	status = cli_rpc_pipe_open_noauth(*cli, &ndr_table_lsarpc.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Error connecting to LSA pipe. Error was %s\n",
			nt_errstr(status)));
		goto done;
	}
					   ...
					   struct cli_state *cli)
{
	...
	status = cli_rpc_pipe_open_noauth(cli, &ndr_table_samr.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Error connecting to SAM pipe. Error was %s\n",
			nt_errstr(status)));
		goto done;
	}
	...
	if (r->in.join_flags & WKSSVC_JOIN_FLAGS_ACCOUNT_CREATE) {
		...
		DEBUG_OUT (10,("Creating account with desired access mask: %d\n",
			access_desired));
		...
		if (!NT_STATUS_IS_OK(status) &&
			...
			DEBUG_OUT (10,("Creation of workstation account failed: %s\n",
				nt_errstr(status)));
	...
	if (name_types.ids[0] != SID_NAME_USER) {
		DEBUG_OUT (0,("%s is not a user account (type=%d)\n",
			...
			const char *dc_name)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (0,("libnet_join_ok: failed to get schannel session "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("libnet_join_ok: failed to open schannel session "
...
static void libnet_join_add_dom_rids_to_builtins(struct dom_sid *domain_sid)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_PROTOCOL_UNREACHABLE)) {
		DEBUG_OUT (10,("Unable to auto-add domain administrators to "
			  "BUILTIN\\Administrators during join because "
			  "winbindd must be running."));
	} else if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("Failed to auto-add domain administrators to "
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_PROTOCOL_UNREACHABLE)) {
		DEBUG_OUT (10,("Unable to auto-add domain users to BUILTIN\\users "
