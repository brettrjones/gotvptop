				   ...
				   struct dssync_context *ctx)
{
	...
	default:
		DEBUG_OUT (1, ("Warning: invalid info length in bind info: %d\n",
			  bind_info.length));
		break;
	}
					   ...
					   struct replUpToDateVectorBlob **pnew_utdv)
{
	...
	for (y=0, last_query = false; !last_query; y++) {
		...
		if (level == 8) {
			DEBUG_OUT (1,("start[%d] tmp_higest_usn: %llu , highest_usn: %llu\n",y,
		...
		if (out_level == 1) {
			DEBUG_OUT (1,("end[%d] tmp_highest_usn: %llu , highest_usn: %llu\n",y,
