				   ...
				   struct libnet_keytab_context **ctx)
{
	...
	if (ret) {
		DEBUG_OUT (1,("keytab_init: could not krb5_init_context: %s\n",
			error_message(ret)));
		return ret;
	}
	...
	if (ret) {
		DEBUG_OUT (1,("keytab_init: smb_krb5_open_keytab failed (%s)\n",
			error_message(ret)));
						    ...
						    bool ignore_kvno)
{
	...
	while (krb5_kt_next_entry(context, keytab, &kt_entry, &cursor) == 0)
	{
		...
		ret = smb_krb5_unparse_name(talloc_tos(), context, kt_entry.principal,
					    &princ_s);
		if (ret) {
			DEBUG_OUT (5, ("smb_krb5_unparse_name failed (%s)\n",
				  error_message(ret)));
			goto cont;
		}
		...
		DEBUG_OUT (10, ("found entry for principal %s, kvno %d, "
		...
		ZERO_STRUCT(cursor);
		if (ret) {
			DEBUG_OUT (5, ("krb5_kt_end_seq_get failed (%s)\n",
				  error_message(ret)));
			goto cont;
		}
		...
		ret = krb5_kt_remove_entry(context, keytab,
					   &kt_entry);
		if (ret) {
			DEBUG_OUT (5, ("krb5_kt_remove_entry failed (%s)\n",
				  error_message(ret)));
			goto cont;
		}
		DEBUG_OUT (10, ("removed entry for principal %s, kvno %d, "
			   "enctype %d\n", princ_s, kt_entry.vno,
			   KRB5_KEY_TYPE(keyp)));

		ret = krb5_kt_start_seq_get(context, keytab, &cursor);
		if (ret) {
			DEBUG_OUT (5, ("krb5_kt_start_seq_get failed (%s)\n",
				  error_message(ret)));
			goto cont;
		}
					       ...
					       krb5_data password)
{
	...
	ret = libnet_keytab_remove_entries(context, keytab, princ_s, kvno,
					   enctype, false);
	if (ret) {
		DEBUG_OUT (1, ("libnet_keytab_remove_entries failed: %s\n",
			  error_message(ret)));
	}
	...
	if (ret) {
		DEBUG_OUT (1, ("smb_krb5_parse_name(%s) failed (%s)\n",
	...
	if (ret) {
		DEBUG_OUT (1, ("adding entry to keytab failed (%s)\n",
			  error_message(ret)));
	}
...
krb5_error_code libnet_keytab_add(struct libnet_keytab_context *ctx)
{
	...
	if (ctx->clean_old_entries) {
		DEBUG_OUT (0, ("cleaning old entries...\n"));
		for (i=0; i < ctx->count; i++) {
			...
			ret = libnet_keytab_remove_entries(ctx->context,
							   ...
							   true);
			if (ret) {
				DEBUG_OUT (1,("libnet_keytab_add: Failed to remove "
						 ...
						 TALLOC_CTX *mem_ctx)
{
	...
	if (ret) {
		DEBUG_OUT (10, ("krb5_kt_start_seq_get failed: %s\n",
			  error_message(ret)));
		return NULL;
	}
	...
	while (krb5_kt_next_entry(ctx->context, ctx->keytab, &kt_entry, &cursor) == 0)
	{
		...
		if (!entry->principal) {
			DEBUG_OUT (3, ("talloc_strdup_failed\n"));
			goto fail;
		}
		...
		if (!entry->password.data) {
			DEBUG_OUT (3, ("data_blob_talloc failed\n"));
			goto fail;
		}

		DEBUG_OUT (10, ("found entry\n"));
