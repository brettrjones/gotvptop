			     ...
			     struct drsuapi_DsReplicaObjectListItemEx *cur)
{
		...
		switch (attr->attid) {
			...
			case DRSUAPI_ATTID_supplementalCredentials:
				status = parse_supplemental_credentials(mem_ctx,
									...
									&pkb4);
				if (!NT_STATUS_IS_OK(status)) {
					DEBUG_OUT (2, ("parsing of supplemental "
	...
	if (!got_pwd) {
		DEBUG_OUT (10, ("no password (unicodePwd) found - skipping.\n"));
		return NT_STATUS_OK;
	}
	...
	if (name) {
		...
		DEBUG_OUT (10, ("looking for SAMACCOUNTNAME/%s@%s in keytayb...\n",
		...
		if (entry) {
			...
			if (!name) {
				...
				DEBUG_OUT (10, ("found name %s\n", name));
			}
			TALLOC_FREE(entry);
		} else {
			DEBUG_OUT (10, ("entry not found\n"));
		}
		TALLOC_FREE(principal);
	}

	if (!name) {
		DEBUG_OUT (10, ("no name (sAMAccountName) found - skipping.\n"));
		return NT_STATUS_OK;
	}

	DEBUG_OUT (1,("#%02d: %s:%d, ", ctx->count, name, kvno));
