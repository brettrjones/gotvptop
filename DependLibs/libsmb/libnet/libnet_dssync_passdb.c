					   ...
					   void *private_data)
{
	...
	if (mem->obj == NULL) {
		DEBUG_OUT (0,("alias[%s] member[%s] can't resolve member - ignoring\n",
	...
	case ATYPE_DISTRIBUTION_GLOBAL_GROUP:
		DEBUG_OUT (0, ("alias[%s] ignore distribution group [%s]\n",
			  sid_string_dbg(&alias_sid),
			  member_dn));
		return 0;
	default:
	...
	status = pdb_enum_aliasmem(&alias_sid, talloc_tos(),
				   &members, &num_members);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not find current alias members %s - %s\n",
			  sid_string_dbg(&alias_sid),
			  nt_errstr(status)));
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not %s %s as alias members of %s - %s\n",
			  action,
			  sid_string_dbg(&member_sid),
			  sid_string_dbg(&alias_sid),
			  nt_errstr(status)));
					   ...
					   void *private_data)
{
	...
	mem->obj = dssync_search_obj_by_guid(pctx, pctx->all, &mem->cur->guid);
	if (mem->obj == NULL) {
		DEBUG_OUT (0,("group[%s] member[%s] can't resolve member - ignoring\n",
	...
	case ATYPE_SECURITY_GLOBAL_GROUP:
		DEBUG_OUT (0, ("Group[%s] ignore member group [%s]\n",
			  sid_string_dbg(&group_sid),
			  sid_string_dbg(&member_sid)));
		return 0;
	...
	case ATYPE_DISTRIBUTION_LOCAL_GROUP:
	case ATYPE_DISTRIBUTION_GLOBAL_GROUP:
		DEBUG_OUT (0, ("Group[%s] ignore distribution group [%s]\n",
			  sid_string_dbg(&group_sid),
			  member_dn));
		return 0;
	default:
	...
	if (!get_domain_group_from_sid(group_sid, &map)) {
		DEBUG_OUT (0, ("Could not find global group %s\n",
			  sid_string_dbg(&group_sid)));
	...
	if (!(grp = getgrgid(map.gid))) {
		DEBUG_OUT (0, ("Could not find unix group %lu\n", (unsigned long)map.gid));
		//return NT_STATUS_NO_SUCH_GROUP;
		return -1;
	}

	DEBUG_OUT (0,("Group members of %s: ", grp->gr_name));
	...
	if (!pdb_getsampwsid(member, &member_sid)) {
		DEBUG_OUT (1, ("Found bogus group member: (member_sid=%s group=%s)\n",
				...
				struct passwd **passwd_p)
{
	...
	if (acct_flags & ACB_NORMAL) {
		...
		DEBUG_OUT (1, ("Unknown user type: %s\n",
	...
	if (*add_script) {
		...
		DEBUG_OUT (add_ret ? 0 : 1,("fetch_account: Running the command `%s' "
			 "gave %d\n", add_script, add_ret));
		if (add_ret == 0) {
...
#define GET_BLOB_EX(attr, needed) do { \
	...
	if (!NT_STATUS_IS_OK(_status)) { \
		DEBUG_OUT (0,(__location__ "attr[%s] %s\n", \
	...
	if (_cnt == 0) { \
		if (needed) { \
			talloc_free(_vals); \
			DEBUG_OUT (0,(__location__ "attr[%s] count[%u]\n", #attr, _cnt)); \
				...
				struct drsuapi_DsReplicaObjectListItemEx *cur)
{
	...
	DEBUG_OUT (0,("sam_account_from_object(%s, %s) start\n",
	...
	*/
	...
	DEBUG_OUT (0,("sam_account_from_object(%s, %s) done\n",
		 sAMAccountName, sid_string_dbg(&objectSid)));
	return NT_STATUS_OK;
}
				      ...
				      struct dssync_passdb_obj *obj)
{
	...
	if (rid == DOMAIN_RID_GUEST) {
		 ...
		 * code...
		 */
		DEBUG_OUT (0,("Ignore %s - %s\n", account, sid_string_dbg(&user_sid)));
		return NT_STATUS_OK;
	}
	DEBUG_OUT (0,("Creating account: %s\n", account));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Could not create posix account info for '%s'- %s\n",
			account, nt_errstr(status)));
		TALLOC_FREE(sam_account);
		return status;
	}

	DEBUG_OUT (3, ("Attempting to find SID %s for user %s in the passdb\n",
		  sid_string_dbg(&user_sid), account));
	if (!pdb_getsampwsid(sam_account, &user_sid)) {
		sam_account_from_object(sam_account, cur);
		DEBUG_OUT (3, ("Attempting to add user SID %s for user %s in the passdb\n",
			  sid_string_dbg(&user_sid),
			  pdb_get_username(sam_account)));
		if (!NT_STATUS_IS_OK(pdb_add_sam_account(sam_account))) {
			DEBUG_OUT (1, ("SAM Account for %s failed to be added to the passdb!\n",
				  account));
			TALLOC_FREE(sam_account);
		...
		sam_account_from_object(sam_account, cur);
		DEBUG_OUT (3, ("Attempting to update user SID %s for user %s in the passdb\n",
			  sid_string_dbg(&user_sid),
			  pdb_get_username(sam_account)));
		if (!NT_STATUS_IS_OK(pdb_update_sam_account(sam_account))) {
			DEBUG_OUT (1, ("SAM Account for %s failed to be updated in the passdb!\n",
				  account));
			TALLOC_FREE(sam_account);
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	if (!pdb_getgrsid(&map, group_sid)) {
		DEBUG_OUT (0, ("Primary group of %s has no mapping!\n",
			  pdb_get_username(sam_account)));
	} else {
		if (map.gid != passwd->pw_gid) {
			if (!(grp = getgrgid(map.gid))) {
				DEBUG_OUT (0, ("Could not find unix group %lu for user %s (group SID=%s)\n",
	...
	if ( !passwd ) {
		DEBUG_OUT (1, ("No unix user for this account (%s), cannot adjust mappings\n",
			pdb_get_username(sam_account)));
	}
				    ...
				    struct dssync_passdb_obj *obj)
{
	...
	DEBUG_OUT (0,("Creating alias[%s] - %s members[%u]\n",
	...
	if (grp == NULL) {
		...
		if ((grp = getgrnam(name)) == NULL) {
			...
			DEBUG_OUT (0,("Creating unix group: '%s'\n", name));
				    ...
				    struct dssync_passdb_obj *obj)
{
	...
	sid_to_fstring(sid_string, &group_sid);
	DEBUG_OUT (0,("Creating group[%s] - %s members [%u]\n",
		  name, sid_string, num_members));
