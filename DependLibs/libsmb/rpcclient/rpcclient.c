...
static void fetch_machine_sid(struct cli_state *cli)
{
	...
	if (!(mem_ctx=talloc_init("fetch_machine_sid"))) {
		DEBUG_OUT (0,("fetch_machine_sid: talloc_init returned NULL!\n"));
		goto error;
	}
		       ...
		       int argc, char **argv)
{
	...
	if ((cmd_entry->interface != NULL) && (cmd_entry->rpc_pipe == NULL)) {
		...
		case DCERPC_AUTH_TYPE_KRB5:
			...
			break;
		default:
			DEBUG_OUT (0, ("Could not initialise %s. Invalid "
				  "auth type %u\n",
		...
		if (!NT_STATUS_IS_OK(ntresult)) {
			DEBUG_OUT (0, ("Could not initialise %s. Error was %s\n",
				  get_pipe_name_from_syntax(
		...
		if (ndr_syntax_id_equal(cmd_entry->interface,
			...
			if (!NT_STATUS_IS_OK(ntresult)) {
				DEBUG_OUT (0, ("Could not initialise credentials for %s.\n",
					  get_pipe_name_from_syntax(
						  talloc_tos(),
						  cmd_entry->interface)));
				return ntresult;
 ...
 int main(int argc, char *argv[])
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("Cannot connect to server.  Error was %s\n", nt_errstr(nt_status)));
		result = 1;
		goto done;
	}
