...
static NTSTATUS gpo_sync_files(struct sync_context *ctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("listing [%s] failed with error: %s\n",
			  ...
			  void *state)
{
	...
	DEBUG_OUT (5,("gpo_sync_func: got mask: [%s], name: [%s]\n",
		mask, info->name));
	...
	if (info->mode & FILE_ATTRIBUTE_DIRECTORY) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (1,("failed to copy dir: %s\n",
				nt_errstr(result)));
			return result;
		}
		...
		if (!ctx->local_path || !ctx->mask || !ctx->remote_path) {
			DEBUG_OUT (0,("gpo_sync_func: ENOMEM\n"));
			return NT_STATUS_NO_MEMORY;
		}
		result = gpo_sync_files(ctx);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (0,("could not sync files\n"));
			return result;
		}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (1,("failed to copy file: %s\n",
			nt_errstr(result)));
	}
	return result;
}
