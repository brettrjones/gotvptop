...
struct security_token *registry_create_system_token(TALLOC_CTX *mem_ctx)
{
	...
	if (!NT_STATUS_IS_OK(add_sid_to_array(token, &global_sid_System,
		...
		DEBUG_OUT (1,("Error adding nt-authority system sid to token\n"));
		return NULL;
	}
			  ...
			  struct GROUP_POLICY_OBJECT *gpo_list)
{
	...
	werr = gp_secure_key(mem_ctx, flags, reg_ctx->curr_key,
			     &token->sids[0]);
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("failed to secure key: %s\n", win_errstr(werr)));
		goto done;
	}

	werr = gp_reg_store_groupmembership(mem_ctx, reg_ctx, token, flags);
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("failed to store group membership: %s\n", win_errstr(werr)));
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("failed to delete old state: %s\n", win_errstr(werr)));
		/* goto done; */
	}
	...
	for (gpo = gpo_list; gpo; gpo = gpo->next) {
		...
		if (!W_ERROR_IS_OK(werr)) {
			DEBUG_OUT (0,("gp_reg_state_store: "
		  ...
		  struct registry_value *val)
{
	...
	if (!val) {
		DEBUG_OUT (lvl,("no val!\n"));
		return;
	}

	type_str = str_regtype(val->type);

	DEBUG_OUT (lvl,("\tdump_reg_val:\t%s '%s'\n\t\t\t'%s' %s: ",
	...
	switch (val->type) {
		...
		case REG_BINARY:
			...
			break;
		default:
			DEBUG_OUT (lvl,("unsupported type: %d\n", val->type));
			break;
	}
}
				...
				uint32_t flags)
{
	...
	werr = gp_store_reg_subkey(mem_ctx, entry->key,
				   ...
				   /* reg_ctx->curr_key, &key); */
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("gp_store_reg_subkey failed: %s\n", win_errstr(werr)));
		return werr;
	}
	...
	switch (entry->action) {
		...
		case GP_REG_ACTION_SEC_KEY_SET:
			werr = gp_secure_key(mem_ctx, flags,
					     ...
					     &token->sids[0]);
			if (!W_ERROR_IS_OK(werr)) {
				DEBUG_OUT (0,("reg_apply_registry_entry: "
