    ...
    enum onefs_callback_state expected_state)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (5, ("Could not find callback record for id %llu\n", id));
		return NULL;
	}

	if (rec->state != expected_state) {
		DEBUG_OUT (0, ("Expected cb type %d, got %s", expected_state,
			  onefs_cb_record_str_dbg(rec)));
		SMB_ASSERT(0);
		return NULL;
	}
...
void destroy_onefs_callback_record(uint64_t id)
{
	...
	if (id == 0) {
		DEBUG_OUT (10, ("destroy_onefs_callback_record: Nothing to "
			   "destroy\n"));
		return;
	}
	...
	for (rec = callback_recs; rec; rec = rec->next) {
		if (rec->id == id) {
			...
			SAFE_FREE(rec);
			DEBUG_OUT (10, ("removed cb rec %llu\n", id));
			return;
		}
	}

	DEBUG_OUT (0, ("Could not find cb rec %llu to delete", id));
	SMB_ASSERT(0);
}
...
uint64_t onefs_oplock_wait_record(uint64_t mid)
{
	...
	DEBUG_OUT (10, ("New cb rec %llu created\n", result->id));
...
void onefs_set_oplock_callback(uint64_t id, files_struct *fsp)
{
	...
	DEBUG_OUT (10, ("onefs_set_oplock_callback called for cb rec %llu\n", id));
...
static void oplock_break_to_none_handler(uint64_t id)
{
	...
	DEBUG_OUT (10, ("oplock_break_to_none_handler called for id %llu\n", id));

	if (!(cb = onefs_find_cb(id, ONEFS_OPEN_FILE))) {
		DEBUG_OUT (3, ("oplock_break_to_none_handler: could not find "
			  "callback id %llu\n", id));
		return;
	}

	DEBUG_OUT (10, ("oplock_break_to_none_handler called for file %s\n",
...
static void oplock_break_to_level_two_handler(uint64_t id)
{
	...
	DEBUG_OUT (10, ("oplock_break_to_level_two_handler called for id %llu\n",
		   id));

	if (!(cb = onefs_find_cb(id, ONEFS_OPEN_FILE))) {
		DEBUG_OUT (3, ("oplock_break_to_level_two_handler: could not find "
			  "callback id %llu\n", id));
		return;
	}

	DEBUG_OUT (10, ("oplock_break_to_level_two_handler called for file %s\n",
		   fsp_str_dbg(cb->data.fsp)));
...
static void oplock_revoked_handler(uint64_t id)
{
	...
	DEBUG_OUT (10, ("oplock_revoked_handler called for id %llu\n", id));

	if (!(cb = onefs_find_cb(id, ONEFS_OPEN_FILE))) {
		DEBUG_OUT (3, ("oplock_revoked_handler: could not find "
			  "callback id %llu\n", id));
		return;
	}
	...
	DEBUG_OUT (0,("Level 1 oplock break failed for file %s. Forcefully "
		 "revoking oplock\n", fsp_str_dbg(fsp)));
...
static void semlock_available_handler(uint64_t id)
{
	...
	DEBUG_OUT (10, ("semlock_available_handler called: %llu\n", id));

	if (!(cb = onefs_find_cb(id, ONEFS_WAITING_FOR_OPLOCK))) {
		DEBUG_OUT (5, ("semlock_available_handler: Did not find callback "
			  "%llu\n", id));
		return;
	}

	DEBUG_OUT (10, ("Got semlock available for mid %llu\n",
...
static void semlock_async_failure_handler(uint64_t id)
{
	...
	DEBUG_OUT (1, ("semlock_async_failure_handler called: %llu\n", id));

	if (!(cb = onefs_find_cb(id, ONEFS_WAITING_FOR_OPLOCK))) {
		DEBUG_OUT (5, ("semlock_async_failure_handler: Did not find callback "
			  "%llu\n", id));
		return;
	}

	DEBUG_OUT (1, ("Got semlock_async_failure message for mid %llu\n",
		(unsigned long long)cb->data.mid));
	...
	if (!get_open_deferred_message_state(cb->data.mid, NULL, &state)) {
		DEBUG_OUT (0, ("Could not find deferred request for "
			  "mid %d\n", cb->data.mid));
		destroy_onefs_callback_record(id);
		return;
	}
					...
					files_struct *fsp, int oplock_type)
{
	...
	DEBUG_OUT (10, ("onefs_release_kernel_oplock: Releasing %s to type %s\n",
	...
	if (ifs_oplock_downgrade(fsp->fh->fd, oplock)) {
		DEBUG_OUT (1,("ifs_oplock_downgrade failed: %s\n",
			 strerror(errno)));
	}
}
				...
				enum semlock_operation semlock_op)
{
	...
	case LEVEL2_CONTEND_POSIX_BRL:
		DEBUG_OUT (10, ("Taking %d write semlock for cmd %d on fd: %d\n",
			   semlock_op, type, fd));
		ret = ifs_semlock_write(fd, semlock_op);
		if (ret) {
			DEBUG_OUT (0,("ifs_semlock_write failed taking %d write "
				 "semlock for cmd %d on fd: %d: %s",
				 semlock_op, type, fd, strerror(errno)));
		}
		break;
	default:
		DEBUG_OUT (10, ("Skipping write semlock for cmd %d on fd: %d\n",
			   type, fd));
	}
}
...
int onefs_oplock_to_samba_oplock(enum oplock_type onefs_oplock)
{
	...
	default:
		DEBUG_OUT (0, ("unknown oplock type %d found\n", onefs_oplock));
		break;
	}
	return NO_OPLOCK;
}
					   ...
					   void *private_data)
{
	...
	if (oplocks_event_dispatcher(ctx->onefs_ops)) {
		DEBUG_OUT (0, ("oplocks_event_dispatcher failed: %s\n",
			  strerror(errno)));
	}
}
...
struct kernel_oplocks *onefs_init_kernel_oplocks(TALLOC_CTX *mem_ctx)
{
	...
	if (ctx->onefs_event_fd == -1) {
		DEBUG_OUT (0, ("oplocks_event_register failed: %s\n",
			   strerror(errno)));
		goto err_out;
	}

	DEBUG_OUT (10, ("oplock event_fd = %d\n", ctx->onefs_event_fd));
