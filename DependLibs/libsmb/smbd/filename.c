				...
				char **pp_start)
{
	...
	*pp_dirpath = talloc_strdup(ctx, parent_fname.base_name);
	...
	DEBUG_OUT (5,("check_parent_exists: name "
		"= %s, dirpath = %s, "
		"start = %s\n",
		smb_fname->base_name,
		*pp_dirpath,
		      ...
		      uint32_t ucf_flags)
{
	...
	*smb_fname_out = NULL;
	...
	DEBUG_OUT (5, ("unix_convert called on file \"%s\"\n", orig_path));
	...
	if (!*orig_path) {
		...
		DEBUG_OUT (5, ("conversion finished \"\" -> %s\n",
			  smb_fname->base_name));
		goto done;
	}
	...
	DEBUG_OUT (5,("unix_convert begin: name = %s, dirpath = %s, start = %s\n",
	...
	if (!name_has_wildcard) {
		...
		if (ret == 0) {
			...
			stat_cache_add(orig_path, smb_fname->base_name,
				       conn->case_sensitive);
			DEBUG_OUT (5,("conversion of base_name finished %s -> %s\n",
		...
		if (ret == 0) {
			...
			if (name_has_wildcard ||
				...
				if (end) {
					 ...
					 * can't be found.
					 */
					DEBUG_OUT (5,("Intermediate not found %s\n",
							start));
					*end = '/';
	...
	DEBUG_OUT (5,("conversion finished %s -> %s\n", orig_path,
		 smb_fname->base_name));
	...
	*smb_fname_out = smb_fname;
	return NT_STATUS_OK;
 fail:
	DEBUG_OUT (10, ("dirpath = [%s] start = [%s]\n", dirpath, start));
	if (*dirpath != '\0') {
...
NTSTATUS check_veto_path(connection_struct *conn, const char *name)
{
	if (IS_VETO_PATH(conn, name))  {
		...
		if (!(ISDOT(name) || ISDOTDOT(name))) {
			DEBUG_OUT (5,("check_veto_path: file path name %s vetoed\n",
						name));
...
NTSTATUS check_name(connection_struct *conn, const char *name)
{
	...
	if (!lp_widelinks(SNUM(conn)) || !lp_symlinks(SNUM(conn))) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (5,("check_name: name %s failed with %s\n",name,
						nt_errstr(status)));
			return status;
		}
	}
				       ...
				       TALLOC_CTX *mem_ctx, char **found_name)
{
	...
	if (!(cur_dir = OpenDir(talloc_tos(), conn, path, NULL, 0))) {
		DEBUG_OUT (3,("scan dir didn't open dir [%s]\n",path));
				  ...
				  struct smb_filename *smb_fname)
{
	...
	if (SMB_VFS_STAT(conn, smb_fname) == 0) {
		DEBUG_OUT (10, ("'%s' exists\n", smb_fname_str_dbg(smb_fname)));
		return NT_STATUS_OK;
	}

	if (errno != ENOENT) {
		DEBUG_OUT (10, ("vfs_stat failed: %s\n", strerror(errno)));
		status = map_nt_error_from_unix(errno);
		goto fail;
	}
	...
	for (i=0; i<num_streams; i++) {
		DEBUG_OUT (10, ("comparing [%s] and [%s]: ",
	...
	DEBUG_OUT (10, ("case insensitive stream. requested: %s, actual: %s\n",
		smb_fname->stream_name, streams[i].name));
				...
				struct smb_filename **pp_smb_fname)
{
	...
	*pp_smb_fname = NULL;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("filename_convert: resolve_dfspath failed "
			"for name %s with %s\n",
			name_in,
			nt_errstr(status) ));
		return status;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("filename_convert: unix_convert failed "
			"for name %s with %s\n",
			fname,
			nt_errstr(status) ));
		return status;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3,("filename_convert: check_name failed "
			"for name %s with %s\n",
