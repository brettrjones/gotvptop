...
static bool recalc_brl_timeout(struct smbd_server_connection *sconn)
{
	...
	if (timeval_is_zero(&next_timeout)) {
		DEBUG_OUT (10, ("Next timeout = Infinite.\n"));
		return True;
	}
	...
	*/
	...
	if (DEBUGLVL(10)) {
		...
		DEBUG_OUT (10, ("Next timeout = %d.%d seconds from now.\n",
		...
		uint64_t blocking_smblctx)
{
	...
	if(req_is_in_chain(req)) {
		DEBUG_OUT (0,("push_blocking_lock_request: cannot queue a chained request (currently).\n"));
		return False;
	}
	...
	if (blr == NULL) {
		DEBUG_OUT (0,("push_blocking_lock_request: Malloc fail !\n" ));
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("push_blocking_lock_request: failed to add PENDING_LOCK record.\n"));
		TALLOC_FREE(blr);
		return False;
	}
	...
	DEBUG_OUT (3,("push_blocking_lock_request: lock request blocked with "
...
{
	DEBUG_OUT (10, ("Replying with error=%s. BLR = %p\n", nt_errstr(status), blr));
	...
	case SMBtranss2:
		...
		break;
	default:
		DEBUG_OUT (0,("blocking_lock_reply_error: PANIC - unknown type on blocking lock queue - exiting.!\n"));
...
static bool process_lockingX(struct blocking_lock_record *blr)
{
	...
	if(blr->lock_num == num_locks) {
		...
		DEBUG_OUT (3,("process_lockingX file = %s, fnum=%d type=%d "
	...
	DEBUG_OUT (10,("process_lockingX: only got %d locks of %d needed for file %s, fnum = %d. \
Waiting....\n", 
...
static bool blocking_lock_record_process(struct blocking_lock_record *blr)
{
	switch(blr->req->cmd) {
		...
		default:
			DEBUG_OUT (0,("blocking_lock_record_process: PANIC - unknown type on blocking lock queue - exiting.!\n"));
			exit_server("PANIC - unknown type on blocking lock queue");
	}
			...
			enum file_close_type close_type)
{
	...
	for(blr = sconn->smb1.locks.blocking_lock_queue; blr; blr = next) {
		...
		DEBUG_OUT (10, ("remove_pending_lock_requests_by_fid - removing "
	...
	struct smbd_server_connection *sconn, uint64_t mid)
{
	...
	for(blr = sconn->smb1.locks.blocking_lock_queue; blr; blr = next) {
		...
		if (br_lck) {
			DEBUG_OUT (10, ("remove_pending_lock_requests_by_mid_smb1 - "
				...
				DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("received_unlock_msg\n"));
	process_blocking_lock_queue(sconn);
}
...
void process_blocking_lock_queue(struct smbd_server_connection *sconn)
{
	...
	for (blr = sconn->smb1.locks.blocking_lock_queue; blr; blr = next) {
		...
		if(blocking_lock_record_process(blr)) {
			...
			DEBUG_OUT (10, ("BLR_process returned true: cancelling and "
			    "removing lock. BLR = %p\n", blr));
		...
		if (!timeval_is_zero(&blr->expire_time) && timeval_compare(&blr->expire_time, &tv_curr) <= 0) {
			...
			DEBUG_OUT (10, ("Lock timed out! BLR = %p\n", blr));
			...
			if (br_lck) {
				DEBUG_OUT (5,("process_blocking_lock_queue: "
						 ...
						 DATA_BLOB *data)
{
	...
	if (data->length != MSG_BLOCKING_LOCK_CANCEL_SIZE) {
		DEBUG_OUT (0, ("process_blocking_lock_cancel_message: "
	...
	DEBUG_OUT (10,("process_blocking_lock_cancel_message: returning error %s\n",
		nt_errstr(err) ));
