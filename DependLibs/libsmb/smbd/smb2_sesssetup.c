					...
					uint64_t *out_session_id)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1,("smb2: Failed to verify incoming ticket with error %s!\n",
			nt_errstr(status)));
	...
	status = make_server_info_krb5(session,
					...
					&session->session_info);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("smb2: make_server_info_krb5 failed\n"));
		goto fail;
	}
	...
	if (!session->session_info->security_token ) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10,("smb2: failed to create local token: %s\n",
				nt_errstr(status)));
			goto fail;
		}
	}
	...
	if (!session_claim(session->sconn, session->compat_vuser)) {
		DEBUG_OUT (1, ("smb2: Failed to claim session "
			"for vuid=%d\n",
					...
					uint64_t *out_session_id)
{
	...
	if (kerb_mech) {
		...
		DEBUG_OUT (3,("smb2: Got krb5 ticket in SPNEGO "
					...
					uint64_t *out_session_id)
{
	...
	*out_session_id = 0;
	...
	/* Unknown packet type. */
	DEBUG_OUT (1,("Unknown packet type %u in smb2 sessionsetup\n",
