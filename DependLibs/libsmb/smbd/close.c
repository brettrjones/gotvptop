...
static NTSTATUS check_magic(struct files_struct *fsp)
{
	...
	DEBUG_OUT (5,("checking magic for %s\n", fsp_str_dbg(fsp)));
	...
	ret = smbrun(p,&tmp_fd);
	DEBUG_OUT (3,("Invoking magic command %s gave %d\n",
		p,ret));
...
NTSTATUS delete_all_streams(connection_struct *conn, const char *fname)
{
	...
	DEBUG_OUT (10, ("delete_all_streams found %d streams\n",
		   num_streams));
	...
	for (i=0; i<num_streams; i++) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("talloc_aprintf failed\n"));
			goto fail;
		}
		...
		if (res == -1) {
			status = map_nt_error_from_unix(errno);
			DEBUG_OUT (10, ("Could not delete stream %s: %s\n",
					...
					enum file_close_type close_type)
{
	...
	if (lck == NULL) {
		DEBUG_OUT (0, ("close_remove_share_mode: Could not get share mode "
			  "lock for file %s\n", fsp_str_dbg(fsp)));
	...
	if (fsp->write_time_forced) {
		DEBUG_OUT (10,("close_remove_share_mode: write time forced "
			"for file %s\n",
			fsp_str_dbg(fsp)));
		...
		if (null_timespec(fsp->close_write_time)) {
			DEBUG_OUT (10,("close_remove_share_mode: update to current time "
				"for file %s\n",
				fsp_str_dbg(fsp)));
			/* Update to current time due to "normal" write. */
			set_close_write_time(fsp, timespec_current());
		} else {
			DEBUG_OUT (10,("close_remove_share_mode: write time pending "
				"for file %s\n",
				fsp_str_dbg(fsp)));
	...
	if (!del_share_mode(lck, fsp)) {
		DEBUG_OUT (0, ("close_remove_share_mode: Could not delete share "
			  "entry for file %s\n",
			  fsp_str_dbg(fsp)));
	}
	...
	DEBUG_OUT (5,("close_remove_share_mode: file %s. Delete on close was set "
		 "- deleting file.\n", fsp_str_dbg(fsp)));
	...
	if (!unix_token_equal(del_token, get_current_utok(conn))) {
		...
		DEBUG_OUT (5,("close_remove_share_mode: file %s. "
	...
	if (!NT_STATUS_IS_OK(tmp_status)) {
		DEBUG_OUT (5,("close_remove_share_mode: file %s. Delete on close "
	...
	if (!file_id_equal(&fsp->file_id, &id)) {
		DEBUG_OUT (5,("close_remove_share_mode: file %s. Delete on close "
			 ...
			 fsp_str_dbg(fsp)));
		DEBUG_OUT (5,("close_remove_share_mode: file %s. stored file_id %s, "
			 "stat file_id %s\n",
			 fsp_str_dbg(fsp),
	...
	if ((conn->fs_capabilities & FILE_NAMED_STREAMS)
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (5, ("delete_all_streams failed: %s\n",
				  nt_errstr(status)));
			goto done;
		}
	}
...
{
	DEBUG_OUT (6,("close_write_time: %s" , time_to_asc(convert_timespec_to_time_t(ts))));
...
static NTSTATUS update_write_time_on_close(struct files_struct *fsp)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("update_write_time_on_close: smb_set_file_time "
				  ...
				  enum file_close_type close_type)
{
	...
	DEBUG_OUT (2,("%s closed file %s (numopen=%d) %s\n",
...
static NTSTATUS rmdir_internals(TALLOC_CTX *ctx, files_struct *fsp)
{
  ...
  err:
	...
	if (ret != 0) {
		DEBUG_OUT (3,("rmdir_internals: couldn't remove directory %s : "
				...
				enum file_close_type close_type)
{
	...
	if (lck == NULL) {
		DEBUG_OUT (0, ("close_directory: Could not get share mode lock for "
			  "%s\n", fsp_str_dbg(fsp)));
		status = NT_STATUS_INVALID_PARAMETER;
		goto out;
	}
	...
	if (!del_share_mode(lck, fsp)) {
		DEBUG_OUT (0, ("close_directory: Could not delete share entry for "
			  "%s\n", fsp_str_dbg(fsp)));
	}
	...
	if ((close_type == NORMAL_CLOSE || close_type == SHUTDOWN_CLOSE) &&
		...
		DEBUG_OUT (5,("close_directory: %s. Delete on close was set - "
	...
	if (!NT_STATUS_IS_OK(status1)) {
		DEBUG_OUT (0, ("Could not close dir! fname=%s, fd=%d, err=%d=%s\n",
			...
			DATA_BLOB *data)
{
	...
	if(DEBUGLVL(10)) {
		...
		DEBUG_OUT (10,("msg_close_file: got request to close share mode "
			"entry %s\n", sm_str));
		TALLOC_FREE(sm_str);
	}
	...
	if (!fsp) {
		DEBUG_OUT (10,("msg_close_file: failed to find file.\n"));
		return;
	}
