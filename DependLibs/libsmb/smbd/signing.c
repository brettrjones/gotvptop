			...
			bool trusted_channel)
{
	...
	if (trusted_channel) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1,("smb_signing_check_pdu: trusted channel passed %s\n",
				 nt_errstr(status)));
			return false;
		}
		     ...
		     const DATA_BLOB response)
{
	...
	if (!negotiated && !mandatory) {
		DEBUG_OUT (5,("srv_set_signing: signing negotiated = %u, "
	...
	DEBUG_OUT (3,("srv_set_signing: turning on SMB signing: "
