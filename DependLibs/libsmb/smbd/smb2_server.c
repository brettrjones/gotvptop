					 ...
					 struct smbd_smb2_request **_req)
{
	...
	if (size < (4 + SMB2_HDR_BODY + 2)) {
		DEBUG_OUT (0,("Invalid SMB2 packet length count %ld\n", (long)size));
	...
	if (protocol_version != SMB2_MAGIC) {
		DEBUG_OUT (0,("Invalid SMB packet: protocol prefix: 0x%08X\n",
			 protocol_version));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (cmd != SMB2_OP_NEGPROT) {
		DEBUG_OUT (0,("Invalid SMB packet: first request: 0x%04X\n",
			 cmd));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (next_command_ofs != 0) {
		DEBUG_OUT (0,("Invalid SMB packet: next_command: 0x%08X\n",
			 next_command_ofs));
		return NT_STATUS_INVALID_PARAMETER;
	}
					  ...
					  uint64_t message_id, uint64_t seq_id)
{
	...
	if (seq_id < sconn->smb2.seqnum_low) {
		DEBUG_OUT (0,("smb2_validate_sequence_number: bad message_id "
			"%llu (sequence id %llu) "
	...
	if (bitmap_query(credits_bm, offset)) {
		DEBUG_OUT (0,("smb2_validate_sequence_number: duplicate message_id "
			"%llu (sequence id %llu) "
			"(granted = %u, low = %llu, range = %u) "
			"(bm offset %u)\n",
			(unsigned long long)message_id,
	...
	while (bitmap_query(credits_bm, offset)) {
		DEBUG_OUT (10,("smb2_validate_sequence_number: clearing "
				...
				const uint8_t *inhdr)
{
	...
	DEBUG_OUT (11, ("smb2_validate_message_id: mid %llu, credits_granted %llu, "
	...
	if (sconn->smb2.credits_granted < 1) {
		DEBUG_OUT (0, ("smb2_validate_message_id: client used more "
			...
			struct iovec *out_vector)
{
	...
	DEBUG_OUT (10,("smb2_set_operation_credit: requested %u, "
...
void smbd_server_connection_terminate_ex(struct smbd_server_connection *sconn,
					 ...
					 const char *location)
{
	DEBUG_OUT (10,("smbd_server_connection_terminate_ex: reason[%s] at %s\n",
		  reason, location));
  ...
  out:
	...
	if (req->async) {
...

		{
			...
			DEBUG_OUT (10,("smbd_smb2_request_pending_queue: opcode[%s] mid %llu "
				"going async\n",
...
static NTSTATUS smbd_smb2_request_process_cancel(struct smbd_smb2_request *req)
{
	...
	if (cur && cur->subreq) {
		...
		DEBUG_OUT (10,("smbd_smb2_request_process_cancel: attempting to "
...
NTSTATUS smbd_smb2_request_dispatch(struct smbd_smb2_request *req)
{
	...
	DEBUG_OUT (10,("smbd_smb2_request_dispatch: opcode[%s] mid = %llu\n",
					...
					void *private_data)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("smbd_smb2_request_dispatch_immediate: idx[%d] of %d vectors\n",
...
static void smbd_smb2_request_writev_done(struct tevent_req *subreq)
{
	...
	if (ret == -1) {
		...
		DEBUG_OUT (2,("smbd_smb2_request_writev_done: client write error %s\n",
			nt_errstr(status)));
		smbd_server_connection_terminate(sconn, nt_errstr(status));
		return;
	}
				   ...
				   const char *location)
{
	...
	DEBUG_OUT (10,("smbd_smb2_request_done_ex: "
				    ...
				    const char *location)
{
	...
	DEBUG_OUT (10,("smbd_smb2_request_error_ex: idx[%d] status[%s] |%s| at %s\n",
			     ...
			     const uint8_t *inbuf, size_t size)
{
	...
	DEBUG_OUT (10,("smbd_smb2_first_negprot: packet length %u\n",
		 (unsigned int)size));
...
static void smbd_smb2_request_incoming(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("smbd_smb2_request_incoming: client read error %s\n",
			nt_errstr(status)));
		smbd_server_connection_terminate(sconn, nt_errstr(status));
		return;
	}
	...
	if (req->in.nbt_hdr[0] != 0x00) {
		DEBUG_OUT (1,("smbd_smb2_request_incoming: ignore NBT[0x%02X] msg\n",
			 req->in.nbt_hdr[0]));
		TALLOC_FREE(req);
		goto next;
	}
	...
	DEBUG_OUT (10,("smbd_smb2_request_incoming: idx[%d] of %d vectors\n",
		 req->current_idx, req->in.vector_count));
