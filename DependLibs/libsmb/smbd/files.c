		  ...
		  files_struct **result)
{
	...
	if (i == -1) {
		DEBUG_OUT (0,("ERROR! Out of file structures\n"));
	...
	DEBUG_OUT (5,("allocated file structure %d, fnum = %d (%d used)\n",
...
bool file_init(struct smbd_server_connection *sconn)
{
	...
	if(sconn->real_max_open_files != request_max_open_files) {
		DEBUG_OUT (1, ("file_init: Information only: requested %d "
	...
	for (fsp=sconn->files; fsp; fsp=fsp->next,count++) {
		...
		if (file_id_equal(&fsp->file_id, &id) &&
			...
			if ((fsp->fh->fd == -1) &&
				...
				DEBUG_OUT (0,("file_find_dif: file %s file_id = "
...
void file_free(struct smb_request *req, files_struct *fsp)
{
	...
	DEBUG_OUT (5,("freed files structure %d (%d used)\n",
			...
			const char *name, uint32_t *p_name_hash)
{
	...
	*p_name_hash = tdb_jenkins_hash(&key);
	...
	DEBUG_OUT (10,("file_name_hash: %s hash 0x%x\n",
		fullpath,
