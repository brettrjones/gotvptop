...
static NTSTATUS notify_load(struct notify_context *notify, struct db_record *rec)
{
	...
	if (blob.length > 0) {
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			...
			DEBUG_OUT (2, ("notify_array is corrupt, discarding it\n"));
				...
				void *ptr, struct notify_event *ev)
{
	...
	DEBUG_OUT (10, ("sys_notify_callback called with action=%d, for %s\n",
		   ev->action, ev->path));
	listel->callback(listel->private_data, ev);
}
				...
				struct notify_entry *e, void *private_data)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (10, ("notify_add_onelevel: fetch_locked for %s failed"
	...
	if (blob.length > 0) {
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			DEBUG_OUT (10, ("ndr_pull_notify_entry_array failed: %s\n",
				   ndr_errstr(ndr_err)));
			TALLOC_FREE(array);
			return;
		}
		if (DEBUGLEVEL >= 10) {
			DEBUG_OUT (10, ("notify_add_onelevel:\n"));
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (10, ("ndr_push_notify_entry_array failed: %s\n",
			   ndr_errstr(ndr_err)));
		TALLOC_FREE(array);
		return;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("notify_add_onelevel: store failed: %s\n",
			   nt_errstr(status)));
		return;
	}
	e->filter = 0;
				...
				void *private_data)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (10, ("notify_remove_onelevel: fetch_locked for %s failed"
	...
	if (blob.length > 0) {
		...
		if (DEBUGLEVEL >= 10) {
			DEBUG_OUT (10, ("notify_remove_onelevel:\n"));
			NDR_PRINT_DEBUG(notify_entry_array, array);
		}
	}
		     ...
		     uint32_t filter, struct file_id fid, const char *name)
{
	...
	for (i=0; i<array->num_entries; i++) {
		...
		DEBUG_OUT (10, ("Deleting notify entries for process %s because "
		    ...
		    uint32_t action, uint32_t filter, const char *path)
{
	...
	DEBUG_OUT (10, ("notify_trigger called action=0x%x, filter=0x%x, "
	...
	for (depth=0,p=path;
		...
		for (i=min_i;i<d->num_entries;i++) {
			...
			if (NT_STATUS_EQUAL(
				...
				DEBUG_OUT (10, ("Deleting notify entries for "
