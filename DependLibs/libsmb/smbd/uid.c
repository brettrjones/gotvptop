			...
			int snum)
{
	...
	if (!readonly_share &&
		...
		readonly_share = True;
		DEBUG_OUT (5,("falling back to read-only access-evaluation due to "
			 "security descriptor\n"));
	}
	...
	if (admin_user) {
		DEBUG_OUT (2,("check_user_ok: user %s is an admin user. "
			"Setting uid as %d\n",
				    ...
				    uint16_t vuid)
{
	...
	if (!ok) {
		DEBUG_OUT (2,("SMB user %s (unix user %s) "
	...
	DEBUG_OUT (5, ("Impersonated user: uid=(%d,%d), gid=(%d,%d)\n",
		 (int)getuid(),
		 (int)geteuid(),
		 (int)getgid(),
		 (int)getegid()));
...
bool change_to_user(connection_struct *conn, uint16_t vuid)
{
	...
	if (!conn) {
		DEBUG_OUT (2,("Connection not open\n"));
		return(False);
	}
	...
	if((lp_security() == SEC_SHARE) && (current_user.conn == conn) &&
	   (current_user.ut.uid == conn->session_info->utok.uid)) {
		DEBUG_OUT (4,("Skipping user change - already "
			 "user\n"));
		return(True);
	...
	if (session_info == NULL) {
		...
		DEBUG_OUT (2,("Invalid vuid %d used on "
	...
	if (!conn->force_user && vuser == NULL) {
		DEBUG_OUT (2,("Invalid vuid used %d in accessing "
			"share %s.\n", vuid, lp_servicename(snum) ));
		return False;
	}
			       ...
			       const struct auth_serversupplied_info *session_info)
{
	...
	if ((current_user.conn == conn) &&
		...
		DEBUG_OUT (7, ("Skipping user change - already user\n"));
...
bool change_to_root_user(void)
{
	...
	DEBUG_OUT (5,("change_to_root_user: now uid=(%d,%d) gid=(%d,%d)\n",
...
static void push_conn_ctx(void)
{
	...
	if (conn_ctx_stack_ndx == MAX_SEC_CTX_DEPTH) {
		DEBUG_OUT (0, ("Connection context stack overflow!\n"));
	...
	DEBUG_OUT (4, ("push_conn_ctx(%u) : conn_ctx_stack_ndx = %d\n",
...
static void pop_conn_ctx(void)
{
	...
	if (conn_ctx_stack_ndx == 0) {
		DEBUG_OUT (0, ("Connection context stack underflow!\n"));
