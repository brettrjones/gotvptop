			     ...
			     DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("smb_conf_updated: Got message saying smb.conf was "
		  "updated. Reloading.\n"));
	change_to_root_user();
			     ...
			     DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("Got message saying pcap was updated. Reloading.\n"));
	change_to_root_user();
				  ...
				  DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("smb_stat_cache_delete: delete name %s\n", name));
...
static void msg_sam_sync(struct messaging_context *msg,
			 ...
			 DATA_BLOB *data)
{
        DEBUG_OUT (10, ("** sam sync message received, ignoring\n"));
}
...
static void msg_exit_server(struct messaging_context *msg,
			    ...
			    DATA_BLOB *data)
{
	DEBUG_OUT (3, ("got a SHUTDOWN message\n"));
			     ...
			     DATA_BLOB *data)
{
	...
	if (data->length != sizeof(sig)) {
		DEBUG_OUT (0, ("Process %s sent bogus signal injection request\n",
...
#if HAVE_STRSIGNAL
	DEBUG_OUT (0, ("Process %s requested injection of signal %d (%s)\n",
		  procid_str_static(&src), sig, strsignal(sig)));
#else
	DEBUG_OUT (0, ("Process %s requested injection of signal %d\n",
				...
				void *private_data)
{
	...
	DEBUG_OUT (1,("Cleaning up brl and lock database after unclean shutdown\n"));
...
static void remove_child_pid(pid_t pid, bool unclean_shutdown)
{
	...
	if (child == NULL) {
		...
		DEBUG_OUT (2, ("Could not find child %d -- ignoring\n", (int)pid));
		return;
	}
	...
	if (unclean_shutdown) {
		/* a child terminated uncleanly so tickle all
		   ...
		   pending locks
                */
		DEBUG_OUT (3,(__location__ " Unclean shutdown of pid %u\n",
			(unsigned int)pid));
		if (!cleanup_te) {
			...
			cleanup_te = event_add_timed(smbd_event_context(), NULL,
						...
						cleanup_timeout_fn,
						&cleanup_te);
			DEBUG_OUT (1,("Scheduled cleanup of brl and lock database after unclean shutdown\n"));
		}
	}

	if (!serverid_deregister(child_id)) {
		DEBUG_OUT (1, ("Could not remove pid %d from serverid.tdb\n",
			  (int)pid));
	}
}
				   ...
				   void *private_data)
{
	...
	if (fd == -1) {
		DEBUG_OUT (0,("open_sockets_smbd: accept: %s\n",
			 strerror(errno)));
		return;
	}
	...
	if (pid == 0) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			if (NT_STATUS_EQUAL(status,
				...
				DEBUG_OUT (0,("child process cannot initialize "
			...
			if (lp_clustering() &&
				...
				DEBUG_OUT (1,("child process cannot initialize "
	...
	if (pid < 0) {
		DEBUG_OUT (0,("smbd_accept_connection: sys_fork() failed: %s\n",
			 strerror(errno)));
	}
				 ...
				 uint16_t port)
{
	...
	s->fd = open_socket_in(SOCK_STREAM,
			       ...
			       true);
	if (s->fd == -1) {
		DEBUG_OUT (0,("smbd_open_once_socket: open_socket_in: "
			"%s\n", strerror(errno)));
		TALLOC_FREE(s);
		/*
	...
	if (listen(s->fd, SMBD_LISTEN_BACKLOG) == -1) {
		DEBUG_OUT (0,("open_sockets_smbd: listen: "
			"%s\n", strerror(errno)));
			close(s->fd);
		TALLOC_FREE(s);
		return false;
			      ...
			      const char *smb_ports)
{
	...
	if (parent->sockets == NULL) {
		DEBUG_OUT (0,("open_sockets_smbd: No "
	...
	if (!serverid_register(procid_self(),
			       FLAG_MSG_GENERAL|FLAG_MSG_SMBD
			       |FLAG_MSG_DBWRAP)) {
		DEBUG_OUT (0, ("open_sockets_smbd: Failed to register "
	...
	if (lp_multicast_dns_register() && (dns_port != 0)) {
		...
		if (avahi_conn == NULL) {
			DEBUG_OUT (10, ("avahi_start_register failed\n"));
		}
#endif
	}
...
static void smbd_parent_loop(struct smbd_parent_context *parent)
{
	...
	DEBUG_OUT (2,("waiting for connections\n"));
	while (1) {
		int ret;
 ...
 int main(int argc,const char *argv[])
{
	...
	if (log_stdout && Fork) {
		DEBUG_OUT (0,("ERROR: Can't log to stdout (-S) unless daemon is in foreground (-F) or interactive (-i)\n"));
		exit(1);
	}
	...
	DEBUG_OUT (0,("smbd version %s started.\n", samba_version_string()));
	DEBUGADD(0,("%s\n", COPYRIGHT_STARTUP_MESSAGE));

	DEBUG_OUT (2,("uid=%d gid=%d euid=%d egid=%d\n",
	...
	if (sizeof(uint16) < 2 || sizeof(uint32) < 4) {
		DEBUG_OUT (0,("ERROR: Samba is not configured correctly for the word size on your machine\n"));
		exit(1);
	}
	...
	DEBUG_OUT (3,("loaded services\n"));

	init_structs();

#ifdef WITH_PROFILE
	if (!profile_setup(smbd_messaging_context(), False)) {
		DEBUG_OUT (0,("ERROR: failed to setup profiling\n"));
		return -1;
	}
	if (profile_level != NULL) {
		...
		DEBUG_OUT (1, ("setting profiling level: %s\n",profile_level));
		src.pid = getpid();
	...
	if (!is_daemon && !is_a_socket(0)) {
		if (!interactive)
			DEBUG_OUT (0,("standard input is not a socket, assuming -D option\n"));
	...
	if (!secrets_init()) {
		DEBUG_OUT (0, ("ERROR: smbd can not open secrets.tdb\n"));
		exit(1);
	}

	if (lp_server_role() == ROLE_DOMAIN_BDC || lp_server_role() == ROLE_DOMAIN_PDC) {
		if (!open_schannel_session_store(NULL, lp_private_dir())) {
			DEBUG_OUT (0,("ERROR: Samba cannot open schannel store for secured NETLOGON operations.\n"));
			exit(1);
		}
	}

	if(!get_global_sam_sid()) {
		DEBUG_OUT (0,("ERROR: Samba cannot create a SAM SID.\n"));
		exit(1);
	}
	...
	if (!share_info_db_init()) {
		DEBUG_OUT (0,("ERROR: failed to load share info db.\n"));
		exit(1);
	}
	...
	if (!init_guest_info()) {
		DEBUG_OUT (0,("ERROR: failed to setup guest info.\n"));
		return -1;
	}

	if (!file_init(smbd_server_conn)) {
		DEBUG_OUT (0, ("ERROR: file_init failed\n"));
		return -1;
	}
