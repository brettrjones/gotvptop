...
static void get_challenge(struct smbd_server_connection *sconn, uint8 buff[8])
{
	...
	if (sconn->smb1.negprot.auth_context) {
		DEBUG_OUT (3, ("get challenge: is this a secondary negprot? "
			  "sconn->negprot.auth_context is non-NULL!\n"));
			TALLOC_FREE(sconn->smb1.negprot.auth_context);
	}

	DEBUG_OUT (10, ("get challenge: creating negprot_global_auth_context\n"));
	nt_status = make_auth_context_subsystem(
		sconn, &sconn->smb1.negprot.auth_context);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("make_auth_context_subsystem returned %s",
			  nt_errstr(nt_status)));
		smb_panic("cannot make_negprot_global_auth_context!");
	}
	DEBUG_OUT (10, ("get challenge: getting challenge\n"));
...
static void reply_nt1(struct smb_request *req, uint16 choice)
{
	...
	if (lp_server_signing()) {
			...
			DEBUG_OUT (0,("reply_nt1: smb signing is incompatible with share level security !\n"));
			if (lp_server_signing() == Required) {
	...
	if (!negotiate_spnego) {
		...
		if (sconn->smb1.negprot.encrypted_passwords) {
			...
			if (ret == -1) {
				DEBUG_OUT (0, ("Could not push challenge\n"));
		...
		ret = message_push_string(&req->outbuf, lp_workgroup(),
					  ...
					  |STR_NOALIGN);
		if (ret == -1) {
			DEBUG_OUT (0, ("Could not push workgroup string\n"));
			reply_nterror(req, NT_STATUS_NO_MEMORY);
			return;
		}
		DEBUG_OUT (3,("not using SPNEGO\n"));
	} else {
		...
		if (ret == -1) {
			DEBUG_OUT (0, ("Could not push spnego blob\n"));
			reply_nterror(req, NT_STATUS_NO_MEMORY);
			return;
		}
...
void reply_negprot(struct smb_request *req)
{
	...
	if (req->buflen == 0) {
		DEBUG_OUT (0, ("negprot got no protocols\n"));
	...
	if (req->buf[req->buflen-1] != '\0') {
		DEBUG_OUT (0, ("negprot protocols not 0-terminated\n"));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		END_PROFILE(SMBnegprot);
		return;
	}
	...
	while (smbreq_bufrem(req, p) > 0) {
		...
		if (!pull_ascii_talloc(cliprotos, &cliprotos[num_cliprotos], p,
				       &converted_size)) {
			DEBUG_OUT (0, ("pull_ascii_talloc failed\n"));
			TALLOC_FREE(cliprotos);
			reply_nterror(req, NT_STATUS_NO_MEMORY);
			END_PROFILE(SMBnegprot);
			return;
		...
		DEBUG_OUT (3, ("Requested protocol [%s]\n",
	...
	if(choice != -1) {
		...
		DEBUG_OUT (3,("Selected protocol %s\n",supported_protocols[protocol].proto_name));
	} else {
		DEBUG_OUT (0,("No protocol supported !\n"));
		reply_outbuf(req, 1, 0);
		SSVAL(req->outbuf, smb_vwv0, choice);
	}

	DEBUG_OUT ( 5, ( "negprot index=%d\n", choice ) );
