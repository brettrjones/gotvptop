				...
				unsigned int uLevel, char *id1, char *id2)
{
	...
	switch( uLevel ) {
		...
		case 52:
			...
			break;
		default:
			DEBUG_OUT (0,("check_printq_info: invalid level %d\n",
				uLevel ));
			return False;
	}
	if (id1 == NULL || strcmp(desc->format,id1) != 0) {
		DEBUG_OUT (0,("check_printq_info: invalid format %s\n",
			id1 ? id1 : "<NULL>" ));
		return False;
	}
	if (desc->subformat && (id2 == NULL || strcmp(desc->subformat,id2) != 0)) {
		DEBUG_OUT (0,("check_printq_info: invalid subformat %s\n",
			id2 ? id2 : "<NULL>" ));
		return False;
	}
	return True;
				...
				const char *printer_name)
{
	...
	DEBUG_OUT (3,("Printer Driver Name: %s:\n",driver->driver_name));
	DEBUG_OUT (3,("Driver: %s:\n",driver->driver_path));
	DEBUG_OUT (3,("Data File: %s:\n",driver->data_file));
	DEBUG_OUT (3,("Language Monitor: %s:\n",driver->monitor_name));
	DEBUG_OUT (3,("Driver Location: %s:\n",location));
	DEBUG_OUT (3,("Data Type: %s:\n",driver->default_datatype));
	DEBUG_OUT (3,("Help File: %s:\n",driver->help_file));
	...
	for ( i=0; i<count && driver->dependent_files && *driver->dependent_files[i]; i++)
	{
		...
		DEBUG_OUT (3,("Dependent File: %s:\n", driver->dependent_files[i]));
	}

	/* sanity check */
	if ( i != count )
		DEBUG_OUT (3,("fill_printq_info_52: file count specified by client [%d] != number of dependent files [%i]\n",
			count, i));

	DEBUG_OUT (3,("fill_printq_info on <%s> gave %d entries\n", printer_name, i));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("api_DosPrintQGetInfo uLevel=%d name=%s\n",uLevel,QueueName));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_DosPrintQGetInfo: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
	...
	if (uLevel==52) {
		...
		DEBUG_OUT (3,("api_DosPrintQGetInfo: Driver files count: %d\n",count));
	} else {
		uint32_t num_jobs;
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("printqgetinfo: errorcode %d\n",desc.errcode));
				...
				int *rdata_len, int *rparam_len)
{
	...
	DEBUG_OUT (3,("DosPrintQEnum uLevel=%d\n",uLevel));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_DosPrintQEnum: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
	...
	if((subcntarr = SMB_MALLOC_ARRAY(int,queuecnt)) == NULL) {
		DEBUG_OUT (0,("api_DosPrintQEnum: malloc fail !\n"));
		goto err;
	}
			   ...
			   const char *domain)
{
	...
	DEBUG_OUT (4,("Servertype search: %8x\n",servertype));
	...
	for (i=0;lines[i];i++) {
		...
		if (count == alloced) {
			...
			*servers = SMB_REALLOC_ARRAY(*servers,struct srv_info_struct, alloced);
			if (!*servers) {
				DEBUG_OUT (0,("get_session_info: failed to enlarge servers info struct!\n"));
				TALLOC_FREE(lines);
				return 0;
			}
		...
		if(local_list_only && ((s->type & SV_TYPE_LOCAL_LIST_ONLY) == 0)) {
			DEBUG_OUT (4,("r: local list only"));
			ok = False;
		}

		/* doesn't match up: don't want it */
		if (!(servertype & s->type)) {
			DEBUG_OUT (4,("r:serv type "));
			ok = False;
		}

		if ((servertype & SV_TYPE_DOMAIN_ENUM) !=
				(s->type & SV_TYPE_DOMAIN_ENUM)) {
			DEBUG_OUT (4,("s: dom mismatch "));
			ok = False;
		}
		...
		if (ok) {
			DEBUG_OUT (4,("**SV** %20s %8x %25s %15s\n",
				s->name, s->type, s->comment, s->domain));
			s->server_added = True;
			count++;
		} else {
			DEBUG_OUT (4,("%20s %8x %25s %15s\n",
				s->name, s->type, s->comment, s->domain));
		}
	}
				...
				char **rparam, int *rdata_len, int *rparam_len)
{
	...
	DEBUG_OUT (4, ("server request level: %s %8x ", str2, servertype));
	DEBUG_OUT (4, ("domains_req:%s ", BOOLSTR(domain_request)));
	DEBUG_OUT (4, ("local_only:%s\n", BOOLSTR(local_request)));
...

	{
		...
		for (i=0;i<total;i++) {
			...
			DEBUG_OUT (4,("fill_srv_info[%d] %20s %8x %25s %15s\n",
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (3,("NetServerEnum2 domain = %s uLevel=%d counted=%d total=%d\n",
				...
				char **rparam, int *rdata_len, int *rparam_len)
{
	...
	DEBUG_OUT (4, ("domain: '%s' first_name: '%s'\n",
		  domain, first_name));
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (3,("NetServerEnum3 domain = %s uLevel=%d first=%d[%s => %s] counted=%d total=%d\n",
				...
				char **rparam, int *rdata_len, int *rparam_len)
{
	...
	DEBUG_OUT (5,("RNetGroupGetUsers: %s %s %s %d %d\n",
				...
				int               *rparam_len )
{
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (3,("RNetShareEnum gave %d entries of %d (%d %d %d %d)\n",
				...
				int *rdata_len,int *rparam_len)
{
	...
	{
		DEBUG_OUT (0,("api_RNetShareAdd: pull_ascii_talloc failed: %s",
			 strerror(errno)));
	}
	...
	status = rpc_pipe_open_interface(mem_ctx, &ndr_table_srvsvc.syntax_id,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_RNetShareAdd: could not connect to srvsvc: %s\n",
			  nt_errstr(status)));
				...
				int *rdata_len,int *rparam_len)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("api_RNetUserEnum: Could not connect to samr: %s\n",
			  nt_errstr(status)));
		return false;
	}
	...
	status = dcerpc_samr_Connect2(b, talloc_tos(), global_myname(),
				      ...
				      &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("api_RNetUserEnum: samr_Connect2 failed: %s\n",
			  nt_errstr(status)));
		return false;
	}
	...
	status = dcerpc_samr_OpenDomain(b, talloc_tos(), &samr_handle,
					...
					&result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("api_RNetUserEnum: samr_OpenDomain failed: %s\n",
			  nt_errstr(status)));
	...
	DEBUG_OUT (10,("api_RNetGroupEnum:resume context: %d, client buffer size: "
	...
	*rdata = smb_realloc_limit(*rdata,*rdata_len);
	...
	while (true) {
		...
		status = dcerpc_samr_EnumDomainGroups(b, talloc_tos(),
						      ...
						      &result);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("dcerpc_samr_EnumDomainGroups returned "
				   "%s\n", nt_errstr(status)));
			break;
		}
		if (!NT_STATUS_IS_OK(result)) {
		...
		for(i=0; i<num_entries; i++) {
			...
			if( ((PTR_DIFF(p,*rdata)+21) > *rdata_len) ) {
				/* set overflow error */
				DEBUG_OUT (3,("overflow on entry %d group %s\n", i,
					 name));
				errflags=234;
				break;
			}
			...
			memset(p, 0, 21);
			strlcpy(p, name, 21);
			DEBUG_OUT (10,("adding entry %d group %s\n", i, p));
			p += 21;
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata = smb_realloc_limit(*rdata,*rdata_len);
	...
	status = dcerpc_samr_LookupNames(b, talloc_tos(),
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("api_RNetUserEnum: samr_LookupNames failed: %s\n",
			  nt_errstr(status)));
		goto close_domain;
	}
	if (!NT_STATUS_IS_OK(result)) {
	...
	if (type.ids[0] != SID_NAME_USER) {
		DEBUG_OUT (10, ("%s is a %s, not a user\n", UserName,
				...
				int *rdata_len,int *rparam_len)
{
	...
	resume_context = get_safe_SVAL(param,tpscnt,p,0,-1);
	cli_buf_size= get_safe_SVAL(param,tpscnt,p,2,0);
	DEBUG_OUT (10,("api_RNetUserEnum:resume context: %d, client buffer size: %d\n",
			resume_context, cli_buf_size));
	...
	*rdata = smb_realloc_limit(*rdata,*rdata_len);
	...
	while (true) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("dcerpc_samr_EnumDomainUsers returned "
				   "%s\n", nt_errstr(status)));
			break;
		}
		if (!NT_STATUS_IS_OK(result)) {
		...
		for (i=0; i<num_entries; i++) {
			...
			if(((PTR_DIFF(p,*rdata)+21)<=*rdata_len)
			   &&(strlen(name)<=21)) {
				...
				DEBUG_OUT (10,("api_RNetUserEnum:adding entry %d "
					  "username %s\n",count_sent,p));
				p += 21;
				count_sent++;
			} else {
				/* set overflow error */
				DEBUG_OUT (10,("api_RNetUserEnum:overflow on entry %d "
					  "username %s\n",count_sent,name));
				errflags=234;
				break;
			}
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	DEBUG_OUT (3,("Set password for <%s> (encrypted: %d, min_pwd_length: %d)\n",
	...
	status = rpc_pipe_open_interface(mem_ctx, &ndr_table_samr.syntax_id,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_SetUserPassword: could not connect to samr: %s\n",
			  nt_errstr(status)));
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	if(!strequal(p, "zsT")) {
		DEBUG_OUT (0,("api_SamOEMChangePassword: Invalid parameter string %s\n", p));
		return False;
	}
	p = skip_string(param, tpscnt, p);
	if (!p) {
	...
	if(!strequal(p, "B516B16")) {
		DEBUG_OUT (0,("api_SamOEMChangePassword: Invalid data parameter string %s\n", p));
		return False;
	}
	p = skip_string(param,tpscnt,p);
	if (!p) {
	...
	DEBUG_OUT (3,("api_SamOEMChangePassword: Change password for <%s>\n",user));
	...
	status = rpc_pipe_open_interface(mem_ctx, &ndr_table_samr.syntax_id,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_SamOEMChangePassword: could not connect to samr: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_RDosPrintJobDel: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_WPrintQueueCtrl: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
			       ...
			       int uLevel, char* id)
{
	...
	default:
		DEBUG_OUT (0,("check_printjob_info: invalid level %d\n",
			uLevel ));
		return False;
	}
	if (id == NULL || strcmp(desc->format,id) != 0) {
		DEBUG_OUT (0,("check_printjob_info: invalid format %s\n",
			id ? id : "<NULL>" ));
		return False;
	}
	return True;
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_PrintJobInfo: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (4,("NetServerGetInfo level %d\n",uLevel));
	...
	*rdata = smb_realloc_limit(*rdata,*rdata_len);
	...
	status = rpc_pipe_open_interface(mem_ctx, &ndr_table_srvsvc.syntax_id,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_RNetServerGetInfo: could not connect to srvsvc: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (4,("NetWkstaGetInfo level %d\n",level));
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("RNetUserGetInfo level=%d\n", uLevel));
	...
	*rdata = smb_realloc_limit(*rdata,*rdata_len);
	...
	status = rpc_pipe_open_interface(mem_ctx, &ndr_table_samr.syntax_id,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_RNetUserGetInfo: could not connect to samr: %s\n",
			  nt_errstr(status)));
		errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	if(vuser != NULL) {
		DEBUG_OUT (3,("  Username of UID %d is %s\n",
	...
	DEBUG_OUT (3,("WWkstaUserLogon uLevel=%d name=%s\n",uLevel,name));
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WWkstaUserLogon: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WAccessGetUserPerms user=%s resource=%s\n",user,resource));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintJobGetInfo uLevel=%d uJobId=0x%X\n",uLevel,SVAL(p,0)));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_WPrintJobGetInfo: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintJobGetInfo: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintJobEnumerate uLevel=%d name=%s\n",uLevel,name));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_WPrintJobEnumerate: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintJobEnumerate: errorcode %d\n",desc.errcode));
				...
				int uLevel, char* id)
{
	...
	switch( uLevel ) {
		...
		case 3:
			...
			break;
		default:
			DEBUG_OUT (0,("check_printdest_info: invalid level %d\n",
				uLevel));
			return False;
	}
	if (id == NULL || strcmp(desc->format,id) != 0) {
		DEBUG_OUT (0,("check_printdest_info: invalid string %s\n",
			id ? id : "<NULL>" ));
		return False;
	}
	return True;
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintDestGetInfo uLevel=%d PrinterName=%s\n",uLevel,PrinterName));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_WPrintDestGetInfo: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintDestGetInfo: errorcode %d\n",desc.errcode));
	SAFE_FREE(tmpdata);
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintDestEnum uLevel=%d\n",uLevel));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("api_WPrintDestEnum: could not connect to spoolss: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintDestEnumerate: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintDriverEnum uLevel=%d\n",uLevel));
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintDriverEnum: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintQProcEnum uLevel=%d\n",uLevel));
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintQProcEnum: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG_OUT (3,("WPrintPortEnum uLevel=%d\n",uLevel));
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("WPrintPortEnum: errorcode %d\n",desc.errcode));
...

{
	...
	DEBUG_OUT (3,("RNetSessionEnum uLevel=%d\n",uLevel));
	DEBUG_OUT (7,("RNetSessionEnum req string=%s\n",str1));
	DEBUG_OUT (7,("RNetSessionEnum ret string=%s\n",str2));
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("RNetSessionEnum: could not connect to srvsvc: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
	...
	status = dcerpc_srvsvc_NetSessEnum(b, mem_ctx,
					   ...
					   &werr);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("RNetSessionEnum: dcerpc_srvsvc_NetSessEnum failed: %s\n",
			  nt_errstr(status)));
		desc.errcode = W_ERROR_V(ntstatus_to_werror(status));
		goto out;
	}
 ...
 out:
	...
	*rparam = smb_realloc_limit(*rparam,*rparam_len);
	...
	DEBUG_OUT (4,("RNetSessionEnum: errorcode %d\n",desc.errcode));
			 ...
			 int *rdata_len, int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	DEBUG_OUT (3,("Supplied buffer too small in API command\n"));
				...
				int *rdata_len, int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	DEBUG_OUT (3,("Unsupported API command\n"));
	       ...
	       int mdrcnt, int mprcnt)
{
	...
	if (!params) {
		DEBUG_OUT (0,("ERROR: NULL params in api_reply()\n"));
	...
	DEBUG_OUT (3,("Got API command %d of form <%s> <%s> (tdscnt=%d,tpscnt=%d,mdrcnt=%d,mprcnt=%d)\n",
		api_command,
		name1,
		name2,
	...
	if(!rdata || !rparam) {
		DEBUG_OUT (0,("api_reply: malloc fail !\n"));
		SAFE_FREE(rdata);
		SAFE_FREE(rparam);
