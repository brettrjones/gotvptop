				...
				const char *name)
{
	...
	if ( string_to_sid( &sid, name ) ) {
		DEBUG_OUT (5,("token_contains_name: Checking for SID [%s] in token\n", name));
	...
	if (!do_group_checks(&name, &prefix)) {
		if (!lookup_name_smbconf(mem_ctx, name, LOOKUP_NAME_ALL,
				 NULL, NULL, &sid, &type)) {
			DEBUG_OUT (5, ("lookup_name %s failed\n", name));
			return False;
		}
		if (type != SID_NAME_USER) {
			DEBUG_OUT (5, ("%s is a %s, expected a user\n",
	...
	for (/* initialized above */ ; *prefix != '\0'; prefix++) {
		if (*prefix == '+') {
			...
			if ((type != SID_NAME_DOM_GRP) &&
				...
				DEBUG_OUT (5, ("%s is a %s, expected a group\n",
					  name, sid_type_lookup(type)));
				return False;
			}
		   ...
		   const struct security_token *token, int snum)
{
	if (lp_invalid_users(snum) != NULL) {
		if (token_contains_name_in_list(username, domain,
			...
			DEBUG_OUT (10, ("User %s in 'invalid users'\n", username));
			return False;
		}
	}
	...
	if (lp_valid_users(snum) != NULL) {
		if (!token_contains_name_in_list(username, domain,
						 ...
						 lp_valid_users(snum))) {
			DEBUG_OUT (10, ("User %s not in 'valid users'\n",
				   username));
			return False;
		}
	}
	...
	if (lp_onlyuser(snum)) {
		...
		if ((list[0] == NULL) || (*list[0] == '\0')) {
			DEBUG_OUT (0, ("'only user = yes' and no 'username ='\n"));
			return False;
		}
		if (!token_contains_name_in_list(NULL, domain,
						 lp_servicename(snum),
						 token, list)) {
			DEBUG_OUT (10, ("%s != 'username'\n", username));
			return False;
		}
	}

	DEBUG_OUT (10, ("user_ok_token: share %s is ok for unix user %s\n",
				  ...
				  connection_struct *conn)
{
	...
	DEBUG_OUT (10,("is_share_read_only_for_user: share %s is %s for unix user "
		  "%s\n", lp_servicename(snum),
