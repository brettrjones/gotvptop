...
find_again:
	...
	if (i == -1) {
                ...
                if (newsz <= oldsz) {
                        /* Integer wrap. */
		        DEBUG_OUT (0,("ERROR! Out of connection structures\n"));
                        return NULL;
                }

		DEBUG_OUT (4,("resizing connections bitmap from %d to %d\n",
                        oldsz, newsz));

                nbmap = bitmap_talloc(sconn, newsz);
		if (!nbmap) {
			DEBUG_OUT (0,("ERROR! malloc fail.\n"));
			return NULL;
		}
	 ...
	 */
	if (i > 65534) {
		DEBUG_OUT (0, ("Maximum connection limit reached\n"));
		return NULL;
	}
		    ...
		    DATA_BLOB *data)
{
	...
	if (strcmp(sharename, "*") == 0) {
		DEBUG_OUT (1,("Forcing close of all shares\n"));
			...
			for (tcon = sess->tcons.list; tcon; tcon = tc_next) {
				...
				if (tcon->compat_conn &&
						strequal(lp_servicename(SNUM(tcon->compat_conn)),
								sharename)) {
					DEBUG_OUT (1,("Forcing close of share %s cnum=%d\n",
