...
int linux_set_lease_sighandler(int fd)
{
        if (fcntl(fd, F_SETSIG, RT_SIGNAL_LEASE) == -1) {
                DEBUG_OUT (3,("Failed to set signal handler for kernel lease\n"));
                return -1;
        }
					...
					void *_info, void *private_data)
{
	...
	if (fsp == NULL) {
		DEBUG_OUT (0,("linux_oplock_signal_handler: failed to find fsp for file fd=%d (file was closed ?)\n", fd ));
		return;
	}
				    ...
				    files_struct *fsp, int oplock_type)
{
	if ( SMB_VFS_LINUX_SETLEASE(fsp, F_WRLCK) == -1) {
		DEBUG_OUT (3,("linux_set_kernel_oplock: Refused oplock on file %s, "
			 "fd = %d, file_id = %s. (%s)\n",
	...
	DEBUG_OUT (3,("linux_set_kernel_oplock: got kernel oplock on file %s, "
		 "file_id = %s gen_id = %lu\n",
...
struct kernel_oplocks *linux_init_kernel_oplocks(TALLOC_CTX *mem_ctx)
{
	...
	if (!linux_oplocks_available()) {
		DEBUG_OUT (3,("Linux kernel oplocks not available\n"));
		return NULL;
	}

	ctx = talloc_zero(mem_ctx, struct kernel_oplocks);
	if (!ctx) {
		DEBUG_OUT (0,("Linux Kernel oplocks talloc_Zero failed\n"));
		return NULL;
	}
	...
	se = tevent_add_signal(smbd_event_context(),
			       ...
			       ctx);
	if (!se) {
		DEBUG_OUT (0,("Failed to setup RT_SIGNAL_LEASE handler"));
		TALLOC_FREE(ctx);
		return NULL;
	}
	...
	DEBUG_OUT (3,("Linux kernel oplocks enabled\n"));
