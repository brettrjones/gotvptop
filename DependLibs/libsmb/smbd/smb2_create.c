...
int map_smb2_oplock_levels_to_samba(uint8_t in_oplock_level)
{
	...
	case SMB2_OPLOCK_LEVEL_LEASE:
		DEBUG_OUT (2,("map_smb2_oplock_levels_to_samba: "
...
static void smbd_smb2_request_create_done(struct tevent_req *tsubreq)
{
	...
	if (smb2req->cancelled) {
		...
		DEBUG_OUT (10,("smbd_smb2_request_create_done: cancelled mid %llu\n",
			...
			struct smb2_create_blobs in_context_blobs)
{
	...
	if (!smb2req->async) {
		...
		DEBUG_OUT (10,("smbd_smb2_create: name[%s]\n",
			in_name));
	} else {
		...
		DEBUG_OUT (10,("smbd_smb2_create_send: reentrant for file %s\n",
			in_name ));
	}
	...
	if (IS_IPC(smb1req->conn)) {
		...
		if (exta) {
			...
			if (!ea_list) {
				DEBUG_OUT (10,("smbd_smb2_create_send: read_ea_name_list failed.\n"));
		...
		if (secd) {
			...
			if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
				DEBUG_OUT (2,("ndr_pull_security_descriptor failed: %s\n",
					 ndr_errstr(ndr_err)));
				tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
				return tevent_req_post(req, ev);
			}
...
bool open_was_deferred_smb2(struct smbd_server_connection *sconn, uint64_t mid)
{
	...
	if (!smb2req) {
		DEBUG_OUT (10,("open_was_deferred_smb2: mid %llu smb2req == NULL\n",
			(unsigned long long)mid));
		return false;
	}
	if (!smb2req->subreq) {
	...
	DEBUG_OUT (10,("open_was_deferred_smb2: mid = %llu\n",
			(unsigned long long)mid));
							...
							uint64_t mid)
{
	...
	DEBUG_OUT (10,("remove_deferred_open_message_smb2_internal: "
		"mid %llu\n",
		(unsigned long long)mid ));
	...
	struct smbd_server_connection *sconn, uint64_t mid)
{
	...
	if (!smb2req) {
		DEBUG_OUT (10,("remove_deferred_open_message_smb2: "
			"can't find mid %llu\n",
			(unsigned long long)mid ));
		return;
	}
					...
					void *private_data)
{
	...
	DEBUG_OUT (10,("smbd_smb2_create_request_dispatch_immediate: "
	...
	struct smbd_server_connection *sconn, uint64_t mid)
{
	...
	if (!smb2req) {
		DEBUG_OUT (10,("schedule_deferred_open_message_smb2: "
			"can't find mid %llu\n",
			(unsigned long long)mid ));
		return;
	}
					...
					void *private_data)
{
	...
	DEBUG_OUT (10,("smb2_deferred_open_timer: [idx=%d], %s\n",
		smb2req->current_idx,
                                ...
                                size_t priv_len)
{
	...
	*/
	 ...
	 */
	if (timeout.tv_sec < 2) {
		DEBUG_OUT (10,("push_deferred_open_message_smb2: "
