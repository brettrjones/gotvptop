				...
				char *rdata, int data_offset, int data_len)
{
	...
	DEBUG_OUT (5,("copy_trans_params_and_data: params[%d..%d] data[%d..%d] (align %d)\n",
		      ...
		      bool buffer_too_large)
{
	...
	if (buffer_too_large)
		DEBUG_OUT (5,("send_trans_reply: buffer %d too large\n", ldata ));
...
static void api_dcerpc_cmd_write_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status) || (nwritten != state->num_data)) {
		DEBUG_OUT (10, ("Could not write to pipe: %s (%d/%d)\n",
...
static void api_dcerpc_cmd_read_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Could not read from to pipe: %s\n",
			   nt_errstr(status)));
		      ...
		      struct files_struct *fsp, char *param, int param_len)
{
	...
	DEBUG_OUT (4,("WaitNamedPipeHandleState priority %x\n",
		 (int)SVAL(param,0)));
		      ...
		      struct files_struct *fsp, char *param, int param_len)
{
	...
	DEBUG_OUT (4,("SetNamedPipeHandleState to code %x\n", (int)SVAL(param,0)));
...
static void api_no_reply(connection_struct *conn, struct smb_request *req)
{
	...
	DEBUG_OUT (3,("Unsupported API fd command\n"));
			 ...
			 int mdrcnt, int mprcnt)
{
	...
	DEBUG_OUT (5,("api_fd_reply\n"));

	/* First find out the name of this file. */
	if (suwcnt != 2) {
		DEBUG_OUT (0,("Unexpected named pipe transaction.\n"));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	if (!fsp_is_np(fsp)) {
		if (subcommand == TRANSACT_WAITNAMEDPIPEHANDLESTATE) {
			...
			DEBUG_OUT (3,("Got TRANSACT_WAITNAMEDPIPEHANDLESTATE on text pipe name\n"));
			send_trans_reply(conn, req, NULL, 0, NULL, 0, False);
			return;
		}

		DEBUG_OUT (1,("api_fd_reply: INVALID PIPE HANDLE: %x\n", pnum));
		reply_nterror(req, NT_STATUS_INVALID_HANDLE);
		return;
	}

	if (vuid != fsp->vuid) {
		DEBUG_OUT (1, ("Got pipe request (pnum %x) using invalid VUID %d, "
			  "expected %d\n", pnum, vuid, fsp->vuid));
		reply_nterror(req, NT_STATUS_INVALID_HANDLE);
		return;
	}

	DEBUG_OUT (3,("Got API command 0x%x on pipe \"%s\" (pnum %x)\n",
		 subcommand, fsp_str_dbg(fsp), pnum));

	DEBUG_OUT (10, ("api_fd_reply: p:%p max_trans_reply: %d\n", fsp, mdrcnt));
...
{
	DEBUG_OUT (3,("named pipe command on <%s> name\n", name));
	...
	if (strequal(name,"WKSSVC") ||
		...
		DEBUG_OUT (4,("named pipe command from Win95 (wow!)\n"));
	...
	if (setup)
		DEBUG_OUT (3,("unknown named pipe: setup 0x%X setup1=%d\n",
			 ...
			 struct trans_state *state)
{
	...
	DEBUG_OUT (3,("trans <%s> data=%u params=%u setup=%u\n",
	...
	DEBUG_OUT (5,("calling named_pipe\n"));
...
void reply_trans(struct smb_request *req)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (2, ("Got invalid trans request: %s\n",
			  nt_errstr(result)));
	...
	if (state->total_data)  {
		...
		if (state->data == NULL) {
			DEBUG_OUT (0,("reply_trans: data malloc fail for %u "
	...
	if (state->setup_count) {
		...
		if((state->setup = TALLOC_ARRAY(
			...
			DEBUG_OUT (0,("reply_trans: setup malloc fail for %u "
				 "bytes !\n", (unsigned int)
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_trans: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBtrans);
...
void reply_transs(struct smb_request *req)
{
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_transs: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
