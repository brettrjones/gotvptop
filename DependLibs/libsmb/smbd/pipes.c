		      ...
		      struct files_struct **pfsp)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("np_open(%s) returned %s\n", name,
			   nt_errstr(status)));
		file_free(smb_req, fsp);
		return status;
	}
...
void reply_pipe_write(struct smb_request *req)
{
	...
	DEBUG_OUT (6, ("reply_pipe_write: %x name: %s len: %d\n", (int)fsp->fnum,
...
static void pipe_write_done(struct tevent_req *subreq)
{
	...
	DEBUG_OUT (3,("write-IPC nwritten=%d\n", (int)nwritten));
...
void reply_pipe_write_and_X(struct smb_request *req)
{
	...
	DEBUG_OUT (6, ("reply_pipe_write_and_X: %x name: %s len: %d\n",
	...
	if (state->pipe_start_message_raw) {
		...
		if (state->numtowrite < 2) {
			DEBUG_OUT (0,("reply_pipe_write_and_X: start of message "
...
static void pipe_write_andx_done(struct tevent_req *subreq)
{
	...
	DEBUG_OUT (3,("writeX-IPC nwritten=%d\n", (int)nwritten));
...
static void pipe_read_andx_done(struct tevent_req *subreq)
{
	...
	DEBUG_OUT (3,("readX-IPC min=%d max=%d nread=%d\n",
