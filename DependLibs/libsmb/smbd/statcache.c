		...
		bool case_sensitive)
{
	...
	if (original_path_length != translated_path_length) {
		if (original_path_length < translated_path_length) {
			DEBUG_OUT (0, ("OOPS - tried to store stat cache entry "
	...
	DEBUG_OUT (5,("stat_cache_add: Added entry (%lx:size %x) %s -> %s\n",
			...
			SMB_STRUCT_STAT *pst)
{
	...
	*pp_start = *pp_name;
	...
	if (conn->case_sensitive) {
		...
		if (!chk_name) {
			DEBUG_OUT (0, ("stat_cache_lookup: strdup failed!\n"));
			return False;
		}
		...
		if (!chk_name) {
			DEBUG_OUT (0, ("stat_cache_lookup: talloc_strdup_upper failed!\n"));
			return False;
		}
	...
	while (1) {
		...
		DEBUG_OUT (10,("stat_cache_lookup: lookup failed for name [%s]\n",
				chk_name ));
		/*
	...
	DEBUG_OUT (10,("stat_cache_lookup: lookup succeeded for name [%s] "
...
void stat_cache_delete(const char *name)
{
	...
	DEBUG_OUT (10,("stat_cache_delete: deleting name [%s] -> %s\n",
			lname, name ));
