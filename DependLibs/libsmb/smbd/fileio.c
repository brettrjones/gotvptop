...
tryagain:
		...
		if (readret == -1) {
			if ((errno == EAGAIN) && numretries) {
				DEBUG_OUT (3,("read_file EAGAIN retry in 10 seconds\n"));
				(void)sleep(10);
				--numretries;
				goto tryagain;
			}
	...
	DEBUG_OUT (10,("read_file (%s): pos = %.0f, size = %lu, returned %lu\n",
				...
				size_t n)
{
	...
	DEBUG_OUT (10,("real_write_file (%s): pos = %.0f, size = %lu, returned %ld\n",
		  fsp_str_dbg(fsp), (double)pos, (unsigned long)n, (long)ret));
...
static int wcp_file_size_change(files_struct *fsp)
{
	...
	if (ret == -1) {
		DEBUG_OUT (0,("wcp_file_size_change (%s): ftruncate of size %.0f "
			 "error %s\n", fsp_str_dbg(fsp),
				      ...
				      void *private_data)
{
	...
	DEBUG_OUT (5, ("Update write time on %s\n", fsp_str_dbg(fsp)));
...
void trigger_write_time_update(struct files_struct *fsp)
{
	...
	DEBUG_OUT (5, ("Update write time %d usec later on %s\n",
...
void trigger_write_time_update_immediate(struct files_struct *fsp)
{
	...
	TALLOC_FREE(fsp->update_write_time_event);
	DEBUG_OUT (5, ("Update write time immediate on %s\n",
		  fsp_str_dbg(fsp)));
			...
			size_t n)
{
	...
	if (profile_p && profile_p->writecache_total_writes % 500 == 0) {
		DEBUG_OUT (3,("WRITECACHE: initwrites=%u abutted=%u total=%u \
nonop=%u allocated=%u active=%u direct=%u perfect=%u readhits=%u\n",
		...
		DEBUG_OUT (3,("WRITECACHE: Flushes SEEK=%d, READ=%d, WRITE=%d, READRAW=%d, OPLOCK=%d, CLOSE=%d, SYNC=%d\n",
	...
	DEBUG_OUT (9,("write_file (%s)(fd=%d pos=%.0f size=%u) wcp->offset=%.0f "
   ...
   +-------------------+
    ...
    Case 3).
		  ...
		  */
			...
			DEBUG_OUT (9,("write_file: non cacheable write : fd = %d, pos = %.0f, len = %u, current cache pos = %.0f \
len = %u\n",fsp->fh->fd, (double)pos, (unsigned int)n, (double)wcp->offset, (unsigned int)wcp->data_size ));
			...
			if ( n <= wcp->alloc_size && n > wcp->data_size) {
				...
				if ((pos <= wcp->offset) &&
					...
					DEBUG_OUT (9,("write_file: discarding overwritten write \
cache: fd = %d, off=%.0f, size=%u\n", fsp->fh->fd, (double)wcp->offset, (unsigned int)wcp->data_size ));
		...
		if (cache_flush_needed) {
			DEBUG_OUT (3,("WRITE_FLUSH:%d: due to noncontinuous write: fd = %d, size = %.0f, pos = %.0f, \
n = %u, wcp->offset=%.0f, wcp->data_size=%u\n",
	...
	if (n) {
		...
		DEBUG_OUT (9,("wcp->offset = %.0f wcp->data_size = %u cache return %u\n",
...
void delete_write_cache(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("delete_write_cache: File %s deleted write cache\n",
		  fsp_str_dbg(fsp)));
}
...
static bool setup_write_cache(files_struct *fsp, SMB_OFF_T file_size)
{
	...
	if((wcp = SMB_MALLOC_P(write_cache)) == NULL) {
		DEBUG_OUT (0,("setup_write_cache: malloc fail.\n"));
		return False;
	}
	...
	if((wcp->data = (char *)SMB_MALLOC(wcp->alloc_size)) == NULL) {
		DEBUG_OUT (0,("setup_write_cache: malloc fail for buffer size %u.\n",
	...
	DEBUG_OUT (10,("setup_write_cache: File %s allocated write cache size %lu\n",
...
ssize_t flush_write_cache(files_struct *fsp, enum flush_reason_enum reason)
{
	...
	DEBUG_OUT (9,("flushing write cache: fd = %d, off=%.0f, size=%u\n",
