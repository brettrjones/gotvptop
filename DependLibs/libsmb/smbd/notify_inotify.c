...
{
	DEBUG_OUT (10, ("filter_match: e->mask=%x, w->mask=%x, w->filter=%x\n",
			     ...
			     struct inotify_event *e2)
{
	...
	DEBUG_OUT (10, ("inotify_dispatch called with mask=%x, name=[%s]\n",
		   e->mask, e->len ? e->name : ""));
	...
	DEBUG_OUT (10, ("inotify_dispatch: ne.action = %d, ne.path = %s\n",
		   ne.action, ne.path));
			    ...
			    uint16_t flags, void *private_data)
{
	...
	*/
	if (ioctl(in->fd, FIONREAD, &bufsize) != 0 || 
	    bufsize == 0) {
		DEBUG_OUT (0,("No data on inotify fd?!\n"));
		TALLOC_FREE(fde);
		return;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Failed to read all inotify data - %s\n",
			nt_errstr(status)));
		talloc_free(e0);
...
static NTSTATUS inotify_setup(struct sys_notify_context *ctx)
{
	...
	in->fd = inotify_init();
	if (in->fd == -1) {
		DEBUG_OUT (0,("Failed to init inotify - %s\n", strerror(errno)));
		talloc_free(in);
...
static int watch_destructor(struct inotify_watch_context *w)
{
	...
	if (w == NULL) {
		DEBUG_OUT (10, ("Deleting inotify watch %d\n", wd));
		if (inotify_rm_watch(in->fd, wd) == -1) {
			DEBUG_OUT (1, ("inotify_rm_watch returned %s\n",
				  strerror(errno)));
		}
		       ...
		       void *handle_p)
{
	...
	if (wd == -1) {
		e->filter = filter;
		DEBUG_OUT (1, ("inotify_add_watch returned %s\n", strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	DEBUG_OUT (10, ("inotify_add_watch for %s mask %x returned wd %d\n",
		   e->path, mask, wd));
