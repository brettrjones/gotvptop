			...
			bool uc)
{
	...
	DEBUG_OUT (8,("put name [%s] from [%s] into dir struct\n",buf+30, fname));
	return True;
}
...
{
	if (dptr->dir_hnd) {
		DEBUG_OUT (4,("Idling dptr dnum %d\n",dptr->dnum));
...
static void dptr_idleoldest(struct smbd_server_connection *sconn)
{
	...
	if(!dptr) {
		DEBUG_OUT (0,("No dptrs available to idle ?\n"));
		return;
	}
	...
	for(dptr = sconn->searches.dirptrs; dptr; dptr = dptr->next) {
		if(dptr->dnum == key) {
			if (!forclose && !dptr->dir_hnd) {
				...
				DEBUG_OUT (4,("dptr_get: Reopening dptr key %d\n",key));
				if (!(dptr->dir_hnd = OpenDir(
					      NULL, dptr->conn, dptr->path,
					      dptr->wcard, dptr->attr))) {
					DEBUG_OUT (4,("dptr_get: Failed to open %s (%s)\n",dptr->path,
						strerror(errno)));
					return False;
				}
			}
...
static void dptr_close_internal(struct dptr_struct *dptr)
{
	...
	DEBUG_OUT (4,("closing dptr key %d\n",dptr->dnum));
	...
	if (!bitmap_query(sconn->searches.dptr_bmap, dptr->dnum - 1)) {
		DEBUG_OUT (0,("dptr_close_internal : Error - closing dnum = %d and bitmap not set !\n",
			dptr->dnum ));
	}
...
void dptr_close(struct smbd_server_connection *sconn, int *key)
{
	...
	if (!dptr) {
		DEBUG_OUT (0,("Invalid key %d given to dptr_close\n", *key));
		return;
	}
			      ...
			      bool old)
{
	...
	if(!dptr) {
		DEBUG_OUT (0,("No old dptrs available to close oldest ?\n"));
		return;
	}
		...
		const char *wcard, bool wcard_has_wild, uint32 attr, struct dptr_struct **dptr_ret)
{
	...
	DEBUG_OUT (5,("dptr_create dir=%s\n", path));

	if (sconn == NULL) {
		DEBUG_OUT (0,("dptr_create: called with fake connection_struct\n"));
	...
	if(!dptr) {
		DEBUG_OUT (0,("malloc fail in dptr_create.\n"));
		TALLOC_FREE(dir_hnd);
	...
	if(old_handle) {
		...
		if(dptr->dnum == -1 || dptr->dnum > 254) {
			...
			dptr->dnum = bitmap_find(sconn->searches.dptr_bmap, 0);
			if(dptr->dnum == -1 || dptr->dnum > 254) {
				DEBUG_OUT (0,("dptr_create: returned %d: Error - all old dirptrs in use ?\n", dptr->dnum));
				SAFE_FREE(dptr->path);
				SAFE_FREE(dptr->wcard);
				SAFE_FREE(dptr);
				TALLOC_FREE(dir_hnd);
		...
		if(dptr->dnum == -1 || dptr->dnum < 255) {
			...
			if(dptr->dnum == -1 || dptr->dnum < 255) {
				DEBUG_OUT (0,("dptr_create: returned %d: Error - all new dirptrs in use ?\n", dptr->dnum));
				SAFE_FREE(dptr->path);
				SAFE_FREE(dptr->wcard);
				SAFE_FREE(dptr);
				TALLOC_FREE(dir_hnd);
	...
	DEBUG_OUT (3,("creating new dirptr %d for path %s, expect_close = %d\n",
	       ...
	       char *buf1,unsigned int key)
{
	...
	uint32 offset;
	if (!dptr) {
		DEBUG_OUT (1,("filling null dirptr %d\n",key));
		return(False);
	}
	offset = (uint32)TellDir(dptr->dir_hnd);
	DEBUG_OUT (6,("fill on key %u dirptr 0x%lx now at %d\n",key,
			       ...
			       char *buf, int *num)
{
	...
	if (!dptr) {
		DEBUG_OUT (3,("fetched null dirptr %d\n",key));
		return(NULL);
	}
	*num = key;
	offset = IVAL(buf,1);
	...
	DEBUG_OUT (3,("fetching dirptr %d for path %s at offset %d\n",
				       ...
				       int dptr_num)
{
	...
	DEBUG_OUT (3,("fetching dirptr %d for path %s\n",dptr_num,dptr->path));
	return(dptr);
}
			   ...
			   long *_prev_offset)
{
	...
	*_mode = 0;
	...
	while (true) {
		...
		DEBUG_OUT (6,("smbd_dirptr_get_entry: dirptr 0x%lx now at offset %ld\n",
		...
		if (!dir_check_ftype(conn, mode, dirtype)) {
			DEBUG_OUT (5,("[%s] attribs 0x%x didn't match 0x%x\n",
		...
		DEBUG_OUT (3,("smbd_dirptr_get_entry mask=[%s] found %s "
			"fname=%s (%s)\n",
				    ...
				    uint32_t *_mode)
{
	...
	if (!VALID_STAT(smb_fname->st)) {
		if ((SMB_VFS_STAT(conn, smb_fname)) != 0) {
			DEBUG_OUT (5,("smbd_dirptr_8_3_mode_fn: "
		     ...
		     const char *name, SMB_STRUCT_STAT *pst, bool use_veto)
{
	...
	if (use_veto && IS_VETO_PATH(conn, name)) {
		DEBUG_OUT (10,("is_visible_file: file %s is vetoed.\n", name ));
		return False;
	}
	...
	if (hide_unreadable || hide_unwriteable || hide_special) {
		...
		if (hide_unreadable &&
			...
			DEBUG_OUT (10,("is_visible_file: file %s is unreadable.\n",
				 entry ));
			ret = false;
			goto out;
		}
		...
		if (hide_unwriteable && !user_can_write_file(conn,
							     smb_fname_base)) {
			DEBUG_OUT (10,("is_visible_file: file %s is unwritable.\n",
				 entry ));
			ret = false;
			goto out;
		}
		...
		if (hide_special && file_is_special(conn, smb_fname_base)) {
			DEBUG_OUT (10,("is_visible_file: file %s is special.\n",
				 entry ));
			ret = false;
			goto out;
		}
			...
			uint32 attr)
{
	...
	if (!dirp->dir) {
		DEBUG_OUT (5,("OpenDir: Can't open %s. %s\n", dirp->dir_path,
			 strerror(errno) ));
		goto fail;
	}
			...
			uint32 attr)
{
	...
	if (fsp->is_directory && fsp->fh->fd != -1) {
		...
		if (dirp->dir == NULL) {
			DEBUG_OUT (10,("OpenDir_fsp: SMB_VFS_FDOPENDIR on %s returned "
				"NULL (%s)\n",
				dirp->dir_path,
				strerror(errno)));
			if (errno != ENOSYS) {
	...
	if (!dirp->dir) {
		DEBUG_OUT (5,("OpenDir_fsp: Can't open %s. %s\n", dirp->dir_path,
			 strerror(errno) ));
		goto fail;
	}
...
NTSTATUS can_delete_directory_fsp(files_struct *fsp)
{
	...
	while ((dname = ReadDirName(dir_hnd, &dirpos, &st, &talloced))) {
		...
		DEBUG_OUT (10,("can_delete_directory_fsp: got name %s - can't delete\n",
			 dname ));
