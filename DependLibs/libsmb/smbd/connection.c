...
bool yield_connection(connection_struct *conn, const char *name)
{
	...
	DEBUG_OUT (3,("Yielding connection to %s\n",name));

	rec = connections_fetch_entry(talloc_tos(), conn, name);
	if (rec == NULL) {
		DEBUG_OUT (0, ("connections_fetch_entry failed\n"));
		return False;
	}

	status = rec->delete_rec(rec);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT ( NT_STATUS_EQUAL(status, NT_STATUS_NOT_FOUND) ? 3 : 0,
		    ...
		    void *udp)
{
	...
	if (cs->Clear && !process_exists(crec->pid) && (errno == ESRCH)) {
		NTSTATUS status;
		DEBUG_OUT (2,("pid %s doesn't exist - deleting connections %d [%s]\n",
			 procid_str_static(&crec->pid), crec->cnum,
			 crec->servicename));

		status = rec->delete_rec(rec);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("count_fn: tdb_delete failed with error %s\n",
				 nt_errstr(status)));
		}
		return 0;
	}
...
int count_current_connections( const char *sharename, bool clear  )
{
	...
	if (connections_forall(count_fn, &cs) == -1) {
		DEBUG_OUT (0,("count_current_connections: traverse of "
...
bool claim_connection(connection_struct *conn, const char *name)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("claim_connection: tdb_store failed with error %s.\n",
			 nt_errstr(status)));
		return False;
	}
