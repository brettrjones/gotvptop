...
static void smbd_smb2_request_lock_done(struct tevent_req *subreq)
{
	...
	if (smb2req->cancelled) {
		...
		DEBUG_OUT (10,("smbd_smb2_request_lock_done: cancelled mid %llu\n",
						 ...
						 struct smbd_smb2_lock_element *in_locks)
{
	...
	DEBUG_OUT (10,("smbd_smb2_lock_send: %s - fnum[%d]\n",
	...
	for (i=0; i<in_lock_count; i++) {
		...
		DEBUG_OUT (10,("smbd_smb2_lock_send: index %d offset=%llu, count=%llu, "
			"smblctx = %llu type %d\n",
			i,
				...
				DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("received_unlock_msg (SMB2)\n"));
...
static bool recalc_smb2_brl_timeout(struct smbd_server_connection *sconn)
{
	...
	if (timeval_is_zero(&next_timeout)) {
		DEBUG_OUT (10, ("recalc_smb2_brl_timeout:Next "
			"timeout = Infinite.\n"));
		return true;
	}
				...
				uint64_t blocking_smblctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("push_blocking_lock_request_smb2: "
	...
	DEBUG_OUT (10,("push_blocking_lock_request_smb2: file %s timeout %d\n",
		fsp_str_dbg(fsp),
		lock_timeout ));
			...
			struct blocking_lock_record *blr)
{
	...
	DEBUG_OUT (10, ("remove_pending_lock: BLR = %p\n", blr));
				...
				struct timeval tv_curr)
{
	...
	if(blr->lock_num == state->lock_count) {
		...
		DEBUG_OUT (3,("reprocess_blocked_smb2_lock SUCCESS file = %s, "
			"fnum=%d num_locks=%d\n",
			fsp_str_dbg(fsp),
			fsp->fnum,
	...
	DEBUG_OUT (10,("reprocess_blocked_smb2_lock: only got %d locks of %d needed "
