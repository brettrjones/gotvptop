...
int register_initial_vuid(struct smbd_server_connection *sconn)
{
	...
	if((vuser = talloc_zero(NULL, user_struct)) == NULL) {
		DEBUG_OUT (0,("register_initial_vuid: "
	...
	DEBUG_OUT (10,("register_initial_vuid: allocated vuid = %u\n",
...
int register_homes_share(const char *username)
{
	...
	if (result != -1) {
		DEBUG_OUT (3, ("Using static (or previously created) service for "
			  "user '%s'; path = '%s'\n", username,
			  lp_pathname(result)));
		return result;
	}
	...
	if ((pwd == NULL) || (pwd->pw_dir[0] == '\0')) {
		DEBUG_OUT (3, ("No home directory defined for user '%s'\n",
			  username));
		TALLOC_FREE(pwd);
		return -1;
	}

	DEBUG_OUT (3, ("Adding homes service for user '%s' using home directory: "
		  "'%s'\n", username, pwd->pw_dir));
			...
			const char *smb_name)
{
	...
	DEBUG_OUT (10,("register_existing_vuid: (%u,%u) %s %s %s guest=%d\n",
	...
	DEBUG_OUT (3, ("register_existing_vuid: User name: %s\t"
		  "Real name: %s\n", vuser->session_info->unix_name,
		  vuser->session_info->info3->base.full_name.string ?
		  vuser->session_info->info3->base.full_name.string : ""));

	if (!vuser->session_info->security_token) {
		DEBUG_OUT (1, ("register_existing_vuid: session_info does not "
			"contain a user_token - cannot continue\n"));
		goto fail;
	}

	DEBUG_OUT (3,("register_existing_vuid: UNIX uid %d is UNIX user %s, "
		"and will be vuid %u\n", (int)vuser->session_info->utok.uid,
		 vuser->session_info->unix_name, vuser->vuid));

	if (!session_claim(sconn, vuser)) {
		DEBUG_OUT (1, ("register_existing_vuid: Failed to claim session "
			"for vuid=%d\n",
			vuser->vuid));
		goto fail;
	}
		      ...
		      const char *user)
{
	...
	if (strlen(sconn->smb1.sessions.session_userlist) > 128 * 1024) {
		DEBUG_OUT (3,("add_session_user: session userlist already "
			 "too large.\n"));
		goto done;
	}
		...
		if (gptr) {
			...
			*member_list = '\0';
			...
			for(i = 0; gptr->gr_mem && gptr->gr_mem[i]; i++) {
				...
				DEBUG_OUT (10,("validate_group: = gr_mem = "
					  "%s\n", gptr->gr_mem[i]));
			...
			while (*member) {
				...
				DEBUG_OUT (10,("validate_group = member = %s\n",
					  member));
		     ...
		     bool *guest)
{
	...
	DEBUG_OUT (100,("authorise_login: checking authorisation on "
		   "user=%s pass=%s\n", user,password.data));
#endif
	...
	if (!ok) {
		...
		for (auser = strtok_r(user_list, LIST_SEP, &saveptr);
			...
			if (password_ok(actx, enc,
				...
				ok = True;
				fstrcpy(user,user2);
				DEBUG_OUT (3,("authorise_login: ACCEPTED: session "
		...
		for (auser = strtok_r(user_list, LIST_SEP, &saveptr);
			...
			if (*auser == '@') {
				...
				if (auser) {
					ok = True;
					fstrcpy(user,auser);
					DEBUG_OUT (3,("authorise_login: ACCEPTED: "
  ...
  check_guest:
	...
	if (!ok && GUEST_OK(snum)) {
		...
		if (guest_pw != NULL) {
			...
			ok = True;
			DEBUG_OUT (3,("authorise_login: ACCEPTED: guest account "
				 "and guest ok (%s)\n",	user));
		} else {
			DEBUG_OUT (0,("authorise_login: Invalid guest account "
				 "%s??\n",guestname));
		}
	...
	if (ok && !user_ok(user, snum)) {
		DEBUG_OUT (0,("authorise_login: rejected invalid user %s\n",user));
		ok = False;
	}
