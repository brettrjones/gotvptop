				  ...
				  const struct smb_perfcount_handlers *handlers)
{
	...
	if (interface_version != SMB_PERFCOUNTER_INTERFACE_VERSION) {
		DEBUG_OUT (0, ("Failed to register perfcount module.\n"
	...
	if (!name || !name[0] || !handlers) {
		DEBUG_OUT (0,("smb_register_perfcounter() called with NULL pointer "
			"or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (smb_perfcount_find_module(name)) {
		DEBUG_OUT (3,("Perfcount Module %s already loaded!\n", name));
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (3, ("Successfully added perfcounter module '%s'\n", name));
	return NT_STATUS_OK;
}
...
static bool smb_load_perfcount_module(const char *name)
{
	...
	DEBUG_OUT (3, ("Initialising perfcounter module [%s]\n", name));

	if (g_smb_perfcount_handlers) {
		DEBUG_OUT (3,("Only 1 perfcount handler may be registered in "
			"smb.conf\n"));
		return true;
	}
	...
	if((entry = smb_perfcount_find_module(module_name)) ||
	   (NT_STATUS_IS_OK(smb_probe_module("perfcount", module_path)) &&
		...
		DEBUG_OUT (3,("Successfully loaded perfcounter module [%s] \n", name));
	} else {
		DEBUG_OUT (0,("Can't find a perfcounter module [%s]\n",name));
		goto fail;
	}
...
bool smb_perfcount_init(void)
{
	...
	if (!smb_load_perfcount_module(perfcount_object)) {
		DEBUG_OUT (0, ("smbd_load_percount_module failed for %s\n",
			perfcount_object));
		return False;
	}
