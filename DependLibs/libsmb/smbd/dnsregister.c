				    ...
				    void *private_data)
{
	...
	DEBUG_OUT (6, ("registering _smb._tcp service on port %d\n",
		  dns_state->port));
	...
	if (err != kDNSServiceErr_NoError) {
		 ...
		 */
		DEBUG_OUT (3, ("unable to register with mDNS (err %d)\n", err));
		goto retry;
	}
					  ...
					  void *private_data)
{
	...
	if (err != kDNSServiceErr_NoError) {
		DEBUG_OUT (3, ("failed to process mDNS result (err %d), re-trying\n",
			    err));
		goto retry;
	}
