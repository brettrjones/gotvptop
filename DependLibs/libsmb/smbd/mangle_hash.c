		   ...
		   const struct share_params *p)
{
	...
	if (!push_ucs2_talloc(NULL, &ucs2name, f, &size)) {
		DEBUG_OUT (0,("is_8_3: internal error push_ucs2_talloc() failed!\n"));
		goto done;
	}
				...
				const char *raw_name )
{
	...
	if (tdb_store_bystring(tdb_mangled_cache, mangled_name_key, data_val, TDB_REPLACE) != 0) {
		DEBUG_OUT (0,("cache_mangled_name: Error storing entry %s -> %s\n", mangled_name_key, raw_name));
	} else {
		DEBUG_OUT (5,("cache_mangled_name: Stored entry %s -> %s\n", mangled_name_key, raw_name));
	}
			...
			const struct share_params *p)
{
	...
	DEBUG_OUT (5,("hash_name_to_8_3( %s, cache83 = %s)\n", in,
		 cache83 ? "True" : "False"));
	...
	DEBUG_OUT (5,("hash_name_to_8_3(%s) ==> [%s]\n", in, out));
	return True;
}
