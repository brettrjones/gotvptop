				...
				gss_cred_id_t *p_srv_cred)
{
	...
	DEBUG_OUT (10,("get_srv_gss_creds: imported name %s\n",
		host_princ_s ));
	...
	if (ret != GSS_S_COMPLETE) {
		...
		DEBUG_OUT (10,("get_srv_gss_creds: gss_acquire_cred failed with %s\n",
			ads_errstr(adss)));
...
static NTSTATUS srv_enc_spnego_gss_negotiate(unsigned char **ppdata, size_t *p_data_size, DATA_BLOB secblob)
{
	...
	if (ret == GSS_S_COMPLETE) {
		if ((flags & (GSS_C_INTEG_FLAG|GSS_C_CONF_FLAG|GSS_C_REPLAY_FLAG|GSS_C_SEQUENCE_FLAG)) !=
			...
			DEBUG_OUT (0,("srv_enc_spnego_gss_negotiate: quality of service not good enough "
				"for SMB sealing.\n"));
					...
					size_t *p_param_size)
{
	...
	*p_param_size = 0;
	...
	DEBUG_OUT (1,("srv_request_encryption_setup: Unknown packet\n"));
...
NTSTATUS srv_encryption_start(connection_struct *conn)
{
	...
	DEBUG_OUT (1,("srv_encryption_start: context negotiated\n"));
	return NT_STATUS_OK;
}
