				...
				bool *ppath_contains_wcard)
{
	...
	if (!sconn->using_smb2 && (*pathname != sepchar)) {
		DEBUG_OUT (10,("parse_dfs_path: path %s doesn't start with %c\n",
			pathname, sepchar ));
		/*
		...
		p = temp;
		DEBUG_OUT (10,("parse_dfs_path: trying to convert %s to a "
			"local path\n",
			temp));
		goto local_path;
	}
	...
	DEBUG_OUT (10,("parse_dfs_path: temp = |%s| after trimming %c's\n",
		temp, sepchar));
	...
	if(p == NULL) {
		DEBUG_OUT (10,("parse_dfs_path: can't parse hostname from path %s\n",
			temp));
		/*
		 * Possibly client sent a local path by mistake.
		 * Try and convert to a local path.
		...
		p = temp;
		DEBUG_OUT (10,("parse_dfs_path: trying to convert %s "
			"to a local path\n",
			temp));
		goto local_path;
	}
	*p = '\0';
	...
	DEBUG_OUT (10,("parse_dfs_path: hostname: %s\n",pdp->hostname));
	...
	if (conn && !( strequal(servicename, lp_servicename(SNUM(conn)))
		...
		DEBUG_OUT (10,("parse_dfs_path: %s is not our servicename\n",
			servicename));
	...
	DEBUG_OUT (10,("parse_dfs_path: servicename: %s\n",pdp->servicename));
  ...
  local_path:
	...
	*ppath_contains_wcard = False;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("parse_dfs_path: '%s' failed with %s\n",
			p, nt_errstr(status) ));
		return status;
	}

	DEBUG_OUT (10,("parse_dfs_path: rest of the path: %s\n",pdp->reqpath));
	return NT_STATUS_OK;
}
				...
				char **poldcwd)
{
	...
	if (session_info != NULL) {
		...
		if (conn->session_info == NULL) {
			DEBUG_OUT (0, ("copy_serverinfo failed\n"));
			TALLOC_FREE(conn);
			return NT_STATUS_NO_MEMORY;
		}
	...
	if (conn->session_info) {
		...
		if ((conn->share_access & FILE_WRITE_DATA) == 0) {
			if ((conn->share_access & FILE_READ_DATA) == 0) {
				...
				DEBUG_OUT (0,("create_conn_struct: connection to %s "
	...
	if (!smbd_vfs_init(conn)) {
		...
		DEBUG_OUT (0,("create_conn_struct: smbd_vfs_init failed.\n"));
		conn_free(conn);
		return status;
	}
	...
	if (SMB_VFS_CONNECT(conn, lp_servicename(snum), vfs_user) < 0) {
		DEBUG_OUT (0,("VFS connect failed!\n"));
		conn_free(conn);
	...
	if (oldcwd == NULL) {
		NTSTATUS status = map_nt_error_from_unix(errno);
		DEBUG_OUT (3, ("vfs_GetWd failed: %s\n", strerror(errno)));
		conn_free(conn);
		return status;
	}
	...
	if (vfs_ChDir(conn,conn->connectpath) != 0) {
		NTSTATUS status = map_nt_error_from_unix(errno);
		DEBUG_OUT (3,("create_conn_struct: Can't ChDir to new conn path %s. "
			"Error was %s\n",
				...
				int *refcount)
{
	...
	if (!prot) {
		DEBUG_OUT (0,("parse_msdfs_symlink: invalid path !\n"));
		return False;
	}
	...
	DEBUG_OUT (10,("parse_msdfs_symlink: count=%d\n", count));
	...
	for(i=0;i<count;i++) {
		...
		DEBUG_OUT (10, ("parse_msdfs_symlink: Created alt path: %s\n",
			...
			SMB_STRUCT_STAT *sbufp)
{
	...
	if (SMB_VFS_LSTAT(conn, &smb_fname) != 0) {
		DEBUG_OUT (5,("is_msdfs_link_read_target: %s does not exist.\n",
			path));
		goto err;
	}
	if (!S_ISLNK(smb_fname.st.st_ex_mode)) {
		DEBUG_OUT (5,("is_msdfs_link_read_target: %s is not a link.\n",
					path));
		goto err;
	}
	if (sbufp != NULL) {
	...
	if (referral_len == -1) {
		DEBUG_OUT (0,("is_msdfs_link_read_target: Error reading "
			"msdfs link %s: %s\n",
			path, strerror(errno)));
		goto err;
	}
	...
	DEBUG_OUT (5,("is_msdfs_link_internal: %s -> %s\n",path,
				link_target));
		...
		char **pp_targetpath)
{
	...
	DEBUG_OUT (10,("dfs_path_lookup: Conn path = %s reqpath = %s\n",
	...
	if (is_msdfs_link_internal(ctx, conn, smb_fname->base_name,
				   pp_targetpath, NULL)) {
		if (search_flag) {
			DEBUG_OUT (6,("dfs_path_lookup (FindFirst) No redirection "
				 "for dfs link %s.\n", dfspath));
			status = NT_STATUS_OK;
			goto out;
		}

		DEBUG_OUT (6,("dfs_path_lookup: %s resolves to a "
			"valid dfs link %s.\n", dfspath,
	...
	while (p) {
		*p = '\0';
		...
		if (is_msdfs_link_internal(ctx, conn,
					   ...
					   NULL)) {
			DEBUG_OUT (4, ("dfs_path_lookup: Redirecting %s because "
				  "parent %s is dfs link\n", dfspath,
				  smb_fname_str_dbg(smb_fname)));

			if (consumedcntp) {
				*consumedcntp = strlen(canon_dfspath);
				DEBUG_OUT (10, ("dfs_path_lookup: Path consumed: %s "
					"(%d)\n",
					canon_dfspath,
					*consumedcntp));
			}
			...
			bool *ppath_contains_wcard)
{
	...
	if (pdp->reqpath[0] == '\0') {
		...
		*pp_path_out = talloc_strdup(ctx, "");
		...
		DEBUG_OUT (5,("dfs_redirect: self-referral.\n"));
		return NT_STATUS_OK;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		if (NT_STATUS_EQUAL(status, NT_STATUS_PATH_NOT_COVERED)) {
			DEBUG_OUT (3,("dfs_redirect: Redirecting %s\n", path_in));
		} else {
			DEBUG_OUT (10,("dfs_redirect: dfs_path_lookup "
				"failed for %s with %s\n",
	...
	DEBUG_OUT (3,("dfs_redirect: Not redirecting %s.\n", path_in));
	...
	*pp_path_out = talloc_strdup(ctx, pdp->reqpath);
	...
	DEBUG_OUT (3,("dfs_redirect: Path %s converted to non-dfs path %s\n",
				path_in,
				*pp_path_out));
			...
			bool *self_referralp)
{
	...
	*self_referralp = False;
	...
	if (!lp_msdfs_root(snum) && (*lp_msdfs_proxy(snum) == '\0')) {
		DEBUG_OUT (3,("get_referred_path: |%s| in dfs path %s is not "
			"a dfs root.\n",
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_PATH_NOT_COVERED)) {
		DEBUG_OUT (3,("get_referred_path: No valid referrals for path %s\n",
			dfs_path));
		goto err_exit;
	}
	...
	if (!parse_msdfs_symlink(ctx, targetpath,
				&jucn->referral_list,
				&jucn->referral_count)) {
		DEBUG_OUT (3,("get_referred_path: failed to parse symlink "
			"target %s\n", targetpath ));
				...
				bool self_referral)
{
	...
	DEBUG_OUT (10,("Setting up version2 referral\nRequested path:\n"));
				...
				bool self_referral)
{
	...
	DEBUG_OUT (10,("setting up version3 referral\n"));
	...
	pdata = (char *)SMB_REALLOC(pdata,reply_size);
	if(pdata == NULL) {
		DEBUG_OUT (0,("version3 referral setup:"
			...
			char **ppdata, NTSTATUS *pstatus)
{
	...
	*pstatus = get_referred_path(ctx, pathnamep, junction,
	...
	DEBUG_OUT (10,("max_referral_level :%d\n",max_referral_level));
	...
	case 3:
		...
		break;
	default:
		DEBUG_OUT (0,("setup_dfs_referral: Invalid dfs referral "
			"version: %d\n",
			max_referral_level));
		talloc_destroy(ctx);
		...
		struct junction_map *jucn)
{
	...
	if (!is_myname_or_ipaddr(pdp->hostname)) {
		DEBUG_OUT (4,("create_junction: Invalid hostname %s "
			"in dfs path %s\n",
	...
	if(snum < 0 || !lp_msdfs_root(snum)) {
		DEBUG_OUT (4,("create_junction: %s is not an msdfs root.\n",
			pdp->servicename));
		TALLOC_FREE(pdp);
		return False;
	}
...
bool create_msdfs_link(const struct junction_map *jucn)
{
	...
	DEBUG_OUT (5,("create_msdfs_link: Creating new msdfs link: %s -> %s\n",
		path, msdfs_link));
	...
	if(SMB_VFS_SYMLINK(conn, msdfs_link, path) < 0) {
		...
		if (SMB_VFS_SYMLINK(conn, msdfs_link, path) < 0) {
			DEBUG_OUT (1,("create_msdfs_link: symlink failed "
				 "%s -> %s\nError: %s\n",
...
static int count_dfs_links(TALLOC_CTX *ctx, int snum)
{
	...
	status = create_conn_struct(talloc_tos(), &conn, snum, connect_path,
				    NULL, &cwd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("create_conn_struct failed: %s\n",
			  nt_errstr(status)));
		return 0;
	}
				...
				size_t jn_remain)
{
	...
	*/
	...
	while ((dname = vfs_readdirname(conn, dirp, NULL, &talloced))
		...
		char *link_target = NULL;
		if (cnt >= jn_remain) {
			DEBUG_OUT (2, ("form_junctions: ran out of MSDFS "
				"junction slots"));
			TALLOC_FREE(talloced);
			goto out;
		}
