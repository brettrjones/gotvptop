		      ...
		      const char *ea_name, struct ea_struct *pea)
{
 ...
 again:
	...
	DEBUG_OUT (10,("get_ea_value: EA %s is of length %u\n", ea_name, (unsigned int)sizeret));
				...
				char ***pnames, size_t *pnum_names)
{
	...
	DEBUG_OUT (10, ("get_ea_list_from_file: ea_namelist size = %u\n",
	...
	*pea_total_len = 0;
	...
	for (i=0; i<num_names; i++) {
		...
		*pea_total_len +=
		...
		DEBUG_OUT (10,("get_ea_list_from_file: total_len = %u, %s, val len "
	...
	DEBUG_OUT (10, ("get_ea_list_from_file: total_len = %u\n",
	...
	connection_struct *conn, struct ea_list *ea_list)
{
	...
	DEBUG_OUT (10,("fill_ea_buffer: data_size = %u\n", ret_data_size ));
				       ...
				       struct ea_list *ea_list)
{
	...
	*ret_data_size = PTR_DIFF(p, pdata);
	DEBUG_OUT (10,("fill_ea_chained_buffer: data_size = %u\n", *ret_data_size));
	return NT_STATUS_OK;
}
...
static void canonicalize_ea_name(connection_struct *conn, files_struct *fsp, const char *fname, fstring unix_ea_name)
{
	...
	for (; ea_list; ea_list = ea_list->next) {
		if (strequal(&unix_ea_name[5], ea_list->ea.name)) {
			DEBUG_OUT (10,("canonicalize_ea_name: %s -> %s\n",
		...
		const struct smb_filename *smb_fname, struct ea_list *ea_list)
{
	...
	for (;ea_list; ea_list = ea_list->next) {
		...
		DEBUG_OUT (10,("set_ea: ea_name %s ealen = %u\n", unix_ea_name, (unsigned int)ea_list->ea.value.length));

		if (samba_private_attr_name(unix_ea_name)) {
			DEBUG_OUT (10,("set_ea: ea name %s is a private Samba name.\n", unix_ea_name));
			return NT_STATUS_ACCESS_DENIED;
		}
		...
		if (ea_list->ea.value.length == 0) {
			...
			if (ret == -1 && errno == ENOATTR) {
				DEBUG_OUT (10,("set_ea: deleting ea name %s didn't exist - succeeding by default.\n",
						unix_ea_name));
				ret = 0;
			}
#endif
			...
			if (fsp && (fsp->fh->fd != -1)) {
				DEBUG_OUT (10,("set_ea: setting ea name %s on file "
					  "%s by file descriptor.\n",
					  unix_ea_name, fsp_str_dbg(fsp)));
				ret = SMB_VFS_FSETXATTR(fsp, unix_ea_name,
							ea_list->ea.value.data, ea_list->ea.value.length, 0);
			} else {
				DEBUG_OUT (10,("set_ea: setting ea name %s on file %s.\n",
					unix_ea_name, fname));
...
static struct ea_list *read_ea_name_list(TALLOC_CTX *ctx, const char *pdata, size_t data_size)
{
	...
	while (offset + 2 < data_size) {
		...
		if (!pull_ascii_talloc(ctx, &eal->ea.name, &pdata[offset],
				       &converted_size)) {
			DEBUG_OUT (0,("read_ea_name_list: pull_ascii_talloc "
		...
		DEBUG_OUT (10,("read_ea_name_list: read ea name %s\n", eal->ea.name));
	}
...
struct ea_list *read_ea_list_entry(TALLOC_CTX *ctx, const char *pdata, size_t data_size, size_t *pbytes_used)
{
	...
	if (!pull_ascii_talloc(ctx, &eal->ea.name, pdata + 4, &converted_size)) {
		DEBUG_OUT (0,("read_ea_list_entry: pull_ascii_talloc failed: %s",
			 strerror(errno)));
	}
	if (!eal->ea.name) {
		return NULL;
	...
	DEBUG_OUT (10,("read_ea_list_entry: read ea name %s\n", eal->ea.name));
			 ...
			 int max_data_bytes)
{
	...
	if (max_data_bytes > 0 && datasize > max_data_bytes) {
		DEBUG_OUT (5,("send_trans2_replies: max_data_bytes %d exceeded by data %d\n",
	...
	if (useable_space < 0) {
		DEBUG_OUT (0, ("send_trans2_replies failed sanity useable_space "
			  "= %d!!!", useable_space));
	...
	while (params_to_send || data_to_send) {
		...
		DEBUG_OUT (9,("t2_rep: params_sent_thistime = %d, data_sent_thistime = %d, useable_space = %d\n",
			params_sent_thistime, data_sent_thistime, useable_space));
		DEBUG_OUT (9,("t2_rep: params_to_send = %d, data_to_send = %d, paramsize = %d, datasize = %d\n",
		...
		/* Sanity check */
		if(params_to_send < 0 || data_to_send < 0) {
			DEBUG_OUT (0,("send_trans2_replies failed sanity check pts = %d, dts = %d\n!!!",
			    ...
			    unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (3,("call_trans2open %s deny_mode=0x%x attr=%d ofun=0x%x size=%d\n",
	...
	if (total_data && (total_data != 4)) {
		...
		if (IVAL(pdata,0) > total_data) {
			DEBUG_OUT (10,("call_trans2open: bad total data size (%u) > %u\n",
...
static uint32 unix_filetype(mode_t mode)
{
	...
	DEBUG_OUT (0,("unix_filetype: unknown filetype %u\n", (unsigned)mode));
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	if(lp_host_msdfs() &&
		...
		DEBUG_OUT (5,("check_msdfs_link: Masquerading msdfs link %s "
			"as a directory\n",
			pathname));
					...
					uint32_t *_mode)
{
	...
	if (INFO_LEVEL_IS_UNIX(state->info_level)) {
		if (SMB_VFS_LSTAT(state->conn, smb_fname) != 0) {
			DEBUG_OUT (5,("smbd_dirptr_lanman2_mode_fn: "
				 "Couldn't lstat [%s] (%s)\n",
				    ...
				    uint64_t *last_entry_off)
{
	...
	*out_of_space = false;
	...
	if (pad && pad > space_remaining) {
		*out_of_space = true;
		DEBUG_OUT (9,("smbd_marshall_dir_entry: out of space "
	...
	DEBUG_OUT (10,("smbd_marshall_dir_entry: space_remaining = %d\n",
		space_remaining ));
	...
	case SMB_FIND_INFO_STANDARD:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_INFO_STANDARD\n"));
	...
	case SMB_FIND_EA_SIZE:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_EA_SIZE\n"));
		if (requires_resume_key) {
			SIVAL(p,0,reskey);
			p += 4;
		}
	...
	case SMB_FIND_EA_LIST:
	{
		...
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_EA_LIST\n"));
		if (!name_list) {
			return false;
		}
		if (requires_resume_key) {
	...
	case SMB_FIND_FILE_BOTH_DIRECTORY_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_BOTH_DIRECTORY_INFO\n"));
	...
	case SMB_FIND_FILE_DIRECTORY_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_DIRECTORY_INFO\n"));
		p += 4;
		SIVAL(p,0,reskey); p += 4;
		put_long_date_timespec(conn->ts_res,p,create_date_ts); p += 8;
		put_long_date_timespec(conn->ts_res,p,adate_ts); p += 8;
	...
	case SMB_FIND_FILE_FULL_DIRECTORY_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_FULL_DIRECTORY_INFO\n"));
		p += 4;
		SIVAL(p,0,reskey); p += 4;
		put_long_date_timespec(conn->ts_res,p,create_date_ts); p += 8;
		put_long_date_timespec(conn->ts_res,p,adate_ts); p += 8;
	...
	case SMB_FIND_FILE_NAMES_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_NAMES_INFO\n"));
		p += 4;
		SIVAL(p,0,reskey); p += 4;
		p += 4;
	...
	case SMB_FIND_ID_FULL_DIRECTORY_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_ID_FULL_DIRECTORY_INFO\n"));
		p += 4;
		SIVAL(p,0,reskey); p += 4;
		put_long_date_timespec(conn->ts_res,p,create_date_ts); p += 8;
		put_long_date_timespec(conn->ts_res,p,adate_ts); p += 8;
	...
	case SMB_FIND_ID_BOTH_DIRECTORY_INFO:
		DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_ID_BOTH_DIRECTORY_INFO\n"));
		was_8_3 = mangle_is_8_3(fname, True, conn->params);
		p += 4;
		SIVAL(p,0,reskey); p += 4;
		put_long_date_timespec(conn->ts_res,p,create_date_ts); p += 8;
	...
	case SMB_FIND_FILE_UNIX_INFO2:
		...
		if (info_level == SMB_FIND_FILE_UNIX) {
			DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_UNIX\n"));
			...
			DEBUG_OUT (10,("smbd_marshall_dir_entry: SMB_FIND_FILE_UNIX_INFO2\n"));
				 ...
				 unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (3,("call_trans2findfirst: dirtype = %x, maxentries = %d, close_after_first=%d, \
close_if_end = %d requires_resume_key = %d level = 0x%x, max_data_bytes = %d\n",
	...
	if (info_level == SMB_FIND_EA_LIST) {
		...
		if (ea_size != total_data) {
			DEBUG_OUT (4,("call_trans2findfirst: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
	...
	*pparams = (char *)SMB_REALLOC(*pparams, 10);
	...
	DEBUG_OUT (4,("dptr_num is %d, wcard = %s, attr = %d\n", dptr_num, mask, dirtype));
	...
	DEBUG_OUT (8,("dirpath=<%s> dontdescend=<%s>\n",
	...
	if(close_after_first || (finished && close_if_end)) {
		DEBUG_OUT (5,("call_trans2findfirst - (2) closing dptr_num %d\n", dptr_num));
	...
	DEBUG_OUT ( 4, ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
		smb_fn_name(req->cmd),
				...
				unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (3,("call_trans2findnext: dirhandle = %d, max_data_bytes = %d, maxentries = %d, \
...
resume_key = %d resume name = %s continue=%d level = %d\n",
	...
	if (info_level == SMB_FIND_EA_LIST) {
		...
		ea_size = IVAL(pdata,0);
		if (ea_size != total_data) {
			DEBUG_OUT (4,("call_trans2findnext: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
			reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
			return;
		}
	...
	*pparams = (char *)SMB_REALLOC(*pparams, 6*SIZEOFWORD);
	...
	if((p = dptr_wcard(sconn, dptr_num))== NULL) {
		DEBUG_OUT (2,("dptr_num %d has no wildcard\n", dptr_num));
		reply_nterror(req, STATUS_NO_MORE_FILES);
		return;
	}
	...
	DEBUG_OUT (3,("dptr_num is %d, mask = %s, attr = %x, dirptr=(0x%lX,%ld)\n",
		dptr_num, mask, dirtype,
		(long)dirptr,
		dptr_TellDir(dirptr)));
	...
	DEBUG_OUT ( 3, ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
		smb_fn_name(req->cmd),
		mask, directory, dirtype, numentries ) );

	/* Check if we can close the dirptr */
	if(close_after_request || (finished && close_if_end)) {
		DEBUG_OUT (5,("call_trans2findnext: closing dptr_num = %d\n", dptr_num));
			 ...
			 int *ret_data_len)
{
	...
	if (IS_IPC(conn)) {
		if (info_level != SMB_QUERY_CIFS_UNIX_INFO) {
			DEBUG_OUT (0,("smbd_do_qfsinfo: not an allowed "
	...
	*ppdata = (char *)SMB_REALLOC(
	...
	switch (info_level) {
		case SMB_INFO_ALLOCATION:
		{
			...
			DEBUG_OUT (5,("smbd_do_qfsinfo : SMB_INFO_ALLOCATION id=%x, bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)st.st_ex_dev, (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_INFO_VOLUME:
			...
			DEBUG_OUT (5,("smbd_do_qfsinfo : time = %x, namelen = %d, name = %s\n",
		...
		case SMB_FS_VOLUME_INFORMATION:
			...
			DEBUG_OUT (5,("smbd_do_qfsinfo : SMB_QUERY_FS_VOLUME_INFO namelen = %d, vol=%s serv=%s\n",
		...
		case SMB_FS_SIZE_INFORMATION:
		{
			...
			bytes_per_sector = 512;
			sectors_per_unit = bsize/bytes_per_sector;
			DEBUG_OUT (5,("smbd_do_qfsinfo : SMB_QUERY_FS_SIZE_INFO bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_FS_FULL_SIZE_INFORMATION:
		{
			...
			bytes_per_sector = 512;
			sectors_per_unit = bsize/bytes_per_sector;
			DEBUG_OUT (5,("smbd_do_qfsinfo : SMB_QUERY_FS_FULL_SIZE_INFO bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
				(unsigned int)bytes_per_sector, (unsigned int)dsize, (unsigned int)dfree));
		 ...
		 */
		{
			...
			if (get_current_uid(conn) != 0) {
				DEBUG_OUT (0,("set_user_quota: access_denied "
					 "service [%s] user [%s]\n",
			...
			if (vfs_get_ntquota(&fsp, SMB_USER_FS_QUOTA_TYPE, NULL, &quotas)!=0) {
				DEBUG_OUT (0,("vfs_get_ntquota() failed for service [%s]\n",lp_servicename(SNUM(conn))));
				return map_nt_error_from_unix(errno);
			}

			data_len = 48;

			DEBUG_OUT (10,("SMB_FS_QUOTA_INFORMATION: for service [%s]\n",
				  lp_servicename(SNUM(conn))));
		...
		case SMB_QUERY_POSIX_FS_INFO:
		{
			...
			if (!rc) {
				...
				DEBUG_OUT (5,("smbd_do_qfsinfo : SMB_QUERY_POSIX_FS_INFO succsessful\n"));
#ifdef EOPNOTSUPP
			} else if (rc == EOPNOTSUPP) {
				return NT_STATUS_INVALID_LEVEL;
#endif /* EOPNOTSUPP */
			} else {
				DEBUG_OUT (0,("vfs_statvfs() failed for service [%s]\n",lp_servicename(SNUM(conn))));
			       ...
			       unsigned int max_data_bytes)
{
	...
	if (ENCRYPTION_REQUIRED(conn) && !req->encrypted) {
		if (info_level != SMB_QUERY_CIFS_UNIX_INFO) {
			DEBUG_OUT (0,("call_trans2qfsinfo: encryption required "
	...
	DEBUG_OUT (3,("call_trans2qfsinfo: level = %d\n", info_level));
				 ...
				 unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (10,("call_trans2setfsinfo: for service [%s]\n",lp_servicename(SNUM(conn))));

	/*  */
	if (total_params < 4) {
		DEBUG_OUT (0,("call_trans2setfsinfo: requires total_params(%d) >= 4 bytes!\n",
			total_params));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	if (IS_IPC(conn)) {
		if (info_level != SMB_REQUEST_TRANSPORT_ENCRYPTION &&
			...
			DEBUG_OUT (0,("call_trans2setfsinfo: not an allowed "
				"info level (0x%x) on IPC$.\n",
				(unsigned int)info_level));
			reply_nterror(req, NT_STATUS_ACCESS_DENIED);
			return;
	...
	if (ENCRYPTION_REQUIRED(conn) && !req->encrypted) {
		if (info_level != SMB_REQUEST_TRANSPORT_ENCRYPTION) {
			DEBUG_OUT (0,("call_trans2setfsinfo: encryption required "
				"and info level 0x%x sent.\n",
				(unsigned int)info_level));
			exit_server_cleanly("encryption required "
				"on connection");
		...
		case SMB_SET_CIFS_UNIX_INFO:
			{
				...
				DEBUG_OUT (10,("call_trans2setfsinfo: set unix info. major = %u, minor = %u \
cap_low = 0x%x, cap_high = 0x%x\n",
		...
		case SMB_REQUEST_TRANSPORT_ENCRYPTION:
			{
				...
				if (req->sconn->smb1.echo_handler.trusted_fde) {
					DEBUG_OUT ( 2,("call_trans2setfsinfo: "
				...
				DEBUG_OUT ( 4,("call_trans2setfsinfo: "
		...
		case SMB_FS_QUOTA_INFORMATION:
			{
				...
				if ((get_current_uid(conn) != 0) || !CAN_WRITE(conn)) {
					DEBUG_OUT (0,("set_user_quota: access_denied service [%s] user [%s]\n",
						 lp_servicename(SNUM(conn)),
						 conn->session_info->unix_name));
					reply_nterror(req, NT_STATUS_ACCESS_DENIED);
					return;
				...
				if (!check_fsp_ntquota_handle(conn, req,
							      fsp)) {
					DEBUG_OUT (3,("TRANSACT_GET_USER_QUOTA: no valid QUOTA HANDLE\n"));
					reply_nterror(
				...
				if (total_data < 42) {
					DEBUG_OUT (0,("call_trans2setfsinfo: SET_FS_QUOTA: requires total_data(%d) >= 42 bytes!\n",
						total_data));
					reply_nterror(
						req,
						NT_STATUS_INVALID_PARAMETER);
				...
				if (vfs_set_ntquota(fsp, SMB_USER_FS_QUOTA_TYPE, NULL, &quotas)!=0) {
					DEBUG_OUT (0,("vfs_set_ntquota() failed for service [%s]\n",lp_servicename(SNUM(conn))));
		...
		default:
			DEBUG_OUT (3,("call_trans2setfsinfo: unknown level (0x%X) not implemented yet.\n",
				info_level));
			reply_nterror(req, NT_STATUS_INVALID_LEVEL);
			return;
			break;
...
static bool marshall_posix_acl(connection_struct *conn, char *pdata, SMB_STRUCT_STAT *pst, SMB_ACL_T posix_acl)
{
	...
	while ( posix_acl && (SMB_VFS_SYS_ACL_GET_ENTRY(conn, posix_acl, entry_id, &entry) == 1)) {
		...
		if (SMB_VFS_SYS_ACL_GET_TAG_TYPE(conn, entry, &tagtype) == -1) {
			DEBUG_OUT (0,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_TAG_TYPE failed.\n"));
			return False;
		}

		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, entry, &permset) == -1) {
			DEBUG_OUT (0,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_PERMSET failed.\n"));
			return False;
		}
		...
		switch (tagtype) {
			...
			case SMB_ACL_USER:
				{
					...
					if (!puid) {
						DEBUG_OUT (0,("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_QUALIFIER failed.\n"));
						return False;
					}
			...
			case SMB_ACL_OTHER:
				...
				break;
			default:
				DEBUG_OUT (0,("marshall_posix_acl: unknown tagtype.\n"));
				return False;
		}
				...
				const SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG_OUT (10,("store_file_unix_basic: SMB_QUERY_FILE_UNIX_BASIC\n"));
	DEBUG_OUT (4,("store_file_unix_basic: st_mode=%o\n",(int)psbuf->st_ex_mode));
				     ...
				     unsigned int *data_size)
{
	...
	for (i = 0; i < num_streams; i++) {
		...
		if ((ofs + 24 + namelen) > max_data_bytes) {
			DEBUG_OUT (10, ("refusing to overflow reply at stream %u\n",
				i));
			TALLOC_FREE(namebuf);
		...
		if (i == num_streams-1) {
			...
			if ((next_offset + align) > max_data_bytes) {
				DEBUG_OUT (10, ("refusing to overflow align "
					"reply at stream %u\n",
					i));
				TALLOC_FREE(namebuf);
				return STATUS_BUFFER_OVERFLOW;
	...
	DEBUG_OUT (10, ("max_data: %u, data_size: %u\n", max_data_bytes, ofs));
			       ...
			       unsigned int *pdata_size)
{
	...
	DEBUG_OUT (5,("smbd_do_qfilepathinfo: %s (fnum = %d) level=%d max_data=%u\n",
	...
	*ppdata = (char *)SMB_REALLOC(*ppdata, data_size); 
	...
	switch (info_level) {
		case SMB_INFO_STANDARD:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_INFO_STANDARD\n"));
			data_size = 22;
		...
		case SMB_INFO_QUERY_EA_SIZE:
		{
			unsigned int ea_size =
			    estimate_ea_size(conn, fsp,
					     smb_fname->base_name);
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_INFO_QUERY_EA_SIZE\n"));
			data_size = 26;
		...
		case SMB_INFO_IS_NAME_VALID:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_INFO_IS_NAME_VALID\n"));
			if (fsp) {
		...
		case SMB_INFO_QUERY_EAS_FROM_LIST:
		{
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_INFO_QUERY_EAS_FROM_LIST\n"));
		...
		case SMB_INFO_QUERY_ALL_EAS:
		{
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_INFO_QUERY_ALL_EAS\n"));
		...
		case 0xFF0F:/*SMB2_INFO_QUERY_ALL_EAS*/
		{
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB2_INFO_QUERY_ALL_EAS\n"));
		...
		case SMB_QUERY_FILE_BASIC_INFO:
			...
			if (info_level == SMB_QUERY_FILE_BASIC_INFO) {
				DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_QUERY_FILE_BASIC_INFO\n"));
				data_size = 36; /* w95 returns 40 bytes not 36 - why ?. */
			} else {
				DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_BASIC_INFORMATION\n"));
				data_size = 40;
				SIVAL(pdata,36,0);
			}
			...
			DEBUG_OUT (5,("SMB_QFBI - "));
			DEBUG_OUT (5,("create: %s ", ctime(&create_time)));
			DEBUG_OUT (5,("access: %s ", ctime(&atime)));
			DEBUG_OUT (5,("write: %s ", ctime(&mtime)));
			DEBUG_OUT (5,("change: %s ", ctime(&c_time)));
			DEBUG_OUT (5,("mode: %x\n", mode));
			break;
		...
		case SMB_QUERY_FILE_STANDARD_INFO:
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_STANDARD_INFORMATION\n"));
			data_size = 24;
		...
		case SMB_QUERY_FILE_EA_INFO:
		{
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_EA_INFORMATION\n"));
			data_size = 4;
		...
		case SMB_FILE_ALTERNATE_NAME_INFORMATION:
		{
			int len;
			char mangled_name[13];
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ALTERNATE_NAME_INFORMATION\n"));
		...
		case SMB_QUERY_FILE_NAME_INFO:
		{
			...
			len = srvstr_push(dstart, flags2,
					  ...
					  PTR_DIFF(dend, pdata+4),
					  STR_UNICODE);
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_QUERY_FILE_NAME_INFO\n"));
			data_size = 4 + len;
			SIVAL(pdata,0,len);
			break;
		}
		...
		case SMB_QUERY_FILE_ALLOCATION_INFO:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ALLOCATION_INFORMATION\n"));
			data_size = 8;
			SOFF_T(pdata,0,allocation_size);
			break;
		...
		case SMB_QUERY_FILE_END_OF_FILEINFO:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_END_OF_FILE_INFORMATION\n"));
			data_size = 8;
		...
		case SMB_FILE_ALL_INFORMATION:
		{
			...
			unsigned int ea_size =
			    estimate_ea_size(conn, fsp, smb_fname->base_name);
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ALL_INFORMATION\n"));
			put_long_date_timespec(conn->ts_res,pdata,create_time_ts);
			put_long_date_timespec(conn->ts_res,pdata+8,atime_ts);
			put_long_date_timespec(conn->ts_res,pdata+16,mtime_ts); /* write time */
			put_long_date_timespec(conn->ts_res,pdata+24,ctime_ts); /* change time */
		...
		case 0xFF12:/*SMB2_FILE_ALL_INFORMATION*/
		{
			...
			unsigned int ea_size =
			    estimate_ea_size(conn, fsp, smb_fname->base_name);
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB2_FILE_ALL_INFORMATION\n"));
		...
		case SMB_FILE_INTERNAL_INFORMATION:
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_INTERNAL_INFORMATION\n"));
			SBVAL(pdata, 0, file_index);
			data_size = 8;
			break;

		case SMB_FILE_ACCESS_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ACCESS_INFORMATION\n"));
		...
		case SMB_FILE_NAME_INFORMATION:
			/* Pathname with leading '\'. */
			{
				...
				DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_NAME_INFORMATION\n"));
		...
		case SMB_FILE_DISPOSITION_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_DISPOSITION_INFORMATION\n"));
			data_size = 1;
			SCVAL(pdata,0,delete_pending);
			break;

		case SMB_FILE_POSITION_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_POSITION_INFORMATION\n"));
			data_size = 8;
			SOFF_T(pdata,0,pos);
			break;

		case SMB_FILE_MODE_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_MODE_INFORMATION\n"));
			SIVAL(pdata,0,mode);
			data_size = 4;
			break;

		case SMB_FILE_ALIGNMENT_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ALIGNMENT_INFORMATION\n"));
		...
		case SMB_FILE_STREAM_INFORMATION: {
			...
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: "
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (10, ("could not get stream info: %s\n",
					   nt_errstr(status)));
				return status;
			}
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (10, ("marshall_stream_info failed: %s\n",
					   nt_errstr(status)));
				TALLOC_FREE(streams);
				return status;
			}
		...
		case SMB_FILE_COMPRESSION_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_COMPRESSION_INFORMATION\n"));
			SOFF_T(pdata,0,file_size);
			SIVAL(pdata,8,0); /* ??? */
			SIVAL(pdata,12,0); /* ??? */
			data_size = 16;
		...
		case SMB_FILE_NETWORK_OPEN_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_NETWORK_OPEN_INFORMATION\n"));
			put_long_date_timespec(conn->ts_res,pdata,create_time_ts);
			put_long_date_timespec(conn->ts_res,pdata+8,atime_ts);
			put_long_date_timespec(conn->ts_res,pdata+16,mtime_ts); /* write time */
			put_long_date_timespec(conn->ts_res,pdata+24,ctime_ts); /* change time */
		...
		case SMB_FILE_ATTRIBUTE_TAG_INFORMATION:
			DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_FILE_ATTRIBUTE_TAG_INFORMATION\n"));
			SIVAL(pdata,0,mode);
			SIVAL(pdata,4,0);
			data_size = 8;
			break;
		...
		case SMB_QUERY_FILE_UNIX_BASIC:
			...
			DEBUG_OUT (4,("smbd_do_qfilepathinfo: "
		...
		case SMB_QUERY_FILE_UNIX_INFO2:
			...
			{
				int i;
				DEBUG_OUT (4,("smbd_do_qfilepathinfo: SMB_QUERY_FILE_UNIX_INFO2 "));

				for (i=0; i<100; i++)
					DEBUG_OUT (4,("%d=%x, ",i, (*ppdata)[i]));
				DEBUG_OUT (4,("\n"));
			}
		...
		case SMB_QUERY_FILE_UNIX_LINK:
			{
				...
				DEBUG_OUT (10,("smbd_do_qfilepathinfo: SMB_QUERY_FILE_UNIX_LINK\n"));
#ifdef S_ISLNK
		...
		case SMB_QUERY_POSIX_ACL:
			{
				...
				if (file_acl == NULL && no_acl_syscall_error(errno)) {
					DEBUG_OUT (5,("smbd_do_qfilepathinfo: ACLs "
						 "not implemented on "
				...
				if ( data_size < (num_file_acls + num_def_acls)*SMB_POSIX_ACL_ENTRY_SIZE + SMB_POSIX_ACL_HEADER_SIZE) {
					DEBUG_OUT (5,("smbd_do_qfilepathinfo: data_size too small (%u) need %u\n",
						data_size,
				     ...
				     unsigned int max_data_bytes)
{
	...
	if (tran_call == TRANSACT2_QFILEINFO) {
		...
		DEBUG_OUT (3,("call_trans2qfilepathinfo: TRANSACT2_QFILEINFO: level = %d\n", info_level));
		...
		if(fsp->fake_file_handle) {
			...
			if (INFO_LEVEL_IS_UNIX(info_level)) {
				...
				if (SMB_VFS_LSTAT(conn, smb_fname)) {
					DEBUG_OUT (3,("call_trans2qfilepathinfo: "
			...
			if (SMB_VFS_FSTAT(fsp, &smb_fname->st) != 0) {
				DEBUG_OUT (3, ("fstat of fnum %d failed (%s)\n",
		...
		DEBUG_OUT (3,("call_trans2qfilepathinfo: TRANSACT2_QPATHINFO: level = %d\n", info_level));
	...
	DEBUG_OUT (3,("call_trans2qfilepathinfo %s (fnum = %d) level=%d call=%d "
		 "total_data=%d\n", smb_fname_str_dbg(smb_fname),
	...
	switch (info_level) {
		case SMB_INFO_QUERY_EAS_FROM_LIST:
		{
			...
			if (total_data > 0 && ea_size != total_data) {
				DEBUG_OUT (4,("call_trans2qfilepathinfo: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
				reply_nterror(
					req, NT_STATUS_INVALID_PARAMETER);
				return;
		...
		struct smb_filename *smb_fname_new)
{
	...
	DEBUG_OUT (10,("hardlink_internals: doing hard link %s -> %s\n",
	...
	if (SMB_VFS_LINK(conn, smb_fname_old->base_name,
		...
		DEBUG_OUT (3,("hardlink_internals: Error %s hard link %s -> %s\n",
			   ...
			   bool setting_write_time)
{
	...
	DEBUG_OUT (5,("smb_set_filetime: actime: %s\n ",
		time_to_asc(convert_timespec_to_time_t(ft->atime))));
	DEBUG_OUT (5,("smb_set_filetime: modtime: %s\n ",
		time_to_asc(convert_timespec_to_time_t(ft->mtime))));
	DEBUG_OUT (5,("smb_set_filetime: ctime: %s\n ",
		time_to_asc(convert_timespec_to_time_t(ft->ctime))));
	DEBUG_OUT (5,("smb_set_file_time: createtime: %s\n ",
	...
	if (setting_write_time) {
		...
		DEBUG_OUT (10,("smb_set_file_time: setting pending modtime to %s\n",
			  time_to_asc(convert_timespec_to_time_t(ft->mtime))));
	...
	DEBUG_OUT (10,("smb_set_file_time: setting utimes to modified values.\n"));
				     ...
				     uint32 dosmode)
{
	...
	DEBUG_OUT (6,("smb_set_file_dosmode: dosmode: 0x%x\n", (unsigned int)dosmode));

	/* check the mode isn't different, before changing it */
	if ((dosmode != 0) && (dosmode != dos_mode(conn, smb_fname_base))) {
		DEBUG_OUT (10,("smb_set_file_dosmode: file %s : setting dos mode "
			  "0x%x\n", smb_fname_str_dbg(smb_fname_base),
			  (unsigned int)dosmode));

		if(file_set_dosmode(conn, smb_fname_base, dosmode, NULL,
				    false)) {
			DEBUG_OUT (2,("smb_set_file_dosmode: file_set_dosmode of "
				 "%s failed (%s)\n",
				 smb_fname_str_dbg(smb_fname_base),
				 strerror(errno)));
			status = map_nt_error_from_unix(errno);
				  ...
				  bool fail_after_createfile)
{
	...
	DEBUG_OUT (6,("smb_set_file_size: size: %.0f ", (double)size));

	if (size == get_file_size_stat(psbuf)) {
		return NT_STATUS_OK;
	}

	DEBUG_OUT (10,("smb_set_file_size: file %s : setting new size to %.0f\n",
				...
				const struct smb_filename *smb_fname)
{
	...
	if (IVAL(pdata,0) > total_data) {
		DEBUG_OUT (10,("smb_info_set_ea: bad total data size (%u) > %u\n",
			IVAL(pdata,0), (unsigned int)total_data));
		return NT_STATUS_INVALID_PARAMETER;
	}
				...
				files_struct *fsp)
{
	...
	if (!lp_ea_support(SNUM(conn))) {
		DEBUG_OUT (10, ("smb_set_file_full_ea_info - ea_len = %u but "
			"EA's not supported.\n",
	...
	if (total_data < 10) {
		DEBUG_OUT (10, ("smb_set_file_full_ea_info - ea_len = %u "
			"too small.\n",
			(unsigned int)total_data));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG_OUT (10, ("smb_set_file_full_ea_info on file %s returned %s\n",
				...
				struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_file_disposition_info: file %s, dosmode = %u, "
		"delete_on_close = %u\n",
		smb_fname_str_dbg(smb_fname),
		(unsigned int)dosmode,
				...
				files_struct *fsp)
{
	...
	DEBUG_OUT (10,("smb_file_position_information: Set file position "
				       ...
				       const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_file_unix_link: SMB_SET_FILE_UNIX_LINK doing symlink %s -> %s\n",
			newname, link_target ));
					...
					struct smb_filename *smb_fname_new)
{
	...
	DEBUG_OUT (10,("smb_set_file_unix_hlink: SMB_SET_FILE_UNIX_LINK doing hard link %s -> %s\n",
					    ...
					    struct smb_filename *smb_fname_src)
{
	...
	DEBUG_OUT (10,("smb2_file_rename_information: got name |%s|\n",
				newname));
	...
	DEBUG_OUT (10,("smb2_file_rename_information: "
					    ...
					    struct smb_filename *smb_fname_src)
{
	...
	DEBUG_OUT (10,("smb_file_link_information: got name |%s|\n",
				newname));
	...
	DEBUG_OUT (10,("smb_file_link_information: "
					    ...
					    struct smb_filename *smb_fname_src)
{
	...
	DEBUG_OUT (10,("smb_file_rename_information: got name |%s|\n",
				newname));
	...
	if (fsp) {
		DEBUG_OUT (10,("smb_file_rename_information: "
			  "SMB_FILE_RENAME_INFORMATION (fnum %d) %s -> %s\n",
			  fsp->fnum, fsp_str_dbg(fsp),
			  smb_fname_str_dbg(smb_fname_dst)));
				...
				const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_posix_acl: file %s num_file_acls = %u, num_def_acls = %u\n",
				...
				files_struct *fsp)
{
	...
	DEBUG_OUT (10,("smb_set_posix_lock: file %s, lock_type = %u,"
					...
					const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10, ("smb_set_file_basic_info: file %s\n",
		   smb_fname_str_dbg(smb_fname)));
					...
					const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_info_standard: file %s\n",
		smb_fname_str_dbg(smb_fname)));
					...
					struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_file_allocation_info: Set file allocation info for "
		  "file %s to %.0f\n", smb_fname_str_dbg(smb_fname),
	...
	DEBUG_OUT (10,("smb_set_file_allocation_info: file %s : setting new "
...
#endif /* LARGE_SMB_OFF_T */
	DEBUG_OUT (10,("smb_set_file_end_of_file_info: Set end of file info for "
		  "file %s to %.0f\n", smb_fname_str_dbg(smb_fname),
		  (double)size));
					...
					const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_unix_mknod: SMB_SET_FILE_UNIX_BASIC doing mknod dev "
					...
					const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC: name = "
	...
	if (raw_unixmode != SMB_MODE_NO_CHANGE) {
		DEBUG_OUT (10,("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC "
				...
				int *pdata_return_size)
{
	...
	DEBUG_OUT (10,("smb_posix_mkdir: file %s, mode 0%o\n",
				...
				int *pdata_return_size)
{
	...
	switch (wire_open_mode & SMB_ACCMODE) {
		...
		case SMB_O_RDWR:
			...
			break;
		default:
			DEBUG_OUT (5,("smb_posix_open: invalid open mode 0x%x\n",
	...
	switch (wire_open_mode & (SMB_O_CREAT | SMB_O_EXCL)) {
		...
		default:
			DEBUG_OUT (5,("smb_posix_open: invalid create mode 0x%x\n",
				(unsigned int)wire_open_mode ));
			return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG_OUT (10,("smb_posix_open: file %s, smb_posix_flags = %u, mode 0%o\n",
		smb_fname_str_dbg(smb_fname),
		(unsigned int)wire_open_mode,
		(unsigned int)unixmode ));
				...
				struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (10,("smb_posix_unlink: %s %s\n",
	...
	lck = get_share_mode_lock(talloc_tos(), fsp->file_id, NULL, NULL,
				  NULL);
	if (lck == NULL) {
		DEBUG_OUT (0, ("smb_posix_unlink: Could not get share mode "
			  "lock for file %s\n", fsp_str_dbg(fsp)));
		close_file(req, fsp, NORMAL_CLOSE);
		return NT_STATUS_INVALID_PARAMETER;
	}
				...
				int *ret_data_size)
{
	...
	*ret_data_size = 0;
	...
	DEBUG_OUT (3,("smbd_do_setfilepathinfo: %s (fnum %d) info_level=%d "
		 "totdata=%d\n", smb_fname_str_dbg(smb_fname),
				       ...
				       unsigned int max_data_bytes)
{
	...
	if (tran_call == TRANSACT2_SETFILEINFO) {
		...
		if(fsp->fh->fd == -1) {
			...
			if (INFO_LEVEL_IS_UNIX(info_level)) {
				/* Always do lstat for UNIX calls. */
				if (SMB_VFS_LSTAT(conn, smb_fname)) {
					DEBUG_OUT (3,("call_trans2setfilepathinfo: "
						 "SMB_VFS_LSTAT of %s failed "
						 "(%s)\n",
						 smb_fname_str_dbg(smb_fname),
						 strerror(errno)));
			 ...
			 */
			if (SMB_VFS_FSTAT(fsp, &smb_fname->st) != 0) {
				DEBUG_OUT (3,("call_trans2setfilepathinfo: fstat "
					 "of fnum %d failed (%s)\n", fsp->fnum,
					 strerror(errno)));
				reply_nterror(req, map_nt_error_from_unix(errno));
				return;
		...
		if (INFO_LEVEL_IS_UNIX(info_level)) {
			...
			DEBUG_OUT (3,("call_trans2setfilepathinfo: SMB_VFS_STAT of "
				 "%s failed (%s)\n",
				 smb_fname_str_dbg(smb_fname),
				 strerror(errno)));
			reply_nterror(req, map_nt_error_from_unix(errno));
	...
	DEBUG_OUT (3,("call_trans2setfilepathinfo(%d) %s (fnum %d) info_level=%d "
			     ...
			     unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (3,("call_trans2mkdir : name = %s\n", directory));
	...
	if (total_data && (total_data != 4)) {
		...
		if (IVAL(pdata,0) > total_data) {
			DEBUG_OUT (10,("call_trans2mkdir: bad total data size (%u) > %u\n",
				IVAL(pdata,0), (unsigned int)total_data));
			reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
			goto out;
		}
				       ...
				       unsigned int max_data_bytes)
{
	...
	info_level = SVAL(params,4);
	DEBUG_OUT (3,("call_trans2findnotifyfirst - info_level %d\n", info_level));
				      ...
				      unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (3,("call_trans2findnotifynext\n"));
				      ...
				      unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (10,("call_trans2getdfsreferral\n"));
			     ...
			     unsigned int max_data_bytes)
{
	...
	DEBUG_OUT (2,("Unknown TRANS2_IOCTL\n"));
	reply_nterror(req, NT_STATUS_NOT_IMPLEMENTED);
}
...
void reply_findclose(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_findclose, dptr_num = %d\n", dptr_num));

	dptr_close(sconn, &dptr_num);

	reply_outbuf(req, 0, 0);

	DEBUG_OUT (3,("SMBfindclose dptr_num = %d\n", dptr_num));
...
void reply_findnclose(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_findnclose, dptr_num = %d\n", dptr_num));
	...
	DEBUG_OUT (3,("SMB_findnclose dptr_num = %d\n", dptr_num));
			  ...
			  struct trans_state *state)
{
	...
	if (conn->encrypt_level == Required && !req->encrypted) {
		if (state->call != TRANSACT2_QFSINFO &&
			...
			DEBUG_OUT (0,("handle_trans2: encryption required "
				"with call 0x%x\n",
	...
	default:
		/* Error in request */
		DEBUG_OUT (2,("Unknown request %d in trans2 call\n", state->call));
		reply_nterror(req, NT_STATUS_NOT_IMPLEMENTED);
	}
}
...
void reply_trans2(struct smb_request *req)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (2, ("Got invalid trans2 request: %s\n",
			  nt_errstr(result)));
	...
	if (state->setup_count != 1) {
		...
		if ( (state->setup_count == 4)
		     ...
		     &&	(SVAL(req->vwv+17, 0) == LMFUNC_GETJOBID)) {
			DEBUG_OUT (2,("Got Trans2 DevIOctl jobid\n"));
		} else {
			DEBUG_OUT (2,("Invalid smb_sucnt in trans2 call(%u)\n",state->setup_count));
			DEBUG_OUT (2,("Transaction is %d\n",tran_call));
			TALLOC_FREE(state);
			reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
			END_PROFILE(SMBtrans2);
			return;
	...
	if (state->total_data) {
		...
		if (state->data == NULL) {
			DEBUG_OUT (0,("reply_trans2: data malloc fail for %u "
	...
	if (state->total_param) {
		...
		if (state->param == NULL) {
			DEBUG_OUT (0,("reply_trans: param malloc fail for %u "
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_trans2: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBtrans2);
...
void reply_transs2(struct smb_request *req)
{
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_transs2: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
