...
static struct vfs_init_function_entry *vfs_find_backend_entry(const char *name)
{
	...
	DEBUG_OUT (10, ("vfs_find_backend_entry called for %s\n", name));
			  ...
			  const struct vfs_fn_pointers *fns)
{
	struct vfs_init_function_entry *entry = backends;
 	...
 	if ((version != SMB_VFS_INTERFACE_VERSION)) {
		DEBUG_OUT (0, ("Failed to register vfs module.\n"
	...
	if (!name || !name[0]) {
		DEBUG_OUT (0,("smb_register_vfs() called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (vfs_find_backend_entry(name)) {
		DEBUG_OUT (0,("VFS module %s already loaded!\n", name));
	...
	DEBUG_OUT (5, ("Successfully added vfs backend '%s'\n", name));
	return NT_STATUS_OK;
}
...
{
	DEBUG_OUT (3, ("Initialising default vfs hooks\n"));
...
bool vfs_init_custom(connection_struct *conn, const char *vfs_object)
{
	...
	if (!conn||!vfs_object||!vfs_object[0]) {
		DEBUG_OUT (0, ("vfs_init_custom() called with NULL pointer or "
			  "empty vfs_object!\n"));
		return False;
	}
	...
	DEBUG_OUT (3, ("Initialising custom vfs hooks from [%s]\n", vfs_object));
	...
	if (!entry) {
		...
		DEBUG_OUT (5, ("vfs module [%s] not loaded - trying to load...\n",
			  vfs_object));

		status = smb_probe_module("vfs", module_path);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("error probing vfs module '%s': %s\n",
		...
		entry = vfs_find_backend_entry(module_name);
		if (!entry) {
			DEBUG_OUT (0,("Can't find a vfs module [%s]\n",vfs_object));
			goto fail;
		}
	}
...
bool smbd_vfs_init(connection_struct *conn)
{
	...
	for (j=i-1; j >= 0; j--) {
		if (!vfs_init_custom(conn, vfs_objects[j])) {
			DEBUG_OUT (0, ("smbd_vfs_init: vfs_init_custom failed for %s\n", vfs_objects[j]));
			return False;
		}
	}
	return True;
...
int vfs_allocate_file_space(files_struct *fsp, uint64_t len)
{
	...
	DEBUG_OUT (10,("vfs_allocate_file_space: file %s, len %.0f\n",
		  fsp_str_dbg(fsp), (double)len));

	if (((SMB_OFF_T)len) < 0) {
		DEBUG_OUT (0,("vfs_allocate_file_space: %s negative len "
			 "requested.\n", fsp_str_dbg(fsp)));
		errno = EINVAL;
		return -1;
	}
	...
	if (len < (uint64_t)fsp->fsp_name->st.st_ex_size) {
		...
		DEBUG_OUT (10,("vfs_allocate_file_space: file %s, shrink. Current "
			  "size %.0f\n", fsp_str_dbg(fsp),
	...
	DEBUG_OUT (10,("vfs_allocate_file_space: file %s, grow. Current size %.0f, "
...
int vfs_set_filelen(files_struct *fsp, SMB_OFF_T len)
{
	...
	DEBUG_OUT (10,("vfs_set_filelen: ftruncate %s to len %.0f\n",
		  fsp_str_dbg(fsp), (double)len));
	flush_write_cache(fsp, SIZECHANGE_FLUSH);
...
int vfs_slow_fallocate(files_struct *fsp, SMB_OFF_T offset, SMB_OFF_T len)
{
	...
	while (total < len) {
		...
		if (pwrite_ret == -1) {
			DEBUG_OUT (10,("vfs_slow_fallocate: SMB_VFS_PWRITE for file "
...
int vfs_fill_sparse(files_struct *fsp, SMB_OFF_T len)
{
	...
	DEBUG_OUT (10,("vfs_fill_sparse: write zeros in file %s from len %.0f to "
		  "len %.0f (%.0f bytes)\n", fsp_str_dbg(fsp),
	...
	if (fsp->base_fsp == NULL) {
		...
		DEBUG_OUT (10,("vfs_fill_sparse: SMB_VFS_FALLOCATE failed with "
...
char *vfs_GetWd(TALLOC_CTX *ctx, connection_struct *conn)
{
	...
	*s = 0;
	...
	if (SMB_VFS_STAT(conn, smb_fname_dot) == -1) {
		 ...
		 */
		DEBUG_OUT (1,("vfs_GetWd: couldn't stat \".\" error %s "
			 "(NFS problem ?)\n", strerror(errno) ));
		goto nocache;
	}
 ...
 nocache:
	...
	if (!SMB_VFS_GETWD(conn,s)) {
		DEBUG_OUT (0, ("vfs_GetWd: SMB_VFS_GETWD call failed: %s\n",
			  strerror(errno)));
		goto out;
	}
...
NTSTATUS check_reduced_name(connection_struct *conn, const char *fname)
{
	...
	DEBUG_OUT (3,("check_reduced_name [%s] [%s]\n", fname, conn->connectpath));
	...
	if (!resolved_name) {
		switch (errno) {
			case ENOTDIR:
				DEBUG_OUT (3,("check_reduced_name: Component not a "
			...
			default:
				DEBUG_OUT (3,("check_reduced_name: couldn't get "
					 "realpath for %s\n", fname));
				return map_nt_error_from_unix(errno);
		}
	}

	DEBUG_OUT (10,("check_reduced_name realpath [%s] -> [%s]\n", fname,
		  resolved_name));

	if (*resolved_name != '/') {
		DEBUG_OUT (0,("check_reduced_name: realpath doesn't return "
			 "absolute paths !\n"));
		SAFE_FREE(resolved_name);
	...
	if (!allow_widelinks || !allow_symlinks) {
		...
		if (conn_rootdir == NULL) {
			DEBUG_OUT (2, ("check_reduced_name: Could not get "
				"conn_rootdir\n"));
			SAFE_FREE(resolved_name);
		...
		if (strncmp(conn_rootdir, resolved_name,
				rootdir_len) != 0) {
			DEBUG_OUT (2, ("check_reduced_name: Bad access "
		...
		if (!allow_symlinks) {
			...
			if (*p != '/') {
				DEBUG_OUT (2, ("check_reduced_name: logic error (%c) "
					"in resolved_name: %s\n",
					*p,
					fname));
				SAFE_FREE(resolved_name);
  ...
  out:
	...
	DEBUG_OUT (3,("check_reduced_name: %s reduced to %s\n", fname,
		 resolved_name));
	SAFE_FREE(resolved_name);
	return NT_STATUS_OK;
}
...
NTSTATUS vfs_chown_fsp(files_struct *fsp, uid_t uid, gid_t gid)
{
	...
	if (as_root) {
		...
		if (!saved_dir) {
			...
			DEBUG_OUT (0,("vfs_chown_fsp: failed to get "
