...
static int dmapi_init_session(struct smbd_dmapi_context *ctx)
{
	...
	if (session_name == NULL) {
		DEBUG_OUT (0,("Out of memory in dmapi_init_session\n"));
		talloc_free(tmp_ctx);
		return -1;
	}
	...
	if (dm_init_service(&version) < 0) {
		DEBUG_OUT (0, ("dm_init_service failed - disabling DMAPI\n"));
		talloc_free(tmp_ctx);
		return -1;
	}
	...
	if (ctx->session == DM_NO_SESSION) {
		...
		DEBUG_OUT (0, ("created new DMAPI session named '%s' for %s\n",
...
bool dmapi_destroy_session(void)
{
	...
	if (dmapi_ctx->session != DM_NO_SESSION) {
		...
		if (0 == dm_destroy_session(dmapi_ctx->session)) {
			...
			DEBUG_OUT (0,("Couldn't destroy DMAPI session: %s\n",
				 strerror(errno)));
		}
		unbecome_root();
	}
...
uint32 dmapi_file_flags(const char * const path)
{
	...
	if (err < 0) {
		DEBUG_OUT (DMAPI_TRACE, ("dm_path_to_handle(%s): %s\n",
			    path, strerror(errno)));
	...
	if (err < 0) {
		DEBUG_OUT (DMAPI_TRACE, ("dm_get_eventlist(%s): %s\n",
			    path, strerror(errno)));
	 ...
	 * offline.
	 */
	DEBUG_OUT (DMAPI_TRACE, ("DMAPI event list for %s\n", path));
	...
	if (flags & FILE_ATTRIBUTE_OFFLINE) {
		DEBUG_OUT (DMAPI_TRACE, ("%s is OFFLINE\n", path));
	}
