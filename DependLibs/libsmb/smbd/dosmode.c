...
static uint32_t filter_mode_by_protocol(uint32_t mode)
{
	if (get_Protocol() <= PROTOCOL_LANMAN2) {
		DEBUG_OUT (10,("filter_mode_by_protocol: "
		 ...
		 const char *inherit_from_dir)
{
	...
	if ((inherit_from_dir != NULL) && lp_inherit_perms(SNUM(conn))) {
		...
		DEBUG_OUT (2, ("unix_mode(%s) inheriting from %s\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1,("unix_mode(%s) failed, [dir %s]: %s\n",
				 smb_fname_str_dbg(smb_fname),
		...
		if (SMB_VFS_STAT(conn, smb_fname_parent) != 0) {
			DEBUG_OUT (4,("unix_mode(%s) failed, [dir %s]: %s\n",
				 smb_fname_str_dbg(smb_fname),
		...
		DEBUG_OUT (2,("unix_mode(%s) inherit mode %o\n",
	...
	DEBUG_OUT (3,("unix_mode(%s) returning 0%o\n", smb_fname_str_dbg(smb_fname),
		 (int)result));
	return(result);
}
				 ...
				 const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (8,("dos_mode_from_sbuf returning "));

	if (result & FILE_ATTRIBUTE_HIDDEN) DEBUG_OUT (8, ("h"));
	if (result & FILE_ATTRIBUTE_READONLY ) DEBUG_OUT (8, ("r"));
	if (result & FILE_ATTRIBUTE_SYSTEM) DEBUG_OUT (8, ("s"));
	if (result & FILE_ATTRIBUTE_DIRECTORY   ) DEBUG_OUT (8, ("d"));
	if (result & FILE_ATTRIBUTE_ARCHIVE  ) DEBUG_OUT (8, ("a"));
				 ...
				 uint32 *pattr)
{
...
#endif
			DEBUG_OUT (1,("get_ea_dos_attribute: Cannot get attribute "
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (1,("get_ea_dos_attribute: bad ndr decode "
			 "from EA on file %s: Error = %s\n",
			 smb_fname_str_dbg(smb_fname),
			 ndr_errstr(ndr_err)));
		return false;
	...
	DEBUG_OUT (10,("get_ea_dos_attribute: %s attr = %s\n",
	...
	switch (dosattrib.version) {
		...
		case 1:
			...
			if (!null_nttime(dosattrib.info.info1.create_time)) {
				...
				DEBUG_OUT (10,("get_ea_dos_attribute: file %s case 1 "
					"set btime %s\n",
					smb_fname_str_dbg(smb_fname),
		...
		case 3:
			...
			if ((dosattrib.info.info3.valid_flags & XATTR_DOSINFO_CREATE_TIME) &&
				...
				DEBUG_OUT (10,("get_ea_dos_attribute: file %s case 3 "
					"set btime %s\n",
					smb_fname_str_dbg(smb_fname),
					time_to_asc(convert_timespec_to_time_t(
						create_time)) ));
			...
			break;
		default:
			DEBUG_OUT (1,("get_ea_dos_attribute: Badly formed DOSATTRIB on "
				 "file %s - %s\n", smb_fname_str_dbg(smb_fname),
				 attrstr));
	                return false;
	}
	...
	*pattr = (uint32)(dosattr & (SAMBA_ATTRIBUTES_MASK|FILE_ATTRIBUTE_SPARSE));
	...
	DEBUG_OUT (8,("get_ea_dos_attribute returning (0x%x)", dosattr));

	if (dosattr & FILE_ATTRIBUTE_HIDDEN) DEBUG_OUT (8, ("h"));
	if (dosattr & FILE_ATTRIBUTE_READONLY ) DEBUG_OUT (8, ("r"));
	if (dosattr & FILE_ATTRIBUTE_SYSTEM) DEBUG_OUT (8, ("s"));
	if (dosattr & FILE_ATTRIBUTE_DIRECTORY   ) DEBUG_OUT (8, ("d"));
	if (dosattr & FILE_ATTRIBUTE_ARCHIVE  ) DEBUG_OUT (8, ("a"));
				 ...
				 uint32 dosmode)
{
	...
	DEBUG_OUT (10,("set_ea_dos_attributes: set attribute 0x%x, btime = %s on file %s\n",
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (5, ("create_acl_blob: ndr_push_xattr_DOSATTRIB failed: %s\n",
			ndr_errstr(ndr_err)));
		return false;
	}
...
#endif
				DEBUG_OUT (1,("set_ea_dos_attributes: Cannot set "
	...
	DEBUG_OUT (10,("set_ea_dos_attribute: set EA 0x%x on file %s\n",
		(unsigned int)dosmode,
		smb_fname_str_dbg(smb_fname)));
	return true;
}
		      ...
		      const struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (8,("dos_mode_msdfs returning "));
	...
	if (result & FILE_ATTRIBUTE_DIRECTORY   ) DEBUG_OUT (8, ("d"));
	if (result & FILE_ATTRIBUTE_ARCHIVE  ) DEBUG_OUT (8, ("a"));
	if (result & FILE_ATTRIBUTE_SPARSE ) DEBUG_OUT (8, ("[sparse]"));
			       ...
			       uint32_t *dosmode)
{
	...
	DEBUG_OUT (5, ("Getting stat dos attributes for %s.\n",
		  smb_fname_str_dbg(smb_fname)));
			       ...
			       bool *attributes_changed)
{
	...
	*attributes_changed = false;
	...
	DEBUG_OUT (5, ("Setting stat dos attributes for %s.\n",
		  smb_fname_str_dbg(smb_fname)));
	...
	DEBUG_OUT (5, ("Setting stat dos attributes=0x%x, prev=0x%x\n", new_flags,
		  smb_fname->st.st_ex_flags));

	/* Set new flags with chflags. */
	error = SMB_VFS_CHFLAGS(conn, smb_fname->base_name, new_flags);
	if (error) {
		DEBUG_OUT (0, ("Failed setting new stat dos attributes (0x%x) on "
			  "file %s! errno=%d\n", new_flags,
...
uint32 dos_mode(connection_struct *conn, struct smb_filename *smb_fname)
{
	...
	DEBUG_OUT (8,("dos_mode returning "));
		     ...
		     uint32 dosmode, const char *parent_dir, bool newfile)
{
	...
	DEBUG_OUT (10,("file_set_dosmode: setting dos mode 0x%x on file %s\n",
	...
	if (dosmode & FILE_ATTRIBUTE_OFFLINE) {
		if (!(old_mode & FILE_ATTRIBUTE_OFFLINE)) {
			...
			if (lret == -1) {
				DEBUG_OUT (0, ("set_dos_mode: client has asked to "
	...
	if (S_ISDIR(smb_fname->st.st_ex_mode) && (unixmode & S_ISGID) &&
		...
		DEBUG_OUT (3,("file_set_dosmode: setgid bit cannot be "
			"set for directory %s\n",
			smb_fname_str_dbg(smb_fname)));
		errno = EPERM;
		return -1;
			 ...
			 bool sparse)
{
	...
	if (!CAN_WRITE(conn)) {
		DEBUG_OUT (9,("file_set_sparse: fname[%s] set[%u] "
			"on readonly share[%s]\n",
	...
	DEBUG_OUT (10,("file_set_sparse: setting sparse bit %u on file %s\n",
		...
		struct smb_file_time *ft)
{
	...
	DEBUG_OUT (6, ("file_ntime: actime: %s",
		  time_to_asc(convert_timespec_to_time_t(ft->atime))));
	DEBUG_OUT (6, ("file_ntime: modtime: %s",
		  time_to_asc(convert_timespec_to_time_t(ft->mtime))));
	DEBUG_OUT (6, ("file_ntime: ctime: %s",
		  time_to_asc(convert_timespec_to_time_t(ft->ctime))));
	DEBUG_OUT (6, ("file_ntime: createtime: %s",
				...
				struct timespec create_time)
{
	...
	DEBUG_OUT (10,("set_create_timespec_ea: wrote create time EA for file %s\n",
		smb_fname_str_dbg(smb_fname)));
