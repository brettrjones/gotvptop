...
static struct fake_file_handle *init_fake_file_handle(enum FAKE_FILE_TYPE type)
{
	...
	DEBUG_OUT (5,("init_fake_file_handle: for [%s]\n",fake_files[i].name));
...
enum FAKE_FILE_TYPE is_fake_file_path(const char *path)
{
	...
	for (i=0;fake_files[i].name!=NULL;i++) {
		if (strncmp(path,fake_files[i].name,strlen(fake_files[i].name))==0) {
			DEBUG_OUT (5,("is_fake_file: [%s] is a fake file\n",path));
				...
				files_struct **result)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("open_fake_file: smbd_calculate_access_mask "
	...
	if (geteuid() != sec_initial_uid()) {
		DEBUG_OUT (3, ("open_fake_file_shared: access_denied to "
	...
	DEBUG_OUT (5,("open_fake_file_shared: fname = %s, FID = %d, access_mask = 0x%x\n",
