...
NTSTATUS smbd_smb2_request_process_write(struct smbd_smb2_request *req)
{
	...
	if (in_data_length > req->sconn->smb2.max_write) {
		DEBUG_OUT (2,("smbd_smb2_request_process_write : "
...
NTSTATUS smb2_write_complete(struct tevent_req *req, ssize_t nwritten, int err)
{
	...
	if ((nwritten == 0) && (state->in_length != 0)) {
		DEBUG_OUT (5,("smb2: write [%s] disk full\n",
			fsp_str_dbg(fsp)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("smb2: sync_file for %s returned %s\n",
			fsp_str_dbg(fsp),
			nt_errstr(status)));
		return status;
	}
					       ...
					       uint32_t in_flags)
{
	...
	DEBUG_OUT (10,("smbd_smb2_write: %s - fnum[%d]\n",
