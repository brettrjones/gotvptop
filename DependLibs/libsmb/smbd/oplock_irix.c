...
static bool irix_oplocks_available(void)
{
	...
	if(pipe(pfd) != 0) {
		DEBUG_OUT (0,("check_kernel_oplocks: Unable to create pipe. Error "
			 "was %s\n",
			 strerror(errno) ));
		TALLOC_FREE(ctx);
		return False;
	...
	if((fd = sys_open(tmpname, O_RDWR|O_CREAT|O_EXCL|O_TRUNC, 0600)) < 0) {
		DEBUG_OUT (0,("check_kernel_oplocks: Unable to open temp test file "
			 "%s. Error was %s\n",
	...
	if(sys_fcntl_long(fd, F_OPLKREG, pfd[1]) == -1) {
		DEBUG_OUT (0,("check_kernel_oplocks: Kernel oplocks are not "
	...
	if(sys_fcntl_long(fd, F_OPLKACK, OP_REVOKE) < 0 ) {
		DEBUG_OUT (0,("check_kernel_oplocks: Error when removing kernel "
...
static files_struct *irix_oplock_receive_message(struct kernel_oplocks *_ctx)
{
	...
	if(read(ctx->read_fd, &dummy, 1) != 1) {
		DEBUG_OUT (0,("irix_oplock_receive_message: read of kernel "
	...
	if(sys_fcntl_ptr(ctx->read_fd, F_OPLKSTAT, &os) < 0) {
		DEBUG_OUT (0,("irix_oplock_receive_message: fcntl of kernel "
			 "notification failed. Error was %s.\n",
			 strerror(errno) ));
		if(errno == EAGAIN) {
			/*
	...
	if ((fsp = file_find_di_first(smbd_server_conn, fileid)) == NULL) {
		DEBUG_OUT (0,("irix_oplock_receive_message: unable to find open "
			 "file with dev = %x, inode = %.0f\n",
			 (unsigned int)os.os_dev, (double)os.os_ino ));
		return NULL;
	}
     
	DEBUG_OUT (5,("irix_oplock_receive_message: kernel oplock break request "
				   ...
				   files_struct *fsp, int oplock_type)
{
	...
	if (sys_fcntl_long(fsp->fh->fd, F_OPLKREG, ctx->write_fd) == -1) {
		if(errno != EAGAIN) {
			DEBUG_OUT (0,("irix_set_kernel_oplock: Unable to get "
			...
			DEBUG_OUT (5,("irix_set_kernel_oplock: Refused oplock on "
	...
	DEBUG_OUT (10,("irix_set_kernel_oplock: got kernel oplock on file %s, file_id = %s "
		  "gen_id = %ul\n",
...
struct kernel_oplocks *irix_init_kernel_oplocks(TALLOC_CTX *mem_ctx)
{
	...
	if(pipe(pfd) != 0) {
		talloc_free(_ctx);
		DEBUG_OUT (0,("setup_kernel_oplock_pipe: Unable to create pipe. "
			 "Error was %s\n", strerror(errno) ));
		return False;
	}
