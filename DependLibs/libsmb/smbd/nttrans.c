		     ...
		     char *pdata, int datasize)
{
	...
	while (params_to_send || data_to_send) {
		...
		DEBUG_OUT (9,("nt_rep: params_sent_thistime = %d, data_sent_thistime = %d, useable_space = %d\n",
			params_sent_thistime, data_sent_thistime, useable_space));
		DEBUG_OUT (9,("nt_rep: params_to_send = %d, data_to_send = %d, paramsize = %d, datasize = %d\n",
		...
		if(params_to_send < 0 || data_to_send < 0) {
			DEBUG_OUT (0,("send_nt_replies failed sanity check pts = %d, dts = %d\n!!!",
			 ...
			 struct smb_request *req, int *ppnum)
{
	...
	DEBUG_OUT (4,("nt_open_pipe: Opening pipe %s.\n", fname));
				  ...
				  struct smb_request *req)
{
	...
	DEBUG_OUT (5,("do_ntcreate_pipe_open: open pipe = %s\n", fname));
...
void reply_ntcreate_and_X(struct smb_request *req)
{
	...
	DEBUG_OUT (10,("reply_ntcreate_and_X: flags = 0x%x, access_mask = 0x%x "
	...
	DEBUG_OUT (5,("reply_ntcreate_and_X: fnum = %d, open name = %s\n",
				       ...
				       char **ppdata, uint32 data_count)
{
	...
	if(parameter_count < 54) {
		DEBUG_OUT (0,("do_nt_transact_create_pipe - insufficient parameters (%u)\n", (unsigned int)parameter_count));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	DEBUG_OUT (5,("do_nt_transact_create_pipe: open name = %s\n", fname));
		       ...
		       uint32_t security_info_sent)
{
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("set_sd for file %s\n", fsp_str_dbg(fsp)));
				    ...
				    uint32 max_data_count)
{
	...
	DEBUG_OUT (5,("call_nt_transact_create\n"));
	...
	if(parameter_count < 54) {
		DEBUG_OUT (0,("call_nt_transact_create - insufficient parameters (%u)\n", (unsigned int)parameter_count));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		goto out;
	}
	...
	if ((ea_len + sd_len > data_count)
		...
		DEBUG_OUT (10, ("call_nt_transact_create - ea_len = %u, sd_len = "
			   "%u, data_count = %u\n", (unsigned int)ea_len,
	...
	if (sd_len) {
		DEBUG_OUT (10, ("call_nt_transact_create - sd_len = %d\n",
			   sd_len));

		status = unmarshall_sec_desc(ctx, (uint8_t *)data, sd_len,
					     &sd);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("call_nt_transact_create: "
	...
	if (ea_len) {
		if (!lp_ea_support(SNUM(conn))) {
			DEBUG_OUT (10, ("call_nt_transact_create - ea_len = %u but "
				   "EA's not supported.\n",
				   (unsigned int)ea_len));
		...
		if (ea_len < 10) {
			DEBUG_OUT (10,("call_nt_transact_create - ea_len = %u - "
	...
	DEBUG_OUT (5,("call_nt_transact_create: open name = %s\n",
...
void reply_ntcancel(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_ntcancel: cancel called on mid = %llu.\n",
				...
				uint32 attrs)
{
	...
	DEBUG_OUT (10,("copy_internals: doing file copy %s to %s\n",
 ...
 out:
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3,("copy_internals: Error %s copy file %s to %s\n",
...
void reply_ntrename(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_ntrename: %s -> %s\n",
					   ...
					   uint32 max_param_count)
{
	...
	DEBUG_OUT (3,("call_nt_transact_notify_change\n"));
...

	{
		...
		DEBUG_OUT (3,("call_nt_transact_notify_change: notify change "
	...
	if (fsp->notify == NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("change_notify_create returned %s\n",
				   nt_errstr(status)));
			reply_nterror(req, status);
			return;
		}
				    ...
				    uint32 max_data_count)
{
	...
	DEBUG_OUT (3,("nt transact rename from = %s, to = %s ignored!\n",
...
static NTSTATUS get_null_nt_acl(TALLOC_CTX *mem_ctx, struct security_descriptor **ppsd)
{
	...
	*ppsd = make_standard_sec_desc( mem_ctx, &global_sid_World, &global_sid_World, NULL, &sd_size);
	if(!*ppsd) {
		DEBUG_OUT (0,("get_null_nt_acl: Unable to malloc space for security descriptor.\n"));
					...
					size_t *psd_size)
{
	...
	*psd_size = ndr_size_security_descriptor(psd, 0);
	...
	DEBUG_OUT (3,("smbd_do_query_security_desc: sd_size = %lu.\n",
		(unsigned long)*psd_size));

	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("smbd_do_query_security_desc for file %s\n",
			  fsp_str_dbg(fsp)));
		NDR_PRINT_DEBUG(security_descriptor, psd);
	}
						 ...
						 uint32 max_data_count)
{
	...
	DEBUG_OUT (3,("call_nt_transact_query_security_desc: file = %s, "
		 "info_wanted = 0x%x\n", fsp_str_dbg(fsp),
					       ...
					       uint32 max_data_count)
{
	...
	DEBUG_OUT (3,("call_nt_transact_set_security_desc: file = %s, sent 0x%x\n",
		       ...
		       uint32_t *out_len)
{
	...
	case FSCTL_SET_SPARSE:
	{
		...
		DEBUG_OUT (NT_STATUS_IS_OK(status) ? 10 : 9,
	...
	case FSCTL_CREATE_OR_GET_OBJECT_ID:
	{
		...
		DEBUG_OUT (10,("FSCTL_CREATE_OR_GET_OBJECT_ID: called on FID[0x%04X]\n",fsp->fnum));
	...
	case FSCTL_GET_REPARSE_POINT:
	{
		...
		DEBUG_OUT (10, ("FSCTL_GET_REPARSE_POINT: called on FID[0x%04X] Status: NOT_IMPLEMENTED\n", fsp->fnum));
	...
	{
		/* Fail it with STATUS_NOT_A_REPARSE_POINT */
		DEBUG_OUT (10, ("FSCTL_SET_REPARSE_POINT: called on FID[0x%04X] Status: NOT_IMPLEMENTED\n", fsp->fnum));
		return NT_STATUS_NOT_A_REPARSE_POINT;
	}
	...
	case FSCTL_GET_SHADOW_COPY_DATA:
	{
		...
		if (max_out_len < 16) {
			DEBUG_OUT (0,("FSCTL_GET_SHADOW_COPY_DATA: max_data_count(%u) < 16 is invalid!\n",
				max_out_len));
			return NT_STATUS_INVALID_PARAMETER;
		}
		...
		if (shadow_data == NULL) {
			DEBUG_OUT (0,("TALLOC_ZERO() failed!\n"));
			return NT_STATUS_NO_MEMORY;
		}
		...
		if (SMB_VFS_GET_SHADOW_COPY_DATA(fsp, shadow_data, labels)!=0) {
			...
			if (errno == ENOSYS) {
				DEBUG_OUT (5,("FSCTL_GET_SHADOW_COPY_DATA: connectpath %s, not supported.\n", 
					fsp->conn->connectpath));
				return NT_STATUS_NOT_SUPPORTED;
			} else {
				DEBUG_OUT (0,("FSCTL_GET_SHADOW_COPY_DATA: connectpath %s, failed.\n", 
					fsp->conn->connectpath));
		...
		if (max_out_len < *out_len) {
			DEBUG_OUT (0,("FSCTL_GET_SHADOW_COPY_DATA: max_data_count(%u) too small (%u) bytes needed!\n",
				max_out_len, *out_len));
			TALLOC_FREE(shadow_data);
			return NT_STATUS_BUFFER_TOO_SMALL;
		}
		...
		*out_data = cur_pdata;
		...
		DEBUG_OUT (10,("FSCTL_GET_SHADOW_COPY_DATA: %u volumes for path[%s].\n",
	...
	case FSCTL_FIND_FILES_BY_SID:
	{
		...
		DEBUG_OUT (10,("FSCTL_FIND_FILES_BY_SID: called on FID[0x%04X]\n", fsp->fnum));
		...
		if (!sid_to_uid(&sid, &uid)) {
			DEBUG_OUT (0,("sid_to_uid: failed, sid[%s] sid_len[%lu]\n",
				 sid_string_dbg(&sid),
	...
	case FSCTL_QUERY_ALLOCATED_RANGES:
	{
		...
		if (in_len != 16) {
			DEBUG_OUT (0,("FSCTL_QUERY_ALLOCATED_RANGES: data_count(%u) != 16 is invalid!\n",
				in_len));
			return NT_STATUS_INVALID_PARAMETER;
		}

		if (max_out_len < 16) {
			DEBUG_OUT (0,("FSCTL_QUERY_ALLOCATED_RANGES: max_out_len (%u) < 16 is invalid!\n",
				max_out_len));
			return NT_STATUS_INVALID_PARAMETER;
		}
		...
		*out_len = 16;
		...
		if (out_data_tmp == NULL) {
			DEBUG_OUT (10, ("unable to allocate memory for response\n"));
			return NT_STATUS_NO_MEMORY;
		}
	...
	{
		DEBUG_OUT (10,("FSCTL_IS_VOLUME_DIRTY: called on FID[0x%04X] "
			  "(but not implemented)\n", fsp->fnum));
		/*
	...
	default:
		...
		if (!vfswrap_logged_ioctl_message) {
			...
			DEBUG_OUT (2, ("%s (0x%x): Currently not implemented.\n",
				   ...
				   uint32 max_data_count)
{
	...
	if (setup_count != 8) {
		DEBUG_OUT (3,("call_nt_transact_ioctl: invalid setup count %d\n", setup_count));
	...
	DEBUG_OUT (10, ("call_nt_transact_ioctl: function[0x%08X] FID[0x%04X] isFSctl[0x%02X] compfilter[0x%02X]\n",
	 ...
	 */
	if (!isFSctl) {
		DEBUG_OUT (10, ("isFSctl: 0x%02X indicates IOCTL, not FSCTL!\n",
			isFSctl));
		reply_nterror(req, NT_STATUS_NOT_SUPPORTED);
		return;
	}
					    ...
					    uint32 max_data_count)
{
	...
	if (get_current_uid(conn) != 0) {
		DEBUG_OUT (1,("get_user_quota: access_denied service [%s] user "
	...
	if (parameter_count < 4) {
		DEBUG_OUT (0,("TRANSACT_GET_USER_QUOTA: requires %d >= 4 bytes parameters\n",parameter_count));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	switch (level) {
		...
		case TRANSACT_GET_USER_QUOTA_FOR_SID:
			...
			if (data_count < 8) {
				DEBUG_OUT (0,("TRANSACT_GET_USER_QUOTA_FOR_SID: requires %d >= %d bytes data\n",data_count,8));
			...
			if (data_count < 8+sid_len) {
				DEBUG_OUT (0,("TRANSACT_GET_USER_QUOTA_FOR_SID: requires %d >= %lu bytes data\n",data_count,(unsigned long)(8+sid_len)));
				reply_nterror(req, NT_STATUS_INVALID_LEVEL);
				return;
			}
			...
			if (max_data_count < data_len) {
				DEBUG_OUT (0,("TRANSACT_GET_USER_QUOTA_FOR_SID: max_data_count(%d) < data_len(%d)\n",
		...
		default:
			DEBUG_OUT (0,("do_nt_transact_get_user_quota: fnum %d unknown level 0x%04hX\n",fsp->fnum,level));
			reply_nterror(req, NT_STATUS_INVALID_LEVEL);
			return;
			break;
	}
					    ...
					    uint32 max_data_count)
{
	...
	/* access check */
	if (get_current_uid(conn) != 0) {
		DEBUG_OUT (1,("set_user_quota: access_denied service [%s] user "
			 "[%s]\n", lp_servicename(SNUM(conn)),
			 conn->session_info->unix_name));
		reply_nterror(req, NT_STATUS_ACCESS_DENIED);
		return;
	...
	if (parameter_count < 2) {
		DEBUG_OUT (0,("TRANSACT_SET_USER_QUOTA: requires %d >= 2 bytes parameters\n",parameter_count));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	if (data_count < 40) {
		DEBUG_OUT (0,("TRANSACT_SET_USER_QUOTA: requires %d >= %d bytes data\n",data_count,40));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
	...
	if (data_count < 40+sid_len || (40+sid_len < sid_len)) {
		DEBUG_OUT (0,("TRANSACT_SET_USER_QUOTA: requires %d >= %lu bytes data\n",data_count,(unsigned long)40+sid_len));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
			   ...
			   struct smb_request *req)
{
	...
	switch(state->call) {
		...
		default:
			/* Error in request */
			DEBUG_OUT (0,("handle_nttrans: Unknown request %d in "
				 "nttrans call\n", state->call));
			reply_nterror(req, NT_STATUS_INVALID_LEVEL);
			return;
	}
...
void reply_nttrans(struct smb_request *req)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (2, ("Got invalid nttrans request: %s\n", nt_errstr(result)));
	...
	if(req->wct != 19 + (state->setup_count/2)) {
		DEBUG_OUT (2,("Invalid smb_wct %d in nttrans call (should be %d)\n",
	...
	if (state->total_data)  {
		...
		if ((state->data = (char *)SMB_MALLOC(state->total_data)) == NULL) {
			DEBUG_OUT (0,("reply_nttrans: data malloc fail for %u "
	...
	if (state->total_param) {
		...
		if ((state->param = (char *)SMB_MALLOC(state->total_param)) == NULL) {
			DEBUG_OUT (0,("reply_nttrans: param malloc fail for %u "
	...
	if(state->setup_count > 0) {
		DEBUG_OUT (10,("reply_nttrans: state->setup_count = %d\n",
			  state->setup_count));
		...
		if (state->setup == NULL) {
			DEBUG_OUT (0,("reply_nttrans : Out of memory\n"));
			SAFE_FREE(state->data);
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_nttrans: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
...
void reply_nttranss(struct smb_request *req)
{
  ...
  bad_param:
	...
	DEBUG_OUT (0,("reply_nttranss: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
