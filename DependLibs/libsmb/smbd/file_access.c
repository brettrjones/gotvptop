			 ...
			 uint32_t access_mask)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("Could not get acl: %s\n", nt_errstr(status)));
		ret = false;
		goto out;
	}
	...
	if (DEBUGLEVEL >= 10) {
		DEBUG_OUT (10,("can_access_file_acl for file %s "
				  ...
				  const struct smb_filename *smb_fname)
{
	...
	if (smb_fname_parent->st.st_ex_mode & S_ISVTX) {
		...
		if ((get_current_uid(conn) !=
			...
			DEBUG_OUT (10,("can_delete_file_in_directory: not "
			  ...
			  uint32 access_mask)
{
	...
	DEBUG_OUT (10,("can_access_file_data: requesting 0x%x on file %s\n",
