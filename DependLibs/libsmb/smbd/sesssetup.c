			...
			const char *user, const char *domain)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_SUCH_USER)) {
		if ((lp_map_to_guest() == MAP_TO_GUEST_ON_BAD_USER) ||
			...
			DEBUG_OUT (3,("No such user %s [%s] - using guest account\n",
				 user, domain));
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_WRONG_PASSWORD)) {
		if (lp_map_to_guest() == MAP_TO_GUEST_ON_BAD_PASSWORD) {
			DEBUG_OUT (3,("Registered username %s for guest access\n",
				user));
			status = make_server_info_guest(NULL, server_info);
		}
	}
...
static NTSTATUS check_guest_password(struct auth_serversupplied_info **server_info)
{
	...
	DEBUG_OUT (3,("Got anonymous request\n"));
...
static bool make_krb5_skew_error(DATA_BLOB *pblob_out)
{
	...
	*pblob_out = data_blob_null;
	...
	if (kerr) {
		DEBUG_OUT (10,("make_krb5_skew_error: smb_krb5_parse_name failed "
			"for name %s: Error %s\n",
	...
	kerr = smb_krb5_mk_error(context, KRB5KRB_AP_ERR_SKEW,
			host_princ, &reply);
	if (kerr) {
		DEBUG_OUT (10,("make_krb5_skew_error: smb_krb5_mk_error "
			"failed: Error %s\n",
			error_message(kerr) ));
		goto out;
	}
				  ...
				  bool *p_invalidate_vuid)
{
	...
	*p_invalidate_vuid = True;
...
#endif
		DEBUG_OUT (1,("Failed to verify incoming ticket with error %s!\n",
				nt_errstr(ret)));
		talloc_destroy(mem_ctx);
	...
	ret = make_server_info_krb5(mem_ctx,
				    ...
				    &server_info);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (1, ("make_server_info_krb5 failed!\n"));
		data_blob_free(&ap_rep);
		data_blob_free(&session_key);
		TALLOC_FREE(mem_ctx);
		reply_nterror(req, nt_status_squash(ret));
	...
	if ( !server_info->security_token ) {
		...
		if ( !NT_STATUS_IS_OK(ret) ) {
			DEBUG_OUT (10,("failed to create local token: %s\n",
				nt_errstr(ret)));
			data_blob_free(&ap_rep);
			data_blob_free(&session_key);
			TALLOC_FREE( mem_ctx );
		...
		char **kerb_mechOID)
{
	...
	*kerb_mechOID = NULL;
	...
	for (i=0;OIDs[i];i++) {
		DEBUG_OUT (5,("parse_spnego_mechanisms: Got OID %s\n", OIDs[i]));
		talloc_free(OIDs[i]);
	}
	return ret;
}
						...
						uint16 vuid)
{
	...
	DEBUG_OUT (3,("reply_spnego_downgrade_to_ntlmssp: Got krb5 ticket in SPNEGO "
				   ...
				   struct auth_ntlmssp_state **auth_ntlmssp_state)
{
	...
	DEBUG_OUT (3,("reply_spnego_negotiate: Got secblob of size %lu\n",
			      ...
			      struct auth_ntlmssp_state **auth_ntlmssp_state)
{
	...
	if (auth.length > 0 && auth.data[0] == ASN1_APPLICATION(0)) {
		...
		DEBUG_OUT (3,("reply_spnego_auth: Got secblob of size %lu\n",
				(unsigned long)secblob.length));
#ifdef HAVE_KRB5
		if (kerb_mech && ((lp_security()==SEC_ADS) ||
					USE_KERBEROS_KEYTAB)) {
		...
		if (kerb_mech) {
			/* Kill the intermediate vuid */
			invalidate_vuid(sconn, vuid);
			DEBUG_OUT (3,("reply_spnego_auth: network "
					   ...
					   DATA_BLOB *pblob)
{
	...
	if (pblob->length == 0) {
		/* Caller can cope. */
		DEBUG_OUT (2,("check_spnego_blob_complete: zero blob length !\n"));
	...
	if (pad) {
		...
		if (pad->partial_data.length + copy_len <
			...
			DEBUG_OUT (2,("check_spnego_blob_complete: integer wrap "
		...
		DEBUG_OUT (10,("check_spnego_blob_complete: "
			"pad->partial_data.length = %u, "
			"pad->needed_len = %u, "
			"copy_len = %u, "
			"pblob->length = %u,\n",
	...
	DEBUG_OUT (10,("check_spnego_blob_complete: needed_len = %u, "
		"pblob->length = %u\n",
	...
	if (needed_len <= pblob->length) {
		...
		DEBUG_OUT (0,("check_spnego_blob_complete: logic "
			"error (needed_len = %u, "
			"pblob->length = %u).\n",
			(unsigned int)needed_len,
			(unsigned int)pblob->length ));
	...
	if (needed_len > 65536) {
		DEBUG_OUT (2,("check_spnego_blob_complete: needed_len "
			"too large (%u)\n",
			(unsigned int)needed_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
static void reply_sesssetup_and_X_spnego(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("Doing spnego session setup\n"));
	...
	DEBUG_OUT (3,("NativeOS=[%s] NativeLanMan=[%s] PrimaryDomain=[%s]\n",
	...
	if (!is_partial_auth_vuid(sconn, vuid)) {
		...
		pad = get_pending_auth_data(sconn, smbpid);
		if (pad) {
			DEBUG_OUT (10,("reply_sesssetup_and_X_spnego: found "
				"pending vuid %u\n",
	...
	DEBUG_OUT (1,("Unknown packet in reply_sesssetup_and_X_spnego\n"));
				...
				void *private_data)
{
	...
	DEBUG_OUT (10, ("shutdown_other_smbds: %s, %s\n",
		   procid_str(talloc_tos(), &crec->pid), crec->addr));

	if (!process_exists(crec->pid)) {
		DEBUG_OUT (10, ("process does not exist\n"));
		return 0;
	}

	if (procid_is_me(&crec->pid)) {
		DEBUG_OUT (10, ("It's me\n"));
		return 0;
	}

	if (strcmp(state->ip, crec->addr) != 0) {
		DEBUG_OUT (10, ("%s does not match %s\n", state->ip, crec->addr));
		return 0;
	}

	DEBUG_OUT (1, ("shutdown_other_smbds: shutting down pid %u "
...
{
	DEBUG_OUT (2,("setup_new_vc_session: New VC == 0, if NT4.x "
...
void reply_sesssetup_and_X(struct smb_request *req)
{
	...
	if (req->wct == 12 &&
		...
		if (!sconn->smb1.negprot.spnego) {
			DEBUG_OUT (0,("reply_sesssetup_and_X:  Rejecting attempt "
	...
	if (get_Protocol() < PROTOCOL_NT1) {
		...
		DEBUG_OUT (3,("Domain=[%s]  NativeOS=[%s] NativeLanMan=[%s] "
			"PrimaryDomain=[%s]\n",
	...
	DEBUG_OUT (3,("sesssetupX:name=[%s]\\[%s]@[%s]\n",
	...
	if (!*user) {
		...
		if (!negprot_auth_context) {
			DEBUG_OUT (0, ("reply_sesssetup_and_X:  Attempted encrypted "
	...
	if (!server_info->security_token) {
		...
		if (!NT_STATUS_IS_OK(nt_status)) {
			DEBUG_OUT (10, ("create_local_token failed: %s\n",
				   nt_errstr(nt_status)));
			data_blob_free(&nt_resp);
			data_blob_free(&lm_resp);
			data_blob_clear_free(&plaintext_password);
