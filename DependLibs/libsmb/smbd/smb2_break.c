						      ...
						      uint8_t in_oplock_level)
{
	...
	DEBUG_OUT (10,("smbd_smb2_oplock_break_send: %s - fnum[%d] "
		  "samba level %d\n",
	...
	DEBUG_OUT (5,("smbd_smb2_oplock_break_send: got SMB2 oplock break (%u) from client "
		"for file %s fnum = %d\n",
	...
	if (!result) {
		DEBUG_OUT (0, ("smbd_smb2_oplock_break_send: error in removing "
			"oplock on file %s\n", fsp_str_dbg(fsp)));
...
void send_break_message_smb2(files_struct *fsp, int level)
{
	...
	DEBUG_OUT (10,("send_break_message_smb2: sending oplock break "
