...
bool session_claim(struct smbd_server_connection *sconn, user_struct *vuser)
{
	...
	if (lp_utmp()) {
		...
		for (i=1;i<MAX_SESSION_ID;i++) {
			...
			if (rec == NULL) {
				DEBUG_OUT (1, ("Could not lock \"%s\"\n", keystr));
				return False;
			}

			if (rec->value.dsize != sizeof(sessionid)) {
				DEBUG_OUT (1, ("Re-using invalid record\n"));
				break;
			}
			...
			if (!process_exists(sess_pid)) {
				DEBUG_OUT (5, ("%s has died -- re-using session\n",
		...
		if (i == MAX_SESSION_ID) {
			SMB_ASSERT(rec == NULL);
			DEBUG_OUT (1,("session_claim: out of session IDs "
				 "(max is %d)\n", MAX_SESSION_ID));
			return False;
		}
	...
	*/
	...
	if (!smb_pam_claim_session(sessionid.username, sessionid.id_str,
				   sessionid.hostname)) {
		DEBUG_OUT (1,("pam_session rejected the session for %s [%s]\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1,("session_claim: unable to create session id "
			 "record: %s\n", nt_errstr(status)));
		return False;
	}
	...
	if (!vuser->session_keystr) {
		DEBUG_OUT (0, ("session_claim:  talloc_strdup() failed for session_keystr\n"));
		return False;
	}
	return True;
}
			      ...
			      void *private_data)
{
	...
	DEBUG_OUT (7, ("gather_sessioninfo session from %s@%s\n",
...
int list_sessions(TALLOC_CTX *mem_ctx, struct sessionid **session_list)
{
	...
	if (ret == -1) {
		DEBUG_OUT (3, ("Session traverse failed\n"));
