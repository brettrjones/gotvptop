				...
				uint32_t *access_granted)
{
	...
	if ((access_mask & DELETE_ACCESS) && !lp_acl_check_permissions(SNUM(conn))) {
		*access_granted = access_mask;
		...
		DEBUG_OUT (10,("smbd_check_open_rights: not checking ACL "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("smbd_check_open_rights: Could not get acl "
			"on %s: %s\n",
			smb_fname_str_dbg(smb_fname),
			nt_errstr(status)));
		return status;
	...
	DEBUG_OUT (10,("smbd_check_open_rights: file %s requesting "
	...
	if (!NT_STATUS_IS_OK(status)) {
		if (DEBUGLEVEL >= 10) {
			DEBUG_OUT (10,("smbd_check_open_rights: acl for %s is:\n",
				smb_fname_str_dbg(smb_fname) ));
		    ...
		    mode_t mode)
{
	...
	if (fsp->fh->fd == -1) {
		...
		if (errno == EMFILE) {
			...
			if (time((time_t *) NULL) > last_warned) {
				DEBUG_OUT (0,("Too many open files, unable "
	...
	DEBUG_OUT (10,("fd_open: name %s, flags = 0%o mode = 0%o, fd = %d. %s\n",
					...
					files_struct *fsp)
{
	...
	if (ret == -1) {
		DEBUG_OUT (0,("change_file_owner_to_parent: failed to stat parent "
	...
	if (smb_fname_parent->st.st_ex_uid == fsp->fsp_name->st.st_ex_uid) {
		...
		DEBUG_OUT (10,("change_file_owner_to_parent: file %s "
	...
	unbecome_root();
	if (ret == -1) {
		DEBUG_OUT (0,("change_file_owner_to_parent: failed to fchown "
			 "file %s to parent directory uid %u. Error "
			 "was %s\n", fsp_str_dbg(fsp),
			 (unsigned int)smb_fname_parent->st.st_ex_uid,
			 strerror(errno) ));
	} else {
		DEBUG_OUT (10,("change_file_owner_to_parent: changed new file %s to "
				       ...
				       SMB_STRUCT_STAT *psbuf)
{
	...
	if (ret == -1) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0,("change_dir_owner_to_parent: failed to stat parent "
			 "directory %s. Error was %s\n",
			 smb_fname_str_dbg(smb_fname_parent),
			 strerror(errno)));
		goto out;
	...
	*/
	...
	if (!saved_dir) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0,("change_dir_owner_to_parent: failed to get "
	...
	if (vfs_ChDir(conn, fname) == -1) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0,("change_dir_owner_to_parent: failed to change "
	...
	if (ret == -1) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (0,("change_dir_owner_to_parent: failed to stat "
			 "directory '.' (%s) Error was %s\n",
			 fname, strerror(errno)));
		goto chdir;
	}
	...
	if (smb_fname_cwd->st.st_ex_dev != psbuf->st_ex_dev ||
		...
		DEBUG_OUT (0,("change_dir_owner_to_parent: "
	...
	if (smb_fname_parent->st.st_ex_uid == smb_fname_cwd->st.st_ex_uid) {
		/* Already this uid - no need to change. */
		DEBUG_OUT (10,("change_dir_owner_to_parent: directory %s "
			"is already owned by uid %d\n",
			fname,
	...
	if (ret == -1) {
		status = map_nt_error_from_unix(errno);
		DEBUG_OUT (10,("change_dir_owner_to_parent: failed to chown "
	...
	DEBUG_OUT (10,("change_dir_owner_to_parent: changed ownership of new "
		  "directory %s to parent directory uid %u.\n",
			  ...
			  uint32 open_access_mask) /* what we're actually using in the open. */
{
	...
	if (!CAN_WRITE(conn)) {
		...
		if(accmode != O_RDONLY || (flags & O_TRUNC) || (flags & O_APPEND)) {
			DEBUG_OUT (3,("Permission denied opening %s\n",
				 smb_fname_str_dbg(smb_fname)));
			return NT_STATUS_ACCESS_DENIED;
	...
	if ((accmode == O_RDONLY) && ((flags & O_TRUNC) == O_TRUNC)) {
		DEBUG_OUT (10,("open_file: truncate requested on read-only open "
			  "for file %s\n", smb_fname_str_dbg(smb_fname)));
	...
	if ((open_access_mask & (FILE_READ_DATA|FILE_WRITE_DATA|FILE_APPEND_DATA|FILE_EXECUTE)) ||
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3,("Error opening file %s (%s) (local_flags=%d) "
				 "(flags=%d)\n", smb_fname_str_dbg(smb_fname),
		...
		if (file_existed) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				if (NT_STATUS_EQUAL(status, NT_STATUS_ACCESS_DENIED)) {
					 ...
					 * nowhere. Allow. */
					DEBUG_OUT (10,("open_file: allowing POSIX "
	...
	if (!file_existed) {
		...
		if (fsp->fh->fd == -1) {
			...
			if (ret == -1) {
				DEBUG_OUT (0,("Error doing fstat on open file %s "
					 "(%s)\n",
					 smb_fname_str_dbg(smb_fname),
					 strerror(errno) ));
			}
	...
	DEBUG_OUT (2,("%s opened file %s read=%s write=%s (numopen=%d)\n",
...
static bool share_conflict(struct share_mode_entry *entry,
			   ...
			   uint32 share_access)
{
	DEBUG_OUT (10,("share_conflict: entry->access_mask = 0x%x, "
	...
	DEBUG_OUT (10,("share_conflict: access_mask = 0x%x, share_access = 0x%x\n",
	...
	if ((entry->access_mask & (FILE_WRITE_DATA|
				   ...
				   FILE_EXECUTE|
				   DELETE_ACCESS)) == 0) {
		DEBUG_OUT (10,("share_conflict: No conflict due to "
...
#define CHECK_MASK(num, am, right, sa, share) \
	DEBUG_OUT (10,("share_conflict: [%d] am (0x%x) & right (0x%x) = 0x%x\n", \
		(unsigned int)(num), (unsigned int)(am), \
		(unsigned int)(right), (unsigned int)(am)&(right) )); \
	DEBUG_OUT (10,("share_conflict: [%d] sa (0x%x) & share (0x%x) = 0x%x\n", \
		(unsigned int)(num), (unsigned int)(sa), \
		(unsigned int)(share), (unsigned int)(sa)&(share) )); \
	if (((am) & (right)) && !((sa) & (share))) { \
		DEBUG_OUT (10,("share_conflict: check %d conflict am = 0x%x, right = 0x%x, \
...
sa = 0x%x, share = 0x%x\n", (num), (unsigned int)(am), (unsigned int)(right), (unsigned int)(sa), \
	...
	DEBUG_OUT (10,("share_conflict: No conflict.\n"));
	return False;
}
				      ...
				      struct share_mode_entry *share_entry)
{
	...
	if (!fsp) {
		DEBUG_OUT (0,("validate_my_share_entries: PANIC : %s\n",
			 share_mode_str(talloc_tos(), num, share_entry) ));
					...
					int oplock_request)
{
	...
	DEBUG_OUT (10, ("Sending break request to PID %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Could not send oplock break message: %s\n",
			  nt_errstr(status)));
	}
				...
				bool got_a_none_oplock)
{
	...
	if (oplock_request & INTERNAL_OPEN_ONLY) {
		...
		DEBUG_OUT (10,("grant_fsp_oplock_type: oplock type 0x%x on file %s\n",
			fsp->oplock_type, fsp_str_dbg(fsp)));
		return;
	} else if (br_lck && br_lck->num_locks > 0) {
		DEBUG_OUT (10,("grant_fsp_oplock_type: file %s has byte range locks\n",
			fsp_str_dbg(fsp)));
		fsp->oplock_type = NO_OPLOCK;
	}
		       ...
		       struct deferred_open_record *state)
{
	...
	for (i=0; i<lck->num_share_modes; i++) {
		...
		if (procid_is_me(&e->pid) && (e->op_mid == req->mid)) {
			DEBUG_OUT (0, ("Trying to defer an already deferred "
	...
	DEBUG_OUT (10,("defer_open_sharing_error: time [%u.%06u] adding deferred "
			   ...
			   mode_t *returned_unx_mode)
{
	...
	DEBUG_OUT (10,("open_match_attributes: old_dos_attr = 0x%x, "
				     ...
				     uint32 create_options)
{
	...
	DEBUG_OUT (5,("fcb_or_dos_open: attempting old open semantics for "
		 "file %s.\n", smb_fname_str_dbg(smb_fname)));

	for(fsp = file_find_di_first(conn->sconn, id); fsp;
	    fsp = file_find_di_next(fsp)) {

		DEBUG_OUT (10,("fcb_or_dos_open: checking file %s, fd = %d, "
		...
		if (fsp->fh->fd != -1 &&
			...
			DEBUG_OUT (10,("fcb_or_dos_open: file match\n"));
			break;
		}
	}
	...
	if (is_executable(smb_fname->base_name) &&
	    (fsp->fh->private_options & NTCREATEX_OPTIONS_PRIVATE_DENY_DOS)) {
		DEBUG_OUT (10,("fcb_or_dos_open: file fail due to is_executable.\n"));
				    ...
				    uint32_t *access_mask_out)
{
	...
	if (access_mask & MAXIMUM_ALLOWED_ACCESS) {
		if (file_existed) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (10,("smbd_calculate_access_mask: "
	...
	if (rejected_share_access) {
		DEBUG_OUT (10, ("smbd_calculate_access_mask: Access denied on "
				...
				struct server_id pid)
{
	struct share_mode_lock *lck = get_share_mode_lock(talloc_tos(), id,
			NULL, NULL, NULL);
	if (lck == NULL) {
		DEBUG_OUT (0, ("could not get share mode lock\n"));
	} else {
				...
				struct byte_range_lock **p_br_lck)
{
	...
	if (lp_locking(fsp->conn->params)) {
		*p_br_lck = brl_get_locks_readonly(fsp);
		if (*p_br_lck == NULL) {
			DEBUG_OUT (0, ("Could not get br_lock\n"));
			return false;
		}
			    ...
			    files_struct *fsp)
{
	...
	if (conn->printer) {
		...
		DEBUG_OUT (10, ("open_file_ntcreate: printer open fname=%s\n",
			   smb_fname_str_dbg(smb_fname)));

		if (!req) {
			DEBUG_OUT (0,("open_file_ntcreate: printer open without "
				"an SMB request!\n"));
	...
	DEBUG_OUT (10, ("open_file_ntcreate: fname=%s, dos_attrs=0x%x "
	...
	if ((req == NULL) && ((oplock_request & INTERNAL_OPEN_ONLY) == 0)) {
		DEBUG_OUT (0, ("No smb request but not an internal only open!\n"));
		return NT_STATUS_INTERNAL_ERROR;
	}
	...
	if (!lp_posix_pathnames() && strstr(smb_fname->base_name,".+,;=[].")) {
		 ...
		 * release. */
		DEBUG_OUT (5,("open_file_ntcreate: OS/2 long filenames are not "
			 "supported.\n"));
		if (use_nt_status()) {
	...
	switch( create_disposition ) {
		...
		case FILE_OPEN:
			...
			if (!file_existed) {
				DEBUG_OUT (5,("open_file_ntcreate: FILE_OPEN "
		...
		case FILE_OVERWRITE:
			 ...
			 * error. */
			if (!file_existed) {
				DEBUG_OUT (5,("open_file_ntcreate: FILE_OVERWRITE "
					 "requested for file %s and file "
					 "doesn't exist.\n",
					 smb_fname_str_dbg(smb_fname) ));
				errno = ENOENT;
		...
		case FILE_CREATE:
			 ...
			 * create. */
			if (file_existed) {
				DEBUG_OUT (5,("open_file_ntcreate: FILE_CREATE "
					 "requested for file %s and file "
					 "already exists.\n",
					 smb_fname_str_dbg(smb_fname)));
				if (S_ISDIR(smb_fname->st.st_ex_mode)) {
	...
	if (!posix_open && file_existed && ((create_disposition == FILE_OVERWRITE) ||
		...
		if (!open_match_attributes(conn, existing_dos_attributes,
			...
			DEBUG_OUT (5,("open_file_ntcreate: attributes missmatch "
	...
	status = smbd_calculate_access_mask(conn, smb_fname, file_existed,
					...
					&access_mask); 
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("open_file_ntcreate: smbd_calculate_access_mask "
	...
	DEBUG_OUT (10, ("open_file_ntcreate: fname=%s, after mapping "
		   "access_mask=0x%x\n", smb_fname_str_dbg(smb_fname),
		    access_mask));
	...
	if (flags != O_RDONLY && file_existed &&
		...
		DEBUG_OUT (5,("open_file_ntcreate: write access requested for "
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			if (private_flags &
			    (NTCREATEX_OPTIONS_PRIVATE_DENY_DOS|
			     NTCREATEX_OPTIONS_PRIVATE_DENY_FCB)) {
				if (req == NULL) {
					DEBUG_OUT (0, ("DOS open without an SMB "
						  "request!\n"));
					TALLOC_FREE(lck);
					return NT_STATUS_INTERNAL_ERROR;
				}
	...
	DEBUG_OUT (4,("calling open_file with flags=0x%X flags2=0x%X mode=0%o, "
	...
	if (!posix_open && !file_existed && !def_acl) {
...

		{
			...
			if (ret == -1 && errno == ENOSYS) {
				...
				DEBUG_OUT (5, ("open_file_ntcreate: reset "
		...
		if ((ret == -1) &&
			...
			DEBUG_OUT (5, ("open_file_ntcreate: failed to reset "
				  "attributes of file %s to 0%o\n",
				  smb_fname_str_dbg(smb_fname),
				  (unsigned int)new_unx_mode));
	}
			       ...
			       uint32 file_attributes)
{
	...
	if(!CAN_WRITE(conn)) {
		DEBUG_OUT (5,("mkdir_internal: failing create on read-only share "
	...
	if (SMB_VFS_LSTAT(conn, smb_dname) == -1) {
		DEBUG_OUT (2, ("Could not stat directory '%s' just created: %s\n",
			  smb_fname_str_dbg(smb_dname), strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	if (!S_ISDIR(smb_dname->st.st_ex_mode)) {
		DEBUG_OUT (0, ("Directory just '%s' created is not a directory\n",
			       ...
			       files_struct **result)
{
	...
	DEBUG_OUT (5,("open_directory: opening directory %s, access_mask = 0x%x, "
	...
	if (!(file_attributes & FILE_FLAG_POSIX_SEMANTICS) &&
		...
		DEBUG_OUT (2, ("open_directory: %s is a stream name!\n",
			  smb_fname_str_dbg(smb_dname)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("open_directory: smbd_calculate_access_mask "
			"on file %s returned %s\n",
			smb_fname_str_dbg(smb_dname),
			nt_errstr(status)));
		return status;
	...
	if ((access_mask & SEC_FLAG_SYSTEM_SECURITY) &&
			!security_token_has_privilege(get_current_nttok(conn),
					SEC_PRIV_SECURITY)) {
		DEBUG_OUT (10, ("open_directory: open on %s "
	...
	switch( create_disposition ) {
		...
		case FILE_CREATE:
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (2, ("open_directory: unable to create "
					  "%s. Error was %s\n",
					  smb_fname_str_dbg(smb_dname),
					  nt_errstr(status)));
				return status;
		...
		case FILE_OVERWRITE_IF:
		default:
			DEBUG_OUT (5,("open_directory: invalid create_disposition "
	...
	if(!S_ISDIR(smb_dname->st.st_ex_mode)) {
		DEBUG_OUT (5,("open_directory: %s is not a directory !\n",
			 smb_fname_str_dbg(smb_dname)));
		return NT_STATUS_NOT_A_DIRECTORY;
	}
	...
	if (info == FILE_WAS_OPENED) {
		...
		if ((NT_STATUS_EQUAL(status, NT_STATUS_ACCESS_DENIED) &&
			...
			DEBUG_OUT (10,("open_directory: overrode ACCESS_DENIED "
				"on directory %s\n",
				smb_fname_str_dbg(smb_dname)));
			status = NT_STATUS_OK;
		}
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("open_directory: smbd_check_open_rights on "
...
#endif
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("open_directory: Could not open fd for "
			"%s (%s)\n",
			smb_fname_str_dbg(smb_dname),
			nt_errstr(status)));
		file_free(req, fsp);
	...
	if (!check_same_stat(&smb_dname->st, &fsp->fsp_name->st)) {
		DEBUG_OUT (5,("open_directory: stat struct differs for "
			"directory %s.\n",
			smb_fname_str_dbg(smb_dname)));
		fd_close(fsp);
		file_free(req, fsp);
	...
	if (lck == NULL) {
		DEBUG_OUT (0, ("open_directory: Could not get share mode lock for "
			  "%s\n", smb_fname_str_dbg(smb_dname)));
		fd_close(fsp);
		file_free(req, fsp);
		return NT_STATUS_SHARING_VIOLATION;
			  ...
			  DATA_BLOB *data)
{
	...
	if (sconn == NULL) {
		DEBUG_OUT (1, ("could not find sconn\n"));
		return;
	}

	if (data->data == NULL
	    || data->length < MSG_FILE_RENAMED_MIN_SIZE + 2) {
                DEBUG_OUT (0, ("msg_file_was_renamed: Got invalid msg len %d\n",
			  (int)data->length));
                return;
        }
	...
	DEBUG_OUT (10,("msg_file_was_renamed: Got rename message for sharepath %s, new name %s, "
		"file_id %s\n",
	...
	for(fsp = file_find_di_first(sconn, id); fsp;
		...
		if (memcmp(fsp->conn->connectpath, sharepath, sp_len) == 0) {
			...
			DEBUG_OUT (10,("msg_file_was_renamed: renaming file fnum %d from %s -> %s\n",
			...
			/* Now we have the complete path we can work out if this is
			   actually within this share and adjust newname accordingly. */
	                DEBUG_OUT (10,("msg_file_was_renamed: share mismatch (sharepath %s "
				"not sharepath %s) "
				"fnum %d from %s -> %s\n",
					...
					const char *fname)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NOT_IMPLEMENTED)
		...
		DEBUG_OUT (10, ("no streams around\n"));
		TALLOC_FREE(frame);
		return NT_STATUS_OK;
	}

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("vfs_streaminfo failed: %s\n",
			   nt_errstr(status)));
		goto fail;
	}

	DEBUG_OUT (10, ("open_streams_for_delete found %d streams\n",
		   num_streams));
	...
	for (i=0; i<num_streams; i++) {
		...
		if (SMB_VFS_STAT(conn, smb_fname) == -1) {
			DEBUG_OUT (10, ("Unable to stat stream: %s\n",
				   smb_fname_str_dbg(smb_fname)));
		}
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("Could not open stream %s: %s\n",
				   smb_fname_str_dbg(smb_fname),
				   nt_errstr(status)));
	...
	for (i -= 1 ; i >= 0; i--) {
		...
		DEBUG_OUT (10, ("Closing stream # %d, %s\n", i,
				     ...
				     int *pinfo)
{
	...
	DEBUG_OUT (10,("create_file_unixpath: access_mask = 0x%x "
	...
	if ((create_disposition != FILE_CREATE)
		...
		status = NT_STATUS_ACCESS_DENIED;
		DEBUG_OUT (10,("create_file_unixpath: open file %s "
	...
	if ((access_mask & SEC_FLAG_SYSTEM_SECURITY) &&
			!security_token_has_privilege(get_current_nttok(conn),
					SEC_PRIV_SECURITY)) {
		DEBUG_OUT (10, ("create_file_unixpath: open on %s "
			"failed - SEC_FLAG_SYSTEM_SECURITY denied.\n",
			smb_fname_str_dbg(smb_fname)));
	...
	if ((conn->fs_capabilities & FILE_NAMED_STREAMS)
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("create_file_unixpath for base %s failed: "
				   "%s\n", smb_fname->base_name,
				   nt_errstr(status)));
			goto fail;
		}
	...
	DEBUG_OUT (10, ("create_file_unixpath: info=%d\n", info));
 ...
 fail:
	DEBUG_OUT (10, ("create_file_unixpath: %s\n", nt_errstr(status)));
			     ...
			     int *pinfo)
{
	...
	DEBUG_OUT (10,("create_file: access_mask = 0x%x "
		  "file_attributes = 0x%x, share_access = 0x%x, "
		  "create_disposition = 0x%x create_options = 0x%x "
		  "oplock_request = 0x%x "
		  "private_flags = 0x%x "
	...
	DEBUG_OUT (10, ("create_file: info=%d\n", info));
