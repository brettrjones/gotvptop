		       ...
		       uint64_t *dfree, uint64_t *dsize)
{
	...
	*bsize = *dfree = *dsize = (uint64_t)0;
	...
	DEBUG_OUT (5,("nfs_quotas: looking for mount on \"%s\"\n", cutstr));
	DEBUG_OUT (5,("nfs_quotas: of path \"%s\"\n", mnttype));
	testpath=strchr_m(mnttype, ':');
	args.gqa_pathp = testpath+1;
	args.gqa_uid = uid;

	DEBUG_OUT (5, ("nfs_quotas: Asking for host \"%s\" rpcprog \"%i\" rpcvers "
	...
	DEBUG_OUT (9,("nfs_quotas: auth_success\n"));
	...
	if (clnt_stat != RPC_SUCCESS) {
		DEBUG_OUT (9,("nfs_quotas: clnt_call fail\n"));
		ret = False;
		goto out;
	}
	...
	switch (gqr.status) {
	case 0:
		DEBUG_OUT (9, ("nfs_quotas: Remote Quotas Failed!  Error \"%i\" \n",
			  gqr.status));
		ret = False;
		goto out;

	case 1:
		DEBUG_OUT (9,("nfs_quotas: Good quota data\n"));
	...
	case 3:
		D.softlimit = 1;
		D.curblocks = 1;
		DEBUG_OUT (9, ("nfs_quotas: Remote Quotas returned \"%i\" \n",
			  gqr.status));
		break;

	default:
		DEBUG_OUT (9, ("nfs_quotas: Remote Quotas Questionable!  "
			  "Error \"%i\" \n", gqr.status));
		break;
	}

	DEBUG_OUT (10, ("nfs_quotas: Let`s look at D a bit closer... "
  ...
  out:
	...
	DEBUG_OUT (5, ("nfs_quotas: For path \"%s\" returning "
		  "bsize %.0f, dfree %.0f, dsize %.0f\n",
		  args.gqa_pathp, (double)*bsize, (double)*dfree,
		  (double)*dsize));

	SAFE_FREE(cutstr);
	DEBUG_OUT (10,("nfs_quotas: End of nfs_quotas\n" ));
	return ret;
}
...
static int get_smb_linux_xfs_quota(char *path, uid_t euser_id, gid_t egrp_id, LINUX_SMB_DISK_QUOTA *dp)
{
	DEBUG_OUT (0,("XFS quota support not available\n"));
	errno = ENOSYS;
	return -1;
}
#endif
...
static bool nfs_quotas(char *nfspath, uid_t euser_id, uint64_t *bsize, uint64_t *dfree, uint64_t *dsize)
{
	...
	*bsize = *dfree = *dsize = (uint64_t)0;
	...
	DEBUG_OUT (5,("nfs_quotas: Asking for host \"%s\" rpcprog \"%i\" rpcvers \"%i\" network \"%s\"\n", host, RQUOTAPROG, RQUOTAVERS, "udp"));
	...
	default:
		DEBUG_OUT (9,("nfs_quotas: Remote Quotas Questionable!  Error \"%i\" \n", gqr.status ));
		break;
	}

	DEBUG_OUT (10,("nfs_quotas: Let`s look at D a bit closer... status \"%i\" bsize \"%i\" active? \"%i\" bhard \"%i\" bsoft \"%i\" curb \"%i\" \n",
			gqr.status,
			gqr.getquota_rslt_u.gqr_rquota.rq_bsize,
			gqr.getquota_rslt_u.gqr_rquota.rq_active,
			gqr.getquota_rslt_u.gqr_rquota.rq_bhardlimit,
  ...
  out:
	...
	DEBUG_OUT (5,("nfs_quotas: For path \"%s\" returning  bsize %.0f, dfree %.0f, dsize %.0f\n",args.gqa_pathp,(double)*bsize,(double)*dfree,(double)*dsize));
		...
		uint64_t *dsize)
{
	...
	devno = sbuf.st_ex_dev ;
	DEBUG_OUT (5,("disk_quotas: looking for path \"%s\" devno=%x\n",
	...
	while (getmntent(fd, &mnt) == 0) {
		...
		DEBUG_OUT (5,("disk_quotas: testing \"%s\" devno=%x\n",
	...
	if (strcmp(mnt.mnt_fstype, "nfs") == 0) {
		bool retval;
		DEBUG_OUT (5,("disk_quotas: looking for mountpath (NFS) \"%s\"\n",
					mnt.mnt_special));
	...
	DEBUG_OUT (5,("disk_quotas: looking for quotas file \"%s\"\n", name));
	...
	close(file);
#else
	DEBUG_OUT (5,("disk_quotas: trying quotactl on device \"%s\"\n", name));
	...
	*dsize = D.dqb_bsoftlimit;
	...
	DEBUG_OUT (5,("disk_quotas for path \"%s\" returning "
		"bsize %.0f, dfree %.0f, dsize %.0f\n",
...
bool disk_quotas(const char *path, uint64_t *bsize, uint64_t *dfree, uint64_t *dsize)
{
  ...
  *bsize = 512;
  ...
  else if ( 0 == strcmp ( mnt->mnt_type, "xfs" ))
  {
    ...
    if (r==-1)
    {
      DEBUG_OUT (5, ("quotactl for uid=%u: %s", euser_id, strerror(errno)));
      return(False);
    }
...
bool disk_quotas_vxfs(const char *name, char *path, uint64_t *bsize, uint64_t *dfree, uint64_t *dsize)
{
  ...
  DEBUG_OUT (5,("disk_quotas: looking for VxFS quotas file \"%s\"\n", qfname));
  ...
  if (ret < 0) {
    DEBUG_OUT (5,("disk_quotas ioctl (VxFS) failed. Error = %s\n", strerror(errno) ));
    return(False);
  }
   ...
   */
  DEBUG_OUT (5,("disk_quotas for path \"%s\" block c/s/h %ld/%ld/%ld; file c/s/h %ld/%ld/%ld\n",
  ...
  *dsize = D.dqb_bsoftlimit;
  ...
  DEBUG_OUT (5,("disk_quotas for path \"%s\" returning  bsize %.0f, dfree %.0f, dsize %.0f\n",
         path,(double)*bsize,(double)*dfree,(double)*dsize));
