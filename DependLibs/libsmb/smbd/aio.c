...
static bool initialize_async_io_handler(void)
{
	...
	aio_signal_event = tevent_add_signal(smbd_event_context(),
					     ...
					     NULL);
	if (!aio_signal_event) {
		DEBUG_OUT (10, ("Failed to setup RT_SIGNAL_AIO handler\n"));
		return false;
	}
			     ...
			     size_t smb_maxcnt)
{
	...
	if (fsp->base_fsp != NULL) {
		...
		DEBUG_OUT (10, ("AIO on streams not yet supported\n"));
		return NT_STATUS_RETRY;
	}
	...
	if ((!min_aio_read_size || (smb_maxcnt < min_aio_read_size))
		...
		DEBUG_OUT (10,("schedule_aio_read_and_X: read size (%u) too small "
	...
	if (outstanding_aio_calls >= aio_pending_size) {
		DEBUG_OUT (10,("schedule_aio_read_and_X: Already have %d aio "
	...
	if ((aio_ex = create_aio_extra(NULL, fsp, bufsize)) == NULL) {
		DEBUG_OUT (10,("schedule_aio_read_and_X: malloc fail.\n"));
	...
	if (ret == -1) {
		DEBUG_OUT (0,("schedule_aio_read_and_X: aio_read failed. "
			 "Error %s\n", strerror(errno) ));
	...
	DEBUG_OUT (10,("schedule_aio_read_and_X: scheduled aio_read for file %s, "
			      ...
			      size_t numtowrite)
{
	...
	if ((!min_aio_write_size || (numtowrite < min_aio_write_size))
		...
		DEBUG_OUT (10,("schedule_aio_write_and_X: write size (%u) too "
	...
	if (outstanding_aio_calls >= aio_pending_size) {
		DEBUG_OUT (3,("schedule_aio_write_and_X: Already have %d aio "
			 "activities outstanding.\n",
			  outstanding_aio_calls ));
		DEBUG_OUT (10,("schedule_aio_write_and_X: failed to schedule "
	...
	if (!(aio_ex = create_aio_extra(NULL, fsp, bufsize))) {
		DEBUG_OUT (0,("schedule_aio_write_and_X: malloc fail.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if (ret == -1) {
		DEBUG_OUT (3,("schedule_aio_wrote_and_X: aio_write failed. "
			 "Error %s\n", strerror(errno) ));
		SMB_VFS_STRICT_UNLOCK(conn, fsp, &aio_ex->lock);
		TALLOC_FREE(aio_ex);
		return NT_STATUS_RETRY;
	...
	if (!aio_ex->write_through && !lp_syncalways(SNUM(fsp->conn))
		...
		DEBUG_OUT (10,("schedule_aio_write_and_X: scheduled aio_write "
			  "behind for file %s\n", fsp_str_dbg(fsp)));
	}

	DEBUG_OUT (10,("schedule_aio_write_and_X: scheduled aio_write for file "
				...
				size_t smb_maxcnt)
{
	...
	if ((!min_aio_read_size || (smb_maxcnt < min_aio_read_size))
	    && !SMB_VFS_AIO_FORCE(fsp)) {
		/* Too small a read for aio request. */
		DEBUG_OUT (10,("smb2: read size (%u) too small "
			"for minimum aio_read of %u\n",
			(unsigned int)smb_maxcnt,
			(unsigned int)min_aio_read_size ));
		return NT_STATUS_RETRY;
	...
	if (outstanding_aio_calls >= aio_pending_size) {
		DEBUG_OUT (10,("smb2: Already have %d aio "
			"activities outstanding.\n",
			outstanding_aio_calls ));
		return NT_STATUS_RETRY;
	}
	...
	*preadbuf = data_blob_talloc(ctx, NULL, smb_maxcnt);
	...
	ret = SMB_VFS_AIO_READ(fsp, a);
	if (ret == -1) {
		DEBUG_OUT (0,("smb2: aio_read failed. "
			"Error %s\n", strerror(errno) ));
		SMB_VFS_STRICT_UNLOCK(conn, fsp, &aio_ex->lock);
		TALLOC_FREE(aio_ex);
		return NT_STATUS_RETRY;
	...
	DEBUG_OUT (10,("smb2: scheduled aio_read for file %s, "
		"offset %.0f, len = %u (mid = %u)\n",
		fsp_str_dbg(fsp), (double)startpos, (unsigned int)smb_maxcnt,
		(unsigned int)aio_ex->smbreq->mid ));
				...
				bool write_through)
{
	...
	if ((!min_aio_write_size || (in_data.length < min_aio_write_size))
	    && !SMB_VFS_AIO_FORCE(fsp)) {
		/* Too small a write for aio request. */
		DEBUG_OUT (10,("smb2: write size (%u) too "
			"small for minimum aio_write of %u\n",
	...
	if (outstanding_aio_calls >= aio_pending_size) {
		DEBUG_OUT (3,("smb2: Already have %d aio "
			"activities outstanding.\n",
			outstanding_aio_calls ));
		return NT_STATUS_RETRY;
	}
	...
	ret = SMB_VFS_AIO_WRITE(fsp, a);
	if (ret == -1) {
		DEBUG_OUT (3,("smb2: aio_write failed. "
			"Error %s\n", strerror(errno) ));
		SMB_VFS_STRICT_UNLOCK(conn, fsp, &aio_ex->lock);
		TALLOC_FREE(aio_ex);
		return NT_STATUS_RETRY;
	...
	* smbreq are children of smbreq->smb2req. */
	...
	DEBUG_OUT (10,("smb2: scheduled aio_write for file "
		"%s, offset %.0f, len = %u (mid = %u) "
		"outstanding_aio_calls = %d\n",
		fsp_str_dbg(fsp),
		(double)in_offset,
...
static int handle_aio_read_complete(struct aio_extra *aio_ex, int errcode)
{
	...
	if (nread < 0) {
		...
		DEBUG_OUT ( 3,( "handle_aio_read_complete: file %s nread == %d. "
			   "Error = %s\n",
		...
		DEBUG_OUT ( 3, ( "handle_aio_read_complete file %s max=%d "
			    "nread=%d\n",
	...
	DEBUG_OUT (10,("handle_aio_read_complete: scheduled aio_read completed "
...
static int handle_aio_write_complete(struct aio_extra *aio_ex, int errcode)
{
	...
	if (fsp->aio_write_behind) {
		if (nwritten != numtowrite) {
			if (nwritten == -1) {
				DEBUG_OUT (5,("handle_aio_write_complete: "
					 "aio_write_behind failed ! File %s "
					 "is corrupt ! Error %s\n",
					 fsp_str_dbg(fsp), strerror(errcode)));
			} else {
				DEBUG_OUT (0,("handle_aio_write_complete: "
					 "aio_write_behind failed ! File %s "
			...
			DEBUG_OUT (10,("handle_aio_write_complete: "
	...
	if(nwritten == -1) {
		DEBUG_OUT ( 3,( "handle_aio_write: file %s wanted %u bytes. "
			   "nwritten == %d. Error = %s\n",
		...
		DEBUG_OUT (3,("handle_aio_write: fnum=%d num=%d wrote=%d\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			ERROR_BOTH(map_nt_error_from_unix(errcode),
				   ERRHRD, ERRdiskfull);
			srv_set_message(outbuf,0,0,true);
                	DEBUG_OUT (5,("handle_aio_write: sync_file for %s returned %s\n",
	...
	DEBUG_OUT (10,("handle_aio_write_complete: scheduled aio_write completed "
...
static int handle_aio_smb2_read_complete(struct aio_extra *aio_ex, int errcode)
{
	...
	DEBUG_OUT (10,("smb2: scheduled aio_read completed "
		"for file %s, offset %.0f, len = %u "
...
static int handle_aio_smb2_write_complete(struct aio_extra *aio_ex, int errcode)
{
	...
	DEBUG_OUT (10,("smb2: scheduled aio_write completed "
...
static bool handle_aio_completed(struct aio_extra *aio_ex, int *perr)
{
	...
	if(!aio_ex) {
	        DEBUG_OUT (3, ("handle_aio_completed: Non-existing aio_ex passed\n"));
		return false;
	}
	...
	if (err == EINPROGRESS) {
		DEBUG_OUT (10,( "handle_aio_completed: operation mid %llu still in "
			"process for file %s\n",
	...
	if (err == ECANCELED) {
		 ...
		 * client. */
	        DEBUG_OUT (10,( "handle_aio_completed: operation mid %llu"
			" canceled\n",
			(unsigned long long)aio_ex->smbreq->mid));
		return True;
        }
...
void smbd_aio_complete_aio_ex(struct aio_extra *aio_ex)
{
	...
	DEBUG_OUT (10,("smbd_aio_complete_mid: mid[%llu]\n",
		(unsigned long long)aio_ex->smbreq->mid));
	...
	if (fsp == NULL) {
		 ...
		 * ignore. */
		DEBUG_OUT ( 3,( "smbd_aio_complete_mid: file closed whilst "
...
int wait_for_aio_completion(files_struct *fsp)
{
	...
	for (seconds_left = SMB_TIME_FOR_AIO_COMPLETE_WAIT;
		...
		DEBUG_OUT (3,("wait_for_aio_completion: waiting for %d aio events "
			 "to complete.\n", aio_completion_count ));
		...
		DEBUG_OUT (10,("wait_for_aio_completion: %d events, doing a wait "
			  "of %d seconds.\n",
		...
		DEBUG_OUT (10,("wait_for_aio_completion: returned err = %d, "
			  "errno = %s\n", err, strerror(errno) ));

		if (err == -1 && errno == EAGAIN) {
			DEBUG_OUT (0,("wait_for_aio_completion: aio_suspend timed "
	 ...
	 * else EIO. */
	DEBUG_OUT (10,("wait_for_aio_completion: aio_suspend timed out waiting "
		  "for %d events\n",
		  aio_completion_count));
