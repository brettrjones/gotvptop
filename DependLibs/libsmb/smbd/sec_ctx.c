...
static bool become_uid(uid_t uid)
{
	...
	if (uid == (uid_t)-1 || 
		...
		if (!become_uid_done) {
			DEBUG_OUT (1,("WARNING: using uid %d is a security risk\n",
				 (int)uid));
...
static bool become_gid(gid_t gid)
{
	...
	if (gid == (gid_t)-1 || ((sizeof(gid_t) == 2) && 
				 (gid == (gid_t)65535))) {
		if (!become_gid_done) {
			DEBUG_OUT (1,("WARNING: using gid %d is a security risk\n",
				 (int)gid));  
...
static int get_current_groups(gid_t gid, uint32_t *p_ngroups, gid_t **p_groups)
{
	...
	if((groups = SMB_MALLOC_ARRAY(gid_t, ngroups+1)) == NULL) {
		DEBUG_OUT (0,("setup_groups malloc fail !\n"));
		goto fail;
	}
	...
	DEBUG_OUT ( 4, ( "get_current_groups: user is in %u groups: ", ngroups));
	for (i = 0; i < ngroups; i++ ) {
		DEBUG_OUT ( 4, ( "%s%d", (i ? ", " : ""), (int)groups[i] ) );
	}
	DEBUG_OUT ( 4, ( "\n" ) );
...
bool push_sec_ctx(void)
{
	...
	if (sec_ctx_stack_ndx == MAX_SEC_CTX_DEPTH) {
		DEBUG_OUT (0, ("Security context stack overflow!\n"));
	...
	ctx_p->ut.gid = getegid();
 	...
 	DEBUG_OUT (4, ("push_sec_ctx(%u, %u) : sec_ctx_stack_ndx = %d\n", 
	...
	if (ctx_p->ut.ngroups != 0) {
		if (!(ctx_p->ut.groups = SMB_MALLOC_ARRAY(gid_t, ctx_p->ut.ngroups))) {
			DEBUG_OUT (0, ("Out of memory in push_sec_ctx()\n"));
...
static void set_unix_security_ctx(uid_t uid, gid_t gid, int ngroups, gid_t *groups)
{
	...
	if (syscall(SYS_initgroups, (ngroups > max) ? max : ngroups,
		...
		DEBUG_OUT (0, ("WARNING: failed to set group list "
...
void set_sec_ctx(uid_t uid, gid_t gid, int ngroups, gid_t *groups, const struct security_token *token)
{
	...
	DEBUG_OUT (4, ("setting sec ctx (%u, %u) - sec_ctx_stack_ndx = %d\n", 
...
bool pop_sec_ctx(void)
{
	...
	if (sec_ctx_stack_ndx == 0) {
		DEBUG_OUT (0, ("Security context stack underflow!\n"));
	...
	DEBUG_OUT (4, ("pop_sec_ctx (%u, %u) - sec_ctx_stack_ndx = %d\n", 
