				       ...
				       void *userdata)
{
	...
	case AVAHI_ENTRY_GROUP_ESTABLISHED:
		DEBUG_OUT (10, ("avahi_entry_group_callback: "
				  ...
				  void *userdata)
{
	...
	case AVAHI_CLIENT_S_RUNNING:
		...
		if (avahi_entry_group_add_service(
			...
			error = avahi_client_errno(c);
			DEBUG_OUT (10, ("avahi_entry_group_add_service failed: "
				   "%s\n", avahi_strerror(error)));
		...
		if (avahi_entry_group_commit(state->entry_group) < 0) {
			error = avahi_client_errno(c);
			DEBUG_OUT (10, ("avahi_entry_group_commit failed: "
				   "%s\n", avahi_strerror(error)));
			avahi_entry_group_free(state->entry_group);
			state->entry_group = NULL;
			break;
	...
	case AVAHI_CLIENT_FAILURE:
		...
		DEBUG_OUT (10, ("avahi_client_callback: AVAHI_CLIENT_FAILURE: %s\n",
			   avahi_strerror(error)));
		...
		if (state->client == NULL) {
			DEBUG_OUT (10, ("avahi_client_new failed: %s\n",
				   avahi_strerror(error)));
			break;
		}
		break;
	case AVAHI_CLIENT_S_COLLISION:
		DEBUG_OUT (10, ("avahi_client_callback: "
