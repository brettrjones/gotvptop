...
bool set_conn_connectpath(connection_struct *conn, const char *connectpath)
{
	...
	*d = '\0';
	...
	DEBUG_OUT (10,("set_conn_connectpath: service %s, connectpath = %s\n",
...
bool set_current_service(connection_struct *conn, uint16 flags, bool do_chdir)
{
	...
	if (do_chdir &&
	    ...
	    vfs_ChDir(conn,conn->origpath) != 0) {
                DEBUG_OUT (((errno!=EACCES)?0:3),("chdir (%s) failed, reason: %s\n",
...
{
	DEBUG_OUT (8, ("load_registry_shares()\n"));
	if (!lp_registry_shares()) {
		return;
	}
...
int find_service(TALLOC_CTX *ctx, const char *service_in, char **p_service_out)
{
	...
	*p_service_out = talloc_strdup(ctx, service_in);
	...
	if (iService < 0) {
		...
		DEBUG_OUT (3,("checking for home directory %s gave %s\n",*p_service_out,
	...
	if (iService < 0) {
		...
		if (iPrinterService >= 0) {
			DEBUG_OUT (3,("checking whether %s is a valid printer name...\n",
				*p_service_out));
			if (pcap_printername_ok(*p_service_out)) {
				DEBUG_OUT (3,("%s is a valid printer name\n",
					*p_service_out));
				DEBUG_OUT (3,("adding %s as a printer service\n",
					*p_service_out));
				lp_add_printer(*p_service_out, iPrinterService);
				iService = lp_servicenumber(*p_service_out);
				if (iService < 0) {
					DEBUG_OUT (0,("failed to add %s as a printer service!\n",
						*p_service_out));
				}
			} else {
				DEBUG_OUT (3,("%s is not a valid printer name\n",
					*p_service_out));
			}
		}
	}
	...
	if (iService >= 0) {
		if (!VALID_SNUM(iService)) {
			DEBUG_OUT (0,("Invalid snum %d for %s\n",iService,
				*p_service_out));
			iService = -1;
		}
	}
  ...
  fail:
	...
	if (iService < 0) {
		DEBUG_OUT (3,("find_service() failed to find service %s\n",
			*p_service_out));
	}
				  ...
				  gid_t *pgid)
{
	...
	if (groupname == NULL) {
		DEBUG_OUT (1, ("talloc_string_sub failed\n"));
		result = NT_STATUS_NO_MEMORY;
		goto done;
	}
	...
	if (!lookup_name_smbconf(talloc_tos(), groupname,
		...
		DEBUG_OUT (10, ("lookup_name_smbconf(%s) failed\n",
			   groupname));
		goto done;
	}
	...
	if ((type != SID_NAME_DOM_GRP) && (type != SID_NAME_ALIAS) &&
		...
		DEBUG_OUT (10, ("%s is a %s, not a group\n", groupname,
			   sid_type_lookup(type)));
		goto done;
	}

	if (!sid_to_gid(&group_sid, &gid)) {
		DEBUG_OUT (10, ("sid_to_gid(%s) for %s failed\n",
	...
	if (force_user && user_must_be_member) {
		if (user_in_group_sid(username, &group_sid)) {
			...
			*pgid = gid;
			DEBUG_OUT (3,("Forced group %s for member %s\n",
				 groupname, username));
		} else {
			DEBUG_OUT (0,("find_forced_group: forced user %s is not a member "
                                              ...
                                              struct auth_serversupplied_info **presult)
{
		...
		struct auth_serversupplied_info *result;
                ...
                if (vuid_serverinfo->guest) {
                        if (!lp_guest_ok(snum)) {
                                DEBUG_OUT (2, ("guest user (from session setup) "
					   ...
					   vuid_serverinfo->info3->base.domain.string,
                                ...
                                DEBUG_OUT (2, ("user '%s' (from session setup) not "
		...
		*presult = result;
		...
		bool guest;
                ...
                if (!authorise_login(sconn, snum,user,password,&guest)) {
                        DEBUG_OUT ( 2, ( "Invalid username/password for [%s]\n",
                                    lp_servicename(snum)) );
	...
	DEBUG_OUT (0, ("invalid VUID (vuser) but not in security=share\n"));
	return NT_STATUS_ACCESS_DENIED;
}
					...
					NTSTATUS *pstatus)
{
	...
	*pstatus = share_sanity_checks(&sconn->client_id, snum, dev);
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("create_connection_session_info failed: %s\n",
			  nt_errstr(status)));
		*pstatus = status;
		goto err_root_exit;
	}
...

	{
		...
		DEBUG_OUT (3,("Connect path is '%s' for service [%s]\n",s,
			 lp_servicename(snum)));
		TALLOC_FREE(s);
	}
	...
	if ((conn->share_access & FILE_WRITE_DATA) == 0) {
		if ((conn->share_access & FILE_READ_DATA) == 0) {
			...
			DEBUG_OUT (0,("make_connection: connection to %s "
	...
	if (!smbd_vfs_init(conn)) {
		DEBUG_OUT (0, ("vfs_init failed for service %s\n",
			  lp_servicename(snum)));
...
/* ROOT Activities: */
	...
	if ((lp_max_connections(snum) > 0)
		...
		DEBUG_OUT (1, ("Max connections (%d) exceeded for %s\n",
	...
	if (!claim_connection(conn, lp_servicename(snum))) {
		DEBUG_OUT (1, ("Could not store connections entry\n"));
	...
	if (SMB_VFS_CONNECT(conn, lp_servicename(snum),
		...
		DEBUG_OUT (0,("make_connection: VFS make connection failed!\n"));
	...
	if (*lp_rootpreexec(snum)) {
		...
		if (ret != 0 && lp_rootpreexec_close(snum)) {
			DEBUG_OUT (1,("root preexec gave %d - failing "
				 "connection\n", ret));
			*pstatus = NT_STATUS_ACCESS_DENIED;
			goto err_root_exit;
		}
...
/* USER Activites: */
	if (!change_to_user(conn, conn->vuid)) {
		...
		DEBUG_OUT (0,("Can't become connected user!\n"));
	...
	if (*lp_preexec(snum)) {
		...
		if (ret != 0 && lp_preexec_close(snum)) {
			DEBUG_OUT (1,("preexec gave %d - failing connection\n",
				 ret));
			*pstatus = NT_STATUS_ACCESS_DENIED;
			goto err_root_exit;
		}
...
/* ROOT Activites: */
	...
	if (!lp_widelinks(snum)) {
		if (!canonicalize_connect_path(conn)) {
			DEBUG_OUT (0, ("canonicalize_connect_path failed "
	...
	if ((ret = SMB_VFS_STAT(conn, smb_fname_cpath)) != 0 ||
		...
		if (ret == 0 && !S_ISDIR(smb_fname_cpath->st.st_ex_mode)) {
			DEBUG_OUT (0,("'%s' is not a directory, when connecting to "
				 "[%s]\n", conn->connectpath,
				 lp_servicename(snum)));
		} else {
			DEBUG_OUT (0,("'%s' does not exist or permission denied "
					...
					NTSTATUS *pstatus)
{
	...
	if (!conn) {
		DEBUG_OUT (0,("make_connection_smb1: Couldn't find free connection.\n"));
		*pstatus = NT_STATUS_INSUFFICIENT_RESOURCES;
		return NULL;
	}
					...
					NTSTATUS *pstatus)
{
	...
	connection_struct *conn = conn_new(sconn);
	if (!conn) {
		DEBUG_OUT (0,("make_connection_smb2: Couldn't find free connection.\n"));
		*pstatus = NT_STATUS_INSUFFICIENT_RESOURCES;
		return NULL;
	}
	conn->cnum = tcon->tid;
				   ...
				   NTSTATUS *status)
{
	...
	if (!non_root_mode() && (euid = geteuid()) != 0) {
		DEBUG_OUT (0,("make_connection: PANIC ERROR. Called as nonroot "
			 "(%u)\n", (unsigned int)euid ));
	...
	if(lp_security() != SEC_SHARE) {
		...
		if (!vuser) {
			DEBUG_OUT (1,("make_connection: refusing to connect with "
				 "no session setup\n"));
	...
	*/
	...
	if (strequal(service_in,HOMES_NAME)) {
		if(lp_security() != SEC_SHARE) {
			...
			if (vuser->homes_snum == -1) {
				DEBUG_OUT (2, ("[homes] share not available for "
			...
			DEBUG_OUT (5, ("making a connection to [homes] service "
			...
			if (snum != -1) {
				DEBUG_OUT (5, ("making a connection to 'homes' "
					  "service %s based on "
					  "security=share\n", service_in));
				return make_connection_smb1(sconn,
							    snum, NULL,
		...
		DATA_BLOB no_pw = data_blob_null;
		DEBUG_OUT (5, ("making a connection to 'homes' service [%s] "
			  "created at session setup time\n", service_in));
		return make_connection_smb1(sconn,
					    vuser->homes_snum,
					    vuser, no_pw, 
	...
	if (snum < 0) {
		if (strequal(service,"IPC$") ||
			...
			DEBUG_OUT (3,("refusing IPC connection to %s\n", service));
			*status = NT_STATUS_ACCESS_DENIED;
			return NULL;
		}

		DEBUG_OUT (3,("%s (%s) couldn't find service %s\n",
	...
	if (lp_host_msdfs() && (*lp_msdfs_proxy(snum) != '\0'))  {
		DEBUG_OUT (3, ("refusing connection to dfs proxy share '%s' "
			  "(pointing to %s)\n", 
	...
	DEBUG_OUT (5, ("making a connection to 'normal' service %s\n", service));
...
void close_cnum(connection_struct *conn, uint16 vuid)
{
	...
	DEBUG_OUT (IS_IPC(conn)?3:1, ("%s (%s) closed connection to service %s\n",
				 get_remote_machine_name(),
				 conn->sconn->client_id.addr,
				 lp_servicename(SNUM(conn))));
