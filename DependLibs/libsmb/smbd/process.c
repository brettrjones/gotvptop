...
static bool smbd_lock_socket_internal(struct smbd_server_connection *sconn)
{
	...
	DEBUG_OUT (10,("pid[%d] wait for socket lock\n", (int)sys_getpid()));
	...
	if (!ok) {
		DEBUG_OUT (1, ("fcntl_lock failed: %s\n", strerror(errno)));
		return false;
	}

	DEBUG_OUT (10,("pid[%d] got for socket lock\n", (int)sys_getpid()));
...
static bool smbd_unlock_socket_internal(struct smbd_server_connection *sconn)
{
	...
	DEBUG_OUT (10,("pid[%d] unlocked socket\n", (int)sys_getpid()));
		  ...
		  struct smb_perfcount_data *pcd)
{
	...
	if (do_encrypt) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("send_smb: SMB encryption failed "
	...
	if (ret <= 0) {
		 ...
		 * client failed.
		 */
		DEBUG_OUT (1,("pid[%d] Error writing %d bytes to client %s. %d. (%s)\n",
...
static bool valid_packet_size(size_t len)
{
	...
	if (len > (BUFFER_SIZE + LARGE_WRITEX_HDR_SIZE)) {
		DEBUG_OUT (0,("Invalid packet length! (%lu bytes).\n",
					(unsigned long)len));
		return false;
	}
	return true;
				      ...
				      unsigned int timeout, ssize_t len)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		char addr[INET6_ADDRSTRLEN];
		DEBUG_OUT (0, ("read_fd_with_timeout failed for client %s read "
			  "error = %s.\n",
						    ...
						    size_t *len_ret)
{
	...
	if (is_valid_writeX_buffer(sconn, (uint8_t *)writeX_header)) {
		...
		*buffer = (char *)TALLOC_MEMDUP(mem_ctx,
		...
		if (*buffer == NULL) {
			DEBUG_OUT (0, ("Could not allocate inbuf of length %d\n",
	...
	*buffer = TALLOC_ARRAY(mem_ctx, char, len+4);
	...
	if(toread > 0) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("receive_smb_raw_talloc_partial_read: %s\n",
				   nt_errstr(status)));
			return status;
		}
	}
				   ...
				   bool trusted_channel)
{
	...
	*p_encrypted = false;
	...
	status = receive_smb_raw_talloc(mem_ctx, sconn, sock, buffer, timeout,
					p_unread, &len);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (NT_STATUS_EQUAL(status, NT_STATUS_END_OF_FILE)?5:1,
	...
	if (is_encrypted_packet((uint8_t *)*buffer)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("receive_smb_talloc: SMB decryption failed on "
	...
	if (!srv_check_sign_mac(sconn, *buffer, seqnum, trusted_channel)) {
		DEBUG_OUT (0, ("receive_smb: SMB Signature verification failed on "
			  "incoming packet!\n"));
			     ...
			     uint32_t seqnum)
{
	...
	if (req_size < smb_size) {
		DEBUG_OUT (0,("init_smb_request: invalid request size %u\n",
	...
	if (smb_size + req->wct*2 > req_size) {
		DEBUG_OUT (0,("init_smb_request: invalid wct number %u (size %u)\n",
	...
	if (((uint8 *)smb_buf(inbuf)) + req->buflen > inbuf + req_size) {
		DEBUG_OUT (0,("init_smb_request: invalid bcc number %u "
			"(wct = %u, size %u)\n",
				     ...
				     void *private_data)
{
	...
	DEBUG_OUT (5,("smbd_deferred_open_timer: trigger mid %llu.\n",
				...
				char *private_data, size_t private_len)
{
	...
	if(msg == NULL) {
		DEBUG_OUT (0,("push_message: malloc fail (1)\n"));
		return False;
	}

	msg->buf = data_blob_talloc(msg, req->inbuf, msg_len);
	if(msg->buf.data == NULL) {
		DEBUG_OUT (0,("push_message: malloc fail (2)\n"));
		TALLOC_FREE(msg);
		return False;
	}
	...
	if (private_data) {
		...
		if (msg->private_data.data == NULL) {
			DEBUG_OUT (0,("push_message: malloc fail (3)\n"));
			TALLOC_FREE(msg);
			return False;
		}
	}
	...
	msg->te = event_add_timed(smbd_event_context(),
				  ...
				  msg);
	if (!msg->te) {
		DEBUG_OUT (0,("push_message: event_add_timed failed\n"));
		TALLOC_FREE(msg);
		return false;
	}
	...
	DEBUG_OUT (10,("push_message: pushed message length %u on "
...
void remove_deferred_open_message_smb(uint64_t mid)
{
	...
	for (pml = deferred_open_queue; pml; pml = pml->next) {
		if (mid == (uint64_t)SVAL(pml->buf.data,smb_mid)) {
			DEBUG_OUT (10,("remove_deferred_open_message_smb: "
...
void schedule_deferred_open_message_smb(uint64_t mid)
{
	...
	for (pml = deferred_open_queue; pml; pml = pml->next) {
		...
		DEBUG_OUT (10,("schedule_deferred_open_message_smb: [%d] "
			"msg_mid = %llu\n",
			i++,
		...
		if (mid == msg_mid) {
			...
			if (pml->processed) {
				 ...
				 * rescheduled. */
				DEBUG_OUT (0,("schedule_deferred_open_message_smb: LOGIC ERROR "
					"message mid %llu was already processed\n",
					(unsigned long long)msg_mid ));
				continue;
			}

			DEBUG_OUT (10,("schedule_deferred_open_message_smb: "
				"scheduling mid %llu\n",
				(unsigned long long)mid ));
	...
	DEBUG_OUT (10,("schedule_deferred_open_message_smb: failed to "
			       ...
			       char *private_data, size_t priv_len)
{
	...
	if (req->unread_bytes) {
		DEBUG_OUT (0,("push_deferred_open_message_smb: logic error ! "
			"unread_bytes = %u\n",
	...
	DEBUG_OUT (10,("push_deferred_open_message_smb: pushing message "
				    ...
				    void *private_data)
{
	...
	DEBUG_OUT (10,("smbd_idle_event_handler: %s %p called\n",
		  event->name, event->te));

	if (!event->handler(&now, event->private_data)) {
		DEBUG_OUT (10,("smbd_idle_event_handler: %s %p stopped\n",
			  event->name, event->te));
	...
	DEBUG_OUT (10,("smbd_idle_event_handler: %s %p rescheduled\n",
		  event->name, event->te));
				  ...
				  void *private_data)
{
	...
	if (result->te == NULL) {
		DEBUG_OUT (0, ("event_add_timed failed\n"));
		TALLOC_FREE(result);
		return NULL;
	}

	DEBUG_OUT (10,("event_add_idle: %s %p\n", result->name, result->te));
	return result;
}
				  ...
				  void *private_data)
{
	...
	change_to_root_user();
	DEBUG_OUT (1,("Reloading services after SIGHUP\n"));
...
static void smb_dump(const char *name, int type, const char *data, ssize_t len)
{
	...
	if (fd != -1) {
		...
		if (ret != len)
			DEBUG_OUT (0,("smb_dump: problem: write returned %d\n", (int)ret ));
		close(fd);
		DEBUG_OUT (0,("created %s len %lu\n", fname, (unsigned long)len));
	}
	SAFE_FREE(fname);
}
...
static connection_struct *switch_message(uint8 type, struct smb_request *req, int size)
{
	...
	if ((size < (smb_size - 4)) ||
		...
		DEBUG_OUT (2,("Non-SMB packet of length %d. Terminating server\n",
			 smb_len(req->inbuf)));
		exit_server_cleanly("Non-SMB packet");
	}

	if (smb_messages[type].fn == NULL) {
		DEBUG_OUT (0,("Unknown message type %d!\n",type));
	...
	DEBUG_OUT (3,("switch message %s (pid %d) conn 0x%lx\n", smb_fn_name(type),
	...
	if (flags & AS_USER) {
		...
		if (!change_to_user(conn,session_tag)) {
			DEBUG_OUT (0, ("Error: Could not change to user. Removing "
			...
			struct smb_perfcount_data *deferred_pcd)
{
	...
	DEBUG_OUT ( 6, ( "got message type 0x%x of len 0x%x\n", msg_type,
		    smb_len(inbuf) ) );
	DEBUG_OUT (3, ("Transaction %d of length %d (%u toread)\n",
			     ...
			     uint32_t num_bytes, const uint8_t *bytes)
{
	...
	if ((smb_command != SMBwriteX) && (new_size > 0xffff)) {
		DEBUG_OUT (1, ("splice_chain: %u bytes won't fit\n",
			  (unsigned)new_size));
		return false;
	}
	...
	*poutbuf = outbuf;
	...
	if (first_request) {
		...
		if (!find_andx_cmd_ofs(outbuf, &andx_cmd_ofs)) {
			DEBUG_OUT (1, ("invalid command chain\n"));
	...
	struct smbd_server_connection *conn, int fd)
{
	...
	if (from_client) {
		...
		if (lp_async_smb_echo_handler() && !fd_is_readable(fd)) {
			DEBUG_OUT (10,("the echo listener was faster\n"));
...
static void release_ip(const char *ip, void *priv)
{
	...
	DEBUG_OUT (10, ("Got release IP message for %s, "
		   "our address is %s\n", ip, p));
	...
	if ((strcmp(p, ip) == 0) || ((p != addr) && strcmp(addr, ip) == 0)) {
		/* we can't afford to do a clean exit - that involves
		   ...
		   away */
		DEBUG_OUT (0,("Got release IP message for our IP %s - exiting immediately\n",
			ip));
...
static bool keepalive_fn(const struct timeval *now, void *private_data)
{
	...
	if (!ret) {
		 ...
		 * client failed.
		 */
		DEBUG_OUT (0, ("send_keepalive failed for client %s. "
			  "Error %s - exiting\n",
			  get_peer_addr(sconn->sock, addr, sizeof(addr)),
			  strerror(errno)));
		return False;
...
static bool deadtime_fn(const struct timeval *now, void *private_data)
{
	...
	if ((conn_num_open(sconn) == 0)
		...
		DEBUG_OUT ( 2, ( "Closing idle connection\n" ) );
...
static void smbd_echo_activate_writer(struct smbd_echo_state *state)
{
	...
	if (state->write_req == NULL) {
		DEBUG_OUT (1, ("writev_send failed\n"));
		exit(1);
	}
...
static void smbd_echo_writer_done(struct tevent_req *req)
{
	...
	if (written == -1) {
		DEBUG_OUT (1, ("writev to parent failed: %s\n", strerror(err)));
		exit(1);
	}
	DEBUG_OUT (10,("echo_handler[%d]: forwarded pdu to main\n", (int)sys_getpid()));
			    ...
			    uint32_t seqnum)
{
	...
	if ((inbuf_len == 4) && (CVAL(inbuf, 0) == SMBkeepalive)) {
		DEBUG_OUT (10, ("Got netbios keepalive\n"));
		/*
		 * Just swallow it
		 */
		return true;
	...
	if (inbuf_len < smb_size) {
		DEBUG_OUT (10, ("Got short packet: %d bytes\n", (int)inbuf_len));
		return false;
	}
	if (!valid_smb_header(inbuf)) {
		DEBUG_OUT (10, ("Got invalid SMB header\n"));
		return false;
	}
	...
	DEBUG_OUT (10, ("smbecho handler got cmd %d (%s)\n", (int)req.cmd,
	...
	if (!create_outbuf(talloc_tos(), &req, (char *)req.inbuf, &outbuf,
			   1, req.buflen)) {
		DEBUG_OUT (10, ("create_outbuf failed\n"));
		return false;
	}
	req.outbuf = (uint8_t *)outbuf;
...
static void smbd_echo_exit(struct tevent_context *ev,
			   ...
			   void *private_data)
{
	DEBUG_OUT (2, ("smbd_echo_exit: lost connection to parent\n"));
	exit(0);
}
			     ...
			     void *private_data)
{
	...
	if (!ok) {
		DEBUG_OUT (0, ("%s: failed to lock socket\n",
			__location__));
		exit(1);
	}

	if (!fd_is_readable(sconn->sock)) {
		DEBUG_OUT (10,("echo_handler[%d] the parent smbd was faster\n",
			  (int)sys_getpid()));
		ok = smbd_unlock_socket_internal(sconn);
		if (!ok) {
			DEBUG_OUT (1, ("%s: failed to unlock socket in\n",
				__location__));
			exit(1);
		}
		return;
	...
	tmp = talloc_realloc(state, state->pending, struct iovec,
			     num_pending+1);
	if (tmp == NULL) {
		DEBUG_OUT (1, ("talloc_realloc failed\n"));
		exit(1);
	}
	state->pending = tmp;

	DEBUG_OUT (10,("echo_handler[%d]: reading pdu\n", (int)sys_getpid()));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("echo_handler[%d]: receive_smb_raw_talloc failed: %s\n",
	...
	reply = smbd_echo_reply((uint8_t *)state->pending[num_pending].iov_base,
				...
				seqnum);
	if (reply) {
		DEBUG_OUT (10,("echo_handler[%d]: replied to client\n", (int)sys_getpid()));
	...
	DEBUG_OUT (10,("echo_handler[%d]: forward to main\n", (int)sys_getpid()));
	smbd_echo_activate_writer(state);
}
			   ...
			   int parent_pipe)
{
	...
	if (state->ev == NULL) {
		DEBUG_OUT (1, ("tevent_context_init failed\n"));
		TALLOC_FREE(state);
		return;
	}
	...
	if (state->parent_fde == NULL) {
		DEBUG_OUT (1, ("tevent_add_fd failed\n"));
		TALLOC_FREE(state);
		return;
	}
	...
	while (true) {
		if (tevent_loop_once(state->ev) == -1) {
			DEBUG_OUT (1, ("tevent_loop_once failed: %s\n",
				  strerror(errno)));
			break;
		}
	}
...
bool fork_echo_handler(struct smbd_server_connection *sconn)
{
	...
	if (child == 0) {
		...
		status = reinit_after_fork(sconn->msg_ctx,
					   ...
					   procid_self(), false);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("reinit_after_fork failed: %s\n",
				  nt_errstr(status)));
			exit(1);
		}
	...
	DEBUG_OUT (10,("fork_echo_handler: main[%d] echo_child[%d]\n", (int)sys_getpid(), child));
	...
	if (sconn->smb1.echo_handler.trusted_fde == NULL) {
		DEBUG_OUT (1, ("event_add_fd failed\n"));
		goto fail;
	}
...
void smbd_process(struct smbd_server_connection *sconn)
{
	...
	if (ret != 0) {
		...
		DEBUG_OUT (level,("getpeername() failed - %s\n", strerror(errno)));
	...
	ret = tsocket_address_bsd_from_sockaddr(sconn,
						...
						&remote_address);
	if (ret != 0) {
		DEBUG_OUT (0,("%s: tsocket_address_bsd_from_sockaddr remote failed - %s\n",
	...
	if (ret != 0) {
		int level = (errno == ENOTCONN)?2:0;
		DEBUG_OUT (level,("getsockname() failed - %s\n", strerror(errno)));
	...
	if (!allow_access(lp_hostsdeny(-1), lp_hostsallow(-1),
		...
		DEBUG_OUT ( 1, ("Connection denied from %s to %s\n",
	...
	DEBUG_OUT (10, ("Connection allowed from %s to %s\n",
		   tsocket_address_string(remote_address, talloc_tos()),
		   tsocket_address_string(local_address, talloc_tos())));
	...
	if (*lp_rootdir()) {
		if (chroot(lp_rootdir()) != 0) {
			DEBUG_OUT (0,("Failed to change root to %s\n", lp_rootdir()));
			exit_server("Failed to chroot()");
		}
		if (chdir("/") == -1) {
			DEBUG_OUT (0,("Failed to chdir to / on chroot to %s\n", lp_rootdir()));
			exit_server("Failed to chroot()");
		}
		DEBUG_OUT (0,("Changed root to %s\n", lp_rootdir()));
	}
	...
	if ((lp_keepalive() != 0)
	    && !(event_add_idle(smbd_event_context(), NULL,
				...
				NULL))) {
		DEBUG_OUT (0, ("Could not add keepalive event\n"));
		exit(1);
	}
	...
	if (!(event_add_idle(smbd_event_context(), NULL,
		...
		DEBUG_OUT (0, ("Could not add deadtime event\n"));
		exit(1);
	}
	...
	if (!(event_add_idle(smbd_event_context(), NULL,
		...
		DEBUG_OUT (0, ("Could not add housekeeping event\n"));
		exit(1);
	}
	...
	if (lp_clustering()) {
		...
		if (client_get_tcp_info(sconn->sock, &srv, &clnt) == 0) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (0, ("ctdbd_register_ips failed: %s\n",
					  nt_errstr(status)));
			}
		} else
		{
			DEBUG_OUT (0,("Unable to get tcp info for "
	...
	while (True) {
		...
		if (!NT_STATUS_EQUAL(status, NT_STATUS_RETRY) &&
			...
			DEBUG_OUT (3, ("smbd_server_connection_loop_once failed: %s,"
				  " exiting\n", nt_errstr(status)));
			break;
		}
