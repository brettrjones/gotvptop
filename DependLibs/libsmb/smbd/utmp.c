		...
		const char *uw_default)
{
	...
	if (strlen(uw_default) == 0) {
		 ...
		 */
		DEBUG_OUT (2,("uw_pathname: unable to determine pathname\n"));
	}
...
{
	DEBUG_OUT (1,("pututline_my: not yet implemented\n"));
...
static void utmp_nox_update(struct utmp *u, bool claim)
{
	...
	DEBUG_OUT (2,("utmp_nox_update: uname:%s\n", uname));
...
# if defined(PUTUTLINE_RETURNS_UTMP)
	...
	endutent();
	if (urc == NULL) {
		DEBUG_OUT (2,("utmp_nox_update: pututline() failed\n"));
		return;
	}
...
# endif	/* PUTUTLINE_RETURNS_UTMP */
	...
	DEBUG_OUT (2,("utmp_nox_update: wname:%s\n", wname));
	if (strlen(wname) != 0) {
#ifdef HAVE_UPDWTMP
		updwtmp(wname, u);
		/*
...
static void sys_utmp_update(struct utmp *u, const char *hostname, bool claim)
{
	...
	DEBUG_OUT (1,("utmp_update: have utmpx.h but no pututxline() function\n"));
	utmp_nox_update(u, claim);
#elif !defined(HAVE_GETUTMPX)
	/* Odd.  Have utmpx.h but no "getutmpx()".  Drop to non-x stuff */
	DEBUG_OUT (1,("utmp_update: have utmpx.h but no getutmpx() function\n"));
	utmp_nox_update(u, claim);
#elif !defined(HAVE_UPDWTMPX)
	/* Have utmpx.h but no "updwtmpx()".  Drop to non-x stuff */
	DEBUG_OUT (1,("utmp_update: have utmpx.h but no updwtmpx() function\n"));
	utmp_nox_update(u, claim);
#else
	char *uname = NULL;
	char *wname = NULL;
	...
	if (uname && wname) {
		DEBUG_OUT (2,("utmp_update: uname:%s wname:%s\n", uname, wname));
	}
	...
	if (!uname || !wname || (strlen(uname) == 0) || (strlen(wname) == 0)) {
		...
		endutxent();
		if (uxrc == NULL) {
			DEBUG_OUT (2,("utmp_update: pututxline() failed\n"));
			return;
		}
		updwtmpx(wname, &ux);
	}
			...
			const char *id_str, int id_num)
{
	...
	if (strlen(id_str) > sizeof(u->ut_line)) {
		DEBUG_OUT (1,("id_str [%s] is too long for %lu char utmp field\n",
...
#error "with-utmp must have UT_TIME or UT_TV"
	...
	if (ut_id_encode(id_num, u->ut_id) != 0) {
		DEBUG_OUT (1,("utmp_fill: cannot encode id %d\n", id_num));
		return False;
	}
#endif
