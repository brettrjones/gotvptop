                              ...
                              uint64_t *bsize,uint64_t *dfree,uint64_t *dsize)
{
	...
	if (dfree_command && *dfree_command) {
		...
		DEBUG_OUT (3, ("disk_free: Running command %s\n", syscmd));

		lines = file_lines_pload(syscmd, NULL);
		if (lines) {
			char *line = lines[0];

			DEBUG_OUT (3, ("Read input from dfree, \"%s\"\n", line));
			...
			*dsize = STR_TO_SMB_BIG_UINT(line, &p);
			...
			if (p && *p)
				...
				*bsize = 1024;
			TALLOC_FREE(lines);
			DEBUG_OUT (3, ("Parsed output of dfree, dsize=%u, dfree=%u, bsize=%u\n",
			...
			DEBUG_OUT (0, ("disk_free: sys_popen() failed for command %s. Error was : %s\n",
				syscmd, strerror(errno) ));
			if (sys_fsusage(path, dfree, dsize) != 0) {
				DEBUG_OUT (0, ("disk_free: sys_fsusage() failed. Error was : %s\n",
					strerror(errno) ));
				return (uint64_t)-1;
			}
		}
	...
	if (*bsize < 256) {
		DEBUG_OUT (5,("disk_free:Warning: bsize == %d < 256 . Changing to assumed correct bsize = 512\n",(int)*bsize));
		*bsize = 512;
	}

	if ((*dsize)<1) {
		if (!dfree_broken) {
			DEBUG_OUT (0,("WARNING: dfree is broken on this system\n"));
			dfree_broken=true;
		}
