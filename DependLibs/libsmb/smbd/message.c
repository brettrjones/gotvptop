...
static void msg_deliver(struct msg_state *state)
{
	...
	if (! (*lp_msg_command())) {
		DEBUG_OUT (1,("no messaging command specified\n"));
		goto done;
	}
	...
	if (fd == -1) {
		DEBUG_OUT (1, ("can't open message file %s: %s\n", name,
			  strerror(errno)));
		goto done;
	}
	...
	if (!convert_string_talloc(talloc_tos(), CH_DOS, CH_UNIX, state->msg,
				   ...
				   &len, true)) {
		DEBUG_OUT (3, ("Conversion failed, delivering message in DOS "
			  "codepage format\n"));
		msg = state->msg;
	}
	...
	for (i = 0; i < len; i++) {
		...
		sz = write(fd, &msg[i], 1);
		if ( sz != 1 ) {
			DEBUG_OUT (0, ("Write error to fd %d: %ld(%s)\n", fd,
...
void reply_sendstrt(struct smb_request *req)
{
	...
	DEBUG_OUT ( 3, ( "SMBsendstrt (from %s to %s)\n", smbd_msg_state->from,
		    smbd_msg_state->to ) );
