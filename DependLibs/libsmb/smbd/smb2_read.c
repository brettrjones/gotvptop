...
NTSTATUS smbd_smb2_request_process_read(struct smbd_smb2_request *req)
{
	...
	if (in_length > req->sconn->smb2.max_read) {
		DEBUG_OUT (0,("here:%s: 0x%08X: 0x%08X\n",
...
static int smb2_sendfile_send_data(struct smbd_smb2_read_state *state)
{
	...
	nread = SMB_VFS_SENDFILE(fsp->conn->sconn->sock,
					...
					in_offset,
					in_length);
	DEBUG_OUT (10,("smb2_sendfile_send_data: SMB_VFS_SENDFILE returned %d on file %s\n",
		(int)nread,
		fsp_str_dbg(fsp) ));
	...
	if (nread == -1) {
		if (errno == ENOSYS || errno == EINTR) {
			...
			*/
			...
			if (nread == -1) {
				DEBUG_OUT (0,("smb2_sendfile_send_data: "
		...
		DEBUG_OUT (0,("smb2_sendfile_send_data: sendfile failed for file "
			"%s (%s). Terminating\n",
			fsp_str_dbg(fsp),
			strerror(errno)));
...
NTSTATUS smb2_read_complete(struct tevent_req *req, ssize_t nread, int err)
{
	...
	if (nread < 0) {
		...
		DEBUG_OUT ( 3,( "smb2_read_complete: file %s nread = %d. "
			"Error = %s (NTSTATUS %s)\n",
			fsp_str_dbg(fsp),
			(int)nread,
			strerror(err),
	...
	if (nread == 0 && state->in_length != 0) {
		DEBUG_OUT (5,("smb2_read_complete: read_file[%s] end of file\n",
			fsp_str_dbg(fsp)));
		return NT_STATUS_END_OF_FILE;
	}

	if (nread < state->in_minimum) {
		DEBUG_OUT (5,("smb2_read_complete: read_file[%s] read less %d than "
	...
	DEBUG_OUT (3,("smbd_smb2_read: fnum=[%d/%s] length=%lu offset=%lu read=%lu\n",
		fsp->fnum,
		fsp_str_dbg(fsp),
					      ...
					      uint32_t in_remaining)
{
	...
	DEBUG_OUT (10,("smbd_smb2_read: %s - fnum[%d]\n",
	...
	DEBUG_OUT (10,("smbd_smb2_read: file %s fnum[%d] offset=%llu "
