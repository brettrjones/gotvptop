			...
			switch(siz) {
				...
				case 1:
					*d++ = *s++;
					break;
				default:
					DEBUG_OUT (0,("check_path_syntax_internal: character length assumptions invalid !\n"));
					*d = '\0';
				     ...
				     const char *name, int name_type)
{
	...
	DEBUG_OUT (10, ("retargeting %s to %s\n", trim_name_type, retarget));
	...
	if (!ret) {
		DEBUG_OUT (10, ("could not resolve %s\n", retarget));
		goto fail;
	}

	if (retarget_addr.ss_family != AF_INET) {
		DEBUG_OUT (10, ("Retarget target not an IPv4 addr\n"));
		goto fail;
	}
...
void reply_special(struct smbd_server_connection *sconn, char *inbuf, size_t inbuf_size)
{
	...
	case 0x81: /* session request */
	{
		...
		*name1 = *name2 = 0;
		...
		if (name_len1 <= 0 || name_len1 > inbuf_size - 4) {
			DEBUG_OUT (0,("Invalid name length in session request\n"));
			break;
		}
		...
		if (name_type1 == -1 || name_type2 == -1) {
			DEBUG_OUT (0,("Invalid name type in session request\n"));
			break;
		}

		DEBUG_OUT (2,("netbios connect: name1=%s0x%x name2=%s0x%x\n",
		...
		DEBUG_OUT (2,("netbios connect: local=%s remote=%s, name type = %x\n",
	...
	case 0x84: /* retarget session response */
		DEBUG_OUT (0,("Unexpected session response\n"));
		break;
	...
	DEBUG_OUT (5,("init msg_type=0x%x msg_flags=0x%x\n",
		    msg_type, msg_flags));
...
void reply_tcon(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("tcon service=%s cnum=%d\n",
		 service, conn->cnum));
...
void reply_tcon_and_X(struct smb_request *req)
{
	...
	DEBUG_OUT (4,("Client requested device type [%s] for share [%s]\n", client_devicetype, service));
	...
	if (get_Protocol() < PROTOCOL_NT1) {
		...
		if (lp_msdfs_root(SNUM(conn)) && lp_host_msdfs()) {
			DEBUG_OUT (2,("Serving %s as a Dfs root\n",
...
{
	DEBUG_OUT (0, ("unknown command type (%s): type=%d (0x%X)\n",
...
void reply_ioctl(struct smb_request *req)
{
	...
	DEBUG_OUT (4, ("Received IOCTL (code 0x%x)\n", ioctl_code));
...
void reply_checkpath(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_checkpath %s mode=%d\n", name, (int)SVAL(req->vwv+0, 0)));
	...
	if (!VALID_STAT(smb_fname->st) &&
		...
		DEBUG_OUT (3,("reply_checkpath: stat of %s failed (%s)\n",
...
void reply_getatr(struct smb_request *req)
{
	...
	if (*fname == '\0') {
		...
		if (!VALID_STAT(smb_fname->st) &&
		    (SMB_VFS_STAT(conn, smb_fname) != 0)) {
			DEBUG_OUT (3,("reply_getatr: stat of %s failed (%s)\n",
	...
	DEBUG_OUT (3,("reply_getatr: name=%s mode=%d size=%u\n",
...
void reply_setatr(struct smb_request *req)
{
	...
	DEBUG_OUT (3, ("setatr name=%s mode=%d\n", smb_fname_str_dbg(smb_fname),
		 mode));
 out:
	TALLOC_FREE(smb_fname);
...
void reply_search(struct smb_request *req)
{
  ...
  SearchEmpty:
	...
	DEBUG_OUT (4,("%s mask=%s path=%s dtype=%d nument=%u of %u\n",
		smb_fn_name(req->cmd),
		mask,
		directory,
		dirtype,
...
void reply_open(struct smb_request *req)
{
	...
	if (fattr & FILE_ATTRIBUTE_DIRECTORY) {
		DEBUG_OUT (3,("attempt to open a directory %s\n",
			 fsp_str_dbg(fsp)));
...
void reply_ulogoffX(struct smb_request *req)
{
	...
	if(vuser == NULL) {
		DEBUG_OUT (3,("ulogoff, vuser id %d does not map to user.\n",
			 req->vuid));
	}
...
void reply_mknew(struct smb_request *req)
{
	...
	if (fattr & FILE_ATTRIBUTE_VOLUME) {
		DEBUG_OUT (0,("Attempt to create file (%s) with volid set - "
			 "please report this\n",
			 smb_fname_str_dbg(smb_fname)));
	}
	...
	DEBUG_OUT (2, ("reply_mknew: file %s\n", smb_fname_str_dbg(smb_fname)));
	DEBUG_OUT (3, ("reply_mknew %s fd=%d dmode=0x%x\n",
...
void reply_ctemp(struct smb_request *req)
{
	...
	DEBUG_OUT (2, ("reply_ctemp: created temp file %s\n", fsp_str_dbg(fsp)));
	DEBUG_OUT (3, ("reply_ctemp %s fd=%d umode=0%o\n", fsp_str_dbg(fsp),
			...
			uint32 dirtype)
{
	...
	DEBUG_OUT (10,("do_unlink: %s, dirtype = %d\n",
		  smb_fname_str_dbg(smb_fname),
		  dirtype));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("SMB_VFS_CREATEFILE failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("do_unlink can_set_delete_on_close for file %s - "
			"(%s)\n",
			smb_fname_str_dbg(smb_fname),
			nt_errstr(status)));
	...
	if (!has_wild) {
		...
		*/
		...
		while ((dname = ReadDirName(dir_hnd, &offset,
			...
			count++;
			DEBUG_OUT (3,("unlink_internals: successful unlink [%s]\n",
				 smb_fname->base_name));
...
ssize_t fake_sendfile(files_struct *fsp, SMB_OFF_T startpos, size_t nread)
{
	...
	while (tosend > 0) {
		...
		if (write_data(fsp->conn->sconn->sock, buf, cur_read)
			 ...
			 * client failed.
			 */
			DEBUG_OUT (0, ("write_data failed for client %s. "
				  "Error %s\n",
...
#define SHORT_SEND_BUFSIZE 1024
	if (nread < headersize) {
		DEBUG_OUT (0,("sendfile_short_send: sendfile failed to send "
	...
	if (nread < smb_maxcnt) {
		...
		DEBUG_OUT (0,("sendfile_short_send: filling truncated file %s "
			"with zeros !\n", fsp_str_dbg(fsp)));
	...
	if ( !req_is_in_chain(req) && (nread > 0) && (fsp->base_fsp == NULL) &&
		...
		if (sendfile_read == -1) {
			...
			if (errno == EINTR) {
				...
				DEBUG_OUT (0,("send_file_readbraw: sendfile not available. Faking..\n"));

				if (fake_sendfile(fsp, startpos, nread) == -1) {
					DEBUG_OUT (0,("send_file_readbraw: "
			...
			DEBUG_OUT (0,("send_file_readbraw: sendfile failed for "
			 ...
			 */
			DEBUG_OUT (3, ("send_file_readbraw: sendfile sent zero "
...
normal_readbraw:
	...
	if (!outbuf) {
		DEBUG_OUT (0,("send_file_readbraw: TALLOC_ARRAY failed for size %u.\n",
			(unsigned)(nread+4)));
...
void reply_readbraw(struct smb_request *req)
{
	...
	if (sconn->smb1.echo_handler.trusted_fde) {
		DEBUG_OUT (2,("SMBreadbraw rejected with NOT_SUPPORTED because of "
	...
	if (!fsp || !conn || conn != fsp->conn ||
		 ...
		 */
		DEBUG_OUT (3,("reply_readbraw: fnum %d not valid "
			"- cache prime?\n",
	...
	if (!(fsp->can_read ||
		...
		DEBUG_OUT (3,("reply_readbraw: fnum %d not readable.\n",
				(int)SVAL(req->vwv+0, 0)));
		reply_readbraw_error(sconn);
		END_PROFILE(SMBreadbraw);
		return;
	...
	if(req->wct == 10) {
		...
		if(IVAL(req->vwv+8, 0) != 0) {
			DEBUG_OUT (0,("reply_readbraw: large offset "
		...
		if(startpos < 0) {
			DEBUG_OUT (0,("reply_readbraw: negative 64 bit "
				"readraw offset (%.0f) !\n",
				(double)startpos ));
			reply_readbraw_error(sconn);
			END_PROFILE(SMBreadbraw);
	...
	DEBUG_OUT ( 3, ( "reply_readbraw: fnum=%d start=%.0f max=%lu "
		"min=%lu nread=%lu\n",
	...
	DEBUG_OUT (5,("reply_readbraw finished\n"));
...
void reply_lockread(struct smb_request *req)
{
	...
	if (numtoread > sconn->smb1.negprot.max_recv) {
		DEBUG_OUT (0,("reply_lockread: requested read size (%u) is greater than maximum allowed (%u). \
Returning short read of maximum allowed for compatibility with Windows 2000.\n",
	...
	DEBUG_OUT (3,("lockread fnum=%d num=%d nread=%d\n",
...
void reply_read(struct smb_request *req)
{
	 ...
	 */
	if (numtoread > sconn->smb1.negprot.max_recv) {
		DEBUG_OUT (0,("reply_read: requested read size (%u) is greater than maximum allowed (%u). \
Returning short read of maximum allowed for compatibility with Windows 2000.\n",
			(unsigned int)numtoread,
			(unsigned int)sconn->smb1.negprot.max_recv));
		numtoread = MIN(numtoread, sconn->smb1.negprot.max_recv);
	...
	DEBUG_OUT ( 3, ( "read fnum=%d num=%d nread=%d\n",
		fsp->fnum, (int)numtoread, (int)nread ) );
	...
	if (!req_is_in_chain(req) &&
		...
		if (nread == -1) {
			...
			if (errno == EINTR) {
				/* Ensure we don't do this again. */
				set_use_sendfile(SNUM(conn), False);
				DEBUG_OUT (0,("send_file_readX: sendfile not available. Faking..\n"));
				nread = fake_sendfile(fsp, startpos,
						      smb_maxcnt);
				if (nread == -1) {
					DEBUG_OUT (0,("send_file_readX: "
						 "fake_sendfile failed for "
						 "file %s (%s).\n",
						 fsp_str_dbg(fsp),
						 strerror(errno)));
				...
				DEBUG_OUT ( 3, ( "send_file_readX: fake_sendfile fnum=%d max=%d nread=%d\n",
					fsp->fnum, (int)smb_maxcnt, (int)nread ) );
				/* No outbuf here means successful sendfile. */
				goto strict_unlock;
			}

			DEBUG_OUT (0,("send_file_readX: sendfile failed for file "
				 "%s (%s). Terminating\n", fsp_str_dbg(fsp),
				 strerror(errno)));
			 ...
			 * the correct byte count.
			 */
			DEBUG_OUT (3, ("send_file_readX: sendfile sent zero bytes "
				  "falling back to the normal read: %s\n",
				  fsp_str_dbg(fsp)));
			goto normal_read;
		}

		DEBUG_OUT ( 3, ( "send_file_readX: sendfile fnum=%d max=%d nread=%d\n",
			fsp->fnum, (int)smb_maxcnt, (int)nread ) );
...
normal_read:
	...
	if ((smb_maxcnt & 0xFF0000) > 0x10000) {
		...
		if (write_data(req->sconn->sock, (char *)headerbuf,
			...
			DEBUG_OUT (0,("send_file_readX: write_data failed for file "
				 "%s (%s). Terminating\n", fsp_str_dbg(fsp),
				 strerror(errno)));
			exit_server_cleanly("send_file_readX sendfile failed");
		}
		...
		if (nread == -1) {
			DEBUG_OUT (0,("send_file_readX: fake_sendfile failed for "
				 "file %s (%s).\n", fsp_str_dbg(fsp),
				 strerror(errno)));
			exit_server_cleanly("send_file_readX: fake_sendfile failed");
		}
...
nosendfile_read:
	...
	DEBUG_OUT ( 3, ( "send_file_readX fnum=%d max=%d nread=%d\n",
		    fsp->fnum, (int)smb_maxcnt, (int)nread ) );
...
void reply_read_and_X(struct smb_request *req)
{
	...
	if (req->wct == 12) {
		...
		if(IVAL(req->vwv+10, 0) != 0) {
			DEBUG_OUT (0,("reply_read_and_X - large offset (%x << 32) "
				...
				size_t *len)
{
	...
	while (msgtype == SMBkeepalive) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			DEBUG_OUT (0, ("read_fd_with_timeout failed for "
	...
	DEBUG_OUT (10,("read_smb_length: got smb length of %lu\n",
		  (unsigned long)len));
...
void reply_writebraw(struct smb_request *req)
{
	...
	if (req->sconn->smb1.echo_handler.trusted_fde) {
		DEBUG_OUT (2,("SMBwritebraw rejected with NOT_SUPPORTED because of "
			 "'async smb echo handler = yes'\n"));
		reply_nterror(req, NT_STATUS_NOT_SUPPORTED);
		error_to_writebrawerr(req);
		END_PROFILE(SMBwritebraw);
	...
	DEBUG_OUT (3,("reply_writebraw: initial write fnum=%d start=%.0f num=%d "
			"wrote=%d sync=%d\n",
	...
	if (numtowrite != 0) {
		...
		if (numtowrite > 0xFFFF) {
			DEBUG_OUT (0,("reply_writebraw: Oversize secondary write "
		...
		if (tcount > nwritten+numtowrite) {
			DEBUG_OUT (3,("reply_writebraw: Client overestimated the "
				"write %d %d %d\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("reply_writebraw: sync_file for %s returned %s\n",
	...
	DEBUG_OUT (3,("reply_writebraw: secondart write fnum=%d start=%.0f num=%d "
		"wrote=%d\n",
		fsp->fnum, (double)startpos, (int)numtowrite,
		(int)total_written));
...
void reply_writeunlock(struct smb_request *req)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("reply_writeunlock: sync_file for %s returned %s\n",
			 fsp_str_dbg(fsp), nt_errstr(status)));
		reply_nterror(req, status);
		goto strict_unlock;
	}
	...
	DEBUG_OUT (3,("writeunlock fnum=%d num=%d wrote=%d\n",
...
void reply_write(struct smb_request *req)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("reply_write: sync_file for %s returned %s\n",
			 fsp_str_dbg(fsp), nt_errstr(status)));
		reply_nterror(req, status);
		goto strict_unlock;
	}
	...
	DEBUG_OUT (3,("write fnum=%d num=%d wrote=%d\n", fsp->fnum, (int)numtowrite, (int)nwritten));
			    ...
			    const uint8_t *inbuf)
{
	...
	if (CVAL(inbuf,smb_vwv0) != 0xFF ||
		...
		DEBUG_OUT (10,("is_valid_writeX_buffer: chained or "
			"invalid word length.\n"));
		return false;
	}
	...
	if (conn == NULL) {
		DEBUG_OUT (10,("is_valid_writeX_buffer: bad tid\n"));
		return false;
	}
	if (IS_IPC(conn)) {
		DEBUG_OUT (10,("is_valid_writeX_buffer: IPC$ tid\n"));
		return false;
	}
	if (IS_PRINT(conn)) {
		DEBUG_OUT (10,("is_valid_writeX_buffer: printing tid\n"));
		return false;
	}
	...
	if (numtowrite == 0) {
		DEBUG_OUT (10,("is_valid_writeX_buffer: zero write\n"));
		return false;
	}
	...
	if (doff < STANDARD_WRITE_AND_X_HEADER_SIZE) {
		...
		DEBUG_OUT (10,("is_valid_writeX_buffer: small doff %u (min %u)\n",
			(unsigned int)doff,
	...
	if (len - doff != numtowrite) {
		DEBUG_OUT (10,("is_valid_writeX_buffer: doff mismatch "
	...
	DEBUG_OUT (10,("is_valid_writeX_buffer: true "
		"len = %u, doff = %u, numtowrite = %u\n",
		(unsigned int)len,
		(unsigned int)doff,
		(unsigned int)numtowrite ));
...
void reply_write_and_X(struct smb_request *req)
{
	...
	if(req->wct == 14) {
		...
		if(IVAL(req->vwv+12, 0) != 0) {
			DEBUG_OUT (0,("reply_write_and_X - large offset (%x << 32) "
				 "used and we don't support 64 bit offsets.\n",
	...
	DEBUG_OUT (3,("writeX fnum=%d num=%d wrote=%d\n",
		fsp->fnum, (int)numtowrite, (int)nwritten));

	status = sync_file(conn, fsp, write_through);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5,("reply_write_and_X: sync_file for %s returned %s\n",
			 fsp_str_dbg(fsp), nt_errstr(status)));
		reply_nterror(req, status);
		goto out;
	}
...
void reply_lseek(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("lseek fnum=%d ofs=%.0f newpos = %.0f mode=%d\n",
...
void reply_flush(struct smb_request *req)
{
	...
	if (!fsp) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (5,("reply_flush: sync_file for %s returned %s\n",
				fsp_str_dbg(fsp), nt_errstr(status)));
			reply_nterror(req, status);
...
void reply_close(struct smb_request *req)
{
	...
	if(fsp->is_directory) {
		 ...
		 */
		DEBUG_OUT (3,("close directory fnum=%d\n", fsp->fnum));
		...
		DEBUG_OUT (3,("close fd=%d fnum=%d (numopen=%d)\n",
			 fsp->fh->fd, fsp->fnum,
			 conn->num_files_open));
...
void reply_writeclose(struct smb_request *req)
{
	...
	if (numtowrite) {
		DEBUG_OUT (3,("reply_writeclose: zero length write doesn't close "
			 "file %s\n", fsp_str_dbg(fsp)));
		close_status = close_file(req, fsp, NORMAL_CLOSE);
	}

	DEBUG_OUT (3,("writeclose fnum=%d num=%d wrote=%d (numopen=%d)\n",
		 fsp->fnum, (int)numtowrite, (int)nwritten,
		 conn->num_files_open));
...
void reply_lock(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("lock fd=%d fnum=%d offset=%.0f count=%.0f\n",
...
void reply_unlock(struct smb_request *req)
{
	...
	DEBUG_OUT ( 3, ( "unlock fd=%d fnum=%d offset=%.0f count=%.0f\n",
		    fsp->fh->fd, fsp->fnum, (double)offset, (double)count ) );
...
void reply_tdis(struct smb_request *req)
{
	...
	if (!conn) {
		DEBUG_OUT (4,("Invalid connection in tdis\n"));
...
void reply_echo(struct smb_request *req)
{
	...
	if (smb_reverb > 100) {
		DEBUG_OUT (0,("large reverb (%d)?? Setting to 100\n",smb_reverb));
		smb_reverb = 100;
	}
...
void reply_printopen(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("openprint fd=%d fnum=%d\n",
		 fsp->fh->fd, fsp->fnum));
...
void reply_printclose(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("printclose fd=%d fnum=%d\n",
		 fsp->fh->fd,fsp->fnum));
...
void reply_printqueue(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("printqueue start_index=%d max_count=%d\n",
...

	{
		...
		DEBUG_OUT (3, ("%u entries returned in queue\n",
			  (unsigned)count));
...
void reply_printwrite(struct smb_request *req)
{
	...
	DEBUG_OUT ( 3, ( "printwrite fnum=%d num=%d\n", fsp->fnum, numtowrite ) );
...
void reply_mkdir(struct smb_request *req)
{
	...
	DEBUG_OUT (5, ("create_directory returned %s\n", nt_errstr(status)));
			      ...
			      const struct smb_filename *smb_fname_dst)
{
	...
	for(fsp = file_find_di_first(conn->sconn, lck->id); fsp;
		...
		DEBUG_OUT (10, ("rename_open_files: renaming file fnum %d "
			   "(file_id %s) from %s -> %s\n", fsp->fnum,
	...
	if (!did_rename) {
		DEBUG_OUT (10, ("rename_open_files: no open files on file_id %s "
			...
			bool replace_if_exists)
{
	...
	if (strcsequal(fsp->fsp_name->base_name, smb_fname_dst->base_name) &&
		...
		DEBUG_OUT (3, ("rename_internals_fsp: identical names in rename %s "
			  "- returning success\n",
			  smb_fname_str_dbg(smb_fname_dst)));
		status = NT_STATUS_OK;
		goto out;
	...
	if(!replace_if_exists && dst_exists) {
		DEBUG_OUT (3, ("rename_internals_fsp: dest exists doing rename "
	...
	if (dst_exists) {
		...
		if (dst_fsp && !new_is_stream) {
			DEBUG_OUT (3, ("rename_internals_fsp: Target file open\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("rename_internals_fsp: Error %s rename %s -> %s\n",
	...
	if(SMB_VFS_RENAME(conn, fsp->fsp_name, smb_fname_dst) == 0) {
		...
		DEBUG_OUT (3, ("rename_internals_fsp: succeeded doing rename on "
			  "%s -> %s\n", smb_fname_str_dbg(fsp->fsp_name),
			  smb_fname_str_dbg(smb_fname_dst)));
			...
			uint32_t access_mask)
{
	...
	if (!src_has_wild) {
		...
		DEBUG_OUT (3, ("rename_internals: case_sensitive = %d, "
		...
		if (dest_has_wild) {
			...
			if (!resolve_wildcards(smb_fname_dst,
					       ...
					       smb_fname_dst->base_name,
					       &fname_dst_mod)) {
				DEBUG_OUT (6, ("rename_internals: resolve_wildcards "
					  "%s %s failed\n",
					  smb_fname_src->base_name,
					  smb_fname_dst->base_name));
				status = NT_STATUS_NO_MEMORY;
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3, ("Could not open rename source %s: %s\n",
				  smb_fname_str_dbg(smb_fname_src),
				  nt_errstr(status)));
			goto out;
		}
		...
		DEBUG_OUT (3, ("rename_internals: Error %s rename %s -> %s\n",
	...
	while ((dname = ReadDirName(dir_hnd, &offset, &smb_fname_src->st,
		...
		if (!resolve_wildcards(ctx, smb_fname_src->base_name,
				       smb_fname_dst->base_name,
				       &destname)) {
			DEBUG_OUT (6, ("resolve_wildcards %s %s failed\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3,("rename_internals: SMB_VFS_CREATE_FILE "
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3, ("rename_internals_fsp returned %s for "
				  "rename %s -> %s\n", nt_errstr(status),
				  smb_fname_str_dbg(smb_fname_src),
				  smb_fname_str_dbg(smb_fname_dst)));
			break;
		...
		DEBUG_OUT (3,("rename_internals: doing rename on %s -> "
			 "%s\n", smb_fname_str_dbg(smb_fname_src),
			 smb_fname_str_dbg(smb_fname_src)));
		TALLOC_FREE(talloced);
	}
			...
			bool target_is_directory)
{
	...
	if (ofun & OPENX_FILE_EXISTS_OPEN) {
		...
		if (ret == -1) {
			DEBUG_OUT (0, ("error - vfs lseek returned error %s\n",
				strerror(errno)));
			status = map_nt_error_from_unix(errno);
			close_file(NULL, fsp1, ERROR_CLOSE);
...
void reply_copy(struct smb_request *req)
{
	...
	DEBUG_OUT (3,("reply_copy : %s -> %s\n", fname_src, fname_dst));

	if (tid2 != conn->cnum) {
		/* can't currently handle inter share copies XXXX */
		DEBUG_OUT (3,("Rejecting inter-share copy\n"));
	...
	if ((flags&(1<<5)) && VALID_STAT_OF_DIR(smb_fname_src->st)) {
		...
		DEBUG_OUT (3,("Rejecting tree copy\n"));
		reply_nterror(req, NT_STATUS_INVALID_PARAMETER);
		goto out;
	}
	...
	if (!source_has_wild) {
		...
		while ((dname = ReadDirName(dir_hnd, &offset,
			...
			DEBUG_OUT (3,("reply_copy : doing copy on %s -> %s\n",
				smb_fname_src->base_name,
				smb_fname_dst->base_name));
			...
			bool large_file_format)
{
	...
	if(!large_file_format) {
		...
		if(IVAL(data,SMB_LARGE_LKLEN_OFFSET_HIGH(data_offset)) != 0) {
			DEBUG_OUT (3,("get_lock_count: truncating lock count (high)0x%x (low)0x%x to just low count.\n",
	...
	*err = False;
	...
	if(!large_file_format) {
		...
		if(IVAL(data,SMB_LARGE_LKOFF_OFFSET_HIGH(data_offset)) != 0) {
			...
			DEBUG_OUT (3,("get_lock_offset: truncating lock offset (high)0x%x (low)0x%x to offset 0x%x.\n",
			 ...
			 bool *async)
{
	...
	*async = false;
	...
	for(i = 0; i < (int)num_ulocks; i++) {
		...
		DEBUG_OUT (10,("smbd_do_locking: unlock start=%.0f, len=%.0f for "
			  "pid %u, file %s\n",
			  (double)e->offset,
			  (double)e->count,
		...
		DEBUG_OUT (10, ("smbd_do_locking: unlock returned %s\n",
		    nt_errstr(status)));
	...
	for(i = 0; i < (int)num_locks; i++) {
		...
		DEBUG_OUT (10,("smbd_do_locking: lock start=%.0f, len=%.0f for smblctx "
		...
		if (type & LOCKING_ANDX_CANCEL_LOCK) {
			...
			if (num_locks > 1) {
				...
				/* Windows behavior. */
				if (i != 0) {
					DEBUG_OUT (10,("smbd_do_locking: ignoring subsequent "
						"cancel request\n"));
					continue;
				}
#endif
	...
	DEBUG_OUT (3, ("smbd_do_locking: fnum=%d type=%d num_locks=%d num_ulocks=%d\n",
...
void reply_lockingX(struct smb_request *req)
{
	...
	*/
	if ((locktype & LOCKING_ANDX_OPLOCK_RELEASE)) {
		...
		DEBUG_OUT (5,("reply_lockingX: oplock break reply (%u) from client "
		...
		if (fsp->oplock_type == 0) {
			...
			DEBUG_OUT (5,("reply_lockingX: Error : oplock break from "
		...
		if (!result) {
			DEBUG_OUT (0, ("reply_lockingX: error in removing "
				  "oplock on file %s\n", fsp_str_dbg(fsp)));
		...
		if (num_locks == 0 && num_ulocks == 0) {
			/* Sanity check - ensure a pure oplock break is not a
			   chained request. */
			if(CVAL(req->vwv+0, 0) != 0xff)
				DEBUG_OUT (0,("reply_lockingX: Error : pure oplock "
	...
	DEBUG_OUT (3, ("lockingX fnum=%d type=%d num_locks=%d num_ulocks=%d\n",
...
void reply_setattrE(struct smb_request *req)
{
	...
	DEBUG_OUT ( 3, ( "reply_setattrE fnum=%d actime=%u modtime=%u "
	       " createtime=%u\n",
		fsp->fnum,
...
void reply_getattrE(struct smb_request *req)
{
	...
	DEBUG_OUT ( 3, ( "reply_getattrE fnum=%d\n", fsp->fnum));
