...
static uint32_t get_pai_entry_val(struct pai_entry *paie)
{
	switch (paie->owner_type) {
		case UID_ACE:
			DEBUG_OUT (10,("get_pai_entry_val: uid = %u\n", (unsigned int)paie->unix_ug.uid ));
			return (uint32_t)paie->unix_ug.uid;
		case GID_ACE:
			DEBUG_OUT (10,("get_pai_entry_val: gid = %u\n", (unsigned int)paie->unix_ug.gid ));
			return (uint32_t)paie->unix_ug.gid;
		case WORLD_ACE:
		default:
			DEBUG_OUT (10,("get_pai_entry_val: world ace\n"));
			return (uint32_t)-1;
	}
}
...
static uint32_t get_entry_val(canon_ace *ace_entry)
{
	switch (ace_entry->owner_type) {
		case UID_ACE:
			DEBUG_OUT (10,("get_entry_val: uid = %u\n", (unsigned int)ace_entry->unix_ug.uid ));
			return (uint32_t)ace_entry->unix_ug.uid;
		case GID_ACE:
			DEBUG_OUT (10,("get_entry_val: gid = %u\n", (unsigned int)ace_entry->unix_ug.gid ));
			return (uint32_t)ace_entry->unix_ug.gid;
		case WORLD_ACE:
		default:
			DEBUG_OUT (10,("get_entry_val: world ace\n"));
			return (uint32_t)-1;
	}
}
				...
				size_t *store_size)
{
	...
	DEBUG_OUT (10,("create_pai_buf_v2: num_entries = %u, num_def_entries = %u\n", num_entries, num_def_entries ));
	...
	*store_size = PAI_V2_ENTRIES_BASE +
	...
	DEBUG_OUT (10,("create_pai_buf_v2: sd_type = 0x%x\n",
	...
	for (ace_list = file_ace_list; ace_list; ace_list = ace_list->next) {
		...
		DEBUG_OUT (10,("create_pai_buf_v2: entry %u [0x%x] [0x%x] [0x%x]\n",
			i,
					...
					uint16_t sd_type)
{
	...
	DEBUG_OUT (10,("store_inheritance_attribute: type 0x%x for file %s\n",
		(unsigned int)sd_type,
		fsp_str_dbg(fsp)));

	if (ret == -1 && !no_acl_syscall_error(errno)) {
		DEBUG_OUT (1,("store_inheritance_attribute: Error %s\n", strerror(errno) ));
	}
}
...
static bool get_pai_owner_type(struct pai_entry *paie, const char *entry_offset)
{
	...
	switch( paie->owner_type) {
		case UID_ACE:
			...
			DEBUG_OUT (10,("get_pai_owner_type: uid = %u\n",
				(unsigned int)paie->unix_ug.uid ));
			break;
		case GID_ACE:
			paie->unix_ug.gid = (gid_t)IVAL(entry_offset,1);
			DEBUG_OUT (10,("get_pai_owner_type: gid = %u\n",
				(unsigned int)paie->unix_ug.gid ));
			break;
		case WORLD_ACE:
			paie->unix_ug.world = -1;
			DEBUG_OUT (10,("get_pai_owner_type: world ace\n"));
			break;
		default:
			DEBUG_OUT (10,("get_pai_owner_type: unknown type %u\n",
...
static struct pai_val *create_pai_val_v1(const char *buf, size_t size)
{
	...
	DEBUG_OUT (10,("create_pai_val: num_entries = %u, num_def_entries = %u\n",
...
static struct pai_val *create_pai_val_v2(const char *buf, size_t size)
{
	...
	DEBUG_OUT (10,("create_pai_val_v2: sd_type = 0x%x num_entries = %u, num_def_entries = %u\n",
...
static struct pai_val *fload_inherited_info(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("load_inherited_info: ret = %lu for file %s\n",
...
#if defined(ENOATTR)
		if (errno != ENOATTR)
			DEBUG_OUT (10,("load_inherited_info: Error %s\n", strerror(errno) ));
#else
		if (errno != ENOSYS)
			DEBUG_OUT (10,("load_inherited_info: Error %s\n", strerror(errno) ));
#endif
	...
	if (paiv) {
		DEBUG_OUT (10,("load_inherited_info: ACL type is 0x%x for file %s\n",
					   ...
					   const char *fname)
{
	...
	if (paiv) {
		DEBUG_OUT (10,("load_inherited_info: ACL type 0x%x for file %s\n",
			(unsigned int)paiv->sd_type,
			fname));
	}
				...
				bool directory_ace)
{
	...
	*pacl_type = SEC_ACE_TYPE_ACCESS_ALLOWED;
	...
	DEBUG_OUT (10,("map_canon_ace_perms: Mapped (UNIX) %x to (NT) %x\n",
			...
			security_descriptor *psd)
{
	...
	*pgrp = (gid_t)-1;
	...
	if(security_info_sent == 0) {
		DEBUG_OUT (0,("unpack_nt_owners: no security info sent !\n"));
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (5,("unpack_nt_owners: validating owner_sids.\n"));
		...
		if (!sid_to_uid(&owner_sid, puser)) {
			if (lp_force_unknown_acl_user(SNUM(conn))) {
				...
				*puser = get_current_uid(conn);
				...
				DEBUG_OUT (3,("unpack_nt_owners: unable to validate"
					 " owner sid for %s\n",
					 sid_string_dbg(&owner_sid)));
				return NT_STATUS_INVALID_OWNER;
			}
		}
		DEBUG_OUT (3,("unpack_nt_owners: owner sid mapped to uid %u\n",
			 (unsigned int)*puser ));
 	}
	...
	if (security_info_sent & SECINFO_GROUP) {
		...
		DEBUG_OUT (3,("unpack_nt_owners: group sid mapped to gid %u\n",
			 (unsigned int)*pgrp));
 	}

	DEBUG_OUT (5,("unpack_nt_owners: owner_sids validated.\n"));
					...
					bool setting_acl)
{
	...
	if (!got_user) {
		if ((pace = SMB_MALLOC_P(canon_ace)) == NULL) {
			DEBUG_OUT (0,("ensure_canon_entry_valid: malloc fail.\n"));
			return False;
		}
...
static void check_owning_objs(canon_ace *ace, struct dom_sid *pfile_owner_sid, struct dom_sid *pfile_grp_sid)
{
	...
	if (got_user_obj && got_group_obj) {
		DEBUG_OUT (10,("check_owning_objs: ACL had owning user/group entries.\n"));
		return;
	}
	...
	if (!got_user_obj)
		DEBUG_OUT (10,("check_owning_objs: ACL is missing an owner entry.\n"));
	if (!got_group_obj)
		DEBUG_OUT (10,("check_owning_objs: ACL is missing an owning group entry.\n"));
}
	...
	*ppdir_ace = NULL;
	...
	for(i = 0; i < dacl->num_aces; i++) {
		...
		if((psa->type != SEC_ACE_TYPE_ACCESS_ALLOWED) && (psa->type != SEC_ACE_TYPE_ACCESS_DENIED)) {
			DEBUG_OUT (3,("create_canon_ace_lists: unable to set anything but an ALLOW or DENY ACE.\n"));
			return False;
		}
	...
	for(i = 0; i < dacl->num_aces; i++) {
		...
		if ((current_ace = SMB_MALLOC_P(canon_ace)) == NULL) {
			...
			DEBUG_OUT (0,("create_canon_ace_lists: malloc fail.\n"));
			return False;
		}
		...
		if( dom_sid_equal(&current_ace->trustee, &global_sid_World)) {
			...
			if (non_mappable_sid(&psa->trustee)) {
				DEBUG_OUT (10, ("create_canon_ace_lists: ignoring "
					   "non-mappable SID %s\n",
			...
			free_canon_ace_list(file_ace);
			free_canon_ace_list(dir_ace);
			DEBUG_OUT (0, ("create_canon_ace_lists: unable to map SID "
				  "%s to uid or gid.\n",
		...
		if (fsp->is_directory) {
			...
			if ((psa->flags & (SEC_ACE_FLAG_OBJECT_INHERIT|SEC_ACE_FLAG_CONTAINER_INHERIT)) ==
				...
				if ((current_ace->attr == DENY_ACE) && got_dir_allow) {
					DEBUG_OUT (0,("create_canon_ace_lists: "
						 "malformed ACL in "
		...
		if (current_ace && !(psa->flags & SEC_ACE_FLAG_INHERIT_ONLY)) {
			...
			if ((current_ace->attr == DENY_ACE) && got_file_allow) {
				DEBUG_OUT (0,("create_canon_ace_lists: malformed "
	...
	if (fsp->is_directory && all_aces_are_inherit_only) {
		...
		DEBUG_OUT (10,("create_canon_ace_lists: Win2k inherit acl traverse. Ignoring DACL.\n"));
		free_canon_ace_list(file_ace);
		free_canon_ace_list(dir_ace);
		file_ace = NULL;
		dir_ace = NULL;
				...
				const struct security_descriptor *psd)
{
	...
	*ppdir_ace = NULL;
	...
	if(security_info_sent == 0) {
		DEBUG_OUT (0,("unpack_canon_ace: no security info sent !\n"));
		return False;
	}
...
static void arrange_posix_perms(const char *filename, canon_ace **pp_list_head)
{
	...
	if (!owner_ace || !other_ace) {
		DEBUG_OUT (0,("arrange_posix_perms: Invalid POSIX permissions for file %s, missing owner or other.\n",
			filename ));
		return;
	}
				   ...
				   const struct dom_sid *powner, const struct dom_sid *pgroup, struct pai_val *pal, SMB_ACL_TYPE_T the_acl_type)
{
	...
	while ( posix_acl && (SMB_VFS_SYS_ACL_GET_ENTRY(conn, posix_acl, entry_id, &entry) == 1)) {
		...
		switch(tagtype) {
			...
			case SMB_ACL_USER:
				{
					...
					if (puid == NULL) {
						DEBUG_OUT (0,("canonicalise_acl: Failed to get uid.\n"));
						continue;
					}
					uid_to_sid( &sid, *puid);
					unix_ug.uid = *puid;
			...
			case SMB_ACL_GROUP:
				{
					...
					if (pgid == NULL) {
						DEBUG_OUT (0,("canonicalise_acl: Failed to get gid.\n"));
						continue;
					}
					gid_to_sid( &sid, *pgid);
					unix_ug.gid = *pgid;
			...
			case SMB_ACL_OTHER:
				...
				break;
			default:
				DEBUG_OUT (0,("canonicalise_acl: Unknown tagtype %u\n", (unsigned int)tagtype));
				continue;
		}
	...
	DEBUG_OUT (10,("canonicalise_acl: %s ace entries before arrange :\n", is_default_acl ?  "Default" : "Access"));
				...
				bool *pacl_set_support)
{
	...
	if (the_acl == NULL) {
		...
		if (!no_acl_syscall_error(errno)) {
			 ...
			 */
			DEBUG_OUT (0,("set_canon_ace_list: Unable to init %s ACL. (%s)\n",
	...
	for (i = 0, p_ace = the_ace; p_ace; p_ace = p_ace->next, i++ ) {
		...
		if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &the_acl, &the_entry) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to create entry %d. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}
		...
		*pacl_set_support = True;
		...
		if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, the_entry, p_ace->type) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to set tag type on entry %d. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}
		...
		if ((p_ace->type == SMB_ACL_USER) || (p_ace->type == SMB_ACL_GROUP)) {
			if (SMB_VFS_SYS_ACL_SET_QUALIFIER(conn, the_entry,(void *)&p_ace->unix_ug.uid) == -1) {
				DEBUG_OUT (0,("set_canon_ace_list: Failed to set qualifier on entry %d. (%s)\n",
					i, strerror(errno) ));
				goto fail;
			}
		}
		...
		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, the_entry, &the_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to get permset on entry %d. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}

		if (map_acl_perms_to_permset(conn, p_ace->perms, &the_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to create permset for mode (%u) on entry %d. (%s)\n",
		...
		if (SMB_VFS_SYS_ACL_SET_PERMSET(conn, the_entry, the_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to add permset on entry %d. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}
	...
	if (needs_mask && !got_mask_entry) {
		if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &the_acl, &mask_entry) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to create mask entry. (%s)\n", strerror(errno) ));
			goto fail;
		}

		if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, mask_entry, SMB_ACL_MASK) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to set tag type on mask entry. (%s)\n",strerror(errno) ));
			goto fail;
		}

		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, mask_entry, &mask_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to get mask permset. (%s)\n", strerror(errno) ));
			goto fail;
		}

		if (map_acl_perms_to_permset(conn, S_IRUSR|S_IWUSR|S_IXUSR, &mask_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to create mask permset. (%s)\n", strerror(errno) ));
			goto fail;
		}

		if (SMB_VFS_SYS_ACL_SET_PERMSET(conn, mask_entry, mask_permset) == -1) {
			DEBUG_OUT (0,("set_canon_ace_list: Failed to add mask permset. (%s)\n", strerror(errno) ));
			goto fail;
		}
	}
	...
	if(default_ace || fsp->is_directory || fsp->fh->fd == -1) {
		if (SMB_VFS_SYS_ACL_SET_FILE(conn, fsp->fsp_name->base_name,
			...
			if (acl_group_override(conn, fsp->fsp_name)) {
				...
				DEBUG_OUT (5,("set_canon_ace_list: acl group "
			...
			if (ret == False) {
				DEBUG_OUT (2,("set_canon_ace_list: "
...
static bool convert_canon_ace_to_posix_perms( files_struct *fsp, canon_ace *file_ace_list, mode_t *posix_perms)
{
	...
	if (ace_count != 3) {
		DEBUG_OUT (3,("convert_canon_ace_to_posix_perms: Too many ACE "
	...
	if (!owner_ace || !group_ace || !other_ace) {
		DEBUG_OUT (3,("convert_canon_ace_to_posix_perms: Can't get "
	...
	*posix_perms = (((*posix_perms) & and_bits)|or_bits);
	...
	DEBUG_OUT (10,("convert_canon_ace_to_posix_perms: converted u=%o,g=%o,w=%o "
		...
		for (j = i+1; j < num_aces; j++) {
			...
			if ((nt_ace_list[i].type == nt_ace_list[j].type) &&
				...
				if (nt_ace_list[i].access_mask == 0) {
					...
					DEBUG_OUT (10,("merge_default_aces: Merging zero access ACE %u onto ACE %u.\n",
					...
					DEBUG_OUT (10,("merge_default_aces: Merging ACE %u onto ACE %u.\n",
				...
				uint32_t mask, uint8_t flags)
{
	...
	if (i < *num_aces) { /* found */
		...
		DEBUG_OUT (10, ("Replacing ACE %d with SID %s and flags %02x\n",
				      ...
				      struct security_descriptor **ppdesc)
{
	...
	if ((security_info & SECINFO_DACL) && !(security_info & SECINFO_PROTECTED_DACL)) {
		...
		if (count_canon_ace_list(file_ace) == 0) {
			DEBUG_OUT (0,("get_nt_acl : No ACLs on file (%s) !\n", name));
			goto done;
		}
...

		{
			...
			if ((nt_ace_list = SMB_MALLOC_ARRAY(struct security_ace,num_acls + num_profile_acls + num_def_acls)) == NULL) {
				DEBUG_OUT (0,("get_nt_acl: Unable to malloc space for nt_ace_list.\n"));
				goto done;
			}
		...
		if (num_aces) {
			if((psa = make_sec_acl( talloc_tos(), NT4_ACL_REVISION, num_aces, nt_ace_list)) == NULL) {
				DEBUG_OUT (0,("get_nt_acl: Unable to malloc space for acl.\n"));
				goto done;
			}
		}
	...
	if(!psd) {
		DEBUG_OUT (0,("get_nt_acl: Unable to malloc space for security descriptor.\n"));
		sd_size = 0;
		goto done;
	}
			   ...
			   struct security_descriptor **ppdesc)
{
	...
	*ppdesc = NULL;
	...
	DEBUG_OUT (10,("posix_fget_nt_acl: called for file %s\n",
		  fsp_str_dbg(fsp)));
			  ...
			  uint32_t security_info, struct security_descriptor **ppdesc)
{
	...
	*ppdesc = NULL;
	...
	DEBUG_OUT (10,("posix_get_nt_acl: called for file %s\n", name ));
	...
	for (j = 0; j < parent_sd->dacl->num_aces; j++) {
		...
		if (fsp->is_directory) {
			if (!(se->flags & SEC_ACE_FLAG_CONTAINER_INHERIT)) {
				...
				DEBUG_OUT (10,("append_parent_acl: directory %s "
			...
			if (!(se->flags & SEC_ACE_FLAG_OBJECT_INHERIT)) {
				...
				DEBUG_OUT (10,("append_parent_acl: file %s "
					"ignoring non object "
					"inherit flags %u on ACE with sid %s "
					"from parent %s\n",
					fsp_str_dbg(fsp),
		...
		if (is_dacl_protected) {
			...
			if (k < psd->dacl->num_aces) {
				/* SID matched. Ignore. */
				DEBUG_OUT (10,("append_parent_acl: path %s "
...
NTSTATUS set_nt_acl(files_struct *fsp, uint32 security_info_sent, const struct security_descriptor *psd_orig)
{
	...
	DEBUG_OUT (10,("set_nt_acl: called for file %s\n",
		  fsp_str_dbg(fsp)));

	if (!CAN_WRITE(conn)) {
		DEBUG_OUT (10,("set acl rejected on read-only share\n"));
		return NT_STATUS_MEDIA_WRITE_PROTECTED;
	}
	...
	if (((user != (uid_t)-1) && (fsp->fsp_name->st.st_ex_uid != user)) ||
		...
		DEBUG_OUT (3,("set_nt_acl: chown %s. uid = %u, gid = %u.\n",
			 fsp_str_dbg(fsp), (unsigned int)user,
			 (unsigned int)grp));

		status = try_chown(fsp, user, grp);
		if(!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3,("set_nt_acl: chown %s, %u, %u failed. Error "
				"= %s.\n", fsp_str_dbg(fsp),
				(unsigned int)user,
				(unsigned int)grp,
				nt_errstr(status)));
	...
	if (!acl_perms) {
		DEBUG_OUT (3,("set_nt_acl: cannot set permissions\n"));
	...
	if (acl_perms && file_ace_list) {
		...
		if (acl_set_support && ret == false) {
			DEBUG_OUT (3,("set_nt_acl: failed to set file acl on file "
				 "%s (%s).\n", fsp_str_dbg(fsp),
				 strerror(errno)));
			free_canon_ace_list(file_ace_list);
			free_canon_ace_list(dir_ace_list);
	...
	if (acl_perms && acl_set_support && fsp->is_directory) {
		if (dir_ace_list) {
			...
			if (ret == false) {
				DEBUG_OUT (3,("set_nt_acl: failed to set default "
			...
			if (sret == -1) {
				if (acl_group_override(conn, fsp->fsp_name)) {
					DEBUG_OUT (5,("set_nt_acl: acl group "
				...
				if (sret == -1) {
					DEBUG_OUT (3,("set_nt_acl: sys_acl_delete_def_file failed (%s)\n", strerror(errno)));
					free_canon_ace_list(file_ace_list);
					free_canon_ace_list(dir_ace_list);
					return map_nt_error_from_unix(errno);
				}
	...
	if(!acl_set_support && acl_perms) {
		...
		if (!convert_canon_ace_to_posix_perms( fsp, file_ace_list, &posix_perms)) {
			free_canon_ace_list(file_ace_list);
			free_canon_ace_list(dir_ace_list);
			DEBUG_OUT (3,("set_nt_acl: failed to convert file acl to "
		...
		if (orig_mode != posix_perms) {
			...
			DEBUG_OUT (3,("set_nt_acl: chmod %s. perms = 0%o.\n",
			...
			if(sret == -1) {
				...
				if (sret == -1) {
					DEBUG_OUT (3,("set_nt_acl: chmod %s, 0%o "
						 "failed. Error = %s.\n",
						 fsp_str_dbg(fsp),
...
static SMB_ACL_T create_posix_acl_from_wire(connection_struct *conn, uint16 num_acls, const char *pdata)
{
	...
	for (i = 0; i < num_acls; i++) {
		...
		if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &the_acl, &the_entry) == -1) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to create entry %u. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}

		if (!unix_ex_wire_to_tagtype(CVAL(pdata,(i*SMB_POSIX_ACL_ENTRY_SIZE)), &tag_type)) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: invalid wire tagtype %u on entry %u.\n",
				CVAL(pdata,(i*SMB_POSIX_ACL_ENTRY_SIZE)), i ));
			goto fail;
		}

		if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, the_entry, tag_type) == -1) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to set tagtype on entry %u. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}
		...
		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, the_entry, &the_permset) == -1) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to get permset on entry %u. (%s)\n",
                                i, strerror(errno) ));
                        goto fail;
                }
		...
		if (!unix_ex_wire_to_permset(conn, CVAL(pdata,(i*SMB_POSIX_ACL_ENTRY_SIZE)+1), &the_permset)) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: invalid permset %u on entry %u.\n",
		...
		if (SMB_VFS_SYS_ACL_SET_PERMSET(conn, the_entry, the_permset) == -1) {
			DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to add permset on entry %u. (%s)\n",
				i, strerror(errno) ));
			goto fail;
		}
		...
		if (tag_type == SMB_ACL_USER) {
			...
			if (SMB_VFS_SYS_ACL_SET_QUALIFIER(conn, the_entry,(void *)&uid) == -1) {
				DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to set uid %u on entry %u. (%s)\n",
		...
		if (tag_type == SMB_ACL_GROUP) {
			...
			if (SMB_VFS_SYS_ACL_SET_QUALIFIER(conn, the_entry,(void *)&gid) == -1) {
				DEBUG_OUT (0,("create_posix_acl_from_wire: Failed to set gid %u on entry %u. (%s)\n",
				...
				uint16 num_def_acls, const char *pdata)
{
	...
	if (!S_ISDIR(psbuf->st_ex_mode)) {
		if (num_def_acls) {
			DEBUG_OUT (5,("set_unix_posix_default_acl: Can't set default ACL on non-directory file %s\n", fname ));
			errno = EISDIR;
			return False;
		} else {
			return True;
	...
	if (!num_def_acls) {
		...
		if (SMB_VFS_SYS_ACL_DELETE_DEF_FILE(conn, fname) == -1) {
			DEBUG_OUT (5,("set_unix_posix_default_acl: acl_delete_def_file failed on directory %s (%s)\n",
	...
	if (SMB_VFS_SYS_ACL_SET_FILE(conn, fname, SMB_ACL_TYPE_DEFAULT, def_acl) == -1) {
		DEBUG_OUT (5,("set_unix_posix_default_acl: acl_set_file failed on directory %s (%s)\n",
			fname, strerror(errno) ));
	        SMB_VFS_SYS_ACL_FREE_ACL(conn, def_acl);
		return False;
	}

	DEBUG_OUT (10,("set_unix_posix_default_acl: set default acl for file %s\n", fname ));
	SMB_VFS_SYS_ACL_FREE_ACL(conn, def_acl);
	return True;
}
...
static bool remove_posix_acl(connection_struct *conn, files_struct *fsp, const char *fname)
{
	...
	if (new_file_acl == NULL) {
		DEBUG_OUT (5,("remove_posix_acl: failed to init new ACL with 3 entries for file %s.\n", fname));
		return False;
	}

	/* Now create the u/g/w entries. */
	if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &new_file_acl, &user_ent) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to create user entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}
	if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, user_ent, SMB_ACL_USER_OBJ) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to set user entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}

	if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &new_file_acl, &group_ent) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to create group entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}
	if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, group_ent, SMB_ACL_GROUP_OBJ) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to set group entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}

	if (SMB_VFS_SYS_ACL_CREATE_ENTRY(conn, &new_file_acl, &other_ent) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to create other entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}
	if (SMB_VFS_SYS_ACL_SET_TAG_TYPE(conn, other_ent, SMB_ACL_OTHER) == -1) {
		DEBUG_OUT (5,("remove_posix_acl: Failed to set other entry for file %s. (%s)\n",
			fname, strerror(errno) ));
		goto done;
	}
	...
	if (file_acl == NULL) {
		...
		DEBUG_OUT (5,("remove_posix_acl: failed to get ACL from file %s (%s).\n",
			fname, strerror(errno) ));
		goto done;
	}
	...
	while ( SMB_VFS_SYS_ACL_GET_ENTRY(conn, file_acl, entry_id, &entry) == 1) {
		...
		if (SMB_VFS_SYS_ACL_GET_TAG_TYPE(conn, entry, &tagtype) == -1) {
			DEBUG_OUT (5,("remove_posix_acl: failed to get tagtype from ACL on file %s (%s).\n",
				fname, strerror(errno) ));
			goto done;
		}

		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, entry, &permset) == -1) {
			DEBUG_OUT (5,("remove_posix_acl: failed to get permset from ACL on file %s (%s).\n",
				fname, strerror(errno) ));
			goto done;
		}
		...
		if (tagtype == SMB_ACL_USER_OBJ) {
			if (SMB_VFS_SYS_ACL_SET_PERMSET(conn, user_ent, permset) == -1) {
				DEBUG_OUT (5,("remove_posix_acl: failed to set permset from ACL on file %s (%s).\n",
					fname, strerror(errno) ));
			}
	...
	if (fsp && fsp->fh->fd != -1) {
		if (SMB_VFS_SYS_ACL_SET_FD(fsp, new_file_acl) == -1) {
			DEBUG_OUT (5,("remove_posix_acl: acl_set_file failed on %s (%s)\n",
				fname, strerror(errno) ));
			goto done;
		}
	} else {
...
bool set_unix_posix_acl(connection_struct *conn, files_struct *fsp, const char *fname, uint16 num_acls, const char *pdata)
{
	...
	if (fsp && fsp->fh->fd != -1) {
		...
		if (SMB_VFS_SYS_ACL_SET_FD(fsp, file_acl) == -1) {
			DEBUG_OUT (5,("set_unix_posix_acl: acl_set_file failed on %s (%s)\n",
				fname, strerror(errno) ));
		        SMB_VFS_SYS_ACL_FREE_ACL(conn, file_acl);
			return False;
		}
	...
	DEBUG_OUT (10,("set_unix_posix_acl: set acl for file %s\n", fname ));
	SMB_VFS_SYS_ACL_FREE_ACL(conn, file_acl);
	return True;
}
...
struct security_descriptor *get_nt_acl_no_snum( TALLOC_CTX *ctx, const char *fname)
{
	...
	if (!(conn->params = TALLOC_P(conn, struct share_params))) {
		DEBUG_OUT (0,("get_nt_acl_no_snum: talloc() failed!\n"));
		TALLOC_FREE(conn);
		return NULL;
	}
	...
	if (!smbd_vfs_init(conn)) {
		DEBUG_OUT (0,("get_nt_acl_no_snum: Unable to create a fake connection struct!\n"));
		conn_free(conn);
		return NULL;
        }
	...
	if (!NT_STATUS_IS_OK(SMB_VFS_FGET_NT_ACL( &finfo, SECINFO_DACL, &psd))) {
		DEBUG_OUT (0,("get_nt_acl_no_snum: get_nt_acl returned zero.\n"));
					...
					struct security_descriptor **ppdesc)
{
	...
	DEBUG_OUT (10,("make_default_filesystem_acl: file %s mode = 0%o\n",
		name, (int)mode ));
