...
bool set_file_oplock(files_struct *fsp, int oplock_type)
{
	if (fsp->oplock_type == LEVEL_II_OPLOCK) {
		if (koplocks &&
			...
			DEBUG_OUT (10, ("Refusing level2 oplock, kernel oplocks "
				   "don't support them\n"));
			return false;
		}
	}
	...
	DEBUG_OUT (5,("set_file_oplock: granted oplock on file %s, %s/%lu, "
		    "tv_sec = %x, tv_usec = %x\n",
...
{
	if (!EXCLUSIVE_OPLOCK_TYPE(fsp->oplock_type)) {
		DEBUG_OUT (0, ("trying to downgrade an already-downgraded oplock!\n"));
		return;
	}
...
bool remove_oplock(files_struct *fsp)
{
	...
	lck = get_share_mode_lock(talloc_tos(), fsp->file_id, NULL, NULL,
				  NULL);
	if (lck == NULL) {
		DEBUG_OUT (0,("remove_oplock: failed to lock share entry for "
			 "file %s\n", fsp_str_dbg(fsp)));
		return False;
	}
	ret = remove_share_oplock(lck, fsp);
	if (!ret) {
		DEBUG_OUT (0,("remove_oplock: failed to remove share oplock for "
			 "file %s fnum %d, %s\n",
...
bool downgrade_oplock(files_struct *fsp)
{
	...
	lck = get_share_mode_lock(talloc_tos(), fsp->file_id, NULL, NULL,
				  NULL);
	if (lck == NULL) {
		DEBUG_OUT (0,("downgrade_oplock: failed to lock share entry for "
			 "file %s\n", fsp_str_dbg(fsp)));
		return False;
	}
	ret = downgrade_share_oplock(lck, fsp);
	if (!ret) {
		DEBUG_OUT (0,("downgrade_oplock: failed to downgrade share oplock "
				   ...
				   void *private_data)
{
	...
	TALLOC_FREE(fsp->oplock_timeout);
	DEBUG_OUT (0, ("Oplock break failed for file %s -- replying anyway\n",
		  fsp_str_dbg(fsp)));
	remove_oplock(fsp);
...
static void add_oplock_timeout_handler(files_struct *fsp)
{
	...
	if (fsp->oplock_timeout != NULL) {
		DEBUG_OUT (0, ("Logic problem -- have an oplock event hanging "
			  "around\n"));
	}
	...
	if (fsp->oplock_timeout == NULL) {
		DEBUG_OUT (0, ("Could not add oplock timeout handler\n"));
	}
}
...
void break_level2_to_none_async(files_struct *fsp)
{
	...
	if (fsp->oplock_type == NO_OPLOCK) {
		 ...
		 * it.  just ignore. */
		DEBUG_OUT (3, ("process_oplock_async_level2_break_message: already "
	...
	if (fsp->oplock_type == FAKE_LEVEL_II_OPLOCK) {
		...
		DEBUG_OUT (3, ("process_oplock_async_level2_break_message: "
	...
	DEBUG_OUT (10,("process_oplock_async_level2_break_message: sending break "
						      ...
						      DATA_BLOB *data)
{
	...
	if (data->length != MSG_SMB_SHARE_MODE_ENTRY_SIZE) {
		DEBUG_OUT (0, ("Got invalid msg len %d\n", (int)data->length));
		return;
	}
	...
	DEBUG_OUT (10, ("Got oplock async level 2 break message from pid %s: "
	...
	if (fsp == NULL) {
		...
		DEBUG_OUT (3, ("process_oplock_async_level2_break_message: Did not find fsp, ignoring\n"));
		return;
	}
					 ...
					 DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("Got oplock break message from pid %s: %s/%lu\n",
	...
	if (EXCLUSIVE_OPLOCK_TYPE(msg.op_type) &&
		...
		DEBUG_OUT (3, ("Already downgraded oplock on %s: %s\n",
			  file_id_string_tos(&fsp->file_id),
			  fsp_str_dbg(fsp)));
		/* We just send the same message back. */
		messaging_send_buf(msg_ctx, src, MSG_SMB_BREAK_RESPONSE,
					...
					DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("Got kernel oplock break message from pid %s: %s/%u\n",
	...
	if (fsp == NULL) {
		DEBUG_OUT (3, ("Got a kernel oplock break message for a file "
			  "I don't know about\n"));
		return;
	}
	...
	if (fsp->sent_oplock_break != NO_BREAK_SENT) {
		...
		DEBUG_OUT (3, ("Got a kernel oplock request while waiting for a "
			  "break reply\n"));
		return;
	}
					  ...
					  DATA_BLOB *data)
{
	...
	if (data->length != MSG_SMB_SHARE_MODE_ENTRY_SIZE) {
		DEBUG_OUT (0, ("Got invalid msg len %u\n",
	...
	DEBUG_OUT (10, ("Got oplock break response from pid %s: %s/%lu mid %llu\n",
		   procid_str(talloc_tos(), &src), file_id_string_tos(&msg.id),
				       ...
				       DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("Got open retry msg from pid %s: %s mid %llu\n",
		   procid_str(talloc_tos(), &src), file_id_string_tos(&msg.id),
					      ...
					      enum level2_contention_type type)
{
	...
	lck = get_share_mode_lock(talloc_tos(), fsp->file_id, NULL, NULL,
				  NULL);
	if (lck == NULL) {
		DEBUG_OUT (0,("release_level_2_oplocks_on_change: failed to lock "
			 "share mode entry for file %s.\n", fsp_str_dbg(fsp)));
		return;
	}

	DEBUG_OUT (10,("release_level_2_oplocks_on_change: num_share_modes = %d\n", 
		  lck->num_share_modes ));
	...
	for(i = 0; i < lck->num_share_modes; i++) {
		...
		DEBUG_OUT (10,("release_level_2_oplocks_on_change: "
		...
		if (EXCLUSIVE_OPLOCK_TYPE(share_entry->op_type)) {
			DEBUG_OUT (0,("release_level_2_oplocks_on_change: PANIC. "
		...
		if (procid_is_me(&share_entry->pid)) {
			...
			if (cur_fsp != NULL) {
				...
				DEBUG_OUT (3, ("release_level_2_oplocks_on_change: "
...
{
	DEBUG_OUT (3,("init_oplocks: initializing messages.\n"));
