...
NTSTATUS add_initial_entry(gid_t gid, const char *sid, enum lsa_SidType sid_name_use, const char *nt_name, const char *comment)
{
	...
	if(!init_group_mapping()) {
		DEBUG_OUT (0,("failed to initialize group mapping\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}

	map.gid=gid;
	if (!string_to_sid(&map.sid, sid)) {
		DEBUG_OUT (0, ("string_to_sid failed: %s", sid));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
bool get_domain_group_from_sid(struct dom_sid sid, GROUP_MAP *map)
{
	...
	DEBUG_OUT (10, ("get_domain_group_from_sid\n"));
	...
	DEBUG_OUT (10, ("get_domain_group_from_sid: SID found in passdb\n"));
	...
	DEBUG_OUT (10, ("get_domain_group_from_sid: SID is a domain group\n"));

	if (map->gid==-1) {
		return False;
	}

	DEBUG_OUT (10, ("get_domain_group_from_sid: SID is mapped to gid:%lu\n",(unsigned long)map->gid));

	grp = getgrgid(map->gid);
	if ( !grp ) {
		DEBUG_OUT (10, ("get_domain_group_from_sid: gid DOESN'T exist in UNIX security\n"));
		return False;
	}

	DEBUG_OUT (10, ("get_domain_group_from_sid: gid exists in UNIX security\n"));
...
int smb_create_group(const char *unix_group, gid_t *new_gid)
{
	...
	*new_gid = 0;
	...
	if ( *lp_addgroup_script() ) {
		...
		DEBUG_OUT (ret ? 0 : 3,("smb_create_group: Running the command `%s' gave %d\n",add_script,ret));
		if (ret == 0) {
...
int smb_delete_group(const char *unix_group)
{
	...
	if ( *lp_delgroup_script() ) {
		...
		DEBUG_OUT (ret ? 0 : 3,("smb_delete_group: Running the command `%s' gave %d\n",del_script,ret));
		if (ret == 0) {
			smb_nscd_flush_group_cache();
		}
		return ret;
...
int smb_set_primary_group(const char *unix_group, const char* unix_user)
{
	...
	if ( *lp_setprimarygroup_script() ) {
		...
		flush_pwnam_cache();
		DEBUG_OUT (ret ? 0 : 3,("smb_set_primary_group: "
...
int smb_add_user_group(const char *unix_group, const char *unix_user)
{
	...
	if ( *lp_addusertogroup_script() ) {
		...
		ret = smbrun(add_script,NULL);
		DEBUG_OUT (ret ? 0 : 3,("smb_add_user_group: Running the command `%s' gave %d\n",add_script,ret));
		if (ret == 0) {
			smb_nscd_flush_group_cache();
		}
		return ret;
...
int smb_delete_user_group(const char *unix_group, const char *unix_user)
{
	...
	if ( *lp_deluserfromgroup_script() ) {
		...
		ret = smbrun(del_script,NULL);
		DEBUG_OUT (ret ? 0 : 3,("smb_delete_user_group: Running the command `%s' gave %d\n",del_script,ret));
		if (ret == 0) {
			smb_nscd_flush_group_cache();
		}
		return ret;
				  ...
				  const char *name, uint32 *rid)
{
	...
	DEBUG_OUT (10, ("Trying to create alias %s\n", name));
	...
	if (!pdb_new_rid(&new_rid)) {
		DEBUG_OUT (0, ("Could not allocate a RID.\n"));
	...
	if (!winbind_allocate_gid(&gid)) {
		DEBUG_OUT (3, ("Could not get a gid out of winbind - "
			  "wasted a rid :-(\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG_OUT (10, ("Creating alias %s with gid %u and rid %u\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not add group mapping entry for alias %s "
				   ...
				   struct acct_info *info)
{
	...
	if ((map.sid_name_use != SID_NAME_ALIAS) &&
		...
		DEBUG_OUT (2, ("%s is a %s, expected an alias\n",
			  sid_string_dbg(sid),
...
NTSTATUS pdb_create_builtin_alias(uint32 rid)
{
	...
	DEBUG_OUT (10, ("Trying to create builtin alias %d\n", rid));
	...
	if (!winbind_allocate_gid(&gid)) {
		DEBUG_OUT (3, ("pdb_create_builtin_alias: Could not get a gid out of winbind\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG_OUT (10,("Creating alias %s with gid %u\n", groupname, (unsigned int)gid));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("pdb_create_builtin_alias: Could not add group mapping entry for alias %d "
