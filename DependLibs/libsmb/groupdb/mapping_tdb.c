...
static bool init_group_mapping(void)
{
	...
	if (db == NULL) {
		DEBUG_OUT (0, ("Failed to open group mapping database: %s\n",
			  strerror(errno)));
		return false;
	}
...
static bool get_group_map_from_sid(struct dom_sid sid, GROUP_MAP *map)
{
	...
	if ( ret == -1 ) {
		DEBUG_OUT (3,("get_group_map_from_sid: tdb_unpack failure\n"));
		return false;
	}
...
static int find_map(struct db_record *rec, void *private_data)
{
	...
	if (!dbrec2map(rec, state->map)) {
		DEBUG_OUT (10, ("failed to unpack map\n"));
		return 0;
	}
...
static int collect_map(struct db_record *rec, void *private_data)
{
	...
	if (state->sid_name_use != SID_NAME_UNKNOWN
		...
		DEBUG_OUT (11,("enum_group_mapping: group %s is not of the "
			  "requested type\n", map.nt_name));
		return 0;
	}

	if ((state->unix_only == ENUM_ONLY_MAPPED) && (map.gid == -1)) {
		DEBUG_OUT (11,("enum_group_mapping: group %s is non mapped\n",
			  map.nt_name));
		return 0;
	}
	...
	if ((state->domsid != NULL) &&
		...
		DEBUG_OUT (11,("enum_group_mapping: group %s is not in domain\n",
	...
	if (!(tmp = SMB_REALLOC_ARRAY(state->maps, GROUP_MAP,
				      state->num_maps+1))) {
		DEBUG_OUT (0,("enum_group_mapping: Unable to enlarge group "
			 "map!\n"));
		return 1;
	}
...
static NTSTATUS add_aliasmem(const struct dom_sid *alias, const struct dom_sid *member)
{
	...
	if (rec == NULL) {
		DEBUG_OUT (10, ("fetch_lock failed\n"));
		TALLOC_FREE(key);
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Could not store record: %s\n", nt_errstr(status)));
		goto cancel;
	}
...
static NTSTATUS del_aliasmem(const struct dom_sid *alias, const struct dom_sid *member)
{
 ...
 commit:
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("dbwrap_store_bystring failed: %s\n",
			   nt_errstr(status)));
		goto cancel;
	}
			      ...
			      TDB_DATA data, void *ptr)
{
	...
	case LTDB_PACKING_FORMAT:
		...
		if (strncmp((char *)p, "rid=", 4)) {
			...
			DEBUG_OUT (3, ("Found unknown entry in group mapping "
				  "database named [%s]\n", (char *)p));
			return 0;
		}
	...
	if (remaining != 0) {
		DEBUG_OUT (0, ("Errror: %d bytes unread in ltdb_unpack_data\n",
			  remaining));
	}
...
static bool mapping_switch(const char *ldb_path)
{
	...
	if (rename(ldb_path, new_path) != 0) {
		DEBUG_OUT (0,("Failed to rename old group mapping database\n"));
		goto failed;
	}
	TALLOC_FREE(frame);
	return True;
...
failed:
	DEBUG_OUT (0, ("Failed to switch to tdb group mapping database\n"));
...
const struct mapping_backend *groupdb_tdb_init(void)
{
	if (!init_group_mapping()) {
		DEBUG_OUT (0,("Failed to initialise tdb mapping backend\n"));
		return NULL;
	}
