...
void set_server_role(void)
{
	...
	switch (lp_security()) {
		case SEC_SHARE:
			if (lp_domain_logons())
				DEBUG_OUT (0, ("Server's Role (logon server) conflicts with share-level security\n"));
			break;
		case SEC_SERVER:
			if (lp_domain_logons())
				DEBUG_OUT (0, ("Server's Role (logon server) conflicts with server-level security\n"));
			/* this used to be considered ROLE_DOMAIN_MEMBER but that's just wrong */
			server_role = ROLE_STANDALONE;
			break;
		case SEC_DOMAIN:
			if (lp_domain_logons()) {
				DEBUG_OUT (1, ("Server's Role (logon server) NOT ADVISED with domain-level security\n"));
		...
		case SEC_USER:
			...
			break;
		default:
			DEBUG_OUT (0, ("Server's Role undefined due to unknown security mode\n"));
			break;
	}

	DEBUG_OUT (10, ("set_server_role: role = %s\n", server_role_str(server_role)));
}
