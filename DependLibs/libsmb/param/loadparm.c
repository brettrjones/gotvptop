...
static int max_open_files(void)
{
	...
	if (sysctl_max < MIN_OPEN_FILES_WINDOWS) {
		DEBUG_OUT (2,("max_open_files: increasing sysctl_max (%d) to "
	...
	if (rlimit_max < MIN_OPEN_FILES_WINDOWS) {
		DEBUG_OUT (2,("rlimit_max: increasing rlimit_max (%d) to "
			"minimum Windows limit (%d)\n",
			rlimit_max,
			MIN_OPEN_FILES_WINDOWS));
...
static bool apply_lp_set_cmdline(void)
{
	...
	for (entry = stored_options; entry != NULL; entry = entry->next) {
		if (!lp_set_cmdline_helper(entry->label, entry->value, false)) {
			DEBUG_OUT (0, ("Failed to re-apply cmdline parameter %s = %s\n",
...
static void init_globals(bool reinit_globals)
{
	...
	DEBUG_OUT (3, ("Initialising global parameters\n"));
...
const char *lp_passdb_backend(void)
{
	...
	if (quote == NULL) {
		DEBUG_OUT (0, ("WARNING: Your 'passdb backend' configuration is invalid due to a missing second \" char.\n"));
		goto out;
	} else if (*(quote+1) == '\0') {
...
warn:
	DEBUG_OUT (0, ("WARNING: Your 'passdb backend' configuration includes multiple backends.  This\n"
...
int lp_winbind_max_domain_connections(void)
{
	if (lp_winbind_offline_logon() &&
		...
		DEBUG_OUT (1, ("offline logons active, restricting max domain "
			  "connections to 1\n"));
		return 1;
	}
...
int lp_maxprotocol(void)
{
	...
	if ((ret == PROTOCOL_SMB2) && (lp_security() == SEC_SHARE)) {
		DEBUG_OUT (2,("WARNING!!: \"security = share\" is incompatible "
...
#define MISSING_PARAMETER(name) \
    DEBUG_OUT (0, ("%s(): value is NULL or empty!\n", #name))
...
static bool lp_bool(const char *s)
{
	...
	if (!set_boolean(s, &ret)) {
		DEBUG_OUT (0,("lp_bool(%s): value is not boolean!\n",s));
		return False;
	}
...
static int lp_enum(const char *s,const struct enum_list *_enum)
{
	...
	DEBUG_OUT (0,("lp_enum(%s,enum): value is not in enum_list!\n",s));
	return (-1);
}
...
static void free_param_opts(struct param_opt_struct **popts)
{
	...
	if (*popts != NULL) {
		DEBUG_OUT (5, ("Freeing parametrics:\n"));
	}
	opt = *popts;
	while (opt != NULL) {
		string_free(&opt->key);
...
static void free_service(struct service *pservice)
{
	...
	if (pservice->szService)
		DEBUG_OUT (5, ("free_service: Freeing service %s\n",
		       pservice->szService));
...
static int add_a_service(const struct service *pservice, const char *name)
{
	...
	if (i == iNumServices) {
		...
		if (tsp == NULL) {
			DEBUG_OUT (0,("add_a_service: failed to enlarge ServicePtrs!\n"));
			return (-1);
		}
		ServicePtrs = tsp;
		ServicePtrs[iNumServices] = SMB_MALLOC_P(struct service);
		if (!ServicePtrs[iNumServices]) {
			DEBUG_OUT (0,("add_a_service: out of memory!\n"));
			return (-1);
		}
		iNumServices++;
		...
		tinvalid = SMB_REALLOC_ARRAY_KEEP_OLD_ON_ERROR(invalid_services, int,
					     num_to_alloc);
		if (tinvalid == NULL) {
			DEBUG_OUT (0,("add_a_service: failed to enlarge "
				 "invalid_services!\n"));
			return (-1);
		}
	...
	DEBUG_OUT (8,("add_a_service: Creating snum = %d for %s\n", 
...
char *canonicalize_servicename(TALLOC_CTX *ctx, const char *src)
{
	...
	if ( !src ) {
		DEBUG_OUT (0,("canonicalize_servicename: NULL source name!\n"));
		return NULL;
	}
...
static bool hash_a_service(const char *name, int idx)
{
	...
	if ( !ServiceHash ) {
		DEBUG_OUT (10,("hash_a_service: creating servicehash\n"));
		ServiceHash = db_open_rbt(NULL);
		if ( !ServiceHash ) {
			DEBUG_OUT (0,("hash_a_service: open tdb servicehash failed!\n"));
			return False;
		}
	}

	DEBUG_OUT (10,("hash_a_service: hashing index %d for service name %s\n",
		idx, name));
		 ...
		 const char *user, const char *pszHomedir)
{
	...
	DEBUG_OUT (3, ("adding home's share [%s] for user '%s' at '%s'\n", pszHomename, 
...
static bool lp_add_ipc(const char *ipc_name, bool guest_ok)
{
	...
	DEBUG_OUT (3, ("adding IPC service\n"));
...
bool lp_add_printer(const char *pszPrintername, int iDefaultService)
{
	...
	DEBUG_OUT (3, ("adding printer service %s\n", pszPrintername));
...
static int map_parameter(const char *pszParmName)
{
	...
	if (strchr(pszParmName, ':') == NULL)
		DEBUG_OUT (1, ("Unknown parameter encountered: \"%s\"\n", pszParmName));
...
bool service_ok(int iService)
{
	...
	if (ServicePtrs[iService]->szService[0] == '\0') {
		DEBUG_OUT (0, ("The following message indicates an internal error:\n"));
		DEBUG_OUT (0, ("No service name in service entry.\n"));
		bRetval = False;
	}
	...
	if (strwicmp(ServicePtrs[iService]->szService, PRINTERS_NAME) == 0) {
		if (!ServicePtrs[iService]->bPrint_ok) {
			DEBUG_OUT (0, ("WARNING: [%s] service MUST be printable!\n",
	...
	if (ServicePtrs[iService]->szPath[0] == '\0' &&
		...
		DEBUG_OUT (0, ("WARNING: No path in service %s - making it unavailable!\n",
			ServicePtrs[iService]->szService));
	...
	if (!ServicePtrs[iService]->bAvailable)
		DEBUG_OUT (1, ("NOTE: Service %s is flagged unavailable.\n",
			  ServicePtrs[iService]->szService));
...
static struct smbconf_ctx *lp_smbconf_ctx(void)
{
	...
	if (conf_ctx == NULL) {
		...
		if (!SBC_ERROR_IS_OK(err)) {
			DEBUG_OUT (1, ("error initializing registry configuration: "
				  "%s\n", sbcErrorString(err)));
			conf_ctx = NULL;
		}
	}
...
bool process_registry_service(const char *service_name)
{
	...
	DEBUG_OUT (5, ("process_registry_service: service name %s\n", service_name));
...
bool lp_file_list_changed(void)
{
	struct file_lists *f = file_lists;
 	...
 	DEBUG_OUT (6, ("lp_file_list_changed()\n"));
...
static bool handle_netbios_name(int snum, const char *pszParmValue, char **ptr)
{
	...
	DEBUG_OUT (4, ("handle_netbios_name: set global_myname to: %s\n",
	       global_myname()));
...
static bool handle_dos_charset(int snum, const char *pszParmValue, char **ptr)
{
	...
	if (strcmp(*ptr, pszParmValue) != 0) {
		if (is_utf8) {
			DEBUG_OUT (0,("ERROR: invalid DOS charset: 'dos charset' must not "
...
static bool handle_include(int snum, const char *pszParmValue, char **ptr)
{
	...
	if (include_depth >= MAX_INCLUDE_DEPTH) {
		DEBUG_OUT (0, ("Error: Maximum include depth (%u) exceeded!\n",
			  include_depth));
		return false;
	}
	...
	if (strequal(pszParmValue, INCLUDE_REGISTRY_NAME)) {
		...
		if (bInGlobalSection) {
			...
			DEBUG_OUT (1, ("\"include = registry\" only effective "
				  "in %s section\n", GLOBAL_NAME));
			return false;
		}
	}
	...
	DEBUG_OUT (2, ("Can't find include file %s\n", fname));
	TALLOC_FREE(fname);
	return true;
}
...
static bool handle_copy(int snum, const char *pszParmValue, char **ptr)
{
	...
	DEBUG_OUT (3, ("Copying service from service %s\n", pszParmValue));

	if ((iTemp = getservicebyname(pszParmValue, &serviceTemp)) >= 0) {
		if (iTemp == iServiceIndex) {
			DEBUG_OUT (0, ("Can't copy service %s - unable to copy self!\n", pszParmValue));
		} else {
		...
		DEBUG_OUT (0, ("Unable to copy service - source not found: %s\n", pszParmValue));
		bRetval = False;
	}
...
static const char *append_ldap_suffix( const char *str )
{
	...
	if ( !suffix_string ) {
		DEBUG_OUT (0,("append_ldap_suffix: talloc_asprintf() failed!\n"));
		return "";
	}
                              ...
                              int *ptr )
{
	...
	DEBUG_OUT (0, ("WARNING: Ignoring invalid value '%s' for parameter '%s'\n",
...
bool lp_do_parameter(int snum, const char *pszParmName, const char *pszParmValue)
{
	...
	if (parmnum < 0) {
		if (strchr(pszParmName, ':') == NULL) {
			DEBUG_OUT (0, ("Ignoring unknown parameter \"%s\"\n",
				  pszParmName));
			return (True);
		}
	...
	if (parm_table[parmnum].flags & FLAG_DEPRECATED) {
		DEBUG_OUT (1, ("WARNING: The \"%s\" option is deprecated\n",
			  pszParmName));
	}
	...
	switch (parm_table[parmnum].type)
	{
		...
		case P_OCTAL:
			...
			if ( i != 1 ) {
			    DEBUG_OUT ( 0, ("Invalid octal number %s\n", pszParmName ));
			}
			break;
...
static bool do_section(const char *pszSectionName, void *userdata)
{
	...
	if (bInGlobalSection) {
		DEBUG_OUT (3, ("Processing section \"[%s]\"\n", pszSectionName));
		return (True);
	}
	...
	if (bRetval) {
		...
		DEBUG_OUT (2, ("Processing section \"[%s]\"\n", pszSectionName));

		if ((iServiceIndex = add_a_service(&sDefault, pszSectionName))
		    < 0) {
			DEBUG_OUT (0, ("Failed to add a new service\n"));
			return (False);
		}
				 ...
				 const SMB_STRUCT_STAT *psbuf)
{
	if (!S_ISREG(psbuf->st_ex_mode)) {
		DEBUG_OUT (0,("check_usershare_stat: file %s owned by uid %u is "
			"not a regular file\n",
	...
	if (psbuf->st_ex_mode & S_IWOTH) {
		DEBUG_OUT (0,("check_usershare_stat: file %s owned by uid %u allows "
			...
			bool *pallow_guest)
{
	...
	*pallow_guest = False;
	...
	if (sharepath[0] != '/') {
		DEBUG_OUT (2,("parse_usershare_file: share %s: path %s is not an absolute path.\n",
	...
	if (prefixdenylist) {
		...
		for ( i=0; prefixdenylist[i]; i++ ) {
			DEBUG_OUT (10,("parse_usershare_file: share %s : checking prefixdenylist[%d]='%s' against %s\n",
				servicename, i, prefixdenylist[i], sharepath ));
			if (memcmp( sharepath, prefixdenylist[i], strlen(prefixdenylist[i])) == 0) {
				DEBUG_OUT (2,("parse_usershare_file: share %s path %s starts with one of the "
	...
	if (prefixallowlist) {
		...
		for ( i=0; prefixallowlist[i]; i++ ) {
			DEBUG_OUT (10,("parse_usershare_file: share %s checking prefixallowlist[%d]='%s' against %s\n",
		...
		if (prefixallowlist[i] == NULL) {
			DEBUG_OUT (2,("parse_usershare_file: share %s path %s doesn't start with one of the "
	...
	if (!dp) {
		DEBUG_OUT (2,("parse_usershare_file: share %s path %s is not a directory.\n",
			servicename, sharepath));
	...
	if (sys_stat(sharepath, &sbuf, false) == -1) {
		DEBUG_OUT (2,("parse_usershare_file: share %s : stat failed on path %s. %s\n",
...
static int process_usershare_file(const char *dir_name, const char *file_name, int snum_template)
{
	...
	if (!validate_net_name(file_name, INVALID_SHARENAME_CHARS, strlen(file_name))) {
		DEBUG_OUT (0,("process_usershare_file: share name %s contains "
	...
	if (sys_lstat(fname, &lsbuf, false) != 0) {
		DEBUG_OUT (0,("process_usershare_file: stat of %s failed. %s\n",
	...
	if (iService != -1 &&
		...
		DEBUG_OUT (10,("process_usershare_file: service %s not changed.\n",
			canon_name ));
	...
	if (fd == -1) {
		DEBUG_OUT (0,("process_usershare_file: unable to open %s. %s\n",
			fname, strerror(errno) ));
		goto out;
	}
	...
	if (sys_fstat(fd, &sbuf, false) != 0) {
		close(fd);
		DEBUG_OUT (0,("process_usershare_file: fstat of %s failed. %s\n",
			fname, strerror(errno) ));
		goto out;
	}
	...
	if (lsbuf.st_ex_dev != sbuf.st_ex_dev || lsbuf.st_ex_ino != sbuf.st_ex_ino) {
		close(fd);
		DEBUG_OUT (0,("process_usershare_file: fstat of %s is a different file from lstat. "
	...
	close(fd);
	if (lines == NULL) {
		DEBUG_OUT (0,("process_usershare_file: loading file %s owned by %u failed.\n",
	...
	if (iService < 0) {
		...
		if ((iService = add_a_service(sp, cp_service_name)) < 0) {
			DEBUG_OUT (0, ("process_usershare_file: Failed to add "
				"new service %s\n", cp_service_name));
			goto out;
		}
	...
	if (!set_share_security(canon_name, psd)) {
		 DEBUG_OUT (0, ("process_usershare_file: Failed to set share "
...
int load_usershare_service(const char *servicename)
{
	...
	if (sys_stat(usersharepath, &sbuf, false) != 0) {
		DEBUG_OUT (0,("load_usershare_service: stat of %s failed. %s\n",
			usersharepath, strerror(errno) ));
		return -1;
	}

	if (!S_ISDIR(sbuf.st_ex_mode)) {
		DEBUG_OUT (0,("load_usershare_service: %s is not a directory.\n",
			usersharepath ));
		return -1;
	}
...
#endif
		DEBUG_OUT (0,("load_usershare_service: directory %s is not owned by root "
	...
	if (Globals.szUsershareTemplateShare[0]) {
		...
		if (snum_template == -1) {
			DEBUG_OUT (0,("load_usershare_service: usershare template share %s "
				"does not exist.\n",
				Globals.szUsershareTemplateShare ));
			return -1;
		}
...
int load_usershare_shares(void)
{
	...
	if (sys_stat(usersharepath, &sbuf, false) != 0) {
		DEBUG_OUT (0,("load_usershare_shares: stat of %s failed. %s\n",
			usersharepath, strerror(errno) ));
		return ret;
	}
	...
	if (sbuf.st_ex_uid != 0 || (sbuf.st_ex_mode & S_IWOTH)) {
#endif
		DEBUG_OUT (0,("load_usershare_shares: directory %s is not owned by root "
			"or does not have the sticky bit 't' set or is writable by anyone.\n",
			usersharepath ));
		return ret;
	}
	...
	if (Globals.szUsershareTemplateShare[0]) {
		...
		if (snum_template == -1) {
			DEBUG_OUT (0,("load_usershare_shares: usershare template share %s "
				"does not exist.\n",
				Globals.szUsershareTemplateShare ));
			return ret;
		}
	...
	if (!dp) {
		DEBUG_OUT (0,("load_usershare_shares:: failed to open directory %s. %s\n",
			usersharepath, strerror(errno) ));
		return ret;
	}
	...
	for (num_dir_entries = 0, num_bad_dir_entries = 0, num_tmp_dir_entries = 0;
		...
		if (num_tmp_dir_entries > allowed_tmp_entries) {
			DEBUG_OUT (0,("load_usershare_shares: too many temp entries (%u) "
				"in directory %s\n",
		...
		if (r == 0) {
			...
			if (num_usershares >= max_user_shares) {
				DEBUG_OUT (0,("load_usershare_shares: max user shares reached "
		...
		if (num_bad_dir_entries > allowed_bad_entries) {
			DEBUG_OUT (0,("load_usershare_shares: too many bad entries (%u) "
				"in directory %s\n",
		...
		if (num_dir_entries > max_user_shares + allowed_bad_entries) {
			DEBUG_OUT (0,("load_usershare_shares: too many total entries (%u) "
			"in directory %s\n",
	...
	for (iService = iNumServices - 1; iService >= 0; iService--) {
		if (VALID(iService) && (ServicePtrs[iService]->usershare == USERSHARE_PENDING_DELETE)) {
			...
			DEBUG_OUT (10,("load_usershare_shares: Removing deleted usershare %s\n",
		       ...
		       bool allow_registry_shares)
{
	...
	DEBUG_OUT (3, ("lp_load_ex: refreshing parameters\n"));
	...
	if (lp_config_backend_is_file()) {
		...
		DEBUG_OUT (4, ("pm_process() returned %s\n", BOOLSTR(bRetval)));
		if (bRetval) {
			if (iServiceIndex >= 0) {
				bRetval = service_ok(iServiceIndex);
			}
		...
		if (lp_config_backend_is_registry()) {
			...
			/* start over */
			DEBUG_OUT (1, ("lp_load_ex: changing to config backend "
				  "registry\n"));
			init_globals(true);
		...
		DEBUG_OUT (0, ("Illegal config  backend given: %d\n",
			  lp_config_backend()));
		bRetval = false;
	}
	...
	if (lp_security() == SEC_SHARE) {
		DEBUG_OUT (1, ("WARNING: The security=share option is deprecated\n"));
	} else if (lp_security() == SEC_SERVER) {
		DEBUG_OUT (1, ("WARNING: The security=server option is deprecated\n"));
	}

	if (lp_security() == SEC_ADS && strchr(lp_passwordserver(), ':')) {
		DEBUG_OUT (1, ("WARNING: The optional ':port' in password server = %s is deprecated\n",
			  lp_passwordserver()));
	}
...
int lp_servicenumber(const char *pszServiceName)
{
	...
	if (iService < 0) {
		DEBUG_OUT (7,("lp_servicenumber: couldn't find %s\n", pszServiceName));
		return GLOBAL_SECTION_SNUM;
	}
...
void widelinks_warning(int snum)
{
	...
	if (lp_unix_extensions() && lp_widelinks_internal(snum)) {
		DEBUG_OUT (0,("Share '%s' has wide links and unix extensions enabled. "
