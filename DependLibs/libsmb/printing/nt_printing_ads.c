			       ...
			       const char *printer, struct GUID guid)
{
	...
	if (!tmp_ctx) {
		DEBUG_OUT (0, ("store_printer_guid: Out of memory?!\n"));
		return;
	}

	status = make_session_info_system(tmp_ctx, &session_info);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("store_printer_guid: "
				     ...
				     struct spoolss_PrinterInfo2 *pinfo2)
{
	...
	DEBUG_OUT (5, ("publishing printer %s\n", printer));
	...
	if (!ADS_ERR_OK(ads_rc)) {
		DEBUG_OUT (3, ("error publishing %s: %s\n",
                                       ...
                                       const char *printer)
{
	...
	DEBUG_OUT (5, ("unpublishing printer %s\n", printer));
			  ...
			  int action)
{
	...
	if (!ADS_ERR_OK(ads_rc)) {
		DEBUG_OUT (3, ("ads_connect failed: %s\n", ads_errstr(ads_rc)));
...
WERROR check_published_printers(struct messaging_context *msg_ctx)
{
	...
	status = make_session_info_system(tmp_ctx, &session_info);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("check_published_printers: "
			  "Could not create system session_info\n"));
		result = WERR_ACCESS_DENIED;
		goto done;
	}
			  ...
			  struct spoolss_PrinterInfo2 **info2)
{
	...
	case REG_BINARY:
		...
		break;
	default:
		DEBUG_OUT (0,("is_printer_published: GUID value stored as "
			 "invaluid type (%d)\n", type));
		break;
	}
