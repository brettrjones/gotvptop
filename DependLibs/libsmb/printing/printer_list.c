...
bool printer_list_parent_init(void)
{
	...
	if (db == NULL) {
		DEBUG_OUT (1, ("could not open Printer List Database: %s\n",
			  strerror(errno)));
		return false;
	}
	return true;
				  ...
				  time_t *last_refresh)
{
	...
	if (!key) {
		DEBUG_OUT (0, ("Failed to allocate key name!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	data = dbwrap_fetch_bystring_upper(db, key, key);
	if (data.dptr == NULL) {
		DEBUG_OUT (6, ("Failed to fetch record! "
	...
	if (ret == -1) {
		DEBUG_OUT (1, ("Failed to un pack printer data"));
	...
	if (comment) {
		*comment = talloc_strdup(mem_ctx, cstr);
		if (!*comment) {
			DEBUG_OUT (1, ("Failed to strdup comment!\n"));
	...
	if (location) {
		*location = talloc_strdup(mem_ctx, lstr);
		if (*location == NULL) {
			DEBUG_OUT (1, ("Failed to strdup location!\n"));
			status = NT_STATUS_NO_MEMORY;
			goto done;
		}
	}
				  ...
				  time_t last_refresh)
{
	...
	if (!data.dptr) {
		DEBUG_OUT (0, ("Failed to allocate tdb data buffer!\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
	data.dsize = len;
...
NTSTATUS printer_list_get_last_refresh(time_t *last_refresh)
{
	...
	if (data.dptr == NULL) {
		DEBUG_OUT (1, ("Failed to fetch record!\n"));
		status = NT_STATUS_NOT_FOUND;
		goto done;
	}
