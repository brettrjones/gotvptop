...
bool pcap_cache_add_specific(struct pcap_cache **ppcache, const char *name, const char *comment, const char *location)
{
	...
	DEBUG_OUT (11,("pcap_cache_add_specific: Adding name %s info %s, location: %s\n",
...
bool pcap_cache_replace(const struct pcap_cache *pcache)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to mark printer list for reload!\n"));
		return false;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to cleanup printer list!\n"));
		return false;
	}
						  ...
						  struct messaging_context *))
{
	...
	DEBUG_OUT (3, ("reloading printcap cache\n"));

	/* only go looking if no printcap name supplied */
	if (pcap_name == NULL || *pcap_name == 0) {
		DEBUG_OUT (0, ("No printcap file name configured!\n"));
		return;
	}
...
void pcap_printer_fn(void (*fn)(const char *, const char *, const char *, void *), void *pdata)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Failed to run fn for all printers!\n"));
	}
	return;
}
