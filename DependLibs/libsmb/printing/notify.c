						  ...
						  unsigned int timeout)
{
	...
	for (pq = notify_queue_head; pq; pq = pq->next) {
		if (strequal(printer, pq->msg->printer)) {
			if (!flatten_message(pq)) {
				DEBUG_OUT (0,("print_notify_send_messages: Out of memory\n"));
	...
	DEBUG_OUT (5, ("print_notify_send_messages_to_printer: sending %lu print notify message%s to printer %s\n", 
...
static bool copy_notify2_msg( SPOOLSS_NOTIFY_MSG *to, SPOOLSS_NOTIFY_MSG *from )
{
	...
	if ( from->len ) {
		...
		if ( !to->notify.data ) {
			DEBUG_OUT (0,("copy_notify2_msg: TALLOC_MEMDUP() of size [%d] failed!\n", from->len ));
			return False;
		}
	}
				     ...
				     SPOOLSS_NOTIFY_MSG *msg)
{
		    ...
		    || msg->field == JOB_NOTIFY_FIELD_TOTAL_PAGES ))
	{
		...
		for (tmp_ptr = notify_queue_head; tmp_ptr; tmp_ptr = tmp_ptr->next) 
		{
			if (tmp_ptr->msg->type == msg->type &&
				...
				DEBUG_OUT (5,("send_spoolss_notify2_msg: replacing message 0x%02x/0x%02x for "
	...
	if (!pnqueue) {
		DEBUG_OUT (0,("send_spoolss_notify2_msg: Out of memory.\n"));
		return;
	}
	...
	if ( !(pnqueue->msg = TALLOC_P(send_ctx, SPOOLSS_NOTIFY_MSG)) ) {
		DEBUG_OUT (0,("send_spoolss_notify2_msg: talloc() of size [%lu] failed!\n", 
	...
	DEBUG_OUT (5, ("send_spoolss_notify2_msg: appending message 0x%02x/0x%02x for printer %s \
				  ...
				  size_t *p_num_pids, pid_t **pp_pid_list)
{
	...
	*pp_pid_list = NULL;
	...
	if (tdb_read_lock_bystring_with_timeout(tdb, NOTIFY_PID_LIST_KEY, 10) == -1) {
		DEBUG_OUT (0,("print_notify_pid_list: Failed to lock printer %s database\n",
					printername));
		if (pdb)
