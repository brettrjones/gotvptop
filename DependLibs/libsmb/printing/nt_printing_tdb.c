...
static bool upgrade_to_version_3(void)
{
	...
	DEBUG_OUT (0,("upgrade_to_version_3: upgrading print tdb's to version 3\n"));
	...
	for (kbuf = tdb_firstkey(tdb_drivers); kbuf.dptr;
		...
		if (strncmp((const char *)kbuf.dptr, FORMS_PREFIX, strlen(FORMS_PREFIX)) == 0) {
			DEBUG_OUT (0,("upgrade_to_version_3:moving form\n"));
			if (tdb_store(tdb_forms, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to move form. Error (%s).\n", tdb_errorstr(tdb_forms)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to delete form. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}

		if (strncmp((const char *)kbuf.dptr, PRINTERS_PREFIX, strlen(PRINTERS_PREFIX)) == 0) {
			DEBUG_OUT (0,("upgrade_to_version_3:moving printer\n"));
			if (tdb_store(tdb_printers, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to move printer. Error (%s)\n", tdb_errorstr(tdb_printers)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to delete printer. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}

		if (strncmp((const char *)kbuf.dptr, SECDESC_PREFIX, strlen(SECDESC_PREFIX)) == 0) {
			DEBUG_OUT (0,("upgrade_to_version_3:moving secdesc\n"));
			if (tdb_store(tdb_printers, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to move secdesc. Error (%s)\n", tdb_errorstr(tdb_printers)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG_OUT (0,("upgrade_to_version_3: failed to delete secdesc. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}
                            ...
                            TDB_DATA data, void *state )
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		/* delete bad entries */
		DEBUG_OUT (0,("sec_desc_upg_fn: Failed to parse original sec_desc for %si.  Deleting....\n",
			(const char *)key.dptr ));
	...
	if ( !(sd_store = sec_desc_merge_buf( ctx, sd_new, sd_orig )) ) {
		DEBUG_OUT (0,("sec_desc_upg_fn: Failed to update sec_desc for %s\n", key.dptr ));
		return 0;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("sec_desc_upg_fn: Failed to parse new sec_desc for %s\n", key.dptr ));
		return 0;
	}
...
static bool upgrade_to_version_4(void)
{
	...
	DEBUG_OUT (0,("upgrade_to_version_4: upgrading printer security descriptors\n"));
                                  ...
                                  TDB_DATA data, void *state )
{
	...
	if ( tdb_delete( the_tdb, key ) != 0 ) {
		DEBUG_OUT (0,("normalize_printers_fn: tdb_delete for [%s] failed!\n",
			key.dptr));
		return 1;
	}

	if ( tdb_store( the_tdb, new_key, data, TDB_REPLACE) != 0 ) {
		DEBUG_OUT (0,("normalize_printers_fn: failed to store new record for [%s]!\n",
			key.dptr));
		return 1;
	}
...
static bool upgrade_to_version_5(void)
{
	...
	DEBUG_OUT (0,("upgrade_to_version_5: normalizing printer keys\n"));
...
bool nt_printing_tdb_upgrade(void)
{
	...
	tdb_drivers = tdb_open_log(drivers_path,
				   ...
				   0600);
	if (tdb_drivers == NULL) {
		DEBUG_OUT (0,("nt_printing_init: Failed to open nt drivers "
			 "database %s (%s)\n",
	...
	if (tdb_printers == NULL) {
		DEBUG_OUT (0,("nt_printing_init: Failed to open nt printers "
			 "database %s (%s)\n",
	...
	tdb_forms = tdb_open_log(forms_path,
				 ...
				 0600);
	if (tdb_forms == NULL) {
		DEBUG_OUT (0,("nt_printing_init: Failed to open nt forms "
			 "database %s (%s)\n",
	...
	if (vers_id != NTDRIVERS_DATABASE_VERSION_5) {
		...
		if (vers_id != NTDRIVERS_DATABASE_VERSION_5) {
			DEBUG_OUT (0,("nt_printing_init: Unknown printer database version [%d]\n", vers_id));
			return false;
		}
	}
