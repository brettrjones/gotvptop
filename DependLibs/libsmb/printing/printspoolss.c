			  ...
			  uint16_t current_vuid)
{
	...
	if (fd == -1) {
		if (errno == EACCES) {
			...
			DEBUG_OUT (0, ("Insufficient permissions "
				  "to open spool file %s.\n",
				  pf->filename));
		} else {
			/* Normal case, report at level 3 and above. */
			DEBUG_OUT (3, ("can't open spool file %s,\n",
				  pf->filename));
		      ...
		      SMB_OFF_T offset, uint32_t *written)
{
	...
	*written = 0;
	...
	if (sys_fstat(fsp->fh->fd, &st, false) != 0) {
		ret = errno;
		DEBUG_OUT (3, ("printfile_offset: sys_fstat failed on %s (%s)\n",
...
void print_spool_end(files_struct *fsp, enum file_close_type close_type)
{
	...
	case SHUTDOWN_CLOSE:
		...
		if (!NT_STATUS_IS_OK(status) ||
			...
			DEBUG_OUT (3, ("Failed to close printer %s [%s]\n",
			   ...
			   struct print_file_data *print_file)
{
	...
	status = rpc_pipe_open_interface(conn,
					 ...
					 &conn->spoolss_pipe);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("print_spool_terminate: "
	...
	if (!NT_STATUS_IS_OK(status) ||
	    !NT_STATUS_IS_OK(status = werror_to_ntstatus(werr))) {
		DEBUG_OUT (3, ("Failed to delete job %d [%s]\n",
