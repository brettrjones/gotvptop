...
static const char *iprint_server(void)
{
	if ((lp_iprint_server() != NULL) && (strlen(lp_iprint_server()) > 0)) {
		DEBUG_OUT (10, ("iprint server explicitly set to %s\n",
			   lp_iprint_server()));
		return lp_iprint_server();
	}

	DEBUG_OUT (10, ("iprint server left to default %s\n", cupsServer()));
	return cupsServer();
}
...
bool iprint_cache_reload(void)
{
	...
	DEBUG_OUT (5, ("reloading iprint printcap cache\n"));
	...
	*/
	...
	if ((http = httpConnect(iprint_server(), ippPort())) == NULL) {
		DEBUG_OUT (0,("Unable to connect to iPrint server %s - %s\n", 
...
static int iprint_job_delete(const char *sharename, const char *lprm_command, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("iprint_job_delete(%s, %p (%d))\n", sharename, pjob, pjob->sysjob));
...
static int iprint_job_pause(int snum, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("iprint_job_pause(%d, %p (%d))\n", snum, pjob, pjob->sysjob));
...
static int iprint_job_resume(int snum, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("iprint_job_resume(%d, %p (%d))\n", snum, pjob, pjob->sysjob));
			     ...
			     char *lpq_cmd)
{
	...
	DEBUG_OUT (5,("iprint_job_submit(%d, %p (%d))\n", snum, pjob, pjob->sysjob));
			    ...
			    print_status_struct *status)
{
	...
	*q = NULL;
	...
	DEBUG_OUT (5,("iprint_queue_get(%s, %p, %p)\n", printername, q, status));
	...
	for (attr = ippFirstAttribute(response); attr != NULL; attr = ippNextAttribute(response)) {
		...
		*/
		if (qcount >= qalloc) {
			...
			if (queue == NULL) {
				DEBUG_OUT (0,("iprint_queue_get: Not enough memory!"));
				qcount = 0;
				goto out;
			}
		}
