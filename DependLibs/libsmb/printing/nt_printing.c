...
bool nt_printing_init(struct messaging_context *msg_ctx)
{
	...
	if ( lp_security() == SEC_ADS ) {
		...
		if (!W_ERROR_IS_OK(win_rc))
			DEBUG_OUT (0, ("nt_printing_init: error checking published printers: %s\n", win_errstr(win_rc)));
	}
...
const char *get_short_archi(const char *long_archi)
{
        ...
        DEBUG_OUT (107,("Getting architecture dependent directory\n"));
        do {
                i++;
...
static int get_file_version(files_struct *fsp, char *fname,uint32 *major, uint32 *minor)
{
	...
	if ((buf=(char *)SMB_MALLOC(DOS_HEADER_SIZE)) == NULL) {
		DEBUG_OUT (0,("get_file_version: PE file [%s] DOS Header malloc failed bytes = %d\n",
				fname, DOS_HEADER_SIZE));
		goto error_exit;
	}

	if ((byte_count = vfs_read_data(fsp, buf, DOS_HEADER_SIZE)) < DOS_HEADER_SIZE) {
		DEBUG_OUT (3,("get_file_version: File [%s] DOS header too short, bytes read = %lu\n",
	...
	if (SVAL(buf,DOS_HEADER_MAGIC_OFFSET) != DOS_HEADER_MAGIC) {
		DEBUG_OUT (6,("get_file_version: File [%s] bad DOS magic = 0x%x\n",
	...
	if (SMB_VFS_LSEEK(fsp, SVAL(buf,DOS_HEADER_LFANEW_OFFSET), SEEK_SET) == (SMB_OFF_T)-1) {
		DEBUG_OUT (3,("get_file_version: File [%s] too short, errno = %d\n",
				fname, errno));
	...
	if ((byte_count = vfs_read_data(fsp, buf, NE_HEADER_SIZE)) < NE_HEADER_SIZE) {
		DEBUG_OUT (3,("get_file_version: File [%s] Windows header too short, bytes read = %lu\n",
			 fname, (unsigned long)byte_count));
		/* Assume this isn't an error... the file just looks sort of like a PE/NE file */
		goto no_version_info;
	}
	...
	if (IVAL(buf,PE_HEADER_SIGNATURE_OFFSET) == PE_HEADER_SIGNATURE) {
		...
		if (SMB_VFS_LSEEK(fsp,
			...
			DEBUG_OUT (3,("get_file_version: File [%s] Windows optional header too short, errno = %d\n",
				fname, errno));
			goto error_exit;
		}
		...
		if ((buf=(char *)SMB_MALLOC(section_table_bytes)) == NULL) {
			DEBUG_OUT (0,("get_file_version: PE file [%s] section table malloc failed bytes = %d\n",
					fname, section_table_bytes));
			goto error_exit;
		}

		if ((byte_count = vfs_read_data(fsp, buf, section_table_bytes)) < section_table_bytes) {
			DEBUG_OUT (3,("get_file_version: PE file [%s] Section header too short, bytes read = %lu\n",
				 fname, (unsigned long)byte_count));
			goto error_exit;
		}
		...
		for (i = 0; i < num_sections; i++) {
			...
			if (strcmp(".rsrc", &buf[sec_offset+PE_HEADER_SECT_NAME_OFFSET]) == 0) {
				...
				if ((buf=(char *)SMB_MALLOC(section_bytes)) == NULL) {
					DEBUG_OUT (0,("get_file_version: PE file [%s] version malloc failed bytes = %d\n",
							fname, section_bytes));
					goto error_exit;
				}
				...
				if (SMB_VFS_LSEEK(fsp, section_pos, SEEK_SET) == (SMB_OFF_T)-1) {
					DEBUG_OUT (3,("get_file_version: PE file [%s] too short for section info, errno = %d\n",
							fname, errno));
					goto error_exit;
				}

				if ((byte_count = vfs_read_data(fsp, buf, section_bytes)) < section_bytes) {
					DEBUG_OUT (3,("get_file_version: PE file [%s] .rsrc section too short, bytes read = %lu\n",
						 fname, (unsigned long)byte_count));
					goto error_exit;
				}
					...
					if (buf[i] == 'V' && buf[i+1] == '\0' && buf[i+2] == 'S') {
						...
						if (IVAL(buf,pos) == VS_MAGIC_VALUE) {
							...
							*minor = IVAL(buf,pos+VS_MINOR_OFFSET);
							...
							DEBUG_OUT (6,("get_file_version: PE file [%s] Version = %08x:%08x (%d.%d.%d.%d)\n",
									  fname, *major, *minor,
		...
		DEBUG_OUT (10,("get_file_version: PE file [%s] has no version info\n", fname));
		SAFE_FREE(buf);
		return 0;

	} else if (SVAL(buf,NE_HEADER_SIGNATURE_OFFSET) == NE_HEADER_SIGNATURE) {
		if (CVAL(buf,NE_HEADER_TARGET_OS_OFFSET) != NE_HEADER_TARGOS_WIN ) {
			DEBUG_OUT (3,("get_file_version: NE file [%s] wrong target OS = 0x%x\n",
		...
		if ((buf=(char *)SMB_MALLOC(VS_NE_BUF_SIZE)) == NULL) {
			DEBUG_OUT (0,("get_file_version: NE file [%s] malloc failed bytes  = %d\n",
			...
			for(i=0; i<byte_count; i++) {
				...
				if (i>byte_count-VS_VERSION_INFO_SIZE) {
					...
					if ((bc = vfs_read_data(fsp, &buf[byte_count-i], VS_NE_BUF_SIZE-
						...
						DEBUG_OUT (0,("get_file_version: NE file [%s] Read error, errno=%d\n",
								 fname, errno));
						goto error_exit;
					}
				...
				if (strcmp(&buf[i], VS_SIGNATURE) == 0) {
					...
					*minor = IVAL(buf,i+sizeof(VS_SIGNATURE)+skip+VS_MINOR_OFFSET);
					DEBUG_OUT (6,("get_file_version: NE file [%s] Version = %08x:%08x (%d.%d.%d.%d)\n",
							  fname, *major, *minor,
							  (*major>>16)&0xffff, *major&0xffff,
							  (*minor>>16)&0xffff, *minor&0xffff));
					SAFE_FREE(buf);
		...
		/* Version info not found, fall back to origin date/time */
		DEBUG_OUT (0,("get_file_version: NE file [%s] Version info not found\n", fname));
		SAFE_FREE(buf);
		return 0;

	} else
		/* Assume this isn't an error... the file just looks sort of like a PE/NE file */
		DEBUG_OUT (3,("get_file_version: File [%s] unknown file format, signature = 0x%x\n",
...
static int file_version_is_newer(connection_struct *conn, fstring new_file, fstring old_file)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (10,("file_version_is_newer: Can't open old file [%s], "
		...
		if (!ret) {
			DEBUG_OUT (6,("file_version_is_newer: Version info not found [%s], use mod time\n",
					 old_file));
			use_version = false;
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (3,("file_version_is_newer: Can't open new file [%s], "
	...
	if (use_version && (new_major != old_major || new_minor != old_minor)) {
		...
		if (new_major > old_major ||
			...
			DEBUG_OUT (6,("file_version_is_newer: Replacing [%s] with [%s]\n", old_file, new_file));
			ret = 1;
			goto done;
		}
		else {
			DEBUG_OUT (6,("file_version_is_newer: Leaving [%s] unchanged\n", old_file));
			ret = 0;
			goto done;
		}
				   ...
				   WERROR *perr)
{
	...
	*perr = WERR_INVALID_PARAM;
	...
	if (strcmp(architecture, SPL_ARCH_WIN40) == 0) {
		DEBUG_OUT (10,("get_correct_cversion: Driver is Win9x, cversion = 0\n"));
		*perr = WERR_OK;
		return 0;
	}
	...
	if (strcmp(architecture, SPL_ARCH_X64) == 0) {
		DEBUG_OUT (10,("get_correct_cversion: Driver is x64, cversion = 3\n"));
		*perr = WERR_OK;
		return 3;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("get_correct_cversion: create_conn_struct "
			 "returned %s\n", nt_errstr(nt_status)));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("failed set force user / group\n"));
		*perr = ntstatus_to_werror(nt_status);
		goto error_free_conn;
	}

	if (!become_user_by_session(conn, session_info)) {
		DEBUG_OUT (0, ("failed to become user\n"));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("get_correct_cversion: vfs_file_exist failed\n"));
		*perr = WERR_BADFILE;
		goto error_exit;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (3,("get_correct_cversion: Can't open file [%s], errno = "
			 "%d\n", smb_fname_str_dbg(smb_fname), errno));
		*perr = WERR_ACCESS_DENIED;
		goto error_exit;
	} else {
		...
		if (ret == -1) {
			*perr = WERR_INVALID_PARAM;
			...
			DEBUG_OUT (6,("get_correct_cversion: Version info not "
				 "found [%s]\n",
				 smb_fname_str_dbg(smb_fname)));
			*perr = WERR_INVALID_PARAM;
			goto error_exit;
		...
		*/
		...
		switch (cversion) {
			...
			default:
				DEBUG_OUT (6,("get_correct_cversion: cversion "
		...
		DEBUG_OUT (10,("get_correct_cversion: Version info found [%s] major"
			  " = 0x%x  minor = 0x%x\n",
			  smb_fname_str_dbg(smb_fname), major, minor));
	}

	DEBUG_OUT (10,("get_correct_cversion: Driver file [%s] cversion = %d\n",
						...
						uint32_t version)
{
	...
	if (version != -1 && (version = file_version_is_newer(conn, old_name, new_name)) > 0) {
		...
		DEBUG_OUT (10,("move_driver_file_to_download_area: copying '%s' to "
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("move_driver_file_to_download_area: Unable "
				 "to rename [%s] to [%s]: %s\n",
				    ...
				    struct spoolss_AddDriverInfoCtr *r)
{
	...
	case 6:
		...
		break;
	default:
		DEBUG_OUT (0,("move_driver_to_download_area: Unknown info level (%u)\n", (unsigned int)r->level));
	...
	nt_status = create_conn_struct(talloc_tos(), &conn, printdollar_snum,
				       ...
				       session_info, &oldcwd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("move_driver_to_download_area: create_conn_struct "
			 "returned %s\n", nt_errstr(nt_status)));
                           ...
                           const struct spoolss_DriverInfo8 *r)
{
	...
	DEBUG_OUT (10,("printer_driver_in_use: Beginning search through ntprinters.tdb...\n"));
	...
	DEBUG_OUT (10,("printer_driver_in_use: Completed search through ntprinters.tdb...\n"));

	if ( in_use ) {
		struct spoolss_DriverInfo8 *driver;
		WERROR werr;

		DEBUG_OUT (5,("printer_driver_in_use: driver \"%s\" is currently in use\n", r->driver_name));
		...
		if (!strequal("Windows NT x86", r->architecture)) {
			...
			DEBUG_OUT (0, ("printer_driver_in_use: ERROR!"
				   ...
				   const struct spoolss_DriverInfo8 *drv)
{
	...
	if (drv_file_in_use(src->driver_path, drv)) {
		in_use = True;
		DEBUG_OUT (10,("Removing driverfile [%s] from list\n", src->driver_path));
	...
	if (drv_file_in_use(src->data_file, drv)) {
		in_use = True;
		DEBUG_OUT (10,("Removing datafile [%s] from list\n", src->data_file));
	...
	if (drv_file_in_use(src->config_file, drv)) {
		in_use = True;
		DEBUG_OUT (10,("Removing configfile [%s] from list\n", src->config_file));
	...
	if (drv_file_in_use(src->help_file, drv)) {
		in_use = True;
		DEBUG_OUT (10,("Removing helpfile [%s] from list\n", src->help_file));
	...
	while (src->dependent_files[i] && *src->dependent_files[i]) {
		if (drv_file_in_use(src->dependent_files[i], drv)) {
			in_use = True;
			DEBUG_OUT (10,("Removing [%s] from dependent file list\n", src->dependent_files[i]));
				 ...
				 struct spoolss_DriverInfo8 *info)
{
	...
	DEBUG_OUT (5,("printer_driver_files_in_use: Beginning search of drivers...\n"));
	...
	DEBUG_OUT (5,("printer_driver_files_in_use: Completed search of drivers...\n"));
			 ...
			 const struct spoolss_DriverInfo8 *r)
{
	...
	DEBUG_OUT (6,("delete_driver_files: deleting driver [%s] - version [%d]\n",
	...
	nt_status = create_conn_struct(talloc_tos(), &conn, printdollar_snum,
				       ...
				       session_info, &oldcwd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("delete_driver_files: create_conn_struct "
			 "returned %s\n", nt_errstr(nt_status)));
		return false;
	}
	...
	if ( !CAN_WRITE(conn) ) {
		DEBUG_OUT (3,("delete_driver_files: Cannot delete print driver when [print$] is read-only\n"));
		ret = false;
		goto err_out;
	}
	...
	if (short_arch == NULL) {
		DEBUG_OUT (0, ("bad architecture %s\n", r->architecture));
		ret = false;
		goto err_out;
	}
	...
	if (r->dependent_files) {
		...
		while (r->dependent_files[i] && r->dependent_files[i][0]) {
			DEBUG_OUT (10,("deleting dependent file [%s]\n", r->dependent_files[i]));
			...
			int access_type)
{
	...
	DEBUG_OUT (4, ("access check was %s\n", NT_STATUS_IS_OK(status) ? "SUCCESS" : "FAILURE"));
			...
			const char *printer)
{
	...
	result = winreg_delete_printer_key_internal(mem_ctx, session_info, msg_ctx,
					   printer, "");
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("nt_printer_remove: failed to remove printer %s: "
		    ...
		    const char *printer)
{
	...
	result = winreg_create_printer_internal(mem_ctx, session_info, msg_ctx,
						printer);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (0, ("nt_printer_add: failed to add printer %s: %s\n",
			  printer, win_errstr(result)));
	}
}
