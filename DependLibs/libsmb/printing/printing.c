...
uint16 pjobid_to_rap(const char* sharename, uint32 jobid)
{
	...
	DEBUG_OUT (10,("pjobid_to_rap: called.\n"));
	...
	if (data.dptr && data.dsize == sizeof(uint16)) {
		...
		SAFE_FREE(data.dptr);
		DEBUG_OUT (10,("pjobid_to_rap: jobid %u maps to RAP jobid %u\n",
	...
	DEBUG_OUT (10,("pjobid_to_rap: created jobid %u maps to RAP jobid %u\n",
		(unsigned int)jobid, (unsigned int)rap_jobid));
	return rap_jobid;
}
...
bool rap_to_pjobid(uint16 rap_jobid, fstring sharename, uint32 *pjobid)
{
	...
	DEBUG_OUT (10,("rap_to_pjobid called.\n"));
	...
	if ( data.dptr && data.dsize == sizeof(struct rap_jobid_key) )
	{
		...
		*pjobid = jinfo->jobid;
		DEBUG_OUT (10,("rap_to_pjobid: jobid %u maps to RAP jobid %u\n",
			(unsigned int)*pjobid, (unsigned int)rap_jobid));
		SAFE_FREE(data.dptr);
		return True;
	}

	DEBUG_OUT (10,("rap_to_pjobid: Failed to lookup RAP jobid %u\n",
...
void rap_jobid_delete(const char* sharename, uint32 jobid)
{
	...
	DEBUG_OUT (10,("rap_jobid_delete: called.\n"));
	...
	data = tdb_fetch(rap_tdb, key);
	if (!data.dptr || (data.dsize != sizeof(uint16))) {
		DEBUG_OUT (10,("rap_jobid_delete: cannot find jobid %u\n",
			(unsigned int)jobid ));
		SAFE_FREE(data.dptr);
		return;
	}

	DEBUG_OUT (10,("rap_jobid_delete: deleting jobid %u\n",
		(unsigned int)jobid ));
...
bool print_backend_init(struct messaging_context *msg_ctx)
{
	...
	for (snum = 0; snum < services; snum++) {
		...
		if (tdb_lock_bystring(pdb->tdb, sversion) == -1) {
			DEBUG_OUT (0,("print_backend_init: Failed to open printer %s database\n", lp_const_servicename(snum) ));
...
static int pack_devicemode(struct spoolss_DeviceMode *devmode, uint8 *buf, int buflen)
{
	...
	if (devmode) {
		DEBUG_OUT (8, ("Packed devicemode [%s]\n", devmode->formname));
	}
		      ...
		      struct spoolss_DeviceMode **devmode)
{
	...
	*devmode = NULL;
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (10, ("unpack_devicemode: "
			   "error parsing spoolss_DeviceMode\n"));
		goto done;
	}

	DEBUG_OUT (8, ("Unpacked devicemode [%s](%s)\n",
		  dm->devicename, dm->formname));
	if (dm->driverextra_data.data) {
		DEBUG_OUT (8, ("with a private section of %d bytes\n",
				       ...
				       uint32 jobid)
{
	...
	DEBUG_OUT (10,("print_job_find: looking up job %u for share %s\n",
	...
	if (!ret.dptr) {
		DEBUG_OUT (10, ("print_job_find: failed to find jobid %u.\n",
			   jobid));
		return NULL;
	}
	...
	if (unpack_pjob(mem_ctx, ret.dptr, ret.dsize, pjob) == -1) {
		DEBUG_OUT (10, ("failed to unpack jobid %u.\n", jobid));
		talloc_free(pjob);
		pjob = NULL;
		goto err_out;
	}

	DEBUG_OUT (10,("print_job_find: returning system job %d for jobid %u.\n",
		  pjob->sysjob, jobid));
...
static bool add_to_jobs_changed(struct tdb_print_db *pdb, uint32_t jobid)
{
	...
	DEBUG_OUT (10,("add_to_jobs_added: Added jobid %u\n", (unsigned int)jobid ));
...
static bool remove_from_jobs_changed(const char* sharename, uint32_t jobid)
{
  ...
  out:
	...
	release_print_db(pdb);
	if (ret)
		DEBUG_OUT (10,("remove_from_jobs_changed: removed jobid %u\n", (unsigned int)jobid ));
	else
		DEBUG_OUT (10,("remove_from_jobs_changed: Failed to remove jobid %u\n", (unsigned int)jobid ));
	return ret;
}
		       ...
		       struct printjob *pjob)
{
	...
	do {
		...
		if (buflen != len) {
			...
			if (!buf) {
				DEBUG_OUT (0,("pjob_store: failed to enlarge buffer!\n"));
				goto done;
			}
			newlen = len;
		}
			...
			const char* sharename, uint32 jobid)
{
	...
	if (!pjob) {
		DEBUG_OUT (5, ("we were asked to delete nonexistent job %u\n",
			  jobid));
		goto err_release;
	}
...
static int traverse_fn_delete(TDB_CONTEXT *t, TDB_DATA key, TDB_DATA data, void *state)
{
	...
	if (!pjob.smbjob) {
		...
		if (i == ts->qcount) {
			DEBUG_OUT (10,("traverse_fn_delete: pjob %u deleted due to !smbjob\n",
						(unsigned int)jobid ));
	...
	if (!pjob.spooled) {
		...
		if (!process_exists_by_pid(pjob.pid)) {
			DEBUG_OUT (10,("traverse_fn_delete: pjob %u deleted due to !process_exists (%u)\n",
	...
	if (i == ts->qcount) {
		...
		if (pjob.starttime < ts->lpq_time) {
			DEBUG_OUT (10,("traverse_fn_delete: pjob %u deleted due to pjob.starttime (%u) < ts->lpq_time (%u)\n",
						(unsigned int)jobid,
...
static void set_updating_pid(const fstring sharename, bool updating)
{
	...
	DEBUG_OUT (5, ("set_updating_pid: %s updating lpq cache for print share %s\n",
		updating ? "" : "not ",
		sharename ));
...
static bool print_cache_expired(const char *sharename, bool check_pending)
{
		...
		|| last_qscan_time > (time_now + MAX_CACHE_VALID_TIME))
	{
		...
		DEBUG_OUT (4, ("print_cache_expired: cache expired for queue %s "
		...
		{
			DEBUG_OUT (4,("print_cache_expired: message already pending for %s.  Accepting cache\n",
				sharename));
			goto done;
		}
                                        ...
                                        char *lpq_command, char *lprm_command)
{
	...
	DEBUG_OUT (5,("print_queue_update_internal: printer = %s, type = %d, lpq command = [%s]\n",
	...
	DEBUG_OUT (3, ("print_queue_update_internal: %d job%s in queue for %s\n",
	...
	*/
	...
	for (i=0; i<qcount; i++) {
		...
		if (!pjob) {
			/* err, somethings wrong. Probably smbd was restarted
			   ...
			   like unix jobs. Pity. */
			DEBUG_OUT (1, ("queued print job %d not found in jobs list, "
				  "assuming unix job\n", jobid));
			print_unix_job(ev, msg_ctx,
				       sharename, &queue[i], jobid);
			continue;
	...
	DEBUG_OUT (10,("print_queue_update_internal: printer %s INFO/total_jobs = %d\n",
	...
	if (old_status.qcount != qcount)
		DEBUG_OUT (10,("print_queue_update_internal: queue status change %d jobs -> %d jobs for printer %s\n",
	...
	if ( !tdb_store_uint32( pdb->tdb, keystr, 0 ) ) {
		...
		DEBUG_OUT (0,("print_queue_update: failed to store MSG_PENDING flag for [%s]!\n",
			sharename));
	}
                                          ...
                                          char *lpq_command, char *lprm_command )
{
	...
	DEBUG_OUT (5,("print_queue_update_with_lock: printer share = %s\n", sharename));
	pdb = get_print_db_byname(sharename);
	if (!pdb)
		return;

	if ( !print_cache_expired(sharename, False) ) {
		DEBUG_OUT (5,("print_queue_update_with_lock: print cache for %s is still ok\n", sharename));
		release_print_db(pdb);
		return;
	}
	...
	if (tdb_lock_bystring_with_timeout(pdb->tdb, keystr, 10) == -1) {
		DEBUG_OUT (0,("print_queue_update_with_lock: Failed to lock printer %s database\n", sharename));
		release_print_db(pdb);
		return;
	}
				...
				DATA_BLOB *data)
{
	...
	if ( len == -1 ) {
		...
		DEBUG_OUT (0,("print_queue_receive: Got invalid print queue update message\n"));
		return;
	}
...
static void add_child_pid(pid_t pid)
{
	struct child_pid *child;
        ...
        if (child == NULL) {
                DEBUG_OUT (0, ("Could not add child struct -- malloc failed\n"));
                return;
        }
        child->pid = pid;
			    ...
			    struct messaging_context *msg_ctx)
{
	...
	DEBUG_OUT (3,("start_background_queue: Starting background LPQ thread\n"));

	if (pipe(pause_pipe) == -1) {
		DEBUG_OUT (5,("start_background_queue: cannot create pipe. %s\n", strerror(errno) ));
		exit(1);
	}
	...
	if (background_lpq_updater_pid == -1) {
		DEBUG_OUT (5,("start_background_queue: background LPQ thread failed to start. %s\n", strerror(errno) ));
		exit(1);
	}
	...
	if(background_lpq_updater_pid == 0) {
		...
		/* Child. */
		DEBUG_OUT (5,("start_background_queue: background LPQ thread started\n"));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0,("reinit_after_fork() failed\n"));
		...
		fde = tevent_add_fd(ev, ev, pause_pipe[1], TEVENT_FD_READ,
				    ...
				    NULL);
		if (!fde) {
			DEBUG_OUT (0,("tevent_add_fd() failed for pause_pipe\n"));
		...
		if (!(event_add_idle(ev, NULL,
				     ...
				     NULL))) {
			DEBUG_OUT (0, ("Could not add printing housekeeping event\n"));
			exit(1);
		}

		DEBUG_OUT (5,("start_background_queue: background LPQ thread waiting for messages\n"));
		ret = tevent_loop_wait(ev);
		/* should not be reached */
		DEBUG_OUT (0,("background_queue: tevent_loop_wait() exited with %d - %s\n",
			       ...
			       int snum, bool force)
{
	...
	if ( force || background_lpq_updater_pid == -1 ) {
		DEBUG_OUT (4,("print_queue_update: updating queue [%s] myself\n", sharename));
	...
	DEBUG_OUT (10,("print_queue_update: Sending message -> printer = %s, "
...
bool print_notify_register_pid(int snum)
{
	...
	if (tdb_lock_bystring_with_timeout(tdb, NOTIFY_PID_LIST_KEY, 10) == -1) {
		DEBUG_OUT (0,("print_notify_register_pid: Failed to lock printer %s\n",
					printername));
		if (pdb)
			release_print_db(pdb);
		return False;
	...
	if (i == data.dsize) {
		...
		if (!data.dptr) {
			DEBUG_OUT (0,("print_notify_register_pid: Relloc fail for printer %s\n",
						printername));
			goto done;
		}
		data.dsize += 8;
	...
	if (tdb_store_bystring(tdb, NOTIFY_PID_LIST_KEY, data, TDB_REPLACE) == -1) {
		DEBUG_OUT (0,("print_notify_register_pid: Failed to update pid \
list for printer %s\n", printername));
		goto done;
	}
...
bool print_notify_deregister_pid(int snum)
{
	...
	if (tdb_lock_bystring_with_timeout(tdb, NOTIFY_PID_LIST_KEY, 10) == -1) {
		DEBUG_OUT (0,("print_notify_register_pid: Failed to lock \
printer %s database\n", printername));
		if (pdb)
			release_print_db(pdb);
		return False;
...
static bool remove_from_jobs_added(const char* sharename, uint32 jobid)
{
  ...
  out:
	...
	release_print_db(pdb);
	if (ret)
		DEBUG_OUT (10,("remove_from_jobs_added: removed jobid %u\n", (unsigned int)jobid ));
	else
		DEBUG_OUT (10,("remove_from_jobs_added: Failed to remove jobid %u\n", (unsigned int)jobid ));
	return ret;
}
			      ...
			      int snum, uint32 jobid)
{
	...
	if (pjob->sysjob == -1) {
		DEBUG_OUT (5, ("attempt to delete job %u not seen by lpr\n", (unsigned int)jobid));
	}
			...
			int snum, uint32_t jobid)
{
	...
	if (!owner &&
		...
		DEBUG_OUT (3, ("delete denied by security descriptor\n"));
...
pause, or resume print job. User name: %s. Printer name: %s.",
	...
	if (!pjob || pjob->spooled || pjob->pid != getpid()) {
		DEBUG_OUT (10, ("Skipping spool file removal for job %u\n", jobid));
	} else {
		DEBUG_OUT (10, ("Removing spool file [%s]\n", pjob->filename));
		     ...
		     int snum, uint32 jobid)
{
	...
	pjob = print_job_find(tmp_ctx, sharename, jobid);
	if (!pjob || !server_info) {
		DEBUG_OUT (10, ("print_job_pause: no pjob or user for jobid %u\n",
			(unsigned int)jobid ));
	...
	if (!pjob->spooled || pjob->sysjob == -1) {
		DEBUG_OUT (10, ("print_job_pause: not spooled or bad sysjob = %d for jobid %u\n",
	...
	if (!is_owner(server_info, lp_const_servicename(snum), jobid) &&
	    !print_access_check(server_info, msg_ctx, snum,
				JOB_ACCESS_ADMINISTER)) {
		DEBUG_OUT (3, ("pause denied by security descriptor\n"));
		      ...
		      int snum, uint32 jobid)
{
	...
	pjob = print_job_find(tmp_ctx, sharename, jobid);
	if (!pjob || !server_info) {
		DEBUG_OUT (10, ("print_job_resume: no pjob or user for jobid %u\n",
			(unsigned int)jobid ));
		werr = WERR_INVALID_PARAM;
		goto err_out;
	}
	...
	if (!pjob->spooled || pjob->sysjob == -1) {
		DEBUG_OUT (10, ("print_job_resume: not spooled or bad sysjob = %d for jobid %u\n",
			(int)pjob->sysjob, (unsigned int)jobid ));
		werr = WERR_INVALID_PARAM;
		goto err_out;
	}
	...
	if (!is_owner(server_info, lp_const_servicename(snum), jobid) &&
	    !print_access_check(server_info, msg_ctx, snum,
				JOB_ACCESS_ADMINISTER)) {
		DEBUG_OUT (3, ("resume denied by security descriptor\n"));
	...
	*pjobid = (uint32)-1;
	...
	for (i = 0; i < 3; i++) {
		...
		ret = tdb_lock_bystring_with_timeout(pdb->tdb,
						     "INFO/nextjob", 20);
		if (ret == -1) {
			DEBUG_OUT (0, ("allocate_print_jobid: "
		...
		if (!tdb_fetch_uint32(pdb->tdb, "INFO/nextjob", &jobid)) {
			...
			DEBUG_OUT (10, ("allocate_print_jobid: "
		...
		if (ret == -1) {
			terr = tdb_error(pdb->tdb);
			DEBUG_OUT (3, ("allocate_print_jobid: "
			       ...
			       int snum, int *njobs)
{
	...
	if (lp_autoloaded(snum) && !pcap_printername_ok(sharename)) {
		DEBUG_OUT (3, ("print_job_checks: printer name %s check failed.\n",
			  sharename));
		return WERR_ACCESS_DENIED;
	}
	...
	*njobs = print_queue_length(msg_ctx, snum, NULL);
	if (*njobs > lp_maxprintjobs(snum)) {
		DEBUG_OUT (3, ("print_job_checks: Queue %s number of jobs (%d) "
				   ...
				   struct printjob *pjob)
{
	...
	if (output_file) {
		...
		if (strncmp(output_file, path, len) == 0 &&
			...
			DEBUG_OUT (3, ("print_job_spool_file:"
	...
	if (pjob->fd == -1) {
		...
		if (W_ERROR_EQUAL(werr, WERR_ACCESS_DENIED)) {
			...
			DEBUG_OUT (0, ("print_job_spool_file: "
...
fail:
	...
	DEBUG_OUT (3, ("print_job_start: returning fail. "
		  "Error = %s\n", win_errstr(werr)));
	return werr;
}
		       ...
		       uint32 jobid, enum file_close_type close_type)
{
	...
	if (pjob->size == 0 || pjob->status == LPQ_DELETING) {
		...
		DEBUG_OUT (5,("print_job_end: canceling spool of %s (%s)\n",
				  ...
				  int *pcount, print_queue_struct **ppqueue)
{
	...
	*ppqueue = NULL;
	...
	DEBUG_OUT (5,("get_stored_queue_info: qcount = %u, extra_count = %u\n", (unsigned int)qcount, (unsigned int)extra_count));
	...
	for( i  = 0; i < extra_count; i++) {
		...
		DEBUG_OUT (5,("get_stored_queue_info: added job = %u\n", (unsigned int)jobid));
		pjob = print_job_find(tmp_ctx, lp_const_servicename(snum), jobid);
		if (!pjob) {
			DEBUG_OUT (5,("get_stored_queue_info: failed to find added job = %u\n", (unsigned int)jobid));
			remove_from_jobs_added(sharename, jobid);
			continue;
		}
	...
	for (i = 0; i < changed_count; i++) {
		...
		if (found) {
			...
			DEBUG_OUT (5,("get_stored_queue_info: changed job: %u\n",
				 (unsigned int) jobid));

			pjob = print_job_find(tmp_ctx, sharename, jobid);
			if (pjob == NULL) {
				DEBUG_OUT (5,("get_stored_queue_info: failed to find "
					 "changed job = %u\n",
					 (unsigned int) jobid));
			...
			DEBUG_OUT (5,("get_stored_queue_info: updated queue[%u], jobid: %u, jobname: %s\n",
	...
	DEBUG_OUT (5,("get_stored_queue_info: total_count = %u\n", (unsigned int)total_count));
