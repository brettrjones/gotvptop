				 ...
				 struct rpc_pipe_client *winreg_pipe)
{
	...
	if (tdb == NULL && errno == ENOENT) {
		...
		DEBUG_OUT (4, ("No printers database to migrate in %s\n", tdb_path));
		return NT_STATUS_OK;
	}
	if (tdb == NULL) {
		DEBUG_OUT (2, ("Failed to open tdb file: %s\n", tdb_path));
	     ...
	     newkey = tdb_nextkey(tdb, kbuf), free(kbuf.dptr), kbuf = newkey)
	{
		...
		if (strncmp((const char *) kbuf.dptr, SECDESC_PREFIX, strlen(SECDESC_PREFIX)) == 0) {
			...
			if (NT_STATUS_EQUAL(status, werror_to_ntstatus(WERR_BADFILE))) {
				DEBUG_OUT (2, ("Skipping secdesc migration for non-existent "
						"printer: %s\n", secdesc_name));
	...
	if (rc != 0) {
		DEBUG_OUT (0, ("Error moving tdb to '%s%s'\n",
...
bool nt_printing_tdb_migrate(struct messaging_context *msg_ctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Couldn't create session_info: %s\n",
			  nt_errstr(status)));
		talloc_free(tmp_ctx);
		return false;
	}
	...
	status = rpc_pipe_open_interface(tmp_ctx,
					...
					&winreg_pipe);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Couldn't open internal winreg pipe: %s\n",
			  nt_errstr(status)));
		talloc_free(tmp_ctx);
		return false;
	}
	...
	if (drivers_exists) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Couldn't migrate drivers tdb file: %s\n",
			  nt_errstr(status)));
			talloc_free(tmp_ctx);
			return false;
		}
	...
	if (printers_exists) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Couldn't migrate printers tdb file: %s\n",
				  nt_errstr(status)));
			talloc_free(tmp_ctx);
			return false;
		}
	...
	if (forms_exists) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Couldn't migrate forms tdb file: %s\n",
				  nt_errstr(status)));
			talloc_free(tmp_ctx);
			return false;
		}
