				   ...
				   size_t length)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (2, ("Form pull failed: %s\n",
			  ndr_errstr(ndr_err)));
				     ...
				     size_t length)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (2, ("Driver pull failed: %s\n",
			  ndr_errstr(ndr_err)));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (2, ("Migrating Printer Driver: %s\n", key_name));
				      ...
				      size_t length)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (2, ("printer pull failed: %s\n",
			  ndr_errstr(ndr_err)));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (2, ("Migrating Printer: %s\n", key_name));
	...
	result = winreg_update_printer(mem_ctx, b,
				       ...
				       NULL);
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (2, ("SetPrinter(%s) level 2 refused -- %s.\n",
	...
	for (j = 0; j < r.count; j++) {
		...
		if (!W_ERROR_IS_OK(result)) {
			DEBUG_OUT (2, ("SetPrinterDataEx: printer [%s], keyname [%s], "
				      ...
				      size_t length)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (2, ("security descriptor pull failed: %s\n",
			  ndr_errstr(ndr_err)));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (2, ("Migrating Security Descriptor: %s\n", key_name));
