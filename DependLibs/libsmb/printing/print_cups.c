...
static http_t *cups_connect(TALLOC_CTX *frame)
{
	...
	DEBUG_OUT (10, ("connecting to cups server %s:%d\n",
		   server, port));
	...
	if (http == NULL) {
		DEBUG_OUT (0,("Unable to connect to CUPS server %s:%d - %s\n",
...
static bool send_pcap_blob(DATA_BLOB *pcap_blob, int fd)
{
	...
	DEBUG_OUT (10, ("successfully sent blob of len %d\n", (int)ret));
	return true;
}
...
static bool recv_pcap_blob(TALLOC_CTX *mem_ctx, int fd, DATA_BLOB *pcap_blob)
{
	...
	*pcap_blob = data_blob_talloc_named(mem_ctx, NULL, blob_len,
	...
	DEBUG_OUT (10, ("successfully recvd blob of len %d\n", (int)ret));
	return true;
}
...
static bool cups_cache_reload_async(int fd)
{
	...
	DEBUG_OUT (5, ("reloading cups printcap cache\n"));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG_OUT (0,("Unable to get printer list - %s\n",
	...
	if (!ret) {
		DEBUG_OUT (0,("failed to process cups response\n"));
		goto out;
	}
				 ...
				 int *pfd)
{
	...
	*pfd = -1;
	...
	if (cache_fd_event) {
		DEBUG_OUT (3,("cups_pcap_load_async: already waiting for "
			"a refresh event\n" ));
		return false;
	}

	DEBUG_OUT (5,("cups_pcap_load_async: asynchronously loading cups printers\n"));
	...
	pid = sys_fork();
	if (pid == (pid_t)-1) {
		DEBUG_OUT (10,("cups_pcap_load_async: fork failed %s\n",
			strerror(errno) ));
		close(fds[0]);
		close(fds[1]);
		return false;
	...
	if (pid) {
		DEBUG_OUT (10,("cups_pcap_load_async: child pid = %u\n",
			(unsigned int)pid ));
		/* Parent. */
		close(fds[1]);
		*pfd = fds[0];
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("cups_pcap_load_async: reinit_after_fork() failed\n"));
				...
				void *p)
{
	...
	DEBUG_OUT (5,("cups_async_callback: callback received for printer data. "
		"fd = %d\n", cb_args->pipe_fd));

	ret_ok = recv_pcap_blob(frame, cb_args->pipe_fd, &pcap_blob);
	if (!ret_ok) {
		DEBUG_OUT (0,("failed to recv pcap blob\n"));
		goto err_out;
	}
	...
	if (!NT_STATUS_IS_OK(pcap_data.status)) {
		DEBUG_OUT (0,("failed to retrieve printer list: %s\n",
	...
	for (i = 0; i < pcap_data.count; i++) {
		...
		if (!ret_ok) {
			DEBUG_OUT (0, ("failed to add to tmp pcap cache\n"));
			goto err_out;
		}
	}
	...
	if (!ret_ok) {
		DEBUG_OUT (0, ("failed to replace pcap cache\n"));
						  ...
						  struct messaging_context *))
{
	...
	*p_pipe_fd = -1;
	...
	DEBUG_OUT (10,("cups_cache_reload: async read on fd %d\n",
		*p_pipe_fd ));
...
static int cups_job_delete(const char *sharename, const char *lprm_command, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("cups_job_delete(%s, %p (%d))\n", sharename, pjob, pjob->sysjob));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/jobs")) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to cancel job %d - %s\n", pjob->sysjob,
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
static int cups_job_pause(int snum, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("cups_job_pause(%d, %p (%d))\n", snum, pjob, pjob->sysjob));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/jobs")) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to hold job %d - %s\n", pjob->sysjob,
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
static int cups_job_resume(int snum, struct printjob *pjob)
{
	...
	DEBUG_OUT (5,("cups_job_resume(%d, %p (%d))\n", snum, pjob, pjob->sysjob));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/jobs")) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to release job %d - %s\n", pjob->sysjob,
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
			   ...
			   char *lpq_cmd)
{
	...
	DEBUG_OUT (5,("cups_job_submit(%d, %p)\n", snum, pjob));
	...
	*/
	...
	if ((response = cupsDoFileRequest(http, request, uri, pjob->filename)) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to print file to %s - %s\n",
				 lp_printername(snum),
			         ippErrorString(cupsLastError())));
		} else {
			ret = 0;
			...
			if(attr_job_id) {
				...
				DEBUG_OUT (5,("cups_job_submit: job-id %d\n", pjob->sysjob));
			} else {
				DEBUG_OUT (0,("Missing job-id attribute in IPP response"));
			}
		}
	} else {
		DEBUG_OUT (0,("Unable to print file to `%s' - %s\n",
               ...
               print_status_struct *status)
{
	...
	*q = NULL;
	...
	DEBUG_OUT (5,("cups_queue_get(%s, %p, %p)\n", lpq_command, q, status));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG_OUT (0,("Unable to get jobs for %s - %s\n", uri,
			 ippErrorString(cupsLastError())));
		goto out;
	}
	...
	queue  = NULL;
		...
		*/
		if (qcount >= qalloc) {
			...
			if (queue == NULL) {
				DEBUG_OUT (0,("cups_queue_get: Not enough memory!"));
				qcount = 0;
				goto out;
			}
		}
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG_OUT (0,("Unable to get printer status for %s - %s\n", printername,
			 ippErrorString(cupsLastError())));
		goto out;
	}
...
static int cups_queue_pause(int snum)
{
	...
	DEBUG_OUT (5,("cups_queue_pause(%d)\n", snum));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/admin/")) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to pause printer %s - %s\n",
				 lp_printername(snum),
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
...
static int cups_queue_resume(int snum)
{
	...
	DEBUG_OUT (5,("cups_queue_resume(%d)\n", snum));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/admin/")) != NULL) {
		if (ippGetStatusCode(response) >= IPP_OK_CONFLICT) {
			DEBUG_OUT (0,("Unable to resume printer %s - %s\n",
				 lp_printername(snum),
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
