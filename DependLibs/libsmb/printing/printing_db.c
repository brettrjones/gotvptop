...
struct tdb_print_db *get_print_db_byname(const char *printername)
{
		...
		for (p = print_db_head; p; p = p->next) {
			...
			if (p->tdb) {
				if (tdb_close(print_db_head->tdb)) {
					DEBUG_OUT (0,("get_print_db: Failed to close tdb for printer %s\n",
	...
	if (!p)	{
		...
		if (!p) {
			DEBUG_OUT (0,("get_print_db: malloc fail !\n"));
			return NULL;
		}
		ZERO_STRUCTP(p);
	...
	if (!p->tdb) {
		DEBUG_OUT (0,("get_print_db: Failed to open printer backend database %s.\n",
					printdb_path ));
		DLIST_REMOVE(print_db_head, p);
...
struct TDB_DATA get_printer_notify_pid_list(struct tdb_context *tdb, const char *printer_name, bool cleanlist)
{
	...
	if (data.dsize % 8) {
		DEBUG_OUT (0,("get_printer_notify_pid_list: Size of record for printer %s not a multiple of 8 !\n", printer_name ));
	...
	for( i = 0; i < data.dsize; i += 8) {
		...
		while ((i < data.dsize) && ((IVAL(data.dptr, i + 4) == 0) || !process_exists_by_pid(pid))) {
			...
			if (IVAL(data.dptr, i + 4) == 0) {
				DEBUG_OUT (0,("get_printer_notify_pid_list: Refcount == 0 for pid = %u printer %s !\n",
