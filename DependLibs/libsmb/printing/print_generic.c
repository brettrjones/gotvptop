			     ...
			     const char *command, int *outfd, ...)
{
	...
	DEBUG_OUT (3,("Running the command `%s' gave %d\n",syscmd,ret));
                             ...
                             print_status_struct *status)
{
	...
	if (fd == -1) {
		DEBUG_OUT (5,("generic_queue_get: Can't read print queue status for printer %s\n",
			printer_name ));
		return 0;
	}
			      ...
			      char *lpq_cmd)
{
	...
	*p++ = 0;
	...
	if (ret > 0) {
		...
		for (i = 0; i < ret; i++) {
			if (strcmp(q[i].fs_file, p) == 0) {
				...
				DEBUG_OUT (5, ("new job %u (%s) matches sysjob %d\n",
	...
	if (pjob->sysjob == -1) {
		DEBUG_OUT (2, ("failed to get sysjob for job %u (%s), tracking as "
			  "Unix job\n", pjob->jobid, jobname));
	}
