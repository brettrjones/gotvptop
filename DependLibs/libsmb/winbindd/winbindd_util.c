						  ...
						  const struct dom_sid *sid)
{
	...
	if (idmap_config_option == NULL) {
		DEBUG_OUT (0, ("talloc failed, not looking for idmap config\n"));
		goto done;
	}

	param = lp_parm_const_string(-1, idmap_config_option, "range", NULL);

	DEBUG_OUT (10, ("%s : range = %s\n", idmap_config_option,
		   param ? param : "not defined"));

	if (param != NULL) {
		unsigned low_id, high_id;
		if (sscanf(param, "%u - %u", &low_id, &high_id) != 2) {
			DEBUG_OUT (1, ("invalid range syntax in %s: %s\n",
				  idmap_config_option, param));
			goto done;
		}
		if (low_id > high_id) {
			DEBUG_OUT (1, ("invalid range in %s: %s\n",
				  idmap_config_option, param));
			goto done;
		}
	...
	DEBUG_OUT (2,("Added domain %s %s %s\n",
...
static void add_trusted_domains( struct winbindd_domain *domain )
{
	...
	if (req == NULL) {
		DEBUG_OUT (1, ("wb_domain_request_send failed\n"));
		TALLOC_FREE(state);
		return;
	}
...
static void trustdom_list_done(struct tevent_req *req)
{
	...
	if ((res == -1) || (response->result != WINBINDD_OK)) {
		DEBUG_OUT (1, ("Could not receive trustdoms\n"));
		TALLOC_FREE(state);
		return;
	}
	...
	while ((p != NULL) && (*p != '\0')) {
		...
		alt_name = strchr(p, '\\');
		if (alt_name == NULL) {
			DEBUG_OUT (0, ("Got invalid trustdom response\n"));
			break;
		}
	...
	for ( i=0; i<num_trusts; i++ ) {
		...
		if (d == NULL) {
			continue;
       		...
       		DEBUG_OUT (10,("rescan_forest_root_trusts: Following trust path "
...
static void rescan_forest_trusts( void )
{
	...
	for ( i=0; i<num_trusts; i++ ) {
		     ...
		     (attribs == NETR_TRUST_ATTRIBUTE_FOREST_TRANSITIVE) )
		{
			...
			DEBUG_OUT (10,("Following trust path for domain %s (%s)\n",
				  d->name, d->alt_name ));
			add_trusted_domains( d );
		}
	}
						   ...
						   struct winbindd_cli_state *state)
{
	...
	if (!domain->initialized) {
		...
		*/
		DEBUG_OUT (5, ("winbindd_dual_init_connection: %s returning without initialization "
...
bool init_domain_list(void)
{
	...
	if ( role == ROLE_DOMAIN_MEMBER ) {
		...
		if (!secrets_fetch_domain_sid(lp_workgroup(), &our_sid)) {
			DEBUG_OUT (0, ("Could not fetch our SID - did we join?\n"));
			return False;
		}
...
struct winbindd_domain *find_lookup_domain_from_sid(const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10, ("find_lookup_domain_from_sid(%s)\n", sid_string_dbg(sid)));

	if (IS_DC || is_internal_domain(sid) || is_in_internal_domain(sid)) {
		DEBUG_OUT (10, ("calling find_domain_from_sid\n"));
	...
	DEBUG_OUT (10, ("calling find_our_domain\n"));
				  ...
				  uint32_t *p_num_groups, struct dom_sid **user_sids)
{
	...
	DEBUG_OUT (3,(": lookup_usergroups_cached\n"));
	...
	*p_num_groups = num_groups;
	...
	DEBUG_OUT (3,(": lookup_usergroups_cached succeeded\n"));
			     ...
			     char **normalized)
{
	...
	if (NT_STATUS_EQUAL(nt_status, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND)) {
		DEBUG_OUT (5,("normalize_name_map: Setting domain %s offline\n",
			 domain->name));
			      ...
			      char **normalized)
{
	...
	if (NT_STATUS_EQUAL(nt_status, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND)) {
		DEBUG_OUT (5,("normalize_name_unmap: Setting domain %s offline\n",
			 domain->name));
		set_domain_offline(domain);
		return nt_status;
	}
...
bool winbindd_can_contact_domain(struct winbindd_domain *domain)
{
	...
	if ((tdc = wcache_tdc_fetch_domain(frame, domain->name)) == NULL) {
		DEBUG_OUT (10,("winbindd_can_contact_domain: %s not found in cache\n",
			  domain->name));
		return false;
	}
	...
	{
		DEBUG_OUT (10, ("winbindd_can_contact_domain: %s is an AD domain "
...
static void winbindd_set_locator_kdc_env(const struct winbindd_domain *domain)
{
	...
	if (domain->initialized && !domain->active_directory) {
		DEBUG_OUT (lvl,("winbindd_set_locator_kdc_env: %s not AD\n",
			domain->alt_name));
		return;
	}
	...
	kdc = addr;
	if (!*kdc) {
		DEBUG_OUT (lvl,("winbindd_set_locator_kdc_env: %s no DC IP\n",
			domain->alt_name));
		kdc = domain->dcname;
	}

	if (!kdc || !*kdc) {
		DEBUG_OUT (lvl,("winbindd_set_locator_kdc_env: %s no DC at all\n",
			domain->alt_name));
		return;
	}
	...
	DEBUG_OUT (lvl,("winbindd_set_locator_kdc_env: setting var: %s to: %s\n",
		var, kdc));
		   ...
		   struct dom_sid **sids, uint32_t *num_sids)
{
	...
	while (p[0] != '\0') {
		...
		if (!dom_sid_parse_endp(p, &sid, &q)) {
			DEBUG_OUT (1, ("Could not parse sid %s\n", p));
			return false;
		}
		if ((q == NULL) || (q[0] != '\n')) {
			DEBUG_OUT (1, ("Got invalid sidstr: %s\n", p));
			return false;
		}
