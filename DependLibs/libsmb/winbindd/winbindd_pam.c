				    ...
				    struct netr_SamInfo3 *info3)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (0,("append_info3_as_ndr: failed to append\n"));
				     ...
				     const char *name_user)
{
	...
	DEBUG_OUT (5, ("Setting unix username to [%s]\n",
				 ...
				 const char *name_user)
{
	...
	DEBUG_OUT (10, ("Generating token for user %s\n", afsname));
 ...
 */
{
	...
	while (next_token_talloc(talloc_tos(), &p, &req_sid, ",")) {
		if (!string_to_sid(&sid, req_sid)) {
			DEBUG_OUT (0, ("check_info3_in_group: could not parse %s "
				  "as a SID!", req_sid));
			TALLOC_FREE(frame);
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("add_sid_to_array failed\n"));
			TALLOC_FREE(frame);
			return status;
		}
	}
	...
	if (!NT_STATUS_IS_OK(status = add_aliases(get_global_sam_sid(),
	    ...
	    || !NT_STATUS_IS_OK(status = add_aliases(&global_sid_Builtin,
						     token))) {
		DEBUG_OUT (3, ("could not add aliases: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(frame);
		return status;
	}
					 ...
					 const char *domain_name)
{
	...
	if (IS_DC) {
		...
		if (domain == NULL) {
			DEBUG_OUT (3, ("Authentication for domain [%s] refused "
	...
	if (flags & WBFLAG_PAM_CONTACT_TRUSTDOM) {
		domain = find_domain_from_name_noinit(domain_name);
		if (domain == NULL) {
			DEBUG_OUT (3, ("Authentication for domain [%s] skipped "
				  "as it is not a trusted domain\n",
				  domain_name));
		} else {
			return domain;
				       ...
				       struct winbindd_response *response)
{
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (5,("fillup_password_policy: No inbound trust to "
			 "contact domain %s\n", domain->name));
					...
					const char **user_ccache_file)
{
	...
	*user_ccache_file = gen_cc;
	...
	if (gen_cc == NULL) {
  	...
  	if (gen_cc == NULL) {
		DEBUG_OUT (0,("out of memory\n"));
		return NULL;
	}

	DEBUG_OUT (10, ("using ccache: %s%s\n", gen_cc,
					    ...
					    fstring krb5ccname)
{
	...
	*info3 = NULL;
	...
	if (user_ccache_file != NULL) {
		...
		DEBUG_OUT (10,("winbindd_raw_kerberos_login: uid is %d\n", uid));
	}
	...
	*info3 = &logon_info->info3;
	...
	DEBUG_OUT (10,("winbindd_raw_kerberos_login: winbindd validated ticket of %s\n",
		principal_s));
	...
	if (user_ccache_file != NULL) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("winbindd_raw_kerberos_login: failed to add ccache to list: %s\n",
				nt_errstr(result)));
		}
	} else {
		...
		if (krb5_ret) {
			DEBUG_OUT (3,("winbindd_raw_kerberos_login: "
...
bool check_request_flags(uint32_t flags)
{
	...
	DEBUG_OUT (1, ("check_request_flags: invalid request flags[0x%08X]\n",
		  flags));
				 ...
				 const char *name_user)
{
	...
	if (request_flags & WBFLAG_PAM_UNIX_NAME) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("Failed to append Unix Username: %s\n",
				nt_errstr(result)));
			return result;
		}
	}
	...
	if (request_flags & WBFLAG_PAM_INFO3_NDR) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("Failed to append INFO3 (NDR): %s\n",
				nt_errstr(result)));
			return result;
		}
	}
	...
	if (request_flags & WBFLAG_PAM_INFO3_TEXT) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("Failed to append INFO3 (TXT): %s\n",
				nt_errstr(result)));
			return result;
		}
	}
	...
	if (request_flags & WBFLAG_PAM_AFS_TOKEN) {
		result = append_afs_token(mem_ctx, resp,
					  info3, name_domain, name_user);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("Failed to append AFS token: %s\n",
				nt_errstr(result)));
			return result;
		}
	}
					      ...
					      struct netr_SamInfo3 **info3)
{
	...
	*info3 = NULL;
	...
	DEBUG_OUT (10,("winbindd_dual_pam_auth_cached\n"));
	...
	if (!lookup_cached_name(name_domain,
				...
				&sid,
				&type)) {
		DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: no such user in the cache\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (type != SID_NAME_USER) {
		DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: not a user (%s)\n", sid_type_lookup(type)));
		return NT_STATUS_LOGON_FAILURE;
	}
	...
	result = winbindd_get_creds(domain,
				    ...
				    &cached_salt);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: failed to get creds: %s\n", nt_errstr(result)));
		return result;
	}
	...
	*info3 = my_info3;
	...
	if (password_good) {
		...
		if (!(my_info3->base.acct_flags & ACB_NORMAL)) {
			DEBUG_OUT (0,("winbindd_dual_pam_auth_cached: whats wrong with that one?: 0x%08x\n",
		...
		if ((state->request->flags & WBFLAG_PAM_KRB5) &&
			...
			if (uid == -1) {
				DEBUG_OUT (0,("winbindd_dual_pam_auth_cached: invalid uid\n"));
				return NT_STATUS_INVALID_PARAMETER;
			}
			...
			if (user_ccache_file != NULL) {
				...
				if (!NT_STATUS_IS_OK(result)) {
					DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: failed "
 ...
 success:
		...
		result = winbindd_update_creds_by_info3(domain,
							...
							my_info3);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (1,("winbindd_dual_pam_auth_cached: failed to update creds: %s\n",
				nt_errstr(result)));
			return result;
		}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: failed to get max_allowed_bad_attempts. "
	...
	if (my_info3->base.bad_password_count >= max_allowed_bad_attempts) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_cached: failed to get password properties.\n"));
		}
...
failed:
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("winbindd_dual_pam_auth_cached: failed to update creds %s\n",
			nt_errstr(result)));
	}
						...
						struct netr_SamInfo3 **info3)
{
	...
	DEBUG_OUT (10,("winbindd_dual_pam_auth_kerberos\n"));
	...
	if ( IS_DC ) {
		if (!(contact_domain = find_domain_from_name(name_domain))) {
			DEBUG_OUT (3, ("Authentication for domain for [%s] -> [%s]\\[%s] failed as %s is not a trusted domain\n",
		...
		if (is_myname(name_domain)) {
			DEBUG_OUT (3, ("Authentication for domain %s (local domain to this server) not supported at this stage\n", name_domain));
			result =  NT_STATUS_NO_SUCH_USER;
			goto done;
		}
	...
	if (!contact_domain->active_directory) {
		DEBUG_OUT (3,("krb5 auth requested but domain is not Active Directory\n"));
					  ...
					  struct netr_SamInfo3 **pinfo3)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("make_user_info failed: %s\n", nt_errstr(status)));
		return status;
	}
	...
	DEBUG_OUT (10, ("Authenticaticating user %s\\%s returned %s\n", domain,
					    ...
					    struct netr_SamInfo3 **info3)
{
	...
	do {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (3,("Could not open handle to NETLOGON pipe "
				 "(error: %s, attempts: %d)\n",
				  nt_errstr(result), netr_attempts));

			/* After the first retry always close the connection */
			if (netr_attempts > 0) {
				DEBUG_OUT (3, ("This is again a problem for this "
			...
			if (netr_attempts > 1) {
				 ...
				 */
				DEBUG_OUT (3, ("This is the third problem for this "
			...
			if (netr_attempts < 3) {
				DEBUG_OUT (3, ("The connection to netlogon "
					  "failed, retrying\n"));
				netr_attempts++;
				retry = true;
				continue;
		...
		if (NT_STATUS_EQUAL(result, NT_STATUS_RPC_PROCNUM_OUT_OF_RANGE)) {
			...
			if (domain->can_do_samlogon_ex) {
				DEBUG_OUT (3, ("Got a DC that can not do NetSamLogonEx, "
		...
		if (domain->can_do_validation6 &&
			...
			DEBUG_OUT (3,("Got a DC that can not do validation level 6, "
		...
		if ( NT_STATUS_EQUAL(result, NT_STATUS_ACCESS_DENIED) ) {
			DEBUG_OUT (3,("winbind_samlogon_retry_loop: sam_logon returned "
	...
	if (NT_STATUS_EQUAL(result, NT_STATUS_IO_TIMEOUT)) {
		DEBUG_OUT (3,("winbind_samlogon_retry_loop: sam_network_logon(ex) "
						...
						struct netr_SamInfo3 **info3)
{
	...
	*info3 = NULL;
	...
	DEBUG_OUT (10,("winbindd_dual_pam_auth_samlogon\n"));
	...
	if (lp_client_ntlmv2_auth()) {
		...
		*/
		...
		if (!SMBNTLMv2encrypt(mem_ctx, name_user, name_domain,
			...
			DEBUG_OUT (0, ("winbindd_pam_auth: SMBNTLMv2encrypt() failed!\n"));
	...
	if ((request_flags & WBFLAG_PAM_INFO3_TEXT) &&
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG_OUT (3, ("could not open handle to SAMR pipe: %s\n",
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG_OUT (3, ("could not open user handle on SAMR pipe: %s\n",
				nt_errstr(status_tmp)));
			goto done;
		}
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG_OUT (3, ("could not query user info on SAMR pipe: %s\n",
				nt_errstr(status_tmp)));
		...
		DEBUG_OUT (10,("successfully retrieved acct_flags 0x%x\n", acct_flags));
					    ...
					    struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: dual pam auth %s\n", (unsigned long)state->pid,
		  state->request->data.auth.user));
	...
	if (!domain->online) {
		...
		if (domain->startup) {
			...
			DEBUG_OUT (10,("winbindd_dual_pam_auth: domain: %s offline and auth "
	...
	DEBUG_OUT (10,("winbindd_dual_pam_auth: domain: %s last was %s\n", domain->name, domain->online ? "online":"offline"));
	...
	if (domain->online && (state->request->flags & WBFLAG_PAM_KRB5)) {
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_kerberos succeeded\n"));
			goto process_result;
		} else {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_kerberos failed: %s\n", nt_errstr(result)));
		}

		if (NT_STATUS_EQUAL(result, NT_STATUS_NO_LOGON_SERVERS) ||
		    NT_STATUS_EQUAL(result, NT_STATUS_IO_TIMEOUT) ||
		    NT_STATUS_EQUAL(result, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND)) {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_kerberos setting domain to offline\n"));
		...
		if (state->request->flags & WBFLAG_PAM_FALLBACK_AFTER_KRB5) {
			DEBUG_OUT (3,("falling back to samlogon\n"));
			goto sam_logon;
		} else {
			goto cached_logon;
		}
...
sam_logon:
	...
	if (domain->online) {
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_samlogon succeeded\n"));
		...
		DEBUG_OUT (10,("winbindd_dual_pam_auth_samlogon failed: %s\n",
			  nt_errstr(result)));
		...
		if (NT_STATUS_EQUAL(result, NT_STATUS_NO_LOGON_SERVERS) ||
		    ...
		    NT_STATUS_EQUAL(result, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND))
		{
			DEBUG_OUT (10,("winbindd_dual_pam_auth_samlogon setting domain to offline\n"));
			set_domain_offline( domain );
			goto cached_logon;
		}
...
cached_logon:
	...
	if (!domain->online && (state->request->flags & WBFLAG_PAM_CACHED_LOGIN) &&
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_cached succeeded\n"));
			goto process_result;
		} else {
			DEBUG_OUT (10,("winbindd_dual_pam_auth_cached failed: %s\n", nt_errstr(result)));
			goto done;
		}
	}
...
process_result:
	...
	if (NT_STATUS_IS_OK(result)) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (3, ("User %s is not in the required group (%s), so plaintext authentication is rejected\n",
				  state->request->data.auth.user,
				  state->request->data.auth.require_membership_of_sid));
			goto done;
		}
		...
		if (state->request->flags & WBFLAG_PAM_GET_PWD_POLICY) {
			...
			{
				DEBUG_OUT (10,("Failed to get password policies for domain %s: %s\n",
	...
	DEBUG_OUT (NT_STATUS_IS_OK(result) ? 5 : 2, ("Plain-text authentication for user %s returned %s (PAM: %d)\n",
	      state->request->data.auth.user,
						 ...
						 struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: pam auth crap domain: %s user: %s\n", (unsigned long)state->pid,
		  name_domain, name_user));
	...
	if (state->request->data.auth_crap.lm_resp_len > sizeof(state->request->data.auth_crap.lm_resp)
		...
		if (!(state->request->flags & WBFLAG_BIG_NTLMV2_BLOB) ||
			...
			DEBUG_OUT (0, ("winbindd_pam_auth_crap: invalid password length %u/%u\n",
...
process_result:
	...
	if (NT_STATUS_IS_OK(result)) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (3, ("User %s is not in the required group (%s), so "
	...
	DEBUG_OUT (NT_STATUS_IS_OK(result) ? 5 : 2,
						 ...
						 struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: dual pam chauthtok %s\n", (unsigned long)state->pid,
		  state->request->data.auth.user));
	...
	result = cm_connect_sam(contact_domain, state->mem_ctx, &cli,
				&dom_pol);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (1, ("could not get SAM handle on DC for %s\n", domain));
		goto done;
	}
	...
	if (NT_STATUS_EQUAL(result, NT_STATUS_RPC_PROCNUM_OUT_OF_RANGE) ||
		...
		DEBUG_OUT (10,("Password change with chgpasswd_user3 failed with: %s, retrying chgpasswd_user2\n",
			nt_errstr(result)));
	...
	if (NT_STATUS_IS_OK(result)
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (10, ("Failed to store creds: %s\n",
				   nt_errstr(result)));
			goto process_result;
		}
	}
	...
	if (!NT_STATUS_IS_OK(result) && !got_info && contact_domain) {
		...
		if (!NT_STATUS_IS_OK(policy_ret)) {
			DEBUG_OUT (10,("Failed to get password policies: %s\n", nt_errstr(policy_ret)));
			goto process_result;
		}
	}
					      ...
					      struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: pam dual logoff %s\n", (unsigned long)state->pid,
	...
	if (state->request->data.logoff.uid < 0) {
		DEBUG_OUT (0,("winbindd_pam_logoff: invalid uid\n"));
		goto process_result;
	}
	...
	if (!ccache_entry_exists(state->request->data.logoff.user)) {
		result = NT_STATUS_OK;
		DEBUG_OUT (10,("winbindd_pam_logoff: no entry found.\n"));
		goto process_result;
	}
	...
	if (!ccache_entry_identical(state->request->data.logoff.user,
		...
		DEBUG_OUT (0,("winbindd_pam_logoff: cached entry differs.\n"));
		goto process_result;
	}

	result = remove_ccache(state->request->data.logoff.user);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("winbindd_pam_logoff: failed to remove ccache: %s\n",
			nt_errstr(result)));
		goto process_result;
	}
...
enum winbindd_result winbindd_dual_pam_chng_pswd_auth_crap(struct winbindd_domain *domainSt, struct winbindd_cli_state *state)
{
	...
	*user = 0;
	...
	DEBUG_OUT (3, ("[%5lu]: pam change pswd auth crap domain: %s user: %s\n",
		  (unsigned long)state->pid,
		  state->request->data.chng_pswd_auth_crap.domain,
		  state->request->data.chng_pswd_auth_crap.user));

	if (lp_winbind_offline_logon()) {
		DEBUG_OUT (0,("Refusing password change as winbind offline logons are enabled. "));
	...
	if (*state->request->data.chng_pswd_auth_crap.domain) {
		...
		if(!*domain) {
			DEBUG_OUT (3,("no domain specified with username (%s) - "
				 "failing auth\n",
				 state->request->data.chng_pswd_auth_crap.user));
			result = NT_STATUS_NO_SUCH_USER;
			goto done;
