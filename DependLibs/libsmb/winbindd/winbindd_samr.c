					...
					struct rpc_pipe_client **samr_pipe)
{
	...
	if (session_info == NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("open_samr_pipe: Could not create auth_serversupplied_info: %s\n",
				  nt_errstr(status)));
			return status;
		}
	}
	...
	status = rpc_pipe_open_interface(mem_ctx,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("open_samr_pipe: Could not connect to samr_pipe: %s\n",
			  nt_errstr(status)));
		return status;
	}
				       ...
				       struct rpc_pipe_client **lsa_pipe)
{
	...
	if (session_info == NULL) {
		status = make_session_info_system(mem_ctx, &session_info);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("open_lsa_pipe: Could not create auth_serversupplied_info: %s\n",
				  nt_errstr(status)));
			return status;
		}
	}
	...
	status = rpc_pipe_open_interface(mem_ctx,
					...
					&cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("open_lsa_pipe: Could not connect to lsa_pipe: %s\n",
			  nt_errstr(status)));
		return status;
	}
				    ...
				    struct wb_acct_info **pinfo)
{
	...
	DEBUG_OUT (3,("sam_enum_dom_groups\n"));
				    ...
				    struct wbint_userinfo **pinfo)
{
	...
	DEBUG_OUT (3,("samr_query_user_list\n"));
				    ...
				    struct netr_DomainTrustList *ptrust_list)
{
	...
	DEBUG_OUT (3,("samr: trusted domains\n"));
				    ...
				    uint32_t **pname_types)
{
	...
	DEBUG_OUT (3,("sam_lookup_groupmem\n"));
				      ...
				      struct wb_acct_info **pinfo)
{
	...
	DEBUG_OUT (3,("samr: enum local groups\n"));
				...
				enum lsa_SidType *ptype)
{
	...
	if (!sid_check_is_in_builtin(sid) &&
		...
		DEBUG_OUT (0, ("sam_sid_to_name: possible deadlock - trying to "
			  "lookup SID %s\n", sid_string_dbg(sid)));
				  ...
				  enum lsa_SidType **ptypes)
{
	...
	DEBUG_OUT (3,("sam_rids_to_names for %s\n", domain->name));
	...
	if (!sid_check_is_builtin(domain_sid) &&
		...
		DEBUG_OUT (0, ("sam_rids_to_names: possible deadlock - trying to "
				   ...
				   struct samr_DomInfo12 *lockout_policy)
{
	...
	DEBUG_OUT (3,("sam_lockout_policy\n"));
				    ...
				    struct samr_DomInfo1 *passwd_policy)
{
	...
	DEBUG_OUT (3,("sam_password_policy\n"));
				      ...
				      struct dom_sid **puser_grpsids)
{
	...
	DEBUG_OUT (3,("sam_lookup_usergroups\n"));
				       ...
				       uint32_t **palias_rids)
{
	...
	DEBUG_OUT (3,("sam_lookup_useraliases\n"));
				    ...
				    uint32_t *pseq)
{
	...
	DEBUG_OUT (3,("samr: sequence number\n"));
