			     ...
			     struct wb_acct_info **pinfo)
{
	...
	*pnum_info = 0;
	...
	do {
		...
		if (!NT_STATUS_IS_OK(result)) {
			if (!NT_STATUS_EQUAL(result, STATUS_MORE_ENTRIES)) {
				DEBUG_OUT (2,("query_user_list: failed to enum domain groups: %s\n",
					 nt_errstr(result)));
				return result;
			}
		}
			 ...
			 enum lsa_SidType *type)
{
	...
	DEBUG_OUT (3,("name_to_sid: %s for domain %s\n",
	...
	status = rpccli_lsa_lookup_names(lsa_pipe,
					 ...
					 &types);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("name_to_sid: failed to lookup name: %s\n",
			nt_errstr(status)));
		return status;
	}
			 ...
			 enum lsa_SidType *ptype)
{
	...
	status = rpccli_lsa_lookup_sids(lsa_pipe,
					...
					&types);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2,("sid_to_name: failed to lookup sids: %s\n",
			nt_errstr(status)));
		return status;
	}
			   ...
			   enum lsa_SidType **ptypes)
{
	...
	if (!NT_STATUS_IS_OK(status) &&
		...
		DEBUG_OUT (2,("rids_to_names: failed to lookup sids: %s\n",
			nt_errstr(status)));
		return status;
	}
...
#define MAX_SAM_ENTRIES_W2K 0x400 /* 1024 */
	...
	do {
		...
		DEBUG_OUT (10,("rpc: lookup_useraliases: entering query %d for %d sids\n",
	...
	DEBUG_OUT (10,("rpc: rpc_lookup_useraliases: got %d aliases in %d queries "
...
seq_num:
	if (got_seq_num) {
		DEBUG_OUT (10,("domain_sequence_number: for domain %s is %u\n",
			  domain_name, (unsigned) *pseq));
	} else {
		DEBUG_OUT (10,("domain_sequence_number: failed to get sequence "
