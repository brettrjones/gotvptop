					...
					void *private_data)
{
	...
	DEBUG_OUT (10,("krb5_ticket_refresh_handler called\n"));
...
rekinit:
		if (cred_ptr && cred_ptr->pass) {
			...
			if (ret) {
				DEBUG_OUT (3,("krb5_ticket_refresh_handler: "
			...
			DEBUG_OUT (10,("krb5_ticket_refresh_handler: successful re-kinit "
				"for: %s in ccache: %s\n",
				     ...
				     void *private_data)
{
	...
	DEBUG_OUT (10,("krb5_ticket_gain_handler called\n"));
	DEBUGADD(10,("event called for: %s, %s\n",
		entry->ccname, entry->username));
	...
	if (!cred_ptr || !cred_ptr->pass) {
		DEBUG_OUT (10,("krb5_ticket_gain_handler: no memory creds\n"));
		return;
	}

	if ((domain = find_domain_from_name(entry->realm)) == NULL) {
		DEBUG_OUT (0,("krb5_ticket_gain_handler: unknown domain\n"));
		return;
	}
	...
	if (ret) {
		DEBUG_OUT (3,("krb5_ticket_gain_handler: "
			"could not kinit: %s\n",
			error_message(ret)));
	...
	DEBUG_OUT (10,("krb5_ticket_gain_handler: "
			    ...
			    const char *ccname)
{
	...
	if (entry->uid != uid) {
		DEBUG_OUT (0,("cache_entry_identical: uid's differ: %u != %u\n",
			(unsigned int)entry->uid, (unsigned int)uid));
		return False;
	}
	if (!strcsequal(entry->ccname, ccname)) {
		DEBUG_OUT (0,("cache_entry_identical: "
			    ...
			    bool postponed_request)
{
	...
	if (ccache_entry_count() + 1 > MAX_CCACHES) {
		DEBUG_OUT (10,("add_ccache_to_list: "
	...
	if (postponed_request) {
		...
		if (ret) {
			DEBUG_OUT (0, ("add_ccache_to_list: failed to destroy "
				   "user krb5 ccache %s with %s\n", ccname,
				   error_message(ret)));
			return krb5_to_nt_status(ret);
		}
		DEBUG_OUT (10, ("add_ccache_to_list: successfully destroyed "
	...
	if (entry) {
		...
		if (!entry->event) {
			...
			if (!entry->event) {
				...
				if (!NT_STATUS_IS_OK(ntret)) {
					DEBUG_OUT (0, ("add_ccache_to_list: Failed to remove krb5 "
						  "ccache %s for user %s\n", entry->ccname,
						  entry->username));
					DEBUG_OUT (0, ("add_ccache_to_list: error is %s\n",
						  nt_errstr(ntret)));
					return ntret;
				}
			...
			DEBUG_OUT (10,("add_ccache_to_list: added krb5_ticket handler\n"));
		...
		DEBUG_OUT (10, ("winbindd_add_memory_creds returned: %s\n",
			nt_errstr(ntret)));
...
NTSTATUS remove_ccache(const char *username)
{
	...
	if (entry->ref_count <= 0) {
		DEBUG_OUT (0,("remove_ccache: logic error. "
	...
	if (entry->ref_count > 0) {
		DEBUG_OUT (10,("remove_ccache: entry %s ref count now %d\n",
			username, entry->ref_count));
		return NT_STATUS_OK;
	}
	...
	TALLOC_FREE(entry);
 	DEBUG_OUT (10,("remove_ccache: removed ccache for user %s\n", username));
				   ...
				   const char *pass)
{
	...
	DEBUG_OUT (10,("mlocking memory: %p\n", memcredp->nt_hash));
#endif
	if ((mlock(memcredp->nt_hash, memcredp->len)) == -1) {
		DEBUG_OUT (0,("failed to mlock memory: %s (%d)\n",
...
static NTSTATUS delete_memory_creds(struct WINBINDD_MEMORY_CREDS *memcredp)
{
	...
	if (munlock(memcredp->nt_hash, memcredp->len) == -1) {
		DEBUG_OUT (0,("failed to munlock memory: %s (%d)\n",
			strerror(errno), errno));
		return map_nt_error_from_unix(errno);
	}
						   ...
						   const char *pass)
{
	...
	if (uid == (uid_t)-1) {
		DEBUG_OUT (0,("winbindd_add_memory_creds_internal: "
	...
	if (memcredp) {
		...
		memcredp->ref_count++;
		DEBUG_OUT (10,("winbindd_add_memory_creds_internal: "
...
NTSTATUS winbindd_delete_memory_creds(const char *username)
{
	...
	if (!memcredp) {
		DEBUG_OUT (10,("winbindd_delete_memory_creds: unknown user %s\n",
			username));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}

	if (memcredp->ref_count <= 0) {
		DEBUG_OUT (0,("winbindd_delete_memory_creds: logic error. "
			"ref count for user %s = %d\n",
			username, memcredp->ref_count));
	...
	if (memcredp->ref_count <= 0) {
		...
		talloc_destroy(memcredp);
		DEBUG_OUT (10,("winbindd_delete_memory_creds: "
				       ...
				       const char *pass)
{
	...
	memcredp = find_memory_creds_by_name(username);
	if (!memcredp) {
		DEBUG_OUT (10,("winbindd_replace_memory_creds: unknown user %s\n",
			username));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}

	DEBUG_OUT (10,("winbindd_replace_memory_creds: replaced creds for user %s\n",
		username));
