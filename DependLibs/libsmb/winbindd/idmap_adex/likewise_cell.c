 ...
 struct likewise_cell* cell_new(void)
{
	...
	if (!c) {
		DEBUG_OUT (0,("cell_new: memory allocation failure!\n"));
		return NULL;
	}
 ...
 NTSTATUS cell_connect_dn(struct likewise_cell **c, const char *dn)
{
	...
	*c = new_cell;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (1,("LWI: Failled to connect to cell \"%s\" (%s)\n",
			  ...
			  LDAPMessage ** msg)
{
	...
	DEBUG_OUT (10, ("cell_do_search: Base = %s,  Filter = %s, Scope = %d, GC = %s\n",
		...
		*msg = NULL;
		...
		if (ADS_ERR_OK(status)) {
			if (DEBUGLEVEL >= 10) {
				...
				DEBUG_OUT (10,("cell_do_search: Located %d entries\n", n));
		...
		DEBUG_OUT (5, ("cell_do_search: search[%d] failed (%s)\n",
	...
	DEBUG_OUT (5, ("cell_do_search: exceeded maximum search count!\n"));
