...
static NTSTATUS dc_add_domain(const char *domain)
{
	...
	DEBUG_OUT (10,("dc_add_domain: Attempting to add domain %s\n", domain));
	...
	if (dc) {
		DEBUG_OUT (10,("dc_add_domain: %s already in list\n", domain));
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (5,("dc_add_domain: Successfully added %s\n", domain));

done:
	if (!NT_STATUS_IS_OK(nt_status)) {
		talloc_destroy(dc);
		DEBUG_OUT (0,("LWI: Failed to add new DC connection for %s (%s)\n",
 ...
 NTSTATUS domain_init_list(void)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("LWI: Failed to initialize DC list (%s)\n",
			 nt_errstr(nt_status)));
	}
