 ...
 NTSTATUS cell_lookup_settings(struct likewise_cell * cell)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (1,("LWI: Failed to obtain cell settings (%s)\n",
			 nt_errstr(nt_status)));
	}
 ...
 NTSTATUS cell_locate_membership(ADS_STRUCT * ads)
{
	...
	DEBUG_OUT (2, ("locate_cell_membership: Located membership "
		  "in cell \"%s\"\n", domain_dn));
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (3,("locate_cell_membership: Failed to find "
			 "domain SID for %s\n", domain_dn));
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("LWI: Failed to locate cell membership (%s)\n",
			 nt_errstr(nt_status)));
	}
