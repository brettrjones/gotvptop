 ...
 NTSTATUS gc_find_forest_root(struct gc_info *gc, const char *domain)
{
	...
	if (!ADS_ERR_OK(ads_status)) {
		DEBUG_OUT (4, ("find_forest_root: ads_connect(%s) failed! (%s)\n",
	...
	if (!ads_cldap_netlogon_5(frame,
				  ...
				  &cldap_reply))
	{
		DEBUG_OUT (4,("find_forest_root: Failed to get a CLDAP reply from %s!\n",
...
static NTSTATUS gc_add_forest(const char *domain)
{
	...
	if (find_gc) {
		DEBUG_OUT (10,("gc_add_forest: %s already in list\n", find_gc->forest_name));
		return NT_STATUS_OK;
	}
	...
	if (find_gc) {
		DEBUG_OUT (10,("gc_add_forest: Forest %s already in list\n",
			  find_gc->forest_name));
		return NT_STATUS_OK;
	}
	...
	DEBUG_OUT (10,("gc_add_forest: Added %s to Global Catalog list of servers\n",
		  gc->forest_name));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		talloc_destroy(gc);
		DEBUG_OUT (3,("LWI: Failed to add new GC connection for %s (%s)\n",
 ...
 NTSTATUS gc_init_list(void)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("LWI: Failed to initialized GC list (%s)\n",
			 nt_errstr(nt_status)));
	}
 ...
 struct gc_info *gc_search_start(void)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("LWI: Failed to initialize GC list (%s)\n",
			 nt_errstr(nt_status)));
	}
			   ...
			   const char *filter)
{
	...
	*msg = m;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("LWI: Forest wide search %s failed (%s)\n",
					...
					char **name)
{
	...
	DEBUG_OUT (10,("get_object_account_name: dn = \"%s\"\n", dn));
