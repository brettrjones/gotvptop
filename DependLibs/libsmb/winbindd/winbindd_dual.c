...
static NTSTATUS child_read_request(struct winbindd_cli_state *state)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("child_read_request: read_data failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	DEBUG_OUT (10, ("Need to read %d extra bytes\n", (int)state->request->extra_len));

	state->request->extra_data.data =
		SMB_MALLOC_ARRAY(char, state->request->extra_len + 1);

	if (state->request->extra_data.data == NULL) {
		DEBUG_OUT (0, ("malloc failed\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Could not read extra data: %s\n",
			  nt_errstr(status)));
	}
	return status;
}
...
static void wb_domain_request_initialized(struct tevent_req *subreq)
{
	...
	if (!string_to_sid(&state->domain->sid,
		...
		DEBUG_OUT (1,("init_child_recv: Could not convert sid %s "
			"from string\n", response->data.domain_info.sid));
				  ...
				  struct winbindd_cli_state *state)
{
	...
	for (; table->name; table++) {
		if (state->request->cmd == table->struct_cmd) {
			DEBUG_OUT (10,("child_process_request: request fn %s\n",
				  table->name));
	...
	DEBUG_OUT (1, ("child_process_request: unknown request fn number %d\n",
...
void winbind_child_died(pid_t pid)
{
	...
	if (child == NULL) {
		DEBUG_OUT (5, ("Already reaped child %u died\n", (unsigned int)pid));
		return;
	}
			 ...
			 DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("winbind_msg_debug: got debug message.\n"));

	debug_message(msg_ctx, private_data, MSG_DEBUG, server_id, data);

	for (child = winbindd_children; child != NULL; child = child->next) {

		DEBUG_OUT (10,("winbind_msg_debug: sending message to pid %u.\n",
			 ...
			 DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("winbind_msg_offline: got offline message.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("winbind_msg_offline: rejecting offline message.\n"));
		return;
	}

	/* Set our global state as offline. */
	if (!set_global_winbindd_state_offline()) {
		DEBUG_OUT (10,("winbind_msg_offline: offline request failed.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG_OUT (5,("winbind_msg_offline: marking %s offline.\n", domain->name));
	...
	for (child = winbindd_children; child != NULL; child = child->next) {
		...
		DEBUG_OUT (10,("winbind_msg_offline: sending message to pid %u for domain %s.\n",
			...
			DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("winbind_msg_online: got online message.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("winbind_msg_online: rejecting online message.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG_OUT (5,("winbind_msg_online: requesting %s to go online.\n", domain->name));
	...
	for (child = winbindd_children; child != NULL; child = child->next) {
		...
		DEBUG_OUT (10,("winbind_msg_online: sending message to pid %u for domain %s.\n",
...
static const char *collect_onlinestatus(TALLOC_CTX *mem_ctx)
{
	...
	DEBUG_OUT (5,("collect_onlinestatus: %s", buf));
			      ...
			      DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("winbind_msg_onlinestatus received.\n"));
				 ...
				 DATA_BLOB *data)
{
	...
	DEBUG_OUT (10,("winbind_msg_dump_event_list received\n"));

	dump_event_list(winbind_event_context());

	for (child = winbindd_children; child != NULL; child = child->next) {

		DEBUG_OUT (10,("winbind_msg_dump_event_list: sending message to pid %u\n",
			(unsigned int)child->pid));
				  ...
				  DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("winbind_msg_dump_domain_list received.\n"));
	...
	if (domain) {
		...
		DEBUG_OUT (5,("winbind_msg_dump_domain_list for domain: %s\n",
			domain));
	...
	DEBUG_OUT (5,("winbind_msg_dump_domain_list all domains\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("failed to send message: %s\n",
		nt_errstr(status)));
	}
					   ...
					   void *private_data)
{
	...
	DEBUG_OUT (10,("account_lockout_policy_handler called\n"));

	TALLOC_FREE(child->lockout_policy_event);

	if ( !winbindd_can_contact_domain( child->domain ) ) {
		DEBUG_OUT (10,("account_lockout_policy_handler: Removing myself since I "
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("account_lockout_policy_handler: lockout_policy failed error %s\n",
			 nt_errstr(result)));
	}
					      ...
					      struct timeval *t)
{
	...
	if (pw == NULL) {
		DEBUG_OUT (0,("cannot fetch own machine password ????"));
		return false;
	}
	...
	if (timeout == 0) {
		DEBUG_OUT (10,("machine password never expires\n"));
		return false;
	}

	tv.tv_sec = pass_last_set_time;
	DEBUG_OUT (10, ("password last changed %s\n",
	...
	if (time(NULL) < (pass_last_set_time + timeout)) {
		...
		DEBUG_OUT (10,("machine password still valid until: %s\n",
		...
		*t = timeval_set(next_change, 0);
		...
		if (lp_clustering()) {
			...
			DEBUG_OUT (10, ("adding %d seconds randomness\n",
				   (int)randbuf));
			t->tv_sec += randbuf;
		}
		return true;
	...
	DEBUG_OUT (10,("machine password expired, needs immediate change\n"));
					    ...
					    void *private_data)
{
	...
	DEBUG_OUT (10,("machine_password_change_handler called\n"));

	TALLOC_FREE(child->machine_password_change_event);

	if (!calculate_next_machine_pwd_change(child->domain->name,
					       &next_change)) {
		DEBUG_OUT (10, ("calculate_next_machine_pwd_change failed\n"));
		return;
	}

	DEBUG_OUT (10, ("calculate_next_machine_pwd_change returned %s\n",
		   timeval_string(talloc_tos(), &next_change, false)));

	if (!timeval_expired(&next_change)) {
		DEBUG_OUT (10, ("Someone else has already changed the pw\n"));
		goto done;
	}

	if (!winbindd_can_contact_domain(child->domain)) {
		DEBUG_OUT (10,("machine_password_change_handler: Removing myself since I "
			  "do not have an incoming trust to domain %s\n",
			  child->domain->name));
		return;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("machine_password_change_handler: "
	...
	if (NT_STATUS_EQUAL(result, NT_STATUS_ACCESS_DENIED) ) {
		DEBUG_OUT (3,("machine_password_change_handler: password set returned "
			      ...
			      DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("child_msg_offline received for domain %s.\n", domainname));

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("child_msg_offline: rejecting offline message.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			DEBUG_OUT (5,("child_msg_offline: marking %s offline.\n", domain->name));
			set_domain_offline(domain);
			     ...
			     DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("child_msg_online received for domain %s.\n", domainname));

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("child_msg_online: rejecting online message.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			DEBUG_OUT (5,("child_msg_online: requesting %s to go online.\n", domain->name));
			winbindd_flush_negative_conn_cache(domain);
			set_domain_online_request(domain);
...
static void child_msg_dump_event_list(struct messaging_context *msg,
				      ...
				      DATA_BLOB *data)
{
	DEBUG_OUT (5,("child_msg_dump_event_list received\n"));
...
static bool fork_domain_child(struct winbindd_child *child)
{
	...
	if (child->domain) {
		DEBUG_OUT (10, ("fork_domain_child called for domain '%s'\n",
			   child->domain->name));
	} else {
		DEBUG_OUT (10, ("fork_domain_child called without domain.\n"));
	}

	if (socketpair(AF_UNIX, SOCK_STREAM, 0, fdpair) != 0) {
		DEBUG_OUT (0, ("Could not open child pipe: %s\n",
			  strerror(errno)));
		return False;
	}
	...
	if (child->pid != 0) {
		...
		if (nread != sizeof(status)) {
			DEBUG_OUT (1, ("fork_domain_child: Could not read child status: "
				  "nread=%d, error=%s\n", (int)nread,
				  strerror(errno)));
			close(fdpair[1]);
			return false;
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("fork_domain_child: Child status is %s\n",
				  nt_errstr(status)));
			close(fdpair[1]);
			return false;
		}
	...
	if (nwritten != sizeof(status)) {
		DEBUG_OUT (1, ("fork_domain_child: Could not write status: "
			  "nwritten=%d, error=%s\n", (int)nwritten,
			  strerror(errno)));
		_exit(0);
	}
	...
	while (1) {
		...
		if (child->domain && child->domain->startup &&
			...
			DEBUG_OUT (10,("fork_domain_child: domain %s no longer in 'startup' mode.\n",
				child->domain->name ));
		...
		if (!event_add_to_poll_args(
			...
			DEBUG_OUT (1, ("event_add_to_poll_args failed\n"));
			_exit(1);
		}
		tp = get_timed_events_timeout(winbind_event_context(), &t);
		if (tp) {
			DEBUG_OUT (11,("select will use timeout of %u.%u seconds\n",
		...
		if (ret == 0) {
			DEBUG_OUT (11,("nothing is ready yet, continue\n"));
			TALLOC_FREE(frame);
			continue;
		}
		...
		if (ret == -1 && errno != EINTR) {
			DEBUG_OUT (0,("poll error occured\n"));
			TALLOC_FREE(frame);
			perror("poll");
			_exit(1);
		}
		...
		DEBUG_OUT (4,("child daemon request %d\n", (int)state.request->cmd));
		...
		DEBUG_OUT (4, ("Finished processing child request %d\n",
			  (int)state.request->cmd));
		...
		DEBUG_OUT (10, ("Writing %d bytes to parent\n",
			   (int)state.response->length));

		if (write_data_iov(state.sock, iov, iov_count) !=
		    state.response->length) {
			DEBUG_OUT (0, ("Could not write result\n"));
			exit(1);
		}
		TALLOC_FREE(frame);
	}
