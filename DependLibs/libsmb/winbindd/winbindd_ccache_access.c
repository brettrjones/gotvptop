					...
					struct WINBINDD_MEMORY_CREDS *entry)
{
	if (client_uid == entry->uid || client_uid == 0) {
		DEBUG_OUT (10, ("Access granted to uid %u\n", (unsigned int)client_uid));
		return True;
	}

	DEBUG_OUT (1, ("Access denied to uid %u (expected %u)\n",
					...
					uint8_t session_key[16])
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Could not set hashes: %s\n",
			nt_errstr(status)));
		goto done;
	}
	...
	*/
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG_OUT (1, ("Failed to create initial message! [%s]\n",
			nt_errstr(status)));
		goto done;
	}
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_OK)) {
		DEBUG_OUT (1, ("We didn't get a response to the challenge! [%s]\n",
			nt_errstr(status)));
		data_blob_free(&reply);
		goto done;
	}
	...
	if (ntlmssp_state->session_key.length != 16) {
		DEBUG_OUT (1, ("invalid session key length %d\n",
...
static bool check_client_uid(struct winbindd_cli_state *state, uid_t uid)
{
	...
	if (ret != 0) {
		DEBUG_OUT (1, ("check_client_uid: Could not get socket peer uid: %s; "
			"denying access\n", strerror(errno)));
		return False;
	}

	if (uid != ret_uid) {
		DEBUG_OUT (1, ("check_client_uid: Client lied about its uid: said %u, "
...
void winbindd_ccache_ntlm_auth(struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: perform NTLM auth on behalf of user %s\n", (unsigned long)state->pid,
		state->request->data.ccache_ntlm_auth.user));
	...
	if (!canonicalize_username(state->request->data.ccache_ntlm_auth.user,
		...
		DEBUG_OUT (5,("winbindd_ccache_ntlm_auth: cannot parse domain and user from name [%s]\n",
			state->request->data.ccache_ntlm_auth.user));
		request_error(state);
		return;
	}
	...
	if (domain == NULL) {
		DEBUG_OUT (5,("winbindd_ccache_ntlm_auth: can't get domain [%s]\n",
			name_domain));
		request_error(state);
		return;
	}
	...
	if (initial_blob_len > extra_len || challenge_blob_len > extra_len ||
		...
		DEBUG_OUT (10,("winbindd_dual_ccache_ntlm_auth: blob lengths overrun "
	...
	if (!parse_domain_user(state->request->data.ccache_ntlm_auth.user, name_domain, name_user)) {
		DEBUG_OUT (10,("winbindd_dual_ccache_ntlm_auth: cannot parse "
	...
	if (entry == NULL || entry->nt_hash == NULL || entry->lm_hash == NULL) {
		DEBUG_OUT (10,("winbindd_dual_ccache_ntlm_auth: could not find "
			"credentials for user %s\n", 
			state->request->data.ccache_ntlm_auth.user));
		goto process_result;
	}

	DEBUG_OUT (10,("winbindd_dual_ccache_ntlm_auth: found ccache [%s]\n", entry->username));
...
void winbindd_ccache_save(struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: save password of user %s\n",
		  (unsigned long)state->pid,
		  state->request->data.ccache_save.user));
	...
	if (!canonicalize_username(state->request->data.ccache_save.user,
				   name_domain, name_user)) {
		DEBUG_OUT (5,("winbindd_ccache_save: cannot parse domain and user "
			 "from name [%s]\n",
			 state->request->data.ccache_save.user));
		request_error(state);
		return;
	...
	domain = find_auth_domain(state->request->flags, name_domain);
	if (domain == NULL) {
		DEBUG_OUT (5, ("winbindd_ccache_save: can't get domain [%s]\n",
			  name_domain));
		request_error(state);
		return;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("winbindd_add_memory_creds failed %s\n",
			  nt_errstr(status)));
		request_error(state);
		return;
	}
