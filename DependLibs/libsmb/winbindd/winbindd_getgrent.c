...
static void winbindd_getgrent_done(struct tevent_req *subreq)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_MORE_ENTRIES)) {
		DEBUG_OUT (10, ("winbindd_getgrent_done: done with %d groups\n",
	...
	if (state->num_groups >= state->max_groups) {
		DEBUG_OUT (10, ("winbindd_getgrent_done: Got enough groups: %d\n",
			   (int)state->num_groups));
		tevent_req_done(req);
		return;
	}
	if (state->cli->grent_state == NULL) {
		DEBUG_OUT (10, ("winbindd_getgrent_done: endgrent called in "
			   "between\n"));
		tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
