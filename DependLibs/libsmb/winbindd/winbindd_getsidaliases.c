					       ...
					       struct winbindd_request *request)
{
	...
	DEBUG_OUT (3, ("getsidaliases %s\n", request->data.sid));

	if (!string_to_sid(&state->sid, request->data.sid)) {
		DEBUG_OUT (1, ("Could not get convert sid %s from string\n",
			  request->data.sid));
	...
	if (domain == NULL) {
		DEBUG_OUT (1,("could not find domain entry for sid %s\n",
			 request->data.sid));
	...
	if (request->extra_data.data != NULL) {
		if (request->extra_data.data[request->extra_len-1] != '\0') {
			DEBUG_OUT (1, ("Got non-NULL terminated sidlist\n"));
			tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
			return tevent_req_post(req, ev);
		}
		if (!parse_sidlist(state, request->extra_data.data,
				   &sids, &num_sids)) {
			DEBUG_OUT (1, ("Could not parse SID list: %s\n",
