...
static struct winbind_cache *get_cache(struct winbindd_domain *domain)
{
	...
	if (!domain->backend) {
		...
		if ((our_domain->active_directory || IS_DC)
		    ...
		    && !lp_winbind_rpc_only()) {
			DEBUG_OUT (5,("get_cache: Setting ADS methods for domain %s\n", domain->name));
			domain->backend = &ads_methods;
		} else {
#endif	/* HAVE_ADS */
			DEBUG_OUT (5,("get_cache: Setting MS-RPC methods for domain %s\n", domain->name));
...
static bool centry_check_bytes(struct cache_entry *centry, size_t nbytes)
{
	if (centry->len - centry->ofs < nbytes) {
		DEBUG_OUT (0,("centry corruption? needed %u bytes, have %d\n", 
			 (unsigned int)nbytes,
...
static char *centry_hash16(struct cache_entry *centry, TALLOC_CTX *mem_ctx)
{
	...
	if (len != 16) {
		DEBUG_OUT (0,("centry corruption? hash len (%u) != 16\n", 
			len ));
		return NULL;
	}
...
static bool wcache_server_down(struct winbindd_domain *domain)
{
	...
	if (ret)
		DEBUG_OUT (10,("wcache_server_down: server for Domain %s down\n", 
			domain->name ));
	return ret;
}
				...
				uint32_t *last_seq_check)
{
	...
	if (wcache->tdb == NULL) {
		DEBUG_OUT (10,("wcache_fetch_seqnum: tdb == NULL\n"));
		return false;
	}

	key = talloc_asprintf(talloc_tos(), "SEQNUM/%s", domain_name);
	if (key == NULL) {
		DEBUG_OUT (10, ("talloc failed\n"));
		return false;
	}
	...
	if (data.dptr == NULL) {
		DEBUG_OUT (10, ("wcache_fetch_seqnum: %s not found\n",
			   domain_name));
		return false;
	}
	if (data.dsize != 8) {
		DEBUG_OUT (10, ("wcache_fetch_seqnum: invalid data size %d\n",
			   (int)data.dsize));
		SAFE_FREE(data.dptr);
		return false;
	}
...
static NTSTATUS fetch_cache_seqnum( struct winbindd_domain *domain, time_t now )
{
	...
	if ( time_diff > lp_winbind_cache_time() ) {
		DEBUG_OUT (10,("fetch_cache_seqnum: timeout [%s][%u @ %u]\n",
			domain->name, domain->sequence_number,
			(uint32)domain->last_seq_check));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG_OUT (10,("fetch_cache_seqnum: success [%s][%u @ %u]\n", 
		domain->name, domain->sequence_number, 
		(uint32)domain->last_seq_check));
			 ...
			 time_t last_seq_check)
{
	...
	if (wcache->tdb == NULL) {
		DEBUG_OUT (10, ("wcache_store_seqnum: wcache->tdb == NULL\n"));
		return false;
	}

	key_str = talloc_asprintf(talloc_tos(), "SEQNUM/%s", domain_name);
	if (key_str == NULL) {
		DEBUG_OUT (10, ("talloc_asprintf failed\n"));
		return false;
	}
	...
	TALLOC_FREE(key_str);
	if (ret == -1) {
		DEBUG_OUT (10, ("tdb_store_bystring failed: %s\n",
			   tdb_errorstr(wcache->tdb)));
		TALLOC_FREE(key_str);
		return false;
	}

	DEBUG_OUT (10, ("wcache_store_seqnum: success [%s][%u @ %u]\n",
...
static void refresh_sequence_number(struct winbindd_domain *domain, bool force)
{
	...
	if (!force && (time_diff < cache_time) &&
		...
		DEBUG_OUT (10, ("refresh_sequence_number: %s time ok\n", domain->name));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("refresh_sequence_number: failed with %s\n", nt_errstr(status)));
		domain->sequence_number = DOM_SEQUENCE_NONE;
	}
	...
	DEBUG_OUT (10, ("refresh_sequence_number: %s seq number is now %d\n", 
		   domain->name, domain->sequence_number));
...
static bool centry_expired(struct winbindd_domain *domain, const char *keystr, struct cache_entry *centry)
{
	...
	if (lp_winbind_offline_logon() && global_winbindd_offline_state) {
		DEBUG_OUT (10,("centry_expired: Key %s for domain %s valid as winbindd is globally offline.\n",
			keystr, domain->name ));
		return false;
	}
	...
	if (!domain->online) {
		DEBUG_OUT (10,("centry_expired: Key %s for domain %s valid as domain is offline.\n",
			keystr, domain->name ));
		return false;
	}
	...
	if ((domain->sequence_number != DOM_SEQUENCE_NONE) &&  
		...
		DEBUG_OUT (10,("centry_expired: Key %s for domain %s invalid sequence.\n",
			keystr, domain->name ));
		return true;
	}
	...
	if (wcache_server_down(domain)
		...
		DEBUG_OUT (10,("centry_expired: Key %s for domain %s is good.\n",
			keystr, domain->name ));
		return false;
	}

	DEBUG_OUT (10,("centry_expired: Key %s for domain %s expired\n",
		keystr, domain->name ));
...
static struct cache_entry *wcache_fetch_raw(char *kstr)
{
	...
	if (centry->len < 16) {
		/* huh? corrupt cache? */
		DEBUG_OUT (10,("wcache_fetch_raw: Corrupt cache for key %s "
			  "(len < 16)?\n", kstr));
		centry_free(centry);
		return NULL;
	}
					...
					const char *format, ...)
{
	...
	if (centry_expired(domain, kstr, centry)) {
		...
		DEBUG_OUT (10,("wcache_fetch: entry %s expired for domain %s\n",
			 kstr, domain->name ));
	...
	DEBUG_OUT (10,("wcache_fetch: returning entry %s for domain %s\n",
		 kstr, domain->name ));
...
static void centry_expand(struct cache_entry *centry, uint32 len)
{
	...
	centry->data = SMB_REALLOC_ARRAY(centry->data, unsigned char,
					 centry->len);
	if (!centry->data) {
		DEBUG_OUT (0,("out of memory: needed %d bytes in centry_expand\n", centry->len));
...
static void centry_put_string(struct cache_entry *centry, const char *s)
{
	...
	if (len > 254) {
		DEBUG_OUT (10,("centry_put_string: truncating len (%d) to: 254\n", len));
		len = 254;
	}
				    ...
				    enum lsa_SidType type)
{
	...
	DEBUG_OUT (10,("wcache_save_name_to_sid: %s\\%s -> %s (%s)\n", domain_name,
				    ...
				    const struct dom_sid *sid, const char *domain_name, const char *name, enum lsa_SidType type)
{
	...
	DEBUG_OUT (10,("wcache_save_sid_to_name: %s -> %s (%s)\n", sid_string, 
			     ...
			     struct wbint_userinfo *info)
{
	...
	centry_end(centry, "U/%s", sid_to_fstring(sid_string,
						  &info->user_sid));
	DEBUG_OUT (10,("wcache_save_user: %s (acct_name %s)\n", sid_string, info->acct_name));
	centry_free(centry);
}
				       ...
				       struct samr_DomInfo12 *lockout_policy)
{
	...
	DEBUG_OUT (10,("wcache_save_lockout_policy: %s\n", domain->name));
					...
					struct samr_DomInfo1 *policy)
{
	...
	DEBUG_OUT (10,("wcache_save_password_policy: %s\n", domain->name));
				       ...
				       const char *name, const char *alias)
{
	...
	DEBUG_OUT (10,("wcache_save_username_alias: %s -> %s\n", name, alias ));
				       ...
				       const char *alias, const char *name)
{
	...
	DEBUG_OUT (10,("wcache_save_alias_username: %s -> %s\n", alias, name ));
				    ...
				    const char *name, char **alias )
{
	...
	*alias = centry_string( centry, mem_ctx );
	...
	DEBUG_OUT (10,("resolve_username_to_alias: [Cached] - mapped %s to %s\n",
...
do_query:
	...
	if ( get_global_winbindd_state_offline() || !domain->online ) {
		DEBUG_OUT (8,("resolve_username_to_alias: rejecting query "
			 "in offline mode\n"));
	...
	DEBUG_OUT (5,("resolve_username_to_alias: backend query returned %s\n",
		 nt_errstr(status)));
				    ...
				    const char *alias, char **name )
{
	...
	*name = centry_string( centry, mem_ctx );
	...
	DEBUG_OUT (10,("resolve_alias_to_username: [Cached] - mapped %s to %s\n",
...
do_query:
	...
	if ( get_global_winbindd_state_offline() || !domain->online ) {
		DEBUG_OUT (8,("resolve_alias_to_username: rejecting query "
			 "in offline mode\n"));
		return NT_STATUS_NOT_FOUND;
	}
	...
	if (strchr(alias, '\\') || strchr(alias, '@')) {
		DEBUG_OUT (10,("resolve_alias_to_username: skipping fully "
			  "qualified name %s\n", alias));
	...
	DEBUG_OUT (5,("resolve_alias_to_username: backend query returned %s\n",
		 nt_errstr(status)));
			  ...
			  const uint8 **cached_salt)
{
	...
	if (!centry) {
		DEBUG_OUT (10,("wcache_get_creds: entry for [CRED/%s] not found\n", 
			  sid_string_dbg(sid)));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}
	...
	*cached_nt_pass = (const uint8 *)centry_hash16(centry, mem_ctx);
	if (*cached_nt_pass == NULL) {
		...
		/* Bad (old) cred cache. Delete and pretend we
		   don't have it. */
		DEBUG_OUT (0,("wcache_get_creds: bad entry for [CRED/%s] - deleting\n", 
				sidstr));
	...
	DEBUG_OUT (10,("wcache_get_creds: [Cached] - cached creds for user %s status: %s\n",
			   ...
			   const uint8 nt_pass[NT_HASH_LEN])
{
	...
	DEBUG_OUT (10,("wcache_save_creds: %s\n", sid_string));
...
do_cached:	
	...
	DEBUG_OUT (10,("query_user_list: [Cached] - cached list for domain %s status: %s\n",
...
do_query:
	...
	*info = NULL;
	...
	do {
		...
		DEBUG_OUT (10,("query_user_list: [Cached] - doing backend query for list for domain %s\n",
			domain->name ));

		status = domain->backend->query_user_list(domain, mem_ctx, num_entries, info);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3, ("query_user_list: returned 0x%08x, "
				  "retrying\n", NT_STATUS_V(status)));
		}
		if (NT_STATUS_EQUAL(status, NT_STATUS_UNSUCCESSFUL)) {
			DEBUG_OUT (3, ("query_user_list: flushing "
				  "connection cache\n"));
...
do_cached:	
	...
	DEBUG_OUT (10,("enum_dom_groups: [Cached] - cached list for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	*info = NULL;
	...
	DEBUG_OUT (10,("enum_dom_groups: [Cached] - doing backend query for list for domain %s\n",
		domain->name ));
...
do_cached:	
	...
	if (wcache_server_down(domain)) {
		DEBUG_OUT (10, ("enum_local_groups: returning cached user list and server was down\n"));
		status = NT_STATUS_MORE_PROCESSING_REQUIRED;
	} else
		status = centry->status;

	DEBUG_OUT (10,("enum_local_groups: [Cached] - cached list for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	*info = NULL;
	...
	DEBUG_OUT (10,("enum_local_groups: [Cached] - doing backend query for list for domain %s\n",
		domain->name ));
			    ...
			    enum lsa_SidType *type)
{
	...
	DEBUG_OUT (10,("name_to_sid: [Cached] - cached name for domain %s status: "
		  "%s\n", domain->name, nt_errstr(status) ));
			    ...
			    enum lsa_SidType *type)
{
	...
	DEBUG_OUT (10,("name_to_sid: [Cached] - doing backend query for name for domain %s\n",
		domain->name ));
			    ...
			    enum lsa_SidType *type)
{
	...
	DEBUG_OUT (10,("sid_to_name: [Cached] - cached name for domain %s status: "
		  "%s\n", domain->name, nt_errstr(status) ));
			    ...
			    enum lsa_SidType *type)
{
	...
	*domain_name = NULL;
	...
	DEBUG_OUT (10,("sid_to_name: [Cached] - doing backend query for name for domain %s\n",
		domain->name ));
			   ...
			   struct wbint_userinfo *info)
{
	...
	if (NT_STATUS_EQUAL(domain->last_status, NT_STATUS_ACCESS_DENIED) &&
		...
		DEBUG_OUT (10, ("query_user: cached access denied and have cached "
			   "info3\n"));
	...
	DEBUG_OUT (10,("query_user: [Cached] - cached info for domain %s status: "
		  "%s\n", domain->name, nt_errstr(status) ));
			   ...
			   struct wbint_userinfo *info)
{
	...
	DEBUG_OUT (10,("query_user: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				  ...
				  struct dom_sid **psids)
{
	...
	if (NT_STATUS_EQUAL(domain->last_status, NT_STATUS_ACCESS_DENIED)
	    && netsamlogon_cache_have(user_sid)) {
		DEBUG_OUT (10, ("lookup_usergroups: cached access denied and have "
			   "cached info3\n"));
		domain->last_status = NT_STATUS_OK;
		centry_free(centry);
		return NT_STATUS_NOT_FOUND;
	...
	DEBUG_OUT (10,("lookup_usergroups: [Cached] - cached info for domain %s "
		  "status: %s\n", domain->name, nt_errstr(status)));
				  ...
				  uint32 *num_groups, struct dom_sid **user_gids)
{
	...
	DEBUG_OUT (10,("lookup_usergroups: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				   ...
				   uint32_t *pnum_aliases, uint32_t **paliases)
{
	...
	DEBUG_OUT (10,("lookup_useraliases: [Cached] - cached info for domain: %s "
		  "status %s\n", domain->name, nt_errstr(status)));
				   ...
				   uint32 *num_aliases, uint32 **alias_rids)
{
	...
	DEBUG_OUT (10,("lookup_usergroups: [Cached] - doing backend query for info "
		  "for domain %s\n", domain->name ));
				...
				uint32_t **name_types)
{
	...
	*name_types = talloc_array(mem_ctx, uint32, *num_names);
	...
	DEBUG_OUT (10,("lookup_groupmem: [Cached] - cached info for domain %s "
		  "status: %s\n", domain->name, nt_errstr(status)));
				...
				uint32 **name_types)
{
	...
	DEBUG_OUT (10,("lookup_groupmem: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
do_query:
	...
	DEBUG_OUT (10,("trusted_domains: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
do_fetch_cache:
	...
	DEBUG_OUT (10,("lockout_policy: [Cached] - cached info for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	DEBUG_OUT (10,("lockout_policy: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
do_query:
	...
	DEBUG_OUT (10,("password_policy: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				...
				const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10, ("wcache_invalidate_samlogon: clearing %s\n", key_str));
...
bool wcache_invalidate_cache(void)
{
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG_OUT (10, ("wcache_invalidate_cache: invalidating cache "
			   "entries for %s\n", domain->name));
		if (cache) {
			if (cache->tdb) {
...
bool init_wcache(void)
{
	...
	if (wcache->tdb == NULL) {
		DEBUG_OUT (0,("Failed to open winbindd_cache.tdb!\n"));
		return false;
	}
...
bool initialize_winbindd_cache(void)
{
	...
	if (!init_wcache()) {
		DEBUG_OUT (0,("initialize_winbindd_cache: init_wcache failed.\n"));
		return false;
	}
	...
	if (cache_bad) {
		DEBUG_OUT (0,("initialize_winbindd_cache: clearing cache "
		...
		if (unlink(cache_path("winbindd_cache.tdb")) == -1) {
			DEBUG_OUT (0,("initialize_winbindd_cache: unlink %s failed %s ",
				cache_path("winbindd_cache.tdb"),
				strerror(errno) ));
			return false;
		}
		if (!init_wcache()) {
			DEBUG_OUT (0,("initialize_winbindd_cache: re-initialization "
					"init_wcache failed.\n"));
			return false;
		}
		...
		if (!tdb_store_uint32(wcache->tdb, WINBINDD_CACHE_VERSION_KEYSTR, WINBINDD_CACHE_VERSION)) {
			DEBUG_OUT (0,("initialize_winbindd_cache: version number store failed %s\n",
				tdb_errorstr(wcache->tdb) ));
			return false;
		}
	}
			       ...
			       TDB_DATA dbuf, void *state)
{
	...
	if (!NT_STATUS_IS_OK(centry->status)) {
		DEBUG_OUT (10,("deleting centry %s\n", (const char *)kbuf.dptr));
		tdb_delete(the_tdb, kbuf);
	}
...
void wcache_flush_cache(void)
{
	...
	DEBUG_OUT (10,("wcache_flush_cache success\n"));
}
				    ...
				    void *state)
{
	...
	if (strncmp((const char *)kbuf.dptr, "CRED/", 5) == 0) {
		...
		if (cred == NULL) {
			DEBUG_OUT (0,("traverse_fn_remove_first_creds: failed to malloc new entry for list\n"));
			return -1;
		}
...
NTSTATUS wcache_remove_oldest_cached_creds(struct winbindd_domain *domain, const struct dom_sid *sid)
{
	...
	/* we possibly already have an entry */
 	if (sid && NT_STATUS_IS_OK(wcache_cached_creds_exist(domain, sid))) {
		...
		DEBUG_OUT (11,("we already have an entry, deleting that\n"));
	...
	for (cred = wcache_cred_list; cred; cred = cred->next) {
		...
		if (!data.dptr) {
			DEBUG_OUT (10,("wcache_remove_oldest_cached_creds: entry for [%s] not found\n", 
				cred->name));
...
bool set_global_winbindd_state_offline(void)
{
	...
	DEBUG_OUT (10,("set_global_winbindd_state_offline: offline requested.\n"));

	/* Only go offline if someone has created
	   the key "WINBINDD_OFFLINE" in the cache tdb. */

	if (wcache == NULL || wcache->tdb == NULL) {
		DEBUG_OUT (10,("set_global_winbindd_state_offline: wcache not open yet.\n"));
		return false;
	}

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("set_global_winbindd_state_offline: rejecting.\n"));
		return false;
	}
	...
	if (!data.dptr || data.dsize != 4) {
		DEBUG_OUT (10,("set_global_winbindd_state_offline: offline state not set.\n"));
		SAFE_FREE(data.dptr);
		return false;
	} else {
		DEBUG_OUT (10,("set_global_winbindd_state_offline: offline state set.\n"));
...
{
	DEBUG_OUT (10,("set_global_winbindd_state_online: online requested.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG_OUT (10,("set_global_winbindd_state_online: rejecting.\n"));
		return;
	}
						  ...
						  struct tdb_validation_status *state)
{
	...
	if (centry->len < 16) {
		/* huh? corrupt cache? */
		DEBUG_OUT (0,("create_centry_validate: Corrupt cache for key %s "
			 "(len < 16) ?\n", kstr));
		centry_free(centry);
...
{
	if (dbuf.dsize != 8) {
		DEBUG_OUT (0,("validate_seqnum: Corrupt cache for key %s (len %u != 8) ?\n",
			    ...
			    struct tdb_validation_status *state)
{
	...
	DEBUG_OUT (10,("validate_loc_pol: %s ok\n", keystr));
	return 0;
}
			    ...
			    struct tdb_validation_status *state)
{
	...
	DEBUG_OUT (10,("validate_pwd_pol: %s ok\n", keystr));
	return 0;
}
			 ...
			 struct tdb_validation_status *state)
{
	...
	DEBUG_OUT (10,("validate_cred: %s ok\n", keystr));
	return 0;
}
		       ...
		       struct tdb_validation_status *state)
{
	...
	if (dbuf.dsize == 0) {
		DEBUG_OUT (0,("validate_dr: Corrupt cache for key %s (len == 0) ?\n",
				keystr));
		state->bad_entry = true;
		state->success = false;
		return 1;
		       ...
		       struct tdb_validation_status *state)
{
	/* Can't say anything about this other than must be nonzero. */
	if (dbuf.dsize == 0) {
		DEBUG_OUT (0,("validate_de: Corrupt cache for key %s (len == 0) ?\n",
				keystr));
		state->bad_entry = true;
		state->success = false;
		return 1;
			   ...
			   TDB_DATA dbuf, struct tdb_validation_status *state)
{
	...
	DEBUG_OUT (10,("validate_pwinfo: %s ok\n", keystr));
	return 0;
}
...
{
	if (dbuf.dsize == 0) {
		DEBUG_OUT (0, ("validate_trustdomcache: Corrupt cache for "
			  "key %s (len ==0) ?\n", keystr));
		state->bad_entry = true;
		state->success = false;
		return 1;
	...
	DEBUG_OUT (10,    ("validate_trustdomcache: %s ok\n", keystr));
...
{
	if (dbuf.dsize != 4) {
		DEBUG_OUT (0,("validate_offline: Corrupt cache for key %s (len %u != 4) ?\n",
				keystr, (unsigned int)dbuf.dsize ));
		state->bad_entry = true;
		state->success = false;
		return 1;
	}
	DEBUG_OUT (10,("validate_offline: %s ok\n", keystr));
	return 0;
}
...
{
	if (dbuf.dsize != 4) {
		DEBUG_OUT (0, ("validate_cache_version: Corrupt cache for "
			  "key %s (len %u != 4) ?\n", 
			  keystr, (unsigned int)dbuf.dsize));
		state->bad_entry = true;
		state->success = false;
	...
	DEBUG_OUT (10, ("validate_cache_version: %s ok\n", keystr));
	return 0;
}
...
static int cache_traverse_validate_fn(TDB_CONTEXT *the_tdb, TDB_DATA kbuf, TDB_DATA dbuf, void *state)
{
	...
	if (kbuf.dsize > max_key_len) {
		DEBUG_OUT (0, ("cache_traverse_validate_fn: key length too large: "
			  "(%u) > (%u)\n\n",
	...
	DEBUG_OUT (0,("cache_traverse_validate_fn: unknown cache entry\nkey :\n"));
...
{
        DEBUG_OUT (0,("validating cache: would panic %s\n", why ));
				    ...
				    void *state)
{
	...
	if (data.dptr == NULL || data.dsize == 0) {
		if (tdb_delete(tdb, key) < 0) {
			DEBUG_OUT (0, ("tdb_delete for [%s] failed!\n",
				  key.dptr));
			return 1;
		}
	}
	...
	if (tdb_store(tdb, key, blob, TDB_REPLACE) < 0) {
		DEBUG_OUT (0, ("tdb_store to update [%s] failed!\n",
			  key.dptr));
		SAFE_FREE(blob.dptr);
		return 1;
	}
...
static bool wbcache_upgrade_v1_to_v2(TDB_CONTEXT *tdb)
{
	...
	DEBUG_OUT (1, ("Upgrade to version 2 of the winbindd_cache.tdb\n"));
...
int winbindd_validate_cache(void)
{
	...
	DEBUG_OUT (10, ("winbindd_validate_cache: replacing panic function\n"));
	...
	tdb = tdb_open_log(tdb_path, 
			   ...
			   0600);
	if (!tdb) {
		DEBUG_OUT (0, ("winbindd_validate_cache: "
	...
	if (vers_id != WINBINDD_CACHE_VERSION) {
		if (vers_id == WINBINDD_CACHE_VER1) {
			...
			if (!ok) {
				DEBUG_OUT (10, ("winbindd_validate_cache: upgrade to version 2 failed.\n"));
				unlink(tdb_path);
				goto done;
			}
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("winbindd_validate_cache: validation not successful.\n"));
		DEBUGADD(10, ("removing tdb %s.\n", tdb_path));
		unlink(tdb_path);
	}

done:
	DEBUG_OUT (10, ("winbindd_validate_cache: restoring panic function\n"));
...
int winbindd_validate_cache_nobackup(void)
{
	...
	if (ret != 0) {
		DEBUG_OUT (10, ("winbindd_validate_cache_nobackup: validation not "
			   "successful.\n"));
	}

	DEBUG_OUT (10, ("winbindd_validate_cache_nobackup: restoring panic "
		   "function\n"));
	smb_panic_fn = smb_panic;
	return ret;
}
...
bool winbindd_cache_validate_and_initialize(void)
{
	...
	if (lp_winbind_offline_logon()) {
		if (winbindd_validate_cache() < 0) {
			DEBUG_OUT (0, ("winbindd cache tdb corrupt and no backup "
				  "could be restored.\n"));
		}
	}
				       ...
				       size_t *num_domains )
{
	...
	for ( i=0; i< (*num_domains); i++ ) {
		if ( strequal( new_dom->name, list[i].domain_name ) ) {
			DEBUG_OUT (10,("add_wbdomain_to_tdc_array: Found existing record for %s\n",
				  new_dom->name));
			idx = i;
			set_only = true;
...
static TDB_DATA make_tdc_key( const char *domain_name )
{
	...
	if ( !domain_name ) {
		DEBUG_OUT (5,("make_tdc_key: Keyname workgroup is NULL!\n"));
		return key;
	}
			     ...
			     unsigned char **buf )
{
	...
	DEBUG_OUT (10,("pack_tdc_domains: Packing %d trusted domains\n",
		  (int)num_domains));
 ...
 again: 
	...
	for ( i=0; i<num_domains; i++ ) {
		...
		if ( buflen > 0 ) {
			DEBUG_OUT (10,("pack_tdc_domains: Packing domain %s (%s)\n",
	...
	if ( buflen < len ) {
		...
		if ( (buffer = SMB_MALLOC_ARRAY(unsigned char, len)) == NULL ) {
			DEBUG_OUT (0,("pack_tdc_domains: failed to alloc buffer!\n"));
			buflen = -1;
			goto done;
		}
		buflen = len;
				  ...
				  struct winbindd_tdc_domain **domains )
{
	...
	if ( len == -1 ) {
		DEBUG_OUT (5,("unpack_tdc_domains: Failed to unpack domain array\n"));		
		return 0;
	}

	list = TALLOC_ARRAY( NULL, struct winbindd_tdc_domain, num_domains );
	if ( !list ) {
		DEBUG_OUT (0,("unpack_tdc_domains: Failed to talloc() domain list!\n"));
		return 0;		
	}
	...
	for ( i=0; i<num_domains; i++ ) {
		...
		DEBUG_OUT (11,("unpack_tdc_domains: Unpacking domain %s (%s) "
		...
		if ( !string_to_sid( &(list[i].sid), sid_string ) ) {			
			DEBUG_OUT (10,("unpack_tdc_domains: no SID for domain %s\n",
				  domain_name));
		}
...
bool wcache_tdc_add_domain( struct winbindd_domain *domain )
{
	...
	DEBUG_OUT (10,("wcache_tdc_add_domain: Adding domain %s (%s), SID %s, "
...
struct winbindd_tdc_domain * wcache_tdc_fetch_domain( TALLOC_CTX *ctx, const char *name )
{
	...
	DEBUG_OUT (10,("wcache_tdc_fetch_domain: Searching for domain %s\n", name));
	...
	for ( i=0; i<num_domains; i++ ) {
		...
		{
			DEBUG_OUT (10,("wcache_tdc_fetch_domain: Found domain %s\n",
				  name));
				     ...
				     const struct dom_sid *sid)
{
	...
	DEBUG_OUT (10,("wcache_tdc_fetch_domainbysid: Searching for domain %s\n",
		  sid_string_dbg(sid)));
	...
	for (i = 0; i<num_domains; i++) {
		if (sid_equal(sid, &(dom_list[i].sid))) {
			DEBUG_OUT (10, ("wcache_tdc_fetch_domainbysid: "
				    ...
				    uint32 gid)
{
	...
	DEBUG_OUT (10,("wcache_save_user_pwinfo: %s\n", sid_string_dbg(user_sid) ));
			      ...
			      const char **gecos, gid_t *p_gid)
{
	...
	*p_gid   = centry_uint32( centry );	
	...
	DEBUG_OUT (10,("nss_get_info_cached: [Cached] - user_sid %s\n",
		  sid_string_dbg(user_sid)));
...
do_query:
	...
	DEBUG_OUT (10, ("nss_get_info returned %s\n", nt_errstr(nt_status)));
	...
	if ( NT_STATUS_EQUAL( nt_status, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND ) ) {
		DEBUG_OUT (5,("nss_get_info_cached: Setting domain %s offline\n",
			 domain->name ));
		set_domain_offline( domain );
	}
		      ...
		      uint32_t opnum, const DATA_BLOB *req, DATA_BLOB *resp)
{
	...
	if (!is_domain_offline(domain)) {
		...
		if (entry_seqnum != dom_seqnum) {
			DEBUG_OUT (10, ("Entry has wrong sequence number: %d\n",
				   (int)entry_seqnum));
			goto fail;
		}
		entry_timeout = BVAL(data.dptr, 4);
		if (time(NULL) > entry_timeout) {
			DEBUG_OUT (10, ("Entry has timed out\n"));
			goto fail;
		}
	}
		      ...
		      const DATA_BLOB *req, const DATA_BLOB *resp)
{
	...
	if (!wcache_fetch_seqnum(domain->name, &dom_seqnum, &last_check)) {
		DEBUG_OUT (10, ("could not fetch seqnum for domain %s\n",
			   domain->name));
		return;
	}
