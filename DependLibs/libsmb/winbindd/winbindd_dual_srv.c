			     ...
			     struct wbint_Sids2UnixIDs *r)
{
	...
	for (i=0; i<r->in.domains->count; i++) {
		...
		if (dom == NULL) {
			DEBUG_OUT (10, ("idmap domain %s not found\n",
				   d->name.string));
			continue;
		}
		...
		DEBUG_OUT (10, ("sids_to_unixids returned %s\n",
			   nt_errstr(status)));
...
NTSTATUS _wbint_DsGetDcName(struct pipes_struct *p, struct wbint_DsGetDcName *r)
{
	...
	reset_cm_connection_on_error(domain, status);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Can't contact the NETLOGON pipe\n"));
		return status;
	}
	...
	reset_cm_connection_on_error(domain, status);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("dcerpc_netr_Get[Any]DCName failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
...
#define MAX_RETRIES 3
	...
	DEBUG_OUT (3,("domain %s secret is %s\n", domain->name,
		NT_STATUS_IS_OK(status) ? "good" : "bad"));

 done:
	DEBUG_OUT (NT_STATUS_IS_OK(status) ? 5 : 2,
...
NTSTATUS _wbint_PingDc(struct pipes_struct *p, struct wbint_PingDc *r)
{
	...
	reset_cm_connection_on_error(domain, status);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (2, ("dcerpc_netr_LogonControl failed: %s\n",
			nt_errstr(status)));
		return status;
	}

	if (!W_ERROR_EQUAL(werr, WERR_NOT_SUPPORTED)) {
		DEBUG_OUT (2, ("dcerpc_netr_LogonControl returned %s, expected "
			  "WERR_NOT_SUPPORTED\n",
			  win_errstr(werr)));
		return werror_to_ntstatus(werr);
	}

	DEBUG_OUT (5, ("winbindd_dual_ping_dc succeeded\n"));
	return NT_STATUS_OK;
}
