...
void winbindd_list_trusted_domains(struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: list trusted domains\n",
							...
							struct winbindd_cli_state *state)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (3, ("winbindd_dual_list_trusted_domains: trusted_domains returned %s\n",
			nt_errstr(result) ));
...
void winbindd_domain_info(struct winbindd_cli_state *cli)
{
	...
	if (req == NULL) {
		DEBUG_OUT (3, ("wb_domain_request_send failed\n"));
		request_error(cli);
		return;
	}
...
static void domain_info_done(struct tevent_req *req)
{
	...
	TALLOC_FREE(req);
	if (ret == -1) {
		DEBUG_OUT (10, ("wb_domain_request failed: %s\n", strerror(errno)));
		request_error(state->cli);
		return;
	}
	if (!state->domain->initialized) {
		DEBUG_OUT (5, ("wb_domain_request did not initialize domain %s\n",
			  state->domain->name));
		request_error(state->cli);
		return;
	}
...
void winbindd_dc_info(struct winbindd_cli_state *cli)
{
	...
	if (cli->request->domain_name[0] != '\0') {
		domain = find_domain_from_name_noinit(
			cli->request->domain_name);
		DEBUG_OUT (10, ("Could not find domain %s\n",
			   cli->request->domain_name));
		if (domain == NULL) {
			request_error(cli);
			return;
	...
	if (!fetch_current_dc_from_gencache(
		...
		DEBUG_OUT (10, ("fetch_current_dc_from_gencache(%s) failed\n",
			   domain->name));
		request_error(cli);
		return;
	}
...
void winbindd_info(struct winbindd_cli_state *state)
{
	...
	DEBUG_OUT (3, ("[%5lu]: request misc info\n", (unsigned long)state->pid));
...
{
	DEBUG_OUT (3, ("[%5lu]: request interface version\n",
		  (unsigned long)state->pid));
...
{
	DEBUG_OUT (3, ("[%5lu]: request domain name\n", (unsigned long)state->pid));
...
{
	DEBUG_OUT (3, ("[%5lu]: request netbios name\n",
		  (unsigned long)state->pid));
...
{
	char *priv_dir;
	DEBUG_OUT (3, ("[%5lu]: request location of privileged pipe\n",
		  (unsigned long)state->pid));
