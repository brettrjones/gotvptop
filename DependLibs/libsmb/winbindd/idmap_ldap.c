				 ...
				 char **dn )
{
	...
	if ( tmp ) {
		if (!dom) {
			DEBUG_OUT (0, ("get_credentials: Invalid domain 'NULL' "
		...
		if (!secret) {
			DEBUG_OUT (0, ("get_credentials: Unable to fetch "
		...
		*dn = talloc_strdup(mem_ctx, tmp);
		...
		if (!fetch_ldap_pw(&user_dn, &secret)) {
			DEBUG_OUT (2, ("get_credentials: Failed to lookup ldap "
...
static NTSTATUS verify_idpool(struct idmap_domain *dom)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1, ("Unable to verify the idpool, "
	...
	if ( count > 1 ) {
		DEBUG_OUT (0,("Multiple entries returned from %s (base == %s)\n",
			filter, ctx->suffix));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
						...
						struct unixid *xid)
{
	...
	default:
		DEBUG_OUT (2, ("Invalid ID type (0x%x)\n", xid->type));
	...
	DEBUG_OUT (10, ("Search of the id pool (filter: %s)\n", filter));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (0,("%s object not found\n", LDAP_OBJ_IDPOOL));
		goto done;
	}
	...
	count = ldap_count_entries(ctx->smbldap_state->ldap_struct, result);
	if (count != 1) {
		DEBUG_OUT (0,("Single %s object not found\n", LDAP_OBJ_IDPOOL));
		goto done;
	}
	...
	id_str = smbldap_talloc_single_attribute(
				...
				entry, type, mem_ctx);
	if (id_str == NULL) {
		DEBUG_OUT (0,("%s attribute not found\n", type));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	case ID_TYPE_UID:
		if (xid->id > dom->high_id) {
			DEBUG_OUT (0,("Cannot allocate uid above %lu!\n",
	...
	case ID_TYPE_GID:
		if (xid->id > dom->high_id) {
			DEBUG_OUT (0,("Cannot allocate gid above %lu!\n",
				 (unsigned long)dom->high_id));
			goto done;
		}
		break;
	...
	if (mods == NULL) {
		DEBUG_OUT (0,("smbldap_set_mod() failed.\n"));
		goto done;
	}

	DEBUG_OUT (10, ("Try to atomically increment the id (%s -> %s)\n",
		   id_str, new_id_str));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (1,("Failed to allocate new %s. "
				       ...
				       struct unixid *id)
{
	...
	if (!strequal(dom->name, "*")) {
		DEBUG_OUT (3, ("idmap_ldap_allocate_id: "
...
static NTSTATUS idmap_ldap_db_init(struct idmap_domain *dom)
{
	...
	if ( ! tmp) {
		DEBUG_OUT (1, ("ERROR: missing idmap ldap url\n"));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	if ( ! tmp || ! *tmp) {
		...
		if ( ! tmp) {
			DEBUG_OUT (1, ("ERROR: missing idmap ldap suffix\n"));
			ret = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
	}
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (1, ("ERROR: smbldap_init (%s) failed!\n", ctx->url));
		goto done;
	}
	...
	ret = get_credentials( ctx, ctx->smbldap_state, config_option,
			       dom, &ctx->user_dn );
	if ( !NT_STATUS_IS_OK(ret) ) {
		DEBUG_OUT (1,("idmap_ldap_db_init: Failed to get connection "
			 "credentials (%s)\n", nt_errstr(ret)));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (1, ("idmap_ldap_db_init: failed to verify ID pool (%s)\n",
			 nt_errstr(ret)));
		goto done;
	}
				       ...
				       const struct id_map *map)
{
	...
	if (rc != LDAP_SUCCESS) {
		...
		DEBUG_OUT (0,("ldap_set_mapping_internals: Failed to add %s to %lu "
			 "mapping [%s]\n", sid,
			 (unsigned long)map->xid.id, type));
		DEBUG_OUT (0, ("ldap_set_mapping_internals: Error was: %s (%s)\n",
	...
	DEBUG_OUT (10,("ldap_set_mapping: Successfully created mapping from %s to "
...
again:
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (3,("Failure looking up ids (%s)\n", ldap_err2string(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if ( ! entry) {
			DEBUG_OUT (2, ("ERROR: Unable to fetch ldap entries "
				  "from results\n"));
			break;
		}
		...
		if ( ! sidstr) { /* no sid, skip entry */
			DEBUG_OUT (2, ("WARNING SID not found on entry\n"));
			continue;
		}
		...
		if ( ! tmp) { /* wow very strange entry, how did it match ? */
			DEBUG_OUT (5, ("Unprobable match on (%s), no uidNumber, "
		...
		if (!idmap_unix_id_is_in_range(id, dom)) {
			DEBUG_OUT (5, ("Requested id (%u) out of range (%u - %u). "
				  "Filtered!\n", id,
		...
		if (!map) {
			DEBUG_OUT (2, ("WARNING: couldn't match sid (%s) "
				  "with requested ids\n", sidstr));
			TALLOC_FREE(sidstr);
			continue;
		}
		...
		if ( ! string_to_sid(map->sid, sidstr)) {
			DEBUG_OUT (2, ("ERROR: Invalid SID on entry\n"));
			TALLOC_FREE(sidstr);
			continue;
		}

		if (map->status == ID_MAPPED) {
			DEBUG_OUT (1, ("WARNING: duplicate %s mapping in LDAP. "
...
again:
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG_OUT (3,("Failure looking up sids (%s)\n",
			 ldap_err2string(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (!map) {
			DEBUG_OUT (2, ("WARNING: couldn't find entry sid (%s) "
				  "in ids", sidstr));
			TALLOC_FREE(sidstr);
			continue;
		}
