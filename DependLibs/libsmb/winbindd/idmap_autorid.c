					      ...
					      void *private_data)
{
	...
	if (!dbwrap_fetch_uint32(db, cfg->sid, &domainnum)) {
		DEBUG_OUT (10, ("Acquiring new range for domain %s\n", cfg->sid));

		/* fetch the current HWM */
		if (!dbwrap_fetch_uint32(db, HWM, &hwm)) {
			DEBUG_OUT (1, ("Fatal error while fetching current "
				  "HWM value!\n"));
		...
		if (hwm >= cfg->globalcfg->maxranges) {
			DEBUG_OUT (1, ("No more domain ranges available!\n"));
		...
		if (!NT_STATUS_IS_OK(ret)) {
			DEBUG_OUT (1, ("Fatal error while fetching a new "
				  "domain range value!\n"));
			goto error;
		}
		...
		if (!NT_STATUS_IS_OK(ret)) {
			DEBUG_OUT (1, ("Fatal error while storing new "
		...
		talloc_free(numstr);
		if (!NT_STATUS_IS_OK(ret)) {
			DEBUG_OUT (1, ("Fatal error while storing "
				  "new domain->range assignment!\n"));
			goto error;
		}
		DEBUG_OUT (5, ("Acquired new range #%d for domain %s\n",
			  domainnum, cfg->sid));
	}

	DEBUG_OUT (10, ("Using range #%d for domain %s\n", domainnum, cfg->sid));
					...
					struct id_map *map)
{
	...
	if (map->xid.id < cfg->minvalue) {
		DEBUG_OUT (10, ("id %d is lower than minimum value, "
	...
	if (map->xid.id > (cfg->minvalue + cfg->rangesize * cfg->maxranges)) {
		DEBUG_OUT (10, ("id %d is outside of maximum id value, "
			   "ignoring mapping request\n", map->xid.id));
		map->status = ID_UNKNOWN;
		return NT_STATUS_OK;
	}
	...
	if (!data.dptr) {
		DEBUG_OUT (4, ("id %d belongs to range %d which does not have "
	...
	if (strncmp((const char *)data.dptr,
		...
		DEBUG_OUT (5, ("id %d belongs to alloc range, cannot map back\n",
			  map->xid.id));
		TALLOC_FREE(data.dptr);
		map->status = ID_UNKNOWN;
		return NT_STATUS_OK;
					...
					struct id_map *map)
{
	...
	if (rid >= global->rangesize) {
		map->status = ID_UNKNOWN;
		DEBUG_OUT (2, ("RID %d is larger then size of range (%d), "
					      ...
					      struct id_map **ids)
{
	...
	for (i = 0; ids[i]; i++) {
		...
		if ((!NT_STATUS_IS_OK(ret)) &&
			...
			DEBUG_OUT (3, ("Unexpected error resolving an ID "
				  " (%d)\n", ids[i]->xid.id));
			goto failure;
		}
	}
					      ...
					      struct id_map **ids)
{
	...
	for (i = 0; ids[i]; i++) {
		...
		if (!sid_split_rid(&domainsid, &rid)) {
			DEBUG_OUT (4, ("Could not determine domain SID from %s, "
				  "ignoring mapping request\n",
		...
		domain = wcache_tdc_fetch_domainbysid(talloc_tos(),
						      &domainsid);
		if (domain == NULL) {
			DEBUG_OUT (10, ("Ignoring unknown domain sid %s\n",
		...
		if (!NT_STATUS_IS_OK(ret)) {
			DEBUG_OUT (3, ("Could not determine range for domain, "
		...
		if ((!NT_STATUS_IS_OK(ret)) &&
		    (!NT_STATUS_EQUAL(ret, NT_STATUS_NONE_MAPPED))) {
			/* some fatal error occurred, log it */
			DEBUG_OUT (3, ("Unexpected error resolving a SID (%s)\n",
				  sid_string_dbg(ids[i]->sid)));
			goto failure;
		}
	}
...
static NTSTATUS idmap_autorid_db_init(void)
{
	...
	if (!autorid_db) {
		DEBUG_OUT (0, ("Unable to open idmap_autorid database '%s'\n",
...
static struct autorid_global_config *idmap_autorid_loadconfig(TALLOC_CTX * ctx)
{
	...
	if (!data.dptr) {
		DEBUG_OUT (10, ("No saved config found\n"));
		return NULL;
	}
	...
	DEBUG_OUT (10, ("Loaded previously stored configuration "
...
static NTSTATUS idmap_autorid_initialize(struct idmap_domain *dom)
{
	...
	if (!strequal(dom->name, "*")) {
		DEBUG_OUT (0, ("idmap_autorid_initialize: Error: autorid configured "
	...
	if (config->rangesize < 2000) {
		DEBUG_OUT (1, ("autorid rangesize must be at least 2000\n"));
	...
	if (config->maxranges == 0) {
		DEBUG_OUT (1, ("allowed uid range is smaller then rangesize, "
	...
	if ((dom->high_id - dom->low_id + 1) % config->rangesize != 0) {
		DEBUG_OUT (5, ("High uid-low uid difference of %d "
	...
	DEBUG_OUT (10, ("Current configuration in config is "
	...
	if (storedconfig &&
		...
		DEBUG_OUT (1, ("New configuration values for rangesize or "
	...
	if (hwm > config->maxranges) {
		DEBUG_OUT (1, ("New upper uid limit is too low to cover "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to store configuration data!\n"));
		goto error;
	}

	DEBUG_OUT (5, ("%d domain ranges with a size of %d are available\n",
...
static NTSTATUS idmap_autorid_allocate_id(struct idmap_domain *dom,
	...
	if (!strequal(dom->name, "*")) {
		DEBUG_OUT (3, ("idmap_autorid_allocate_id: "
	...
	ret = dbwrap_trans_do(autorid_db,
			      ...
			      &domaincfg);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (3, ("Could not determine range for allocation pool, "
			  "check previous messages for reason\n"));
		return ret;
	}
	...
	if (!dbwrap_fetch_uint32(autorid_db, hwmkey, &hwm)) {
		DEBUG_OUT (1, ("Failed to fetch current allocation HWM value: %s\n",
			  nt_errstr(ret)));
		return NT_STATUS_INTERNAL_ERROR;
	}

	if (hwm >= globalcfg->rangesize) {
		DEBUG_OUT (1, ("allocation range is depleted!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	ret = dbwrap_change_uint32_atomic(autorid_db, hwmkey, &(xid->id), 1);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (1, ("Fatal error while allocating new ID!\n"));
		return ret;
	}
	...
	DEBUG_OUT (10, ("Returned new %s %d from allocation range\n",
