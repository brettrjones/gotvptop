...
static NTSTATUS idmap_tdb2_open_db(struct idmap_domain *dom)
{
	...
	if (ctx->db == NULL) {
		DEBUG_OUT (0, ("Unable to open idmap_tdb2 database '%s'\n",
			  db_path));
					      ...
					      void *private_data)
{
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (1, ("Fatal error while fetching a new %s value\n!",
			  state->hwmtype));
		goto done;
	}
				      ...
				      struct unixid *id)
{
	...
	if (!strequal(dom->name, "*")) {
		DEBUG_OUT (3, ("idmap_tdb2_get_new_id: "
...
static NTSTATUS idmap_tdb2_db_init(struct idmap_domain *dom)
{
	...
	if (idmap_script != NULL) {
		DEBUG_OUT (0, ("Warning: 'idmap:script' is deprecated. "
	...
	if (ctx->script) {
		DEBUG_OUT (1, ("using idmap script '%s'\n", ctx->script));
	}
					      ...
					      void *private_data)
{
	...
	ret = dbwrap_store_bystring(db, state->ksidstr,
				    ...
				    TDB_INSERT);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (0, ("Error storing SID -> ID: %s\n", nt_errstr(ret)));
		goto done;
	}
	...
	ret = dbwrap_store_bystring(db, state->kidstr,
				    ...
				    TDB_INSERT);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (0, ("Error storing ID -> SID: %s\n", nt_errstr(ret)));
				  ...
				  const char *fmt, ...)
{
	...
	DEBUG_OUT (10,("idmap script gave: %s\n", line));
	...
	if (sscanf(line, "UID:%lu", &v) == 1) {
		...
		if (!string_to_sid(map->sid, &line[4])) {
			DEBUG_OUT (0,("Bad SID in '%s' from idmap script %s\n",
				 line, ctx->script));
			return NT_STATUS_NONE_MAPPED;			
		}
	} else {
		DEBUG_OUT (0,("Bad reply '%s' from idmap script %s\n",
			 line, ctx->script));
		return NT_STATUS_NONE_MAPPED;
	}
...
static NTSTATUS idmap_tdb2_sid_to_id(struct idmap_domain *dom, struct id_map *map)
{
	...
	if (!data.dptr) {
		...
		DEBUG_OUT (10,(__location__ " Record %s not found\n", keystr));
		...
		if (!idmap_unix_id_is_in_range(map->xid.id, dom)) {
			DEBUG_OUT (5, ("Script returned id (%u) out of range "
				  "(%u - %u). Filtered!\n",
				  map->xid.id, dom->low_id, dom->high_id));
			ret = NT_STATUS_NONE_MAPPED;
			goto done;
						  ...
						  void *private_data)
{
	...
	DEBUG_OUT (10, ("idmap_tdb2_sids_to_unixids_action: "
		   " domain: [%s], allocate: %s\n",
		   state->dom->name,
