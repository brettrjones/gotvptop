...
static void idmap_init(void)
{
	...
	DEBUG_OUT (10, ("idmap_init(): calling static_init_idmap\n"));
			    ...
			    struct idmap_methods *methods)
{
	struct idmap_backend *entry;
 	...
 	if ((version != SMB_IDMAP_INTERFACE_VERSION)) {
		DEBUG_OUT (0, ("Failed to register idmap module.\n"
	...
	if (!name || !name[0] || !methods) {
		DEBUG_OUT (0,("Called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	for (entry = backends; entry != NULL; entry = entry->next) {
		if (strequal(entry->name, name)) {
			DEBUG_OUT (0,("Idmap module %s already registered!\n",
				 name));
	...
	DEBUG_OUT (5, ("Successfully added idmap backend '%s'\n", name));
	return NT_STATUS_OK;
}
					      ...
					      bool check_range)
{
	...
	if (range == NULL) {
		DEBUG_OUT (1, ("idmap range not specified for domain %s\n",
			  result->name));
		if (check_range) {
			goto fail;
		}
			  ...
			  &result->high_id) != 2)
	{
		DEBUG_OUT (1, ("invalid range '%s' specified for domain "
			  "'%s'\n", range, result->name));
		if (check_range) {
			goto fail;
		}
	...
	if (result->low_id > result->high_id) {
		DEBUG_OUT (1, ("Error: invalid idmap range detected: %lu - %lu\n",
	...
	if (result->methods == NULL) {
		DEBUG_OUT (3, ("idmap backend %s not found\n", modulename));

		status = smb_probe_module("idmap", modulename);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3, ("Could not probe idmap module %s\n",
				  modulename));
			goto fail;
		}
	...
	if (result->methods == NULL) {
		DEBUG_OUT (1, ("idmap backend %s not found\n", modulename));
		goto fail;
	}

	status = result->methods->init(result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("idmap initialization returned %s\n",
			  nt_errstr(status)));
		goto fail;
	}
						    ...
						    const char *domname)
{
	...
	if (backend == NULL) {
		DEBUG_OUT (1, ("no backend defined for %s\n", config_option));
		goto fail;
	}
...
static struct idmap_domain *idmap_init_passdb_domain(TALLOC_CTX *mem_ctx)
{
	...
	passdb_idmap_domain = idmap_init_domain(NULL, get_global_sam_name(),
						"passdb", false);
	if (passdb_idmap_domain == NULL) {
		DEBUG_OUT (1, ("Could not init passdb idmap domain\n"));
	}
...
struct idmap_domain *idmap_find_domain(const char *domname)
{
	...
	DEBUG_OUT (10, ("idmap_find_domain called for domain '%s'\n",
...
NTSTATUS idmap_backends_unixid_to_sid(const char *domname, struct id_map *id)
{
	...
	struct id_map *maps[2];
	 ...
	 DEBUG_OUT (10, ("idmap_backend_unixid_to_sid: domain = '%s', xid = %d "
		    "(type %d)\n",
...
NTSTATUS idmap_backends_sid_to_unixid(const char *domain, struct id_map *id)
{
	...
	DEBUG_OUT (10, ("idmap_backends_sid_to_unixid: domain = '%s', sid = [%s]\n",
	    ...
	    || (sid_check_is_in_our_domain(id->sid)))
	{
		...
		DEBUG_OUT (10, ("passdb could not map.\n"));
