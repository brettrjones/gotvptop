...
static ADS_STRUCT *ads_cached_connection(struct winbindd_domain *domain)
{
	...
	DEBUG_OUT (10,("ads_cached_connection\n"));
	...
	if (domain->private_data) {
		...
		DEBUG_OUT (7, ("Current tickets expire in %d seconds (at %d, time is now %d)\n",
		...
		if ( ads->config.realm && (expire > now)) {
			...
			DEBUG_OUT (7,("Deleting expired krb5 credential cache\n"));
			ads->is_mine = True;
			ads_destroy( &ads );
	...
	if (!ads) {
		DEBUG_OUT (1,("ads_init for domain %s failed\n", domain->name));
		return NULL;
	}
	...
	if (!ADS_ERR_OK(status) || !ads->config.realm) {
		DEBUG_OUT (1,("ads_connect for domain %s failed: %s\n",
		...
		if (status.error_type == ENUM_ADS_ERROR_SYSTEM &&
			...
			DEBUG_OUT (1,("Trying MSRPC methods\n"));
			       ...
			       struct wbint_userinfo **pinfo)
{
	...
	*num_entries = 0;
	...
	DEBUG_OUT (3,("ads: query_user_list\n"));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("query_user_list: No incoming trust for domain %s\n",
			  domain->name));		
		return NT_STATUS_OK;
	}
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1,("query_user_list ads_search: %s\n", ads_errstr(rc)));
		status = ads_ntstatus(rc);
	} else if (!res) {
		DEBUG_OUT (1,("query_user_list ads_search returned NULL res\n"));
	...
	if (count == 0) {
		DEBUG_OUT (1,("query_user_list: No users found\n"));
		goto done;
	}
	...
	for (msg = ads_first_entry(ads, res); msg; msg = ads_next_entry(ads, msg)) {
		...
		if (!ads_pull_uint32(ads, msg, "sAMAccountType", &atype) ||
			...
			DEBUG_OUT (1,("Not a user account? atype=0x%x\n", atype));
			continue;
		}
		...
		if (!ads_pull_sid(ads, msg, "objectSid",
				  &info->user_sid)) {
			DEBUG_OUT (1, ("No sid for %s !?\n", info->acct_name));
			continue;
		}

		if (!ads_pull_uint32(ads, msg, "primaryGroupID", &group)) {
			DEBUG_OUT (1, ("No primary group for %s !?\n",
				  info->acct_name));
			continue;
		}
	...
	DEBUG_OUT (3,("ads query_user_list gave %d entries\n", (*num_entries)));
				...
				struct wb_acct_info **info)
{
	...
	*num_entries = 0;
	...
	DEBUG_OUT (3,("ads: enum_dom_groups\n"));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("enum_dom_groups: No incoming trust for domain %s\n",
			  domain->name));		
		return NT_STATUS_OK;
	}
	...
	if (!ADS_ERR_OK(rc)) {
		status = ads_ntstatus(rc);
		DEBUG_OUT (1,("enum_dom_groups ads_search: %s\n", ads_errstr(rc)));
		goto done;
	} else if (!res) {
		DEBUG_OUT (1,("enum_dom_groups ads_search returned NULL res\n"));
		goto done;
	}

	count = ads_count_replies(ads, res);
	if (count == 0) {
		DEBUG_OUT (1,("enum_dom_groups: No groups found\n"));
		goto done;
	}
	...
	DEBUG_OUT (3,("ads enum_dom_groups gave %d entries\n", (*num_entries)));
			   ...
			   struct wbint_userinfo *info)
{
	...
	{
		DEBUG_OUT (5,("query_user: Cache lookup succeeded for %s\n", 
			 sid_string_dbg(sid)));
	...
	if ( !winbindd_can_contact_domain(domain)) {
		DEBUG_OUT (8,("query_user: No incoming trust from domain %s\n",
			 domain->name));
	...
	SAFE_FREE(ldap_exp);
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1,("query_user(sid=%s) ads_search: %s\n",
			 sid_string_dbg(sid), ads_errstr(rc)));
		return ads_ntstatus(rc);
	} else if (!msg) {
		DEBUG_OUT (1,("query_user(sid=%s) ads_search returned NULL res\n",
			 sid_string_dbg(sid)));
	...
	if (count != 1) {
		DEBUG_OUT (1,("query_user(sid=%s): Not found\n",
			 sid_string_dbg(sid)));
		ads_msgfree(ads, msg);
	...
	info->primary_gid = gid;
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("nss_get_info_cached failed: %s\n",
			  nt_errstr(status)));
		return status;
	}
	...
	DEBUG_OUT (3,("ads query_user gave %s\n", info->acct_name));
	return NT_STATUS_OK;
}
					 ...
					 uint32_t *p_num_groups, struct dom_sid **user_sids)
{
	...
	DEBUG_OUT (3,("ads: lookup_usergroups_member\n"));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("lookup_usergroups_members: No incoming trust for domain %s\n",
			  domain->name));		
		return NT_STATUS_OK;
	}
	...
	if (!ldap_exp) {
		DEBUG_OUT (1,("lookup_usergroups(dn=%s) asprintf failed!\n", user_dn));
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1,("lookup_usergroups ads_search member=%s: %s\n", user_dn, ads_errstr(rc)));
		return ads_ntstatus(rc);
	} else if (!res) {
		DEBUG_OUT (1,("lookup_usergroups ads_search returned NULL res\n"));
		return NT_STATUS_INTERNAL_ERROR;
	}
	...
	if (count > 0) {
		for (msg = ads_first_entry(ads, res); msg;
			...
			if (!ads_pull_sid(ads, msg, "objectSid", &group_sid)) {
				DEBUG_OUT (1,("No sid for this group ?!?\n"));
				continue;
			}
	...
	*p_num_groups = num_groups;
	...
	DEBUG_OUT (3,("ads lookup_usergroups (member) succeeded for dn=%s\n", user_dn));
done:
	if (res) 
		ads_msgfree(ads, res);
					   ...
					   struct dom_sid **user_sids)
{
	...
	DEBUG_OUT (3,("ads: lookup_usergroups_memberof\n"));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("lookup_usergroups_memberof: No incoming trust for "
			  "domain %s\n", domain->name));
		return NT_STATUS_OK;
	}
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1,("lookup_usergroups_memberof ads_search "
			"member=%s: %s\n", user_dn, ads_errstr(rc)));
		return ads_ntstatus(rc);
	}
	...
	*user_sids = NULL;
	...
	if (i == 0) {
		DEBUG_OUT (1,("No memberOf for this user?!?\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
	...
	*p_num_groups = num_groups;
	...
	DEBUG_OUT (3,("ads lookup_usergroups (memberof) succeeded for dn=%s\n",
		user_dn));
				  ...
				  uint32 *p_num_groups, struct dom_sid **user_sids)
{
	...
	DEBUG_OUT (3,("ads: lookup_usergroups\n"));
	*p_num_groups = 0;
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("lookup_usergroups: No incoming trust for domain %s\n",
			  domain->name));
	...
	if (!ADS_ERR_OK(rc)) {
		status = ads_ntstatus(rc);
		DEBUG_OUT (1, ("lookup_usergroups(sid=%s) ads_search tokenGroups: "
			  "%s\n", sid_string_dbg(sid), ads_errstr(rc)));
		goto done;
	}
	...
	if (!msg) {
		DEBUG_OUT (1,("lookup_usergroups(sid=%s) ads_search tokenGroups: NULL msg\n", 
			 sid_string_dbg(sid)));
		status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	if (!ads_pull_uint32(ads, msg, "primaryGroupID", &primary_group_rid)) {
		DEBUG_OUT (1,("%s: No primary group for sid=%s !?\n", 
	...
	*p_num_groups = (uint32)num_groups;
	...
	DEBUG_OUT (3,("ads lookup_usergroups (tokenGroups) succeeded for sid=%s\n",
		 sid_string_dbg(sid)));
done:
	TALLOC_FREE(user_dn);
	ads_msgfree(ads, msg);
				...
				uint32 **name_types)
{
	...
	DEBUG_OUT (10,("ads: lookup_groupmem %s sid=%s\n", domain->name,
	...
	*num_names = 0;
	...
	if (!tmp_ctx) {
		DEBUG_OUT (1, ("ads: lookup_groupmem: talloc failed\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("lookup_groupmem: No incoming trust for domain %s\n",
			  domain->name));
		return NT_STATUS_OK;
	}
	...
	if (ldap_exp == NULL) {
		DEBUG_OUT (1, ("ads: lookup_groupmem: talloc_asprintf for ldap_exp failed!\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (0,("ads_ranged_search failed with: %s\n", ads_errstr(rc)));
		status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}

	DEBUG_OUT (10, ("ads lookup_groupmem: got %d sids via extended dn call\n", (int)num_members));
	...
	for (i=0; i<num_members; i++) {
		...
		if (lookup_cached_sid(mem_ctx, &sid, &domain_name, &name,
		    &name_type)) {
			DEBUG_OUT (10,("ads: lookup_groupmem: got sid %s from "
				  "cache\n", sid_string_dbg(&sid)));
			...
			DEBUG_OUT (10, ("ads: lookup_groupmem: sid %s not found in "
				   "cache\n", sid_string_dbg(&sid)));
	...
	DEBUG_OUT (10, ("ads: lookup_groupmem: %d sids found in cache, "
	...
	if (num_nocache > 0) {
		...
		{
			DEBUG_OUT (1, ("lsa_lookupsids call failed with %s "
				  "- retrying...\n", nt_errstr(status)));
		    ...
		    NT_STATUS_EQUAL(status, STATUS_SOME_UNMAPPED))
		{
			...
			DEBUG_OUT (10, ("lookup_groupmem: lsa_lookup_sids could "
			...
			DEBUG_OUT (10, ("lookup_groupmem: Error looking up %d "
	...
	status = NT_STATUS_OK;
	DEBUG_OUT (3,("ads lookup_groupmem for sid=%s succeeded\n",
		 sid_string_dbg(group_sid)));
...
static NTSTATUS sequence_number(struct winbindd_domain *domain, uint32 *seq)
{
	...
	DEBUG_OUT (3,("ads: fetch sequence_number for %s\n", domain->name));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("sequence: No incoming trust for domain %s\n",
			  domain->name));
		*seq = time(NULL);		
		return NT_STATUS_OK;
	}
				...
				struct netr_DomainTrustList *trusts)
{
	...
	DEBUG_OUT (3,("ads: trusted_domains\n"));
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (5, ("trusted_domains: Could not open a connection to %s "
			  "for PIPE_NETLOGON (%s)\n", 
	...
	for (i = 0; i < trusts->count; i++) {
		...
		if ((trust->trust_attributes
		    ...
		    !domain->primary )
		{
			DEBUG_OUT (10,("trusted_domains: Skipping external trusted "
		...
		if ( domain->primary ) {
			DEBUG_OUT (10,("trusted_domains(ads):  Searching "
