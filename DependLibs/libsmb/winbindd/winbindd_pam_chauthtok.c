	...
	struct winbindd_request *request)
{
	...
	DEBUG_OUT (3, ("[%5lu]: pam chauthtok %s\n", (unsigned long)cli->pid,
		  request->data.chauthtok.user));
	...
	if (!canonicalize_username(request->data.chauthtok.user, domain,
				   user)) {
		DEBUG_OUT (10, ("winbindd_pam_chauthtok: canonicalize_username %s "
			   "failed with\n", request->data.chauthtok.user));
	...
	if (contact_domain == NULL) {
		DEBUG_OUT (3, ("Cannot change password for [%s] -> [%s]\\[%s] "
				     ...
				     struct winbindd_response *response)
{
	...
	*response = *state->response;
	...
	if (state->request->flags & WBFLAG_PAM_CACHED_LOGIN) {
		...
		DEBUG_OUT (10, ("winbindd_replace_memory_creds returned %s\n",
			   nt_errstr(status)));
