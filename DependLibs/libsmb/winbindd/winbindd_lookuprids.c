					    ...
					    struct winbindd_request *request)
{
	...
	if (domain == NULL) {
		DEBUG_OUT (5, ("Domain for sid %s not found\n",
	...
	if (request->extra_data.data[request->extra_len-1] != '\0') {
		DEBUG_OUT (5, ("extra_data not 0-terminated\n"));
		tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return tevent_req_post(req, ev);
	}
	...
	if (!parse_ridlist(state, request->extra_data.data,
		...
		DEBUG_OUT (5, ("parse_ridlist failed\n"));
		tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return tevent_req_post(req, ev);
	}
				  ...
				  struct winbindd_response *response)
{
	...
	if (tevent_req_is_nterror(req, &status)) {
		DEBUG_OUT (5, ("Lookuprids failed: %s\n",nt_errstr(status)));
		return status;
	}
			  ...
			  uint32_t **prids, uint32_t *pnum_rids)
{
	...
	for (i=0; i<num_rids; i++) {
		...
		if (*q != '\n') {
			DEBUG_OUT (0, ("Got invalid ridstr: %s\n", p));
			return false;
		}
		p = q+1;
	}
