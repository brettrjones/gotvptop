...
static ADS_STATUS ad_idmap_cached_connection_internal(struct idmap_domain *dom)
{
	...
	DEBUG_OUT (10, ("ad_idmap_cached_connection: called for domain '%s'\n",
		   dom->name));
	...
	DEBUG_OUT (10, ("ldap_server from saf cache: '%s'\n", ldap_server?ldap_server:""));

	wb_dom = find_domain_from_name_noinit(dom->name);
	if (wb_dom == NULL) {
		DEBUG_OUT (10, ("find_domain_from_name_noinit did not find domain '%s'\n",
			   dom->name));
		realm = NULL;
	} else {
		DEBUG_OUT (10, ("find_domain_from_name_noinit found realm '%s' for "
			  " domain '%s'\n", wb_dom->alt_name, dom->name));
		realm = wb_dom->alt_name;
	}

	if ( (ads = ads_init(realm, dom->name, ldap_server)) == NULL ) {
		DEBUG_OUT (1,("ads_init failed\n"));
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG_OUT (1, ("ad_idmap_cached_connection_internal: failed to "
			  "connect to AD\n"));
		ads_destroy(&ads);
		return status;
	}
...
static ADS_STATUS ad_idmap_cached_connection(struct idmap_domain *dom)
{
	     ...
	     (ctx->ad_map_type ==  WB_POSIX_MAP_RFC2307) )
	{
		...
		if ( !ADS_ERR_OK(status) ) {
			DEBUG_OUT (2,("ad_idmap_cached_connection: Failed to obtain schema details!\n"));
		}
	}
...
static NTSTATUS idmap_ad_initialize(struct idmap_domain *dom)
{
	...
	if ( schema_mode && schema_mode[0] ) {
		if ( strequal(schema_mode, "sfu") )
			...
			DEBUG_OUT (0,("idmap_ad_initialize: Unknown schema_mode (%s)\n",
				 schema_mode));
	}
...
static NTSTATUS idmap_ad_unixids_to_sids(struct idmap_domain *dom, struct id_map **ids)
{
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1, ("ADS uninitialized: %s\n", ads_errstr(rc)));
...
again:
	...
	for (i = 0; (i < IDMAP_AD_MAX_IDS) && ids[idx]; i++, idx++) {
		...
		default:
			DEBUG_OUT (3, ("Error: mapping requested but Unknown ID type\n"));
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG_OUT (1, ("ERROR: ads search returned: %s\n", ads_errstr(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}

	if ( (count = ads_count_replies(ctx->ads, res)) == 0 ) {
		DEBUG_OUT (10, ("No IDs found\n"));
	}
	...
	for (i = 0; (i < count) && entry; i++) {
		...
		if ( !entry ) {
			DEBUG_OUT (2, ("ERROR: Unable to fetch ldap entries from results\n"));
			break;
		}

		/* first check if the SID is present */
		if (!ads_pull_sid(ctx->ads, entry, "objectSid", &sid)) {
			DEBUG_OUT (2, ("Could not retrieve SID from entry\n"));
			continue;
		}

		/* get type */
		if (!ads_pull_uint32(ctx->ads, entry, "sAMAccountType", &atype)) {
			DEBUG_OUT (1, ("could not get SAM account type\n"));
			continue;
		}
		...
		case ATYPE_INTERDOMAIN_TRUST:
			...
			break;
		default:
			DEBUG_OUT (1, ("unrecognized SAM account type %08x\n", atype));
			continue;
		}
		...
		if (!ads_pull_uint32(ctx->ads, entry, (type==ID_TYPE_UID) ?
				     ...
				     &id)) 
		{
			DEBUG_OUT (1, ("Could not get unix ID\n"));
			continue;
		}
		...
		if (!map) {
			DEBUG_OUT (2, ("WARNING: couldn't match result with requested ID\n"));
			continue;
		}
...
static NTSTATUS idmap_ad_sids_to_unixids(struct idmap_domain *dom, struct id_map **ids)
{
	...
	if (ctx->ad_schema == NULL) {
		DEBUG_OUT (0, ("haven't got ctx->ad_schema ! \n"));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
...
again:
	...
	for (i = 0; (i < count) && entry; i++) {
		...
		if (!map) {
			DEBUG_OUT (2, ("WARNING: couldn't match result with requested SID\n"));
			continue;
		}
				    ...
				    enum wb_posix_mapping new_ad_map_type)
{
	...
	{
		DEBUG_OUT (2, ("nss_ad_generic_init: "
				  ...
				  uint32 *gid )
{
	...
	DEBUG_OUT (10, ("nss_ad_get_info called for sid [%s] in domain '%s'\n",
	...
	if (!ctx->ad_schema) {
		DEBUG_OUT (10, ("nss_ad_get_info: no ad_schema configured!\n"));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}
	...
	DEBUG_OUT (10, ("nss_ad_get_info: no ads connection given, doing our "
		   "own query\n"));
