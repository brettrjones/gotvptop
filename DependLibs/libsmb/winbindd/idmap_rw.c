			      ...
			      struct id_map *map)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Could not allocate id: %s\n", nt_errstr(status)));
		return status;
	}

	DEBUG_OUT (10, ("Setting mapping: %s <-> %s %lu\n",
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_OBJECT_NAME_COLLISION)) {
		struct id_map *ids[2];
		DEBUG_OUT (5, ("Mapping for %s exists - retrying to map sid\n",
			  sid_string_dbg(map->sid)));
		ids[0] = map;
		ids[1] = NULL;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Could not store the new mapping: %s\n",
			  nt_errstr(status)));
		return status;
	}
