...
static int convert_fn(struct db_record *rec, void *private_data)
{
	...
	*p++ = '/';
	...
	if (domain == NULL) {
		...
		DEBUG_OUT (0,("Unable to find domain %s\n", dom_name ));
		DEBUG_OUT (0,("deleting record %s\n", (const char *)rec->key.dptr ));

		status = rec->delete_rec(rec);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Unable to delete record %s:%s\n",
				(const char *)rec->key.dptr,
				nt_errstr(status)));
			s->failed = true;
			return -1;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Unable to add record %s:%s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Unable to update record %s:%s\n",
...
static bool idmap_tdb_upgrade(struct idmap_domain *dom, struct db_context *db)
{
	...
	DEBUG_OUT (0, ("Upgrading winbindd_idmap.tdb from an old version\n"));
	...
	if (((vers == -1) && bigendianheader) || (IREV(vers) == IDMAP_VERSION)) {
		...
		if (dbwrap_store_int32(db, HWM_USER, wm) == -1) {
			DEBUG_OUT (0, ("Unable to byteswap user hwm in idmap database\n"));
			return False;
		}
		...
		if (dbwrap_store_int32(db, HWM_GROUP, wm) == -1) {
			DEBUG_OUT (0, ("Unable to byteswap group hwm in idmap database\n"));
			return False;
		}
	}
	...
	if (s.failed) {
		DEBUG_OUT (0, ("Problem during conversion\n"));
		return False;
	}

	if (dbwrap_store_int32(db, "IDMAP_VERSION", IDMAP_VERSION) == -1) {
		DEBUG_OUT (0, ("Unable to store idmap version in database\n"));
		return False;
	}
...
static NTSTATUS idmap_tdb_init_hwm(struct idmap_domain *dom)
{
	...
	if (ctx->db->transaction_start(ctx->db) != 0) {
		DEBUG_OUT (0, ("Unable to start upgrade transaction!\n"));
	...
	if (update_uid) {
		...
		if (ret == -1) {
			...
			DEBUG_OUT (0, ("Unable to initialise user hwm in idmap "
				  "database\n"));
			return NT_STATUS_INTERNAL_DB_ERROR;
		}
	}
	...
	if (update_gid) {
		...
		if (ret == -1) {
			ctx->db->transaction_cancel(ctx->db);
			DEBUG_OUT (0, ("Unable to initialise group hwm in idmap "
				  "database\n"));
			return NT_STATUS_INTERNAL_DB_ERROR;
		}
	}
	...
	if (ctx->db->transaction_commit(ctx->db) != 0) {
		DEBUG_OUT (0, ("Unable to commit upgrade transaction!\n"));
		return NT_STATUS_INTERNAL_DB_ERROR;
	}
...
static NTSTATUS idmap_tdb_open_db(struct idmap_domain *dom)
{
	...
	DEBUG_OUT (10,("Opening tdbfile %s\n", tdbfile ));

	/* Open idmap repository */
	db = db_open(mem_ctx, tdbfile, 0, TDB_DEFAULT, O_RDWR | O_CREAT, 0644);
	if (!db) {
		DEBUG_OUT (0, ("Unable to open idmap database\n"));
	...
	if (version != IDMAP_VERSION) {
		if (config_error) {
			DEBUG_OUT (0,("Upgrade of IDMAP_VERSION from %d to %d is not "
		...
		if (!idmap_tdb_upgrade(dom, db)) {
			...
			DEBUG_OUT (0, ("Unable to open idmap database, it's in an old format, and upgrade failed!\n"));
			ret = NT_STATUS_INTERNAL_DB_ERROR;
			goto done;
		}
					     ...
					     void *private_data)
{
	...
	if (hwm > state->high_hwm) {
		DEBUG_OUT (1, ("Fatal Error: %s range full!! (max: %lu)\n",
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (0, ("Fatal error while fetching a new %s value: %s\n!",
				      ...
				      struct unixid *xid)
{
	...
	if (NT_STATUS_IS_OK(status)) {
		xid->id = state.hwm;
		DEBUG_OUT (10,("New %s = %d\n", hwmtype, state.hwm));
	} else {
		DEBUG_OUT (1, ("Error allocating a new %s\n", hwmtype));
	}
				     ...
				     struct unixid *id)
{
	...
	if (!strequal(dom->name, "*")) {
		DEBUG_OUT (3, ("idmap_tdb_get_new_id: "
...
static NTSTATUS idmap_tdb_db_init(struct idmap_domain *dom)
{
	...
	DEBUG_OUT (10, ("idmap_tdb_db_init called for domain '%s'\n", dom->name));
					     ...
					     void *private_data)
{
	...
	ret = dbwrap_store_bystring(db, state->kidstr,
				    ...
				    TDB_REPLACE);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG_OUT (0, ("Error storing ID -> SID (%s -> %s): %s\n",
			  state->kidstr, state->ksidstr, nt_errstr(ret)));
		goto done;
	}

	DEBUG_OUT (10,("Stored %s <-> %s\n", state->ksidstr, state->kidstr));
	ret = NT_STATUS_OK;
				      ...
				      const struct id_map *map)
{
	...
	default:
		DEBUG_OUT (2, ("INVALID unix ID type: 0x02%x\n", map->xid.type));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (kidstr == NULL) {
		DEBUG_OUT (0, ("ERROR: Out of memory!\n"));
		ret = NT_STATUS_NO_MEMORY;
		goto done;
	}
...
static NTSTATUS idmap_tdb_id_to_sid(struct idmap_domain *dom, struct id_map *map)
{
	...
	if (!idmap_unix_id_is_in_range(map->xid.id, dom)) {
		DEBUG_OUT (5, ("Requested id (%u) out of range (%u - %u). Filtered!\n",
	...
	DEBUG_OUT (10,("Fetching record %s\n", keystr));

	/* Check if the mapping exists */
	data = dbwrap_fetch_bystring(ctx->db, NULL, keystr);

	if (!data.dptr) {
		DEBUG_OUT (10,("Record %s not found\n", keystr));
		ret = NT_STATUS_NONE_MAPPED;
		goto done;
	}

	if (!string_to_sid(map->sid, (const char *)data.dptr)) {
		DEBUG_OUT (10,("INVALID SID (%s) in record %s\n",
			(const char *)data.dptr, keystr));
		ret = NT_STATUS_INTERNAL_DB_ERROR;
		goto done;
	}

	DEBUG_OUT (10,("Found record %s -> %s\n", keystr, (const char *)data.dptr));
	ret = NT_STATUS_OK;
...
static NTSTATUS idmap_tdb_sid_to_id(struct idmap_domain *dom, struct id_map *map)
{
	...
	if (sscanf((const char *)data.dptr, "UID %lu", &rec_id) == 1) { /* Try a UID record. */
		...
		DEBUG_OUT (10,("Found uid record %s -> %s \n", keystr, (const char *)data.dptr ));
		ret = NT_STATUS_OK;

	} else if (sscanf((const char *)data.dptr, "GID %lu", &rec_id) == 1) { /* Try a GID record. */
		map->xid.id = rec_id;
		map->xid.type = ID_TYPE_GID;
		DEBUG_OUT (10,("Found gid record %s -> %s \n", keystr, (const char *)data.dptr ));
		ret = NT_STATUS_OK;

	} else { /* Unknown record type ! */
		DEBUG_OUT (2, ("Found INVALID record %s -> %s\n", keystr, (const char *)data.dptr));
		ret = NT_STATUS_INTERNAL_DB_ERROR;
		goto done;
	}
						 ...
						 void *private_data)
{
	...
	DEBUG_OUT (10, ("idmap_tdb_sids_to_unixids_action: "
		   " domain: [%s], allocate: %s\n",
		   state->dom->name,
...
{
	DEBUG_OUT (10, ("calling idmap_tdb_init\n"));
