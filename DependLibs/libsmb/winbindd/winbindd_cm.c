				    ...
				    DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("msg_fail_to_go_online: received for domain %s.\n", domainname));
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			if (domain->online) {
				...
				DEBUG_OUT (5,("msg_fail_to_go_online: domain %s "
					"already online.\n", domainname));
				continue;
			}
				 ...
				 DATA_BLOB *data)
{
	...
	DEBUG_OUT (5,("msg_try_to_go_online: received for domain %s.\n", domainname));
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			...
			if (domain->online) {
				/* We're already online, ignore. */
				DEBUG_OUT (5,("msg_try_to_go_online: domain %s "
					"already online.\n", domainname));
				continue;
			}
...
static bool fork_child_dc_connect(struct winbindd_domain *domain)
{
	...
	if (domain->dc_probe_pid != (pid_t)-1) {
		...
		if (process_exists_by_pid(domain->dc_probe_pid)) {
			DEBUG_OUT (10,("fork_child_dc_connect: pid %u already "
				"checking for DC's.\n",
	...
	if (domain->dc_probe_pid == (pid_t)-1) {
		DEBUG_OUT (0, ("fork_child_dc_connect: Could not fork: %s\n", strerror(errno)));
		return False;
	}
	...
	if (!override_logfile) {
		if (asprintf(&lfile, "%s/log.winbindd-dc-connect", get_dyn_LOGFILEBASE()) == -1) {
			DEBUG_OUT (0, ("fork_child_dc_connect: out of memory.\n"));
			_exit(1);
		}
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("winbindd_reinit_after_fork failed: %s\n",
			  nt_errstr(status)));
	...
	if (!mem_ctx) {
		DEBUG_OUT (0,("talloc_init failed.\n"));
		messaging_send_buf(winbind_messaging_context(),
				   pid_to_procid(parent_pid),
				   MSG_WINBIND_FAILED_TO_GO_ONLINE,
				   (uint8 *)domain->name,
					...
					void *private_data)
{
	...
	DEBUG_OUT (10,("check_domain_online_handler: called for domain "
		  "%s (online = %s)\n", domain->name, 
	...
	if (domain->startup && (time_mono(NULL) > domain->startup_time + 30)) {
		...
		DEBUG_OUT (10,("check_domain_online_handler: domain %s no longer in 'startup' mode.\n",
			domain->name ));
	...
	if (get_global_winbindd_state_offline()) {
		DEBUG_OUT (10,("check_domain_online_handler: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
...
{
	DEBUG_OUT (10,("set_domain_offline: called for domain %s\n",
		domain->name ));

	TALLOC_FREE(domain->check_online_event);

	if (domain->internal) {
		DEBUG_OUT (3,("set_domain_offline: domain %s is internal - logic error.\n",
			domain->name ));
		return;
	}
	...
	if (get_global_winbindd_state_offline()) {
		DEBUG_OUT (10,("set_domain_offline: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
	...
	DEBUG_OUT (10,("set_domain_offline: added event handler for domain %s\n",
		domain->name ));
...
{
	DEBUG_OUT (10,("set_domain_online: called for domain %s\n",
		domain->name ));

	if (domain->internal) {
		DEBUG_OUT (3,("set_domain_online: domain %s is internal - logic error.\n",
			domain->name ));
		return;
	}

	if (get_global_winbindd_state_offline()) {
		DEBUG_OUT (10,("set_domain_online: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
...
void set_domain_online_request(struct winbindd_domain *domain)
{
	...
	DEBUG_OUT (10,("set_domain_online_request: called for domain %s\n",
		domain->name ));

	if (get_global_winbindd_state_offline()) {
		DEBUG_OUT (10,("set_domain_online_request: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}

	if (domain->internal) {
		DEBUG_OUT (10, ("set_domain_online_request: Internal domains are "
			   "always online\n"));
		return;
	}
	...
	if (!domain->check_online_event) {
		...
		DEBUG_OUT (10,("set_domain_online_request: domain %s was globally offline.\n",
			domain->name ));
	}
...
static void cm_get_ipc_userpass(char **username, char **domain, char **password)
{
	...
	*password = (char *)secrets_fetch(SECRETS_AUTH_PASSWORD, NULL);
	...
	if (*username && **username) {
		...
		DEBUG_OUT (3, ("cm_get_ipc_userpass: Retrieved auth-user from secrets.tdb [%s\\%s]\n", 
			  *domain, *username));

	} else {
		DEBUG_OUT (3, ("cm_get_ipc_userpass: No auth-user defined\n"));
				     ...
				     struct sockaddr_storage *dc_ss)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("dcerpc_netr_GetAnyDCName failed: %s\n",
			nt_errstr(result)));
		talloc_destroy(mem_ctx);
		return false;
	}
	...
	DEBUG_OUT (10,("dcerpc_netr_GetAnyDCName returned %s\n", dcname));
				      ...
				      bool *retry)
{
	...
	DEBUG_OUT (10,("cm_prepare_connection: connecting to DC %s for domain %s\n",
	...
	*retry = True;
	...
	if (mutex == NULL) {
		DEBUG_OUT (0,("cm_prepare_connection: mutex grab failed for %s\n",
			 controller));
	...
	if ((*cli = cli_initialise()) == NULL) {
		DEBUG_OUT (1, ("Could not cli_initialize\n"));
	...
	if ((getpeername((*cli)->fd, &peeraddr, &peeraddr_len) != 0)) {
		DEBUG_OUT (0,("cm_prepare_connection: getpeername failed with: %s\n",
			strerror(errno)));
	...
	if ((peeraddr_len != sizeof(struct sockaddr_in))
		...
		DEBUG_OUT (0,("cm_prepare_connection: got unexpected peeraddr len %d\n",
			peeraddr_len));
		result = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	if ((peeraddr_in->sin_family != PF_INET)
		...
		DEBUG_OUT (0,("cm_prepare_connection: got unexpected family %d\n",
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (1, ("cli_negprot failed: %s\n", nt_errstr(result)));
		goto done;
	}
	    ...
	    (*cli)->capabilities & CAP_EXTENDED_SECURITY)
	{
		...
		if (lp_security() == SEC_ADS) {
			...
			(*cli)->use_kerberos = True;
			DEBUG_OUT (5, ("connecting to %s from %s with kerberos principal "
			...
			if (!ADS_ERR_OK(ads_status)) {
				DEBUG_OUT (4,("failed kerberos session setup with %s\n",
		...
		DEBUG_OUT (5, ("connecting to %s from %s with username "
			  "[%s]\\[%s]\n",  controller, global_myname(),
		...
		ads_status = cli_session_setup_spnego(*cli,
						      ...
						      NULL);
		if (!ADS_ERR_OK(ads_status)) {
			DEBUG_OUT (4, ("authenticated session setup failed with %s\n",
				ads_errstr(ads_status)));
		}
	...
	if ((((*cli)->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) != 0) &&
		...
		if (NT_STATUS_IS_OK(cli_session_setup(
			...
			DEBUG_OUT (4, ("authenticated session setup with user %s\\%s failed.\n",
				ipc_domain, ipc_username ));
		}
	}
 ...
 anon_fallback:
	...
	DEBUG_OUT (10,("cm_prepare_connection: falling back to anonymous "
		"connection for DC %s\n",
		controller ));

	if (NT_STATUS_IS_OK(cli_session_setup(*cli, "", NULL, 0,
					      NULL, 0, ""))) {
		DEBUG_OUT (5, ("Connected anonymously\n"));
 ...
 session_setup_done:
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (1,("failed tcon_X with %s\n", nt_errstr(result)));
		goto done;
	}
			      ...
			      struct dc_name_ip **dcs, int *num)
{
	...
	if (!NT_STATUS_IS_OK(check_negative_conn_cache(domain_name, dcname))) {
		DEBUG_OUT (10, ("DC %s was in the negative conn cache\n", dcname));
		return False;
	}
		...
		fstring name )
{
	...
	if (lp_security() == SEC_ADS) {
		...
		if (ADS_ERR_OK(ads_status)) {
			...
			DEBUG_OUT (10,("dcip_to_name: flags = 0x%x\n", (unsigned int)ads->config.flags));
		    ...
		    struct dc_name_ip **dcs, int *num_dcs)
{
		       ...
		       num_dcs) )
	{
		...
		DEBUG_OUT (10, ("Retrieved DC %s at %s via netlogon\n",
			   dcname, addr));
		return True;
	}
 ...
 again:
	...
	if (!NT_STATUS_IS_OK(status)) {
		for (i=0; i<num_dcs; i++) {
			...
			DEBUG_OUT (10, ("find_new_dc: smbsock_any_connect failed for "
				   ...
				   struct winbindd_cm_conn *new_conn)
{
	...
	if ( saf_servername && NT_STATUS_IS_OK(check_negative_conn_cache( domain->name, saf_servername))) {
		...
		DEBUG_OUT (10,("cm_open_connection: saf_servername is '%s' for domain %s\n",
	...
	for (retries = 0; retries < 3; retries++) {
		...
		DEBUG_OUT (10,("cm_open_connection: dcname is '%s' for domain %s\n",
...
static bool connection_ok(struct winbindd_domain *domain)
{
	...
	if (!ok) {
		DEBUG_OUT (3, ("connection_ok: Connection to %s for domain %s is not connected\n",
			  domain->dcname, domain->name));
		return False;
	}

	if (domain->online == False) {
		DEBUG_OUT (3, ("connection_ok: Domain %s is offline\n", domain->name));
		return False;
	}
...
static bool set_dc_type_and_flags_trustinfo( struct winbindd_domain *domain )
{
	...
	DEBUG_OUT (5, ("set_dc_type_and_flags_trustinfo: domain %s\n", domain->name ));
	...
	if ( !connection_ok(our_domain) ) {
		DEBUG_OUT (3,("set_dc_type_and_flags_trustinfo: No connection to our domain!\n"));		
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (5, ("set_dc_type_and_flags_trustinfo: Could not open "
	...
	if ( (mem_ctx = talloc_init("set_dc_type_and_flags_trustinfo")) == NULL ) {
		DEBUG_OUT (0,("set_dc_type_and_flags_trustinfo: talloc_init() failed!\n"));
		return False;
	}	
	...
	result = dcerpc_netr_DsrEnumerateDomainTrusts(b, mem_ctx,
						      ...
						      &werr);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("set_dc_type_and_flags_trustinfo: "
	...
	for ( i=0; i<trusts.count; i++ ) {
		if ( strequal( domain->name, trusts.array[i].netbios_name) ) {
			...
			DEBUG_OUT (5, ("set_dc_type_and_flags_trustinfo: domain %s is %sin "
				  "native mode.\n", domain->name, 
				  domain->native_mode ? "" : "NOT "));

			DEBUG_OUT (5,("set_dc_type_and_flags_trustinfo: domain %s is %s"
...
static void set_dc_type_and_flags_connect( struct winbindd_domain *domain )
{
	...
	mem_ctx = talloc_init("set_dc_type_and_flags on domain %s\n",
			      domain->name);
	if (!mem_ctx) {
		DEBUG_OUT (1, ("set_dc_type_and_flags_connect: talloc_init() failed\n"));
		return;
	}

	DEBUG_OUT (5, ("set_dc_type_and_flags_connect: domain %s\n", domain->name ));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("set_dc_type_and_flags_connect: Could not bind to "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (5, ("set_dc_type_and_flags_connect: rpccli_ds_getprimarydominfo "
...
no_dssetup:
	...
	DEBUG_OUT (5, ("set_dc_type_and_flags_connect: domain %s is %sin native mode.\n",
		  domain->name, domain->native_mode ? "" : "NOT "));

	DEBUG_OUT (5,("set_dc_type_and_flags_connect: domain %s is %srunning active directory.\n",
...
static void set_dc_type_and_flags( struct winbindd_domain *domain )
{
	...
	if ( domain->primary ) {
		DEBUG_OUT (10,("set_dc_type_and_flags: setting up flags for "
			  "primary domain\n"));
			...
			struct rpc_pipe_client **cli, struct policy_handle *sam_handle)
{
	    ...
	    (conn->cli->password == NULL || conn->cli->password[0] == '\0'))
	{
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("cm_connect_sam: No no user available for "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("cm_connect_sam: failed to connect to SAMR "
	...
	DEBUG_OUT (10,("cm_connect_sam: connected to SAMR pipe for "
	...
	DEBUG_OUT (10,("cm_connect_sam: ntlmssp-sealed dcerpc_samr_Connect2 "
 ...
 schannel:
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG_OUT (10, ("cm_connect_sam: Could not get schannel auth info "
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("cm_connect_sam: failed to connect to SAMR pipe for "
			  "domain %s using schannel. Error was %s\n",
			  domain->name, nt_errstr(status) ));
		goto anonymous;
	}
	DEBUG_OUT (10,("cm_connect_sam: connected to SAMR pipe for domain %s using "
		  "schannel.\n", domain->name ));
	...
	DEBUG_OUT (10,("cm_connect_sam: schannel-sealed dcerpc_samr_Connect2 failed "
 ...
 anonymous:
	...
	status = dcerpc_samr_Connect2(conn->samr_pipe->binding_handle, mem_ctx,
				      ...
				      &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("cm_connect_sam: rpccli_samr_Connect2 failed "
	...
	if (!NT_STATUS_IS_OK(result)) {
		status = result;
		DEBUG_OUT (10,("cm_connect_sam: dcerpc_samr_Connect2 failed "
			  "for domain %s Error was %s\n",
			  domain->name, nt_errstr(result)));
		goto done;
	}
			    ...
			    struct rpc_pipe_client **cli)
{
	...
	DEBUG_OUT (10,("cm_connect_lsa_tcp\n"));
	...
	status = cli_rpc_pipe_open_schannel_with_key(conn->cli,
						     ...
						     &conn->lsa_pipe_tcp);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("cli_rpc_pipe_open_schannel_with_key failed: %s\n",
			nt_errstr(status)));
		goto done;
	}
			...
			struct rpc_pipe_client **cli, struct policy_handle *lsa_policy)
{
	...
	if ((conn->cli->user_name[0] == '\0') ||
	    (conn->cli->domain[0] == '\0') || 
	    (conn->cli->password == NULL || conn->cli->password[0] == '\0')) {
		DEBUG_OUT (10, ("cm_connect_lsa: No no user available for "
			   "domain %s, trying schannel\n", conn->cli->domain));
		goto schannel;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (10,("cm_connect_lsa: failed to connect to LSA pipe for "
	...
	DEBUG_OUT (10,("cm_connect_lsa: connected to LSA pipe for domain %s using "
	...
	DEBUG_OUT (10,("cm_connect_lsa: rpccli_lsa_open_policy failed, trying "
		  "schannel\n"));
 ...
 schannel:
	...
	if (!NT_STATUS_IS_OK(result)) {
		/* If this call fails - conn->cli can now be NULL ! */
		DEBUG_OUT (10, ("cm_connect_lsa: Could not get schannel auth info "
			   "for domain %s (error %s), trying anon\n",
			domain->name,
			nt_errstr(result) ));
		goto anonymous;
			     ...
			     struct rpc_pipe_client **cli)
{
	...
	*cli = NULL;
	...
	if ((lp_client_schannel() == True) &&
		...
		DEBUG_OUT (3, ("Server did not offer schannel\n"));
		TALLOC_FREE(netlogon_pipe);
 ...
 no_schannel:
	...
	*/
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (3, ("Could not open schannel'ed NETLOGON pipe. Error "
			  "was %s\n", nt_errstr(result)));
			    ...
			    DATA_BLOB *data)
{
	...
	if ((data == NULL)
		...
		DEBUG_OUT (1, ("invalid msg_ip_dropped message: not a valid "
			  "string\n"));
		return;
	}

	addr = (char *)data->data;
	DEBUG_OUT (10, ("IP %s dropped\n", addr));
	...
	if (!is_ipaddress(addr)) {
		...
		slash = strchr(addr, '/');
		if (slash == NULL) {
			DEBUG_OUT (1, ("invalid msg_ip_dropped message: %s",
				  addr));
			return;
		}
		...
		addr = freeit;
		DEBUG_OUT (10, ("Stripped /netmask to IP %s\n", addr));
	}
