...
static void winbindd_status(void)
{
	...
	DEBUG_OUT (0, ("winbindd status:\n"));

	/* Print client state information */

	DEBUG_OUT (0, ("\t%d clients currently active\n", winbindd_num_clients()));

	if (DEBUGLEVEL >= 2 && winbindd_num_clients()) {
		DEBUG_OUT (2, ("\tclient list:\n"));
...
static void flush_caches(void)
{
	...
	if (!wcache_invalidate_cache()) {
		DEBUG_OUT (0, ("invalidating the cache failed; revalidate the cache\n"));
				      ...
				      void *private_data)
{
	...
	DEBUG_OUT (0,("Got sig[%d] terminate (is_parent=%d)\n",
...
bool winbindd_setup_sig_term_handler(bool parent)
{
	...
	*is_parent = parent;
	...
	se = tevent_add_signal(winbind_event_context(),
			       ...
			       is_parent);
	if (!se) {
		DEBUG_OUT (0,("failed to setup SIGTERM handler"));
	...
	se = tevent_add_signal(winbind_event_context(),
			       ...
			       is_parent);
	if (!se) {
		DEBUG_OUT (0,("failed to setup SIGINT handler"));
		talloc_free(is_parent);
		return false;
	}
			 ...
			 DATA_BLOB *data)
{
	...
	DEBUG_OUT (0,("Got shutdown message\n"));
	terminate(true);
}
				       ...
				       DATA_BLOB *data)
{
	...
	DEBUG_OUT (10, ("winbindd_msg_validate_cache: got validate-cache "
		   "message.\n"));
	...
	if (child_pid == -1) {
		DEBUG_OUT (1, ("winbind_msg_validate_cache: Could not fork: %s\n",
			  strerror(errno)));
		return;
	}
	...
	if (child_pid != 0) {
		/* parent */
		DEBUG_OUT (5, ("winbind_msg_validate_cache: child created with "
			  "pid %d.\n", (int)child_pid));
		return;
	}
	...
	DEBUG_OUT (10, ("winbindd_msg_validata_cache: got return value %d\n", ret));
...
static void process_request(struct winbindd_cli_state *state)
{
	...
	if (atable->send_req != NULL) {
		...
		DEBUG_OUT (10, ("process_request: Handling async request %d:%s\n",
			   (int)state->pid, state->cmd_name));

		req = atable->send_req(state->mem_ctx, winbind_event_context(),
				       state, state->request);
		if (req == NULL) {
			DEBUG_OUT (0, ("process_request: atable->send failed for "
				  "%s\n", atable->cmd_name));
			request_error(state);
			return;
		}
	...
	for (table = dispatch_table; table->fn; table++) {
		if (state->request->cmd == table->cmd) {
			DEBUG_OUT (10,("process_request: request fn %s\n",
	...
	if (!table->fn) {
		DEBUG_OUT (10,("process_request: unknown request fn number %d\n",
...
static void wb_request_done(struct tevent_req *req)
{
	...
	state->response = talloc_zero(state->mem_ctx,
				      struct winbindd_response);
	if (state->response == NULL) {
		DEBUG_OUT (0, ("wb_request_done[%d:%s]: talloc_zero failed - removing client\n",
			  (int)state->pid, state->cmd_name));
		remove_client(state);
		return;
	}
	...
	DEBUG_OUT (10,("wb_request_done[%d:%s]: %s\n",
...
static void request_finished(struct winbindd_cli_state *state)
{
	...
	req = wb_resp_write_send(state, winbind_event_context(),
				 ...
				 state->response);
	if (req == NULL) {
		DEBUG_OUT (10,("request_finished[%d:%s]: wb_resp_write_send() failed\n",
			  (int)state->pid, state->cmd_name));
		remove_client(state);
		return;
	}
...
static void winbind_client_response_written(struct tevent_req *req)
{
	...
	if (ret == -1) {
		close(state->sock);
		state->sock = -1;
		DEBUG_OUT (2, ("Could not write response[%d:%s] to client: %s\n",
			  (int)state->pid, state->cmd_name, strerror(err)));
		remove_client(state);
		return;
	}

	DEBUG_OUT (10,("winbind_client_response_written[%d:%s]: delivered response "
		  "to client\n", (int)state->pid, state->cmd_name));
...
static void new_connection(int listen_sock, bool privileged)
{
	...
	if (sock == -1) {
		if (errno != EINTR) {
			DEBUG_OUT (0, ("Faild to accept socket - %s\n",
				  strerror(errno)));
		}
		return;
	}

	DEBUG_OUT (6,("accepted socket %d\n", sock));
...
static void winbind_client_request_read(struct tevent_req *req)
{
	...
	if (ret == -1) {
		if (err == EPIPE) {
			DEBUG_OUT (6, ("closing socket %d, client exited\n",
				  state->sock));
		} else {
			DEBUG_OUT (2, ("Could not read client request from fd %d: "
...
static void remove_client(struct winbindd_cli_state *state)
{
	...
	if (state->sock != -1) {
		...
		if (nwritten == -1) {
			DEBUG_OUT (2, ("final write to client failed: %s\n",
				strerror(errno)));
		}
...
static bool remove_idle_client(void)
{
	...
	if (remove_state) {
		DEBUG_OUT (5,("Found %d idle client connections, shutting down sock %d, pid %u\n",
					...
					void *private_data)
{
	...
	while (winbindd_num_clients() > lp_winbind_max_clients() - 1) {
		DEBUG_OUT (5,("winbindd: Exceeding %d client "
			 "connections, removing idle "
			 "connection.\n", lp_winbind_max_clients()));
		if (!remove_idle_client()) {
			DEBUG_OUT (0,("winbindd: Exceeding %d "
...
void winbindd_register_handlers(void)
{
	...
	if (!serverid_register(procid_self(),
		...
		DEBUG_OUT (1, ("Could not register myself in serverid.tdb\n"));
		exit(1);
	}
	...
	if (!init_domain_list()) {
		DEBUG_OUT (0,("unable to initialize domain list\n"));
		exit(1);
	}
	...
	if (lp_allow_trusted_domains()) {
		if (tevent_add_timer(winbind_event_context(), NULL, timeval_zero(),
			...
			DEBUG_OUT (0, ("Could not trigger rescan_trusted_domains()\n"));
			exit(1);
		}
	}
				     ...
				     struct messaging_context *msg_ctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("addrchange_context_create failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(state);
		return;
	}
	...
	if (req == NULL) {
		DEBUG_OUT (0, ("addrchange_send failed\n"));
		TALLOC_FREE(state);
		return;
	}
...
static void winbindd_addr_changed(struct tevent_req *req)
{
	...
	TALLOC_FREE(req);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("addrchange_recv failed: %s, stop listening\n",
			   nt_errstr(status)));
		TALLOC_FREE(state);
		return;
	}
	if (type == ADDRCHANGE_DEL) {
		...
		DEBUG_OUT (3, ("winbindd: kernel (AF_NETLINK) dropped ip %s\n",
			  addrstr));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (10, ("messaging_send failed: %s - ignoring\n",
				   nt_errstr(status)));
		}
	}
...
int main(int argc, char **argv, char **envp)
{
	...
	if ( !winbind_off() ) {
		DEBUG_OUT (0,("Failed to disable recusive winbindd calls.  Exiting.\n"));
		exit(1);
	}
	...
	DEBUG_OUT (0,("winbindd version %s started.\n", samba_version_string()));
	DEBUGADD(0,("%s\n", COPYRIGHT_STARTUP_MESSAGE));

	if (!lp_load_initial_only(get_dyn_CONFIGFILE())) {
		DEBUG_OUT (0, ("error opening config file\n"));
		exit(1);
	}
	...
	if (!secrets_init()) {
		...
		DEBUG_OUT (0,("Could not initialize domain trust account secrets. Giving up\n"));
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("ERROR: failed to setup system user info: %s.\n",
			  nt_errstr(status)));
		exit(1);
	}
	...
	if (!winbindd_setup_listeners()) {
		DEBUG_OUT (0,("winbindd_setup_listeners() failed\n"));
		exit(1);
	}
