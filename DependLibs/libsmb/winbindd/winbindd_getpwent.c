...
static void winbindd_getpwent_done(struct tevent_req *subreq)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_MORE_ENTRIES)) {
		DEBUG_OUT (10, ("winbindd_getpwent_done: done with %d users\n",
			   (int)state->num_users));
	...
	if (state->num_users >= state->max_users) {
		DEBUG_OUT (10, ("winbindd_getpwent_done: Got enough users: %d\n",
			   (int)state->num_users));
		tevent_req_done(req);
		return;
	}
	if (state->cli->pwent_state == NULL) {
		DEBUG_OUT (10, ("winbindd_getpwent_done: endpwent called in "
			   "between\n"));
		tevent_req_nterror(req, NT_STATUS_INVALID_PARAMETER);
		return;
	}
