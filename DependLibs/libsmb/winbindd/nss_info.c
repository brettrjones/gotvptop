 ...
 NTSTATUS smb_register_idmap_nss(int version, const char *name, struct nss_info_methods *methods)
{
	...
	if ((version != SMB_NSS_INFO_INTERFACE_VERSION)) {
		DEBUG_OUT (0, ("smb_register_idmap_nss: Failed to register idmap_nss module.\n"
	...
	if (!name || !name[0] || !methods) {
		DEBUG_OUT (0,("smb_register_idmap_nss: called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if ( nss_get_backend(name) ) {
		DEBUG_OUT (0,("smb_register_idmap_nss: idmap module %s "
			 "already registered!\n", name));
	...
	DEBUG_OUT (5, ("smb_register_idmap_nss: Successfully added idmap "
		  "nss backend '%s'\n", name));
					   ...
					   struct nss_function_entry *nss_backend)
{
	...
	if (!nss_domain) {
		DEBUG_OUT (0, ("nss_domain_list_add_domain: talloc() failure!\n"));
	...
	if (domain) {
		...
		if (!nss_domain->domain) {
			DEBUG_OUT (0, ("nss_domain_list_add_domain: talloc() "
				  "failure!\n"));
	...
	if (!NT_STATUS_IS_OK(nss_domain->init_status))  {
		DEBUG_OUT (0, ("nss_init: Failed to init backend '%s' for domain "
			  "'%s'!\n", nss_backend->name, nss_domain->domain));
	}

	DLIST_ADD(nss_domain_list, nss_domain);

	DEBUG_OUT (10, ("Added domain '%s' with backend '%s' to nss_domain_list.\n",
...
static NTSTATUS nss_init(const char **nss_list)
{
	...
	for ( i=0; nss_list && nss_list[i]; i++ ) {
		...
		if ( !parse_nss_parm(nss_list[i], &backend, &domain) ) {
			DEBUG_OUT (0,("nss_init: failed to parse \"%s\"!\n",
				 nss_list[i]));
			continue;
		}

		DEBUG_OUT (10, ("parsed backend = '%s', domain = '%s'\n",
			   backend, domain));
		...
		nss_backend = nss_get_backend(backend);
		if (nss_backend == NULL) {
			DEBUG_OUT (0, ("nss_init: unregistered backend %s!. "
				  "Skipping\n", backend));
			continue;
		}
		...
		if ((domain == NULL) && (default_backend == NULL)) {
			DEBUG_OUT (10, ("nss_init: using '%s' as default backend.\n",
				   backend));
	...
	if ( !nss_domain_list ) {
		DEBUG_OUT (3,("nss_init: no nss backends configured.  "
...
static struct nss_domain_entry *find_nss_domain( const char *domain )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG_OUT (4,("find_nss_domain: Failed to init nss_info API "
			 "(%s)!\n", nt_errstr(status)));
		return NULL;
	}
		       ...
		       const char **gecos, gid_t *p_gid)
{
	...
	DEBUG_OUT (10, ("nss_get_info called for sid [%s] in domain '%s'\n",
		   sid_string_dbg(user_sid), domain?domain:"NULL"));

	if ( (p = find_nss_domain( domain )) == NULL ) {
		DEBUG_OUT (4,("nss_get_info: Failed to find nss domain pointer for %s\n",
			 domain ));
			    ...
			    const char *name, char **alias )
{
	...
	if ( (p = find_nss_domain( domain )) == NULL ) {
		DEBUG_OUT (4,("nss_map_to_alias: Failed to find nss domain pointer for %s\n",
			 domain ));
		return NT_STATUS_NOT_FOUND;
	}
			      ...
			      const char *alias, char **name )
{
	...
	if ( (p = find_nss_domain( domain )) == NULL ) {
		DEBUG_OUT (4,("nss_map_from_alias: Failed to find nss domain pointer for %s\n",
			 domain ));
		return NT_STATUS_NOT_FOUND;
	}
