				      ...
				      struct wbint_userinfo **pinfo)
{
	...
	DEBUG_OUT (3, ("msrpc_query_user_list\n"));
				      ...
				      struct wb_acct_info **pinfo)
{
	...
	DEBUG_OUT (3,("msrpc_enum_dom_groups\n"));
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("enum_domain_groups: No incoming trust for domain %s\n",
			  domain->name));
		status = NT_STATUS_OK;
		goto done;
	}
					...
					struct wb_acct_info **pinfo)
{
	...
	DEBUG_OUT (3,("msrpc_enum_local_groups\n"));
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("enum_local_groups: No incoming trust for domain %s\n",
			  domain->name));
		status = NT_STATUS_OK;
		goto done;
	}
				  ...
				  enum lsa_SidType *type)
{
	...
	if (!full_name) {
		DEBUG_OUT (0, ("talloc_asprintf failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (3, ("msrpc_name_to_sid: name=%s\n", full_name));
	...
	DEBUG_OUT (3,("name_to_sid [rpc] %s for domain %s\n",
				  ...
				  enum lsa_SidType *type)
{
	...
	DEBUG_OUT (3, ("msrpc_sid_to_name: %s for domain %s\n", sid_string_dbg(sid),
		 domain->name ));
	...
	result = winbindd_lookup_sids(mem_ctx,
				      ...
				      &types);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (2,("msrpc_sid_to_name: failed to lookup sids: %s\n",
			nt_errstr(result)));
		return result;
	}
	...
	*name = names[0];
	...
	{
		*name = mapped_name;
		DEBUG_OUT (5,("returning mapped name -- %s\n", *name));
	}
				    ...
				    enum lsa_SidType **types)
{
	...
	DEBUG_OUT (3, ("msrpc_rids_to_names: domain %s\n", domain->name ));
			   ...
			   struct wbint_userinfo *user_info)
{
	...
	DEBUG_OUT (3,("msrpc_query_user sid=%s\n", sid_string_dbg(user_sid)));
	...
	if (user != NULL) {
		DEBUG_OUT (5,("msrpc_query_user: Cache lookup succeeded for %s\n",
			sid_string_dbg(user_sid)));
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("query_user: No incoming trust for domain %s\n",
			  domain->name));
					...
					struct dom_sid **puser_grpsids)
{
	...
	DEBUG_OUT (3,("msrpc_lookup_usergroups sid=%s\n", sid_string_dbg(user_sid)));
					 ...
					 uint32 **palias_rids)
{
	...
	DEBUG_OUT (3,("msrpc_lookup_useraliases\n"));
	...
	if (!winbindd_can_contact_domain(domain)) {
		DEBUG_OUT (10,("msrpc_lookup_useraliases: No incoming trust for domain %s\n",
			  domain->name));
		/* Tell the cache manager not to remember this one */
		status = NT_STATUS_SYNCHRONIZATION_REQUIRED;
		goto done;
				      ...
				      uint32_t **name_types)
{
	...
	DEBUG_OUT (3,("msrpc_lookup_groupmem: %s sid=%s\n", domain->name,
...
static int get_ldap_sequence_number(struct winbindd_domain *domain, uint32 *seq)
{
	...
	if ((ret = get_ldap_seq(addr, LDAP_PORT, seq)) == 0) {
		DEBUG_OUT (3, ("get_ldap_sequence_number: Retrieved sequence "
				      ...
				      uint32_t *pseq)
{
	...
	DEBUG_OUT (3, ("msrpc_sequence_number: fetch sequence_number for %s\n", domain->name));
	...
	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("sequence_number: No incoming trust for domain %s\n",
			  domain->name));
		if (pseq) {
			*pseq = time(NULL);
		}
	...
	if (domain->active_directory) {
		...
		DEBUG_OUT (8,("using get_ldap_seq() to retrieve the "
			 "sequence number\n"));

		rc =  get_ldap_sequence_number(domain, &seq);
		if (rc == 0) {
			DEBUG_OUT (10,("domain_sequence_number: LDAP for "
				  "domain %s is %u\n",
				  domain->name, seq));
		...
		DEBUG_OUT (10,("domain_sequence_number: failed to get LDAP "
				      ...
				      struct netr_DomainTrustList *ptrust_list)
{
	...
	DEBUG_OUT (3,("msrpc_trusted_domains\n"));
				     ...
				     struct samr_DomInfo12 *lockout_policy)
{
	...
	DEBUG_OUT (3, ("msrpc_lockout_policy: fetch lockout policy for %s\n", domain->name));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("msrpc_lockout_policy: No incoming trust for domain %s\n",
			  domain->name));
	...
	*lockout_policy = info->info12;
	...
	DEBUG_OUT (10,("msrpc_lockout_policy: lockout_threshold %d\n",
				      ...
				      struct samr_DomInfo1 *password_policy)
{
	...
	DEBUG_OUT (3, ("msrpc_password_policy: fetch password policy for %s\n",
		  domain->name));

	if ( !winbindd_can_contact_domain( domain ) ) {
		DEBUG_OUT (10,("msrpc_password_policy: No incoming trust for domain %s\n",
			  domain->name));
		return NT_STATUS_NOT_SUPPORTED;
	}
	...
	*password_policy = info->info1;
	...
	DEBUG_OUT (10,("msrpc_password_policy: min_length_password %d\n",
