...
static int process_root(int local_flags)
{
	...
	if (local_flags & LOCAL_SET_LDAP_ADMIN_PW) {
		...
		if ( ! *ldap_admin_dn ) {
			DEBUG_OUT (0,("ERROR: 'ldap admin dn' not defined! Please check your smb.conf\n"));
			goto done;
		}
		...
		if (!store_ldap_admin_pw(ldap_secret)) {
			DEBUG_OUT (0,("ERROR: Failed to store the ldap admin password!\n"));
		}
		goto done;
	}
	...
	if(!initialize_password_db(False, NULL)) {
		DEBUG_OUT (0, ("Failed to open passdb!\n"));
		exit(1);
	}
