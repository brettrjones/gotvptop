			 ...
			 const char *server, struct sockaddr_storage *pss)
{
	...
	/* We return NT_STATUS_INVALID_NETWORK_RESPONSE if the server is refusing
	   to negotiate schannel, but the creds were set up ok. That'll have to do. */
		...
		if (NT_STATUS_EQUAL(ntret, NT_STATUS_INVALID_NETWORK_RESPONSE)) {
			...
			DEBUG_OUT (0,("net_rpc_join_ok: failed to get schannel session "
	...
	if (!NT_STATUS_IS_OK(ntret)) {
		DEBUG_OUT (0,("net_rpc_join_ok: failed to open schannel session "
...
int net_rpc_join_newstyle(struct net_context *c, int argc, const char **argv)
{
...
#endif
	default:
		DEBUG_OUT (0,("secure channel type %d not yet supported\n",
			sec_channel_type));
		break;
	}
	...
	if (!(mem_ctx = talloc_init("net_rpc_join_newstyle"))) {
		DEBUG_OUT (0, ("Could not initialise talloc context\n"));
		goto done;
	}
	...
	if (!domain) {
		DEBUG_OUT (0, ("Could not get domain name.\n"));
		goto done;
	}
	...
	DEBUG_OUT (10, ("Creating account with flags: %d\n",acct_flags));
	...
	status = cli_rpc_pipe_open_noauth(cli, &ndr_table_netlogon.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Error connecting to NETLOGON pipe. Error was %s\n",
			nt_errstr(status) ));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Error in domain join verification (credential setup failed): %s\n\n",
			  nt_errstr(status)));
	...
	if (lp_client_schannel() && (neg_flags & NETLOGON_NEG_SCHANNEL)) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Error in domain join verification (schannel setup failed): %s\n\n",
				  nt_errstr(status)));
	...
	if (!secrets_store_machine_password(clear_trust_password, domain, sec_channel_type)) {
		DEBUG_OUT (0, ("error storing plaintext domain secrets for %s\n", domain));
	}
