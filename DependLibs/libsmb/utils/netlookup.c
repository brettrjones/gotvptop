				    ...
				    TALLOC_CTX *ctx, NTSTATUS *perr)
{
	...
	*perr = NT_STATUS_OK;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("create_cs: Connect failed. Error was %s\n", nt_errstr(nt_status)));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("create_cs: open LSA pipe failed. Error was %s\n", nt_errstr(nt_status)));
		cs->failed_connect = true;
		cs->err = nt_status;
		*perr = nt_status;
		return NULL;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (2,("create_cs: rpccli_lsa_open_policy failed. Error was %s\n", nt_errstr(nt_status)));
		cs->failed_connect = true;
		cs->err = nt_status;
		*perr = nt_status;
		return NULL;
