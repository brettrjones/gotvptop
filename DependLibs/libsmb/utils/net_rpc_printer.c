		  ...
		  bool copy_timestamps, bool is_file)
{
	...
	if (copy_acls) {
		...
		if (!sd) {
			DEBUG_OUT (0,("failed to get security descriptor: %s\n",
	...
	if (copy_attrs || copy_timestamps) {
		...
		if (!NT_STATUS_IS_OK(cli_getattrE(cli_share_src, fnum_src, &attr, NULL,
			...
			DEBUG_OUT (0,("failed to get file-attrs: %s\n",
				cli_errstr(cli_share_src)));
			nt_status = cli_nt_error(cli_share_src);
			goto out;
		}
	...
	if (!NT_STATUS_IS_OK(cli_ntcreate(cli_share_dst, dst_name, 0, WRITE_DAC_ACCESS | WRITE_OWNER_ACCESS, 0,
		...
		DEBUG_OUT (0,("failed to open %s on the destination server: %s: %s\n",
	...
	if (copy_timestamps) {
		...
		if (!NT_STATUS_IS_OK(cli_setattrE(cli_share_dst, fnum_dst, f_ctime, f_atime, f_mtime))) {
			DEBUG_OUT (0,("failed to set file-attrs (timestamps): %s\n",
	...
	if (copy_acls) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("could not set secdesc on %s: %s\n",
	...
	if (copy_attrs) {
		...
		if (!NT_STATUS_IS_OK(cli_setatr(cli_share_dst, dst_name, attr, 0))) {
			DEBUG_OUT (0,("failed to set file-attrs: %s\n",
				cli_errstr(cli_share_dst)));
			nt_status = cli_nt_error(cli_share_dst);
			goto out;
		}
		       ...
		       bool copy_timestamps, bool is_file)
{
	...
	if (!is_file && !NT_STATUS_IS_OK(cli_chkpath(cli_share_dst, dst_name))) {
		...
		if (!NT_STATUS_IS_OK(cli_mkdir(cli_share_dst, dst_name))) {
			DEBUG_OUT (0,("cannot create directory %s: %s\n",
				dst_name, cli_errstr(cli_share_dst)));
...
static NTSTATUS check_arch_dir(struct cli_state *cli_share, const char *short_archi)
{
	...
	DEBUG_OUT (10,("creating print-driver dir for architecture: %s\n",
		short_archi));

	if (!NT_STATUS_IS_OK(cli_mkdir(cli_share, dir))) {
                DEBUG_OUT (1,("cannot create directory %s: %s\n",
					...
					struct policy_handle *hnd)
{
	...
	DEBUG_OUT (10,("connecting to: %s as %s for %s and access: %x\n",
	...
	DEBUG_OUT (2,("got printer handle for printer: %s, server: %s\n",
			     ...
			     union spoolss_DriverInfo *info)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG_OUT (1,("cannot get driver (for architecture: %s): %s\n",
						...
						const char **argv)
{
	...
	*/
	...
	DEBUG_OUT (3,("copying printer ACLs\n"));
						...
						const char **argv)
{
	...
	for (i = 0; i < num_printers; i++) {
		...
		DEBUG_OUT (1,("got %d forms for printer\n", num_forms));
						...
						const char **argv)
{
	...
	DEBUG_OUT (3,("copying printer-drivers\n"));
						...
						const char **argv)
{
	...
	for (i = 0; i < num_printers; i++) {
		...
		if (!net_spoolss_open_printer_ex(pipe_hnd_dst, mem_ctx, sharename,
			...
			DEBUG_OUT (1,("could not open printer: %s\n", sharename));
		}
		...
		if (!net_spoolss_getprinter(pipe_hnd_dst, mem_ctx, &hnd_dst, level, &info_dst)) {
			...
			DEBUG_OUT (1,("printer already exists: %s\n", sharename));
						...
						const char **argv)
{
	...
	DEBUG_OUT (3,("copying printer settings\n"));
	...
	for (i = 0; i < num_printers; i++) {
		...
		*/
		...
		if (info_enum[i].info2.attributes & PRINTER_ATTRIBUTE_PUBLISHED) {
			...
			DEBUG_OUT (3,("republished printer\n"));
		}
