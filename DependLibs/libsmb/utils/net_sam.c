...
static NTSTATUS map_unix_group(const struct group *grp, GROUP_MAP *pmap)
{
	...
	if (lookup_name(talloc_tos(), grpname, LOOKUP_NAME_LOCAL,
		...
		DEBUG_OUT (5, ("%s exists as %s\\%s, retrying as \"%s\"\n",
	...
	if (lookup_name(talloc_tos(), grpname, LOOKUP_NAME_LOCAL,
			NULL, NULL, NULL, NULL)) {
		DEBUG_OUT (3, ("\"%s\" exists, can't map it\n", grp->gr_name));
		return NT_STATUS_GROUP_EXISTS;
	}
	...
	if (pdb_capabilities() & PDB_CAP_STORE_RIDS) {
		if (!pdb_new_rid(&rid)) {
			DEBUG_OUT (3, ("Could not get a new RID for %s\n",
				  grp->gr_name));
...
static NTSTATUS unmap_unix_group(const struct group *grp, GROUP_MAP *pmap)
{
        ...
        if (!lookup_name(talloc_tos(), grpname, LOOKUP_NAME_LOCAL,
                        NULL, NULL, NULL, NULL)) {
                DEBUG_OUT (3, ("\"%s\" does not exist, can't unmap it\n", grp->gr_name));
