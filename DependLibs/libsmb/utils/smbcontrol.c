			 ...
			 const void *buf, int len)
{
	...
	DEBUG_OUT (10,("smbcontrol/send_message: broadcast message to "
		  "%d processes\n", n_sent));
			 ...
			 bool multiple_replies)
{
	...
	if (!(te = tevent_add_timer(messaging_event_context(msg_ctx), NULL,
		...
		DEBUG_OUT (0, ("tevent_add_timer failed\n"));
		return;
	}
			     ...
			     const int argc, const char **argv)
{
	...
	for (retry = 0; retry < 5; retry++) {
		...
		if (!d.dptr || d.dsize != 4) {
			SAFE_FREE(d.dptr);
			DEBUG_OUT (10,("do_winbind_offline: offline state not set - retrying.\n"));
		} else {
			SAFE_FREE(d.dptr);
			break;
		}
