			...
			const char **argv)
{
	...
	if (!cli_arg) {
		...
		if (!NT_STATUS_IS_OK(nt_status)) {
			DEBUG_OUT (1, ("failed to make ipc connection: %s\n",
				  nt_errstr(nt_status)));
			return -1;
		}
	} else {
	...
	if (!(conn_flags & NET_FLAGS_NO_PIPE)) {
		if (lp_client_schannel()
			...
			nt_status = cli_rpc_pipe_open_schannel(
				...
				&pipe_hnd);
			if (!NT_STATUS_IS_OK(nt_status)) {
				DEBUG_OUT (0, ("Could not initialise schannel netlogon pipe. Error was %s\n",
					nt_errstr(nt_status) ));
				goto fail;
			}
		} else {
			...
			if (!NT_STATUS_IS_OK(nt_status)) {
				DEBUG_OUT (0, ("Could not initialise pipe %s. Error was %s\n",
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (1, ("rpc command function failed! (%s)\n", nt_errstr(nt_status)));
	} else {
		ret = 0;
		DEBUG_OUT (5, ("rpc command function succedded\n"));
	}
					...
					const char **argv)
{
	...
	result = cli_rpc_pipe_open_noauth(cli, &ndr_table_netlogon.syntax_id,
					  &pipe_hnd);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("rpc_oldjoin_internals: netlogon pipe open to machine %s failed. "
			"error was %s\n",
			cli->desthost,
			nt_errstr(result) ));
		return result;
	...
	*/
	...
	if (!secrets_store_domain_sid(c->opt_target_workgroup, domain_sid)) {
		DEBUG_OUT (0, ("error storing domain sid for %s\n", c->opt_target_workgroup));
			...
			const char **argv)
{
	...
	if (!secrets_store_domain_sid(domain_name, domain_sid)) {
		DEBUG_OUT (0,("Can't store domain SID\n"));
		    ...
		    const char *mask, void *state)
{
	...
	DEBUG_OUT (3,("got mask: %s, name: %s\n", mask, f->name));

	/* DIRECTORY */
	if (f->mode & FILE_ATTRIBUTE_DIRECTORY) {

		DEBUG_OUT (3,("got dir: %s\n", f->name));
...
static NTSTATUS sync_files(struct copy_clistate *cp_clistate, const char *mask)
{
	...
	DEBUG_OUT (3,("calling cli_list with mask: %s\n", mask));
				...
				const char *sharename)
{
	...
	switch (net_mode_share) {
	case NET_MODE_SHARE_MIGRATE:
		DEBUG_OUT (3,("calling net_copy_fileattr for '.' directory in share %s\n", sharename));
				...
				const char **argv)
{
	...
	for (i=0; i<num_server_aliases; i++) {
		...
		DEBUG_OUT (1, ("%s\\%s %d: ", domains[0], names[0], types[0]));
...
static bool get_user_sids(const char *domain, const char *user, struct security_token *token)
{
	...
	if (!WBC_ERROR_IS_OK(wbc_status)) {
		DEBUG_OUT (1, ("winbind could not find %s: %s\n",
	...
	if (type != WBC_SID_NAME_USER) {
		DEBUG_OUT (1, ("%s is not a user\n", full_name));
		return false;
	}

	if (!string_to_sid(&user_sid, sid_str)) {
		DEBUG_OUT (1,("Could not convert sid %s from string\n", sid_str));
		return false;
	}
	...
	if (!WBC_ERROR_IS_OK(wbc_status)) {
		DEBUG_OUT (1, ("winbind could not get groups of %s: %s\n",
			full_name, wbcErrorString(wbc_status)));
		return false;
	}
	...
	for (i = 0; i < num_groups; i++) {
		...
		if (!WBC_ERROR_IS_OK(wbc_status)) {
			DEBUG_OUT (1, ("winbind could not find SID of gid %u: %s\n",
			    ...
			    struct user_token **user_tokens)
{
	...
	if (!WBC_ERROR_IS_OK(wbc_status)) {
		DEBUG_OUT (1, (_("winbind could not list users: %s\n"),
	...
	if (result == NULL) {
		DEBUG_OUT (1, ("Could not malloc sid array\n"));
		wbcFreeMemory(users);
		return false;
	}
	...
	while (!feof(f)) {
		...
		if (line[0] == ' ') {
			...
			if(!string_to_sid(&sid, &line[1])) {
				DEBUG_OUT (1,("get_user_tokens_from_file: Could "
					"not convert sid %s \n",&line[1]));
				return false;
			}

			if (token == NULL) {
				DEBUG_OUT (0, ("File does not begin with username"));
				return false;
			}
		...
		*tokens = SMB_REALLOC_ARRAY(*tokens, struct user_token, *num_tokens);
		if (*tokens == NULL) {
			DEBUG_OUT (0, ("Could not realloc tokens\n"));
			return false;
		}
			...
			struct user_token *tokens)
{
	...
	if (!NT_STATUS_IS_OK(status) || !W_ERROR_IS_OK(result)) {
		DEBUG_OUT (1, ("Coult not query secdesc for share %s\n",
			  netname));
		return;
	}
	...
	if (share_sd == NULL) {
		DEBUG_OUT (1, ("Got no secdesc for share %s\n",
			  netname));
	}
	...
	for (i=0; i<num_tokens; i++) {
		...
		if (share_sd != NULL) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (1, ("Could not check share_sd for "
					  "user %s\n",
					  tokens[i].name));
				continue;
			}
		...
		status = se_access_check(root_sd, &tokens[i].token,
				     1, &acc_granted);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("Could not check root_sd for user %s\n",
				  tokens[i].name));
			continue;
		}
		d_printf(" %s\n", tokens[i].name);
						...
						const char **argv)
{
	...
	if (f == NULL) {
		DEBUG_OUT (0, ("Could not open userlist: %s\n", strerror(errno)));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (!r) {
		DEBUG_OUT (0, ("Could not read users from file\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
int net_usersidlist(struct net_context *c, int argc, const char **argv)
{
	...
	if (!get_user_tokens(c, &num_tokens, &tokens)) {
		DEBUG_OUT (0, ("Could not get the user/sid list\n"));
		return 0;
	}
					...
					const char **argv)
{
	...
	if (W_ERROR_IS_OK(result)) {
		...
		DEBUG_OUT (5,("cmd_shutdown_abort: query succeeded\n"));
	} else
		DEBUG_OUT (5,("cmd_shutdown_abort: query failed\n"));
						...
						const char **argv)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (5,("cmd_reg_abort_shutdown: query failed\n"));
		return result;
	}
	if (W_ERROR_IS_OK(werr)) {
		d_printf(_("\nShutdown successfully aborted\n"));
		DEBUG_OUT (5,("cmd_reg_abort_shutdown: query succeeded\n"));
	} else
		DEBUG_OUT (5,("cmd_reg_abort_shutdown: query failed\n"));
			      ...
			      const char **argv)
{
	...
	DEBUG_OUT (1, ("initshutdown pipe didn't work, trying winreg pipe\n"));
				     ...
				     const char **argv)
{
	...
	if (W_ERROR_IS_OK(result)) {
		...
		DEBUG_OUT (5,("Shutdown of remote machine succeeded\n"));
	} else {
		DEBUG_OUT (1,("Shutdown of remote machine failed!\n"));
	}
	return werror_to_ntstatus(result);
}
...
static int rpc_shutdown(struct net_context *c, int argc, const char **argv)
{
	...
	if (rc) {
		DEBUG_OUT (1, ("initshutdown pipe failed, trying winreg pipe\n"));
						...
						const char **argv)
{
...

	{
		...
		if (!NT_STATUS_IS_OK(result)) {
			status = result;
			DEBUG_OUT (0,("Could not set trust account password: %s\n",
				 nt_errstr(result)));
			goto done;
		}
	}
				     ...
				     const char *domain_name)
{
	...
	DEBUG_OUT (1,("NetServerEnum2 error: Couldn't find primary domain controller\
		 for domain %s\n", domain_name));
	...
	DEBUG_OUT (1,("netr_GetDcName error: Couldn't find primary domain controller\
		 for domain %s\n", domain_name));
				  ...
				  const char **argv)
{
	...
	if (!net_find_pdc(&server_ss, pdc_name, domain_name)) {
		DEBUG_OUT (0, ("Couldn't find domain controller for domain %s\n", domain_name));
		return -1;
	}
	...
	if (!NT_STATUS_EQUAL(nt_status, NT_STATUS_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT)) {
		...
		DEBUG_OUT (0, ("Couldn't verify trusting domain account. Error was %s\n",
			nt_errstr(nt_status)));
		return -1;
	}
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0, ("Couldn't connect to domain %s controller. Error was %s.\n",
	...
	nt_status = cli_rpc_pipe_open_noauth(cli, &ndr_table_lsarpc.syntax_id,
					     &pipe_hnd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Could not initialise lsa pipe. Error was %s\n", nt_errstr(nt_status) ));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	}
	...
	nt_status = rpccli_lsa_open_policy2(pipe_hnd, mem_ctx, true, KEY_QUERY_VALUE,
	                                 &connect_hnd);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0, ("Couldn't open policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	nt_status = dcerpc_lsa_QueryInfoPolicy(b, mem_ctx,
					       ...
					       &result);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0, ("LSA Query Info failed. Returned error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	if (!pdb_set_trusteddom_pw(domain_name, c->opt_password, domain_sid)) {
		DEBUG_OUT (0, ("Storing password for trusted domain failed.\n"));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	}
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0, ("Couldn't close LSA pipe. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
			       ...
			       const char **argv)
{
	...
	if (!pdb_del_trusteddom_pw(domain_name)) {
		DEBUG_OUT (0, ("Failed to revoke relationship to the trusted domain %s\n",
			  domain_name));
		goto done;
	};
				      ...
				      const char *trusted_dom_name)
{
	...
	nt_status = dcerpc_lsa_QueryTrustedDomainInfoBySid(b, mem_ctx,
							   ...
							   &result);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0,("Could not query trusted domain info. Error was %s\n",
		nt_errstr(nt_status)));
		goto done;
	}
	if (NT_STATUS_IS_ERR(result)) {
	...
	if (!rpccli_get_pwd_hash(pipe_hnd, session_key)) {
		DEBUG_OUT (0, ("Could not retrieve password hash\n"));
		goto done;
	}
	...
	if (cleartextpwd == NULL) {
		DEBUG_OUT (0,("retrieved NULL password\n"));
		nt_status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	DEBUG_OUT (100,("successfully vampired trusted domain [%s], sid: [%s], "
		   "password: [%s]\n", trusted_dom_name,
				...
				const char **argv)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Couldn't connect to domain controller: %s\n",
			  nt_errstr(nt_status)));
		talloc_destroy(mem_ctx);
		return -1;
	};
	...
	do {
		nt_status = dcerpc_lsa_EnumTrustDom(b, mem_ctx,
						    ...
						    &result);
		if (NT_STATUS_IS_ERR(nt_status)) {
			DEBUG_OUT (0, ("Couldn't enumerate trusted domains. Error was %s\n",
				nt_errstr(nt_status)));
			cli_shutdown(cli);
			talloc_destroy(mem_ctx);
			return -1;
	...
	nt_status = dcerpc_lsa_Close(b, mem_ctx, &connect_hnd, &result);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG_OUT (0, ("Couldn't properly close lsa policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
...
static int rpc_trustdom_list(struct net_context *c, int argc, const char **argv)
{
	...
	nt_status = cli_rpc_pipe_open_noauth(cli, &ndr_table_samr.syntax_id,
					     &pipe_hnd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Could not initialise samr pipe. Error was %s\n", nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	};
	...
	nt_status = dcerpc_samr_Connect2(b, mem_ctx,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Couldn't open SAMR policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	nt_status = dcerpc_samr_OpenDomain(b, mem_ctx,
					   ...
					   &result);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Couldn't open domain object. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	do {
		...
		nt_status = dcerpc_samr_EnumDomainUsers(b, mem_ctx,
							...
							&result);
		if (NT_STATUS_IS_ERR(nt_status)) {
			DEBUG_OUT (0, ("Couldn't enumerate accounts. Error was: %s\n",
				nt_errstr(nt_status)));
			cli_shutdown(cli);
			talloc_destroy(mem_ctx);
			return -1;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Couldn't properly close domain policy handle for domain %s\n", domain_name));
	};

	nt_status = dcerpc_samr_Close(b, mem_ctx, &connect_hnd, &result);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("Couldn't properly close samr policy handle for domain %s\n", domain_name));
	};
