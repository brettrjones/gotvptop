...
static NTSTATUS net_ads_join_ok(struct net_context *c)
{
	...
	if (!secrets_init()) {
		DEBUG_OUT (1,("Failed to initialise secrets database\n"));
					...
					int num_addrs)
{
	...
	if ( !NT_STATUS_IS_OK(status) || (ns_count == 0)) {
		...
		if ( !ads->ldap.ld ) {
			...
			if ( !ADS_ERR_OK(ads_status) ) {
				DEBUG_OUT (0,("net_update_dns_internal: Failed to connect to our DC!\n"));
				goto done;
			}
		}
		...
		if ( !NT_STATUS_IS_OK(status) || (ns_count == 0)) {
			DEBUG_OUT (3,("net_update_dns_internal: Failed to find name server for the %s "
			 "realm\n", ads->config.realm));
			goto done;
		}
	...
	for (i=0; i < ns_count; i++) {
		...
		if (ERR_DNS_EQUAL(dns_err, ERROR_DNS_INVALID_NAME_SERVER) ||
			...
			DEBUG_OUT (1,("retrying DNS update with next nameserver after receiving %s\n",
				dns_errstr(dns_err)));
			continue;
		}
				   ...
				   int num_addrs)
{
	...
	if (num_addrs == 0 || iplist == NULL) {
		...
		if ( num_addrs <= 0 ) {
			DEBUG_OUT (4, ("net_update_dns_ext: Failed to find my "
...
static int net_ads_dns_register(struct net_context *c, int argc, const char **argv)
{
	...
	status = ads_startup(c, true, &ads);
	if ( !ADS_ERR_OK(status) ) {
		DEBUG_OUT (1, ("error on ads_startup: %s\n", ads_errstr(status)));
		TALLOC_FREE(ctx);
		return -1;
	}
