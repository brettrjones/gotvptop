                            ...
                            const char *username)
{
	...
	DEBUG_OUT (3, ("export_database: username=\"%s\"\n", username ? username : "(NULL)"));

	u_search = pdb_search_init(talloc_tos(), PDB_USER_SEARCH);
	if (u_search == NULL) {
		DEBUG_OUT (0, ("pdb_search_init failed\n"));
		return 1;
	}

	if (!in->search_users(in, u_search, 0)) {
		DEBUG_OUT (0, ("Could not start searching users\n"));
	...
	while (u_search->next_entry(u_search, &userentry)) {
		...
		DEBUG_OUT (4, ("Processing account %s\n", userentry.account_name));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("getsampwsid failed: %s\n",
				  nt_errstr(status)));
			TALLOC_FREE(user);
			continue;
		}
...
static int print_users_list(bool verbosity, bool smbpwdstyle)
{
	...
	if (!u_search) {
		DEBUG_OUT (0, ("User Search failed!\n"));
		ret = 1;
		goto done;
	}
	...
	while (u_search->next_entry(u_search, &userentry)) {
		...
		if (!bret) {
			DEBUG_OUT (2, ("getsampwsid failed\n"));
			TALLOC_FREE(sam_pwent);
			continue;
		}
			 ...
			 const char *kickoff_time)
{
	...
	if (!pdb_update_autolock_flag(sam_pwent, &updated_autolock)) {
		DEBUG_OUT (2,("pdb_update_autolock_flag failed.\n"));
	}

	if (!pdb_update_bad_password_count(sam_pwent, &updated_badpw)) {
		DEBUG_OUT (2,("pdb_update_bad_password_count failed.\n"));
	}
