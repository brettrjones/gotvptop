				...
				const char *server_name)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG_OUT (1,("Cannot connect to server (anonymously).  Error was %s\n", nt_errstr(nt_status)));
		return nt_status;
	}
}
			...
			const char *server_name)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (1,("Cannot connect to server using kerberos.  Error was %s\n", nt_errstr(nt_status)));
		return nt_status;
	}
			  ...
			  const struct ndr_syntax_id *interface)
{
	...
	nt_status = cli_rpc_pipe_open_noauth(cli_tmp, interface,
					     &pipe_hnd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0, ("couldn't not initialize pipe\n"));
			...
			char **server_name)
{
	...
	if (c->opt_have_ip) {
		*server_ss = c->opt_dest_ip;
		...
		if (!resolve_name(*server_name, server_ss, 0x20, false))  {
			DEBUG_OUT (1,("Unable to resolve server name\n"));
			return false;
		}
		...
		if (!get_pdc_ip(d, &pdc_ss)) {
			DEBUG_OUT (1,("Unable to resolve PDC server address\n"));
			return false;
		}
		...
		*server_ss = pdc_ss;
		...
		if (!resolve_name(d, &msbrow_ss, 0x1B, false))  {
			DEBUG_OUT (1,("Unable to resolve domain browser via name lookup\n"));
			return false;
		}
		*server_ss = msbrow_ss;
		...
		*server_name = SMB_STRDUP(addr);
		...
		if (!resolve_name(d, &brow_ss, 0x1D, false))  {
			...
			DEBUG_OUT (1,("Unable to resolve master browser via name lookup\n"));
			return false;
		}
		*server_ss = brow_ss;
		print_sockaddr(addr, sizeof(addr), server_ss);
		*server_name = SMB_STRDUP(addr);
		...
		if (!interpret_string_addr(server_ss,
			...
			DEBUG_OUT (1,("Unable to resolve 127.0.0.1\n"));
			return false;
		}
	...
	if (!*server_name) {
		DEBUG_OUT (1,("no server to connect to\n"));
		return false;
	}
		     ...
		     struct net_dc_info *dc_info)
{
	...
	status = cli_rpc_pipe_open_noauth(cli, &ndr_table_dssetup.syntax_id,
					  &dssetup_pipe);
        if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10,("net_scan_dc: failed to open dssetup pipe with %s, "
