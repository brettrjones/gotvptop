		      ...
		      uint32_t flags)
{
	...
	DEBUG_OUT (10,("DoDNSUpdate called with flags: 0x%08x\n", flags));
	...
	if (flags & DNS_UPDATE_PROBE) {
		...
		if (!ERR_DNS_IS_OK(err)) {
			DEBUG_OUT (3,("DoDNSUpdate: failed to probe DNS\n"));
		}
	...
	if (flags & DNS_UPDATE_UNSIGNED) {
		...
		if (!ERR_DNS_IS_OK(err)) {
			DEBUG_OUT (3,("DoDNSUpdate: unsigned update failed\n"));
		}
	...
	if (flags & DNS_UPDATE_SIGNED) {
		...
		if (!ERR_DNS_IS_OK(err)) {
			DEBUG_OUT (3,("DoDNSUpdate: signed update failed\n"));
		}
	}
