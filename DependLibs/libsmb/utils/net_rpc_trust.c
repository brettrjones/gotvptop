			     ...
			     struct policy_handle *pol_hnd)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_lsa_Close failed with error [%s].\n",
			  nt_errstr(status)));
		return status;
	}
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0, ("lsa close failed with error [%s].\n",
			  nt_errstr(result)));
		return result;
	}
			     ...
			     struct dom_sid *domsid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_lsa_DeleteTrustedDomain_r failed with [%s]\n",
			  nt_errstr(status)));
		return status;
	}
	if (!NT_STATUS_IS_OK(dr.out.result)) {
		DEBUG_OUT (0, ("DeleteTrustedDomain returned [%s]\n",
			     ...
			     struct lsa_TrustDomainInfoAuthInfoInternal *authinfo)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_lsa_CreateTrustedDomainEx2_r failed "
			  "with error [%s].\n", nt_errstr(status)));
		return status;
	}
	if (!NT_STATUS_IS_OK(r.out.result)) {
		DEBUG_OUT (0, ("CreateTrustedDomainEx2_r returned [%s].\n",
				...
				struct dom_data *dom_data)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("dcerpc_lsa_QueryInfoPolicy2_r failed "
			  "with error [%s].\n", nt_errstr(status)));
		return status;
	}

	if (!NT_STATUS_IS_OK(qr.out.result)) {
		DEBUG_OUT (0, ("QueryInfoPolicy2 returned [%s].\n",
	...
	if (dom_data->domain_name == NULL ||
		...
		DEBUG_OUT (0, ("Copying domain data failed.\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (0, ("Got the following domain info [%s][%s][%s].\n",
				     ...
				     struct dom_data *dom_data)
{
	...
	status = net_make_ipc_connection_ex(net_ctx, NULL, NULL, NULL,
					    NET_FLAGS_PDC, cli);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to connect to [%s] with error [%s]\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to initialise lsa pipe with error [%s]\n",
			  nt_errstr(status)));
		return status;
	}
	...
	status = dcerpc_lsa_open_policy2((*pipe_hnd)->binding_handle,
					 ...
					 &result);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed to open policy handle with error [%s]\n",
			  nt_errstr(status)));
		return status;
	}
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0, ("lsa_open_policy2 with error [%s]\n",
			  nt_errstr(result)));
		return result;
	}
	...
	status = get_domain_info(mem_ctx, (*pipe_hnd)->binding_handle,
				 pol_hnd, dom_data);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("get_domain_info failed with error [%s].\n",
			  nt_errstr(status)));
		return status;
	}
...
static int parse_trust_args(TALLOC_CTX *mem_ctx, int argc, const char **argv, struct other_dom_data **_o, char **_trustpw)
{
	...
	for (c = 0; c < argc; c++) {
		if (strnequal(argv[c], ARG_OTHERSERVER, sizeof(ARG_OTHERSERVER)-1)) {
			...
			DEBUG_OUT (0, ("Unsupported option [%s].\n", argv[c]));
			ret = EINVAL;
			goto failed;
		}
	}
			    ...
			    const char **argv, enum trust_op op)
{
	...
	switch (op) {
		...
		case TRUST_DELETE:
			...
			break;
		default:
			DEBUG_OUT (0, ("Unsupported trust operation.\n"));
			return -1;
	}
	...
	if (ret != 0) {
		if (ret == EINVAL) {
			...
			DEBUG_OUT (0, ("Failed to parse arguments.\n"));
		}
		goto done;
	}
	...
	if (other_dom_data->host != 0) {
		...
		if (dom_data[1].domsid == NULL ||
			...
			DEBUG_OUT (0, ("Missing required argument.\n"));
			usage();
			goto done;
		}
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("connect_and_get_info failed with error [%s]\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	if (op == TRUST_CREATE) {
		if (trust_pw == NULL) {
			if (other_net_ctx == NULL) {
				DEBUG_OUT (0, ("Missing either trustpw or otherhost.\n"));
				goto done;
			}

			DEBUG_OUT (0, ("Using random trust password.\n"));
	 ...
	 * random binary password? */
			...
			if (trust_pw == NULL) {
				DEBUG_OUT (0, ("generate_random_str failed.\n"));
				goto done;
			}
		} else {
			DEBUG_OUT (0, ("Using user provided password.\n"));
		}

		if (!get_trust_domain_passwords_auth_blob(mem_ctx, trust_pw,
							  &auth_blob)) {
			DEBUG_OUT (0, ("get_trust_domain_passwords_auth_blob failed\n"));
			goto done;
		}
		...
		status = create_trust(mem_ctx, pipe_hnd[0]->binding_handle,
				      ...
				      &authinfo);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("create_trust failed with error [%s].\n",
			nt_errstr(status)));
			goto done;
		}
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("delete_trust failed with [%s].\n",
				  nt_errstr(status)));
			goto done;
		}
	...
	status = close_handle(mem_ctx, pipe_hnd[0]->binding_handle,
			      &pol_hnd[0]);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("close_handle failed with error [%s].\n",
			  nt_errstr(status)));
		goto done;
	}
