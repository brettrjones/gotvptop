...
static bool get_workgroups(struct user_auth_info *user_info)
{
	...
	if (!master_workgroup) {
        ...
        if (!use_bcast && !find_master_ip(lp_workgroup(), &server_ss)) {
                DEBUG_OUT (4, ("Unable to find master browser for workgroup %s, falling back to broadcast\n", 
			  master_workgroup));
				use_bcast = True;
		} else if(!use_bcast) {
		...
		if (!(cli = get_ipc_connect_master_ip_bcast(talloc_tos(),
							user_info,
							&master_workgroup))) {
			DEBUG_OUT (4, ("Unable to find master browser by "
				  "broadcast\n"));
			return False;
        }
...
static bool get_servers(char *workgroup, struct user_auth_info *user_info)
{
	...
	char addr[INET6_ADDRSTRLEN];
        ...
        if (!find_master_ip(workgroup, &server_ss)) {
                DEBUG_OUT (4, ("Cannot find master browser for workgroup %s\n",
                          workgroup));
                return False;
        }
