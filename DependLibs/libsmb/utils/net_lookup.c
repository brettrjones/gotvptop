...
static int net_lookup_host(struct net_context *c, int argc, const char **argv)
{
	...
	if (!resolve_name(name, &ss, name_type, false)) {
		/* we deliberately use DEBUG_OUT () here to send it to stderr
		   so scripts aren't mucked up */
		DEBUG_OUT (0,("Didn't find %s#%02x\n", name, name_type));
		return -1;
	}
...
static int net_lookup_ldap(struct net_context *c, int argc, const char **argv)
{
	...
	DEBUG_OUT (9, ("Lookup up ldap for domain %s\n", domain));
	...
	if ( NT_STATUS_IS_OK(status) && numdcs ) {
		...
		return 0;
     	...
     	DEBUG_OUT (9, ("Looking up PDC for domain %s\n", domain));
	...
	DEBUG_OUT (9, ("Found PDC with DNS name %s\n", h_name));
	...
	DEBUG_OUT (9, ("Looking up ldap for domain %s\n", domain));
...
static int net_lookup_kdc(struct net_context *c, int argc, const char **argv)
{
	...
	if (argc > 0) {
		...
		if (rc) {
			DEBUG_OUT (1,("krb5_gethost_realm failed (%s)\n",
				 error_message(rc)));
			return -1;
		}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1,("get_kdc_list failed (%s)\n", nt_errstr(status)));
		return -1;
	}
	...
	return 0;
#endif
	DEBUG_OUT (1, ("No kerberos support\n"));
	return -1;
}
