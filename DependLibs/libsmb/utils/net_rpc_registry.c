			 ...
			 uint32 *reg_type, const char **key_name)
{
	...
	*key_name = talloc_strdup(ctx, tmp_keyname);
		   ...
		   strequal(hivename, "HKEY_PERFORMANCE_DATA"))
	{
		...
		DEBUG_OUT (10,("reg_hive_key: unrecognised hive key %s\n",
			  fullname));
		goto done;
	}
			         ...
			         const char *parentpath )
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0, ("write_registry_tree: regsubkey_ctr_init failed: "
			  "%s\n", win_errstr(werr)));
		return false;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG_OUT (0,("write_registry_tree: talloc() failed!\n"));
		TALLOC_FREE(subkeys);
		return false;
	}
