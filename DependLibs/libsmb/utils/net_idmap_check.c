...
static int add_record(struct check_ctx* ctx, TDB_DATA key, TDB_DATA value)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("could not store record %s\n", nt_errstr(status)));
		return -1;
	}
	ctx->n_diff ++;
	return 0;
...
char* interact_edit(TALLOC_CTX* mem_ctx, const char* str) {
	...
	if (fd == -1) {
		DEBUG_OUT (0, ("failed to mkstemp %s: %s\n", fname,
			  strerror(errno)));
		return NULL;
	}
	...
	file  = fdopen(fd, "w");
	if (!file) {
		DEBUG_OUT (0, ("failed to open %s for writing: %s\n", fname,
			  strerror(errno)));
		close(fd);
		unlink(fname);
		return NULL;
	...
	if (system(buf) != 0) {
		DEBUG_OUT (0, ("failed to start editor %s: %s\n", buf,
			  strerror(errno)));
		unlink(fname);
		return NULL;
	}
	...
	file = fopen(fname, "r");
	if (!file) {
		DEBUG_OUT (0, ("failed to open %s for reading: %s\n", fname,
			  strerror(errno)));
		unlink(fname);
		return NULL;
	}
...
static bool check_do_checks(struct check_ctx* ctx)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("failed to traverse %s\n", ctx->name));
		return false;
	}
...
static void check_diff_list(struct check_ctx* ctx) {
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("failed to traverse diff\n"));
	}
...
int net_idmap_check_db(const char* db, const struct check_options* o)
{
	...
	if (!o->automatic && !isatty(STDIN_FILENO)) {
		DEBUG_OUT (0, ("Interactive use needs tty, use --auto\n"));
		goto done;
	}
	if (o->lock) {
