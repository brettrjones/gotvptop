				...
				uint32_t msg_flags, void *private_data)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		char *str = procid_str(talloc_tos(), id);
		DEBUG_OUT (1, ("Could not delete serverid.tdb record %s: %s\n",
	...
	void *private_data)
{
	if (!serverid_exists(&key->pid)) {
		...
		DEBUG_OUT (10, ("Deleting connections.tdb record for pid %s\n",
			   procid_str(talloc_tos(), &key->pid)));

		status = rec->delete_rec(rec);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("Could not delete connections.tdb record "
				  "for pid %s: %s\n",
				  procid_str(talloc_tos(), &key->pid),
				  nt_errstr(status)));
		}
					  ...
					  void *private_data)
{
	if (!serverid_exists(&session->pid)) {
		...
		DEBUG_OUT (10, ("Deleting sessionid.tdb record for pid %s\n",
			   procid_str(talloc_tos(), &session->pid)));

		status = rec->delete_rec(rec);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("Could not delete session.tdb record "
				  "for pid %s: %s\n",
				  procid_str(talloc_tos(), &session->pid),
				  nt_errstr(status)));
		}
