...
const char *get_winbind_domain(void)
{
	...
	if (winbindd_request_response(WINBINDD_DOMAIN_NAME, NULL, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG_OUT (0, ("could not obtain winbind domain name!\n"));
		return lp_workgroup();
	}
...
const char *get_winbind_netbios_name(void)
{
	...
	if (winbindd_request_response(WINBINDD_NETBIOS_NAME, NULL, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG_OUT (0, ("could not obtain winbind netbios name!\n"));
...
static bool get_require_membership_sid(void) {
	...
	if (!parse_ntlm_auth_domain_user(require_membership_of, 
		...
		DEBUG_OUT (0, ("Could not parse %s into seperate domain/name parts!\n", 
	...
	if (winbindd_request_response(WINBINDD_LOOKUPNAME, &request, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG_OUT (0, ("Winbindd lookupname failed to resolve %s into a SID!\n", 
			  require_membership_of));
		return False;
	}
				 ...
				 bool stdout_diagnostics)
{
	...
	if (stdout_diagnostics) {
		...
		if ((result != NSS_STATUS_SUCCESS) && (response.data.auth.nt_status == 0)) {
			DEBUG_OUT (1, ("Reading winbind reply failed! (0x01)\n"));
		}

		DEBUG_OUT (3, ("%s: %s (0x%x)\n",
			  response.data.auth.nt_status_string,
			  response.data.auth.error_string,
			  response.data.auth.nt_status));
	}
				 ...
				 DATA_BLOB *user_session_key, DATA_BLOB *lm_session_key)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG_OUT (NT_STATUS_EQUAL(nt_status, NT_STATUS_ACCESS_DENIED) ? 0 : 3, 
			       ...
			       DATA_BLOB *user_session_key, DATA_BLOB *lm_session_key)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG_OUT (3, ("Login for user [%s]\\[%s]@[%s] failed due to [%s]\n", 
			  ntlmssp_state->domain, ntlmssp_state->user,
			  ntlmssp_state->client.netbios_name,
			  nt_errstr(nt_status)));
		ntlmssp_state->callback_private = NULL;
...
static NTSTATUS ntlm_auth_start_ntlmssp_client(struct ntlmssp_state **client_ntlmssp_state)
{
	...
	if ( (opt_username == NULL) || (opt_domain == NULL) ) {
		...
		DEBUG_OUT (1, ("Need username and domain for NTLMSSP\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Could not start NTLMSSP client: %s\n",
			  nt_errstr(status)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Could not set username: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(*client_ntlmssp_state);
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Could not set domain: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(*client_ntlmssp_state);
		return status;
	}
	...
	if (opt_password) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("Could not set password: %s\n",
				  nt_errstr(status)));
			TALLOC_FREE(*client_ntlmssp_state);
			return status;
		}
...
static NTSTATUS ntlm_auth_start_ntlmssp_server(struct ntlmssp_state **ntlmssp_state)
{
	...
	status = ntlmssp_server_start(NULL,
				      ...
				      ntlmssp_state);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Could not start NTLMSSP server: %s\n",
			  nt_errstr(status)));
		return status;
	}
					     ...
					     char **response)
{
	...
	if (strlen(buf) < 2) {
		DEBUG_OUT (1, ("NTLMSSP query [%s] invalid\n", buf));
	...
	if (strlen(buf) > 3) {
		if(strncmp(buf, "SF ", 3) == 0){
			DEBUG_OUT (10, ("Setting flags to negotioate\n"));
	...
	if (strncmp(buf, "YR", 2) == 0) {
		...
		DEBUG_OUT (10, ("Requested negotiated NTLMSSP flags\n"));
		...
		DEBUG_OUT (10, ("Requested NTLMSSP session key\n"));
	...
	DEBUG_OUT (10, ("got NTLMSSP packet:\n"));
					 	...
					 	char *buf, int length)
{
	...
	if (strlen(buf) > 3) {
		if(strncmp(buf, "SF ", 3) == 0) {
			DEBUG_OUT (10, ("Looking for flags to negotiate\n"));
			talloc_free(state->want_feature_list);
			state->want_feature_list = talloc_strdup(state->mem_ctx,
					buf+3);
	...
	if (opt_password == NULL && !use_cached_creds) {
		...
		DEBUG_OUT (10, ("Requesting password\n"));
	...
	if (strncmp(buf, "YR", 2) == 0) {
		...
		DEBUG_OUT (10, ("Requested session key\n"));
					...
					char *buf, int length)
{
	...
	if (!pass) {
		DEBUG_OUT (2, ("Password not found. Denying access\n"));
...
static void offer_gss_spnego_mechs(void) {
	...
	if (len == -1) {
		DEBUG_OUT (1, ("Could not write SPNEGO data blob\n"));
	...
	DEBUG_OUT (10, ("sent SPNEGO negTokenInit\n"));
	return;
}
					...
					char *buf, int length)
{
	...
	if (strlen(buf) < 2) {
		DEBUG_OUT (1, ("SPENGO query [%s] invalid\n", buf));
	...
	if (strlen(buf) <= 3) {
		DEBUG_OUT (1, ("GSS-SPNEGO query [%s] invalid\n", buf));
	...
	if ((token.length >= 7)
		...
		DEBUG_OUT (10, ("Could not parse GSS-SPNEGO, trying raw "
			   "ntlmssp\n"));
	...
	if (request.type == SPNEGO_NEG_TOKEN_INIT) {
		...
		if (state->spnego_mech) {
			DEBUG_OUT (1, ("Client restarted SPNEGO with NegTokenInit "
		...
		if ( (request.negTokenInit.mechTypes == NULL) ||
			...
			DEBUG_OUT (1, ("Client did not offer any mechanism\n"));
		...
		if (ntlm_idx > -1) {
			...
			if (state->ntlmssp_state) {
				DEBUG_OUT (1, ("Client wants a new NTLMSSP challenge, but "
					  "already got one\n"));
		...
		if (state->spnego_mech == NULL) {
			DEBUG_OUT (1,("Got netTokenTarg without negTokenInit\n"));
		...
		if ((request.negTokenTarg.supportedMech != NULL) &&
			...
			DEBUG_OUT (1, ("Got a negTokenTarg with mech[%s] while [%s] was already negotiated\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		if (!spnego_parse_krb5_wrap(ctx, raw_in_token,
					    &ticket, tok_id)) {
			DEBUG_OUT (1, ("spnego_parse_krb5_wrap failed\n"));
		...
		if (NT_STATUS_IS_OK(status)) {
			...
			if (domain == NULL) {
				DEBUG_OUT (1, ("Did not get a valid principal "
...
static bool manage_client_ntlmssp_init(struct spnego_data spnego)
{
	...
	DEBUG_OUT (10, ("Got spnego negTokenInit with NTLMSSP\n"));

	if (client_ntlmssp_state != NULL) {
		DEBUG_OUT (1, ("Request for initial SPNEGO request where "
	...
	if ( !(NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED) ||
		...
		DEBUG_OUT (1, ("Expected OK or MORE_PROCESSING_REQUIRED, got: %s\n",
			  nt_errstr(status)));
		TALLOC_FREE(client_ntlmssp_state);
		return False;
	}
...
static void manage_client_ntlmssp_targ(struct spnego_data spnego)
{
	...
	DEBUG_OUT (10, ("Got spnego negTokenTarg with NTLMSSP\n"));

	if (client_ntlmssp_state == NULL) {
		DEBUG_OUT (1, ("Got NTLMSSP tArg without a client state\n"));
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG_OUT (1, ("Expected MORE_PROCESSING_REQUIRED from "
...
static bool manage_client_krb5_init(struct spnego_data spnego)
{
	...
	if ( (spnego.negTokenInit.mechListMIC.data == NULL) ||
		...
		DEBUG_OUT (1, ("Did not get a principal for krb5\n"));
		return False;
	}
	...
	if (principal == NULL) {
		DEBUG_OUT (1, ("Could not malloc principal\n"));
		return False;
	}
	...
	if (retval) {
		...
		if ((retval = kerberos_kinit_password(user, opt_password, 0, NULL))) {
			DEBUG_OUT (10, ("Requesting TGT failed: %s\n", error_message(retval)));
			return False;
		}
		...
		retval = cli_krb5_get_ticket(ctx, principal, 0,
						  ...
						  0, NULL, NULL, NULL);
		if (retval) {
			DEBUG_OUT (10, ("Kinit suceeded, but getting a ticket failed: %s\n", error_message(retval)));
			return False;
		}
	}
	...
	TALLOC_FREE(reply_base64);
	data_blob_free(&to_server);
	DEBUG_OUT (10, ("sent GSS-SPNEGO KERBEROS5 negTokenInit\n"));
	return True;
}
...
static void manage_client_krb5_targ(struct spnego_data spnego)
{
	...
	case SPNEGO_ACCEPT_INCOMPLETE:
		DEBUG_OUT (1, ("Got a Kerberos negTokenTarg with ACCEPT_INCOMPLETE\n"));
		x_fprintf(x_stdout, "BH Got a Kerberos negTokenTarg with "
				    "ACCEPT_INCOMPLETE\n");
		break;
	case SPNEGO_ACCEPT_COMPLETED:
		DEBUG_OUT (10, ("Accept completed\n"));
		x_fprintf(x_stdout, "AF\n");
		break;
	case SPNEGO_REJECT:
		DEBUG_OUT (10, ("Rejected\n"));
		x_fprintf(x_stdout, "NA\n");
		break;
	default:
		DEBUG_OUT (1, ("Got an invalid negTokenTarg\n"));
		x_fprintf(x_stdout, "AF\n");
	}
}
						...
						char *buf, int length)
{
	...
	if (strlen(buf) <= 3) {
		DEBUG_OUT (1, ("SPNEGO query [%s] too short\n", buf));
	...
	if ( (strncmp(buf, "TT ", 3) != 0) &&
	     (strncmp(buf, "AF ", 3) != 0) &&
	     (strncmp(buf, "NA ", 3) != 0) ) {
		DEBUG_OUT (1, ("SPNEGO request [%s] invalid\n", buf));
	...
	if (len == -1) {
		DEBUG_OUT (1, ("Could not read SPNEGO data for [%s]\n", buf));
	...
	if (spnego.type == SPNEGO_NEG_TOKEN_INIT) {
		...
		DEBUG_OUT (1, ("Server offered no compatible mechanism\n"));
	...
	if (spnego.type == SPNEGO_NEG_TOKEN_TARG) {
		...
		if (spnego.negTokenTarg.supportedMech == NULL) {
			...
			case SPNEGO_REJECT:
				...
				break;
			default:
				DEBUG_OUT (1, ("Got a negTokenTarg with no mech and an "
					  "unknown negResult: %d\n",
	...
	DEBUG_OUT (1, ("Got an SPNEGO token I could not handle [%s]!\n", buf));
						...
						char *buf, int length)
{
	...
	if (!parameter) {
		...
		if (!parameter) {
			DEBUG_OUT (0, ("Parameter not found!\n"));
		...
		stdio_helper_function fn)
{
	...
	if (!buf) {
		DEBUG_OUT (0, ("Failed to allocate input buffer.\n"));
	...
	do {
		...
		if (fgets(tmp, sizeof(tmp)-1, stdin) == NULL) {
			if (ferror(stdin)) {
				DEBUG_OUT (1, ("fgets() failed! dying..... errno=%d "
					  "(%s)\n", ferror(stdin),
	...
	*c = '\0';
	...
	DEBUG_OUT (10, ("Got '%s' from squid (length: %d).\n",buf,length));

	if (buf[0] == '\0') {
		DEBUG_OUT (2, ("Invalid Request\n"));
		x_fprintf(x_stderr, "ERR\n");
		talloc_free(buf);
		return;
	}
...
static void squid_stream(enum stdio_helper_mode stdio_mode, stdio_helper_function fn) {
	...
	if (!mem_ctx) {
		DEBUG_OUT (0, ("squid_stream: Failed to create talloc context\n"));
		x_fprintf(x_stderr, "ERR\n");
		exit(1);
	}
	...
	if (!state) {
		DEBUG_OUT (0, ("squid_stream: Failed to talloc ntlm_auth_state\n"));
		x_fprintf(x_stderr, "ERR\n");
		exit(1);
	}
