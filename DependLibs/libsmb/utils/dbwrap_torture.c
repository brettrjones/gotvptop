...
static void test_store_records(struct db_context *db, struct tevent_context *ev)
{
	...
	while ((timelimit == 0) || (timeval_elapsed(&start) < timelimit)) {
		...
		if (!no_trans) {
			if (verbose) DEBUG_OUT (1, ("starting transaction\n"));
			ret = db->transaction_start(db);
			if (ret != 0) {
				DEBUG_OUT (0, ("Failed to start transaction on node "
					  "%d\n", pnn));
				goto fail;
			}
			if (verbose) DEBUG_OUT (1, ("transaction started\n"));
			do_sleep(torture_delay);
		}

		if (verbose) DEBUG_OUT (1, ("calling fetch_lock\n"));
		rec = db->fetch_locked(db, tmp_ctx, key);
		if (rec == NULL) {
			DEBUG_OUT (0, ("Failed to fetch record\n"));
			goto fail;
		}
		if (verbose) DEBUG_OUT (1, ("fetched record ok\n"));
		do_sleep(torture_delay);
		...
		data.dptr = (unsigned char *)talloc_zero_size(tmp_ctx,
							      data.dsize);
		if (data.dptr == NULL) {
			DEBUG_OUT (0, ("Failed to allocate data\n"));
			goto fail;
		}
		...
		if (verbose) DEBUG_OUT (1, ("storing data\n"));
		status = rec->store(rec, data, TDB_REPLACE);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Failed to store record\n"));
			if (!no_trans) {
				ret = db->transaction_cancel(db);
				if (ret != 0) {
					DEBUG_OUT (0, ("Error cancelling transaction.\n"));
				}
			}
			goto fail;
		}
		talloc_free(rec);
		if (verbose) DEBUG_OUT (1, ("stored data ok\n"));
		do_sleep(torture_delay);

		if (!no_trans) {
			if (verbose) DEBUG_OUT (1, ("calling transaction_commit\n"));
			ret = db->transaction_commit(db);
			if (ret != 0) {
				DEBUG_OUT (0, ("Failed to commit transaction\n"));
				goto fail;
			}
			if (verbose) DEBUG_OUT (1, ("transaction committed\n"));
		}
