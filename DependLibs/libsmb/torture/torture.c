...
static void torture_createdel_created(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("cli_ntcreate_recv returned %s\n",
			   nt_errstr(status)));
...
static void torture_createdel_closed(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("cli_close_recv returned %s\n", nt_errstr(status)));
		tevent_req_nterror(req, status);
		return;
	}
	tevent_req_done(req);
...
static void torture_createdels_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)){
		DEBUG_OUT (10, ("torture_createdel_recv returned %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(subreq);
		tevent_req_nterror(req, status);
		return;
	...
	if (i == num_parallel) {
		DEBUG_OUT (10, ("received something we did not send\n"));
...
static void swallow_notify_done(struct tevent_req *subreq)
{
	...
	TALLOC_FREE(subreq);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("cli_notify_recv returned %s\n",
			   nt_errstr(status)));
		tevent_req_nterror(req, status);
		return;
	}
				       ...
				       char **pbase, char **pstream)
{
	...
	DEBUG_OUT (10, ("split_ntfs_stream_name called for [%s]\n", fname));
	...
	if (stype == NULL) {
		...
		if (StrCaseCmp(stype, ":$DATA") != 0) {
			 ...
			 */
			DEBUG_OUT (10, ("[%s] is an invalid stream type\n", stype));
			TALLOC_FREE(base);
