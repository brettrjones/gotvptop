  ...
  while (len > 0)
      ...
      if (outptr != outbuf)
	{
	  ...
	  if (fwrite (outbuf, 1, outptr - outbuf, output)
	      ...
	      || ferror (output))
	    {
	      ...
	      DEBUG_OUT (0, ("conversion stopped due to problem in writing the output"));
	      return -1;
	    }
      ...
      if (errno != E2BIG)
	{
	  ...
	  switch (errno)
	    {
	    case EILSEQ:
	      DEBUG_OUT (0,("illegal input sequence at position %ld", 
		     (long) (addr - start)));
	      break;
	    case EINVAL:
	      DEBUG_OUT (0, ("\
incomplete character or shift sequence at end of buffer"));
	      break;
	    case EBADF:
	      DEBUG_OUT (0, ("internal error (illegal descriptor)"));
	      break;
	    default:
	      DEBUG_OUT (0, ("unknown iconv() error %d", errno));
	      break;
	    }
...
process_fd (smb_iconv_t cd, int fd, FILE *output)
{
  ...
  while (actlen < maxlen)
	...
	{
	  /* Error while reading.  */
	  DEBUG_OUT (0, ("error while reading the input"));
	  return -1;
	}
  ...
  if (actlen == maxlen)
	...
	if (new_inbuf == NULL)
	  {
	    DEBUG_OUT (0, ("unable to allocate buffer for input"));
	    return -1;
	  }
	inbuf = new_inbuf;
	maxlen += 32768;
...
int main(int argc, char *argv[])
{
	...
	if(output) {
		...
		if(!out) {
			DEBUG_OUT (0, ("Can't open output file '%s': %s, exiting...\n", output, strerror(errno)));
			return 1;
		}
	}
	...
	if (cd == (smb_iconv_t)-1) {
		DEBUG_OUT (0,("unable to find from or to encoding, exiting...\n"));
	...
	while((file = poptGetArg(pc))) {
		if(strcmp(file, "-") == 0) fd = 0;
			...
			if(!fd) {
				DEBUG_OUT (0, ("Can't open input file '%s': %s, ignoring...\n", file, strerror(errno)));
				continue;
			}
		}
