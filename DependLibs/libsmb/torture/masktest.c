...
static struct cli_state *connect_one(char *share)
{
 ...
 again:
	...
	if (!(c=cli_initialise())) {
		DEBUG_OUT (0,("Connection to %s failed\n", server_n));
		return NULL;
	}

	status = cli_connect(c, server_n, &ss);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("Connection to %s failed. Error %s\n", server_n, nt_errstr(status) ));
		return NULL;
	}
	...
	if (!cli_session_request(c, &calling, &called)) {
		DEBUG_OUT (0,("session request to %s failed\n", called.name));
		cli_shutdown(c);
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("protocol negotiation failed: %s\n",
			  nt_errstr(status)));
		cli_shutdown(c);
		return NULL;
	}
	...
	status = cli_session_setup(c, username,
				   ...
				   lp_workgroup());
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("session setup failed: %s\n", nt_errstr(status)));
		return NULL;
	}
	...
	if (*c->server_domain || *c->server_os || *c->server_type)
		DEBUG_OUT (1,("Domain=[%s] OS=[%s] Server=[%s]\n",
	...
	status = cli_tcon_andx(c, share, "?????", password,
			       strlen(password)+1);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("tree connect failed: %s\n", nt_errstr(status)));
		cli_shutdown(c);
		return NULL;
	}
...
static void testpair(struct cli_state *cli, const char *mask, const char *file)
{
	...
	if (showall ||
		...
		DEBUG_OUT (0,("%s %s %d mask=[%s] file=[%s] rfile=[%s/%s]\n",
 ...
 int main(int argc,char *argv[])
{
	...
	if (!cli) {
		DEBUG_OUT (0,("Failed to connect to %s\n", share));
		exit(1);
	}
