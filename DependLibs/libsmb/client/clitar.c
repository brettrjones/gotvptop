...
static char *string_create_s(int size)
{
	...
	if (tmp == NULL) {
		DEBUG_OUT (0, ("Out of memory in string_create_s\n"));
	}
			   ...
			   const char *amode, unsigned char ftype)
{
	...
	DEBUG_OUT (5, ("WriteTarHdr, Type = %c, Size= %.0f, Name = %s\n", ftype, (double)size, aname));
	...
	if (l+2 >= NAMSIZ) {
		...
		i = strlen(b)+1;
		DEBUG_OUT (5, ("File name in tar file: %s, size=%d, \n", b, (int)strlen(b)));
...
static long readtarheader(union hblock *hb, file_info2 *finfo, const char *prefix)
{
	...
	DEBUG_OUT (5, ("checksum totals chk=%ld fchk=%ld chksum=%s\n",
			chk, fchk, hb->dbuf.chksum));

	if (fchk != chk) {
		DEBUG_OUT (0, ("checksums don't match %ld %ld\n", fchk, chk));
		dump_data(5, (uint8 *)hb - TBLOCK, TBLOCK *3);
		return -1;
	}

	if ((finfo->name = string_create_s(strlen(prefix) + strlen(hb -> dbuf.name) + 3)) == NULL) {
		DEBUG_OUT (0, ("Out of space creating file_info2 for %s\n", hb -> dbuf.name));
		return(-1);
	}
	...
	if ((hb->dbuf.linkflag != '0') && (hb -> dbuf.linkflag != '5')) {
		if (hb->dbuf.linkflag == 0) {
			DEBUG_OUT (6, ("Warning: NULL link flag (gnu tar archive ?) %s\n",
				finfo->name));
		} else {
			if (hb -> dbuf.linkflag == 'L') { /* We have a longlink */
				...
				DEBUG_OUT (0, ("this tar file appears to contain some kind \
...
static void dozerobuf(int f, int n)
{
	...
	if (n+tp >= tbufsiz) {
		...
		if (sys_write(f, tarbuf, tbufsiz) != tbufsiz) {
			DEBUG_OUT (0, ("dozerobuf: sys_write fail\n"));
			return;
		}
...
static void dotareof(int f)
{
	...
	if (sys_fstat(f, &stbuf, false) == -1) {
		DEBUG_OUT (0, ("Couldn't stat file handle\n"));
		return;
	}
	...
	if (tp > 0) {
		...
		if (sys_write(f, tarbuf, towrite) != towrite) {
			DEBUG_OUT (0,("dotareof: sys_write fail\n"));
		}
	}
}
	...
	while (p) {
		...
		if (!NT_STATUS_IS_OK(cli_chkpath(cli, partpath))) {
			if (!NT_STATUS_IS_OK(cli_mkdir(cli, partpath))) {
				SAFE_FREE(partpath);
				SAFE_FREE(ffname);
				DEBUG_OUT (0, ("Error mkdir %s\n", cli_errstr(cli)));
				return False;
			} else {
				DEBUG_OUT (3, ("mkdirhiering %s\n", partpath));
			}
		}
...
static int padit(char *buf, uint64_t bufsize, uint64_t padsize)
{
	...
	DEBUG_OUT (5, ("Padding with %0.f zeros\n", (double)padsize));
		    ...
		    struct file_info *finfo1)
{
	...
	if (!data) {
		DEBUG_OUT (0,("do_atar: out of memory.\n"));
	...
	if (dry_run) {
		DEBUG_OUT (3,("skipping file %s of size %12.0f bytes\n", finfo1->name,
				(double)finfo.size));
		shallitime=0;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0,("%s opening remote file %s (%s)\n",
	...
	if (finfo.name == NULL) {
		DEBUG_OUT (0, ("Unable to allocate space for finfo.name in do_atar\n"));
		status = NT_STATUS_NO_MEMORY;
		goto cleanup;
	}
	...
	DEBUG_OUT (3,("file %s attrib 0x%X\n",finfo.name,finfo.mode));

	if (tar_inc && !(finfo.mode & FILE_ATTRIBUTE_ARCHIVE)) {
		DEBUG_OUT (4, ("skipping %s - archive bit not set\n", finfo.name));
		shallitime=0;
	} else if (!tar_system && (finfo.mode & FILE_ATTRIBUTE_SYSTEM)) {
		DEBUG_OUT (4, ("skipping %s - system bit is set\n", finfo.name));
		shallitime=0;
	} else if (!tar_hidden && (finfo.mode & FILE_ATTRIBUTE_HIDDEN)) {
		DEBUG_OUT (4, ("skipping %s - hidden bit is set\n", finfo.name));
		shallitime=0;
	} else {
		bool wrote_tar_header = False;

		DEBUG_OUT (3,("getting file %s of size %.0f bytes as a tar file %s",
			finfo.name, (double)finfo.size, lname));

		do {

			DEBUG_OUT (3,("nread=%.0f\n",(double)nread));

			datalen = cli_read(cli, fnum, data, nread, read_size);

			if (datalen == -1) {
				DEBUG_OUT (0,("Error reading file %s : %s\n", rname, cli_errstr(cli)));
			...
			*/
			...
			if (nread > finfo.size) {
				...
				DEBUG_OUT (0,("File size change - truncating %s to %.0f bytes\n",
			...
			* write out in 512 byte intervals */
			...
			if (dotarbuf(tarhandle,data,datalen) != datalen) {
				DEBUG_OUT (0,("Error writing to tar file - %s\n", strerror(errno)));
			...
			if ( (datalen == 0) && (finfo.size != 0) ) {
				...
				DEBUG_OUT (0,("Error reading file %s. Got 0 bytes\n", rname));
				break;
			}
		...
		if (wrote_tar_header) {
			...
			if (nread < finfo.size) {
				DEBUG_OUT (0, ("Didn't get entire file. size=%.0f, nread=%d\n",
							(double)finfo.size, (int)nread));
				if (padit(data, (uint64_t)sizeof(data), finfo.size - nread)) {
					status = map_nt_error_from_unix(errno);
					DEBUG_OUT (0,("Error writing tar file - %s\n", strerror(errno)));
				}
			}
			...
			DEBUG_OUT (4, ("skipping %s - initial read failed (file was locked ?)\n", finfo.name));
			shallitime=0;
			status = NT_STATUS_UNSUCCESSFUL;
		}
	}
		   ...
		   const char *dir)
{
	...
	if (!tar_excl && clipn) {
		...
		DEBUG_OUT (5, ("Excl: strlen(cur_dir) = %d\n", (int)strlen(client_get_cur_dir())));
...
strlen(finfo->name)=%d\nname=%s,cur_dir=%s\n",
		...
		DEBUG_OUT (5, ("Writing a dir, Name = %s\n", client_get_cur_dir()));
		...
		DEBUG_OUT (5, ("Doing list with mtar_mask: %s\n", mtar_mask));
...
static void unfixtarname(char *tptr, char *fp, int l, bool first)
{
	...
	DEBUG_OUT (5, ("firstb=%lX, secondb=%lX, len=%i\n", (long)tptr, (long)fp, l));
...
static int next_block(char *ltarbuf, char **bufferp, int bufsiz)
{
	...
	DEBUG_OUT (5, ("Advancing to next block: %0lx\n", (unsigned long)*bufferp));
	*bufferp += TBLOCK;
	total = TBLOCK;

	if (*bufferp >= (ltarbuf + bufsiz)) {

		DEBUG_OUT (5, ("Reading more data into ltarbuf ...\n"));
		...
		DEBUG_OUT (5, ("Total bytes read ... %i\n", total));
...
static int skip_file(int skipsize)
{
	...
	DEBUG_OUT (5, ("Skiping file. Size = %i\n", skipsize));

	/* FIXME, we should skip more than one block at a time */

	while (dsize > 0) {
		if (next_block(tarbuf, &buffer_p, tbufsiz) <= 0) {
			DEBUG_OUT (0, ("Empty file, short tar file, or read error: %s\n", strerror(errno)));
			return(False);
		}
		dsize -= TBLOCK;
	}
...
static int get_file(file_info2 finfo)
{
	...
	DEBUG_OUT (5, ("get_file: file: %s, size %.0f\n", finfo.name, (double)finfo.size));

	if (!ensurepath(finfo.name)) {
		DEBUG_OUT (0, ("abandoning restore\n"));
		return False;
	}
	...
	while (rsize > 0) {
		...
		DEBUG_OUT (5, ("writing %i bytes, bpos = %i ...\n", dsize, bpos));

		status = cli_writeall(cli, fnum, 0,
				      (uint8_t *)(buffer_p + bpos), pos,
				      dsize, NULL);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Error writing remote file: %s\n",
				  nt_errstr(status)));
			return 0;
		}
	...
	if (!NT_STATUS_IS_OK(cli_close(cli, fnum))) {
		DEBUG_OUT (0, ("Error %s closing remote file\n",
			cli_errstr(cli)));
		return(False);
	}

	/* Now we update the creation date ... */
	DEBUG_OUT (5, ("Updating creation date on %s\n", finfo.name));

	if (!NT_STATUS_IS_OK(cli_setatr(cli, finfo.name, finfo.mode, finfo.mtime_ts.tv_sec))) {
		if (tar_real_noisy) {
			DEBUG_OUT (0, ("Could not set time on file: %s\n", finfo.name));
			/*return(False); */ /* Ignore, as Win95 does not allow changes */
		}
	}

	ntarf++;
	DEBUG_OUT (0, ("restore tar file %s of size %.0f bytes\n", finfo.name, (double)finfo.size));
	return(True);
}
...
{
	DEBUG_OUT (0, ("restore directory %s\n", finfo.name));

	if (!ensurepath(finfo.name)) {
		DEBUG_OUT (0, ("Problems creating directory\n"));
		return(False);
	}
	ntarf++;
	return(True);
...
static char *get_longfilename(file_info2 finfo)
{
	...
	DEBUG_OUT (5, ("Restoring a long file name: %s\n", finfo.name));
	DEBUG_OUT (5, ("Len = %.0f\n", (double)finfo.size));

	if (longname == NULL) {
		DEBUG_OUT (0, ("could not allocate buffer of size %d for longname\n", namesize));
		return(NULL);
	}
	...
	while (left > 0) {
		...
		DEBUG_OUT (5, ("UnfixedName: %s, buffer: %s\n", longname, buffer_p));
...
static void do_tarput(void)
{
	...
	clock_gettime_mono(&tp_start);
	DEBUG_OUT (5, ("RJS do_tarput called ...\n"));
	...
	while (True) {
		...
		switch (readtarheader((union hblock *) buffer_p,
			...
			case -1:
				DEBUG_OUT (0, ("abandoning restore, -1 from read tar header\n"));
				SAFE_FREE(longfilename);
				return;

			case 0: /* chksum is zero - looks like an EOF */
				DEBUG_OUT (0, ("tar: restored %d files and directories\n", ntarf));
				SAFE_FREE(longfilename);
		...
		DEBUG_OUT (5, ("Skip = %i, cliplist=%s, file=%s\n", skip, (cliplist?cliplist[0]:NULL), finfo.name));
		if (skip) {
...
int cmd_block(void)
{
	...
	if (!next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		DEBUG_OUT (0, ("blocksize <n>\n"));
		return 1;
	}

	block=atoi(buf);
	if (block < 0 || block > 65535) {
		DEBUG_OUT (0, ("blocksize out of range"));
		return 1;
	}

	blocksize=block;
	DEBUG_OUT (2,("blocksize is now %d\n", blocksize));
	return 0;
}
...
int cmd_tarmode(void)
{
	...
	while (next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		if (strequal(buf, "full"))
			...
			DEBUG_OUT (0, ("tarmode: unrecognised option %s\n", buf));
		TALLOC_FREE(buf);
	}

	DEBUG_OUT (0, ("tarmode is now %s, %s, %s, %s, %s\n",
...
int cmd_setmode(void)
{
	...
	if (!next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		DEBUG_OUT (0, ("setmode <filename> <[+|-]rsha>\n"));
		return 1;
	}
		...
		while(*q) {
			switch (*q++) {
				...
				case 'a':
					...
					break;
				default:
					DEBUG_OUT (0, ("setmode <filename> <perm=[+|-]rsha>\n"));
					return 1;
			}
		}
	}
...
int cmd_tar(void)
{
	...
	if (!next_token_talloc(ctx, &cmd_ptr,&buf,NULL)) {
		DEBUG_OUT (0,("tar <c|x>[IXbgan] <filename>\n"));
		return 1;
	}
...
int process_tar(void)
{
	...
	switch(tar_type) {
		...
		case 'c':
			if (clipn && tar_excl) {
				...
				for (i=0; i<clipn; i++) {
					...
					if (strrchr_m(cliplist[i], '\\')) {
						...
						DEBUG_OUT (5, ("process_tar, do_list with tarmac: %s\n", tarmac));
				...
				DEBUG_OUT (5, ("process_tar, do_list with mask: %s\n", mask));
			...
			DEBUG_OUT (0, ("tar: dumped %d files and directories\n", ntarf));
			DEBUG_OUT (0, ("Total bytes written: %.0f\n", (double)ttarf));
			break;
	}
...
static int read_inclusion_file(char *filename)
{
	...
	if ((inclusion = x_fopen(filename, O_RDONLY, 0)) == NULL) {
		 ...
		 */
		DEBUG_OUT (0,("Unable to open inclusion file %s\n", filename));
		return 0;
	}
	...
	while ((! error) && (x_fgets(buf, sizeof(buf)-1, inclusion))) {
		if (inclusion_buffer == NULL) {
			...
			if ((inclusion_buffer = (char *)SMB_MALLOC(inclusion_buffer_size)) == NULL) {
				DEBUG_OUT (0,("failure allocating buffer to read inclusion file\n"));
				error = 1;
				break;
			}
		}
		...
		if ((strlen(buf) + 1 + inclusion_buffer_sofar) >= inclusion_buffer_size) {
			...
			if (!inclusion_buffer) {
				DEBUG_OUT (0,("failure enlarging inclusion buffer to %d bytes\n",
	...
	if (! error) {
		...
		if (cliplist == NULL) {
			DEBUG_OUT (0,("failure allocating memory for cliplist\n"));
			error = 1;
		} else {
			cliplist[clipn] = NULL;
			p = inclusion_buffer;
			for (i = 0; (! error) && (i < clipn); i++) {
				...
				if ((tmpstr = (char *)SMB_MALLOC(strlen(p)+1)) == NULL) {
					DEBUG_OUT (0, ("Could not allocate space for a cliplist item, # %i\n", i));
					error = 1;
				} else {
					unfixtarname(tmpstr, p, strlen(p) + 1, True);
					cliplist[i] = tmpstr;
					if ((p = strchr_m(p, '\000')) == NULL) {
						DEBUG_OUT (0,("INTERNAL ERROR: inclusion_buffer is of unexpected contents.\n"));
						abort();
					}
				}
				++p;
...
int tar_parseargs(int argc, char *argv[], const char *Optarg, int Optind)
{
		...
		switch(*Optarg++) {
			...
			case 'b':
				if (Optind>=argc || !(blocksize=atoi(argv[Optind]))) {
					DEBUG_OUT (0,("Option b must be followed by valid blocksize\n"));
					return 0;
				} else {
					Optind++;
					newOptind++;
			...
			case 'N':
				if (Optind>=argc) {
					DEBUG_OUT (0,("Option N must be followed by valid file name\n"));
					return 0;
				} else {
					SMB_STRUCT_STAT stbuf;
					...
					if (sys_stat(argv[Optind], &stbuf,
						...
						newer_than = convert_timespec_to_time_t(
							stbuf.st_ex_mtime);
						DEBUG_OUT (1,("Getting files newer than %s",
							time_to_asc(newer_than)));
						newOptind++;
						Optind++;
					} else {
						DEBUG_OUT (0,("Error setting newer-than time\n"));
						return 0;
					}
				}
				break;
			...
			case 'I':
				if (tar_clipfl) {
					DEBUG_OUT (0,("Only one of I,X,F must be specified\n"));
					return 0;
				}
				tar_clipfl='I';
				break;
			...
			case 'F':
				...
				break;
			case 'r':
				DEBUG_OUT (0, ("tar_re_search set\n"));
				tar_re_search = True;
				break;
			case 'n':
				if (tar_type == 'c') {
					DEBUG_OUT (0, ("dry_run set\n"));
					dry_run = True;
				} else {
					DEBUG_OUT (0, ("n is only meaningful when creating a tar-file\n"));
					return 0;
				}
				break;
			default:
				DEBUG_OUT (0,("Unknown tar option\n"));
				return 0;
		}
	}
	...
	if (tar_clipfl=='F') {
		if (argc-Optind-1 != 1) {
			DEBUG_OUT (0,("Option F must be followed by exactly one filename.\n"));
			return 0;
		}
		newOptind++;
		...
		if ((tmplist=SMB_MALLOC_ARRAY(char *,clipn)) == NULL) {
			DEBUG_OUT (0, ("Could not allocate space to process cliplist, count = %i\n", clipn));
			return 0;
		}

		for (clipcount = 0; clipcount < clipn; clipcount++) {

			DEBUG_OUT (5, ("Processing an item, %s\n", cliplist[clipcount]));
			...
			DEBUG_OUT (5, ("Processed an item, %s\n", tmpstr));

			DEBUG_OUT (5, ("Cliplist is: %s\n", cliplist[0]));
		}
	...
	if (Optind>=argc || !strcmp(argv[Optind], "-")) {
		...
		if (!argv[Optind]) {
			DEBUG_OUT (0,("Must specify tar filename\n"));
			return 0;
		}
		...
		if (tar_type=='c' && dry_run) {
			...
			DEBUG_OUT (0,("Error opening local file %s - %s\n", argv[Optind], strerror(errno)));
			return(0);
		}
		newOptind++;
	}
