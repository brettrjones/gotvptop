			  ...
			  const char *dir)
{
	...
	if (!showacls) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT ( 0, ("display_finfo() Failed to open %s: %s\n",
				   afname, nt_errstr(status)));
		} else {
			struct security_descriptor *sd = NULL;
			sd = cli_query_secdesc(cli_state, fnum, ctx);
			if (!sd) {
				DEBUG_OUT ( 0, ("display_finfo() failed to "
...
static void adjust_do_list_queue(void)
{
	...
	if (do_list_queue == NULL) {
		DEBUG_OUT (4,("do_list_queue is empty\n"));
	...
	if (do_list_queue_start == do_list_queue_end) {
		...
		*do_list_queue = '\0';
		...
		DEBUG_OUT (4,("sliding do_list_queue backward\n"));
...
static void add_to_do_list_queue(const char *entry)
{
	...
	while (new_end > do_list_queue_size) {
		...
		DEBUG_OUT (4,("enlarging do_list_queue to %d\n",
			 (int)do_list_queue_size));
	...
	if (do_list_queue) {
		...
		DEBUG_OUT (4,("added %s to do_list_queue (start=%d, end=%d)\n",
...
static void remove_do_list_queue_head(void)
{
	if (do_list_queue_end > do_list_queue_start) {
		...
		DEBUG_OUT (4,("removed head of do_list_queue (start=%d, end=%d)\n",
...
static int cmd_dir(void)
{
	...
	DEBUG_OUT (3, ("Total bytes listed: %.0f\n", dir_total));
...
static int do_get(const char *rname, const char *lname_in, bool reget)
{
	...
	DEBUG_OUT (1,("getting file %s of size %.0f as %s ",
...

	{
		...
		DEBUG_OUT (1,("(%3.1f KiloBytes/sec) (average %3.1f KiloBytes/sec)\n",
...
static int do_put(const char *rname, const char *lname, bool reput)
{
	...
	DEBUG_OUT (1,("putting file %s as %s ",lname,
		 rname));
...

	{
		...
		DEBUG_OUT (1,("(%3.1f kb/s) (average %3.1f kb/s)\n",
			...
			if (temp_list->isdir) {
				...
				if (prompt && !yesno(quest)) { /* No */
					...
					if (!NT_STATUS_IS_OK(cli_chkpath(cli, rname)) &&
					    !do_mkdir(rname)) {
						DEBUG_OUT (0, ("Unable to make dir, skipping..."));
						/* Skip the directory */
						lname[strlen(lname)-1] = '/';
						if (!seek_list(temp_list, lname)) {
							break;
...
{
	prompt = !prompt;
	DEBUG_OUT (2,("prompting is now %s\n",prompt?"on":"off"));
	return 1;
}
...
{
	lowercase = !lowercase;
	DEBUG_OUT (2,("filename lowercasing is now %s\n",lowercase?"on":"off"));
	return 0;
}
...
static int cmd_setcase(void)
{
	...
	DEBUG_OUT (2,("filename case sensitivity is now %s\n",!orig_case_sensitive ?
		"on":"off"));
	return 0;
}
...
{
	recurse = !recurse;
	DEBUG_OUT (2,("directory recursion is now %s\n",recurse?"on":"off"));
	return 0;
}
...
static int cmd_translate(void)
{
	...
	DEBUG_OUT (2,("CR/LF<->LF and print text translation now %s\n",
		 translation?"on":"off"));
	return 0;
}
...
static int cmd_lcd(void)
{
	...
	DEBUG_OUT (2,("the local directory is now %s\n",sys_getwd(d)));
	return 0;
}
...
static bool browse_host_rpc(bool sort)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Could not connect to srvsvc pipe: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(frame);
		return false;
	}
...
#else
	DEBUG_OUT (0,("no history without readline support\n"));
#endif
 ...
 again:
	...
	*/
	...
	if ((ret > 0) && (revents & (POLLIN|POLLHUP|POLLERR))) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (0, ("Read from server failed, maybe it closed "
				  "the connection\n"));
	...
	/* Ping the server to keep the connection alive using SMBecho. */
	{
		...
		if (!cli_state_is_connected(cli)) {
			DEBUG_OUT (0, ("SMBecho failed (%s). The connection is "
				"disconnected now\n", nt_errstr(status)));
			finished = true;
			smb_readline_done();
		}
 ...
 int main(int argc,char *argv[])
{
	...
	DEBUG_OUT (3,("Client started (version %s).\n", samba_version_string()));
