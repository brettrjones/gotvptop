				       ...
				       struct auth_serversupplied_info **server_info)
{
	...
	if (!auth_context) {
		DEBUG_OUT (3,("Password for user %s cannot be checked because we have no auth_info to get the challenge from.\n", 
			 user_info->mapped.account_name));
		return NT_STATUS_INVALID_PARAMETER;
	}		

	if (strequal(user_info->mapped.domain_name, get_global_sam_name())) {
		DEBUG_OUT (3,("check_winbind_security: Not using winbind, requested domain [%s] was for this SAM.\n",
	...
	if (!WBC_ERROR_IS_OK(wbc_status)) {
		DEBUG_OUT (10,("check_winbind_security: wbcAuthenticateUserEx failed: %s\n",
