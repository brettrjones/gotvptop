...
NTSTATUS smb_register_auth(int version, const char *name, auth_init_function init)
{
	...
	if (version != AUTH_INTERFACE_VERSION) {
		DEBUG_OUT (0,("Can't register auth_method!\n"
	...
	DEBUG_OUT (5,("Attempting to register auth backend %s\n", name));

	if (auth_find_backend_entry(name)) {
		DEBUG_OUT (0,("There already is an auth method registered with the name %s!\n", name));
	...
	DEBUG_OUT (5,("Successfully added auth method '%s'\n", name));
	return NT_STATUS_OK;
}
			       ...
			       uint8_t chal[8])
{
	...
	if (auth_context->challenge.length) {
		DEBUG_OUT (5, ("get_ntlm_challenge (auth subsystem): returning previous challenge by module %s (normal)\n", 
	...
	for (auth_method = auth_context->auth_method_list; auth_method; auth_method = auth_method->next) {
		if (auth_method->get_chal == NULL) {
			DEBUG_OUT (5, ("auth_get_challenge: module %s did not want to specify a challenge\n", auth_method->name));
			continue;
		}

		DEBUG_OUT (5, ("auth_get_challenge: getting challenge from module %s\n", auth_method->name));
		if (challenge_set_by != NULL) {
			DEBUG_OUT (1, ("auth_get_challenge: CONFIGURATION ERROR: authentication method %s has already specified a challenge.  Challenge by %s ignored.\n", 
		...
		challenge = auth_method->get_chal(auth_context, &auth_method->private_data,
						  auth_context);
		if (!challenge.length) {
			DEBUG_OUT (3, ("auth_get_challenge: getting challenge from authentication method %s FAILED.\n", 
				  auth_method->name));
		} else {
			DEBUG_OUT (5, ("auth_get_challenge: successfully got challenge from module %s\n", auth_method->name));
	...
	DEBUG_OUT (5, ("auth_context challenge created by %s\n", challenge_set_by));
	DEBUG_OUT (5, ("challenge is: \n"));
...
static bool check_domain_match(const char *user, const char *domain) 
{
	...
	if (!lp_allow_trusted_domains() &&
	    !(strequal("", domain) || 
	      ...
	      is_myname(domain))) {
		DEBUG_OUT (1, ("check_domain_match: Attempt to connect as user %s from domain %s denied.\n", user, domain));
		return False;
	} else {
		return True;
	}
				    ...
				    struct auth_serversupplied_info **server_info)
{
	...
	DEBUG_OUT (3, ("check_ntlm_password:  Checking password for unmapped user [%s]\\[%s]@[%s] with the new password interface\n", 
		  user_info->client.domain_name, user_info->client.account_name, user_info->workstation_name));

	DEBUG_OUT (3, ("check_ntlm_password:  mapped user is: [%s]\\[%s]@[%s]\n", 
		  user_info->mapped.domain_name, user_info->mapped.account_name, user_info->workstation_name));

	if (auth_context->challenge.length != 8) {
		DEBUG_OUT (0, ("check_ntlm_password:  Invalid challenge stored for this auth context - cannot continue\n"));
		return NT_STATUS_LOGON_FAILURE;
	}

	if (auth_context->challenge_set_by)
		DEBUG_OUT (10, ("check_ntlm_password: auth_context challenge created by %s\n",
					auth_context->challenge_set_by));

	DEBUG_OUT (10, ("challenge is: \n"));
	dump_data(5, auth_context->challenge.data, auth_context->challenge.length);

#ifdef DEBUG_PASSWORD
	DEBUG_OUT (100, ("user_info has passwords of length %d and %d\n", 
	...
	for (auth_method = auth_context->auth_method_list;auth_method; auth_method = auth_method->next) {
		...
		if ( NT_STATUS_V(result) == NT_STATUS_V(NT_STATUS_NOT_IMPLEMENTED) ) {
			DEBUG_OUT (10,("check_ntlm_password: %s had nothing to say\n", auth_method->name));
		...
		if (NT_STATUS_IS_OK(nt_status)) {
			DEBUG_OUT (3, ("check_ntlm_password: %s authentication for user [%s] succeeded\n", 
				  auth_method->name, user_info->client.account_name));
		} else {
			DEBUG_OUT (5, ("check_ntlm_password: %s authentication for user [%s] FAILED with error %s\n", 
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		if (!(*server_info)->guest) {
			...
			if (NT_STATUS_IS_OK(nt_status)) {
				DEBUG_OUT (5, ("check_ntlm_password:  PAM Account for user [%s] succeeded\n", 
					  unix_username));
			} else {
				DEBUG_OUT (3, ("check_ntlm_password:  PAM Account for user [%s] FAILED with error %s\n", 
					  unix_username, nt_errstr(nt_status)));
			} 
		}

		if (NT_STATUS_IS_OK(nt_status)) {
			DEBUG_OUT ((*server_info)->guest ? 5 : 2, 
	...
	DEBUG_OUT (2, ("check_ntlm_password:  Authentication for user [%s] -> [%s] FAILED with error %s\n",
				  ...
				  struct auth_context **auth_context)
{
	...
	if (!ctx) {
		DEBUG_OUT (0,("make_auth_context: talloc failed!\n"));
		      ...
		      const char *module, auth_methods **ret) 
{
	...
	DEBUG_OUT (5,("load_auth_module: Attempting to find an auth method to match %s\n",
		 module));
	...
	if (entry != NULL) {
		if (!NT_STATUS_IS_OK(entry->init(auth_context, module_params, ret))) {
			DEBUG_OUT (0,("load_auth_module: auth method %s did not correctly init\n",
				 module_name));
		} else {
			DEBUG_OUT (5,("load_auth_module: auth method %s has a valid init\n",
				 module_name));
			good = True;
		}
	} else {
		DEBUG_OUT (0,("load_auth_module: can't find auth method %s!\n", module_name));
	}
					    ...
					    char **text_list)
{
	...
	if (!text_list) {
		DEBUG_OUT (2,("make_auth_context_text_list: No auth method list!?\n"));
				     ...
				     struct auth_context **auth_context)
{
	...
	if (auth_method_list == NULL) {
		...
		{
		case SEC_DOMAIN:
			DEBUG_OUT (5,("Making default auth method list for security=domain\n"));
			auth_method_list = str_list_make_v3(
				talloc_tos(), "guest sam winbind:ntdomain",
				NULL);
			break;
		case SEC_SERVER:
			DEBUG_OUT (5,("Making default auth method list for security=server\n"));
			auth_method_list = str_list_make_v3(
		...
		case SEC_USER:
			if (lp_encrypted_passwords()) {	
				if ((lp_server_role() == ROLE_DOMAIN_PDC) || (lp_server_role() == ROLE_DOMAIN_BDC)) {
					DEBUG_OUT (5,("Making default auth method list for DC, security=user, encrypt passwords = yes\n"));
					auth_method_list = str_list_make_v3(
						talloc_tos(),
						"guest sam winbind:trustdomain",
						NULL);
				} else {
					DEBUG_OUT (5,("Making default auth method list for standalone security=user, encrypt passwords = yes\n"));
					auth_method_list = str_list_make_v3(
						talloc_tos(), "guest sam",
						NULL);
				}
			} else {
				DEBUG_OUT (5,("Making default auth method list for security=user, encrypt passwords = no\n"));
				auth_method_list = str_list_make_v3(
		...
		case SEC_SHARE:
			if (lp_encrypted_passwords()) {
				DEBUG_OUT (5,("Making default auth method list for security=share, encrypt passwords = yes\n"));
				auth_method_list = str_list_make_v3(
					talloc_tos(), "guest sam", NULL);
			} else {
				DEBUG_OUT (5,("Making default auth method list for security=share, encrypt passwords = no\n"));
				auth_method_list = str_list_make_v3(
					talloc_tos(), "guest unix", NULL);
			}
			break;
		case SEC_ADS:
			DEBUG_OUT (5,("Making default auth method list for security=ADS\n"));
			auth_method_list = str_list_make_v3(
				talloc_tos(), "guest sam winbind:ntdomain",
				NULL);
			break;
		default:
			DEBUG_OUT (5,("Unknown auth method!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	} else {
		DEBUG_OUT (5,("Using specified auth order\n"));
	}
