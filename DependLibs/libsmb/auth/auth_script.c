					...
					struct auth_serversupplied_info **server_info)
{
	...
	if (!auth_context) {
		DEBUG_OUT (3,("script_check_user_credentials: no auth_info !\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}		
	...
	DEBUG_OUT (10,("script_check_user_credentials: running %s with parameters:\n%s\n",
		script, secret_str ));
	...
	if (ret) {
		DEBUG_OUT (1,("script_check_user_credentials: failed to authenticate %s\\%s\n",
