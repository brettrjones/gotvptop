...
bool nt_token_check_domain_rid( struct security_token *token, uint32 rid )
{
	...
	if ( lp_server_role() == ROLE_DOMAIN_MEMBER ) {
		if ( !secrets_fetch_domain_sid( lp_workgroup(),
						&domain_sid ) ) {
			DEBUG_OUT (1,("nt_token_check_domain_rid: Cannot lookup "
				 "SID for domain [%s]\n", lp_workgroup()));
			return False;
		}
	}
...
struct security_token *get_root_nt_token( void )
{
	...
	if ( !(pw = sys_getpwuid(0)) ) {
		if ( !(pw = sys_getpwnam("root")) ) {
			DEBUG_OUT (0,("get_root_nt_token: both sys_getpwuid(0) "
		     ...
		     struct security_token *token)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("pdb_enum_alias_memberships failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
					  ...
					  struct security_token **ntok)
{
	...
	DEBUG_OUT (10, ("Create local NT token for %s\n",
	...
	for (i = 0; i < info3->base.groups.count; i++) {
		...
		status = add_sid_to_array_unique(usrtok, &tmp_sid,
						 ...
						 &usrtok->num_sids);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3, ("Failed to add SID to nt token\n"));
			TALLOC_FREE(usrtok);
			return status;
		}
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (3, ("Failed to finalize nt token\n"));
		TALLOC_FREE(usrtok);
		return status;
	}
					...
					bool is_guest)
{
	...
	if (!sid_to_gid(&global_sid_Builtin_Administrators, &gid)) {
		...
		if (!secrets_fetch_domain_sid(lp_workgroup(), &dom_sid)) {
			status = NT_STATUS_OK;
			DEBUG_OUT (3, ("Failed to fetch domain sid for %s\n",
				  lp_workgroup()));
		} else {
		...
		if (NT_STATUS_EQUAL(status, NT_STATUS_PROTOCOL_UNREACHABLE)) {
			...
			if ( !NT_STATUS_IS_OK(status) ) {
				DEBUG_OUT (3, ("Failed to check for local "
					  "Administrators membership (%s)\n",
					  nt_errstr(status)));
			}
		} else if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (2, ("WARNING: Failed to create "
	...
	if (!sid_to_gid(&global_sid_Builtin_Users, &gid)) {
		...
		{
			DEBUG_OUT (2, ("WARNING: Failed to create BUILTIN\\Users group! "
				    ...
				    struct security_token **token)
{
	...
	if (!lookup_name_smbconf(tmp_ctx, username, LOOKUP_NAME_ALL,
		...
		DEBUG_OUT (1, ("lookup_name_smbconf for %s failed\n", username));
		goto done;
	}

	if (type != SID_NAME_USER) {
		DEBUG_OUT (1, ("%s is a %s, not a user\n", username,
	...
	if (sid_check_is_in_our_domain(&user_sid)) {
		...
		if (!ret) {
			DEBUG_OUT (1, ("pdb_getsampwsid(%s) for user %s failed\n",
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG_OUT (1, ("enum_group_memberships failed for %s (%s): "
		...
		*found_username = talloc_strdup(mem_ctx,
		...
		if (sam_acct->unix_pw == NULL) {
			...
			if (!pwd) {
				DEBUG_OUT (10, ("Get_Pwnam_alloc failed for %s\n",
					*found_username));
			...
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG_OUT (10, ("samu_set_unix failed for %s\n",
					*found_username));
				result = NT_STATUS_NO_SUCH_USER;
				goto done;
			}
	...
	unix_user:
		...
		if (!sid_to_uid(&user_sid, uid)) {
			DEBUG_OUT (1, ("unix_user case, sid_to_uid for %s (%s) failed\n",
				  username, sid_string_dbg(&user_sid)));
			result = NT_STATUS_NO_SUCH_USER;
			goto done;
		}
		...
		if (pass == NULL) {
			DEBUG_OUT (1, ("getpwuid(%u) for user %s failed\n",
		...
		if (!getgroups_unix_user(tmp_ctx, username, pass->pw_gid,
			...
			DEBUG_OUT (1, ("getgroups_unix_user for user %s failed\n",
				  username));
			goto done;
		}
		...
		if (num_group_sids) {
			...
			if (group_sids == NULL) {
				DEBUG_OUT (1, ("TALLOC_ARRAY failed\n"));
				result = NT_STATUS_NO_MEMORY;
				goto done;
			}
		} else {
		...
		*found_username = talloc_strdup(mem_ctx, pass->pw_name);
		...
		/* We must always assign the *uid. */
		if (!sid_to_uid(&user_sid, uid)) {
			DEBUG_OUT (1, ("winbindd case, sid_to_uid for %s (%s) failed\n",
				  username, sid_string_dbg(&user_sid)));
			result = NT_STATUS_NO_SUCH_USER;
			goto done;
		}
		...
		if (!sid_to_gid(&group_sids[0], gid)) {
			DEBUG_OUT (1, ("sid_to_gid(%s) failed\n",
...
bool user_in_group_sid(const char *username, const struct dom_sid *group_sid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("could not create token for %s\n", username));
		TALLOC_FREE(mem_ctx);
		return False;
	}
...
bool user_in_group(const char *username, const char *groupname)
{
	...
	if (!ret) {
		DEBUG_OUT (10, ("lookup_name for (%s) failed.\n", groupname));
		return False;
	}
