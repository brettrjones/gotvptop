			      ...
			      struct samu *sampass)
{
	...
	if ( !(pwd = Get_Pwnam_alloc(result, username)) ) {
		DEBUG_OUT (1, ("User %s in passdb, but getpwnam() fails!\n",
	...
	if (IS_DC && is_our_machine_account(username)) {
		...
		DEBUG_OUT (10, ("make_server_info_sam: our machine account %s "
			   "turning off winbindd requests.\n", username));
	}

	DEBUG_OUT (5,("make_server_info_sam: made server info for user %s -> %s\n",
