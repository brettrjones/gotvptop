...
void attempt_machine_password_change(void)
{
	...
	if (lock == NULL) {
		DEBUG_OUT (0,("attempt_machine_password_change: unable to lock "
	...
	if(!secrets_fetch_trust_account_password(lp_workgroup(),
		...
		DEBUG_OUT (0,("attempt_machine_password_change: unable to read the "
						...
						struct rpc_pipe_client **pipe_ret)
{
	...
	*pipe_ret = NULL;
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG_OUT (0,("connect_to_domain_password_server: unable to open the domain client session to \
machine %s. Error was : %s.\n", dc_name, nt_errstr(result)));
	...
	if (!lp_client_schannel()) {
		...
		if (!get_trust_pw_hash(domain, machine_pwd, &account_name,
				       &sec_chan_type))
		{
			DEBUG_OUT (0, ("connect_to_domain_password_server: could not fetch "
...

{
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		DEBUG_OUT (0,("domain_client_validate: Domain password server not available.\n"));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("domain_client_validate: unable to validate password "
					...
					struct auth_serversupplied_info **server_info)
{
	...
	if ( lp_server_role() != ROLE_DOMAIN_MEMBER ) {
		DEBUG_OUT (0,("check_ntdomain_security: Configuration error!  Cannot use "
			"ntdomain auth method when not a member of a domain.\n"));
		return NT_STATUS_NOT_IMPLEMENTED;
	}

	if (!user_info || !server_info || !auth_context) {
		DEBUG_OUT (1,("check_ntdomain_security: Critical variables not present.  Failing.\n"));
	...
	if(strequal(get_global_sam_name(), user_info->mapped.domain_name)) {
		DEBUG_OUT (3,("check_ntdomain_security: Requested domain was for this machine.\n"));
		return NT_STATUS_NOT_IMPLEMENTED;
	}
	...
	if ( !get_dc_name(domain, NULL, dc_name, &dc_ss) ) {
		DEBUG_OUT (5,("check_ntdomain_security: unable to locate a DC for domain %s\n",
					   ...
					   struct auth_serversupplied_info **server_info)
{
	...
	if (!user_info || !server_info || !auth_context) {
		DEBUG_OUT (1,("check_trustdomain_security: Critical variables not present.  Failing.\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if( strequal(get_global_sam_name(), user_info->mapped.domain_name)) {
		DEBUG_OUT (3,("check_trustdomain_security: Requested domain [%s] was for this machine.\n",
			user_info->mapped.domain_name));
		return NT_STATUS_NOT_IMPLEMENTED;
	}
	...
	if (!pdb_get_trusteddom_pw(user_info->mapped.domain_name, &trust_password,
				   NULL, NULL)) {
		DEBUG_OUT (0, ("check_trustdomain_security: could not fetch trust "
	...
	DEBUG_OUT (100, ("Trust password for domain %s is %s\n", user_info->mapped.domain_name,
		    trust_password));
#endif
	...
	if ( !get_dc_name(user_info->mapped.domain_name, NULL, dc_name, &dc_ss) ) {
		DEBUG_OUT (5,("check_trustdomain_security: unable to locate a DC for domain %s\n",
			user_info->mapped.domain_name));
		return NT_STATUS_NO_LOGON_SERVERS;
	}
