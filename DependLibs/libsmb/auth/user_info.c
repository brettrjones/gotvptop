			...
			enum auth_password_state password_state)
{
	...
	*ret_user_info = NULL;
	...
	DEBUG_OUT (5,("attempting to make a user_info for %s (%s)\n", internal_username, smb_name));
	...
	if (user_info == NULL) {
		DEBUG_OUT (0,("talloc failed for user_info\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG_OUT (5,("making strings for %s's user_info struct\n", internal_username));
	...
	DEBUG_OUT (5,("making blobs for %s's user_info struct\n", internal_username));
	...
	DEBUG_OUT (10,("made a user_info for %s (%s)\n", internal_username, smb_name));
