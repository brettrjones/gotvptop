				    ...
				    struct auth_serversupplied_info **server_info)
{
	...
	switch ( lp_server_role() ) {
		...
		case ROLE_DOMAIN_MEMBER:
			if ( !is_local_name ) {
				DEBUG_OUT (6,("check_samstrict_security: %s is not one of my local names (%s)\n",
		...
		case ROLE_DOMAIN_BDC:
			if ( !is_local_name && !is_my_domain ) {
				DEBUG_OUT (6,("check_samstrict_security: %s is not one of my local names or domain name (DC)\n",
