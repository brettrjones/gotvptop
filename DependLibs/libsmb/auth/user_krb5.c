				     ...
				     struct passwd **_pw)
{
	...
	DEBUG_OUT (3, ("Kerberos ticket principal name is [%s]\n", princ_name));

	p = strchr_m(princ_name, '@');
	if (!p) {
		DEBUG_OUT (3, ("[%s] Doesn't look like a valid principal\n",
			  princ_name));
	...
	if (!strequal(realm, lp_realm())) {
		DEBUG_OUT (3, ("Ticket for foreign realm %s@%s\n", user, realm));
	...
	if (logon_info && logon_info->info3.base.domain.string) {
		...
		DEBUG_OUT (10, ("Domain is [%s] (using PAC)\n", domain));
	} else {
		...
		DEBUG_OUT (10, ("Mapping [%s] to short name using winbindd\n",
			   realm));
		...
		if (WBC_ERROR_IS_OK(wbc_status)) {
			...
			DEBUG_OUT (3, ("Could not find short name: %s\n",
		...
		DEBUG_OUT (10, ("Domain is [%s] (using Winbind)\n", domain));
	}
	...
	*is_mapped = map_username(mem_ctx, fuser, &fuser);
	...
	if (pw) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("PAM account restrictions prevent user "
				  "[%s] login\n", unixuser));
			return status;
		}
	}
	if (!pw) {
		...
		if (!pw) {
			DEBUG_OUT (1, ("Username %s is invalid on this system\n",
				  fuser));
			return NT_STATUS_LOGON_FAILURE;
		}
	}
				...
				struct auth_serversupplied_info **server_info)
{
	...
	if (mapped_to_guest) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("make_server_info_guest failed: %s!\n",
				  nt_errstr(status)));
			return status;
		}
		...
		status = make_server_info_info3(mem_ctx,
						...
						&logon_info->info3);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("make_server_info_info3 failed: %s!\n",
				  nt_errstr(status)));
			return status;
		}
		...
		if (pdb_getsampwnam(sampass, username)) {
			DEBUG_OUT (10, ("found user %s in passdb, calling "
			 ...
			 */
			DEBUG_OUT (10, ("didn't find user %s in passdb, calling "
				   "make_server_info_pw\n", username));
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (1, ("make_server_info_[sam|pw] failed: %s!\n",
				  nt_errstr(status)));
			return status;
                }
