...
static bool smb_pam_error_handler(pam_handle_t *pamh, int pam_error, const char *msg, int dbglvl)
{
	...
	if( pam_error != PAM_SUCCESS) {
		DEBUG_OUT (dbglvl, ("smb_pam_error_handler: PAM: %s : %s\n",
					    ...
					    NTSTATUS *nt_status)
{
	*nt_status = pam_to_nt_status(pam_error);
	...
	if (NT_STATUS_IS_OK(*nt_status)) {
		/* Complain LOUDLY */
		DEBUG_OUT (0, ("smb_pam_nt_status_error_handler: PAM: BUG: PAM and NT_STATUS \
		    ...
		    void *appdata_ptr)
{
	...
	*resp = NULL;
	...
	if (udp == NULL) {
		DEBUG_OUT (0,("smb_pam_conv: PAM on this system is broken - appdata_ptr == NULL !\n"));
		return PAM_CONV_ERR;
	}
...
static struct chat_struct *make_pw_chat(const char *p) 
{
	...
	while (1) {
		...
		if (!t) {
			DEBUG_OUT (0,("make_pw_chat: malloc failed!\n"));
			TALLOC_FREE(frame);
			return NULL;
		}
				...
				void *appdata_ptr)
{
	...
	*resp = NULL;
	...
	DEBUG_OUT (10,("smb_pam_passchange_conv: starting converstation for %d messages\n", num_msg));
	...
	if (udp == NULL) {
		DEBUG_OUT (0,("smb_pam_passchange_conv: PAM on this system is broken - appdata_ptr == NULL !\n"));
	...
	reply = SMB_MALLOC_ARRAY(struct pam_response, num_msg);
	if (!reply) {
		DEBUG_OUT (0,("smb_pam_passchange_conv: malloc for reply failed!\n"));
		free_pw_chat(pw_chat);
		return PAM_CONV_ERR;
	}
	...
	for (replies = 0; replies < num_msg; replies++) {
		found = False;
		DEBUG_OUT (10,("smb_pam_passchange_conv: Processing message %d\n", replies));
		switch (msg[replies]->msg_style) {
		case PAM_PROMPT_ECHO_ON:
			DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_ON: PAM said: %s\n", msg[replies]->msg));
			fstrcpy(current_prompt, msg[replies]->msg);
			trim_char(current_prompt, ' ', ' ');
			for (t=pw_chat; t; t=t->next) {

				DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_ON: trying to match |%s| to |%s|\n",
						t->prompt, current_prompt ));

				if (unix_wild_match(t->prompt, current_prompt)) {
					fstrcpy(current_reply, t->reply);
					DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_ON: We sent: %s\n", current_reply));
					pwd_sub(current_reply, udp->PAM_username, udp->PAM_password, udp->PAM_newpassword);
#ifdef DEBUG_PASSWORD
					DEBUG_OUT (100,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_ON: We actually sent: %s\n", current_reply));
#endif
					reply[replies].resp_retcode = PAM_SUCCESS;
			...
			/* PAM frees resp */
			if (!found) {
				DEBUG_OUT (3,("smb_pam_passchange_conv: Could not find reply for PAM prompt: %s\n",msg[replies]->msg));
				free_pw_chat(pw_chat);
				SAFE_FREE(reply);
				return PAM_CONV_ERR;
			}
		...
		case PAM_PROMPT_ECHO_OFF:
			DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_OFF: PAM said: %s\n", msg[replies]->msg));
			fstrcpy(current_prompt, msg[replies]->msg);
			trim_char(current_prompt, ' ', ' ');
			for (t=pw_chat; t; t=t->next) {

				DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_OFF: trying to match |%s| to |%s|\n",
						t->prompt, current_prompt ));

				if (unix_wild_match(t->prompt, current_prompt)) {
					fstrcpy(current_reply, t->reply);
					DEBUG_OUT (10,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_OFF: We sent: %s\n", current_reply));
					pwd_sub(current_reply, udp->PAM_username, udp->PAM_password, udp->PAM_newpassword);
					reply[replies].resp_retcode = PAM_SUCCESS;
					reply[replies].resp = smb_pam_copy_fstring(
						current_reply);
#ifdef DEBUG_PASSWORD
					DEBUG_OUT (100,("smb_pam_passchange_conv: PAM_PROMPT_ECHO_OFF: We actually sent: %s\n", current_reply));
#endif
					found = True;
					break;
				}
...
static bool smb_pam_end(pam_handle_t *pamh, struct pam_conv *smb_pam_conv_ptr)
{
	...
	if( pamh != NULL ) {
		...
		if(smb_pam_error_handler(pamh, pam_error, "End Cleanup Failed", 2) == True) {
			DEBUG_OUT (4, ("smb_pam_end: PAM: PAM_END OK.\n"));
			return True;
		}
	}
	DEBUG_OUT (2,("smb_pam_end: PAM: not initialised"));
	return False;
}
...
static bool smb_pam_start(pam_handle_t **pamh, const char *user, const char *rhost, struct pam_conv *pconv)
{
	...
	*pamh = (pam_handle_t *)NULL;
	...
	DEBUG_OUT (4,("smb_pam_start: PAM: Init user: %s\n", user));
...
#if HAVE_PAM_RHOST
	DEBUG_OUT (4,("smb_pam_start: PAM: setting rhost to: %s\n", rhost));
...
#endif
#if HAVE_PAM_TTY
	DEBUG_OUT (4,("smb_pam_start: PAM: setting tty\n"));
...
#endif
	DEBUG_OUT (4,("smb_pam_start: PAM: Init passed for user: %s\n", user));
	return True;
}
...
static NTSTATUS smb_pam_auth(pam_handle_t *pamh, const char *user)
{
	...
	DEBUG_OUT (4,("smb_pam_auth: PAM: Authenticate User: %s\n", user));
	pam_error = pam_authenticate(pamh, PAM_SILENT | lp_null_passwords() ? 0 : PAM_DISALLOW_NULL_AUTHTOK);
	switch( pam_error ){
		case PAM_AUTH_ERR:
			DEBUG_OUT (2, ("smb_pam_auth: PAM: Authentication Error for user %s\n", user));
			break;
		case PAM_CRED_INSUFFICIENT:
			DEBUG_OUT (2, ("smb_pam_auth: PAM: Insufficient Credentials for user %s\n", user));
			break;
		case PAM_AUTHINFO_UNAVAIL:
			DEBUG_OUT (2, ("smb_pam_auth: PAM: Authentication Information Unavailable for user %s\n", user));
			break;
		case PAM_USER_UNKNOWN:
			DEBUG_OUT (2, ("smb_pam_auth: PAM: Username %s NOT known to Authentication system\n", user));
			break;
		case PAM_MAXTRIES:
			DEBUG_OUT (2, ("smb_pam_auth: PAM: One or more authentication modules reports user limit for user %s exceeeded\n", user));
			break;
		case PAM_ABORT:
			DEBUG_OUT (0, ("smb_pam_auth: PAM: One or more PAM modules failed to load for user %s\n", user));
			break;
		case PAM_SUCCESS:
			DEBUG_OUT (4, ("smb_pam_auth: PAM: User %s Authenticated OK\n", user));
			break;
		default:
			DEBUG_OUT (0, ("smb_pam_auth: PAM: UNKNOWN ERROR while authenticating user %s\n", user));
			break;
	}
...
static NTSTATUS smb_pam_account(pam_handle_t *pamh, const char * user)
{
	...
	DEBUG_OUT (4,("smb_pam_account: PAM: Account Management for User: %s\n", user));
	pam_error = pam_acct_mgmt(pamh, PAM_SILENT); /* Is user account enabled? */
	switch( pam_error ) {
		case PAM_AUTHTOK_EXPIRED:
			DEBUG_OUT (2, ("smb_pam_account: PAM: User %s is valid but password is expired\n", user));
			break;
		case PAM_ACCT_EXPIRED:
			DEBUG_OUT (2, ("smb_pam_account: PAM: User %s no longer permitted to access system\n", user));
			break;
		case PAM_AUTH_ERR:
			DEBUG_OUT (2, ("smb_pam_account: PAM: There was an authentication error for user %s\n", user));
			break;
		case PAM_PERM_DENIED:
			DEBUG_OUT (0, ("smb_pam_account: PAM: User %s is NOT permitted to access system at this time\n", user));
			break;
		case PAM_USER_UNKNOWN:
			DEBUG_OUT (0, ("smb_pam_account: PAM: User \"%s\" is NOT known to account management\n", user));
			break;
		case PAM_SUCCESS:
			DEBUG_OUT (4, ("smb_pam_account: PAM: Account OK for User: %s\n", user));
			break;
		default:
			DEBUG_OUT (0, ("smb_pam_account: PAM: UNKNOWN PAM ERROR (%d) during Account Management for User: %s\n", pam_error, user));
			break;
	}
...
static NTSTATUS smb_pam_setcred(pam_handle_t *pamh, const char * user)
{
	...
	DEBUG_OUT (4,("PAM: Account Management SetCredentials for User: %s\n", user));
	pam_error = pam_setcred(pamh, (PAM_ESTABLISH_CRED|PAM_SILENT)); 
	switch( pam_error ) {
		case PAM_CRED_UNAVAIL:
			DEBUG_OUT (0, ("smb_pam_setcred: PAM: Credentials not found for user:%s\n", user ));
			break;
		case PAM_CRED_EXPIRED:
			DEBUG_OUT (0, ("smb_pam_setcred: PAM: Credentials for user: \"%s\" EXPIRED!\n", user ));
			break;
		case PAM_USER_UNKNOWN:
			DEBUG_OUT (0, ("smb_pam_setcred: PAM: User: \"%s\" is NOT known so can not set credentials!\n", user ));
			break;
		case PAM_CRED_ERR:
			DEBUG_OUT (0, ("smb_pam_setcred: PAM: Unknown setcredentials error - unable to set credentials for %s\n", user ));
			break;
		case PAM_SUCCESS:
			DEBUG_OUT (4, ("smb_pam_setcred: PAM: SetCredentials OK for User: %s\n", user));
			break;
		default:
			DEBUG_OUT (0, ("smb_pam_setcred: PAM: UNKNOWN PAM ERROR (%d) during SetCredentials for User: %s\n", pam_error, user));
			break;
	}
...
#if HAVE_PAM_TTY
	DEBUG_OUT (4,("smb_internal_pam_session: PAM: tty set to: %s\n", tty));
...
static bool smb_pam_chauthtok(pam_handle_t *pamh, const char * user)
{
	...
	DEBUG_OUT (4,("smb_pam_chauthtok: PAM: Password Change for User: %s\n", user));

	pam_error = pam_chauthtok(pamh, PAM_SILENT); /* Change Password */

	switch( pam_error ) {
	case PAM_AUTHTOK_ERR:
		DEBUG_OUT (2, ("PAM: unable to obtain the new authentication token - is password to weak?\n"));
		break;

	/* This doesn't seem to be defined on Solaris. JRA */
#ifdef PAM_AUTHTOK_RECOVER_ERR
	case PAM_AUTHTOK_RECOVER_ERR:
		DEBUG_OUT (2, ("PAM: unable to obtain the old authentication token - was the old password wrong?.\n"));
		break;
#endif

	case PAM_AUTHTOK_LOCK_BUSY:
		DEBUG_OUT (2, ("PAM: unable to change the authentication token since it is currently locked.\n"));
		break;
	case PAM_AUTHTOK_DISABLE_AGING:
		DEBUG_OUT (2, ("PAM: Authentication token aging has been disabled.\n"));
		break;
	case PAM_PERM_DENIED:
		DEBUG_OUT (0, ("PAM: Permission denied.\n"));
		break;
	case PAM_TRY_AGAIN:
		DEBUG_OUT (0, ("PAM: Could not update all authentication token(s). No authentication tokens were updated.\n"));
		break;
	case PAM_USER_UNKNOWN:
		DEBUG_OUT (0, ("PAM: User not known to PAM\n"));
		break;
	case PAM_SUCCESS:
		DEBUG_OUT (4, ("PAM: Account OK for User: %s\n", user));
		break;
	default:
		DEBUG_OUT (0, ("PAM: UNKNOWN PAM ERROR (%d) for User: %s\n", pam_error, user));
	}
...
NTSTATUS smb_pam_accountcheck(const char *user, const char *rhost)
{
	...
	if (!NT_STATUS_IS_OK(nt_status = smb_pam_account(pamh, user)))
		DEBUG_OUT (0, ("smb_pam_accountcheck: PAM: Account Validation Failed - Rejecting User %s!\n", user));
			   ...
			   const char * password)
{
	...
	if (!NT_STATUS_IS_OK(nt_status = smb_pam_auth(pamh, user))) {
		DEBUG_OUT (0, ("smb_pam_passcheck: PAM: smb_pam_auth failed - Rejecting User %s !\n", user));
		smb_pam_end(pamh, pconv);
		return nt_status;
	}

	if (!NT_STATUS_IS_OK(nt_status = smb_pam_account(pamh, user))) {
		DEBUG_OUT (0, ("smb_pam_passcheck: PAM: smb_pam_account failed - Rejecting User %s !\n", user));
		smb_pam_end(pamh, pconv);
		return nt_status;
	}

	if (!NT_STATUS_IS_OK(nt_status = smb_pam_setcred(pamh, user))) {
		DEBUG_OUT (0, ("smb_pam_passcheck: PAM: smb_pam_setcred failed - Rejecting User %s !\n", user));
		smb_pam_end(pamh, pconv);
		return nt_status;
	}
			...
			const char *oldpassword, const char *newpassword)
{
	...
	if (!smb_pam_chauthtok(pamh, user)) {
		DEBUG_OUT (0, ("smb_pam_passchange: PAM: Password Change Failed for user %s!\n", user));
		smb_pam_end(pamh, pconv);
		return False;
	}
