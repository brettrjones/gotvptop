...
static bool dfs_auth(char *user, char *password)
{
	...
	if (err != error_status_ok)
	{
		...
		DEBUG_OUT (0, ("DCE can't get current context. %s\n", dce_errstr));
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG_OUT (0, ("DCE can't get expiration. %s\n", dce_errstr));
	...
	if (expire_time < (current_time + 60))
	{
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG_OUT (0, ("DCE can't get pwent. %s\n", dce_errstr));
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG_OUT (0, ("DCE can't refresh identity. %s\n",
				  dce_errstr));
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG_OUT (0, ("DCE can't get key for %s. %s\n",
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG_OUT (0, ("DCE can't free key.\n", dce_errstr));
		}
	}
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG_OUT (0, ("DCE Setup Identity for %s failed: %s\n",
			  user, dce_errstr));
		return (False);
	}
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG_OUT (0, ("DCE can't purge context. %s\n", dce_errstr));
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG_OUT (0, ("DCE certify identity failed: %s\n", dce_errstr));
		goto err;
	}

	if (auth_src != sec_login_auth_src_network)
	{
		DEBUG_OUT (0, ("DCE context has no network credentials.\n"));
	}
	...
	DEBUG_OUT (0, ("DCE login succeeded for principal %s on pid %d\n",
		  user, sys_getpid()));

	DEBUG_OUT (3, ("DCE principal: %s\n"
		  "          uid: %d\n"
		  "          gid: %d\n",
	...
	if (t) {
		...
		if (asct) {
			DEBUG_OUT (0,("DCE context expires: %s", asct));
		}
	}
...
#ifndef HAVE_CRYPT
	DEBUG_OUT (1, ("Warning - no crypt available\n"));
		    ...
		    bool run_cracker)
{
	...
	DEBUG_OUT (100, ("checking user=[%s] pass=[%s]\n", user, password));
#endif
	...
	DEBUG_OUT (4, ("pass_check: Checking (PAM) password for user %s\n", user));

#else /* Not using PAM */

	DEBUG_OUT (4, ("pass_check: Checking password for user %s\n", user));

	if (!pass) {
		DEBUG_OUT (3, ("Couldn't find user %s\n", user));
...
#ifdef OSF1_ENH_SEC
	{
		...
		DEBUG_OUT (5, ("Checking password for user %s in OSF1_ENH_SEC\n",
			  user));
	...
	if (!get_this_crypted() || !*get_this_crypted()) {
		if (!lp_null_passwords()) {
			DEBUG_OUT (2, ("Disallowing %s with null password\n",
				  get_this_user()));
			return NT_STATUS_LOGON_FAILURE;
		}
		if (!*password) {
