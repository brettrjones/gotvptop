	...
	const struct netlogon_creds_CredentialState *creds)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (10, ("ndr_push_netlogon_creds_CredentialState failed: "
			   "%s\n", ndr_errstr(ndr_err)));
		return false;
	}
...
secrets_fetch_local_schannel_creds(TALLOC_CTX *mem_ctx)
{
	...
	blob.data = (uint8_t *)secrets_fetch(SECRETS_LOCAL_SCHANNEL_KEY,
					     &blob.length);
	if (blob.data == NULL) {
		DEBUG_OUT (10, ("secrets_fetch failed\n"));
		return NULL;
	}
	...
	SAFE_FREE(blob.data);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG_OUT (10, ("ndr_pull_netlogon_creds_CredentialState failed: "
			   "%s\n", ndr_errstr(ndr_err)));
		TALLOC_FREE(creds);
		return NULL;
	}
				   ...
				   NTSTATUS *schannel_bind_result)
{
	...
	*schannel_bind_result = NT_STATUS_OK;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("rpc_pipe_open_ncalrpc failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
	...
	status = rpccli_schannel_bind_data(p, lp_workgroup(),
					   ...
					   p->dc, &auth);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("rpccli_schannel_bind_data failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(p);
		return status;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("rpc_pipe_bind failed: %s\n", nt_errstr(status)));
		TALLOC_FREE(p);
	...
	DEBUG_OUT (10, ("rpccli_netlogon_sam_network_logon_ex returned %s\n",
		   nt_errstr(status)));
...
static NTSTATUS get_ldapi_ctx(TALLOC_CTX *mem_ctx, struct tldap_context **pld)
{
	...
	if (res != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("tldap_fetch_rootdse failed: %s\n",
...
static NTSTATUS mymachinepw(uint8_t pwd[16])
{
	...
	if (rootdse == NULL) {
		DEBUG_OUT (10, ("Could not get rootdse\n"));
	...
	if (rc != TLDAP_SUCCESS) {
		DEBUG_OUT (10, ("Could not retrieve our account: %s\n",
	...
	if (num_msg != 1) {
		DEBUG_OUT (10, ("Got %d accounts, expected one\n", num_msg));
	...
	if (!tldap_get_single_valueblob(msg[0], "unicodePwd", &pwdblob)) {
		...
		DEBUG_OUT (10, ("No unicodePwd attribute in %s\n",
			   dn ? dn : "<unknown DN>"));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto fail;
	}
	if (pwdblob.length != 16) {
		DEBUG_OUT (10, ("Password hash hash has length %d, expected 16\n",
			   (int)pwdblob.length));
		status = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto fail;
	}
					 ...
					 struct auth_serversupplied_info **server_info)
{
	...
	DEBUG_OUT (10, ("netlogond_validate returned %s\n", nt_errstr(status)));
 ...
 new_key:
	...
	if (mutex == NULL) {
		DEBUG_OUT (10, ("Could not get mutex LOCAL_SCHANNEL_KEY\n"));
		status = NT_STATUS_ACCESS_DENIED;
		goto done;
	}

	DEBUG_OUT (10, ("schannel bind failed, setting up new key\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("rpccli_anon_bind_data failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("mymachinepw failed: %s\n", nt_errstr(status)));
		goto done;
	}

	DEBUG_OUT (10, ("machinepw "));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("rpccli_netlogon_setup_creds failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
 ...
 okay:
	...
	status = make_server_info_info3(mem_ctx, user_info->client.account_name,
					...
					info3);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("make_server_info_info3 failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(frame);
		return status;
	}
