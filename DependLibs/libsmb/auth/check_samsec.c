	...
	*lm_sess_key = data_blob_null;
	...
	if (acct_ctrl & ACB_PWNOTREQ) {
		if (lp_null_passwords()) {
			DEBUG_OUT (3,("Account for user '%s' has no password and null passwords are allowed.\n", username));
			return NT_STATUS_OK;
		} else {
			DEBUG_OUT (3,("Account for user '%s' has no password and null passwords are NOT allowed.\n", username));
...
static bool logon_hours_ok(struct samu *sampass)
{
	...
	if (!hours) {
		DEBUG_OUT (5,("logon_hours_ok: No hours restrictions for user %s\n",pdb_get_username(sampass)));
		return True;
	}
	...
	if (!utctime) {
		DEBUG_OUT (1, ("logon_hours_ok: failed to get gmtime. Failing logon for user %s\n",
			pdb_get_username(sampass) ));
		return False;
	}
	...
	if (! (hours[bitpos/8] & bitmask)) {
		...
		DEBUG_OUT (1, ("logon_hours_ok: Account for user %s not allowed to "
	...
	DEBUG_OUT (5,("logon_hours_ok: user %s allowed to logon at this time (%s)\n",
			       ...
			       const struct auth_usersupplied_info *user_info)
{
	...
	DEBUG_OUT (4,("sam_account_ok: Checking SMB password for user %s\n",pdb_get_username(sampass)));

	/* Quit if the account was disabled. */
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG_OUT (1,("sam_account_ok: Account for user '%s' was disabled.\n", pdb_get_username(sampass)));
		return NT_STATUS_ACCOUNT_DISABLED;
	}

	/* Quit if the account was locked out. */
	if (acct_ctrl & ACB_AUTOLOCK) {
		DEBUG_OUT (1,("sam_account_ok: Account for user %s was locked out.\n", pdb_get_username(sampass)));
	...
	if (kickoff_time != 0 && time(NULL) > kickoff_time) {
		DEBUG_OUT (1,("sam_account_ok: Account for user '%s' has expired.\n", pdb_get_username(sampass)));
		DEBUG_OUT (3,("sam_account_ok: Account expired at '%ld' unix time.\n", (long)kickoff_time));
	...
	if (!(pdb_get_acct_ctrl(sampass) & ACB_PWNOEXP) && !(pdb_get_acct_ctrl(sampass) & ACB_PWNOTREQ)) {
		...
		if (((acct_ctrl & (ACB_WSTRUST|ACB_SVRTRUST)) == 0) && (last_set_time == 0)) {
			DEBUG_OUT (1,("sam_account_ok: Account for user '%s' password must change!\n", pdb_get_username(sampass)));
			return NT_STATUS_PASSWORD_MUST_CHANGE;
		}

		/* check for expired password */
		if (must_change_time < time(NULL) && must_change_time != 0) {
			DEBUG_OUT (1,("sam_account_ok: Account for user '%s' password expired!\n", pdb_get_username(sampass)));
			DEBUG_OUT (1,("sam_account_ok: Password expired at '%s' (%ld) unix time.\n", http_timestring(talloc_tos(), must_change_time), (long)must_change_time));
	...
	if (*workstation_list) {
		...
		while (next_token_talloc(mem_ctx, &s, &tok, ",")) {
			DEBUG_OUT (10,("sam_account_ok: checking for workstation match %s and %s\n",
			...
			if (tok[0] == '+') {
				DEBUG_OUT (10,("sam_account_ok: checking for workstation %s in group: %s\n",
					machine_name, tok + 1));
	...
	if (acct_ctrl & ACB_DOMTRUST) {
		DEBUG_OUT (2,("sam_account_ok: Domain trust account %s denied by server\n", pdb_get_username(sampass)));
		return NT_STATUS_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT;
	}

	if (acct_ctrl & ACB_SVRTRUST) {
		if (!(user_info->logon_parameters & MSV1_0_ALLOW_SERVER_TRUST_ACCOUNT)) {
			DEBUG_OUT (2,("sam_account_ok: Server trust account %s denied by server\n", pdb_get_username(sampass)));
	...
	if (acct_ctrl & ACB_WSTRUST) {
		if (!(user_info->logon_parameters & MSV1_0_ALLOW_WORKSTATION_TRUST_ACCOUNT)) {
			DEBUG_OUT (2,("sam_account_ok: Wksta trust account %s denied by server\n", pdb_get_username(sampass)));
			    ...
			    struct auth_serversupplied_info **server_info)
{
	...
	if (ret == False) {
		DEBUG_OUT (3,("check_sam_security: Couldn't find user '%s' in "
	...
	if (pdb_get_acct_ctrl(sampass) & ACB_AUTOLOCK) {
		DEBUG_OUT (3,("check_sam_security: Account for user %s was locked out.\n", username));
		return NT_STATUS_ACCOUNT_LOCKED_OUT;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		...
		if (updated_badpw){
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG_OUT (1, ("Failed to modify entry: %s\n",
					  nt_errstr(status)));
			}
		}
		goto done;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (0,("check_sam_security: make_server_info_sam() failed with '%s'\n", nt_errstr(nt_status)));
		goto done;
	}
				  ...
				  struct netr_SamInfo3 **pinfo3)
{
	...
	status = check_sam_security(challenge, talloc_tos(), user_info,
				    &server_info);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("check_sam_security failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("serverinfo_to_SamInfo3 failed: %s\n",
			   nt_errstr(status)));
		goto done;
	}
	*pinfo3 = info3;
