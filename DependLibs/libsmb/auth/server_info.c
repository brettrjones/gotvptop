			  ...
			  struct extra_auth_info *extra)
{
	...
	if (!user_sid || !group_sid) {
		DEBUG_OUT (1, ("Sam account is missing sids!\n"));
	...
	if (sid_check_is_in_unix_users(user_sid)) {
		...
		DEBUG_OUT (10, ("Unix User found in struct samu. Rid marked as "
	...
	if (sid_check_is_in_unix_groups(group_sid)) {
		...
		DEBUG_OUT (10, ("Unix Group found in struct samu. Rid marked as "
			   "special and sid (%s) saved as extra sid\n",
		...
		if (!ok) {
			DEBUG_OUT (1, ("The primary group domain sid(%s) does not "
	...
	status = pdb_enum_group_memberships(mem_ctx, samu,
					    ...
					    &num_group_sids);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (1, ("Failed to get groups from sam account.\n"));
		TALLOC_FREE(info3);
		return status;
	}
					...
					const struct wbcAuthUserInfo *info)
{
	...
	ok = sid_peek_check_rid(&domain_sid, &group_sid,
				&info3->base.primary_gid);
	if (!ok) {
		DEBUG_OUT (1, ("The primary group sid domain does not"
