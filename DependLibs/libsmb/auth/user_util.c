...
bool user_in_netgroup(TALLOC_CTX *ctx, const char *user, const char *ngname)
{
	...
	if (my_yp_domain == NULL) {
		DEBUG_OUT (5,("Unable to get default yp domain, "
			"let's try without specifying it\n"));
	}

	DEBUG_OUT (5,("looking for user %s of domain %s in netgroup %s\n",
		user, my_yp_domain?my_yp_domain:"(ANY)", ngname));

	if (innetgr(ngname, NULL, user, my_yp_domain)) {
		DEBUG_OUT (5,("user_in_netgroup: Found\n"));
		return true;
	}
...
bool user_in_list(TALLOC_CTX *ctx, const char *user,const char **list)
{
	...
	DEBUG_OUT (10,("user_in_list: checking user %s in list\n", user));

	while (*list) {

		DEBUG_OUT (10,("user_in_list: checking user |%s| against |%s|\n",
			  user, *list));
...
bool map_username(TALLOC_CTX *ctx, const char *user_in, char **p_user_out)
{
	...
	*p_user_out = talloc_strdup(ctx, user_in);
	...
	if (strequal(user_in,get_last_from())) {
		DEBUG_OUT (3,("Mapped user %s to %s\n",user_in,get_last_to()));
		TALLOC_FREE(*p_user_out);
	...
	if (!f) {
		DEBUG_OUT (0,("can't open username map %s. Error %s\n",mapfile, strerror(errno) ));
		return False;
	}

	DEBUG_OUT (4,("Scanning username map %s\n",mapfile));
	...
	while((s=fgets_slash(buf,sizeof(buf),f))!=NULL) {
		...
		*dosname++ = 0;
		...
		if (!dosuserlist) {
			DEBUG_OUT (0,("Bad username map entry.  Unable to build user list.  Ignoring.\n"));
			continue;
		}
