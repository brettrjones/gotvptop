			    ...
			    enum auth_password_state password_state)
{
	...
	DEBUG_OUT (5, ("Mapping user [%s]\\[%s] from workstation [%s]\n",
	    ...
	    !strequal(domain, my_sam_name()))
	{
		...
		DEBUG_OUT (5, ("Mapped domain from [%s] to [%s] for user [%s] from "
			  "workstation [%s]\n",
					 ...
					 const uchar *dc_sess_key)
{
	...
	DEBUG_OUT (100,("lm owf password:"));
	dump_data(100, lm_pwd.hash, sizeof(lm_pwd.hash));

	DEBUG_OUT (100,("nt owf password:"));
...
#ifdef DEBUG_PASSWORD
	DEBUG_OUT (100,("decrypt of lm owf password:"));
	dump_data(100, lm_pwd.hash, sizeof(lm_pwd));

	DEBUG_OUT (100,("decrypt of nt owf password:"));
			      ...
			      DATA_BLOB plaintext_password)
{
	...
	DEBUG_OUT (5,("make_user_info_for_reply: User passwords not in encrypted "
		 "format.\n"));
	if (plaintext_password.data && plaintext_password.length) {
		unsigned char local_lm_response[24];

#ifdef DEBUG_PASSWORD
		DEBUG_OUT (10,("Unencrypted password (len %d):\n",
...
static NTSTATUS log_nt_token(struct security_token *token)
{
	...
	DEBUG_OUT (8, ("running command: [%s]\n", command));
	if (smbrun(command, NULL) != 0) {
		DEBUG_OUT (0, ("Could not log NT token\n"));
		TALLOC_FREE(frame);
...
NTSTATUS create_local_token(struct auth_serversupplied_info *server_info)
{
	...
	for (i=1; i<t->num_sids; i++) {
		...
		if (ids[i].type != WBC_ID_TYPE_GID) {
			DEBUG_OUT (10, ("Could not convert SID %s to gid, "
				   "ignoring it\n",
			     ...
			     struct passwd *pwd)
{
	...
	TALLOC_FREE(sampass);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (10, ("Failed to convert samu to info3: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(result);
		return status;
	}
				...
				struct netr_SamInfo3 *info3)
{
	...
	if (pwd == NULL) {
		DEBUG_OUT (0,("SamInfo3_for_guest: Unable to locate guest "
			 "account [%s]!\n", guest_account));
		return NT_STATUS_NO_SUCH_USER;
	}
						 ...
						 struct auth_serversupplied_info **server_info)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("Failed creating system info3 with %s\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	status = make_server_info_info3(mem_ctx,
					...
					&info3);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("make_server_info_info3 failed with %s\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	status = create_local_token(*server_info);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG_OUT (0, ("create_local_token failed: %s\n",
			  nt_errstr(status)));
		goto done;
	}
			      ...
			      bool *username_was_mapped)
{
	...
	*username_was_mapped = map_username(mem_ctx, orig_dom_user, &dom_user);
	...
	if (!passwd) {
		DEBUG_OUT (3, ("Failed to find authenticated user %s via "
				...
				struct netr_SamInfo3 *info3)
{
	...
	*/
	...
	result = make_server_info(NULL);
	if (result == NULL) {
		DEBUG_OUT (4, ("make_server_info failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}
...
bool is_trusted_domain(const char* dom_name)
{
	...
	if ( IS_DC ) {
		become_root();
		DEBUG_OUT (5,("is_trusted_domain: Checking for domain trust with "
			  "[%s]\n", dom_name ));
