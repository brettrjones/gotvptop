...
static struct cli_state *server_cryptkey(TALLOC_CTX *mem_ctx)
{
	...
	p = pserver;
		...
		if(!resolve_name( desthost, &dest_ss, 0x20, false)) {
			DEBUG_OUT (1,("server_cryptkey: Can't resolve address for %s\n",desthost));
			continue;
		}

		if (ismyaddr((struct sockaddr *)&dest_ss)) {
			DEBUG_OUT (1,("Password server loop - disabling password server %s\n",desthost));
			continue;
		}
		...
		if (NT_STATUS_IS_OK(status)) {
			DEBUG_OUT (3,("connected to password server %s\n",desthost));
			connected_ok = True;
			break;
		}
		DEBUG_OUT (10,("server_cryptkey: failed to connect to server %s. Error %s\n",
			desthost, nt_errstr(status) ));
		TALLOC_FREE(mutex);
	}

	if (!connected_ok) {
		DEBUG_OUT (0,("password server not available\n"));
		cli_shutdown(cli);
		return NULL;
	}
	...
	if (!attempt_netbios_session_request(&cli, global_myname(),
					     desthost, &dest_ss)) {
		TALLOC_FREE(mutex);
		DEBUG_OUT (1,("password server fails session request\n"));
		cli_shutdown(cli);
		return NULL;
	}
	...
	DEBUG_OUT (3,("got session\n"));

	status = cli_negprot(cli);

	if (!NT_STATUS_IS_OK(status)) {
		TALLOC_FREE(mutex);
		DEBUG_OUT (1, ("%s rejected the negprot: %s\n",
			  desthost, nt_errstr(status)));
		cli_shutdown(cli);
		return NULL;
	}
	...
	if (cli->protocol < PROTOCOL_LANMAN2 ||
		...
		TALLOC_FREE(mutex);
		DEBUG_OUT (1,("%s isn't in user level security mode\n",desthost));
		cli_shutdown(cli);
		return NULL;
	}
	...
	*/
	...
	if (!NT_STATUS_IS_OK(status)) {
		TALLOC_FREE(mutex);
		DEBUG_OUT (0,("%s rejected the initial session setup (%s)\n",
			 desthost, nt_errstr(status)));
		cli_shutdown(cli);
		return NULL;
	}
	...
	DEBUG_OUT (3,("password server OK\n"));
				  ...
				  void *private_data)
{
	...
	DEBUG_OUT ( 2, ( "send_server_keepalive: password server keepalive "
		    "failed.\n"));
...
static struct server_security_state *make_server_security_state(struct cli_state *cli)
{
	...
	if (lp_keepalive() != 0) {
		...
		if (event_add_idle(server_event_context(), result, interval,
				   ...
				   result) == NULL) {
			DEBUG_OUT (0, ("event_add_idle failed\n"));
			TALLOC_FREE(result);
			return NULL;
		}
	}
					   ...
					   TALLOC_CTX *mem_ctx)
{
	...
	if (cli) {
		DEBUG_OUT (3,("using password server validation\n"));

		if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0) {
			/* We can't work with unencrypted password servers
			   unless 'encrypt passwords = no' */
			DEBUG_OUT (5,("make_auth_info_server: Server is unencrypted, no challenge available..\n"));
			...
			*my_private_data =
			...
			DEBUG_OUT (2,("make_auth_info_server: Didn't receive a full challenge from server\n"));
			cli_shutdown(cli);
			return data_blob_null;
		}
					 ...
					 struct auth_serversupplied_info **server_info)
{
	...
	DEBUG_OUT (10, ("check_smbserver_security: Check auth for: [%s]\n",
		user_info->mapped.account_name));

	if (my_private_data == NULL) {
		DEBUG_OUT (10,("check_smbserver_security: "
	...
	if (!cli || !cli->initialised) {
		DEBUG_OUT (1,("password server is not connected (cli not initialised)\n"));
		return NT_STATUS_LOGON_FAILURE;
	}  

	if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0) {
		if (user_info->password_state != AUTH_PASSWORD_PLAIN) {
			DEBUG_OUT (1,("password server %s is plaintext, but we are encrypted. This just can't work :-(\n", cli->desthost));
			return NT_STATUS_LOGON_FAILURE;		
		}
	} else {
		if (memcmp(cli->secblob.data, auth_context->challenge.data, 8) != 0) {
			DEBUG_OUT (1,("the challenge that the password server (%s) supplied us is not the one we gave our client. This just can't work :-(\n", cli->desthost));
			return NT_STATUS_LOGON_FAILURE;		
		}
	}
	...
	if ((!tested_password_server) && (lp_paranoid_server_security())) {
		...
		if (NT_STATUS_IS_OK(cli_session_setup(cli, baduser,
			...
			if ((SVAL(cli->inbuf,smb_vwv2) & 1) == 0) {
				DEBUG_OUT (0,("server_validate: password server %s allows users as non-guest \
with a bad password.\n", cli->desthost));
				DEBUG_OUT (0,("server_validate: This is broken (and insecure) behaviour. Please do not \
use this machine as the password server.\n"));
		...
		if(bad_password_server) {
			DEBUG_OUT (0,("server_validate: [1] password server %s allows users as non-guest \
with a bad password.\n", cli->desthost));
			DEBUG_OUT (0,("server_validate: [1] This is broken (and insecure) behaviour. Please do not \
use this machine as the password server.\n"));
			return NT_STATUS_LOGON_FAILURE;
		}
	}
	...
	case AUTH_PASSWORD_RESPONSE:
		...
		break;
	default:
		DEBUG_OUT (0,("user_info constructed for user '%s' was invalid - password_state=%u invalid.\n",user_info->mapped.account_name, user_info->password_state));
		nt_status = NT_STATUS_INTERNAL_ERROR;
	}

	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG_OUT (1,("password server %s rejected the password: %s\n",
			 cli->desthost, nt_errstr(nt_status)));
	}

	/* if logged in as guest then reject */
	if (cli->is_guestlogin) {
		DEBUG_OUT (1,("password server %s gave us guest only\n", cli->desthost));
