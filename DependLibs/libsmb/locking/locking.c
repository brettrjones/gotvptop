...
bool strict_lock_default(files_struct *fsp, struct lock_struct *plock)
{
	...
	if (strict_locking == Auto) {
		if  (EXCLUSIVE_OPLOCK_TYPE(fsp->oplock_type) && (plock->lock_type == READ_LOCK || plock->lock_type == WRITE_LOCK)) {
			DEBUG_OUT (10,("is_locked: optimisation - exclusive oplock on file %s\n", fsp_str_dbg(fsp)));
			ret = True;
		} else if ((fsp->oplock_type == LEVEL_II_OPLOCK) &&
			   (plock->lock_type == READ_LOCK)) {
			DEBUG_OUT (10,("is_locked: optimisation - level II oplock on file %s\n", fsp_str_dbg(fsp)));
			ret = True;
		} else {
	...
	DEBUG_OUT (10,("strict_lock_default: flavour = %s brl start=%.0f "
			...
			struct blocking_lock_record *blr)
{
	...
	DEBUG_OUT (10,("do_lock: lock flavour %s lock type %s start=%.0f len=%.0f "
	...
	*perr = brl_lock(msg_ctx,
	...
	DEBUG_OUT (10, ("do_lock: returning status=%s\n", nt_errstr(*perr)));
			...
			enum brl_flavour lock_flav)
{
	...
	DEBUG_OUT (10,("do_unlock: unlock start=%.0f len=%.0f requested for fnum %d file %s\n",
	...
	if (!ok) {
		DEBUG_OUT (10,("do_unlock: returning ERRlock.\n" ));
			...
			struct blocking_lock_record *blr)
{
	...
	DEBUG_OUT (10,("do_lock_cancel: cancel start=%.0f len=%.0f requested for fnum %d file %s\n",
		  (double)offset, (double)count, fsp->fnum,
		  fsp_str_dbg(fsp)));
	...
	if (!ok) {
		DEBUG_OUT (10,("do_lock_cancel: returning ERRcancelviolation.\n" ));
		return NT_STATUS_DOS(ERRDOS, ERRcancelviolation);
	}
...
static bool locking_init_internal(bool read_only)
{
	...
	if (!lock_db) {
		DEBUG_OUT (0,("ERROR: Failed to initialise locking database\n"));
		return False;
	}
...
static void print_share_mode_table(struct locking_data *data)
{
	...
	for (i = 0; i < num_share_modes; i++) {
		...
		DEBUG_OUT (10,("print_share_mode_table: %s\n", str ? str : ""));
		TALLOC_FREE(str);
	}
}
		...
		const TDB_DATA dbuf)
{
	...
	for (i = 0; i < pdata->u.s.num_delete_token_entries; i++) {
		...
		if (pdtl == NULL) {
			DEBUG_OUT (0,("parse_delete_tokens_list: talloc failed"));
			return -1;
		}
		...
		if (p >= end_ptr) {
			DEBUG_OUT (0,("parse_delete_tokens_list: corrupt data"));
			return -1;
		}
		...
		if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			DEBUG_OUT (1, ("parse_delete_tokens_list: "
...
static bool parse_share_modes(const TDB_DATA dbuf, struct share_mode_lock *lck)
{
	...
	DEBUG_OUT (10, ("parse_share_modes: owrt: %s, "
	...
	if ((lck->num_share_modes < 0) || (lck->num_share_modes > 1000000)) {
		DEBUG_OUT (0, ("invalid number of share modes: %d\n",
			  lck->num_share_modes));
	...
	for (i = 0; i < lck->num_share_modes; i++) {
		...
		DEBUG_OUT (10,("parse_share_modes: %s\n",
			str ? str : ""));
		if (!serverid_exists(&entry_p->pid)) {
			DEBUG_OUT (10,("parse_share_modes: deleted %s\n",
				str ? str : ""));
...
static TDB_DATA unparse_share_modes(const struct share_mode_lock *lck)
{
	...
	DEBUG_OUT (10,("unparse_share_modes: owrt: %s cwrt: %s, ntok: %u, "
		  "num: %d\n",
		  timestring(talloc_tos(),
			     convert_timespec_to_time_t(lck->old_write_time)),
		  timestring(talloc_tos(),
	...
	if (data.dptr == NULL) {
		if (!lck->fresh) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				...
				DEBUG_OUT (0, ("delete_rec returned %s\n",
					  nt_errstr(status)));
				 ...
				 const struct timespec *old_write_time)
{
	...
	if (lck->fresh) {
		...
		if (!parse_share_modes(share_mode_data, lck)) {
			DEBUG_OUT (0, ("Could not parse share modes\n"));
			return False;
		}
	}
					    ...
					    const struct timespec *old_write_time)
{
	...
	if (!(lck->record = lock_db->fetch_locked(lock_db, lck, key))) {
		DEBUG_OUT (3, ("Could not lock share entry\n"));
		TALLOC_FREE(lck);
		return NULL;
	}
	...
	if (!fill_share_mode_lock(lck, id, servicepath, smb_fname,
		...
		DEBUG_OUT (3, ("fill_share_mode_lock failed\n"));
		TALLOC_FREE(lck);
		return NULL;
	}
						  ...
						  const struct file_id id)
{
	...
	if (lock_db->fetch(lock_db, lck, key, &data) == -1) {
		DEBUG_OUT (3, ("Could not fetch share entry\n"));
		TALLOC_FREE(lck);
		return NULL;
	}

	if (!fill_share_mode_lock(lck, id, NULL, NULL, data, NULL)) {
		DEBUG_OUT (10, ("fetch_share_mode_unlocked: no share_mode record "
			...
			const struct smb_filename *smb_fname_dst)
{
	...
	DEBUG_OUT (10, ("rename_share_filename: servicepath %s newname %s\n",
	...
	if (lck->base_name == NULL ||
	    (has_stream && lck->stream_name == NULL) ||
	    lck->servicepath == NULL) {
		DEBUG_OUT (0, ("rename_share_filename: talloc failed\n"));
		return False;
	}
	lck->modified = True;
	...
	DEBUG_OUT (10,("rename_share_filename: msg_len = %u\n", (unsigned int)msg_len ));
	...
	for (i=0; i<lck->num_share_modes; i++) {
		...
		DEBUG_OUT (10,("rename_share_filename: sending rename message to "
...
NTSTATUS can_set_delete_on_close(files_struct *fsp, uint32 dosmode)
{
	...
	if ((dosmode & FILE_ATTRIBUTE_READONLY) &&
		...
		DEBUG_OUT (10,("can_set_delete_on_close: file %s delete on close "
	...
	if (!CAN_WRITE(fsp->conn)) {
		DEBUG_OUT (10,("can_set_delete_on_close: file %s delete on "
	...
	if (fsp->is_directory) {
		...
		if (ISDOT(fsp->fsp_name->base_name)) {
			DEBUG_OUT (10,("can_set_delete_on_close: can't set delete on "
			...
			const struct security_unix_token *tok)
{
	...
	DEBUG_OUT (10,("set_delete_on_close: %s delete on close flag for "
		  "fnum = %d, file %s\n",
				...
				const struct security_unix_token **pp_tok)
{
	...
	DEBUG_OUT (10,("get_delete_on_close_token: name_hash = 0x%x\n",
			(unsigned int)name_hash ));

	for (dtl = lck->delete_tokens; dtl; dtl = dtl->next) {
		DEBUG_OUT (10,("get_delete_on_close_token: dtl->name_hash = 0x%x\n",
...
bool set_sticky_write_time(struct file_id fileid, struct timespec write_time)
{
	...
	DEBUG_OUT (5,("set_sticky_write_time: %s id=%s\n",
		 timestring(talloc_tos(),
...
bool set_write_time(struct file_id fileid, struct timespec write_time)
{
	...
	DEBUG_OUT (5,("set_write_time: %s id=%s\n",
		 timestring(talloc_tos(),
			    convert_timespec_to_time_t(write_time)),
		 file_id_string_tos(&fileid)));
...
static int traverse_fn(struct db_record *rec, void *_state)
{
	...
	if (!parse_share_modes(rec->value, lck)) {
		TALLOC_FREE(lck);
		DEBUG_OUT (1, ("parse_share_modes failed\n"));
		return 0;
	}
