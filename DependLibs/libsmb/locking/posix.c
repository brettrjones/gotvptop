...
static int map_posix_lock_type( files_struct *fsp, enum brl_type lock_type)
{
	if((lock_type == WRITE_LOCK) && !fsp->can_write) {
		 ...
		 */
		DEBUG_OUT (10,("map_posix_lock_type: Downgrading write lock to read due to read-only file.\n"));
		return F_RDLCK;
	}
				...
				uint64_t u_offset, uint64_t u_count)
{
	...
	if (count == (SMB_OFF_T)0) {
		DEBUG_OUT (10,("posix_lock_in_range: count = 0, ignoring.\n"));
		return False;
	}
	...
	if (u_offset & ~((uint64_t)max_positive_lock_offset)) {
		DEBUG_OUT (10,("posix_lock_in_range: (offset = %.0f) offset > %.0f and we cannot handle this. Ignoring lock.\n",
	...
	if (count == 0) {
		DEBUG_OUT (10,("posix_lock_in_range: Count = 0. Ignoring lock u_offset = %.0f, u_count = %.0f\n",
	...
	DEBUG_OUT (10,("posix_lock_in_range: offset_out = %.0f, count_out = %.0f\n",
...
static bool posix_fcntl_lock(files_struct *fsp, int op, SMB_OFF_T offset, SMB_OFF_T count, int type)
{
	...
	DEBUG_OUT (8,("posix_fcntl_lock %d %d %.0f %.0f %d\n",fsp->fh->fd,op,(double)offset,(double)count,type));

	ret = SMB_VFS_LOCK(fsp, op, offset, count, type);

	if (!ret && ((errno == EFBIG) || (errno == ENOLCK) || (errno ==  EINVAL))) {

		DEBUG_OUT (0,("posix_fcntl_lock: WARNING: lock request at offset %.0f, length %.0f returned\n",
					(double)offset,(double)count));
		...
		if (offset & ~((SMB_OFF_T)0x7fffffff)) {
			DEBUG_OUT (0,("Offset greater than 31 bits. Returning success.\n"));
			return True;
		}

		if (count & ~((SMB_OFF_T)0x7fffffff)) {
			/* 32 bit NFS file system, retry with smaller offset */
			DEBUG_OUT (0,("Count greater than 31 bits - retrying with 31 bit truncated length.\n"));
			errno = 0;
			count &= 0x7fffffff;
			ret = SMB_VFS_LOCK(fsp, op, offset, count, type);
		}
	...
	DEBUG_OUT (8,("posix_fcntl_lock: Lock call %s\n", ret ? "successful" : "failed"));
	return ret;
}
...
static bool posix_fcntl_getlock(files_struct *fsp, SMB_OFF_T *poffset, SMB_OFF_T *pcount, int *ptype)
{
	...
	DEBUG_OUT (8,("posix_fcntl_getlock %d %.0f %.0f %d\n",
	...
	if (!ret && ((errno == EFBIG) || (errno == ENOLCK) || (errno ==  EINVAL))) {
		...
		DEBUG_OUT (0,("posix_fcntl_getlock: WARNING: lock request at offset %.0f, length %.0f returned\n",
	...
	DEBUG_OUT (8,("posix_fcntl_getlock: Lock query call %s\n", ret ? "successful" : "failed"));
	return ret;
}
			...
			enum brl_flavour lock_flav)
{
	...
	DEBUG_OUT (10,("is_posix_locked: File %s, offset = %.0f, count = %.0f, "
...
bool posix_locking_init(bool read_only)
{
	...
	if (posix_pending_close_db == NULL) {
		DEBUG_OUT (0,("Failed to open POSIX pending close database.\n"));
		return false;
	}
...
static void increment_windows_lock_ref_count(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("increment_windows_lock_ref_count for file now %s = %d\n",
...
void reduce_windows_lock_ref_count(files_struct *fsp, unsigned int dcount)
{
	...
	DEBUG_OUT (10,("reduce_windows_lock_ref_count for file now %s = %d\n",
		  fsp_str_dbg(fsp), lock_ref_count));
}
...
static int get_windows_lock_ref_count(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("get_windows_lock_count for file %s = %d\n",
		  fsp_str_dbg(fsp), lock_ref_count));
...
static void delete_windows_lock_ref_count(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("delete_windows_lock_ref_count for file %s\n",
		  fsp_str_dbg(fsp)));
}
...
static void add_fd_to_close_entry(files_struct *fsp)
{
	...
	DEBUG_OUT (10,("add_fd_to_close_entry: added fd %d file %s\n",
...
int fd_close_posix(struct files_struct *fsp)
{
	...
	if (count) {
		DEBUG_OUT (10,("fd_close_posix: doing close on %u fd's.\n",
			  (unsigned int)count));
						...
						int num_locks)
{
	...
	DEBUG_OUT (10,("posix_lock_list: curr: start=%.0f,size=%.0f\n",
	...
	for (i=0; i<num_locks && lhead; i++) {
		...
		for (l_curr = lhead; l_curr;) {
			...
			DEBUG_OUT (10,("posix_lock_list: lock: fnum=%d: start=%.0f,size=%.0f:type=%s", lock->fnum,
...
**********************************************/
				...
				DEBUG_OUT (10,(" truncate high case: start=%.0f,size=%.0f\n",
...
**********************************************/
				...
				DEBUG_OUT (10,(" truncate low case: start=%.0f,size=%.0f\n",
								(double)l_curr->start, (double)l_curr->size ));
...
**********************************************/
				...
				if(l_new == NULL) {
					DEBUG_OUT (0,("posix_lock_list: talloc fail.\n"));
				...
				DEBUG_OUT (10,(" split case: curr: start=%.0f,size=%.0f \
			...
			int *errno_ret)
{
	...
	DEBUG_OUT (5,("set_posix_lock_windows_flavour: File %s, offset = %.0f, "
		 "count = %.0f, type = %s\n", fsp_str_dbg(fsp),
		 (double)u_offset, (double)u_count,
	...
	if ((l_ctx = talloc_init("set_posix_lock")) == NULL) {
		DEBUG_OUT (0,("set_posix_lock_windows_flavour: unable to init talloc context.\n"));
		return False;
	}

	if ((ll = TALLOC_P(l_ctx, struct lock_list)) == NULL) {
		DEBUG_OUT (0,("set_posix_lock_windows_flavour: unable to talloc unlock list.\n"));
	...
	for (lock_count = 0, ll = llist; ll; ll = ll->next, lock_count++) {
		...
		DEBUG_OUT (5,("set_posix_lock_windows_flavour: Real lock: Type = %s: offset = %.0f, count = %.0f\n",
		...
		if (!posix_fcntl_lock(fsp,SMB_F_SETLK,offset,count,posix_lock_type)) {
			*errno_ret = errno;
			DEBUG_OUT (5,("set_posix_lock_windows_flavour: Lock fail !: Type = %s: offset = %.0f, count = %.0f. Errno = %s\n",
	...
	if (!ret) {
		...
		for (ll = llist; lock_count; ll = ll->next, lock_count--) {
			...
			DEBUG_OUT (5,("set_posix_lock_windows_flavour: Backing out locks: Type = %s: offset = %.0f, count = %.0f\n",
				posix_lock_type_name(posix_lock_type), (double)offset, (double)count ));
				...
				int num_locks)
{
	...
	DEBUG_OUT (5,("release_posix_lock_windows_flavour: File %s, offset = %.0f, "
		 "count = %.0f\n", fsp_str_dbg(fsp),
		 (double)u_offset, (double)u_count));
	...
	if ((ul_ctx = talloc_init("release_posix_lock")) == NULL) {
		DEBUG_OUT (0,("release_posix_lock_windows_flavour: unable to init talloc context.\n"));
		return False;
	}

	if ((ul = TALLOC_P(ul_ctx, struct lock_list)) == NULL) {
		DEBUG_OUT (0,("release_posix_lock_windows_flavour: unable to talloc unlock list.\n"));
	...
	if (deleted_lock_type == WRITE_LOCK &&
		...
		DEBUG_OUT (5,("release_posix_lock_windows_flavour: downgrading lock to READ: offset = %.0f, count = %.0f\n",
			(double)offset, (double)count ));

		if (!posix_fcntl_lock(fsp,SMB_F_SETLK,offset,count,F_RDLCK)) {
			DEBUG_OUT (0,("release_posix_lock_windows_flavour: downgrade of lock failed with error %s !\n", strerror(errno) ));
			talloc_destroy(ul_ctx);
			return False;
		}
	}
	...
	for(; ulist; ulist = ulist->next) {
		...
		DEBUG_OUT (5,("release_posix_lock_windows_flavour: Real unlock: offset = %.0f, count = %.0f\n",
			(double)offset, (double)count ));
			...
			int *errno_ret)
{
	...
	DEBUG_OUT (5,("set_posix_lock_posix_flavour: File %s, offset = %.0f, count "
		 "= %.0f, type = %s\n", fsp_str_dbg(fsp),
		 (double)u_offset, (double)u_count,
		 posix_lock_type_name(lock_type)));
	...
	if (!posix_fcntl_lock(fsp,SMB_F_SETLK,offset,count,posix_lock_type)) {
		*errno_ret = errno;
		DEBUG_OUT (5,("set_posix_lock_posix_flavour: Lock fail !: Type = %s: offset = %.0f, count = %.0f. Errno = %s\n",
			posix_lock_type_name(posix_lock_type), (double)offset, (double)count, strerror(errno) ));
		return False;
	}
	return True;
				...
				int num_locks)
{
	...
	DEBUG_OUT (5,("release_posix_lock_posix_flavour: File %s, offset = %.0f, "
		 "count = %.0f\n", fsp_str_dbg(fsp),
		 (double)u_offset, (double)u_count));
	...
	for(; ulist; ulist = ulist->next) {
		...
		DEBUG_OUT (5,("release_posix_lock_posix_flavour: Real unlock: offset = %.0f, count = %.0f\n",
			(double)offset, (double)count ));
