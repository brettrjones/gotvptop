...
void brl_init(bool read_only)
{
	...
	if (!brlock_db) {
		DEBUG_OUT (0,("Failed to open byte range locking database %s\n",
	...
	if (signal_pending_read) {
		...
		for (i=0; i < br_lck->num_locks; i++) {
			...
			if (pend_lock->lock_type == PENDING_READ_LOCK &&
				...
				DEBUG_OUT (10,("brl_lock_posix: sending unlock message to pid %s\n",
...
#if !ZERO_ZERO
	if (start == 0 && size == 0) {
		DEBUG_OUT (0,("client sent 0/0 lock - please report this\n"));
	}
#endif
  ...
  unlock_continue:
	...
	for (j=0; j < br_lck->num_locks; j++) {
		...
		if (brl_pending_overlap(plock, pend_lock)) {
			DEBUG_OUT (10,("brl_unlock: sending unlock message to pid %s\n",
				procid_str_static(&pend_lock->context.pid )));
			     ...
			     struct lock_struct *plock)
{
	...
	if (plock->start + plock->size < plock->start ||
		...
		DEBUG_OUT (10,("brl_unlock_posix: lock wrap\n"));
		return False;
	}
	...
	if (!tp) {
		DEBUG_OUT (10,("brl_unlock_posix: malloc fail\n"));
		return False;
	}
	...
	if (!overlap_found) {
		...
		SAFE_FREE(tp);
		DEBUG_OUT (10,("brl_unlock_posix: No overlap - unlocked.\n"));
		return True;
	}
	...
	if (count) {
		tp = (struct lock_struct *)SMB_REALLOC(tp, count * sizeof(*locks));
		if (!tp) {
			DEBUG_OUT (10,("brl_unlock_posix: realloc fail\n"));
			return False;
		}
	} else {
		...
		enum brl_flavour lock_flav)
{
	...
	if(lp_posix_locking(fsp->conn->params) && (lock_flav == WINDOWS_LOCK)) {
		...
		DEBUG_OUT (10,("brl_locktest: posix start=%.0f len=%.0f %s for fnum %d file %s\n",
		...
		enum brl_flavour lock_flav)
{
	...
	if(lp_posix_locking(fsp->conn->params)) {
		...
		DEBUG_OUT (10,("brl_lockquery: posix start=%.0f len=%.0f %s for fnum %d file %s\n",
					...
					files_struct *fsp, bool read_only)
{
	...
	if (do_read_only) {
		if (brlock_db->fetch(brlock_db, br_lck, key, &data) == -1) {
			DEBUG_OUT (3, ("Could not fetch byte range lock record\n"));
			TALLOC_FREE(br_lck);
			return NULL;
		}
		br_lck->record = NULL;
		...
		if (br_lck->record == NULL) {
			DEBUG_OUT (3, ("Could not lock byte range lock entry\n"));
			TALLOC_FREE(br_lck);
			return NULL;
		}
	...
	if (DEBUGLEVEL >= 10) {
		unsigned int i;
		struct lock_struct *locks = br_lck->lock_data;
		DEBUG_OUT (10,("brl_get_locks_internal: %u current locks on file_id %s\n",
			br_lck->num_locks,
