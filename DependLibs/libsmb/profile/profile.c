...
void set_profile_level(int level, struct server_id src)
{
	...
	case 0:		/* turn off profiling */
		...
		DEBUG_OUT (1,("INFO: Profiling turned OFF from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 1:		/* turn on counter profiling only */
		do_profile_flag = True;
		do_profile_times = False;
		DEBUG_OUT (1,("INFO: Profiling counts turned ON from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 2:		/* turn on complete profiling */
		do_profile_flag = True;
		do_profile_times = True;
		DEBUG_OUT (1,("INFO: Full profiling turned ON from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 3:		/* reset profile values */
		memset((char *)profile_p, 0, sizeof(*profile_p));
		DEBUG_OUT (1,("INFO: Profiling values cleared from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	}
#else /* WITH_PROFILE */
	DEBUG_OUT (1,("INFO: Profiling support unavailable in this build.\n"));
#endif /* WITH_PROFILE */
}
			    ...
			    DATA_BLOB *data)
{
	...
	if (data->length != sizeof(level)) {
		DEBUG_OUT (0, ("got invalid profile message\n"));
		return;
	}
			       ...
			       DATA_BLOB *data)
{
	...
	level = 0;
#endif
	DEBUG_OUT (1,("INFO: Received REQ_PROFILELEVEL message from PID %u\n",
...
bool profile_setup(struct messaging_context *msg_ctx, bool rdonly)
{
 ...
 again:
	...
	if (shm_id == -1) {
		DEBUG_OUT (0,("Can't create or use IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}   
	...
	if ((long)profile_h == -1) {
		DEBUG_OUT (0,("Can't attach to IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shmctl(shm_id, IPC_STAT, &shm_ds) != 0) {
		DEBUG_OUT (0,("ERROR shmctl : can't IPC_STAT. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shm_ds.shm_perm.cuid != sec_initial_uid() ||
		...
		DEBUG_OUT (0,("ERROR: we did not create the shmem "
	...
	if (shm_ds.shm_segsz != sizeof(*profile_h)) {
		DEBUG_OUT (0,("WARNING: profile size is %d (expected %d). Deleting\n",
	...
	if (!read_only && (shm_ds.shm_nattch == 1)) {
		...
		DEBUG_OUT (3,("Initialised profile area\n"));
	}
