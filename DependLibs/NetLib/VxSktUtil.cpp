...
#ifdef DEBUG_SKTS
				LogMsg( LOG_SKT, "VxConnectTo: SUCCESS skt handle %d connect no block ip %s port %d\n", sktHandle, strRmtIp.c_str(), u16Port );
#endif // DEBUG_SKTS
...
#ifdef DEBUG_SKTS
				LogMsg( LOG_SKT, "VxConnectTo: skt connect to %s %d error %s\n", strRmtIp.c_str(), u16Port, VxDescribeSktError( rc ) );
#endif // DEBUG_SKTS
...
//#define	EINPROGRESS	115	/* Operation now in progress */
				...
				{ 
#ifdef DEBUG_SKTS
					LogMsg( LOG_SKT, "VxConnectTo: skt handle %d connect to %s port %d timed out %d ... canceling\n", sktHandle, strRmtIp.c_str(), u16Port, iConnectTimeoutMs );
#endif // DEBUG_SKTS
					if( 0 != retSktErr )
					{
						*retSktErr = -1;
		...
		else
		{
			...
			if( 0 != iResult )
			{
				...
				rc = VxGetLastError();
#ifdef DEBUG_SKTS
				LogMsg( LOG_SKT, "VxConnectTo: skt handle %d connect to %s error %s\n",
								sktHandle,
								strRmtIp.c_str(),
								VxDescribeSktError( rc ) );
#endif // DEBUG_SKTS
...
//#endif // DEBUG_SKTS
				...
				{ 
					...
					else
					{
						...
						{ 
#ifdef DEBUG_SKTS
							LogMsg( LOG_INFO, "VxConnectTo: skt handle %d Error %d in delayed connection to ip %s\n", sktHandle, valopt, strRmtIp.c_str() );
#endif // DEBUG_SKTS
							VxCloseSktNow( sktHandle );
							break;
						}
				...
				{ 
#ifdef DEBUG_SKTS
					LogMsg( LOG_SKT, "VxConnectTo: skt handle %d connect to %s port %d timed out.. canceling\n", sktHandle, strRmtIp.c_str(), u16Port );
#endif // DEBUG_SKTS
					VxCloseSktNow( sktHandle );
					break; 
				} 
...
RCODE VxGetRmtAddress( SOCKET sktHandle, InetAddrAndPort& oRetAddr )
{
	...
	if ( getpeername( sktHandle, ( struct sockaddr* )&oSktAddr, &iSktAddrLen ) )
	{
		...
		oRetAddr.setToInvalid();
		rc = VxGetLastError();
		LogMsg( LOG_SKT, "VxGetRmtAddress: skt handle %d error %d %s\n", sktHandle, rc, VxDescribeSktError( rc ) );
	}
	else
	{
	...
	*iRetBytesReceived = 0;
	...
	do
	{
		...
		if ( INVALID_SOCKET == sktHandle )
		{
			...
			LogMsg( LOG_INFO, "VxSktCode:recv error %d %s\n", m_rcLastError, VxDescribeSktError( m_rcLastError ) );
			return m_rcLastError;
		}
