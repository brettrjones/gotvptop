#ifndef ifaddrs_androidANDROIDIMPL_H
#define ifaddrs_androidANDROIDIMPL_H
/*
...
//#include "ScopedFd.h"
// Android (bionic) doesn't have getifaddrs_android(3)/freeifaddrs_android(3).
...
struct ifaddrs_android {
    // Pointer to next struct in list, or NULL at end.
    ifaddrs_android* ifa_next;
    // Interface name.
    char* ifa_name;
    // Interface flags.
    ...
    ifaddrs_android(ifaddrs_android* next)
...
private:
    ...
    ifaddrs_android(const ifaddrs_android&);
    void operator=(const ifaddrs_android&);
};
...
inline int getifaddrs_android(ifaddrs_android** result) {
    ...
    *result = NULL;
    ...
    // Read the responses.
    char buf[65536]; // We don't necessarily have std::vector.
    ssize_t bytesRead;
    while ((bytesRead  = recvNetlinkMessage(fd.get(), &buf[0], sizeof(buf))) > 0) {
                    ...
                    while (RTA_OK(rta, ifaPayloadLength)) {
                        if (rta->rta_type == IFA_LOCAL) {
                            ...
                            if (family == AF_INET || family == AF_INET6) {
                                *result = new ifaddrs_android(*result);
...
// Source-compatible with the BSD function.
inline void freeifaddrs_android(ifaddrs_android* addresses) {
    delete addresses;
}
#endif  // TARGET_OS_ANDROID
#endif // ifaddrs_androidANDROIDIMPL_H
