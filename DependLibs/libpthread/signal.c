...
{
	ptw32_handle_t threadInst = pthread_self();

	if( threadInst.p == NULL )
	{
		return ENOENT;
	}
	...
	if( oset != NULL )
	{
		memcpy( oset, &( (( ptw32_thread_t *) threadInst.p)->sigmask ), sizeof( sigset_t ) );
	}
	...
	if( set != NULL )
	{
		...
		unsigned long *dest = ( unsigned long * ) &( ( ( ptw32_thread_t * )threadInst.p)->sigmask );
		...
		case SIG_SETMASK:
			...
			memcpy( &( ( ( ptw32_thread_t * )threadInst.p)->sigmask ), set, sizeof( sigset_t ) );
			break;
		}
	}
