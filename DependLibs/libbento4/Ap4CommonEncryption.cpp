                                               ...
                                               AP4_Array<AP4_UI32>& bytes_of_encrypted_data)
{
    ...
    while ((AP4_Size)(in_end-in) > 1+m_NaluLengthSize) {
        ...
        if (skip) {
            ...
            while (range) {
				AP4_UI16 cleartext_size = (range <= 0xFFFF) ? static_cast<AP4_UI16>(range) : 0xFFFF;
			...
			bytes_of_cleartext_data.Append(static_cast<AP4_UI16>(cleartext_size));
                                ...
                                AP4_CencSampleDecrypter*&       decrypter)
{
	...
	return Create(sample_info_table, algorithm_id, key, key_size, block_cipher_factory, singlesample_decrypter, decrypter);
}
