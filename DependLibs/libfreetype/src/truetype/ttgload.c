...
#define UNSCALED_COMPONENT_OFFSET  0x1000
                       ...
                       FT_Bool    header_only )
  {
    ...
    /* If we are loading glyph data via the incremental freeTypeInterface, set */
    /* the loader stream to a memory stream reading the data returned  */
    /* by the freeTypeInterface.                                               */
    if ( face->root.internal->incremental_interface )
    {
    ...
    {
#ifdef FT_CONFIG_OPTION_INCREMENTAL
      /* for the incremental freeTypeInterface, `glyf_offset' is always zero */
      if ( !face->glyf_offset                          &&
    ...
    if ( loader->byte_len == 0 || loader->n_contours == 0 )
    {
      ...
      /* glyph metrics from the incremental freeTypeInterface        */
