...
#define FREETYPE_VER_FIXED  ( ( (FT_Long)FREETYPE_MAJOR << 16 ) | \
                  ...
                  FT_Bool           value_is_string )
  {
    FT_Module*           cur;
    FT_Module*           limit;
    FT_Module_Interface  freeTypeInterface;
    ...
    /* check whether we have a service freeTypeInterface */
    if ( !cur[0]->clazz->get_interface )
    {
    ...
    freeTypeInterface = cur[0]->clazz->get_interface( cur[0],
                                              FT_SERVICE_ID_PROPERTIES );
    if ( !freeTypeInterface )
    {
      FT_ERROR(( "%s: module `%s' doesn't support properties\n",
                 func_name, module_name ));
      return FT_THROW( Unimplemented_Feature );
    ...
    service = (FT_Service_Properties)freeTypeInterface;
