...
#define next cp_list_next
#define prev cp_list_prev
#define data cp_list_data

#define pool cp_list_pool
#define fre cp_list_free
#define size cp_list_size

#define nilnode cp_list_nilnode
#define nodecount cp_list_nodecount
#define maxcount cp_list_maxcount

#define cp_list_nil(L)		(&(L)->nilnode)
#define cp_list_first_priv(L)	((L)->nilnode.next)
#define cp_list_last_priv(L)	((L)->nilnode.prev)
...
CP_HIDDEN cp_list_t *cp_list_init(cp_list_t *list, listcount_t maxcount)
{
    assert (maxcount != 0);
...
CP_HIDDEN cp_list_t *cp_list_create(listcount_t maxcount)
{
    cp_list_t *new = malloc(sizeof *new);
    if (new) {
	assert (maxcount != 0);
...
CP_HIDDEN void cp_list_destroy(cp_list_t *list)
{
    assert (cp_list_isempty(list));
    free(list);
}
...
CP_HIDDEN void cp_list_destroy_nodes(cp_list_t *list)
{
    lnode_t *lnode = cp_list_first_priv(list), *nil = cp_list_nil(list), *tmp;
    ...
    cp_list_init(list, list->maxcount);
}
...
CP_HIDDEN void cp_list_return_nodes(cp_list_t *list, lnodepool_t *pool)
{
    lnode_t *lnode = cp_list_first_priv(list), *tmp, *nil = cp_list_nil(list);
...
CP_HIDDEN void cp_list_ins_after(cp_list_t *list, lnode_t *new, lnode_t *this)
{
    lnode_t *that = this->next;

    assert (new != NULL);
    assert (!cp_list_contains(list, new));
    assert (!lnode_is_in_a_list(new));
    assert (this == cp_list_nil(list) || cp_list_contains(list, this));
...
CP_HIDDEN void cp_list_ins_before(cp_list_t *list, lnode_t *new, lnode_t *this)
{
...
CP_HIDDEN lnode_t *cp_list_delete(cp_list_t *list, lnode_t *del)
{
    lnode_t *next = del->next;
    lnode_t *prev = del->prev;

    assert (cp_list_contains(list, del));
...
CP_HIDDEN void cp_list_process(cp_list_t *list, void *context,
	void (* function)(cp_list_t *list, lnode_t *lnode, void *context))
{
    lnode_t *node = cp_list_first_priv(list), *next, *nil = cp_list_nil(list);

    while (node != nil) {
	/* check for callback function deleting	*/
	/* the next node from under us		*/
	assert (cp_list_contains(list, node));
	next = node->next;
...
CP_HIDDEN int cp_list_contains(cp_list_t *list, lnode_t *node)
{
    lnode_t *n, *nil = cp_list_nil(list);

    for (n = cp_list_first_priv(list); n != nil; n = lnode_next(n)) {
	if (node == n)
	    return 1;
    }
...
/*
 * A more generalized variant of cp_list_transfer. This one removes a
 * ``slice'' from the source list and appends it to the destination
 * list.
 */

CP_HIDDEN void cp_list_extract(cp_list_t *dest, cp_list_t *source, lnode_t *first, lnode_t *last)
{
    listcount_t moved = 1;

    assert (first == NULL || cp_list_contains(source, first));
    assert (last == NULL || cp_list_contains(source, last));
    ...
    while (first != last) {
	first = first->next;
	assert (first != cp_list_nil(source));	/* oops, last before first! */
	moved++;
    }
    ...
    /* assert list sanity */
    assert (cp_list_verify(source));
    assert (cp_list_verify(dest));
}
...
CP_HIDDEN void cp_list_transfer(cp_list_t *dest, cp_list_t *source, lnode_t *first)
{
    listcount_t moved = 1;
    lnode_t *last;
...
CP_HIDDEN void cp_list_merge(cp_list_t *dest, cp_list_t *sour,
	int compare (const void *, const void *))
{
    lnode_t *dn, *sn, *tn;
    lnode_t *d_nil = cp_list_nil(dest), *s_nil = cp_list_nil(sour);
    ...
    /* overflow check */
    assert (cp_list_count(sour) + cp_list_count(dest) >= cp_list_count(sour));

    /* lists must be sorted */
    assert (cp_list_is_sorted(sour, compare));
    assert (cp_list_is_sorted(dest, compare));

    dn = cp_list_first_priv(dest);
    sn = cp_list_first_priv(sour);

    while (dn != d_nil && sn != s_nil) {
	if (compare(lnode_get(dn), lnode_get(sn)) >= 0) {
	    tn = lnode_next(sn);
	    cp_list_delete(sour, sn);
	    cp_list_ins_before(dest, sn, dn);
	    sn = tn;
	} else {
	    dn = lnode_next(dn);
	}
    ...
    if (sn != s_nil)
	cp_list_transfer(dest, sour, sn);
}

CP_HIDDEN void cp_list_sort(cp_list_t *list, int compare(const void *, const void *))
{
    cp_list_t extra;
    listcount_t middle;
    lnode_t *node;

    if (cp_list_count(list) > 1) {
	middle = cp_list_count(list) / 2;
	node = cp_list_first_priv(list);

	cp_list_init(&extra, cp_list_count(list) - middle);

	while (middle--)
	    node = lnode_next(node);
	
	cp_list_transfer(&extra, list, node);
	cp_list_sort(list, compare);
	cp_list_sort(&extra, compare);
	cp_list_merge(list, &extra, compare);
    } 
    assert (cp_list_is_sorted(list, compare));
}

CP_HIDDEN lnode_t *cp_list_find(cp_list_t *list, const void *key, int compare(const void *, const void *))
{
    lnode_t *node;

    for (node = cp_list_first_priv(list); node != cp_list_nil(list); node = node->next) {
...
CP_HIDDEN int cp_list_is_sorted(cp_list_t *list, int compare(const void *, const void *))
{
    lnode_t *node, *next, *nil;

    next = nil = cp_list_nil(list);
    node = cp_list_first_priv(list);
...
#undef cp_list_isempty
#undef cp_list_isfull
#undef lnode_pool_isempty
#undef cp_list_append
#undef cp_list_prepend
#undef cp_list_first
#undef cp_list_last
#undef cp_list_next
#undef cp_list_prev
#undef cp_list_count
#undef cp_list_del_first
#undef cp_list_del_last
#undef lnode_put
#undef lnode_get
...
CP_HIDDEN int cp_list_isempty(cp_list_t *list)
{
...
CP_HIDDEN int cp_list_isfull(cp_list_t *list)
{
...
CP_HIDDEN void cp_list_append(cp_list_t *list, lnode_t *node)
{
    cp_list_ins_before(list, node, &list->nilnode);
}
...
CP_HIDDEN void cp_list_prepend(cp_list_t *list, lnode_t *node)
{
    cp_list_ins_after(list, node, &list->nilnode);
}
...
CP_HIDDEN lnode_t *cp_list_first(cp_list_t *list)
{
...
CP_HIDDEN lnode_t *cp_list_last(cp_list_t *list)
{
...
CP_HIDDEN listcount_t cp_list_count(cp_list_t *list)
{
...
CP_HIDDEN lnode_t *cp_list_del_first(cp_list_t *list)
{
    return cp_list_delete(list, list->nilnode.next);
}
...
CP_HIDDEN lnode_t *cp_list_del_last(cp_list_t *list)
{
    return cp_list_delete(list, list->nilnode.prev);
}
...
CP_HIDDEN lnode_t *cp_list_next(cp_list_t *list, lnode_t *lnode)
{
    assert (cp_list_contains(list, lnode));

    if (lnode->next == cp_list_nil(list))
	return NULL;
    return lnode->next;
}
...
CP_HIDDEN lnode_t *cp_list_prev(cp_list_t *list, lnode_t *lnode)
{
    assert (cp_list_contains(list, lnode));

    if (lnode->prev == cp_list_nil(list))
	return NULL;
    return lnode->prev;
}
...
CP_HIDDEN int cp_list_verify(cp_list_t *list)
{
    lnode_t *node = cp_list_first_priv(list), *nil = cp_list_nil(list);
    listcount_t count = cp_list_count(list);
...
{
    input_t in;
    cp_list_t *l = cp_list_create(LISTCOUNT_T_MAX);
    lnode_t *ln;
    char *tok1, *val;
    int prompt = 0;
	...
	switch(in[0]) {
		...
		ln = cp_list_find(l, tok1, comparef);
		if (!ln) {
		    puts("cp_list_find failed");
		    break;
		}
		cp_list_delete(l, ln);
		val = lnode_get(ln);
		...
		break;
	    case 's':
		cp_list_sort(l, comparef);
		break;
	    case 'c':
		printf("%lu\n", (unsigned long) cp_list_count(l));
		break;
	    case 't':
		for (ln = cp_list_first(l); ln != 0; ln = cp_list_next(l, ln))
		    puts(lnode_get(ln));
		break;
	    case 'q':
		exit(0);
