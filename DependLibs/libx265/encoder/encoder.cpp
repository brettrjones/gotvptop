...
void Encoder::getStreamHeaders(NALList& list, Entropy& sbacCoder, Bitstream& bs)
{
    ...
    if (m_param->bEmitInfoSEI)
    {
        ...
        if (opts)
        {
            char *buffer = X265_MALLOC(char, strlen(opts) + strlen(PFX_X265(version_str)) +
                                             strlen(PFX_X265(build_info_str)) + 200);
            if (buffer)
            {
                sprintf(buffer, "x265 (build %d) - %s:%s - H.265/HEVC codec - "
                        "Copyright 2013-2017 (c) Multicoreware, Inc - "
                        "http://x265.org - options: %s",
                        X265_BUILD, PFX_X265(version_str), PFX_X265(build_info_str), opts);
