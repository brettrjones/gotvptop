...
extern "C" {
/* cpu-a.asm */
int PFX_X265(cpu_cpuid_test)(void);
void PFX_X265(cpu_cpuid)(uint32_t op, uint32_t *eax, uint32_t *ebx, uint32_t *ecx, uint32_t *edx);
void PFX_X265(cpu_xgetbv)(uint32_t op, uint32_t *eax, uint32_t *edx);
}
...
uint32_t cpu_detect(void)
{
...
#if !X86_64
    if (!PFX_X265(cpu_cpuid_test)())
        return 0;
#endif

	PFX_X265(cpu_cpuid)(0, &eax, vendor + 0, vendor + 2, vendor + 1);
    max_basic_cap = eax;
    if (max_basic_cap == 0)
        return 0;

	PFX_X265(cpu_cpuid)(1, &eax, &ebx, &ecx, &edx);
    if (edx & 0x00800000)
        cpu |= X265_CPU_MMX;
    else
        return cpu;
    ...
    if (max_basic_cap >= 7)
    {
		PFX_X265(cpu_cpuid)(7, &eax, &ebx, &ecx, &edx);
	...
	PFX_X265(cpu_cpuid)(0x80000000, &eax, &ebx, &ecx, &edx);
    max_extended_cap = eax;

    if (max_extended_cap >= 0x80000001)
    {
		PFX_X265(cpu_cpuid)(0x80000001, &eax, &ebx, &ecx, &edx);
    ...
    if ((!strcmp((char*)vendor, "GenuineIntel") || !strcmp((char*)vendor, "CyrixInstead")) && !(cpu & X265_CPU_SSE42))
    {
        ...
        {
			PFX_X265(cpu_cpuid)(0x80000006, &eax, &ebx, &ecx, &edx);
        ...
        if (!cache && max_basic_cap >= 2)
        {
            ...
            do
            {
                PFX_X265(cpu_cpuid)(2, buf + 0, buf + 1, buf + 2, buf + 3);
                max = buf[0] & 0xff;
                buf[0] &= ~0xff;
                for (int j = 0; j < 4; j++)
                {
...
extern "C" {
void PFX_X265(cpu_neon_test)(void);
int PFX_X265(cpu_fast_neon_mrc_test)(void);
}
...
#ifndef __MACH__
    flags |= PFX_X265(cpu_fast_neon_mrc_test)() ? X265_CPU_FAST_NEON_MRC : 0;
#endif
