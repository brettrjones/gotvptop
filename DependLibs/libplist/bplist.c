...
#include <libplist/libcnary/include/node.h>
#include <libplist/libcnary/include/node_iterator.h>
#include <libplist/plist_internal.h>
...
#ifdef _MSC_VER
uint64_t ConvertToHostEndian( const char * someData, int dataSize )
{ 
    switch( dataSize )
    {
    case 2:
        return  be16toh( *(( uint16_t* )someData) );
    case 4:
        return  be32toh( *( ( uint32_t* )someData ) );
    default:
        return  be64toh( *( ( uint64_t* )someData ) );
    }
}

#undef UINT_TO_HOST
#define UINT_TO_HOST ConvertToHostEndian
#undef get_unaligned
#define get_unaligned(val) (*(val))
