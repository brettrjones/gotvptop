 ...
 */
#include <libplist/config_libplist.h>
...
#include <libplist/libcnary/include/node_list.h>
...
static size_t dtostr(char *buf, size_t bufsize, double realval)
{
    ...
    f = modf(f, &ip);
    len = snprintf(buf, bufsize, "%s%lli", ((f < 0) && (ip >= 0)) ? "-" : "", (int64_t)ip);
    if (len >= bufsize) {
        return 0;
    }
...
static void node_to_xml(node_t* node, bytearray_t **outbuf, uint32_t depth)
{
    ...
    case PLIST_UINT:
        ...
        if (node_data->length == 16) {
            val_len = snprintf(val, 64, "%lld", node_data->intval);
        } else {
            val_len = snprintf(val, 64, "%lld", node_data->intval);
        }
        break;
