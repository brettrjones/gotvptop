...
import com.picklingtools.pickle.objects.Time;
import com.picklingtools.pickle.objects.TimeDelta;
...
public class Pickler {
    ...
    /*
    void put_string(String string, boolean raw_bytes) throws IOException {
	if (raw_bytes) {
	    byte[] enc = new byte[string.length()];
	    for (int ii=0; ii<string.length(); ii++) {
		byte b = (byte)string.charAt(ii);
		enc[ii] = b;
	    }
	    out.write(Opcodes.BINSTRING);
	    out.write(utils.integer_to_bytes(enc.length));
	    out.write(enc);
	    return;
	} else {
	    put_string(string);
    ...
    */
    void put_string(String string) throws IOException {
		boolean is_7bits = true;
	        for (int ii=0; ii<string.length(); ii++) {
		        if (string.charAt(ii)>127) {
			    is_7bits = false;
			    break;
			}
		}
		...
		if (is_7bits) {
		    //byte[] encoded = new byte[string.length()];
		    //for (int ii=0; ii<encoded.length; ii++) {
		    //encoded[ii] = (byte)string.charAt(ii);
		    //}
		        byte[] encoded=string.getBytes("US-ASCII");
			out.write(Opcodes.BINSTRING);
