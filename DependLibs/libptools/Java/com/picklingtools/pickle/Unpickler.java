...
import com.picklingtools.pickle.objects.AnyClassConstructor;
import com.picklingtools.pickle.objects.ArrayConstructor;
import com.picklingtools.pickle.objects.ByteArrayConstructor;
import com.picklingtools.pickle.objects.ComplexNumber;
import com.picklingtools.pickle.objects.DateTimeConstructor;
import com.picklingtools.pickle.objects.SetConstructor;
...
public class Unpickler {
	...
	void load_empty_list() {
	    //RTS: stack.add(new ArrayList<Object>(0));
	    stack.add(new Arr());
	}

	void load_empty_dictionary() {
	    // RTS: stack.add(new HashMap<Object, Object>(0));
	    stack.add(new Tab());
	}

	void load_list() {
	    // RTS: ArrayList<Object> top = stack.pop_all_since_marker();
	        Arr top = stack.pop_all_since_marker();
		stack.add(top); // simply add the top items as a list to the stack again
	}

	void load_dict() {
		ArrayList<Object> top = stack.pop_all_since_marker();
		// RTS: HashMap<Object, Object> map = new HashMap<Object, Object>(top.size());
		Tab map = new Tab(top.size());
		for (int i = 0; i < top.size(); i += 2) {
		        Object key = top.get(i);
		    // RTS: String key = (String)top.get(i);
	...
	void load_append() {
		...
		// RTS: ArrayList<Object> list = (ArrayList<Object>) stack.peek();
		Arr list = (Arr) stack.peek();
		list.add(value);
	}
	...
	void load_setitem() {
	        ...
	        // RTS: String key = (String)stack.pop();
		@SuppressWarnings("unchecked")
		// RTS: Map<String, Object> dict = (Map<String, Object>) stack.peek();
		Map<Object, Object> dict = (Map<Object, Object>) stack.peek();
		dict.put(key, value);
	}

	void load_setitems() {
	       //RTS: HashMap<Object, Object> newitems = new HashMap<Object, Object>();
	        Tab newitems = new Tab();
		Object value = stack.pop();
