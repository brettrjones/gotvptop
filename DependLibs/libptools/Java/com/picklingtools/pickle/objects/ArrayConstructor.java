...
import com.picklingtools.pickle.PickleUtils;
...
public class ArrayConstructor implements IObjectConstructor {
	...
	public Object construct(Object[] args) throws PickleException {
		...
		//System.out.println(ii+": type:"+ args[ii].getClass().getName()+ " \n   args[ii]= " + args[ii]);
		//if (args[ii] instanceof String) {
		//    String ss = (String)args[ii];
		...
		//@SuppressWarnings("unchecked")
			...
			//try {
				...
				ba[ii] = (byte)s.charAt(ii);
			    }
			    //} catch (UnsupportedEncodingException e) {
			    //ba = s.getBytes();
			    //}
			char cc = typecode.charAt(0);
			...
			case 'c':// character 1 -> char[]
				...
				int jj = 0;
				for (int ii=0; ii<s.length(); ii+=1) {
				    result[jj++] = (char)s.charAt(ii);
				}
				return result;
			    }
			case 'u':// Unicode character 2 -> char[]
				...
				for (int ii=0; ii<s.length(); ii+=2) {
				    int temp=PickleUtils.bytes_to_integer(ba, ii, 2);
				    result[jj++] = (char)temp;
				}
				return result;
			    }
			...
			case 'B':// unsigned integer 1 -> short[]
				...
				int jj = 0;
				for (int ii=0; ii<s.length(); ii+=1) {
				    result[jj++] = (short)s.charAt(ii); 
				}
				return result;
			    }
			case 'h':// signed integer 2 -> short[]
			    {
				short[] result = new short[s.length()/2];
				int jj = 0;
				for (int ii=0; ii<s.length(); ii+=2) {
				    int temp = 
					PickleUtils.bytes_to_integer(ba, ii, 2);
				    result[jj++] = (short)temp;
			...
			case 'l':// signed integer 4 -> int[]
			    {
				int inc = (cc=='l' || cc=='i') ? 4 : 2;
				int[] result = new int[s.length()/inc];
				int jj = 0;
				for (int ii=0; ii<s.length(); ii+=inc) {
				    result[jj++] = 
					PickleUtils.bytes_to_integer(ba,ii,inc);
				}
				return result;
			    }
			...
			case 'L':// unsigned integer 4 -> long[]
			    {
				long[] result = new long[s.length()/4];
				int jj = 0;
				for (int ii=0; ii<s.length(); ii+=4) {
				    result[jj++] = 
					PickleUtils.bytes_to_long(ba, ii);
				}
				return result;
			    }
			case 'f':// floating point 4 -> float[]
				...
				//int jj = 0;
				//for (int ii=0; ii<s.length(); ii+=4) {
				//    result[jj++] = PickleUtils.bytes_to_float(ba, ii);
				//}
				return constructFloatArray(0, ba);
			    }
			case 'd':// floating point 8 -> double[]
			    {
				//double[] result = new double[s.length()/8];
				//int jj = 0;	
				//for (int ii=0; ii<s.length(); ii+=8) {
				//    result[jj++] = PickleUtils.bytes_to_double(ba, ii);
				//}
				//return result;
				return constructDoubleArray(0, ba);
			    }
			default:
