...
static int build_session_id1(ssh_session session, ssh_string servern,
...
#endif
  md5_update_ssh(md5,ssh_string_data(hostn),ssh_string_len(hostn));
  md5_update_ssh(md5,ssh_string_data(servern),ssh_string_len(servern));
  md5_update_ssh(md5,session->next_crypto->server_kex.cookie,8);
  ...
  md5_final_ssh(session->next_crypto->session_id,md5);
#ifdef DEBUG_CRYPTO
