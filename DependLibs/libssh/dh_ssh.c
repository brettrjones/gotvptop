...
/* todo: remove it */
#include "libssh/libssh_string.h"
...
int ssh_get_pubkey_hash(ssh_session session, unsigned char **hash) {
  ...
  *hash = NULL;
  ...
  md5_update_ssh(ctx, ssh_string_data(pubkey), ssh_string_len(pubkey));
  md5_final_ssh(h, ctx);
    ...
    case SSH_PUBLICKEY_HASH_MD5:
        {
            ...
            md5_update_ssh(ctx, ssh_string_data(blob), ssh_string_len(blob));
            md5_final_ssh(h, ctx);
