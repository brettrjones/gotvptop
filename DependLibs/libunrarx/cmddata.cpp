...
#define SILENT 1 // solves link error for ListArchive
...
void CommandData::OutHelp()
{
	...
	for (int I = 0; I < sizeof(Help) / sizeof(Help[0]); I++)
	{
		...
		if (Found)      continue;
#endif
#if !defined(_UNIX) && !defined(TARGET_OS_WINDOWS)
...
{
#if defined(TARGET_OS_WINDOWS) || defined(_EMX)
	return(Ch == '-' || Ch == '/');
#else
	return(Ch == '-');
#endif
	...
	else
	{
		...
		for (Attr = 0; *Str; Str++)
			...
			case 'V':
				Attr |= S_IFCHR;
				break;
#elif defined(TARGET_OS_WINDOWS) || defined(_EMX)
			case 'R':
				Attr |= 0x1;
				break;
			case 'H':
