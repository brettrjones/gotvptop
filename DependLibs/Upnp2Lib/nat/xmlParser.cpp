...
{
    LPTSTR dest=(LPTSTR)malloc((lengthXMLString(source)+1)*sizeof(wxChar));
...
LPTSTR toXMLStringFast(LPTSTR *dest,int *destSz, LPCTSTR source)
{
    ...
    if (l>*destSz) { *destSz=l; *dest=(LPTSTR)realloc(*dest,l*sizeof(wxChar)); }
    return toXMLString(*dest,source);
}
    ...
    while (((lo--)>0)&&(*s))
    {
        if (*s==_T('&'))
        {
            ...
            else
            {
                ...
                d=(LPTSTR)malloc((ll+1)*sizeof(wxChar));
                d[ll]=0;
                while(ll--) d[ll]=s[ll];
#ifdef _UNICODE
...
static inline wxChar getNextChar(XML *pXML)
{
    wxChar ch = pXML->lpXML[pXML->nIndex];
    if (ch!=0) pXML->nIndex++;
    return ch;
}

// Find next non-white space character.
static wxChar FindNonWhiteSpace(XML *pXML)
{
    wxChar ch;
    int nExit = FALSE;
...
LPTSTR stringDup(LPCTSTR lpszData, int cbData)
{
    ...
    lpszNew = (LPTSTR)malloc((cbData+1) * sizeof(wxChar));
    if (lpszNew)
    {
        memcpy(lpszNew, lpszData, (cbData) * sizeof(wxChar));
        lpszNew[cbData] = (wxChar)NULL;
    }
    return lpszNew;
}
...
static inline void charmemset(LPTSTR dest,wxChar c,int l) { while (l--) *(dest++)=c; }
// private:
...
#define LENSTR(lpsz) (lpsz ? _tcslen(lpsz) : 0)
    ...
    if (cbElement)
    {
        ...
        if (lpszMarker)
        {
            if (cb) charmemset(lpszMarker, INDENwxChar, sizeof(wxChar)*cb);
            nResult = cb;
        ...
        case eNodeText:
            ...
            if (cb)
            {
                if (nFormat!=-1)
                {
                    if (lpszMarker)
                    {
                        charmemset(&lpszMarker[nResult],INDENwxChar,sizeof(wxChar)*(nFormat + 1));
        ...
        if (bHasChildren)
        {
            ...
            if (lpszMarker)
            {
                if (nFormat != -1)
                {
                    if (nFormat)
                    {
                        charmemset(&lpszMarker[nResult], INDENwxChar,sizeof(wxChar)*nFormat);
                        nResult+=nFormat;
                    }
                }
...
LPTSTR XMLNode::createXMLString(int nFormat, int *pnSize)
{
    ...
    lpszResult=(LPTSTR)malloc((cbStr+1)*sizeof(wxChar));
