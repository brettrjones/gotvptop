...
#include "libavutil/avutil_intmath.h"
...
static int decode_receive_frame_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (!ret) {
        ...
        /* BRJ this can happen if decoding mjpeg as the frame->private_ref is not set.. instead picture_ptr->private_ref is set
            therefore do check instead of asserting
            av_assert0((frame->private_ref && frame->private_ref->size == sizeof(FrameDecodeData)) ||
                   !(avctx->codec->capabilities & AV_CODEC_CAP_DR1));
        */
        // BRJ check for valid ref
        if( ( frame->private_ref && frame->private_ref->size == sizeof( FrameDecodeData ) ) ||
                    !( avctx->codec->capabilities & AV_CODEC_CAP_DR1 ) );
        {
