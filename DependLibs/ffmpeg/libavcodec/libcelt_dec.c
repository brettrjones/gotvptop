...
#include <libcelt/celt_header_libcelt.h>
#include "avcodec.h"
...
static int ff_celt_bitstream_version_hack(CELTMode *mode)
{
    ...
    libcelt_header_init(&header, mode, 960, 2);
...
static av_cold int libcelt_dec_init(AVCodecContext *c)
{
    ...
    celt->mode = libcelt_mode_create(c->sample_rate, c->frame_size, &err);
    if (!celt->mode)
        return ff_celt_error_to_averror(err);
    celt->dec = libcelt_decoder_create_custom(celt->mode, c->channels, &err);
    if (!celt->dec) {
        libcelt_mode_destroy(celt->mode);
        return ff_celt_error_to_averror(err);
    }
...
static av_cold int libcelt_dec_close(AVCodecContext *c)
{
    ...
    libcelt_decoder_destroy(celt->dec);
    libcelt_mode_destroy(celt->mode);
    return 0;
}
                              ...
                              int *got_frame_ptr, AVPacket *pkt)
{
    ...
    err = libcelt_decode(celt->dec, pkt->data, pkt->size, pcm, c->frame_size);
    if (err < 0)
        return ff_celt_error_to_averror(err);
    if (celt->discard) {
