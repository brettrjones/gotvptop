 ...
 */
static GOTV_INLINE int ls_get_code_regular(GetBitContext *gb, JLSState *state, int Q)
{
    int k, ret;
    ...
    if (!state->near_jpeg && !k && (2 * state->B[Q] <= -state->N[Q]))
        ret = -(ret + 1);
 ...
 */
static GOTV_INLINE int ls_get_code_runterm(GetBitContext *gb, JLSState *state,
 ...
 */
static GOTV_INLINE void ls_decode_line(JLSState *state, MJpegDecodeContext *s,
                                  ...
                                  int stride, int comp, int bits)
{
    ...
    while (x < w) {
        ...
        D2 = Rc - Ra;
        /* run mode */
        if ((FFABS(D0) <= state->near_jpeg) &&
            (FFABS(D1) <= state->near_jpeg) &&
            (FFABS(D2) <= state->near_jpeg)) {
            int r;
            int RItype;
            ...
            Rb     = R(last, x);
            RItype = (FFABS(Ra - Rb) <= state->near_jpeg) ? 1 : 0;
            err    = ls_get_code_runterm(&s->gb, state, RItype,
                                         ff_log2_run[state->run_index[comp]]);
            if (state->run_index[comp])
                state->run_index[comp]--;

            if (state->near_jpeg && RItype) {
                pred = Ra + err;
            } else {
                if (Rb < Ra)
                    pred = Rb - err;
        ...
        if (state->near_jpeg) {
            if (pred < -state->near_jpeg)
                pred += state->range * state->twonear;
            else if (pred > state->maxval + state->near_jpeg)
                pred -= state->range * state->twonear;
...
int ff_jpegls_decode_picture(MJpegDecodeContext *s, int near_jpeg,
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO) {
        av_log(s->avctx, AV_LOG_DEBUG,
               ...
               "RESET=%i, LIMIT=%i, qbpp=%i, RANGE=%i\n",
                s->width, s->height, state->near_jpeg, state->maxval,
