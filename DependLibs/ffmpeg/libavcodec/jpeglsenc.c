                                     ...
                                     int err)
{
    ...
    map = !state->near_jpeg && !k && (2 * state->B[Q] <= -state->N[Q]);
                                  ...
                                  int stride, int comp, int bits)
{
    ...
    while (x < w) {
        ...
        if ((FFABS(D0) <= state->near_jpeg) &&
            ...
            run    = 0;
            RUNval = Ra;
            while (x < w && (FFABS(R(cur, x) - RUNval) <= state->near_jpeg)) {
                run++;
                W(cur, x, Ra);
                x += stride;
            }
            ...
            if (x >= w)
                return;
            Rb     = R(last, x);
            RItype = FFABS(Ra - Rb) <= state->near_jpeg;
            pred   = RItype ? Ra : Rb;
            err    = R(cur, x) - pred;
            ...
            if (state->near_jpeg) {
                if (err > 0)
                    err =  (state->near_jpeg + err) / state->twonear;
                else
                    err = -(state->near_jpeg - err) / state->twonear;
...
static void ls_store_lse(JLSState *state, PutBitContext *pb)
{
    ...
    state2.near_jpeg = state->near_jpeg;
