...
uint64_t GetTickCount64()
{
    ...
    return ( (int64_t)now.tv_sec * 1000L ) + ( (int64_t)now.tv_nsec / 1000 );
#else
    struct timeval tv;
...
static void InitializeTimeIfNeeded( )
{
    ...
    int64_t localTimeMs = ( ( int64_t )time( NULL ) * 1000 ); // only second resolution but does not matter as an offset
