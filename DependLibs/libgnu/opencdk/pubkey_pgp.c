...
static uint16_t checksum_mpi(bigint_t m)
{
 ...
 **/
uint32_t
cdk_pk_fingerprint_get_keyid(const byte * fpr, size_t fprlen, uint32_t * keyid)
{
	uint32_t lowbits = 0;
 ...
 **/
uint32_t cdk_pk_get_keyid(cdk_pubkey_t pk, uint32_t * keyid)
{
	uint32_t lowbits = 0;
	byte buf[24];
 ...
 **/
uint32_t cdk_sk_get_keyid(cdk_pkt_seckey_t sk, uint32_t * keyid)
{
	uint32_t lowbits = 0;
 ...
 **/
uint32_t cdk_sig_get_keyid(cdk_pkt_signature_t sig, uint32_t * keyid)
{
	uint32_t lowbits = sig ? sig->keyid[1] : 0;
...
uint32_t _cdk_pkt_get_keyid(cdk_packet_t pkt, uint32_t * keyid)
{
	uint32_t lowbits;
