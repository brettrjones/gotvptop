...
cdk_error_t
cdk_keydb_get_bykeyid(cdk_keydb_hd_t hd, uint32_t * keyid,
		      cdk_kbnode_t * ret_key)
{
	cdk_error_t rc;
...
static cdk_kbnode_t
keydb_find_bykeyid(cdk_kbnode_t root, const uint32_t * keyid, int search_mode)
{
	cdk_kbnode_t node;
	uint32_t kid[2];
 ...
 **/
cdk_error_t
cdk_keydb_get_pk(cdk_keydb_hd_t hd, uint32_t * keyid, cdk_pubkey_t * r_pk)
{
 ...
 **/
cdk_error_t
cdk_keydb_get_sk(cdk_keydb_hd_t hd, uint32_t * keyid, cdk_seckey_t * ret_sk)
{
...
static int is_selfsig(cdk_kbnode_t node, const uint32_t * keyid)
{
...
static cdk_error_t keydb_merge_selfsig(cdk_kbnode_t key, uint32_t * keyid)
{
...
keydb_parse_allsigs(cdk_kbnode_t knode, cdk_keydb_hd_t hd, int check)
{
	...
	cdk_pkt_pubkey_t pk;
	cdk_subpkt_t s = NULL;
	uint32_t expiredate = 0, curtime = (uint32_t) gnutls_time(NULL);
	uint32_t keyid[2];
...
static void
add_key_usage(cdk_kbnode_t knode, uint32_t keyid[2], unsigned int usage)
{
	cdk_kbnode_t p, ctx;
	cdk_packet_t pkt;
...
cdk_error_t
_cdk_keydb_check_userid(cdk_keydb_hd_t hd, uint32_t * keyid, const char *id)
{
 ...
 * in the key database.
 **/
cdk_error_t cdk_keydb_check_sk(cdk_keydb_hd_t hd, uint32_t * keyid)
{
	cdk_stream_t db;
	cdk_packet_t pkt;
	cdk_error_t rc;
