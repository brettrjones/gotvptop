...
//#if 0 // BRJ fips mode is us government encryption standard and NOT recommended
...
{
//#if 0 // microsoft and others recommend do NOT use fips mode ( us government enforced standard crypto )
	static int done;
	gpg_error_t err;

	/* Make sure we are not accidental called twice.  */
	if( done )
	{
		if( fips_mode() )
		{
...
#ifdef HAVE_SYSLOG
			LogMsg( LOG_ERROR, "Libgcrypt error: "
...
{
#if 0 // BRJ fips is us government encryption standard and not recommended
	...
	if( !inactive_fips_mode )
	{
		...
		unlock_fsm();
#ifdef HAVE_SYSLOG
		LogMsg( LOG_WARN, "Libgcrypt warning: "
...
{
#ifdef HAVE_SYSLOG
	LogMsg( LOG_ERROR, "Libgcrypt terminated the application" );
#endif /*HAVE_SYSLOG*/
	fflush( NULL );
	abort();
	/*NOTREACHED*/
...
//#endif // #if 0 // BRJ fips mode is us government encryption standard and NOT recommended
