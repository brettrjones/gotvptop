...
/* Qt .qm files are read by the QTranslator::load() function and written
   ...
   in qt-3.0.5, is as follows:
     ...
     It's a binary data format. Elements are u8 (byte), uint16_t, uint32_t. They are
     ...
     has this structure:
       struct {
         ...
         uint32_t length; // number of bytes of the data
         u8 data[length];
       };
     ...
     structure:
       It's a sorted array of
         struct {
           uint32_t hashcode; // elfHash of the concatenation of msgid and
                         // disambiguating-comment
           uint32_t offset; // offset within the data[] of the messages section
         };
...
/* Write a uint16_t (two bytes) to the output stream.  */
static inline void
...
/* Write a uint32_t (four bytes) to the output stream.  */
static inline void
...
/* Add a uint16_t (two bytes) to an obstack.  */
static void
...
/* Add a uint32_t (four bytes) to an obstack.  */
static void
