...
static bool string_equals( const void *x1, const void *x2 )
{
...
open_temp( const char *file_name, int flags, mode_t mode )
{
  ...
  /* Use _O_TEMPORARY when possible, to increase the chances that the
	...
	if( supports_delete_on_close() )
		fd = open_os( file_name, flags | _O_TEMPORARY, mode );
	else
#endif
		fd = open_os( file_name, flags, mode );
	saved_errno = errno;
	if( fd >= 0 )
		register_fd( fd );
	unblock_fatal_signals();
