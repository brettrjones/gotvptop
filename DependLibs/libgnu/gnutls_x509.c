...
#include <libgnu/ocsp.h>
//#include "x509/gnutils_common.h"
	       ...
	       const char *certfile, gnutls_x509_crt_fmt_t type)
{
	...
	data = read_binary_file_gnu(certfile, &size);
	      ...
	      const char *pass, unsigned int flags)
{
	...
	data = read_binary_file_gnu(keyfile, &size);
...
{
	int ret, i, j;
	gnutls_x509_crt_t * new_list = ( gnutls_x509_crt_t *)alloca( sizeof( gnutls_x509_crt_t  ) * ca_list_size );
...
int
 gnutls_certificate_set_x509_simple_pkcs12_file
	...
	p12blob.data = (void *) read_binary_file_gnu(pkcs12file, &size);
