...
#if defined(HAVE_FCNTL) && defined(HAVE_FTRUNCATE) && !defined(TARGET_OS_WINDOWS)
...
#endif
#if LOCK_SEED_FILE && !defined( TARGET_OS_WINDOWS )
  struct flock lck;
  struct timeval tv;
  int backoff=0;
...
read_seed_file (void)
{
  ...
  fd = open_os( seed_file_name, O_RDONLY | O_BINARY );
#else
  fd = open_os( seed_file_name, O_RDONLY );
#endif
...
# else
    fd = open_os(seed_file_name, O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR );
# endif
#endif
  ...
  if (fd == -1 )
  ...
  else
    {
      do
        {
          i = write_os(fd, keypool, POOLSIZE );
        }
