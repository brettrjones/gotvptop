...
/* Fat library initialization works as follows. The main function is
   nettle_fat_init. We try to do initialization only once, but since it is
   ...
   The nettle_fat_init function checks the cpuid flags, and sets function
   ...
   We try to register nettle_fat_init as a constructor function to be called
   at load time. If this is unavailable or non-working, we instead
   arrange nettle_fat_init to be called lazily.
   ...
   * If ifunc support is available, function pointers are statically
     ...
     _nettle_aes_encrypt_resolve, which call nettle_fat_init, and then return
   ...
   * If ifunc is not available, we have to define a wrapper function
     ...
     which calls nettle_fat_init, and then invokes the right function. This
...
extern void nettle_fat_init( void );
