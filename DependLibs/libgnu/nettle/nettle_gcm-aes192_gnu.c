...
{
  GCM_SET_KEY(ctx, nettle_aes192_set_encrypt_key, nettle_aes192_encrypt, key);
}
...
{
  GCM_ENCRYPT(ctx, nettle_aes192_encrypt, length, dst, src);
}
...
nettle_gcm_aes192_decrypt(struct gcm_aes192_ctx *ctx,
		   size_t length, uint8_t *dst, const uint8_t *src)
{
  GCM_DECRYPT(ctx, nettle_aes192_encrypt, length, dst, src);
}
...
{
  GCM_DIGEST(ctx, nettle_aes192_encrypt, length, digest);
}
