...
{
  ccm_set_nonce(&ctx->ccm, &ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
...
{
  ccm_update(&ctx->ccm, &ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
	     length, data);
}
...
{
  ccm_encrypt(&ctx->ccm, &ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
	      length, dst, src);
}
...
ccm_aes128_decrypt(struct ccm_aes128_ctx *ctx,
		   size_t length, uint8_t *dst, const uint8_t *src)
{
  ccm_decrypt(&ctx->ccm, &ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
	      length, dst, src);
}
...
{
  ccm_digest(&ctx->ccm, &ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
	     length, digest);
}
...
{
  ccm_encrypt_message(&ctx->cipher, (nettle_cipher_func *) nettle_aes128_encrypt,
