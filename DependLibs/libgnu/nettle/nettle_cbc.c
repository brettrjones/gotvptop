  ...
  if (src != dst)
    {
      ...
      nettle_memxor(dst, iv, block_size);
      nettle_memxor(dst + block_size, src, length - block_size);
  ...
  else
    {
	 ...
	 depend on nettle_memxor3 working from the end of the area, allowing
	    ...
	    length -= buffer_size, src += buffer_size, dst += buffer_size)
	{
	  ...
	  nettle_memxor3(dst + block_size, buffer + block_size, src,
		  buffer_size - block_size);
	  nettle_memxor3(dst, buffer, initial_iv, block_size);
	}
...
static void foo(void)
{
  ...
  CBC_ENCRYPT(&ctx, nettle_aes_encrypt, TWOFISH_BLOCK_SIZE, dst, src);
...
static void foo2(void)
{
  ...
  /* Should result in a warning */
  CBC_ENCRYPT2(&ctx, nettle_aes_encrypt, TWOFISH_BLOCK_SIZE, iv, TWOFISH_BLOCK_SIZE, dst, src);
}
