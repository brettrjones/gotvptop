	      ...
	      unsigned pstring_size, const uint8_t * pstring)
{
	...
	nettle_aes_set_encrypt_key(&ctx->key, DRBG_AES_KEY_SIZE, tmp);
		...
		const uint8_t pdata[DRBG_AES_SEED_SIZE])
{
	...
	while (len < DRBG_AES_SEED_SIZE) {
		...
		nettle_aes_encrypt(&ctx->key, AES_BLOCK_SIZE, t, ctx->v);
		t += AES_BLOCK_SIZE;
		len += AES_BLOCK_SIZE;
	}

	nettle_memxor(tmp, pdata, DRBG_AES_SEED_SIZE);
		...
		unsigned add_size, const uint8_t * add)
{
	...
	nettle_memxor(tmp, entropy, entropy_size);
			...
			unsigned add_size, const uint8_t *add)
{
	...
	if (ctx->prev_block_present == 0) {
		INCREMENT(sizeof(ctx->v), ctx->v);
		nettle_aes_encrypt(&ctx->key, AES_BLOCK_SIZE, ctx->prev_block, ctx->v);
	...
	for (left = length; left >= AES_BLOCK_SIZE;
		...
		INCREMENT(sizeof(ctx->v), ctx->v);
		nettle_aes_encrypt(&ctx->key, AES_BLOCK_SIZE, dst, ctx->v);
	...
	if (left > 0) {		/* partial fill */
		...
		INCREMENT(sizeof(ctx->v), ctx->v);
		nettle_aes_encrypt(&ctx->key, AES_BLOCK_SIZE, tmp, ctx->v);
