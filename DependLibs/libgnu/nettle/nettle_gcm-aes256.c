...
{
  GCM_SET_KEY(ctx, nettle_aes256_set_encrypt_key, nettle_aes256_encrypt, key);
}
...
{
  GCM_ENCRYPT(ctx, nettle_aes256_encrypt, length, dst, src);
}
...
nettle_gcm_aes256_decrypt(struct gcm_aes256_ctx *ctx,
		   size_t length, uint8_t *dst, const uint8_t *src)
{
  GCM_DECRYPT(ctx, nettle_aes256_encrypt, length, dst, src);
}
...
{
  GCM_DIGEST(ctx, nettle_aes256_encrypt, length, digest);
}
