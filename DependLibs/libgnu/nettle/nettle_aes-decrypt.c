	    ...
	    const uint8_t *src)
{
  ...
  _nettle_aes_encrypt(ctx->rounds, ctx->keys, &_aes_decrypt_table,
	       length, dst, src);
}
...
{
  assert(!(length % AES_BLOCK_SIZE) );
  _nettle_aes_encrypt(_AES128_ROUNDS, ctx->keys, &_aes_decrypt_table,
	       length, dst, src);
}
...
{
  assert(!(length % AES_BLOCK_SIZE) );
  _nettle_aes_encrypt(_AES192_ROUNDS, ctx->keys, &_aes_decrypt_table,
	       length, dst, src);
}
...
{
  assert(!(length % AES_BLOCK_SIZE) );
  _nettle_aes_encrypt(_AES256_ROUNDS, ctx->keys, &_aes_decrypt_table,
	       length, dst, src);
}
