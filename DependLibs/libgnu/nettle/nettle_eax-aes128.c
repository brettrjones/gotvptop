...
{
  EAX_SET_KEY(ctx,
	      nettle_aes128_set_encrypt_key, nettle_aes128_encrypt,
	      key);
}
...
{
  EAX_SET_NONCE(ctx, nettle_aes128_encrypt, length, iv);
}

void
eax_aes128_update(struct eax_aes128_ctx *ctx, size_t length, const uint8_t *data)
{
  EAX_UPDATE(ctx, nettle_aes128_encrypt, length, data);
}
...
{
  EAX_ENCRYPT(ctx, nettle_aes128_encrypt, length, dst, src);
}
...
eax_aes128_decrypt(struct eax_aes128_ctx *ctx,
		   size_t length, uint8_t *dst, const uint8_t *src)
{
  EAX_DECRYPT(ctx, nettle_aes128_encrypt, length, dst, src);
}
...
{
  EAX_DIGEST(ctx, nettle_aes128_encrypt, length, digest);
}
