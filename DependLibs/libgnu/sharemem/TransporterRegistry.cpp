...
TransporterRegistry::init(NodeId nodeId) {
  ...
  DEBUG_OUT ("TransporterRegistry started node: " << localNodeId);
...
TransporterRegistry::removeTransporter(NodeId nodeId) {
  ...
  DEBUG_OUT ("Removing transporter from " << localNodeId
	<< " to " << nodeId);
...
TransporterRegistry::prepareSend(const SignalHeader * const signalHeader, 
  ...
  if(t != NULL && 
	...
	return SEND_MESSAGE_TOO_BIG;
      ...
      DEBUG_OUT ("Signal to " << nodeId << " lost(disconnect) ");
      return SEND_DISCONNECTED;
    }
  } else {
    DEBUG_OUT ("Discarding message to block: " 
...
TransporterRegistry::setIOState(NodeId nodeId, IOState state) {
  DEBUG_OUT ("TransporterRegistry::setIOState("
	<< nodeId << ", " << state << ")");
