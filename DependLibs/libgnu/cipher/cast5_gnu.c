...
# ifdef HAVE_COMPATIBLE_GCC_ARM_PLATFORM_AS && HAVE_ARM_ASM
...
typedef struct {
    ...
    byte Kr[16];
#ifdef USE_ARM_ASM
    uint32_t Kr_arm_enc[16 / sizeof(uint32_t)];
    uint32_t Kr_arm_dec[16 / sizeof(uint32_t)];
#endif
} CAST5_context;
...
const uint32_t _gcry_cast5_s1to4[4][256] = { {
...
static void
key_schedule( uint32_t *x, uint32_t *z, uint32_t *k )
{
