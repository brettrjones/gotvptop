...
_salsa20_core(uint32_t *dst, const uint32_t *src, unsigned rounds)
{
  ...
  for (i = 0; i < SALSA20_INPUT_LENGTH; i++)
    {
      uint32_t t = x[i] + LE_SWAP32(src[i]);
      dst[i] = LE_SWAP32(t);
    }
}
...
static void
_scryptBlockMix (uint32_t r, unsigned char *B, unsigned char *tmp2)
{
  uint64_t i;
  ...
  for (i = 0; i <= 2 * r - 1; i++)
    {
      ...
      /* X = Salsa (T) */
      _salsa20_core ((uint32_t*)X, (uint32_t*)X, 8);
...
static void
_scryptROMix (uint32_t r, unsigned char *B, uint64_t N,
