...
qcopy_file_preserving (const char *src_filename, const char *dest_filename)
{
  ...
  src_fd = open_os(src_filename, O_RDONLY | O_BINARY);
  if (src_fd < 0)
    {
  ...
  dest_fd = open_os (dest_filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
  if (dest_fd < 0)
    {
...
#if !USE_ACL
  if (close_os (dest_fd) < 0)
    {
      err = GL_COPY_ERR_WRITE;
      goto error_src;
    }
  if (close_os (src_fd) < 0)
    {
