...
#ifdef _MSC_VER
#define log LogMsg
#define log_errno LogMsg
#define bug LogMsg
#define log_message LogMsg
...
no_mem_exit			(void)
{
	LogMsg( LOG_FATAL, "Out of memory.");
}
	...
	if (likely (1 == adaptation_field_control)) {
		...
		if (adaptation_field_length > 0) {
			...
			if (discontinuity_indicator) {
				LogMsg( LOG_DEBUG,  "TS discontinuity\n");
				ts_subt_reset ();
			}
		}
