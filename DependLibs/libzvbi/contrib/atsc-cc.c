...
#  include "config_libzvbi.h"
#ifdef BUILD_ATSC_CC_APP
...
#undef DLIST_PARENT
#define DLIST_PARENT(_ptr, _type, _member) ({					\
...
enum edge {
	EDGE_NONE = 0,
	EDGE_RAISED_ATSC, // conflicts with windows.. changed EDGE_RAISED to EDGE_RAISED_ATSC
	EDGE_DEPRESSED,
	EDGE_UNIFORM,
	EDGE_SHADOW_LEFT,
	EDGE_SHADOW_RIGHT
...
static bool				capture_thread_running = 0;
...
no_mem_exit			(void)
{
	LogMsg( LOG_FATAL, "atsc-cc Out of memory." );
}
...
do {									\
	...
	if (-1 == r) {							\
		LogMsg( LOG_FATAL, #request " failed");			\
	}								\
} while (0)
...
open_output_file		(const char *		name)
{
	...
	if (NULL == name || 0 == strcmp (name, "-")) {
		...
		fp = fopen (name, "a");
		if (NULL == fp) {
			LogMsg( LOG_FATAL, "Cannot open output file '%s'",
				    name);
		}
	}
...
open_test_file			(const char *		name)
{
	...
	if (NULL == name || 0 == strcmp (name, "-")) {
		...
		fp = fopen (name, "r");
		if (NULL == fp) {
			LogMsg( LOG_FATAL, "Cannot open test file '%s'",
				    name);
		}
	}
				 ...
				 const char *		extension)
{
	...
	if (0 != stat (buf, &st)) {
		LogMsg( LOG_FATAL, "Cannot open '%s'", buf);
	} else if (!S_ISDIR(st.st_mode)) {
		LogMsg( LOG_FATAL, "'%s' is not a directory.\n", buf);
	}
	...
	if (0 != stat (buf, &st)) {
		...
		if (-1 == mkdir (buf, /* mode */ 0777))
			LogMsg( LOG_FATAL, "Cannot create '%s'", buf);
	} else if (!S_ISDIR(st.st_mode)) {
		LogMsg( LOG_FATAL, "'%s' is not a directory.\n", buf);
	}
	...
	failed:
		LogMsg( LOG_FATAL, "Cannot open output file '%s'", buf);
	}

	free (buf);

	/* Will try again later. */
	LogMsg( LOG_DEBUG, "Cannot open output file '%s'", buf);
				 ...
				 vbi_bool		print_msec)
{
	...
	if (tm->tm_mday <= 0) {
		...
		if (NULL == gmtime_r (&sec, tm)) {
			/* Should not happen. */
			LogMsg( LOG_FATAL, "System time invalid.\n");
		}
	}
...
cr_minicut			(struct caption_recorder *cr,
				 ...
				 vbi_pgno		channel)
{
#ifndef _MSC_VER // FIXME will not compile is Visual Studio
	struct program *pr = DLIST_PARENT (cr, struct program, cr);
	...
	if (tm->tm_min != cr->minicut_min[channel - 1]) {
		...
		if (NULL != fp) {
			if (0 != fclose (fp)) {
				LogMsg( LOG_DEBUG, "Station %u CC file "
					   "write error",
					   station_num (pr));
			}
		}
				 ...
				 unsigned int		last_row)
{
	...
	for (row = first_row; row <= last_row; ++row) {
...

		{
			...
			pr = DLIST_PARENT (cd, struct program, cr.cc);
				 ...
				 unsigned int		row)
{
...

	{
		...
		pr = DLIST_PARENT (dc, struct program, cr.dtvcc);
				 ...
				 unsigned int		n_bytes)
{
	...
	case 0x80: /* CWx SetCurrentWindow */
	case 0x81:
	case 0x82:
	case 0x83:
	case 0x84:
				 ...
				 int64_t		pts)
{
	...
	if (dc->next_sequence_number >= 0
		...
		pr = DLIST_PARENT (dc, struct program, cr.dtvcc);
		LogMsg( LOG_DEBUG, "Station %u DTVCC packet lost.\n",
		     station_num (pr));
		dtvcc_reset (dc);
		return;
	}
	...
	if (packet_size > dc->packet_size) {
		...
		pr = DLIST_PARENT (dc, struct program, cr.dtvcc);
		LogMsg( LOG_DEBUG, "Station %u DTVCC packet incomplete (%u/%u).\n",
		     station_num (pr),
	...
	for (i = 0; i < 2; ++i) {
		...
		pr = DLIST_PARENT (dc, struct program, cr.dtvcc);
		LogMsg( LOG_DEBUG, "Station %u DTVCC invalid "
		     "syntactic element (%u).\n",
 ...
 service_block_incomplete:
	{
		...
		pr = DLIST_PARENT (dc, struct program, cr.dtvcc);
		LogMsg( LOG_DEBUG, "Station %u DTVCC incomplete "
		     "service block (%u).\n",
		     station_num (pr), i);
		if (option_debug & DEBUG_DTVCC_PACKET) {
			fprintf (stderr, "Packet (%d/%d):\n",
				 ...
				 unsigned int		n_bytes)
{
	...
	if (NULL != pr->cr.ccd.cc_data_tap_fp) {
		...
		if (out != fwrite (output_buffer, 1, out,
			...
			LogMsg( LOG_FATAL, "cc_data tap write error");
		}
	}
				 ...
				 const char *		test_file_name)
{
	...
	for (;;) {
		...
		if (ferror (test_fp)) {
			LogMsg( LOG_FATAL, "CC data file read error");
		} else {
			LogMsg( LOG_VERBOSE, "End of CC data file.\n");
			fclose (test_fp);
			return;
		}
	}
...
close_ts_file			(struct video_recorder *vr)
{
	...
	if (NULL != vr->minicut_fp) {
		if (0 != fclose (vr->minicut_fp)) {
			...
			pr = DLIST_PARENT (vr, struct program, vr);
			LogMsg( LOG_DEBUG, "TS stream %u close error",
...
{
#ifndef _MSC_VER // FIXME will not compile is Visual Studio
	struct program *pr = DLIST_PARENT (vr, struct program, vr);
	size_t actual;
	...
	if (pr->now.tv_sec >= vr->minicut_end) {
		...
		if (NULL != vr->minicut_fp) {
			...
			if (2 * 188 != actual) {
				LogMsg( LOG_DEBUG, "TS stream %u write error",
					   station_num (pr));
			}
		}
	}
...
{
#ifndef _MSC_VER // FIXME will not compile is Visual Studio
	struct program *pr = DLIST_PARENT (vd, struct program, vesd);
				 ...
				 unsigned int		min_bytes_valid)
{
	...
	case B_TYPE:
		   ...
		   references we lost that I or P picture. */
		{
			...
			pr = DLIST_PARENT (vd, struct program, vesd);
				 ...
				 vbi_bool		data_lost)
{
	...
	if (unlikely (NULL != vd->video_es_tap_fp)) {
		...
		if (n != fwrite (buf, 1, n, vd->video_es_tap_fp)) {
			LogMsg( LOG_FATAL, "Video ES tap write error");
		}
	}
}
				 ...
				 unsigned int		required)
{
	...
	if (unlikely (in + required > capacity)) {
		...
		if (required > capacity - unconsumed) {
			...
			LogMsg( LOG_FATAL, "Video ES buffer overflow.\n");
		}
				 ...
				 const char *		test_file_name)
{
	...
	for (;;) {
		...
		if (likely (actual == todo)) {
			...
			LogMsg( LOG_FATAL, "Video ES read error");
		} else {
			LogMsg( LOG_VERBOSE, "End of video ES file.\n");
			fclose (test_fp);
			return;
		}
	}
				 ...
				 unsigned int		es_num)
{
	...
	if (likely (1 == adaptation_field_control)) {
		...
		if (adaptation_field_length > 0) {
			...
			if (adaptation_field_length > 182) {
				LogMsg( LOG_DEBUG,  "Invalid TS header "
	...
	if (unlikely (0 != ((pr->tsd.next_ts_cc[es_num]
		...
		if (pr->tsd.next_ts_cc[es_num] < 0) {
			...
			LogMsg( LOG_DEBUG,  "TS continuity error "
			     "on station %u, stream %u.\n",
			     station_num (pr), pid);
			data_lost = TRUE;
		}
...
ts_decoder			(const uint8_t		buf[188])
{
	...
	if (unlikely (NULL != ts_tap_fp)) {
		if (188 != fwrite (buf, 1, 188, ts_tap_fp)) {
			LogMsg( LOG_FATAL, "TS tap write error");
		}
	}

	if (unlikely (buf[1] & 0x80)) {
		LogMsg( LOG_DEBUG,  "TS transmission error.\n");
...
ts_test_loop			(const char *		test_file_name)
{
	...
	for (;;) {
		...
		if (likely (actual == todo)) {
			...
			LogMsg( LOG_FATAL, "TS read error");
		} else {
			LogMsg( LOG_VERBOSE, "End of TS file.\n");
			fclose (test_fp);
			return;
		}
	}
...
ct_resync			(const uint8_t		buf[2 * 188])
{
	...
	LogMsg( LOG_VERBOSE, "Capture thread cannot synchronize.\n");
	...
	do {
		...
		if (actual > 0) {
			if (unlikely (actual >= todo)) {
				LogMsg( LOG_VERBOSE, "DVB device read size "
				     "error.\n");
				return FALSE;
			}
			...
			LogMsg( LOG_DEBUG, "DVB device read error");

			errno = saved_errno;

			return FALSE;
		} else {
			LogMsg( LOG_DEBUG,  "EOF from DVB device (ignored).\n");
			return FALSE;
		}
	} while (--retry > 0);

	LogMsg( LOG_DEBUG,  "DVB device read error: EINTR or "
	     "read size problem.\n");
		...
		while (s < e) {
			if (0x47 != s[0] || 0x47 != s[188]) {
				if (ts_n_packets_in > 0) {
					LogMsg( LOG_DEBUG,  "Capture thread "
					     "lost sync.\n");
				}
				 ...
				 int			flags)
{
	...
	if (!S_ISCHR (st.st_mode)) {
		LogMsg( LOG_FATAL, "'%s' is not a DVB device.\n",
			    dev_name);
	}
 ...
 open_failed:
	LogMsg( LOG_FATAL, "Cannot open '%s'", dev_name);
...
open_device			(void)
{
	...
	LogMsg( LOG_DEBUG,  "Opening dvb/adapter%lu.\n",
	...
	case FE_OFDM:
		if (fe_info.type != station->type) {
			LogMsg( LOG_FATAL, "'%s' is not %s device.\n",
				    dev_name,
	...
	case FE_QPSK:
	case FE_QAM:
		LogMsg( LOG_FATAL, "'%s' is not an ATSC device.\n",
			    dev_name);
		break;
	}
	...
	for (retry = 0;;) {
		...
		if (++retry > 20) {
			LogMsg( LOG_FATAL, "No signal detected.\n");
		}

		if (7 == (retry & 7)) {
			LogMsg( LOG_DEBUG,  "Waiting for a signal.\n");
		}

		usleep (250000);
	}

	LogMsg( LOG_DEBUG,  "Signal detected.\n");
	...
	if (0 != pthread_create (&capture_thread_id,
				 ...
				 capture_thread,
				 /* arg */ NULL)) {
		LogMsg( LOG_FATAL, "Cannot start capture thread");
	}

	LogMsg( LOG_DEBUG,  "Opened dvb/adapter%lu, tuned to %.3f MHz and "
				 ...
				 const char *		buffer)
{
	...
	if (NULL != st2) {
		LogMsg( LOG_FATAL, "Duplicate station name '%s' "
			    "in %s line %u.\n",
 ...
 invalid:
	LogMsg( LOG_FATAL, "Invalid %s field in '%s' line %u.\n",
...
get_channel_conf_name		(enum fe_type		type)
{
	...
	if (0 == strncmp (s, "~/", 2)) {
		home = getenv ("HOME");
		if (NULL == home) {
			LogMsg( LOG_FATAL, "Cannot open '%s' because the "
...
read_channel_conf		(void)
{
	...
	if (NULL == fp) {
		LogMsg( LOG_FATAL, "Cannot open '%s'",
			    channel_conf_name);
	}
	...
	if (ferror (fp) || 0 != fclose (fp)) {
		LogMsg( LOG_FATAL, "Error while reading '%s'",
			    channel_conf_name);
	}
...
list_programs			(void)
{
	...
	for (i = 0; i < n_programs; ++i) {
		...
		pr = &program_table[i];
		LogMsg( LOG_VERBOSE, "Station %u: '%s'\n",
		     station_num (pr),
		...
		if (pr->cr.usexds) {
			LogMsg( LOG_VERBOSE, "  XDS -> '%s'\n",
			     pr->cr.option_xds_output_file_name);
		}
		if (NULL != pr->option_minicut_dir_name) {
				LogMsg( LOG_VERBOSE, "  TS -> '%s/"
				     "YYYYMMDDHH0000/"
				     "YYYYMMDDHHMM00.ts'\n",
				     pr->option_minicut_dir_name);
		}
		if (NULL != pr->vesd.video_es_tap_fp) {
			...
			LogMsg( LOG_VERBOSE, "  V-ES -> '%s'\n",
			     name);
		}
		if (NULL != pr->vr.aesp.audio_es_tap_fp) {
			LogMsg( LOG_VERBOSE, "  A-ES -> '%s'\n",
			     pr->vr.aesp.option_audio_es_tap_file_name);
		}
		if (NULL != pr->cr.ccd.cc_data_tap_fp) {
			LogMsg( LOG_VERBOSE, "  cc_data -> '%s'\n",
...
look_up_station_names		(void)
{
	...
	for (i = 0; i < n_programs; ++i) {
		...
		if (NULL == st) {
			LogMsg( LOG_FATAL, "Station '%s' is unknown. List "
		...
		if (NULL == station) {
			...
			LogMsg( LOG_FATAL, "To receive multiple programs the "
				 ...
				 vbi_bool		have_xds_filter_option)
{
	if (NULL != pr->option_minicut_dir_name) {
		...
		if (!(pr->cr.usecc | pr->cr.usexds)) {
			LogMsg( LOG_FATAL, "Please give option -c or -x, "
				    "or -h for help.\n");
		}
				 ...
				 const char *		optarg)
{
	...
	if (i >= N_ELEMENTS (formats)) {
		LogMsg( LOG_FATAL, "Invalid caption format '%s'. "
...
debug_option			(const char *		optarg)
{
	...
	while (0 != *s) {
		...
		LogMsg( LOG_FATAL, "Invalid debugging switch '%s'. "
			    "Try --debug help.\n", optarg);
...
add_program			(void)
{
	...
	if (n_programs >= N_ELEMENTS (program_table)) {
		LogMsg( LOG_FATAL, "Sorry, too many programs.\n");
	}
...
invalid:
	LogMsg( LOG_FATAL, "Invalid date format '%s'\n", s);
dangit:
	LogMsg( LOG_FATAL, "Cannot parse dates, sorry.\n");
}
				 ...
				 char **		argv)
{
	...
	for (;;) {
		...
		case 'l':
			...
			if (ch < 1 || ch > 10) {
				LogMsg( LOG_FATAL, "Invalid caption stream "
					    "number %ld. The valid "
					    "range is 1 ... 10.\n",
					    ch);
			}
	...
	if (pr->cr.usesen
		...
		LogMsg( LOG_FATAL, "Sorry, option -s does not "
			    "combine with -m or -M.\n");
	}
	...
	if (SOURCE_DVB_DEVICE == option_source
		...
		if (n_programs > 1) {
			...
			LogMsg( LOG_VERBOSE, "Ignoring station name.\n");
		}
	}
 ...
 no_station:
	LogMsg( LOG_FATAL, "Please give a station name. "
		    "List all stations with the -L option.\n");

 too_many_stations:
	LogMsg( LOG_FATAL, "Sorry, only one program can be decoded with "
		    "the --cc-data, --es or\n"
		    "--pes option.\n");
}
				 ...
				 char **		argv)
{
...
#ifndef TARGET_OS_WINDOWS // mlockall not supported on windows
	mlockall (MCL_CURRENT | MCL_FUTURE);
#endif // TARGET_OS_WINDOWS // mlockall not supported on windows
	...
	case SOURCE_STDIN_PES:
		LogMsg( LOG_FATAL, "Sorry, the --pes option "
