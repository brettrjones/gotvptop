...
parse_mot(struct ttx_magazine *mag, uint8_t *raw, int packet)
{
	...
	case 22:/* level 3.5 pops */
	case 23:
		packet--;

	case 19: /* level 2.5 pops */
	case 20:
	{
		...
		for (i = 0; i < 4; raw += 10, pop++, i++) {
			...
			pop->pgno = (((n[0] & 7) ? 0 : 8) << 8) + (n[1] << 4) + n[2];
	...
	case 24:	/* level 3.5 drcs */
		...
		for (i = 0; i < 8; raw += 4, index++, i++) {
			...
			mag->drcs_link[0][index] = (((n[0] & 7) ? 0 : 8) << 8) + (n[1] << 4) + n[2];
	...
	int pgno, int code, int *subp_index)
{
	...
	if ( ( ( code >= 0x02 ) && ( code <= 0x4F ) )
		 || ( ( code >= 0x82 ) && ( code <= 0xCF ) ) )
	{
		subc = code & 0x7F;
		code = ( code >= 0x80 ) ? VBI_PROGR_SCHEDULE : VBI_NORMAL_PAGE;
	}
	else
	{
		switch( code )
...
//			printf("Hamming error in packet 0 page number\n");
		...
		if( cvtp->function == PAGE_FUNCTION_UNKNOWN ) {
			...
			if( ( ( ps->page_type >= 0x01 ) && ( ps->page_type <= 0x51 ) )
				|| ( ( ps->page_type >= 0x70 ) && ( ps->page_type <= 0x7F ) )
				|| ( ( ps->page_type >= 0x81 ) && ( ps->page_type <= 0xD1 ) )
				|| ( ( ps->page_type >= 0xF4 ) && ( ps->page_type <= 0xF7 ) )
				|| ( ps->page_type == VBI_TOP_BLOCK )
				|| ( ps->page_type == VBI_TOP_GROUP )
				)
			{
			...
			else
			{
				...
				case 0xE5:
				//case 0xE7: // same as VBI_SYSTEM_PAGE:
				case 0xE8:
				case 0xE9:
				case 0xEA:
				case 0xEB:
				...
				case 0xDE:
				case 0xDF:
				case 0xE0:	/* data broadcasting */
				case 0xE1:
				case 0xE2 :

				case 0xE4:		/* data broadcasting */
				case 0xF0:	/* broadcaster system page */
				case 0xF1:
				case 0xF2:
				case 0xF3:
