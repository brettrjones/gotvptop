...
#include <libzvbi/config_libzvbi.h>
...
for (verify_ring (l), p = DLIST_PARENT ((l)->_succ, (* p), _node);	\
     p1 = DLIST_PARENT (p->_node._succ, (* p), _node), 		\
     &p->_node != (l); p = p1)

#define FOR_ALL_NODES_REVERSE(p, p1, l, _node)				\
for (verify_ring (l), p = DLIST_PARENT ((l)->_pred, (* p), _node);	\
     p1 = DLIST_PARENT (p->_node._pred, (* p), _node),		\
     &p->_node != (l); p = p1)
#else
#define FOR_ALL_NODES(p, p1, l, _node)					\
for (verify_ring (l), p = DLIST_PARENT ((l)->_succ, __typeof__ (* p), _node);	\
     p1 = DLIST_PARENT (p->_node._succ, __typeof__ (* p), _node), 		\
     &p->_node != (l); p = p1)

#define FOR_ALL_NODES_REVERSE(p, p1, l, _node)				\
for (verify_ring (l), p = DLIST_PARENT ((l)->_pred, __typeof__ (* p), _node);	\
     p1 = DLIST_PARENT (p->_node._pred, __typeof__ (* p), _node),		\
     &p->_node != (l); p = p1)
#endif // _MSC_VER
