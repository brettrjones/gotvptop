				 ...
				 unsigned int		n_sliced_lines)
{
	...
	if (NULL == buf) 
	{
		LogMsg( LOG_ERROR, "libzvbi: Out of memory.");
		errno = ENOMEM;
		return FALSE;
	}
...
_vbi_capture_sim_get_flags	(vbi_capture *		cap)
{
	...
	sim = DLIST_PARENT (cap, vbi_capture_sim, cap);
...
{
	vbi_capture_sim *sim = DLIST_PARENT (cap, vbi_capture_sim, cap);
	unsigned int n_lines;
