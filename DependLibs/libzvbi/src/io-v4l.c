...
#define LogMsg( LOG_DEBUG, format, args...)						\
do {									\
	if (v->do_trace) {						\
...
v4l_suspend(vbi_capture_v4l *v)
{
	...
	if (v->read_active) {
		LogMsg( LOG_DEBUG, "Suspending read: re-open device...\n");
		...
		if (-1 == fd) {
			LogMsg( LOG_DEBUG, "v4l2-suspend: failed to re-open "
				"VBI device: %d: %s\n",
...
v4l_read_alloc(vbi_capture_v4l *v, char ** errstr)
{
	...
	LogMsg( LOG_DEBUG, "Capture buffer allocated: %d bytes\n", v->raw_buffer[0].size);
...
{
	vbi_capture_v4l *v = DLIST_PARENT(vc, vbi_capture_v4l, capture);
	vbi_capture_buffer *my_raw = v->raw_buffer;
	struct timeval tv;
	int r;

	if (my_raw == NULL) {
		LogMsg( LOG_DEBUG, "read buffer not allocated (must add services first)\n");
		errno = EINVAL;
		return -1;
	}
	...
	while (1)
	{
		...
		if (v->flush_frame_count > 0) {
			...
			LogMsg( LOG_DEBUG, "Skipping frame (%d remaining)\n", v->flush_frame_count);
		}
		else
			break;
	}
...
perm_check(vbi_capture_v4l *v, const char *name)
{
	...
	if (stat(name, &st) == -1) {
		LogMsg( LOG_DEBUG, "stat %s failed: %d, %s\n", name, errno, strerror(errno));
		errno = old_errno;
		return;
	}

	LogMsg( LOG_DEBUG, "%s permissions: user=%d.%d mode=0%o, I am %d.%d\n",
...
reverse_lookup(vbi_capture_v4l *v, int fd, struct stat *vbi_stat)
{
	...
	if (-1 == xioctl_fd (v, fd, VIDIOCGCAP, &vcap)) {
		LogMsg( LOG_DEBUG, "Driver doesn't support VIDIOCGCAP, "
			"probably not V4L API\n");
		return FALSE;
	}

	if (!(vcap.type & VID_TYPE_CAPTURE)) {
		LogMsg( LOG_DEBUG, "Driver is no video capture device\n");
		return FALSE;
	}
	...
	if (-1 == xioctl_fd (v, fd, VIDIOCGUNIT, &vunit)) {
		LogMsg( LOG_DEBUG, "Driver doesn't support VIDIOCGUNIT\n");
		return FALSE;
	}

	if (vunit.vbi != (int) minor(vbi_stat->st_rdev)) {
		LogMsg( LOG_DEBUG, "Driver reports vbi minor %d, need %d\n",
			vunit.vbi, minor(vbi_stat->st_rdev));
		return FALSE;
	}

	LogMsg( LOG_DEBUG, "Matched\n");
	return TRUE;
}
...
set_scanning_from_mode(vbi_capture_v4l *v, int mode, int * strict)
{
	...
	case VIDEO_MODE_NTSC:
		LogMsg( LOG_DEBUG, "Videostandard is NTSC\n");
		v->dec.scanning = 525;
		break;

	case VIDEO_MODE_PAL:
	case VIDEO_MODE_SECAM:
		LogMsg( LOG_DEBUG, "Videostandard is PAL/SECAM\n");
		v->dec.scanning = 625;
		break;
	...
	default:
		 ...
		 */
		LogMsg( LOG_DEBUG, "Videostandard unknown (%d)\n", mode);
		v->dec.scanning = 0;
		*strict = TRUE;
		break;
	}
...
get_videostd(vbi_capture_v4l *v, int fd, int *mode)
{
	...
	if (0 == xioctl_fd (v, fd, VIDIOCGTUNER, &vtuner)) {
		LogMsg( LOG_DEBUG, "Driver supports VIDIOCGTUNER: "
			"mode %d (0=PAL, 1=NTSC, 2=SECAM)\n", vtuner.mode);
		*mode = vtuner.mode;
		return TRUE;
	} else if (0 == xioctl_fd (v, fd, VIDIOCGCHAN, &vchan)) {
		LogMsg( LOG_DEBUG, "Driver supports VIDIOCGCHAN: norm %d\n", vchan.norm);
		*mode = vchan.norm;
		return TRUE;
	} else
		LogMsg( LOG_DEBUG, "Driver doesn't support VIDIOCGTUNER or VIDIOCGCHAN\n");
...
probe_video_device(vbi_capture_v4l *v, const char *name, struct stat *vbi_stat )
{
	...
	if (stat(name, &vid_stat) == -1) {
		LogMsg( LOG_DEBUG, "stat failed: %d, %s\n",	errno, strerror(errno));
		return -1;
	}

	if (!S_ISCHR(vid_stat.st_mode)) {
		LogMsg( LOG_DEBUG, "%s is no character special file\n", name);
		return -1;
	}

	if (major(vid_stat.st_rdev) != major(vbi_stat->st_rdev)) {
		LogMsg( LOG_DEBUG, "Mismatch of major device number: "
			"%s: %d, %d; vbi: %d, %d\n", name,
	...
	if (minor(vid_stat.st_rdev) >= 64) {
		LogMsg( LOG_DEBUG, "Not a v4l video minor device number (i.e. >= 64): "
			"%s: %d, %d\n", name,
			major(vid_stat.st_rdev), minor(vid_stat.st_rdev));
		return -1;
	}
	...
	if (-1 == video_fd) {
		LogMsg( LOG_DEBUG, "Cannot open %s: %d, %s\n",
			name, errno, strerror(errno));
		perm_check(v, name);
		return -1;
	}
...
open_video_dev(vbi_capture_v4l *v, struct stat *p_vbi_stat, vbi_bool do_dev_scan)
{
	...
	for (i = 0; i < sizeof(video_devices) / sizeof(video_devices[0]); i++) {
		LogMsg( LOG_DEBUG, "Try %s: ", video_devices[i]);
	...
	if (do_dev_scan) {
		...
		LogMsg( LOG_DEBUG, "Traversing /dev\n");

		if (!xopendir ("/dev", &dir, &dirent)) {
			LogMsg( LOG_DEBUG, "Cannot open /dev: %d, %s\n",
				errno, strerror (errno));
			perm_check (v, "/dev");
			goto done;
		}
		...
		LogMsg( LOG_DEBUG, "Traversing finished\n");
	       ...
	       int given_fd, int scanning)
{
	...
	LogMsg( LOG_DEBUG, "Attempt to guess the videostandard\n");
	 ...
	 */
	LogMsg( LOG_DEBUG, "Attempt to find a reverse VIDIOCGUNIT\n");

	if (fstat(v->fd, &vbi_stat) == -1) {
		LogMsg( LOG_DEBUG, "fstat failed: %d, %s\n", errno, strerror(errno));
		goto finish;
	}

	if (!S_ISCHR(vbi_stat.st_mode)) {
		LogMsg( LOG_DEBUG, "VBI device is no character special file, reject\n");
		return FALSE;
	}

	if (major(vbi_stat.st_rdev) != 81) {
		LogMsg( LOG_DEBUG, "VBI device CSF has major number %d, expect 81\n"
			"Warning: will assume this is still a v4l device\n",
			major(vbi_stat.st_rdev));
		goto finish;
	}

	LogMsg( LOG_DEBUG, "VBI device type verified\n");

	if (given_fd > -1) {
		LogMsg( LOG_DEBUG, "Try suggested corresponding video fd\n");
...
v4l_update_scanning(vbi_capture_v4l *v, int * p_strict)
{
	...
	if ( get_videostd(v, v->fd, &mode) ) {
                ...
                if (-1 != video_fd) {
                        ...
                        LogMsg( LOG_DEBUG, "Failed to open video device '%d': %s", errno, strerror(errno));
                }
...
v4l_get_scanning(vbi_capture *vc)
{
	...
	LogMsg( LOG_DEBUG, "Guessed video standard %d\n", new_scanning);
	       ...
	       char **errstr)
{
	...
	if (v->has_s_fmt == -1) {
		...
		v->has_s_fmt = (0 == xioctl (v, VIDIOCSVBIFMT, &vfmt_temp)
				|| errno == EBUSY);
		LogMsg( LOG_DEBUG, "Driver does%s support VIDIOCSVBIFMT\n",
			v->has_s_fmt ? "" : " not");
	}
	...
	LogMsg( LOG_DEBUG, "Attempt to set vbi capture parameters\n");
	...
	*services &= sup_services;
	...
	case EBUSY:
		...
		LogMsg( LOG_DEBUG, "VIDIOCSVBIFMT returned EBUSY, "
	...
	case EINVAL:
                if (strict < 2) {
		        LogMsg( LOG_DEBUG, "VIDIOCSVBIFMT returned EINVAL, "
		               "will try the current parameters\n");
                        *p_vfmt = vfmt_temp;
                        return TRUE;
                }
		    ...
		    char ** errstr)
{
	...
	if (0 == xioctl (v, VIDIOCGVBIFMT, &vfmt)) {
		...
		LogMsg( LOG_DEBUG, "Driver supports VIDIOCGVBIFMT, "
		...
		LogMsg( LOG_DEBUG, "Accept current vbi parameters\n");
		...
		** note: must do this even if service add fails later, to stay in sync with driver */
		 ...
		 */
		LogMsg( LOG_DEBUG, "Driver doesn't support VIDIOCGVBIFMT (errno %d), "
		...
		LogMsg( LOG_DEBUG, "Attempt to determine vbi frame size\n");

		size = xioctl (v, BTTV_VBISIZE, 0);
		if (-1 == size) {
			LogMsg( LOG_DEBUG, "Driver does not support BTTV_VBISIZE, "
			...
			LogMsg( LOG_DEBUG, "Driver supports BTTV_VBISIZE: %d bytes, "
		...
		default:
			...
			goto io_error;
#endif
			LogMsg( LOG_DEBUG, "Warning: Videostandard not confirmed, "
	...
	if (services & ~(VBI_SLICED_VBI_525 | VBI_SLICED_VBI_625)) {
		...
		LogMsg( LOG_DEBUG, "Nyquist check passed\n");

		LogMsg( LOG_DEBUG, "Request decoding of services 0x%08x, strict level %d\n", services, strict);
...
failure:
        v->services |= services;
	LogMsg( LOG_DEBUG, "Will capture services 0x%08x, added 0x%0x commit:%d\n", v->services, services, commit);
	...
	char **errstr, vbi_bool trace)
{
	...
	*errstr = NULL;
	...
	LogMsg( LOG_DEBUG, "Try to open v4l vbi device, "
	...
	if (-1 == xioctl (v, VIDIOCGCAP, &v->vcap)) {
		...
		if (v->vcap.name[0] != 0) {
			LogMsg( LOG_DEBUG, "Driver name '%s'\n", v->vcap.name);
		} else {
			strlcpy(v->vcap.name, _("driver unknown"), sizeof(v->vcap.name) - 1);
			v->vcap.name[sizeof(v->vcap.name) - 1] = 0;
		}
	...
	LogMsg( LOG_DEBUG, "%s (%s) is a v4l vbi device\n", v->p_dev_name, v->vcap.name);
	...
	LogMsg( LOG_DEBUG, "Hinted video standard %d, guessed %d\n",
        ...
        if (services != NULL) {
                ...
                *services = v->services;
                ...
                if (!v->dec.scanning && strict >= 1) {
                        LogMsg( LOG_DEBUG, "Try to guess video standard from vbi bottom field "
                ...
                LogMsg( LOG_DEBUG, "Guessed videostandard %d\n", v->dec.scanning);
        }

	if (!v->has_select)
		LogMsg( LOG_DEBUG, "Warning: no read select, reading will block\n");

	LogMsg( LOG_DEBUG, "Successful opened %s (%s)\n",
	       v->p_dev_name, v->vcap.name);
