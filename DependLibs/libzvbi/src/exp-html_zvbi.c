...
{
	html_instance *html = DLIST_PARENT(e, html_instance, export);
...
static void exp_printf( vbi_export * e, const char * templ, ...)
{
	va_list argList;
	va_start( argList, templ );
	vbi_export_printf( e, templ, argList );
	va_end( argList );
}
...
{
	exp_printf( &html->export, "#%02x%02x%02x",
		VBI_R(color),
		VBI_G(color),
		VBI_B(color));
}
...
title(html_instance *html, vbi_page *pg)
{
	...
	if (pg->pgno < 0x100) {
		...
		exp_printf( &html->export, _("Teletext Page %3x.%x"),
			pg->pgno, pg->subno);
	} else {
		exp_printf( &html->export, _("Teletext Page %3x"),
			pg->pgno);
	}
...
header(html_instance *html, vbi_page *pg)
{
	...
	if (!html->headerless) {
		...
		exp_printf( &html->export, "<!DOCTYPE HTML PUBLIC "
		...
		if (html->color) {
			...
			for (s = html->styles, ord = 1; s; s = s->next)
				if (s != &html->def && s->ref_count > 1) {
					exp_printf( &html->export, "span.c%d { color:", ord);
		...
		if (lang && *lang)
			exp_printf( &html->export, "lang=\"%s\" ", lang);

		if (dir && *dir)
			exp_printf( &html->export, "dir=\"%s\" ", dir);
		...
		for (j = 0; j < pg.columns; j++) {
			if ((html->color
				...
				if (acp[j].link && !html->link) {
					...
					case VBI_LINK_FTP:
					case VBI_LINK_EMAIL:
						exp_printf( &html->export, "<a href=\"%s\">", link.url);
						html->link = TRUE;
				...
				if (html->color) {
					...
					if (s != &html->def) {
						if (s && !html->headerless) {
							...
							html->flash = s->flash;
							exp_printf( &html->export, "<span class=\"c%d\">", ord);
						} else {
			...
			if (vbi_is_print(acp[j].unicode)) {
				...
				if ((size_t) -1 == r
					...
					exp_printf( &html->export, "&#%u;", acp[j].unicode);
				} else {
