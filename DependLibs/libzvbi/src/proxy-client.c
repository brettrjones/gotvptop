...
#define LogMsg( LOG_DEBUG, fmt, arg...)    do {if (vpc->trace >= 1) fprintf(stderr, "proxy-client: " fmt, ## arg);} while(0)
#define LogMsg( LOG_INFO, fmt, arg...)    do {if (vpc->trace >= 2) fprintf(stderr, "proxy-client: " fmt, ## arg);} while(0)
...
static vbi_bool proxy_client_connect_server( vbi_proxy_client * vpc )
{
   ...
   {
      LogMsg( LOG_DEBUG, "connect_server: hostname or port not configured\n");
...
static vbi_bool proxy_client_alloc_msg_buf( vbi_proxy_client * vpc )
{
       ...
       || (vpc->p_client_msg == NULL))
   {
      ...
      LogMsg( LOG_INFO, "alloc_msg_buf: allocate buffer for "
                                        ...
                                        VBIPROXY_MSG * pMsg )
{
   ...
   switch (pHead->type)
   {
      ...
      case MSG_TYPE_CHN_TOKEN_IND:
         result = (len == sizeof(VBIPROXY_MSG_HEADER) + sizeof(pBody->chn_token_ind)); // BRJ may need to modify to leave out sizeof(pBody->chn_token_ind)
         break;
      ...
      case MSG_TYPE_DAEMON_PID_CNF:
         LogMsg( LOG_DEBUG, "check_msg: recv server msg type %d (%s)\n", pHead->type, vbi_proxy_msg_debug_get_type_str(pHead->type));
         result = FALSE;
         break;
      default:
         LogMsg( LOG_DEBUG, "check_msg: unknown msg type %d\n", pHead->type);
         result = FALSE;
         break;
   }
...
static vbi_bool proxy_client_take_message( vbi_proxy_client * vpc )
{
   ...
   switch (vpc->p_client_msg->head.type)
   {
      ...
      case MSG_TYPE_CHN_CHANGE_IND:
         LogMsg( LOG_DEBUG, "channel change indication: new scanning %d\n", pMsg->chn_change_ind.scanning);
   ...
   {
      LogMsg( LOG_DEBUG, "take_message: message type %d (len %d) not expected in state %d\n", vpc->p_client_msg->head.type, vpc->p_client_msg->head.len, vpc->state);
...
static void proxy_client_close( vbi_proxy_client * vpc )
{
   ...
   if (vpc != NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "proxy_client-close: illegal NULL ptr param");
}
...
static int proxy_client_wait_select( vbi_proxy_client * vpc, struct timeval * timeout )
{
   ...
   if (vpc->io.sock_fd != -1)
   {
      ...
      if (ret > 0) {
	 LogMsg( LOG_INFO, "wait_select: waited for %c -> sock r/w %d/%d\n",
		  (vpc->io.writeLen > 0) ? 'w':'r',
		  (int) FD_ISSET(vpc->io.sock_fd, &fd_rd),
		  (int) FD_ISSET(vpc->io.sock_fd, &fd_wr));
      } else if (ret == 0) {
	 LogMsg( LOG_DEBUG, "wait_select: timeout\n");
      } else {
	 LogMsg( LOG_DEBUG, "wait_select: error %d (%s)\n", errno, strerror(errno));
      }
   }
   else
   {
      LogMsg( LOG_DEBUG, "wait_select: socket not open\n");
      ret = -1;
   }
...
static vbi_bool proxy_client_start_acq( vbi_proxy_client * vpc )
{
   ...
   if (vpc->p_client_msg->head.type == MSG_TYPE_CONNECT_CNF)
   {
      ...
      {  /* version ok -> request block forwarding */
         LogMsg( LOG_DEBUG, "Successfully connected to proxy (version %x.%x.%x, protocol %x.%x.%x)\n",
...
static void proxy_client_stop_acq( vbi_proxy_client * vpc )
{
   if (vpc->state != CLNT_STATE_NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "stop_acq: acq not enabled\n");
}
                                  ...
                                  vbi_channel_profile * p_chn_profile )
{
   ...
   if (vpc != NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "Request for channel token: prio=%d\n", chn_prio);
                                 ...
                                 unsigned int scanning )
{
   ...
   if (vpc != NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "Send channel notification: flags 0x%X, scanning %d (prio=%d, has_token=%d)\n", notify_flags, scanning, vpc->chn_prio, vpc->has_token);
      ...
      if (vpc->state == CLNT_STATE_CAPTURING)
      {
         ...
         if (size >= 0)
         {
                 ...
                 (vpc->chn_prio > VBI_CHN_PRIO_BACKGROUND) || vpc->has_token )
            {
               ...
               LogMsg( LOG_DEBUG, "Forwarding ioctl: 0x%X, argp=0x%lX\n", request, (long)p_arg);
            ...
            {
               LogMsg( LOG_DEBUG, "vbi_proxy-client_ioctl: request not allowed without obtaining token first\n");
               errno = EBUSY;
            }
         }
         else
         {
            LogMsg( LOG_DEBUG, "vbi_proxy-client_ioctl: unknown or not allowed request: 0x%X\n", request);
            errno = EINVAL;
         }
      }
      else
         LogMsg( LOG_DEBUG, "vbi_proxy-client_ioctl: client in invalid state %d\n", vpc->state);

      vbi_proxy_process_callbacks(vpc);
   }
   else
      LogMsg( LOG_DEBUG, "vbi_proxy-client_ioctl: invalid NULL ptr param\n");
...
vbi_proxy_client_has_channel_control( vbi_proxy_client * vpc )
{
   ...
   {
      LogMsg( LOG_DEBUG, "vbi_proxy_client-has_channel_token: NULL client param");
      return FALSE;
   }
}
                               ...
                               VBI_PROXY_CLIENT_CALLBACK * p_callback, void * p_data )
{
   ...
   if (vpc != NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "vbi_proxy_client-set_callback: invalid pointer arg\n");
...
{
   vbi_proxy_client * vpc = DLIST_PARENT(vc, vbi_proxy_client, capt_api);
                                  ...
                                  char ** pp_errorstr )
{
   ...
   if (vc != NULL)
   {
      ...
      LogMsg( LOG_DEBUG, "update_services: send service req: srv %d, strict %d\n", services, strict);
      ...
      if (vpc->p_client_msg->head.type == MSG_TYPE_SERVICE_CNF)
      {
         ...
         LogMsg( LOG_DEBUG, "service cnf: granted service %d\n", vpc->dec.services);
      }
      else
      {
         /* process the message */
                       ...
                       char **pp_errorstr )
{
   if (vpc != NULL)
   {
           ...
           (vpc->state == CLNT_STATE_ERROR) )
      {
         ...
         LogMsg( LOG_DEBUG, "vbi_proxy-client_start: illegal state %d for start\n", vpc->state);
   }
   else
      LogMsg( LOG_DEBUG, "vbi_proxy-client_start: illegal NULL ptr param\n");
