...
void vbi_proxy_msg_logger( int level, int clnt_fd, int errCode, const char * pText, ... )
{
	...
	char timestamp[ 32 ];
	char fdstr[ 20 ];
	char logMsgPrefix[ 32 ];
	char totalLogMsg[ 4096 ];

	const char *argv[ 10 ];
	uint32_t argc, idx;
	time_t now = time( NULL );

	uint32_t logMsgLevel = LOG_DEBUG;
	switch ( level )
	{
	case VBI_LOG_ERROR:
		logMsgLevel = LOG_ERROR;
		break;
	case VBI_LOG_WARNING:
		logMsgLevel = LOG_WARN;
		break;
	case VBI_LOG_NOTICE:
	case VBI_LOG_INFO:
	...
	if ( pText != NULL )
	{
		...
		if ( proxy_msg_logcf.do_logtty && ( level <= VBI_LOG_WARNING ) )
		{
			strcpy( logMsgPrefix, "vbiproxy: " );
		}
		...
		//argv[argc++] = pText;
		snprintf( totalLogMsg, sizeof( totalLogMsg ),  "%s %s %s %s ", timestamp, logMsgPrefix, fdstr, pText );
		...
		for ( idx = 0; idx < argc; idx++ )
		{
			/*       if (fd >= 0)
				   ...
				   if (proxy_msg_logcf.do_logtty && (level <= LOG_WARNING))
					  LogMsg( LOG_ERROR, "%s", argv[idx]);*/
			strcat( totalLogMsg, argv[ idx ] );
		}

		LogMsg( logMsgLevel, totalLogMsg );
	...
	*pBlocked = FALSE;
	...
	else if ( len < 0 )
	{
		...
		{  /* network error -> close the connection */
			LogMsg( LOG_DEBUG, "handle_io: write error on fd %d: %s\n", pIO->sock_fd, strerror( errno ) );
			result = FALSE;
		}
	...
	if ( pReadBuf != NULL )
	{
		...
		else
		{
			...
			{  /* zero bytes read after select returned readability -> network error or connection closed by peer */
				LogMsg( LOG_DEBUG, "handle_io: zero len read on fd %d\n", (int)pIO->sock_fd );
				errno = ECONNRESET;
				result = FALSE;
			}
			else if ( ( len < 0 ) && ( errno != EAGAIN ) && ( errno != EINTR ) )
			{  /* network error -> close the connection */
				LogMsg( LOG_DEBUG, "handle_io: read error on fd %d: len=%ld, %s\n",
						  ...
						  uint32_t msgLen, VBIPROXY_MSG * pMsg, vbi_bool freeBuf )
{
	...
	LogMsg( LOG_INFO, "write: len %ld, msg type %d (%s)\n",
...
int vbi_proxy_msg_listen_socket( vbi_bool is_tcp_ip, const char * listen_ip, const char * listen_port )
{
	...
	{  /* try IP-v6: not supported everywhere yet, so errors must be silently ignored */
		...
		if ( rc == 0 )
		{
			...
			if ( sock_fd == -1 )
			{
				LogMsg( LOG_INFO, "listen_socket: socket (ipv6)\n" );
				freeaddrinfo( res );
				res = NULL;
			}
		}
		else
			LogMsg( LOG_INFO, "listen_socket: getaddrinfo (ipv6): %s\n", gai_strerror( rc ) );
	}
#endif
	...
	if ( sock_fd == -1 )
	{
		...
		if ( rc == 0 )
		{
			...
			if ( sock_fd == -1 )
			{
				LogMsg( LOG_ERROR, "socket create failed: %d", errno );
			}
		}
		else
			LogMsg( LOG_ERROR, "Invalid hostname or service/port: %s ", gai_strerror( rc ) );
	}
	...
	if ( sock_fd != -1 )
	{
		...
		if ( setsockopt( sock_fd, SOL_SOCKET, SO_REUSEADDR, (void *)&opt, sizeof( opt ) ) == 0 )
		{
			...
			if ( 0 == VxSetSktBlocking( sock_fd, false ) )
			{
				/* bind the socket */
				if ( bind( sock_fd, res->ai_addr, res->ai_addrlen ) == 0 )
				{
					...
					{
						LogMsg( LOG_ERROR, "socket listen failed: %d", errno );
						if ( ( is_tcp_ip == FALSE ) && ( listen_port != NULL ) )
							unlink( listen_port );
					}
				}
				else
					LogMsg( LOG_ERROR, "socket bind failed: %d ", errno );
			}
			else
				LogMsg( LOG_ERROR, "failed to set socket non-blocking: %d ", errno );
		}
		else
			LogMsg( LOG_ERROR, "socket setsockopt(SOL_SOCKET=SO_REUSEADDR) failed: %d ", errno );
	}
			...
			if( 0 == VxSetSktBlocking( sock_fd, false ) )
			{
				...
				else if ( peerAddr.sa.sa_family == AF_INET6 )
				{
					...
					{  /* resolver failed - but numeric conversion was successful */
						LogMsg( LOG_INFO, "accept_connection: IPv6 resolver failed for %s\n", hname_buf );
						vbi_proxy_msg_logger( LOG_INFO, sock_fd, 0, "new connection from ", hname_buf, NULL );
						result = TRUE;
					}
					else
				...
				{  /* neither INET nor named socket -> internal error */
					...
					LogMsg( LOG_WARN, "new connection via unexpected protocol family %s", hname_buf );
				}
			}
			else
			{  /* fcntl failed: OS error (should never happen) */
				LogMsg( LOG_WARN, "new connection: failed to set socket to non-blocking: %d\n", errno );
			}
		}
		else
		{  /* socket address buffer too small: internal error */
			snprintf( hname_buf, sizeof( hname_buf ), "need %d, have %d", length, maxLength );
			LogMsg( LOG_WARN, "new connection: saddr buffer too small:%s ", hname_buf );
		}
...
static char * vbi_proxy_msg_resolve_symlinks( const char * p_dev_name )
{
...
#ifdef _MSC_VER
	strncpy( link_name, p_dev_name, sizeof( link_name ) );
	return link_name;
#else
	struct stat stbuf;
	char * p_path;
	...
	for ( slink_idx = 0; slink_idx < 100; slink_idx++ )
	{
		...
		if ( ( res == 0 ) && S_ISLNK( stbuf.st_mode ) )
		{
			...
			if ( ( name_len > 0 ) && ( name_len < ( int ) sizeof( link_name ) ) )
			{
				...
				LogMsg( LOG_INFO, "resolve_symlinks: following symlink %s to: %s\n", p_path, link_name );
				if ( link_name[ 0 ] != '/' )
			...
			{  /* symlink string too long for the buffer */
				if ( name_len > 0 )
				{
					...
					LogMsg( LOG_DEBUG, "resolve_symlinks: abort: symlink too long: %s\n", link_name );
				}
				else
					LogMsg( LOG_DEBUG, "resolve_symlinks: zero length symlink - abort\n" );
				break;
			}
		}
		else
	...
	if ( slink_idx >= 100 )
		LogMsg( LOG_DEBUG, "resolve_symlinks: symlink level too deep: abort after %d\n", slink_idx );
...
int vbi_proxy_msg_connect_to_server( vbi_bool use_tcp_ip, const char * pSrvHost, const char * pSrvPort, char ** ppErrorText )
{
	...
	{  /* try IP-v6: not supported everywhere yet, so errors must be silently ignored */
		...
		if ( rc == 0 )
		{
			...
			if ( sock_fd == -1 )
			{
				freeaddrinfo( res );
				res = NULL;
				/*LogMsg( LOG_INFO, "socket (ipv6)\n"); */
			}
		}
		else
			LogMsg( LOG_INFO, "getaddrinfo (ipv6): %s\n", gai_strerror( rc ) );
	}
#endif
	...
	if ( sock_fd == -1 )
	{
		...
		if ( rc == 0 )
		{
			...
			if ( sock_fd == -1 )
			{
				LogMsg( LOG_DEBUG, "socket (ipv4): error %d, %s\n", errno, strerror( errno ) );
		...
		{
			LogMsg( LOG_DEBUG, "getaddrinfo (ipv4): %s\n", gai_strerror( rc ) );
	...
	if ( sock_fd != -1 )
	{
		if( 0 == VxSetSktBlocking( sock_fd, false ) )
		{
			...
			{
				LogMsg( LOG_DEBUG, "connect: error %d, %s\n", errno, strerror( errno ) );
				if ( use_tcp_ip )
		...
		{
			LogMsg( LOG_DEBUG, "fcntl (F_SETFL=O_NONBLOCK): error %d, %s\n",
					errno, strerror( errno ) );
...
vbi_bool vbi_proxy_msg_finish_connect( int sock_fd, char ** ppErrorText )
{
	...
	if ( getsockopt( sock_fd, SOL_SOCKET, SO_ERROR, (void *)&sockerr, &sockerrlen ) == 0 )
	{
		...
		{  /* success -> send the first message of the startup protocol to the server */
			LogMsg( LOG_INFO, "finish_connect: socket connect succeeded\n" );
			result = TRUE;
		}
		else
		{  /* failed to establish a connection to the server */
			LogMsg( LOG_DEBUG, "finish_connect: socket connect failed: %s\n", strerror( sockerr ) );
	...
	{
		LogMsg( LOG_DEBUG, "finish_connect: getsockopt: %s\n", strerror( errno ) );
		asprintf( ppErrorText, _( "Socket I/O error: %s." ),
				  strerror( errno ) );
	}
...
#ifdef ENABLE_V4L
	case VIDIOCGCAP:
		LogMsg( LOG_INFO, "v4l_ioctl CGCAP, arg size %ld\n", ( long ) sizeof( struct video_capability ) );
		return sizeof( struct video_capability );
	case VIDIOCGCHAN:
		LogMsg( LOG_INFO, "v4l_ioctl CGCHAN, arg size %ld\n", ( long ) sizeof( struct video_channel ) );
		return sizeof( struct video_channel );
	case VIDIOCSCHAN:
		LogMsg( LOG_INFO, "v4l_ioctl CSCHAN, arg size %ld\n", ( long ) sizeof( struct video_channel ) );
		*req_perm = TRUE;
		return sizeof( struct video_channel );
	case VIDIOCGTUNER:
		LogMsg( LOG_INFO, "v4l_ioctl CGTUNER, arg size %ld\n", ( long ) sizeof( struct video_tuner ) );
		return sizeof( struct video_tuner );
	case VIDIOCSTUNER:
		LogMsg( LOG_INFO, "v4l_ioctl CSTUNER, arg size %ld\n", ( long ) sizeof( struct video_tuner ) );
		*req_perm = TRUE;
		return sizeof( struct video_tuner );
	case VIDIOCGFREQ:
		LogMsg( LOG_INFO, "v4l_ioctl CGFREQ, arg size %ld\n", ( long ) sizeof( unsigned long ) );
		return sizeof( unsigned long );
	case VIDIOCSFREQ:
		LogMsg( LOG_INFO, "v4l_ioctl CSFREQ, arg size %ld\n", ( long ) sizeof( unsigned long ) );
		*req_perm = TRUE;
		return sizeof( unsigned long );
	case VIDIOCGUNIT:
		LogMsg( LOG_INFO, "v4l_ioctl CGUNIT, arg size %ld\n", ( long ) sizeof( struct video_unit ) );
...
vbi_proxy_msg_v4l2_ioctl( unsigned int request, void * p_arg, vbi_bool * req_perm )
{
	...
	case VIDIOC_QUERYCAP:
		LogMsg( LOG_INFO, "v4l2_ioctl QUERYCAP, arg size %ld\n", ( long ) sizeof( struct v4l2_capability ) );
		return sizeof( struct v4l2_capability );
	case VIDIOC_QUERYSTD:
		LogMsg( LOG_INFO, "v4l2_ioctl QUERYSTD, arg size %ld\n", ( long ) sizeof( v4l2_std_id ) );
		return sizeof( v4l2_std_id );
	case VIDIOC_G_STD:
		LogMsg( LOG_INFO, "v4l2_ioctl G_STD, arg size %ld\n", ( long ) sizeof( v4l2_std_id ) );
		return sizeof( v4l2_std_id );
	case VIDIOC_S_STD:
		LogMsg( LOG_INFO, "v4l2_ioctl S_STD, arg size %ld\n", ( long ) sizeof( v4l2_std_id ) );
		*req_perm = TRUE;
		return sizeof( v4l2_std_id );
	case VIDIOC_ENUMSTD:
		LogMsg( LOG_INFO, "v4l2_ioctl ENUMSTD, arg size %ld\n", ( long ) sizeof( struct v4l2_standard ) );
		return sizeof( struct v4l2_standard );
	case VIDIOC_ENUMINPUT:
		LogMsg( LOG_INFO, "v4l2_ioctl ENUMINPUT, arg size %ld\n", ( long ) sizeof( struct v4l2_input ) );
		return sizeof( struct v4l2_input );
	case VIDIOC_G_CTRL:
		LogMsg( LOG_INFO, "v4l2_ioctl G_CTRL, arg size %ld\n", ( long ) sizeof( struct v4l2_control ) );
		return sizeof( struct v4l2_control );
	case VIDIOC_S_CTRL:
		LogMsg( LOG_INFO, "v4l2_ioctl S_CTRL, arg size %ld\n", ( long ) sizeof( struct v4l2_control ) );
		return sizeof( struct v4l2_control );
	case VIDIOC_G_TUNER:
		LogMsg( LOG_INFO, "v4l2_ioctl G_TUNER, arg size %ld\n", ( long ) sizeof( struct v4l2_tuner ) );
		return sizeof( struct v4l2_tuner );
	case VIDIOC_S_TUNER:
		LogMsg( LOG_INFO, "v4l2_ioctl S_TUNER, arg size %ld\n", ( long ) sizeof( struct v4l2_tuner ) );
		*req_perm = TRUE;
		return sizeof( struct v4l2_tuner );
	case VIDIOC_QUERYCTRL:
		LogMsg( LOG_INFO, "v4l2_ioctl QUERYCTRL, arg size %ld\n", ( long ) sizeof( struct v4l2_queryctrl ) );
		return sizeof( struct v4l2_queryctrl );
	case VIDIOC_QUERYMENU:
		LogMsg( LOG_INFO, "v4l2_ioctl QUERYMENU, arg size %ld\n", ( long ) sizeof( struct v4l2_querymenu ) );
		return sizeof( struct v4l2_querymenu );
	case VIDIOC_G_INPUT:
		LogMsg( LOG_INFO, "v4l2_ioctl G_INPUT, arg size %ld\n", ( long ) sizeof( int ) );
		return sizeof( int );
	case VIDIOC_S_INPUT:
		LogMsg( LOG_INFO, "v4l2_ioctl S_INPUT, arg size %ld\n", ( long ) sizeof( int ) );
		*req_perm = TRUE;
		return sizeof( int );
	case VIDIOC_G_MODULATOR:
		LogMsg( LOG_INFO, "v4l2_ioctl G_MODULATOR, arg size %ld\n", ( long ) sizeof( struct v4l2_modulator ) );
		return sizeof( struct v4l2_modulator );
	case VIDIOC_S_MODULATOR:
		LogMsg( LOG_INFO, "v4l2_ioctl S_MODULATOR, arg size %ld\n", ( long ) sizeof( struct v4l2_modulator ) );
		*req_perm = TRUE;
		return sizeof( struct v4l2_modulator );
	case VIDIOC_G_FREQUENCY:
		LogMsg( LOG_INFO, "v4l2_ioctl G_FREQUENCY, arg size %ld\n", ( long ) sizeof( struct v4l2_frequency ) );
		return sizeof( struct v4l2_frequency );
	case VIDIOC_S_FREQUENCY:
		LogMsg( LOG_INFO, "v4l2_ioctl S_FREQUENCY, arg size %ld\n", ( long ) sizeof( struct v4l2_frequency ) );
		*req_perm = TRUE;
		return sizeof( struct v4l2_frequency );
#endif
	default:
							   ...
							   int request, void * p_arg, vbi_bool * req_perm )
{
	*req_perm = FALSE;
	...
	default:
		LogMsg( LOG_DEBUG, "v4l2_ioctl: API #%d not supported\n", vbi_api );
		return -1;
	}
}
