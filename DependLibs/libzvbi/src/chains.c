...
#if !defined (__NetBSD__) && !defined (__FreeBSD__) && !defined (__FreeBSD_kernel__) && !defined (TARGET_OS_WINDOWS)
...
//#define LogMsg( LOG_DEBUG, fmt, ...)    do {if (vbi_chains_debug >= 1) fprintf(stderr, "proxy-chains: " fmt, ## arg);} while(0)
//#define LogMsg( LOG_INFO, fmt, ...)    do {if (vbi_chains_debug >= 2) fprintf(stderr, "proxy-chains: " fmt, ## arg);} while(0)
...
static void vbi_chains_doinit(void)
{
   ...
   else
   {
      if (strlen(vbi_chains_device) > 0)
         LogMsg( LOG_DEBUG, "Will redirect access to device %s\n", vbi_chains_device);
      else
         LogMsg( LOG_DEBUG, "No device specified: will redirect access to any VBI device\n");
   }
...
int open(const char *pathname, int flags, ...)
{
   ...
   else
   {
      ...
      LogMsg( LOG_DEBUG, "hijacking open on %s...\n", pathname);
      ...
      if (p_proxy_client == NULL)
      {
         ...
         if (p_errmsg != NULL)
         {
            LogMsg( LOG_DEBUG, "Failed to connect to proxy: %s\n", p_errmsg);
            free(p_errmsg);
         }

         if ((vbi_fd != -1) || (errno != 2))
         {
            LogMsg( LOG_INFO, "open returns %d errno=%d (%s)\n", vbi_fd, errno, strerror(errno));
            return vbi_fd;
         }
         else
         {
            LogMsg( LOG_DEBUG, "proxy not running - trying the actual device...\n");
...
int ioctl(int fd, ioctl_request_t request, ...)
{
   ...
   {
      /*int *arg = (int *) argp; */
      LogMsg( LOG_DEBUG, "hijacking ioctl (%d : %x - %p)\n",
      ...
      switch (request)
      {
         ...
         case VIDIOC_TRY_FMT:
         {
            ...
            if (p_errmsg != NULL)
            {
               LogMsg( LOG_DEBUG, "Failed to update services: %s\n", p_errmsg);
               free(p_errmsg);
            }
            return res;
#else
         ...
         case BTTV_VERSION:
            LogMsg( LOG_DEBUG, "ioctl BTTV_VERSION\n");
            return ((7 << 16) | (100 << 8));
         ...
         case BTTV_VBISIZE:
         {
            ...
            LogMsg( LOG_DEBUG, "ioctl BTTV_VBISIZE: %d\n", size);
            return size;
         }
...
ssize_t write(int fd, const void *buf, size_t count)
{
   ...
   {
      LogMsg( LOG_DEBUG, "write() called for VBI - ignored\n");
...
ssize_t read(int fd, void *buf, size_t count)
{
   ...
   else
   {
      ...
      LogMsg( LOG_INFO, "read %d bytes buf=0x%lX\n", (int) count, (long) buf);
      vbi_chains_working = 1;
      ...
      vbi_chains_working = 0;
      LogMsg( LOG_INFO, "read returns %d (of %d)\n", (int) result, vbi_buf_size);
