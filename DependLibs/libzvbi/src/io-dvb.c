...
//
//#define LogMsg( LOG_DEBUG, format, ...)	\
//do {								\
//	if (dvb->do_trace) {			\
//		va_list argList;			\
//		va_start( argList, format ); \
//		fprintf (stderr, "libzvbi: " format , argList);		\
//		fflush (stderr);			\
//		va_end(argList);			\
//	}								\
//} while (0)
...
dvb_read			(vbi_capture *		cap,
				 ...
				 const struct timeval *	timeout)
{
	vbi_capture_dvb *dvb = DLIST_PARENT (cap, vbi_capture_dvb, capture);
				 ...
				 int			pid)
{
	...
	LogMsg( LOG_DEBUG, "Capturing PES packets with PID %d\n", pid);
...
dvb_delete			(vbi_capture *		cap)
{
	...
	dvb = DLIST_PARENT (cap, vbi_capture_dvb, capture);
				 ...
				 vbi_bool		trace)
{
	...
	*errstr = NULL;
	...
	LogMsg( LOG_DEBUG, "Opened device %s\n", device_name);
