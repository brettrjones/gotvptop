...
{
	vbi_capture_bktr *v = DLIST_PARENT(vc, vbi_capture_bktr, capture);
				 ...
				 vbi_bool		trace)
{
	...
	*errstr = NULL;
	...
	LogMsg( LOG_DEBUG, "Try to open bktr vbi device, "
	...
	LogMsg( LOG_DEBUG, "Will decode services 0x%08x\n", *services);
	...
	LogMsg( LOG_DEBUG, "Capture buffer allocated\n");
				 ...
				 vbi_bool		trace)
{
	...
	LogMsg( LOG_DEBUG, "Libzvbi bktr interface rev.\n  %s\n", rcsid);
