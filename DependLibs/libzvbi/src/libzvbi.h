...
#include <sys/types.h> /* size_t, ssize_t */
	...
	extern ssize_t			vbi_export_mem( vbi_export *		e,
											void *			buffer,
											size_t			buffer_size,
											const vbi_page *	pg )
		_vbi_nonnull( ( 1 ) ); /* sic */
...
#define VBI_IDL_DEPENDENT	(1 << 3)
	...
	typedef vbi_bool				vbi_pfc_demux_cb( struct vbi_pfc_demux *	dx,
													  void *			user_data,
	...
	extern void						vbi_pfc_demux_reset( vbi_pfc_demux *	dx );
		//_vbi_nonnull( ( 1 ) );

	extern vbi_bool					vbi_pfc_demux_feed( vbi_pfc_demux *	dx,
														const uint8_t		buffer[ 42 ] );
#ifndef DOXYGEN_SHOULD_SKIP_THIS
		//_vbi_nonnull( ( 1, 2 ) )
#endif
...
#include <sys/time.h> /* struct timeval */
	...
	/* forward declaration from io_zvbi_capture.h */
	...
	*/
	...
	extern void				vbi_set_log_fn( vbi_log_mask		mask,
											vbi_log_fn *		log_fn,
											void *			user_data );
