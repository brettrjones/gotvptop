...
//#define LogMsg( LOG_DEBUG, templ, args...)						\
//do {									\
//	if (TELETEXT_DEBUG)						\
		...
		if (PAGE_FUNCTION_AIT == vbi->cn->btt_link[i].function) {
			...
			vtp = _vbi_cache_get_page
				(vbi->ca, vbi->cn,
				 ...
				 /* subno_mask */ 0x3f7f);
			if (!vtp) {
				LogMsg( LOG_DEBUG, "top ait page %x not cached\n",
					vbi->cn->btt_link[i].pgno);
				continue;
			} else if (vtp->function != PAGE_FUNCTION_AIT) {
				LogMsg( LOG_DEBUG, "no ait page %x\n", vtp->pgno);
		   ...
		   cache_page *vtp)
{
	...
	LogMsg( LOG_DEBUG, "PAGE MIP/BTT: %d\n", ps->page_type);
			...
			if (PAGE_FUNCTION_AIT == vbi->cn->btt_link[i].function) {
				...
				vtp = _vbi_cache_get_page
					(vbi->ca, vbi->cn,
					 ...
					 /* subno_mask */ 0x3f7f);
				if (!vtp) {
					LogMsg( LOG_DEBUG, "p/t top ait page %x not cached\n", vbi->cn->btt_link[i].pgno);
					continue;
				} else if (vtp->function != PAGE_FUNCTION_AIT) {
					LogMsg( LOG_DEBUG, "p/t no ait page %x\n", vtp->pgno);
					cache_page_unref (vtp);
					vtp = NULL;
					continue;
				}
				 ...
				 int *			remaining)
{
	...
	LogMsg( LOG_DEBUG, "obj invocation, source page %03x/%04x, "
		"pointer packet %d triplet %d\n", pgno, s1, packet + 1, i);

	vtp = _vbi_cache_get_page (vbi->ca, vbi->cn, pgno, s1, 0x000F);

	if (!vtp) {
		LogMsg( LOG_DEBUG, "... page not cached\n");
		return 0;
	}
	...
	if (vtp->function == PAGE_FUNCTION_UNKNOWN) {
		...
		if (NULL == new_cp) {
			LogMsg( LOG_DEBUG, "... no g/pop page or hamming error\n");
			cache_page_unref (vtp);
			vtp = NULL;
			return 0;
		} else {
		...
		LogMsg( LOG_DEBUG, "... source page wrong function %d, expected %d\n",
			vtp->function, function);
		cache_page_unref (vtp);
		vtp = NULL;
		return 0;
	...
	LogMsg( LOG_DEBUG, "... triplet pointer %d\n", pointer);

	if (pointer > 506) {
		LogMsg( LOG_DEBUG, "... triplet pointer out of bounds (%d)\n", pointer);
		cache_page_unref (vtp);
		vtp = NULL;
		return 0;
	}
	...
	if (TELETEXT_DEBUG) {
		...
		if (packet <= 25)
			LogMsg( LOG_DEBUG, "... object start in packet %d, triplet %d (pointer %d)\n",
				packet, pointer % 13, pointer);
		else
			LogMsg( LOG_DEBUG, "... object start in packet 26/%d, triplet %d (pointer %d)\n",
	...
	*remaining = elements(vtp->data.pop.triplet) - (pointer+1);
	...
	LogMsg( LOG_DEBUG, "... obj def: ad 0x%02x mo 0x%04x dat %d=0x%x\n",
	...
	if (trip->mode != (type + 0x14) || (address & 0x1FF)) {
		LogMsg( LOG_DEBUG, "... no object definition\n");
		cache_page_unref (vtp);
		vtp = NULL;
		return 0;
	}
...
enhance_flush(struct enhance_state *es, int column)
{
	...
	LogMsg( LOG_DEBUG, "flush [%04x%c,F%d%c,B%d%c,S%d%c,O%d%c,H%d%c] %d ... %d\n",
	...
	for (i = es->inv_column + es->active_column; i < es->inv_column + column;) {
		...
		if (es->type != OBJECT_TYPE_PASSIVE
			...
			switch (raw) {
			case 0x00 :	/* alpha + foreground color */
			case 0x01:
			case 0x02:
			case 0x03:
			case 0x04:
			...
			case 0x06:
			case 0x07:
			case 0x10: /* mosaic + foreground color */
			case 0x11:
			case 0x12:
			case 0x13:
			case 0x14:
			...
			case 0x16:
			case 0x17:
				LogMsg( LOG_DEBUG, "... fg term %d %02x\n", i, raw);
				es->mac.foreground = 0;
				es->mac.conceal = 0;
				break;
			...
			case 0x0B:		/* start box */
				if (i < COLUMNS && vbi_unpar8 (es->vtp->data.lop.raw[row][i]) == raw) {
					LogMsg( LOG_DEBUG, "... boxed term %d %02x\n", i, raw);
					es->mac.opacity = 0;
				}
			...
			case 0x0F:		/* double size */
				LogMsg( LOG_DEBUG, "... size term %d %02x\n", i, raw);
				es->mac.size = 0;
				break;
			}
			...
			case 0x1D:		/* new background */
				LogMsg( LOG_DEBUG, "... bg term %d %02x\n", i, raw);
				es->mac.background = 0;
				break;
			}
		}
	...
	struct pex26 *ptable)
{
	...
	for (; max_triplets>0; p++, max_triplets--) {
		if (p->address >= COLUMNS) {
			 ...
			 */
			int s = p->data >> 5;
			int row = (p->address - COLUMNS) ? 0 : (ROWS - 1);
			int column = 0;
			...
			set_active:
				...
				LogMsg( LOG_DEBUG, "enh set_active row %d col %d\n", row, column);
			...
			case 0x10:		/* origin modifier */
				...
				LogMsg( LOG_DEBUG, "enh origin modifier col %+d row %+d\n",
					offset_column, offset_row);

				break;

			case 0x11:/* object invocation */
			case 0x12:
			case 0x13:
			{
				...
				LogMsg( LOG_DEBUG, "enh obj invocation "
				       "source %d type %d\n",
				       source, new_type);

				if (new_type <= type) { /* 13.2++ */
					LogMsg( LOG_DEBUG, "... priority violation\n");
					break;
				}
				...
				if (source == 0) /* illegal */
					...
					LogMsg( LOG_DEBUG, "... local obj %d/%d\n", designation, triplet);

					if (!(vtp->x26_designations & 1)) {
						LogMsg( LOG_DEBUG, "... no packet %d\n", designation);
						return FALSE;
					}
				...
				else /* global / public */
				{
					...
					if (source == 3) {
						...
						if (NO_PAGE(pgno)) {
							...
							LogMsg( LOG_DEBUG, "... X/27/4 GPOP overrides MOT\n");
					} else {
						...
						if (NO_PAGE(pgno)) {
							if ((i = mag->pop_lut[vtp->pgno & 0xFF]) == 0) {
								LogMsg( LOG_DEBUG, "... MOT pop_lut empty\n");
							...
							LogMsg( LOG_DEBUG, "... X/27/4 POP overrides MOT\n");
					}

					if (NO_PAGE(pgno)) {
						LogMsg( LOG_DEBUG, "... dead MOT link %d\n", i);
						return FALSE; /* has no link (yet) */
					}

					LogMsg( LOG_DEBUG, "... %s obj\n", (source == 3) ? "global" : "public");
			...
			case 0x15:	/* object definition */
			case 0x16:
			case 0x17:
				enhance_flush_row (&es);
				LogMsg( LOG_DEBUG, "enh obj definition 0x%02x 0x%02x\n", p->mode, p->data);
				LogMsg( LOG_DEBUG, "enh terminated\n");
				goto swedish;

			case 0x18:		/* drcs mode */
				LogMsg( LOG_DEBUG, "enh DRCS mode 0x%02x\n", p->data);
			...
			default:
	                terminate:
				enhance_flush_row (&es);
				LogMsg( LOG_DEBUG, "enh terminated %02x\n", p->mode);
				goto swedish;
			}
		} else {
			/*
			...
			case 0x00:		/* foreground color */
				if (max_level >= VBI_WST_LEVEL_2p5 && s == 0) {
					...
					LogMsg( LOG_DEBUG, "enh col %d foreground %d\n",
			...
			case 0x03:		/* background color */
				if (max_level >= VBI_WST_LEVEL_2p5 && s == 0) {
					...
					LogMsg( LOG_DEBUG, "enh col %d background %d\n",
			...
			case 0x07:		/* additional flash functions */
				if (max_level >= VBI_WST_LEVEL_2p5) {
					...
					LogMsg( LOG_DEBUG, "enh col %d flash 0x%02x\n", es.active_column, p->data);
				}
			...
			case 0x08:		/* modified G0 and G2 character set designation */
				if (max_level >= VBI_WST_LEVEL_2p5) {
					...
					LogMsg( LOG_DEBUG, "enh col %d modify character set %d\n",
					       es.active_column, p->data);
				}
			...
			case 0x0C:		/* display attributes */
				...
				LogMsg( LOG_DEBUG, "enh col %d display attr 0x%02x\n",
				       es.active_column, p->data);
			...
			case 0x0D:		/* drcs character invocation */
			{
				...
				LogMsg( LOG_DEBUG, "enh col %d DRCS %d/0x%02x\n",
				...
				/* if (!pg->drcs[page]) */ {
					...
					if (!normal) {
						...
						if (NO_PAGE(pgno)) {
							...
							LogMsg( LOG_DEBUG, "... X/27/4 GDRCS overrides MOT\n");
					} else {
						...
						if (NO_PAGE(pgno)) {
							if ((i = mag->drcs_lut[vtp->pgno & 0xFF]) == 0) {
								LogMsg( LOG_DEBUG, "... MOT drcs_lut empty\n");
								return FALSE; /* has no link (yet) */
							}
							...
							LogMsg( LOG_DEBUG, "... X/27/4 DRCS overrides MOT\n");
					}
					...
					LogMsg( LOG_DEBUG, "... %s drcs from page %03x/%04x\n",
					...
					if (dvtp->function == PAGE_FUNCTION_UNKNOWN) {
						...
						new_cp = vbi_convert_page
							(vbi, dvtp, TRUE,
							 function);
						if (NULL == new_cp) {
							LogMsg( LOG_DEBUG, "... no g/drcs page or hamming error\n");
					...
					if (dvtp->data.drcs.invalid & (1ULL << offset)) {
						LogMsg( LOG_DEBUG, "... invalid drcs, prob. tx error\n");
						cache_page_unref (dvtp);
						dvtp = NULL;
						return FALSE;
					}
			...
			case 0x0E:		/* font style */
			{
				...
				LogMsg( LOG_DEBUG, "enh col %d font style 0x%02x\n",
				       es.active_column, p->data);
			...
			case 0x10: /* characters including diacritical marks */
			case 0x11:
			case 0x12:
			case 0x13:
			case 0x14:
			...
			case 0x1F:
				if (p->data >= 0x20) {
					...
					unicode = vbi_teletext_composed_unicode(
						p->mode - 0x10, p->data);
			store:
					LogMsg( LOG_DEBUG, "enh row %d col %d print 0x%02x/0x%02x -> 0x%04x\n",
...
post_enhance(vbi_page *pg, int display_rows)
{
	...
	for (row = 0; row <= last_row; row++) {
		for (column = 0; column < COLUMNS; acp++, column++) {
			if (1)
				LogMsg( LOG_DEBUG, "%c", _vbi_to_ascii (acp->unicode));
			else
				LogMsg( LOG_DEBUG, "%04xF%dB%dS%dO%d ", acp->unicode,
				 ...
				 vbi_bool		header_only)
{
	...
	if (max_level < VBI_WST_LEVEL_3p5 || NO_PAGE(pop->pgno)) {
		...
		if (NO_PAGE(pop->pgno)) {
			LogMsg( LOG_DEBUG, "default object has dead MOT pop link %d\n", i);
			return FALSE;
		}
	}
	...
	for (i = 0; i < 2; i++) {
		...
		LogMsg( LOG_DEBUG, "default object #%d invocation, type %d\n", i ^ order, type);
		   ...
		   int display_rows, vbi_bool navigation)
{
	...
	for (row = 0; row < display_rows; row++) {
		...
		for (column = 0; column < COLUMNS; ++column) {
			...
			case 0x0E:		/* double width */
				LogMsg( LOG_DEBUG, "spacing col %d row %d double width\n", column, row);
				if (column < (COLUMNS - 1))
					ac.size = VBI_DOUBLE_WIDTH;
				break;

			case 0x0F:		/* double size */
				LogMsg( LOG_DEBUG, "spacing col %d row %d double size\n", column, row);
	...
	if (max_level >= VBI_WST_LEVEL_1p5 && display_rows > 0) {
		...
		if (vtp->x26_designations & 1) {
			LogMsg( LOG_DEBUG, "enhancement packets %08x\n",
