				 ...
				 unsigned int		n_bytes)
{
	...
	for (;;) {
		...
		LogMsg( LOG_DEBUG,  "%p: %s", src, buffer);
...
{
	LogMsg( LOG_DEBUG,  "Discarding raw VBI line.");
				 ...
				 const vbi_sliced *	s)
{
	...
	LogMsg( LOG_DEBUG,  "DU-TTX %u >%s<", s->line, buffer);
}
				 ...
				 unsigned int *		src_left)
{
	...
	for (;;) {
		...
		LogMsg( LOG_DEBUG,  "TS packet tei=%u pusi=%u tp=%u "
		...
		if (unlikely (0 != (b1 & 0x80))) {
			LogMsg( LOG_DEBUG,  "Transport error.");
			if (0) {
				err = VBI_ERR_SYNC_LOST;
		...
		if (0 == dx->ts_pes_todo) {
			...
			if (unlikely (0 != (b1 & 0x40))) {
				LogMsg( LOG_DEBUG,  "Unexpected TS "
					"payload_unit_start_indicator.");
				if (0) {
					err = VBI_ERR_STREAM_SYNTAX;
					goto bad_ts_packet_return;
