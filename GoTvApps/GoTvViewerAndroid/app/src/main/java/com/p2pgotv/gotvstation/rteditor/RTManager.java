...
package com.p2pgotv.gotvstation.rteditor;
...
import com.p2pgotv.gotvstation.rteditor.LinkFragment.Link;
import com.p2pgotv.gotvstation.rteditor.LinkFragment.LinkEvent;
import com.p2pgotv.gotvstation.rteditor.RTOperationManager.TextChangeOperation;
...
import com.p2pgotv.gotvstation.rteditor.effects.AbsoluteSizeEffect;
import com.p2pgotv.gotvstation.rteditor.effects.AlignmentEffect;
import com.p2pgotv.gotvstation.rteditor.effects.BackgroundColorEffect;
import com.p2pgotv.gotvstation.rteditor.effects.BoldEffect;
import com.p2pgotv.gotvstation.rteditor.effects.BulletEffect;
import com.p2pgotv.gotvstation.rteditor.effects.Effect;
import com.p2pgotv.gotvstation.rteditor.effects.Effects;
import com.p2pgotv.gotvstation.rteditor.effects.ForegroundColorEffect;
import com.p2pgotv.gotvstation.rteditor.effects.ItalicEffect;
import com.p2pgotv.gotvstation.rteditor.effects.NumberEffect;
import com.p2pgotv.gotvstation.rteditor.effects.SpanCollectMode;
import com.p2pgotv.gotvstation.rteditor.effects.StrikethroughEffect;
import com.p2pgotv.gotvstation.rteditor.effects.SubscriptEffect;
import com.p2pgotv.gotvstation.rteditor.effects.SuperscriptEffect;
import com.p2pgotv.gotvstation.rteditor.effects.TypefaceEffect;
import com.p2pgotv.gotvstation.rteditor.effects.UnderlineEffect;
import com.p2pgotv.gotvstation.rteditor.fonts.RTTypeface;
import com.p2pgotv.gotvstation.rteditor.media.choose.MediaChooserActivity;
import com.p2pgotv.gotvstation.rteditor.spans.ImageSpan;
import com.p2pgotv.gotvstation.rteditor.spans.LinkSpan;
import com.p2pgotv.gotvstation.rteditor.spans.RTSpan;
import com.p2pgotv.gotvstation.rteditor.utils.Constants;
import com.p2pgotv.gotvstation.rteditor.utils.Constants.MediaAction;
import com.p2pgotv.gotvstation.rteditor.utils.Helper;
import com.p2pgotv.gotvstation.rteditor.utils.Selection;
...
import com.p2pgotv.gotvstation.event.EventBus;
            ...
            public void run() 
            {
                ...
                if( image != null && editor != null ) 
                {
                    ...
                    try 
                    {
                        ...
                        String imagePath = image.getFilePath( RTFormat.SPANNED );
                        String rootPath = editor.getRootPath();
                        if( ( false == imagePath.contains( "/" ) ) // just file name
                        	|| ( ( imagePath.length() >= rootPath.length() ) // full path already in root path
                        	&& !rootPath.isEmpty()
                        	&& imagePath.regionMatches( 0, rootPath, 0, rootPath.length() ) ) )
                        {
                        	// image is already in the root folder
                        	isRelativePath = true;
                        }
                        else if( !rootPath.isEmpty() )
                        {
	                        // create a unique name in dest root folder
	                        String destPathAndName = editor.getRootPath() + image.getFileName();
	                        destPathAndName = VxFileUtil.getUniqueFileName( destPathAndName );
	                        if( true == VxImageUtil.copyImageUrlToStdSizeBitmapFile( imagePath, destPathAndName ) )
	                        {
	                        	image.setFilePath( VxFileUtil.getJustFileName( destPathAndName ) );
	                        	VxFileUtil.deleteFile( imagePath );
	                        	isRelativePath = true;
	                        }
                        } 
                        
                        ImageSpan imageSpan = new ImageSpan(image, false, isRelativePath, rootPath );
