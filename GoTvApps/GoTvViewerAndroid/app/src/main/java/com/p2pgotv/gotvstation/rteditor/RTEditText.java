...
import com.p2pgotv.gotvstation.rteditor.api.format.RTEditable;
import com.p2pgotv.gotvstation.rteditor.api.format.RTFormat;
import com.p2pgotv.gotvstation.rteditor.api.format.RTHtml;
import com.p2pgotv.gotvstation.rteditor.api.format.RTPlainText;
import com.p2pgotv.gotvstation.rteditor.api.format.RTText;
...
import com.p2pgotv.gotvstation.rteditor.spans.LinkSpan.LinkSpanListener;
import com.p2pgotv.gotvstation.rteditor.spans.MediaSpan;
...
public class RTEditText extends EditText implements TextWatcher, SpanWatcher, LinkSpanListener 
{
     ...
     */
    public void setRichTextEditing( String rootDir, boolean useRTFormatting, String content) {
        assertRegistration();
        ...
        RTText rtText = useRTFormatting ?
                new RTHtml<RTImage, RTAudio, RTVideo>( rootDir, RTFormat.HTML, content) :
    ...
    public void setText(RTText rtText) 
    {
        ...
        if (rtText.getFormat() instanceof RTFormat.Html) 
        {
            if( mUseRTFormatting ) 
            {
                RTText rtSpanned = rtText.convertTo( getRootPath(), RTFormat.SPANNED, mMediaFactory );
            ...
            {
                RTText rtPlainText = rtText.convertTo( getRootPath(), RTFormat.PLAIN_TEXT, mMediaFactory);
    ...
    public RTText getRichText( RTFormat format ) 
    {
        ...
        return rtEditable.convertTo( getRootPath(), format, mMediaFactory);
    }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if(state instanceof SavedState) 
        {
            ...
            setRichTextEditing( m_RootPath, savedState.useRTFormatting(), savedState.getContent());
        }
        else 
        {
