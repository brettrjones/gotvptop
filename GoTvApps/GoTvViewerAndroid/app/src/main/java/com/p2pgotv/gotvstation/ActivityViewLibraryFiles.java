...
public class ActivityViewLibraryFiles  extends ActivityListBase implements ToGuiFileXferInterface
{
	//=== constants ===//
	public static final String LOG_TAG = "ActivityViewLibraryFiles:";
    ...
    {
        setContentView(R.layout.activity_view_library_files);
        super.onCreate(savedInstanceState);
        setPageTitle( R.string.activity_view_library_files_title );
		...
		this.m_AddFilesBut = (Button)this.findViewById(R.id.library_files_add_button);
	 	...
	 	this.m_FileFilterSpinner 	= (Spinner)this.findViewById( R.id.library_files_file_filter_spinner );	
	...
	{
		Intent myIntent = new Intent( ActivityViewLibraryFiles.this, ActivityBrowseFiles.class );
		...
		ActivityViewLibraryFiles.this.startActivityForResult( myIntent, Constants.ACTIVITY_BROWSE_DEVICE_FILES );
	}
	...
	private void doRequestFileList() 
	{
		...
		clearFileList();
		NativeTxTo.fromGuiGetFileLibraryList( fileFilterMask  );
	}
			...
			if( null != fileInfo )
			{
				...
				else
				{
						...
						&& ( null != fileInfo ) )
					{
			        	...
			        	if( !fileName.isEmpty() )
			        	{
			        		resultIntent.putExtra( Constants.PARAM_FILE_NAME, fileName );
			        		resultIntent.putExtra( Constants.PARAM_FILE_TYPE_MASK, fileInfo.getFileType() );				        		
			        	}

			        	ActivityViewLibraryFiles.this.setResult( Activity.RESULT_OK, resultIntent );
						finish();
					}
					else
					{
