...
import com.p2pgotv.gotvstation.rteditor.converter.ConverterSpannedToHtml;
...
public final class RTEditable extends RTSpanned 
{
    ...
    public RTText convertTo( String rootPath, RTFormat destFormat, RTMediaFactory<RTImage, RTAudio, RTVideo> mediaFactory) 
    {
        ...
        {
            clean();
            return new ConverterSpannedToHtml().convert( rootPath, mEditor.getText(), (RTFormat.Html) destFormat);
        } 
        else if (destFormat instanceof RTFormat.PlainText) 
        {
            clean();
            RTHtml<RTImage, RTAudio, RTVideo> rtHtml = new ConverterSpannedToHtml().convert( rootPath, mEditor.getText(), RTFormat.HTML);
            RTText rtText = rtHtml.convertTo( rootPath, RTFormat.PLAIN_TEXT, mediaFactory );
