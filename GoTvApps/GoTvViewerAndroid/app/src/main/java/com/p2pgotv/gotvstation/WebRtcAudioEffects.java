...
class WebRtcAudioEffects {
  ...
  private static final String LOG_TAG = "WebRtcAudioEffects";
  ...
  public static boolean isAcousticEchoCancelerBlacklisted() {
    ...
    if (isBlacklisted) {
      Log.w(LOG_TAG, Build.MODEL + " is blacklisted for HW AEC usage!");
    }
  ...
  public static boolean isAutomaticGainControlBlacklisted() {
   List<String> blackListedModels =
    ...
    boolean isBlacklisted = blackListedModels.contains(Build.MODEL);
    if (isBlacklisted) {
      Log.w(LOG_TAG, Build.MODEL + " is blacklisted for HW AGC usage!");
    }
    return isBlacklisted;
  }
  ...
  public static boolean isNoiseSuppressorBlacklisted() {
    ...
    boolean isBlacklisted = blackListedModels.contains(Build.MODEL);
    if (isBlacklisted) {
      Log.w(LOG_TAG, Build.MODEL + " is blacklisted for HW NS usage!");
    }
    return isBlacklisted;
  }
  ...
  public static boolean canUseAcousticEchoCanceler() {
    if (canUseAcousticEchoCanceler == null) {
      ...
      Log.d(LOG_TAG, "canUseAcousticEchoCanceler: "
  ...
  public static boolean canUseAutomaticGainControl() {
    if (canUseAutomaticGainControl == null) {
      ...
      Log.d(LOG_TAG, "canUseAutomaticGainControl: "
  ...
  public static boolean canUseNoiseSuppressor() {
    if (canUseNoiseSuppressor == null) {
      ...
      Log.d(LOG_TAG, "canUseNoiseSuppressor: " + canUseNoiseSuppressor);
    }
  ...
  static WebRtcAudioEffects create() {
    ...
    if (!WebRtcAudioUtils.runningOnJellyBeanOrHigher()) {
      Log.w(LOG_TAG, "API level 16 or higher is required!");
      return null;
    }
  ...
  public boolean setAEC(boolean enable) {
    Log.d(LOG_TAG, "setAEC(" + enable + ")");
    if (!canUseAcousticEchoCanceler()) {
      Log.w(LOG_TAG, "Platform AEC is not supported");
      shouldEnableAec = false;
      return false;
    }
    if (aec != null && (enable != shouldEnableAec)) {
      Log.e(LOG_TAG, "Platform AEC state can't be modified while recording");
      return false;
    }
  ...
  public boolean setAGC(boolean enable) {
    Log.d(LOG_TAG, "setAGC(" + enable + ")");
    if (!canUseAutomaticGainControl()) {
      Log.w(LOG_TAG, "Platform AGC is not supported");
      shouldEnableAgc = false;
      return false;
    }
    if (agc != null && (enable != shouldEnableAgc)) {
      Log.e(LOG_TAG, "Platform AGC state can't be modified while recording");
      return false;
    }
  ...
  public boolean setNS(boolean enable) {
    Log.d(LOG_TAG, "setNS(" + enable + ")");
    if (!canUseNoiseSuppressor()) {
      Log.w(LOG_TAG, "Platform NS is not supported");
      shouldEnableNs = false;
      return false;
    }
    if (ns != null && (enable != shouldEnableNs)) {
      Log.e(LOG_TAG, "Platform NS state can't be modified while recording");
      return false;
    }
  ...
  public void enable(int audioSession) {
    Log.d(LOG_TAG, "enable(audioSession=" + audioSession + ")");
    assertTrue(aec == null);
    assertTrue(agc == null);
    assertTrue(ns == null);
    ...
    if (isAcousticEchoCancelerSupported()) {
      ...
      if (aec != null) {
        ...
        if (aec.setEnabled(enable) != AudioEffect.SUCCESS) {
          Log.e(LOG_TAG, "Failed to set the AcousticEchoCanceler state");
        }
        Log.d(LOG_TAG, "AcousticEchoCanceler: was "
            + (enabled ? "enabled" : "disabled")
            + ", enable: " + enable + ", is now: "
            + (aec.getEnabled() ? "enabled" : "disabled"));
      } else {
        Log.e(LOG_TAG, "Failed to create the AcousticEchoCanceler instance");
      }
    }
    ...
    if (isAutomaticGainControlSupported()) {
      ...
      if (agc != null) {
        ...
        if (agc.setEnabled(enable) != AudioEffect.SUCCESS) {
          Log.e(LOG_TAG, "Failed to set the AutomaticGainControl state");
        }
        Log.d(LOG_TAG, "AutomaticGainControl: was "
            + (enabled ? "enabled" : "disabled")
            + ", enable: " + enable + ", is now: "
            + (agc.getEnabled() ? "enabled" : "disabled"));
      } else {
        Log.e(LOG_TAG, "Failed to create the AutomaticGainControl instance");
      }
    }
    ...
    if (isNoiseSuppressorSupported()) {
      ...
      if (ns != null) {
        ...
        if (ns.setEnabled(enable) != AudioEffect.SUCCESS) {
          Log.e(LOG_TAG, "Failed to set the NoiseSuppressor state");
        }
        Log.d(LOG_TAG, "NoiseSuppressor: was "
            + (enabled ? "enabled" : "disabled")
            + ", enable: " + enable + ", is now: "
            + (ns.getEnabled() ? "enabled" : "disabled"));
      } else {
        Log.e(LOG_TAG, "Failed to create the NoiseSuppressor instance");
      }
    }
  }
