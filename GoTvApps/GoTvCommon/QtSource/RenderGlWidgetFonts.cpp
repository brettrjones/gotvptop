...
bool RenderGlWidget::firstBegin( CGUIFontTTFQt * font )
{
    ...
    {
        if( m_GlThreadFunctions->glIsTexture( font->m_nTexture ) )
    ...
    if( font->m_textureStatus == CGUIFontTTFQt::TEXTURE_VOID )
    {
        ...
        m_GlThreadFunctions->glGenTextures( 1, ( GLuint* )&font->m_nTexture );

        // Bind the texture object
        m_GlThreadFunctions->glBindTexture( GL_TEXTURE_2D, font->m_nTexture );

        // Set the texture's stretching properties
        m_GlThreadFunctions->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
        m_GlThreadFunctions->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );

        // Set the texture image -- THIS WORKS, so the pixels must be wrong.
        m_GlThreadFunctions->glTexImage2D( GL_TEXTURE_2D, 0, pixformat, font->m_texture->GetWidth(), font->m_texture->GetHeight(), 0,
    ...
    {
        m_GlThreadFunctions->glBindTexture( GL_TEXTURE_2D, font->m_nTexture );
        m_GlThreadFunctions->glTexSubImage2D( GL_TEXTURE_2D, 0, 0, font->m_updateY1, font->m_texture->GetWidth(), font->m_updateY2 - font->m_updateY1, pixformat, GL_UNSIGNED_BYTE,
...
void RenderGlWidget::lastEnd( CGUIFontTTFQt * font )
{
    ...
    if( !font->m_vertex.empty() )
    {
        ...
        m_GlThreadFunctions->glVertexAttribPointer( posLoc, 3, GL_FLOAT, GL_FALSE, sizeof( SVertex ), ( char* )vertices + offsetof( SVertex, x ) );
        m_GlThreadFunctions->glVertexAttribPointer( colLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( SVertex ), ( char* )vertices + offsetof( SVertex, r ) );
        m_GlThreadFunctions->glVertexAttribPointer( tex0Loc, 2, GL_FLOAT, GL_FALSE, sizeof( SVertex ), ( char* )vertices + offsetof( SVertex, u ) );

        m_GlThreadFunctions->glDrawArrays( GL_TRIANGLES, 0, ( GLsizei)vecVertices.size() );
    }
    ...
    if( !font->m_vertexTrans.empty() )
    {
        ...
        m_GlThreadFunctions->glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, font->m_elementArrayHandle );
        ...
        for( size_t i = 0; i < font->m_vertexTrans.size(); i++ )
        {
            ...
            m_GlThreadFunctions->glUniformMatrix4fv( modelLoc, 1, GL_FALSE, glMatrixModview.Get() );

            // Bind the buffer to the OpenGL context's GL_ARRAY_BUFFER binding point
            m_GlThreadFunctions->glBindBuffer( GL_ARRAY_BUFFER, font->m_vertexTrans[ i ].vertexBuffer->bufferHandle );
            ...
            for( size_t character = 0; font->m_vertexTrans[ i ].vertexBuffer->size > character; character += ELEMENT_ARRAY_MAX_CHAR_INDEX )
            {
                ...
                m_GlThreadFunctions->glVertexAttribPointer( posLoc, 3, GL_FLOAT, GL_FALSE, sizeof( SVertex ), ( GLvoid * )( character * sizeof( SVertex ) * 4 + offsetof( SVertex, x ) ) );
                m_GlThreadFunctions->glVertexAttribPointer( colLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( SVertex ), ( GLvoid * )( character * sizeof( SVertex ) * 4 + offsetof( SVertex, r ) ) );
                m_GlThreadFunctions->glVertexAttribPointer( tex0Loc, 2, GL_FLOAT, GL_FALSE, sizeof( SVertex ), ( GLvoid * )( character * sizeof( SVertex ) * 4 + offsetof( SVertex, u ) ) );

                m_GlThreadFunctions->glDrawElements( GL_TRIANGLES, 6 * count, GL_UNSIGNED_SHORT, 0 );
            }
        ...
        m_GlThreadFunctions->glBindBuffer( GL_ARRAY_BUFFER, 0 );
        m_GlThreadFunctions->glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, 0 );
    }
...
CVertexBuffer RenderGlWidget::createVertexBuffer( CGUIFontTTFQt * font, const std::vector<SVertex>& vertices )
{
    ...
    GLuint bufferHandle;
    m_GlThreadFunctions->glGenBuffers( 1, &bufferHandle );
    // Bind the buffer to the OpenGL context's GL_ARRAY_BUFFER binding point
    m_GlThreadFunctions->glBindBuffer( GL_ARRAY_BUFFER, bufferHandle );
    ...
    m_GlThreadFunctions->glBufferData( GL_ARRAY_BUFFER, vertices.size() * sizeof( SVertex ), &vertices[ 0 ], GL_STATIC_DRAW );
...
void RenderGlWidget::destroyVertexBuffer( CGUIFontTTFQt * font, CVertexBuffer& vertBuffer )
{
    if( vertBuffer.bufferHandle != 0 )
    {
        ...
        m_GlThreadFunctions->glDeleteBuffers( 1, ( GLuint * )&vertBuffer.bufferHandle );
...
void RenderGlWidget::createStaticVertexBuffers( CGUIFontTTFQt * font )
{
    ...
    m_GlThreadFunctions->glGenBuffers( 1, &font->m_elementArrayHandle );
    m_GlThreadFunctions->glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, font->m_elementArrayHandle );
    ...
    m_GlThreadFunctions->glBufferData( GL_ELEMENT_ARRAY_BUFFER, sizeof index, index, GL_STATIC_DRAW );
    m_GlThreadFunctions->glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, 0 );
...
{
    m_GlThreadFunctions->glDeleteBuffers( 1, &font->m_elementArrayHandle );
