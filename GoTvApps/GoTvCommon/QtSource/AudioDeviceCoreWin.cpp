...
bool AudioDeviceWindowsCore::CoreAudioIsSupported()
{
    ...
    if (FAILED(hr))
    {
        ...
        StringCchPrintfW(buf, MAXERRORLENGTH, TEXT(L"Error details: "));
    ...
    {
       LogMsg( LOG_INFO, "*** Windows Core Audio is supported ***");
    }
    else
    {
       LogMsg( LOG_INFO, "*** Windows Core Audio is NOT supported => will revert to the Wave API ***");
    }
...
AudioDeviceWindowsCore::AudioDeviceWindowsCore(const int32_t id) :
    ...
    _newMicLevel(0)
{
    LogMsg( LOG_INFO, "%s created", __FUNCTION__);
    ...
    if (!_avrtLibrary)
    {
        ...
        if (_avrtLibrary)
        {
            ...
            LogMsg( LOG_INFO,"AudioDeviceWindowsCore::AudioDeviceWindowsCore() The Avrt DLL module is now loaded");
            ...
            {
                LogMsg( LOG_INFO,"AudioDeviceWindowsCore::AudioDeviceWindowsCore() AvRevertMmThreadCharacteristics() is OK");
                LogMsg( LOG_INFO,"AudioDeviceWindowsCore::AudioDeviceWindowsCore() AvSetMmThreadCharacteristicsA() is OK");
                LogMsg( LOG_INFO,"AudioDeviceWindowsCore::AudioDeviceWindowsCore() AvSetMmThreadPriority() is OK");
    ...
    _recChannelsPrioList[0] = 1;    // mono is prio 1
    _recChannelsPrioList[1] = 2;    // stereo is prio 2

    // list of number of channels to use on playout side
    _playChannelsPrioList[0] = 1;    // mono is prio 1
    _playChannelsPrioList[1] = 2;    // stereo is prio 2
...
{
    LogMsg( LOG_INFO,"%s destroyed", __FUNCTION__);
...
int32_t AudioDeviceWindowsCore::InitSpeaker()
{
    ...
    if (_usingOutputDeviceIndex)
    {
        ...
        {
            LogMsg( LOG_ERROR,"current device selection is invalid => unable to initialize");
            return -1;
        }
    }
    ...
    {
        LogMsg( LOG_ERROR,"failed to initialize the rendering enpoint device");
        SAFE_RELEASE(_ptrDeviceOut);
        return -1;
    }
...
int32_t AudioDeviceWindowsCore::InitMicrophone()
{
    ...
    {
        LogMsg( LOG_ERROR,"failed to initialize the capturing enpoint device");
        SAFE_RELEASE(_ptrDeviceIn);
        return -1;
    }
...
{
    LogMsg( LOG_INFO,"AudioDeviceWindowsCore::SetMicrophoneVolume(volume=%u)", volume);
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(uint16_t index)
{
    ...
    {
        LogMsg( LOG_INFO,"friendly name: \"%S\"", szDeviceName);
    }
    ...
    char guid[kAdmMaxGuidSize])
{
    ...
    if (index == (uint16_t)(-1))
    {
        ...
        index = 0;
        LogMsg( LOG_INFO,"Default Communication endpoint device will be used");
    }
    ...
    if (ret == 0)
    {
        ...
        {
            LogMsg( LOG_ERROR,"WideCharToMultiByte(CP_UTF8) failed with error code %d", GetLastError());
        }
    }
...
int32_t AudioDeviceWindowsCore::InitPlayout()
{
    ...
    if (InitSpeaker() == -1)
    {
        LogMsg( LOG_WARN, "InitSpeaker() failed");
    }
    ...
    if (SUCCEEDED(hr))
    {
        LogMsg( LOG_INFO,"Audio Engine's current rendering mix format:\n");
        // format type
        LogMsg( LOG_INFO,"wFormatTag     : 0x%X (%u)\n", pWfxOut->wFormatTag, pWfxOut->wFormatTag);
        // number of channels (i.e. mono, stereo...)
        LogMsg( LOG_INFO,"nChannels      : %d\n", pWfxOut->nChannels);
        // sample rate
        LogMsg( LOG_INFO,"nSamplesPerSec : %d\n", pWfxOut->nSamplesPerSec);
        // for buffer estimation
        LogMsg( LOG_INFO,"nAvgBytesPerSec: %d\n", pWfxOut->nAvgBytesPerSec);
        // block size of data
        LogMsg( LOG_INFO,"nBlockAlign    : %d\n", pWfxOut->nBlockAlign);
        // number of bits per sample of mono data
        LogMsg( LOG_INFO,"wBitsPerSample : %d\n", pWfxOut->wBitsPerSample);
        LogMsg( LOG_INFO,"cbSize         : %d\n", pWfxOut->cbSize);
    }
    ...
    const int freqs[] = {8000, 16000, 32000, 48000, 44100 };
    hr = S_FALSE;
    ...
    for (unsigned int freq = 0; freq < sizeof(freqs)/sizeof(freqs[0]); freq++)
    {
        for (unsigned int chan = 0; chan < sizeof(_playChannelsPrioList)/sizeof(_playChannelsPrioList[0]); chan++)
        {
            ...
            {
                LogMsg( LOG_INFO,"nChannels=%d, nSamplesPerSec=%d is not supported\n",
    ...
    if (hr == S_OK)
    {
        ...
        LogMsg( LOG_INFO,"VoE selected this rendering format:\n");
        ...
        LogMsg( LOG_INFO,"wBitsPerSample     : %d\n", Wfx.wBitsPerSample);
        LogMsg( LOG_INFO,"cbSize             : %d\n", Wfx.cbSize);
        LogMsg( LOG_INFO,"Additional settings:\n");
        LogMsg( LOG_INFO,"_playAudioFrameSize: %d\n", _playAudioFrameSize);
        LogMsg( LOG_INFO,"_playBlockSize     : %d\n", _playBlockSize);
        LogMsg( LOG_INFO,"_playChannels      : %d\n", _playChannels);
    }
    ...
    if (FAILED(hr))
    {
        LogMsg( LOG_ERROR,"IAudioClient::Initialize() failed:\n");
        if (pWfxClosestMatch != NULL)
        {
            LogMsg( LOG_ERROR,"closest mix format: #channels=%d, samples/sec=%d, bits/sample=%d\n",
                pWfxClosestMatch->nChannels, pWfxClosestMatch->nSamplesPerSec, pWfxClosestMatch->wBitsPerSample);
        }
        else
        {
            LogMsg( LOG_ERROR,"no format suggested");
        }
    }
    EXIT_ON_ERROR(hr);
    ...
    else
    {
        ...
        LogMsg( LOG_INFO,"AudioDeviceBuffer must be attached before streaming can start\n");
    }
    ...
    if (SUCCEEDED(hr))
    {
        LogMsg( LOG_INFO,"IAudioClient::GetBufferSize() => %u (<=> %u bytes)\n",
    ...
    LogMsg( LOG_INFO,"render side is now initialized\n");
    return 0;
...
int32_t AudioDeviceWindowsCore::InitRecordingDMO()
{
    ...
    ptrWav->nSamplesPerSec = 8000;
    ptrWav->nAvgBytesPerSec = 16000;
...
int32_t AudioDeviceWindowsCore::InitRecording()
{
    ...
    if (InitMicrophone() == -1)
    {
        LogMsg( LOG_WARN, "InitMicrophone() failed\n");
    }
    ...
    if (SUCCEEDED(hr))
    {
        LogMsg( LOG_INFO,"Audio Engine's current capturing mix format:\n");
        // format type
        LogMsg( LOG_INFO,"wFormatTag     : 0x%X (%u)\n", pWfxIn->wFormatTag, pWfxIn->wFormatTag);
        // number of channels (i.e. mono, stereo...)
        LogMsg( LOG_INFO,"nChannels      : %d\n", pWfxIn->nChannels);
        // sample rate
    ...
    const int freqs[6] = {8000, 16000, 32000, 48000, 44100 };
    hr = S_FALSE;
    ...
    if (hr == S_OK)
    {
        ...
        LogMsg( LOG_INFO,"VoE selected this capturing format:\n");
        LogMsg( LOG_INFO,"wFormatTag        : 0x%X (%u)\n", Wfx.wFormatTag, Wfx.wFormatTag);
        LogMsg( LOG_INFO,"nChannels         : %d\n", Wfx.nChannels);
        LogMsg( LOG_INFO,"nSamplesPerSec    : %d\n", Wfx.nSamplesPerSec);
        LogMsg( LOG_INFO,"nAvgBytesPerSec   : %d\n", Wfx.nAvgBytesPerSec);
        ...
        LogMsg( LOG_INFO,"cbSize            : %d\n", Wfx.cbSize);
        LogMsg( LOG_INFO,"Additional settings:\n");
        LogMsg( LOG_INFO,"_recAudioFrameSize: %d\n", _recAudioFrameSize);
        LogMsg( LOG_INFO,"_recBlockSize     : %d\n", _recBlockSize);
        LogMsg( LOG_INFO,"_recChannels      : %d\n", _recChannels);
    }
    ...
    LogMsg( LOG_INFO,"capture side is now initialized\n");
    return 0;
...
DWORD AudioDeviceWindowsCore::DoRenderThread()
{
    ...
    if (_winSupportAvrt)
    {
        ...
        if (hMmTask)
        {
            ...
            {
                LogMsg( LOG_WARN, "failed to boost play-thread using MMCSS");
            }
            LogMsg( LOG_INFO,"render thread is now registered with MMCSS (taskIndex=%d)\n", taskIndex);
        }
        else
        {
            LogMsg( LOG_WARN, "failed to enable MMCSS on render thread (err=%d)\n", GetLastError());
    ...
    EXIT_ON_ERROR(hr);
    LogMsg( LOG_INFO,"[REND] size of buffer       : %u\n", bufferLength);
    ...
    LogMsg( LOG_INFO,"[REND] max stream latency   : %u (%3.2f ms)\n",
    ...
    LogMsg( LOG_INFO,"[REND] device period        : %u (%3.2f ms)\n",
    ...
    while (keepPlaying)
    {
        ...
        case WAIT_OBJECT_0 + 1:     // _hRenderSamplesReadyEvent
            break;
        case WAIT_TIMEOUT:          // timeout notification
            LogMsg( LOG_WARN, "render event timed out after 0.5 seconds\n");
            goto Exit;
        default:                    // unexpected error
            LogMsg( LOG_WARN, "unknown wait termination on render side\n");
            goto Exit;
        }
        ...
        while (keepPlaying)
        {
            ...
            // LogMsg( LOG_INFO,"#avaliable audio frames = %u", framesAvailable);
            ...
            for (uint32_t n = 0; n < n10msBuffers; n++)
            {
                ...
                if (_ptrAudioBuffer)
                {
                    ...
                    {
                        _UnLock();
                        LogMsg( LOG_FATAL, "output state has been modified during unlocked period\n");
                        goto Exit;
                    }
                    if (nSamples != static_cast<int32_t>(_playBlockSize))
                    {
                        LogMsg( LOG_WARN, "nSamples(%d) != _playBlockSize(%d)", nSamples, _playBlockSize);
                    }
    ...
    VxSleep(static_cast<DWORD>(endpointBufferSizeMS+0.5));
...
Exit:
    ...
    if (keepPlaying)
    {
        ...
        _playError = 1;
       LogMsg( LOG_ERROR, "kPlayoutError message posted: rendering thread has ended pre-maturely");
    }
    else
    {
        LogMsg( LOG_INFO,"_Rendering thread is now terminated properly");
    }
...
DWORD AudioDeviceWindowsCore::DoCaptureThread()
{
    ...
    EXIT_ON_ERROR(hr);
    LogMsg( LOG_INFO,"[CAPT] size of buffer       : %u\n", bufferLength);
    ...
    LogMsg( LOG_INFO,"[CAPT] size of sync buffer  : %u [bytes]\n", syncBufferSize);

    // Get maximum latency for the current stream (will not change for the lifetime of the IAudioClient object).
    //
    REFERENCE_TIME latency;
    _ptrClientIn->GetStreamLatency(&latency);
    LogMsg( LOG_INFO,"[CAPT] max stream latency   : %u (%3.2f ms)\n",
        (DWORD)latency, (double)(latency / 10000.0));
    ...
    LogMsg( LOG_INFO,"[CAPT] device period        : %u (%3.2f ms)\n",
        (DWORD)devPeriod, (double)(devPeriod / 10000.0));

    double extraDelayMS = (double)((latency + devPeriod) / 10000.0);
    LogMsg( LOG_INFO,"[CAPT] extraDelayMS         : %3.2f\n", extraDelayMS);

    double endpointBufferSizeMS = 10.0 * ((double)bufferLength / (double)_recBlockSize);
    LogMsg( LOG_INFO,"[CAPT] endpointBufferSizeMS : %3.2f\n", endpointBufferSizeMS);
    ...
    while (keepRecording)
    {
        ...
        case WAIT_OBJECT_0 + 1:        // _hCaptureSamplesReadyEvent
            break;
        case WAIT_TIMEOUT:            // timeout notification
            LogMsg( LOG_WARN, "capture event timed out after 0.5 seconds");
            goto Exit;
        default:                    // unexpected error
            LogMsg( LOG_WARN, "unknown wait termination on capture side");
            goto Exit;
        }
        ...
        while (keepRecording)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (flags & AUDCLNT_BUFFERFLAGS_SILENT)
                {
                    ...
                    LogMsg( LOG_WARN, "AUDCLNT_BUFFERFLAGS_SILENT");
                    pData = NULL;
                }
                ...
                while (syncBufIndex >= _recBlockSize)
                {
                    if (_ptrAudioBuffer)
                    {
                        ...
                        {
                            _UnLock();
                            LogMsg( LOG_FATAL, "input state has been modified during unlocked period\n");
                            goto Exit;
                        }
                    }
                ...
                if (_AGC)
                {
                    ...
                    if (newMicLevel != 0)
                    {
                        ...
                        LogMsg( LOG_INFO,"AGC change of volume: new=%u\n",  newMicLevel);
            ...
            else
            {
                ...
                LogMsg( LOG_ERROR,
                    "IAudioCaptureClient::GetBuffer returned AUDCLNT_E_BUFFER_ERROR, hr = 0x%08X\n",  hr);
                goto Exit;
            }
...
Exit:
    ...
    if (keepRecording)
    {
        ...
        // Trigger callback from module process thread
        _recError = 1;
       LogMsg( LOG_ERROR, "kRecordingError message posted: capturing thread has ended pre-maturely\n");
    }
    else
    {
        LogMsg( LOG_INFO,"_Capturing thread is now terminated properly\n");
    }
                                               ...
                                               int bufferLen)
{
    ...
    if (pDevice != NULL)
    {
        ...
        {
            LogMsg( LOG_ERROR,
                "IMMDevice::OpenPropertyStore failed, hr = 0x%08X\n", hr);
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            LogMsg( LOG_ERROR,
                "IPropertyStore::GetValue failed, hr = 0x%08X\n", hr);
        }
    }
    ...
    if ((SUCCEEDED(hr)) && (VT_EMPTY == varName.vt))
    {
        hr = E_FAIL;
        LogMsg( LOG_ERROR,
            "IPropertyStore::GetValue returned no value, hr = 0x%08X\n", hr);
    }
    ...
    if ((SUCCEEDED(hr)) && (VT_LPWSTR != varName.vt))
    {
        ...
        hr = E_UNEXPECTED;
        LogMsg( LOG_ERROR,
            "IPropertyStore::GetValue returned unexpected type, hr = 0x%08X\n", hr);
    }
...
int32_t AudioDeviceWindowsCore::_EnumerateEndpointDevicesAll(EDataFlow dataFlow) const
{
    ...
    EXIT_ON_ERROR(hr);
    if (dataFlow == eRender)
        LogMsg( LOG_INFO,"#rendering endpoint devices (counting all): %u\n", count);
    else if (dataFlow == eCapture)
        LogMsg( LOG_INFO,"#capturing endpoint devices (counting all): %u\n", count);
    ...
    for (ULONG i = 0; i < count; i++)
    {
        ...
        hr = pProps->GetValue(
                       ...
                       &varName);
        CONTINUE_ON_ERROR(hr);
        LogMsg( LOG_INFO,"friendly name: \"%S\"\n", varName.pwszVal);
        ...
        if (dwState & DEVICE_STATE_ACTIVE)
            LogMsg( LOG_INFO,"state (0x%x)  : *ACTIVE*\n", dwState);
        if (dwState & DEVICE_STATE_DISABLED)
            LogMsg( LOG_INFO,"state (0x%x)  : DISABLED\n", dwState);
        if (dwState & DEVICE_STATE_NOTPRESENT)
            LogMsg( LOG_INFO,"state (0x%x)  : NOTPRESENT\n", dwState);
        if (dwState & DEVICE_STATE_UNPLUGGED)
            LogMsg( LOG_INFO,"state (0x%x)  : UNPLUGGED\n", dwState);
        ...
        hr = pEndpointVolume->GetChannelCount(
                                &nChannelCount);
        CONTINUE_ON_ERROR(hr);
        LogMsg( LOG_INFO,"#channels    : %u", nChannelCount);
        ...
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME)
        {
            ...
            hr = pEndpointVolume->GetVolumeRange(
                                    ...
                                    &fVolumeIncrementDB);
            CONTINUE_ON_ERROR(hr);
            LogMsg( LOG_INFO,"volume range : %4.2f (min), %4.2f (max), %4.2f (inc) [dB]",
            ...
            hr = pEndpointVolume->GetVolumeStepInfo(
                                    ...
                                    &nStepCount);
            CONTINUE_ON_ERROR(hr);
            LogMsg( LOG_INFO,"volume steps : %d (nStep), %d (nStepCount)\n", nStep, nStepCount);
        }
Next:
        if (FAILED(hr)) {
          LogMsg( LOG_INFO,
...
void AudioDeviceWindowsCore::_TraceCOMError(HRESULT hr) const
{
    ...
    LogMsg( LOG_ERROR,
        "Core Audio method failed (hr=0x%x)\n", hr);
    StringCchPrintfW(buf, MAXERRORLENGTH, TEXT(L"Error details: "));
    StringCchCatW(buf, MAXERRORLENGTH, errorText);
    LogMsg( LOG_ERROR,"%s\n", WideToUTF8(buf));
}
